...
MyNetwork::MyNetwork(const mbus::RPCNetworkParams& params) : mbus::RPCNetwork(params), _nodes() {
}

bool MyNetwork::allocServiceAddress(mbus::RoutingNode& recipient) {
    recipient.setServiceAddress(
        mbus::IServiceAddress::UP(new MyServiceAddress(recipient.getRoute().getHop(0).toString())));
    return true;
}

void MyNetwork::freeServiceAddress(mbus::RoutingNode& recipient) {
    recipient.setServiceAddress(mbus::IServiceAddress::UP());
}

void MyNetwork::send(const mbus::Message& msg, const std::vector<mbus::RoutingNode*>& nodes) {
    (void)msg;
    _nodes.insert(_nodes.begin(), nodes.begin(), nodes.end());
}

void MyNetwork::removeNodes(std::vector<mbus::RoutingNode*>& nodes) {
