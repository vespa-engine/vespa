...
void ClusterList::configure(const cloud::config::ClusterListConfig& config) {
    _contentClusters.clear();
    for (uint32_t i = 0; i < config.storage.size(); i++) {
        _contentClusters.push_back(Cluster(config.storage[i].name, config.storage[i].configid));
    }
}

std::string ClusterList::getContentClusterList() const {
...
const ClusterList::Cluster& ClusterList::verifyContentCluster(const std::string& cluster) const {
    if (cluster.length()) {
        for (uint32_t j = 0; j < _contentClusters.size(); j++) {
        ...
        std::ostringstream ost;
        ost << "Cluster " << cluster << " has not been configured in the vespa cluster. Legal clusters are ["
