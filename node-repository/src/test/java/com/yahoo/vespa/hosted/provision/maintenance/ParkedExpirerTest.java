...
public class ParkedExpirerTest {
    ...
    @Test
    public void ensure_parked_nodes_are_deallocated_in_test_quickly() {
        ParkScenario scenario = new ParkScenario(SystemName.main, Environment.test)
                .withNode("node1")
                .withNode("node2")
        ...
        scenario.assertNodesIn(Node.State.parked, "node1"); // None moved yet

        scenario.clock().advance(Duration.ofMinutes(15));
        scenario.expirer().run();
        scenario.assertNodesIn(Node.State.dirty, "node1");
    }

    @Test
    public void ensure_parked_nodes_are_deallocated_in_prod() {
        ParkScenario scenario = new ParkScenario(SystemName.main, Environment.prod)
                .withNode("node1")
                .withNode("node2")
                .withNode("node3")
                .setReady("node1", "node2", "node3")
                .allocate(ClusterSpec.Type.content, "node1", "node2", "node3")
                .parkNode("node1", "node2", "node3");
        scenario.clock().advance(Duration.ofDays(2));
        scenario.expirer().run();
        scenario.assertNodesIn(Node.State.parked, "node1", "node2", "node3"); // None moved yet
        scenario.clock().advance(Duration.ofDays(2));
        scenario.expirer().run();
        scenario.assertNodesIn(Node.State.dirty, "node1", "node2", "node3");
    }

    @Test
    public void ensure_parked_nodes_are_deallocated_in_dev() {
        ParkScenario scenario = new ParkScenario(SystemName.main, Environment.dev)
                .withNode("node1")
                .withNode("node2")
                .withNode("node3")
                .setReady("node1", "node2", "node3")
    ...
    @Test
    public void ensure_parked_docker_host_is_deallocated_in_prod_unless_it_has_hw_failure() {
        ParkScenario scenario = new ParkScenario(SystemName.main, Environment.prod)
                .withNode(NodeType.host, ParkScenario.defaultFlavor, "parent1")
                .withNode(NodeType.host, ParkScenario.defaultFlavor, "parent2")
                .withNode(NodeType.host, ParkScenario.defaultFlavor, "parent3")
                ...
                .withNode(NodeType.tenant, ParkScenario.dockerFlavor, "node1", "parent1")
                .withNode(NodeType.tenant, ParkScenario.dockerFlavor, "node2", "parent2")
                .withNode(NodeType.tenant, ParkScenario.dockerFlavor, "node3", "parent3")
                .setReady("node1", "node2", "node3")
                .allocate(ClusterSpec.Type.content, ParkScenario.dockerFlavor, "node1", "node2", "node3")
                .parkNode("node3", "parent2")
                .parkNode(50, "parent3")
                .parkWithHardwareFailure("parent1");

        scenario.clock.advance(Duration.ofDays(5));
        scenario.expirer().run();
        scenario.assertNodesIn(Node.State.dirty, "node3", "parent2");
    }

    @Test
    public void ensure_non_tenant_nodes_and_hosts_are_not_recycled() {
        ParkScenario scenario = new ParkScenario(SystemName.main, Environment.prod)
                .withNode(NodeType.proxy, ParkScenario.defaultFlavor, "proxy1")
                .withNode(NodeType.proxy, ParkScenario.defaultFlavor, "proxy2")
                .withNode(NodeType.proxy, ParkScenario.defaultFlavor, "proxy3")
        ...
        scenario.assertNodesIn(Node.State.parked, "proxy1");
        scenario.assertNodesIn(Node.State.active, "proxy2", "proxy3");
    }

    private static class ParkScenario {
        ...
        private final ParkedExpirer expirer;
        private final ProvisioningTester tester;

        public ParkScenario(SystemName system, Environment environment) {
            ...
            this.expirer = new ParkedExpirer(nodeRepository, zone, Duration.ofMinutes(30), new TestMetric());
        }
        ...
        public ParkScenario withNode(NodeType type, NodeResources resources, String hostname, String parentHostname) {
        ...
        public ParkScenario withNode(NodeType type, NodeResources flavor, String hostname) {
            return withNode(type, flavor, hostname, null);
        }

        public ParkScenario withNode(String hostname) {
            return withNode(NodeType.tenant, defaultFlavor, hostname, null);
        }

        public ParkScenario parkNode(int times, String... hostname) {
            Stream.of(hostname).forEach(h -> {
                Node node = get(h);
                nodeRepository.nodes().write(node.with(node.status().withFailCount(times)), () -> {});
                nodeRepository.nodes().park(h, false, Agent.system, "Parked by unit test");
            });
            return this;
        }

        public ParkScenario parkNode(String... hostname) {
            Stream.of(hostname).forEach(h -> {
                Node node = get(h);
                nodeRepository.nodes().write(node.with(node.status()), () -> {});
                nodeRepository.nodes().park(h, false, Agent.system, "Parked by unit test");
            });
            return this;
        }

        public ParkScenario parkWithHardwareFailure(String... hostname) {
            Stream.of(hostname).forEach(h -> {
                Node node = get(h);
        ...
        public ParkScenario setReady(String... hostname) {
        ...
        public ParkScenario allocate(ClusterSpec.Type clusterType, String... hostname) {
            return allocate(clusterType, defaultFlavor, hostname);
        }

        public ParkScenario allocate(ClusterSpec.Type clusterType, NodeResources flavor, String... hostname) {
        ...
        public ParkScenario allocate(ApplicationId applicationId, ClusterSpec clusterSpec, Capacity capacity) {
