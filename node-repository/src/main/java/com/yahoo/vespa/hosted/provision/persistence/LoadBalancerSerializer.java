...
public class LoadBalancerSerializer {
    ...
    public static LoadBalancer fromJson(byte[] data) {
        ...
        object.field(realsField).traverse((ArrayTraverser) (i, realObject) -> {
            reals.add(new Real(HostName.of(realObject.field(hostnameField).asString()),
        ...
        Optional<HostName> hostname = optionalString(object.field(hostnameField), Function.identity()).filter(s -> !s.isEmpty()).map(HostName::of);
