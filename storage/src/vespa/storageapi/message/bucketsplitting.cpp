...
void SplitBucketCommand::print(std::ostream& out, bool verbose, const std::string& indent) const {
    out << "SplitBucketCommand(" << getBucketId();
    if (_minDocCount != std::numeric_limits<uint32_t>::max() || _minByteSize != std::numeric_limits<uint32_t>::max())
    {
        out << "Max doc count: " << _minDocCount << ", Max total doc size: " << _minByteSize;
...
SplitBucketReply::SplitBucketReply(const SplitBucketCommand& cmd) : BucketReply(cmd) {
}

void SplitBucketReply::print(std::ostream& out, bool verbose, const std::string& indent) const {
    ...
    if (_result.empty()) {
        ...
        for (uint32_t i = 0; i < _result.size(); ++i) {
            out << "\n" << indent << "  " << _result[i].first << ": " << _result[i].second;
        }
    }
    out << ")";
    if (verbose) {
...
JoinBucketsCommand::JoinBucketsCommand(const document::Bucket& target)
    : MaintenanceCommand(MessageType::JOINBUCKETS, target), _minJoinBits(0) {
}

void JoinBucketsCommand::print(std::ostream& out, bool verbose, const std::string& indent) const {
...
JoinBucketsReply::JoinBucketsReply(const JoinBucketsCommand& cmd)
    : BucketInfoReply(cmd), _sources(cmd.getSourceBuckets()) {
}
...
void JoinBucketsReply::print(std::ostream& out, bool verbose, const std::string& indent) const {
