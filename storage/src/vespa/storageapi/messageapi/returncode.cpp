...
ReturnCode::ReturnCode(Result result, std::string_view msg) : _result(result), _message() {
    if (!msg.empty()) {
        _message = std::make_unique<std::string>(msg);
    }
}

ReturnCode::ReturnCode(const ReturnCode& rhs) : _result(rhs._result), _message() {
    if (rhs._message) {
        _message = std::make_unique<std::string>(*rhs._message);
    }
}

ReturnCode& ReturnCode::operator=(const ReturnCode& rhs) {
    return operator=(ReturnCode(rhs));
}

std::string ReturnCode::getResultString(Result result) {
...
bool ReturnCode::isNodeDownOrNetwork() const {
    switch (static_cast<uint32_t>(_result)) {
...
bool ReturnCode::isCriticalForMaintenance() const {
...
bool ReturnCode::isCriticalForVisitor() const {
...
bool ReturnCode::isCriticalForVisitorDispatcher() const {
    return isCriticalForVisitor();
}

bool ReturnCode::isNonCriticalForIntegrityChecker() const {
    switch (static_cast<uint32_t>(_result)) {
...
bool ReturnCode::isShutdownRelated() const {
    switch (static_cast<uint32_t>(_result)) {
    case Protocol::ERROR_ABORTED:
        return true;
    default:
...
bool ReturnCode::isBucketDisappearance() const {
    switch (static_cast<uint32_t>(_result)) {
    case Protocol::ERROR_BUCKET_NOT_FOUND:
    case Protocol::ERROR_BUCKET_DELETED:
        return true;
...
bool ReturnCode::operator==(const ReturnCode& code) const {
...
std::string_view ReturnCode::getMessage() const noexcept {
    return _message ? _message->operator std::string_view() : std::string_view();
}
} // namespace storage::api
