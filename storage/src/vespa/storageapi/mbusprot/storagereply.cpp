...
StorageReply::StorageReply(mbus::BlobRef data, const ProtocolSerialization& serializer)
    : _serializer(&serializer), _sz(data.size()), _buffer(Alloc::alloc(_sz)), _mbusType(0), _reply() {
...
StorageReply::StorageReply(api::StorageReply::SP reply)
    : _serializer(nullptr), _sz(0), _buffer(), _mbusType(reply->getType().getId()), _reply(std::move(reply)) {
}
