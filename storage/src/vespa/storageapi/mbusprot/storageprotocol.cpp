...
mbus::IRoutingPolicy::UP StorageProtocol::createPolicy(const mbus::string&, const mbus::string&) const {
...
static mbus::Blob encodeMessage(const ProtocolSerialization& serializer, const mbus::Routable& routable,
                                const StorageMessage& message, const vespalib::Version& serializerVersion,
    ...
    if (LOG_WOULD_LOG(spam)) {
        ...
        LOG(spam,
            "Encoded message of protocol %s type %s using "
            "%s serialization as version is %s:\n%s",
            routable.getProtocol().c_str(), message.getInternalMessage()->getType().toString().c_str(),
            serializerVersion.toString().c_str(), actualVersion.toString().c_str(), messageStream.str().c_str());
    }

    return blob;
}

mbus::Blob StorageProtocol::encode(const vespalib::Version& version, const mbus::Routable& routable) const {
    ...
    try {
        ...
        if (version < version7_0) {
            LOGBP(error,
                  ...
                  version.toString().c_str(), version7_0.toString().c_str(),
        ...
        LOGBP(warning, "Failed to encode %s storage protocol message %s: %s", version.toString().c_str(),
              message.getInternalMessage()->toString().c_str(), e.what());
    }

    return mbus::Blob(0);
}

static mbus::Routable::UP decodeMessage(const ProtocolSerialization& serializer, mbus::BlobRef data,
                                        const api::MessageType& type, const vespalib::Version& serializerVersion,
                                        const vespalib::Version& actualVersion) {
    if (LOG_WOULD_LOG(spam)) {
        std::ostringstream messageStream;
        ...
        LOG(spam,
            ...
            type.toString().c_str(), actualVersion.toString().c_str(), serializerVersion.toString().c_str(),
...
mbus::Routable::UP StorageProtocol::decode(const vespalib::Version& version, mbus::BlobRef data) const {
    try {
        document::ByteBuffer buf(data.data(), data.size());
        auto& type = api::MessageType::get(static_cast<api::MessageType::Id>(SerializationHelper::getInt(buf)));

        StorageMessage::UP message;
        if (version < version7_0) {
            LOGBP(error, "Cannot decode message on version %s. Minimum version is %s.", version.toString().c_str(),
