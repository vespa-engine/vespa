...
public:
    ...
    HttpUrlPath(std::string path, std::map<std::string, std::string> attributes, std::string serverSpec);
    ~HttpUrlPath();
    ...
    std::string getAttribute(const std::string& id, const std::string& defaultValue = "") const;

    const std::string& getServerSpec() const { return _serverSpec; }

    template <typename T> T get(const std::string& id, const T& defaultValue = T()) const;
