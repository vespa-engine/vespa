...
XmlStatusReporter::XmlStatusReporter(std::string_view id, std::string_view name) : StatusReporter(id, name) {
}

XmlStatusReporter::~XmlStatusReporter() {
}

void XmlStatusReporter::initXmlReport(vespalib::XmlOutputStream& xos, const HttpUrlPath&) const {
    using namespace vespalib::xml;
    xos << XmlTag("status") << XmlAttribute("id", getId()) << XmlAttribute("name", getName());
}
...
bool XmlStatusReporter::reportStatus(std::ostream& out, const HttpUrlPath& path) const {
    out << "<?xml version=\"1.0\"?>\n";
