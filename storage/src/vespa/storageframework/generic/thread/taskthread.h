...
template <typename Task> class TaskThread : public TickingThread {
    ThreadLock&               _lock;
...
template <typename Task> TaskThread<Task>::TaskThread(ThreadLock& lock) : _lock(lock) {
}

template <typename Task> void TaskThread<Task>::addTask(const Task& t) {
    TickingLockGuard lock(_lock.freezeCriticalTicks());
    _enqueued.push_back(t);
    lock.broadcast();
}

template <typename Task> ThreadWaitInfo TaskThread<Task>::doCriticalTick(ThreadIndex) {
