...
ThreadPoolImpl::ThreadPoolImpl(Clock& clock) : _clock(clock), _stopping(false) {
}
...
Thread::UP ThreadPoolImpl::startThread(Runnable& runnable, std::string_view id, vespalib::duration waitTime,
...
void ThreadPoolImpl::visitThreads(ThreadVisitor& visitor) const {
    std::lock_guard lock(_threadVectorLock);
...
void ThreadPoolImpl::unregisterThread(ThreadImpl& t) {
    std::lock_guard          lock(_threadVectorLock);
