...
ThreadImpl::ThreadImpl(ThreadPoolImpl& pool, Runnable& runnable, std::string_view id, vespalib::duration waitTime,
...
bool ThreadImpl::interrupted() const {
    return _interrupted.load(std::memory_order_relaxed);
}

bool ThreadImpl::joined() const {
    return _joined;
}
...
std::string ThreadImpl::get_live_thread_stack_trace() const {
...
void ThreadImpl::registerTick(CycleType cycleType) {
...
void ThreadImpl::registerTick(CycleType cycleType, vespalib::steady_time now) {
    ...
    if (data._lastTick.time_since_epoch() == vespalib::duration::zero()) {
        return;
    }

    if (previousTick > now) {
        LOGBP(warning,
              "Thread is registering tick at time %" PRIu64 ", but "
              "last time it registered a tick, the time was %" PRIu64 ". Assuming clock has been adjusted backwards",
...
ThreadTickData ThreadImpl::getTickData() const {
    return _tickData[load_acquire(_tickDataPtr)].loadRelaxed();
}

void ThreadImpl::setTickData(const ThreadTickData& tickData) {
...
ThreadTickData ThreadImpl::AtomicThreadTickData::loadRelaxed() const noexcept {
...
void ThreadImpl::AtomicThreadTickData::storeRelaxed(const ThreadTickData& newState) noexcept {
    constexpr auto relaxed = std::memory_order_relaxed;
