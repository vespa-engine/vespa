...
StatusWebServer::StatusWebServer(framework::ComponentRegister& componentRegister,
                                 framework::StatusReporterMap& reporterMap, const config::ConfigUri& configUri)
...
struct HandleGetTask : vespalib::Executor::Task {
    ...
    HandleGetTask(vespalib::Portal::GetRequest request_in, std::function<void(vespalib::Portal::GetRequest)> fun_in)
...
void StatusWebServer::WebServer::get(vespalib::Portal::GetRequest request) {
    auto fun = [this](vespalib::Portal::GetRequest req) { handle_get(std::move(req)); };
    _executor.execute(std::make_unique<HandleGetTask>(std::move(request), std::move(fun)));
}

void StatusWebServer::WebServer::handle_get(vespalib::Portal::GetRequest request) {
...
class IndexPageReporter : public framework::HtmlStatusReporter {
    ...
    void reportHtmlStatus(std::ostream& out, const framework::HttpUrlPath&) const override { out << ost.str(); }

public:
    IndexPageReporter();
    ~IndexPageReporter() override;

    template <typename T> IndexPageReporter& operator<<(const T& t) {
        ost << t;
        return *this;
    }
};
...
int StatusWebServer::getListenPort() const {
    return _httpServer ? _httpServer->getListenPort() : -1;
}

void StatusWebServer::invoke_reporter(const framework::StatusReporter& reporter,
                                      const framework::HttpUrlPath& url_path, vespalib::Portal::GetRequest& request) {
    try {
...
void StatusWebServer::handlePage(const framework::HttpUrlPath& urlpath, vespalib::Portal::GetRequest request) {
    ...
    if (!link.empty()) {
        ...
        if (reporter != nullptr) {
            ...
            if (auth_ctx.capabilities().contains_all(reporter->required_capabilities())) {
                ...
                LOG(warning,
                    "Peer with %s denied status page access to '%s' due to insufficient "
                    "credentials (had %s, needed %s)",
                    auth_ctx.peer_credentials().to_string().c_str(), link.c_str(),
        ...
        indexRep << "<p><b>Binary version of Vespa:</b> " << vespalib::Vtag::currentVersion.toString() << "</p>\n";
        {
            for (const framework::StatusReporter* reporter : _reporterMap.getStatusReporters()) {
                indexRep << "<a href=\"" << reporter->getId() << "\">" << reporter->getName() << "</a><br>\n";
            }
        }
        std::ostringstream content;
