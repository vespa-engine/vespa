...
struct Column {
    ...
    virtual void printElementStop(std::ostream& out, [[maybe_unused]] uint16_t row) { out << "</td>"; }
    ...
    virtual void printValue(std::ostream& out, uint16_t) { out << "&nbsp;"; }
};
...
struct ColHeader {
    ...
    ColHeader(const std::string& name, uint32_t span) : _name(name), _span(span) {}
};
...
struct RowHeader {
    ...
    RowHeader(const std::string& s) : _name(s), _backgroundColor(Column::DEFAULT_COLOR) {}
};
...
public:
    ...
    void addTotalRow(const std::string& name) { _totalRow.reset(new std::string(name)); }
    void addColumnHeader(const std::string& name, uint32_t span) { _colHeaders.push_back(ColHeader(name, span)); }
    ...
    void addRow(uint64_t id) {
        ...
        ost << id;
        _rows.push_back(ost.str());
    }
    void setRowHeaderColor(Column::Color c) { _rows.back()._backgroundColor = c; }
...
template <typename T> struct ValueColumn : public Column {
    ...
    ValueColumn(const std::string& colName, const std::string& denomination = "", HtmlTable* table = 0);
    ~ValueColumn();
    ...
    void addColorLimit(T limit, Color c) { _colorLimits[limit] = c; }
...
struct ByteSizeColumn : public ValueColumn<uint64_t> {
    ...
    ByteSizeColumn(const std::string& colName, HtmlTable* table = 0) : ValueColumn<uint64_t>(colName, "", table) {}
