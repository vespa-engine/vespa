...
HtmlTable::HtmlTable(const std::string& rowId) : _rowId(rowId), _columns(), _rows() {
}
...
void HtmlTable::print(std::ostream& out) {
    ...
    if (!_colHeaders.empty()) {
        for (uint32_t i = 0; i < _colHeaders.size(); ++i) {
            out << "<th colspan=\"" << _colHeaders[i]._span << "\">" << _colHeaders[i]._name << "</th>";
        }
        out << "</tr>\n";
    }
...
void PercentageColumn::finalize() {
...
void ByteSizeColumn::printValue(std::ostream& out, uint16_t row) {
    uint64_t value;
    if (row == TOTAL) {
        value = getTotalValue();
    } else {
...
template <typename T>
ValueColumn<T>::ValueColumn(const std::string& colName, const std::string& denomination, HtmlTable* table)
    : Column(colName, table), _values(), _denomination(denomination), _colorLimits(), _totalIsAvg(false) {
    _valuePrinter << std::fixed << std::setprecision(2);
}

template <typename T> ValueColumn<T>::~ValueColumn() {
}

template <typename T> void ValueColumn<T>::finalize() {
...
template <typename T> T ValueColumn<T>::getTotalValue() {
    T value = 0;
    for (const auto& val : _values) {
        value += val.second;
    }
...
template <typename T> void ValueColumn<T>::printValue(std::ostream& out, uint16_t row) {
    T value;
    if (row == TOTAL) {
        value = getTotalValue();
    } else {
