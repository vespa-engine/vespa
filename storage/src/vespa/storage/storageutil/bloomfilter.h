...
uint32_t BloomFilter::hash(const uint32_t* data, int len, uint32_t multiplier, uint32_t max) {
    uint32_t val = 1;
    for (int i = 0; i < len; i++) {
...
uint32_t BloomFilter::hash(const char* data, int len, uint32_t multiplier, uint32_t max) {
    uint32_t val = 1;
    for (int i = 0; i < len; i++) {
        val = (multiplier * val + data[i]) % max;
    }
...
uint32_t BloomFilter::hash(const char* data, uint32_t multiplier, uint32_t max) {
    uint32_t val = 1;
...
bool BloomFilter::check(const uint32_t* data, int len, bool add) {
    uint32_t hash1 = hash(data, len, MULT1, _size);
    uint32_t hash2 = hash(data, len, MULT2, _size);
    return check(hash1, hash2, add);
}

bool BloomFilter::check(const char* data, int len, bool add) {
    uint32_t hash1 = hash(data, len, MULT1, _size);
    uint32_t hash2 = hash(data, len, MULT2, _size);
    return check(hash1, hash2, add);
}
bool BloomFilter::check(const char* data, bool add) {
    uint32_t hash1 = hash(data, MULT1, _size);
    uint32_t hash2 = hash(data, MULT2, _size);
    return check(hash1, hash2, add);
}

bool BloomFilter::check(uint32_t hash1, uint32_t hash2, bool add) {
    bool found = true;
