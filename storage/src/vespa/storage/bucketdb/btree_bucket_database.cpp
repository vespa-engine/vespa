...
template <typename ReplicaStore> vespalib::datastore::ArrayStoreConfig make_default_array_store_config() {
...
struct BTreeBucketDatabase::ReplicaValueTraits {
    ...
    static ValueType make_invalid_value() { return Entry::createInvalid(); }
 ...
 */
void BTreeBucketDatabase::getParents(const BucketId& bucket, std::vector<Entry>& entries) const {
    _impl->find_parents_and_self<ByValue>(
        bucket, [&entries]([[maybe_unused]] uint64_t key, Entry entry) { entries.emplace_back(std::move(entry)); });
}

void BTreeBucketDatabase::getAll(const BucketId& bucket, std::vector<Entry>& entries) const {
    _impl->find_parents_self_and_children<ByValue>(
        bucket, [&entries]([[maybe_unused]] uint64_t key, Entry entry) { entries.emplace_back(std::move(entry)); });
}
...
void BTreeBucketDatabase::process_update(const document::BucketId& bucket, EntryUpdateProcessor& processor,
...
void BTreeBucketDatabase::print(std::ostream& out, bool verbose, const std::string& indent) const {
...
class BTreeBucketDatabase::ReadGuardImpl final : public bucketdb::ReadGuard<Entry, ConstEntryRef> {
...
BTreeBucketDatabase::ReadGuardImpl::ReadGuardImpl(const BTreeBucketDatabase& db) : _snapshot(*db._impl) {
}

BTreeBucketDatabase::ReadGuardImpl::~ReadGuardImpl() = default;

std::vector<Entry> BTreeBucketDatabase::ReadGuardImpl::find_parents_and_self(const document::BucketId& bucket) const {
    std::vector<Entry> entries;
    _snapshot.find_parents_and_self<ByValue>(
        bucket, [&entries]([[maybe_unused]] uint64_t key, Entry entry) { entries.emplace_back(std::move(entry)); });
    return entries;
}
...
BTreeBucketDatabase::ReadGuardImpl::find_parents_self_and_children(const document::BucketId& bucket) const {
    std::vector<Entry> entries;
    _snapshot.find_parents_self_and_children<ByValue>(
        bucket, [&entries]([[maybe_unused]] uint64_t key, Entry entry) { entries.emplace_back(std::move(entry)); });
    return entries;
}
...
std::unique_ptr<bucketdb::ConstIterator<ConstEntryRef>> BTreeBucketDatabase::ReadGuardImpl::create_iterator() const {
    return _snapshot.create_iterator(); // TODO test
}

std::unique_ptr<bucketdb::ReadGuard<Entry, ConstEntryRef>> BTreeBucketDatabase::acquire_read_guard() const {
