 ...
 * and by read guards.
 */
template <typename T> class StripedBTreeLockableMap final : public AbstractBucketMap<T> {
public:
...
public:
    ...
    void insert(const key_type& key, const mapped_type& value, const char* client_id, bool has_lock,
                bool& pre_existed) override;

    bool erase(const key_type& key, const char* client_id) { return erase(key, client_id, false); }
    void insert(const key_type& key, const mapped_type& value, const char* client_id, bool& pre_existed) {
...
private:
    ...
    void do_for_each(std::function<Decision(uint64_t, const mapped_type&)> func, const char* client_id) override;

    void do_for_each_chunked(std::function<Decision(uint64_t, const mapped_type&)> func, const char* client_id,
                             vespalib::duration yield_time, uint32_t chunk_size) override;
