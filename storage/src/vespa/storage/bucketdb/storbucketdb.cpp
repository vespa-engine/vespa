...
void StorageBucketInfo::print(std::ostream& out, bool, const std::string&) const {
    out << info;
}
...
void StorBucketDatabase::insert(const document::BucketId& bucket, const bucketdb::StorageBucketInfo& entry,
                                const char* clientId) {
    bool preExisted;
    return _impl->insert(bucket.toKey(), entry, clientId, false, preExisted);
}

bool StorBucketDatabase::erase(const document::BucketId& bucket, const char* clientId) {
    return _impl->erase(bucket.stripUnused().toKey(), clientId, false);
}

StorBucketDatabase::WrappedEntry StorBucketDatabase::get(const document::BucketId& bucket, const char* clientId,
                                                         Flag flags) {
...
StorBucketDatabase::EntryMap StorBucketDatabase::getAll(const BucketId& bucketId, const char* clientId) {
    return _impl->getAll(bucketId, clientId);
}

StorBucketDatabase::EntryMap StorBucketDatabase::getContained(const BucketId& bucketId, const char* clientId) {
...
std::unique_ptr<bucketdb::ReadGuard<StorBucketDatabase::Entry>> StorBucketDatabase::acquire_read_guard() const {
