 ...
 */
template <typename T> class BTreeLockableMap final : public AbstractBucketMap<T> {
    struct ValueTraits;
...
private:
    ...
    bool findNextKey(key_type& key, mapped_type& val, const char* clientId, std::unique_lock<std::mutex>& guard);
    ...
    void do_for_each(std::function<Decision(uint64_t, const mapped_type&)> func, const char* clientId) override;
     ...
     */
    bool processNextChunk(std::function<Decision(uint64_t, const mapped_type&)>& func, key_type& key,
                          const char* client_id, uint32_t chunk_size);

    /**
     * Returns the given bucket, its super buckets and its sub buckets.
     */
    void getAllWithoutLocking(const BucketId& bucket, std::vector<BucketId::Type>& keys);

    /**
     * Find the given list of keys in the map and add them to the map of
     * results, locking them in the process.
     */
    void addAndLockResults(const std::vector<BucketId::Type>& keys, const char* clientId,
                           std::map<BucketId, WrappedEntry>& results, std::unique_lock<std::mutex>& guard);
};
