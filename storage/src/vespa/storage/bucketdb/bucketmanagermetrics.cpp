...
ContentBucketDbMetrics::ContentBucketDbMetrics(metrics::MetricSet* owner)
    : metrics::MetricSet("bucket_db", {}, "", owner), memory_usage(this) {
}
...
BucketSpaceMetrics::BucketSpaceMetrics(const std::string& space_name, metrics::MetricSet* owner)
    : metrics::MetricSet("bucket_space", {{"bucketSpace", space_name}}, "", owner),
      buckets_total("buckets_total", {}, "Total number buckets present in the bucket space (ready + not ready)",
                    this),
...
BucketManagerMetrics::BucketManagerMetrics(const ContentBucketSpaceRepo& repo)
    : metrics::MetricSet("datastored", {}, ""),
      ...
      simpleBucketInfoRequestSize("simplebucketinforeqsize", {},
                                  "Amount of buckets returned in simple bucket info requests", this),
      fullBucketInfoRequestSize("fullbucketinforeqsize", {},
                                "Amount of distributors answered at once in full bucket info requests.", this),
      fullBucketInfoLatency("fullbucketinfolatency", {}, "Amount of time spent to process a full bucket info request",
                            this) {
