...
struct Options : public vespalib::ProgramOptions {
    ...
    void finalize() {
        if (highRange) {
            nodeCounts.insert(nodeCounts.begin(),
                              {16, 20, 32, 48, 64, 100, 128, 160, 200, 256, 350, 500, 800, 1000, 5000});
        } else {
...
double generateSkew(uint32_t nodes, uint32_t distributionBits, uint16_t redundancy) {
    lib::Distribution distr(lib::Distribution::getDefaultDistributionConfig(redundancy, nodes));
    ...
    for (uint32_t i = 0; i <= lastbucket; ++i) {
        std::vector<uint16_t> curr(
            distr.getIdealStorageNodes(state, document::BucketId(distributionBits, i).stripUnused()));
...
Options::Options(int argc, const char* const* argv) : vespalib::ProgramOptions(argc, argv) {
    setSyntaxMessage("Utility program for calculating skew of buckets stored on storage nodes.");
    addOption("r redundancy", redundancy, 2u, "Number of copies stored on the nodes.");
    addOption("b maxbit", maxBit, 32u, "Maximum distribution bit count to calculate for.");
    addOption("h hide", hideUtilizationAbove, 0.3, "Hide utilizations worse than this.");
    addOption("s skip", skipGood, false,
              "Attempt to skip computations for node counts that already have good distributions");
    addOption("highrange", highRange, false, "Compute distribution for large systems instead of small systems");
    addOption("html", printHtml, false, "Print result as an HTML table");
    addOption("skipbitsbelow", skipBitsBelow, 0u, "Skip calculating for bits below given value");
    addOption("skipnodecountsbelow", skipNodeCountsBelow, 0u, "Skip calculating for node counts below given value");
    addOption("startatnodecount", startAtNodeCount, 0u, "Start calculating for first bit at given node count");
}
Options::~Options() {
}
...
int main(int argc, char** argv) {
    ...
    std::cout << "Distribution with redundancy " << std::setprecision(2) << o.redundancy << ":\n";
    if (o.printHtml) {
        std::cout << "</b>";
    }
    if (o.printHtml) {
    ...
    for (uint32_t bitIndex = 0; bitIndex < o.bitCounts.size(); ++bitIndex) {
        ...
        for (uint32_t nodeIndex = 0; nodeIndex < o.nodeCounts.size(); ++nodeIndex) {
            uint32_t nodes = o.nodeCounts[nodeIndex];
            if (nodes < o.skipNodeCountsBelow || (nodes < o.startAtNodeCount && firstBitCalculated)) {
                std::cerr << "Skipping calculating data for " << bits << " bits and " << nodes << " nodes\n";
                if (o.printHtml) {
                    (printedStart ? std::cout : start) << "  <td>-</td>\n";
                } else {
                    (printedStart ? std::cout : start) << std::setw(8) << std::setfill(' ') << "-";
                }
            } else if (bitIndex - firstBitIndex > 3 && results[nodeIndex][bitIndex - 1] <= o.htmlInfoAbove &&
                       ...
                       results[nodeIndex][bitIndex - 4] <= o.htmlInfoAbove)
            {
                ...
                if (skew > o.hideUtilizationAbove) {
                    if (o.printHtml) {
                        (printedStart ? std::cout : start)
                            << "  <td" << color << ">" << std::setprecision(4) << std::fixed << skew << "</td>\n"
                            << std::flush;
                        continue;
                    } else {
                        break;
                ...
                if (o.printHtml) {
                    std::cout << "  <td" << color << ">" << std::setprecision(4) << std::fixed << skew << "</td>\n"
                              << std::flush;
                } else {
                    std::cout << std::setw(8) << std::setfill(' ') << std::setprecision(4) << std::fixed << skew
                              << std::flush;
                }
            }
        }
