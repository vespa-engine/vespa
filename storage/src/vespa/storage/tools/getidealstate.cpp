...
Options::Options(int argc, const char* const* argv) : vespalib::ProgramOptions(argc, argv) {
    ...
    addOption("s clusterstate", clusterState, std::string(""), "The state of the cluster to calculate position in");
    addOption("r redundancy", redundancy, uint32_t(2), "The redundancy to keep for each bucket");
    addOption("u upstates", upStates, std::string("uims"), "States to consider as up in ideal state calculations");
    addOption("i stdin", bucketsOnStdIn, false, "Read stdin to get buckets to calculate ideal position for");
    addOption("v verbose", verbose, false, "Print extra information while running");
    addArgument("bucket", bucket, std::string(""), "Bucket for which to calculate ideal state");
    ...
    addOption("c clustername", clusterName, std::string(""), "Name of the cluster to get config from");
}
Options::~Options() {
}
...
int run(int argc, char** argv) {
    ...
    if (o.useConfig()) {
        try {
            if (o.verbose) {
                std::cerr << "Fetching distribution config using config id '" << o.getConfigId() << "'.\n";
            }
            config::ConfigUri                                               uri(o.getConfigId());
            std::unique_ptr<vespa::config::content::StorDistributionConfig> config =
                config::ConfigGetter<vespa::config::content::StorDistributionConfig>::getConfig(uri.getConfigId(),
                                                                                                uri.getContext());
        ...
        uint16_t distributorCount(clusterState.getNodeCount(lib::NodeType::DISTRIBUTOR));
        if (o.verbose) {
            std::cerr << "Not reading config. Assuming one top group with all " << distributorCount
                      << " distributors having redundancy " << redundancy << " with cluster state " << clusterState
                      << "\n";
        }
    ...
    if (!o.bucket.empty()) {
        ...
        if (*endp == '\0') {
            ...
            std::cerr << "Skipping bucket " << o.bucket << " which failed to parse as a bucket. Failed to "
                      << "parse: " << endp << "\n";
        }
        ...
        while (getline(std::cin, line)) {
            ...
            if (*endp == '\0') {
                ...
                std::cerr << "Skipping bucket " << line << " which failed to parse as a bucket. Failed to "
                          << "parse: " << endp << "\n";
            }
        }
    } else {
