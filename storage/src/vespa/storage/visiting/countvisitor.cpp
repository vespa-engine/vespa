...
CountVisitor::CountVisitor(StorageComponent& component, const vdslib::Parameters& params)
    : Visitor(component),
...
void CountVisitor::handleDocuments(const document::BucketId& /*bucketId*/, DocEntryList& entries,
    ...
    for (const auto& entry : entries) {
        if (!entry->isRemove()) {
            ...
            if (doc) {
                ...
                if (_doNamespace) {
                    _namespaceCount[std::string(idString.getNamespace())]++;
                }
                ...
                if (_doGroup && idString.hasGroup()) {
                    _groupCount[std::string(idString.getGroup())]++;
                }
