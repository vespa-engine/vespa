...
class MessageBusVisitorMessageSession : public VisitorMessageSession, public mbus::IReplyHandler {
public:
    ...
    MessageBusVisitorMessageSession(Visitor& visitor, VisitorThread& thread)
        : _visitor(visitor), _visitorThread(thread) {}

    void setSourceSession(mbus::SourceSession::UP sourceSession) { _sourceSession = std::move(sourceSession); }
    ...
    uint32_t pending() override { return _sourceSession->getPendingCount(); }
