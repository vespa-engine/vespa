...
VisitorManager::VisitorManager(const StorVisitorConfig& bootstrap_config, StorageComponentRegister& componentRegister,
                               VisitorMessageSessionFactory& messageSF, VisitorFactory::Map externalFactories,
...
void VisitorManager::updateMetrics(const MetricLockGuard&) {
    _metrics->queueSize.addValue(static_cast<int64_t>(_visitorQueue.relaxed_atomic_size()));
}

void VisitorManager::onClose() {
    {
...
void VisitorManager::print(std::ostream& out, bool verbose, const std::string& indent) const {
    (void)verbose;
    (void)indent;
    out << "VisitorManager";
}

void VisitorManager::on_configure(const vespa::config::content::core::StorVisitorConfig& config) {
    std::lock_guard sync(_visitorLock);

    // Do some sanity checking of input. Cannot haphazardly mix and match
    // old and new max concurrency config values
    if (config.maxconcurrentvisitors == 0 && config.maxconcurrentvisitorsFixed == 0) {
        throw config::InvalidConfigException("Maximum concurrent visitor count cannot be 0.");
    } else if (config.maxconcurrentvisitorsFixed == 0 && config.maxconcurrentvisitorsVariable != 0) {
        throw config::InvalidConfigException("Cannot specify 'variable' parameter for max concurrent "
    ...
    if (liveUpdate) {
        if (_visitorThread.size() != static_cast<uint32_t>(config.visitorthreads)) {
            LOG(warning,
                "Ignoring config change requesting %u visitor "
                "threads, still running %u. Restart storage to apply "
                "change.",
                config.visitorthreads, (uint32_t)_visitorThread.size());
        }
        ...
        {
            LOG(info,
                "Altered max concurrent visitors setting from "
                "(fixed=%u, variable=%u) to (fixed=%u, variable=%u).",
                _maxFixedConcurrentVisitors, _maxVariableConcurrentVisitors, maxConcurrentVisitorsFixed,
                maxConcurrentVisitorsVariable);
        }

        if (_maxVisitorQueueSize != static_cast<uint32_t>(config.maxvisitorqueuesize)) {
            LOG(info, "Altered max visitor queue size setting from %u to %u.", _maxVisitorQueueSize,
                config.maxvisitorqueuesize);
        }
    } else {
        if (config.visitorthreads == 0) {
            throw config::InvalidConfigException("No visitor threads configured. If you don't want visitors "
                                                 "to run, don't use visitormanager.",
                                                 VESPA_STRLOC);
        }
...
void VisitorManager::run(framework::ThreadHandle& thread) {
    ...
    while (true) {
        ...
        for (auto& timed_out_entry : timedOut) {
            ...
            _metrics->queueTimeoutWaitTime.addValue(vespalib::to_s(currentTime - timed_out_entry._deadline) *
                                                    1000.0); // Double metric in millis
...
namespace {
template <typename T> uint32_t getLeastLoadedThread(const T& t, uint32_t& totalCount) {
...
uint32_t VisitorManager::getActiveVisitorCount() const {
    std::lock_guard sync(_visitorLock);
...
bool VisitorManager::hasPendingMessageState() const {
    std::lock_guard sync(_visitorLock);
    return !_visitorMessages.empty();
}

void VisitorManager::setTimeBetweenTicks(uint32_t time) {
    std::lock_guard sync(_visitorLock);
    for (auto& visitor_thread : _visitorThread) {
...
bool VisitorManager::scheduleVisitor(const std::shared_ptr<api::CreateVisitorCommand>& cmd, bool skipQueue,
    ...
    std::pair<NameToIdMap::iterator, bool> newEntry;
    {
            ...
            if (_enforceQueueUse || totCount >= maximumConcurrent(*cmd)) {
                ...
                if (cmd->getQueueTimeout() > vespalib::duration::zero() && _maxVisitorQueueSize > 0) {
                    if (_visitorQueue.size() < _maxVisitorQueueSize) {
                        ...
                        if (cmd->getPriority() < tail->getPriority()) {
                            ...
                            // TODO is this really tracking what the metric description implies it's tracking...?
                            _metrics->queueEvictedWaitTime.addValue(vespalib::to_s(now - evictCommand.second) *
                                                                    1000.0); // Double metric in millis
                ...
                if (failCommand) {
                    ...
                    if (cmd->getQueueTimeout() <= vespalib::duration::zero()) {
                        ost << "Already running the maximum amount (" << maximumConcurrent(*failCommand)
                            << ") of visitors for this priority ("
                            << static_cast<uint32_t>(failCommand->getPriority()) << "), and queue timeout is 0.";
                    } else if (_maxVisitorQueueSize == 0) {
                        ost << "Already running the maximum amount (" << maximumConcurrent(*failCommand)
                            << ") of visitors for this priority ("
                            << static_cast<uint32_t>(failCommand->getPriority()) << "), and maximum queue size is 0.";
                    } else {
        ...
        while (true) {
            ...
            if (usedIds.size() == minLoadCount && usedIds.find(id) == usedIds.end()) {
    ...
    if (!newEntry.second) {
        ...
        std::ostringstream ost;
        ost << "Already running a visitor named " << cmd->getInstanceId() << ". Not creating visitor.";
...
bool VisitorManager::onCreateVisitor(const std::shared_ptr<api::CreateVisitorCommand>& cmd) {
    MonitorGuard sync(_visitorLock);
    scheduleVisitor(cmd, false, sync);
    return true;
}

bool VisitorManager::onDown(const std::shared_ptr<api::StorageMessage>& r) {
...
bool VisitorManager::onInternalReply(const std::shared_ptr<api::InternalReply>& r) {
    switch (r->getType()) {
...
bool VisitorManager::processReply(const std::shared_ptr<api::StorageReply>& reply) {
    api::VisitorId id;
    {
        std::lock_guard sync(_visitorLock);
...
void VisitorManager::send(const std::shared_ptr<api::StorageReply>& reply) {
...
void VisitorManager::closed(api::VisitorId id) {
 ...
 */
void VisitorManager::reportHtmlStatus(std::ostream& out, const framework::HttpUrlPath& path) const {
    ...
    if (showStatus) {
            ...
            for (uint32_t i = 0; i < _visitorThread.size(); ++i) {
                ...
                if (_visitorThread[i].second.empty()) {
                    ...
                    for (const auto& id_and_visitor : _visitorThread[i].second) {
                        out << " " << xml_content_escaped(id_and_visitor.second) << " (" << id_and_visitor.first
                            << ")";
                    }
                }
                out << "<br>\n";
        ...
        out << "\n<p>Running " << visitorCount << " visitors. Max concurrent "
            ...
            << ", variable = " << _maxVariableConcurrentVisitors << ", waiting visitors " << _visitorQueue.size()
            << "<br>\n";
    }
    ...
    for (const auto& request : _statusRequest) {
        out << "<h2>" << request->getSortToken() << "</h2>\n" << request->getStatus() << "\n";
    }
