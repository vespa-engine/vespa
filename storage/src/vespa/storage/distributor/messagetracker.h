...
public:
    ...
    void reserve_more_commands(size_t sz) { _commandQueue.reserve(_commandQueue.size() + sz); }

    void flushQueue(MessageSender& sender);

    /**
       If the reply is for a message that is being tracked here, returns the node the message was sent to. If not,
       returns (uint16_t)-1
    */
    ...
    */
    bool finished() const noexcept { return _sentMessages.empty(); }
