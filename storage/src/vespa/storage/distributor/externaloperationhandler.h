...
public:
    ...
    ExternalOperationHandler(DistributorNodeContext& node_ctx, DistributorStripeOperationContext& op_ctx,
                             DistributorMetricSet& metrics, ChainedMessageSender& msg_sender,
                             OperationSequencer& operation_sequencer, NonTrackingMessageSender& non_tracking_sender,
                             DocumentSelectionParser& parser, const MaintenanceOperationGenerator& gen,
                             OperationOwner& operation_owner);

    ~ExternalOperationHandler() override;

    bool handleMessage(const std::shared_ptr<api::StorageMessage>& msg, Operation::SP& operation);

    void rejectFeedBeforeTimeReached(TimePoint timePoint) noexcept { _rejectFeedBeforeTimeReached = timePoint; }
    ...
    // Exposed for testing
    OperationSequencer& operation_sequencer() noexcept { return _operation_sequencer; }
...
private:
    ...
    void bounce_or_invoke_read_only_op(api::StorageCommand& cmd, const document::Bucket& bucket,
                                       PersistenceOperationMetricSet& metrics, Func f);
    ...
    void bounce_with_busy_during_state_transition(api::StorageCommand& cmd, const lib::ClusterState& current_state,
    ...
    bool checkTimestampMutationPreconditions(api::StorageCommand& cmd, const document::BucketId& bucketId,
                                             PersistenceOperationMetricSet& persistenceMetrics);
    static std::shared_ptr<api::StorageMessage>
    make_concurrent_mutation_rejection_reply(api::StorageCommand& cmd, const document::DocumentId& doc_id,
