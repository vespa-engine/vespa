...
PersistenceMessageTracker::PersistenceMessageTracker(PersistenceOperationMetricSet&        metric,
...
PersistenceMessageTracker::prune_cancelled_nodes_if_present(BucketInfoMap&     bucket_and_replicas,
    ...
    return (any_replicas ? PostPruningStatus ::ReplicasStillPresent : PostPruningStatus::NoReplicasPresent);
}

void PersistenceMessageTracker::updateDB() {
    if (_cancel_scope.is_cancelled()) {
        ...
        const bool any_remapped =
            still_has_replicas(prune_cancelled_nodes_if_present(_remapBucketInfo, _cancel_scope));
...
void PersistenceMessageTracker::updateMetrics() {
...
void PersistenceMessageTracker::fail(MessageSender& sender, const api::ReturnCode& result) {
    if (_reply.get()) {
...
uint16_t PersistenceMessageTracker::receiveReply(MessageSender& sender, api::BucketInfoReply& reply) {
...
void PersistenceMessageTracker::queueMessageBatch(std::vector<MessageTracker::ToSend> messages) {
...
bool PersistenceMessageTracker::canSendReplyEarly() const {
    ...
    for (const MessageBatch& batch : _messageBatches) {
        ...
        if (messagesDone < distribution.getInitialRedundancy()) {
            LOG(spam, "Not returning early because only %d messages out of %d are done", messagesDone,
                distribution.getInitialRedundancy());
            return false;
        }
    }
...
void PersistenceMessageTracker::addBucketInfoFromReply(uint16_t node, const api::BucketInfoReply& reply) {
    document::Bucket       bucket(reply.getBucket());
    const api::BucketInfo& bucketInfo(reply.getBucketInfo());

    if (reply.hasBeenRemapped()) {
        LOG(debug, "Bucket %s: Received remapped bucket info %s from node %d", bucket.toString().c_str(),
            bucketInfo.toString().c_str(), node);
        _remapBucketInfo[bucket].emplace_back(_op_ctx.generate_unique_timestamp(), node, bucketInfo);
    } else {
        LOG(debug, "Bucket %s: Received bucket info %s from node %d", bucket.toString().c_str(),
            bucketInfo.toString().c_str(), node);
        _bucketInfo[bucket].emplace_back(_op_ctx.generate_unique_timestamp(), node, bucketInfo);
    }
}

void PersistenceMessageTracker::logSuccessfulReply(uint16_t node, const api::BucketInfoReply& reply) const {
    LOG(spam, "Bucket %s: Received successful reply %s", reply.getBucketId().toString().c_str(),
        reply.toString().c_str());

    if (!reply.getBucketInfo().valid()) {
        LOG(error,
            "Reply %s from node %d contained invalid bucket information %s. This is a bug! "
...
void PersistenceMessageTracker::sendReply(MessageSender& sender) {
...
void PersistenceMessageTracker::updateFailureResult(const api::BucketInfoReply& reply) {
    LOG(debug, "Bucket %s: Received failed reply %s with result %s", reply.getBucketId().toString().c_str(),
        reply.toString().c_str(), reply.getResult().toString().c_str());
...
bool PersistenceMessageTracker::node_is_effectively_cancelled(uint16_t node) const noexcept {
    return _cancel_scope.node_is_cancelled(node); // Implicitly covers the fully cancelled case
}

void PersistenceMessageTracker::handleCreateBucketReply(api::BucketInfoReply& reply, uint16_t node) {
    LOG(spam, "Received CreateBucket reply for %s from node %u", reply.getBucketId().toString().c_str(), node);
    if (!reply.getResult().success() && (reply.getResult().getResult() != api::ReturnCode::EXISTS) &&
...
void PersistenceMessageTracker::handlePersistenceReply(api::BucketInfoReply& reply, uint16_t node) {
...
void PersistenceMessageTracker::transfer_trace_state_to_reply() {
    if (!_trace.isEmpty()) {
...
void PersistenceMessageTracker::updateFromReply(MessageSender& sender, api::BucketInfoReply& reply, uint16_t node) {
...
void PersistenceMessageTracker::add_trace_tree_to_reply(vespalib::Trace trace) {
