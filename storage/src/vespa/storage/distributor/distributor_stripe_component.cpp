...
void DistributorStripeComponent::sendDown(const api::StorageMessage::SP& msg) {
    _distributor.getMessageSender().sendDown(msg);
}

void DistributorStripeComponent::sendUp(const api::StorageMessage::SP& msg) {
...
public:
    UpdateBucketDatabaseProcessor(const framework::Clock& clock, const std::vector<BucketCopy>& changed_nodes,
                                  ConstNodesRef ideal_nodes, bool reset_trusted);
...
UpdateBucketDatabaseProcessor::UpdateBucketDatabaseProcessor(const framework::Clock&        clock,
...
BucketDatabase::Entry UpdateBucketDatabaseProcessor::create_entry(const document::BucketId& bucket) const {
    return BucketDatabase::Entry(bucket, BucketInfo());
}

bool UpdateBucketDatabaseProcessor::process_entry(BucketDatabase::Entry& entry) const {
...
void DistributorStripeComponent::update_bucket_database(const document::Bucket&        bucket,
                                                        const std::vector<BucketCopy>& changed_nodes,
    ...
    UpdateBucketDatabaseProcessor processor(
        getClock(), found_down_node ? up_nodes : changed_nodes,
    ...
    bucketSpace.getBucketDatabase().process_update(bucket.getBucketId(), processor,
                                                   (update_flags & DatabaseUpdate::CREATE_IF_NONEXISTING) != 0);
}

// Implements DistributorNodeContext
api::StorageMessageAddress DistributorStripeComponent::node_address(uint16_t node_index) const noexcept {
...
void DistributorStripeComponent::remove_nodes_from_bucket_database(const document::Bucket&      bucket,
    ...
    if (dbentry.valid()) {
        for (uint32_t i = 0; i < nodes.size(); ++i) {
            if (dbentry->removeNode(nodes[i])) {
                LOG(debug, "Removed node %d from bucket %s. %u copies remaining", nodes[i], bucket.toString().c_str(),
        ...
        if (dbentry->getNodeCount() != 0) {
            ...
            LOG(debug, "After update, bucket %s now has no copies. Removing from database.",
...
void DistributorStripeComponent::recheck_bucket_info(uint16_t node_index, const document::Bucket& bucket) {
    _distributor.recheckBucketInfo(node_index, bucket);
}

document::BucketId DistributorStripeComponent::get_sibling(const document::BucketId& bid) const {
...
bool DistributorStripeComponent::has_pending_message(uint16_t node_index, const document::Bucket& bucket,
...
const lib::ClusterStateBundle& DistributorStripeComponent::cluster_state_bundle() const {
    return _distributor.getClusterStateBundle();
}

bool DistributorStripeComponent::storage_node_is_up(document::BucketSpace bucket_space, uint32_t node_index) const {
    const auto& ns = cluster_state_bundle()
                         .getDerivedClusterState(bucket_space)
                         ->getNodeState(lib::Node(lib::NodeType::STORAGE, node_index));

    return ns.getState().oneOf(storage_node_up_states());
}

const NodeSupportedFeaturesRepo& DistributorStripeComponent::node_supported_features_repo() const noexcept {
...
void DistributorStripeComponent::update_bucket_database(const document::Bucket& bucket,
                                                        const BucketCopy& changed_node, uint32_t update_flags) {
    update_bucket_database(bucket, toVector<BucketCopy>(changed_node), update_flags);
}

void DistributorStripeComponent::remove_node_from_bucket_database(const document::Bucket& bucket,
                                                                  uint16_t                node_index) {
    remove_nodes_from_bucket_database(bucket, toVector<uint16_t>(node_index));
}

MemoryUsageToken DistributorStripeComponent::make_memory_usage_token(uint32_t bytes_used) noexcept {
