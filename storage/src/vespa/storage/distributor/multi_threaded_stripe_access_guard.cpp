...
void MultiThreadedStripeAccessGuard::flush_and_close() {
    for_each_stripe([](TickableStripe& stripe) { stripe.flush_and_close(); });
}

void MultiThreadedStripeAccessGuard::update_total_distributor_config(
    std::shared_ptr<const DistributorConfiguration> config) {
    for_each_stripe([&](TickableStripe& stripe) { stripe.update_total_distributor_config(config); });
}

void MultiThreadedStripeAccessGuard::update_distribution_config(
    const lib::BucketSpaceDistributionConfigs& new_configs) {
    for_each_stripe([&](TickableStripe& stripe) { stripe.update_distribution_config(new_configs); });
}

void MultiThreadedStripeAccessGuard::set_pending_cluster_state_bundle(const lib::ClusterStateBundle& pending_state) {
    for_each_stripe([&](TickableStripe& stripe) { stripe.set_pending_cluster_state_bundle(pending_state); });
}

void MultiThreadedStripeAccessGuard::clear_pending_cluster_state_bundle() {
    for_each_stripe([](TickableStripe& stripe) { stripe.clear_pending_cluster_state_bundle(); });
}
...
void MultiThreadedStripeAccessGuard::enable_cluster_state_bundle(const lib::ClusterStateBundle& new_state,
    ...
    for_each_stripe(
        [&](TickableStripe& stripe) { stripe.enable_cluster_state_bundle(new_state, has_bucket_ownership_change); });
}

void MultiThreadedStripeAccessGuard::notify_distribution_change_enabled() {
    for_each_stripe([](TickableStripe& stripe) { stripe.notify_distribution_change_enabled(); });
}

PotentialDataLossReport MultiThreadedStripeAccessGuard::remove_superfluous_buckets(document::BucketSpace bucket_space,
...
void MultiThreadedStripeAccessGuard::merge_entries_into_db(
    document::BucketSpace bucket_space, api::Timestamp gathered_at_timestamp, const lib::Distribution& distribution,
    const lib::ClusterState& new_state, const char* storage_up_states, const OutdatedNodes& outdated_nodes,
    ...
    for (size_t i = 1; i < entries.size(); ++i) {
        ...
        if (curr_stripe != next_stripe) {
            curr_stripe->merge_entries_into_db(bucket_space, gathered_at_timestamp, distribution, new_state,
                                               storage_up_states, outdated_nodes, stripe_entries);
...
void MultiThreadedStripeAccessGuard::update_read_snapshot_before_db_pruning() {
    for_each_stripe([](TickableStripe& stripe) { stripe.update_read_snapshot_before_db_pruning(); });
}

void MultiThreadedStripeAccessGuard::update_read_snapshot_after_db_pruning(const lib::ClusterStateBundle& new_state) {
    for_each_stripe([&](TickableStripe& stripe) { stripe.update_read_snapshot_after_db_pruning(new_state); });
}

void MultiThreadedStripeAccessGuard::update_read_snapshot_after_activation(
    const lib::ClusterStateBundle& activated_state) {
    for_each_stripe([&](TickableStripe& stripe) { stripe.update_read_snapshot_after_activation(activated_state); });
}

void MultiThreadedStripeAccessGuard::clear_read_only_bucket_repo_databases() {
    for_each_stripe([](TickableStripe& stripe) { stripe.clear_read_only_bucket_repo_databases(); });
}
...
void MultiThreadedStripeAccessGuard::update_node_supported_features_repo(
    ...
    for_each_stripe([&](TickableStripe& stripe) { stripe.update_node_supported_features_repo(features_repo); });
}

void MultiThreadedStripeAccessGuard::report_bucket_db_status(document::BucketSpace bucket_space,
                                                             std::ostream&         out) const {
    for_each_stripe([&](TickableStripe& stripe) { stripe.report_bucket_db_status(bucket_space, out); });
}

StripeAccessGuard::PendingOperationStats MultiThreadedStripeAccessGuard::pending_operation_stats() const {
    StripeAccessGuard::PendingOperationStats stats(0, 0);
    for_each_stripe([&](const TickableStripe& stripe) { stats.merge(stripe.pending_operation_stats()); });
    return stats;
}

void MultiThreadedStripeAccessGuard::report_single_bucket_requests(vespalib::xml::XmlOutputStream& xos) const {
    for_each_stripe([&](TickableStripe& stripe) { stripe.report_single_bucket_requests(xos); });
}

void MultiThreadedStripeAccessGuard::report_delayed_single_bucket_requests(
    vespalib::xml::XmlOutputStream& xos) const {
    for_each_stripe([&](TickableStripe& stripe) { stripe.report_delayed_single_bucket_requests(xos); });
}

template <typename Func> void MultiThreadedStripeAccessGuard::for_each_stripe(Func&& f) {
    for (auto& stripe_thread : _stripe_pool) {
        f(stripe_thread->stripe());
    }
}

template <typename Func> void MultiThreadedStripeAccessGuard::for_each_stripe(Func&& f) const {
