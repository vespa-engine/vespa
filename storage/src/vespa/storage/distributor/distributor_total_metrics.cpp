...
DistributorTotalMetrics::DistributorTotalMetrics(uint32_t num_distributor_stripes)
    : DistributorMetricSet(), _stripes_metrics(), _top_level_metrics() {
...
void DistributorTotalMetrics::aggregate_helper(DistributorMetricSet& total) const {
...
void DistributorTotalMetrics::aggregate() {
    DistributorMetricSet::reset();
    aggregate_helper(*this);
}

void DistributorTotalMetrics::addToSnapshot(Metric& m, std::vector<Metric::UP>& ownerList) const {
    DistributorMetricSet total;
    aggregate_helper(total);
    total.addToSnapshot(m, ownerList);
}

void DistributorTotalMetrics::reset() {
    DistributorMetricSet::reset();
