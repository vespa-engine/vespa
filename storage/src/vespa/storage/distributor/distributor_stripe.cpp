...
DistributorStripe::invalidate_internal_db_dependent_stats()
{
    _bucketDBMetricUpdater.reset();
    {
        ...
        _global_stats = BucketSpacesStatsProvider::GlobalStats::make_invalid();
...
BucketSpacesStatsProvider::GlobalStats
DistributorStripe::global_stats() const
{
    std::lock_guard guard(_metricLock);
...
DistributorStripe::updateInternalMetricsForCompletedScan()
{
    ...
    _global_stats = BucketSpacesStatsProvider::GlobalStats(_bucketDbStats._docCount, _bucketDbStats._byteCount);
