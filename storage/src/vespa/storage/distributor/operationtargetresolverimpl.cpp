...
BucketInstance::BucketInstance(const document::BucketId& id, const api::BucketInfo& info, lib::Node node,
                               uint16_t ideal_location_priority, uint16_t db_entry_order, bool trusted,
                               bool exist) noexcept
    : _bucket(id),
      _info(info),
      _node(node),
...
void BucketInstance::print(vespalib::asciistream& out, const PrintProperties&) const {
    std::string infoString(_info.toString());
    infoString = infoString.substr(10, infoString.size() - 10);

    std::ostringstream ost;
    ost << std::hex << _bucket.getId();
    out << "(" << ost.str() << ", " << infoString << ", node " << _node.getIndex() << ", ideal "
        << _ideal_location_priority << (_trusted ? ", trusted" : "") << (_exists ? "" : ", new copy") << ")";
}
...
void BucketInstanceList::add(const BucketDatabase::Entry&                    e,
    ...
    for (uint32_t i = 0; i < e.getBucketInfo().getNodeCount(); ++i) {
        ...
        _instances.emplace_back(e.getBucketId(), copy.getBucketInfo(), node, idealState.lookup(copy.getNode()), i,
                                copy.trusted(), true);
    }
}

void BucketInstanceList::populate(const document::BucketId&     specificId,
                                  const DistributorBucketSpace& distributor_bucket_space, BucketDatabase& db) {
    std::vector<BucketDatabase::Entry> entries;
    db.getParents(specificId, entries);
    for (const auto& entry : entries) {
        auto node2Index = distributor_bucket_space.get_ideal_service_layer_nodes_bundle(entry.getBucketId())
                              .nonretired_or_maintenance_to_index();
        add(entry, node2Index);
    }
}

void BucketInstanceList::removeNodeDuplicates() {
...
void BucketInstanceList::limitToRedundancyCopies(uint16_t redundancy) {
...
void BucketInstanceList::extendToEnoughCopies(const DistributorBucketSpace& distributor_bucket_space,
                                              const BucketDatabase&         db,
...
OperationTargetList BucketInstanceList::createTargets(document::BucketSpace bucketSpace) {
...
void BucketInstanceList::print(vespalib::asciistream& out, const PrintProperties& p) const {
...
BucketInstanceList OperationTargetResolverImpl::getAllInstances(OperationType type, const document::BucketId& id) {
