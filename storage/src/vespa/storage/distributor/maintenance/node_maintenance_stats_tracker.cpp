...
NodeMaintenanceStats& NodeMaintenanceStatsTracker::stats(uint16_t node, document::BucketSpace bucketSpace) {
    return _node_stats[BucketSpaceAndNode(node, bucketSpace)];
}

const NodeMaintenanceStats& NodeMaintenanceStatsTracker::stats(uint16_t              node,
                                                               document::BucketSpace bucketSpace) const noexcept {
...
const NodeMaintenanceStats& NodeMaintenanceStatsTracker::forNode(uint16_t              node,
                                                                 document::BucketSpace bucketSpace) const noexcept {
    return stats(node, bucketSpace);
}

bool NodeMaintenanceStatsTracker::operator==(const NodeMaintenanceStatsTracker& rhs) const noexcept {
    return ((_node_stats == rhs._node_stats) &&
            (_max_observed_time_since_last_gc == rhs._max_observed_time_since_last_gc));
}

void NodeMaintenanceStatsTracker::merge(const NodeMaintenanceStatsTracker& rhs) {
    for (const auto& entry : rhs._node_stats) {
        auto key = entry.first;
        _node_stats[key].merge(entry.second);
    }
    _max_observed_time_since_last_gc =
        std::max(_max_observed_time_since_last_gc, rhs._max_observed_time_since_last_gc);
}

std::ostream& operator<<(std::ostream& os, const NodeMaintenanceStats& stats) {
    os << "NodeStats("
       << "movingOut=" << stats.movingOut << ",syncing=" << stats.syncing << ",copyingIn=" << stats.copyingIn
       << ",copyingOut=" << stats.copyingOut << ",total=" << stats.total << ")";
    return os;
}

NodeMaintenanceStatsTracker::NodeMaintenanceStatsTracker() noexcept
    : _node_stats(), _total_stats(), _max_observed_time_since_last_gc(0) {
}
...
void NodeMaintenanceStatsTracker::reset(size_t nodes) {
    _node_stats.clear();
