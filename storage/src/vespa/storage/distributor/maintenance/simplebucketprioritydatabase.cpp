...
SimpleBucketPriorityDatabase::SimpleBucketPriorityDatabase()
    : _pri_fifo_buckets(), _bucket_to_pri_iterators(), _fifo_seq_num(0) {
}

SimpleBucketPriorityDatabase::~SimpleBucketPriorityDatabase() = default;

void SimpleBucketPriorityDatabase::clearAllEntriesForBucket(const document::Bucket& bucket) {
...
void SimpleBucketPriorityDatabase::setPriority(const PrioritizedBucket& bucket) {
...
void SimpleBucketPriorityDatabase::PriFifoMappingConstIteratorImpl::increment() noexcept {
    if (_pri_fifo_iter != _pri_fifo_end) {
        ++_pri_fifo_iter;
    }
}

bool SimpleBucketPriorityDatabase::PriFifoMappingConstIteratorImpl::equal(
    const ConstIteratorImpl& other) const noexcept {
...
SimpleBucketPriorityDatabase::const_iterator SimpleBucketPriorityDatabase::begin() const {
    return const_iterator(
        std::make_unique<PriFifoMappingConstIteratorImpl>(_pri_fifo_buckets.begin(), _pri_fifo_buckets.end()));
}

SimpleBucketPriorityDatabase::const_iterator SimpleBucketPriorityDatabase::end() const {
    return const_iterator(
        std::make_unique<PriFifoMappingConstIteratorImpl>(_pri_fifo_buckets.end(), _pri_fifo_buckets.end()));
}

std::string SimpleBucketPriorityDatabase::toString() const {
