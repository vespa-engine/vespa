...
public:
    ...
    PrioritizedBucket() noexcept : _bucket(), _priority(MaintenancePriority::NO_MAINTENANCE_NEEDED) {}

    PrioritizedBucket(const document::Bucket& bucket, Priority pri) noexcept : _bucket(bucket), _priority(pri) {}

    document::Bucket getBucket() const noexcept { return _bucket; }

    Priority getPriority() const noexcept { return _priority; }

    [[nodiscard]] bool valid() const noexcept { return _bucket.getBucketId().getRawId() != 0; }

    std::string toString() const {
        return vespalib::make_string("PrioritizedBucket(%s, pri %s)", _bucket.toString().c_str(),
    ...
    [[nodiscard]] bool moreImportantThan(Priority otherPri) const noexcept { return _priority > otherPri; }
...
std::ostream& operator<<(std::ostream& os, const PrioritizedBucket& bucket);
