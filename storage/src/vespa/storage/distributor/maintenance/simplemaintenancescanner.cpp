...
bool SimpleMaintenanceScanner::GlobalMaintenanceStats::operator==(const GlobalMaintenanceStats& rhs) const noexcept {
    return pending == rhs.pending;
}

void SimpleMaintenanceScanner::GlobalMaintenanceStats::merge(const GlobalMaintenanceStats& rhs) noexcept {
    for (size_t i = 0; i < pending.size(); ++i) {
        pending[i] += rhs.pending[i];
    }
}

void SimpleMaintenanceScanner::PendingMaintenanceStats::merge(const PendingMaintenanceStats& rhs) {
...
SimpleMaintenanceScanner::PendingMaintenanceStats::PendingMaintenanceStats(PendingMaintenanceStats&&) noexcept =
    default;
...
MaintenanceScanner::ScanResult SimpleMaintenanceScanner::scanNext() {
    for (;;) {
...
SimpleMaintenanceScanner::PendingMaintenanceStats SimpleMaintenanceScanner::fetch_and_reset() {
    _bucketCursor = document::BucketId();
    _bucketSpaceItr = _bucketSpaceRepo.begin();
    return _pendingMaintenance.fetch_and_reset();
}

void SimpleMaintenanceScanner::countBucket(document::BucketSpace bucketSpace, const BucketInfo& info) {
...
void SimpleMaintenanceScanner::prioritizeBucket(const document::Bucket& bucket) {
...
std::ostream& operator<<(std::ostream& os, const SimpleMaintenanceScanner::GlobalMaintenanceStats& stats) {
    using MO = MaintenanceOperation;
    os << "delete bucket: " << stats.pending[MO::DELETE_BUCKET]
       << ", merge bucket: " << stats.pending[MO::MERGE_BUCKET]
       << ", split bucket: " << stats.pending[MO::SPLIT_BUCKET] << ", join bucket: " << stats.pending[MO::JOIN_BUCKET]
