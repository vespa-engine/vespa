...
PendingClusterState::PendingClusterState(const framework::Clock& clock, const ClusterInformation::CSP& clusterInfo,
...
void PendingClusterState::initializeBucketSpaceTransitions(bool                    distributionChanged,
    ...
    for (const auto& elem : _bucket_space_states) {
        ...
        auto        pendingTransition = std::make_unique<PendingBucketSpaceDbTransition>(
            elem.first, *elem.second, distributionChanged, outdatedNodes, _clusterInfo,
            *_newClusterStateBundle.getDerivedClusterState(elem.first), _creationTimestamp);
...
void PendingClusterState::logConstructionInformation() const {
    const auto& distribution =
        _bucket_space_states.get(document::FixedBucketSpaces::default_space()).get_distribution();
    LOG(debug,
...
bool PendingClusterState::storageNodeUpInNewState(document::BucketSpace bucketSpace, uint16_t node) const {
    return _newClusterStateBundle.getDerivedClusterState(bucketSpace)
        ->getNodeState(Node(NodeType::STORAGE, node))
        .getState()
        .oneOf(_clusterInfo->getStorageUpStates());
}

PendingClusterState::OutdatedNodesMap PendingClusterState::getOutdatedNodesMap() const {
...
bool PendingClusterState::shouldRequestBucketInfo() const {
    if (clusterIsDown()) {
...
bool PendingClusterState::clusterIsDown() const {
    return _newClusterStateBundle.getBaselineClusterState()->getClusterState() == lib::State::DOWN;
}

bool PendingClusterState::iAmDown() const {
    const lib::NodeState& myState(_newClusterStateBundle.getBaselineClusterState()->getNodeState(
        Node(NodeType::DISTRIBUTOR, _sender.getDistributorIndex())));
    return myState.getState() == lib::State::DOWN;
}

void PendingClusterState::requestNodes() {
    LOG(debug, "New system state: Old state was %s, new state is %s", getPrevClusterStateBundleString().c_str(),
        getNewClusterStateBundleString().c_str());

    requestBucketInfoFromStorageNodesWithChangedState();
}

void PendingClusterState::requestBucketInfoFromStorageNodesWithChangedState() {
...
void PendingClusterState::requestNode(BucketSpaceAndNode bucketSpaceAndNode) {
    ...
    LOG(debug,
        "Requesting bucket info for bucket space %" PRIu64
        " node %d with cluster state '%s' and distribution hash '%s'",
    ...
    auto cmd = std::make_shared<api::RequestBucketInfoCommand>(
        bucketSpaceAndNode.bucketSpace, _sender.getDistributorIndex(),
        *_newClusterStateBundle.getDerivedClusterState(bucketSpaceAndNode.bucketSpace), distributionHash);
...
bool PendingClusterState::onRequestBucketInfoReply(const std::shared_ptr<api::RequestBucketInfoReply>& reply) {
...
void PendingClusterState::merge_into_bucket_databases(StripeAccessGuard& guard) {
    for (auto& elem : _pendingTransitions) {
        elem.second->merge_into_bucket_databases(guard);
    }
}

void PendingClusterState::printXml(vespalib::XmlOutputStream& xos) const {
    using namespace vespalib::xml;
    xos << XmlTag("systemstate_pending") << XmlAttribute("state", *_newClusterStateBundle.getBaselineClusterState());
    for (auto& elem : _sentMessages) {
        xos << XmlTag("pending") << XmlAttribute("node", elem.second.node) << XmlEndTag();
    }
    xos << XmlEndTag();
}

PendingClusterState::Summary PendingClusterState::getSummary() const {
    return {getPrevClusterStateBundleString(), getNewClusterStateBundleString(),
...
std::string PendingClusterState::getNewClusterStateBundleString() const {
    return _newClusterStateBundle.getBaselineClusterState()->toString();
}
std::string PendingClusterState::getPrevClusterStateBundleString() const {
    return _prevClusterStateBundle.getBaselineClusterState()->toString();
}

void PendingClusterState::update_node_supported_features_from_reply(uint16_t                           node,
                                                                    const api::RequestBucketInfoReply& reply) {
