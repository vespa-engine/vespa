...
BucketSpaceDistributionContext::BucketSpaceDistributionContext(
    ...
    std::shared_ptr<const lib::Distribution> distribution, uint16_t this_node_index) noexcept
...
std::shared_ptr<BucketSpaceDistributionContext> BucketSpaceDistributionContext::make_state_transition(
    ...
    return std::make_shared<BucketSpaceDistributionContext>(
        std::move(active_cluster_state), std::move(default_active_cluster_state), std::move(pending_cluster_state),
...
std::shared_ptr<BucketSpaceDistributionContext> BucketSpaceDistributionContext::make_stable_state(
    ...
    return std::make_shared<BucketSpaceDistributionContext>(
        ...
        std::shared_ptr<const lib::ClusterState>(), std::move(distribution), this_node_index);
}
...
BucketSpaceDistributionContext::make_not_yet_initialized(uint16_t this_node_index) {
    return std::make_shared<BucketSpaceDistributionContext>(
        std::make_shared<const lib::ClusterState>(), std::make_shared<const lib::ClusterState>(),
        std::shared_ptr<const lib::ClusterState>(), std::make_shared<const lib::Distribution>(), this_node_index);
}
