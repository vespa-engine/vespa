...
StripeBucketDBUpdater::StripeBucketDBUpdater(const DistributorNodeContext&      node_ctx,
    ...
    for (auto& elem : _op_ctx.bucket_space_repo()) {
        _active_distribution_contexts.emplace(
            elem.first, BucketSpaceDistributionContext::make_not_yet_initialized(_node_ctx.node_index()));
...
void StripeBucketDBUpdater::flush() {
...
void StripeBucketDBUpdater::print(std::ostream& out, bool, const std::string&) const {
    out << "StripeBucketDBUpdater";
}

bool StripeBucketDBUpdater::shouldDeferStateEnabling() const noexcept {
    return stale_reads_enabled();
}

bool StripeBucketDBUpdater::hasPendingClusterState() const {
...
const lib::ClusterState* StripeBucketDBUpdater::pendingClusterStateOrNull(const document::BucketSpace& space) const {
    auto& distr_space = _op_ctx.bucket_space_repo().get(space);
    return (distr_space.has_pending_cluster_state() ? &distr_space.get_pending_cluster_state() : nullptr);
}

void StripeBucketDBUpdater::sendRequestBucketInfo(uint16_t node, const document::Bucket& bucket,
    ...
    _sentMessages[msg->getMsgId()] =
        BucketRequest(node, _op_ctx.generate_unique_timestamp(), bucket, mergeReplyGuard);
    _sender.sendCommand(msg);
}

void StripeBucketDBUpdater::recheckBucketInfo(uint32_t nodeIdx, const document::Bucket& bucket) {
    sendRequestBucketInfo(nodeIdx, bucket, std::shared_ptr<MergeReplyGuard>());
}

void StripeBucketDBUpdater::handle_activated_cluster_state_bundle() {
...
public:
    explicit ReadOnlyDbMergingInserter(const NewEntries& new_entries)
        : _current(new_entries.cbegin()), _last(new_entries.cend()) {}
...
PotentialDataLossReport StripeBucketDBUpdater::remove_superfluous_buckets(document::BucketSpace    bucket_space,
    ...
    MergingNodeRemover proc(new_state, _node_ctx.node_index(), new_distribution, up_states, move_to_read_only_db);
...
void StripeBucketDBUpdater::merge_entries_into_db(document::BucketSpace    bucket_space,
    ...
    PendingBucketSpaceDbTransition::DbMerger merger(gathered_at_timestamp, distribution, new_state, storage_up_states,
                                                    outdated_nodes, entries);
    bucket_db.merge(merger);
}

void StripeBucketDBUpdater::clearReadOnlyBucketRepoDatabases() {
...
void StripeBucketDBUpdater::update_read_snapshot_after_db_pruning(const lib::ClusterStateBundle& new_state) {
    ...
    const auto      old_default_state =
        _op_ctx.bucket_space_repo().get(document::FixedBucketSpaces::default_space()).cluster_state_sp();
    for (auto& elem : _op_ctx.bucket_space_repo()) {
        ...
        _active_distribution_contexts.insert_or_assign(
            elem.first, BucketSpaceDistributionContext::make_state_transition(
                            std::move(old_cluster_state), old_default_state, std::move(new_cluster_state),
                            std::move(new_distribution), _node_ctx.node_index()));
...
void StripeBucketDBUpdater::update_read_snapshot_after_activation(const lib::ClusterStateBundle& activated_state) {
    std::lock_guard lock(_distribution_context_mutex);
    const auto&     default_cluster_state =
        activated_state.getDerivedClusterState(document::FixedBucketSpaces::default_space());
    for (auto& elem : _op_ctx.bucket_space_repo()) {
        auto new_distribution = elem.second->distribution_sp();
        auto new_cluster_state = activated_state.getDerivedClusterState(elem.first);
        _active_distribution_contexts.insert_or_assign(
            elem.first,
            BucketSpaceDistributionContext::make_stable_state(std::move(new_cluster_state), default_cluster_state,
                                                              std::move(new_distribution), _node_ctx.node_index()));
    }
}
...
bool StripeBucketDBUpdater::onMergeBucketReply(const std::shared_ptr<api::MergeBucketReply>& reply) {
...
void StripeBucketDBUpdater::enqueueRecheckUntilPendingStateEnabled(uint16_t node, const document::Bucket& bucket) {
    LOG(spam,
...
void StripeBucketDBUpdater::sendAllQueuedBucketRechecks() {
    LOG(spam,
...
bool StripeBucketDBUpdater::onNotifyBucketChange(const std::shared_ptr<api::NotifyBucketChangeCommand>& cmd) {
    ...
    LOG(debug, "Received notify bucket change from node %u for bucket %s with %s.", cmd->getSourceIndex(),
        cmd->getBucketId().toString().c_str(), cmd->getBucketInfo().toString().c_str());

    if (hasPendingClusterState()) {
        enqueueRecheckUntilPendingStateEnabled(cmd->getSourceIndex(), cmd->getBucket());
    } else {
        sendRequestBucketInfo(cmd->getSourceIndex(), cmd->getBucket(), std::shared_ptr<MergeReplyGuard>());
    }
...
bool sort_pred(const BucketListMerger::BucketEntry& left, const BucketListMerger::BucketEntry& right) {
    return left.first < right.first;
}

} // namespace

bool StripeBucketDBUpdater::onRequestBucketInfoReply(const std::shared_ptr<api::RequestBucketInfoReply>& repl) {
    return processSingleBucketInfoReply(repl);
}

void StripeBucketDBUpdater::handleSingleBucketInfoFailure(const std::shared_ptr<api::RequestBucketInfoReply>& repl,
                                                          const BucketRequest&                                req) {
    LOG(debug, "Request bucket info failed towards node %d: error was %s", req.targetNode,
        repl->getResult().toString().c_str());
...
void StripeBucketDBUpdater::resendDelayedMessages() {
...
void StripeBucketDBUpdater::convertBucketInfoToBucketList(const std::shared_ptr<api::RequestBucketInfoReply>& repl,
                                                          uint16_t                      targetNode,
...
void StripeBucketDBUpdater::mergeBucketInfoWithDatabase(const std::shared_ptr<api::RequestBucketInfoReply>& repl,
                                                        const BucketRequest&                                req) {
...
bool StripeBucketDBUpdater::processSingleBucketInfoReply(const std::shared_ptr<api::RequestBucketInfoReply>& repl) {
    ...
    LOG(debug, "Received single bucket info reply from node %u: %s", req.targetNode,
        repl->toString(true).c_str()); // Verbose mode to include bucket info in output
    mergeBucketInfoWithDatabase(repl, req);
    return true;
}

bool StripeBucketDBUpdater::cancel_message_by_id(uint64_t msg_id) {
...
void StripeBucketDBUpdater::addBucketInfoForNode(const BucketDatabase::Entry& e, uint16_t node,
                                                 BucketListMerger::BucketList& existing) {
...
void StripeBucketDBUpdater::findRelatedBucketsInDatabase(uint16_t node, const document::Bucket& bucket,
                                                         BucketListMerger::BucketList& existing) {
...
void StripeBucketDBUpdater::updateDatabase(document::BucketSpace bucketSpace, uint16_t node,
                                           BucketListMerger& merger) {
    ...
    for (const BucketListMerger::BucketEntry& entry : merger.getAddedEntries()) {
        ...
        _op_ctx.update_bucket_database(bucket, BucketCopy(merger.getTimestamp(), node, entry.second),
...
std::string StripeBucketDBUpdater::getReportContentType(const framework::HttpUrlPath&) const {
    return "text/xml";
}
...
void StripeBucketDBUpdater::BucketRequest::print_xml_tag(
    vespalib::xml::XmlOutputStream& xos, const vespalib::xml::XmlAttribute& timestampAttribute) const {
    using namespace vespalib::xml;
    xos << XmlTag("storagenode") << XmlAttribute("index", targetNode);
...
bool StripeBucketDBUpdater::reportStatus(std::ostream& out, const framework::HttpUrlPath& path) const {
    using namespace vespalib::xml;
    ...
    xos << XmlTag("status") << XmlAttribute("id", BUCKETDB) << XmlAttribute("name", BUCKETDB_UPDATER);
    reportXmlStatus(xos, path);
    xos << XmlEndTag();
    return true;
}

std::string StripeBucketDBUpdater::reportXmlStatus(vespalib::xml::XmlOutputStream& xos,
                                                   const framework::HttpUrlPath&) const {
    using namespace vespalib::xml;
    xos << XmlTag("bucketdb") << XmlTag("systemstate_active")
        << XmlContent(_op_ctx.cluster_state_bundle().getBaselineClusterState()->toString()) << XmlEndTag();
...
void StripeBucketDBUpdater::report_single_bucket_requests(vespalib::xml::XmlOutputStream& xos) const {
    for (const auto& entry : _sentMessages) {
        entry.second.print_xml_tag(xos, XmlAttribute("sendtimestamp", entry.second.timestamp));
    }
}

void StripeBucketDBUpdater::report_delayed_single_bucket_requests(vespalib::xml::XmlOutputStream& xos) const {
    for (const auto& entry : _delayedRequests) {
        entry.second.print_xml_tag(
            xos,
            XmlAttribute("resendtimestamp", vespalib::count_ms(vespalib::to_utc(entry.first).time_since_epoch())));
    }
}

StripeBucketDBUpdater::MergingNodeRemover::MergingNodeRemover(const lib::ClusterState& s, uint16_t localIndex,
                                                              const lib::Distribution& distribution,
...
void StripeBucketDBUpdater::MergingNodeRemover::logRemove(const document::BucketId& bucketId, const char* msg) {
    LOG(spam, "Removing bucket %s: %s", bucketId.toString().c_str(), msg);
}

bool StripeBucketDBUpdater::MergingNodeRemover::distributorOwnsBucket(const document::BucketId& bucketId) const {
...
void StripeBucketDBUpdater::MergingNodeRemover::setCopiesInEntry(BucketDatabase::Entry&         e,
                                                                 const std::vector<BucketCopy>& copies) const {
    e->clear();

    std::vector<uint16_t> order = _distribution.getIdealStorageNodes(_state, e.getBucketId(), _upStates);
...
bool StripeBucketDBUpdater::MergingNodeRemover::has_unavailable_nodes(const storage::BucketDatabase::Entry& e) const {
...
bool StripeBucketDBUpdater::MergingNodeRemover::storage_node_is_available(uint16_t index) const noexcept {
    return ((index < _available_nodes.size()) && _available_nodes[index]);
}

bool StripeBucketDBUpdater::MergingNodeRemover::node_is_present_in_config(uint16_t node_index) const noexcept {
...
public:
    explicit MergingGcTimeSetter(vespalib::system_time gc_time_point) noexcept : _last_gc(gc_time_point) {}
