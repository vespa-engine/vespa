...
SentMessageMap::SentMessageMap() : _map() {
}

SentMessageMap::~SentMessageMap() = default;

Operation* SentMessageMap::find_by_id_or_nullptr(api::StorageMessage::Id id) const noexcept {
    auto iter = _map.find(id);
    return ((iter != _map.end()) ? iter->second.get() : nullptr);
}

std::shared_ptr<Operation> SentMessageMap::find_by_id_or_empty(api::StorageMessage::Id id) const noexcept {
    auto iter = _map.find(id);
    return ((iter != _map.end()) ? iter->second : std::shared_ptr<Operation>());
}

std::shared_ptr<Operation> SentMessageMap::pop() {
...
std::shared_ptr<Operation> SentMessageMap::pop(api::StorageMessage::Id id) {
...
void SentMessageMap::insert(api::StorageMessage::Id id, const std::shared_ptr<Operation>& callback) {
    LOG(spam, "Inserting callback %p for message %" PRIu64 "", callback.get(), id);
    _map[id] = callback;
}

std::string SentMessageMap::toString() const {
