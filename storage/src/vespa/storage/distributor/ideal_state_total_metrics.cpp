...
void IdealStateTotalMetrics::aggregate_helper(IdealStateMetricSet& total) const {
...
IdealStateTotalMetrics::IdealStateTotalMetrics(uint32_t num_distributor_stripes)
    : IdealStateMetricSet(), _stripes_metrics() {
...
void IdealStateTotalMetrics::aggregate() {
    IdealStateMetricSet::reset();
    aggregate_helper(*this);
}

void IdealStateTotalMetrics::addToSnapshot(Metric& m, std::vector<Metric::UP>& owner_list) const {
    IdealStateMetricSet total;
    aggregate_helper(total);
    total.addToSnapshot(m, owner_list);
}

void IdealStateTotalMetrics::reset() {
    IdealStateMetricSet::reset();
    for (auto& stripe_metrics : _stripes_metrics) {
