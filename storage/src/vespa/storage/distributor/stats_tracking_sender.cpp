...
StatsTrackingSender::StatsTrackingSender(DistributorMessageSender& fwd_sender)
    : _fwd_sender(fwd_sender), _stats_tracker(), _stats_mutex() {
}
...
uint64_t StatsTrackingSender::sendToNode(const lib::NodeType& nodeType, uint16_t node,
...
ContentNodeMessageStatsTracker::NodeStats StatsTrackingSender::node_stats() const {
    std::lock_guard lock(_stats_mutex);
    return _stats_tracker.node_stats();
}

void StatsTrackingSender::observe_incoming_response_result(uint16_t from_node, api::MessageType::Id msg_type_id,
