...
TopLevelBucketDBUpdater::TopLevelBucketDBUpdater(
    const DistributorNodeContext& node_ctx, DistributorOperationContext& op_ctx,
    DistributorInterface& distributor_interface, DistributorMessageSender& nested_msg_sender,
    ChainedMessageSender& chained_sender, std::shared_ptr<const lib::Distribution> bootstrap_distribution,
    StripeAccessor& stripe_accessor, ClusterStateBundleActivationListener* state_activation_listener)
...
void TopLevelBucketDBUpdater::propagate_active_state_bundle_internally(bool has_bucket_ownership_transfer) {
...
void TopLevelBucketDBUpdater::bootstrap_distribution_config(std::shared_ptr<const lib::Distribution> distribution) {
...
void TopLevelBucketDBUpdater::propagate_distribution_config(const lib::BucketSpaceDistributionConfigs& configs) {
...
void TopLevelBucketDBUpdater::flush() {
}

void TopLevelBucketDBUpdater::print(std::ostream& out, bool verbose, const std::string& indent) const {
    (void)verbose;
    (void)indent;
    out << "TopLevelBucketDBUpdater";
}

bool TopLevelBucketDBUpdater::should_defer_state_enabling() const noexcept {
    return stale_reads_enabled();
}

bool TopLevelBucketDBUpdater::has_pending_cluster_state() const {
    return static_cast<bool>(_pending_cluster_state);
}

void TopLevelBucketDBUpdater::remove_superfluous_buckets(StripeAccessGuard&             guard,
    ...
    for (auto& elem : _op_ctx.bucket_space_states()) {
            ...
            db_pruning_may_be_elided(old_cluster_state, *new_cluster_state, up_states))
        {
            LOG(debug, "[bucket space '%s']: eliding DB pruning for state transition '%s' -> '%s'",
                document::FixedBucketSpaces::to_string(elem.first).c_str(), old_cluster_state.toString().c_str(),
                new_cluster_state->toString().c_str());
            continue;
        }
        auto maybe_lost =
            guard.remove_superfluous_buckets(elem.first, *new_cluster_state, is_distribution_config_change);
        if (maybe_lost.buckets != 0) {
            LOGBM(info,
                  "After cluster state change %s, %zu buckets no longer "
                  "have available replicas. %zu documents in these buckets will "
                  "be unavailable until nodes come back up",
                  old_cluster_state.getTextualDifference(*new_cluster_state).c_str(), maybe_lost.buckets,
                  maybe_lost.documents);
        }
...
void TopLevelBucketDBUpdater::maybe_inject_simulated_db_pruning_delay() {
    maybe_sleep_for(_op_ctx.distributor_config().simulated_db_pruning_latency());
}

void TopLevelBucketDBUpdater::maybe_inject_simulated_db_merging_delay() {
    maybe_sleep_for(_op_ctx.distributor_config().simulated_db_merging_latency());
}

void TopLevelBucketDBUpdater::ensure_transition_timer_started() {
...
void TopLevelBucketDBUpdater::complete_transition_timer() {
    _distributor_interface.metrics().stateTransitionTime.addValue(_transition_timer.getElapsedTimeAsDouble());
}

void TopLevelBucketDBUpdater::storage_distribution_changed(const lib::BucketSpaceDistributionConfigs& configs) {
    auto guard = _stripe_accessor.rendezvous_and_hold_all();
    storage_distribution_changed_impl(*guard, configs, false);
}

void TopLevelBucketDBUpdater::storage_distribution_changed_impl(StripeAccessGuard&                         guard,
    ...
    auto clusterInfo = std::make_shared<const SimpleClusterInformation>(_node_ctx.node_index(), _active_state_bundle,
    ...
    _pending_cluster_state = PendingClusterState::createForDistributionChange(
        _node_ctx.clock(), std::move(clusterInfo), _sender, _op_ctx.bucket_space_states(),
        _op_ctx.generate_unique_timestamp(), inhibit_request_sending);
...
void TopLevelBucketDBUpdater::reply_to_previous_pending_cluster_state_if_any() {
    if (_pending_cluster_state.get() && _pending_cluster_state->hasCommand()) {
        _chained_sender.sendUp(std::make_shared<api::SetSystemStateReply>(*_pending_cluster_state->getCommand()));
    }
}

void TopLevelBucketDBUpdater::reply_to_activation_with_actual_version(
    const api::ActivateClusterStateVersionCommand& cmd, uint32_t actualVersion) {
...
bool TopLevelBucketDBUpdater::onSetSystemState(const std::shared_ptr<api::SetSystemStateCommand>& cmd) {
    LOG(debug, "Received new cluster state %s", cmd->getSystemState().toString().c_str());
    ...
    if (bundle.has_distribution_config()) {
        ...
        if (_distributor_interface.receive_distribution_from_cluster_controller(
                distr_bundle->default_distribution_sp()))
        {
    ...
    _pending_cluster_state = PendingClusterState::createForClusterStateChange(
        _node_ctx.clock(), std::move(clusterInfo), _sender, _op_ctx.bucket_space_states(), cmd, _outdated_nodes_map,
...
bool TopLevelBucketDBUpdater::onActivateClusterStateVersion(
    ...
    if (has_pending_cluster_state() && _pending_cluster_state->isVersionedTransition()) {
        ...
        if (pending_version == cmd->version()) {
            if (is_pending_cluster_state_completed()) {
                ...
                LOG(error,
                    "Received cluster state activation for pending version %u "
                    ...
                    "reported that state processing is complete.",
                    pending_version);
                reply_to_activation_with_actual_version(
                    *cmd, 0); // Invalid version, will cause re-send (hopefully when completed).
                return true;
            }
        } else {
        ...
        LOG(warning,
            "Received cluster state activation command for version %u, which "
            "has no corresponding pending state. Likely resent operation.",
            cmd->version());
    } else {
        LOG(debug,
            "Received cluster state activation command for version %u, but distributor "
            "config does not have deferred activation enabled. Treating as no-op.",
            cmd->version());
    }
...
bool TopLevelBucketDBUpdater::onRequestBucketInfoReply(const std::shared_ptr<api::RequestBucketInfoReply>& repl) {
    attempt_accept_reply_by_current_pending_state(repl);
    return true;
}

void TopLevelBucketDBUpdater::attempt_accept_reply_by_current_pending_state(
    const std::shared_ptr<api::RequestBucketInfoReply>& repl) {
    if (_pending_cluster_state.get() && _pending_cluster_state->onRequestBucketInfoReply(repl)) {
        if (is_pending_cluster_state_completed()) {
            auto guard = _stripe_accessor.rendezvous_and_hold_all();
            process_completed_pending_cluster_state(*guard);
        }
        ...
        // does not expect it.
        LOG(spam, "Reply %s was not accepted by pending cluster state", repl->toString().c_str());
    }
}

void TopLevelBucketDBUpdater::resend_delayed_messages() {
    if (_pending_cluster_state) {
        _pending_cluster_state->resendDelayedMessages();
    }
}

bool TopLevelBucketDBUpdater::is_pending_cluster_state_completed() const {
    return _pending_cluster_state.get() && _pending_cluster_state->done();
}

void TopLevelBucketDBUpdater::process_completed_pending_cluster_state(StripeAccessGuard& guard) {
...
void TopLevelBucketDBUpdater::activate_pending_cluster_state(StripeAccessGuard& guard) {
    framework::MilliSecTimer process_timer(_node_ctx.clock());
    ...
    _node_supported_features_repo =
        _node_supported_features_repo->make_union_of(_pending_cluster_state->gathered_node_supported_features());
...
void TopLevelBucketDBUpdater::enable_current_cluster_state_bundle_in_distributor_and_stripes(
...
void TopLevelBucketDBUpdater::add_current_state_to_cluster_state_history() {
...
std::string TopLevelBucketDBUpdater::getReportContentType(const framework::HttpUrlPath&) const {
    return "text/xml";
}
...
bool TopLevelBucketDBUpdater::reportStatus(std::ostream& out, const framework::HttpUrlPath& path) const {
...
std::string TopLevelBucketDBUpdater::report_xml_status(vespalib::xml::XmlOutputStream& xos,
                                                       const framework::HttpUrlPath&) const {
    using namespace vespalib::xml;
    xos << XmlTag("bucketdb") << XmlTag("systemstate_active")
        << XmlContent(_active_state_bundle.getBaselineClusterState()->toString()) << XmlEndTag();
    if (_pending_cluster_state) {
        xos << *_pending_cluster_state;
    }
    xos << XmlTag("systemstate_history");
    for (auto i(_history.rbegin()), e(_history.rend()); i != e; ++i) {
        xos << XmlTag("change") << XmlAttribute("from", i->_prevClusterState)
            << XmlAttribute("to", i->_newClusterState)
            << XmlAttribute("processingtime", vespalib::count_us(i->_processingTime)) << XmlEndTag();
    }
    xos << XmlEndTag() << XmlTag("single_bucket_requests");
    auto guard = _stripe_accessor.rendezvous_and_hold_all();
