...
BucketSpaceState::BucketSpaceState() : _cluster_state(), _distribution() {
}

void BucketSpaceState::set_cluster_state(std::shared_ptr<const lib::ClusterState> cluster_state) {
    _cluster_state = std::move(cluster_state);
}

void BucketSpaceState::set_distribution(std::shared_ptr<const lib::Distribution> distribution) {
    _distribution = distribution;
}

BucketSpaceStateMap::BucketSpaceStateMap() : _map() {
...
void BucketSpaceStateMap::set_cluster_state(std::shared_ptr<const lib::ClusterState> cluster_state) {
    for (auto& space : _map) {
        space.second->set_cluster_state(cluster_state);
    }
}

void BucketSpaceStateMap::set_distribution(std::shared_ptr<const lib::Distribution> distribution) {
    for (auto& space : _map) {
