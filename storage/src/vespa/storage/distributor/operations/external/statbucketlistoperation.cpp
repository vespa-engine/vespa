...
StatBucketListOperation::StatBucketListOperation(const BucketDatabase&                             bucketDb,
                                                 const MaintenanceOperationGenerator&              generator,
                                                 uint16_t                                          distributorIndex,
                                                 const std::shared_ptr<api::GetBucketListCommand>& cmd)
    : _bucketDb(bucketDb), _generator(generator), _distributorIndex(distributorIndex), _command(cmd) {
}

StatBucketListOperation::~StatBucketListOperation() = default;

void StatBucketListOperation::getBucketStatus(const BucketDatabase::Entry& entry, std::ostream& ost) const {
...
void StatBucketListOperation::onStart(DistributorStripeMessageSender& sender) {
