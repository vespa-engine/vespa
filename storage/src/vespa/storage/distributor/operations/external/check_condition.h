...
public:
    ...
    public:
        ...
        [[nodiscard]] bool failed() const noexcept { return _error_code.failed(); }

        const api::ReturnCode& error_code() const noexcept { return _error_code; }

        [[nodiscard]] bool matched_condition() const noexcept { return _result == Result::MatchedCondition; }

        [[nodiscard]] bool not_found() const noexcept { return _result == Result::NotFound; }

        [[nodiscard]] const vespalib::Trace& trace() const noexcept { return _trace; }
...
public:
    CheckCondition(Outcome known_outcome, const DistributorBucketSpace& bucket_space,
                   const DistributorNodeContext& node_ctx, private_ctor_tag);
    CheckCondition(const document::Bucket& bucket, const document::DocumentId& doc_id,
                   const documentapi::TestAndSetCondition& tas_condition, const DistributorBucketSpace& bucket_space,
                   const DistributorNodeContext& node_ctx, PersistenceOperationMetricSet& condition_probe_metrics,
                   uint32_t trace_level, private_ctor_tag);
    ~CheckCondition();

    void start_and_send(DistributorStripeMessageSender& sender);
    void handle_reply(DistributorStripeMessageSender& sender, const std::shared_ptr<api::StorageReply>& reply);
    ...
    [[nodiscard]] std::optional<Outcome>& maybe_outcome() noexcept { return _outcome; }

    [[nodiscard]] static std::shared_ptr<CheckCondition> create_if_inconsistent_replicas(
        const document::Bucket& bucket, const DistributorBucketSpace& bucket_space,
        const document::DocumentId& doc_id, const documentapi::TestAndSetCondition& tas_condition,
        const DistributorNodeContext& node_ctx, const DistributorStripeOperationContext& op_ctx,
...
private:
    ...
    newest_replica_to_outcome(const std::optional<NewestReplica>& newest) noexcept;

    [[nodiscard]] static bool bucket_has_consistent_replicas(std::span<const BucketDatabase::Entry> entries);

    [[nodiscard]] static bool
    all_nodes_support_document_condition_probe(std::span<const BucketDatabase::Entry>   entries,
                                               const DistributorStripeOperationContext& op_ctx);

    [[nodiscard]] static std::vector<BucketDatabase::Entry>
    get_bucket_database_entries(const DistributorBucketSpace& bucket_space, const document::BucketId& bucket_id);

    [[nodiscard]] static std::shared_ptr<CheckCondition> create_not_found(const DistributorBucketSpace& bucket_space,
