...
public:
    struct Config {
        Config(uint32_t minBucketsPerVisitor_, uint32_t maxVisitorsPerNodePerVisitor_)
    ...
    VisitorOperation(const DistributorNodeContext& node_ctx, DistributorStripeOperationContext& op_ctx,
                     DistributorBucketSpace& bucketSpace, const std::shared_ptr<api::CreateVisitorCommand>& msg,
    ...
    void assign_put_lock_access_token(const std::string& token);
...
private:
    ...
    struct SuperBucketInfo {
        ...
        explicit SuperBucketInfo(const document::BucketId& b = document::BucketId(0))
            : bid(b), subBucketsCompletelyExpanded(false) {}
        ~SuperBucketInfo();
    };
    ...
    void expandBucket();
    int pickTargetNode(const BucketDatabase::Entry& entry, const std::vector<uint16_t>& triedNodes);
    ...
    bool sendStorageVisitors(const NodeToBucketsMap& nodeToBucketsMap, DistributorStripeMessageSender& sender);
    void sendStorageVisitor(uint16_t node, const std::vector<document::BucketId>& buckets, uint32_t pending,
     ...
     * failing node.
     */
    void markOperationAsFailedDueToNodeError(const api::ReturnCode& result, uint16_t fromFailingNodeIndex);
    /**
