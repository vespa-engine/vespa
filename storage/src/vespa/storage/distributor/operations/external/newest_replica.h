...
struct NewestReplica {
    ...
    static NewestReplica of(api::Timestamp timestamp, const document::BucketId& bucket_id, uint16_t node,
                            bool is_tombstone, bool condition_matched) noexcept {
        return {timestamp, bucket_id, node, is_tombstone, condition_matched};
    }

    static NewestReplica make_empty() { return {api::Timestamp(0), document::BucketId(), 0, false, false}; }

    bool operator==(const NewestReplica& rhs) const noexcept {
        return ((timestamp == rhs.timestamp) && (bucket_id == rhs.bucket_id) && (node == rhs.node) &&
                (is_tombstone == rhs.is_tombstone) && (condition_matched == rhs.condition_matched));
    }
    bool operator!=(const NewestReplica& rhs) const noexcept { return !(*this == rhs); }
};
