...
public:
     ...
     */
    JoinOperation(const ClusterContext& cluster_ctx, const BucketAndNodes& nodes,
    ...
    const char* getName() const noexcept override { return "join"; }

    Type getType() const noexcept override { return JOIN_BUCKET; }

    bool isBlocked(const DistributorStripeOperationContext& ctx, const OperationSequencer& op_seq) const override;

protected:
    using NodeToBuckets = std::map<uint16_t, std::vector<document::BucketId>>;
    NodeToBuckets resolveSourceBucketsPerTargetNode() const;

    void fillMissingSourceBucketsForInconsistentJoins(NodeToBuckets& nodeToBuckets) const;
