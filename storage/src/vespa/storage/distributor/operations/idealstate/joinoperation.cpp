...
JoinOperation::JoinOperation(const ClusterContext& clusterName, const BucketAndNodes& nodes,
                             const std::vector<document::BucketId>& bucketsToJoin)
    : IdealStateOperation(nodes), _tracker(clusterName), _bucketsToJoin(bucketsToJoin) {
}

JoinOperation::~JoinOperation() = default;

void JoinOperation::onStart(DistributorStripeMessageSender& sender) {
    _ok = false;

    if (_bucketsToJoin.size() == 1) {
        LOG(debug, "Starting join operation for %s -> %s", _bucketsToJoin[0].toString().c_str(),
            getBucketId().toString().c_str());
    } else {
        LOG(debug, "Starting join operation for (%s,%s) -> %s", _bucketsToJoin[0].toString().c_str(),
            _bucketsToJoin[1].toString().c_str(), getBucketId().toString().c_str());
    }
    ...
    if (!_ok) {
        LOGBP(debug, "Unable to join bucket %s, since no copies are available (some in maintenance?)",
              getBucketId().toString().c_str());
        done();
    } else {
...
JoinOperation::NodeToBuckets JoinOperation::resolveSourceBucketsPerTargetNode() const {
...
void JoinOperation::fillMissingSourceBucketsForInconsistentJoins(NodeToBuckets& nodeToBuckets) const {
...
bool JoinOperation::enqueueJoinMessagePerTargetNode(const NodeToBuckets& nodeToBuckets) {
...
void JoinOperation::onReceive(DistributorStripeMessageSender&, const api::StorageReply::SP& msg) {
    ...
    if (_cancel_scope.node_is_cancelled(node)) {
        ...
        const std::vector<document::BucketId>& sourceBuckets(rep.getSourceBuckets());
               ...
               _bucketSpace->getBucketDatabase().get(getBucketId())->getNode(node) != nullptr)
    {
        ...
        LOGBP(warning, "Join failed to find %s: %s", getBucketId().toString().c_str(),
        ...
        LOGBP(warning, "Join failed for %s: %s with error '%s'", getBucketId().toString().c_str(),
              msg->toString().c_str(), msg->getResult().toString().c_str());
    } else {
        LOG(debug, "Join failed for %s with non-critical failure: %s", getBucketId().toString().c_str(),
            rep.getResult().toString().c_str());
    }
...
document::Bucket JoinOperation::getJoinBucket(size_t idx) const {
    return {getBucket().getBucketSpace(), _bucketsToJoin[idx]};
}

bool JoinOperation::isBlocked(const DistributorStripeOperationContext& ctx, const OperationSequencer& op_seq) const {
    return (checkBlock(getBucket(), ctx, op_seq) || checkBlock(getJoinBucket(0), ctx, op_seq) ||
