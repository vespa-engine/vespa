...
SplitOperation::SplitOperation(const ClusterContext& cluster_ctx, const BucketAndNodes& nodes, uint32_t maxBits,
...
void SplitOperation::onStart(DistributorStripeMessageSender& sender) {
    _ok = false;
    ...
    if (!_ok) {
        LOGBP(debug, "Unable to split bucket %s, since no copies are available (some in maintenance?)",
...
void SplitOperation::onReceive(DistributorStripeMessageSender&, const api::StorageReply::SP& msg) {
               ...
               _bucketSpace->getBucketDatabase().get(rep.getBucketId())->getNode(node) != nullptr)
    {
        ...
        LOGBP(debug,
              "Split failed for %s: bucket not found. Storage and distributor bucket databases might be out of sync: "
              "%s",
              getBucketId().toString().c_str(), std::string(rep.getResult().getMessage()).c_str());
        _ok = false;
        ...
        LOGBP(warning, "Split failed for %s: %s with error '%s'", getBucketId().toString().c_str(),
              msg->toString().c_str(), msg->getResult().toString().c_str());
        _ok = false;
    } else {
        LOG(debug, "Split failed for %s with non-critical failure: %s", getBucketId().toString().c_str(),
...
bool SplitOperation::isBlocked(const DistributorStripeOperationContext& ctx, const OperationSequencer& op_seq) const {
    return checkBlockForAllNodes(getBucket(), ctx, op_seq);
}

bool SplitOperation::shouldBlockThisOperation(uint32_t msgType, [[maybe_unused]] uint16_t node, uint8_t pri) const {
