...
public:
    EqualCopies() : _checksum(0), _trustedCopies(0) {}
    ...
    bool operator==(const MergeMetadata& mmd) const noexcept { return (_checksum == mmd.checksum()); }
...
public:
    ...
    Statistics(const MergeLimiter::NodeArray& a) : _trustedCopies(0) {
...
// some of each.
void addNodes(uint32_t max, Statistics& stats, MergeLimiter::NodeArray& result) {
...
void MergeLimiter::limitMergeToMaxNodes(NodeArray& nodes) {
