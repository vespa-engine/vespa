...
bool ThrottlingOperationStarter::canStart(uint32_t currentOperationCount, Priority priority) const {
...
bool ThrottlingOperationStarter::start(const std::shared_ptr<Operation>& operation, Priority priority) {
...
bool ThrottlingOperationStarter::may_allow_operation_with_priority(Priority priority) const noexcept {
