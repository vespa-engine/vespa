...
PendingMessageTracker::clearMessagesForNode(uint16_t node)
{
    ...
    for (auto& entry : range) {
        ...
        _node_stats.stats_for(entry.nodeIdx).observe_cancelled();
    }
...
PendingMessageTracker::insert(const std::shared_ptr<api::StorageMessage>& msg)
{
    if (msg->getAddress()) {
        ...
        document::Bucket bucket = getBucket(*msg);
        {
            ...
            _node_stats.stats_for(node_index).observe_outgoing_request();
        }
...
PendingMessageTracker::reply(const api::StorageReply& r)
{
    ...
    std::unique_lock guard(_lock);
    // TODO _node_stats.observe_incoming_response_result(node_index, code);
    ...
    if (iter != msgs.end()) {
        ...
        _node_stats.stats_for(node_index).observe_incoming_response_result(r.getType().getId(), code);
