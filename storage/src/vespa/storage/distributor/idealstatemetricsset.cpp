...
OperationMetricSet::OperationMetricSet(const std::string& name, metrics::Metric::Tags tags,
    ...
    : MetricSet(name, std::move(tags), description, owner),
      ...
      blocked("blocked", {{"logdefault"}, {"yamasdefault"}},
              "The number of operations blocked by blocking operation starter", this),
      throttled("throttled", {{"logdefault"}, {"yamasdefault"}},
                "The number of operations throttled by throttling operation starter", this) {
}

OperationMetricSet::~OperationMetricSet() = default;

GcMetricSet::GcMetricSet(const std::string& name, metrics::Metric::Tags tags, const std::string& description,
                         MetricSet* owner)
    : OperationMetricSet(name, std::move(tags), description, owner),
      documents_removed("documents_removed", {{"logdefault"}, {"yamasdefault"}},
                        "Number of documents removed by GC operations", this) {
}

GcMetricSet::~GcMetricSet() = default;

MergeBucketMetricSet::MergeBucketMetricSet(const std::string& name, metrics::Metric::Tags tags,
                                           const std::string& description, MetricSet* owner)
    : OperationMetricSet(name, std::move(tags), description, owner),
      source_only_copy_changed("source_only_copy_changed", {{"logdefault"}, {"yamasdefault"}},
                               "The number of merge operations where source-only copy changed", this),
      source_only_copy_delete_blocked(
          "source_only_copy_delete_blocked", {{"logdefault"}, {"yamasdefault"}},
          "The number of merge operations where delete of unchanged source-only copies was blocked", this),
      source_only_copy_delete_failed(
          "source_only_copy_delete_failed", {{"logdefault"}, {"yamasdefault"}},
...
void IdealStateMetricSet::createOperationMetrics() {
    ...
    operations[ISO::MERGE_BUCKET] = std::make_shared<MergeBucketMetricSet>(
        "merge_bucket", metrics::Metric::Tags{{"logdefault"}, {"yamasdefault"}},
    ...
    operations[ISO::SET_BUCKET_STATE] = std::shared_ptr<OperationMetricSet>(
        new OperationMetricSet("set_bucket_state", {{"logdefault"}, {"yamasdefault"}},
                               "Operations to set active/ready state for bucket copies", this));
    operations[ISO::GARBAGE_COLLECTION] =
        std::shared_ptr<OperationMetricSet>(new GcMetricSet("garbage_collection", {{"logdefault"}, {"yamasdefault"}},
...
IdealStateMetricSet::IdealStateMetricSet()
    : MetricSet("idealstate", {{"idealstate"}}, "Statistics for ideal state generation"),
      idealstate_diff("idealstate_diff", {{"logdefault"}, {"yamasdefault"}},
                      ...
                      "is getting closer to the ideal state",
                      this),
      ...
      buckets_rechecking("buckets_rechecking", {{"logdefault"}, {"yamasdefault"}},
                         ...
                         this),
      buckets_replicas_moving_out("bucket_replicas_moving_out", {{"logdefault"}, {"yamasdefault"}},
                                  ...
                                  this),
      buckets_replicas_copying_in("bucket_replicas_copying_in", {{"logdefault"}, {"yamasdefault"}},
                                  "Bucket replicas that should be copied in, e.g. node does not have a "
                                  "replica for a bucket that it is in ideal state for",
                                  this),
      buckets_replicas_copying_out("bucket_replicas_copying_out", {{"logdefault"}, {"yamasdefault"}},
                                   ...
                                   this),
      buckets_replicas_syncing("bucket_replicas_syncing", {{"logdefault"}, {"yamasdefault"}},
                               "Bucket replicas that need syncing due to mismatching metadata", this),
      max_observed_time_since_last_gc_sec(
          "max_observed_time_since_last_gc_sec", {{"logdefault"}, {"yamasdefault"}},
          "Maximum time (in seconds) since GC was last successfully run for a bucket. "
          "Aggregated max value across all buckets on the distributor.",
          this),
...
void IdealStateMetricSet::setPendingOperations(std::span<uint64_t, IdealStateOperation::OPERATION_COUNT> newMetrics) {
    ...
    idealstate_diff.set(operations[IdealStateOperation::DELETE_BUCKET]->pending.getLast() +
