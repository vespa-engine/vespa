...
public:
    ...
    public:
        DbMerger(api::Timestamp creation_timestamp, const lib::Distribution& distribution,
                 const lib::ClusterState& new_state, const char* storage_up_states,
                 const OutdatedNodes& outdated_nodes, const std::vector<dbtransition::Entry>& entries)
        ...
        bool nodeIsOutdated(uint16_t node) const { return (_outdated_nodes.find(node) != _outdated_nodes.end()); }
    };

    PendingBucketSpaceDbTransition(document::BucketSpace bucket_space, const BucketSpaceState& bucket_space_state,
                                   bool distributionChanged, const OutdatedNodes& outdatedNodes,
    ...
    void incrementRequestRejections(uint16_t node) { _rejectedRequests[node]++; }
    ...
    void increment_request_failures(uint16_t node) { _failed_requests[node]++; }
