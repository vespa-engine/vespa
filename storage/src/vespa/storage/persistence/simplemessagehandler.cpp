...
document::FieldSet::SP getFieldSet(const document::FieldSetRepo& repo, std::string_view name,
                                   MessageTracker& tracker) {
    try {
        return repo.getFieldSet(name);
    } catch (document::FieldNotFoundException& e) {
        tracker.fail(
            storage::api::ReturnCode::ILLEGAL_PARAMETERS,
...
SimpleMessageHandler::SimpleMessageHandler(const PersistenceUtil& env, spi::PersistenceProvider& spi,
    ...
    : _env(env), _spi(spi), _bucket_id_factory(bucket_id_factory) {
}

MessageTracker::UP SimpleMessageHandler::handle_conditional_get(api::GetCommand&   cmd,
                                                                MessageTracker::UP tracker) const {
    if (cmd.getFieldSet() == document::NoFields::NAME) {
        TestAndSetHelper tas_helper(_env, _spi, _bucket_id_factory, cmd.condition(), cmd.getBucket(),
                                    cmd.getDocumentId(), nullptr);
...
MessageTracker::UP SimpleMessageHandler::handleGet(api::GetCommand& cmd, MessageTracker::UP tracker) const {
    ...
    spi::GetResult result = _spi.get(_env.getBucket(cmd.getDocumentId(), cmd.getBucket()), *fieldSet,
                                     cmd.getDocumentId(), tracker->context());

    if (tracker->checkForError(result)) {
        if (!result.hasDocument() && (document::FieldSet::Type::NONE != fieldSet->getType())) {
            metrics.notFound.inc();
        }
        tracker->setReply(std::make_shared<api::GetReply>(cmd, result.getDocumentPtr(), result.getTimestamp(), false,
...
MessageTracker::UP SimpleMessageHandler::handleGetIter(GetIterCommand& cmd, MessageTracker::UP tracker) const {
...
MessageTracker::UP SimpleMessageHandler::handleCreateIterator(CreateIteratorCommand& cmd,
                                                              MessageTracker::UP     tracker) const {
    ...
    spi::CreateIteratorResult result(_spi.createIterator(spi::Bucket(cmd.getBucket()), std::move(fieldSet),
                                                         cmd.getSelection(), cmd.getIncludedVersions(),
                                                         tracker->context()));
    if (tracker->checkForError(result)) {
