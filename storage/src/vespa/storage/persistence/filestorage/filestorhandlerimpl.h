...
class FileStorHandlerImpl final : private framework::MetricUpdateHook,
...
public:
    struct MessageEntry {
        ...
        MessageEntry(const std::shared_ptr<api::StorageMessage>& cmd, const document::Bucket& bucket,
        ...
        bool operator<(const MessageEntry& entry) const noexcept { return (_priority < entry._priority); }
    };
    ...
    struct PriorityQueue {
        ...
        template <EntryCmp cmp> struct OrderCmp {
            ...
            template <typename T> bool operator()(EntryPtr a, const T& b) const noexcept {
                return b.convert(a) < b.convert();
            }
            template <typename T> bool operator()(const T& a, EntryPtr b) const noexcept {
                return a.convert() < a.convert(b);
            }
        };
        static bool compareByPriority(const MessageEntry& a, const MessageEntry& b) {
            return a._priority < b._priority;
        }
        static bool compareByBucket(const MessageEntry& a, const MessageEntry& b) { return a._bucket < b._bucket; }
        ...
        template <typename I, typename V> struct ordered_iterator {
            ...
            bool operator==(const ordered_iterator& other) const noexcept { return _place == other._place; }
            bool operator!=(const ordered_iterator& other) const noexcept { return _place != other._place; }
        ...
        struct BucketIdxView {
            ...
            std::pair<iterator, iterator> equal_range(const document::Bucket& bucket) {
                ...
                return std::make_pair(iterator(inner_range.first), iterator(inner_range.second));
            }
        };

        PriorityQueue() : _main_map(), _sequence_ids_by_priority(), _sequence_ids_by_bucket() {}
        ~PriorityQueue();
    ...
    public:
        ...
        void unsafe_update_cached_queue_size() { _cached_queue_size.store_relaxed(_queue->size()); }
        ...
        bool isLocked(const monitor_guard&, const document::Bucket&, api::LockingRequirements lockReq) const noexcept;

        void lock(const monitor_guard&, const document::Bucket& bucket, api::LockingRequirements lockReq,
                  bool count_as_active_maintenance, const LockEntry& lockEntry);

        [[nodiscard]] std::shared_ptr<FileStorHandler::BucketLockInterface> lock(const document::Bucket&  bucket,
        ...
        [[nodiscard]] FileStorHandler::LockedMessageBatch next_message_batch(vespalib::steady_time now,
        ...
        [[nodiscard]] ConstPriorityIdxView exposePriorityIdxView() const noexcept {
            return ConstPriorityIdxView(*_queue);
        }
    ...
    private:
        ...
        [[nodiscard]] FileStorHandler::LockedMessage next_message_impl(monitor_guard&        held_lock,
                                                                       vespalib::steady_time deadline);
        void fill_feed_op_batch(monitor_guard& held_lock, LockedMessageBatch& batch, uint32_t max_batch_size,
    ...
    public:
        ...
        BucketLock(const monitor_guard& guard, Stripe& disk, const document::Bucket& bucket, uint8_t priority,
                   api::MessageType::Id msgType, api::StorageMessage::Id, api::LockingRequirements lockReq);
        ~BucketLock() override;
    ...
    FileStorHandlerImpl(MessageSender& sender, FileStorMetrics& metrics, ServiceLayerComponentRegister& compReg);
    FileStorHandlerImpl(
    ...
    LockedMessageBatch next_message_batch(uint32_t stripe, vespalib::steady_time now,
                                          vespalib::steady_time deadline) override;
    ...
    std::shared_ptr<FileStorHandler::BucketLockInterface> lock(const document::Bucket&  bucket,
                                                               api::LockingRequirements lockReq) override {
    ...
    void reconfigure_dynamic_operation_throttler(
        const vespalib::SharedOperationThrottler::DynamicThrottleParams& params) override;
    void reconfigure_dynamic_maintenance_throttler(
        const vespalib::SharedOperationThrottler::DynamicThrottleParams& params) override;
...
private:
    ...
    document::Bucket remapMessage(api::StorageMessage& msg, const document::Bucket& source, Operation op,
     ...
     * be made by callers.
     */
    DiskState getState() const noexcept { return _state.load(std::memory_order_relaxed); }
    /**
     * No assumption on memory ordering around disk state writes should
     * be made by callers.
     */
    void setState(DiskState s) noexcept { _state.store(s, std::memory_order_relaxed); }
    ...
    Stripe& stripe(const document::Bucket& bucket) { return _stripes[stripe_index(bucket)]; }
