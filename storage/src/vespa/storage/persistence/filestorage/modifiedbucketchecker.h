...
class ModifiedBucketChecker : public StorageLink, public framework::Runnable, public Types {
public:
    using StorServerConfig = vespa::config::content::core::StorServerConfig;
    using UP = std::unique_ptr<ModifiedBucketChecker>;

    ModifiedBucketChecker(ServiceLayerComponentRegister& compReg, spi::PersistenceProvider& provide,
    ...
    void setUnitTestingSingleThreadedMode() { _singleThreadMode = true; }

private:
    bool onInternalReply(const std::shared_ptr<api::InternalReply>&) override;
    bool currentChunkFinished() const { return _pendingRequests == 0; }
    bool moreChunksRemaining() const { return !_rechecksNotStarted.empty(); }
    ...
    public:
        ...
        document::BucketSpace next() { return _bucketSpaces[(_idx++) % _bucketSpaces.size()]; }
    };
    ...
    public:
        BucketIdListResult();
        void reset(document::BucketSpace bucketSpace, document::bucket::BucketIdList& buckets);
