...
void write_attribute_usage(vespalib::JsonStream& output, const std::string& label,
                           const spi::AttributeResourceUsage& usage) {
    output << label << Object();
...
bool want_immediate_report(const spi::ResourceUsage& old_usage, const spi::ResourceUsage& new_usage,
    ...
    auto address_space_diff = fabs(new_usage.get_attribute_address_space_usage().get_usage() -
                                   old_usage.get_attribute_address_space_usage().get_usage());
    ...
    return ((disk_usage_diff > noise_level) || (memory_usage_diff > noise_level) ||
            (address_space_diff > noise_level) || address_space_got_valid);
}

} // namespace

ServiceLayerHostInfoReporter::ServiceLayerHostInfoReporter(NodeStateUpdater& node_state_updater, double noise_level)
    : HostReporter(),
...
void ServiceLayerHostInfoReporter::set_noise_level(double level) {
    _noise_level.store(level, std::memory_order_relaxed);
}

namespace {

std::string to_string(const spi::ResourceUsage& usage) {
...
void ServiceLayerHostInfoReporter::update_resource_usage(const spi::ResourceUsage& resource_usage) {
...
void ServiceLayerHostInfoReporter::report(vespalib::JsonStream& output) {
