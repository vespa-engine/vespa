...
MergeStatus::MergeStatus(const framework::Clock& clock, api::StorageMessage::Priority priority, uint32_t traceLevel)
    : reply(),
      full_node_list(),
      nodeList(),
      maxTimestamp(0),
 ...
 */
bool MergeStatus::removeFromDiff(const std::vector<api::ApplyBucketDiffCommand::Entry>& part, uint16_t hasMask,
        ...
        if (it->_timestamp != it2->_entry._timestamp) {
                ...
                ((it2->_entry._hasMask == 0) && (remap_mask(0u, it->_hasMask) == 0u)))
            {
                if (it2->_entry._hasMask == 0) {
                    LOG(debug, "Merge entry %s no longer exists on any nodes", it2->toString().c_str());
                }
                // Timestamp equal. Should really be the same entry. If not
                // though, there is nothing we can do but accept it.
                if (!(*it == it2->_entry)) {
                    LOG(warning,
                        "Merge retrieved entry %s for entry %s but "
                        "these do not match.",
    ...
    if (it2 != part.end()) {
        ...
        LOG(warning,
            "Apply bucket diff contained %u entries not existing in "
            "the request.",
            counter);
    }
...
void MergeStatus::print(std::ostream& out, bool verbose, const std::string& indent) const {
    if (reply.get()) {
        (void)verbose;
        ...
        for (std::deque<api::GetBucketDiffCommand::Entry>::const_iterator it = diff.begin(); it != diff.end(); ++it) {
...
void MergeStatus::set_delayed_error(std::future<std::string>&& delayed_error_in) {
    delayed_error = std::move(delayed_error_in);
}

void MergeStatus::check_delayed_error(api::ReturnCode& return_code) {
