...
template <typename T> void merge_min(std::optional<T>& min, const std::optional<T>& rhs_min) {
...
template <typename T> void merge_max(std::optional<T>& max, const std::optional<T>& rhs_max) {
...
template <typename T> void merge_min_max_sum(std::optional<T>& lhs, const std::optional<T>& rhs) {
    if (!rhs.has_value()) {
        return;
    }
    if (lhs.has_value()) {
...
void ActiveOperationsStats::update_size() noexcept {
    ++_size_samples;
    _total_size += _active_size;
    update_min_max(_active_size, _min_size, _max_size);
}

ActiveOperationsStats& ActiveOperationsStats::operator-=(const ActiveOperationsStats& rhs) noexcept {
...
void ActiveOperationsStats::merge(const ActiveOperationsStats& rhs) noexcept {
    _size_samples += rhs._size_samples;
    _total_size += rhs._total_size;
...
void ActiveOperationsStats::operation_started() noexcept {
    ++_active_size;
    update_size();
}

void ActiveOperationsStats::operation_done(double latency) noexcept {
    --_active_size;
    update_size();
    ++_latency_samples;
...
void ActiveOperationsStats::reset_min_max() noexcept {
    _min_size.reset();
    _max_size.reset();
    _min_latency.reset();
    _max_latency.reset();
