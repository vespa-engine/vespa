...
public:
    AsyncMessageBatch(std::shared_ptr<FileStorHandler::BucketLockInterface> bucket_lock, const PersistenceUtil& env,
...
public:
    ...
    [[nodiscard]] const api::StorageReply& getReply() const { return *_reply; }
    [[nodiscard]] api::StorageReply& getReply() { return *_reply; }
    [[nodiscard]] std::shared_ptr<api::StorageReply>&& stealReplySP() && { return std::move(_reply); }
    ...
    [[nodiscard]] document::BucketId getBucketId() const { return _bucketLock->getBucket().getBucketId(); }
    ...
    [[nodiscard]] std::shared_ptr<FileStorHandler::OperationSyncPhaseDoneNotifier>
    sync_phase_done_notifier_or_nullptr() const;

    static MessageTracker::UP createForTesting(const framework::MilliSecTimer& timer, PersistenceUtil& env,
...
private:
    MessageTracker(const framework::MilliSecTimer& timer, const PersistenceUtil& env, MessageSender& reply_sender,
                   bool update_bucket_info, std::shared_ptr<FileStorHandler::BucketLockInterface> bucket_lock,
                   std::shared_ptr<AsyncMessageBatch> part_of_batch, std::shared_ptr<api::StorageMessage> msg,
                   ThrottleToken throttle_token);
...
public:
    ...
    PersistenceUtil(const ServiceLayerComponent&, FileStorHandler& fileStorHandler, FileStorThreadMetrics& metrics,
    ...
    LockResult lockAndGetDisk(const document::Bucket&  bucket,
...
private:
    bool componentHasChanged() const { return _lastGeneration != _component.getGeneration(); }
