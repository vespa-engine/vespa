...
struct BucketVisitor : public BucketProcessor::EntryProcessor {
    ...
    struct DocInfo {
        ...
        DocInfo(uint64_t ts, const document::DocumentId& id, document::BucketId& bid)
            : timestamp(ts), docId(id), bucketId(bid) {}
    };
    ...
    void process(std::unique_ptr<spi::DocEntry> entry) override {
        ...
        while ((bucket.getRawId() & _splitMask) != (_refBucket.getRawId() & _splitMask)) {
            --_splitBit;
    ...
    void printEntrySummary(std::ostream& out) {
        for (uint32_t i = 0; i < _firstDocs.size(); ++i) {
            out << "\n" << _firstDocs[i].timestamp << ' ' << _firstDocs[i].bucketId << ' ' << _firstDocs[i].docId;
        }
    }
};
...
bool deduceBucketIsInconsistentlySplit(uint32_t minCount) {
...
SplitBitDetector::Result SplitBitDetector::detectSplit(spi::PersistenceProvider& provider, const spi::Bucket& source,
                                                       uint32_t maxSplitBits, spi::Context& context,
    ...
    if (maxSplitBits <= source.getBucketId().getUsedBits()) {
        ...
        LOG(warning, "split(%s): %s", source.getBucketId().toString().c_str(), error.str().c_str());
        return Result(error.str());
    }
    document::BucketIdFactory factory;
    BucketVisitor             detector(factory);

    BucketProcessor::iterateAll(provider, source, "", std::make_shared<document::DocIdOnly>(), detector,
                                spi::ALL_VERSIONS, context);
    ...
    if (maxSplitBits != 0 && maxSplitBits < splitBit) {
        ...
        singleTarget = true;
        LOG(debug, "split(%s) - Found split bit %u but max is %u.", source.toString().c_str(), splitBit,
            maxSplitBits);
    }
    ...
    if (smallerThanSizeLimit(minCount, minSize, source, provider)) {
        if (LOG_WOULD_LOG(debug)) {
            spi::BucketInfo info(provider.getBucketInfo(source).getBucketInfo());
            LOG(debug,
                "split(%s) - Bucket too small to trigger split. "
                "%u docs, %u size. (Split size at %u/%u). "
                "Only splitting to %u.",
                source.toString().c_str(), info.getDocumentCount(), info.getDocumentSize(), minCount, minSize,
    ...
    if (splitBit == 58) {
        ...
        if (deduceBucketIsInconsistentlySplit(minCount)) {
            ...
            LOGBT(warning, source.getBucketId().toString(), "split(%s): %s", source.getBucketId().toString().c_str(),
                  error.str().c_str());
            splitBit = 57; // + 1 below.
        }
    }
    ...
    if (splitBit < source.getBucketId().getUsedBits()) {
        LOG(error,
            "split(%s): Document(s) in wrong bucket, and thus "
    ...
    document::BucketId target2(splitBit + 1, base.getId() | (uint64_t(1) << splitBit));
    return Result(target1, target2, singleTarget);
}

void SplitBitDetector::Result::print(std::ostream& out, bool verbose, const std::string& indent) const {
    (void)verbose;
    (void)indent;
    out << "SplitTargets(";
    switch (_result) {
