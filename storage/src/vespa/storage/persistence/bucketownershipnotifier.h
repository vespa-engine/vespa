...
public:
    BucketOwnershipNotifier(const ServiceLayerComponent& component, MessageSender& sender)
        : _component(component), _sender(sender) {}

    [[nodiscard]] bool distributorOwns(uint16_t distributor, const document::Bucket& bucket) const;
    void notifyIfOwnershipChanged(const document::Bucket& bucket, uint16_t sourceIndex,
...
private:
    enum IndexMeta { FAILED_TO_RESOLVE = 0xffff };
    ...
    void logNotification(const document::Bucket& bucket, uint16_t sourceIndex, uint16_t currentOwnerIndex,
...
class NotificationGuard {
    struct BucketToCheck {
        BucketToCheck(const document::Bucket& _bucket, uint16_t _sourceIndex, const api::BucketInfo& _info)
            : bucket(_bucket), info(_info), sourceIndex(_sourceIndex), alwaysSend(false) {}
...
public:
    explicit NotificationGuard(BucketOwnershipNotifier& notifier) : _notifier(notifier), _bucketsToCheck() {}
