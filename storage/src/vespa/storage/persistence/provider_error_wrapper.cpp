...
template <typename ResultType> ResultType ProviderErrorWrapper::checkResult(ResultType&& result) const {
    handle(result);
    return std::forward<ResultType>(result);
}

ProviderErrorWrapper::ProviderErrorWrapper(spi::PersistenceProvider& impl) noexcept : _impl(impl), _mutex() {
}
ProviderErrorWrapper::~ProviderErrorWrapper() = default;

void ProviderErrorWrapper::handle(const spi::Result& result) const {
...
spi::Result ProviderErrorWrapper::initialize() {
    return checkResult(_impl.initialize());
}

spi::BucketIdListResult ProviderErrorWrapper::listBuckets(BucketSpace bucketSpace) const {
    return checkResult(_impl.listBuckets(bucketSpace));
}

spi::Result ProviderErrorWrapper::setClusterState(BucketSpace bucketSpace, const spi::ClusterState& state) {
    return checkResult(_impl.setClusterState(bucketSpace, state));
}

void ProviderErrorWrapper::setActiveStateAsync(const spi::Bucket& bucket, spi::BucketInfo::ActiveState newState,
...
spi::BucketInfoResult ProviderErrorWrapper::getBucketInfo(const spi::Bucket& bucket) const {
    return checkResult(_impl.getBucketInfo(bucket));
}

spi::GetResult ProviderErrorWrapper::get(const spi::Bucket& bucket, const document::FieldSet& fieldSet,
                                         const document::DocumentId& docId, spi::Context& context) const {
    return checkResult(_impl.get(bucket, fieldSet, docId, context));
}

spi::CreateIteratorResult ProviderErrorWrapper::createIterator(const spi::Bucket& bucket, FieldSetSP fieldSet,
...
spi::IterateResult ProviderErrorWrapper::iterate(spi::IteratorId iteratorId, uint64_t maxByteSize) const {
    return checkResult(_impl.iterate(iteratorId, maxByteSize));
}

spi::Result ProviderErrorWrapper::destroyIterator(spi::IteratorId iteratorId) {
    return checkResult(_impl.destroyIterator(iteratorId));
}

void ProviderErrorWrapper::deleteBucketAsync(const spi::Bucket&         bucket,
                                             spi::OperationComplete::UP onComplete) noexcept {
    onComplete->addResultHandler(this);
    _impl.deleteBucketAsync(bucket, std::move(onComplete));
}

void ProviderErrorWrapper::createBucketAsync(const spi::Bucket&         bucket,
                                             spi::OperationComplete::UP onComplete) noexcept {
    onComplete->addResultHandler(this);
    _impl.createBucketAsync(bucket, std::move(onComplete));
}

spi::BucketIdListResult ProviderErrorWrapper::getModifiedBuckets(BucketSpace bucketSpace) const {
    return checkResult(_impl.getModifiedBuckets(bucketSpace));
}

spi::Result ProviderErrorWrapper::split(const spi::Bucket& source, const spi::Bucket& target1,
                                        const spi::Bucket& target2) {
    return checkResult(_impl.split(source, target1, target2));
}

spi::Result ProviderErrorWrapper::join(const spi::Bucket& source1, const spi::Bucket& source2,
...
spi::Result ProviderErrorWrapper::removeEntry(const spi::Bucket& bucket, spi::Timestamp ts) {
...
void ProviderErrorWrapper::removeAsync(const spi::Bucket& bucket, std::vector<spi::IdAndTimestamp> ids,
                                       spi::OperationComplete::UP onComplete) {
    onComplete->addResultHandler(this);
    _impl.removeAsync(bucket, std::move(ids), std::move(onComplete));
}

void ProviderErrorWrapper::removeByGidAsync(const spi::Bucket& bucket, std::vector<spi::DocTypeGidAndTimestamp> ids,
                                            std::unique_ptr<spi::OperationComplete> onComplete) {
    onComplete->addResultHandler(this);
    _impl.removeByGidAsync(bucket, std::move(ids), std::move(onComplete));
}

void ProviderErrorWrapper::removeIfFoundAsync(const spi::Bucket& bucket, spi::Timestamp ts,
...
void ProviderErrorWrapper::updateAsync(const spi::Bucket& bucket, spi::Timestamp ts, spi::DocumentUpdateSP upd,
                                       spi::OperationComplete::UP onComplete) {
    onComplete->addResultHandler(this);
