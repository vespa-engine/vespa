...
public:
    std::ostream& ost;
    explicit StatEntryProcessor(std::ostream& o) : ost(o) {};
    ...
    void process(std::unique_ptr<spi::DocEntry> entry) override {
        ...
        if (e.getDocument() != nullptr) {
            ost << "Doc(" << e.getDocument()->getId() << ")"
                << ", " << e.getDocument()->getId().getGlobalId().toString() << ", size: " << e.getSize();
        } else if (e.getDocumentId() != nullptr) {
            ost << *e.getDocumentId() << ", " << e.getDocumentId()->getGlobalId().toString();
        } else {
            ost << "metadata only";
        }
        if (e.isRemove()) {
...
MessageTracker::UP ProcessAllHandler::handleStatBucket(api::StatBucketCommand& cmd,
    ...
    BucketProcessor::iterateAll(_spi, bucket, cmd.getDocumentSelection(), std::make_shared<document::DocIdOnly>(),
