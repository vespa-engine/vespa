...
uint16_t BucketOwnershipNotifier::getOwnerDistributorForBucket(const document::Bucket& bucket) const {
    try {
        ...
        LOGBP(debug,
              "Too few bucket bits used for %s to be assigned to a distributor."
...
bool BucketOwnershipNotifier::distributorOwns(uint16_t distributor, const document::Bucket& bucket) const {
    return (distributor == getOwnerDistributorForBucket(bucket));
}

void BucketOwnershipNotifier::sendNotifyBucketToDistributor(uint16_t distributorIndex, const document::Bucket& bucket,
                                                            const api::BucketInfo& infoToSend) {
    if (!infoToSend.valid()) {
        LOG(error, "Trying to send invalid bucket info to distributor %u: %s. %s", distributorIndex,
            infoToSend.toString().c_str(), vespalib::getStackTrace(0).c_str());
        return;
    }
    ...
    notifyCmd->setAddress(
        api::StorageMessageAddress::create(cluster_np, lib::NodeType::DISTRIBUTOR, distributorIndex));
    notifyCmd->setSourceIndex(_component.getIndex());
    LOG(debug, "Sending notify to distributor %u: %s", distributorIndex, notifyCmd->toString().c_str());
    _sender.sendCommand(notifyCmd);
}

void BucketOwnershipNotifier::logNotification(const document::Bucket& bucket, uint16_t sourceIndex,
    ...
    LOG(debug,
        ...
        bucket.getBucketId().toString().c_str(), currentOwnerIndex, sourceIndex, newInfo.toString().c_str());
}

void BucketOwnershipNotifier::notifyIfOwnershipChanged(const document::Bucket& bucket, uint16_t sourceIndex,
                                                       const api::BucketInfo& infoToSend) {
    ...
    if (sourceIndex == FAILED_TO_RESOLVE) {
        LOG(debug,
            ...
            sourceIndex, vespalib::getStackTrace(0).c_str());
        return;
    }
...
void BucketOwnershipNotifier::sendNotifyBucketToCurrentOwner(const document::Bucket& bucket,
                                                             const api::BucketInfo&  infoToSend) {
    uint16_t distributor(getOwnerDistributorForBucket(bucket));
...
void NotificationGuard::notifyIfOwnershipChanged(const document::Bucket& bucket, uint16_t sourceIndex,
                                                 const api::BucketInfo& infoToSend) {
    _bucketsToCheck.push_back(BucketToCheck(bucket, sourceIndex, infoToSend));
}

void NotificationGuard::notifyAlways(const document::Bucket& bucket, const api::BucketInfo& infoToSend) {
