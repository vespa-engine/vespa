...
void TestAndSetHelper::resolveDocumentType(const document::DocumentTypeRepo& documentTypeRepo) {
    ...
    if (_docTypePtr == nullptr) {
        throw TestAndSetException(
            api::ReturnCode(api::ReturnCode::ILLEGAL_PARAMETERS, "Document type does not exist"));
    }
}
...
void TestAndSetHelper::parseDocumentSelection(const document::DocumentTypeRepo& documentTypeRepo,
    ...
    try {
        ...
        throw TestAndSetException(api::ReturnCode(api::ReturnCode::ILLEGAL_PARAMETERS,
...
TestAndSetHelper::Result TestAndSetHelper::fetch_and_match_selection(spi::Context& context) {
    ...
    return {result.getTimestamp(),
            result.is_tombstone() ? Result::ConditionOutcome::IsTombstone : Result::ConditionOutcome::DocNotFound};
}

api::ReturnCode TestAndSetHelper::to_api_return_code(const Result& result) const {
    ...
    case Result::ConditionOutcome::DocNotFound:
        if (!_missingDocumentImpliesMatch) {
            return {api::ReturnCode::TEST_AND_SET_CONDITION_FAILED,
                    vespalib::make_string("Document does not exist nodeIndex=%d bucket=%" PRIx64, _env._nodeIndex,
                                          _bucket.getBucketId().getRawId())};
        }
...
TestAndSetHelper::Result TestAndSetHelper::fetch_and_match_raw(spi::Context& context) {
...
api::ReturnCode TestAndSetHelper::retrieveAndMatch(spi::Context& context) {
