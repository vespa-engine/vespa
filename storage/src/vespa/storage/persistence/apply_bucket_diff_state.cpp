...
ApplyBucketDiffState::ApplyBucketDiffState(const MergeBucketInfoSyncer& merge_bucket_info_syncer,
                                           MergeHandlerMetrics& merge_handler_metrics, const framework::Clock& clock,
                                           const spi::Bucket& bucket, RetainGuard&& retain_guard)
...
void ApplyBucketDiffState::on_entry_complete(std::unique_ptr<Result> result, const document::DocumentId& doc_id,
                                             const char* op) {
    if (result->hasError() && !_failed_flag.test_and_set()) {
        vespalib::asciistream ss;
        ss << "Failed " << op << " for " << doc_id.toString() << " in " << _bucket << ": " << result->toString();
        _fail_message = ss.view();
    }
}

void ApplyBucketDiffState::mark_stale_bucket_info() {
    _stale_bucket_info = true;
}

void ApplyBucketDiffState::sync_bucket_info() {
...
std::future<std::string> ApplyBucketDiffState::get_future() {
    _promise = std::promise<std::string>();
    return _promise.value().get_future();
}

void ApplyBucketDiffState::set_delayed_reply(std::unique_ptr<MessageTracker>&&    tracker,
                                             std::shared_ptr<api::StorageReply>&& delayed_reply) {
    _tracker = std::move(tracker);
    _delayed_reply = std::move(delayed_reply);
}

void ApplyBucketDiffState::set_delayed_reply(std::unique_ptr<MessageTracker>&& tracker, MessageSender& sender,
                                             FileStorThreadMetrics::Op*           op_metrics,
                                             const framework::MilliSecTimer&      op_start_time,
                                             std::shared_ptr<api::StorageReply>&& delayed_reply) {
    _tracker = std::move(tracker);
    _sender = &sender;
...
void ApplyBucketDiffState::set_tracker(std::unique_ptr<MessageTracker>&& tracker) {
    _tracker = std::move(tracker);
}

void ApplyBucketDiffState::set_merge_start_time(const framework::MilliSecTimer& merge_start_time) {
    _merge_start_time = merge_start_time;
}

std::shared_ptr<ApplyBucketDiffState>
ApplyBucketDiffState::create(const MergeBucketInfoSyncer& merge_bucket_info_syncer,
                             MergeHandlerMetrics& merge_handler_metrics, const framework::Clock& clock,
                             const spi::Bucket& bucket, RetainGuard&& retain_guard) {
    std::unique_ptr<ApplyBucketDiffState> state(new ApplyBucketDiffState(
        merge_bucket_info_syncer, merge_handler_metrics, clock, bucket, std::move(retain_guard)));
