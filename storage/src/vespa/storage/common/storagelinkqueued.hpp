...
template <typename Message> void StorageLinkQueued::Dispatcher<Message>::shutdown() {
    if (_thread) {
        _thread->interrupt();
        {
...
StorageLinkQueued::Dispatcher<Message>::Dispatcher(StorageLinkQueued& parent, unsigned int maxQueueSize,
    ...
    name << "Queued storage " << (_replyDispatcher ? "up" : "down") << "link - " << _parent.getName();
    _component = std::make_unique<framework::Component>(parent.getComponentRegister(), name.str());
}

template <typename Message> StorageLinkQueued::Dispatcher<Message>::~Dispatcher() {
    shutdown();
}
...
template <typename Message> void StorageLinkQueued::Dispatcher<Message>::run(framework::ThreadHandle& h) {
    while (!h.interrupted()) {
        ...
        try {
            ...
            _parent.logError(vespalib::make_string("When running command %s, caught exception %s. "
                                                   "Discarding message",
                                                   message->toString().c_str(), e.what())
                                 .c_str());
        }
...
template <typename Message> void StorageLinkQueued::Dispatcher<Message>::flush() {
