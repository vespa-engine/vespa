...
StatusMetricConsumer::StatusMetricConsumer(StorageComponentRegister& compReg, metrics::MetricManager& manager,
...
std::string StatusMetricConsumer::getReportContentType(const framework::HttpUrlPath& path) const {
...
bool StatusMetricConsumer::reportStatus(std::ostream& out, const framework::HttpUrlPath& path) const {
    ...
    if (path.hasAttribute("interval")) {
        ...
        if (intervalS == -2) {
            ...
            generated = std::make_unique<metrics::MetricSnapshot>("Total metrics from start until current time", 0s,
