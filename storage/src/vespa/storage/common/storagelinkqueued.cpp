...
StorageLinkQueued::StorageLinkQueued(const std::string& name, framework::ComponentRegister& cr)
    : StorageLink(name), _compReg(cr), _replyDispatcher(*this), _closeState(0) {
}

StorageLinkQueued::~StorageLinkQueued() {
    if (_closeState != 7) {
        LOG(error,
            "Link %s has closing state %u at destruction. Has likely "
...
void StorageLinkQueued::dispatchUp(const std::shared_ptr<api::StorageMessage>& msg) {
