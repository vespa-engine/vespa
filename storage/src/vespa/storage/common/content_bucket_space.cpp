...
ContentBucketSpace::ContentBucketSpace(document::BucketSpace bucketSpace, const ContentBucketDbOptions& db_opts)
...
std::shared_ptr<const ClusterStateAndDistribution> ContentBucketSpace::state_and_distribution() const noexcept {
    std::lock_guard guard(_lock);
    return _state_and_distribution;
}

bool ContentBucketSpace::getNodeUpInLastNodeStateSeenByProvider() const {
    std::lock_guard guard(_lock);
    return _nodeUpInLastNodeStateSeenByProvider;
}

void ContentBucketSpace::setNodeUpInLastNodeStateSeenByProvider(bool nodeUpInLastNodeStateSeenByProvider) {
    std::lock_guard guard(_lock);
    _nodeUpInLastNodeStateSeenByProvider = nodeUpInLastNodeStateSeenByProvider;
}

bool ContentBucketSpace::getNodeMaintenanceInLastNodeStateSeenByProvider() const {
    std::lock_guard guard(_lock);
    return _nodeMaintenanceInLastNodeStateSeenByProvider;
}

void ContentBucketSpace::setNodeMaintenanceInLastNodeStateSeenByProvider(
