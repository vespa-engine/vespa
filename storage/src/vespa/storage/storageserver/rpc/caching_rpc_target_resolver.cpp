...
std::string CachingRpcTargetResolver::address_to_slobrok_id(const api::StorageMessageAddress& address) {
...
std::shared_ptr<RpcTarget> CachingRpcTargetResolver::lookup_target(const api::StorageMessageAddress& address,
                                                                   uint64_t bucket_id, uint32_t curr_slobrok_gen) {
...
CachingRpcTargetResolver::consider_update_target_pool(const api::StorageMessageAddress& address, uint64_t bucket_id,
                                                      const std::string& connection_spec, uint32_t curr_slobrok_gen,
    ...
    if (itr != _targets.end()) {
        ...
        if (target->is_valid() && (pool->spec() == connection_spec)) {
            LOG(debug, "Updating existing mapping '%s' -> '%s' (gen %u) to gen %u", address.toString().c_str(),
                connection_spec.c_str(), pool->slobrok_gen(), curr_slobrok_gen);
...
std::shared_ptr<RpcTarget>
CachingRpcTargetResolver::insert_new_target_mapping(const api::StorageMessageAddress& address, uint64_t bucket_id,
                                                    const std::string& connection_spec, uint32_t curr_slobrok_gen,
                                                    [[maybe_unused]] const UniqueLock& targets_lock) {
    ...
    LOG(debug, "Added mapping '%s' -> '%s' at gen %u", address.toString().c_str(), connection_spec.c_str(),
        curr_slobrok_gen);
    return pool->get_target(bucket_id);
}

std::shared_ptr<RpcTarget> CachingRpcTargetResolver::resolve_rpc_target(const api::StorageMessageAddress& address,
                                                                        uint64_t                          bucket_id) {
