...
class CachingRpcTargetResolver {
    ...
    using TargetHashMap =
        vespalib::hash_map<api::StorageMessageAddress, std::shared_ptr<RpcTargetPool>, AddressInternalHasher>;
    ...
    std::shared_ptr<RpcTarget> lookup_target(const api::StorageMessageAddress& address, uint64_t bucket_id,
                                             uint32_t curr_slobrok_gen);
    std::shared_ptr<RpcTarget> consider_update_target_pool(const api::StorageMessageAddress& address,
                                                           uint64_t bucket_id, const std::string& connection_spec,
                                                           uint32_t curr_slobrok_gen, const UniqueLock& targets_lock);
...
public:
    CachingRpcTargetResolver(const slobrok::api::IMirrorAPI& slobrok_mirror, const RpcTargetFactory& target_factory,
    ...
    std::shared_ptr<RpcTarget> resolve_rpc_target(const api::StorageMessageAddress& address, uint64_t bucket_id);
