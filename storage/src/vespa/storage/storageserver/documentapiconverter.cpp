...
DocumentApiConverter::DocumentApiConverter(std::shared_ptr<const BucketResolver> bucketResolver)
    : _priConverter(std::make_unique<PriorityConverter>()), _bucketResolver(std::move(bucketResolver)) {
}

DocumentApiConverter::~DocumentApiConverter() = default;

std::unique_ptr<api::StorageCommand> DocumentApiConverter::toStorageAPI(documentapi::DocumentMessage& fromMsg) {
    ...
    case DocumentProtocol::MESSAGE_REMOVEDOCUMENT: {
        ...
        auto to = std::make_unique<api::RemoveCommand>(bucketResolver()->bucketFromId(from.getDocumentId()),
                                                       from.getDocumentId(), 0);
        to->setCondition(from.getCondition());
        toMsg = std::move(to);
        break;
    }
    case DocumentProtocol::MESSAGE_GETDOCUMENT: {
        ...
        auto  get_cmd = std::make_unique<api::GetCommand>(bucketResolver()->bucketFromId(from.getDocumentId()),
                                                          from.getDocumentId(), from.getFieldSet());
    ...
    case DocumentProtocol::MESSAGE_CREATEVISITOR: {
        ...
        auto  to = std::make_unique<api::CreateVisitorCommand>(
            bucketResolver()->bucketSpaceFromName(from.getBucketSpace()), from.getLibraryName(), from.getInstanceId(),
    ...
    if (toMsg) {
        ...
        vespalib::duration cappedTimeout =
            (fromMsg.getTimeRemaining() < 1ms * INT_MAX) ? fromMsg.getTimeRemaining() : 1ms * INT_MAX;
        toMsg->setTimeout(cappedTimeout);
        toMsg->setPriority(_priConverter->toStoragePriority(fromMsg.getPriority()));

        LOG(spam, "Converted command %s, mapped priority %d to %d", toMsg->toString().c_str(), fromMsg.getPriority(),
            toMsg->getPriority());
    }
    return toMsg;
}

std::unique_ptr<api::StorageReply> DocumentApiConverter::toStorageAPI(documentapi::DocumentReply& fromReply,
...
std::unique_ptr<mbus::Message> DocumentApiConverter::toDocumentAPI(api::StorageCommand& fromMsg) {
    ...
    case api::MessageType::QUERYRESULT_ID: {
        ...
        toMsg = std::make_unique<documentapi::QueryResultMessage>(std::move(from.getSearchResult()),
                                                                  from.getDocumentSummary());
        break;
    }
    ...
    case api::MessageType::VISITOR_CREATE_ID: {
        ...
        auto  to = std::make_unique<documentapi::CreateVisitorMessage>(
            from.getLibraryName(), from.getInstanceId(), from.getControlDestination(), from.getDataDestination());
    ...
    case api::MessageType::STATBUCKET_ID: {
        ...
        auto  statMsg = std::make_unique<documentapi::StatBucketMessage>(from.getBucket().getBucketId(),
                                                                         from.getDocumentSelection());
...
void DocumentApiConverter::transferReplyState(api::StorageReply& fromMsg, mbus::Reply& toMsg) {
    ...
    if (fromMsg.getResult().failed()) {
        ...
        LOG(debug, "Converted storageapi error code %d to %s", fromMsg.getResult().getResult(),
