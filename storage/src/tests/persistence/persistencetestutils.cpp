...
template <typename T> struct ConfigReader : public T::Subscriber {
    ...
    ConfigReader(const std::string& configId) { T::subscribe(configId, *this); }
...
PersistenceTestEnvironment::PersistenceTestEnvironment()
    ...
    _diskEnv = std::make_unique<PersistenceUtil>(_component, *_handler, *_metrics.threads[0],
...
PersistenceTestUtils::MockBucketLocks::MockBucketLocks() : _mutex(), _cv(), _locked_buckets() {
}
...
void PersistenceTestUtils::MockBucketLocks::lock(document::Bucket bucket) {
    std::unique_lock<std::mutex> guard(_mutex);
...
PersistenceTestUtils::PersistenceTestUtils()
    : _env(std::make_unique<PersistenceTestEnvironment>()),
      ...
      _persistenceHandler() {
    setupExecutor(1);
    _persistenceHandler = std::make_unique<PersistenceHandler>(
        *_sequenceTaskExecutor, _env->_component, MERGE_CHUNK_SIZE, ENABLE_MULTIBIT_SPLIT, getPersistenceProvider(),
        getEnv()._fileStorHandler, _bucketOwnershipNotifier, getEnv()._metrics);
}
PersistenceTestUtils::~PersistenceTestUtils() = default;

std::string PersistenceTestUtils::dumpBucket(const document::BucketId& bid) {
    return dynamic_cast<spi::dummy::DummyPersistence&>(_env->_node.getPersistenceProvider())
        .dumpBucket(makeSpiBucket(bid));
}

VESPA_THREAD_STACK_TAG(test_executor)

void PersistenceTestUtils::setupExecutor(uint32_t numThreads) {
    _sequenceTaskExecutor = vespalib::SequencedTaskExecutor::create(test_executor, numThreads, 1000, true,
                                                                    vespalib::Executor::OptimizeFor::ADAPTIVE);
}

StorBucketDatabase::WrappedEntry PersistenceTestUtils::getBucket(const document::BucketId& id) {
    return _env->_node.getStorageBucketDatabase().get(id, "foo");
}

StorBucketDatabase::WrappedEntry PersistenceTestUtils::createBucket(const document::BucketId& id) {
    return _env->_node.getStorageBucketDatabase().get(id, "foo", StorBucketDatabase::CREATE_IF_NONEXISTING);
}
...
std::string PersistenceTestUtils::getBucketStatus(const document::BucketId& id) {
    std::ostringstream               ost;
    StorBucketDatabase::WrappedEntry entry(_env->_node.getStorageBucketDatabase().get(id, "foo"));
...
document::Document::SP PersistenceTestUtils::doPutOnDisk(uint32_t location, spi::Timestamp timestamp,
...
bool PersistenceTestUtils::doRemoveOnDisk(const document::BucketId& bucketId, const document::DocumentId& docId,
                                          spi::Timestamp timestamp, bool persistRemove) {
    if (persistRemove) {
...
bool PersistenceTestUtils::doUnrevertableRemoveOnDisk(const document::BucketId&   bucketId,
                                                      const document::DocumentId& docId, spi::Timestamp timestamp) {
    spi::RemoveResult result = getPersistenceProvider().remove(makeSpiBucket(bucketId), timestamp, docId);
    return result.wasFound();
}

spi::GetResult PersistenceTestUtils::doGetOnDisk(const document::BucketId&   bucketId,
...
PersistenceTestUtils::createBodyUpdate(const document::DocumentId&           docId,
                                       std::unique_ptr<document::FieldValue> updateValue) {
    ...
    update->addUpdate(document::FieldUpdate(docType->getField("content"))
                          .addUpdate(std::make_unique<document::AssignValueUpdate>(std::move(updateValue))));
    return update;
}

document::DocumentUpdate::SP
PersistenceTestUtils::createHeaderUpdate(const document::DocumentId&           docId,
                                         std::unique_ptr<document::FieldValue> updateValue) {
    const DocumentType* docType(getTypeRepo()->getDocumentType("testdoctype1"));
    auto                update = std::make_shared<document::DocumentUpdate>(*getTypeRepo(), *docType, docId);
    update->addUpdate(document::FieldUpdate(docType->getField("headerval"))
                          .addUpdate(std::make_unique<document::AssignValueUpdate>(std::move(updateValue))));
    return update;
}

void PersistenceTestUtils::doPut(const document::Document::SP& doc, spi::Timestamp time, uint16_t usedBits) {
...
void PersistenceTestUtils::doPut(const document::Document::SP& doc, document::BucketId bid, spi::Timestamp time) {
    spi::Bucket b(makeSpiBucket(bid));
    getPersistenceProvider().createBucket(b);
    getPersistenceProvider().put(b, time, doc);
}

spi::UpdateResult PersistenceTestUtils::doUpdate(document::BucketId bid, const document::DocumentUpdate::SP& update,
                                                 spi::Timestamp time) {
    spi::Context context(spi::Priority(0), spi::Trace::TraceLevel(0));
    return getPersistenceProvider().update(makeSpiBucket(bid), time, update);
}

void PersistenceTestUtils::doRemove(const document::DocumentId& id, spi::Timestamp time, bool unrevertableRemove,
                                    uint16_t usedBits) {
    document::BucketId bucket(_env->_component.getBucketIdFactory().getBucketId(id));
    bucket.setUsedBits(usedBits);
    spi::Context context(spi::Priority(0), spi::Trace::TraceLevel(0));
    if (unrevertableRemove) {
        ...
        if (!result.wasFound()) {
            throw vespalib::IllegalStateException("Attempted to remove non-existing doc " + id.toString(),
                                                  VESPA_STRLOC);
        }
    }
}

void PersistenceTestUtils::clearBody(document::Document& doc) {
...
document::Document::UP PersistenceTestUtils::createRandomDocumentAtLocation(uint64_t location, uint32_t seed,
                                                                            uint32_t minDocSize,
                                                                            uint32_t maxDocSize) {
    return _env->_testDocMan.createRandomDocumentAtLocation(location, seed, minDocSize, maxDocSize);
}

void PersistenceTestUtils::createTestBucket(const document::Bucket& bucket) {
    ...
    for (uint32_t useHeaderOnly = 0; useHeaderOnly < 2; ++useHeaderOnly) {
        ...
        for (uint32_t optype = 0; optype < 4; ++optype) {
            for (uint32_t i = 0; i < opsPerType; ++i) {
                ...
                document::Document::SP doc(createRandomDocumentAtLocation(location, seed, minDocSize, maxDocSize));
                if (headerOnly) {
                    clearBody(*doc);
                }
