...
public:
    DummyMergeBucketInfoSyncer(uint32_t& sync_count) : MergeBucketInfoSyncer(), _sync_count(sync_count), _fail() {}
...
void make_result(ApplyBucketDiffState& state, spi::Result& spi_result, const DocumentId& doc_id) {
...
public:
    ...
    std::shared_ptr<ApplyBucketDiffState> make_state() {
        return ApplyBucketDiffState::create(syncer, merge_handler_metrics, clock, spi::Bucket(dummy_document_bucket),
                                            RetainGuard(monitored_ref_count));
    }

    MessageTracker::UP create_tracker(std::shared_ptr<api::StorageMessage> cmd, document::Bucket bucket) {
        return MessageTracker::createForTesting(framework::MilliSecTimer(clock), getEnv(), message_sender,
                                                NoBucketLock::make(bucket), std::move(cmd));
    }
};
...
public:
    ...
    ApplyBucketDiffStateTest() : ApplyBucketDiffStateTestBase(), state(make_state()) {}

    void reset() { state = make_state(); }
...
void ApplyBucketDiffStateTest::test_delayed_reply(bool failed, bool async_failed, bool chained_reply) {
    auto                               cmd = std::make_shared<api::MergeBucketCommand>(dummy_document_bucket,
                                                                                       std::vector<api::MergeBucketCommand::Node>{}, 0);
    ...
    if (chained_reply) {
        state->set_delayed_reply(std::move(tracker), message_sender, &op_metrics, framework::MilliSecTimer(clock),
                                 std::move(reply));
    } else {
    ...
    EXPECT_NE(
        failed || async_failed,
        std::dynamic_pointer_cast<api::MergeBucketReply>(message_sender.replies.front())->getResult().success());
}
