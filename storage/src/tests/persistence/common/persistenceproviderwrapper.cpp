...
const char* includedVersionsToString(spi::IncludedVersions versions) {
    switch (versions) {
...
PersistenceProviderWrapper::PersistenceProviderWrapper(spi::PersistenceProvider& spi)
    : _spi(spi), _result(spi::Result(spi::Result::ErrorType::NONE, "")), _lock(), _log(), _failureMask(0) {
}
PersistenceProviderWrapper::~PersistenceProviderWrapper() = default;

std::string PersistenceProviderWrapper::toString() const {
...
spi::BucketIdListResult PersistenceProviderWrapper::listBuckets(BucketSpace bucketSpace) const {
...
void PersistenceProviderWrapper::createBucketAsync(const spi::Bucket&         bucket,
...
spi::BucketInfoResult PersistenceProviderWrapper::getBucketInfo(const spi::Bucket& bucket) const {
    LOG_SPI("getBucketInfo(" << bucket << ")");
    CHECK_ERROR(spi::BucketInfoResult, FAIL_BUCKET_INFO);
    return _spi.getBucketInfo(bucket);
}

void PersistenceProviderWrapper::putAsync(const spi::Bucket& bucket, spi::Timestamp timestamp,
                                          document::Document::SP doc, spi::OperationComplete::UP onComplete) {
...
void PersistenceProviderWrapper::removeAsync(const spi::Bucket& bucket, std::vector<spi::IdAndTimestamp> ids,
...
void PersistenceProviderWrapper::removeByGidAsync(const spi::Bucket&                       bucket,
                                                  std::vector<spi::DocTypeGidAndTimestamp> ids,
                                                  std::unique_ptr<spi::OperationComplete>  onComplete) {
    for (const auto& dt_gid_ts : ids) {
        LOG_SPI("removeByGid(" << bucket << ", " << dt_gid_ts.timestamp << ", " << dt_gid_ts.doc_type << ", "
                               << dt_gid_ts.gid << ")");
    }
    CHECK_ERROR_ASYNC(spi::RemoveResult, FAIL_REMOVE, onComplete);
...
void PersistenceProviderWrapper::removeIfFoundAsync(const spi::Bucket& bucket, spi::Timestamp timestamp,
...
void PersistenceProviderWrapper::updateAsync(const spi::Bucket& bucket, spi::Timestamp timestamp,
...
spi::GetResult PersistenceProviderWrapper::get(const spi::Bucket& bucket, const document::FieldSet& fieldSet,
...
spi::CreateIteratorResult PersistenceProviderWrapper::createIterator(const spi::Bucket& bucket, FieldSetSP fields,
                                                                     const spi::Selection& sel,
                                                                     spi::IncludedVersions versions,
                                                                     spi::Context&         context) {
    // TODO: proper printing of FieldSet and Selection

    LOG_SPI("createIterator(" << bucket << ", " << includedVersionsToString(versions) << ")");
    CHECK_ERROR(spi::CreateIteratorResult, FAIL_CREATE_ITERATOR);
    return _spi.createIterator(bucket, fields, sel, versions, context);
}

spi::IterateResult PersistenceProviderWrapper::iterate(spi::IteratorId iterId, uint64_t maxByteSize) const {
...
spi::Result PersistenceProviderWrapper::destroyIterator(spi::IteratorId iterId) {
    LOG_SPI("destroyIterator(" << uint64_t(iterId) << ")");
    CHECK_ERROR(spi::Result, FAIL_DESTROY_ITERATOR);
    return _spi.destroyIterator(iterId);
}

void PersistenceProviderWrapper::deleteBucketAsync(const spi::Bucket&         bucket,
...
spi::Result PersistenceProviderWrapper::split(const spi::Bucket& source, const spi::Bucket& target1,
...
spi::Result PersistenceProviderWrapper::join(const spi::Bucket& source1, const spi::Bucket& source2,
...
spi::Result PersistenceProviderWrapper::removeEntry(const spi::Bucket& bucket, spi::Timestamp timestamp) {
    LOG_SPI("revert(" << bucket << ", " << timestamp << ")");
    CHECK_ERROR(spi::Result, FAIL_REVERT);
    return _spi.removeEntry(bucket, timestamp);
}

spi::Result PersistenceProviderWrapper::initialize() {
...
spi::Result PersistenceProviderWrapper::setClusterState(spi::PersistenceProvider::BucketSpace bucketSpace,
                                                        const spi::ClusterState&              state) {
    return _spi.setClusterState(bucketSpace, state);
}

void PersistenceProviderWrapper::setActiveStateAsync(const spi::Bucket& bucket, spi::BucketInfo::ActiveState state,
                                                     spi::OperationComplete::UP onComplete) {
