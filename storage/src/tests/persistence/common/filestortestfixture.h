...
public:
    ...
    api::ReturnCode::Result resultOf(const api::StorageReply& reply) const { return reply.getResult().getResult(); }
    ...
    void expectNoReplies(DummyStorageLink& link) { EXPECT_EQ(0, link.getNumReplies()); }

    template <typename ReplyType> void expectReply(DummyStorageLink& link, api::ReturnCode::Result result) {
        link.waitForMessages(1, 60 * 1000);
        api::StorageReply* reply(dynamic_cast<ReplyType*>(link.getReply(0).get()));
        if (reply == 0) {
            FAIL() << "got unexpected reply " << link.getReply(0)->toString(true);
        }
        EXPECT_EQ(result, reply->getResult().getResult());
    }

    template <typename ReplyType> void expectAbortedReply(DummyStorageLink& link) {
        expectReply<ReplyType>(link, api::ReturnCode::ABORTED);
    }

    template <typename ReplyType> void expectOkReply(DummyStorageLink& link) {
    ...
    public:
        ...
        void sendPut(const document::BucketId& bid, uint32_t docIdx, uint64_t timestamp);
