...
struct BucketOwnershipNotifierTest : public Test {
    ...
    BucketOwnershipNotifierTest() : _app(), _clusterState("distributor:2 storage:1") {}

    void SetUp() override;

    bool ownsBucket(uint16_t distributorIndex, const document::BucketId& bucket) const {
        uint16_t distributor = _app->getDistribution()->getIdealDistributorNode(_clusterState, bucket);
    ...
    void doTestNotification(const document::Bucket& bucket, const api::BucketInfo& info,
                            const std::string& wantedSend);
};

void BucketOwnershipNotifierTest::SetUp() {
...
void BucketOwnershipNotifierTest::doTestNotification(const document::Bucket& bucket, const api::BucketInfo& info,
                                                     const std::string& wantedSend) {
...
TEST_F(BucketOwnershipNotifierTest, send_notify_bucket_change_if_owning_distributor_changed) {
    ...
    wanted << "NotifyBucketChangeCommand(" << bucket.getBucketId() << ", " << info << ") => 1";
...
TEST_F(BucketOwnershipNotifierTest, guard_notify_always) {
    ...
    std::ostringstream      wanted;
    {
        ...
        wanted << "NotifyBucketChangeCommand(" << bucket1.getBucketId() << ", " << info << ") => 0,"
               << "NotifyBucketChangeCommand(" << bucket2.getBucketId() << ", " << info << ") => 1";
    }
