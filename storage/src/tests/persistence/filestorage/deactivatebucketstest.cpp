...
struct DeactivateBucketsTest : FileStorTestFixture {
    ...
    void SetUp() override {
        ...
        _node->getStateUpdater().setClusterState(std::make_shared<const lib::ClusterState>(up_state));

        createBucket(test_bucket());

        api::BucketInfo serviceLayerInfo(1, 2, 3, 4, 5, true, true);
        {
            StorBucketDatabase::WrappedEntry entry(_node->getStorageBucketDatabase().get(
                test_bucket(), "foo", StorBucketDatabase::CREATE_IF_NONEXISTING));
    ...
    static document::BucketId test_bucket() noexcept { return {8, 123}; }
...
TEST_F(DeactivateBucketsTest, bucket_space_subset_in_maintenance_deactivates_buckets) {
    ...
    _node->getStateUpdater().setClusterStateBundle(std::make_shared<const lib::ClusterStateBundle>(
        *state_of("storage:2 .1.s:m distributor:2"), std::move(derived)));
    EXPECT_FALSE(is_active(test_bucket()));
}
