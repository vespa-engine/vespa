...
struct MergeHandlerTest : PersistenceTestUtils {
    ...
    template <typename T> std::shared_ptr<T> fetchSingleMessage();
    ...
    std::shared_ptr<api::ApplyBucketDiffCommand> createDummyApplyDiff(int timestampOffset, uint16_t hasMask = 0x1,
                                                                      bool filled = true);

    std::shared_ptr<api::GetBucketDiffCommand> createDummyGetBucketDiff(int timestampOffset, uint16_t hasMask);
    ...
    class NoReplyHandlerInvoker : public HandlerInvoker {
    public:
        std::string afterInvoke(MergeHandlerTest&, MergeHandler&) override;
    };

    template <typename ExpectedMessage> std::string checkMessage(api::ReturnCode::Result expectedResult);

    class HandleMergeBucketInvoker : public NoReplyHandlerInvoker {
    public:
        void invoke(MergeHandlerTest&, MergeHandler&, spi::Context&) override;
    };

    class HandleGetBucketDiffInvoker : public NoReplyHandlerInvoker {
    public:
        void invoke(MergeHandlerTest&, MergeHandler&, spi::Context&) override;
    };

    class MultiPositionHandlerInvoker : public HandlerInvoker {
    public:
        MultiPositionHandlerInvoker() : _pos(FRONT) {}
    ...
    class HandleGetBucketDiffReplyInvoker : public HandlerInvoker {
    public:
    ...
    class HandleApplyBucketDiffInvoker : public NoReplyHandlerInvoker {
    public:
    ...
    class HandleApplyBucketDiffReplyInvoker : public MultiPositionHandlerInvoker {
    public:
    ...
    std::string doTestSPIException(MergeHandler& handler, PersistenceProviderWrapper& providerWrapper,
                                   HandlerInvoker& invoker, const ExpectedExceptionSpec& spec);

    MergeHandler createHandler(uint32_t maxChunkSize = 0x400000) {
        return {getEnv(),
                getPersistenceProvider(),
                getEnv()._component.cluster_context(),
                getEnv()._component.getClock(),
...
void MergeHandlerTest::SetUp() {
    ...
    getEnv()
        .getBucketDatabase(_bucket.getBucketSpace())
        .insert(_bucket.getBucketId(), bucketDBEntry, "mergetestsetup");
...
void MergeHandlerTest::setUpChain(ChainPos pos) {
    _nodes.clear();
    if (pos != FRONT) {
...
void MergeHandlerTest::testGetBucketDiffChain(bool midChain) {
...
void MergeHandlerTest::testApplyBucketDiffChain(bool midChain) {
    setUpChain(midChain ? MIDDLE : BACK);
    MergeHandler handler = createHandler();
...
template <typename T> std::shared_ptr<T> MergeHandlerTest::fetchSingleMessage() {
    std::vector<api::StorageMessage::SP>& msgs(messageKeeper()._msgs);
    if (msgs.empty()) {
        std::ostringstream oss;
        oss << "No messages available to fetch (expected type " << typeid(T).name() << ")";
        throw std::runtime_error(oss.str());
    }
    std::shared_ptr<T> ret(std::dynamic_pointer_cast<T>(messageKeeper()._msgs.back()));
    if (!ret) {
        std::ostringstream oss;
        oss << "Expected message of type " << typeid(T).name() << ", but got "
...
size_t getFilledCount(const std::vector<api::ApplyBucketDiffCommand::Entry>& diff) {
...
size_t getFilledDataSize(const std::vector<api::ApplyBucketDiffCommand::Entry>& diff) {
    size_t filledSize = 0;
    for (const auto& i : diff) {
...
TEST_F(MergeHandlerTest, chunked_apply_bucket_diff) {
    ...
    while (seen.size() != totalDiffs) {
        ...
        for (auto& i : diff) {
            ...
            if (!inserted.second) {
                FAIL() << "Diff for " << i << " has already been seen in another ApplyBucketDiff";
            }
        }
        ...
        auto applyBucketDiffReply = std::make_shared<api::ApplyBucketDiffReply>(*applyBucketDiffCmd);
        {
            handler.handleApplyBucketDiffReply(*applyBucketDiffReply, messageKeeper(),
                                               createTracker(applyBucketDiffReply, _bucket));
...
void MergeHandlerTest::fillDummyApplyDiff(std::vector<api::ApplyBucketDiffCommand::Entry>& diff) const {
...
std::shared_ptr<api::ApplyBucketDiffCommand> MergeHandlerTest::createDummyApplyDiff(int      timestampOffset,
                                                                                    uint16_t hasMask, bool filled) {
    ...
    }
    {
        ...
        e._entry._hasMask = hasMask;
        e._entry._flags = MergeHandler::IN_USE | MergeHandler::DELETED | MergeHandler::DELETED_IN_PLACE;
        applyDiff.push_back(e);
    }
...
std::shared_ptr<api::GetBucketDiffCommand> MergeHandlerTest::createDummyGetBucketDiff(int      timestampOffset,
                                                                                      uint16_t hasMask) {
    ...
    }
    {
        ...
        e._hasMask = hasMask;
        e._flags = MergeHandler::IN_USE | MergeHandler::DELETED | MergeHandler::DELETED_IN_PLACE;
        diff.push_back(e);
    }
...
TEST_F(MergeHandlerTest, bucket_not_found_in_db) {
    ...
    auto cmd = std::make_shared<api::MergeBucketCommand>(makeDocumentBucket(document::BucketId(16, 6789)), _nodes,
                                                         _maxTimestamp);
    MessageTracker::UP tracker = handler.handleMergeBucket(*cmd, createTracker(cmd, _bucket));
...
TEST_F(MergeHandlerTest, entry_removed_after_get_bucket_diff) {
    ...
    auto applyBucketDiffReply =
        std::dynamic_pointer_cast<api::ApplyBucketDiffReply>(std::move(*tracker).stealReplySP());
...
void MergeHandlerTest::convert_delayed_error_to_exception(MergeHandler& handler) {
...
std::string MergeHandlerTest::doTestSPIException(MergeHandler& handler, PersistenceProviderWrapper& providerWrapper,
                                                 HandlerInvoker& invoker, const ExpectedExceptionSpec& spec) {
    ...
    try {
        ...
        if (std::string(e.what()).find(expectedSubstring) == std::string::npos) {
            return (std::string("Expected exception to contain substring '") + expectedSubstring +
                    "', but message was: " + e.what());
        }
    }
    if (fsHandler().isMerging(_bucket)) {
        return (std::string("After operation with expected exception '") + expectedSubstring +
                "', merge state was not cleared");
    }
    // Postcondition check.
...
std::string MergeHandlerTest::NoReplyHandlerInvoker::afterInvoke(MergeHandlerTest& test, MergeHandler& handler) {
    (void)handler;
    if (!test.messageKeeper()._msgs.empty()) {
        std::ostringstream ss;
        ss << "Expected 0 explicit replies, got " << test.messageKeeper()._msgs.size();
        return ss.str();
    }
    return "";
}
...
std::string MergeHandlerTest::checkMessage(api::ReturnCode::Result expectedResult) {
    try {
        std::shared_ptr<ExpectedMessage> msg(fetchSingleMessage<ExpectedMessage>());
...
void MergeHandlerTest::HandleMergeBucketInvoker::invoke(MergeHandlerTest& test, MergeHandler& handler,
                                                        spi::Context&) {
...
void MergeHandlerTest::HandleGetBucketDiffInvoker::invoke(MergeHandlerTest& test, MergeHandler& handler,
                                                          spi::Context&) {
...
void MergeHandlerTest::HandleApplyBucketDiffInvoker::invoke(MergeHandlerTest& test, MergeHandler& handler,
                                                            spi::Context&) {
    ++_counter;
...
void MergeHandlerTest::HandleGetBucketDiffReplyInvoker::beforeInvoke(MergeHandlerTest& test, MergeHandler& handler,
                                                                     spi::Context&) {
    auto cmd = std::make_shared<api::MergeBucketCommand>(test._bucket, test._nodes, test._maxTimestamp);
    handler.handleMergeBucket(*cmd, test.createTracker(cmd, test._bucket));
...
void MergeHandlerTest::HandleGetBucketDiffReplyInvoker::invoke(MergeHandlerTest& test, MergeHandler& handler,
                                                               spi::Context&) {
    (void)test;
...
std::string MergeHandlerTest::HandleGetBucketDiffReplyInvoker::afterInvoke(MergeHandlerTest& test,
    ...
    return test.checkMessage<api::MergeBucketReply>(api::ReturnCode::INTERNAL_FAILURE);
}
...
void MergeHandlerTest::HandleApplyBucketDiffReplyInvoker::beforeInvoke(MergeHandlerTest& test, MergeHandler& handler,
                                                                       spi::Context&) {
    ++_counter;
    _stub.clear();
...
void MergeHandlerTest::HandleApplyBucketDiffReplyInvoker::convert_delayed_error_to_exception(MergeHandlerTest& test,
                                                                                             MergeHandler& handler) {
    handler.drain_async_writes();
...
void MergeHandlerTest::HandleApplyBucketDiffReplyInvoker::invoke(MergeHandlerTest& test, MergeHandler& handler,
                                                                 spi::Context&) {
    (void)test;
...
std::string MergeHandlerTest::HandleApplyBucketDiffReplyInvoker::afterInvoke(MergeHandlerTest& test,
                                                                             MergeHandler&     handler) {
    (void)handler;
    if (!_stub.commands.empty()) {
        return "Unexpected commands in reply stub";
    ...
    if (getChainPos() == FRONT) {
        ...
        return test.checkMessage<api::ApplyBucketDiffReply>(api::ReturnCode::INTERNAL_FAILURE);
    }
}
...
TEST_F(MergeHandlerTest, remove_put_on_existing_timestamp) {
    ...
    for (const auto& e : getBucketDiffCmd->getDiff()) {
        if (e._timestamp == ts) {
            EXPECT_EQ(uint16_t(MergeHandler::IN_USE | MergeHandler::DELETED), e._flags);
            foundTimestamp = true;
            break;
        }
    }
...
storage::api::GetBucketDiffCommand::Entry make_entry(uint64_t timestamp, uint16_t mask) {
...
void fill_entry(storage::api::ApplyBucketDiffCommand::Entry& e, const document::Document& doc,
...
struct EntryCheck {
    ...
    EntryCheck(uint64_t timestamp, uint16_t hasMask) : _timestamp(timestamp), _hasMask(hasMask) {}
...
TEST_F(MergeHandlerTest, multiple_versions_in_apply_diff_only_writes_newest_version) {
    ...
    auto tracker =
        handler.handleApplyBucketDiff(*apply_bucket_diff_cmd, createTracker(apply_bucket_diff_cmd, _bucket));
    ASSERT_FALSE(tracker);
    handler.drain_async_writes();
