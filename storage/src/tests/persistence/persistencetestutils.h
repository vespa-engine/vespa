...
public:
    ...
    public:
        MockBucketLock(document::Bucket bucket, MockBucketLocks& locks) noexcept : _bucket(bucket), _locks(locks) {
            _locks.lock(bucket);
        }
    ...
    struct ReplySender : public MessageSender {
        void sendCommand(const std::shared_ptr<api::StorageCommand>&) override { abort(); }

        void sendReply(const std::shared_ptr<api::StorageReply>& ptr) override { queue.enqueue(std::move(ptr)); }
    ...
    std::shared_ptr<const document::DocumentTypeRepo> getTypeRepo() {
        return _env->_component.getTypeRepo()->documentTypeRepo;
    }
    ...
    MessageTracker::UP createTracker(api::StorageMessage::SP cmd, document::Bucket bucket) {
    ...
    MessageTracker::UP createLockedTracker(api::StorageMessage::SP cmd, document::Bucket bucket) {
        return MessageTracker::createForTesting(framework::MilliSecTimer(getEnv()._component.getClock()), getEnv(),
                                                _replySender, MockBucketLock::make(bucket, _mock_bucket_locks),
                                                std::move(cmd));
    }

    template <typename T>
        requires std::is_base_of_v<api::StorageReply, T>
    [[nodiscard]] std::shared_ptr<T> fetch_single_reply(MessageTracker::UP tracker) {
    ...
    api::ReturnCode fetchResult(const MessageTracker::UP& tracker) {
        if (tracker) {
    ...
    */
    document::Document::SP doPutOnDisk(uint32_t location, spi::Timestamp timestamp, uint32_t minSize = 0,
                                       uint32_t maxSize = 128);

    document::Document::SP doPut(uint32_t location, spi::Timestamp timestamp, uint32_t minSize = 0,
                                 uint32_t maxSize = 128) {
    ...
    */
    bool doRemoveOnDisk(const document::BucketId& bid, const document::DocumentId& id, spi::Timestamp timestamp,
                        bool persistRemove);
    bool doRemove(const document::BucketId& bid, const document::DocumentId& id, spi::Timestamp timestamp,
                  bool persistRemove) {
        return doRemoveOnDisk(bid, id, timestamp, persistRemove);
    }

    bool doUnrevertableRemoveOnDisk(const document::BucketId& bid, const document::DocumentId& id,
                                    spi::Timestamp timestamp);

    bool doUnrevertableRemove(const document::BucketId& bid, const document::DocumentId& id,
                              spi::Timestamp timestamp) {
     ...
     */
    void doRemove(const document::DocumentId& id, spi::Timestamp, bool unrevertableRemove = false,
                  uint16_t usedBits = 16);
    ...
    std::shared_ptr<document::DocumentUpdate> createBodyUpdate(const document::DocumentId&           id,
                                                               std::unique_ptr<document::FieldValue> updateValue);

    std::shared_ptr<document::DocumentUpdate> createHeaderUpdate(const document::DocumentId&           id,
                                                                 std::unique_ptr<document::FieldValue> updateValue);
    ...
    spi::UpdateResult doUpdate(document::BucketId bid, const std::shared_ptr<document::DocumentUpdate>& update,
                               spi::Timestamp time);

    document::Document::UP createRandomDocumentAtLocation(uint64_t location, uint32_t seed, uint32_t minDocSize,
                                                          uint32_t maxDocSize);
