...
std::shared_ptr<const lib::ClusterStateBundle> TestNodeStateUpdater::getClusterStateBundle() const {
    return _clusterStateBundle;
}

void TestNodeStateUpdater::patch_distribution(std::shared_ptr<const lib::Distribution> distribution) {
    _clusterStateBundle =
        _clusterStateBundle->clone_with_new_distribution(lib::DistributionConfigBundle::of(std::move(distribution)));
}

void TestNodeStateUpdater::setClusterState(std::shared_ptr<const lib::ClusterState> c) {
    setClusterStateBundle(std::make_shared<const lib::ClusterStateBundle>(std::move(c)));
}

void TestNodeStateUpdater::setClusterStateBundle(std::shared_ptr<const lib::ClusterStateBundle> clusterStateBundle) {
