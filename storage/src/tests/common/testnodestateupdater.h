...
public:
    ...
    void immediately_send_get_node_state_replies() override { ++_explicit_node_state_reply_send_invocations; }

    void request_almost_immediate_node_state_replies() override { ++_requested_almost_immediate_node_state_replies; }

    void setCurrentNodeState(const lib::NodeState& state) { _current = std::make_shared<lib::NodeState>(state); }
