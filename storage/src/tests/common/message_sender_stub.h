...
struct MessageSenderStub : MessageSender {
    ...
    void sendCommand(const std::shared_ptr<api::StorageCommand>& cmd) override { commands.push_back(cmd); }

    void sendReply(const std::shared_ptr<api::StorageReply>& reply) override { replies.push_back(reply); }
    ...
    static std::string dumpMessage(const api::StorageMessage& msg, bool includeAddress, bool verbose);
};
