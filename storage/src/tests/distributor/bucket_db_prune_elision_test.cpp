...
TEST(IdempotentStateTransitionTest, differing_cluster_state_disallows_elision) {
    EXPECT_FALSE(
        db_pruning_may_be_elided(state_of("cluster:d distributor:3 storage:3"), state_of("distributor:3 storage:3")));
    EXPECT_FALSE(
        db_pruning_may_be_elided(state_of("distributor:3 storage:3"), state_of("cluster:d distributor:3 storage:3")));
}
...
TEST(IdempotentStateTransitionTest, differing_distribution_bit_count_disallows_elision) {
    ...
    EXPECT_FALSE(
        db_pruning_may_be_elided(state_of("bits:8 distributor:3 storage:3"), state_of("distributor:3 storage:3")));
    EXPECT_FALSE(
        db_pruning_may_be_elided(state_of("distributor:3 storage:3"), state_of("bits:8 distributor:3 storage:3")));
}

TEST(IdempotentStateTransitionTest, same_implicit_distribution_bit_count_allows_elision) {
    EXPECT_TRUE(
        db_pruning_may_be_elided(state_of("distributor:3 storage:3"), state_of("bits:16 distributor:3 storage:3")));
}

TEST(IdempotentStateTransitionTest, changed_distributor_node_count_disallows_elision) {
    EXPECT_FALSE(db_pruning_may_be_elided(state_of("distributor:3 storage:3"), state_of("distributor:4 storage:3")));
    EXPECT_FALSE(db_pruning_may_be_elided(state_of("distributor:4 storage:3"), state_of("distributor:3 storage:3")));
}

TEST(IdempotentStateTransitionTest, changed_distributor_node_state_disallows_elision) {
    EXPECT_FALSE(
        db_pruning_may_be_elided(state_of("distributor:3 .0.s:d storage:3"), state_of("distributor:3 storage:3")));

    EXPECT_FALSE(
        db_pruning_may_be_elided(state_of("distributor:3 storage:3"), state_of("distributor:3 .0.s:d storage:3")));
...
TEST(IdempotentStateTransitionTest, changed_storage_node_count_disallows_elision) {
    EXPECT_FALSE(db_pruning_may_be_elided(state_of("distributor:3 storage:3"), state_of("distributor:3 storage:4")));
    EXPECT_FALSE(db_pruning_may_be_elided(state_of("distributor:3 storage:4"), state_of("distributor:3 storage:3")));
}

TEST(IdempotentStateTransitionTest, changed_storage_node_state_disallows_elision) {
    EXPECT_FALSE(
        db_pruning_may_be_elided(state_of("distributor:3 storage:3 .0.s:d"), state_of("distributor:3 storage:3")));

    EXPECT_FALSE(
        db_pruning_may_be_elided(state_of("distributor:3 storage:3"), state_of("distributor:3 storage:3 .0.s:d")));
...
TEST(IdempotentStateTransitionTest, may_elide_for_transition_between_different_effective_storage_up_states) {
    EXPECT_TRUE(
        db_pruning_may_be_elided(state_of("distributor:3 storage:3 .0.s:i"), state_of("distributor:3 storage:3")));

    EXPECT_TRUE(
        db_pruning_may_be_elided(state_of("distributor:3 storage:3 .1.s:r"), state_of("distributor:3 storage:3")));
    EXPECT_TRUE(
        db_pruning_may_be_elided(state_of("distributor:3 storage:3"), state_of("distributor:3 storage:3 .2.s:r")));
}
