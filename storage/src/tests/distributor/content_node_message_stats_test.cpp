...
TEST(ContentNodeMessageStatsTest, errors_are_categorized_based_on_result_code) {
    ...
    s.observe_incoming_response_result(id,
...
void PrintTo(const ContentNodeMessageStatsTracker::NodeStats& stats, std::ostream* os) {
    std::vector<std::pair<uint16_t, ContentNodeMessageStats>> ordered_stats(stats.per_node.begin(),
                                                                            stats.per_node.end());
...
TEST_F(ContentNodeMessageStatsTrackerTest, stats_are_tracked_across_nodes) {
    ...
    _tracker.stats_for(5).observe_incoming_response_result(api::MessageType::PUT_REPLY_ID,
                                                           api::ReturnCode::NOT_CONNECTED);

    EXPECT_EQ(node_stats(),
              NodeStats({{0, Stats(1, 0, 0, 0, 0, 0)}, {2, Stats(1, 0, 0, 0, 0, 1)}, {5, Stats(1, 0, 1, 0, 0, 0)}}));
}
...
TEST_F(ContentNodeMessageStatsTrackerTest, stats_can_be_merged_across_nodes) {
    ...
    EXPECT_EQ(stats,
              NodeStats({{0, Stats(2, 0, 0, 0, 0, 0)}, {2, Stats(1, 0, 0, 0, 0, 0)}, {5, Stats(1, 0, 0, 0, 0, 0)}}));
}

TEST_F(ContentNodeMessageStatsTrackerTest, node_stats_subtraction_returns_per_node_delta) {
    _tracker.stats_for(0).observe_outgoing_request();
    _tracker.stats_for(0).observe_incoming_response_result(api::MessageType::PUT_REPLY_ID,
                                                           api::ReturnCode::NOT_CONNECTED);
    ...
    _tracker.stats_for(0).observe_outgoing_request();
    _tracker.stats_for(1).observe_outgoing_request();
    _tracker.stats_for(2).observe_incoming_response_result(api::MessageType::PUT_REPLY_ID,
                                                           api::ReturnCode::NOT_CONNECTED);
    const auto stats_after = node_stats();
    const auto delta = stats_after.sparse_subtracted(stats_before);

    EXPECT_EQ(delta,
              NodeStats({{0, Stats(1, 0, 0, 0, 0, 0)}, {1, Stats(1, 0, 0, 0, 0, 0)}, {2, Stats(0, 0, 1, 0, 0, 0)}}));
}
