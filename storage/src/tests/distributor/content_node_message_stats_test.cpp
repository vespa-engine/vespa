...
struct ContentNodeMessageStatsTrackerTest : Test {
    ...
    Tracker::NodeStats node_stats() const { return _tracker.node_stats(); }
};

TEST_F(ContentNodeMessageStatsTrackerTest, snapshot_is_intially_empty) {
    EXPECT_EQ(node_stats(), Tracker::NodeStats());
}

TEST_F(ContentNodeMessageStatsTrackerTest, counters_are_monotonic) {
    _tracker.stats_for(0).observe_outgoing_request();
    EXPECT_EQ(node_stats(), Tracker::NodeStats({{0, Stats(1, 0, 0, 0, 0, 0)}}));
    _tracker.stats_for(0).observe_outgoing_request();
    EXPECT_EQ(node_stats(), Tracker::NodeStats({{0, Stats(2, 0, 0, 0, 0, 0)}}));
    _tracker.stats_for(0).observe_cancelled();
    EXPECT_EQ(node_stats(), Tracker::NodeStats({{0, Stats(2, 0, 0, 0, 0, 1)}}));
    _tracker.stats_for(0).observe_cancelled();
    EXPECT_EQ(node_stats(), Tracker::NodeStats({{0, Stats(2, 0, 0, 0, 0, 2)}}));
}
