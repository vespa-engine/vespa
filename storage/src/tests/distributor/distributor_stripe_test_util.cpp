...
void DistributorStripeTestUtil::createLinks() {
...
void DistributorStripeTestUtil::setup_stripe(int redundancy, int nodeCount, const std::string& systemState,
                                             uint32_t earlyReturn, bool requirePrimaryToBeWritten) {
    setup_stripe(redundancy, nodeCount, lib::ClusterStateBundle(lib::ClusterState(systemState)), earlyReturn,
                 requirePrimaryToBeWritten);
}

void DistributorStripeTestUtil::setup_stripe(int redundancy, int node_count, const lib::ClusterStateBundle& state,
...
void DistributorStripeTestUtil::set_redundancy(uint32_t redundancy) {
    auto distribution =
        std::make_shared<lib::Distribution>(lib::Distribution::getDefaultDistributionConfig(redundancy, 100));
...
void DistributorStripeTestUtil::trigger_distribution_change(std::shared_ptr<lib::Distribution> distr) {
...
void DistributorStripeTestUtil::simulate_distribution_config_change(std::shared_ptr<lib::Distribution> new_config) {
    ...
    for (auto& space : _stripe->getBucketSpaceRepo()) {
        auto cur_state =
            current_cluster_state_bundle().getDerivedClusterState(space.first); // no change in state itself
        _stripe->remove_superfluous_buckets(space.first, *cur_state, true);
    }
}

std::shared_ptr<lib::Distribution> DistributorStripeTestUtil::make_default_distribution_config(uint16_t redundancy,
                                                                                               uint16_t node_count) {
    lib::Distribution::DistributionConfigBuilder config(
        lib::Distribution::getDefaultDistributionConfig(redundancy, node_count).get());
    config.redundancy = redundancy;
...
std::shared_ptr<DistributorConfiguration> DistributorStripeTestUtil::make_config() const {
    return std::make_shared<DistributorConfiguration>(_stripe->_component);
}

void DistributorStripeTestUtil::configure_stripe(std::shared_ptr<const DistributorConfiguration> config) {
    _stripe->update_total_distributor_config(config);
}

void DistributorStripeTestUtil::configure_stripe(const ConfigBuilder& builder) {
    auto config = make_config();
    config->configure(builder);
    configure_stripe(config);
}

void DistributorStripeTestUtil::receive_set_system_state_command(const std::string& state_str) {
...
void DistributorStripeTestUtil::handle_top_level_message(const std::shared_ptr<api::StorageMessage>& msg) {
    _stripe->handleMessage(msg);
}

void DistributorStripeTestUtil::simulate_set_pending_cluster_state(const std::string& state_str) {
...
void DistributorStripeTestUtil::clear_pending_cluster_state_bundle() {
    _stripe->clear_pending_cluster_state_bundle();
}

void DistributorStripeTestUtil::setTypeRepo(const std::shared_ptr<const document::DocumentTypeRepo>& repo) {
    _node->getComponentRegister().setDocumentTypeRepo(repo);
}

void DistributorStripeTestUtil::close() {
...
std::string DistributorStripeTestUtil::getNodes(document::BucketId id) {
...
std::string DistributorStripeTestUtil::getIdealStr(document::BucketId id, const lib::ClusterState& state) {
...
void DistributorStripeTestUtil::addIdealNodes(const lib::ClusterState& state, const document::BucketId& id) {
    BucketDatabase::Entry entry = getBucket(id);
...
void DistributorStripeTestUtil::addNodesToBucketDB(const document::Bucket& bucket, const std::string& nodeStr) {
    ...
    for (uint32_t i = 0; i < tokenizer.size(); ++i) {
        ...
        api::BucketInfo info(atoi(tok3[0].data()), atoi(tok3.size() > 1 ? tok3[1].data() : tok3[0].data()),
...
void DistributorStripeTestUtil::addNodesToBucketDB(const document::BucketId& id, const std::string& nodeStr) {
    addNodesToBucketDB(document::Bucket(makeBucketSpace(), id), nodeStr);
}

void DistributorStripeTestUtil::removeFromBucketDB(const document::BucketId& id) {
    getBucketDatabase().remove(id);
}

void DistributorStripeTestUtil::addIdealNodes(const document::BucketId& id) {
...
void DistributorStripeTestUtil::insertBucketInfo(document::BucketId id, uint16_t node, uint32_t checksum,
...
void DistributorStripeTestUtil::insertBucketInfo(document::BucketId id, uint16_t node, const api::BucketInfo& info,
...
std::string DistributorStripeTestUtil::dumpBucket(const document::BucketId& bid) {
    return getBucketDatabase().get(bid).toString();
}

void DistributorStripeTestUtil::sendReply(Operation& op, int idx, api::ReturnCode::Result result) {
    if (idx == -1) {
...
BucketDatabase::Entry DistributorStripeTestUtil::getBucket(const document::Bucket& bucket) const {
    return getBucketDatabase(bucket.getBucketSpace()).get(bucket.getBucketId());
}

BucketDatabase::Entry DistributorStripeTestUtil::getBucket(const document::BucketId& bId) const {
    return getBucketDatabase().get(bId);
}

void DistributorStripeTestUtil::disableBucketActivationInConfig(bool disable) {
    ConfigBuilder builder;
    builder.disableBucketActivation = disable;
    configure_stripe(builder);
}

StripeBucketDBUpdater& DistributorStripeTestUtil::getBucketDBUpdater() {
    return _stripe->bucket_db_updater();
}

IdealStateManager& DistributorStripeTestUtil::getIdealStateManager() {
    return _stripe->ideal_state_manager();
}

ExternalOperationHandler& DistributorStripeTestUtil::getExternalOperationHandler() {
    return _stripe->external_operation_handler();
}

const storage::distributor::DistributorNodeContext& DistributorStripeTestUtil::node_context() const {
    return _stripe->_component;
}

storage::distributor::DistributorStripeOperationContext& DistributorStripeTestUtil::operation_context() {
    return _stripe->_component;
}

const DocumentSelectionParser& DistributorStripeTestUtil::doc_selection_parser() const {
    return _stripe->_component;
}

DistributorMetricSet& DistributorStripeTestUtil::metrics() {
    return *_metrics;
}

bool DistributorStripeTestUtil::tick() {
    return _stripe->tick();
}

const DistributorConfiguration& DistributorStripeTestUtil::getConfig() {
    return _stripe->getConfig();
}

DistributorBucketSpace& DistributorStripeTestUtil::getDistributorBucketSpace() {
    return getBucketSpaceRepo().get(makeBucketSpace());
}

const DistributorBucketSpace& DistributorStripeTestUtil::getDistributorBucketSpace() const {
    return getBucketSpaceRepo().get(makeBucketSpace());
}

BucketDatabase& DistributorStripeTestUtil::getBucketDatabase() {
    return getDistributorBucketSpace().getBucketDatabase();
}

BucketDatabase& DistributorStripeTestUtil::getBucketDatabase(document::BucketSpace space) {
    return getBucketSpaceRepo().get(space).getBucketDatabase();
}

const BucketDatabase& DistributorStripeTestUtil::getBucketDatabase() const {
    return getBucketSpaceRepo().get(makeBucketSpace()).getBucketDatabase();
}

const BucketDatabase& DistributorStripeTestUtil::getBucketDatabase(document::BucketSpace space) const {
    return getBucketSpaceRepo().get(space).getBucketDatabase();
}

DistributorBucketSpaceRepo& DistributorStripeTestUtil::getBucketSpaceRepo() {
    return _stripe->getBucketSpaceRepo();
}

const DistributorBucketSpaceRepo& DistributorStripeTestUtil::getBucketSpaceRepo() const {
    return _stripe->getBucketSpaceRepo();
}

DistributorBucketSpaceRepo& DistributorStripeTestUtil::getReadOnlyBucketSpaceRepo() {
    return _stripe->getReadOnlyBucketSpaceRepo();
}

const DistributorBucketSpaceRepo& DistributorStripeTestUtil::getReadOnlyBucketSpaceRepo() const {
    return _stripe->getReadOnlyBucketSpaceRepo();
}

bool DistributorStripeTestUtil::stripe_is_in_recovery_mode() const noexcept {
    return _stripe->isInRecoveryMode();
}

const lib::ClusterStateBundle& DistributorStripeTestUtil::current_cluster_state_bundle() const noexcept {
    return _stripe->getClusterStateBundle();
}

std::string DistributorStripeTestUtil::active_ideal_state_operations() const {
    return _stripe->getActiveIdealStateOperations();
}

const PendingMessageTracker& DistributorStripeTestUtil::pending_message_tracker() const noexcept {
    return _stripe->getPendingMessageTracker();
}

PendingMessageTracker& DistributorStripeTestUtil::pending_message_tracker() noexcept {
    return _stripe->getPendingMessageTracker();
}

std::chrono::steady_clock::duration DistributorStripeTestUtil::db_memory_sample_interval() const noexcept {
    return _stripe->db_memory_sample_interval();
}

void DistributorStripeTestUtil::set_node_supported_features(uint16_t node, const NodeSupportedFeatures& features) {
...
const lib::Distribution& DistributorStripeTestUtil::getDistribution() const {
    return getBucketSpaceRepo().get(makeBucketSpace()).getDistribution();
}

std::vector<document::BucketSpace> DistributorStripeTestUtil::getBucketSpaces() const {
...
void DistributorStripeTestUtil::enable_cluster_state(std::string_view state) {
    getBucketDBUpdater().simulate_cluster_state_bundle_activation(lib::ClusterStateBundle(lib::ClusterState(state)));
}

void DistributorStripeTestUtil::enable_cluster_state(const lib::ClusterStateBundle& state) {
    getBucketDBUpdater().simulate_cluster_state_bundle_activation(state);
}

void DistributorStripeTestUtil::setSystemState(const lib::ClusterState& systemState) {
    _stripe->enableClusterStateBundle(lib::ClusterStateBundle(systemState));
}

void DistributorStripeTestUtil::tag_content_node_supports_condition_probing(uint16_t index, bool supported) {
