...
std::optional<int64_t> document_count_total_from(const vespalib::Slime& root) {
    auto& node = root["distributor"]["document-count-total"];
    if (!node.valid()) {
        return std::nullopt;
    }
    return node.asLong();
...
std::optional<int64_t> bytes_total_from(const vespalib::Slime& root) {
    auto& node = root["distributor"]["bytes-total"];
    if (!node.valid()) {
        return std::nullopt;
    }
    return node.asLong();
...
TEST_F(DistributorHostInfoReporterTest, generate_example_json) {
    ...
    f.bucketSpacesStatsProvider._global_stats = CrossBucketSpaceStats(1337, 456789);
...
TEST_F(DistributorHostInfoReporterTest, global_stats_are_reported_when_present) {
    ...
    f.bucketSpacesStatsProvider._global_stats = CrossBucketSpaceStats(12345, 567890);
    {
        auto root = f.to_slime();
