...
struct ExternalOperationHandlerTest : Test, DistributorStripeTestUtil {
    ...
    void assert_rejection_due_to_unsafe_time(std::shared_ptr<api::StorageCommand> cmd);

    void assert_second_command_rejected_due_to_concurrent_mutation(std::shared_ptr<api::StorageCommand> cmd1,
                                                                   std::shared_ptr<api::StorageCommand> cmd2,
                                                                   const std::string& expected_id_in_message);
    void assert_second_command_not_rejected_due_to_concurrent_mutation(std::shared_ptr<api::StorageCommand> cmd1,
                                                                       std::shared_ptr<api::StorageCommand> cmd2);
    void do_test_second_command_rejected_due_to_oversize(std::shared_ptr<api::StorageCommand> cmd1,
                                                         std::shared_ptr<api::StorageCommand> cmd2);
    ...
    void process_tas_and_get_forwarded_condition(std::shared_ptr<api::TestAndSetCommand> client_cmd_in,
    ...
    template <typename OperationType> void do_test_tas_condition_rewrite(std::shared_ptr<api::TestAndSetCommand> cmd);
    template <typename OperationType>
...
TEST_F(ExternalOperationHandlerTest, bucket_split_mask) {
    {
        ...
        EXPECT_EQ(document::BucketId(16, 0xffff), operation_context()
                                                      .make_split_bit_constrained_bucket_id(document::DocumentId(
                                                          vespalib::make_string("id:ns:test:n=%d::", 0xffff)))
                                                      .stripUnused());
        EXPECT_EQ(document::BucketId(16, 0), operation_context()
                                                 .make_split_bit_constrained_bucket_id(document::DocumentId(
        ...
        EXPECT_EQ(document::BucketId(16, 0xffff), operation_context()
                                                      .make_split_bit_constrained_bucket_id(document::DocumentId(
                                                          vespalib::make_string("id:ns:test:n=%d::", 0xffff)))
                                                      .stripUnused());
        EXPECT_EQ(document::BucketId(16, 0x100), operation_context()
                                                     .make_split_bit_constrained_bucket_id(document::DocumentId(
    ...
    }
    {
        ...
        createLinks();
        EXPECT_EQ(document::BucketId(20, 0x11111), operation_context()
                                                       .make_split_bit_constrained_bucket_id(document::DocumentId(
                                                           vespalib::make_string("id:ns:test:n=%d::", 0x111111)))
                                                       .stripUnused());
        EXPECT_EQ(document::BucketId(20, 0x22222), operation_context()
                                                       .make_split_bit_constrained_bucket_id(document::DocumentId(
...
document::BucketId ExternalOperationHandlerTest::findNonOwnedUserBucketInState(std::string_view statestr) {
...
document::BucketId ExternalOperationHandlerTest::findOwned1stNotOwned2ndInStates(std::string_view statestr1,
...
std::shared_ptr<api::GetCommand> ExternalOperationHandlerTest::makeGetCommand(const std::string& id) const {
    return std::make_shared<api::GetCommand>(makeDocumentBucket(document::BucketId(0)), DocumentId(id),
                                             document::AllFields::NAME);
}

std::shared_ptr<api::GetCommand> ExternalOperationHandlerTest::makeGetCommandForUser(uint64_t id) const {
    DocumentId docId(vespalib::make_string("id:foo:test:n=%" PRIu64 ":bar", id));
    return std::make_shared<api::GetCommand>(makeDocumentBucket(document::BucketId(0)), docId,
                                             document::AllFields::NAME);
}

std::shared_ptr<api::UpdateCommand> ExternalOperationHandlerTest::makeUpdateCommand(const std::string& doc_type,
                                                                                    const std::string& id) const {
    auto update = std::make_shared<document::DocumentUpdate>(
        _testDocMan.getTypeRepo(), *_testDocMan.getTypeRepo().getDocumentType(doc_type), document::DocumentId(id));
    return std::make_shared<api::UpdateCommand>(makeDocumentBucket(document::BucketId(0)), std::move(update),
                                                api::Timestamp(0));
}

std::shared_ptr<api::UpdateCommand> ExternalOperationHandlerTest::makeUpdateCommand() const {
    return makeUpdateCommand("testdoctype1", "id:foo:testdoctype1::baz");
}

std::shared_ptr<api::UpdateCommand> ExternalOperationHandlerTest::makeUpdateCommandForUser(uint64_t id) const {
    return makeUpdateCommand("testdoctype1", vespalib::make_string("id::testdoctype1:n=%" PRIu64 ":bar", id));
}

std::shared_ptr<api::PutCommand> ExternalOperationHandlerTest::makePutCommand(const std::string& doc_type,
                                                                              const std::string& id) const {
    auto doc = _testDocMan.createDocument(doc_type, id);
    return std::make_shared<api::PutCommand>(makeDocumentBucket(document::BucketId(0)), std::move(doc),
                                             api::Timestamp(0));
}

std::shared_ptr<api::RemoveCommand> ExternalOperationHandlerTest::makeRemoveCommand(const std::string& id) const {
    return std::make_shared<api::RemoveCommand>(makeDocumentBucket(document::BucketId(0)), DocumentId(id),
                                                api::Timestamp(0));
}
...
void ExternalOperationHandlerTest::verify_busy_bounced_due_to_no_active_state(
    ...
    ASSERT_FALSE(genOp.get());
    ASSERT_EQ(1, _sender.replies().size());
    EXPECT_EQ("ReturnCode(BUSY, No cluster state activated yet)", _sender.reply(0)->getResult().toString());
}
...
void ExternalOperationHandlerTest::assert_rejection_due_to_unsafe_time(std::shared_ptr<api::StorageCommand> cmd) {
    createLinks();
...
TEST_F(ExternalOperationHandlerTest, get_not_rejected_by_unsafe_time_point) {
    ...
    Operation::SP generated;
    getExternalOperationHandler().handleMessage(makeGetCommandForUser(0), generated);
...
TEST_F(ExternalOperationHandlerTest, mutation_not_rejected_when_safe_point_reached) {
    ...
    getExternalOperationHandler().handleMessage(
        std::make_shared<api::RemoveCommand>(makeDocumentBucket(document::BucketId(0)), id, api::Timestamp(0)),
        generated);
    ASSERT_NE(generated.get(), nullptr);
    ASSERT_EQ(0, _sender.replies().size());
...
void ExternalOperationHandlerTest::start_operation_verify_not_rejected(std::shared_ptr<api::StorageCommand> cmd,
...
void ExternalOperationHandlerTest::start_operation_verify_rejected(std::shared_ptr<api::StorageCommand> cmd) {
    Operation::SP generated;
    _sender.replies().clear();
    getExternalOperationHandler().handleMessage(cmd, generated);
    ASSERT_EQ(generated.get(), nullptr);
...
void ExternalOperationHandlerTest::assert_second_command_rejected_due_to_concurrent_mutation(
    std::shared_ptr<api::StorageCommand> cmd1, std::shared_ptr<api::StorageCommand> cmd2,
    const std::string& expected_id_in_message) {
    ...
    EXPECT_EQ(vespalib::make_string("ReturnCode(BUSY, A mutating operation for document "
                                    "'%s' is already in progress)",
                                    expected_id_in_message.c_str()),
              _sender.reply(0)->getResult().toString());
}
...
TEST_F(ExternalOperationHandlerTest, reject_put_with_concurrent_mutation_to_same_id) {
    ASSERT_NO_FATAL_FAILURE(assert_second_command_rejected_due_to_concurrent_mutation(
        makePutCommand("testdoctype1", _dummy_id), makePutCommand("testdoctype1", _dummy_id), _dummy_id));
...
TEST_F(ExternalOperationHandlerTest, do_not_reject_remove_operations_to_different_ids) {
    ASSERT_NO_FATAL_FAILURE(assert_second_command_not_rejected_due_to_concurrent_mutation(
        makeRemoveCommand("id:foo:testdoctype1::baz"), makeRemoveCommand("id:foo:testdoctype1::foo")));
...
TEST_F(ExternalOperationHandlerTest, reject_update_with_concurrent_mutation_to_same_id) {
    ASSERT_NO_FATAL_FAILURE(assert_second_command_rejected_due_to_concurrent_mutation(
        makeUpdateCommand("testdoctype1", _dummy_id), makeUpdateCommand("testdoctype1", _dummy_id), _dummy_id));
...
TEST_F(ExternalOperationHandlerTest, oversized_put_is_rejected) {
    do_test_second_command_rejected_due_to_oversize(makePutCommand("testdoctype1", "id:foo:testdoctype1::foo"),
                                                    makePutCommand("testdoctype1", "id:foo:testdoctype1::bar"));
}

TEST_F(ExternalOperationHandlerTest, oversized_update_is_rejected) {
    do_test_second_command_rejected_due_to_oversize(makeUpdateCommand("testdoctype1", "id:foo:testdoctype1::foo"),
...
TEST_F(ExternalOperationHandlerTest, sequencing_works_across_mutation_types) {
    ...
    ASSERT_NO_FATAL_FAILURE(
        start_operation_verify_not_rejected(makePutCommand("testdoctype1", _dummy_id), generated));
...
TEST_F(ExternalOperationHandlerTest, bucket_level_lock_rejection_includes_reason_in_error_message) {
    ...
    EXPECT_EQ(vespalib::make_string("ReturnCode(BUSY, A reindexing operation is in progress for "
                                    "the data bucket containing '%s')",
                                    _dummy_id.c_str()),
              _sender.reply(0)->getResult().toString());
}
...
TEST_F(ExternalOperationHandlerTest, gets_are_started_with_mutable_db_outside_transition_period) {
    ...
    Operation::SP op;
    ASSERT_NO_FATAL_FAILURE(start_operation_verify_not_rejected(makeGetCommandForUser(b.withoutCountBits()), op));
...
TEST_F(ExternalOperationHandlerTest, gets_are_started_with_read_only_db_during_transition_period) {
    ...
    Operation::SP op;
    ASSERT_NO_FATAL_FAILURE(
        start_operation_verify_not_rejected(makeGetCommandForUser(non_owned_bucket.withoutCountBits()), op));
    auto&       get_op = dynamic_cast<GetOperation&>(*op);
...
void ExternalOperationHandlerTest::do_test_get_weak_consistency_is_propagated(bool use_weak) {
    ...
    EXPECT_EQ(get_op.desired_read_consistency(),
              (use_weak ? api::InternalReadConsistency::Weak : api::InternalReadConsistency::Strong));
}
...
TEST_F(ExternalOperationHandlerTest, puts_are_rejected_if_feed_is_blocked) {
    ...
    ASSERT_NO_FATAL_FAILURE(
        start_operation_verify_rejected(makePutCommand("testdoctype1", "id:foo:testdoctype1::foo")));
...
TEST_F(ExternalOperationHandlerTest, non_trivial_updates_are_rejected_if_feed_is_blocked) {
    ...
    cmd->getUpdate()->addUpdate(
        FieldUpdate(doc_type->getField("title"))
...
void ExternalOperationHandlerTest::set_two_nodes_tas_cond_timestamp_supported(bool node0, bool node1) {
...
template <typename OperationType>
void ExternalOperationHandlerTest::process_tas_and_get_forwarded_condition(
    std::shared_ptr<api::TestAndSetCommand> client_cmd_in, const documentapi::TestAndSetCondition& cond_in,
    documentapi::TestAndSetCondition& fwd_cond_out) {
...
template <typename OperationType>
void ExternalOperationHandlerTest::do_test_tas_condition_rewrite(std::shared_ptr<api::TestAndSetCommand> cmd) {
    createLinks();
    ...
    ASSERT_NO_FATAL_FAILURE(
        process_tas_and_get_forwarded_condition<OperationType>(std::move(cmd), cond_before, cond_after));
...
TEST_F(ExternalOperationHandlerTest,
       remove_timestamp_from_tas_condition_if_not_supported_across_all_nodes_remove_case) {
    do_test_tas_condition_rewrite<RemoveOperation>(makeRemoveCommand(_dummy_id));
}

TEST_F(ExternalOperationHandlerTest,
       remove_timestamp_from_tas_condition_if_not_supported_across_all_nodes_update_case) {
    do_test_tas_condition_rewrite<TwoPhaseUpdateOperation>(makeUpdateCommand("testdoctype1", _dummy_id));
}

template <typename OperationType>
void ExternalOperationHandlerTest::do_test_tas_condition_no_rewrite(std::shared_ptr<api::TestAndSetCommand> cmd) {
    createLinks();
    setup_stripe(1, 2, "version:1 distributor:1 storage:2");
...
TEST_F(ExternalOperationHandlerTest,
       do_not_remove_timestamp_from_tas_condition_if_supported_across_all_nodes_put_case) {
    do_test_tas_condition_no_rewrite<PutOperation>(makePutCommand("testdoctype1", _dummy_id));
}

TEST_F(ExternalOperationHandlerTest,
       do_not_remove_timestamp_from_tas_condition_if_supported_across_all_nodes_remove_case) {
    do_test_tas_condition_no_rewrite<RemoveOperation>(makeRemoveCommand(_dummy_id));
}

TEST_F(ExternalOperationHandlerTest,
       do_not_remove_timestamp_from_tas_condition_if_supported_across_all_nodes_update_case) {
...
TEST_F(OperationHandlerSequencingTest,
       put_not_allowed_through_locked_bucket_if_tas_token_mismatches_current_lock_tkoen) {
    auto put = makePutCommand("testdoctype1", "id:foo:testdoctype1:n=1:bar");
