...
public:
     ...
     */
    void add_nodes_to_stripe_bucket_db(const document::Bucket& bucket, const std::string& nodeStr);
    ...
    framework::defaultimplementation::FakeClock& fake_clock() noexcept { return _node->getClock(); }
    ...
    void setup_distributor(int redundancy, int node_count, const std::string& systemState,
                           uint32_t early_return = false, bool require_primary_to_be_written = true);

    void setup_distributor(int redundancy, int node_count, const lib::ClusterStateBundle& state,
                           uint32_t early_return = false, bool require_primary_to_be_written = true);
