...
struct NodeFactory {
    ...
    NodeFactory& add(int index, int crc) {
        _bucketDatabase.emplace_back(std::make_unique<BucketCopy>(0, index, api::BucketInfo(crc, 5, 10)));
...
TEST_F(MergeLimiterTest, keeps_all_below_limit) {
    MergeLimiter::NodeArray nodes(
        NodeFactory().addTrusted(3, 0x4).addTrusted(5, 0x4).add(9, 0x6).add(2, 0x6).add(4, 0x5));
...
TEST_F(MergeLimiterTest, more_than_max_untrusted) {
    MergeLimiter::NodeArray nodes(
        NodeFactory().addTrusted(3, 0x4).addTrusted(5, 0x4).add(9, 0x6).add(2, 0x6).add(13, 0x9).add(1, 0x7).add(
            4, 0x5));
...
TEST_F(MergeLimiterTest, all_untrusted_less_than_max_variants) {
    MergeLimiter::NodeArray nodes(
        NodeFactory().add(3, 0x4).add(5, 0x4).add(9, 0x6).add(2, 0x6).add(13, 0x3).add(1, 0x3).add(4, 0x3));
...
TEST_F(MergeLimiterTest, all_untrusted_more_than_max_variants) {
    MergeLimiter::NodeArray nodes(
        NodeFactory().add(3, 0x4).add(5, 0x5).add(9, 0x6).add(2, 0x6).add(13, 0x3).add(1, 0x9).add(4, 0x8));
...
TEST_F(MergeLimiterTest, non_source_only_replica_chosen_from_in_sync_group) {
    ...
    MergeLimiter::NodeArray nodes(
        NodeFactory().add(9, 0x6).add(2, 0x6).add(13, 0x6).setSourceOnly().add(1, 0x7).setSourceOnly());
    ASSERT_EQ(limit(2, nodes), "2,13s");
    ASSERT_EQ(limit(3, nodes), "2,13s,1s");
}
...
TEST_F(MergeLimiterTest, non_source_only_replicas_preferred_when_replicas_not_in_sync) {
    MergeLimiter::NodeArray nodes(
        NodeFactory().add(9, 0x4).add(2, 0x5).add(13, 0x6).setSourceOnly().add(1, 0x7).setSourceOnly());
...
TEST_F(MergeLimiterTest, missing_replica_distinct_from_empty_replica) {
    MergeLimiter::NodeArray nodes(NodeFactory().addEmpty(3).addEmpty(5).addMissing(1).addMissing(2));
