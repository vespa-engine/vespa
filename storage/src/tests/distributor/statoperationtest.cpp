...
TEST_F(StatOperationTest, bucket_info) {
    ...
    StatBucketOperation op(getDistributorBucketSpace(), std::make_shared<api::StatBucketCommand>(
...
TEST_F(StatOperationTest, bucket_list) {
    ...
    for (uint32_t i = 0; i < 2; ++i) {
        insertBucketInfo(document::BucketId(16, 5), i, 0xff, 100, 200, true, (i == 1));
    }

    auto msg = std::make_shared<api::GetBucketListCommand>(makeDocumentBucket(document::BucketId(16, 5)));

    StatBucketListOperation op(getDistributorBucketSpace().getBucketDatabase(), getIdealStateManager(),
                               node_context().node_index(), msg);
    op.start(_sender);
