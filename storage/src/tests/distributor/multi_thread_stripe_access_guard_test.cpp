...
struct AggregationTestingMockTickableStripe : MockTickableStripe {
    ...
    StripeAccessGuard::PendingOperationStats pending_operation_stats() const override { return pending_stats; }
...
struct MultiThreadedStripeAccessGuardTest : Test {
    ...
    MultiThreadedStripeAccessGuardTest() : _pool(), _accessor(_pool) {}

    ~MultiThreadedStripeAccessGuardTest() override;

    void start_pool_with_stripes() { _pool.start({{&_stripe0, &_stripe1, &_stripe2, &_stripe3}}); }

    void start_pool_with_one_stripe() { _pool.start({&_stripe0}); }
    ...
    void merge_entries_into_db(const RawIdVector& raw_ids) {
        ...
        guard->merge_entries_into_db(document::FixedBucketSpaces::default_space(), api::Timestamp(),
                                     lib::Distribution(), lib::ClusterState(), "", {}, entries);
    }
};
...
TEST_F(MultiThreadedStripeAccessGuardTest, remove_superfluous_buckets_aggregates_reports_across_stripes) {
    ...
    auto guard = _accessor.rendezvous_and_hold_all();
    auto report =
        guard->remove_superfluous_buckets(document::FixedBucketSpaces::default_space(), lib::ClusterState(), false);
