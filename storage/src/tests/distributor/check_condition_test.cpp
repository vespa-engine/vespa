...
class CheckConditionTest : public Test, public DistributorStripeTestUtil {
public:
    ...
    static std::shared_ptr<api::GetReply> make_reply(const api::GetCommand& cmd, api::Timestamp ts, bool is_tombstone,
...
// TODO deprecate in favor of cancelling
TEST_F(CheckConditionTest,
       check_fails_if_bucket_ownership_changed_between_start_and_completion_pending_transition_case) {
    test_cond_with_2_gets_sent(
        [&](auto& cond) {
            cond.handle_reply(_sender, make_matched_reply(0));
            simulate_set_pending_cluster_state(
                "version:2 storage:1 distributor:1 .0.s:d"); // technically, no distributors own anything
            cond.handle_reply(_sender, make_matched_reply(1));
        },
        [&](auto& outcome) {
            EXPECT_FALSE(outcome.matched_condition());
...
// TODO deprecate in favor of cancelling
TEST_F(CheckConditionTest,
       check_fails_if_bucket_ownership_changed_between_start_and_completion_completed_transition_case) {
    test_cond_with_2_gets_sent(
        [&](auto& cond) {
            cond.handle_reply(_sender, make_matched_reply(0));
...
TEST_F(CheckConditionTest, condition_evaluation_increments_probe_latency_metrics) {
    ...
    test_cond_with_2_gets_sent(
        ...
        [&](auto& outcome) noexcept { (void)outcome; });
