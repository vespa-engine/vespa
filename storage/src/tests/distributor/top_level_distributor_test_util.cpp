...
void TopLevelDistributorTestUtil::create_links() {
    ...
    _distributor.reset(new TopLevelDistributor(_node->getComponentRegister(), _node->node_identity(), *_thread_pool,
                                               *_stripe_pool, *this, _num_distributor_stripes, _host_info,
                                               &_message_sender));
    _component = std::make_unique<storage::DistributorComponent>(_node->getComponentRegister(), "distrtestutil");
};

void TopLevelDistributorTestUtil::setup_distributor(int redundancy, int node_count, const std::string& cluster_state,
                                                    uint32_t early_return, bool require_primary_to_be_written) {
    setup_distributor(redundancy, node_count, lib::ClusterStateBundle(lib::ClusterState(cluster_state)), early_return,
                      require_primary_to_be_written);
}

void TopLevelDistributorTestUtil::setup_distributor(int redundancy, int node_count,
                                                    const lib::ClusterStateBundle& state, uint32_t early_return,
...
size_t TopLevelDistributorTestUtil::stripe_index_of_bucket(const document::BucketId& id) const noexcept {
    return stripe_of_bucket_key(id.toKey(), _distributor->_n_stripe_bits);
}

size_t TopLevelDistributorTestUtil::stripe_index_of_bucket(const document::Bucket& bucket) const noexcept {
    return stripe_of_bucket_key(bucket.getBucketId().toKey(), _distributor->_n_stripe_bits);
}

void TopLevelDistributorTestUtil::receive_set_system_state_command(const std::string& state_str) {
...
bool TopLevelDistributorTestUtil::handle_top_level_message(const std::shared_ptr<api::StorageMessage>& msg) {
    return _distributor->onDown(msg);
}

void TopLevelDistributorTestUtil::close() {
    _component.reset();
    if (_distributor) {
...
void TopLevelDistributorTestUtil::add_nodes_to_stripe_bucket_db(const document::Bucket& bucket,
...
std::string TopLevelDistributorTestUtil::get_ideal_str(document::BucketId id, const lib::ClusterState& state) {
...
void TopLevelDistributorTestUtil::add_ideal_nodes(const lib::ClusterState& state, const document::BucketId& id) {
...
std::string TopLevelDistributorTestUtil::get_nodes(document::BucketId id) {
    BucketDatabase::Entry entry = get_bucket(id);
...
void TopLevelDistributorTestUtil::add_nodes_to_stripe_bucket_db(const document::BucketId& id,
                                                                const std::string&        nodeStr) {
    add_nodes_to_stripe_bucket_db(document::Bucket(makeBucketSpace(), id), nodeStr);
}

BucketDatabase::Entry TopLevelDistributorTestUtil::get_bucket(const document::Bucket& bucket) const {
    return stripe_bucket_database(stripe_index_of_bucket(bucket), bucket.getBucketSpace()).get(bucket.getBucketId());
}

BucketDatabase::Entry TopLevelDistributorTestUtil::get_bucket(const document::BucketId& bId) const {
    return stripe_bucket_database(stripe_index_of_bucket(bId)).get(bId);
}

BucketSpaceStateMap& TopLevelDistributorTestUtil::bucket_space_states() noexcept {
    return _distributor->_component.bucket_space_states();
}

const BucketSpaceStateMap& TopLevelDistributorTestUtil::bucket_space_states() const noexcept {
    return _distributor->_component.bucket_space_states();
}

std::unique_ptr<StripeAccessGuard> TopLevelDistributorTestUtil::acquire_stripe_guard() {
...
TopLevelBucketDBUpdater& TopLevelDistributorTestUtil::bucket_db_updater() {
...
DistributorBucketSpace& TopLevelDistributorTestUtil::distributor_bucket_space(const document::BucketId& id) {
...
DistributorStripe& TopLevelDistributorTestUtil::stripe_of_bucket(const document::BucketId& id) noexcept {
    return *_distributor->_stripes[stripe_index_of_bucket(id)];
}

const DistributorStripe& TopLevelDistributorTestUtil::stripe_of_bucket(const document::BucketId& id) const noexcept {
    return *_distributor->_stripes[stripe_index_of_bucket(id)];
}

DistributorStripe& TopLevelDistributorTestUtil::stripe_of_bucket(const document::Bucket& bucket) noexcept {
...
bool TopLevelDistributorTestUtil::tick(bool only_tick_top_level) {
    framework::ThreadWaitInfo res(framework::ThreadWaitInfo::NO_MORE_CRITICAL_WORK_KNOWN);
    {
...
const DistributorManagerConfig& TopLevelDistributorTestUtil::current_distributor_config() const {
    return _component->getDistributorConfig();
}

void TopLevelDistributorTestUtil::reconfigure(const DistributorManagerConfig& cfg) {
    _node->getComponentRegister().setDistributorConfig(cfg);
    tick(); // Config is propagated upon next top-level tick
}

framework::MetricUpdateHook& TopLevelDistributorTestUtil::distributor_metric_update_hook() {
...
std::vector<DistributorStripe*> TopLevelDistributorTestUtil::distributor_stripes() const {
...
bool TopLevelDistributorTestUtil::all_distributor_stripes_are_in_recovery_mode() const {
...
void TopLevelDistributorTestUtil::enable_distributor_cluster_state(std::string_view state,
                                                                   bool             has_bucket_ownership_transfer) {
    bucket_db_updater().simulate_cluster_state_bundle_activation(lib::ClusterStateBundle(lib::ClusterState(state)),
                                                                 has_bucket_ownership_transfer);
}

void TopLevelDistributorTestUtil::enable_distributor_cluster_state(const lib::ClusterStateBundle& state) {
    bucket_db_updater().simulate_cluster_state_bundle_activation(state);
}

std::vector<document::BucketSpace> TopLevelDistributorTestUtil::bucket_spaces() {
    return {document::FixedBucketSpaces::default_space(), document::FixedBucketSpaces::global_space()};
}

void TopLevelDistributorTestUtil::trigger_distribution_change(std::shared_ptr<lib::Distribution> distr) {
...
void TopLevelDistributorTestUtil::enable_next_distribution_if_changed() {
...
void TopLevelDistributorTestUtil::tick_distributor_and_stripes_n_times(uint32_t n) {
    for (uint32_t i = 0; i < n; ++i) {
        tick(false);
    }
}

void TopLevelDistributorTestUtil::tick_top_level_distributor_n_times(uint32_t n) {
    for (uint32_t i = 0; i < n; ++i) {
        tick(true);
    }
}

void TopLevelDistributorTestUtil::complete_recovery_mode_on_all_stripes() {
    for (auto* s : distributor_stripes()) {
        s->scanAllBuckets();
    }
}
