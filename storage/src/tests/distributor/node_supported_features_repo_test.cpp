...
struct NodeSupportedFeaturesRepoTest : Test {
    ...
    static NodeSupportedFeatures set_features() noexcept {
        ...
        f.timestamps_in_tas_conditions = true;
        return f;
    }
...
TEST_F(NodeSupportedFeaturesRepoTest, feature_set_is_empty_by_default) {
    ...
    EXPECT_EQ(_repo.supported_by_all_nodes(), unset_features());
}
...
TEST_F(NodeSupportedFeaturesRepoTest, features_supported_by_all_nodes_requires_feature_intersection) {
    FeatureMap fm;
    fm[1] = set_features();
    fm[60] = set_features();
    auto new_repo = _repo.make_union_of(fm);
    EXPECT_EQ(new_repo->supported_by_all_nodes(), set_features());

    fm[1].timestamps_in_tas_conditions = false;
    new_repo = _repo.make_union_of(fm);
    NodeSupportedFeatures expected;
    expected.unordered_merge_chaining = true;
    EXPECT_EQ(new_repo->supported_by_all_nodes(), expected);
