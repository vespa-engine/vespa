...
class TopLevelBucketDBUpdaterTest : public Test, public TopLevelDistributorTestUtil {
public:
    ...
    std::shared_ptr<RequestBucketInfoReply> make_fake_bucket_reply(const lib::ClusterState&        state,
                                                                   const RequestBucketInfoCommand& cmd,
                                                                   int storageIndex, uint32_t bucketCount,
        ...
        for (uint32_t i = 0; i < bucketCount + invalidBucketCount; i++) {
            ...
            distributor_bucket_space(bucket).getDistribution().getIdealNodes(lib::NodeType::STORAGE, state, bucket,
                                                                             nodes, "uim");
    ...
    void fake_bucket_reply(const lib::ClusterState& state, const api::StorageCommand& cmd, uint32_t bucket_count,
        ...
        bucket_db_updater().onRequestBucketInfoReply(
            make_fake_bucket_reply(state, dynamic_cast<const RequestBucketInfoCommand&>(cmd), address.getIndex(),
    ...
    std::string verify_bucket(document::BucketId id, const lib::ClusterState& state) {
        ...
        std::vector<uint16_t> nodes;
        distributor_bucket_space(id).getDistribution().getIdealNodes(lib::NodeType::STORAGE, state,
                                                                     document::BucketId(id), nodes, "uim");
        ...
        for (uint32_t i = 0; i < nodes.size(); i++) {
            ...
            if (!found) {
                return vespalib::make_string("Bucket Id %s has no copy from node %d", id.toString().c_str(),
                                             nodes[i]);
            }
        }
    ...
    void sort_sent_messages_by_index(DistributorMessageSenderStub& sender, size_t sortFromOffset = 0) {
        std::sort(sender.commands().begin() + sortFromOffset, sender.commands().end(), OrderByIncreasingNodeIndex());
    }
    ...
    void set_cluster_state(const std::string& state_str) { set_cluster_state(lib::ClusterState(state_str)); }
    ...
    void complete_bucket_info_gathering(const lib::ClusterState& state, size_t expected_msgs,
                                        uint32_t bucket_count = 1, uint32_t invalid_bucket_count = 0) {
        ASSERT_EQ(expected_msgs, _sender.commands().size());

        for (uint32_t i = 0; i < _sender.commands().size(); i++) {
            ASSERT_NO_FATAL_FAILURE(
                fake_bucket_reply(state, *_sender.command(i), bucket_count, invalid_bucket_count));
        }
    }
    ...
    void assert_correct_buckets(int num_buckets, const std::string& state_str) {
        ...
        for (int i = 0; i < num_buckets; i++) {
            ASSERT_EQ(get_ideal_str(document::BucketId(16, i), state), get_nodes(document::BucketId(16, i)));
        }
    }
    ...
    std::string dump_bucket(const document::BucketId& id) const { return get_bucket(id).toString(); }
    ...
    void complete_state_transition_in_seconds(const std::string& stateStr, uint32_t seconds, uint32_t expectedMsgs) {
        _sender.clear();
    ...
    ClusterInformation::CSP create_cluster_info(const std::string& clusterStateString) {
        ...
        auto cluster_info = std::make_shared<SimpleClusterInformation>(_distributor->node_identity().node_index(),
                                                                       cluster_state_bundle, "ui");
    ...
    struct PendingClusterStateFixture {
        ...
        PendingClusterStateFixture(TopLevelBucketDBUpdaterTest& owner, const std::string& old_cluster_state,
            ...
            state = PendingClusterState::createForClusterStateChange(
                clock, cluster_info, sender, owner.bucket_space_states(), cmd, outdated_nodes_map, api::Timestamp(1));
        }
    ...
    create_pending_state_fixture_for_state_change(const std::string& oldClusterState,
                                                  const std::string& newClusterState) {
        return std::make_unique<PendingClusterStateFixture>(*this, oldClusterState, newClusterState);
    }

    std::unique_ptr<PendingClusterStateFixture>
    create_pending_state_fixture_for_distribution_change(const std::string& oldClusterState) {
        return std::make_unique<PendingClusterStateFixture>(*this, oldClusterState);
    }

    std::string get_sent_nodes(const std::string& old_cluster_state, const std::string& new_cluster_state);
    ...
    std::string merge_bucket_lists(const lib::ClusterState& old_state, const std::string& existing_data,
                                   const lib::ClusterState& new_state, const std::string& new_data,
                                   bool include_bucket_info = false);

    std::string merge_bucket_lists(const std::string& existingData, const std::string& newData,
                                   bool includeBucketInfo = false);

    std::vector<uint16_t> get_send_set() const;

    std::vector<uint16_t> get_sent_nodes_with_preemption(const std::string& old_cluster_state,
    ...
    void trigger_completed_but_not_yet_activated_transition(std::string_view initial_state_str,
                                                            uint32_t initial_buckets, uint32_t initial_expected_msgs,
                                                            std::string_view pending_state_str,
                                                            uint32_t pending_buckets, uint32_t pending_expected_msgs);
...
TopLevelBucketDBUpdaterTest::TopLevelBucketDBUpdaterTest() : TopLevelDistributorTestUtil(), _bucket_spaces() {
}
...
std::string make_string_list(std::string s, uint32_t count) {
...
std::string make_request_bucket_info_strings(uint32_t count) {
    return make_string_list("Request bucket info", count);
}

} // namespace

std::string TopLevelBucketDBUpdaterTest::get_node_list(const std::vector<uint16_t>& nodes, size_t count) {
    std::ostringstream ost;
    bool               first = true;
...
std::string TopLevelBucketDBUpdaterTest::get_node_list(const std::vector<uint16_t>& nodes) {
    return get_node_list(nodes, _bucket_spaces.size());
}

void TopLevelBucketDBUpdaterTest::trigger_completed_but_not_yet_activated_transition(
    std::string_view initial_state_str, uint32_t initial_buckets, uint32_t initial_expected_msgs,
    std::string_view pending_state_str, uint32_t pending_buckets, uint32_t pending_expected_msgs) {
    ...
    ASSERT_NO_FATAL_FAILURE(
        complete_bucket_info_gathering(initial_state, message_count(initial_expected_msgs), initial_buckets));
    _sender.clear();
    ...
    ASSERT_NO_FATAL_FAILURE(
        complete_bucket_info_gathering(pending_state, message_count(pending_expected_msgs), pending_buckets));
    _sender.clear();
}

void TopLevelBucketDBUpdaterTest::verify_state_bundle_propagated_to_stripes(
    const lib::ClusterStateBundle& expected_bundle) {
...
TEST_F(TopLevelBucketDBUpdaterTest, normal_usage) {
    ...
    ASSERT_NO_FATAL_FAILURE(fake_bucket_reply(lib::ClusterState("distributor:2 storage:3"), *_sender.command(0), 10));
    ...
    ASSERT_NO_FATAL_FAILURE(
        complete_bucket_info_gathering(lib::ClusterState("distributor:2 storage:3"), message_count(3), 10));
...
TEST_F(TopLevelBucketDBUpdaterTest, distributor_change) {
    ...
    ASSERT_EQ(message_count(3), _sender.commands().size());
    ASSERT_NO_FATAL_FAILURE(
        complete_bucket_info_gathering(lib::ClusterState("distributor:2 storage:3"), message_count(3), num_buckets));
    _sender.clear();
...
TEST_F(TopLevelBucketDBUpdaterTest, distributor_change_with_grouping) {
    ...
    ASSERT_NO_FATAL_FAILURE(
        complete_bucket_info_gathering(lib::ClusterState("distributor:6 storage:6"), message_count(6), numBuckets));
    _sender.clear();
...
TEST_F(TopLevelBucketDBUpdaterTest, normal_usage_initializing) {
    ...
    ASSERT_NO_FATAL_FAILURE(
        complete_bucket_info_gathering(lib::ClusterState("distributor:1 storage:1"), _bucket_spaces.size(), 10, 10));
    ...
    ASSERT_NO_FATAL_FAILURE(
        complete_bucket_info_gathering(lib::ClusterState("distributor:1 storage:1"), _bucket_spaces.size(), 20));
...
TEST_F(TopLevelBucketDBUpdaterTest, failed_request_bucket_info) {
...

    {
        for (uint32_t i = 0; i < _bucket_spaces.size(); ++i) {
            auto reply = make_fake_bucket_reply(lib::ClusterState("distributor:1 .0.s:i storage:1"),
                                                dynamic_cast<RequestBucketInfoCommand&>(*_sender.command(i)), 0, 10);
    ...
    for (uint32_t i = 0; i < _bucket_spaces.size(); ++i) {
        ASSERT_NO_FATAL_FAILURE(
            fake_bucket_reply(lib::ClusterState("distributor:1 .0.s:i storage:1"), // FIXME init mode why?
                              *_sender.command(_bucket_spaces.size() + i), 10));
    }

    for (int i = 0; i < 10; i++) {
        EXPECT_EQ("", verify_bucket(document::BucketId(16, i), lib::ClusterState("distributor:1 storage:1")));
    }
...
TEST_F(TopLevelBucketDBUpdaterTest, down_while_init) {
    ...
    ASSERT_NO_FATAL_FAILURE(fake_bucket_reply(lib::ClusterState("distributor:1 storage:3"), *_sender.command(0), 5));

    set_cluster_state("distributor:1 storage:3 .1.s:d");

    ASSERT_NO_FATAL_FAILURE(fake_bucket_reply(lib::ClusterState("distributor:1 storage:3"), *_sender.command(2), 5));

    ASSERT_NO_FATAL_FAILURE(fake_bucket_reply(lib::ClusterState("distributor:1 storage:3"), *_sender.command(1), 5));
}
...
TEST_F(TopLevelBucketDBUpdaterTest, recheck_node) {
    ...
    for (uint32_t i = 0; i < 3; i++) {
        EXPECT_EQ(get_ideal_str(document::BucketId(16, i), state), get_nodes(document::BucketId(16, i)));
    }
...
TEST_F(TopLevelBucketDBUpdaterTest, notify_bucket_change) {
...

    {
        ...
        auto            cmd =
            std::make_shared<api::NotifyBucketChangeCommand>(makeDocumentBucket(document::BucketId(16, 1)), info);
...

    {
        ...
        auto            cmd =
            std::make_shared<api::NotifyBucketChangeCommand>(makeDocumentBucket(document::BucketId(16, 2)), info);
        cmd->setSourceIndex(0);
...
TEST_F(TopLevelBucketDBUpdaterTest, notify_change_with_pending_state_queues_bucket_info_requests) {
    ...
    ASSERT_NO_FATAL_FAILURE(
        complete_bucket_info_gathering(lib::ClusterState("distributor:1 storage:1"), _bucket_spaces.size(), 10));
...
TEST_F(TopLevelBucketDBUpdaterTest, merge_reply) {
    ...
    for (uint32_t i = 0; i < 3; i++) {
        ...
        reqreply->getBucketInfo().emplace_back(bucket_id,
...
std::string TopLevelBucketDBUpdaterTest::get_sent_nodes(const std::string& old_cluster_state,
                                                        const std::string& new_cluster_state) {
...
std::string TopLevelBucketDBUpdaterTest::get_sent_nodes_distribution_changed(const std::string& old_cluster_state) {
    DistributorMessageSenderStub sender;

    framework::defaultimplementation::FakeClock clock;
    auto                                        cluster_info = create_cluster_info(old_cluster_state);
    std::unique_ptr<PendingClusterState>        state(PendingClusterState::createForDistributionChange(
...
TEST_F(TopLevelBucketDBUpdaterTest, pending_cluster_state_send_messages) {
    EXPECT_EQ(get_node_list({0, 1, 2}), get_sent_nodes("cluster:d", "distributor:1 storage:3"));

    EXPECT_EQ(get_node_list({0, 1}), get_sent_nodes("cluster:d", "distributor:1 storage:3 .2.s:m"));

    EXPECT_EQ(get_node_list({2}), get_sent_nodes("distributor:1 storage:2", "distributor:1 storage:3"));

    EXPECT_EQ(get_node_list({2, 3, 4, 5}), get_sent_nodes("distributor:1 storage:2", "distributor:1 storage:6"));

    EXPECT_EQ(get_node_list({0, 1, 2}), get_sent_nodes("distributor:4 storage:3", "distributor:3 storage:3"));
    ...
    EXPECT_EQ(get_node_list({2}), get_sent_nodes("distributor:3 storage:3 .2.s:i", "distributor:3 storage:3"));

    EXPECT_EQ(get_node_list({1}), get_sent_nodes("distributor:3 storage:3 .1.s:d", "distributor:3 storage:3"));
    ...
    EXPECT_EQ(get_node_list({0, 1, 2}), get_sent_nodes_distribution_changed("distributor:3 storage:3"));

    EXPECT_EQ(get_node_list({0, 1}), get_sent_nodes("distributor:10 storage:2", "distributor:10 .1.s:d storage:2"));

    EXPECT_TRUE(get_sent_nodes("distributor:2 storage:2", "distributor:3 .2.s:i storage:2").empty());

    EXPECT_EQ(get_node_list({0, 1, 2}), get_sent_nodes("distributor:3 storage:3", "distributor:3 .2.s:s storage:3"));

    EXPECT_TRUE(get_sent_nodes("distributor:3 .2.s:s storage:3", "distributor:3 .2.s:d storage:3").empty());

    EXPECT_EQ(get_node_list({1}), get_sent_nodes("distributor:3 storage:3 .1.s:m", "distributor:3 storage:3"));
...
TEST_F(TopLevelBucketDBUpdaterTest, pending_cluster_state_receive) {
    ...
    OutdatedNodesMap                            outdated_nodes_map;
    std::unique_ptr<PendingClusterState>        state(PendingClusterState::createForClusterStateChange(
        clock, cluster_info, sender, bucket_space_states(), cmd, outdated_nodes_map, api::Timestamp(1)));
...
void parse_input_data(const std::string& data, uint64_t timestamp, PendingClusterState& state,
            ...
            if (include_bucket_info) {
                ...
                pending_transition.addNodeInfo(
                    ...
                    BucketCopy(timestamp, node,
                               api::BucketInfo(atoi(tok4[1].data()), atoi(tok4[2].data()), atoi(tok4[3].data()),
                                               atoi(tok4[2].data()), atoi(tok4[3].data()))));
            } else {
...
struct BucketDumper : public BucketDatabase::EntryProcessor {
    ...
    explicit BucketDumper(bool include_bucket_info) : _include_bucket_info(include_bucket_info) {}
    ...
    bool process(const BucketDatabase::ConstEntryRef& e) override {
        ...
        for (uint32_t i = 0; i < e->getNodeCount(); ++i) {
            ...
            if (_include_bucket_info) {
                ost << "/" << copy.getChecksum() << "/" << copy.getDocumentCount() << "/"
                    << copy.getTotalDocumentSize() << "/" << (copy.trusted() ? "t" : "u");
            }
        }
        ost << "|";
        return true;
...
std::string TopLevelBucketDBUpdaterTest::merge_bucket_lists(const lib::ClusterState& old_state,
                                                            const std::string&       existing_data,
                                                            const lib::ClusterState& new_state,
                                                            const std::string& new_data, bool include_bucket_info) {
    framework::defaultimplementation::FakeClock clock;
...

    {
        ...
        auto state = PendingClusterState::createForClusterStateChange(
            clock, cluster_info, sender, bucket_space_states(), cmd, outdated_nodes_map, before_time);
...

    {
        ...
        auto state = PendingClusterState::createForClusterStateChange(
            clock, cluster_info, sender, bucket_space_states(), cmd, outdated_nodes_map, after_time);
...
std::string TopLevelBucketDBUpdaterTest::merge_bucket_lists(const std::string& existing_data,
                                                            const std::string& new_data, bool include_bucket_info) {
    return merge_bucket_lists(lib::ClusterState("distributor:1 storage:3"), existing_data,
                              lib::ClusterState("distributor:1 storage:3"), new_data, include_bucket_info);
}
...
TEST_F(TopLevelBucketDBUpdaterTest, pending_cluster_state_merge) {
    ...
    EXPECT_EQ("4:0,1|2:0,1|6:1,2|1:0,2|5:2,0|3:2,1|", merge_bucket_lists("", "0:1,2,4,5|1:2,3,4,6|2:1,3,5,6"));

    // New node came up
    EXPECT_EQ("4:0,1|2:0,1|6:1,2,3|1:3,0,2|5:2,0,3|3:2,3,1|",
              merge_bucket_lists(lib::ClusterState("distributor:1 storage:3"), "0:1,2,4,5|1:2,3,4,6|2:1,3,5,6",
                                 lib::ClusterState("distributor:1 storage:4"), "3:1,3,5,6"));
    ...
    EXPECT_EQ("8:0|4:0,1|2:0,1|6:1,0,2|1:0,2|5:2,0|3:2,1|",
              merge_bucket_lists("0:1,2,4,5|1:2,3,4,6|2:1,3,5,6", "0:1,2,6,8"));

    // Bucket info format is "bucketid/checksum/count/size"
    // Node went from initializing to up and invalid bucket went to empty.
    EXPECT_EQ("2:0/0/0/0/t|", merge_bucket_lists("0:2/0/0/1", "0:2/0/0/0", true));

    EXPECT_EQ("5:1/2/3/4/u,0/0/0/0/u|", merge_bucket_lists("", "0:5/0/0/0|1:5/2/3/4", true));
}
...
TEST_F(TopLevelBucketDBUpdaterTest, pending_cluster_state_merge_replica_changed) {
    ...
    EXPECT_EQ("2:0/2/3/4/t|3:0/2/4/6/t|",
              merge_bucket_lists(lib::ClusterState("distributor:1 storage:1 .0.s:i"), "0:2/1/2/3,3/2/4/6",
                                 lib::ClusterState("distributor:1 storage:1"), "0:2/2/3/4,3/2/4/6", true));
}
...
TEST_F(TopLevelBucketDBUpdaterTest, cluster_state_always_sends_full_fetch_when_distribution_change_pending) {
    ...
    for (uint32_t i = 0; i < message_count(6); ++i) {
        ASSERT_NO_FATAL_FAILURE(
            fake_bucket_reply(lib::ClusterState("distributor:6 storage:6"), *_sender.command(i), num_buckets));
    }
    ...
    for (uint32_t i = 0; i < message_count(6); ++i) {
        ASSERT_NO_FATAL_FAILURE(fake_bucket_reply(lib::ClusterState("distributor:6 .2.t:12345 storage:6"),
                                                  *_sender.command(i + message_count(6)), num_buckets));
    }
...
TEST_F(TopLevelBucketDBUpdaterTest, changed_distribution_config_triggers_recovery_mode) {
    ...
    ASSERT_NO_FATAL_FAILURE(
        set_and_enable_cluster_state(lib::ClusterState("distributor:6 storage:6"), message_count(6), num_buckets));
    _sender.clear();
...
TEST_F(TopLevelBucketDBUpdaterTest, changed_distribution_config_does_not_elide_bucket_db_pruning) {
    ...
    for (auto* s : distributor_stripes()) {
        const auto& db =
            s->getBucketSpaceRepo().get(document::FixedBucketSpaces::default_space()).getBucketDatabase();
...
TEST_F(TopLevelBucketDBUpdaterTest, newer_mutations_not_overwritten_by_earlier_bucket_fetch) {
    ...
    stripe_of_bucket(bucket).bucket_db_updater().operation_context().update_bucket_database(
        makeDocumentBucket(bucket), BucketCopy(insertion_timestamp, 0, wanted_info),
...
std::vector<uint16_t> TopLevelBucketDBUpdaterTest::get_send_set() const {
    std::vector<uint16_t> nodes;
    std::transform(_sender.commands().begin(), _sender.commands().end(), std::back_inserter(nodes), [](auto& cmd) {
...
std::vector<uint16_t> TopLevelBucketDBUpdaterTest::get_sent_nodes_with_preemption(
    const std::string& old_cluster_state, uint32_t expected_old_state_messages,
    const std::string& preempted_cluster_state, const std::string& new_cluster_state) {
    uint32_t dummy_buckets_to_return = 10;
    // FIXME cannot chain assertion checks in non-void function
    set_and_enable_cluster_state(lib::ClusterState(old_cluster_state), expected_old_state_messages,
...
std::vector<uint16_t> TopLevelBucketDBUpdaterTest::expand_node_vec(const std::vector<uint16_t>& nodes) {
...
TEST_F(TopLevelBucketDBUpdaterTest, preempted_distributor_change_carries_node_set_over_to_next_state_fetch) {
    EXPECT_EQ(expand_node_vec({0, 1, 2, 3, 4, 5}),
              get_sent_nodes_with_preemption("version:1 distributor:6 storage:6", message_count(6),
...
TEST_F(TopLevelBucketDBUpdaterTest, preempted_storage_change_carries_node_set_over_to_next_state_fetch) {
    EXPECT_EQ(expand_node_vec({2, 3}),
              get_sent_nodes_with_preemption("version:1 distributor:6 storage:6 .2.s:d", message_count(5),
...
TEST_F(TopLevelBucketDBUpdaterTest, do_not_send_to_preempted_node_now_in_down_state) {
    EXPECT_EQ(NodeVec{}, get_sent_nodes_with_preemption("version:1 distributor:6 storage:6 .2.s:d", message_count(5),
...
TEST_F(TopLevelBucketDBUpdaterTest, do_not_send_to_preempted_node_not_part_of_new_state) {
    ...
    EXPECT_EQ(expand_node_vec({0, 1, 2, 3, 4, 5}),
              get_sent_nodes_with_preemption("version:1 distributor:6 storage:100", message_count(100),
...
TEST_F(TopLevelBucketDBUpdaterTest, DISABLED_cluster_config_downsize_only_sends_to_available_nodes) {
    ...
    ASSERT_NO_FATAL_FAILURE(set_and_enable_cluster_state(lib::ClusterState("distributor:6 storage:6"), expected_msgs,
                                                         dummy_buckets_to_return));
    _sender.clear();
...
TEST_F(TopLevelBucketDBUpdaterTest, node_missing_from_config_is_treated_as_needing_ownership_transfer) {
    ...
    ASSERT_NO_FATAL_FAILURE(set_and_enable_cluster_state(lib::ClusterState("distributor:3 storage:3"), expected_msgs,
                                                         dummy_buckets_to_return));
    _sender.clear();
    ...
    // contains this node.
    const char* downsize_cfg = "redundancy 2\n"
...
TEST_F(TopLevelBucketDBUpdaterTest, changed_distributor_set_implies_ownership_transfer) {
    auto fixture =
        create_pending_state_fixture_for_state_change("distributor:2 storage:2", "distributor:1 storage:2");
    EXPECT_TRUE(fixture->state->hasBucketOwnershipTransfer());

    fixture =
        create_pending_state_fixture_for_state_change("distributor:2 storage:2", "distributor:2 .1.s:d storage:2");
    EXPECT_TRUE(fixture->state->hasBucketOwnershipTransfer());
}

TEST_F(TopLevelBucketDBUpdaterTest, unchanged_distributor_set_implies_no_ownership_transfer) {
    auto fixture =
        create_pending_state_fixture_for_state_change("distributor:2 storage:2", "distributor:2 storage:1");
    EXPECT_FALSE(fixture->state->hasBucketOwnershipTransfer());

    fixture =
        create_pending_state_fixture_for_state_change("distributor:2 storage:2", "distributor:2 storage:2 .1.s:d");
    EXPECT_FALSE(fixture->state->hasBucketOwnershipTransfer());
}
...
TEST_F(TopLevelBucketDBUpdaterTest, transition_time_tracked_across_preempted_transitions) {
    ...
    ASSERT_NO_FATAL_FAILURE(
        complete_state_transition_in_seconds("version:2 distributor:2 storage:3", 3, message_count(3)));
...
TEST_F(TopLevelBucketDBUpdaterTest, batch_update_of_existing_diverging_replicas_does_not_mark_any_as_trusted) {
    ...
    EXPECT_EQ("5:1/7/8/9/u,0/1/2/3/u|",
              merge_bucket_lists(lib::ClusterState("distributor:1 storage:3 .0.s:i"), "0:5/0/0/0|1:5/7/8/9",
                                 lib::ClusterState("distributor:1 storage:3 .0.s:u"), "0:5/1/2/3|1:5/7/8/9", true));
}

TEST_F(TopLevelBucketDBUpdaterTest, batch_add_of_new_diverging_replicas_does_not_mark_any_as_trusted) {
    EXPECT_EQ("5:1/7/8/9/u,0/1/2/3/u|", merge_bucket_lists("", "0:5/1/2/3|1:5/7/8/9", true));
}

TEST_F(TopLevelBucketDBUpdaterTest, batch_add_with_single_resulting_replica_implicitly_marks_as_trusted) {
    EXPECT_EQ("5:0/1/2/3/t|", merge_bucket_lists("", "0:5/1/2/3", true));
}

TEST_F(TopLevelBucketDBUpdaterTest, identity_update_of_single_replica_does_not_clear_trusted) {
    EXPECT_EQ("5:0/1/2/3/t|", merge_bucket_lists("0:5/1/2/3", "0:5/1/2/3", true));
}

TEST_F(TopLevelBucketDBUpdaterTest, identity_update_of_diverging_untrusted_replicas_does_not_mark_any_as_trusted) {
    EXPECT_EQ("5:1/7/8/9/u,0/1/2/3/u|", merge_bucket_lists("0:5/1/2/3|1:5/7/8/9", "0:5/1/2/3|1:5/7/8/9", true));
}

TEST_F(TopLevelBucketDBUpdaterTest, adding_diverging_replica_to_existing_trusted_does_not_remove_trusted) {
    EXPECT_EQ("5:1/2/3/4/u,0/1/2/3/t|", merge_bucket_lists("0:5/1/2/3", "0:5/1/2/3|1:5/2/3/4", true));
}
...
TEST_F(TopLevelBucketDBUpdaterTest,
    ...
    // response).
    EXPECT_EQ("5:1/7/8/9/u,0/1/2/3/u|",
              merge_bucket_lists(lib::ClusterState("distributor:2 storage:3"), "0:5/1/2/3|1:5/7/8/9",
                                 lib::ClusterState("distributor:1 storage:3"), "0:5/1/2/3|1:5/7/8/9", true));
}
...
template <typename Func> void for_each_bucket(const DistributorBucketSpaceRepo& repo, Func&& f) {
...
TEST_F(TopLevelBucketDBUpdaterTest, non_owned_buckets_moved_to_read_only_db_on_ownership_change) {
    ...
    set_cluster_state_bundle(
        lib::ClusterStateBundle(initial_state, {}, false)); // Skip activation step for simplicity
    ...
    for (auto* s : distributor_stripes()) {
        for_each_bucket(mutable_repo(*s), [&](const auto& space, const auto& entry) {
            if (!distributor_bucket_space(entry.getBucketId())
                     .owns_bucket_in_state(pending_state, entry.getBucketId()))
            {
...
TEST_F(TopLevelBucketDBUpdaterTest, buckets_no_longer_available_are_not_moved_to_read_only_database) {
    ...
    trigger_completed_but_not_yet_activated_transition("version:1 distributor:1 storage:4", n_buckets, 4,
                                                       "version:2 distributor:1 storage:4 .0.s:d .1.s:m", n_buckets,
                                                       0);
...
TEST_F(TopLevelBucketDBUpdaterTest, deferred_activated_state_does_not_enable_state_until_activation_received) {
    ...
    ASSERT_NO_FATAL_FAILURE(trigger_completed_but_not_yet_activated_transition(
        "version:1 distributor:2 storage:4", 0, 4, "version:2 distributor:1 storage:4", n_buckets, 4));
...
TEST_F(TopLevelBucketDBUpdaterTest, read_only_db_cleared_once_pending_state_is_activated) {
    ...
    constexpr uint32_t n_buckets = 10;
    ASSERT_NO_FATAL_FAILURE(trigger_completed_but_not_yet_activated_transition(
        "version:1 distributor:1 storage:4", n_buckets, 4, "version:2 distributor:2 storage:4", n_buckets, 0));
    EXPECT_FALSE(activate_cluster_state_version(2));
...
TEST_F(TopLevelBucketDBUpdaterTest, read_only_db_is_populated_even_when_self_is_marked_down) {
    ...
    constexpr uint32_t n_buckets = 10;
    ASSERT_NO_FATAL_FAILURE(trigger_completed_but_not_yet_activated_transition(
        "version:1 distributor:1 storage:4", n_buckets, 4, "version:2 distributor:1 .0.s:d storage:4", n_buckets, 0));
...
TEST_F(TopLevelBucketDBUpdaterTest, activate_cluster_state_request_with_mismatching_version_returns_actual_version) {
    ...
    constexpr uint32_t n_buckets = 10;
    ASSERT_NO_FATAL_FAILURE(trigger_completed_but_not_yet_activated_transition(
        "version:4 distributor:1 storage:4", n_buckets, 4, "version:5 distributor:2 storage:4", n_buckets, 0));
...
TEST_F(TopLevelBucketDBUpdaterTest, pending_cluster_state_getter_is_non_null_only_when_state_is_pending) {
    ...
    lib::ClusterStateBundle initial_bundle(*initial_baseline,
                                           {{FixedBucketSpaces::default_space(), initial_default},
...
TEST_F(TopLevelBucketDBUpdaterTest, node_feature_sets_are_aggregated_from_nodes_and_propagated_to_stripes) {
    ...
    for (uint32_t i = 0; i < _sender.commands().size(); i++) {
        ASSERT_NO_FATAL_FAILURE(
            fake_bucket_reply(state, *_sender.command(i), dummy_buckets_to_return, [i](auto& reply) noexcept {
    ...
    for (auto* s : stripes) {
        ...
        EXPECT_FALSE(
            s->node_supported_features_repo().node_supported_features(0).no_implicit_indexing_of_active_buckets);
...
TEST_F(TopLevelBucketDBUpdaterTest, state_bundle_with_changed_config_but_unchanged_state_sends_to_full_node_set) {
    ...
    lib::ClusterStateBundle initial_bundle(initial_state, {}, {}, lib::DistributionConfigBundle::of(initial_dist),
                                           false);
    set_cluster_state_bundle(initial_bundle);
...
TEST_F(BucketDBUpdaterSnapshotTest, snapshot_returns_unroutable_for_non_owned_bucket_in_current_state) {
    ASSERT_NO_FATAL_FAILURE(trigger_completed_but_not_yet_activated_transition(
        "version:1 distributor:2 storage:4", 0, 4, "version:2 distributor:2 .0.s:d storage:4", 0, 0));
    EXPECT_FALSE(activate_cluster_state_version(2));
...
TEST_F(BucketDBUpdaterSnapshotTest,
    ...
    constexpr uint32_t n_buckets = 10;
    ASSERT_NO_FATAL_FAILURE(trigger_completed_but_not_yet_activated_transition(
        "version:1 distributor:1 storage:4", n_buckets, 4, "version:2 distributor:2 .0.s:d storage:4", 0, 0));
...
TEST_F(BucketDBUpdaterSnapshotTest,
       snapshot_is_unroutable_if_stale_reads_disabled_and_bucket_not_owned_in_pending_state) {
    set_stale_reads_enabled(false);
    constexpr uint32_t n_buckets = 10;
    ASSERT_NO_FATAL_FAILURE(trigger_completed_but_not_yet_activated_transition(
        "version:1 distributor:1 storage:4", n_buckets, 4, "version:2 distributor:2 .0.s:d storage:4", 0, 0));
