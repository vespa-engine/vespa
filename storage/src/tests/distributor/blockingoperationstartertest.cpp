...
struct FakeDistributorStripeOperationContext : DistributorStripeOperationContext {
    ...
    api::Timestamp generate_unique_timestamp() override { abort(); }
    const DistributorBucketSpaceRepo& bucket_space_repo() const noexcept override { abort(); }
    DistributorBucketSpaceRepo& bucket_space_repo() noexcept override { abort(); }
    const DistributorBucketSpaceRepo& read_only_bucket_space_repo() const noexcept override { abort(); }
    DistributorBucketSpaceRepo& read_only_bucket_space_repo() noexcept override { abort(); }
    const DistributorConfiguration& distributor_config() const noexcept override { abort(); }
    // From DistributorStripeOperationContext:
    void update_bucket_database(const document::Bucket&, const BucketCopy&, uint32_t) override { abort(); }
    void update_bucket_database(const document::Bucket&, const std::vector<BucketCopy>&, uint32_t) override {
        abort();
    }
    void remove_node_from_bucket_database(const document::Bucket&, uint16_t) override { abort(); }
    void remove_nodes_from_bucket_database(const document::Bucket&, const std::vector<uint16_t>&) override {
        abort();
    }
    document::BucketId make_split_bit_constrained_bucket_id(const document::DocumentId&) const override { abort(); }
    void recheck_bucket_info(uint16_t, const document::Bucket&) override { abort(); }
    document::BucketId get_sibling(const document::BucketId&) const override { abort(); }
    void send_inline_split_if_bucket_too_large(document::BucketSpace, const BucketDatabase::Entry&,
                                               uint8_t) override {
        abort();
    }
    OperationRoutingSnapshot read_snapshot_for_bucket(const document::Bucket&) const override { abort(); }
    PendingMessageTracker& pending_message_tracker() noexcept override { return _message_tracker; }
    const PendingMessageTracker& pending_message_tracker() const noexcept override { return _message_tracker; }
    bool has_pending_message(uint16_t, const document::Bucket&, uint32_t) const override { abort(); }
    const lib::ClusterState* pending_cluster_state_or_null(const document::BucketSpace&) const override { abort(); }
    const lib::ClusterStateBundle& cluster_state_bundle() const override { abort(); }
    bool storage_node_is_up(document::BucketSpace, uint32_t) const override { abort(); }
    const BucketGcTimeCalculator::BucketIdHasher& bucket_id_hasher() const override { abort(); }
    const NodeSupportedFeaturesRepo& node_supported_features_repo() const noexcept override { abort(); }
    MemoryUsageToken make_memory_usage_token(uint32_t) noexcept override { abort(); }
};
...
void BlockingOperationStarterTest::SetUp() {
