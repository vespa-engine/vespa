...
class MockMaintenancePriorityGenerator : public MaintenancePriorityGenerator {
    MaintenancePriorityAndType prioritize(const document::Bucket&,
...
public:
    explicit MockOperation(const document::Bucket& bucket)
        : _bucket(bucket), _shouldBlock(false), _was_blocked(false), _was_throttled(false) {}

    [[nodiscard]] std::string toString() const override { return _bucket.toString(); }
    ...
    [[nodiscard]] const std::string& getDetailedReason() const override { return _reason; }
    ...
    void setShouldBlock(bool shouldBlock) { _shouldBlock = shouldBlock; }
...
class MockMaintenanceOperationGenerator : public MaintenanceOperationGenerator {
public:
...
class MockOperationStarter : public OperationStarter {
...
public:
    ...
    bool start(const std::shared_ptr<Operation>& operation, Priority priority) override {
        if (_shouldStart) {
            _started << operation->toString() << ", pri " << static_cast<int>(priority) << "\n";
            _operations.push_back(operation);
        }
        return _shouldStart;
    }

    void setShouldStartOperations(bool shouldStart) { _shouldStart = shouldStart; }

    std::vector<Operation::SP>& getOperations() { return _operations; }

    std::string toString() const { return _started.str(); }
};
...
public:
    void allow_operations(bool allow) noexcept { _allow = allow; }
