...
struct OperationTargetResolverTest : Test, DistributorStripeTestUtil {
    ...
    void SetUp() override {
        _repo.reset(new document::DocumentTypeRepo(*config::ConfigGetter<DocumenttypesConfig>::getConfig(
...
#define MY_ASSERT_THAT(id)                                                                                 \
    {                                                                                                      \
        struct MyAsserter : public Asserter {                                                              \
            void assertEqualMsg(std::string t1, OperationTargetList t2, OperationTargetList t3) override { \
                ASSERT_EQ(t2, t3) << t1;                                                                   \
            }                                                                                              \
        };                                                                                                 \
...
struct Asserter {
    ...
    virtual void assertEqualMsg(std::string t1, OperationTargetList t2, OperationTargetList t3) = 0;
};
...
struct TestTargets {
    ...
    TestTargets(const BucketId& id, OperationTargetResolverTest& test, Asserter& asserter)
    ...
    ~TestTargets() {
        ...
        _asserter.assertEqualMsg(all.toString(), _expected, result.createTargets(makeBucketSpace()));
    ...
    TestTargets& sendsTo(const BucketId& id, uint16_t node) {
        _expected.push_back(OperationTarget(makeDocumentBucket(id), lib::Node(lib::NodeType::STORAGE, node), false));
        return *this;
    }
    TestTargets& createsAt(const BucketId& id, uint16_t node) {
        _expected.push_back(OperationTarget(makeDocumentBucket(id), lib::Node(lib::NodeType::STORAGE, node), true));
        return *this;
    }

    static TestTargets createTest(const BucketId& id, OperationTargetResolverTest& test, Asserter& asserter) {
...
BucketInstanceList OperationTargetResolverTest::getInstances(const BucketId& id, bool stripToRedundancy,
                                                             bool symmetry_mode) {
    auto&                       bucketSpaceRepo(operation_context().bucket_space_repo());
    auto&                       distributorBucketSpace(bucketSpaceRepo.get(makeBucketSpace()));
    OperationTargetResolverImpl resolver(distributorBucketSpace, distributorBucketSpace.getBucketDatabase(), 16,
                                         distributorBucketSpace.getDistribution().getRedundancy(), makeBucketSpace());
...
TEST_F(OperationTargetResolverTest, simple) {
    ...
    MY_ASSERT_THAT(BucketId(32, 0)).sendsTo(BucketId(16, 0), 1).sendsTo(BucketId(16, 0), 0);
}
...
TEST_F(OperationTargetResolverTest, choose_ideal_state_when_many_copies) {
    ...
    MY_ASSERT_THAT(BucketId(32, 0)).sendsTo(BucketId(16, 0), 1).sendsTo(BucketId(16, 0), 3);
}
...
TEST_F(OperationTargetResolverTest, legacy_prefers_trusted_over_ideal_state) {
    ...
    // ideal nodes: 1, 3
    MY_ASSERT_THAT(BucketId(32, 0))
        .with_symmetric_replica_selection(false)
...
TEST_F(OperationTargetResolverTest, prefer_ready_over_ideal_state_order) {
    ...
    MY_ASSERT_THAT(BucketId(32, 0)).sendsTo(BucketId(16, 0), 0).sendsTo(BucketId(16, 0), 2);
}
...
TEST_F(OperationTargetResolverTest, prefer_replicas_with_more_docs_over_replicas_with_fewer_docs) {
    ...
    MY_ASSERT_THAT(BucketId(32, 0)).sendsTo(BucketId(16, 0), 2).sendsTo(BucketId(16, 0), 0);
}
...
TEST_F(OperationTargetResolverTest, fall_back_to_active_state_and_db_index_if_all_other_fields_equal) {
    ...
    MY_ASSERT_THAT(BucketId(32, 0)).sendsTo(BucketId(16, 0), 0).sendsTo(BucketId(16, 0), 3);
}
...
TEST_F(OperationTargetResolverTest, choose_highest_split_bucket) {
    ...
    MY_ASSERT_THAT(BucketId(32, 0)).sendsTo(BucketId(17, 0), 1).sendsTo(BucketId(17, 0), 0);
}
...
TEST_F(OperationTargetResolverTest, choose_highest_split_bucket_per_node) {
    ...
    MY_ASSERT_THAT(BucketId(32, 0)).sendsTo(BucketId(17, 0), 0).sendsTo(BucketId(16, 0), 1);
}
...
TEST_F(OperationTargetResolverTest, inconsistent_buckets_are_not_explicitly_created) {
    ...
    MY_ASSERT_THAT(BucketId(32, 0)).sendsTo(BucketId(15, 0), 1).createsAt(BucketId(16, 0), 0);
}
...
TEST_F(OperationTargetResolverTest, no_trusted_or_ideal_state_copy_available) {
    ...
    MY_ASSERT_THAT(BucketId(32, 0)).sendsTo(BucketId(18, 0), 0).sendsTo(BucketId(16, 0), 2);
}
...
TEST_F(OperationTargetResolverTest, no_existing_copies) {
    ...
    MY_ASSERT_THAT(BucketId(32, 0)).createsAt(BucketId(16, 0), 1).createsAt(BucketId(16, 0), 3);
}

TEST_F(OperationTargetResolverTest, count_maintenance_nodes_as_down) {
    setup_stripe(2, 5, "storage:5 .1.s:m distributor:1");

    MY_ASSERT_THAT(BucketId(32, 0)).createsAt(BucketId(16, 0), 3).createsAt(BucketId(16, 0), 2);
}
