...
TestVisitorMessageSession::TestVisitorMessageSession(VisitorThread& t, Visitor& v, const mbus::Error& autoReplyError,
                                                     bool autoReply)
    : _autoReplyError(autoReplyError), _autoReply(autoReply), thread(t), visitor(v), pendingCount(0) {
}

void TestVisitorMessageSession::reply(mbus::Reply::UP rep) {
    {
...
mbus::Result TestVisitorMessageSession::send(std::unique_ptr<documentapi::DocumentMessage> message) {
    std::lock_guard guard(_waitMonitor);
    if (_autoReply) {
        pendingCount++;
...
void TestVisitorMessageSession::waitForMessages(unsigned int msgCount) {
    ...
    while (sentMessages.size() < msgCount) {
        if (clock.getMonotonicTime() > endTime) {
            throw vespalib::IllegalStateException(vespalib::make_string("Timed out waiting for %u messages "
                                                                        "in test visitor session",
                                                                        msgCount),
                                                  VESPA_STRLOC);
        }
