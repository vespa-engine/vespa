...
public:
    Config(size_t heap_size, size_t nprocs, size_t allocs_per_thread, duration alloc_time)
        : _heap_size(heap_size), _nprocs(nprocs), _allocs_per_thread(allocs_per_thread), _alloc_time(alloc_time) {}
...
Allocations::Allocations(const Config& config) : _cfg(config), _mutex(), _total_errors(0), _allocations() {
    _allocations.reserve(config.nprocs() * config.allocs_per_thread());
    std::cout << "Starting memory stress with " << config.nprocs() << " threads and heap size "
              << (config.heap_size() / 1_Mi) << " mb. Allocation size = " << config.alloc_size() << std::endl;
}

Allocations::~Allocations() = default;

size_t Allocations::make_and_load_alloc_per_thread() {
...
size_t Allocations::verify_random_allocation(unsigned int* seed) const {
    const char* alloc;
    {
        std::lock_guard guard(_mutex);
...
size_t Allocations::verify_allocation(const char* alloc) const {
    size_t error_count = 0;
    for (size_t i = 0; i < cfg().alloc_size(); i++) {
        if (alloc[i] != 0) {
            error_count++;
            std::lock_guard guard(log_mutex);
            std::cout << "Thread " << std::this_thread::get_id() << ": Unexpected byte(" << std::hex << int(alloc[i])
                      << ") at " << static_cast<const void*>(alloc + i) << std::endl;
        }
    }
    return error_count;
}
...
FileBackedMemory::FileBackedMemory(const Config& config, std::string dir)
    : _cfg(config), _mutex(), _allocator(dir), _allocations() {
    _allocations.reserve(config.nprocs() * config.allocs_per_thread());
...
size_t FileBackedMemory::make_and_load_alloc_per_thread() {
    PtrAndSize alloc;
    {
        std::lock_guard guard(_mutex);
...
void FileBackedMemory::random_write(unsigned int* seed) {
...
void stress_and_validate_heap(Allocations* allocs) {
    ...
    std::lock_guard guard(log_mutex);
    std::cout << "Thread " << std::this_thread::get_id() << ": Completed " << num_verifications
              << " verifications with " << num_errors << std::endl;
}

void stress_file_backed_memory(FileBackedMemory* mmapped) {
    size_t       num_writes = 0;
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i + 2 <= argc; i += 2) {
        ...
        switch (option) {
        case 'h':
            heapSize = atof(arg) * 1_Gi;
            break;
        case 's':
            swap_dir = arg;
            break;
        case 't':
            runTime = from_s(atof(arg));
            break;
        default:
