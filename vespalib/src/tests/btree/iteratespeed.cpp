...
enum class IterateMethod { FORWARD, BACKWARDS, LAMBDA };

class IterateSpeed {
    template <typename Traits, IterateMethod iterateMethod>
    void workLoop(int loops, bool enableForward, bool enableBackwards, bool enableLambda, int leafSlots);
    void usage();
...
template <typename Traits, IterateMethod iterateMethod>
void IterateSpeed::workLoop(int loops, bool enableForward, bool enableBackwards, bool enableLambda, int leafSlots) {
    ...
    for (int l = 0; l < loops; ++l) {
        ...
        for (size_t innerl = 0; innerl < numInnerLoops; ++innerl) {
            if (iterateMethod == IterateMethod::FORWARD) {
                ...
                tree.getAllocator().foreach_key(tree.getRoot(), [&](int key) { sum += key; });
            }
        }
        ...
        printf("Elapsed time for iterating %ld steps is %8.5f, "
               ...
               numEntries * numInnerLoops, used, iterateMethodName(iterateMethod),
               static_cast<int>(Traits::LEAF_SLOTS), static_cast<int>(Traits::INTERNAL_SLOTS), sum);
        fflush(stdout);
    }
}

void IterateSpeed::usage() {
    printf("iteratspeed "
           "[-F <leafSlots>] "
           "[-b] "
           "[-c <numLoops>] "
...
int IterateSpeed::main(int argc, char** argv) {
    int  c;
    int  loops = 1;
