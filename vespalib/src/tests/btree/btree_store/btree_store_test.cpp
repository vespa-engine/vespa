...
BTreeStoreTest::BTreeStoreTest() : _gen_handler(), _store() {}
...
ChangeWriter::ChangeWriter(uint32_t capacity) : _old_refs() { _old_refs.reserve(capacity); }
...
void BTreeStoreTest::test_compact_sequence(uint32_t sequence_length) {
    auto&                 store = _store;
...
TEST_F(BTreeStoreTest, require_that_short_arrays_are_compacted) { test_compact_sequence(4); }

TEST_F(BTreeStoreTest, require_that_btree_roots_are_compacted) { test_compact_sequence(10); }
