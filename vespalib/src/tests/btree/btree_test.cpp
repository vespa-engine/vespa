...
void apply_tree_mutations(TreeStore& s, EntryRef& root, const AdditionsVector& additions,
    ...
    for (size_t i = 0; i < before_iterators.size(); ++i) {
        for (size_t j = 0; j < after_iterators.size(); ++j) {
            ...
            if (i <= j) {
                ...
                before_iterators[i].foreach_key_range(after_iterators[j],
                                                      [&validate_keys](int key) { validate_keys(key); });
                EXPECT_EQ(j, validate_keys.curr_pos());
            }
            if (j <= i) {
                KeyRangeValidator validate_keys(before_list, j);
                EXPECT_EQ(j, validate_keys.curr_pos());
                after_iterators[j].foreach_key_range(before_iterators[i],
                                                     [&validate_keys](int key) { validate_keys(key); });
                EXPECT_EQ(i, validate_keys.curr_pos());
            }
        }
