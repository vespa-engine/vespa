...
enum class ScanMethod { ITERATOR, FUNCTOR };

class ScanSpeed {
    template <typename Traits> void work_loop(ScanMethod scan_method);
...
template <typename Tree> class ScanTree : public ScanOnce {
protected:
    const Tree& _tree;
    int         _startval;
    int         _endval;
...
public:
    ScanTree(const Tree& tree, int startval, int endval) : _tree(tree), _startval(startval), _endval(endval) {}
    ~ScanTree() override {}
};

template <typename Tree> class ScanWithIterator : public ScanTree<Tree> {
public:
    ScanWithIterator(const Tree& tree, int startval, int endval) : ScanTree<Tree>(tree, startval, endval) {}
    ~ScanWithIterator() override = default;
    void operator()(std::vector<bool>& bv) override;
};

template <typename Tree> void ScanWithIterator<Tree>::operator()(std::vector<bool>& bv) {
...
template <typename Tree> class ScanWithFunctor : public ScanTree<Tree> {

public:
    ScanWithFunctor(const Tree& tree, int startval, int endval) : ScanTree<Tree>(tree, startval, endval) {}
    ~ScanWithFunctor() override = default;
    void operator()(std::vector<bool>& bv) override;
};

template <typename Tree> void ScanWithFunctor<Tree>::operator()(std::vector<bool>& bv) {
    using ConstIterator = typename Tree::ConstIterator;
...
template <typename Traits> void ScanSpeed::work_loop(ScanMethod scan_method) {
    ...
    printf("Elapsed time for scanning %ld entries is %8.5f, "
           ...
           numEntries * numInnerLoops, used, scan_method_name(scan_method), static_cast<int>(Traits::LEAF_SLOTS),
