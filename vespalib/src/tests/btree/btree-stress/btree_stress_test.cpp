...
EntryRef RealIntStore::move(EntryRef ref) { return add(get(ref)); }
...
template <typename Params> void Fixture<Params>::readWork() { readWork(std::numeric_limits<uint32_t>::max()); }
...
TYPED_TEST(BTreeStressTest, basic_lower_bound) { this->basic_lower_bound(); }
