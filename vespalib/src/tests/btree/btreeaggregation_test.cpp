...
int32_t toVal(uint32_t key) {
    return key + 1000;
}

int32_t toHighVal(uint32_t key) {
    return toVal(key) + 1000;
}

int32_t toLowVal(uint32_t key) {
...
template <typename TreeStore>
bool BTreeAggregationTest::assertAggregated(const MockTree& m, const TreeStore& s, EntryRef ref,
...
template <typename Tree> void populateLeafNode(Tree& t) {
    populateTree(t, 4, 2);
}
