...
int32_t toVal(uint32_t key) { return key + 1000; }

int32_t toHighVal(uint32_t key) { return toVal(key) + 1000; }

int32_t toLowVal(uint32_t key) { return toVal(key) - 1000000; }
...
typedef BTree<MyKey, int32_t, btree::MinMaxAggregated, MyComp, MyTraits, MinMaxAggrCalc>                MyTree;
typedef BTreeStore<MyKey, int32_t, btree::MinMaxAggregated, MyComp, BTreeDefaultTraits, MinMaxAggrCalc> MyTreeStore;
...
public:
    ...
    void insert(uint32_t key, int32_t val) {
        erase(key);
        _tree[key] = val;
...
MockTree::MockTree() : _tree(), _rtree() {}
MockTree::~MockTree() {}

template <typename ManagerType> void freezeTree(GenerationHandler& g, ManagerType& m) {
    m.freeze();
...
template <typename ManagerType> void cleanup(GenerationHandler& g, ManagerType& m) { freezeTree(g, m); }
...
protected:
    ...
    template <typename Tree> bool assertAggregated(const MockTree& m, const Tree& t, const std::string& label);

    template <typename TreeStore>
    bool assertAggregated(const MockTree& m, const TreeStore& s, EntryRef ref, const std::string& label);
...
template <typename Tree> bool BTreeAggregationTest::assertTree(const std::string& exp, const Tree& t) {
    std::stringstream                                                       ss;
...
bool BTreeAggregationTest::assertAggregated(const MockTree& m, const Tree& t, const std::string& label) {
    SCOPED_TRACE(label);
...
template <typename TreeStore>
bool BTreeAggregationTest::assertAggregated(
    const MockTree& m, const TreeStore& s, EntryRef ref, const std::string& label) {
    SCOPED_TRACE(label);
...
TEST_F(BTreeAggregationTest, require_that_tree_insert_works) {
    { // multi level node split
        ...
        EXPECT_TRUE(assertTree(
            ...
            "{25:125,27:127,29:129,31:131[min=125,max=131]}}",
            t));
        t.insert(33, 133);
        EXPECT_TRUE(assertTree(
            "{{23,33[min=101,max=133]}} -> "
            "{{7,15,23[min=101,max=123]},{29,33[min=125,max=133]}} -> "
            "{{1:101,3:103,5:105,7:107[min=101,max=107]},"
            "{9:109,11:111,13:113,15:115[min=109,max=115]},"
            "{17:117,19:119,21:121,23:123[min=117,max=123]},"
            "{25:125,27:127,29:129[min=125,max=129]},"
            "{31:131,33:133[min=131,max=133]}}",
            t));
    }
    { // give to left node to avoid split
        ...
        EXPECT_TRUE(assertTree("{{9,15[min=101,max=115]}} -> "
                               ...
                               "{10:110,11:111,13:113,15:115[min=110,max=115]}}",
                               t));
    }
    { // give to left node to avoid split, and move to left node
        ...
        EXPECT_TRUE(assertTree("{{9,15[min=101,max=115]}} -> "
                               ...
                               "{11:111,13:113,15:115[min=111,max=115]}}",
                               t));
    }
    ...
    { // give to right node to avoid split and move to right node
        ...
        EXPECT_TRUE(assertTree("{{11,17[min=101,max=117]}} -> "
                               ...
                               "{13:113,15:115,17:117[min=113,max=117]}}",
                               t));
        t.insert(10, 110);
        EXPECT_TRUE(assertTree("{{7,17[min=101,max=117]}} -> "
                               "{{1:101,3:103,5:105,7:107[min=101,max=107]},"
                               "{9:109,10:110,11:111,13:113,15:115,17:117[min=109,max=117]}}",
                               t));
    }
}
...
TEST_F(BTreeAggregationTest, require_that_node_steal_works) {
    typedef BTree<MyKey, int32_t, btree::MinMaxAggregated, MyComp, BTreeStealTraits, MinMaxAggrCalc> MyStealTree;
    { // steal all from left
        MyStealTree t;
        t.insert(10, 110);
        t.insert(20, 120);
...
void BTreeAggregationTest::buildSubTree(const std::vector<LeafPair>& sub, size_t numEntries) {
    GenerationHandler g;
    MyTree            tree;
...
void insert(MyTreeStore& s, EntryRef& root, MyTreeStore::KeyDataType addition) {
    s.apply(root, &addition, &addition + 1, nullptr, nullptr);
}

void remove(MyTreeStore& s, EntryRef& root, MyTreeStore::KeyType removal) {
