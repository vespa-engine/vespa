...
struct AsyncService {
    ...
    auto                         get_value() {
        return wait_for<int>([&](WaitingFor<int> handle) { pending.push_back(std::move(handle)); });
    }
};
...
struct AsyncVoidService {
    ...
    auto               get_value() {
        return wait_for<int>([&](WaitingFor<int> handle) { pending.push_back(handle.release()); });
    }
};
...
template <typename Service> Lazy<int> wait_for_value(Service& service) {
    int value = co_await service.get_value();
    co_return value;
}

template <typename T> Lazy<T> wait_for_fun(auto&& fun) {
