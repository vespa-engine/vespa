...
template <std::ranges::input_range T> size_t calc_sum(T&& values) {
    size_t sum = 0;
...
struct MySeq : Sequence<size_t> {
    ...
    size_t                     pos;
    MySeq(const std::vector<size_t>& data_in) : data(data_in), pos(0) {}
    bool   valid() const override { return pos < data.size(); }
    size_t get() const override { return data[pos]; }
    void   next() override { ++pos; }
};

size_t calc_sum_direct(const std::vector<size_t>& data) { return calc_sum(data); }
...
size_t calc_sum_generator(const std::vector<size_t>& data) { return calc_sum(gen_values(data)); }
...
size_t calc_sum_generator_noinline(const std::vector<size_t>& data) { return calc_sum(gen_values_noinline(data)); }
