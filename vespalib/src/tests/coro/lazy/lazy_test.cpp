...
Lazy<int> make_lazy(int value) { co_return value; }
...
Lazy<int> async_sum(Lazy<int> a, Lazy<int> b) { co_return (co_await a + co_await b); }

Lazy<std::unique_ptr<int>> move_only_int() { co_return std::make_unique<int>(123); }
...
template <typename T> Lazy<T> forward_value(Lazy<T> value) { co_return co_await std::move(value); }

template <typename T> Lazy<std::pair<bool, T>> try_schedule_on(Executor& executor, Lazy<T> value) {
...
template <typename T> Lazy<T> schedule_on(Executor& executor, Lazy<T> value) {
    std::cerr << "switching from thread " << std::this_thread::get_id() << std::endl;
...
struct Refs {
    Gate&                           gate;
    Received<std::unique_ptr<int>>& result;
    Refs(Gate& gate_in, Received<std::unique_ptr<int>>& result_in) : gate(gate_in), result(result_in) {}
};
