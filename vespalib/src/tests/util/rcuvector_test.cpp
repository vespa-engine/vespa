...
GrowStrategy growStrategy(size_t initial, float factor, size_t delta, size_t minimal = 0) {
...
TEST(RcuVectorTest, generation_handling) {
    ...
    v.setGeneration(Generation(1));
    v.push_back(30);
    EXPECT_EQ(8u, v.getMemoryUsage().allocatedBytesOnHold());
    v.push_back(40); // new array
    EXPECT_EQ(24u, v.getMemoryUsage().allocatedBytesOnHold());

    v.setGeneration(Generation(2));
    v.push_back(50);
    v.reclaim_memory(Generation(3));
    EXPECT_EQ(0u, v.getMemoryUsage().allocatedBytesOnHold());
...
struct ShrinkFixture {
    ...
    ShrinkFixture()
        : g(),
          ...
          vec(growStrategy(initial_capacity, 0.50, 0), g, alloc::Alloc::allocMMap()),
          oldPtr() {
    ...
    void assertOldEqualNewBuffer() { EXPECT_EQ(oldPtr, &vec[0]); }
    void assertEmptyHoldList() { EXPECT_EQ(0u, g.get_held_bytes()); }
...
Fixture::Fixture() : FixtureBase(), arr(growStrategy(16, 1.0, 0), g, initial_alloc) {
    arr.reserve(100);
}
...
void StressFixture::sync() {
    writer.sync();
    readers.sync();
}

void StressFixture::read_work() {
...
void StressFixture::write_work(uint32_t cnt) {
...
void StressFixture::run_test(uint32_t cnt, uint32_t num_readers) {
