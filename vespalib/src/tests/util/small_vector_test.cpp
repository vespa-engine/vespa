...
struct Alive {
    uint32_t& cnt;
    uint32_t  id;
    Alive(uint32_t& cnt_in, uint32_t id_in) noexcept : cnt(cnt_in), id(id_in) { ++cnt; }
    ~Alive() { --cnt; }
};
...
template <typename T, size_t N, size_t M> void verify_eq(const SmallVector<T, N>& a, const SmallVector<T, M>& b) {
    EXPECT_TRUE(a == b);
    EXPECT_TRUE(b == a);
}

template <typename T, size_t N, size_t M> void verify_not_eq(const SmallVector<T, N>& a, const SmallVector<T, M>& b) {
    EXPECT_FALSE(a == b);
    EXPECT_FALSE(b == a);
}
...
// to check "back() const"
template <size_t N> int last_value_of(const SmallVector<int, N>& v) { return v.back(); }
