...
TEST(NexusTest, run_void_tasks) {
    ...
    auto                task = [&value](Nexus&) { value.fetch_add(1, std::memory_order_relaxed); };
    Nexus::run(10, task);
    EXPECT_EQ(value, 10);
    Nexus::run(10, task);
    EXPECT_EQ(value, 20);
...
TEST(NexusTest, run_value_tasks_merge_results) {
    std::atomic<size_t> value = 0;
    auto                task = [&value](Nexus&) { return value.fetch_add(1, std::memory_order_relaxed) + 1; };
...
TEST(NexusTest, exception_is_captured_and_propagated) {
    ...
    auto   task = [&](Nexus&) { throw(std::runtime_error(std::format("failed"))); };
...
TEST(NexusTest, return_unwinding_destroys_nexus_barrier) {
    ...
    VESPA_EXPECT_EXCEPTION(Nexus::run(num_threads, task), IllegalStateException,
                           "trying to use destroyed rendezvous");
}
