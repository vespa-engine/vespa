...
size_t active_enums() { return SharedStringRepo::stats().active_entries; }

bool will_reclaim() { return SharedStringRepo::will_reclaim(); }
...
std::vector<std::string> copy_strings(const std::vector<std::string>& strings) { return strings; }
...
template <typename T> void verify_equal(const std::vector<T>& a, const std::vector<T>& b) {
...
struct Fixture {
    ...
    explicit Fixture(size_t num_threads)
        : avg(num_threads),
          vote(num_threads),
          work(make_strings(work_size)),
          direct_work(make_direct_strings(work_size)),
    ...
    [[nodiscard]] bool         has_budget() const { return to_s(steady_clock::now() - start_time) < budget; }
    template <typename F> void measure_task(const std::string& tag, bool is_master, F&& task) {
...
void verifySelfAssignment(Handle& a, const Handle& b) { a = b; }
...
TEST_P(SharedStringRepoBenchmarkTest, benchmark_with_threads) {
    ...
    auto    task = [&](Nexus& ctx) { f1.benchmark(ctx.thread_id() == 0); };
    Nexus::run(num_threads, task);
}

INSTANTIATE_TEST_SUITE_P(, SharedStringRepoBenchmarkTest, ::testing::Values(1, 2, 4, 8, 16, 32, 64),
                         [](const ::testing::TestParamInfo<size_t>& pi) { return std::format("{}", pi.param); });
