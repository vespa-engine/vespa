...
std::optional<uint32_t> do_calculate(
    std::string_view left, std::string_view right, uint32_t threshold, LevenshteinDfa::Casing casing,
    LevenshteinDfa::DfaType dfa_type, LevenshteinDfa::Matching matching) {
...
struct LevenshteinDfaTest : TestWithParam<CasingAndDfaType> {
    ...
    static std::optional<uint32_t> prefix_calculate(
        std::string_view left, std::string_view right, uint32_t threshold) {
...
INSTANTIATE_TEST_SUITE_P(
    AllCasingAndDfaTypes, LevenshteinDfaTest,
    Combine(
        Values(LevenshteinDfa::Casing::Uncased, LevenshteinDfa::Casing::Cased),
        Values(LevenshteinDfa::DfaType::Explicit, LevenshteinDfa::DfaType::Implicit, LevenshteinDfa::DfaType::Table)),
...
void test_dfa_successor(const LevenshteinDfa& dfa, std::string_view source, std::string_view expected_successor,
    ...
    if (m.matches()) {
        FAIL() << "Expected '" << source << "' to emit a successor, but it "
               << "matched with " << static_cast<uint32_t>(m.edits()) << " edits (of max "
               << static_cast<uint32_t>(m.max_edits()) << " edits)";
    }
...
TEST_P(LevenshteinDfaTest, can_generate_successors_to_mismatching_source_strings) {
    ...
    test_dfa_successor(dfa,
                       "\xf0\x9f\xa4\xa9"
                       "abc", // <starry eyed emoji>abc
                       "\xf0\x9f\xa4\xa9"
                       "food"); // <starry eyed emoji>food
...
struct LevenshteinDfaCasingTest : TestWithParam<LevenshteinDfa::DfaType> {
    ...
    static std::optional<uint32_t> calculate_cased(
        std::string_view left, std::string_view right, uint32_t threshold) {
        return do_calculate(left, right, threshold, LevenshteinDfa::Casing::Cased, dfa_type());
    }

    static std::optional<uint32_t> calculate_uncased(
        std::string_view left, std::string_view right, uint32_t threshold) {
...
INSTANTIATE_TEST_SUITE_P(
    AllDfaTypes, LevenshteinDfaCasingTest,
    Values(LevenshteinDfa::DfaType::Explicit, LevenshteinDfa::DfaType::Implicit, LevenshteinDfa::DfaType::Table),
...
template <std::unsigned_integral T> std::string bits_to_str(T v) {
...
using CasingAndDfaTypeAndMaxEdits =
    std::tuple<LevenshteinDfa::Casing, LevenshteinDfa::DfaType, LevenshteinDfa::Matching, uint32_t>;
...
struct LevenshteinDfaSuccessorTest : TestWithParam<CasingAndDfaTypeAndMaxEdits> {
    ...
    static std::string stringify_params(const TestParamInfo<ParamType>& info) {
        std::ostringstream ss;
        ss << std::get<0>(info.param) << '_' << std::get<1>(info.param) << '_' << std::get<2>(info.param) << '_'
...
INSTANTIATE_TEST_SUITE_P(
    SupportedMaxEdits, LevenshteinDfaSuccessorTest,
    Combine(
        Values(LevenshteinDfa::Casing::Uncased, LevenshteinDfa::Casing::Cased),
        Values(LevenshteinDfa::DfaType::Explicit, LevenshteinDfa::DfaType::Implicit, LevenshteinDfa::DfaType::Table),
        Values(LevenshteinDfa::Matching::FullString, LevenshteinDfa::Matching::Prefix), Values(1, 2)),
...
void explore(const DfaSteppingBase<FixedMaxEditDistanceTraits<MaxEdits>>&                     stepper,
    ...
    ASSERT_EQ(stepper.can_match(stepper.step(in_state, WILDCARD)), stepper.can_wildcard_step(in_state));
...
using StateStepperTypes =
    Types<DfaSteppingBase<FixedMaxEditDistanceTraits<1>>, DfaSteppingBase<FixedMaxEditDistanceTraits<2>>>;

template <typename SteppingBase> struct LevenshteinSparseStateTest : Test {};
...
template <typename T> void do_not_optimize_away(T&& t) noexcept {
    asm volatile("" : : "m"(t) : "memory"); // Clobber the value to avoid losing it to compiler optimizations
}

enum class BenchmarkType { DfaExplicit, DfaImplicit, Legacy };

const char* to_s(BenchmarkType t) noexcept {
    switch (t) {
    case BenchmarkType::DfaExplicit:
        return "DfaExplicit";
    case BenchmarkType::DfaImplicit:
        return "DfaImplicit";
    case BenchmarkType::Legacy:
        return "Legacy";
    }
    abort();
}

[[nodiscard]] bool benchmarking_enabled() noexcept { return !benchmark_dictionary.empty(); }

[[nodiscard]] std::vector<uint32_t> string_lengths() { return {2, 8, 16, 64, 256, 1024, 1024 * 16, 1024 * 64}; }
...
INSTANTIATE_TEST_SUITE_P(
    AllDfaTypes, LevenshteinBenchmarkTest,
    Combine(Values(BenchmarkType::DfaExplicit, BenchmarkType::DfaImplicit, BenchmarkType::Legacy),
            Values(LevenshteinDfa::Casing::Cased, LevenshteinDfa::Casing::Uncased)),
        ...
        for (uint32_t sz : string_lengths()) {
            ...
            if (type == BenchmarkType::DfaExplicit || type == BenchmarkType::DfaImplicit) {
                ...
                min_time_s = BenchmarkTimer::benchmark(
                    [&] {
                        auto str_u32 =
                            utf8_string_to_utf32(str); // Must be done per term, so included in benchmark body
...
TEST(LevenshteinExplicitDfaBenchmarkTest, benchmark_explicit_dfa_construction) {
    ...
    const auto casing =
        LevenshteinDfa::Casing::Cased; // For building, casing only affects initial string normalization
    using vespalib::BenchmarkTimer;
    for (uint8_t k : {1, 2}) {
        for (uint32_t sz : string_lengths()) {
            std::string str = repeated_string("aBcDeFgHiJ", sz);
            ...
            if (type == BenchmarkType::DfaExplicit || type == BenchmarkType::DfaImplicit) {
                ...
                min_time_s = BenchmarkTimer::benchmark(
                    [&] {
                        ...
                        for (const auto& line : dict) {
                            std::vector<uint32_t> line_u32 =
                                ((casing() == LevenshteinDfa::Casing::Uncased)
