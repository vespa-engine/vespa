...
template <typename T> class RoundRobinAllocator {
public:
    ...
    RoundRobinAllocator() {}
    template <typename _Tp1> RoundRobinAllocator(const RoundRobinAllocator<_Tp1>&) noexcept {}
...
template <typename T> size_t RoundRobinAllocator<T>::_r = 0;
template <typename T> size_t RoundRobinAllocator<T>::_w = 0;
template <typename T> size_t RoundRobinAllocator<T>::_sz = 10000000;
template <typename T> T*     RoundRobinAllocator<T>::_memory = static_cast<T*>(malloc(10000000 * sizeof(T)));
...
struct IndirectCmp {
    bool operator()(const Slot* s1, const Slot* s2) noexcept { return s1->cmp(*s2) < 0; }
};
...
void* runBenchMark(const std::vector<Slot*>* indirectSlotVector) {
    int uniq(0);
    switch (_type) {
    case 'm':
        uniq = benchMap(*indirectSlotVector);
        break;
    case 'M':
        uniq = benchMapIntelligent(*indirectSlotVector);
        break;
    case 'v':
        uniq = benchSort(*indirectSlotVector);
        break;
    case 'h':
        uniq = benchHashStl(*indirectSlotVector);
        break;
    case 'H':
        uniq = benchHashStlIntelligent(*indirectSlotVector);
        break;
    case 'a':
        uniq = benchHashStlFastAlloc(*indirectSlotVector);
        break;
    case 'g':
        uniq = benchHashVespaLib(*indirectSlotVector);
        break;
    case 'G':
        uniq = benchHashVespaLibIntelligent(*indirectSlotVector);
        break;
    case 'J':
        uniq = benchHashVespaLibIntelligentAndFast(*indirectSlotVector);
        break;
    default:
        break;
    }
