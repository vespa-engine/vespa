...
template <typename S> void fill(S& s, size_t count) {
    for (size_t i(0); i < count; i++) {
        s.insert(i);
    }
}

template <typename M> void fillM(M& m, size_t count) {
    for (size_t i(0); i < count; i++) {
        m[i] = i;
    }
}

template <typename S> size_t lookup_bench(S& s, size_t count, size_t rep) {
...
template <typename S> size_t bench(S& set, size_t sz, size_t numLookups) {
    fill(set, sz);
    return lookup_bench(set, sz, numLookups / sz);
}

template <typename M> size_t benchM(M& map, size_t sz, size_t numLookups) {
    fillM(map, sz);
...
size_t benchHashVespaLib2(size_t sz, size_t numLookups) {
    vespalib::hash_set<uint32_t, vespalib::hash<uint32_t>, std::equal_to<uint32_t>,
...
size_t benchHashMapVespaLib2(size_t sz, size_t numLookups) {
    vespalib::hash_map<uint32_t, uint32_t, vespalib::hash<uint32_t>, std::equal_to<uint32_t>,
                       vespalib::hashtable_base::and_modulator>
        set(3 * sz);
    return benchM(set, sz, numLookups);
}
...
int main(int argc, char* argv[]) {
    ...
    switch (type) {
    case 'm':
        found = benchMap(count, rep);
        break;
    case 'h':
        found = benchHashStl(count, rep);
        break;
    case 'g':
        found = benchHashVespaLib(count, rep);
        break;
    case 'G':
        found = benchHashVespaLib2(count, rep);
        break;
    case 'k':
        found = benchHashMapVespaLib(count, rep);
        break;
    case 'K':
        found = benchHashMapVespaLib2(count, rep);
        break;
    case 'x':
        found = benchXXHash32(count, rep);
        break;
    case 'X':
        found = benchXXHash64(count, rep);
        break;
    case 'l':
        found = benchLegacyHash(count, rep);
        break;
    default:
