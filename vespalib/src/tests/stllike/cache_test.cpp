...
TEST_F(LfuCacheTest, enabled_lfu_gates_probationary_segment_displacing) {
    ...
    cache.enable_frequency_sketch(3);
...
TEST_F(LfuCacheTest, enabled_lfu_gates_protected_segment_displacing) {
    cache<CacheParam<P, B, zero<uint32_t>, size<std::string>>> cache(m, -1, -1);
    cache.maxElements(4, 2);
    cache.enable_frequency_sketch(4);
    ASSERT_EQ(cache.read(1), "a");
    ASSERT_EQ(cache.read(2), "b");
    ASSERT_EQ(cache.read(3), "c");
    ASSERT_EQ(cache.read(4), "d");
    ...
    // Both 1+2 are trending higher on X than 3+4. Touching 3+4 will bump them in the
    // head of the LRU, but not into the protected segment (yet).
    ASSERT_EQ(cache.read(3), "c");
...
TEST_F(LfuCacheTest, enabled_lfu_gates_probationary_inserts_on_write_through) {
    cache<CacheParam<P, B, zero<uint32_t>, size<std::string>>> cache(m, -1, 0);
    cache.maxElements(2, 0);
    cache.enable_frequency_sketch(2);
...
TEST_F(LfuCacheTest, lfu_gating_considers_capacity_bytes) {
    ...
    cache.enable_frequency_sketch(10);
    cache.write(100, "foo");
