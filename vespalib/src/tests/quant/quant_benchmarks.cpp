...
// Quantized vector vs. quantized vector dot product with args <bits, dimensions>
template <typename... Args>
void BM_quantized_lhs_rhs_dot_product(benchmark::State& state, Args&&... args) {
    const auto    my_args = std::make_tuple(std::move(args)...);
    const uint8_t bits = std::get<0>(my_args);
    const size_t  dims = state.range();
    ...
    EdenQuantizer          quant(dims, bits, prng());
    std::vector<float>     v(dims);
    std::vector<uint8_t>   q_v_lhs(quant.quantized_size());
    std::vector<uint8_t>   q_v_rhs(quant.quantized_size());

    fill_random(v, prng);
    quant.quantize(v, q_v_lhs, QuantMode::InnerProduct);
    fill_random(v, prng);
    quant.quantize(v, q_v_rhs, QuantMode::InnerProduct);

    for (auto _ : state) {
        float dot = quant.quantized_lhs_rhs_dot_product(q_v_lhs, q_v_rhs);
        benchmark::DoNotOptimize(dot);
    }
}
...
BENCHMARK_CAPTURE(BM_quantized_lhs_rhs_dot_product, 1_bit, 1)->Range(8, 8192);
BENCHMARK_CAPTURE(BM_quantized_lhs_rhs_dot_product, 2_bits, 2)->Range(8, 8192);
BENCHMARK_CAPTURE(BM_quantized_lhs_rhs_dot_product, 3_bits, 3)->Range(8, 8192);
BENCHMARK_CAPTURE(BM_quantized_lhs_rhs_dot_product, 4_bits, 4)->Range(8, 8192);
