...
public:
    ...
    bool   operator==(const A& rhs) const override { return dynamic_cast<const Primitive&>(rhs).value() == value(); }
    void   assign(const A& rhs) override { _v = dynamic_cast<const Primitive&>(rhs).value(); }
    void   print(std::ostream& os) const override { os << _v; }
...
public:
    ...
    size_t   value() const { return _v; }
    bool     operator==(const A& rhs) const override { return dynamic_cast<const Complex&>(rhs).value() == value(); }
    Complex* clone() const override { return new Complex(_v); }
    void     print(std::ostream& os) const override { os << _v; }
...
template <typename T> void verifyArray(IArrayT<A>& array) {
