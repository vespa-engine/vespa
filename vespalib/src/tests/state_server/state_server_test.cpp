...
std::string getFull(int port, const std::string& path) {
    return getPage(port, path, "-D -");
}

std::pair<std::string, std::string> get_body_and_content_type(const JsonGetHandler& handler, const std::string& host,
...
std::string get_json(const JsonGetHandler& handler, const std::string& host, const std::string& path,
                     const std::map<std::string, std::string>& params) {
...
struct DummyHandler : JsonGetHandler {
    ...
    Response get(const std::string&, const std::string&, const std::map<std::string, std::string>&,
...
struct EchoHost : JsonGetHandler {
    ~EchoHost() override;
    Response get(const std::string& host, const std::string&, const std::map<std::string, std::string>&,
                 const net::ConnectionAuthContext&) const override {
...
TEST(StateServerTest, require_that_host_is_passed_correctly) {
    ...
    EXPECT_EQ(silly_result,
              run_cmd(make_string("curl -s http://localhost:%d/my/path -H \"Host: sillyserver\"", f2.port())));
...
TEST(StateServerTest, require_that_top_level_urls_are_generated_correctly) {
    ...
    EXPECT_EQ(get_json(f4, host_tag, root_path, empty_params), get_json(f4, host_tag, short_root_path, empty_params));
}
...
TEST(StateServerTest, require_that_health_resource_works_as_expected) {
    ...
    SimpleComponentConfigProducer f3;
    StateApi                      f4(f1, f2, f3);
    EXPECT_EQ("{\"status\":{\"code\":\"up\"}}", get_json(f4, host_tag, health_path, empty_params));
...
TEST(StateServerTest, require_that_metrics_resource_works_as_expected) {
    ...
    SimpleComponentConfigProducer f3;
    StateApi                      f4(f1, f2, f3);
    EXPECT_EQ("{\"status\":{\"code\":\"up\"}}", get_json(f4, host_tag, metrics_path, empty_params));
    f1.setFailed("FAIL MSG");
    EXPECT_EQ("{\"status\":{\"code\":\"down\",\"message\":\"FAIL MSG\"}}",
...
TEST(StateServerTest, require_that_config_resource_works_as_expected) {
    ...
    SimpleComponentConfigProducer f3;
    StateApi                      f4(f1, f2, f3);
    EXPECT_EQ("{\"config\":{}}", get_json(f4, host_tag, config_path, empty_params));
    ...
    EXPECT_EQ(
        "{\"config\":{\"generation\":4,\"bar\":{\"generation\":4,\"message\":\"error\"},\"foo\":{\"generation\":4}}}",
        get_json(f4, host_tag, config_path, empty_params));
    f3.removeConfig("bar");
...
TEST(StateServerTest, version_resource_yields_current_version_number) {
    ...
    EXPECT_EQ(os.str(), get_json(f4, host_tag, version_path, empty_params));
}
...
TEST(StateServerTest, require_that_state_api_also_can_return_total_metric) {
    ...
    EXPECT_EQ(R"({"foo":"bar"})", get_json(f4, host_tag, total_metrics_path, empty_params));
...
TEST(StateServerTest, require_that_empty_v1_metrics_consumer_defaults_to_statereporter) {
    ...
    EXPECT_EQ(R"({"status":{"code":"up"},"metrics":["JSON", "statereporter"]})",
              get_json(f4, host_tag, metrics_path, empty_params));
    EXPECT_EQ(R"(["Prometheus", "statereporter"])", get_json(f4, host_tag, metrics_path, {{"format", "prometheus"}}));
}
...
TEST(StateServerTest, require_that_metrics_consumer_is_passed_correctly) {
    ...
    EXPECT_EQ(R"({"status":{"code":"up"},"metrics":["JSON", "ME"]})",
              get_json(f4, host_tag, metrics_path, my_params));
