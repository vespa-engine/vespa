...
public:
    MyTask(Gate& entryGate, CountDownLatch& exitLatch) : _entryGate(entryGate), _exitLatch(exitLatch) {}
    void run() override {
...
void blockedExecute(BlockingThreadStackExecutor* executor, Gate* workersEntryGate, CountDownLatch* workersExitLatch,
                    Gate* exitGate) {
...
struct Fixture {
    ...
    Fixture(uint32_t taskLimit, uint32_t tasksToWaitFor)
        : executor(1, taskLimit), workersEntryGate(), workersExitLatch(tasksToWaitFor), blockedExecuteGate() {}
    ~Fixture();
    ...
    void updateTaskLimit(uint32_t taskLimit) { executor.setTaskLimit(taskLimit); }
    void openForWorkers() { workersEntryGate.countDown(); }
    void waitForWorkers() { workersExitLatch.await(waitTime); }
    ...
    ThreadUP blockedExecuteThread() {
        return std::make_unique<std::thread>(blockedExecute, &executor, &workersEntryGate, &workersExitLatch,
                                             &blockedExecuteGate);
    }
