...
void blockedExecute(
    BlockingThreadStackExecutor* executor, Gate* workersEntryGate, CountDownLatch* workersExitLatch, Gate* exitGate) {
...
struct Fixture {
    ...
    ThreadUP blockedExecuteThread() {
        return std::make_unique<std::thread>(
            blockedExecute, &executor, &workersEntryGate, &workersExitLatch, &blockedExecuteGate);
    }
