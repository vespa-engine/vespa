...
MyTask::~MyTask() { deleteCnt.fetch_add(1); }
...
struct MyState {
    ...
    bool                      checked;
    MyState() : gate(), latch(10), executor(NUM_THREADS, 20), checked(false) { MyTask::resetStats(); }
    ~MyState();
    ...
    MyState& check(uint32_t expect_rejected, uint32_t expect_queue, uint32_t expect_running,
