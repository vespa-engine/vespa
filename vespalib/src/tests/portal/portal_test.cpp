...
std::string do_http(
    int port, CryptoEngine::SP crypto, const std::string& method, const std::string& uri, bool send_host = true) {
    ...
    std::string http_req = vespalib::make_string(
        "%s %s HTTP/1.1\r\n"
        "My-Header: my value\r\n"
        "%s"
        "\r\n",
...
std::string make_expected_response(const std::string& content_type, const std::string& content) {
    return vespalib::make_string(
        ...
        "\r\n"
        "%s",
        content_type.c_str(), content.size(), content.c_str());
}
...
std::vector<Encryption> crypto_list = {
    {"no encryption", null_crypto()},
...
struct MyGetHandler : public Portal::GetHandler {
    ...
    template <typename F> MyGetHandler(F&& f) : fun(std::move(f)) {}
    void get(Portal::GetRequest request) override { fun(std::move(request)); }
...
TEST(PortalTest, require_that_GET_handler_can_return_HTTP_error) {
    ...
    MyGetHandler handler([](Portal::GetRequest request) { request.respond_with_error(123, "My Error"); });
    auto         bound = portal->bind(path, handler);
...
TEST(PortalTest, require_that_the_handler_with_the_longest_matching_prefix_is_selected) {
    ...
    EXPECT_EQ(fetch(portal->listen_port(), null_crypto(), "/foo/bar"),
              make_expected_response("text/plain", "handler2"));
    EXPECT_EQ(fetch(portal->listen_port(), null_crypto(), "/foo/bar/baz"),
              make_expected_response("text/plain", "handler3"));
    bound3.reset();
    EXPECT_EQ(fetch(portal->listen_port(), null_crypto(), "/foo/bar/baz"),
              make_expected_response("text/plain", "handler2"));
    bound2.reset();
    EXPECT_EQ(fetch(portal->listen_port(), null_crypto(), "/foo/bar/baz"),
              make_expected_response("text/plain", "handler1"));
}
