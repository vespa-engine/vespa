...
TEST(BenchmarkTimerTest, require_that_the_benchmark_timer_all_in_one_benchmarking_works) {
    ...
    double   t = BenchmarkTimer::benchmark(
        [sleep_time]() { std::this_thread::sleep_for(std::chrono::milliseconds(sleep_time)); }, 1.0);
    fprintf(stderr, "5 ms sleep takes: %g ms\n", t * 1000.0);
}
...
TEST(BenchmarkTimerTest,
     require_that_the_benchmark_timer_all_in_one_benchmarking_with_baseline_and_specified_loop_count_works) {
