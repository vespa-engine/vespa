...
INSTANTIATE_TEST_SUITE_P(
    , CpuSamplingTest, ::testing::Values(true, false),
    [](const ::testing::TestParamInfo<bool>& pi) { return pi.param ? "force_mock" : "no_mock"; });
...
TEST(CpuUsageTest, measure_thread_CPU_clock_overhead) {
    ...
    duration d;
    double   min_time_us =
        BenchmarkTimer::benchmark([&d, &sampler]() noexcept { d = sampler->sample(); }, budget) * 1000000.0;
...
TEST(CpuUsageTest, measure_total_cpu_usage_overhead) {
    duration d;
    double   min_time_us =
        BenchmarkTimer::benchmark([&d]() noexcept { d = cpu_usage::total_cpu_usage(); }, budget) * 1000000.0;
...
struct CpuUsage::Test {
    struct BlockingTracker : ThreadTracker {
        ...
        BlockingTracker() : called(0), sync_entry(2), swap_sample() {}
    ...
    struct SimpleTracker : ThreadTracker {
        Sample              my_sample;
        std::atomic<size_t> called;
        SimpleTracker(Sample sample) noexcept : my_sample(sample), called(0) {}
        Sample sample() noexcept override {
            ++called;
            return my_sample;
        }
    ...
    struct TrackerImpl {
        ...
        TrackerImpl(cpu_usage::ThreadSampler::UP sampler) : impl(std::move(sampler)) {}
...
TEST(CpuUsageTest, use_top_level_API_to_sample_CPU_usage) {
    ...
    auto    task = [&](Nexus& ctx) {
        ...
        case 0:
            return do_sample_cpu_usage(f1);
        case 1:
            return do_full_work(CpuUsage::Category::WRITE, f1);
        case 2:
            return do_some_work(CpuUsage::Category::READ, f1);
        case 3:
            return do_nested_work(CpuUsage::Category::OTHER, CpuUsage::Category::READ, f1);
        case 4:
            return do_external_work(CpuUsage::Category::COMPACT, f1);
        default:
            FAIL() << "missing thread id case";
        }
    };
    Nexus::run(num_threads, task);
}
