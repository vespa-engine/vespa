...
template <> int unwrap<Int>(const Int& x) {
    return x;
}
template <> int unwrap<std::unique_ptr<Int>>(const std::unique_ptr<Int>& x) {
    return *x;
}
...
template <typename T> void subTestCopy() {
    FAIL() << "undefined";
}
...
TEST(ArrayQueueTest, test_with_copyable_items) {
    dispatchTypedTests<Copy>();
}
TEST(ArrayQueueTest, test_with_movable_items) {
    dispatchTypedTests<Move>();
}
TEST(ArrayQueueTest, test_with_emplaced_items) {
    dispatchTypedTests<Emplace>();
}
