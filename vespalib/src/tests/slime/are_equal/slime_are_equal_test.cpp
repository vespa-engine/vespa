...
std::string path_to_str(const Path& path) {
    ...
    for (const auto& item : path) {
        ...
        std::visit(vespalib::overload{[&str](size_t value) noexcept { str.append(fmt("%zu", value)); },
                                      [&str](std::string_view value) noexcept { str.append(value); }},
                   item);
    }
    str.append("]");
    return str;
...
Hook dump_mismatches(Hook hook) {
    return [hook](const Path& path, const Inspector& a, const Inspector& b) {
        ...
        fprintf(stderr, "mismatch at %s: %s vs %s (%s)\n", path_to_str(path).c_str(), to_str(a).c_str(),
                to_str(b).c_str(), result ? "allowed" : "FAIL");
        return result;
    };
}
