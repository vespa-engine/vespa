...
struct ExpBuffer {
    ...
    ExpBuffer(std::initializer_list<uint8_t> buf_in) : buf(buf_in) {}
    ExpBuffer(size_t size) : buf(size) {}
...
bool operator==(const ExpBuffer& exp, const nbostream& stream) {
    return ((exp.size() == stream.size()) && (memcmp(exp.data(), stream.peek(), exp.size()) == 0));
}
...
struct Fixture {
    ...
    template <typename T> void assertSerialize(const ExpBuffer& exp, const T& val) {
        _stream << val;
