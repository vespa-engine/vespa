...
TEST(Base64Test, base64_test) {
    ...
    EXPECT_TRUE(static_cast<uint32_t>(Base64::getMaximumEncodeLength(source.size())) >= encoded.size());
    EXPECT_TRUE(static_cast<uint32_t>(Base64::getMaximumDecodeLength(encoded.size())) >= source.size());
    ...
    for (uint32_t i = 0; i < minSizeNeeded; ++i) {
        EXPECT_EQ(-1, Base64::encode(source.c_str(), source.size(), &buffer[0], i));
    }
    EXPECT_EQ(80, Base64::encode(source.c_str(), source.size(), &buffer[0], minSizeNeeded));
    EXPECT_EQ(Base64::encode(source), std::string(&buffer[0], 80));
    EXPECT_TRUE(minSizeNeeded <= static_cast<uint32_t>(Base64::getMaximumEncodeLength(source.size())));
    ...
    minSizeNeeded = 60;
    for (uint32_t i = 0; i < minSizeNeeded; ++i) {
        EXPECT_EQ(-1, Base64::decode(encoded.c_str(), encoded.size(), &buffer[0], i));
    }
    EXPECT_EQ(60, Base64::decode(encoded.c_str(), encoded.size(), &buffer[0], minSizeNeeded));
    EXPECT_EQ(source, std::string(&buffer[0], 60));
}
