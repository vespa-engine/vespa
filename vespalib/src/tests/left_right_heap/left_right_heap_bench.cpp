...
template <typename H> struct IsRight {
    enum { VALUE = 0 };
};
...
template <typename H> struct Name {
    static const char* value() { return "<unknown>"; }
};
template <> struct Name<LeftHeap> {
    static const char* value() { return "LeftHeap"; }
};
template <> struct Name<RightHeap> {
    static const char* value() { return "RightHeap"; }
};
template <> struct Name<LeftArrayHeap> {
    static const char* value() { return "LeftArrayHeap"; }
};
template <> struct Name<RightArrayHeap> {
    static const char* value() { return "RightArrayHeap"; }
};
template <> struct Name<LeftStdHeap> {
    static const char* value() { return "LeftStdHeap"; }
};

struct MyCmp {
    const uint32_t* values;
    MyCmp(uint32_t* v) : values(v) {}
    bool operator()(const uint16_t& a, const uint16_t& b) const { return (values[a] < values[b]); }
};

struct MyInvCmp {
    const uint32_t* values;
    MyInvCmp(uint32_t* v) : values(v) {}
    bool operator()(const uint16_t& a, const uint16_t& b) const { return (values[b] < values[a]); }
};
...
struct Timer {
    ...
    void start() { start_time = vespalib::steady_clock::now(); }
    void stop() {
...
template <typename T, typename C> bool verifyOrder(T* begin, T* end, const C& cmp, bool inv) {
    size_t len = (end - begin);
    for (size_t i = 0; i < len; ++i) {
        if ((i + 1) < len) {
            bool failed = inv ? cmp(begin[i], begin[i + 1]) : cmp(begin[i + 1], begin[i]);
            if (failed) {
                return false;
            }
        }
...
template <typename H> struct Loops {
    ...
    static void                                   fiddle(T* begin, T* end, C cmp, T* first, T* last) noinline__;
...
template <typename H> template <typename T, typename C> void Loops<H>::push(T* begin, T* end, C cmp) {
    if (IsRight<H>::VALUE) {
        for (T* pos = end; pos != begin; --pos) {
...
template <typename H> template <typename T, typename C> void Loops<H>::pop(T* begin, T* end, C cmp) {
    if (IsRight<H>::VALUE) {
        for (T* pos = begin; pos != end; ++pos) {
            H::pop(pos, end, cmp);
        }
...
template <typename H, typename D> struct BenchmarkHD : Benchmark {
    std::string legend() const override { return make_string("[%s, %s]", Name<H>::value(), D::name()); }
    ...
    std::pair<double, double> sort(size_t maxHeapSize, size_t loop) override {
        ...
        for (size_t i = 0; i < loop; ++i) {
            ...
            t1.start();
            Loops<H>::push(&*d.data.begin(), &*d.data.end(), d.cmp);
            t1.stop();
            t2.start();
            Loops<H>::pop(&*d.data.begin(), &*d.data.end(), d.cmp);
            t2.stop();
...
TEST(LeftRightHeapBenchTest, benchmark_std_heap_with_direct_uint16_t_values) {
    ...
    for (size_t l = 0; l < 1000; ++l) {
        f3.init(false);
        f1.start();
        std_push_loop(&*f3.data.begin(), &*f3.data.end(), cmp);
        f1.stop();
        f2.start();
        std_pop_loop(&*f3.data.begin(), &*f3.data.end(), cmp);
        f2.stop();
        EXPECT_TRUE(verifyOrder(&*f3.data.begin(), &*f3.data.end(), cmp, false));
    }
    fprintf(stderr, "STD HEAP 16: %g ms [%g ms (push) %g ms (pop)]\n", f1.minTime + f2.minTime, f1.minTime,
            f2.minTime);
}
...
struct BenchmarkFactory {
    enum DataType { DATA_16 = 0, DATA_32p = 1 };
    static constexpr int DATA_CNT = 2;
    enum HeapType { HEAP_LEFT = 0, HEAP_RIGHT = 1, HEAP_ARRAY_LEFT = 2, HEAP_ARRAY_RIGHT = 3, HEAP_STD_LEFT = 4 };
    static constexpr int                       HEAP_CNT = 5;
    template <typename H> static Benchmark::UP create(DataType d) {
        switch (d) {
        case DATA_16:
            return Benchmark::UP(new BenchmarkHD<H, Data16>());
        case DATA_32p:
            return Benchmark::UP(new BenchmarkHD<H, Data32p>());
        }
        abort();
    }
    static Benchmark::UP create(HeapType h, DataType d) {
        switch (h) {
        case HEAP_LEFT:
            return create<LeftHeap>(d);
        case HEAP_RIGHT:
            return create<RightHeap>(d);
        case HEAP_ARRAY_LEFT:
            return create<LeftArrayHeap>(d);
        case HEAP_ARRAY_RIGHT:
            return create<RightArrayHeap>(d);
        case HEAP_STD_LEFT:
            return create<LeftStdHeap>(d);
        }
        abort();
    }
};
...
void findFiddleLimit(Benchmark& a, Benchmark& b, size_t min, size_t max, bool adjust) {
    fprintf(stderr,
            "looking for the fiddle limit for %s(A) and %s(B) in the range [%zu, %zu]... (use adjust = '%s')\n",
...
TEST(LeftRightHeapBenchTest, benchmark) {
    for (int d = 0; d < BenchmarkFactory::DATA_CNT; ++d) {
        for (int h = 0; h < BenchmarkFactory::HEAP_CNT; ++h) {
            Benchmark::UP benchmark =
                BenchmarkFactory::create(BenchmarkFactory::HeapType(h), BenchmarkFactory::DataType(d));
