...
struct CleanContextBase {
    ...
    CleanContextBase() : _extra_used_bytes(0), _extra_hold_bytes(0) {}
};

struct MyCleanContext : public CleanContextBase, public BufferTypeBase::CleanContext {
    MyCleanContext() : CleanContextBase(), BufferTypeBase::CleanContext(_extra_used_bytes, _extra_hold_bytes) {}
};
...
struct Counts {
    ...
    Counts(uint32_t def_constructs, uint32_t value_constructs, uint32_t copy_constructs, uint32_t destructs,
           uint32_t assigns)
    ...
    bool operator==(const Counts& rhs) const {
        return _def_constructs == rhs._def_constructs && _value_constructs == rhs._value_constructs &&
               _copy_constructs == rhs._copy_constructs && _destructs == rhs._destructs && _assigns == rhs._assigns;
    }
};
...
std::ostream& operator<<(std::ostream& os, const Counts& c) {
    os << "{def_constructs=" << c._def_constructs << ", value_constructs=" << c._value_constructs
       << ", copy_constructs=" << c._copy_constructs << ", destructs=" << c._destructs << ", assigns=" << c._assigns
       << "}";
    return os;
}
...
struct WrapInt32 {
    ...
    WrapInt32() : _v(0) { ++counts._def_constructs; }
    WrapInt32(int v) : _v(v) { ++counts._value_constructs; }
    WrapInt32(const WrapInt32& rhs) : _v(rhs._v) { ++counts._copy_constructs; }
    WrapInt32& operator=(const WrapInt32& rhs) {
        _v = rhs._v;
        ++counts._assigns;
        return *this;
    }
    ~WrapInt32() { ++counts._destructs; }
};
...
protected:
    ...
    template <typename ElemT> uint32_t get_entry_size(uint32_t array_size);
...
template <typename ElemT> uint32_t DynamicArrayBufferTypeTest::get_entry_size(uint32_t array_size) {
...
std::vector<int> DynamicArrayBufferTypeTest::get_vector(const void* buffer, uint32_t offset, uint32_t array_size) {
...
std::vector<int> DynamicArrayBufferTypeTest::get_vector(const void* buffer, uint32_t offset) {
    auto e = BufferType::get_entry(buffer, offset, _entry_size);
...
std::vector<int> DynamicArrayBufferTypeTest::get_max_vector(const void* buffer, uint32_t offset) {
    auto array_size = _buffer_type.getArraySize();
    return get_vector(buffer, offset, array_size);
}

void DynamicArrayBufferTypeTest::write_entry1() {
