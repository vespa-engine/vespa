...
template <typename ElemT> class MyArrayStoreSimpleTypeMapper : public ArrayStoreSimpleTypeMapper<ElemT> {
public:
    MyArrayStoreSimpleTypeMapper(uint32_t, double, size_t) : ArrayStoreSimpleTypeMapper<ElemT>() {}
};

} // namespace

template <typename TestT, typename ElemT, typename RefT = EntryRefT<19>,
          typename TypeMapper = ArrayStoreDynamicTypeMapper<ElemT>>
struct ArrayStoreTest : public TestT {
    ...
    using TypeMapperWrappedType =
        std::conditional_t<simple_type_mapper, MyArrayStoreSimpleTypeMapper<ElemT>, TypeMapperType>;
    ...
    ArrayStoreTest(uint32_t max_type_id = 3, bool enable_free_lists = true, bool add_using_allocate_in = false,
                   double type_mapper_grow_factor_in = 2.0)
        : type_mapper(max_type_id, type_mapper_grow_factor_in, ArrayStoreConfig::default_max_buffer_size),
          store(ArrayStoreConfig(
                    max_type_id, ArrayStoreConfig::AllocSpec(16, RefT::offsetSize(), 8_Ki, ALLOC_GROW_FACTOR))
                    .enable_free_lists(enable_free_lists),
                std::make_unique<MemoryAllocatorObserver>(stats), TypeMapperType(type_mapper)),
          refStore(),
          generation(1),
    ...
    void     remove(const ElemVector& input) { remove(getEntryRef(input)); }
...
template <typename TestT, typename ElemT, typename RefT, typename TypeMapper>
size_t ArrayStoreTest<TestT, ElemT, RefT, TypeMapper>::reference_store_count(EntryRef ref) const {
...
template <typename Param> struct NumberStoreTest : public NumberStoreTestWithParam<typename Param::TypeMapper> {
...
struct NumberStoreFreeListsDisabledTest : public NumberStoreTestWithParam<typename Param::TypeMapper> {
    ...
    NumberStoreFreeListsDisabledTest()
        : Parent(3, false, Param::add_using_allocate, Param::type_mapper_grow_factor) {}
};

using NumberStoreTestTypes =
    testing::Types<SimpleTypeMapperAdd, SimpleTypeMapperAllocate, DynamicTypeMapperAddGrow1,
                   DynamicTypeMapperAllocateGrow1, DynamicTypeMapperAddGrow2, DynamicTypeMapperAllocateGrow2>;
...
void test_compaction(NumberStoreBasicTest& f) {
...
TEST_F(NumberStoreTwoSmallBufferTypesTest, buffer_with_most_dead_space_is_compacted) { test_compaction(*this); }

namespace {

template <typename Fixture> void testCompaction(Fixture& f, bool compactMemory, bool compactAddressSpace) {
    EntryRef size1Ref = f.add({1});
    EntryRef size2Ref = f.add({2, 2});
    EntryRef size3Ref = f.add({3, 3, 3});
...
TYPED_TEST(NumberStoreTest, used_onHold_and_dead_memory_usage_is_tracked_for_large_arrays) {
    ...
    this->reclaim_memory();
    this->assertMemoryUsage(exp.decUsed(this->elem_size() * 5)
                                .decHold(this->largeArraySize() + this->elem_size() * 5)
...
struct ByteStoreTest
    : public ArrayStoreTest<testing::Test, uint8_t, EntryRefT<19>, ArrayStoreSimpleTypeMapper<uint8_t>> {
    ByteStoreTest()
        : ArrayStoreTest<testing::Test, uint8_t, EntryRefT<19>, ArrayStoreSimpleTypeMapper<uint8_t>>(
              ByteStoreTest::ArrayStoreType::optimizedConfigForHugePage(
                  1023, vespalib::alloc::MemoryAllocator::HUGEPAGE_SIZE, vespalib::alloc::MemoryAllocator::PAGE_SIZE,
                  ArrayStoreConfig::default_max_buffer_size, 8_Ki, ALLOC_GROW_FACTOR)) {}
};

TEST_F(
    ByteStoreTest,
    offset_in_EntryRefT_is_within_bounds_when_allocating_memory_buffers_where_wanted_number_of_bytes_is_not_a_power_of_2_and_less_than_huge_page_size) {
    // The array store config used in this test is equivalent to the one multi-value attribute uses when initializing
    // multi-value mapping. See similar test in datastore_test.cpp for more details on what happens during memory
    // allocation.
