...
CompactBufferCandidatesTest::CompactBufferCandidatesTest()
    : ::testing::Test(), candidates(num_buffers, 1, default_active_buffers_ratio, default_ratio, default_slack) {}

CompactBufferCandidatesTest::~CompactBufferCandidatesTest() = default;

void CompactBufferCandidatesTest::reset_candidates(uint32_t max_buffers, double active_buffers_ratio) {
    candidates =
        CompactBufferCandidates(num_buffers, max_buffers, active_buffers_ratio, default_ratio, default_slack);
}

CompactBufferCandidatesTest& CompactBufferCandidatesTest::add(uint32_t buffer_id, size_t used, size_t dead) {
    candidates.add(buffer_id, used, dead);
    return *this;
}

void CompactBufferCandidatesTest::set_free_buffers(uint32_t free_buffers) {
    candidates.set_free_buffers(free_buffers);
}

void CompactBufferCandidatesTest::assert_select(const std::vector<uint32_t>& exp) {
