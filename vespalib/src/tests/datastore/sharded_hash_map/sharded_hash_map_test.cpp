...
public:
    MyCompactable(MyAllocator& allocator, std::vector<EntryRef>& new_refs)
        : ICompactable(), _allocator(allocator), _new_refs(new_refs) {}
...
DataStoreShardedHashTest::~DataStoreShardedHashTest() {
    ...
    commit();
    if (_report_work) {
        LOG(info, "read_work=%ld, write_work=%ld, found_count=%ld", _done_read_work.load(), _done_write_work.load(),
            _found_count.load());
    }
}

void DataStoreShardedHashTest::commit() {
...
void DataStoreShardedHashTest::insert(uint32_t key) {
...
void DataStoreShardedHashTest::remove(uint32_t key) {
    auto comp = _comparator.make_for_lookup(key);
...
void DataStoreShardedHashTest::read_work(uint32_t cnt) {
    vespalib::Rand48 rnd;
    long             found = 0;
...
void DataStoreShardedHashTest::read_work() { read_work(std::numeric_limits<uint32_t>::max()); }

void DataStoreShardedHashTest::write_work(uint32_t cnt) {
...
void DataStoreShardedHashTest::populate_sample_data(uint32_t cnt) {
    for (uint32_t i = 0; i < cnt; ++i) {
        insert(i);
    }
}

void DataStoreShardedHashTest::populate_sample_values(uint32_t cnt) {
    for (uint32_t i = 0; i < cnt; ++i) {
...
void DataStoreShardedHashTest::clear_sample_values(uint32_t cnt) {
    for (uint32_t i = 0; i < cnt; ++i) {
        auto comp = _comparator.make_for_lookup(i);
        auto result = _hash_map.find(comp, EntryRef());
        ASSERT_NE(result, nullptr);
...
template <typename RefT> EntryRefFilter make_entry_ref_filter(bool one_filter) {
    if (one_filter) {
...
void DataStoreShardedHashTest::test_normalize_values(bool use_filter, bool one_filter) {
    ...
    if (use_filter) {
        ...
        EXPECT_TRUE(_hash_map.normalize_values(
            [](std::vector<EntryRef>& refs) noexcept {
                for (auto& ref : refs) {
                    RefT iref(ref);
                    ref = RefT(iref.offset() + 300, iref.bufferId());
                }
            },
            filter));
    } else {
        EXPECT_TRUE(_hash_map.normalize_values([](EntryRef ref) noexcept {
            RefT iref(ref);
...
void DataStoreShardedHashTest::test_foreach_value(bool one_filter) {
    populate_sample_data(large_population);
    ASSERT_NO_FATAL_FAILURE(populate_sample_values(large_population));

    auto                  filter = make_entry_ref_filter<RefT>(one_filter);
    std::vector<EntryRef> exp_refs;
    EXPECT_FALSE(_hash_map.normalize_values(
        [&exp_refs](std::vector<EntryRef>& refs) { exp_refs.insert(exp_refs.end(), refs.begin(), refs.end()); },
        filter));
    std::vector<EntryRef> act_refs;
    _hash_map.foreach_value(
        [&act_refs](const std::vector<EntryRef>& refs) { act_refs.insert(act_refs.end(), refs.begin(), refs.end()); },
        filter);
...
TEST_F(DataStoreShardedHashTest, normalize_values_works) { test_normalize_values(false, false); }

TEST_F(DataStoreShardedHashTest, normalize_values_all_filter_works) { test_normalize_values(true, false); }

TEST_F(DataStoreShardedHashTest, normalize_values_one_filter_works) { test_normalize_values(true, true); }

TEST_F(DataStoreShardedHashTest, foreach_value_all_filter_works) { test_foreach_value(false); }

TEST_F(DataStoreShardedHashTest, foreach_value_one_filter_works) { test_foreach_value(true); }
