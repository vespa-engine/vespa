...
std::function<size_t(uint32_t)> default_type_id_to_entry_size = [](size_t type_id) noexcept {
    return type_id * sizeof(int);
};
std::function<size_t(uint32_t)> large_entry_type_id_to_entry_size = [](size_t type_id) noexcept {
    return type_id * 16_Ki;
};
...
struct Fixture {
    ...
    Fixture(uint32_t max_type_id, const AllocSpec& defaultSpec) : cfg(max_type_id, defaultSpec) {}

    Fixture(uint32_t max_type_id, size_t hugePageSize, size_t smallPageSize, size_t max_buffer_size,
            size_t                          min_num_entries_for_new_buffer,
            std::function<size_t(uint32_t)> type_id_to_entry_size = default_type_id_to_entry_size)
        : cfg(ArrayStoreConfig::optimizeForHugePage(
              max_type_id, type_id_to_entry_size, hugePageSize, smallPageSize, EntryRefType::offsetSize(),
              max_buffer_size, min_num_entries_for_new_buffer, ALLOC_GROW_FACTOR)) {}
    ...
    void assertSpec(uint32_t type_id, uint32_t max_entries, uint32_t num_entries_for_new_buffer) {
        assertSpec(type_id, AllocSpec(0, max_entries, num_entries_for_new_buffer, ALLOC_GROW_FACTOR));
    }
...
AllocSpec makeSpec(size_t min_entries_in_buffer, size_t max_entries_in_buffer, size_t num_entries_for_new_buffer) {
...
TEST(ArrayStoreConfigTest,
     require_that_we_can_generate_config_optimized_for_a_given_huge_page_without_capped_buffer_sizes) {
    BigBuffersFixture f;
...
TEST(ArrayStoreConfigTest,
     require_that_we_can_generate_config_optimized_for_a_given_huge_page_with_capped_buffer_sizes) {
...
struct CappedBuffersWithLargeEntriesFixture : public Fixture {
    CappedBuffersWithLargeEntriesFixture()
        : Fixture(3, 2_Mi, 4_Ki, 256_Mi, 8_Ki, large_entry_type_id_to_entry_size) {}
