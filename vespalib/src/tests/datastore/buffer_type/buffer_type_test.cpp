...
struct MySetup {
    ...
    MySetup& min_entries(uint32_t value) {
        _min_entries = value;
        return *this;
    }
    MySetup& used(size_t value) {
        _used_entries = value;
        return *this;
    }
    MySetup& needed(size_t value) {
        _needed_entries = value;
        return *this;
    }
    MySetup& dead(size_t value) {
        _dead_entries = value;
        return *this;
    }
    MySetup& bufferId(uint32_t value) {
        _bufferId = value;
        return *this;
    }
    MySetup& resizing(bool value) {
        _resizing = value;
        return *this;
    }
};
...
struct Fixture {
    ...
    Fixture(const MySetup& setup_)
        : setups(),
          bufferType(ARRAYS_SIZE, setup_._min_entries, MAX_ENTRIES, NUM_ENTRIES_FOR_NEW_BUFFER,
                     setup_._allocGrowFactor),
          buffer() {
        setups.reserve(4);
    ...
    MySetup& curr_setup() { return setups.back(); }
    ...
    void onActive() {
        bufferType.on_active(
            curr_setup()._bufferId, &curr_setup()._used_entries, &curr_setup()._dead_entries, &buffer[0]);
    }
    size_t entries_to_alloc() {
        return bufferType.calc_entries_to_alloc(
            curr_setup()._bufferId, curr_setup()._needed_entries, curr_setup()._resizing);
    }
...
size_t entries_to_alloc(const MySetup& setup) {
    Fixture f(setup);
    f.onActive();
...
TEST(BufferTypeTest,
     entries_to_alloc_considers_used_entries_across_all_active_buffers_of_same_type_when_not_resizing) {
...
TEST(BufferTypeTest,
     entries_to_alloc_considers_and_subtracts_dead_entries_across_all_active_buffers_of_same_type_when_not_resizing) {
...
TEST(BufferTypeTest,
     arrays_to_alloc_considers_and_subtracts_dead_elements_across_all_active_buffers_of_same_type_when_resizing) {
    Fixture f(MySetup().used(6).dead(2));
    f.assert_entries_to_alloc((6 - 2) * 0.5);
