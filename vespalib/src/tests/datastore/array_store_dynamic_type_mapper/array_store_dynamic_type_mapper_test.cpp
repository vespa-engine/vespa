...
template <typename ElemT> class TestBase : public testing::Test {
protected:
...
TestBase<ElemT>::TestBase() : testing::Test(), _mapper(5, default_grow_factor, default_max_buffer_size) {}

template <typename ElemT> TestBase<ElemT>::~TestBase() = default;

template <typename ElemT> void TestBase<ElemT>::setup_mapper(uint32_t max_buffer_type_id, double grow_factor) {
    _mapper = ArrayStoreDynamicTypeMapper<ElemT>(max_buffer_type_id, grow_factor, default_max_buffer_size);
}

template <typename ElemT> std::vector<size_t> TestBase<ElemT>::get_array_sizes(uint32_t num_array_sizes) {
...
template <typename ElemT> std::vector<size_t> TestBase<ElemT>::get_entry_sizes(uint32_t num_entry_sizes) {
...
template <typename ElemT> std::vector<size_t> TestBase<ElemT>::get_large_array_sizes(uint32_t num_large_array_sizes) {
...
template <typename ElemT> void TestBase<ElemT>::select_type_ids(std::vector<size_t> array_sizes) {
    uint32_t              type_id = 0;
...
template <typename ElemT>
uint32_t TestBase<ElemT>::calc_max_buffer_type_id(double grow_factor, size_t max_buffer_size) {
