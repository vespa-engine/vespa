...
bool fileExists(const std::string& name) { return std::filesystem::exists(std::filesystem::path(name)); }
...
TEST(FileUtilTest, require_that_vespalib__getOpenErrorString_works) {
    ...
    std::string expErr1 = "error=x fileStat[name=mydir/foo mode=x uid=x gid=x size=x mtime=x] dirStat[name=mydir "
                          "mode=x uid=x gid=x size=x mtime=x]";
    std::cerr << "getOpenErrorString(1, \"mydir/foo\") is " << err1 << ", normalized to " << normErr1 << std::endl;
    ...
    std::cerr << "getOpenErrorString(1, \"notFound\") is " << err2 << ", normalized to " << normErr2 << std::endl;
