...
SocketTest::SocketTest() : testing::Test() {}

SocketTest::~SocketTest() = default;

void SocketTest::SetUpTestSuite() {
...
void SocketTest::TearDownTestSuite() {}

int SocketTest::my_inet() {
    if (ipv6_enabled) {
        return AF_INET6;
    } else {
        return AF_INET;
...
SocketHandle connect_sockets(bool is_server, ServerSocket& server_socket) {
    if (is_server) {
        ...
        fprintf(stderr, "connecting to '%s' (server: %s) (client: %s)\n", spec.c_str(), get_meta(server).c_str(),
                get_meta(client).c_str());
...
TEST_F(SocketTest,
       require_that_a_server_socket_will_fail_to_listen_to_a_path_that_is_already_taken_by_another_server) {
    remove_file("my_socket");
