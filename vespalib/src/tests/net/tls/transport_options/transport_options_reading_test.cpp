...
public:
    ...
    ~ConfigWriter();
    ConfigWriter private_key(std::optional<std::string> value) && {
        _private_key = value;
        return std::move(*this);
    }
    ConfigWriter ca_certificates(std::optional<std::string> value) && {
        _ca_certificates = value;
        return std::move(*this);
    }
    ConfigWriter certificates(std::optional<std::string> value) && {
        _certificates = value;
        return std::move(*this);
    }
    ConfigWriter accepted_ciphers(std::optional<std::string> value) && {
        _accepted_ciphers = value;
        return std::move(*this);
    }
    ConfigWriter authorized_peers(std::optional<std::string> value) && {
        _authorized_peers = value;
        return std::move(*this);
    }
    ConfigWriter disable_hostname_validation(std::optional<std::string> value) && {
        _disable_hostname_validation = value;
        return std::move(*this);
    }
    ConfigWriter flipper_the_dolphin(std::optional<std::string> value) && {
        _flipper_the_dolphin = value;
        return std::move(*this);
    }
...
void ConfigWriter::write(std::ostream& os) {
    os << "{\n" << R"(  "files": {)";
...
TransportSecurityOptionsTest::TransportSecurityOptionsTest() : ::testing::Test() {}

TransportSecurityOptionsTest::~TransportSecurityOptionsTest() = default;

void TransportSecurityOptionsTest::SetUpTestSuite() {
    std::ofstream ok_config("ok_config.json");
    ConfigWriter().write(ok_config);
    ok_config.close();
}

void TransportSecurityOptionsTest::TearDownTestSuite() { std::filesystem::remove("ok_config.json"); }
...
TEST_F(TransportSecurityOptionsTest, missing_json_file_throws_exception) {
    EXPECT_THAT([]() { read_options_from_json_file("missing_config.json"); },
                testing::ThrowsMessage<IllegalArgumentException>(
                    testing::HasSubstr("TLS config file 'missing_config.json' could not be read")));
}
...
TEST_F(TransportSecurityOptionsTest, bad_json_content_throws_exception) {
    ...
    EXPECT_THAT([bad_json]() { read_options_from_json_string(bad_json); },
                testing::ThrowsMessage<IllegalArgumentException>(
                    testing::HasSubstr("Provided TLS config file is not valid JSON")));
}
...
TEST_F(TransportSecurityOptionsTest, missing_files_field_throws_exception) {
    ...
    EXPECT_THAT([incomplete_json]() { read_options_from_json_string(incomplete_json); },
                testing::ThrowsMessage<IllegalArgumentException>(
                    testing::HasSubstr("TLS config root field 'files' is missing or empty")));
}
...
TEST_F(TransportSecurityOptionsTest, missing_private_key_field_throws_exception) {
    ...
    EXPECT_THAT([incomplete_json]() { read_options_from_json_string(incomplete_json); },
                testing::ThrowsMessage<IllegalArgumentException>(
                    testing::HasSubstr("TLS config field 'private-key' has not been set")));
}
...
TEST_F(TransportSecurityOptionsTest, missing_certificates_field_throws_exception) {
    ...
    EXPECT_THAT([incomplete_json]() { read_options_from_json_string(incomplete_json); },
                testing::ThrowsMessage<IllegalArgumentException>(
                    testing::HasSubstr("TLS config field 'certificates' has not been set")));
}
...
TEST_F(TransportSecurityOptionsTest, missing_ca_certificates_field_throws_exception) {
    ...
    EXPECT_THAT([incomplete_json]() { read_options_from_json_string(incomplete_json); },
                testing::ThrowsMessage<IllegalArgumentException>(
                    testing::HasSubstr("TLS config field 'ca-certificates' has not been set")));
}
...
TEST_F(TransportSecurityOptionsTest, missing_file_referenced_by_field_throws_exception) {
    ...
    EXPECT_THAT([incomplete_json]() { read_options_from_json_string(incomplete_json); },
                testing::ThrowsMessage<IllegalArgumentException>(
                    testing::HasSubstr("File 'missing_privkey.txt' referenced by TLS config does not exist")));
}
...
TEST_F(TransportSecurityOptionsTest, empty_policy_array_throws_exception) {
    EXPECT_THAT([]() { parse_policies(""); }, testing::ThrowsMessage<IllegalArgumentException>(testing::HasSubstr(
...
TEST_F(TransportSecurityOptionsTest, can_parse_single_peer_policy_with_multiple_requirements) {
    ...
    EXPECT_EQ(authorized_peers({policy_with({required_san_dns("hello.world"), required_san_uri("foo://bar/baz"),
...
TEST_F(TransportSecurityOptionsTest, unknown_field_type_throws_exception) {
    ...
    EXPECT_THAT([json]() { parse_policies(json); },
                testing::ThrowsMessage<IllegalArgumentException>(testing::HasSubstr(
...
TEST_F(TransportSecurityOptionsTest, empty_required_credentials_array_throws_exception) {
    ...
    EXPECT_THAT([json]() { parse_policies(json); },
                testing::ThrowsMessage<IllegalArgumentException>(
                    testing::HasSubstr("\"required-credentials\" array can't be empty (would allow all peers)")));
}
...
TEST_F(TransportSecurityOptionsTest,
       hostname_validation_is_DISABLED_by_default_when_creating_options_from_config_file) {
    auto json = ConfigWriter().write();
    EXPECT_TRUE(read_options_from_json_string(json)->disable_hostname_validation());
}

TEST_F(TransportSecurityOptionsTest,
       transport_security_options_builder_does_not_disable_hostname_validation_by_default) {
    auto ts_builder =
        vespalib::net::tls::TransportSecurityOptions::Params()
            .ca_certs_pem("foo")
            .cert_chain_pem("bar")
...
TEST_F(TransportSecurityOptionsTest, policy_without_explicit_capabilities_implicitly_get_all_capabilities) {
    ...
    EXPECT_EQ(authorized_peers(
                  {policy_with({required_san_dns("hello.world")}, CapabilitySet::make_with_all_capabilities())}),
              parse_policies(json).authorized_peers());
}
...
TEST_F(TransportSecurityOptionsTest, specifying_a_capability_set_adds_all_its_underlying_capabilities) {
    ...
    EXPECT_EQ(authorized_peers({policy_with(
                  {required_san_dns("*.cool-content-clusters.example")}, CapabilitySet::content_node())}),
              parse_policies(json).authorized_peers());
}
...
TEST_F(TransportSecurityOptionsTest, can_specify_single_leaf_capabilities) {
    ...
    EXPECT_EQ(authorized_peers(
                  {policy_with({required_san_dns("*.cool-content-clusters.example")},
                               CapabilitySet::of({Capability::content_metrics_api(), Capability::slobrok_api()}))}),
              parse_policies(json).authorized_peers());
}
