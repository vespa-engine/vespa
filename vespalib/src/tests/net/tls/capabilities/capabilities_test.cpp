...
TEST(CapabilitiesTest, all_known_capabilities_can_be_looked_up_by_name_and_resolve_back_to_same_name) {
    ...
    check_capability_mapping(
        "vespa.configproxy.filedistribution_api", Capability::configproxy_filedistribution_api());
    check_capability_mapping("vespa.configserver.config_api", Capability::configserver_config_api());
    check_capability_mapping(
        "vespa.configserver.filedistribution_api", Capability::configserver_filedistribution_api());
...
TEST(CapabilitiesTest, test_CapabilitySet_instances_can_be_stringified) {
    EXPECT_EQ(
        CapabilitySet::content_node().to_string(),
...
TEST(CapabilitiesTest, resolving_multiple_capabilities_sets_adds_union_of_capabilities) {
    ...
    EXPECT_EQ(caps, CapabilitySet::of({Capability::content_storage_api(), Capability::content_document_api(),
                                       Capability::container_document_api(), Capability::slobrok_api(),
    ...
    EXPECT_EQ(caps, CapabilitySet::of({Capability::content_storage_api(), Capability::content_document_api(),
                                       Capability::container_document_api(), Capability::slobrok_api(),
                                       Capability::content_search_api(), Capability::content_metrics_api()})
                        .union_of(CapabilitySet::shared_app_node_capabilities()));
}
