...
// TODO redesign...
template <typename T> struct NativeFromT { using value_type = T; };
template <> struct NativeFromT<Float8E4M3FN> { using value_type = uint8_t; };
template <> struct NativeFromT<Float8E5M2> { using value_type = uint8_t; };

template <typename T> struct IsFinite {
    bool operator()(const T&) const noexcept { return true; }
};
template <> struct IsFinite<Float8E4M3FN> {
    bool operator()(uint8_t v) const noexcept { return Float8E4M3FN::is_finite(v); }
};
template <> struct IsFinite<Float8E5M2> {
    bool operator()(uint8_t v) const noexcept { return Float8E5M2::is_finite(v); }
};
...
void register_accel_binary_arg_benchmark(std::string_view name, std::unique_ptr<IAccelerated> accel, Fn&& fn) {
    ...
    auto bench_fn = [fn = std::forward<Fn>(fn), accel = std::move(accel)](benchmark::State& state) {
        auto [a, b] = create_and_fill_lhs_rhs<typename NativeFromT<T>::value_type>(state.range(), IsFinite<T>{});
...
void register_all_benchmark_suites() {
    ...
    auto dot_product_fp8_e4m3fn_fn = [](const auto* lhs, const auto* rhs, size_t my_sz) {
        return dot_product(lhs, rhs, my_sz, dispatch::Fp8E4M3FNTag{});
    };
    register_benchmarks<Float8E4M3FN>("Dot Product", FnTable::FnId::DOT_PRODUCT_F8_E4M3FN, dot_product_fp8_e4m3fn_fn);
    auto dot_product_fp8_e5m2_fn = [](const auto* lhs, const auto* rhs, size_t my_sz) {
        return dot_product(lhs, rhs, my_sz, dispatch::Fp8E5M3Tag{});
    };
    register_benchmarks<Float8E5M2>("Dot Product", FnTable::FnId::DOT_PRODUCT_F8_E5M2, dot_product_fp8_e5m2_fn);
