...
public:
    ...
    {
        auto composite_table = dispatch::build_composite_fn_table_from_baseline(new_sparse_table);
        dispatch::thread_unsafe_update_vector_dispatch_function_pointers(composite_table);
    }
    ~ScopedFnTableOverride() {
        dispatch::thread_unsafe_update_vector_dispatch_function_pointers(_original_fn_table);
    }
};
