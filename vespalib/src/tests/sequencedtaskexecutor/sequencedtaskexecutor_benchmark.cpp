...
int main(int argc, char** argv) {
    ...
    if (use_adaptive_executor) {
        executor =
            std::make_unique<AdaptiveSequencedExecutor>(num_strands, num_threads, max_waiting, task_limit, true);
    } else {
        auto optimize = optimize_for_throughput ? vespalib::Executor::OptimizeFor::THROUGHPUT
    ...
    for (size_t task_id = 0; task_id < num_tasks; ++task_id) {
        executor->executeTask(ExecutorId(task_id % num_strands), vespalib::makeLambdaTask([&counter, work_size] {
                                  (void)do_work(work_size);
                                  counter++;
                              }));
    }
    executor.reset();
