...
public:
    ...
    Fixture(bool is_task_limit_hard = true)
        : _threads(SequencedTaskExecutor::create(
              sequenced_executor, 2, 1000, is_task_limit_hard, Executor::OptimizeFor::LATENCY)) {}
};
...
public:
    ...
    TestObj() noexcept : _m(), _cv(), _done(0), _fail(0), _val(0) {}
...
TEST(SequencedTaskExecutorTest, require_that_task_with_same_component_id_are_serialized_when_executed_with_list) {
    ...
    list.emplace_back(executorId, makeLambdaTask([=]() {
                          usleep(2000);
                          tv->modify(0, 14);
                      }));
...
TEST(SequencedTaskExecutorTest, require_that_task_with_same_string_component_id_are_serialized) {
    ...
    f._threads->execute(f._threads->getExecutorIdFromName(ZERO), [=]() {
        usleep(2000);
        tv->modify(0, 14);
    });
...
TEST(SequencedTaskExecutorTest,
     require_that_task_with_different_string_component_ids_mapping_to_the_same_executor_id_are_serialized) {
    Fixture     f;
...
void verifyHardLimitForLatency(bool expect_hard) {
    auto sequenced =
        SequencedTaskExecutor::create(sequenced_executor, 1, 100, expect_hard, Executor::OptimizeFor::LATENCY);
...
void verifyHardLimitForThroughput(bool expect_hard) {
    auto sequenced =
        SequencedTaskExecutor::create(sequenced_executor, 1, 100, expect_hard, Executor::OptimizeFor::THROUGHPUT);
    const SequencedTaskExecutor& seq = dynamic_cast<const SequencedTaskExecutor&>(*sequenced);
