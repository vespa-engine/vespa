...
public:
    ...
    static uint32_t convert(uint32_t codepoint) noexcept {
        if (codepoint < 0x100) [[likely]] {
            ...
            case 19:
                return lowercase_19_block[lb];
            case 28:
                return lowercase_28_block[lb];
            case 30:
            ...
            case 166:
                return lowercase_166_block[lb];
            case 167:
                return lowercase_167_block[lb];
            case 255:
                return lowercase_255_block[lb];
            case 260:
                return lowercase_260_block[lb];
            case 268:
                return lowercase_268_block[lb];
            case 280:
                return lowercase_280_block[lb];
            case 366:
                return lowercase_366_block[lb];
            case 489:
                return lowercase_489_block[lb];
            default:
                return codepoint;
            }
        }
     ...
     **/
    static unsigned char convert(unsigned char c) { return convert((uint32_t)c); }

    /**
     * lowercase a single ASCII (or iso 8859-1) character
     **/
    static char convert(char c) { return convert((unsigned char)c); }

    /**
     * lowercase a single ASCII (or iso 8859-1) character
     **/
    static signed char convert(signed char c) { return convert((unsigned char)c); }
