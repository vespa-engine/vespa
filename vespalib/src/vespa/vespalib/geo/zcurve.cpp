...
struct Area {
    ...
    int64_t size() const {
        return (static_cast<int64_t>(max.x) - min.x + 1) * (static_cast<int64_t>(max.y) - min.y + 1);
    }
...
private:
    struct MaxAreaErrorCmp {
        bool operator()(const Area& a, const Area& b) const { return (a.error() > b.error()); }
    };
...
ZCurve::BoundingBox::BoundingBox(int32_t minx, int32_t maxx, int32_t miny, int32_t maxy)
    : _zMinx(ZCurve::encode(minx, 0)),
      _zMaxx(ZCurve::encode(maxx, 0)),
      _zMiny(ZCurve::encode(0, miny)),
      _zMaxy(ZCurve::encode(0, maxy)) {}

ZCurve::RangeVector ZCurve::find_ranges(int min_x, int min_y, int max_x, int max_y) {
    uint64_t x_size = (static_cast<int64_t>(max_x) - min_x + 1);
    uint64_t y_size = (static_cast<int64_t>(max_y) - min_y + 1);
    uint64_t total_size =
        (x_size > std::numeric_limits<uint32_t>::max() && y_size > std::numeric_limits<uint32_t>::max())
            ? std::numeric_limits<uint64_t>::max()
            : (x_size * y_size);
    int64_t estimate_target =
        (total_size > std::numeric_limits<int64_t>::max() / 4)
...
int64_t ZCurve::encodeSlow(int32_t x, int32_t y) {
    uint64_t res = 0;
    uint32_t ibit = 1;
    uint64_t obit = 1;
...
void ZCurve::decodeSlow(int64_t enc, int32_t* xp, int32_t* yp) {
    uint32_t x = 0;
    uint32_t y = 0;
    uint64_t ibit = 1;
    uint32_t obit = 1;
