...
public:
    ...
    public:
        ...
        bool getzFailBoundingBoxTest(int64_t docxy) const {
            ...
            return (doczy < getzMiny() || doczy > getzMaxy() ||
                    static_cast<int64_t>(doczx << 1) < static_cast<int64_t>(getzMinx() << 1) ||
                    static_cast<int64_t>(doczx << 1) > static_cast<int64_t>(getzMaxx() << 1));
        }
    };
     ...
     */
    static int64_t encode(int32_t x, int32_t y) {

        uint64_t rx = (static_cast<uint64_t>(static_cast<uint32_t>(x) & 0xffff0000u) << 16) |
                      (static_cast<uint32_t>(x) & 0x0000ffffu);
        uint64_t ry = (static_cast<uint64_t>(static_cast<uint32_t>(y) & 0xffff0000u) << 16) |
                      (static_cast<uint32_t>(y) & 0x0000ffffu);
        rx = ((rx & UINT64_C(0xff00ff00ff00ff00)) << 8) | (rx & UINT64_C(0x00ff00ff00ff00ff));
        ry = ((ry & UINT64_C(0xff00ff00ff00ff00)) << 8) | (ry & UINT64_C(0x00ff00ff00ff00ff));
        rx = ((rx & UINT64_C(0xf0f0f0f0f0f0f0f0)) << 4) | (rx & UINT64_C(0x0f0f0f0f0f0f0f0f));
        ry = ((ry & UINT64_C(0xf0f0f0f0f0f0f0f0)) << 4) | (ry & UINT64_C(0x0f0f0f0f0f0f0f0f));
        rx = ((rx & UINT64_C(0xcccccccccccccccc)) << 2) | (rx & UINT64_C(0x3333333333333333));
        ry = ((ry & UINT64_C(0xcccccccccccccccc)) << 2) | (ry & UINT64_C(0x3333333333333333));
        rx = ((rx & UINT64_C(0xaaaaaaaaaaaaaaaa)) << 1) | (rx & UINT64_C(0x5555555555555555));
        ry = ((ry & UINT64_C(0xaaaaaaaaaaaaaaaa)) << 1) | (ry & UINT64_C(0x5555555555555555));
     ...
     */
    static void decode(int64_t enc, int32_t* xp, int32_t* yp) {
        ...
        x = ((x & UINT64_C(0xcccccccccccccccc)) >> 1) | (x & UINT64_C(0x3333333333333333));
        y = ((y & UINT64_C(0xcccccccccccccccc)) >> 1) | (y & UINT64_C(0x3333333333333333));
        x = ((x & UINT64_C(0xf0f0f0f0f0f0f0f0)) >> 2) | (x & UINT64_C(0x0f0f0f0f0f0f0f0f));
        y = ((y & UINT64_C(0xf0f0f0f0f0f0f0f0)) >> 2) | (y & UINT64_C(0x0f0f0f0f0f0f0f0f));
        x = ((x & UINT64_C(0xff00ff00ff00ff00)) >> 4) | (x & UINT64_C(0x00ff00ff00ff00ff));
        y = ((y & UINT64_C(0xff00ff00ff00ff00)) >> 4) | (y & UINT64_C(0x00ff00ff00ff00ff));
        x = ((x & UINT64_C(0xffff0000ffff0000)) >> 8) | (x & UINT64_C(0x0000ffff0000ffff));
        y = ((y & UINT64_C(0xffff0000ffff0000)) >> 8) | (y & UINT64_C(0x0000ffff0000ffff));
        x = ((x & UINT64_C(0xffffffff00000000)) >> 16) | (x & UINT64_C(0x00000000ffffffff));
        y = ((y & UINT64_C(0xffffffff00000000)) >> 16) | (y & UINT64_C(0x00000000ffffffff));
     ...
     **/
    static RangeVector find_ranges(int min_x, int min_y, int max_x, int max_y);
