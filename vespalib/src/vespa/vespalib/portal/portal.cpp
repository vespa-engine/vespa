...
template <typename T> void remove_handle(T& collection, uint64_t handle) {
    collection.erase(std::remove_if(collection.begin(), collection.end(),
                                    [handle](const typename T::value_type& item) { return (item.handle == handle); }),
                     collection.end());
}
...
Portal::Token::~Token() { _portal.cancel_token(*this); }
...
Portal::Token::UP Portal::make_token() { return Token::UP(new Token(*this, _handle_manager.create())); }

void Portal::cancel_token(Token& token) {
    _handle_manager.destroy(token._handle);
    evict_handle(token._handle);
}

portal::HandleGuard Portal::lookup_get_handler(const std::string& uri, GetHandler*& handler) {
...
void Portal::evict_handle(uint64_t handle) {
    std::lock_guard guard(_lock);
    remove_handle(_bind_list, handle);
}

void Portal::handle_accept(portal::HandleGuard guard, SocketHandle socket) {
    socket.set_blocking(false);
    socket.set_keepalive(true);
    new HttpConnection(std::move(guard), _reactor, _crypto->create_server_crypto_socket(std::move(socket)),
                       [this](HttpConnection* conn) { handle_http(conn); });
}

void Portal::handle_http(portal::HttpConnection* conn) {
...
Portal::Portal(CryptoEngine::SP crypto, int port)
    : _crypto(std::move(crypto)),
      ...
      _bind_list(),
      _my_host() {
    _listener = std::make_unique<portal::Listener>(_reactor, port, [this](SocketHandle socket) {
...
Portal::Token::UP Portal::bind(const std::string& path_prefix, GetHandler& handler) {
