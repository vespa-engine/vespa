...
Lz4OutputEncoder::Lz4OutputEncoder(Output& output, size_t buffer_size)
    : _output(output), _buffer(buffer_size, 0), _used(0), _limit(buffer_size) {}
...
WritableMemory Lz4OutputEncoder::reserve(size_t bytes) {
...
Output& Lz4OutputEncoder::commit(size_t bytes) {
    _used += bytes;
    if (_used >= _limit) {
        encode_frame();
    }
