...
namespace {
size_t padbefore(size_t alignment, const char* buf) { return (alignment - (size_t(buf) % alignment)) % alignment; }
} // namespace
...
bool DataBuffer::shrink(size_t newsize) {
...
void DataBuffer::pack(size_t needbytes) {
    needbytes += (_alignment - 1);
    size_t dataLen = getDataLen();

    if ((getDeadLen() + getFreeLen()) < needbytes || (getDeadLen() + getFreeLen()) * 4 < dataLen) {
...
bool DataBuffer::equals(DataBuffer* other) {
...
bool DataBuffer::referencesExternalData() const { return (_externalBuf == _bufstart) && (getBufSize() > 0); }
