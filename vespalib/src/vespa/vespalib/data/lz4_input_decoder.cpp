...
void Lz4InputDecoder::fail(const char* reason) {
    _failed = true;
    _reason = reason;
    _eof = true;
}
...
void Lz4InputDecoder::decode_more() {
    ...
    auto   decode_res = LZ4F_decompress(_ctx, &_buffer[0], &output_size, memory.data, &input_size, nullptr);
...
Lz4InputDecoder::~Lz4InputDecoder() { LZ4F_freeDecompressionContext(_ctx); }

Memory Lz4InputDecoder::obtain() {
...
Input& Lz4InputDecoder::evict(size_t bytes) {
    _pos += bytes;
    return *this;
}
