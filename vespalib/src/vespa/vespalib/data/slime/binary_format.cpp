...
void write_cmpr_ulong_impl(OutputWriter& out, uint64_t value) {
...
struct BinaryEncoder : public ArrayTraverser, public ObjectSymbolTraverser {
    OutputWriter& out;
    BinaryEncoder(OutputWriter& out_in) : out(out_in) {}
    void encodeNix() { out.write(NIX::ID); }
    void encodeBool(bool value) { out.write(encode_type_and_meta(BOOL::ID, value ? 1 : 0)); }
    void encodeLong(int64_t value) { write_type_and_bytes<false>(out, LONG::ID, encode_zigzag(value)); }
    void encodeDouble(double value) { write_type_and_bytes<true>(out, DOUBLE::ID, encode_double(value)); }
    ...
    void encodeValue(const Inspector& inspector) {
        ...
        case DATA::ID:
            return encodeData(inspector.asData());
        case ARRAY::ID:
...
void BinaryEncoder::entry(size_t, const Inspector& inspector) { encodeValue(inspector); }
...
struct MappedSymbols {
    ...
    void                hint_symbol_count(size_t n) { symbol_mapping.reserve(n); }
    bool                add_symbol(Symbol symbol, size_t, InputReader&) {
        symbol_mapping.push_back(symbol);
        return true;
    }
    Symbol map_symbol(Symbol symbol) const {
        return (symbol.getValue() < symbol_mapping.size()) ? symbol_mapping[symbol.getValue()] : symbol;
    }
};

template <bool remap_symbols> struct SymbolHandler {
    using type = typename std::conditional<remap_symbols, MappedSymbols, DirectSymbols>::type;
};

template <bool remap_symbols> struct BinaryDecoder : SymbolHandler<remap_symbols>::type {
    ...
    Cursor& decodeNix(const Inserter& inserter) { return inserter.insertNix(); }

    Cursor& decodeBool(const Inserter& inserter, uint32_t meta) { return inserter.insertBool(meta != 0); }
    ...
    Cursor& decodeValue(const Inserter& inserter, uint32_t type, uint32_t meta) {
        ...
        case BOOL::ID:
            return decodeBool(inserter, meta);
        case LONG::ID:
            return decodeLong(inserter, meta);
        case DOUBLE::ID:
            return decodeDouble(inserter, meta);
        case STRING::ID:
            return decodeString(inserter, meta);
        case DATA::ID:
            return decodeData(inserter, meta);
        case ARRAY::ID:
            return decodeArray(inserter, meta);
        case OBJECT::ID:
            return decodeObject(inserter, meta);
        }
        LOG_ABORT("should not be reached");
    }
...
Cursor& BinaryDecoder<remap_symbols>::decodeArray(const Inserter& inserter, uint32_t meta) {
...
template <bool remap_symbols>
Cursor& BinaryDecoder<remap_symbols>::decodeObject(const Inserter& inserter, uint32_t meta) {
...
template <bool remap_symbols> size_t decode(const Memory& memory, Slime& slime, const Inserter& inserter) {
...
void write_cmpr_ulong(OutputWriter& out, uint64_t value) { write_cmpr_ulong_impl(out, value); }
...
void BinaryFormat::encode(const Slime& slime, Output& output) {
...
size_t BinaryFormat::decode(const Memory& memory, Slime& slime) {
    return binary_format::decode<false>(memory, slime, SlimeInserter(slime));
}

size_t BinaryFormat::decode_into(const Memory& memory, Slime& slime, const Inserter& inserter) {
