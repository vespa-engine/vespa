...
template <bool COMPACT> struct JsonEncoder : public ArrayTraverser, public ObjectTraverser {
    OutputWriter& out;
    int           level;
    bool          head;

    JsonEncoder(OutputWriter& out_in) : out(out_in), level(0), head(true) {}
    ...
    void encodeNIX() { out.write("null", 4); }
...
template <bool COMPACT> void JsonEncoder<COMPACT>::entry(size_t, const Inspector& inspector) {
    separate(true);
    encodeValue(inspector);
}

template <bool COMPACT> void JsonEncoder<COMPACT>::field(const Memory& symbol_name, const Inspector& inspector) {
    separate(true);
...
struct JsonDecoder {
    ...
    void     decodeValue(Inserter& inserter) {
        ...
        case '"':
        case '\'':
            return decodeString(inserter);
        case '{':
            return decodeObject(inserter);
        case '[':
            return decodeArray(inserter);
        case 't':
            expect("true");
...
uint32_t JsonDecoder::readHexValue(uint32_t len) {
    uint32_t ret = 0;
...
uint32_t JsonDecoder::dequoteUtf16() {
    expect("u");
...
void JsonDecoder::decodeString(Inserter& inserter) {
    readString(value);
    inserter.insertString(value);
}

void JsonDecoder::decodeObject(Inserter& inserter) {
...
void JsonDecoder::decodeArray(Inserter& inserter) {
...
void JsonDecoder::decodeNumber(Inserter& inserter) {
    bool isLong = true;
    value.clear();
    value.push_back(c);
    next();
    for (;;) {
        ...
        default:
            ...
            if ((endp == value.c_str()) || (errorCode != 0)) {
                std::stringstream ss;
                ss << "error inserting number " << value << ". error code: " << errorCode
                   << ". endp - value: " << (endp - value.c_str());
                in.fail(ss.str());
            }
            return;
        }
...
int insertNumber(Inserter& inserter, bool isLong, const std::string& value, char** endp) {
    int errorCode = 0;
    errno = 0;
    if (isLong) {
...
void JsonFormat::encode(const Inspector& inspector, Output& output, bool compact) {
...
void JsonFormat::encode(const Slime& slime, Output& output, bool compact) { encode(slime.get(), output, compact); }

size_t JsonFormat::decode(Input& input, Slime& slime) {
...
size_t JsonFormat::decode(const Memory& memory, Slime& slime) {
