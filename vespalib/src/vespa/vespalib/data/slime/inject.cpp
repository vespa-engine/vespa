...
void injectNix(const Inserter& inserter) {
    inserter.insertNix();
}
void injectBool(const Inserter& inserter, bool value) {
    inserter.insertBool(value);
}
void injectLong(const Inserter& inserter, int64_t value) {
    inserter.insertLong(value);
}
void injectDouble(const Inserter& inserter, double value) {
    inserter.insertDouble(value);
}
void injectString(const Inserter& inserter, const Memory& memory) {
    inserter.insertString(memory);
}
void injectData(const Inserter& inserter, const Memory& memory) {
    inserter.insertData(memory);
}
...
void injectValue(const Inserter& inserter, const Inspector& inspector, const Inspector* guard) {
    ...
    case NIX::ID:
        return injectNix(inserter);
    case BOOL::ID:
        return injectBool(inserter, inspector.asBool());
    case LONG::ID:
        return injectLong(inserter, inspector.asLong());
    case DOUBLE::ID:
        return injectDouble(inserter, inspector.asDouble());
    case STRING::ID:
        return injectString(inserter, inspector.asString());
    case DATA::ID:
        return injectData(inserter, inspector.asData());
    case ARRAY::ID:
        return injectArray(inserter, inspector, guard);
    case OBJECT::ID:
        return injectObject(inserter, inspector, guard);
    }
    LOG_ABORT("should not be reached");
}

void NestedInjector::entry(size_t, const Inspector& inspector) {
    if (&inspector == guard) {
        return;
    }
...
void NestedInjector::field(const Memory& symbol_name, const Inspector& inspector) {
    if (&inspector == guard) {
        return;
    }
