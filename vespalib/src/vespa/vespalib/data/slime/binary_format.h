...
inline uint64_t encode_zigzag(int64_t x) { return ((x << 1) ^ (x >> 63)); }

inline int64_t decode_zigzag(uint64_t x) { return ((x >> 1) ^ (-(x & 0x1))); }
...
inline char encode_type_and_meta(uint32_t type, uint32_t meta) { return (((type & 0x7) | (meta << 3)) & 0xff); }

inline uint32_t decode_type(uint32_t type_and_meta) { return (type_and_meta & 0x7); }

inline uint32_t decode_meta(uint32_t type_and_meta) { return ((type_and_meta >> 3) & 0x1f); }
...
inline uint64_t read_size(InputReader& in, uint32_t meta) { return (meta == 0) ? read_cmpr_ulong(in) : (meta - 1); }

template <bool top> void write_type_and_bytes(OutputWriter& out, uint32_t type, uint64_t bits) {
...
template <bool top> uint64_t read_bytes(InputReader& in, uint32_t bytes) {
    uint64_t value = 0;
    int      shift = top ? 56 : 0;
