...
SymbolTable::SymbolTable(size_t expectedNumSymbols) : _stash(), _symbols(3 * expectedNumSymbols), _names() {
...
Symbol SymbolTable::insert(const Memory& name) {
...
Symbol SymbolTable::lookup(const Memory& name) const {
    SymbolMap::const_iterator pos = _symbols.find(name);
    if (pos == _symbols.end()) {
        return {};
    }
