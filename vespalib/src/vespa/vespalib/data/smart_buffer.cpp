...
void SmartBuffer::ensure_free(size_t bytes) {
    if (write_len() >= bytes) {
        return;
    }
...
SmartBuffer::SmartBuffer(size_t initial_size)
    : _data(alloc::Alloc::alloc(initial_size)), _read_pos(0), _write_pos(0) {}

SmartBuffer::~SmartBuffer() = default;

Memory SmartBuffer::obtain() { return Memory(read_ptr(), read_len()); }
...
WritableMemory SmartBuffer::reserve(size_t bytes) {
    ensure_free(bytes);
