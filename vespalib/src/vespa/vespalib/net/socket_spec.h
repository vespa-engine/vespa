...
private:
    ...
    SocketSpec(Type type, const std::string& node, int port) : _type(type), _node(node), _port(port) {}
...
public:
    ...
    static SocketSpec from_path(const std::string& path) { return SocketSpec(Type::PATH, path, -1); }
    static SocketSpec from_name(const std::string& name) { return SocketSpec(Type::NAME, name, -1); }
    static SocketSpec from_host_port(const std::string& host, int port) {
        return SocketSpec(Type::HOST_PORT, host, port);
    }
    static SocketSpec  from_port(int port) { return SocketSpec(Type::PORT, "", port); }
