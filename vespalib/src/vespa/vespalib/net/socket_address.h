...
public:
    SocketAddress() noexcept : _size(0) { memset(&_addr, 0, sizeof(sockaddr_storage)); }
    SocketAddress(const SocketAddress& rhs) noexcept : _size(rhs._size) {
    ...
    SocketHandle    connect() const {
        return connect([](SocketHandle&) noexcept { return true; });
    }
