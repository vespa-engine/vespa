...
private:
    ...
    bool looksLikeTlsToMe(const char* buf) {
        return (net::tls::snooping::snoop_client_hello_header(buf) ==
                net::tls::snooping::TlsSnoopingResult::ProbablyTls);
    }
    ...
    HandshakeResult handshake() override {
        if (_factory) {
            ...
            if (looksLikeTlsToMe(src.data)) {
                ...
                auto              tls_socket =
                    std::make_unique<net::tls::CryptoCodecAdapter>(std::move(_socket), std::move(tls_codec));
