...
CapabilitySet CapabilitySet::all() noexcept { return make_with_all_capabilities(); }

CapabilitySet CapabilitySet::content_node() noexcept {
    return CapabilitySet::of({Capability::content_storage_api(), Capability::content_document_api(),
...
CapabilitySet CapabilitySet::container_node() noexcept {
    return CapabilitySet::of({Capability::content_document_api(), Capability::container_document_api(),
                              Capability::content_search_api()})
        .union_of(shared_app_node_capabilities());
}

CapabilitySet CapabilitySet::telemetry() noexcept {
    return CapabilitySet::of({Capability::content_status_pages(), Capability::content_state_api(),
                              Capability::content_metrics_api(), Capability::container_state_api(),
                              Capability::metricsproxy_metrics_api(), Capability::sentinel_connectivity_check()});
}

CapabilitySet CapabilitySet::cluster_controller_node() noexcept {
    return CapabilitySet::of(
               {Capability::content_cluster_controller_internal_state_api(), Capability::client_slobrok_api(),
                Capability::container_document_api()}) // for reindexing
        .union_of(shared_app_node_capabilities());
}

CapabilitySet CapabilitySet::logserver_node() noexcept { return shared_app_node_capabilities(); }

CapabilitySet CapabilitySet::config_server_node() noexcept {
    return CapabilitySet::of({Capability::client_filereceiver_api(), Capability::container_management_api(),
                              Capability::slobrok_api(), Capability::cluster_controller_reindexing(),
                              Capability::cluster_controller_state(), Capability::logserver_api()})
        .union_of(telemetry());
}

CapabilitySet CapabilitySet::shared_app_node_capabilities() noexcept {
    return CapabilitySet::of({Capability::logserver_api(), Capability::configserver_config_api(),
                              Capability::configserver_filedistribution_api(), Capability::configproxy_config_api(),
                              Capability::configproxy_filedistribution_api(), Capability::slobrok_api()})
        .union_of(telemetry());
}
