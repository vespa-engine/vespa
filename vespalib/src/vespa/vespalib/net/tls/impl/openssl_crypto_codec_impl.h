...
class OpenSslCryptoCodecImpl : public CryptoCodec {
    ...
    struct DeferredHandshakeParams {
        ...
        DeferredHandshakeParams(
            const char* from_peer_, size_t from_peer_buf_size_, char* to_peer_, size_t to_peer_buf_size_) noexcept
...
public:
    ...
    static std::unique_ptr<OpenSslCryptoCodecImpl> make_client_codec(
        std::shared_ptr<OpenSslTlsContextImpl> ctx, const SocketSpec& peer_spec, const SocketAddress& peer_address);
    static std::unique_ptr<OpenSslCryptoCodecImpl> make_server_codec(
    ...
    size_t min_encode_buffer_size() const noexcept override { return MaximumTlsFrameSize; }
    size_t min_decode_buffer_size() const noexcept override { return MaximumFramePlaintextSize; }

    HandshakeResult handshake(
        const char* from_peer, size_t from_peer_buf_size, char* to_peer, size_t to_peer_buf_size) noexcept override;

    void do_handshake_work() noexcept override;

    EncodeResult encode(
        const char* plaintext, size_t plaintext_size, char* ciphertext, size_t ciphertext_size) noexcept override;
    DecodeResult decode(
        const char* ciphertext, size_t ciphertext_size, char* plaintext, size_t plaintext_size) noexcept override;
    EncodeResult half_close(char* ciphertext, size_t ciphertext_size) noexcept override;

    [[nodiscard]] const PeerCredentials& peer_credentials() const noexcept override { return _peer_credentials; }

    [[nodiscard]] CapabilitySet granted_capabilities() const noexcept override { return _granted_capabilities; }
    ...
    void set_peer_credentials(PeerCredentials peer_credentials) { _peer_credentials = std::move(peer_credentials); }
    void set_granted_capabilities(CapabilitySet granted_capabilities) {
        _granted_capabilities = granted_capabilities;
    }

private:
    OpenSslCryptoCodecImpl(std::shared_ptr<OpenSslTlsContextImpl> ctx, const SocketSpec& peer_spec,
                           const SocketAddress& peer_address, Mode mode);
