...
std::unique_ptr<net::tls::CryptoCodec> MaybeTlsCryptoEngine::create_tls_client_crypto_codec(
    const SocketHandle& socket, const SocketSpec& spec) {
    return _tls_engine->create_tls_client_crypto_codec(socket, spec);
}

std::unique_ptr<net::tls::CryptoCodec> MaybeTlsCryptoEngine::create_tls_server_crypto_codec(
    const SocketHandle& socket) {
    return _tls_engine->create_tls_server_crypto_codec(socket);
}

CryptoSocket::UP MaybeTlsCryptoEngine::create_client_crypto_socket(SocketHandle socket, const SocketSpec& spec) {
...
CryptoSocket::UP MaybeTlsCryptoEngine::create_server_crypto_socket(SocketHandle socket) {
