...
std::string char_delimited_glob_to_regex(std::string_view glob, char delimiter) {
    std::string ret = "^";
...
public:
    ...
    [[nodiscard]] bool matches(std::string_view str) const noexcept override { return (str == _must_match_exactly); }
};

} // namespace

std::shared_ptr<const CredentialMatchPattern> CredentialMatchPattern::create_from_dns_glob(
...
std::shared_ptr<const CredentialMatchPattern> CredentialMatchPattern::create_from_uri_glob(
    std::string_view glob_pattern) {
    return std::make_shared<const RegexHostMatchPattern>(RegexHostMatchPattern::from_uri_glob_pattern(glob_pattern));
}

std::shared_ptr<const CredentialMatchPattern> CredentialMatchPattern::create_exact_match(std::string_view str) {
...
PeerPolicy::PeerPolicy(
    std::vector<RequiredPeerCredential> required_peer_credentials, CapabilitySet granted_capabilities)
    : _required_peer_credentials(std::move(required_peer_credentials)), _granted_capabilities(granted_capabilities) {}
...
namespace {
template <typename Collection> void print_joined(std::ostream& os, const Collection& coll, const char* sep) {
    bool first = true;
...
std::ostream& operator<<(std::ostream& os, const RequiredPeerCredential& cred) {
    os << "RequiredPeerCredential(" << to_string(cred.field()) << " matches '" << cred.original_pattern() << "')";
    return os;
}
