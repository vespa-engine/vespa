...
public:
     ...
     */
    virtual HandshakeResult handshake(const char* from_peer, size_t from_peer_buf_size, char* to_peer,
                                      size_t to_peer_buf_size) noexcept = 0;
     ...
     */
    virtual EncodeResult encode(const char* plaintext, size_t plaintext_size, char* ciphertext,
                                size_t ciphertext_size) noexcept = 0;
    /*
     ...
     */
    virtual DecodeResult decode(const char* ciphertext, size_t ciphertext_size, char* plaintext,
                                size_t plaintext_size) noexcept = 0;
     ...
     */
    static std::unique_ptr<CryptoCodec> create_default_client_codec(std::shared_ptr<TlsContext> ctx,
                                                                    const SocketSpec&           peer_spec,
                                                                    const SocketAddress&        peer_address);
    static std::unique_ptr<CryptoCodec> create_default_server_codec(std::shared_ptr<TlsContext> ctx,
                                                                    const SocketAddress&        peer_address);
};
