...
std::unique_ptr<CryptoCodec> CryptoCodec::create_default_client_codec(std::shared_ptr<TlsContext> ctx,
...
std::unique_ptr<CryptoCodec> CryptoCodec::create_default_server_codec(std::shared_ptr<TlsContext> ctx,
                                                                      const SocketAddress&        peer_address) {
    auto ctx_impl = std::dynamic_pointer_cast<impl::OpenSslTlsContextImpl>(ctx); // only takes by const ref
    assert(ctx_impl);
