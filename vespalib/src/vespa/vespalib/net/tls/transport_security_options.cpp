...
TransportSecurityOptions::TransportSecurityOptions(
    std::string ca_certs_pem, std::string cert_chain_pem, std::string private_key_pem,
...
TransportSecurityOptions TransportSecurityOptions::copy_without_private_key() const {
    return TransportSecurityOptions(
        _ca_certs_pem, _cert_chain_pem, "", _authorized_peers, _disable_hostname_validation);
}

void secure_memzero(void* buf, size_t size) noexcept { OPENSSL_cleanse(buf, size); }
...
TransportSecurityOptions::Params::~Params() { secure_memzero(&_private_key_pem[0], _private_key_pem.size()); }
...
TransportSecurityOptions::Params& TransportSecurityOptions::Params::operator=(
    TransportSecurityOptions::Params&&) noexcept = default;
