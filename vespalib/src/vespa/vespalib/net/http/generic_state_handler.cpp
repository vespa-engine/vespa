...
public:
    Url(const std::string& host, const std::vector<std::string>& items) : _url("http://") {
        _url.append(host);
        _url += '/';
    ...
    Url(const Url& parent, const std::string& item) : _url(parent._url) { append(item); }
...
JsonGetHandler::Response explore(
    const StateExplorer& state, const std::string& host, const std::vector<std::string>& items, size_t pos) {
    if (pos == items.size()) {
...
GenericStateHandler::GenericStateHandler(const std::string& root_path, const StateExplorer& state)
    : _root(split_path(root_path)), _state(state) {}

JsonGetHandler::Response GenericStateHandler::get(
    const std::string& host, const std::string& path, const std::map<std::string, std::string>&,
