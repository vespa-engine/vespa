...
std::string get_param(const std::map<std::string, std::string>& params, std::string_view param_name,
...
std::string respond_json_metrics(const std::string& consumer, const HealthProducer& healthProducer,
...
JsonGetHandler::Response cap_checked(const net::ConnectionAuthContext& auth_ctx, CapabilitySet required_caps,
...
JsonGetHandler::Response cap_checked(const net::ConnectionAuthContext& auth_ctx, Capability required_cap,
                                     std::function<std::string()> fn) {
...
JsonGetHandler::Response StateApi::get(const std::string& host, const std::string& path,
        ...
        if (path == "/state/v1/health") {
            ...
            return cap_check_and_respond_metrics(auth_ctx, params, "statereporter", [&](auto& consumer, auto format) {
                if (format == MetricsProducer::ExpositionFormat::Prometheus) {
                    ...
                    return JsonGetHandler::Response::make_ok_with_content_type(std::move(metrics_text),
