...
template <typename T> void remove_seq(T& collection, size_t seq) {
    collection.erase(std::remove_if(collection.begin(), collection.end(),
                                    [seq](const typename T::value_type& item) { return (item.seq == seq); }),
                     collection.end());
}

} // namespace

size_t JsonHandlerRepo::State::bind(std::string_view path_prefix, const JsonGetHandler& get_handler) {
...
size_t JsonHandlerRepo::State::add_root_resource(std::string_view path) {
    std::lock_guard<std::mutex> guard(lock);
    size_t                      my_seq = ++seq;
...
void JsonHandlerRepo::State::unbind(size_t my_seq) {
    std::lock_guard<std::mutex> guard(lock);
...
JsonHandlerRepo::JsonHandlerRepo() : _state(std::make_shared<State>()) {
}
JsonHandlerRepo::~JsonHandlerRepo() = default;
JsonHandlerRepo::Token::UP JsonHandlerRepo::bind(std::string_view path_prefix, const JsonGetHandler& get_handler) {
    return std::make_unique<Unbinder>(_state, _state->bind(path_prefix, get_handler));
}

JsonHandlerRepo::Token::UP JsonHandlerRepo::add_root_resource(std::string_view path) {
    return std::make_unique<Unbinder>(_state, _state->add_root_resource(path));
}

std::vector<std::string> JsonHandlerRepo::get_root_resources() const {
...
JsonGetHandler::Response JsonHandlerRepo::get(const std::string& host, const std::string& path,
