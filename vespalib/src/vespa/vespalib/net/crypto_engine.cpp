...
AuthorizationMode authorization_mode_from_env() {
    ...
    if (mode == "enforce") {
        ...
        LOG(warning,
            "VESPA_TLS_INSECURE_AUTHORIZATION_MODE environment variable has "
...
CryptoEngine::SP create_default_crypto_engine() {
    ...
    if (mixed_mode == "plaintext_client_mixed_server") {
        ...
        LOG(warning, "bad TLS insecure mixed-mode specified: '%s' (ignoring)", mixed_mode.c_str());
    }
    return tls;
}
...
CryptoEngine::SP CryptoEngine::get_default() {
...
CryptoSocket::UP NullCryptoEngine::create_client_crypto_socket(SocketHandle socket, const SocketSpec&) {
    net::tls::ConnectionStatistics::get(false).inc_insecure_connections();
    return std::make_unique<NullCryptoSocket>(std::move(socket));
}

CryptoSocket::UP NullCryptoEngine::create_server_crypto_socket(SocketHandle socket) {
