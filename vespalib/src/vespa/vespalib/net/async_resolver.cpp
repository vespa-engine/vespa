...
AsyncResolver::time_point AsyncResolver::SteadyClock::now() {
...
bool AsyncResolver::CachingHostResolver::should_evict_oldest_entry(const std::lock_guard<std::mutex>&,
                                                                   time_point now) {
    if (_queue.empty()) {
        return false;
    }
...
AsyncResolver::CachingHostResolver::CachingHostResolver(Clock::SP clock, HostResolver::SP resolver,
                                                        size_t max_cache_size, seconds max_result_age) noexcept
...
void AsyncResolver::wait_for_pending_resolves() {
    _executor->sync();
}
...
AsyncResolver::SP AsyncResolver::create(Params params) {
    ...
    auto cacher = std::make_shared<CachingHostResolver>(std::move(params.clock), std::move(logger),
                                                        params.max_cache_size, params.max_result_age);
