...
struct BlockingGuard {
    int fd;
    BlockingGuard(int fd_in) : fd(fd_in) { SocketOptions::set_blocking(fd, true); }
    ~BlockingGuard() { SocketOptions::set_blocking(fd, false); }
};
...
struct UringProbe {
    ...
    bool        check(int opcode) { return probe && io_uring_opcode_supported(probe, opcode); }
    static bool check_support() {
        UringProbe probe;
        return probe.check(IORING_OP_ACCEPT) && probe.check(IORING_OP_CONNECT) && probe.check(IORING_OP_READ) &&
...
auto wait_for_sqe(auto* sqe) {
    return wait_for<cqe_res_t>([sqe](auto wf) { io_uring_sqe_set_data(sqe, wf.release()); });
}
...
struct IoUringThread : AsyncIo {
    ...
    IoUringThread()
        : _uring(), _event(eventfd(0, 0)), _thread(), _thread_id(std::thread::id()), _todo(), _lock(), _queue() {
...
void IoUringThread::init_shutdown() { async_shutdown(); }

void IoUringThread::fini_shutdown() { _thread.join(); }
