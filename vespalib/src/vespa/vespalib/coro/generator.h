 ...
 **/
template <typename R, typename V = void> class [[nodiscard]] Generator {
public:
    ...
    static constexpr bool extra_yield =
        std::is_rvalue_reference_v<yield_type> && std::constructible_from<cpy_type, cref_yield>;
    ...
    private:
        ...
        struct copy_awaiter : std::suspend_always {
            copy_awaiter(const cpy_type& value, ptr_type& ptr) : value_cpy(value) { ptr = std::addressof(value_cpy); }
    ...
    public:
        ...
        auto yield_value(cref_yield value)
            requires extra_yield
        {
            return copy_awaiter(value, _state);
        }
        void                      return_void() noexcept {}
        void                      unhandled_exception() { throw; }
        ref_type                  result() noexcept { return static_cast<ref_type>(*_state); }
    ...
    public:
        ...
        explicit Iterator(Handle handle) : _handle(handle) { _handle.resume(); }
        ...
        ref_type operator*() const { return _handle.promise().result(); }
    };
