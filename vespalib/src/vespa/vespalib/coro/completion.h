...
template <typename T, typename R> Detached connect_resume(Lazy<T> value, R receiver) {
...
template <typename T> std::future<T> make_future(Lazy<T> value) {
 ...
 **/
template <typename T> T sync_wait(Lazy<T> value) { return make_future(std::move(value)).get(); }
 ...
 * afterwards.
 **/
template <typename T, typename F> void async_wait(Lazy<T> value, F&& f) {
