...
namespace {
bool can_use_io_uring() {
    return vespalib::coro::UringProbe::check_support();
}
vespalib::coro::AsyncIo::SP create_io_uring_thread() {
    return std::make_shared<vespalib::coro::IoUringThread>();
}
} // namespace
#else
namespace {
...
void SelectorThread::init_shutdown() {
    async_shutdown();
}

void SelectorThread::fini_shutdown() {
    _thread.join();
}
