...
template <typename T> PromiseState<T>::~PromiseState() = default;
...
template <typename T> class WaitingFor {
private:
...
public:
    ...
    template <typename RET> void set_value(RET&& value) { _state->result.set_value(std::forward<RET>(value)); }
    void                         set_error(std::exception_ptr exception) { _state->result.set_error(exception); }
    void                         set_done() { _state->result.set_done(); }
    void*                        release() { return std::exchange(_state, nullptr); }
    static WaitingFor            from_pointer(void* ptr) {
        PromiseState<T>* state = reinterpret_cast<PromiseState<T>*>(ptr);
        return {state};
    }
    static WaitingFor from_state(PromiseState<T>& state) { return {&state}; }
    ...
    [[nodiscard]] static std::coroutine_handle<> nop() noexcept { return std::noop_coroutine(); }
};

template <typename T> WaitingFor<T>::~WaitingFor() {
    if (_state) {
        _state->waiter.resume();
    }
}
