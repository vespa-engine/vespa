...
LevenshteinDfa::MatchResult LevenshteinDfa::match(std::string_view u8str) const {
...
LevenshteinDfa::MatchResult LevenshteinDfa::match(std::string_view       u8str,
...
LevenshteinDfa LevenshteinDfa::build(std::string_view target_string, uint8_t max_edits, Casing casing,
    ...
    if (dfa_type == DfaType::Implicit) {
        ...
        if (max_edits == 1) {
            return ExplicitLevenshteinDfaBuilder<FixedMaxEditDistanceTraits<1>>(std::move(target_string_u32),
                                                                                is_cased, is_prefix)
                .build_dfa();
        } else { // max_edits == 2
            return ExplicitLevenshteinDfaBuilder<FixedMaxEditDistanceTraits<2>>(std::move(target_string_u32),
                                                                                is_cased, is_prefix)
                .build_dfa();
        }
    } else { // DfaType::Table
