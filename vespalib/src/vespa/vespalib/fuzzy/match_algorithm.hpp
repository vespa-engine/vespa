...
template <uint8_t MaxEdits> struct MatchAlgorithm {
    ...
    static MatchResult minimal_matching_prefix_distance(const Matcher& matcher, typename Matcher::StateType state,
     ...
     */
    template <DfaMatcher Matcher, typename SuccessorT>
    static void backtrack_and_emit_greater_suffix(const Matcher&                   matcher,
    ...
    template <DfaMatcher Matcher, typename SuccessorT>
    static void emit_smallest_matching_suffix(const Matcher& matcher, typename Matcher::StateParamType from,
                                              SuccessorT& str) {
        auto state = from;
