...
concept DfaMatcher = requires(T a, std::string u8str, std::vector<uint32_t> u32str) {
    ...
    {
        a.match_edit_distance(typename T::StateType{})
    } -> std::same_as<uint8_t>;
    ...
    {
        a.match_input(typename T::StateType{}, uint32_t{})
    } -> std::same_as<typename T::StateType>;
    ...
    {
        a.match_wildcard(typename T::StateType{})
    } -> std::same_as<typename T::StateType>;
    ...
    {
        a.implies_exact_match_suffix(typename T::StateType{})
    } -> std::same_as<bool>;
    ...
    {
        a.emit_exact_match_suffix(typename T::StateType{}, u8str)
    } -> std::same_as<void>;

    // Same as above, but for raw UTF-32 code point output
    {
        a.emit_exact_match_suffix(typename T::StateType{}, u32str)
    } -> std::same_as<void>;
};
