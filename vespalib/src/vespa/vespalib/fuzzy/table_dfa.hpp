...
template <> constexpr size_t num_states<1>() {
    return 6;
}
template <> constexpr size_t num_states<2>() {
    return 31;
}
template <> constexpr size_t num_states<3>() {
    return 197;
}

template <uint8_t N> constexpr size_t window_size() {
    return 2 * N + 1;
}
template <uint8_t N> constexpr size_t num_transitions() {
    return 1 << window_size<N>();
}
...
struct Position {
    uint32_t index;
    uint32_t edits;
    Position(uint32_t index_in, uint32_t edits_in) noexcept : index(index_in), edits(edits_in) {}
    ...
    bool operator==(const Position& rhs) const noexcept { return (index == rhs.index) && (edits == rhs.edits); }
...
struct State {
    ...
    bool operator<(const State& rhs) const { return list < rhs.list; }
...
// separately (index).
template <uint8_t N> struct TfaState {
    uint32_t index;
    uint32_t state;
    ...
    constexpr bool is_valid_edge(uint32_t bits) const noexcept { return InlineTfa<N>::table[state][bits].state != 0; }
...
template <uint8_t N> struct TableMatcher {
    using S = TfaState<N>;
    ...
    TableMatcher(const TableDfa<N>::Lookup* lookup_in, uint32_t end_in, bool cased_in, bool prefix_in) noexcept
        : lookup(lookup_in), end(end_in), cased(cased_in), prefix(prefix_in) {}
...
template <uint8_t N> auto TableDfa<N>::make_lookup(const std::vector<uint32_t>& str) -> std::vector<Lookup> {
...
TableDfa<N>::TableDfa(std::vector<uint32_t> str, bool is_cased, bool is_prefix)
    : _lookup(make_lookup(str)), _is_cased(is_cased), _is_prefix(is_prefix) {
}

template <uint8_t N> TableDfa<N>::~TableDfa() = default;

template <uint8_t N> LevenshteinDfa::MatchResult TableDfa<N>::match(std::string_view u8str) const {
    TableMatcher<N> matcher(_lookup.data(), _lookup.size() - 1, _is_cased, _is_prefix);
    return MatchAlgorithm<N>::match(matcher, u8str);
}

template <uint8_t N>
LevenshteinDfa::MatchResult TableDfa<N>::match(std::string_view u8str, std::string& successor_out) const {
    TableMatcher<N> matcher(_lookup.data(), _lookup.size() - 1, _is_cased, _is_prefix);
    return MatchAlgorithm<N>::match(matcher, u8str, successor_out);
}

template <uint8_t N>
LevenshteinDfa::MatchResult TableDfa<N>::match(std::string_view u8str, std::vector<uint32_t>& successor_out) const {
    TableMatcher<N> matcher(_lookup.data(), _lookup.size() - 1, _is_cased, _is_prefix);
    return MatchAlgorithm<N>::match(matcher, u8str, successor_out);
}

template <uint8_t N> size_t TableDfa<N>::memory_usage() const noexcept {
    return _lookup.size() * sizeof(Lookup);
}

template <uint8_t N> void TableDfa<N>::dump_as_graphviz(std::ostream& os) const {
    ...
    auto explore = [explored = std::set<state_t>()](state_t state) mutable { return explored.insert(state).second; };
    auto edits = [end = (_lookup.size() - 1)](state_t state) noexcept -> uint32_t { return state.exact_edits(end); };
    struct Edge {
        uint32_t input;
        state_t  from;
        state_t  to;
