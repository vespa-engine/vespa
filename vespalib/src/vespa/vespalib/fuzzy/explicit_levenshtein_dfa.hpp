...
template <uint8_t MaxEdits>
LevenshteinDfa::MatchResult ExplicitLevenshteinDfaImpl<MaxEdits>::match(std::string_view u8str,
                                                                        std::string&     successor_out) const {
    ExplicitDfaMatcher<MaxEdits> matcher(_nodes, _is_cased, _is_prefix);
...
template <uint8_t MaxEdits>
LevenshteinDfa::MatchResult ExplicitLevenshteinDfaImpl<MaxEdits>::match(std::string_view       u8str,
                                                                        std::vector<uint32_t>& successor_out) const {
    ExplicitDfaMatcher<MaxEdits> matcher(_nodes, _is_cased, _is_prefix);
    return MatchAlgorithm<MaxEdits>::match(matcher, u8str, successor_out);
}
