...
vespalib::FuzzyMatcher::FuzzyMatcher(
    std::string_view term, uint32_t max_edit_distance, uint32_t prefix_size, bool is_cased, bool is_prefix)
...
std::span<const uint32_t> vespalib::FuzzyMatcher::get_prefix(
    const std::vector<uint32_t>& termCodepoints, uint32_t prefixLength) {
...
std::span<const uint32_t> vespalib::FuzzyMatcher::get_suffix(
    const std::vector<uint32_t>& termCodepoints, uint32_t prefixLength) {
...
bool vespalib::FuzzyMatcher::isMatch(std::string_view target) const {
    std::vector<uint32_t> targetCodepoints =
        _is_cased ? cased_convert_to_ucs4(target) : LowerCase::convert_to_ucs4(target);
    ...
    return LevenshteinDistance::calculate(_folded_term_codepoints_suffix, get_suffix(targetCodepoints, _prefix_size),
                                          _max_edit_distance, _is_prefix)
        .has_value();
}
