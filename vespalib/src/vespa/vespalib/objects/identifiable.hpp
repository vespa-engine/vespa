...
template <typename T> Serializer& Identifiable::serialize(const T& v, Serializer& os) {
    uint32_t sz(v.size());
    os.put(sz);
...
template <typename T> Deserializer& Identifiable::deserialize(T& v, Deserializer& is) {
    uint32_t sz(0);
    is.get(sz);
    v.resize(sz);
    for (size_t i(0); i < sz; i++) {
...
template <typename T> class IdentifiablePtr : public CloneablePtr<T> {
public:
    ...
    IdentifiablePtr(std::unique_ptr<T>&& rhs) noexcept : CloneablePtr<T>(std::move(rhs)) {}
        ...
        if (hasObject) {
            ...
            if (up->inherits(T::classId)) {
                ...
                fprintf(stderr, "ERROR: tried to deserialize %s (%x) but got %s (%x)\n",
                        Identifiable::classFromId(T::classId)->name(), T::classId, up->getClass().name(),
                        up->getClass().id());
                throw std::bad_cast();
            }
        }
...
template <typename T> class IdentifiableSharedPtr : public std::shared_ptr<T> {
public:
    ...
    bool        operator<(const IdentifiableSharedPtr<T>& rhs) const { return (cmp(rhs) < 0); }
    Serializer& serialize(Serializer& os) const {
        if (this->get()) {
            os.put(uint8_t(1)) << *this->get();
        } else {
