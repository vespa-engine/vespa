...
private:
    ...
    struct EqualName {
        bool operator()(const RuntimeClass* a, const RuntimeClass* b) const {
...
Register::Register() : _listById(), _listByName() {}
...
const Identifiable::RuntimeClass* Identifiable::classFromId(unsigned id) { return _register->classFromId(id); }

const Identifiable::RuntimeClass* Identifiable::classFromName(const char* name) {
    return _register->classFromName(name);
}

Identifiable::RuntimeClass::RuntimeClass(RuntimeInfo* info_) : _rt(info_) {
    if (_rt->_factory) {
        ...
        for (const RuntimeInfo* curr = _rt; curr && curr != curr->_base; curr = curr->_base) {
            ...
            if (!curr->_tryCast(tmp.get())) {
                throw std::runtime_error(make_string("(%s, %s) is not a baseclass of (%s, %s)", curr->_name,
                                                     curr->_typeId().name(), name(), typeid(tmpref).name()));
            }
        }
    }
    ...
    if (!_register->append(this)) {
        ...
        throw std::runtime_error(make_string("Duplicate Identifiable object(%s, %s, %d) being registered. Choose a "
                                             "unique id. Object (%s, %s, %d) is using it.",
...
Deserializer& operator>>(Deserializer& os, Identifiable& obj) {
    ...
    if (cid == obj.getClass().id()) {
        ...
        throw std::runtime_error(
            make_string("Failed deserializing %s : Received cid %d(%0x) != %d(%0x)", obj.getClass().name(), cid, cid,
    ...
    if (rtc != nullptr) {
        ...
        if (obj.get()) {
            ...
            throw std::runtime_error(make_string(
                "Failed deserializing an Identifiable for classId %d(%0x). "
                "It is abstract, so it can not be instantiated. Does it need to be abstract ?",
                cid, cid));
        }
    } else {
        throw std::runtime_error(
            make_string("Failed deserializing an Identifiable with unknown classId %d(%0x)", cid, cid));
    }
    return obj;
}

std::string Identifiable::getNativeClassName() const { return vespalib::getClassName(*this); }

std::string Identifiable::asString() const {
    ObjectDumper dumper;
    visit(dumper, "", this);
...
void Identifiable::visitMembers(ObjectVisitor& visitor) const { visitor.visitNotImplemented(); }

void Identifiable::select(const ObjectPredicate& predicate, ObjectOperation& operation) {
...
void Identifiable::selectMembers(const ObjectPredicate& predicate, ObjectOperation& operation) {
    (void)predicate;
    (void)operation;
}

Serializer& Identifiable::serialize(Serializer& os) const { return os.put(*this); }

Deserializer& Identifiable::deserialize(Deserializer& is) { return is.get(*this); }

Serializer& Identifiable::onSerialize(Serializer& os) const { return os; }

Deserializer& Identifiable::onDeserialize(Deserializer& is) { return is; }
