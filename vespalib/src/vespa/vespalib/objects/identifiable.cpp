...
const Identifiable::RuntimeClass* Identifiable::classFromId(unsigned id) {
...
Deserializer& operator>>(Deserializer& os, Identifiable& obj) {
    ...
    if (cid == obj.getClass().id()) {
        ...
        throw std::runtime_error(make_string("Failed deserializing %s : Received cid %d(%0x) != %d(%0x)",
                                             obj.getClass().name(), cid, cid, obj.getClass().id(),
                                             obj.getClass().id()));
...
std::string Identifiable::getNativeClassName() const {
...
void Identifiable::visitMembers(ObjectVisitor& visitor) const {
