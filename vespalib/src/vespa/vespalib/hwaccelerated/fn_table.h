...
struct FnTable {
    ...
    // New entries to the function table must also be added to VESPA_HWACCEL_VISIT_FN_TABLE
...
// the current run-time platform environment. The function pointers will never
// change after this, and may be considered immutable as soon as main() has
// been ceremoniously entered. I.e. all pointer loads can be non-atomic.
...
// Defined in iaccelerated.cpp
[[nodiscard]] FnTable optimal_composite_fn_table() noexcept;
[[nodiscard]] FnTable complete_baseline_fn_table() noexcept;
// This will usually be equal to optimal_composite_fn_table() unless overridden at runtime
...
// Convenience function to build a composite table on top of the best baseline
// function table _without_ ignoring suboptimal functions in `fn_table`.
[[nodiscard]] FnTable build_composite_fn_table_from_baseline(const FnTable& fn_table) noexcept;
...
void thread_unsafe_update_vector_dispatch_function_pointers(const FnTable& fns);
