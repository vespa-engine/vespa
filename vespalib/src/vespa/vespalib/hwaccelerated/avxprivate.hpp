...
template <typename T, typename V> T sumT(const V& v) noexcept {
    T sum(0);
...
template <typename T, size_t C> T sumR(const T* v) noexcept {
    if (C == 1) {
        return v[0];
    } else if (C == 2) {
        return v[0] + v[1];
...
double euclideanDistanceT(const T* af, const T* bf, size_t sz) noexcept {
    ...
    for (size_t i(0); i < numChunks; i++) {
        for (size_t j(0); j < VectorsPerChunk; j++) {
            partial[j] += (a[VectorsPerChunk * i + j] - b[VectorsPerChunk * i + j]) *
                          (a[VectorsPerChunk * i + j] - b[VectorsPerChunk * i + j]);
        }
    }
    double sum(0);
    for (size_t i(numChunks * ChunkSize); i < sz; i++) {
