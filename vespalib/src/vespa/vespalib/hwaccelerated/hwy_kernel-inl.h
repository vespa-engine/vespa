...
// definitions and usages of a given accumulator. This is to avoid stalling
// the CPU pipeline by having to wait for in-flight instructions to settle
// the next time the accumulator is loaded.
// We do this by constexpr-"striping" accumulator references based on which
// iteration in the unrolled loop body we're currently at. For instance, with
// 8x unrolling, 4x accumulators and an 1-ary kernel, the loop body will be:
