...
template <size_t Idx, typename KernelFn, typename VecT, typename AccuV>
HWY_INLINE void dispatch(FnAccuArity<1>, IterNum<Idx>, KernelFn&& kernel_fn, VecT vec, AccuV& accu0, AccuV& accu1,
                         AccuV& accu2, AccuV& accu3, AccuV& accu4, AccuV& accu5, AccuV& accu6,
                         AccuV& accu7) noexcept {
...
template <size_t Idx, typename KernelFn, typename LhsT, typename RhsT, typename AccuV>
HWY_INLINE void dispatch_pairwise(FnAccuArity<1>, IterNum<Idx>, KernelFn&& kernel_fn, LhsT lhs, RhsT rhs,
                                  AccuV& accu0, AccuV& accu1, AccuV& accu2, AccuV& accu3, AccuV& accu4, AccuV& accu5,
...
template <size_t Idx, typename KernelFn, typename VecT, typename AccuV>
HWY_INLINE void dispatch(FnAccuArity<8>, IterNum<Idx>, KernelFn&& kernel_fn, VecT vec, AccuV& accu0, AccuV& accu1,
                         AccuV& accu2, AccuV& accu3, AccuV& accu4, AccuV& accu5, AccuV& accu6,
                         AccuV& accu7) noexcept {
...
template <size_t Idx, typename KernelFn, typename LhsT, typename RhsT, typename AccuV>
HWY_INLINE void dispatch_pairwise(FnAccuArity<8>, IterNum<Idx>, KernelFn&& kernel_fn, LhsT lhs, RhsT rhs,
                                  AccuV& accu0, AccuV& accu1, AccuV& accu2, AccuV& accu3, AccuV& accu4, AccuV& accu5,
                                  AccuV& accu6, AccuV& accu7) noexcept {
...
template <> struct UnrolledLoopBody<1> {
    ...
    HWY_INLINE static void elementwise_load_and_dispatch(const FnAccuArity<Arity> arity, D d, const T* HWY_RESTRICT a,
                                                         size_t i, const size_t N, KernelFn kernel_fn,
    ...
    template <size_t Arity, typename D, typename T, typename KernelFn, typename... AccuVecs>
    HWY_INLINE static void pairwise_load_and_dispatch(const FnAccuArity<Arity> arity, D d, const T* HWY_RESTRICT a,
                                                      const T* HWY_RESTRICT b, size_t i, const size_t N,
                                                      KernelFn kernel_fn, AccuVecs&&... accu_vecs) noexcept {
        (void)N;
        const auto a0 = hn::LoadU(d, a + i);
...
template <size_t UnrollFactor> struct KernelBody {
    ...
    HWY_INLINE static void pairwise_body_impl(const FnAccuArity<Arity> arity, const D d, const T* HWY_RESTRICT a,
    ...
    template <size_t Arity, typename D, typename T = hn::TFromD<D>, typename KernelFn, typename... Accumulators>
    HWY_INLINE static void elementwise_body_impl(const FnAccuArity<Arity> arity, const D d, const T* HWY_RESTRICT a,
...
struct HwyReduceKernel<UsesNAccumulators<4>, UnrolledByT, FnHasAccuArityT> {
    ...
    HWY_INLINE static AccuV parallel_reduce_accumulators(AccuReducerFn accu_reducer_fn, AccuV accu0, AccuV accu1,
                                                         AccuV accu2, AccuV accu3) noexcept {
    ...
    [[nodiscard]] static R elementwise(const D d, const DA da, const T* HWY_RESTRICT a, const size_t n_elems,
                                       const hn::Vec<DA> init_accu, const KernelFn kernel_fn,
        ...
        KernelBody<UnrollFactor>::elementwise_body_impl(FnAccuArity<Arity>{}, d, a, n_elems, kernel_fn, accu0, accu1,
                                                        accu2, accu3);
    ...
    template <typename D, typename DA, typename T = hn::TFromD<D>, typename R = hn::TFromD<DA>, typename KernelFn,
              typename AccuReducerFn, typename LaneReducerFn>
    [[nodiscard]] static R pairwise(const D d, const DA da, const T* HWY_RESTRICT a, const T* HWY_RESTRICT b,
                                    const size_t n_elems, const hn::Vec<DA> init_accu, const KernelFn kernel_fn,
                                    const AccuReducerFn accu_reducer_fn,
                                    const LaneReducerFn lane_reducer_fn) noexcept {
        using AccuV = hn::Vec<DA>;
        AccuV accu0 = init_accu;
...
struct HwyReduceKernel<UsesNAccumulators<8>, UnrolledByT, FnHasAccuArityT> {
    ...
    template <typename AccuReducerFn, typename AccuV>
    HWY_INLINE static AccuV parallel_reduce_accumulators(AccuReducerFn accu_reducer_fn, AccuV accu0, AccuV accu1,
                                                         AccuV accu2, AccuV accu3, AccuV accu4, AccuV accu5,
                                                         AccuV accu6, AccuV accu7) noexcept {
...
[[nodiscard]] SumT compute_chunked_sum(F&& fn, const T* HWY_RESTRICT lhs, const T* HWY_RESTRICT rhs,
