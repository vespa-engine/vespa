...
public:
    constexpr TargetInfo() noexcept
        : _implementation_name("Unknown"), _target_name("Unknown"), _vector_width_bytes(16) {}

    constexpr TargetInfo(
        const char* implementation_name, const char* target_name, uint16_t vector_width_bytes) noexcept
    ...
    [[nodiscard]] constexpr const char* implementation_name() const noexcept { return _implementation_name; }
    ...
    [[nodiscard]] constexpr const char* target_name() const noexcept { return _target_name; }
    [[nodiscard]] constexpr uint16_t    vector_width_bytes() const noexcept { return _vector_width_bytes; }
    [[nodiscard]] constexpr uint16_t    vector_width_bits() const noexcept { return _vector_width_bytes * 8; }
    ...
    [[nodiscard]] constexpr bool operator==(const TargetInfo& rhs) const noexcept {
        return (std::string_view{_implementation_name} == rhs._implementation_name &&
                std::string_view{_target_name} == rhs._target_name && _vector_width_bytes == rhs._vector_width_bytes);
    }
