 ...
 */
template <std::unsigned_integral T, size_t ExpBits, size_t ExpBias> struct Float4E2M1Conv {
    ...
    constexpr static T widen(uint8_t v) noexcept {
        ...
        return ((my_sign << (wide_bits - 1)) | (adj_exp << wide_mantissa_bit_count) | adj_mantissa);
    }
};
