...
void verify_or_128(const std::vector<std::vector<uint64_t>>& vectors, size_t offset, size_t num_vectors,
                   bool invertSome) {
...
void verify_and_128(const std::vector<std::vector<uint64_t>>& vectors, size_t offset, size_t num_vectors,
                    bool invertSome) {
    std::vector<std::pair<const void*, bool>> vRefs;
    for (size_t j(0); j < num_vectors; j++) {
        vRefs.emplace_back(&vectors[j][0], should_invert(invertSome));
...
FnTable build_composite_fn_table(const FnTable& fn_table, const FnTable& base_table,
