...
#define VESPA_HWACCEL_PATCH_FN_TABLE_VISITOR2(fn_type, fn_field, fn_id) \
    if ((src_tbl.fn_field) != nullptr && (ignore_suboptimal || !src_tbl.fn_is_tagged_as_suboptimal(fn_id))) { \
        ...
        composite_tbl._fn_target_info[static_cast<size_t>(fn_id)] = src_tbl._fn_target_info[static_cast<size_t>(fn_id)]; \
    }
...
[[nodiscard]] FnTable build_composite_fn_table(std::span<const FnTable> fn_tables, bool ignore_suboptimal) noexcept {
    ...
    for (const auto& src_tbl : std::ranges::reverse_view(fn_tables)) {
        VESPA_HWACCEL_VISIT_FN_TABLE(VESPA_HWACCEL_PATCH_FN_TABLE_VISITOR2);
    }
    return composite_tbl;
}

FnTable build_composite_fn_table_from_baseline(const FnTable& fn_table) noexcept {
    std::vector<FnTable> fn_tables;
    fn_tables.emplace_back(fn_table);
    fn_tables.emplace_back(complete_baseline_fn_table());
    return build_composite_fn_table(fn_tables, false);
}
...
FnTable complete_baseline_fn_table() noexcept {
    static auto baseline_table = IAccelerated::create_best_auto_vectorized_target()->fn_table();
...
BuildFnTableAndPatchFunctionsAtStartup::BuildFnTableAndPatchFunctionsAtStartup() {
    thread_unsafe_update_vector_dispatch_function_pointers(optimal_composite_fn_table());
}
