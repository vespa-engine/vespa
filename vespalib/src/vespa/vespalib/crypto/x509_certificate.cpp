...
X509Certificate::DistinguishedName& X509Certificate::DistinguishedName::operator=(
    DistinguishedName&&) noexcept = default;
...
X509Certificate::Params X509Certificate::Params::self_signed(SubjectInfo subject, std::shared_ptr<PrivateKey> key) {
    Params params;
...
X509Certificate::Params X509Certificate::Params::issued_by(
    SubjectInfo subject, std::shared_ptr<PrivateKey> subject_key, std::shared_ptr<X509Certificate> issuer,
...
X509Certificate::SubjectInfo::SubjectInfo(DistinguishedName dn_) noexcept : dn(std::move(dn_)), subject_alt_names() {}
...
CertKeyWrapper::CertKeyWrapper(std::shared_ptr<X509Certificate> cert_, std::shared_ptr<PrivateKey> key_)
    : cert(std::move(cert_)), key(std::move(key_)) {}
