...
bool TestDataBase::equiv_buffers(const nbostream& lhs, const nbostream& rhs) {
    return lhs.size() == rhs.size() && memcmp(lhs.data(), rhs.data(), lhs.size()) == 0;
}

nbostream TestDataBase::read_buffer_from_file(const std::string& path) {
...
void TestDataBase::write_buffer_to_file(const nbostream& buf, const std::string& path) {
