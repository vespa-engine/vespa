...
PtrAndSize MemoryAllocatorObserver::alloc(size_t sz) const {
    ++_stats.alloc_cnt;
    return _backing_allocator->alloc(sz);
}

void MemoryAllocatorObserver::free(PtrAndSize alloc) const noexcept {
    ++_stats.free_cnt;
    _backing_allocator->free(alloc);
}

size_t MemoryAllocatorObserver::resize_inplace(PtrAndSize current, size_t newSize) const {
