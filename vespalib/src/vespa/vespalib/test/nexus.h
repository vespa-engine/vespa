...
class Nexus;
template <typename T>
concept nexus_thread_entry = requires(Nexus& ctx, T&& entry) { entry(ctx); };
...
private:
    ...
    vote_t& _vote;
    size_t  _thread_id;
    Nexus(vote_t& vote, size_t thread_id) noexcept : _vote(vote), _thread_id(thread_id) {}
    ~Nexus();
...
public:
    ...
    constexpr static auto merge_sum() {
        return [](auto a, auto b) { return a + b; };
    }
    static auto run(size_t num_threads, auto&& entry, auto&& merge)
        requires nexus_thread_entry<decltype(entry)>
    {
        ThreadPool pool;
        ...
        auto handle_result = [&](Nexus& ctx, stored_t thread_result) noexcept {
            if constexpr (std::same_as<std::decay_t<decltype(merge)>, select_thread_0>) {
                ...
                std::lock_guard guard(lock);
                if (result.has_value()) {
                    result = merge(std::move(result).value(), std::move(thread_result));
                } else {
                    result = std::move(thread_result);
                }
            }
    ...
    static auto run(size_t num_threads, auto&& entry)
        requires nexus_thread_entry<decltype(entry)>
    {
