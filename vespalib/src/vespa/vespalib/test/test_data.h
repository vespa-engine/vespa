...
template <class Derived> class TestData : public TestDataBase {
protected:
...
template <class Derived> std::string TestData<Derived>::_source_testdata;

template <class Derived> std::string TestData<Derived>::_build_testdata;

template <class Derived>
void TestData<Derived>::setup_test_data(const std::string& source_testdata_in, const std::string& build_testdata_in) {
...
template <class Derived> void TestData<Derived>::tear_down_test_data() { std::filesystem::remove(build_testdata()); }

template <class Derived>
void TestData<Derived>::remove_unchanged_build_testdata_file_or_fail(
    const nbostream& buf, const std::string& file_name) {
    ...
    ASSERT_TRUE(equiv_buffers(exp_buf, buf))
        << "Files " << exp_path << " and  " << act_path << " have diferent contents";
