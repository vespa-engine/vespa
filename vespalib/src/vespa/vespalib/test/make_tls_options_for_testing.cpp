...
struct TransientCryptoCredentials {
    ...
    static CertKeyWrapper make_host_creds(const CertKeyWrapper& root_ca_creds, const std::string& extra_san_entry) {
        auto dn = X509Certificate::DistinguishedName()
                      .country("US")
                      .state("CA")
                      .locality("Sunnyvale")
        ...
        auto subject = X509Certificate::SubjectInfo(std::move(dn));
        subject.add_subject_alt_name("DNS:localhost").add_subject_alt_name(extra_san_entry);
        auto key = PrivateKey::generate_p256_ec_key();
        auto params =
            X509Certificate::Params::issued_by(std::move(subject), key, root_ca_creds.cert, root_ca_creds.key);
      ...
      cached_constrained_transport_options(
          vespalib::net::tls::TransportSecurityOptions::Params()
              ...
              .authorized_peers(authorized_peers(
                  {policy_with({required_san_dns("anvils.example")}, CapabilitySet::telemetry())}))) {}
