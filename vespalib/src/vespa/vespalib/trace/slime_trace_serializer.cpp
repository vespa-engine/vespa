...
SlimeTraceSerializer::SlimeTraceSerializer(Cursor& cursor) : _cursors() { _cursors.push(&cursor); }
...
void SlimeTraceSerializer::addTimestamp(Cursor& current, const TraceNode& node) {
    current.setLong(TIMESTAMP, count_ms(node.getTimestamp().time_since_epoch()));
}

void SlimeTraceSerializer::addPayload(Cursor& current, const TraceNode& node) {
    if (node.hasNote()) {
        current.setString(PAYLOAD, Memory(node.getNote()));
    }
}

void SlimeTraceSerializer::addChildrenCursors(Cursor& current, const TraceNode& node) {
    if (node.getNumChildren() > 0) {
        addChildrenCursorsToStack(current.setArray(CHILDREN), node);
    }
}

void SlimeTraceSerializer::addChildrenCursorsToStack(Cursor& childrenArray, const TraceNode& node) {
