...
const std::string& NameRepo::metricName(MetricId metric) const {
    return _metricNames.lookup(metric.id());
}

const std::string& NameRepo::dimensionName(Dimension dim) const {
    return _dimensionNames.lookup(dim.id());
}

const std::string& NameRepo::labelValue(Label l) const {
    return _labelValues.lookup(l.id());
}

const PointMap& NameRepo::pointMap(Point from) const {
