...
private:
    ...
    SimpleMetricsManager(const SimpleManagerConfig& config, Tick::UP tick_supplier);

public:
    ~SimpleMetricsManager();
    static std::shared_ptr<MetricsManager> create(const SimpleManagerConfig& config);
    static std::shared_ptr<MetricsManager> createForTest(const SimpleManagerConfig& config, Tick::UP tick_supplier);
    ...
    void add(Counter::Increment inc) override { _currentSamples.add(inc); }
    // for use from Gauge only
    void sample(Gauge::Measurement value) override { _currentSamples.sample(value); }
};
