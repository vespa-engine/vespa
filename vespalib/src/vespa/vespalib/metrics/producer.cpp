...
Producer::Producer(std::shared_ptr<MetricsManager> m) : _manager(std::move(m)) {}

Producer::~Producer() = default;

namespace {

std::string format_snapshot(const Snapshot& snapshot, MetricsProducer::ExpositionFormat format) {
    switch (format) {
...
std::string Producer::getMetrics(const std::string&, ExpositionFormat format) {
    Snapshot snap = _manager->snapshot();
    return format_snapshot(snap, format);
}

std::string Producer::getTotalMetrics(const std::string&, ExpositionFormat format) {
