...
template <typename T>
std::vector<typename T::aggregator_type> mergeFromSamples(const StableStore<typename T::sample_type>& source) {
...
template <typename T> struct IdxComparator {
    bool operator()(const T& a, const T& b) { return a.idx < b.idx; }
};

template <typename T> std::vector<T> mergeVectors(const std::vector<T>& a, const std::vector<T>& b) {
    std::vector<T> result;
    visit_ranges(overload{[&result](visit_ranges_either, const T& x) { result.push_back(x); },
...
template <typename T> std::vector<T> findMissing(const std::vector<T>& already, const std::vector<T>& complete) {
    std::vector<T> result;
    visit_ranges(overload{// missing from "complete", should not happen:
                          [](visit_ranges_first, const T&) {},
                          // missing this:
                          [&result](visit_ranges_second, const T& x) { result.push_back(x); },
                          // already have this:
                          [](visit_ranges_both, const T&, const T&) {}},
                 already.begin(), already.end(), complete.begin(), complete.end(), IdxComparator<T>());
    return result;
}
