...
template <typename T> class StableStore {
    using MyClass = StableStore<T>;
    template <typename U> friend void swap(StableStore<U>& a, StableStore<U>& b);
...
template <typename T> StableStore<T>::StableStore() : _size(0), _more(), _mine() { _mine.reserve(3); }

template <typename T>
StableStore<T>::StableStore(size_t sz, UP&& more, std::vector<T>&& mine)
    : _size(sz), _more(std::move(more)), _mine(std::move(mine)) {}

template <typename T> void swap(StableStore<T>& a, StableStore<T>& b) {
    using std::swap;
