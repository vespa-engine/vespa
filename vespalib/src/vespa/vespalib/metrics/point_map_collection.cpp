...
HashedPointMap::HashedPointMap(PointMap&& from) : _map(std::move(from)), _hash(0) {
...
bool HashedPointMap::operator<(const HashedPointMap& other) const {
...
size_t PointMapCollection::resolve(PointMap map) {
    Guard  guard(_lock);
...
size_t PointMapCollection::size() const {
    Guard guard(_lock);
    return _vec.size();
}
