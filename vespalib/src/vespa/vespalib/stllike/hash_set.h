...
template <typename K, typename H = vespalib::hash<K>, typename EQ = std::equal_to<>,
          typename M = hashtable_base::and_modulator>
class hash_set {
private:
...
public:
    ...
    template <typename InputIterator> hash_set(InputIterator first, InputIterator last);
    ...
    template <typename Func> void for_each(Func func) const { _ht.for_each(func); }

    template <typename AltKey> bool contains(const AltKey key) const { return _ht.find(key) != end(); }

    template <typename AltKey> const_iterator find(const AltKey& key) const { return _ht.template find<AltKey>(key); }

    template <typename AltKey> iterator find(const AltKey& key) { return _ht.template find<AltKey>(key); }
