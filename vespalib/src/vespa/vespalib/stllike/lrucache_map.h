...
template <typename V> struct LinkedValue : public LinkedValueBase {
...
template <typename K, typename V, typename H = vespalib::hash<K>, typename EQ = std::equal_to<K>> struct LruParam {
...
template <typename P> class lrucache_map : private P::HashTable {
...
public:
    ...
    public:
        ...
        [[nodiscard]] const K& key() const noexcept { return _cache->getByInternalIndex(_current).first; }
...
private:
    ...
    public:
        ...
        RecordMoves(lrucache_map& lru) noexcept : _lru(lru) { _lru._moveRecordingEnabled = true; }
        ~RecordMoves();
