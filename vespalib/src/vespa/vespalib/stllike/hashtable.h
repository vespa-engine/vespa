...
public:
    ...
    static size_t getModuloSimple(size_t size) noexcept { return std::max(size_t(8), roundUp2inN(size)); }
...
template <typename V> class hash_node {
public:
    using next_t = hashtable_base::next_t;
    enum { npos = -1u, invalid = -2u };
    constexpr hash_node() noexcept : _next(invalid) {}
    constexpr hash_node(const V& node, next_t next = npos) noexcept(std::is_nothrow_copy_constructible_v<V>)
        : _next(next) {
        new (_node) V(node);
    }
    constexpr hash_node(V&& node, next_t next = npos) noexcept : _next(next) { new (_node) V(std::move(node)); }
    constexpr hash_node(hash_node&& rhs) noexcept : _next(rhs._next) {
        if (rhs.valid()) {
    ...
    constexpr hash_node(const hash_node& rhs) noexcept(std::is_nothrow_copy_constructible_v<V>) : _next(rhs._next) {
        if (rhs.valid()) {
...
template <typename Key, typename Value, typename Hash, typename Equal, typename KeyExtract,
          typename Modulator = hashtable_base::prime_modulator>
...
public:
    ...
    public:
        ...
        static constexpr const_iterator end(const hashtable* hash) noexcept {
            return const_iterator(hash, hash->initializedSize());
        }
...
public:
    ...
    template <typename AltKey> iterator find(const AltKey& key) noexcept;
    iterator                            find(const Key& key) noexcept;

    template <typename AltKey> constexpr const_iterator find(const AltKey& key) const noexcept {
        next_t h = hash(key);
    ...
    template <typename V> insert_result insert(V&& node) { return insert_internal(std::forward<V>(node)); }
...
protected:
    template <typename V> insert_result insert_internal(V&& node);
    ...
    template <typename MoveHandler> void   erase(MoveHandler& moveHandler, next_t h, const const_iterator& key);
    template <typename K> constexpr next_t hash(const K& key) const noexcept { return modulator(_hasher(key)); }
...
private:
    ...
    template <typename MoveHandler> void move(MoveHandler& moveHandler, next_t from, next_t to) {
        _nodes[to] = std::move(_nodes[from]);
        moveHandler.move(from, to);
    }
    template <typename MoveHandler> VESPA_DLL_LOCAL void reclaim(MoveHandler& moveHandler, next_t node);
    template <typename V>
