 ...
 *
 **/
template <typename K, typename V, typename LT = std::less<K>> class vector_map {
public:
...
template <typename K, typename V, typename LT> void swap(vector_map<K, V, LT>& a, vector_map<K, V, LT>& b) {
    a.swap(b);
}

template <typename K, typename V, typename LT> V& vector_map<K, V, LT>::operator[](const K& key) {
    LT       lt;
    iterator f = std::lower_bound(begin(), end(), key, KeyOrder());
    if (f == end()) {
