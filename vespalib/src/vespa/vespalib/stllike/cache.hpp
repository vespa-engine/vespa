...
cache<P>::SizeConstrainedLru::SizeConstrainedLru(cache& owner, size_t capacity_bytes)
    : Lru(Lru::UNLIMITED), _owner(owner), _size_bytes(0), _capacity_bytes(capacity_bytes) {}

template <typename P> cache<P>::SizeConstrainedLru::~SizeConstrainedLru() = default;

template <typename P> bool cache<P>::SizeConstrainedLru::removeOldest(const typename P::value_type& kv) {
...
template <typename P> bool cache<P>::SizeConstrainedLru::has_key(const KeyT& key) const noexcept {
    return Lru::hasKey(key);
}

template <typename P> void cache<P>::SizeConstrainedLru::insert_and_update_size(const KeyT& key, ValueT value) {
...
template <typename P> bool cache<P>::SizeConstrainedLru::try_erase_and_update_size(const KeyT& key) {
...
template <typename P> const typename P::Value& cache<P>::SizeConstrainedLru::get_existing(const KeyT& key) const {
    return Lru::get(key);
}

template <typename P> const typename P::Key* cache<P>::SizeConstrainedLru::last_key_or_nullptr() const noexcept {
...
template <typename P> bool cache<P>::SizeConstrainedLru::try_get_and_ref(const KeyT& key, ValueT& val_out) {
...
template <typename P> template <typename F> void cache<P>::SizeConstrainedLru::for_each_key(F fn) {
    for (auto it = Lru::begin(); it != Lru::end(); ++it) {
        fn(it.key());
    }
}

template <typename P> std::vector<typename P::Key> cache<P>::SizeConstrainedLru::dump_segment_keys_in_lru_order() {
    std::vector<KeyT> lru_keys;
    lru_keys.reserve(size());
    for_each_key([&lru_keys](const KeyT& k) { lru_keys.emplace_back(k); });
    return lru_keys;
}
...
template <typename P> cache<P>::ProbationarySegmentLru::~ProbationarySegmentLru() = default;

template <typename P> bool cache<P>::ProbationarySegmentLru::removeOldest(const typename P::value_type& kv) {
...
template <typename P> cache<P>::ProtectedSegmentLru::~ProtectedSegmentLru() = default;

template <typename P> bool cache<P>::ProtectedSegmentLru::removeOldest(const typename P::value_type& kv) {
...
template <typename P> cache<P>& cache<P>::maxElements(size_t probationary_elems, size_t protected_elems) {
...
template <typename P> cache<P>& cache<P>::maxElements(size_t elems) {
    maxElements(elems, 0);
    return *this;
}

template <typename P> cache<P>& cache<P>::setCapacityBytes(size_t probationary_sz, size_t protected_sz) {
    std::lock_guard guard(_hashLock);
...
template <typename P> cache<P>& cache<P>::setCapacityBytes(size_t sz) {
    setCapacityBytes(sz, 0);
    return *this;
}

template <typename P> void cache<P>::trim_segments() {
...
template <typename P> void cache<P>::invalidate(const K& key) {
    UniqueLock guard(_hashLock);
    invalidate(guard, key);
}

template <typename P> bool cache<P>::hasKey(const K& key) const {
    UniqueLock guard(_hashLock);
    return hasKey(guard, key);
}

template <typename P> cache<P>::~cache() = default;

template <typename P>
cache<P>::cache(BackingStore& backing_store, size_t max_probationary_bytes, size_t max_protected_bytes)
    : _hit(0),
      _miss(0),
      _non_existing(0),
      _race(0),
...
template <typename P>
cache<P>::cache(BackingStore& backing_store, size_t max_bytes) : cache(backing_store, max_bytes, 0) {}

template <typename P> void cache<P>::set_frequency_sketch_size(size_t cache_max_elem_count) {
    std::lock_guard guard(_hashLock);
...
template <typename P> void cache<P>::lfu_add(const K& key) noexcept {
    if (_sketch) {
        _sketch->add(key);
    }
}

template <typename P> uint8_t cache<P>::lfu_add_and_count(const K& key) noexcept {
    return _sketch ? _sketch->add_and_count(key) : 0;
}

template <typename P>
bool cache<P>::lfu_accepts_insertion(
...
template <typename P>
bool cache<P>::lfu_accepts_insertion(const K& key, const V& value, const SizeConstrainedLru& segment) {
    return !_sketch || lfu_accepts_insertion(key, value, segment, _sketch->count_min(key));
}

template <typename P> MemoryUsage cache<P>::getStaticMemoryUsage() const {
    MemoryUsage usage;
...
template <typename P> std::unique_lock<std::mutex> cache<P>::getGuard() const { return UniqueLock(_hashLock); }

template <typename P>
template <typename... BackingStoreArgs>
typename P::Value cache<P>::read(const K& key, BackingStoreArgs&&... backing_store_args) {
    V value;
    {
        std::lock_guard guard(_hashLock);
...
template <typename P>
bool cache<P>::try_fill_from_cache(const K& key, V& val_out, const std::lock_guard<std::mutex>& guard) {
...
template <typename P> void cache<P>::write(const K& key, V value) {
...
template <typename P> void cache<P>::erase(const K& key) {
    std::lock_guard storeGuard(getLock(key));
    invalidate(key);
    _store.erase(key);
}

template <typename P> void cache<P>::invalidate(const UniqueLock& guard, const K& key) {
...
template <typename P> bool cache<P>::hasKey(const UniqueLock& guard, const K& key) const {
    verifyHashLock(guard);
...
template <typename P> std::vector<typename P::Key> cache<P>::dump_segment_keys_in_lru_order(CacheSegment seg) {
    std::lock_guard guard(_hashLock);
...
template <typename P> size_t cache<P>::segment_size(CacheSegment seg) const noexcept {
    std::lock_guard guard(_hashLock);
    return (seg == CacheSegment::Probationary) ? _probationary_segment.size() : _protected_segment.size();
}

template <typename P> size_t cache<P>::segment_size_bytes(CacheSegment seg) const noexcept {
    std::lock_guard guard(_hashLock);
    return (seg == CacheSegment::Probationary) ? _probationary_segment.size_bytes() : _protected_segment.size_bytes();
}

template <typename P> size_t cache<P>::segment_capacity(CacheSegment seg) const noexcept {
    std::lock_guard guard(_hashLock);
    return (seg == CacheSegment::Probationary) ? _probationary_segment.capacity() : _protected_segment.capacity();
}

template <typename P> size_t cache<P>::segment_capacity_bytes(CacheSegment seg) const noexcept {
    std::lock_guard guard(_hashLock);
