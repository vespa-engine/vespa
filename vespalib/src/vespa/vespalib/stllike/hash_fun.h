...
template <typename K> struct hash {
    ...
    size_t operator()(const K& v) const noexcept(noexcept(v.hash())) { return v.hash(); }
};
...
template <> struct hash<float> {
    ...
    size_t operator()(float arg) const noexcept {
        U t;
        t.f = arg;
        return t.i;
    }
...
template <> struct hash<double> {
    ...
    size_t operator()(double arg) const noexcept {
        U t;
        t.f = arg;
        return t.i;
    }
...
inline size_t hashValue(const void* buf, size_t sz) noexcept { return xxhash::xxh3_64(buf, sz); }
