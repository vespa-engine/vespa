...
public:
    ...
    void assign_generation(Generation current_gen) { _hold_list.assign_generation(current_gen); }
     ...
     * moving btree nodes.
     */
    void foreach_value(const std::function<void(const std::vector<EntryRef>&)>& callback,
                       const EntryRefFilter&                                    filter);
};
