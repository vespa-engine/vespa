...
BufferTypeBase::BufferTypeBase(
    uint32_t entry_size_in, uint32_t buffer_underflow_size_in, uint32_t arraySize, uint32_t min_entries,
    uint32_t max_entries, uint32_t num_entries_for_new_buffer, float allocGrowFactor) noexcept
...
BufferTypeBase::BufferTypeBase(uint32_t entry_size_in, uint32_t buffer_underflow_size_in, uint32_t arraySize,
                               uint32_t min_entries, uint32_t max_entries) noexcept
    : BufferTypeBase(entry_size_in, buffer_underflow_size_in, arraySize, min_entries, max_entries, 0u,
                     DEFAULT_ALLOC_GROW_FACTOR) {}
...
EntryCount BufferTypeBase::get_reserved_entries(uint32_t bufferId) const noexcept { return bufferId == 0 ? 1u : 0u; }

void BufferTypeBase::on_active(
    uint32_t bufferId, std::atomic<EntryCount>* used_entries, std::atomic<EntryCount>* dead_entries, void* buffer) {
...
void BufferTypeBase::on_hold(
    uint32_t buffer_id, const std::atomic<EntryCount>* used_entries, const std::atomic<EntryCount>* dead_entries) {
    ++_holdBuffers;
...
void BufferTypeBase::resume_primary_buffer(
...
const alloc::MemoryAllocator* BufferTypeBase::get_memory_allocator() const { return nullptr; }

bool BufferTypeBase::is_dynamic_array_buffer_type() const noexcept { return false; }

void BufferTypeBase::clamp_max_entries(uint32_t max_entries) noexcept {
...
size_t BufferTypeBase::calc_entries_to_alloc(uint32_t bufferId, EntryCount free_entries_needed, bool resizing) const {
    ...
    size_t wanted_entries =
        std::max((resizing ? used_entries : 0u) + grow_entries, static_cast<size_t>(_min_entries));
    ...
    if (result < needed_entries) {
        ...
        s << "BufferTypeBase::calcArraysToAlloc(" << "bufferId=" << bufferId
          << ",free_entries_needed=" << free_entries_needed << ",resizing=" << (resizing ? "true" : "false") << ")"
          << " wanted_entries=" << wanted_entries << ", _arraySize=" << _arraySize << ", _max_entries="
          << _max_entries << ", reserved_entries=" << reserved_entries << ", live_entries=" << live_entries
          << ", grow_entries=" << grow_entries << ", used_entries=" << used_entries << ", typeid(*this).name=\""
          << typeid(*this).name() << "\"" << ", new_entries=" << result << " < needed_entries=" << needed_entries;
        throw vespalib::OverflowException(s.c_str());
    }
    return result;
}

uint32_t BufferTypeBase::get_scaled_num_entries_for_new_buffer() const noexcept {
...
BufferTypeBase::AggregatedBufferCounts::AggregatedBufferCounts() noexcept : _counts() {}

void BufferTypeBase::AggregatedBufferCounts::add_buffer(
    const std::atomic<EntryCount>* used_entries, const std::atomic<EntryCount>* dead_entries) {
...
void BufferTypeBase::AggregatedBufferCounts::remove_buffer(
    const std::atomic<EntryCount>* used_entries, const std::atomic<EntryCount>* dead_entries) {
    auto itr =
...
BufferTypeBase::BufferCounts BufferTypeBase::AggregatedBufferCounts::all_buffers() const noexcept {
    BufferCounts result;
    for (const auto& elem : _counts) {
