...
void BufferTypeBase::on_hold(uint32_t buffer_id, const std::atomic<EntryCount>* used_entries,
...
const alloc::MemoryAllocator* BufferTypeBase::get_memory_allocator() const {
    return nullptr;
}

bool BufferTypeBase::is_dynamic_array_buffer_type() const noexcept {
    return false;
}
...
size_t BufferTypeBase::calc_entries_to_alloc(uint32_t bufferId, EntryCount free_entries_needed, bool resizing) const {
    ...
    if (result < needed_entries) {
        ...
        s << "BufferTypeBase::calcArraysToAlloc(" << "bufferId=" << bufferId
          ...
          << " wanted_entries=" << wanted_entries << ", _arraySize=" << _arraySize
          << ", _max_entries=" << _max_entries << ", reserved_entries=" << reserved_entries
...
void BufferTypeBase::AggregatedBufferCounts::remove_buffer(const std::atomic<EntryCount>* used_entries,
                                                           const std::atomic<EntryCount>* dead_entries) {
    auto itr =
