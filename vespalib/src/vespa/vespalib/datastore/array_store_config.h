...
public:
    ...
    struct AllocSpec {
        ...
        AllocSpec(size_t min_entries_in_buffer_, size_t max_entries_in_buffer_, size_t num_entries_for_new_buffer_,
...
private:
    ...
    /**
     * Setup an array store where buffer type ids [1-(allocSpecs.size()-1)] are used to allocate small arrays in
     * datastore buffers and larger arrays are heap allocated. The allocation spec for a given buffer type is found in
     * the given vector. Allocation spec for large arrays is located at position 0.
     */
    ArrayStoreConfig(const AllocSpecVector& allocSpecs);

public:
    /**
     * Setup an array store where buffer type ids [1-max_type_id] are used to allocate small arrays in datastore
     * buffers with the given default allocation spec. Larger arrays are heap allocated.
     */
     ...
     */
    static ArrayStoreConfig optimizeForHugePage(
        uint32_t max_type_id, std::function<size_t(uint32_t)> type_id_to_entry_size, size_t hugePageSize,
        size_t smallPageSize, size_t maxEntryRefOffset, size_t max_buffer_size, size_t min_num_entries_for_new_buffer,
        float allocGrowFactor);
};
