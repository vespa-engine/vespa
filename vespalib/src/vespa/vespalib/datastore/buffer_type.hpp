...
BufferType<ElemT, EmptyT>::BufferType(uint32_t arraySize, uint32_t min_entries, uint32_t max_entries,
    ...
    : BufferTypeBase(arraySize * sizeof(ElemT), 0u, arraySize, min_entries, max_entries, num_entries_for_new_buffer,
                     allocGrowFactor) {}

template <typename ElemT, typename EmptyT> BufferType<ElemT, EmptyT>::~BufferType() = default;

template <typename ElemT, typename EmptyT>
void BufferType<ElemT, EmptyT>::destroy_entries(void* buffer, EntryCount num_entries) {
...
template <typename ElemT, typename EmptyT>
void BufferType<ElemT, EmptyT>::fallback_copy(void* newBuffer, const void* oldBuffer, EntryCount num_entries) {
    auto            num_elems = num_entries * getArraySize();
...
template <typename ElemT, typename EmptyT>
void BufferType<ElemT, EmptyT>::initialize_reserved_entries(void* buffer, EntryCount reserved_entries) {
...
template <typename ElemT, typename EmptyT>
void BufferType<ElemT, EmptyT>::clean_hold(void* buffer, size_t offset, EntryCount num_entries, CleanContext) {
    auto        num_elems = num_entries * getArraySize();
...
template <typename ElemT, typename EmptyT> const ElemT& BufferType<ElemT, EmptyT>::empty_entry() noexcept {
