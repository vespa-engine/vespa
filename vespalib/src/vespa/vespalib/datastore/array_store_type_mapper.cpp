...
ArrayStoreTypeMapper::ArrayStoreTypeMapper() : _array_sizes() {}

ArrayStoreTypeMapper::~ArrayStoreTypeMapper() = default;

uint32_t ArrayStoreTypeMapper::get_type_id(size_t array_size) const {
...
size_t ArrayStoreTypeMapper::get_array_size(uint32_t type_id) const {
    assert(type_id > 0 && type_id < _array_sizes.size());
    return _array_sizes[type_id];
}

uint32_t ArrayStoreTypeMapper::get_max_type_id(uint32_t max_type_id) const noexcept {
