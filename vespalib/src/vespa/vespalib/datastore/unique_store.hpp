...
using DefaultDictionary = btree::BTree<AtomicEntryRef, btree::BTreeNoLeafData, btree::NoAggregated,
                                       EntryComparatorWrapper, DefaultDictionaryTraits>;
...
template <typename EntryT, typename RefT, typename Comparator, typename Allocator>
void UniqueStore<EntryT, RefT, Comparator, Allocator>::set_dictionary(std::unique_ptr<IUniqueStoreDictionary> dict) {
    _dict = std::move(dict);
}
...
template <typename RefT> class CompactionContext : public UniqueStoreRemapper<RefT>, public ICompactable {
private:
    using DictionaryTraits = btree::BTreeTraits<32, 32, 7, true>;
    using Dictionary =
        btree::BTree<EntryRef, uint32_t, btree::NoAggregated, EntryComparatorWrapper, DictionaryTraits>;
    ...
    void fillMapping() { _dict.move_keys_on_compact(*this, _filter); }

public:
    CompactionContext(IUniqueStoreDictionary& dict, ICompactable& store,
...
UniqueStore<EntryT, RefT, Comparator, Allocator>::compact_worst(CompactionSpec            compaction_spec,
    ...
    if (compacting_buffers->empty()) {
        ...
        return std::make_unique<uniquestore::CompactionContext<RefT>>(*_dict, _allocator,
                                                                      std::move(compacting_buffers));
    }
}

template <typename EntryT, typename RefT, typename Comparator, typename Allocator>
vespalib::MemoryUsage UniqueStore<EntryT, RefT, Comparator, Allocator>::getMemoryUsage() const {
...
template <typename EntryT, typename RefT, typename Comparator, typename Allocator>
vespalib::AddressSpace UniqueStore<EntryT, RefT, Comparator, Allocator>::get_values_address_space_usage() const {
    return _allocator.get_data_store().getAddressSpaceUsage();
}

template <typename EntryT, typename RefT, typename Comparator, typename Allocator>
const BufferState& UniqueStore<EntryT, RefT, Comparator, Allocator>::bufferState(EntryRef ref) const {
    RefT internalRef(ref);
    return _store.getBufferState(internalRef.bufferId());
}

template <typename EntryT, typename RefT, typename Comparator, typename Allocator>
void UniqueStore<EntryT, RefT, Comparator, Allocator>::assign_generation(Generation current_gen) {
    _dict->assign_generation(current_gen);
    _store.assign_generation(current_gen);
}

template <typename EntryT, typename RefT, typename Comparator, typename Allocator>
void UniqueStore<EntryT, RefT, Comparator, Allocator>::reclaim_memory(Generation oldest_used_gen) {
    _dict->reclaim_memory(oldest_used_gen);
    _store.reclaim_memory(oldest_used_gen);
}

template <typename EntryT, typename RefT, typename Comparator, typename Allocator>
void UniqueStore<EntryT, RefT, Comparator, Allocator>::freeze() {
    _dict->freeze();
}
...
template <typename EntryT, typename RefT, typename Comparator, typename Allocator>
uint32_t UniqueStore<EntryT, RefT, Comparator, Allocator>::getNumUniques() const {
