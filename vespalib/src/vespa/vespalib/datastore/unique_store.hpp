...
public:
    CompactionContext(
        IUniqueStoreDictionary& dict, ICompactable& store, std::unique_ptr<CompactingBuffers> compacting_buffers)
...
UniqueStore<EntryT, RefT, Comparator, Allocator>::compact_worst(
    ...
    if (compacting_buffers->empty()) {
        ...
        return std::make_unique<uniquestore::CompactionContext<RefT>>(
            *_dict, _allocator, std::move(compacting_buffers));
    }
}
...
template <typename EntryT, typename RefT, typename Comparator, typename Allocator>
void UniqueStore<EntryT, RefT, Comparator, Allocator>::assign_generation(generation_t current_gen) {
    _dict->assign_generation(current_gen);
    _store.assign_generation(current_gen);
}

template <typename EntryT, typename RefT, typename Comparator, typename Allocator>
void UniqueStore<EntryT, RefT, Comparator, Allocator>::reclaim_memory(generation_t oldest_used_gen) {
