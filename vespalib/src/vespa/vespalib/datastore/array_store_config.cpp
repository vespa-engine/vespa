...
ArrayStoreConfig::ArrayStoreConfig(uint32_t max_type_id, const AllocSpec& defaultSpec)
    : _allocSpecs(), _enable_free_lists(false) {
...
ArrayStoreConfig::ArrayStoreConfig(const AllocSpecVector& allocSpecs)
    : _allocSpecs(allocSpecs), _enable_free_lists(false) {
}
...
size_t capToLimits(size_t value, size_t minLimit, size_t maxLimit) {
    size_t result = std::max(value, minLimit);
    return std::min(result, maxLimit);
}

size_t alignToSmallPageSize(size_t value, size_t minLimit, size_t smallPageSize) {
...
ArrayStoreConfig ArrayStoreConfig::optimizeForHugePage(uint32_t                        max_type_id,
                                                       std::function<size_t(uint32_t)> type_id_to_entry_size,
                                                       size_t hugePageSize, size_t smallPageSize,
                                                       size_t maxEntryRefOffset, size_t max_buffer_size,
    ...
    allocSpecs.emplace_back(0, capped_max_entries, capped_min_num_entries_for_new_buffer,
                            allocGrowFactor); // large array spec;
    for (uint32_t type_id = 1; type_id <= max_type_id; ++type_id) {
        ...
        num_entries_for_new_buffer =
            capToLimits(num_entries_for_new_buffer, capped_min_num_entries_for_new_buffer, capped_max_entries);
        num_entries_for_new_buffer =
            alignToSmallPageSize(num_entries_for_new_buffer, capped_min_num_entries_for_new_buffer, smallPageSize);
