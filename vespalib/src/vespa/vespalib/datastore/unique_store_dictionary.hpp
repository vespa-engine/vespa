...
UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::UniqueStoreDictionary(
...
template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
void UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::freeze() {
...
template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
void UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::assign_generation(generation_t current_gen) {
    if constexpr (has_btree_dictionary) {
...
template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
void UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::reclaim_memory(generation_t oldest_used_gen) {
    if constexpr (has_btree_dictionary) {
...
template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
UniqueStoreAddResult UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::add(
    const EntryComparator& comp, std::function<EntryRef()> insertEntry) {
    if constexpr (has_btree_dictionary) {
        ...
        bool                      inserted = false;
        std::function<EntryRef()> insert_hash_entry([&inserted, &insertEntry]() {
            inserted = true;
            return insertEntry();
        });
...
template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
EntryRef UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::find(const EntryComparator& comp) {
    if constexpr (has_btree_dictionary) {
        auto itr = this->_btree_dict.lowerBound(AtomicEntryRef(), comp);
        if (itr.valid() && !comp.less(EntryRef(), itr.getKey().load_relaxed())) {
            if constexpr (has_hash_dictionary) {
...
template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
void UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::remove(
    const EntryComparator& comp, EntryRef ref) {
    assert(ref.valid());
    if constexpr (has_btree_dictionary) {
...
template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
void UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::move_keys_on_compact(
    ICompactable& compactable, const EntryRefFilter& compacting_buffers) {
    if constexpr (has_btree_dictionary) {
...
template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
uint32_t UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::get_num_uniques() const {
    if constexpr (has_btree_dictionary) {
...
template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
vespalib::MemoryUsage UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::get_memory_usage() const {
...
template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
void UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::build(
    std::span<const EntryRef> refs, std::span<const uint32_t> ref_counts, std::function<void(EntryRef)> hold) {
...
UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::get_read_snapshot() const {
    if constexpr (has_btree_dictionary) {
        return std::make_unique<UniqueStoreBTreeDictionaryReadSnapshot<BTreeDictionaryT>>(
            this->_btree_dict.getFrozenView());
    }
    if constexpr (has_hash_dictionary) {
...
template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
bool UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::get_has_btree_dictionary() const {
    return has_btree_dictionary;
}

template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
bool UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::get_has_hash_dictionary() const {
...
template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
bool UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::has_held_buffers() const {
    if constexpr (has_btree_dictionary) {
...
template <typename BTreeDictionaryT, typename ParentT, typename HashDictionaryT>
void UniqueStoreDictionary<BTreeDictionaryT, ParentT, HashDictionaryT>::compact_worst(
    bool compact_btree_dictionary, bool compact_hash_dictionary, const CompactionStrategy& compaction_strategy) {
    if constexpr (has_btree_dictionary) {
