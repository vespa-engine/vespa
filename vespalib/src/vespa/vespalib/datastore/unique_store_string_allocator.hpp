...
UniqueStoreStringAllocator<RefT>::UniqueStoreStringAllocator(std::shared_ptr<alloc::MemoryAllocator> memory_allocator)
    : ICompactable(), _store(), _type_handlers() {
    _type_handlers.emplace_back(
        std::make_unique<UniqueStoreExternalStringBufferType>(1, RefT::offsetSize(), memory_allocator));
    for (auto size : string_allocator::array_sizes) {
        _type_handlers.emplace_back(
            std::make_unique<UniqueStoreSmallStringBufferType>(size, RefT::offsetSize(), memory_allocator));
    }
...
template <typename RefT> UniqueStoreStringAllocator<RefT>::~UniqueStoreStringAllocator() {
    _store.reclaim_all_memory();
    _store.dropBuffers();
}

template <typename RefT> EntryRef UniqueStoreStringAllocator<RefT>::allocate(const char* value) {
    ...
    if (type_id != 0) {
        ...
        auto handle = _store
                          .template freeListAllocator<WrappedExternalEntryType,
                                                      UniqueStoreEntryReclaimer<WrappedExternalEntryType>>(0)
                          .alloc(std::string(value));
...
template <typename RefT> void UniqueStoreStringAllocator<RefT>::hold(EntryRef ref) {
    RefT     iRef(ref);
...
template <typename RefT> EntryRef UniqueStoreStringAllocator<RefT>::move_on_compact(EntryRef ref) {
    RefT     iRef(ref);
    uint32_t type_id = _store.getTypeId(iRef.bufferId());
    if (type_id != 0) {
        ...
        auto handle = _store.template allocator<WrappedExternalEntryType>(0).alloc(
            *_store.template getEntry<WrappedExternalEntryType>(iRef));
