...
template <typename RefT> void DataStoreT<RefT>::reclaim_entry_refs(Generation oldest_used_gen) {
    _entry_ref_hold_list.reclaim(oldest_used_gen,
                                 [this](const auto& elem) { free_entry_internal(elem.ref, elem.num_entries); });
}
