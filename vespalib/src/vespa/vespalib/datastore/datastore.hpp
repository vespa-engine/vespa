...
template <typename RefT> DataStoreT<RefT>::~DataStoreT() = default;

template <typename RefT> void DataStoreT<RefT>::free_entry_internal(EntryRef ref, size_t num_entries) {
    RefType      intRef(ref);
    BufferState& state = getBufferState(intRef.bufferId());
    state.free_entries(ref, num_entries, intRef.offset());
}

template <typename RefT> void DataStoreT<RefT>::hold_entries(EntryRef ref, size_t num_entries, size_t extraBytes) {
    RefType      intRef(ref);
    BufferState& state = getBufferState(intRef.bufferId());
...
template <typename RefT> void DataStoreT<RefT>::reclaim_entry_refs(generation_t oldest_used_gen) {
    _entry_ref_hold_list.reclaim(
        oldest_used_gen, [this](const auto& elem) { free_entry_internal(elem.ref, elem.num_entries); });
}

template <typename RefT> void DataStoreT<RefT>::reclaim_all_entry_refs() {
...
template <typename EntryType, typename RefT> DataStore<EntryType, RefT>::~DataStore() {
    dropBuffers(); // Drop buffers before type handlers are dropped
}

template <typename EntryType, typename RefT> EntryRef DataStore<EntryType, RefT>::addEntry(const EntryType& e) {
