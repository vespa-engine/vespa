...
template <typename BTreeDictionaryT> class UniqueStoreBTreeDictionaryBase {
protected:
    BTreeDictionaryT _btree_dict;

public:
    static constexpr bool has_btree_dictionary = true;
    UniqueStoreBTreeDictionaryBase() : _btree_dict() {}
};

template <> class UniqueStoreBTreeDictionaryBase<NoBTreeDictionary> {
public:
    static constexpr bool has_btree_dictionary = false;
    UniqueStoreBTreeDictionaryBase() {}
};

template <typename HashDictionaryT> class UniqueStoreHashDictionaryBase {
protected:
    HashDictionaryT _hash_dict;

public:
    static constexpr bool has_hash_dictionary = true;
    UniqueStoreHashDictionaryBase(std::unique_ptr<EntryComparator> compare) : _hash_dict(std::move(compare)) {}
};

template <> class UniqueStoreHashDictionaryBase<NoHashDictionary> {
public:
 ...
 */
template <typename BTreeDictionaryT, typename ParentT = IUniqueStoreDictionary,
          typename HashDictionaryT = NoHashDictionary>
class UniqueStoreDictionary
    : public ParentT,
      public UniqueStoreBTreeDictionaryBase<BTreeDictionaryT>,
...
public:
    ...
    void                  build(std::span<const EntryRef>, std::span<const uint32_t> ref_counts,
                                std::function<void(EntryRef)> hold) override;
    ...
    void compact_worst(bool compact_btree_dictionary, bool compact_hash_dictionary,
