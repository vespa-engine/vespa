 ...
 */
template <typename ElemT> class SmallArrayBufferType : public BufferType<ElemT> {
...
public:
    ...
    SmallArrayBufferType(uint32_t array_size, const AllocSpec& spec,
                         std::shared_ptr<alloc::MemoryAllocator> memory_allocator) noexcept;
    template <typename TypeMapper>
    SmallArrayBufferType(uint32_t array_size, const AllocSpec& spec,
                         std::shared_ptr<alloc::MemoryAllocator> memory_allocator, TypeMapper&) noexcept
