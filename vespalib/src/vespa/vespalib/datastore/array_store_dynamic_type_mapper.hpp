...
ArrayStoreDynamicTypeMapper<ElemT>::ArrayStoreDynamicTypeMapper()
    : ArrayStoreTypeMapper(), _max_static_array_buffer_type_id(0) {}

template <typename ElemT>
ArrayStoreDynamicTypeMapper<ElemT>::ArrayStoreDynamicTypeMapper(
    uint32_t max_buffer_type_id, double grow_factor, size_t max_buffer_size)
    : ArrayStoreTypeMapper(), _max_static_array_buffer_type_id(0) {
    setup_array_sizes(max_buffer_type_id, grow_factor, max_buffer_size);
}

template <typename ElemT>
void ArrayStoreDynamicTypeMapper<ElemT>::setup_array_sizes(
    uint32_t max_buffer_type_id, double grow_factor, size_t max_buffer_size) {
    _array_sizes.clear();
    ...
    for (uint32_t type_id = 1; type_id <= max_buffer_type_id; ++type_id) {
        ...
        if (entry_size > std::numeric_limits<uint32_t>::max() || entry_size >= 2 * max_buffer_size) {
            break;
        }
...
template <typename ElemT> ArrayStoreDynamicTypeMapper<ElemT>::~ArrayStoreDynamicTypeMapper() = default;

template <typename ElemT> size_t ArrayStoreDynamicTypeMapper<ElemT>::get_entry_size(uint32_t type_id) const {
