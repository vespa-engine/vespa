...
template <typename EntryT, typename... Args> struct Assigner {
    static void assign(EntryT& entry, Args&&... args) { entry = EntryT(std::forward<Args>(args)...); }
};

template <typename EntryT> struct Assigner<EntryT> {
    static void assign(EntryT& entry) { (void)entry; }
};

// Assignment operator
template <typename EntryT> struct Assigner<EntryT, const EntryT&> {
    static void assign(EntryT& entry, const EntryT& rhs) { entry = rhs; }
};

// Move assignment
template <typename EntryT> struct Assigner<EntryT, EntryT&&> {
    static void assign(EntryT& entry, EntryT&& rhs) { entry = std::move(rhs); }
};
...
typename Allocator<EntryT, RefT>::HandleType FreeListAllocator<EntryT, RefT, ReclaimerT>::alloc(Args&&... args) {
...
template <typename EntryT, typename RefT, typename ReclaimerT>
typename Allocator<EntryT, RefT>::HandleType FreeListAllocator<EntryT, RefT, ReclaimerT>::allocArray(
    ConstArrayRef array) {
    auto& free_list = _store.getFreeList(_typeId);
    if (free_list.empty()) {
...
typename Allocator<EntryT, RefT>::HandleType FreeListAllocator<EntryT, RefT, ReclaimerT>::alloc_dynamic_array(
    ConstArrayRef array) {
    auto& free_list = _store.getFreeList(_typeId);
    if (free_list.empty()) {
