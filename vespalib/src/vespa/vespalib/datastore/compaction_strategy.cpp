...
bool CompactionStrategy::should_compact_memory(const MemoryUsage& memory_usage) const noexcept {
    return should_compact_memory(memory_usage.usedBytes(), memory_usage.deadBytes());
}

bool CompactionStrategy::should_compact_address_space(const AddressSpace& address_space) const noexcept {
    return should_compact_address_space(address_space.used(), address_space.dead());
}

CompactionSpec CompactionStrategy::should_compact(
    const MemoryUsage& memory_usage, const AddressSpace& address_space) const noexcept {
...
CompactionStrategy CompactionStrategy::make_compact_all_active_buffers_strategy() noexcept {
