 ...
 */
template <typename EntryT> class UniqueStoreComparatorHelper {
public:
    static bool   less(const EntryT& lhs, const EntryT& rhs) noexcept { return lhs < rhs; }
    static bool   equal(const EntryT& lhs, const EntryT& rhs) noexcept { return lhs == rhs; }
 ...
 */
template <typename EntryT> class UniqueStoreFloatingPointComparatorHelper {
public:
    ...
    static size_t hash(EntryT rhs) noexcept {
        if (std::isnan(rhs)) {
            ...
            union U {
                EntryT                                                                 f;
                std::conditional_t<std::is_same_v<double, EntryT>, uint64_t, uint32_t> i;
            };
            U t;
            t.f = rhs;
            return t.i;
 ...
 * special handling of NAN.
 */
template <> class UniqueStoreComparatorHelper<float> : public UniqueStoreFloatingPointComparatorHelper<float> {};

/**
 * Specialized helper class for comparing double elements in unique store with
 * special handling of NAN.
 */
template <> class UniqueStoreComparatorHelper<double> : public UniqueStoreFloatingPointComparatorHelper<double> {};
 ...
 */
template <typename EntryT, typename RefT> class UniqueStoreComparator : public EntryComparator {
protected:
...
public:
    UniqueStoreComparator(const DataStoreType& store) noexcept : _store(store), _lookup_value() {}
