...
template <typename EntryT, typename RefT>
typename FreeListRawAllocator<EntryT, RefT>::HandleType FreeListRawAllocator<EntryT, RefT>::alloc(
    size_t num_entries) {
...
typename FreeListRawAllocator<EntryT, RefT>::HandleType FreeListRawAllocator<EntryT, RefT>::alloc_dynamic_array(
    size_t array_size) {
    auto& free_list = _store.getFreeList(_typeId);
    if (free_list.empty()) {
