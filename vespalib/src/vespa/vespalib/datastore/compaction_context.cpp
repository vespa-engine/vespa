...
CompactionContext::CompactionContext(ICompactable& store, std::unique_ptr<CompactingBuffers> compacting_buffers)
    : _store(store),
      _compacting_buffers(std::move(compacting_buffers)),
      _filter(_compacting_buffers->make_entry_ref_filter()) {}

CompactionContext::~CompactionContext() { _compacting_buffers->finish(); }

void CompactionContext::compact(std::span<AtomicEntryRef> refs) {
