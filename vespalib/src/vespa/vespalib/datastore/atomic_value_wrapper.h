 ...
 */
template <typename T> class AtomicValueWrapper {
...
public:
    ...
    AtomicValueWrapper(const AtomicValueWrapper& rhs) noexcept : _value(rhs._value.load(std::memory_order_relaxed)) {}
    AtomicValueWrapper(AtomicValueWrapper&& rhs) noexcept : _value(rhs._value.load(std::memory_order_relaxed)) {}
    AtomicValueWrapper& operator=(const AtomicValueWrapper& rhs) noexcept {
        _value.store(rhs._value.load(std::memory_order_relaxed), std::memory_order_relaxed);
        return *this;
    }
    void            store_release(T value) noexcept { _value.store(value, std::memory_order_release); }
    void            store_relaxed(T value) noexcept { _value.store(value, std::memory_order_relaxed); }
    [[nodiscard]] T load_acquire() const noexcept { return _value.load(std::memory_order_acquire); }
    [[nodiscard]] T load_relaxed() const noexcept { return _value.load(std::memory_order_relaxed); }

    [[nodiscard]] bool operator==(const AtomicValueWrapper& rhs) const noexcept {
        return (load_relaxed() == rhs.load_relaxed());
    }
    [[nodiscard]] bool operator!=(const AtomicValueWrapper& rhs) const noexcept { return !(*this == rhs); }
};
