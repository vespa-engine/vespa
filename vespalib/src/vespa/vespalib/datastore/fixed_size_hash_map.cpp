...
void FixedSizeHashMap::Node::on_free() {
...
FixedSizeHashMap::KvType& FixedSizeHashMap::add(const ShardedHashComparator& comp,
...
void FixedSizeHashMap::reclaim_memory(Generation oldest_used_gen) {
...
ChangeWriter::ChangeWriter(uint32_t capacity) : _atomic_refs() {
...
bool FixedSizeHashMap::normalize_values(const std::function<void(std::vector<EntryRef>&)>& normalize,
...
void FixedSizeHashMap::foreach_value(const std::function<void(const std::vector<EntryRef>&)>& callback,
                                     const EntryRefFilter&                                    filter) {
    std::vector<EntryRef> refs;
    refs.reserve(1024);
    for (auto& chain_head : _chain_heads) {
