...
void FixedSizeHashMap::Node::on_free() { _kv = std::make_pair(AtomicEntryRef(), AtomicEntryRef()); }
...
FixedSizeHashMap::FixedSizeHashMap(uint32_t modulo, uint32_t capacity, uint32_t num_shards,
                                   const FixedSizeHashMap& orig, const EntryComparator& comp)
...
FixedSizeHashMap::~FixedSizeHashMap() { _hold_list.reclaim_all(); }

void FixedSizeHashMap::force_add(const EntryComparator& comp, const KvType& kv) {
...
FixedSizeHashMap::KvType& FixedSizeHashMap::add(
    const ShardedHashComparator& comp, std::function<EntryRef()>& insert_entry) {
...
FixedSizeHashMap::KvType* FixedSizeHashMap::remove(const ShardedHashComparator& comp) {
    uint32_t hash_idx = comp.hash_idx() % _modulo;
    auto&    chain_head = _chain_heads[hash_idx];
    uint32_t node_idx = chain_head.load_relaxed();
...
void FixedSizeHashMap::reclaim_memory(generation_t oldest_used_gen) {
...
MemoryUsage FixedSizeHashMap::get_memory_usage() const {
    ...
    return MemoryUsage(fixed_size + chain_heads_size + nodes_alloc_size,
                       fixed_size + chain_heads_size + nodes_used_size, nodes_dead_size, nodes_hold_size);
}

void FixedSizeHashMap::foreach_key(const std::function<void(EntryRef)>& callback) const {
...
void FixedSizeHashMap::move_keys_on_compact(ICompactable& compactable, const EntryRefFilter& compacting_buffers) {
    for (auto& chain_head : _chain_heads) {
        uint32_t node_idx = chain_head.load_relaxed();
        while (node_idx != no_node_idx) {
            auto&    node = _nodes[node_idx];
...
bool FixedSizeHashMap::normalize_values(const std::function<EntryRef(EntryRef)>& normalize) {
    bool changed = false;
    for (auto& chain_head : _chain_heads) {
        uint32_t node_idx = chain_head.load_relaxed();
        while (node_idx != no_node_idx) {
...
ChangeWriter::ChangeWriter(uint32_t capacity) : _atomic_refs() { _atomic_refs.reserve(capacity); }
...
bool FixedSizeHashMap::normalize_values(
    const std::function<void(std::vector<EntryRef>&)>& normalize, const EntryRefFilter& filter) {
...
void FixedSizeHashMap::foreach_value(
