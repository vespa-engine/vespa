...
UniqueStoreRemapper<RefT>::UniqueStoreRemapper(EntryRefFilter&& filter) : _filter(std::move(filter)), _mapping() {}
template <typename RefT> UniqueStoreRemapper<RefT>::~UniqueStoreRemapper() = default;
...
template <typename RefT> void UniqueStoreRemapper<RefT>::remap(std::span<AtomicEntryRef> refs) const {
