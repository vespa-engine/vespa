...
RawAllocator<EntryT, RefT>::RawAllocator(DataStoreBase& store, uint32_t typeId) : _store(store), _typeId(typeId) {}

template <typename EntryT, typename RefT>
typename RawAllocator<EntryT, RefT>::HandleType RawAllocator<EntryT, RefT>::alloc(
    size_t num_entries, size_t extra_entries) {
...
typename RawAllocator<EntryT, RefT>::HandleType RawAllocator<EntryT, RefT>::alloc_dynamic_array(size_t array_size) {
