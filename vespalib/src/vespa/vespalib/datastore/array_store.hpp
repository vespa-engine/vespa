...
BufferTypeBase* ArrayStore<ElemT, RefT, TypeMapperT>::initArrayType(
...
template <typename ElemT, typename RefT, typename TypeMapperT>
void ArrayStore<ElemT, RefT, TypeMapperT>::initArrayTypes(
    const ArrayStoreConfig& cfg, std::shared_ptr<alloc::MemoryAllocator> memory_allocator) {
...
template <typename ElemT, typename RefT, typename TypeMapperT>
ArrayStore<ElemT, RefT, TypeMapperT>::ArrayStore(
    const ArrayStoreConfig& cfg, std::shared_ptr<alloc::MemoryAllocator> memory_allocator, TypeMapper&& mapper)
    : _largeArrayTypeId(0),
...
template <typename ElemT, typename RefT, typename TypeMapperT>
vespalib::MemoryUsage ArrayStore<ElemT, RefT, TypeMapperT>::getMemoryUsage() const {
...
template <typename ElemT, typename RefT, typename TypeMapperT> ArrayStore<ElemT, RefT, TypeMapperT>::~ArrayStore() {
    _store.reclaim_all_memory();
    _store.dropBuffers();
}

template <typename ElemT, typename RefT, typename TypeMapperT>
EntryRef ArrayStore<ElemT, RefT, TypeMapperT>::add(ConstArrayRef array) {
    if (array.size() == 0) {
        return EntryRef();
    }
...
template <typename ElemT, typename RefT, typename TypeMapperT>
EntryRef ArrayStore<ElemT, RefT, TypeMapperT>::allocate(size_t array_size) {
    if (array_size == 0) {
        return EntryRef();
    }
...
template <typename ElemT, typename RefT, typename TypeMapperT>
EntryRef ArrayStore<ElemT, RefT, TypeMapperT>::addSmallArray(ConstArrayRef array, uint32_t type_id) {
...
template <typename ElemT, typename RefT, typename TypeMapperT>
EntryRef ArrayStore<ElemT, RefT, TypeMapperT>::allocate_small_array(uint32_t type_id) {
    return _store.template freeListRawAllocator<ElemT>(type_id).alloc(1).ref;
}

template <typename ElemT, typename RefT, typename TypeMapperT>
template <typename BufferType>
EntryRef ArrayStore<ElemT, RefT, TypeMapperT>::add_dynamic_array(ConstArrayRef array, uint32_t type_id) {
    using NoOpReclaimer = DefaultReclaimer<ElemT>;
    return _store.template freeListAllocator<ElemT, NoOpReclaimer>(type_id)
        .template alloc_dynamic_array<BufferType>(array)
        .ref;
}

template <typename ElemT, typename RefT, typename TypeMapperT>
template <typename BufferType>
EntryRef ArrayStore<ElemT, RefT, TypeMapperT>::allocate_dynamic_array(size_t array_size, uint32_t type_id) {
    return _store.template freeListRawAllocator<ElemT>(type_id)
        .template alloc_dynamic_array<BufferType>(array_size)
        .ref;
}

template <typename ElemT, typename RefT, typename TypeMapperT>
EntryRef ArrayStore<ElemT, RefT, TypeMapperT>::addLargeArray(ConstArrayRef array) {
...
template <typename ElemT, typename RefT, typename TypeMapperT>
EntryRef ArrayStore<ElemT, RefT, TypeMapperT>::allocate_large_array(size_t array_size) {
    using NoOpReclaimer = DefaultReclaimer<LargeArray>;
    auto handle = _store.template freeListAllocator<LargeArray, NoOpReclaimer>(_largeArrayTypeId)
                      .alloc(array_size, _largeArrayType.initial_alloc());
    auto& state = _store.getBufferState(RefT(handle.ref).bufferId());
...
template <typename ElemT, typename RefT, typename TypeMapperT>
void ArrayStore<ElemT, RefT, TypeMapperT>::remove(EntryRef ref) {
    if (ref.valid()) {
...
template <typename ElemT, typename RefT, typename TypeMapperT>
EntryRef ArrayStore<ElemT, RefT, TypeMapperT>::move_on_compact(EntryRef ref) {
    return add(get(ref));
}

template <typename ElemT, typename RefT, typename TypeMapperT>
ICompactionContext::UP ArrayStore<ElemT, RefT, TypeMapperT>::compact_worst(
...
template <typename ElemT, typename RefT, typename TypeMapperT>
std::unique_ptr<CompactingBuffers> ArrayStore<ElemT, RefT, TypeMapperT>::start_compact_worst_buffers(
    const CompactionStrategy& compaction_strategy) {
    return _store.start_compact_worst_buffers(_compaction_spec, compaction_strategy);
}

template <typename ElemT, typename RefT, typename TypeMapperT>
vespalib::AddressSpace ArrayStore<ElemT, RefT, TypeMapperT>::addressSpaceUsage() const {
    return _store.getAddressSpaceUsage();
}

template <typename ElemT, typename RefT, typename TypeMapperT>
vespalib::MemoryUsage ArrayStore<ElemT, RefT, TypeMapperT>::update_stat(
    const CompactionStrategy& compaction_strategy) {
...
template <typename ElemT, typename RefT, typename TypeMapperT>
const BufferState& ArrayStore<ElemT, RefT, TypeMapperT>::bufferState(EntryRef ref) {
    RefT internalRef(ref);
    return _store.getBufferState(internalRef.bufferId());
}

template <typename ElemT, typename RefT, typename TypeMapperT>
std::unique_ptr<StateExplorer> ArrayStore<ElemT, RefT, TypeMapperT>::make_state_explorer() const {
    return std::make_unique<DataStoreExplorer>(_store);
}

template <typename ElemT, typename RefT, typename TypeMapperT>
ArrayStoreConfig ArrayStore<ElemT, RefT, TypeMapperT>::optimizedConfigForHugePage(
    ...
    TypeMapper mapper;
    return optimizedConfigForHugePage(max_type_id, mapper, hugePageSize, smallPageSize, max_buffer_size,
                                      min_num_entries_for_new_buffer, allocGrowFactor);
}

template <typename ElemT, typename RefT, typename TypeMapperT>
ArrayStoreConfig ArrayStore<ElemT, RefT, TypeMapperT>::optimizedConfigForHugePage(
    uint32_t max_type_id, const TypeMapper& mapper, size_t hugePageSize, size_t smallPageSize, size_t max_buffer_size,
    size_t min_num_entries_for_new_buffer, float allocGrowFactor) {
    return ArrayStoreConfig::optimizeForHugePage(
        mapper.get_max_type_id(max_type_id),
        [&](uint32_t type_id) noexcept { return mapper.get_entry_size(type_id); }, hugePageSize, smallPageSize,
        RefT::offsetSize(), max_buffer_size, min_num_entries_for_new_buffer, allocGrowFactor);
}
