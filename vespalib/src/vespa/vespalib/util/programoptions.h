...
public:
    ...
    void addConfigurable(Configurable& c) {
        ...
        c.registerCommandLineOptions(*this, LifetimeToken::UP(new LifetimeToken(*this)));
    }
    ...
    OptionParser& addOption(const std::string& optionNameList, Type& value, const std::string& description);

    /** Add an optional option. Default value will be used if not set. */
    template <typename Type>
    OptionParser& addOption(const std::string& optionNameList, Type& value, const Type& defaultValue,
                            const std::string& description);

    template <typename Type>
    OptionParser& addArgument(const std::string& optionNameList, Type& value, const std::string& description);

    template <typename Type>
    OptionParser& addArgument(const std::string& optionNameList, Type& value, const Type& defaultValue,
                              const std::string& description);

    template <typename Type>
    OptionParser& addListArgument(const std::string& optionNameList, std::vector<Type>& value,
                                  const std::string& description);
    ...
    void setSyntaxPageMaxLeftColumnSize(uint32_t cols) { _maxLeftColumnSize = cols; }
...
template <> inline const char* getTypeName<int32_t>() {
    return "int";
}
template <> inline const char* getTypeName<uint32_t>() {
    return "uint";
}
template <> inline const char* getTypeName<int64_t>() {
    return "long";
}
template <> inline const char* getTypeName<uint64_t>() {
    return "ulong";
}
template <> inline const char* getTypeName<float>() {
    return "float";
}
template <> inline const char* getTypeName<double>() {
    return "double";
}
...
struct ProgramOptions::OptionParser {
    ...
    OptionParser(const std::string& nameList, uint32_t argCount, const std::string& desc);
    OptionParser(const std::string& nameList, uint32_t argCount, const std::string& defString,
    ...
    virtual bool hideFromSyntaxPage() const { return !isHeader() && _names.empty(); }
};
...
template <typename Number> struct ProgramOptions::NumberOptionParser : public OptionParser {
    Number& _number;
    Number  _defaultValue;

    std::string getStringValue(Number n);

    NumberOptionParser(const std::string& nameList, Number& number, const std::string& description)
        : OptionParser(nameList, 1, description), _number(number), _defaultValue(number) {}

    NumberOptionParser(const std::string& nameList, Number& number, const Number& defValue, const std::string& desc)
        : OptionParser(nameList, 1, getStringValue(defValue), desc), _number(number), _defaultValue(defValue) {}
    ...
    std::string getArgType(uint32_t /* index */) const override { return getTypeName<Number>(); }
};

template <typename Number> ProgramOptions::NumberOptionParser<Number>::~NumberOptionParser() = default;
...
struct ProgramOptions::FlagOptionParser : public OptionParser {
    ...
    FlagOptionParser(const std::string& nameList, bool& value, const bool& unsetValue,
                     const std::string& description);
...
struct ProgramOptions::StringOptionParser : public OptionParser {
    ...
    StringOptionParser(const std::string& nameList, std::string& value, const std::string& defVal,
                       const std::string& desc);
...
struct ProgramOptions::MapOptionParser : public OptionParser {
    ...
    MapOptionParser(const std::string& nameList, std::map<std::string, std::string>& value,
                    const std::string& description);
    ~MapOptionParser() override;

    void set(const std::vector<std::string>& arguments) override { _value[arguments[0]] = arguments[1]; }
...
template <typename T> struct ProgramOptions::ListOptionParser : public OptionParser {
    std::vector<T>&  _value;
    T                _singleValue;
    OptionParser::UP _entryParser;

    ListOptionParser(const std::string& nameList, std::vector<T>& value, const std::string& description)
        : OptionParser(nameList, 0, description), _value(value) {}

    T& getSingleValue() { return _singleValue; }

    void setEntryParser(OptionParser::UP entryParser) { _entryParser = std::move(entryParser); }
    ...
    void setDefault() override { _value.clear(); }
    std::string getArgType(uint32_t index) const override { return _entryParser->getArgType(index) + "[]"; }
};
