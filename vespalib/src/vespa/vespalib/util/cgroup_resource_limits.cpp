...
std::string combine_paths(
    const std::string& base_path, const std::string& controller, const std::string& cgroup_path) {
...
CGroupResourceLimits::CGroupResourceLimits() : CGroupResourceLimits("/sys/fs/cgroup", "/proc/self/cgroup") {}
...
void CGroupResourceLimits::apply_memory_limit(uint64_t memory_limit) {
    if (!_memory_limit.has_value() || _memory_limit.value() > memory_limit) {
        _memory_limit = memory_limit;
    }
}

void CGroupResourceLimits::apply_cpu_limit(uint32_t cpu_limit) {
    if (!_cpu_limit.has_value() || _cpu_limit.value() > cpu_limit) {
        _cpu_limit = cpu_limit;
    }
}

void CGroupResourceLimits::get_cgroup_paths() {
    /*
...
void CGroupResourceLimits::foreach_cgroup_level(const std::string& controller, const std::string& cgroup_path,
...
void CGroupResourceLimits::foreach_cgroup_v1_level(
    const std::string& controller, const std::function<void(const std::string&)>& callback) {
...
void CGroupResourceLimits::foreach_cgroup_v2_level(const std::function<void(const std::string&)>& callback) {
    foreach_cgroup_level(empty_std_string, _cgroup_v2_path, callback);
}

void CGroupResourceLimits::get_memory_limits_v1(const std::string& dir) {
...
void CGroupResourceLimits::get_memory_limits_v1() {
    foreach_cgroup_v1_level("memory", [this](const std::string& dir) { get_memory_limits_v1(dir); });
}

void CGroupResourceLimits::get_cpu_limits_v1(const std::string& dir) {
...
void CGroupResourceLimits::get_cpu_limits_v1() {
    foreach_cgroup_v1_level("cpu", [this](const std::string& dir) { get_cpu_limits_v1(dir); });
}

void CGroupResourceLimits::get_limits_v1() {
    get_memory_limits_v1();
    get_cpu_limits_v1();
}

void CGroupResourceLimits::get_memory_limits_v2(const std::string& dir) {
...
void CGroupResourceLimits::get_cpu_limits_v2(const std::string& dir) {
    int32_t       cpu_cfs_period_us = 0;
    int32_t       cpu_cfs_quota_us = 0;
...
void CGroupResourceLimits::get_limits_v2(const std::string& dir) {
    get_memory_limits_v2(dir);
    get_cpu_limits_v2(dir);
}

void CGroupResourceLimits::get_limits_v2() {
