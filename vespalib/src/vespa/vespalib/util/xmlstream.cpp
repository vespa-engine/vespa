...
XmlOutputStream& XmlOutputStream::operator<<(char c) {
...
XmlOutputStream& XmlOutputStream::operator<<(float f) {
    return *this << XmlContent(vespalib::make_string("%g", f));
}

XmlOutputStream& XmlOutputStream::operator<<(double d) {
...
XmlTag::XmlTag(const std::string& name, XmlTagFlags flags) : _name(name), _attributes(), _content(), _flags(flags) {
    ...
    if (!isLegalName(_name)) {
        throw vespalib::IllegalArgumentException("Name '" + _name +
                                                 "' contains "
...
XmlAttribute::XmlAttribute(const std::string& name, const char* value, uint32_t flags)
    ...
    _value = ost.view();
    if (!isLegalName(name)) {
        throw vespalib::IllegalArgumentException("Name '" + name +
                                                 "' contains "
