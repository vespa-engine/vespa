...
std::vector<bool> legalIdentifierFirstChar(getLegalIdentifierFirstCharacters());
...
void writeBase64Encoded(std::ostream& out, const std::string& s) { out << vespalib::Base64::encode(&s[0], s.size()); }
} // namespace
...
XmlOutputStream::XmlOutputStream(std::ostream& ostream, const std::string& indent)
    : _indent(indent), _wrappedStream(ostream), _tagStack(), _cachedTag(), _cachedAttributes(), _cachedContent() {}
...
XmlOutputStream& XmlOutputStream::operator<<(const XmlAttribute& attribute) {
    // std::cerr << "Adding attribute\n";
    if (_cachedTag.get() == nullptr) {
        throw vespalib::IllegalStateException("Cannot add attribute " + attribute.getName() + ", as no tag is open");
    }
    _cachedAttributes.push_back(attribute);
    return *this;
}

XmlOutputStream& XmlOutputStream::operator<<(const XmlEndTag&) {
...
XmlOutputStream& XmlOutputStream::operator<<(const XmlContent& content) {
    // std::cerr << "Adding content\n";
    if (_cachedTag.get() == nullptr && _tagStack.empty()) {
        throw vespalib::IllegalStateException("No open tag to write content in");
    }
    if (_cachedTag.get() != nullptr) {
        ...
        if (content.getType() == XmlContent::AUTO) { // Do nothing.. Always ok
            ...
            throw vespalib::IllegalStateException("Have already added content of different type");
        }
        _cachedContent.push_back(content);
    } else {
        if (content.getType() == XmlContent::BASE64) {
            throw vespalib::IllegalStateException("Cannot add Base64 encoded content after tag content");
        }
...
XmlOutputStream& XmlOutputStream::operator<<(const XmlSerializable& serializable) {
    // std::cerr << "Adding serializable\n";
    serializable.printXml(*this);
    return *this;
}

XmlOutputStream& XmlOutputStream::operator<<(const std::string& content) {
    // std::cerr << "Adding content string\n";
    return *this << XmlContent(content);
}

XmlOutputStream& XmlOutputStream::operator<<(char c) { return *this << XmlContent(std::string(&c, 1)); }

XmlOutputStream& XmlOutputStream::operator<<(int32_t i) {
    return *this << XmlContent(vespalib::make_string("%d", i));
}

XmlOutputStream& XmlOutputStream::operator<<(int64_t i) {
    return *this << XmlContent(vespalib::make_string("%" PRId64, i));
}

XmlOutputStream& XmlOutputStream::operator<<(float f) { return *this << XmlContent(vespalib::make_string("%g", f)); }

XmlOutputStream& XmlOutputStream::operator<<(double d) { return *this << XmlContent(vespalib::make_string("%g", d)); }

void XmlOutputStream::flush(bool endTag) {
    // std::cerr << "Flushing\n";
    if (_cachedTag.get() == nullptr) {
    ...
    for (std::list<XmlAttribute>::const_iterator it = _cachedAttributes.begin(); it != _cachedAttributes.end(); ++it)
    {
        _wrappedStream << ' ' << it->getName() << "=\"" << xml_attribute_escaped(it->getValue()) << '"';
    }
    ...
    if (_cachedContent.empty() && endTag) {
        ...
        if (_cachedContentType == XmlContent::AUTO) {
            _cachedContentType = XmlContent::ESCAPED;
            for (std::list<XmlContent>::const_iterator it = _cachedContent.begin(); it != _cachedContent.end(); ++it)
            {
...
XmlTag::XmlTag(const XmlTag& tag) : _name(tag._name), _attributes(), _content(), _flags(tag._flags) {}

XmlTag::~XmlTag() {}

XmlTag::XmlTag(const std::string& name, XmlTagFlags flags) : _name(name), _attributes(), _content(), _flags(flags) {
...
XmlAttribute::XmlAttribute(const XmlAttribute& attribute)
    : _name(attribute._name), _value(attribute._value), _next() {}
...
XmlContent::XmlContent(Type type) : _type(type), _content(), _nextContent(), _nextTag() {}

XmlContent::XmlContent() : _type(AUTO), _content(), _nextContent(), _nextTag() {}

XmlContent::XmlContent(const XmlContent& content)
    : _type(content._type), _content(content._content), _nextContent(), _nextTag() {}

XmlContent::XmlContent(const std::string& value) : _type(AUTO), _content(value), _nextContent(), _nextTag() {}

XmlContentWrapper::XmlContentWrapper(const XmlContentWrapper& wrapper) : XmlContent(wrapper) {}

XmlContentWrapper::XmlContentWrapper(const char* value) : XmlContent(std::string(value)) {}

XmlContentWrapper::XmlContentWrapper(const char* value, uint32_t size) : XmlContent(std::string(value, size)) {}
