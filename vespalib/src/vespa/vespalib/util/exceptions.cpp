...
const char* ExceptionWithPayload::what() const noexcept {
    return _msg.c_str();
}

ExceptionWithPayload::ExceptionWithPayload(std::string_view msg) : std::exception(), _msg(msg), _payload() {
}
ExceptionWithPayload::ExceptionWithPayload(std::string_view msg, Anything::UP payload)
    : std::exception(), _msg(msg), _payload(std::move(payload)) {
}
...
std::string PortListenException::make_message(int port, std::string_view protocol, std::string_view msg) {
    return make_string("failed to listen on port %d with protocol %s%s%s", port, std::string(protocol).c_str(),
                       msg.empty() ? "" : ": ", std::string(msg).c_str());
}

PortListenException::PortListenException(int port, std::string_view protocol, std::string_view msg,
                                         std::string_view location, int skipStack)
    : Exception(make_message(port, protocol, msg), location, skipStack + 1), _port(port), _protocol(protocol) {
}

PortListenException::PortListenException(int port, std::string_view protocol, const Exception& cause,
                                         std::string_view msg, std::string_view location, int skipStack)
    : Exception(make_message(port, protocol, msg), cause, location, skipStack + 1), _port(port), _protocol(protocol) {
}
...
IoException::IoException(std::string_view msg, Type type, std::string_view location, int skipStack)
    : Exception(createMessage(msg, type), location, skipStack + 1), _type(type) {
}

IoException::IoException(std::string_view msg, Type type, const Exception& cause, std::string_view location,
                         int skipStack)
    : Exception(createMessage(msg, type), cause, location, skipStack + 1), _type(type) {
}
...
std::string IoException::createMessage(std::string_view msg, Type type) {
    ...
    case TOO_MANY_OPEN_FILES:
        ...
        break;
    case DIRECTORY_HAVE_CONTENT:
        ost << "DIRECTORY HAVE CONTENT: ";
        break;
    case FILE_FULL:
        ost << "FILE_FULL: ";
        break;
...
IoException::Type IoException::getErrorType(int error) {
    switch (error) {
    case ENOENT:
...
template <typename T> bool check_type(const std::exception& e) {
    return (dynamic_cast<const T*>(&e) != nullptr);
}

void rethrow_if_unsafe(const std::exception& e) {
    if (check_type<std::bad_alloc>(e) || check_type<OOMException>(e) || check_type<FatalException>(e)) {
        throw;
    }
}
