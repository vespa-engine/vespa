...
std::string getAssertLogFileName(const char* key) {
    std::string relative =
        make_string("var/db/vespa/tmp/%s.%s.assert", key, Vtag::currentVersion.toAbbreviatedString().c_str());
...
void assertOnceOrLog(const char* expr, const char* key, size_t freq) {
    ...
    if (count) {
        if ((count % freq) == 0) {
            LOG(error, "assert(%s) named '%s' has failed %zu times. Stacktrace = %s", expr, key, count + 1,
        ...
        if (prevAssertFile) {
            ...
            {
                LOG(error, "assert(%s) named '%s' failed first time. Stacktrace = %s", expr, key,
                    vespalib::getStackTrace(0).c_str());
                std::ofstream assertStream(rememberAssert.c_str());
                assertStream << to_string(system_clock::now()) << " assert(" << expr << ") named " << key << " failed"
                             << std::endl;
                assertStream.close();
            }
            abort();
