...
void ProgramOptions::OptionParser::setInvalidDefault() {
...
void ProgramOptions::setSyntaxMessage(const std::string& msg) {
    _syntaxMessage = msg;
}

void ProgramOptions::addHiddenIdentifiers(const std::string& optionNameList) {
    if (_options.size() == 0) {
        throw InvalidCommandLineArgumentsException("Cannot add hidden identifier to last "
    ...
    for (uint32_t i = 0; i < newIds.size(); ++i) {
        ...
        if (it != _optionMap.end()) {
            throw InvalidCommandLineArgumentsException("Option '" + newIds[i] + "' is already registered.",
                                                       VESPA_STRLOC);
        }
    }
    for (uint32_t i = 0; i < newIds.size(); ++i) {
...
namespace {
std::vector<std::string> breakText(const std::vector<std::string>& source, uint32_t maxLen,
...
ProgramOptions::OptionParser& ProgramOptions::addOption(OptionParser::SP&& opt) {
    for (uint32_t i = 0; i < opt->_names.size(); ++i) {
        ...
        if (it != _optionMap.end()) {
            throw InvalidCommandLineArgumentsException("Option '" + opt->_names[i] + "' is already registered.",
                                                       VESPA_STRLOC);
        }
    }
...
ProgramOptions::OptionParser& ProgramOptions::addArgument(std::shared_ptr<OptionParser> arg) {
    if (!_arguments.empty() && !_arguments.back()->isRequired()) {
        if (arg->isRequired()) {
            throw InvalidCommandLineArgumentsException("Argument '" + arg->_names[0] +
...
ProgramOptions::FlagOptionParser::FlagOptionParser(const std::string& nameList, bool& value, const bool& unsetValue,
                                                   const std::string& desc)
    : OptionParser(nameList, 0, unsetValue ? "true" : "false", desc), _value(value), _unsetValue(unsetValue) {
    _invalidDefault = true;
}

ProgramOptions::StringOptionParser::StringOptionParser(const std::string& nameList, std::string& val,
                                                       const std::string& desc)
    : OptionParser(nameList, 1, desc), _value(val), _defaultValue() {
}

ProgramOptions::StringOptionParser::StringOptionParser(const std::string& nameList, std::string& value,
                                                       const std::string& defValue, const std::string& desc)
    : OptionParser(nameList, 1, '"' + defValue + '"', desc), _value(value), _defaultValue(defValue) {
}

ProgramOptions::MapOptionParser::MapOptionParser(const std::string&                  nameList,
...
#define VESPALIB_PROGRAMOPTIONS_IMPL_NODEF_ADDOPTION(type, parsertype)                                      \
    template <>                                                                                             \
    ProgramOptions::OptionParser& ProgramOptions::addOption(const std::string& optionNameList, type& value, \
                                                            const std::string& desc) {                      \
...
#define VESPALIB_PROGRAMOPTIONS_IMPL_ADDOPTION(type, parsertype)                                            \
    template <>                                                                                             \
    ProgramOptions::OptionParser& ProgramOptions::addOption(const std::string& optionNameList, type& value, \
                                                            const type& defVal, const std::string& desc) {  \
...
#define VESPALIB_PROGRAMOPTIONS_IMPL_NODEF_ADDARGUMENT(type, parsertype)                            \
    template <>                                                                                     \
    ProgramOptions::OptionParser& ProgramOptions::addArgument(const std::string& name, type& value, \
                                                              const std::string& desc) {            \
...
#define VESPALIB_PROGRAMOPTIONS_IMPL_ADDLISTARGUMENT(type, parsertype)                                               \
    template <>                                                                                                      \
    ProgramOptions::OptionParser& ProgramOptions::addListArgument(const std::string& name, std::vector<type>& value, \
                                                                  const std::string& desc) {                         \
