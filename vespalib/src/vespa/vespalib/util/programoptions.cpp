...
template <typename Number> std::string ProgramOptions::NumberOptionParser<Number>::getStringValue(Number n) {
...
ProgramOptions::OptionParser::OptionParser(const std::string& nameList, uint32_t argCount, const std::string& desc)
...
ProgramOptions::OptionParser::OptionParser(
    const std::string& nameList, uint32_t argCount, const std::string& defString, const std::string& desc)
    : _names(splitString(nameList, ' ')),
      _hiddenNames(),
      _argCount(argCount),
      _argTypes(argCount),
...
void ProgramOptions::OptionParser::setInvalidDefault() { _invalidDefault = true; }
...
void ProgramOptions::setCommandLineArguments(int argc, const char* const* argv) {
    _argc = argc;
    _argv = argv;
}

void ProgramOptions::setSyntaxMessage(const std::string& msg) { _syntaxMessage = msg; }

void ProgramOptions::addHiddenIdentifiers(const std::string& optionNameList) {
    if (_options.size() == 0) {
        throw InvalidCommandLineArgumentsException(
            "Cannot add hidden identifier to last "
            "option as no option has been added yet.",
            VESPA_STRLOC);
    }
    OptionParser::SP opt = _options.back();
    if (opt->isHeader()) {
        throw InvalidCommandLineArgumentsException("Cannot add option arguments to option header.", VESPA_STRLOC);
    }
    std::vector<std::string> newIds(splitString(optionNameList, ' '));
    for (uint32_t i = 0; i < newIds.size(); ++i) {
        std::map<std::string, OptionParser::SP>::const_iterator it(_optionMap.find(newIds[i]));
        if (it != _optionMap.end()) {
            throw InvalidCommandLineArgumentsException(
                "Option '" + newIds[i] + "' is already registered.", VESPA_STRLOC);
        }
    }
    for (uint32_t i = 0; i < newIds.size(); ++i) {
...
void ProgramOptions::setArgumentTypeName(const std::string& name, uint32_t index) {
    if (_options.size() == 0) {
        throw InvalidCommandLineArgumentsException(
            "Cannot add hidden identifier to last "
            "option as no option has been added yet.",
...
void ProgramOptions::addOptionHeader(const std::string& description) {
    _options.push_back(OptionParser::SP(new OptionHeader(description)));
}

namespace {
bool isNumber(const std::string& arg) {
    if (arg.size() > 1 && arg[0] == '-' && arg[1] >= '0' && arg[1] <= '9') {
        return true;
    }
    return false;
}
    ...
    try {
        ...
        for (; optPos < static_cast<uint32_t>(_argc); ++optPos) {
            ...
            if (s.substr(0, 2) == "--") {
                ...
                LOG(debug,
                    "Parsing long option %s at pos %u, arg pos is now "
                    "%u",
                    id.c_str(), optPos, argPos);
                std::map<std::string, OptionParser::SP>::const_iterator it(_optionMap.find(id));
                if (it == _optionMap.end()) {
                    throw InvalidCommandLineArgumentsException("Invalid option '" + id + "'.", VESPA_STRLOC);
                }
                parseOption(id, *it->second, argPos);
                _setOptions.insert(it->second);
            } else {
                LOG(debug,
                    "Parsing short options %s at pos %u, arg pos is now "
                    "%u.",
                    s.c_str() + 1, optPos, argPos);
                for (uint32_t shortPos = 1; shortPos < s.size(); ++shortPos) {
                    std::string id(s.substr(shortPos, 1));
                    LOG(debug, "Parsing short option %s, arg pos is %u.", id.c_str(), argPos);
                    std::map<std::string, OptionParser::SP>::const_iterator it(_optionMap.find(id));
                    if (it == _optionMap.end()) {
                        throw InvalidCommandLineArgumentsException("Invalid option '" + id + "'.", VESPA_STRLOC);
                    }
        ...
        for (uint32_t i = 0; i < _arguments.size(); ++i) {
            ...
            if (opt._argCount == 0) {
                LOG(debug, "Parsing list argument %s. Pos is %u.", opt.getArgName().c_str(), optPos);
                ...
                if (!opt.isRequired()) {
                    ...
                    throw InvalidCommandLineArgumentsException(
                        "Insufficient data is given to set required argument '" + opt.getArgName() + "'.",
                        VESPA_STRLOC);
                }
            } else {
...
void ProgramOptions::setDefaults(bool failUnsetRequired) {
    for (uint32_t i = 0; i < _options.size(); ++i) {
        ...
        if (it == _setOptions.end()) {
            if (opt->_hasDefault) {
                ...
                throw InvalidCommandLineArgumentsException(
                    "Option '" + opt->_names[0] + "' has no default and must be set.", VESPA_STRLOC);
            }
        }
    }
    _defaultsSet = true;
...
void ProgramOptions::parseOption(const std::string& id, OptionParser& opt, uint32_t& argPos) {
    ...
    for (; arguments.size() != opt._argCount; ++argPos) {
        if (argPos >= static_cast<uint32_t>(_argc)) {
            throw InvalidCommandLineArgumentsException(
                vespalib::make_string("Option '%s' needs %u arguments. Only %u available.", id.c_str(), opt._argCount,
                                      (uint32_t)arguments.size()),
                VESPA_STRLOC);
        }
        if (strlen(_argv[argPos]) >= 2 && _argv[argPos][0] == '-' && !isNumber(_argv[argPos])) {
            continue;
        }
...
void ProgramOptions::parseArgument(OptionParser& opt, uint32_t& pos) {
...
namespace {
std::vector<std::string> breakText(
    const std::vector<std::string>& source, uint32_t maxLen, int preserveWordSpaceLimit = -1) {
    ...
    for (uint32_t i = 0; i < source.size(); ++i) {
        ...
        for (uint32_t j = 0; j < split.size(); ++j) {
            ...
            while (true) {
                ...
                std::string::size_type pos(line.rfind(" ", maxLen));
                if (pos != std::string::npos && pos > maxLen - preserveWordSpaceLimit) {
...
std::vector<std::string> breakText(const std::string& source, uint32_t maxLen, int preserveWordSpaceLimit = -1) {
...
void ProgramOptions::writeSyntaxPage(std::ostream& out, bool showDefaults) {
    ...
    if (_arguments.size() > 0) {
        ...
        for (uint32_t i = 0; i < _arguments.size(); ++i) {
            OptionParser& opt(*_arguments[i]);
            argNames[i] = opt.getArgName() + " (" + opt.getArgType(0) + ")";
            if (argNames[i].size() <= _maxLeftColumnSize) {
                argSize = std::max(argSize, static_cast<uint32_t>(argNames[i].size()));
            }
        }
        ...
        for (uint32_t i = 0; i < _arguments.size(); ++i) {
            ...
            std::vector<std::string> message(breakText(opt._description, 80 - indent.size()));
            if (opt._hasDefault) {
    ...
    if (hasOptions) {
        ...
        for (uint32_t i = 0; i < _options.size(); ++i) {
            ...
            if (showDefaults) {
                ...
                if (!opt._hasDefault) {
                    s = "(required)";
                } else if (!opt._invalidDefault && opt._defaultString != UNSET_TOKEN) {
                    s = "(default " + opt._defaultString + ")";
                }
                if (s.size() > 0) {
                    if (message.back().size() + indent.size() + 1 + s.size() <= 80) {
                        message.back() += " " + s;
                    } else {
                        message.push_back(s);
                    }
...
ProgramOptions::OptionParser& ProgramOptions::addOption(OptionParser::SP&& opt) {
    for (uint32_t i = 0; i < opt->_names.size(); ++i) {
        std::map<std::string, OptionParser::SP>::const_iterator it(_optionMap.find(opt->_names[i]));
        if (it != _optionMap.end()) {
            throw InvalidCommandLineArgumentsException(
                "Option '" + opt->_names[i] + "' is already registered.", VESPA_STRLOC);
        }
    }
...
ProgramOptions::OptionParser& ProgramOptions::addArgument(std::shared_ptr<OptionParser> arg) {
    if (!_arguments.empty() && !_arguments.back()->isRequired()) {
        if (arg->isRequired()) {
            throw InvalidCommandLineArgumentsException(
                "Argument '" + arg->_names[0] +
...
ProgramOptions::OptionParser& ProgramOptions::getOptionParser(const std::string& id) {
    std::map<std::string, OptionParser::SP>::const_iterator it(_optionMap.find(id));
    if (it == _optionMap.end()) {
        throw InvalidCommandLineArgumentsException("No option registered with id '" + id + "'.", VESPA_STRLOC);
    }
    return *it->second;
}

ProgramOptions::OptionParser& ProgramOptions::getArgumentParser(uint32_t argIndex) {
    if (argIndex >= _arguments.size()) {
        std::ostringstream ost;
        ost << "Only " << _arguments.size() << " arguments registered. Thus argument " << argIndex
            << " does not exist.";
        throw InvalidCommandLineArgumentsException(ost.str(), VESPA_STRLOC);
    }
...
ProgramOptions::BoolOptionParser::BoolOptionParser(const std::string& nameList, bool& value, const std::string& desc)
    : OptionParser(nameList, 0, UNSET_TOKEN, desc), _value(value), _defaultValue(false) {}

ProgramOptions::BoolOptionParser::~BoolOptionParser() = default;

ProgramOptions::FlagOptionParser::FlagOptionParser(const std::string& nameList, bool& value, const std::string& desc)
    : OptionParser(nameList, 0, UNSET_TOKEN, desc), _value(value), _unsetValue(false) {
    _invalidDefault = true;
}

ProgramOptions::FlagOptionParser::FlagOptionParser(
    const std::string& nameList, bool& value, const bool& unsetValue, const std::string& desc)
    : OptionParser(nameList, 0, unsetValue ? "true" : "false", desc), _value(value), _unsetValue(unsetValue) {
    _invalidDefault = true;
}

ProgramOptions::StringOptionParser::StringOptionParser(
    const std::string& nameList, std::string& val, const std::string& desc)
    : OptionParser(nameList, 1, desc), _value(val), _defaultValue() {}

ProgramOptions::StringOptionParser::StringOptionParser(
    const std::string& nameList, std::string& value, const std::string& defValue, const std::string& desc)
    : OptionParser(nameList, 1, '"' + defValue + '"', desc), _value(value), _defaultValue(defValue) {}

ProgramOptions::MapOptionParser::MapOptionParser(
    const std::string& nameList, std::map<std::string, std::string>& value, const std::string& description)
    : OptionParser(nameList, 2, "empty", description), _value(value) {}
...
#define VESPALIB_PROGRAMOPTIONS_IMPL_NODEF_ADDOPTION(type, parsertype)                   \
    template <>                                                                          \
    ProgramOptions::OptionParser& ProgramOptions::addOption(                             \
        const std::string& optionNameList, type& value, const std::string& desc) {       \
        return addOption(OptionParser::SP(new parsertype(optionNameList, value, desc))); \
    }
...
#define VESPALIB_PROGRAMOPTIONS_IMPL_ADDOPTION(type, parsertype)                                       \
    template <>                                                                                        \
    ProgramOptions::OptionParser& ProgramOptions::addOption(                                           \
        const std::string& optionNameList, type& value, const type& defVal, const std::string& desc) { \
        return addOption(OptionParser::SP(new parsertype(optionNameList, value, defVal, desc)));       \
    }
...
#define VESPALIB_PROGRAMOPTIONS_IMPL_NODEF_ADDARGUMENT(type, parsertype)         \
    template <>                                                                  \
    ProgramOptions::OptionParser& ProgramOptions::addArgument(                   \
        const std::string& name, type& value, const std::string& desc) {         \
        return addArgument(OptionParser::SP(new parsertype(name, value, desc))); \
    }
...
#define VESPALIB_PROGRAMOPTIONS_IMPL_ADDARGUMENT(type, parsertype)                           \
    template <>                                                                              \
    ProgramOptions::OptionParser& ProgramOptions::addArgument(                               \
        const std::string& name, type& value, const type& defVal, const std::string& desc) { \
        return addArgument(OptionParser::SP(new parsertype(name, value, defVal, desc)));     \
    }
...
#define VESPALIB_PROGRAMOPTIONS_IMPL_ADDLISTARGUMENT(type, parsertype)                                 \
    template <>                                                                                        \
    ProgramOptions::OptionParser& ProgramOptions::addListArgument(                                     \
        const std::string& name, std::vector<type>& value, const std::string& desc) {                  \
        ListOptionParser<type>* listParser(new ListOptionParser<type>(name, value, desc));             \
        OptionParser::UP        entryParser(new parsertype(name, listParser->getSingleValue(), desc)); \
