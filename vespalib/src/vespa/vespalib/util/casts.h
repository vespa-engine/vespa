 ...
 * Author: arnej
 */
template <typename T, typename U, typename R = std::conditional_t<std::is_const_v<U>, const T*, T*>>
R char_p_cast(U* input) {
    ...
    static_assert(
        std::is_same<DU, char>::value || std::is_same<DU, unsigned char>::value || std::is_same<DU, char8_t>::value);
...
template <char8_t_string_literal L, std::size_t... I> constexpr auto& make_as_char_buffer(std::index_sequence<I...>) {
 ...
 */
template <vespalib::char8_t_string_literal L> constexpr auto& operator""_C() {
