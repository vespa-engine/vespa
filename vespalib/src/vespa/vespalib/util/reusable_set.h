...
public:
    ...
    explicit ReusableSet(size_t size) : _array(size), _curval(-1), _sz(size) { clear(); }
    ...
    void mark(size_t id) { _array[id] = _curval; }

    bool is_marked(size_t id) const { return (_array[id] == _curval); }

    Mark* bits() { return _array.begin(); }
    Mark generation() const { return _curval; }
    size_t capacity() const { return _sz; }

    size_t memory_usage() const { return (_sz * sizeof(Mark)) + sizeof(ReusableSet); }
