...
void Sha1::get_digest(char* digest, size_t digestLen) {
    uint32_t i, j;
    uint8_t  finalcount[8];

    for (i = 0; i < 8; i++) {
        finalcount[i] =
            static_cast<uint8_t>((_count[(i >= 4 ? 0 : 1)] >> ((3 - (i & 3)) * 8)) & 255); /* Endian independent */
    }
    process("\200", 1);
...
void Sha1::hash(const char* input, size_t inputLen, char* digest, size_t digestLen) {
    Sha1 sha;
