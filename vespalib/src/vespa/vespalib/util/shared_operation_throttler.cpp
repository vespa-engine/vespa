...
public:
    DynamicThrottlePolicy(
        const SharedOperationThrottler::DynamicThrottleParams& params, std::function<steady_time()> time_provider);
    ...
    [[nodiscard]] uint32_t current_window_size() const noexcept { return static_cast<uint32_t>(_window_size); }
...
private:
    ...
    [[nodiscard]] uint64_t current_time_as_millis() noexcept { return count_ms(_time_provider().time_since_epoch()); }
};
...
void DynamicThrottlePolicy::set_window_size_increment(double window_size_increment) noexcept {
    _window_size_increment = window_size_increment;
    _window_size = std::max(_window_size, _window_size_increment);
}

void DynamicThrottlePolicy::set_window_size_backoff(double window_size_backoff) noexcept {
    _window_size_backoff = std::max(0.0, std::min(1.0, window_size_backoff));
}

void DynamicThrottlePolicy::set_resize_rate(double resize_rate) noexcept {
    _resize_rate = std::max(2.0, resize_rate);
}

void DynamicThrottlePolicy::set_max_window_size(double max_size) noexcept {
...
void DynamicThrottlePolicy::set_min_window_size(double min_size) noexcept {
    _min_window_size = min_size;
    _window_size = std::max(_min_window_size, _window_size_increment);
}

void DynamicThrottlePolicy::set_window_size_decrement_factor(double decrement_factor) noexcept {
    _decrement_factor = decrement_factor;
}

void DynamicThrottlePolicy::internal_unconditional_configure(
    const SharedOperationThrottler::DynamicThrottleParams& params) noexcept {
...
void DynamicThrottlePolicy::configure(const SharedOperationThrottler::DynamicThrottleParams& params) noexcept {
...
bool DynamicThrottlePolicy::has_spare_capacity(uint32_t pending_count) noexcept {
...
void DynamicThrottlePolicy::process_request() noexcept {
    ...
    if (throughput > _local_max_throughput) {
        ...
        if (efficiency < _efficiency_threshold) {
            _window_size = std::min(
                _window_size * _window_size_backoff, _window_size - _decrement_factor * _window_size_increment);
            _local_max_throughput = 0;
        } else {
            _window_size += _weight * _window_size_increment;
        }
...
void DynamicThrottlePolicy::process_response(bool success) noexcept {
    if (success) {
        ++_num_ok;
    }
}
...
public:
    DynamicOperationThrottler(const DynamicThrottleParams& params, std::function<steady_time()> time_provider);
...
DynamicOperationThrottler::~DynamicOperationThrottler() { assert(_pending_ops == 0u); }
...
bool DynamicOperationThrottler::has_spare_capacity_in_active_window(uint64_t operation_resource_usage) noexcept {
    ...
    const bool within_resource_limits =
        ((_resource_usage_soft_limit == 0) || // 0 means unlimited
...
void DynamicOperationThrottler::add_one_to_active_window_size() noexcept {
...
DynamicOperationThrottler::Token DynamicOperationThrottler::blocking_acquire_one(
    uint64_t operation_resource_usage) noexcept {
    std::unique_lock lock(_mutex);
    if (!has_spare_capacity_in_active_window(operation_resource_usage)) {
        ++_waiting_threads;
        _cond.wait(lock, [&] { return has_spare_capacity_in_active_window(operation_resource_usage); });
        --_waiting_threads;
    }
...
DynamicOperationThrottler::Token DynamicOperationThrottler::blocking_acquire_one(
    vespalib::steady_time deadline, uint64_t operation_resource_usage) noexcept {
    std::unique_lock lock(_mutex);
    if (!has_spare_capacity_in_active_window(operation_resource_usage)) {
        ++_waiting_threads;
        const bool accepted = _cond.wait_until(
            lock, deadline, [&] { return has_spare_capacity_in_active_window(operation_resource_usage); });
        --_waiting_threads;
        if (!accepted) {
            return Token();
        }
...
DynamicOperationThrottler::Token DynamicOperationThrottler::try_acquire_one(
    uint64_t operation_resource_usage) noexcept {
    std::unique_lock lock(_mutex);
    if (!has_spare_capacity_in_active_window(operation_resource_usage)) {
        return Token();
...
void DynamicOperationThrottler::release_one(uint64_t operation_resource_usage) noexcept {
    std::unique_lock lock(_mutex);
...
uint32_t DynamicOperationThrottler::current_window_size() const noexcept {
    std::lock_guard lock(_mutex);
    return _throttle_policy.current_window_size();
}

uint32_t DynamicOperationThrottler::current_active_token_count() const noexcept {
    std::lock_guard lock(_mutex);
    return _pending_ops;
}

uint32_t DynamicOperationThrottler::waiting_threads() const noexcept {
    std::lock_guard lock(_mutex);
    return _waiting_threads;
}

uint64_t DynamicOperationThrottler::current_resource_usage() const noexcept {
    std::lock_guard lock(_mutex);
    return _current_resource_usage;
}

uint64_t DynamicOperationThrottler::max_resource_usage() const noexcept {
    std::lock_guard lock(_mutex);
    return _max_resource_usage;
}

void DynamicOperationThrottler::reconfigure_dynamic_throttling(const DynamicThrottleParams& params) noexcept {
    std::lock_guard lock(_mutex);
...
std::unique_ptr<SharedOperationThrottler> SharedOperationThrottler::make_unlimited_throttler() {
    return std::make_unique<NoLimitsOperationThrottler>();
}

std::unique_ptr<SharedOperationThrottler> SharedOperationThrottler::make_dynamic_throttler(
...
void DynamicOperationThrottler::Token::reset() noexcept {
    if (_throttler) {
        _throttler->release_one(_operation_resource_usage);
        _throttler = nullptr;
    }
...
DynamicOperationThrottler::Token& DynamicOperationThrottler::Token::operator=(Token&& rhs) noexcept {
    reset();
...
std::string DynamicOperationThrottler::DynamicThrottleParams::to_string() const {
    return std::format("DynamicThrottleParams(window_size_increment={}, min_window_size={}, "
                       ...
                       "window_size_backoff={})",
                       window_size_increment, min_window_size, max_window_size, resize_rate,
                       window_size_decrement_factor, window_size_backoff);
}
