...
public:
    ReusableSetHandle(RSUP backing, ReusableSetPool& owner)
        : _bits(backing->bits()), _curval(backing->generation()), _owned(std::move(backing)), _pool(owner) {}

    ~ReusableSetHandle();

    /** mark an ID */
    void mark(size_t id) { _bits[id] = _curval; }

    /** check if an ID has been marked */
    bool is_marked(size_t id) const { return (_bits[id] == _curval); }
