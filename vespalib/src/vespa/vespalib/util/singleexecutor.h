...
public:
    SingleExecutor(init_fun_t func, uint32_t reservedQueueSize, bool isQueueSizeHard, uint32_t watermark,
...
private:
    ...
    uint64_t index(uint64_t counter) const { return counter & (_taskLimit.load(std::memory_order_relaxed) - 1); }
    uint64_t numTasks();
    uint64_t numTasks(Lock& guard) const { return num_tasks_in_main_q() + num_tasks_in_overflow_q(guard); }
    uint64_t num_tasks_in_overflow_q(Lock&) const { return _overflow ? _overflow->size() : 0; }
