...
bool isLazy(const std::vector<std::unique_ptr<vespalib::SyncableThreadExecutor>>& executors) {
...
ssize_t find(uint16_t key, const uint16_t values[], size_t numValues) {
...
std::unique_ptr<ISequencedTaskExecutor> SequencedTaskExecutor::create(Runnable::init_fun_t func, uint32_t threads) {
    return create(func, threads, 1000);
}

std::unique_ptr<ISequencedTaskExecutor> SequencedTaskExecutor::create(
...
std::unique_ptr<ISequencedTaskExecutor> SequencedTaskExecutor::create(
    Runnable::init_fun_t func, uint32_t threads, uint32_t taskLimit, bool is_task_limit_hard, OptimizeFor optimize) {
    return create(func, threads, taskLimit, is_task_limit_hard, optimize, 0);
}

std::unique_ptr<ISequencedTaskExecutor> SequencedTaskExecutor::create(
    Runnable::init_fun_t func, uint32_t threads, uint32_t taskLimit, bool is_task_limit_hard, OptimizeFor optimize,
    uint32_t kindOfWatermark) {
    if (optimize == OptimizeFor::ADAPTIVE) {
        size_t num_strands = std::min(taskLimit, threads * 32);
        return std::make_unique<AdaptiveSequencedExecutor>(
            num_strands, threads, kindOfWatermark, taskLimit, is_task_limit_hard);
    } else {
        ...
        for (uint32_t id = 0; id < threads; ++id) {
            if (optimize == OptimizeFor::THROUGHPUT) {
                ...
                executors.push_back(
                    std::make_unique<SingleExecutor>(func, taskLimit, is_task_limit_hard, watermark, 100ms));
            } else {
...
SequencedTaskExecutor::~SequencedTaskExecutor() { sync_all(); }
...
SequencedTaskExecutor::SequencedTaskExecutor(std::vector<std::unique_ptr<vespalib::SyncableThreadExecutor>> executors)
    : ISequencedTaskExecutor(executors.size()),
      ...
      _component2IdImperfect(
          vespalib::hashtable_base::getModuloStl(getNumExecutors() * NUM_PERFECT_PER_EXECUTOR), MAGIC),
      _mutex(),
      _nextId(0) {
...
void SequencedTaskExecutor::setTaskLimit(uint32_t taskLimit) {
...
ExecutorStats SequencedTaskExecutor::getStats() {
...
std::vector<ExecutorStats> SequencedTaskExecutor::get_raw_stats() {
...
ISequencedTaskExecutor::ExecutorId SequencedTaskExecutor::getExecutorId(uint64_t componentId) const {
    auto id = getExecutorIdPerfect(componentId);
    return id ? id.value() : getExecutorIdImPerfect(componentId);
}

std::optional<ISequencedTaskExecutor::ExecutorId> SequencedTaskExecutor::getExecutorIdPerfect(
...
ISequencedTaskExecutor::ExecutorId SequencedTaskExecutor::getExecutorIdImPerfect(uint64_t componentId) const {
...
const vespalib::ThreadExecutor* SequencedTaskExecutor::first_executor() const {
    if (_executors.empty()) {
        return nullptr;
    }
