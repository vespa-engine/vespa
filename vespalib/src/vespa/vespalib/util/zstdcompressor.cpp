...
size_t ZStdCompressor::adjustProcessLen(uint16_t, size_t len) const {
    return ZSTD_compressBound(len);
}

bool ZStdCompressor::process(CompressionConfig config, const void* inputV, size_t inputLen, void* outputV,
                             size_t& outputLenV) {
...
bool ZStdCompressor::unprocess(const void* inputV, size_t inputLen, void* outputV, size_t& outputLenV) {
