...
struct Chunk {
    ...
    char* alloc(size_t size, size_t chunk_size) noexcept {
        size_t aligned_size = ((size + (sizeof(char*) - 1)) & ~(sizeof(char*) - 1));
...
public:
    ...
    char* alloc(size_t size) {
        char* ret = __builtin_expect(is_small(size) && _chunks != nullptr, true)
                        ? _chunks->alloc(size, _chunk_size)
                        : nullptr;
    ...
    template <typename T, typename... Args> std::span<T> create_array(size_t size, Args&&... args) {
        if (can_skip_destruction<T>) {
            return std::span<T>(
                init_array<T, Args...>(alloc(size * sizeof(T)), size, std::forward<Args>(args)...), size);
        }
