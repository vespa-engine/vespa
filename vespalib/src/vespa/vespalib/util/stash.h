...
template <typename T> struct DestructObject final : public Cleanup {
...
template <typename T> struct DestructArray final : public Cleanup {
    size_t size;
    explicit DestructArray(Cleanup* next_in, size_t size_in) noexcept : Cleanup(next_in), size(size_in) {}
...
private:
    ...
    template <typename T, typename... Args> T* init_array(char* mem, size_t size, Args&&... args) {
    ...
    template <typename T> T* copy_elements(char* mem, std::span<const T> src) {
        T* array = reinterpret_cast<T*>(mem);
...
public:
    ...
    char* alloc(size_t size) {
        char* ret = __builtin_expect(is_small(size) && _chunks != nullptr, true) ? _chunks->alloc(size, _chunk_size)
                                                                                 : nullptr;
    ...
    template <typename T, typename... Args> std::span<T> create_array(size_t size, Args&&... args) {
        static_assert(stash::align % alignof(T) == 0);
        if (can_skip_destruction<T>) {
            return std::span<T>(init_array<T, Args...>(alloc(size * sizeof(T)), size, std::forward<Args>(args)...),
                                size);
        }
    ...
    template <typename T> std::span<T> create_uninitialized_array(size_t size) {
        static_assert(stash::align % alignof(T) == 0);
    ...
    template <typename T> std::span<T> copy_array(std::span<const T> src) {
        static_assert(stash::align % alignof(T) == 0);
        if (can_skip_destruction<T>) {
