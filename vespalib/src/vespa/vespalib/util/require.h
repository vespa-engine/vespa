...
template <typename A, typename B, class = void> struct are_comparable : std::false_type {};

template <typename A, typename B> struct are_comparable<A, B, std::void_t<comparability<A, B>>> : std::true_type {};

//-----------------------------------------------------------------------------

template <typename S, typename V> using streamability = decltype(std::declval<S&>() << std::declval<const V&>());

template <typename S, typename V, class = void> struct is_streamable : std::false_type {};

template <typename S, typename V> struct is_streamable<S, V, std::void_t<streamability<S, V>>> : std::true_type {};
...
template <class A, class B> constexpr bool eq(const A& a, const B& b) {
    if constexpr (are_comparable<A, B>()) {
        return (a == b);
    } else if constexpr (std::is_same_v<A, B> && std::has_unique_object_representations_v<A>) {
        static_assert(sizeof(A) == sizeof(B));
        return mem_eq(reinterpret_cast<const char*>(std::addressof(a)),
                      reinterpret_cast<const char*>(std::addressof(b)), sizeof(A));
    } else {
...
template <typename S, typename V> void print(S& os, const V& value) {
...
template <typename A, typename B>
void handle_require_eq_failure
    [[noreturn]] (const A& a, const B& b, const char* a_desc, const char* b_desc, const char* description,
...
#define REQUIRE(...)                                                                                   \
    (__VA_ARGS__) ? vespalib::handle_require_success()                                                 \
                  : vespalib::handle_require_failure(VESPA_STRINGIZE(__VA_ARGS__), __FILE__, __LINE__)
...
#define REQUIRE_EQ(a, b)                                                                                        \
    vespalib::require_impl::eq(a, b)                                                                            \
        ? vespalib::handle_require_success()                                                                    \
        : vespalib::handle_require_eq_failure(a, b, VESPA_STRINGIZE(a), VESPA_STRINGIZE(b),                     \
                                              VESPA_STRINGIZE(a) " == " VESPA_STRINGIZE(b), __FILE__, __LINE__)

/**
 * Signal the failure of some requirement with a message.
 * Can be used instead of abort()
 **/
#define REQUIRE_FAILED(msg) vespalib::handle_require_failure(msg, __FILE__, __LINE__)
