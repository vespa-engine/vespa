...
template <typename T, typename B> class PrimitiveArrayT final : public IArrayT<B> {
...
public:
    ...
    iterator    erase(iterator it) override {
        _array.erase(_array.begin() + (it - this->begin()));
        return it;
    }
...
template <typename B> class ComplexArrayT final : public IArrayT<B> {
    using typename IArrayT<B>::iterator;
...
template <typename B> ComplexArrayT<B>::~ComplexArrayT() = default;
