...
JsonStream::StateEntry::StateEntry() noexcept : state(State::ROOT), object_key(""), array_index(size_t(0)) {}
JsonStream::StateEntry::StateEntry(State s) noexcept : state(s), object_key(""), array_index(size_t(0)) {}
...
const char* JsonStream::getStateName(const State& s) {
    switch (s) {
    case State::OBJECT_EXPECTING_KEY:
        return "ObjectExpectingKey";
    case State::OBJECT_EXPECTING_VALUE:
        return "ObjectExpectingValue";
    case State::ARRAY:
        return "ArrayExpectingValue";
    case State::ROOT:
        return "RootExpectingArrayOrObjectStart";
    }
...
JsonStream& JsonStream::operator<<(std::string_view value) {
    if (_state.empty()) {
...
JsonStream& JsonStream::operator<<(long long value) {
    if (_state.empty()) {
...
JsonStream& JsonStream::operator<<(unsigned long long value) {
    if (_state.empty()) {
...
JsonStream& JsonStream::operator<<(const Object&) {
    if (_state.empty()) {
...
JsonStream& JsonStream::operator<<(const Array&) {
    if (_state.empty()) {
...
JsonStream& JsonStream::operator<<(const End&) {
    if (_state.empty()) {
...
JsonStream& JsonStream::finalize() {
    while (!_state.empty()) {
        operator<<(End());
    }
    return *this;
...
std::string JsonStream::getStateString() const {
    asciistream as;
...
std::string JsonStream::getJsonStreamState() const {
    asciistream report;
    report << "Current: " << getStateString();
    return report.str();
}

void JsonStream::fail(std::string_view error) const {
    asciistream report;
    report << "Invalid state on call: " << error << " (" << getStateString() << ")";
