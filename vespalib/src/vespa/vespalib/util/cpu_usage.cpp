...
public:
    ...
    duration sample() const noexcept override { return from_s(to_s(steady_clock::now() - _start) * _util); }
};
...
public:
    LinuxThreadSampler() : _my_clock() { REQUIRE_EQ(pthread_getcpuclockid(pthread_self(), &_my_clock), 0); }
...
CpuUsage::ThreadTrackerImpl::ThreadTrackerImpl(cpu_usage::ThreadSampler::UP sampler)
    : _lock(), _cat(Category::OTHER), _old_usage(), _sampler(std::move(sampler)), _pending() {}

CpuUsage::Category CpuUsage::ThreadTrackerImpl::set_category(Category new_cat) noexcept {
...
CpuUsage::Sample CpuUsage::ThreadTrackerImpl::sample() noexcept {
    Guard guard(_lock);
    if (_cat != Category::OTHER) {
        duration new_usage = _sampler->sample();
        _pending[_cat] += (new_usage - _old_usage);
...
std::string& CpuUsage::name_of(Category cat) {
    static std::array<std::string, num_categories> names = {"setup", "read", "write", "compact", "other"};
    return names[index_of(cat)];
}

CpuUsage::Category CpuUsage::MyUsage::set_cpu_category_for_this_thread(Category cat) noexcept {
    struct Wrapper {
...
CpuUsage::CpuUsage()
    : _lock(), _usage(), _threads(), _sampling(false), _conflict(), _pending_add(), _pending_remove() {}

CpuUsage::~CpuUsage() = default;

CpuUsage& CpuUsage::self() {
    static CpuUsage me;
    return me;
}
...
void CpuUsage::remove_thread(ThreadTracker::SP tracker) {
    Guard guard(_lock);
    if (_sampling) {
...
void CpuUsage::handle_pending(const Guard& guard) {
...
CpuUsage::TimedSample CpuUsage::sample_or_wait() {
...
CpuUsage::TimedSample CpuUsage::sample() { return self().sample_or_wait(); }

Runnable::init_fun_t CpuUsage::wrap(Runnable::init_fun_t init, Category cat) {
...
Executor::Task::UP CpuUsage::wrap(Executor::Task::UP task, Category cat) {
    struct CpuTask : Executor::Task {
        UP       task;
        Category cat;
        CpuTask(UP task_in, Category cat_in) : task(std::move(task_in)), cat(cat_in) {}
        void run() override {
            auto my_usage = CpuUsage::use(cat);
            task->run();
        }
