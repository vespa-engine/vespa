...
duration adjustTimeoutByHz(duration timeout, long hz) { return (timeout * 1000) / hz; }

duration adjustTimeoutByDetectedHz(duration timeout) { return adjustTimeoutByHz(timeout, getVespaTimerHz()); }
...
std::string to_string(system_time time) { return to_string(time.time_since_epoch()); }

std::string to_string(file_time time) { return to_string(time.time_since_epoch()); }
