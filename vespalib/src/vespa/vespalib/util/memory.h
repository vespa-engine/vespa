...
template <typename To, typename From> inline std::unique_ptr<To> static_cast_unique(std::unique_ptr<From> src) {
...
public:
    ...
    MallocPtr(MallocPtr&& rhs) noexcept : _sz(rhs.size()), _p(rhs._p) {
        rhs._sz = 0;
        rhs._p = nullptr;
    }
     ...
     **/
    MallocPtr(const MallocPtr& rhs) noexcept : _sz(rhs.size()), _p(_sz ? malloc(_sz) : nullptr) {
        if (_p == nullptr) {
            _sz = 0;
        }
 ...
 */
template <typename T> class CloneablePtr {
public:
    ...
    CloneablePtr(std::unique_ptr<T>&& rhs) noexcept : _p(rhs.release()) {}
    ...
    bool     operator==(const CloneablePtr& b) const {
        return ((_p == nullptr) && (b._p == nullptr)) || ((_p && b._p) && (*_p == *b._p));
    }
