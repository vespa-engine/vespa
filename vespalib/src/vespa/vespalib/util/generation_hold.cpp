...
GenerationHold::GenerationHold() noexcept
    : _refCount(1),
      _generation(0),
      _next(nullptr)
{ }
...
void
GenerationHold::setValid() noexcept {
    auto old = _refCount.fetch_sub(1, std::memory_order_release);
    assert(!valid(old));
}

bool
GenerationHold::setInvalid() noexcept {
    uint32_t refs = 0;
...
GenerationHold*
GenerationHold::copy(GenerationHold* self) noexcept {
    if (self == nullptr) {
        return nullptr;
    } else {
