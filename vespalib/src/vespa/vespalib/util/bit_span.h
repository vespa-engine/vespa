...
public:
    ...
    BitSpan(const void* data, uint32_t count) noexcept
        : _data(static_cast<const std::byte*>(data)), _offset(0), _count(count) {}
    BitSpan(const void* data, uint32_t offset, uint32_t count) noexcept
        : _data(static_cast<const std::byte*>(data)), _offset(offset), _count(count) {}
    bool operator[](uint32_t i) const noexcept {
        return ((_data[(_offset + i) / 8] >> ((_offset + i) % 8)) & std::byte{1}) != std::byte{0};
    }
