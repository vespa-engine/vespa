...
std::string AsciiPrintable::PrintProperties::indent(uint32_t extraLevels) const {
...
void AsciiPrintable::print(std::ostream& out, bool verbose, const std::string& indent) const {
    vespalib::asciistream as;
    print(as, PrintProperties(verbose ? VERBOSE : NORMAL, indent));
    out << as.view();
}

std::string AsciiPrintable::toString(const PrintProperties& p) const {
    vespalib::asciistream as;
    print(as, p);
    return as.str();
}
