...
public:
    static void reclaim(BTreeNode* node) noexcept { node->unFreeze(); }
};

template <typename ToFreeze> struct FrozenBtreeNode : public ToFreeze {
...
public:
    ...
    template <typename NodeType> const NodeType* mapRef(EntryRef ref) const noexcept {
        RefType iRef(ref);
        return _store.getEntry<NodeType>(iRef);
    }

    template <typename NodeType> NodeType* mapRef(EntryRef ref) noexcept {
        RefType iRef(ref);
        return _store.getEntry<NodeType>(iRef);
    }
    ...
    datastore::MemoryStats getMemStats() const noexcept { return _store.getMemStats(); }
    ...
    // Inherit doc from DataStoreBase
    vespalib::MemoryUsage getMemoryUsage() const noexcept { return _store.getMemoryUsage(); }

    // Inherit doc from DataStoreT
    bool getCompacting(EntryRef ref) noexcept { return _store.getCompacting(ref); }
...
extern template class BTreeNodeStore<uint32_t, uint32_t, NoAggregated, BTreeDefaultTraits::INTERNAL_SLOTS,
                                     BTreeDefaultTraits::LEAF_SLOTS>;
extern template class BTreeNodeStore<uint32_t, BTreeNoLeafData, NoAggregated, BTreeDefaultTraits::INTERNAL_SLOTS,
                                     BTreeDefaultTraits::LEAF_SLOTS>;
extern template class BTreeNodeStore<uint32_t, int32_t, MinMaxAggregated, BTreeDefaultTraits::INTERNAL_SLOTS,
                                     BTreeDefaultTraits::LEAF_SLOTS>;

extern template class BTreeNodeBufferType<
    BTreeInternalNode<uint32_t, NoAggregated, BTreeDefaultTraits::INTERNAL_SLOTS>>;
extern template class BTreeNodeBufferType<
    BTreeInternalNode<uint32_t, MinMaxAggregated, BTreeDefaultTraits::INTERNAL_SLOTS>>;

extern template class BTreeNodeBufferType<
    BTreeLeafNode<uint32_t, uint32_t, NoAggregated, BTreeDefaultTraits::LEAF_SLOTS>>;
extern template class BTreeNodeBufferType<
