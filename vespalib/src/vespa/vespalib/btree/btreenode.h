...
protected:
    ...
    explicit BTreeNode(uint8_t level) noexcept : _level(level), _isFrozen(false), _validSlots(0) {}
 ...
 * overhead.
 */
template <class DataT, uint32_t NumSlots> class BTreeNodeDataWrap {
public:
...
template <uint32_t NumSlots> class BTreeNodeDataWrap<BTreeNoLeafData, NumSlots> {
public:
...
template <typename AggrT> class BTreeNodeAggregatedWrap {
...
public:
    BTreeNodeAggregatedWrap() noexcept : _aggr() {}
...
template <> class BTreeNodeAggregatedWrap<NoAggregated> {
...
template <typename KeyT, uint32_t NumSlots> class BTreeNodeT : public BTreeNode {
protected:
    KeyT _keys[NumSlots];
    explicit BTreeNodeT(uint8_t level) noexcept : BTreeNode(level), _keys() {}

    ~BTreeNodeT() = default;

    BTreeNodeT(const BTreeNodeT& rhs) noexcept : BTreeNode(rhs) {
...
public:
    ...
    template <typename CompareT> uint32_t lower_bound(uint32_t sidx, const KeyT& key, CompareT comp) const noexcept;

    template <typename CompareT> uint32_t lower_bound(const KeyT& key, CompareT comp) const noexcept;

    template <typename CompareT> uint32_t upper_bound(uint32_t sidx, const KeyT& key, CompareT comp) const noexcept;
...
class BTreeNodeTT
    : public BTreeNodeT<KeyT, NumSlots>,
...
protected:
    explicit BTreeNodeTT(uint8_t level) noexcept : ParentType(level), DataWrapType() {}

    ~BTreeNodeTT() = default;

    BTreeNodeTT(const BTreeNodeTT& rhs) noexcept : ParentType(rhs), DataWrapType(rhs), AggrWrapType(rhs) {
...
public:
    ...
    template <typename, typename, typename, size_t, size_t> friend class BTreeNodeAllocator;
    template <typename, typename, typename, size_t, size_t> friend class BTreeNodeStore;
    template <typename, uint32_t> friend class BTreeNodeDataWrap;
    template <typename, typename> friend class datastore::BufferType;
    template <typename, typename> friend class datastore::Allocator;
    template <typename, typename...> friend struct datastore::allocator::Assigner;
...
protected:
    BTreeInternalNode() noexcept : ParentType(EMPTY_LEVEL), _validLeaves(0u) {}

    BTreeInternalNode(const BTreeInternalNode& rhs) noexcept : ParentType(rhs), _validLeaves(rhs._validLeaves) {}
...
public:
    ...
    template <typename NodeAllocatorType>
    void splitInsert(BTreeInternalNode* splitNode, uint32_t idx, const KeyT& key, const BTreeNode::Ref& data,
...
protected:
    ...
    BTreeLeafNode(const BTreeLeafNode& rhs) noexcept : ParentType(rhs) {}
...
public:
    ...
    template <typename FunctionType> void foreach_key(FunctionType func) const noexcept;
