...
void BTreeRemoverBase<KeyT, DataT, AggrT, INTERNAL_SLOTS, LEAF_SLOTS, AggrCalcT>::steal(
    InternalNodeType* pNode, BTreeNode::Ref sNodeRef, NodeType* sNode, uint32_t idx, NodeAllocatorType& allocator,
    [[maybe_unused]] const AggrCalcT& aggrCalc, Iterator& itr, uint32_t level) {
    ...
    if (leftVictim != nullptr && leftVictim->validSlots() + sNode->validSlots() <= NodeType::maxSlots()) {
    ...
    } else if (rightVictim != nullptr && rightVictim->validSlots() + sNode->validSlots() <= NodeType::maxSlots()) {
        sNode->stealAllFromRightNode(rightVictim);
        pNode->update(idx, sNode->getLastKey(), sNodeRef);
        pNode->remove(idx + 1);
        allocator.holdNode(rightVictimRef, rightVictim);
    } else if (leftVictim != nullptr &&
               (rightVictim == nullptr || leftVictim->validSlots() > rightVictim->validSlots()))
    {
        if (leftVictim->getFrozen()) {
            NodeTypeRefPair thawed = allocator.thawNode(leftVictimRef, leftVictim);
        ...
        if (rightVictim->getFrozen()) {
            NodeTypeRefPair thawed = allocator.thawNode(rightVictimRef, rightVictim);
...
void BTreeRemover<KeyT, DataT, AggrT, CompareT, TraitsT, AggrCalcT>::remove(
    BTreeNode::Ref& root, Iterator& itr, const AggrCalcT& aggrCalc) {
    assert(itr.valid());
    root = itr.thaw(root);
    ...
    for (; level < levels; ++level) {
        ...
        if (level == 0) {
            ...
            if (!sNode->isAtLeastHalfFull()) {
                ...
                // right sibling
                ParentType::template steal<LeafNodeType, LeafNodeTypeRefPair>(
                    node, subNode, sNode, idx, allocator, aggrCalc, itr, level);
            }
        } else {
            ...
            if (!sNode->isAtLeastHalfFull()) {
                // too few elements in sub node, steal from left or
                // right sibling
                ParentType::template steal<InternalNodeType, InternalNodeTypeRefPair>(
                    node, subNode, sNode, idx, allocator, aggrCalc, itr, level);
            }
        }
        if constexpr (AggrCalcT::hasAggregated()) {
