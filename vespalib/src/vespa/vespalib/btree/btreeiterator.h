...
template <typename, typename, typename, typename, typename, class> class BTreeInserter;
template <typename, typename, typename, size_t, size_t, class> class BTreeRemoverBase;
template <typename, typename, typename, typename, typename, class> class BTreeRemover;
template <typename, typename, typename, typename, typename> class BTreeIterator;

/**
 * Helper class to provide internal or leaf node and position within node.
 */
template <class NodeT> class NodeElement {
    template <typename, typename, typename, typename, typename, class> friend class BTreeInserter;
    template <typename, typename, typename, size_t, size_t, class> friend class BTreeRemoverBase;
    template <typename, typename, typename, typename, typename, class> friend class BTreeRemover;
    template <typename, typename, typename, typename, typename> friend class BTreeIterator;
...
public:
    ...
    void setNode(const NodeType* node) noexcept { _nodeAndIdx = (_nodeAndIdx & ~NODE_MASK) | uint64_t(node); }
    const NodeType* getNode() const noexcept { return reinterpret_cast<const NodeType*>(_nodeAndIdx & NODE_MASK); }
    void     setIdx(uint32_t idx) noexcept { _nodeAndIdx = (_nodeAndIdx & NODE_MASK) | (uint64_t(idx) << IDX_SHIFT); }
    ...
    bool operator!=(const NodeElement& rhs) const noexcept { return _nodeAndIdx != rhs._nodeAndIdx; }
};
 ...
 */
template <typename KeyT, typename DataT, typename AggrT, uint32_t INTERNAL_SLOTS = BTreeDefaultTraits::INTERNAL_SLOTS,
          uint32_t LEAF_SLOTS = BTreeDefaultTraits::LEAF_SLOTS, uint32_t PATH_SIZE = BTreeDefaultTraits::PATH_SIZE>
...
protected:
    ...
    BTreeIteratorBase(const KeyDataType* shortArray, uint32_t arraySize, const NodeAllocatorType& allocator,
     ...
     */
    template <typename FunctionType> void foreach_key_range_start(uint32_t level, FunctionType func) const {
        if (level > 0u) {
            --level;
     ...
     */
    template <typename FunctionType> void foreach_key_range_end(uint32_t level, FunctionType func) const {
        if (level > 0u) {
            --level;
            auto&    store = _allocator->getNodeStore();
            auto     node = _path[level].getNode();
...
public:
     ...
     */
    void setupEnd() noexcept { _leaf.invalidate(); }
...
class BTreeConstIterator
    : public BTreeIteratorBase<KeyT, DataT, AggrT, TraitsT::INTERNAL_SLOTS, TraitsT::LEAF_SLOTS, TraitsT::PATH_SIZE> {
protected:
    using ParentType =
        BTreeIteratorBase<KeyT, DataT, AggrT, TraitsT::INTERNAL_SLOTS, TraitsT::LEAF_SLOTS, TraitsT::PATH_SIZE>;
...
public:
     ...
     */
    template <class AggrCalcT>
    BTreeConstIterator(const KeyDataType* shortArray, uint32_t arraySize, const NodeAllocatorType& allocator,
                       const AggrCalcT& aggrCalc)
...
public:
    ...
    BTreeIterator(BTreeNode::Ref root, const NodeAllocatorType& allocator) noexcept : ParentType(root, allocator) {}

    template <class AggrCalcT>
    BTreeIterator(const KeyDataType* shortArray, uint32_t arraySize, const NodeAllocatorType& allocator,
                  const AggrCalcT& aggrCalc)
        : ParentType(shortArray, arraySize, allocator, aggrCalc) {}
    ...
    NodeAllocatorType& getAllocator() const noexcept { return const_cast<NodeAllocatorType&>(*_allocator); }

    BTreeNode::Ref moveFirstLeafNode(BTreeNode::Ref rootRef);

    void moveNextLeafNode();

    void writeData(const DataType& data) noexcept { _leaf.getWNode()->writeData(_leaf.getIdx(), data); }
     ...
     */
    template <class AggrCalcT> void updateData(const DataType& data, const AggrCalcT& aggrCalc) noexcept;
...
private:
    ...
    template <class AggrCalcT>
    BTreeNode::Ref addLevel(
        BTreeNode::Ref rootRef, BTreeNode::Ref splitNodeRef, bool inRightSplit, const AggrCalcT& aggrCalc);
