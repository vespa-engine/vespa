...
public:
    ...
    static void           add(MinMaxAggregated& a, const MinMaxAggregated& oldca, const MinMaxAggregated& ca) {
        a.add(oldca, ca);
    }

    /* Returns true if recalculation is needed */
    static bool remove(MinMaxAggregated& a, int32_t val) { return a.remove(val); }

    /* Returns true if recalculation is needed */
    static bool remove(MinMaxAggregated& a, const MinMaxAggregated& oldca, const MinMaxAggregated& ca) {
        return a.remove(oldca, ca);
    }

    /* Returns true if recalculation is needed */
    static bool update(MinMaxAggregated& a, int32_t oldVal, int32_t val) { return a.update(oldVal, val); }

    /* Returns true if recalculation is needed */
    static bool update(MinMaxAggregated& a, const MinMaxAggregated& oldca, const MinMaxAggregated& ca) {
