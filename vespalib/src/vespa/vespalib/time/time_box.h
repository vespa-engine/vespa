...
private:
    ...
    static inline double to_external(clock::duration x) { return std::chrono::duration_cast<seconds>(x).count(); }
...
public:
     ...
     **/
    explicit TimeBox(double budget) : _end_time(clock::now() + to_internal(budget)), _min_time(to_internal(0)) {}
    ...
    TimeBox(double budget, double min_time)
        : _end_time(clock::now() + to_internal(budget)), _min_time(to_internal(min_time)) {}

    /**
     * @return true if there is time left in the budget
     **/
    bool hasTimeLeft() { return clock::now() < _end_time; }
