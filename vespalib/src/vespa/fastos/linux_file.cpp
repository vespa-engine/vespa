        ...
        if (DIRECTIOPOSSIBLE(buffer, length, readOffset)) {
            ...
            if (DIRECTIOPOSSIBLE(buffer, alignedLength, readOffset)) {
                ...
                if (static_cast<size_t>(readResult) == alignedLength && (remain != 0)) {
                    ssize_t readResult2 = readUnalignedEnd(static_cast<char*>(buffer) + alignedLength, remain,
                                                           readOffset + alignedLength);
                    if (readResult == 0) {
        ...
        if (DIRECTIOPOSSIBLE(buffer, length, _filePointer)) {
            ...
            if (DIRECTIOPOSSIBLE(buffer, alignedLength, _filePointer)) {
                ...
                if (static_cast<size_t>(writeRes) == alignedLength && remain != 0) {
                    ssize_t writeRes2 = writeUnalignedEnd(static_cast<const char*>(buffer) + alignedLength, remain,
                                                          _filePointer + alignedLength);
                    if (writeRes == 0) {
                        writeRes = writeRes2;
                    } else if (writeRes2 > 0) {
                        writeRes += writeRes2;
...
size_t FastOS_Linux_File::getMaxDirectIOMemAlign() {
