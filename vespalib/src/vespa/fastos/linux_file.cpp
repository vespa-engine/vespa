...
#define DIRECTIOPOSSIBLE(buf, len, off)                                                    \
    (((off) & (_directIOFileAlign - 1)) == 0 && ((len) & (_directIOFileAlign - 1)) == 0 && \
     (reinterpret_cast<unsigned long>(buf) & (_directIOMemAlign - 1)) == 0)

ssize_t FastOS_Linux_File::readInternal(int fh, void* buffer, size_t length, int64_t readOffset) {
...
ssize_t FastOS_Linux_File::readInternal(int fh, void* buffer, size_t length) {
    char*   data = static_cast<char*>(buffer);
    ssize_t has_read(0);
    while (has_read < ssize_t(length)) {
        size_t  lenNow = std::min(getChunkSize(), length - has_read);
...
ssize_t FastOS_Linux_File::writeInternal(int fh, const void* buffer, size_t length, int64_t writeOffset) {
    return File_RW_Ops::pwrite(fh, buffer, length, writeOffset);
}

ssize_t FastOS_Linux_File::writeInternal(int fh, const void* buffer, size_t length) {
...
ssize_t FastOS_Linux_File::readUnalignedEnd(void* buffer, size_t length, int64_t readOffset) {
    ...
    if (fh < 0) {
        ...
        os << "Failed opening file " << GetFileName()
           << " for reading the unaligend end due to : " << getLastErrorString();
...
ssize_t FastOS_Linux_File::ReadBufInternal(void* buffer, size_t length, int64_t readOffset) {
    if (length == 0) {
        return 0;
    }
    ssize_t readResult;
            ...
            if (DIRECTIOPOSSIBLE(buffer, alignedLength, readOffset)) {
                ...
                if (static_cast<size_t>(readResult) == alignedLength && (remain != 0)) {
                    ssize_t readResult2 = readUnalignedEnd(
                        static_cast<char*>(buffer) + alignedLength, remain, readOffset + alignedLength);
                    if (readResult == 0) {
...
void FastOS_Linux_File::ReadBuf(void* buffer, size_t length, int64_t readOffset) {
...
ssize_t FastOS_Linux_File::Read(void* buffer, size_t len) {
    if (_directIOEnabled) {
...
ssize_t FastOS_Linux_File::Write2(const void* buffer, size_t length) {
...
ssize_t FastOS_Linux_File::internalWrite2(const void* buffer, size_t length) {
    ssize_t writeRes;
    if (_directIOEnabled) {
            ...
            if (DIRECTIOPOSSIBLE(buffer, alignedLength, _filePointer)) {
                ...
                if (static_cast<size_t>(writeRes) == alignedLength && remain != 0) {
                    ssize_t writeRes2 = writeUnalignedEnd(
                        static_cast<const char*>(buffer) + alignedLength, remain, _filePointer + alignedLength);
                    if (writeRes == 0) {
                        writeRes = writeRes2;
                    } else if (writeRes2 > 0) {
                        writeRes += writeRes2;
...
bool FastOS_Linux_File::SetPosition(int64_t desiredPosition) {
...
int64_t FastOS_Linux_File::getPosition() const {
    return _directIOEnabled ? _filePointer : FastOS_UNIX_File::getPosition();
}

bool FastOS_Linux_File::SetSize(int64_t newSize) {
...
void* FastOS_Linux_File::AllocateDirectIOBuffer(size_t byteSize) {
...
size_t FastOS_Linux_File::getMaxDirectIOMemAlign() { return _directIOMemAlign; }

bool FastOS_Linux_File::GetDirectIORestrictions(
...
bool FastOS_Linux_File::DirectIOPadding(int64_t offset, size_t length, size_t& padBefore, size_t& padAfter) {
    if (_directIOEnabled) {
        ...
        if ((padAfter != 0) && (static_cast<int64_t>(offset + length + padAfter) > fileSize) &&
...
void FastOS_Linux_File::EnableDirectIO() {
    if (!IsOpened()) {
        _directIOEnabled = true;
    }
}

bool FastOS_Linux_File::Open(unsigned int openFlags, const char* filename) {
    bool rc;
    _cachedSize = -1;
    _filePointer = -1;
...
int FastOS_Linux_File::count_open_files() {
