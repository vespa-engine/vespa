...
#define LOG_SETUP(...) static ns_log::Logger ns_log_logger(__VA_ARGS__) // NOLINT
...
#define LOG_INDIRECT_WOULD_LOG(levelName) ns_log_indirect_logger->wants(ns_log::Logger::levelName)
...
#define VLOG(level, ...)                                                                                               \
    do {                                                                                                               \
        if (__builtin_expect(LOG_WOULD_VLOG(level), false)) {                                                          \
            ns_log_logger.doLog(level, __FILE__, __LINE__, __VA_ARGS__);                                               \
        }                                                                                                              \
    } while (false)
#endif
...
#define LOG_INDIRECT(level, ...)                                                                                       \
    do {                                                                                                               \
        if (!logInitialised) {                                                                                         \
            logInitialised = true;                                                                                     \
            ns_log_indirect_logger = static_cast<Logger *>(malloc(sizeof *ns_log_indirect_logger));                    \
            new (ns_log_indirect_logger) Logger(logName, indirectRcsId);                                               \
        }                                                                                                              \
        if (LOG_INDIRECT_WOULD_LOG(level)) {                                                                           \
            ns_log_indirect_logger->doLog(ns_log::Logger::level, __FILE__, __LINE__, __VA_ARGS__);                     \
        }                                                                                                              \
    } while (false)
...
#define EV_STOPPED(name, pid, exitcode)                                                                                \
    do {                                                                                                               \
        if (LOG_WOULD_LOG(event)) {                                                                                    \
            ns_log_logger.doEventStopped(name, pid, exitcode);                                                         \
        }                                                                                                              \
    } while (false)
...
#define EV_PROGRESS(name, ...)                                                                                         \
    do {                                                                                                               \
        if (LOG_WOULD_LOG(event)) {                                                                                    \
            ns_log_logger.doEventProgress(name, __VA_ARGS__);                                                          \
        }                                                                                                              \
    } while (false)
...
class Logger {
public:
    enum LogLevel { fatal, error, warning, config, info, event, debug, spam, NUM_LOGLEVELS };
...
private:
    ...
    int tryLog(int sizeofPayload, LogLevel level, const char *file, int line, const char *fmt, va_list args);
...
public:
    ...
    void doLog(LogLevel level, const char *file, int line, const char *fmt, ...) __attribute__((format(printf, 5, 6)));
    /**
     ...
     */
    void doLogCore(const Timer &, LogLevel level, const char *file, int line, const char *msg, size_t msgSize);
...
#define CHARS_TO_UINT(a, b, c, d)                                                                                      \
    (static_cast<unsigned char>(a) | static_cast<unsigned int>(static_cast<unsigned char>(b) << 8u) |                  \
     static_cast<unsigned int>(static_cast<unsigned char>(c) << 16u) |                                                 \
     static_cast<unsigned int>(static_cast<unsigned char>(d) << 24u))

inline bool Logger::wants(LogLevel level) { return _logLevels[level] == CHARS_TO_UINT(' ', ' ', 'O', 'N'); }
...
#define LOG_ABORT(msg) (ns_log::log_abort(msg, __FILE__, __LINE__))

#ifndef NDEBUG
#define LOG_ASSERT(expr) ((void)((expr) ? 0 : (ns_log::log_assert_fail(__STRING(expr), __FILE__, __LINE__), 0)))
#else
