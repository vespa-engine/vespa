...
void LLParser::doInput(char* line) {
    ...
    if (tab) {
        *tab = '\0';
        ...
        if (tab) {
            *tab = '\0';
                ...
                *tab = '\0';
                    ...
                    *tab = '\0';
                        ...
                        *tab = '\0';
                        ...
                        if (validLevel(l)) {
                            ...
                            if (timefield && pidfield == 2) {
                                // missing host
                                makeMessage(first, empty, second, third, fourth, l, sixth);
                                return;
                            }
                            if (timefield && pidfield == 3) {
                                // missing service
                                makeMessage(first, second, third, empty, fourth, l, sixth);
                                return;
                            }
                            if (!timefield && pidfield == 2) {
                                // missing time
                                makeMessage(empty, first, second, third, fourth, l, sixth);
                                return;
                            }
                            if (timefield && pidfield == 0) {
                                // missing pid
                                makeMessage(first, second, empty, third, fourth, l, sixth);
                                return;
                            }
                    ...
                    if (validLevel(l)) {
                        ...
                        if (!timefield && pidfield == 0) {
                            ...
                            makeMessage(empty, first, empty, second, third, l, fifth);
                            return;
                        }
                        if (!timefield && pidfield == 1) {
                            // missing time and host
                            makeMessage(empty, empty, first, second, third, l, fifth);
                            return;
                        }
                        if (!timefield && pidfield == 2) {
                            // missing time and service
                            makeMessage(empty, first, second, empty, third, l, fifth);
                            return;
                        }
                        if (timefield && pidfield == 2) {
                            // missing host and service
                            makeMessage(first, empty, second, empty, third, l, fifth);
                            return;
                        }
                        if (timefield && pidfield == 3) {
                            // missing service and component
                            makeMessage(first, second, third, empty, empty, l, fifth);
                            return;
                        }
                        if (timefield && pidfield == 0) {
                            ...
                            if (_defService == second) {
                                ...
                                makeMessage(first, empty, empty, second, third, l, fifth);
                                return;
                            }
                            makeMessage(first, second, empty, empty, third, l, fifth);
                            return;
                        }
                ...
                if (validLevel(l)) {
                    ...
                    if (timefield && pidfield == 2) {
                        ...
                        makeMessage(first, empty, second, empty, empty, l, fourth);
                        return;
                    }
                    if (timefield && pidfield == 0) {
                        // missing host, pid, service
                        makeMessage(first, empty, empty, empty, second, l, fourth);
                        return;
                    }
                    if (!timefield && pidfield == 1) {
                        // missing time, host, service
                        makeMessage(empty, empty, first, empty, second, l, fourth);
                        return;
                    }
                    if (!timefield && pidfield == 0) {
                        ...
                        if (_defService == first) {
                            // it's the service, assume missing hostname
                            makeMessage(empty, empty, empty, first, second, l, fourth);
                            return;
                        }
                        // missing service
                        makeMessage(empty, first, empty, empty, second, l, fourth);
                        return;
                    }
            ...
            if (validLevel(l)) {
                if (timefield) {
                    // time, level, message
                    makeMessage(first, empty, empty, empty, empty, l, third);
                    return;
                }
                if (pidfield) {
                    // pid, level, message
                    makeMessage(empty, empty, first, empty, empty, l, third);
                    return;
                }
                // component, level, message
                makeMessage(empty, empty, empty, empty, first, l, third);
                return;
            }
            // pretend 2 fields
            tab = third - 1;
        ...
        if (validLevel(l)) {
            makeMessage(empty, empty, empty, empty, empty, l, second);
            return;
        }
        // pretend 1 field
        tab = second - 1;
    ...
    makeMessage(empty, empty, empty, empty, empty, _defLevel, line);
}
...
void LLParser::makeMessage(const char* tmf, const char* hsf, const char* pdf, const char* svf, const char* cmf,
    ...
    if (tmf[0] == '\0') {
        ...
        snprintf(tmbuffer, 24, "%u.%06u", static_cast<unsigned int>(tv.tv_sec), static_cast<unsigned int>(tv.tv_usec));
        tmf = tmbuffer;
    }
    ...
    do {
        ...
        if ((c == '\\' && src[0] == 't') || (c >= 32 && c < '\\') || (c > '\\' && c < 128) || c == 0) {
...
void LLParser::setPid(int p) { snprintf(_defPid, 10, "%d", p); }
