...
Lock::Lock(int arg_fd) : _fd(dup(arg_fd)), _isLocked(false) {
    if (_fd == -1) {
        throwInvalid("Cannot duplicate fd %d for lock: %s", arg_fd, strerror(errno));
    }
}
...
Lock::Lock(const char* filename, int mode) : _fd(open(filename, mode, 0666)), _isLocked(false) {
    if (_fd == -1) {
        ...
        if (_fd == -1) {
            throwInvalid("Cannot open lockfile '%s': %s", filename, strerror(errno));
        }
    }
