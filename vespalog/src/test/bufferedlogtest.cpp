...
void spamLog1(uint64_t& time, int diff) {
    for (int i = 0; i < 100; ++i) {
        ...
        time += diff;
        LOGBT(warning, "Failed to send to node 4", "Failed to send message to node 4: NOT_READY");
        time += diff;
        LOGBT(warning, "Failed to send to node 4", "Failed to send message to node 4: BAAAH");
        time += diff;
        LOGBT(warning, "Failed to send to node 4", "Failed to send message to node 4: RPC FAILURE");
        time += diff;
        LOGBT(warning, "Failed to send to node 4", "Failed to send message to node 4: COSMIC RADIATION");
        time += diff;
        LOGBT(warning, "Failed to send to node 4", "Failed to send message to node 4: ITS SATURDAY");
        time += diff;
        LOGBT(warning, "Failed to send to node 4", "Failed to send message to node 4: Yeah, Right!!");
        time += diff;
        ...
        LOGBT(warning, "Failed to send to node 4", "Failed to send message to node 4: NOT CONNECTED");
        time += diff * 10;
        LOGBT(warning, "Failed to send to node 3", "Failed to send message to node 3: NOT CONNECTED");
    }
}
...
void testThatEntriesWithHighCountIsKept(const std::string& file, uint64_t& timer) {
    ...
    if (result != expected) {
        ...
        [[maybe_unused]] int systemResult = system(("diff -u " + file + " bufferedlogtest.highcountkept.log").c_str());
        std::_Exit(EXIT_FAILURE);
    }
    unlink(file.c_str());
}

void testThatEntriesWithHighCountsAreEventuallyRemoved(const std::string& file, uint64_t& timer) {
    ...
    if (result != expected) {
        ...
        [[maybe_unused]] int systemResult =
            system(("diff -u " + file + " bufferedlogtest.highcountexpire.log").c_str());
        std::_Exit(EXIT_FAILURE);
    }
    unlink(file.c_str());
}

void testThatEntriesExpire(const std::string& file, uint64_t& timer) {
    ...
    if (result != expected) {
        ...
        [[maybe_unused]] int systemResult = system(("diff -u " + file + " bufferedlogtest.expire.log").c_str());
        std::_Exit(EXIT_FAILURE);
    }
    unlink(file.c_str());
}
...
void spamLog3(uint64_t& time, int diff) {
    for (int i = 0; i < 100; ++i) {
        ...
        time += diff;
        LOGBT(warning, "Failed to talk to node 2", "Failed to send message to node 0x2: NOT_READY");
        time += diff;
        LOGBT(warning, "Failed to talk to node 3", "Failed to send message to node 0x3: BAAAH");
        time += diff;
        LOGBT(warning, "Failed to talk to node 4", "Failed to send message to node 0x4: RPC FAILURE");
        time += diff;
        LOGBT(warning, "Failed to talk to node 5", "Failed to send message to node 0x5: COSMIC RADIATION");
        time += diff;
        LOGBT(warning, "Failed to talk to node 6", "Failed to send message to node 0x6: ITS SATURDAY");
        time += diff;
        LOGBT(warning, "Failed to talk to node 7", "Failed to send message to node 0x7: Yeah, Right!!");
        time += diff;
        LOGBT(error, "Failed to talk to node 8", "Failed to send message to node 0x8: NOT CONNECTED");
        time += diff;
        LOGBT(info, "Failed to talk to node 9", "Failed to send message to node 0x9: NOT CONNECTED");
        time += diff;
        LOGBT(warning, "Failed to talk to node 10", "Failed to send message to node 0xa: NOT CONNECTED");
    }
}

void testThatHighCountEntriesDontStarveOthers(const std::string& file, uint64_t& timer) {
    ...
    if (result != expected) {
        ...
        [[maybe_unused]] int systemResult = system(("diff -u " + file + " bufferedlogtest.nostarve.log").c_str());
        std::_Exit(EXIT_FAILURE);
    }
    unlink(file.c_str());
}

void testNoTokenMatchAcrossComponents(const std::string& file, uint64_t& timer) {
    ...
    if (result != expected) {
        ...
        [[maybe_unused]] int systemResult =
            system(("diff -u " + file + " bufferedlogtest.tokenacrossloggers.log").c_str());
        std::_Exit(EXIT_FAILURE);
    }
    unlink(file.c_str());
}
...
void testLogLocationAsToken(const std::string& file, uint64_t& timer) {
    ...
    if (result != expected) {
        ...
        [[maybe_unused]] int systemResult =
            system(("diff -u " + file + " bufferedlogtest.locationastoken.log").c_str());
        std::_Exit(EXIT_FAILURE);
    }
    unlink(file.c_str());
}
...
void testLogMessageAsToken(const std::string& file, uint64_t& timer) {
    ...
    if (result != expected) {
        ...
        [[maybe_unused]] int systemResult = system(("diff -u " + file + " bufferedlogtest.messageastoken.log").c_str());
        std::_Exit(EXIT_FAILURE);
    }
    unlink(file.c_str());
}

void testNonBufferedLoggerTriggersBufferedLogTrim(const std::string& file, uint64_t& timer) {
    ...
    if (result != expected) {
        ...
        [[maybe_unused]] int systemResult = system(("diff -u " + file + " bufferedlogtest.trimcache.log").c_str());
        std::_Exit(EXIT_FAILURE);
    }
    unlink(file.c_str());
}
