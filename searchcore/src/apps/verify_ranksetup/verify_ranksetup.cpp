...
std::optional<std::string> get_file(const std::string& ref, const VerifyRanksetupConfig& myCfg) {
...
RankingExpressions make_expressions(const RankingExpressionsConfig& expressionsCfg,
                                    const VerifyRanksetupConfig& myCfg, std::vector<search::fef::Message>& messages) {
...
OnnxModels make_models(const OnnxModelsConfig& modelsCfg, const VerifyRanksetupConfig& myCfg,
    ...
    for (const auto& entry : modelsCfg.model) {
        if (auto file = get_file(entry.fileref, myCfg)) {
            ...
            messages.emplace_back(search::fef::Level::WARNING,
                                  fmt("could not find file name for onnx model '%s' (ref:'%s')", entry.name.c_str(),
                                      entry.fileref.c_str()));
        }
    }
...
private:
    ...
    bool verifyConfig(const VerifyRanksetupConfig& myCfg, const VsmfieldsConfig& vsmFieldsCcfg,
                      const RankProfilesConfig& rankCfg, const IndexschemaConfig& schemaCfg,
                      const AttributesConfig& attributeCfg, const RankingConstantsConfig& constantsCfg,
                      const RankingExpressionsConfig& expressionsCfg, const OnnxModelsConfig& modelsCfg);
...
struct DummyRankingAssetsRepo : IRankingAssetsRepo {
    ...
    DummyRankingAssetsRepo(const RankingConstantsConfig& cfg_in, RankingExpressions expressions,
...
DummyRankingAssetsRepo::DummyRankingAssetsRepo(const RankingConstantsConfig& cfg_in, RankingExpressions expressions,
                                               OnnxModels onnxModels) noexcept
    : cfg(cfg_in), _expressions(std::move(expressions)), _onnxModels(std::move(onnxModels)) {
}

DummyRankingAssetsRepo::~DummyRankingAssetsRepo() = default;

vespalib::eval::ConstantValue::UP DummyRankingAssetsRepo::getConstant(const std::string& name) const {
...
VerifyRankSetup::VerifyRankSetup(SearchMode mode) : _messages(), _searchMode(mode) {
}

VerifyRankSetup::~VerifyRankSetup() = default;

bool VerifyRankSetup::verifyIndexEnv(const search::fef::IIndexEnvironment& indexEnv) {
    ...
    if (!rankSetup.getSecondPhaseRank().empty()) {
        ok =
            verifyFeature(factory, indexEnv, rankSetup.getSecondPhaseRank(), "second phase ranking", _messages) && ok;
    }
...
bool VerifyRankSetup::verifyConfig(const VerifyRanksetupConfig& myCfg, const VsmfieldsConfig& vsmFieldsCfg,
                                   const RankProfilesConfig& rankCfg, const IndexschemaConfig& schemaCfg,
                                   const AttributesConfig& attributeCfg, const RankingConstantsConfig& constantsCfg,
                                   const RankingExpressionsConfig& expressionsCfg,
                                   const OnnxModelsConfig&         modelsCfg) {
    bool ok = true;
    auto repo = std::make_shared<DummyRankingAssetsRepo>(
        constantsCfg, make_expressions(expressionsCfg, myCfg, _messages), make_models(modelsCfg, myCfg, _messages));

    using IndexEnvFactory =
        std::function<std::unique_ptr<search::fef::IIndexEnvironment>(const search::fef::Properties&)>;
    ...
    for (const auto& profile : rankCfg.rankprofile) {
        ...
        if (verifyIndexEnv(*indexEnvP)) {
            _messages.emplace_back(search::fef::Level::INFO, fmt("rank profile '%s': pass", profile.name.c_str()));
        } else {
            _messages.emplace_back(search::fef::Level::ERROR, fmt("rank profile '%s': FAIL", profile.name.c_str()));
            ok = false;
        }
    }
    return ok;
...
bool VerifyRankSetup::verify(const std::string& configid) {
    bool ok = false;
    try {
        ...
        ConfigHandle<RankingConstantsConfig>::UP constantsHandle =
            subscriber.subscribe<RankingConstantsConfig>(cfgId);
        ConfigHandle<RankingExpressionsConfig>::UP expressionsHandle =
            subscriber.subscribe<RankingExpressionsConfig>(cfgId);
        ...
        ok = verifyConfig(*myHandle->getConfig(), *vsmFieldsCfg, *rankHandle->getConfig(), *schemaHandle->getConfig(),
                          *attributesHandle->getConfig(), *constantsHandle->getConfig(),
                          *expressionsHandle->getConfig(), *modelsHandle->getConfig());
    } catch (ConfigRuntimeException& e) {
        _messages.emplace_back(search::fef::Level::ERROR,
                               fmt("Unable to subscribe to config: %s", e.getMessage().c_str()));
    } catch (InvalidConfigException& e) {
        _messages.emplace_back(search::fef::Level::ERROR, fmt("Error getting config: %s", e.getMessage().c_str()));
    }
    return ok;
}
