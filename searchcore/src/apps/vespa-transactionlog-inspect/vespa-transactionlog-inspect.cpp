...
vespalib::nbostream& operator>>(vespalib::nbostream& stream, ConfigFile& configFile) {
...
struct DummyStreamHandler : public NewConfigOperation::IStreamHandler {
    ...
    DummyStreamHandler() : NewConfigOperation::IStreamHandler(), _cfs() {}

    void serializeConfig(SerialNum, vespalib::nbostream&) override {}

    void deserializeConfig(SerialNum, vespalib::nbostream& is) override {
        _cfs.clear();
        uint32_t numConfigs;
        is >> numConfigs;
...
protected:
    void print(const FeedOperation& op) { std::cout << "OP[" << (_counter++) << "]: " << op.toString() << std::endl; }

public:
    OperationPrinter(DocumentTypeRepo& repo) : _repo(repo), _streamHandler(), _counter(0) {}
    ...
    NewConfigOperation::IStreamHandler& getNewConfigStreamHandler() override { return _streamHandler; }
    document::DocumentTypeRepo& getDeserializeRepo() override { return _repo; }
...
public:
    DocumentPrinter(DocumentTypeRepo& repo, bool printXml_, bool verbose)
        : OperationPrinter(repo), _printXml(printXml_), _verbose(verbose) {}
...
public:
    VisitorCallback(IReplayPacketHandler& handler) : _dispatcher(handler), _eof(false) {}
    client::RPC::Result receive(const Packet& packet) override {
        ...
        try {
            ...
            std::cerr << "Error while handling transaction log packet: '" << std::string(e.what()) << "'"
                      << std::endl;
...
public:
    ...
    virtual std::string toString() const {
        return vespalib::make_string("tlsdir=%s, tlsname=%s, listenport=%d", tlsDir.c_str(), tlsName.c_str(),
                                     listenPort);
    }
    virtual Utility::UP createUtility() const = 0;
};

BaseOptions::BaseOptions(int argc, const char* const* argv) : _opts(argc, argv) {
...
public:
    ...
    ~BaseUtility() override { _transport.ShutDown(true); }
    int run() override = 0;
};
...
struct ListDomainsOptions : public BaseOptions {
    ListDomainsOptions(int argc, const char* const* argv) : BaseOptions(argc, argv) {
...
public:
    ListDomainsUtility(const ListDomainsOptions& opts) : BaseUtility(opts) {}
    int run() override {
...
Utility::UP ListDomainsOptions::createUtility() const {
...
struct DumpOperationsOptions : public BaseOptions {
    ...
    std::string toString() const override {
        return vespalib::make_string("%s, domain=%s, first=%" PRIu64 ", last=%" PRIu64 ", configdir=%s",
                                     BaseOptions::toString().c_str(), domainName.c_str(), firstSerialNum,
                                     lastSerialNum, configDir.c_str());
    }
    Utility::UP createUtility() const override;
};

DumpOperationsOptions::DumpOperationsOptions(int argc, const char* const* argv) : BaseOptions(argc, argv) {
...
public:
    DumpOperationsUtility(const DumpOperationsOptions& oopts) : BaseUtility(oopts), _oopts(oopts) {}
    int run() override {
...
Utility::UP DumpOperationsOptions::createUtility() const {
...
struct DumpDocumentsOptions : public DumpOperationsOptions {
    ...
    std::string toString() const override {
        return vespalib::make_string("%s, format=%s, verbose=%s", DumpOperationsOptions::toString().c_str(),
                                     format.c_str(), (verbose ? "true" : "false"));
    }
    Utility::UP createUtility() const override;
};

DumpDocumentsOptions::DumpDocumentsOptions(int argc, const char* const* argv) : DumpOperationsOptions(argc, argv) {
    _opts.addOption("format", format, std::string("xml"),
                    "Format in which the document operations should be dumped ('xml' or 'text')");
...
public:
    DumpDocumentsUtility(const DumpDocumentsOptions& dopts) : DumpOperationsUtility(dopts), _dopts(dopts) {}
    int run() override {
...
Utility::UP DumpDocumentsOptions::createUtility() const {
...
private:
    ...
    void usageHeader() { std::cout << _programName << " version 0.0\n"; }
