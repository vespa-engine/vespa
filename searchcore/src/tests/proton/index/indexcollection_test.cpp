...
public:
    MockIndexSearchable() : _field_length_info() {}
    explicit MockIndexSearchable(const FieldLengthInfo& field_length_info) : _field_length_info(field_length_info) {}
...
class IndexCollectionTest : public ::testing::Test, public IWarmupDone {
public:
    ...
    std::unique_ptr<IndexCollection> make_unique_collection() const {
        return std::make_unique<IndexCollection>(_selector);
    }

    std::shared_ptr<IndexCollection> make_shared_collection() const {
        return std::make_shared<IndexCollection>(_selector);
    }

    std::shared_ptr<WarmupIndexCollection> create_warmup(const IndexCollection::SP& prev,
                                                         const IndexCollection::SP& next) {
        return std::make_shared<WarmupIndexCollection>(WarmupConfig(1s, false), prev, next, *_warmup, _executor,
                                                       *this);
    }

    void warmupDone(std::shared_ptr<WarmupIndexCollection> current) override { (void)current; }
