...
void FusionRunnerTest::TearDown() {
    std::filesystem::remove_all(std::filesystem::path(base_dir));
    _selector.reset();
}
...
void addDocument(DocBuilder& doc_builder, MemoryIndex& index, ISourceSelector& selector, uint8_t index_id,
                 uint32_t docid, const string& word) {
...
void FusionRunnerTest::createIndex(const string& dir, uint32_t id, bool fusion) {
    ...
    MemoryIndex memory_index(schema, MockFieldLengthInspector(), _service.write().field_writer(),
...
std::string getFusionIndexName(uint32_t fusion_id) {
    vespalib::asciistream ost;
    ost << base_dir << "/index.fusion." << fusion_id;
    return ost.str();
}

void FusionRunnerTest::checkResults(uint32_t fusion_id, const uint32_t* ids, size_t size) {
