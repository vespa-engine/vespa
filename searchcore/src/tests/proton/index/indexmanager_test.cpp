...
struct IndexManagerTest : public ::testing::Test {
    ...
    template <class FunctionType> void runAsMaster(FunctionType&& function) {
        vespalib::Gate gate;
    ...
    template <class FunctionType> void runAsIndex(FunctionType&& function) {
        vespalib::Gate gate;
    ...
    void assertStats(uint32_t expNumDiskIndexes, uint32_t expNumMemoryIndexes, SerialNum expLastDiskIndexSerialNum,
    ...
    bool has_pending_urgent_flush() const { return _index_manager->has_pending_urgent_flush(); }
...
bool IndexManagerTest::has_urgent_memory_index_flush() const {
    IndexFlushTarget target(_index_manager->getMaintainer());
    return target.needUrgentFlush();
}

bool IndexManagerTest::has_urgent_fusion() const {
    IndexFusionTarget target(_index_manager->getMaintainer());
    return target.needUrgentFlush();
}

Document::UP IndexManagerTest::addDocument(uint32_t id) {
    Document::UP   doc = buildDocument(_builder, id, "foo");
    SerialNum      serialNum = ++_serial_num;
    vespalib::Gate gate;
    runAsIndex([&]() {
        _index_manager->putDocument(id, *doc, serialNum, {});
        _index_manager->commit(serialNum, std::make_shared<vespalib::GateCallback>(gate));
    });
    gate.await();
    return doc;
...
void IndexManagerTest::resetIndexManager(SerialNum serial_num) {
    _index_manager.reset();
    _index_manager = std::make_unique<IndexManager>(
        index_dir, std::shared_ptr<search::diskindex::IPostingListCache>(), IndexConfig(),
        getSchema(_interleaved_features), serial_num, _reconfigurer, _service.write(), _service.shared(),
...
void IndexManagerTest::assertStats(uint32_t expNumDiskIndexes, uint32_t expNumMemoryIndexes,
...
void IndexManagerTest::assert_urgent(const std::string& label, bool pending, bool flush, bool fusion) {
    SCOPED_TRACE(label);
...
IMemoryIndex* as_memory_index(const IIndexCollection& col, uint32_t source_id) {
    return as_index_type<IMemoryIndex>(col, source_id);
}

IDiskIndex* as_disk_index(const IIndexCollection& col, uint32_t source_id) {
    return as_index_type<IDiskIndex>(col, source_id);
}

void expect_field_length_info(double exp_average, uint32_t exp_samples, const IndexSearchable& searchable) {
...
EnableInterleavedFeaturesParam::EnableInterleavedFeaturesParam(const EnableInterleavedFeaturesParam&) noexcept =
    default;
...
class IndexManagerEnableInterleavedFeaturesTest
    : public IndexManagerTest,
      public testing::WithParamInterface<std::tuple<bool, bool, EnableInterleavedFeaturesParam>> {
protected:
    void enable_interleaved_features(const std::string& label, bool old_config_docs,
                                     bool                     flushed_interleaved_features,
                                     std::optional<SerialNum> serial_num = std::nullopt);
};

void IndexManagerEnableInterleavedFeaturesTest::enable_interleaved_features(const std::string& label,
                                                                            bool               old_config_docs,
                                                                            bool flushed_interleaved_features,
    ...
    assert_urgent(label, old_config_docs, old_config_docs && !flushed_interleaved_features,
                  old_config_docs && flushed_interleaved_features);
}
...
auto test_values = testing::Combine(
    testing::Bool(), testing::Bool(),
    testing::Values(EnableInterleavedFeaturesParam(), EnableInterleavedFeaturesParam().no_doc_restart1(),
                    EnableInterleavedFeaturesParam().doc_restart1(), EnableInterleavedFeaturesParam().doc_restart2(),
                    EnableInterleavedFeaturesParam().doc_restart2_pruned_config()));

INSTANTIATE_TEST_SUITE_P(MultiIndexManagerEnableInterleavedFeaturesTest, IndexManagerEnableInterleavedFeaturesTest,
                         test_values, param_as_string);
