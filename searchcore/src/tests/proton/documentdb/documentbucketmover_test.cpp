...
struct ControllerFixtureBase : public ::testing::Test {
    ...
    const MoveOperationVector& docsMoved() const { return _moveHandler._moves; }
    const std::vector<BucketId>& bucketsModified() const { return _modifiedHandler._modified; }
    const BucketId::List& calcAsked() const { return _calc->asked(); }
    size_t numPending() {
    ...
    template <typename FunctionType> void masterExecute(FunctionType&& function) {
        _master.execute(vespalib::makeLambdaTask(std::forward<FunctionType>(function)));
        _master.sync();
    }
};

ControllerFixtureBase::ControllerFixtureBase(const BlockableMaintenanceJobConfig& blockableConfig,
                                             bool                                 storeMoveDoneContexts)
    : _builder(),
      ...
      _metrics("test", 1),
      _bmj(BucketMoveJob::create(_calc, RetainGuard(_refCount), _moveHandler, _modifiedHandler, _master,
                                 _bucketExecutor, _ready._subDb, _notReady._subDb, _bucketCreateNotifier,
                                 _clusterStateHandler, _bucketHandler, _resource_usage_notifier, blockableConfig,
                                 _doc_type_name.getName(), makeBucketSpace())),
      _runner(*_bmj) {
}
...
TEST_F(ControllerFixture, require_that_bucket_is_moved_even_with_handler_error) {
    ...
    EXPECT_TRUE(removeFromSubDB(doc.getPrevSubDbId(),
                                doc.getPrevLid())); // Explicit remove as movehandler only observes move attempt.
...
TEST_F(ControllerFixture, require_that_current_bucket_moving_is_cancelled_when_we_change_calculator) {
    ...
    masterExecute([this]() { _bmj->scanAndMove(2, 3); });
    EXPECT_TRUE(_bmj->done());
    sync();
    EXPECT_EQ(3u, docsMoved().size());
    EXPECT_EQ(4u, calcAsked().size());
...
TEST_F(ResourceLimitControllerFixture_1_2,
       require_that_bucket_move_uses_resource_limit_factor_for_disk_resource_limit) {
    testJobNotStopping(ResourceUsageState(ResourceUsageWithLimit(0.8, 0.7), ResourceUsageWithLimit()));
}

TEST_F(ResourceLimitControllerFixture_1_2,
       require_that_bucket_move_uses_resource_limit_factor_for_memory_resource_limit) {
...
TEST_F(MaxOutstandingMoveOpsFixture_1,
       require_that_bucket_move_job_is_blocked_if_it_has_too_many_outstanding_move_operations_max_1) {
    assertRunToBlocked();
    sync();
    assertDocsMoved(1, 1);
    assertRunToBlocked();
...
TEST_F(MaxOutstandingMoveOpsFixture_2,
