...
MyScanIterator::MyScanIterator(const MyHandler& handler, const LidVector& lids)
    : _handler(handler), _lids(lids), _itr(_lids.begin()), _validItr(true) {
}
MyScanIterator::~MyScanIterator() = default;

bool MyScanIterator::valid() const {
    return _validItr;
}
...
document::BucketId MyHandler::createBucketId(uint32_t lid) const {
    return _bucketIdEqualLid ? document::BucketId(lid) : BUCKET_ID_1;
}

void MyHandler::clearMoveDoneContexts() {
    _moveDoneContexts.clear();
}

void MyHandler::run_remove_ops(bool remove_batch) {
...
void MyHandler::stop_remove_ops(bool remove_batch) const {
    if (remove_batch) {
...
std::string MyHandler::getName() const {
    return "myhandler";
}
...
search::DocumentMetadata MyHandler::getMetadata(uint32_t lid) const {
    if (lid < _docs.size()) {
...
MoveOperation::UP MyHandler::createMoveOperation(const search::DocumentMetadata& document, uint32_t moveToLid) const {
    ...
    auto op = std::make_unique<MoveOperation>(entry.first.bucketId, storage::spi::Timestamp(entry.first.timestamp),
                                              entry.second, DbDocumentId(document.lid), 0);
    op->setTargetLid(moveToLid);
    return op;
}

void MyHandler::handleMove(const MoveOperation& op, IDestructorCallback::SP moveDoneCtx) {
    ++_handleMoveCnt;
...
void MyHandler::handleCompactLidSpace(const CompactLidSpaceOperation& op, std::shared_ptr<IDestructorCallback>) {
...
MyHandler::MyHandler(bool storeMoveDoneContexts, bool bucketIdEqualLid)
    ...
    for (uint32_t i(0); i < 10; i++) {
        ...
        _docs.emplace_back(DocumentMetadata(i, TIMESTAMP_1, createBucketId(i), doc->getId().getGlobalId()),
                           std::move(doc));
    }
}
...
void MyStorer::appendOperation(const FeedOperation& op, DoneCallback) {
...
IOperationStorer::CommitResult MyStorer::startCommit(DoneCallback) {
    return CommitResult();
}

MyDocumentStore::MyDocumentStore() : _readDoc(), _readLid(0) {
}

MyDocumentStore::~MyDocumentStore() = default;

document::Document::UP MyDocumentStore::read(search::DocumentIdT lid, const document::DocumentTypeRepo&) const {
    _readLid = lid;
    return Document::UP(_readDoc->clone());
}

MyDocumentRetriever::MyDocumentRetriever(std::shared_ptr<const DocumentTypeRepo> repo_in,
                                         const MyDocumentStore&                  store_in,
                                         const DocTypeName&                      doc_type_name_in) noexcept
    : repo(std::move(repo_in)), store(store_in), doc_type_name(doc_type_name_in) {
}

MyDocumentRetriever::~MyDocumentRetriever() = default;

const document::DocumentTypeRepo& MyDocumentRetriever::getDocumentTypeRepo() const {
    return *repo;
}

const DocTypeName& MyDocumentRetriever::get_doc_type_name() const noexcept {
    return doc_type_name;
}

bool MyDocumentRetriever::can_populate_document_metadata_docid() const noexcept {
    return false;
}

void MyDocumentRetriever::getBucketMetadata(const storage::spi::Bucket&, DocumentMetadata::Vector&, bool) const {
    abort();
}

DocumentMetadata MyDocumentRetriever::getDocumentMetadata(const DocumentId&) const {
    abort();
}

Document::UP MyDocumentRetriever::getFullDocument(DocumentIdT lid) const {
...
CachedSelect::SP MyDocumentRetriever::parseSelect(const std::string&) const {
    abort();
}
...
MySubDb::MySubDb(std::shared_ptr<bucketdb::BucketDBOwner> bucket_db, const MyDocumentStore& store,
    ...
    : sub_db(std::make_unique<DummyDocumentSubDb>(std::move(bucket_db), SUBDB_ID)),
      maintenance_sub_db(sub_db->getName(), sub_db->getSubDbId(), sub_db->getDocumentMetaStoreContext().getSP(),
                         ...
                         std::make_shared<MyFeedView>(repo), &_pendingLidsForCommit) {
}
