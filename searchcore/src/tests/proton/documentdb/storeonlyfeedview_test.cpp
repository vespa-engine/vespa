...
public:
    MySummaryAdapter(std::atomic<int>& removeCount, std::atomic<int>& putCount,
                     std::atomic<int>& heartbeatCount) noexcept
        : _rmCount(removeCount), _putCount(putCount), _heartbeatCount(heartbeatCount) {}
...
struct MyMinimalFeedViewBase {
    ...
    MyMinimalFeedViewBase() : gidToLidChangeHandler(std::make_shared<DummyGidToLidChangeHandler>()) {}
};
...
struct MyMinimalFeedView : public MyMinimalFeedViewBase, public StoreOnlyFeedView {
    ...
    MyMinimalFeedView(const ISummaryAdapter::SP& summaryAdapter, const DocumentMetaStore::SP& metaStore,
                      searchcorespi::index::IThreadingService& writeService, const PersistentParams& params,
        ...
        : MyMinimalFeedViewBase(),
          StoreOnlyFeedView(StoreOnlyFeedView::Context(summaryAdapter, {},
                                                       ...
                                                       myGetDocumentTypeRepo(), std::move(pendingLidsForCommit),
                                                       *gidToLidChangeHandler, writeService),
                            params),
...
struct MoveOperationFeedView : public MyMinimalFeedView {
    ...
    std::mutex                           _mutex;
    MoveOperationFeedView(const ISummaryAdapter::SP& summaryAdapter, const DocumentMetaStore::SP& metaStore,
                          searchcorespi::index::IThreadingService& writeService, const PersistentParams& params,
                          std::shared_ptr<PendingLidTrackerBase> pendingLidsForCommit,
                          std::atomic<int>&                      outstandingMoveOps_)
        : MyMinimalFeedView(summaryAdapter, metaStore, writeService, params, std::move(pendingLidsForCommit),
                            outstandingMoveOps_),
    ...
    void putAttributes(SerialNum, search::DocumentIdT, const document::Document&,
                       const OnPutDoneType& onWriteDone) override {
        ++putAttributesCount;
...
struct MoveOperationCallback : public IDestructorCallback {
    std::atomic<int>& outstandingMoveOps;
    explicit MoveOperationCallback(std::atomic<int>& outstandingMoveOps_) noexcept
        : outstandingMoveOps(outstandingMoveOps_) {
        ++outstandingMoveOps;
    }
...
template <typename FeedViewType> struct FixtureBase {
    ...
    explicit FixtureBase(SubDbType subDbType = SubDbType::READY)
        : removeCount(0),
          ...
          metaStore(std::make_shared<DocumentMetaStore>(std::make_shared<bucketdb::BucketDBOwner>(),
                                                        DocumentMetaStore::getFixedName(), search::GrowStrategy(),
                                                        subDbType)),
          _service(1),
        ...
        feedview = std::make_unique<FeedViewType>(adapter, metaStore, _service.write(), params, pendingLidsForCommit,
                                                  outstandingMoveOps);
    }

    ~FixtureBase() { this->force_commit(); }
    ...
    void addSingleDocToMetaStore(uint32_t expected_lid) {
        ...
        EXPECT_EQ(
            expected_lid,
            metaStore->put(id, id.getGlobalId().convertToBucketId(), Timestamp(10), docSize, inspect.getLid(), 0u)
                .getLid());
    }
    ...
    void force_commit() {
        ...
        runInMaster([this, &gate]() {
            feedview->forceCommit(search::CommitParam(serial_num, search::CommitParam::UpdateStats::SKIP),
...
struct MoveFixture : public FixtureBase<MoveOperationFeedView> {
    ...
    IDestructorCallback::SP beginMoveOp() { return std::make_shared<MoveOperationCallback>(outstandingMoveOps); }
    ...
    void handleMove(const MoveOperation& op, long expected) {
        ...
        runInMasterAndSync([&, ctx]() { feedview->handleMove(op, std::move(ctx)); });
...
MoveOperation::UP makeMoveOp(Document::SP doc, DbDocumentId sourceDbdId, uint32_t targetSubDbId) {
...
MoveOperation::UP makeMoveOp(DbDocumentId sourceDbdId, uint32_t targetSubDbId) {
    return makeMoveOp(std::make_shared<Document>(), sourceDbdId, targetSubDbId);
}

TEST(StoreOnlyFeedViewTest,
     require_that_handleMove_adds_document_to_target_and_removes_it_from_source_and_propagates_destructor_callback) {
    MoveFixture f;
    uint32_t    lid = 0;
...
TEST(StoreOnlyFeedViewTest,
     require_that_handleMove_handles_move_within_same_subdb_and_propagates_destructor_callback) {
    MoveFixture f;
    auto        doc = std::make_shared<Document>();
    DocumentId  doc1id("id:test:foo:g=foo:1");
    uint32_t    docSize = 1;
    f.runInMasterAndSync(
        [&]() { f.metaStore->put(doc1id, doc1id.getGlobalId().convertToBucketId(), Timestamp(9), docSize, 1, 0u); });
    f.runInMasterAndSync([&]() {
        f.metaStore->put(doc->getId(), doc->getId().getGlobalId().convertToBucketId(), Timestamp(10), docSize, 2, 0u);
    });
