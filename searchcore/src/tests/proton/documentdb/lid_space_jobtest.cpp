...
struct MyDirectJobRunner : public IMaintenanceJobRunner {
    ...
    explicit MyDirectJobRunner(IMaintenanceJob& job) : _job(job) { _job.registerRunner(this); }
    void run() override { _job.run(); }
};

struct MyCountJobRunner : public IMaintenanceJobRunner {
    uint32_t runCnt;
    explicit MyCountJobRunner(IMaintenanceJob& job) : runCnt(0) { job.registerRunner(this); }
...
JobTestBase::JobTestBase()
    : _refCount(),
      ...
      _job() {
    init(ALLOWED_LID_BLOAT, ALLOWED_LID_BLOAT_FACTOR, RESOURCE_LIMIT_FACTOR, JOB_DELAY, false,
         MAX_OUTSTANDING_MOVE_OPS);
}

void JobTestBase::init(uint32_t allowedLidBloat, double allowedLidBloatFactor, double resourceLimitFactor,
                       vespalib::duration interval, bool node_retired_or_maintenance,
    ...
    _job = lidspace::CompactionJob::create(compactCfg, RetainGuard(_refCount), _handler, _storer, *_master,
                                           *_bucketExecutor, _resource_usage_notifier, blockableCfg,
                                           _clusterStateHandler, node_retired_or_maintenance,
                                           document::BucketSpace::placeHolder(), _maintenance_job_token_source);
}

void JobTestBase::sync() const {
    if (_bucketExecutor) {
...
JobTestBase& JobTestBase::addStats(uint32_t docIdLimit, const LidVector& usedLids,
                                   const LidPairVector& usedFreePairs) {
    return addMultiStats(docIdLimit, {usedLids}, usedFreePairs);
}

JobTestBase& JobTestBase::addMultiStats(uint32_t docIdLimit, const std::vector<LidVector>& usedLidsVector,
                                        const LidPairVector& usedFreePairs) {
...
JobTestBase& JobTestBase::addStats(uint32_t docIdLimit, uint32_t numDocs, uint32_t lowestFreeLid,
                                   uint32_t highestUsedLid) {
    _handler->_stats.emplace_back(docIdLimit, numDocs, lowestFreeLid, highestUsedLid);
    return *this;
}

bool JobTestBase::run() const {
    return _job->run();
}

JobTestBase& JobTestBase::endScan() {
    EXPECT_FALSE(run());
    return *this;
}

JobTestBase& JobTestBase::compact() {
    EXPECT_FALSE(run());
    EXPECT_TRUE(run());
    return *this;
}

void JobTestBase::notifyNodeRetired(bool nodeRetired) {
...
void JobTestBase::assertJobContext(uint32_t moveToLid, uint32_t moveFromLid, uint32_t handleMoveCnt,
...
void JobTestBase::assertNoWorkDone() const {
    assertJobContext(0, 0, 0, 0, 0);
}

JobTestBase& JobTestBase::setupOneDocumentToCompact() {
...
void JobTestBase::assertOneDocumentCompacted() {
    assertJobContext(2, 9, 1, 0, 0);
    endScan().compact();
    assertJobContext(2, 9, 1, 7, 1);
}

JobTestBase& JobTestBase::setupThreeDocumentsToCompact() {
...
JobTest::JobTest() : JobTestBase(), _jobRunner(std::make_unique<MyDirectJobRunner>(*_job)) {
}

JobTest::~JobTest() = default;

void JobTest::init(uint32_t allowedLidBloat, double allowedLidBloatFactor, double resourceLimitFactor,
                   vespalib::duration interval, bool node_retired_or_maintenance, uint32_t maxOutstandingMoveOps) {
    JobTestBase::init(allowedLidBloat, allowedLidBloatFactor, resourceLimitFactor, interval,
                      node_retired_or_maintenance, maxOutstandingMoveOps);
    _jobRunner = std::make_unique<MyDirectJobRunner>(*_job);
}

void JobTest::init_with_interval(vespalib::duration interval) {
    init(ALLOWED_LID_BLOAT, ALLOWED_LID_BLOAT_FACTOR, RESOURCE_LIMIT_FACTOR, interval);
}

void JobTest::init_with_node_retired(bool retired) {
...
void JobDisabledByRemoveOpsTest::job_is_disabled_while_remove_ops_are_ongoing(bool remove_batch) {
...
void JobDisabledByRemoveOpsTest::job_becomes_disabled_if_remove_ops_starts(bool remove_batch) {
...
void JobDisabledByRemoveOpsTest::job_is_re_enabled_when_remove_ops_are_no_longer_ongoing(bool remove_batch) {
...
MaxOutstandingJobTest::MaxOutstandingJobTest() : JobTest(), runner() {
}

MaxOutstandingJobTest::~MaxOutstandingJobTest() = default;

void MaxOutstandingJobTest::init(uint32_t maxOutstandingMoveOps) {
    JobTest::init(ALLOWED_LID_BLOAT, ALLOWED_LID_BLOAT_FACTOR, RESOURCE_LIMIT_FACTOR, JOB_DELAY, false,
                  maxOutstandingMoveOps);
    runner = std::make_unique<MyCountJobRunner>(*_job);
}

void MaxOutstandingJobTest::assertRunToBlocked() {
...
void MaxOutstandingJobTest::assertRunToNotBlocked() {
    EXPECT_FALSE(run());
    EXPECT_FALSE(_job->isBlocked());
}

void MaxOutstandingJobTest::unblockJob(uint32_t expRunnerCnt) {
