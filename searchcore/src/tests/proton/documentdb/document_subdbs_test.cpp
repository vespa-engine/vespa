...
struct ConfigDir1 {
    static std::string dir() { return TEST_PATH("document_subdbs/cfg1"); }
};
struct ConfigDir2 {
    static std::string dir() { return TEST_PATH("document_subdbs/cfg2"); }
};
struct ConfigDir3 {
    static std::string dir() { return TEST_PATH("document_subdbs/cfg3"); }
};
struct ConfigDir4 {
    static std::string dir() { return TEST_PATH("document_subdbs/cfg4"); }
};
...
struct MyStoreOnlyConfig {
    ...
    explicit MyStoreOnlyConfig(SubDbType subDbType)
        : _cfg(DocTypeName(DOCTYPE_NAME), SUB_NAME, BASE_DIR, 0, subDbType) {}
};
...
struct MyStoreOnlyContext {
    ...
    MyStoreOnlyContext(IThreadingService& writeService, std::shared_ptr<bucketdb::BucketDBOwner> bucketDB,
                       IBucketDBHandlerInitializer& bucketDBHandlerInitializer);
    ~MyStoreOnlyContext();
    const MySubDBOwner& getOwner() const { return _owner; }
};
...
template <bool FastAccessAttributesOnly> struct MyFastAccessConfig {
...
struct MyFastAccessContext {
    ...
    MyFastAccessContext(IThreadingService& writeService, std::shared_ptr<bucketdb::BucketDBOwner> bucketDB,
                        IBucketDBHandlerInitializer& bucketDBHandlerInitializer);
    ~MyFastAccessContext();
    const MyMetricsWireService& getWireService() const { return _wireService; }
    const MySubDBOwner& getOwner() const { return _storeOnlyCtx.getOwner(); }
};
...
struct MySearchableConfig {
    FastAccessConfig _cfg;
    explicit MySearchableConfig(SubDbType subDbType) : _cfg(MyFastAccessConfig<false>(subDbType)._cfg) {}
};
...
struct MySearchableContext {
    ...
    MySearchableContext(IThreadingService& writeService, std::shared_ptr<bucketdb::BucketDBOwner> bucketDB,
                        IBucketDBHandlerInitializer& bucketDBHandlerInitializer);
    ~MySearchableContext();
    const MyMetricsWireService& getWireService() const { return _fastUpdCtx.getWireService(); }
    const MySubDBOwner& getOwner() const { return _fastUpdCtx.getOwner(); }
};
...
DocBuilder::AddFieldsType get_add_fields(bool has_attr2) {
...
Schema make_all_attr_schema(bool has_attr2) {
...
MyConfigSnapshot::MyConfigSnapshot(FNET_Transport& transport, Schema schema, const std::string& cfgDir)
    ...
    _bootstrap = std::make_shared<BootstrapConfig>(
        1, documenttypesConfig, _builder.get_repo_sp(), std::make_shared<ProtonConfig>(),
        std::make_shared<FiledistributorrpcConfig>(), std::make_shared<BucketspacesConfig>(), tuneFileDocumentDB,
        HwInfo(HwInfo::Disk(128_Gi, false, false), HwInfo::Memory(16_Gi), HwInfo::Cpu(8)));
...
template <typename Traits> struct FixtureBase {
    ...
    template <typename FunctionType> void runInMasterAndSync(FunctionType func) {
        proton::test::runInMasterAndSync(_service.write(), func);
    }
    template <typename FunctionType> void runInMaster(FunctionType func) {
    ...
    void basicReconfig(SerialNum serialNum) {
        runInMasterAndSync(
            [&]() { performReconfig(serialNum, make_all_attr_schema(two_attr_schema), ConfigDir2::dir()); });
    }
    ...
    void performReconfig(SerialNum serialNum, Schema reconfigSchema, const std::string& reconfigConfigDir) {
        auto newCfg =
            std::make_unique<MyConfigSnapshot>(_service.transport(), std::move(reconfigSchema), reconfigConfigDir);
        ...
        auto tasks = _subDb.applyConfig(*newCfg->_cfg, *_snapshot->_cfg, serialNum, reconfig_params, resolver,
                                        *prepared_reconfig);
    ...
    const MyMetricsWireService& getWireService() const { return _ctx.getWireService(); }
    const MySubDBOwner& getOwner() const { return _ctx.getOwner(); }
};

template <bool has_attr2_in, typename ConfigDirT, uint32_t ConfigSerial = CFG_SERIAL,
          SubDbType subDbType_in = SubDbType::READY>
struct BaseTraitsT {
...
template <typename ConfigDirT> struct FastAccessOnlyTraitsBase : public BaseTraitsT<two_attr_schema, ConfigDirT> {
...
void assertAttributes1(const AttributeGuardList& attributes) {
    EXPECT_EQ(1u, attributes.size());
    EXPECT_EQ("attr1", attributes[0]->getName());
}

void assertAttributes1(const std::vector<search::AttributeVector*>& attributes) {
    EXPECT_EQ(1u, attributes.size());
    EXPECT_EQ("attr1", attributes[0]->getName());
}

void assertAttributes2(const AttributeGuardList& attributes) {
    EXPECT_EQ(2u, attributes.size());
    EXPECT_EQ("attr1", attributes[0]->getName());
    EXPECT_EQ("attr2", attributes[1]->getName());
}

void assertAttributes2(const std::vector<search::AttributeVector*>& attributes) {
    EXPECT_EQ(2u, attributes.size());
    EXPECT_EQ("attr1", attributes[0]->getName());
    EXPECT_EQ("attr2", attributes[1]->getName());
}

void assertCacheCapacity(const StoreOnlyDocSubDB& db, size_t expected_cache_capacity) {
...
void assertStoreOnly(StoreOnlyDocSubDB& db) {
...
template <typename Fixture> void requireThatAttributeManagerIsInstantiated(Fixture& f) {
...
template <typename Fixture> void requireThatAttributesAreAccessibleViaFeedView(Fixture& f) {
...
template <typename Fixture> void requireThatAttributeManagerCanBeReconfigured(Fixture& f) {
    f.basicReconfig(10);
    std::vector<AttributeGuard> attributes;
    f.getAttributeManager()->getAttributeList(attributes);
...
TEST(DocumentSubDBsTest, require_that_attribute_compaction_config_reflect_retirement_or_maintenance) {
    ...
    EXPECT_EQ(default_cfg,
              dynamic_cast<const proton::DocumentMetaStore&>(f._subDb.getDocumentMetaStoreContext().get())
                  .getConfig()
...
template <typename Fixture> void requireThatReconfiguredAttributesAreAccessibleViaFeedView(Fixture& f) {
    f.basicReconfig(10);
...
template <typename Fixture> void requireThatAttributeMetricsAreRegistered(Fixture& f) {
...
template <typename Fixture> void requireThatAttributeMetricsCanBeReconfigured(Fixture& f) {
    f.basicReconfig(10);
...
template <typename Fixture> IFlushTarget::List getFlushTargets(Fixture& f) {
    IFlushTarget::List targets = (static_cast<IDocumentSubDB&>(f._subDb)).getFlushTargets();
    std::sort(targets.begin(), targets.end(), [](const IFlushTarget::SP& lhs, const IFlushTarget::SP& rhs) {
...
bool assertTarget(const std::string& name, FType type, FComponent component, const IFlushTarget& target) {
    bool failed = false;
...
template <typename FixtureType> struct DocumentHandler {
    FixtureType& _f;
    DocBuilder   _builder;
    ...
    MoveOperation createMove(Document::UP doc, Timestamp timestamp, DbDocumentId sourceDbdId, uint32_t targetSubDbId,
                             SerialNum serialNum) {
        proton::test::Document testDoc(Document::SP(doc.release()), 0, timestamp);
    ...
    void putDoc(PutOperation& op) {
        ...
        _f.runInMaster([&]() {
            ...
            feedView->forceCommit(CommitParam(op.getSerialNum(), CommitParam::UpdateStats::SKIP),
...
void assertAttribute(const AttributeGuard& attr, const std::string& name, uint32_t numDocs, int64_t doc1Value,
                     int64_t doc2Value, SerialNum createSerialNum, SerialNum lastSerialNum, std::string_view label) {
    SCOPED_TRACE(label);
...
void assertAttribute1(const AttributeGuard& attr, SerialNum createSerialNum, SerialNum lastSerialNum,
                      std::string_view label) {
    assertAttribute(attr, "attr1", 3, 22, 44, createSerialNum, lastSerialNum, label);
}

void assertAttribute2(const AttributeGuard& attr, SerialNum createSerialNum, SerialNum lastSerialNum,
                      std::string_view label) {
...
bool assertOperation(DocumentOperation& op, uint32_t expPrevSubDbId, uint32_t expPrevLid, uint32_t expSubDbId,
                     uint32_t expLid) {
    bool failed = false;
...
TEST(DocumentSubDBsTest, require_that_lid_allocation_uses_lowest_free_lid) {
    ...
    EXPECT_TRUE(assertOperation(rmOp, 0, 1, 0, 0));
    doc = handler.createEmptyDoc(2);
    moveOp = handler.createMove(std::move(doc), Timestamp(20), DbDocumentId(0, 2), 0, 60);
...
template <typename FixtureType> struct ExplorerFixture : public FixtureType {
    DocumentSubDBExplorer _explorer;
    ExplorerFixture() : FixtureType(), _explorer(this->_subDb) {}
};
...
void assertExplorer(const StringVector& extraNames, const vespalib::StateExplorer& explorer) {
