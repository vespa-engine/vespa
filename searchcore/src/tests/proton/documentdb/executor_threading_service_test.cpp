...
SequencedTaskExecutor* to_concrete_type(ISequencedTaskExecutor& exec) {
...
public:
    ...
    ExecutorThreadingServiceTest()
        : _transport(1),
          ...
          service(std::make_unique<ExecutorThreadingService>(_transport.shared(), _transport.transport(),
                                                             *field_writer_executor, nullptr,
                                                             ThreadingServiceConfig::make())) {}
    SequencedTaskExecutor* field_writer() { return to_concrete_type(*field_writer_executor); }
};

void assert_executor(SequencedTaskExecutor* exec, uint32_t exp_executors, uint32_t exp_task_limit) {
