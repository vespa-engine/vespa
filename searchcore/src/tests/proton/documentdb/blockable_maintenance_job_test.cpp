...
JobProxy::JobProxy(std::shared_ptr<IMaintenanceJob> job)
    : IMaintenanceJob(job->getName(), job->getDelay(), job->getInterval()), _job(std::move(job)) {
}
...
public:
    ...
    void wait_ready() {
        ...
        _cv.wait(guard, [this]() { return _ready; });
    }
    void wait_done() {
        std::unique_lock guard(_mutex);
        _cv.wait(guard, [this]() { return _done; });
    }
    ...
    void add(uint32_t id) {
        _work_log.push_back(id);
        if (_work_log.size() >= work_log_size) {
            set_done();
        }
    }
...
JobResult::JobResult() noexcept : _mutex(), _cv(), _ready(false), _done(false), _work_log() {
}
...
class MyJob : public BlockableMaintenanceJob, public std::enable_shared_from_this<MyJob> {
    uint32_t                   _id;
    std::shared_ptr<JobResult> _job_result;
    uint32_t                   _remaining;

public:
    MyJob(const std::string& name, std::shared_ptr<MaintenanceJobTokenSource> token_source, uint32_t id,
          std::shared_ptr<JobResult> job_result);
    ~MyJob() override;
    bool run() override;
};

MyJob::MyJob(const std::string& name, std::shared_ptr<MaintenanceJobTokenSource> token_source, uint32_t id,
             std::shared_ptr<JobResult> job_result)
...
struct MyJobRunner : public IMaintenanceJobRunner {
    bool                             _pending;
    std::shared_ptr<IMaintenanceJob> _job;
    explicit MyJobRunner(std::shared_ptr<IMaintenanceJob> job) : _pending(false), _job(std::move(job)) {
...
void BlockableMaintenanceJobTest::TearDown() {
    stop_jobs();
}

void BlockableMaintenanceJobTest::add_job(std::shared_ptr<IMaintenanceJob> job) {
    _runners.emplace_back(
        std::make_unique<MaintenanceJobRunner>(_executor, std::make_unique<JobProxy>(std::move(job))));
    _runners.back()->run();
}

void BlockableMaintenanceJobTest::stop_jobs() {
...
void BlockableMaintenanceJobTest::start_jobs(bool with_token) {
