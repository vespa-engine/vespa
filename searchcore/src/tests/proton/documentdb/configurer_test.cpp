...
std::shared_ptr<const DocumentTypeRepo> createRepo() {
...
struct EmptyConstantValueFactory : public vespalib::eval::ConstantValueFactory {
    vespalib::eval::ConstantValue::UP create(const std::string&, const std::string&) const override { return {}; }
};
...
struct Fixture {
    ...
    void reconfigure(const DocumentDBConfig& new_config_snapshot, const DocumentDBConfig& old_config_snapshot,
                     const ReconfigParams& reconfig_params, IDocumentDBReferenceResolver& resolver,
...
void Fixture::initViewSet(ViewSet& views) {
    ...
    auto attrMgr = make_shared<AttributeManager>(
        BASE_DIR, "test.subdb", TuneFileAttributes(), views._fileHeaderContext,
        std::make_shared<search::attribute::Interlock>(), views._service.write().field_writer(),
        views._service.write().shared(), views._hwInfo);
    auto summaryMgr =
        make_shared<SummaryManager>(_summaryExecutor, search::LogDocumentStore::Config(), search::GrowStrategy(),
                                    BASE_DIR, TuneFileSummary(), views._fileHeaderContext, views._noTlSyncer,
                                    search::IBucketizer::SP(), std::shared_ptr<const search::IDocumentIdProvider>());
    ...
    views.feedView.set(make_shared<SearchableFeedView>(
        ...
        SearchableFeedView::PersistentParams(views.serialNum, views.serialNum, views._docTypeName, 0u,
                                             SubDbType::READY),
        FastAccessFeedView::Context(attrWriter, views._docIdLimit), SearchableFeedView::Context(indexWriter)));
}

void Fixture::reconfigure(const DocumentDBConfig& new_config_snapshot, const DocumentDBConfig& old_config_snapshot,
                          const ReconfigParams& reconfig_params, IDocumentDBReferenceResolver& resolver,
                          SerialNum serial_num) const {
    ...
    auto prepared_reconfig = _configurer->prepare_reconfig(new_config_snapshot, attr_spec_factory, reconfig_params,
                                                           docid_limit, serial_num);
...
IReprocessingInitializer::UP Fixture::reconfigure(const DocumentDBConfig&       new_config_snapshot,
                                                  const DocumentDBConfig&       old_config_snapshot,
                                                  const ReconfigParams&         reconfig_params,
                                                  IDocumentDBReferenceResolver& resolver, uint32_t docid_limit,
                                                  SerialNum serial_num) const {
    ...
    auto prepared_reconfig = _configurer->prepare_reconfig(new_config_snapshot, attr_spec_factory, reconfig_params,
                                                           docid_limit, serial_num);
    prepared_reconfig->complete(docid_limit, serial_num);
    return _configurer->reconfigure(new_config_snapshot, old_config_snapshot, reconfig_params, resolver,
                                    *prepared_reconfig, serial_num);
}
...
void MyFastAccessFeedView::init() {
    ...
    StoreOnlyFeedView::Context              storeOnlyCtx(summaryAdapter, schema, _dmsc, repo, _pendingLidsForCommit,
...
struct FastAccessFixture {
    ...
    IReprocessingInitializer::UP reconfigure(const DocumentDBConfig& new_config_snapshot,
                                             const DocumentDBConfig& old_config_snapshot, uint32_t docid_limit,
                                             SerialNum serial_num);
};

FastAccessFixture::FastAccessFixture() : _service(1), _view(_service.write()), _configurer(_view._feedView, "test") {
    std::filesystem::remove_all(std::filesystem::path(BASE_DIR));
    std::filesystem::create_directory(std::filesystem::path(BASE_DIR));
}
...
IReprocessingInitializer::UP FastAccessFixture::reconfigure(const DocumentDBConfig& new_config_snapshot,
                                                            const DocumentDBConfig& old_config_snapshot,
...
DocumentDBConfig::SP createConfig() {
    return proton::test::DocumentDBConfigBuilder(0, make_shared<Schema>(), "client", DOC_TYPE)
        .repo(createRepo())
        .build();
}

DocumentDBConfig::SP createConfig(std::shared_ptr<const Schema> schema) {
    return proton::test::DocumentDBConfigBuilder(0, std::move(schema), "client", DOC_TYPE).repo(createRepo()).build();
}
...
struct SearchViewComparer {
    ...
    ~SearchViewComparer();
    void expect_equal() { EXPECT_EQ(_old.get(), _new.get()); }
    void expect_not_equal() { EXPECT_NE(_old.get(), _new.get()); }
    void expect_equal_summary_setup() { EXPECT_EQ(_old->getSummarySetup().get(), _new->getSummarySetup().get()); }
    void expect_not_equal_summary_setup() { EXPECT_NE(_old->getSummarySetup().get(), _new->getSummarySetup().get()); }
    void expect_equal_match_view() { EXPECT_EQ(_old->getMatchView().get(), _new->getMatchView().get()); }
    void expect_not_equal_match_view() { EXPECT_NE(_old->getMatchView().get(), _new->getMatchView().get()); }
    void expect_equal_matchers() { EXPECT_EQ(_old->getMatchers().get(), _new->getMatchers().get()); }
    void expect_not_equal_matchers() { EXPECT_NE(_old->getMatchers().get(), _new->getMatchers().get()); }
...
SearchViewComparer::SearchViewComparer(SearchView::SP old, SearchView::SP new_)
    : _old(std::move(old)), _new(std::move(new_)) {
}
...
struct FeedViewComparer {
    ...
    void expect_equal() { EXPECT_EQ(_old.get(), _new.get()); }
    void expect_not_equal() { EXPECT_NE(_old.get(), _new.get()); }
    void expect_equal_index_adapter() { EXPECT_EQ(_old->getIndexWriter().get(), _new->getIndexWriter().get()); }
    ...
    void expect_not_equal_schema() { EXPECT_NE(_old->getSchema().get(), _new->getSchema().get()); }
};
...
const AttributeManager* asAttributeManager(const proton::IAttributeManager::SP& attrMgr) {
...
TEST(DocSubDBConfigurerTest, require_that_we_can_reconfigure_attribute_manager) {
    ...
    f.reconfigure(*createConfig(), *createConfig(), params, f._resolver, 1, reconfig_serial_num);
    ...
    EXPECT_TRUE(asAttributeManager(f._views.getViewPtrs().fv.get()->getAttributeWriter()->getAttributeManager())
                    ->getImportedAttributes() != nullptr);
}

namespace {

AttributeWriter::SP getAttributeWriter(Fixture& f) {
    return f._views.feedView.get()->getAttributeWriter();
}

void checkAttributeWriterChangeOnRepoChange(Fixture& f, bool docTypeRepoChanged) {
...
void assertMaintenanceControllerShouldNotChange(DocumentDBConfig::ComparisonResult result) {
    ReconfigParams params(result);
    EXPECT_FALSE(params.configHasChanged());
    EXPECT_FALSE(params.shouldMaintenanceControllerChange());
}

void assertMaintenanceControllerShouldChange(DocumentDBConfig::ComparisonResult result, std::string_view label) {
    SCOPED_TRACE(label);
    ReconfigParams params(result);
...
void assertSubDbsShouldNotChange(DocumentDBConfig::ComparisonResult result) {
    ReconfigParams params(result);
    EXPECT_FALSE(params.configHasChanged());
    EXPECT_FALSE(params.shouldSubDbsChange());
}

void assertSubDbsShouldChange(DocumentDBConfig::ComparisonResult result, std::string_view label) {
    SCOPED_TRACE(label);
    ReconfigParams params(result);
    EXPECT_TRUE(params.configHasChanged());
...
void assertSummaryManagerShouldNotChange(DocumentDBConfig::ComparisonResult result) {
    ReconfigParams params(result);
    EXPECT_FALSE(params.configHasChanged());
    EXPECT_FALSE(params.shouldSummaryManagerChange());
}

void assertSummaryManagerShouldChange(DocumentDBConfig::ComparisonResult result, std::string_view label) {
    SCOPED_TRACE(label);
    ReconfigParams params(result);
    EXPECT_TRUE(params.configHasChanged());
