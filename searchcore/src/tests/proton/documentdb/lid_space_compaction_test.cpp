...
{
    setupOneDocumentToCompact();
    _resource_usage_notifier.notify({ResourceUsageWithLimit{0, 100}, ResourceUsageWithLimit{101, 100}});
...
TEST_F(JobTest, ending_resource_starvation_resumes_lid_space_compaction)
{
    ...
    EXPECT_TRUE(run()); // scan
    assertNoWorkDone();
    _resource_usage_notifier.notify({ResourceUsageWithLimit{0, 100}, ResourceUsageWithLimit{0, 100}});
    assertOneDocumentCompacted();
}
