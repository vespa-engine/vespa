...
struct JobTestBase : public ::testing::Test {
    ...
    void init(uint32_t allowedLidBloat, double allowedLidBloatFactor, double resourceLimitFactor,
              vespalib::duration interval, bool nodeRetired, uint32_t maxOutstandingMoveOps);
    JobTestBase& addStats(uint32_t docIdLimit, const LidVector& usedLids, const LidPairVector& usedFreePairs);
    JobTestBase& addMultiStats(uint32_t docIdLimit, const std::vector<LidVector>& usedLidsVector,
    ...
    void assertJobContext(uint32_t moveToLid, uint32_t moveFromLid, uint32_t handleMoveCnt, uint32_t wantedLidLimit,
...
struct JobTest : public JobTestBase {
    ...
    JobTest();
    ~JobTest() override;
    void init(uint32_t allowedLidBloat, double allowedLidBloatFactor,
              double resourceLimitFactor = RESOURCE_LIMIT_FACTOR, vespalib::duration interval = JOB_DELAY,
              bool nodeRetired = false, uint32_t maxOutstandingMoveOps = MAX_OUTSTANDING_MOVE_OPS);
