...
void cleanup_dirs(bool file_config) {
...
std::string config_subdir(SerialNum serialNum) {
...
FixtureBase::FixtureBase(bool file_config) : _cleanup(true), _file_config(file_config) {
...
Fixture::Fixture(bool file_config)
    ...
    auto b = std::make_shared<BootstrapConfig>(1, documenttypesConfig, repo, std::make_shared<ProtonConfig>(),
                                               std::make_shared<FiledistributorrpcConfig>(),
                                               std::make_shared<BucketspacesConfig>(), tuneFileDocumentDB, HwInfo());
    mgr.forwardConfig(b);
    ...
    _db = DocumentDB::create(".", mgr.getConfig(), tls_port_spec(), _queryLimiter, DocTypeName("typea"),
                             makeBucketSpace(), *b->getProtonConfigSP(), _myDBOwner, _shared_service, _tls, _dummy,
                             _fileHeaderContext, std::make_shared<search::attribute::Interlock>(),
                             make_config_store(), std::make_shared<vespalib::ThreadStackExecutor>(16), _hwInfo,
...
std::unique_ptr<ConfigStore> Fixture::make_config_store() {
    if (_file_config) {
...
const IFlushTarget* extractRealFlushTarget(const IFlushTarget* target) {
...
void DocumentDBTest::SetUpTestSuite() {
    cleanup_dirs(true);
    DummyFileHeaderContext::setCreator("documentdb_test");
}

void DocumentDBTest::TearDownTestSuite() {
    cleanup_dirs(true);
}
...
template <typename Target> size_t getNumTargets(const std::vector<IFlushTarget::SP>& targets) {
    size_t retval = 0;
...
TEST_F(DocumentDBTest, requireThatFlushTargetsAreNamedBySubDocumentDB) {
    ...
    for (const IFlushTarget::SP& target : f._db->getFlushTargets()) {
        ...
        EXPECT_TRUE((name.find("0.ready.") == 0) || (name.find("1.removed.") == 0) ||
