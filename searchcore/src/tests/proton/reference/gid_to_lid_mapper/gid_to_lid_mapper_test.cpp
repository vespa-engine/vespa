...
struct GidCollector : public search::IGidToLidMapperVisitor {
    GidMap& _map;
    GidCollector(GidMap& map) : IGidToLidMapperVisitor(), _map(map) {}
...
protected:
    ...
    void bumpTimeStamp() { _timestamp = Timestamp(_timestamp.getValue() + 1); }
    ...
    void assertPut(const DocumentId& docId, uint32_t expLid, Generation currentGeneration,
                   Generation firstUsedGeneration, Function&& func, std::string_view label) {
        SCOPED_TRACE(label);
...
TEST_F(GidToLidMapperTest, Test_that_gid_mapper_can_iterate_over_known_gids) {
    ...
    put(doc3);
    EXPECT_EQ((GidMap{{doc1.getGlobalId(), 4}, {doc2.getGlobalId(), 7}, {doc3.getGlobalId(), 1}}),
              collectGids(mapper));
    remove(4);
