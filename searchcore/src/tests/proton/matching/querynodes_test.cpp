...
template <class SearchType> class Create {
    bool                          _strict;
...
public:
    ...
    operator SearchIterator*() { return SearchType::create(std::move(_children), _strict).release(); }
};
...
public:
    ...
    static std::unique_ptr<Iterator> makeDummyIterator() { return std::make_unique<Iterator>(_sourceStoreDummy); }
};
...
public:
    ...
    operator SearchIterator*() {
        return SourceBlenderSearch::create(ISourceSelectorDummy::makeDummyIterator(), _children, _strict).release();
    }
};
...
struct SameElement {
    void addToBuilder(QB& b) { b.addSameElement(2, view, id, weight); }
};
struct Near {
    void addToBuilder(QB& b) { b.addNear(2, distance, 0, 0); }
};
struct ONear {
    void addToBuilder(QB& b) { b.addONear(2, distance, 0, 0); }
};
struct Or {
    void addToBuilder(QB& b) { b.addOr(2); }
};
struct And {
    void addToBuilder(QB& b) { b.addAnd(2); }
};
struct AndNot {
    void addToBuilder(QB& b) { b.addAndNot(2); }
};
struct Rank {
    void addToBuilder(QB& b) { b.addRank(2); }
};
struct Term {};

template <typename Tag> string IteratorStructureTest::getIteratorAsString() {
...
template <> string IteratorStructureTest::getIteratorAsString<Term>() {
    ProtonStringTerm node(term, view, id, weight);
    return getIteratorAsString(node);
}

template <typename T> SearchIterator* getLeaf(const string& fld, const string& tag) {
    return getTerm(term, fld, tag);
}

template <> SearchIterator* getLeaf<Phrase>(const string& fld, const string& tag) {
...
template <typename NearType> SearchIterator* getNearParent(SearchIterator* a, SearchIterator* b) {
    ...
    data.add(&tmd).add(&tmd).add(&tmd).add(&tmd).add(&tmd).add(&tmd).add(&tmd).add(&tmd);
    return new NearType(std::move(children), data, distance, mock_element_gap_inspector, true);
}

template <typename SearchType> SearchIterator* getSimpleParent(SearchIterator* a, SearchIterator* b) {
...
template <typename T> SearchIterator* getParent(SearchIterator* a, SearchIterator* b);

template <> SearchIterator* getParent<Near>(SearchIterator* a, SearchIterator* b) {
    return getNearParent<NearSearch>(a, b);
}

template <> SearchIterator* getParent<ONear>(SearchIterator* a, SearchIterator* b) {
    return getNearParent<ONearSearch>(a, b);
}

template <> SearchIterator* getParent<SameElement>(SearchIterator* a, SearchIterator* b) {
    static TermFieldMatchData                    tmd;
...
template <> SearchIterator* getParent<Or>(SearchIterator* a, SearchIterator* b) {
    return getSimpleParent<OrSearch>(a, b);
}

template <> SearchIterator* getParent<And>(SearchIterator* a, SearchIterator* b) {
    return getSimpleParent<AndSearch>(a, b);
}

template <> SearchIterator* getParent<AndNot>(SearchIterator* a, SearchIterator* b) {
    return getSimpleParent<AndNotSearch>(a, b);
}

template <> SearchIterator* getParent<Rank>(SearchIterator* a, SearchIterator* b) {
    return getSimpleParent<RankSearch>(a, b);
}

template <typename T> bool bothStrict() {
    return false;
}

template <> bool bothStrict<Or>() {
    return true;
}
...
template <typename T> void checkTwoFieldsTwoAttributesTwoIndexes() {
    ...
    SearchIterator::UP expected(
        MyOr()
            ...
            .add(Blender()
                     .add(SourceId(0),
                          MyOr().add(getLeaf<T>(field[0], source_tag[0])).add(getLeaf<T>(field[1], source_tag[0])))
                     .add(SourceId(1),
                          MyOr().add(getLeaf<T>(field[0], source_tag[1])).add(getLeaf<T>(field[1], source_tag[1])))));
...
template <typename T> void checkOneFieldOneAttributeTwoIndexes() {
    ...
    SearchIterator::UP expected(MyOr()
                                    .add(getLeaf<T>(attribute[0], attribute_tag))
                                    .add(Blender()
...
template <typename T> void checkOneFieldNoAttributesTwoIndexes() {
    ...
    SearchIterator::UP expected(Blender()
                                    .add(SourceId(0), getLeaf<T>(field[0], source_tag[0]))
                                    .add(SourceId(1), getLeaf<T>(field[0], source_tag[1])));
    EXPECT_EQ(expected->asString(), structure_test.getIteratorAsString<T>());
}
...
template <typename T> void checkOneFieldNoAttributesOneIndex() {
    ...
    SearchIterator::UP expected(Blender().add(SourceId(0), getLeaf<T>(field[0], source_tag[0])));
    EXPECT_EQ(expected->asString(), structure_test.getIteratorAsString<T>());
}
...
template <> void checkProperBlendingWithParent<SameElement>(const std::string& label) {
    SCOPED_TRACE("checkProperBlendingWithParent<" + label + ">()");
    using T = SameElement;
    IteratorStructureTest structure_test;
    structure_test.setFieldCount(1);
    ...
    SearchIterator::UP expected(getParent<T>(Blender()
