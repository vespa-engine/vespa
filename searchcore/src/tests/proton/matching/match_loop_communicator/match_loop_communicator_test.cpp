...
std::vector<Hit> hit_vec(std::vector<Hit> list) {
    return list;
}
...
Hits makeScores(size_t id) {
    switch (id) {
    case 0:
        return {{1, 5.4}, {2, 4.4}, {3, 3.4}, {4, 2.4}, {5, 1.4}};
    case 1:
        return {{11, 5.3}, {12, 4.3}, {13, 3.3}, {14, 2.3}, {15, 1.3}};
    case 2:
        return {{21, 5.2}, {22, 4.2}, {23, 3.2}, {24, 2.2}, {25, 1.2}};
    case 3:
        return {{31, 5.1}, {32, 4.1}, {33, 3.1}, {34, 2.1}, {35, 1.1}};
    case 4:
        return {{41, 5.0}, {42, 4.0}, {43, 3.0}, {44, 2.0}, {45, 1.0}};
    }
    return {};
}
...
std::tuple<size_t, Hits, RangePair> second_phase(MatchLoopCommunicator& com, const Hits& hits, size_t thread_id,
                                                 double delta = 0.0) {
    ...
    // the DocumentScorer used by the match thread will sort on docid here to ensure increasing seek order, this is
    // not needed here
    ...
    // the HitCollector will sort on docid to prepare for result merging, we do it to simplify comparing with expected
    // results
...
struct EveryOdd : public search::queryeval::IDiversifier {
    bool accepted(uint32_t docId) override { return docId & 0x01; }
};
...
TEST(MatchLoopCommunicatorTest, require_that_selectBest_gives_appropriate_results_for_single_thread) {
    ...
    EXPECT_EQ(hit_vec({{1, 5}, {2, 4}, {3, 3}}),
              selectBest(f1, hit_vec({{1, 5}, {2, 4}, {3, 3}, {4, 2}}), thread_id));
}
...
TEST(MatchLoopCommunicatorTest, require_that_selectBest_gives_appropriate_results_for_single_thread_with_filter) {
    ...
    EXPECT_EQ(hit_vec({{1, 5}, {3, 3}, {5, 1}}),
              selectBest(f1, hit_vec({{1, 5}, {2, 4}, {3, 3}, {4, 2}, {5, 1}, {6, 0}}), thread_id));
}

TEST(MatchLoopCommunicatorTest, require_that_selectBest_works_with_no_hits) {
    constexpr size_t      num_threads = 10;
    MatchLoopCommunicator f1(num_threads, 10);
    auto task = [&f1](Nexus& ctx) { EXPECT_TRUE(selectBest(f1, hit_vec({}), ctx.thread_id()).empty()); };
...
TEST(MatchLoopCommunicatorTest, require_that_rangeCover_works_with_multiple_threads) {
    ...
    auto                  task = [&f1](Nexus& ctx) {
        auto      thread_id = ctx.thread_id();
        RangePair res = rangeCover(
            f1, hit_vec({{thread_id * 100 + 1, 100.0 + thread_id}, {thread_id * 100 + 2, 100.0 - thread_id}}),
            thread_id, 10);
