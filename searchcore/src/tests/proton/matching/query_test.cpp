...
void setupIndexEnvironments() {
    ...
    attribute_index_env.getFields().emplace_back(FieldType::ATTRIBUTE, CollectionType::SINGLE, field, 1);
...
Fixture::Fixture() : _match_data(), _blueprint(), _requestContext() {
}

Fixture::~Fixture() = default;

SearchIterator::UP Fixture::getIterator(Node& node, ISearchContext& context) {
    MatchDataLayout         mdl;
...
std::string termAsString(const search::query::Range& term) {
    vespalib::asciistream os;
    return (os << term).str();
}

const std::string& termAsString(const std::string& term) {
    return term;
}

Node::UP buildQueryTree(const ViewResolver& resolver, const search::fef::IIndexEnvironment& idxEnv) {
...
Node::UP buildSameElementQueryTree(const ViewResolver& resolver, const search::fef::IIndexEnvironment& idxEnv) {
    QueryBuilder<ProtonNodeTypes> query_builder;
...
public:
    template <class TermType> void checkNode(const TermType& n, int estimatedHitCount, bool empty) {
...
TEST(QueryTest, requireThatTermsAreLookedUp) {
    ...
    context.idx(0)
        ...
        .addResult(field, string_term, FakeResult().doc(2).pos(3).doc(3).pos(4))
        .addResult(field, termAsString(int_term), FakeResult().doc(4).pos(5));
    context.idx(1)
        .getFake()
        .addResult(field, string_term, FakeResult().doc(6).pos(7))
        .addResult(field, suffix_term, FakeResult().doc(7).pos(8).doc(8).pos(9))
        .addResult(field, termAsString(float_term), FakeResult().doc(9).pos(10))
        .addResult(field, termAsString(int_term), FakeResult().doc(10).pos(11))
        .addResult(field, termAsString(range_term), FakeResult().doc(12).pos(13).doc(13).pos(14));
...
TEST(QueryTest, requireThatTermsAreLookedUpInMultipleFieldsFromAView) {
    ...
    context.idx(0)
        .getFake()
        .addResult(resolved_field1, prefix_term, FakeResult().doc(1).pos(2))
        .addResult(resolved_field2, string_term, FakeResult().doc(2).pos(3).doc(3).pos(4))
        .addResult(resolved_field1, termAsString(int_term), FakeResult().doc(4).pos(5));
    context.idx(1)
        .getFake()
        .addResult(resolved_field1, string_term, FakeResult().doc(6).pos(7))
        .addResult(resolved_field2, suffix_term, FakeResult().doc(7).pos(8).doc(8).pos(9))
        .addResult(resolved_field1, termAsString(float_term), FakeResult().doc(9).pos(10))
        .addResult(resolved_field2, termAsString(int_term), FakeResult().doc(10).pos(11))
        .addResult(resolved_field1, termAsString(range_term), FakeResult().doc(12).pos(13).doc(13).pos(14));
    context.setLimit(doc_count + 1);
...
class SetUpTermDataTestCheckerVisitor : public CustomTypeTermVisitor<ProtonNodeTypes> {
public:
...
TEST(QueryTest, requireThatTermDataIsFilledIn) {
    ...
    FakeSearchContext  context;
    context.addIdx(1);
    context.idx(0).getFake().addResult(resolved_field1, string_term, FakeResult().doc(1).pos(2).doc(5).pos(3));
    context.setLimit(doc_count + 1);
...
FakeIndexSearchable getFakeSearchable(const string& term, int doc1, int doc2) {
    ...
    source.getFake().addResult(field, term, FakeResult().doc(doc1).pos(2).doc(doc2).pos(3));
    return source;
}
...
TEST(QueryTest, requireThatSingleIndexCanUseBlendingAsBlacklisting) {
    ...
    builder.addStringTerm(string_term, field, 1, Weight(2)).resolve(ViewResolver(), plain_index_env);
...
TEST(QueryTest, requireThatIteratorsAreBuiltForAllTermNodes) {
    ...
    context.addIdx(0)
        .idx(0)
        .getFake()
        .addResult(field, termAsString(float_term), FakeResult().doc(2).pos(2))
        .addResult(field, termAsString(int_term), FakeResult().doc(4).pos(2))
        .addResult(field, prefix_term, FakeResult().doc(8).pos(2))
        .addResult(field, termAsString(range_term), FakeResult().doc(15).pos(2))
...
TEST(QueryTest, requireThatNearIteratorsCanBeBuilt) {
    ...
    context.addIdx(0)
        .idx(0)
        .getFake()
        .addResult(field, prefix_term,
                   FakeResult().doc(4).pos(2).len(50).doc(8).pos(2).len(50).doc(12).pos(2).len(50))
        .addResult(field, string_term,
                   FakeResult().doc(4).pos(40).len(50).doc(8).pos(5).len(50).doc(12).pos(5).len(50))
        .addResult(field, int_term, FakeResult().doc(8).pos(7).len(50));
...
TEST(QueryTest, requireThatONearIteratorsCanBeBuilt) {
    ...
    context.addIdx(0)
        ...
        .getFake()
        .addResult(field, string_term,
                   FakeResult().doc(4).pos(5).len(50).doc(8).pos(2).len(50).doc(12).pos(2).len(50))
        .addResult(field, prefix_term,
                   FakeResult().doc(4).pos(2).len(50).doc(8).pos(5).len(50).doc(12).pos(5).len(50))
        .addResult(field, int_term, FakeResult().doc(8).pos(7).len(50));
...
TEST(QueryTest, requireThatPhraseIteratorsCanBeBuilt) {
    ...
    context.addIdx(0)
        ...
        .addResult(
            field, string_term,
            FakeResult().doc(4).pos(3).len(50).doc(5).pos(2).len(50).doc(8).pos(2).len(50).doc(9).pos(2).len(50))
        .addResult(field, prefix_term, FakeResult().doc(4).pos(2).len(50).doc(5).pos(4).len(50).doc(8).pos(3).len(50))
        .addResult(field, suffix_term,
                   FakeResult().doc(4).pos(1).len(50).doc(5).pos(5).len(50).doc(8).pos(4).len(50));
...
TEST(QueryTest, requireThatUnknownFieldActsEmpty) {
    Fixture           f;
    FakeSearchContext context;
    context.addIdx(0).idx(0).getFake().addResult(unknown_field, string_term,
                                                 FakeResult().doc(4).pos(3).len(50).doc(5).pos(2).len(50));
...
void checkQueryAddsLocation(const string& loc_in, const string& loc_out) {
    ...
    index_environment.getFields().emplace_back(FieldType::INDEX, CollectionType::SINGLE, field, 1);
    index_environment.getFields().emplace_back(FieldType::ATTRIBUTE, CollectionType::SINGLE,
                                               PositionDataType::getZCurveFieldName(loc_field), 2);

    QueryBuilder<ProtonNodeTypes> builder;
    builder.addStringTerm(string_term, field, 1, Weight(2));
    auto  serializedQueryTree = StackDumpCreator::createSerializedQueryTree(*builder.build());
    Query query;
    query.buildTree(*serializedQueryTree, loc_field + ":" + loc_in, ViewResolver(), index_environment);
    vector<const ITermData*> term_data;
    query.extractTerms(term_data);
    ...
    SearchIterator::UP search = query.createSearch(*md);
    ASSERT_TRUE(search);
    EXPECT_NE(string::npos, search->asString().find(loc_out))
        << "search (missing loc_out '" << loc_out << "'): " << search->asString();
}
...
void verifyThatRankBlueprintAndAndNotStaysOnTopAfterLocation(QueryBuilder<ProtonNodeTypes>& builder) {
    ...
    FakeSearchContext context(42);
    context.addIdx(0).idx(0).getFake().addResult(field, "foo", FakeResult().doc(1));
    context.setLimit(42);
...
TEST(QueryTest, requireThatQueryAddsLocation) {
    ...
    checkQueryAddsLocation("{a:12345,b:{x:[8,10],y:[8,10]},p:{x:10,y:10},r:3}",
                           "{p:{x:10,y:10},r:3,a:12345,b:{x:[8,10],y:[8,10]}}");
}
...
TEST(QueryTest, requireThatWhiteListBlueprintCanBeUsed) {
    ...
    FakeSearchContext context(42);
    context.addIdx(0).idx(0).getFake().addResult(field, "foo",
                                                 FakeResult().doc(1).doc(3).doc(5).doc(7).doc(9).doc(11));
    context.setLimit(42);
...
search::query::Node::UP make_same_element_stack_dump(const std::string& prefix, const std::string& term_prefix) {
    QueryBuilder<ProtonNodeTypes> builder;
...
TEST(QueryTest, requireThatSameElementIteratorsCanBeBuilt) {
    ...
    context.addIdx(0)
        .idx(0)
        .getFake()
        .addResult(field, string_term, FakeResult().doc(4).elem(1).pos(0).doc(8).elem(1).pos(0))
        .addResult(field, prefix_term, FakeResult().doc(4).elem(2).pos(0).doc(8).elem(1).pos(1));
    SearchIterator::UP iterator = f.getIterator(*node, context);
    ASSERT_TRUE(iterator);
    EXPECT_TRUE(!iterator->seek(4));
    EXPECT_TRUE(iterator->seek(8));
...
TEST(QueryTest, requireThatConstBoolBlueprintsAreCreatedCorrectly) {
    ...
    FakeSearchContext  context;
    context.setLimit(1000);
    context.addIdx(0).idx(0).getFake().addResult(field, "foo", FakeResult().doc(1).doc(3));
...
public:
    ...
    bool want_global_filter(Blueprint::GlobalFilterLimits&) const override { return _want_global_filter; }
