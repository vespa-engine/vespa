...
struct MockSearch : SearchIterator {
    ...
    explicit MockSearch(std::string term_in)
        : spec("", 0, 0),
          ...
          _strict(vespalib::Trinary::True),
          tfmda(),
          postings_fetched(false) {}
    MockSearch(const FieldSpec& spec_in, std::string term_in, bool strict_in, TermFieldMatchDataArray tfmda_in,
               bool postings_fetched_in)
        : spec(spec_in),
          term(std::move(term_in)),
...
struct MockSearchable : Searchable {
    size_t create_cnt = 0;
    std::unique_ptr<Blueprint> createBlueprint(const IRequestContext& requestContext, const FieldSpec& field,
...
public:
    RangeLimitMetaInfo locate() const override { return {}; }
};
...
struct MaxFilterCoverageLimiterFixture {
    ...
    MatchPhaseLimiter::UP getMaxFilterCoverageLimiter() {
        auto yes_limiter =
            std::make_unique<MatchPhaseLimiter>(10000, rangeLocator, searchable, requestContext,
...
TEST(MatchPhaseLimiterTest,
     require_that_the_match_phase_limiter_may_chose_not_to_limit_the_query_when_considering_max_filter_coverage) {
...
TEST(MatchPhaseLimiterTest,
     require_that_the_match_phase_limiter_may_chose_to_limit_the_query_even_when_considering_max_filter_coverage) {
    MaxFilterCoverageLimiterFixture f;
    MatchPhaseLimiter::UP           limiterUP = f.getMaxFilterCoverageLimiter();
    MaybeMatchPhaseLimiter&         limiter = *limiterUP;
...
TEST(MatchPhaseLimiterTest, require_that_the_match_phase_limiter_is_able_to_pre_limit_the_query) {
    ...
    SearchIterator::UP search =
        limiter.maybe_limit(prepare(new MockSearch("search")), 0.1, 100000, trace.maybeCreateCursor(7, "limit"));
    limiter.updateDocIdSpaceEstimate(1000, 9000);
...
struct RangeLimitFixture {
    RangeLimitFixture()
        : attrSearchable(),
          ...
          a1FieldSpec("a1", 1, 1),
          f1FieldSpec("f1", 2, 2) {
        attrManager.addAttribute(
            search::AttributeFactory::createAttribute(a1FieldSpec.getName(), Config(BasicType::INT64)));
        attrManager.addAttribute(
            search::AttributeFactory::createAttribute(f1FieldSpec.getName(), Config(BasicType::DOUBLE)));
    }
    ~RangeLimitFixture();
...
void verifyLocateRange(const std::string& from, const std::string& to, const FieldSpec& fieldSpec,
                       RangeLimitFixture& f) {
    search::fef::MatchDataLayout    mdl;
    search::query::SimpleNumberTerm term(fmt("[%s;%s]", from.c_str(), to.c_str()), fieldSpec.getName(), 0,
                                         search::query::Weight(1));
    Blueprint::UP                   bp = f.attrSearchable.createBlueprint(f.requestContext, fieldSpec, term, mdl);
    EXPECT_EQ(mdl.empty(), true);
...
void verifyRangeIsReflectedInLimiter(const std::string& from, const std::string& to, const FieldSpec& fieldSpec,
                                     RangeLimitFixture& f) {
    search::fef::MatchDataLayout    mdl;
    search::query::SimpleNumberTerm term(fmt("[%s;%s]", from.c_str(), to.c_str()), fieldSpec.getName(), 0,
                                         search::query::Weight(1));
