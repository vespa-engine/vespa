...
struct ConfigTestFixture {
    ...
    explicit ConfigTestFixture(const std::string& id) : ConfigTestFixture(id, true) {}
    ...
    void removeDocType(const std::string& name) {
        ...
        for (auto it(protonBuilder.documentdb.begin()), mt(protonBuilder.documentdb.end()); it != mt; it++) {
    ...
    BootstrapConfig::SP getBootstrapConfig(int64_t generation, const HwInfo& hwInfo) const {
        return std::make_shared<BootstrapConfig>(
            generation, std::make_shared<DocumenttypesConfig>(documenttypesBuilder),
            std::make_shared<DocumentTypeRepo>(documenttypesBuilder), std::make_shared<ProtonConfig>(protonBuilder),
            std::make_shared<FiledistributorrpcConfig>(), std::make_shared<BucketspacesConfig>(bucketspacesBuilder),
...
DocumentDBConfig::SP getDocumentDBConfig(ConfigTestFixture& f, DocumentDBConfigManager& mgr, const HwInfo& hwInfo) {
...
DocumentDBConfig::SP getDocumentDBConfig(ConfigTestFixture& f, DocumentDBConfigManager& mgr) {
...
TEST(
    ProtonConfigFetcherTest,
    require_that_documentdb_config_manager_builds_schema_with_imported_attribute_fields_and_that_they_are_filtered_from_resulting_attribute_config) {
    ConfigTestFixture       f1("search");
    DocumentDBConfigManager f2(f1.configId + "/typea", "typea");
...
HwInfo createHwInfoWithMemory(uint64_t mem) {
...
GrowStrategy growStrategy(uint32_t initial) {
...
TEST(ProtonConfigFetcherTest, require_that_allocation_config_is_propagated) {
    ...
    auto config = getDocumentDBConfig(f1, f2);
    {
        ...
        EXPECT_EQ(AllocStrategy(growStrategy(20000000), CompactionStrategy(0.25, 0.3), 10000),
                  alloc_config.make_alloc_strategy(SubDbType::READY));
        EXPECT_EQ(AllocStrategy(growStrategy(100000), CompactionStrategy(0.25, 0.3), 10000),
                  alloc_config.make_alloc_strategy(SubDbType::REMOVED));
        EXPECT_EQ(AllocStrategy(growStrategy(30000000), CompactionStrategy(0.25, 0.3), 10000),
                  alloc_config.make_alloc_strategy(SubDbType::NOTREADY));
    }
}
