...
std::string rsString(const ResultSet& s) {
...
SelectPrunerTest::SelectPrunerTest()
    : ::testing::Test(), _amgr(), _repoUP(), _hasFields(true), _has_document_ids(false), _hasDocuments(true) {
...
void SelectPrunerTest::testParse(const string& selection) {
...
void SelectPrunerTest::testParseFail(const string& selection) {
    SCOPED_TRACE(selection);
    const DocumentTypeRepo&  repo(*_repoUP);
    document::select::Parser parser(repo, document::BucketIdFactory());
...
void SelectPrunerTest::testPrune(const string& selection, const string& exp, const string& docTypeName) {
    SCOPED_TRACE(selection);
    const DocumentTypeRepo&  repo(*_repoUP);
    document::select::Parser parser(repo, document::BucketIdFactory());
    ...
    std::cout << "Pruned ParseTree: '" << pos.str() << "', fieldNodes=" << pruner.getFieldNodes()
              << ", attrFieldNodes=" << pruner.getAttrFieldNodes()
              << ", document_id_nodes=" << pruner.get_document_id_nodes() << ", cs=" << csString(pruner)
...
void SelectPrunerTest::testPrune(const string& selection, const string& exp) {
...
TEST_F(SelectPrunerTest, Test_that_boolean_const_shortcuts_are_OK) {
    testPrune("false and false", "false");
    testPrune(false_name + " and " + invalid2_name, "false");
    testPrune(false_name + " and " + valid2_name, "false");
    testPrune("false and true", "false");

    testPrune(invalid_name + " and false", "false");
    testPrune(invalid_name + " and " + invalid2_name, "invalid");
    testPrune(invalid_name + " and " + valid2_name, empty + "invalid and " + valid2_name);
    testPrune(invalid_name + " and true", "invalid");

    testPrune(valid_name + " and false", "false");
    testPrune(valid_name + " and " + invalid2_name, empty + valid_name + " and invalid");
    testPrune(valid_name + " and " + valid2_name, valid_name + " and " + valid2_name);
    testPrune(valid_name + " and true", valid_name);

    testPrune("true and false", "false");
    testPrune(true_name + " and " + invalid2_name, "invalid");
    testPrune(true_name + " and " + valid2_name, valid2_name);
    testPrune("true and true", "true");

    testPrune("false or false", "false");
    testPrune(false_name + " or " + invalid2_name, "invalid");
    testPrune(false_name + " or " + valid2_name, valid2_name);
    testPrune("false or true", "true");

    testPrune(invalid_name + " or false", "invalid");
    testPrune(invalid_name + " or " + invalid2_name, "invalid");
    testPrune(invalid_name + " or " + valid2_name, empty + "invalid or " + valid2_name);
    testPrune(invalid_name + " or true", "true");

    testPrune(valid_name + " or false", valid_name);
    testPrune(valid_name + " or " + invalid2_name, valid_name + " or invalid");
    testPrune(valid_name + " or " + valid2_name, valid_name + " or " + valid2_name);
    testPrune(valid_name + " or true", "true");

    testPrune("true or false", "true");
    testPrune(true_name + " or " + invalid2_name, "true");
    testPrune(true_name + " or " + valid2_name, "true");
    testPrune("true or true", "true");
}

TEST_F(SelectPrunerTest, Test_that_selection_expressions_are_pruned) {
    testPrune("not ((test))", "false");
    testPrune("not ((test and (test.aa > 3999)))", "test.aa <= 3999");
    testPrune("not ((test and (test.ab > 3999)))", "test.ab <= 3999");
    testPrune("not ((test and (test.af > 3999)))", "invalid");
    testPrune("not ((test and (test_2.ac > 3999)))", "invalid");
    testPrune("not ((test and (test.af > 3999)))", "true", "test_2");
    const char* combined = "not ((test and (test.aa > 3999)) or (test_2 and (test_2.ac > 4999)))";
    testPrune(combined, "test.aa <= 3999");
    testPrune(combined, "test_2.ac <= 4999", "test_2");
}

TEST_F(SelectPrunerTest, Test_that_De_Morgans_laws_are_applied) {
    testPrune("not (test.aa < 3901 and test.ab < 3902)", "test.aa >= 3901 or test.ab >= 3902");
    testPrune("not (test.aa < 3903 or test.ab < 3904)", "test.aa >= 3903 and test.ab >= 3904");
    testPrune("not (not (test.aa < 3903 or test.ab < 3904))", "test.aa < 3903 or test.ab < 3904");

    testPrune("not (false and false)", "true");
    testPrune(empty + "not (false and " + invalid2_name + ")", "true");
    testPrune(empty + "not (false and " + valid2_name + ")", "true");
    testPrune("not (false and true)", "true");

    testPrune(empty + "not (" + invalid_name + " and false)", "true");
    testPrune(empty + "not (" + invalid_name + " and " + invalid2_name + ")", "invalid");
    testPrune(empty + "not (" + invalid_name + " and " + valid2_name + ")", empty + "invalid or " + rvalid2_name);
    testPrune(empty + "not (" + invalid_name + " and true)", "invalid");

    testPrune(empty + "not (" + valid_name + " and false)", "true");
    testPrune(empty + "not (" + valid_name + " and " + invalid2_name + ")", empty + rvalid_name + " or invalid");
    testPrune(empty + "not (" + valid_name + " and " + valid2_name + ")", rvalid_name + " or " + rvalid2_name);
    testPrune(empty + "not (" + valid_name + " and true)", rvalid_name);

    testPrune("not (true and false)", "true");
    testPrune(empty + "not (true and " + invalid2_name + ")", "invalid");
    testPrune(empty + "not (true and " + valid2_name + ")", rvalid2_name);
    testPrune("not (true and true)", "false");

    testPrune("not (false or false)", "true");
    testPrune(empty + "not (false or " + invalid2_name + ")", "invalid");
    testPrune(empty + "not (false or " + valid2_name + ")", rvalid2_name);
    testPrune("not (false or true)", "false");

    testPrune(empty + "not (" + invalid_name + " or false)", "invalid");
    testPrune(empty + "not (" + invalid_name + " or " + invalid2_name + ")", "invalid");
    testPrune(empty + "not (" + invalid_name + " or " + valid2_name + ")", empty + "invalid and " + rvalid2_name);
    testPrune(empty + "not (" + invalid_name + " or true)", "false");

    testPrune(empty + "not (" + valid_name + " or false)", rvalid_name);
    testPrune(empty + "not (" + valid_name + " or " + invalid2_name + ")", rvalid_name + " and invalid");
    testPrune(empty + "not (" + valid_name + " or " + valid2_name + ")", rvalid_name + " and " + rvalid2_name);
    testPrune(empty + "not (" + valid_name + " or true)", "false");

    testPrune("not (true or false)", "false");
    testPrune(empty + "not (true or " + invalid2_name + ")", "false");
    testPrune(empty + "not (true or " + valid2_name + ")", "false");
    testPrune("not (true or true)", "false");
}

TEST_F(SelectPrunerTest, Test_that_attribute_fields_and_constants_are_evaluated_before_other_fields) {
    testPrune("test.ia == \"hello\" and test.aa > 5", "test.aa > 5 and test.ia == \"hello\"");
}

TEST_F(SelectPrunerTest, Test_that_functions_are_visited) {
    testPrune("test.ia.lowercase() == \"hello\"", "test.ia.lowercase() == \"hello\"");
    testPrune("test_2.ac.lowercase() == \"hello\"", "invalid");
    testPrune("test.ia.hash() == 45", "test.ia.hash() == 45");
    testPrune("test_2.ic.hash() == 45", "invalid");
    testPrune("test.aa.abs() == 45", "test.aa.abs() == 45");
    testPrune("test_2.ac.abs() == 45", "invalid");
}

TEST_F(SelectPrunerTest, Test_that_arithmethic_values_are_visited) {
    testPrune("test.aa + 4 < 3999", "test.aa + 4 < 3999");
    testPrune("test_2.ac + 4 < 3999", "invalid");
    testPrune("test.aa + 4.2 < 3999", "test.aa + 4.2 < 3999");
    testPrune("test_2.ac + 5.2 < 3999", "invalid");
}

TEST_F(SelectPrunerTest, Test_that_addition_is_associative) {
    testPrune("test.aa + 4 + 5 < 3999", "test.aa + 4 + 5 < 3999");
    testPrune("(test.aa + 6) + 7 < 3999", "test.aa + 6 + 7 < 3999");
    testPrune("test.aa + (8 + 9) < 3999", "test.aa + 8 + 9 < 3999");
}
...
TEST_F(SelectPrunerTest, Test_that_toplevel_functions_are_visited) {
    testPrune("id.scheme == \"doc\"", "id.scheme == \"doc\"");
    testPrune("test.aa < now() - 7200", "test.aa < now() - 7200");
}

TEST_F(SelectPrunerTest, Test_that_variables_are_visited) {
    testPrune("$foovar == 4.3", "$foovar == 4.3");
}

TEST_F(SelectPrunerTest, Test_that_null_is_visited) {
    testPrune("test.aa", "test.aa != null");
    testPrune("test.aa == null", "test.aa == null");
    testPrune("not test.aa", "test.aa == null");
}

TEST_F(SelectPrunerTest, Test_that_operator_inversion_works) {
    testPrune("not test.aa < 3999", "test.aa >= 3999");
    testPrune("not test.aa <= 3999", "test.aa > 3999");
    testPrune("not test.aa > 3999", "test.aa <= 3999");
    testPrune("not test.aa >= 3999", "test.aa < 3999");
    testPrune("not test.aa == 3999", "test.aa != 3999");
...
TEST_F(SelectPrunerTest, Test_that_operator_inversion_is_disabled_for_complex_cases) {
    ...
    testPrune("not test.aa != 3998.5", "not test.aa != 3998.5");
    testPrune("not test.aa != 3998.5 + 2", "not test.aa != 3998.5 + 2");
    testPrune("not test.ad != 3999", "not test.ad != 3999");
    // function
    testPrune("not test.ad.abs() != 3999", "not test.ad.abs() != 3999");
    testPrune("not (test.ad + 3.1).abs() != 3999", "not (test.ad + 3.1).abs() != 3999");
    // array value
    testPrune("not test.aaa != 3999", "not test.aaa != 3999");
    // wset value
    testPrune("not test.aaw != 3999", "not test.aaw != 3999");
    // variable
    testPrune("not $foovar == 4", "not $foovar == 4");
}

TEST_F(SelectPrunerTest, Test_that_fields_are_not_present_in_removed_sub_db) {
    _hasFields = true;
    testPrune("test.aa > 5", "test.aa > 5");
    testPrune("test.aa == test.ab", "test.aa == test.ab");
    testPrune("test.aa != test.ab", "test.aa != test.ab");
    testPrune("not test.aa == test.ab", "test.aa != test.ab");
    testPrune("not test.aa != test.ab", "test.aa == test.ab");
    testPrune("test.ia == \"hello\"", "test.ia == \"hello\"");
    _hasFields = false;
    testPrune("test.aa > 5", "invalid");
    testPrune("test.aa == test.ab", "true");
    testPrune("test.aa != test.ab", "false");
    testPrune("test.aa < test.ab", "invalid");
    testPrune("test.aa > test.ab", "invalid");
    testPrune("test.aa <= test.ab", "invalid");
    testPrune("test.aa >= test.ab", "invalid");
    testPrune("not test.aa == test.ab", "false");
    testPrune("not test.aa != test.ab", "true");
    testPrune("test.ia == \"hello\"", "invalid");
    testPrune("not test.aa < test.ab", "invalid");
    testPrune("not test.aa > test.ab", "invalid");
    testPrune("not test.aa <= test.ab", "invalid");
    testPrune("not test.aa >= test.ab", "invalid");
}
...
TEST_F(SelectPrunerTest, Test_that_some_operators_cannot_be_inverted) {
    testPrune("test.ia == \"hello\"", "test.ia == \"hello\"");
    testPrune("not test.ia == \"hello\"", "test.ia != \"hello\"");
    testPrune("test.ia = \"hello\"", "test.ia = \"hello\"");
    testPrune("not test.ia = \"hello\"", "not test.ia = \"hello\"");
    testPrune("not (test.ia == \"hello\" or test.ia == \"hi\")", "test.ia != \"hello\" and test.ia != \"hi\"");
    testPrune("not (test.ia == \"hello\" or test.ia = \"hi\")", "not (not test.ia != \"hello\" or test.ia = \"hi\")");
    testPrune("not (test.ia = \"hello\" or test.ia == \"hi\")", "not (test.ia = \"hello\" or not test.ia != \"hi\")");
    testPrune("not (test.ia = \"hello\" or test.ia = \"hi\")", "not (test.ia = \"hello\" or test.ia = \"hi\")");
}

TEST_F(SelectPrunerTest, Test_that_complex_field_refs_are_handled) {
    testPrune("test.ia", "test.ia != null");
    testPrune("test.ia != null", "test.ia != null");
    testPrune("test.ia == \"hello\"", "test.ia == \"hello\"");
    testPrune("test.ia.foo == \"hello\"", "invalid");
    testPrune("test.ibs.foo == \"hello\"", "invalid");
    testPrune("test.ibs.x == \"hello\"", "test.ibs.x == \"hello\"");
    testPrune("test.ia[2] == \"hello\"", "invalid");
    testPrune("test.iba[2] == \"hello\"", "test.iba[2] == \"hello\"");
    testPrune("test.ia{foo} == \"hello\"", "invalid");
    testPrune("test.ibw{foo} == 4", "test.ibw{foo} == 4");
    testPrune("test.ibw{foo} == \"hello\"", "test.ibw{foo} == \"hello\"");
    testPrune("test.ibm{foo} == \"hello\"", "test.ibm{foo} == \"hello\"");
    testPrune("test.aa == 4", "test.aa == 4");
    testPrune("test.aa[4] == 4", "invalid");
    testPrune("test.aaa[4] == 4", "test.aaa[4] == 4");
    testPrune("test.aa{4} == 4", "invalid");
    testPrune("test.aaw{4} == 4", "test.aaw{4} == 4");
    testPrune("id.namespace == \"hello\"", "id.namespace == \"hello\"");
    testPrune("test.aa == 4 and id.namespace == \"hello\"", "test.aa == 4 and id.namespace == \"hello\"");
...
TEST_F(SelectPrunerTest, Test_that_field_values_are_invalid_when_disabling_document_access) {
    _hasDocuments = false;
    testPrune("test.ia", "invalid");
    testPrune("test.ia != null", "invalid");
    testPrune("test.ia == \"hello\"", "invalid");
    testPrune("test.ia.foo == \"hello\"", "invalid");
    testPrune("test.ibs.foo == \"hello\"", "invalid");
    testPrune("test.ibs.x == \"hello\"", "invalid");
    testPrune("test.ia[2] == \"hello\"", "invalid");
    testPrune("test.iba[2] == \"hello\"", "invalid");
    testPrune("test.ia{foo} == \"hello\"", "invalid");
    testPrune("test.ibw{foo} == 4", "invalid");
    testPrune("test.ibw{foo} == \"hello\"", "invalid");
    testPrune("test.ibm{foo} == \"hello\"", "invalid");
    testPrune("test.aa == 4", "test.aa == 4");
    testPrune("test.aa[4] == 4", "invalid");
    testPrune("test.aaa[4] == 4", "invalid");
    testPrune("test.aa{4} == 4", "invalid");
    testPrune("test.aaw{4} == 4", "invalid");
    testPrune("id.namespace == \"hello\"", "invalid");
    testPrune("test.aa == 4 and id.namespace == \"hello\"", "test.aa == 4 and invalid");
    testPrune("test.aa == 4 and test.ae == 5 and id.namespace == \"hello\"",
              "test.aa == 4 and test.ae == 5 and invalid");
}

TEST_F(SelectPrunerTest, Imported_fields_with_matching_attribute_names_are_supported) {
    testPrune("test.my_imported_field > 0", "test.my_imported_field > 0");
}
