...
struct Fixture {
    ...
    Fixture() : docType(repo->getDocumentType("testdoc")) {}
    ~Fixture();
    ...
    void applyWeightedSetUpdates(AttributeVector& vec, std::unique_ptr<FieldValue> assign,
                                 std::unique_ptr<FieldValue> first, std::unique_ptr<FieldValue> copyOfFirst,
                                 std::unique_ptr<FieldValue> second) {
        applyValueUpdate(vec, 1, std::make_unique<AssignValueUpdate>(std::move(assign)));
        ...
        applyValueUpdate(
            vec, 5,
            std::make_unique<MapValueUpdate>(
                std::move(copyOfFirst), std::make_unique<ArithmeticValueUpdate>(ArithmeticValueUpdate::Add, 10)));
    }
...
template <typename T> bool check(const AttributePtr& vec, uint32_t docId, const std::vector<T>& values) {
...
TEST(AttributeUpdaterTest, require_that_single_attributes_are_updated) {
    ...
    {
        auto vec =
            AttributeBuilder("in1/string", Config(BasicType::STRING)).fill({"first"s, "first"s, "first"s}).get();
    ...
    }
    {
        ...
        f.applyValueUpdate(
            *vec, 1,
            std::make_unique<AssignValueUpdate>(std::make_unique<ReferenceFieldValue>(
                dynamic_cast<const ReferenceDataType&>(f.docType->getField("ref").getDataType()), DocumentId(doc2))));
        f.applyValueUpdate(*vec, 3, std::make_unique<ClearValueUpdate>());
        EXPECT_EQ(4u, vec->getNumDocs());
...
TEST(AttributeUpdaterTest, require_that_array_attributes_are_updated) {
    ...
    CollectionType ct(CollectionType::ARRAY);
    {
        ...
        auto vec = AttributeBuilder("in1/aint", Config(BasicType::INT32, ct))
                       .fill_array({IL{32}, {32}, {32}, {32}, {32}})
                       .get();
    ...
    }
    {
        ...
        auto vec = AttributeBuilder("in1/afloat", Config(BasicType::FLOAT, ct))
                       .fill_array({DL{55.5}, {55.5}, {55.5}, {55.5}, {55.5}})
                       .get();
    ...
    }
    {
        auto vec = AttributeBuilder("in1/astring", Config(BasicType::STRING, ct))
                       .fill_array({{"first"}, {"first"}, {"first"}, {"first"}, {"first"}})
                       .get();
...
TEST(AttributeUpdaterTest, require_that_weighted_set_attributes_are_updated) {
    ...
    CollectionType ct(CollectionType::WSET);
    {
        ...
        auto vec = AttributeBuilder("in1/wsint", Config(BasicType::INT32, ct))
                       .fill_wset({WIL{{32, 100}}, {{32, 100}}, {{32, 100}}, {{32, 100}}, {{32, 100}}})
                       .get();
        auto first = std::make_unique<IntFieldValue>(32);
        ...
        f.applyWeightedSetUpdates(*vec, std::move(assign), std::move(first), std::move(copyOfFirst),
                                  std::move(second));
    ...
    }
    {
        ...
        auto vec = AttributeBuilder("in1/wsfloat", Config(BasicType::FLOAT, ct))
                       .fill_wset({WDL{{55.5, 100}}, {{55.5, 100}}, {{55.5, 100}}, {{55.5, 100}}, {{55.5, 100}}})
                       .get();
        auto first = std::make_unique<FloatFieldValue>(55.5f);
    ...
    {
        auto vec =
            AttributeBuilder("in1/wsstring", Config(BasicType::STRING, ct))
                .fill_wset({{{"first", 100}}, {{"first", 100}}, {{"first", 100}}, {{"first", 100}}, {{"first", 100}}})
                .get();
        auto first = StringFieldValue::make("first");
        ...
        EXPECT_TRUE(
            check(vec, 2, std::vector<WeightedString>{WeightedString("first", 100), WeightedString("second", 20)}));
        EXPECT_TRUE(check(vec, 3, std::vector<WeightedString>{}));
        EXPECT_TRUE(check(vec, 4, std::vector<WeightedString>{}));
...
const TensorDataType& getTensorDataType(const std::string& spec) {
    auto insres = tensorTypes.insert(std::make_pair(spec, TensorDataType::fromSpec(spec)));
    return *insres.first->second;
}

std::unique_ptr<Value> makeTensor(const TensorSpec& spec) {
    return SimpleValue::from_spec(spec);
}

std::unique_ptr<TensorFieldValue> makeTensorFieldValue(const TensorSpec& spec) {
