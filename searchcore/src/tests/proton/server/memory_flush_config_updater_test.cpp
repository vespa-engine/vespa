...
ProtonConfig::Flush::Memory getConfig(int64_t maxMemory, int64_t eachMaxMemory, int64_t maxTlsSize,
                                      double conservativeMemoryLimitFactor = 0.5,
                                      double conservativeDiskLimitFactor = 0.6, double high_watermark_factor = 0.9,
...
ProtonConfig::Flush::Memory getDefaultConfig() {
    return getConfig(4, 1, 20);
}

ResourceUsageWithLimit aboveLimit() {
    // The high watermark limit is 0.63 (0.7 * 0.9 (factor)).
    return ResourceUsageWithLimit(0.64, 0.7);
}

ResourceUsageWithLimit belowLimit() {
    // The high watermark limit is 0.63 (0.7 * 0.9 (factor)).
...
struct Fixture {
    ...
    Fixture()
        : strategy(std::make_shared<MemoryFlush>(
              MemoryFlushConfigUpdater::convertConfig(getDefaultConfig(), defaultMemory))),
    ...
    void assertStrategyConfig(const std::string& label, uint64_t expMaxGlobalMemory, int64_t expMaxEachMemory,
                              uint64_t expMaxGlobalTlsSize) {
        SCOPED_TRACE(label);
        assertStrategyConfig(expMaxGlobalMemory, expMaxEachMemory, expMaxGlobalTlsSize);
    }
    void assertStrategyDiskConfig(const std::string& label, double expGlobalDiskBloatFactor,
                                  double expDiskBloatFactor) {
        SCOPED_TRACE(label);
    ...
    void set_node_retired_or_maintenance(bool value) { updater.set_node_retired_or_maintenance(value); }
};

Fixture::~Fixture() = default;

void expectEqual(const MemoryFlush::Config& a, const MemoryFlush::Config& b) {
...
void expectNotEqual(const MemoryFlush::Config& a, const MemoryFlush::Config& b) {
...
TEST(MemoryFlushConfigUpdaterTest,
     require_that_strategy_is_updated_with_conservative_max_tls_size_value_if_disk_limit_is_reached) {
    Fixture f;
...
TEST(MemoryFlushConfigUpdaterTest,
     require_that_strategy_is_updated_with_conservative_max_memory_value_if_memory_limit_is_reached) {
    Fixture f;
...
TEST(MemoryFlushConfigUpdaterTest,
     require_that_strategy_is_updated_with_all_conservative_values_if_both_limits_are_reached) {
    Fixture f;
...
TEST(MemoryFlushConfigUpdaterTest,
     require_that_last_disk_and_memory_usage_state_is_remembered_when_setting_new_config) {
    Fixture f;
    f.notifyDiskMemUsage(aboveLimit(), belowLimit());
    f.updater.setConfig(getConfig(6, 3, 30));
...
TEST(MemoryFlushConfigUpdaterTest,
     require_that_we_must_go_below_low_watermark_for_disk_usage_before_using_normal_tls_size_value_again) {
    Fixture f;
...
TEST(MemoryFlushConfigUpdaterTest,
     require_that_we_must_go_below_low_watermark_for_memory_usage_before_using_normal_max_memory_value_again) {
    Fixture f;
...
TEST(MemoryFlushConfigUpdaterTest,
     require_that_more_disk_bloat_is_allowed_while_node_state_is_retired_or_maintenance) {
    Fixture          f;
