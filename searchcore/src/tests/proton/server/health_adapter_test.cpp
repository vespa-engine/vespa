...
struct MyStatusProducer : public StatusProducer {
    ...
    void add(const std::string& comp, StatusReport::State state, const std::string& msg) {
        list.push_back(StatusReport::SP(new StatusReport(StatusReport::Params(comp).state(state).message(msg))));
    }
    StatusReport::List getStatusReports() const override { return list; }
};
