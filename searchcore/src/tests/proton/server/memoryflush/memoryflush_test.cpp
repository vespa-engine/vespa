...
public:
    MyFlushTarget(const std::string& name, MemoryGain memoryGain, DiskGain diskGain, SerialNum flushedSerial,
...
private:
    ...
    void fixupMap(const std::string& name, SerialNum lastSerial) {
        flushengine::TlsStats oldStats = _map[name];
        if (oldStats.getLastSerial() < lastSerial) {
            _map[name] = flushengine::TlsStats(oldStats.getNumBytes(), oldStats.getFirstSerial(), lastSerial);
        }
    }
...
public:
    ContextBuilder();
    ~ContextBuilder();
    void addTls(const std::string& name, const flushengine::TlsStats& tlsStats) { _map[name] = tlsStats; }
...
ContextBuilder::ContextBuilder() : _list(), _handler(new MyFlushHandler("myhandler")), _map(), _active_flushes() {
}
ContextBuilder::~ContextBuilder() = default;

MyFlushTarget::SP createTargetM(const std::string& name, MemoryGain memoryGain) {
    return std::make_shared<MyFlushTarget>(name, memoryGain, DiskGain(), SerialNum(), system_time(), false);
}

MyFlushTarget::SP createTargetD(const std::string& name, DiskGain diskGain, SerialNum serial = 0) {
    return std::make_shared<MyFlushTarget>(name, MemoryGain(), diskGain, serial, system_time(), false);
}

MyFlushTarget::SP createTargetS(const std::string& name, SerialNum serial, system_time timeStamp = system_time()) {
    return std::make_shared<MyFlushTarget>(name, MemoryGain(), DiskGain(), serial, timeStamp, false);
}

MyFlushTarget::SP createTargetT(const std::string& name, system_time lastFlushTime, SerialNum serial = 0) {
    return std::make_shared<MyFlushTarget>(name, MemoryGain(), DiskGain(), serial, lastFlushTime, false);
}

MyFlushTarget::SP createTargetF(const std::string& name, bool urgentFlush) {
    return std::make_shared<MyFlushTarget>(name, MemoryGain(), DiskGain(), SerialNum(), system_time(), urgentFlush);
}

void assertOrder(const StringList& exp, const FlushContext::List& act) {
...
TEST(MemoryFlushTest, order_type_is_preserved) {
    ...
    { // MAXAGE VS DISKBLOAT
        ContextBuilder cb;
        cb.add(createTargetT("t2", ts2, 5), 14).add(createTargetD("t1", DiskGain(100 * milli, 80 * milli), 5));
        MemoryFlush flush({1000, 20_Gi, 1.0, 1000, 0.19, seconds(30)});
        assertOrder({"t1", "t2"}, cb.flush_targets(flush));
    }
    { // DISKBLOAT VS MEMORY
        ContextBuilder cb;
        cb.add(createTargetD("t2", DiskGain(100 * milli, 80 * milli))).add(createTargetM("t1", MemoryGain(100, 80)));
