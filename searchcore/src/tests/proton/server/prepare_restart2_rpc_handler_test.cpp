...
DestructionFutureFactoryBase::DestructionFutureFactoryBase()
    : _destruction_promise(), _destruction_future(_destruction_promise.get_future().share()) {
}
 ...
 */
template <class B> struct DestructionFutureFactory : public DestructionFutureFactoryBase, public B {
    using B::B;
    ~DestructionFutureFactory() override;
};

template <class B> DestructionFutureFactory<B>::~DestructionFutureFactory() = default;
...
public:
    DestructGuardContext() noexcept : _destructed(false), _allow_destruct(false) {}
...
DestructGuardBase::DestructGuardBase() : _destruct_guard_context(std::make_shared<DestructGuardContext>()) {
}
 ...
 */
template <class B> struct DestructGuard : public B, public DestructGuardBase {
    using B::B;
    ~DestructGuard() override;
};

template <typename B> DestructGuard<B>::~DestructGuard() = default;
...
void MyReturnHandler::HandleReturn() {
    _returned = true;
...
FNET_Connection* MyReturnHandler::GetConnection() {
    return _conn.get();
}

void MyReturnHandler::alloc_req() {
    if (_req_destruct_guard_context) {
        _req_destruct_guard_context->set_allow_destruct();
    }
...
void MyReturnHandler::check_conn() const {
...
void MyReturnHandler::check_req() {
...
std::shared_ptr<FlushHistory> make_flush_history() {
    auto history = std::make_shared<FlushHistory>("memory", 200, 3);
    return history;
}

std::shared_ptr<FlushStrategyIdNotifier> make_flush_strategy_id_notifier() {
...
void PrepareRestart2RpcHandlerTest::SetUp() {
    using Conn = DestructGuard<FNET_Connection>;
    auto conn =
        vespalib::make_ref_counted<Conn>(_transport->select_thread(nullptr, 0), &_dummy_streamer, &_dummy_adapter,
                                         vespalib::SocketHandle(socket(AF_INET, SOCK_STREAM, 0)), "dummy_spec");
...
void PrepareRestart2RpcHandlerTest::TearDown() {
    if (_conn_destruct_guard_context) {
        _conn_destruct_guard_context->set_allow_destruct();
    }
}

void PrepareRestart2RpcHandlerTest::SetUpTestSuite() {
    _transport = std::make_unique<FNET_Transport>();
    _transport->Start();
}

void PrepareRestart2RpcHandlerTest::TearDownTestSuite() {
    _transport->ShutDown(true);
    _transport.reset();
}

std::shared_future<void> PrepareRestart2RpcHandlerTest::test_handler(uint32_t               wait_strategy_id,
    ...
    auto handler = std::make_shared<Handler>(_detached_rpc_requests_owner, std::move(req_copy), _notifier,
                                             _transport->GetScheduler(), wait_strategy_id, timeout, _history);
    handler->setup();
...
void PrepareRestart2RpcHandlerTest::expect_result(bool expect_success) {
...
void PrepareRestart2RpcHandlerTest::expect_no_result() {
    _return_handler->check_req();
    EXPECT_TRUE(_return_handler->has_detached());
    ASSERT_FALSE(_return_handler->has_returned());
}

void PrepareRestart2RpcHandlerTest::expect_strategy(Inspector& inspector, Memory key, const std::string& exp_name,
                                                    uint32_t exp_id) {
...
void PrepareRestart2RpcHandlerTest::expect_flush_counts(Inspector& inspector, Memory key, uint32_t exp_flushed,
                                                        uint32_t                exp_flushing,
                                                        std::optional<uint32_t> exp_pending_flushes) {
    SCOPED_TRACE(key.make_stringview());
    auto& strategy = inspector[key];
    EXPECT_TRUE(strategy.valid());
...
TEST_F(PrepareRestart2RpcHandlerTest, poll_sequence) {
     ...
     */
    _history->start_flush(HANDLER1, "a1", strategy_info, 3s, 3s, 5);
    _history->set_strategy(PREPARE_RESTART_STRATEGY, 201, true);
    _history->add_pending_flush(HANDLER2, "a2", strategy_info, 1s, 1s);
    _history->add_pending_flush(HANDLER1, "a3", strategy_info, 4s, 4s);
    _history->add_pending_flush(HANDLER1, "a4", strategy_info, 1s, 1s);
    _history->start_flush(HANDLER2, "a2", strategy_info, 1s, 1s, 6);
    _history->start_flush(HANDLER1, "a3", strategy_info, 4s, 4s, 7);
    ...
    _history->start_flush(HANDLER1, "a4", strategy_info, 1s, 1s, 8);
    _history->set_strategy(MEMORY_STRATEGY, 202, false);
