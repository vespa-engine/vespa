...
struct MyReprocessingHandler : public IReprocessingHandler {
    ...
    void addReader(const IReprocessingReader::SP& reader) override { _reader = reader; }
    void addRewriter(const IReprocessingRewriter::SP& rewriter) override { _rewriters.push_back(rewriter); }
};
...
struct MyConfig {
    ...
    void addIndexField(const std::string& name) { _schema.addIndexField(Schema::IndexField(name, DataType::STRING)); }
};
...
MyConfig::MyConfig()
    : _fileHeaderContext(),
      ...
      _mgr(new AttributeManager(TEST_DIR, "test.subdb", TuneFileAttributes(), _fileHeaderContext,
                                std::make_shared<search::attribute::Interlock>(), _attributeFieldWriter, _shared,
                                _hwInfo)),
      _schema() {
}
...
struct MyDocTypeInspector : public IDocumentTypeInspector {
    ...
    MyDocTypeInspector(const MyConfig& oldCfg, const MyConfig& newCfg) : _oldCfg(oldCfg), _newCfg(newCfg) {}
    bool hasUnchangedField(const std::string& name) const override {
        return _oldCfg._fields.count(name) > 0 && _newCfg._fields.count(name) > 0;
    }
};

struct MyIndexschemaInspector : public IIndexschemaInspector {
    const search::index::Schema& _schema;
    MyIndexschemaInspector(const search::index::Schema& schema) : _schema(schema) {}
...
public:
    ...
    void init() {
        ...
        _initializer.reset(new AttributeReprocessingInitializer(ARIConfig(_newCfg._mgr, _newCfg._schema),
                                                                ARIConfig(_oldCfg._mgr, _oldCfg._schema), _inspector,
                                                                oldIndexschemaInspector, "test", INIT_SERIAL_NUM));
...
TEST_F(InitializerTest, require_that_initializer_can_setup_populate_of_several_attributes) {
    addOldConfig({"a", "b", "c", "d"}, {"a", "b"}).addNewConfig({"a", "b", "c", "d"}, {"a", "b", "c", "d"}).init();
...
TEST_F(InitializerTest,
       require_that_removed_attribute_aspect_when_also_index_field_does_NOT_require_document_field_populate) {
...
TEST_F(InitializerTest, require_that_removing_attribute_aspect_from_tensor_field_require_document_field_populate) {
    addOldConfig({"tensor"}, {"tensor"}).addNewConfig({"tensor"}, {}).init();
    assertAttributes({});
    assertFields({"tensor"});
}
...
TEST(
    InterruptedTest,
    require_that_added_attribute_aspect_with_flushed_attribute_after_interruptted_reprocessing_does_not_require_attribute_populate) {
    {
        ...
        auto av = AttributeFactory::createAttribute(snapshotdir + "/a", Config(BasicType::STRING));
        av->save();
    }
    Fixture f;
...
TEST_F(InitializerTest,
       require_that_removed_attribute_aspect_from_struct_field_does_not_require_document_field_populate) {
