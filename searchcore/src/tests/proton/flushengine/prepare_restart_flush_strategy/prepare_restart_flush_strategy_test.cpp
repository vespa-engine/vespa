...
struct SimpleFlushTarget : public test::DummyFlushTarget {
    ...
    SimpleFlushTarget(const std::string& name, const Type& type, SerialNum flushedSerial_, uint64_t approxDiskBytes_,
    ...
    [[nodiscard]] SerialNum getFlushedSerialNum() const override { return flushedSerial; }
    [[nodiscard]] uint64_t getApproxBytesToWriteToDisk() const override { return approxDiskBytes; }
    [[nodiscard]] uint64_t get_approx_bytes_to_read_from_disk() const noexcept override {
        return approx_disk_read_bytes;
    }
    [[nodiscard]] double get_replay_operation_cost() const override { return replay_operation_cost; }
...
public:
    ...
    ~ContextsBuilder();
    ContextsBuilder& add(const std::string& handlerName, const std::string& targetName, IFlushTarget::Type targetType,
                         SerialNum flushedSerial, uint64_t approxDiskBytes, double replay_operation_cost) {
        IFlushHandler::SP handler = createAndGetHandler(handlerName);
        IFlushTarget::SP  target = std::make_shared<SimpleFlushTarget>(targetName, targetType, flushedSerial,
                                                                       approxDiskBytes, replay_operation_cost);
        _result.push_back(std::make_shared<FlushContext>(handler, target, 0));
        return *this;
    }
    ContextsBuilder& add(const std::string& handlerName, const std::string& targetName, SerialNum flushedSerial,
                         uint64_t approxDiskBytes, double replay_operation_cost = 0.0) {
        return add(handlerName, targetName, IFlushTarget::Type::FLUSH, flushedSerial, approxDiskBytes,
                   replay_operation_cost);
    }
    ContextsBuilder& add(const std::string& targetName, SerialNum flushedSerial, uint64_t approxDiskBytes,
                         double replay_operation_cost = 0.0) {
        return add("handler1", targetName, IFlushTarget::Type::FLUSH, flushedSerial, approxDiskBytes,
                   replay_operation_cost);
    }
    ContextsBuilder& addGC(const std::string& targetName, SerialNum flushedSerial, uint64_t approxDiskBytes,
                           double replay_operation_cost = 0.0) {
...
public:
    ...
    CandidatesBuilder& replayEnd(SerialNum replayEndSerial) {
        ...
        _tlsStats = flushengine::TlsStats(oldTlsStats.getNumBytes(), oldTlsStats.getFirstSerial(), replayEndSerial);
        return *this;
    }
...
void assertCosts(double tlsReplayBytesCost, double tlsReplayOperationsCost, double flushTargetsWriteCost,
                 const FlushTargetCandidates& candidates) {
...
flushengine::TlsStatsMap defaultTransactionLogStats() {
...
struct FlushStrategyFixture {
    ...
    explicit FlushStrategyFixture(const Config& config)
        : _tlsStatsMap(defaultTransactionLogStats()), strategy(config) {}
    FlushStrategyFixture() : FlushStrategyFixture(DEFAULT_CFG) {}
...
std::string toString(const FlushContext::List& flushContexts) {
...
TEST(PrepareRestartFlushStrategyTest, require_that_the_best_strategy_is_flushing_0_targets) {
    ...
    FlushContext::List   targets = f.getFlushTargets(
        ContextsBuilder().add("foo", 10, 167).add("bar", 10, 167).add("baz", 10, 167).build(), f._tlsStatsMap);
...
TEST(PrepareRestartFlushStrategyTest, require_that_the_best_strategy_is_flushing_all_targets) {
    FlushStrategyFixture f;
    FlushContext::List   targets = f.getFlushTargets(
        ContextsBuilder().add("foo", 10, 166).add("bar", 10, 166).add("baz", 10, 166).build(), f._tlsStatsMap);
    EXPECT_EQ("[bar,baz,foo]", toString(targets));
}

TEST(PrepareRestartFlushStrategyTest,
     require_that_the_best_strategy_is_flushing_all_targets_with_different_unflushed_serial) {
    FlushStrategyFixture f;
    FlushContext::List   targets = f.getFlushTargets(
        ContextsBuilder().add("foo", 10, 166).add("bar", 11, 166).add("baz", 12, 166).build(), f._tlsStatsMap);
...
TEST(PrepareRestartFlushStrategyTest, require_that_the_best_strategy_is_flushing_1_target) {
    FlushStrategyFixture f;
    FlushContext::List   targets = f.getFlushTargets(
        ContextsBuilder().add("foo", 10, 249).add("bar", 60, 125).add("baz", 60, 125).build(), f._tlsStatsMap);
...
TEST(PrepareRestartFlushStrategyTest, require_that_the_best_strategy_is_flushing_2_targets) {
    FlushStrategyFixture f;
    FlushContext::List   targets = f.getFlushTargets(
        ContextsBuilder().add("foo", 10, 124).add("bar", 11, 124).add("baz", 60, 251).build(), f._tlsStatsMap);
...
TEST(PrepareRestartFlushStrategyTest, require_that_GC_flush_targets_are_removed) {
    FlushStrategyFixture f;
    FlushContext::List   targets = f.getFlushTargets(
        ContextsBuilder().addGC("foo", 10, 124).add("bar", 11, 124).add("baz", 60, 251).build(), f._tlsStatsMap);
    EXPECT_EQ("[bar]", toString(targets));
}

TEST(PrepareRestartFlushStrategyTest,
     require_that_flush_targets_for_different_flush_handlers_are_treated_independently) {
    FlushStrategyFixture f;
    ...
    FlushContext::List targets = f.getFlushTargets(ContextsBuilder()
                                                       .add("handler1", "foo", 10, 249)
                                                       .add("handler1", "bar", 60, 251)
                                                       .add("handler2", "baz", 10, 499)
                                                       .add("handler2", "quz", 60, 499)
                                                       .build(),
                                                   f._tlsStatsMap);
