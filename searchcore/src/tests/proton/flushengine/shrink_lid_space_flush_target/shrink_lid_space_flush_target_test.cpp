...
public:
    MyLidSpace() noexcept : _canShrink(true), _canFree(16) {}
    ~MyLidSpace() override = default;

    void compactLidSpace(uint32_t wantedDocLidLimit) override { (void)wantedDocLidLimit; }

    bool canShrinkLidSpace() const override { return _canShrink; }

    size_t getEstimatedShrinkLidSpaceGain() const override { return _canShrink ? _canFree : 0; }
...
ShrinkLidSpaceFlushTargetTest::ShrinkLidSpaceFlushTargetTest()
    : ::testing::Test(),
      ...
      _ft(std::make_shared<ShrinkLidSpaceFlushTarget>("name", IFlushTarget::Type::GC,
                                                      IFlushTarget::Component::ATTRIBUTE, 10, vespalib::system_time(),
                                                      _lidSpace)) {
}
...
TEST_F(ShrinkLidSpaceFlushTargetTest,
       require_that_flush_target_returns_no_estimated_memory_gain_when_not_able_to_flush) {
