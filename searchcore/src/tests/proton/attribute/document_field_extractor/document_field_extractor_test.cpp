...
std::unique_ptr<FieldValue> makeIntArray(const std::vector<int32_t>& array) {
...
std::unique_ptr<FieldValue> makeStringArray(const std::vector<std::string>& array) {
...
std::unique_ptr<FieldValue> makeIntWeightedSet(const std::vector<std::pair<int32_t, int32_t>>& array) {
...
std::unique_ptr<FieldValue> makeStringWeightedSet(const std::vector<std::pair<std::string, int32_t>>& array) {
...
struct WrapValue {
    ...
    WrapValue(std::unique_ptr<FieldValue> value) : _value(std::move(value)) {}
...
struct FixtureBase {
    ...
    FieldPath makeFieldPath(const std::string& path) {
        FieldPath fieldPath;
        try {
    ...
    WrapValue extract(const std::string& path) {
...
struct SimpleFixture : public FixtureBase {
    SimpleFixture(bool byteWeight = false) : FixtureBase(byteWeight) {
...
struct StructFixtureBase : public FixtureBase {
    ...
    StructFixtureBase(bool byteWeight) : FixtureBase(byteWeight), structFieldType("struct") {
        structFieldType.addField(weightField);
        structFieldType.addField(nameField);
    }

    std::unique_ptr<StructFieldValue> makeStruct() { return std::make_unique<StructFieldValue>(structFieldType); }

    std::unique_ptr<StructFieldValue> makeStruct(int weight, const std::string& value) {
        auto ret = makeStruct();
    ...
    std::unique_ptr<StructFieldValue> makeStruct(int weight) {
        auto ret = makeStruct();
        ret->setValue(weightField, IntFieldValue(weight));
        return ret;
    }

    std::unique_ptr<StructFieldValue> makeStruct(const std::string& value) {
        auto ret = makeStruct();
        ret->setValue(nameField, StringFieldValue(value));
        return ret;
    }
...
struct PrimitiveMapFixture : public FixtureBase {
    ...
    void makeDoc(const MapVector& input) { FixtureBase::makeDoc()->setValue(mapField, *makeMap(input)); }
};
