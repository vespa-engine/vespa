...
struct AttributesStateExplorerTest : public ::testing::Test {
    ...
    void addAttribute(const std::string& name) { _mgr->addAttribute({name, AttributeUtils::getInt32Config()}, 1); }
    ...
    void addExtraAttribute(const std::string& name) { _mgr->addExtraAttribute(createInt32Attribute(name)); }
    ...
    void add_imported_attributes() {
        ...
        auto attr = ImportedAttributeVectorFactory::create(imported_name, get_reference_attribute(), _dms,
...
AttributesStateExplorerTest::AttributesStateExplorerTest() noexcept
    : _dirHandler(TEST_DIR),
      ...
      _parent_mgr(std::make_shared<AttributeManager>(
          TEST_DIR, "test.parent.subdb", TuneFileAttributes(), _fileHeaderContext,
          std::make_shared<search::attribute::Interlock>(), *_attribute_field_writer, _shared, _hwInfo)),
      _mgr(new AttributeManager(TEST_DIR, "test.subdb", TuneFileAttributes(), _fileHeaderContext,
                                std::make_shared<search::attribute::Interlock>(), *_attribute_field_writer, _shared,
                                _hwInfo)),
      _explorer(_mgr) {
