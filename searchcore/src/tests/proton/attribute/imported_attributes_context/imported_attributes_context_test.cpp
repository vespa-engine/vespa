...
std::shared_ptr<ReferenceAttribute> createReferenceAttribute(const std::string& name) {
...
AttributeVector::SP createTargetAttribute(const std::string& name) {
    return search::AttributeFactory::createAttribute(name, Config(BasicType::STRING));
}

void addDoc(AttributeVector& attr) {
    attr.addDocs(1);
    attr.commit();
}

bool hasActiveEnumGuards(AttributeVector& attr) {
    return std::async(std::launch::async, [&attr] { return attr.hasActiveEnumGuards(); }).get();
}

void assertGuards(AttributeVector& attr, Generation expCurrentGeneration, Generation exp_oldest_used_generation,
...
void addDocAndAssertGuards(std::string_view label, AttributeVector& attr, Generation expCurrentGeneration,
                           Generation expFirstUsedGeneration, bool expHasActiveEnumGuards) {
    SCOPED_TRACE(label);
    addDoc(attr);
...
protected:
    ...
    void addAttribute(const std::string& name) {
        auto attr = ImportedAttributeVectorFactory::create(
            name, createReferenceAttribute(name + "_ref"), std::shared_ptr<search::IDocumentMetaStoreContext>(),
            createTargetAttribute(name + "_target"),
            std::make_shared<const DocumentMetaStoreContext>(std::make_shared<bucketdb::BucketDBOwner>()), false);
        repo.add(name, attr);
    }
...
ImportedAttributesContextTest::ImportedAttributesContextTest()
    : repo(), ctx(std::make_unique<ImportedAttributesContext>(repo)) {
}
