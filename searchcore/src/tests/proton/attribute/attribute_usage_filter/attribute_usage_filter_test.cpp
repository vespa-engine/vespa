...
class MyListener : public IAttributeUsageAndLoadInfoListener {
public:
    AttributeUsageStatsAndLoadInfo stats;
    MyListener() : stats("", 0, 0) {}
    void notify_attribute_usage(AttributeUsageStatsAndLoadInfo stats_in) noexcept override {
...
protected:
    ...
    void setAttributeStats(AttributeUsageStatsAndLoadInfo stats) { filter.setAttributeStats(std::move(stats)); }
};
...
TEST_F(AttributeUsageFilterTest, listener_is_updated_when_attribute_stats_change) {
    AttributeUsageStatsAndLoadInfo stats("doctype", 1, 0);
    AddressSpaceUsage              usage;
    usage.set("my_comp", AddressSpace(12, 10, 15));
    stats.merge(usage, LoadMemoryUsage(), AttributeUsageStatsAndLoadInfo::SubDb::READY, "my_attr", "my_subdb");
    setAttributeStats(stats.clone());
    EXPECT_EQ(stats.usage_stats(), listener->stats.usage_stats());
}
