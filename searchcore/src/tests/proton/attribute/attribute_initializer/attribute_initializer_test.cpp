...
void saveAttr(const std::string& name, const Config& cfg, SerialNum serialNum, SerialNum createSerialNum,
              bool mutate_reserved_doc = false) {
...
Fixture::Fixture()
    : _dirHandler(test_dir), _diskLayout(AttributeDiskLayout::create(test_dir)), _factory(), _executor(1) {
}

Fixture::~Fixture() = default;

std::unique_ptr<AttributeInitializer> Fixture::createInitializer(AttributeSpec&&          spec,
                                                                 std::optional<SerialNum> serialNum) {
    return std::make_unique<AttributeInitializer>(_diskLayout->createAttributeDir(spec.getName()), "test.subdb",
                                                  std::move(spec), serialNum, _factory, _executor);
}
...
AttributeInitializerTest::AttributeInitializerTest() : ::testing::Test() {
}

AttributeInitializerTest::~AttributeInitializerTest() = default;

void AttributeInitializerTest::SetUpTestSuite() {
...
TEST_F(AttributeInitializerTest,
       require_that_predicate_attributes_will_not_be_initialized_with_future_created_attribute) {
...
TEST_F(AttributeInitializerTest,
       require_that_tensor_attributes_will_not_be_initialized_with_future_created_attribute) {
...
TEST_F(
    AttributeInitializerTest,
    require_that_transient_memory_usage_is_reported_for_first_time_posting_list_attribute_load_after_enabling_posting_lists) {
...
TEST_F(AttributeInitializerTest,
       require_that_transient_memory_usage_is_reported_for_normal_posting_list_attribute_load) {
...
TEST_F(AttributeInitializerTest,
       require_that_transient_memory_usage_is_reported_for_attribute_load_without_posting_list) {
    saveAttr("a", int32_wset, 10, 2);
    Fixture f;
