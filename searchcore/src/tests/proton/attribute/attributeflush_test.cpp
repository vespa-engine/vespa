...
public:
    TaskWrapper(Executor::Task::UP task, const GateSP& gate) : _task(std::move(task)), _gate(gate) {}
...
public:
    ...
    FlushHandler() : _executor(1), gate() {}
    ~FlushHandler();

    void doFlushing(Executor::Task::UP task) {
        if (!task) {
...
public:
    UpdaterTask(proton::AttributeManager& am) : _am(am) {}

    void startFlushing(uint64_t syncToken, FlushHandler& handler);
...
void UpdaterTask::startFlushing(uint64_t syncToken, FlushHandler& handler) {
...
void UpdaterTask::run() {
    ...
    for (uint32_t i = incDocs;
         i <= totalDocs || (flushCount + (flushedToken < needFlushToken) <= 2 && i <= totalDocsMax); i += incDocs)
    {
        uint32_t           startDoc = 0;
        uint32_t           lastDoc = 0;
        ...
        for (uint32_t j = i - incDocs; j < i; ++j) {
            if (j >= ag->getNumDocs()) {
                ...
                if (i % (totalDocs / 20) == 0) {
                    LOG(info, "addDocs(%u, %u, %u)", startDoc, lastDoc, ag->getNumDocs());
                }
            }
            ia.update(j, i);
        }
        ...
        if (needFlushToken > flushedToken + slowdownUpdateLim) {
            ...
            if (!slowedDown) {
                LOG(warning, "Slowing down updates due to slow flushing (slow disk ?)");
            }
            slowedDown = true;
        }
    }
...
BaseFixture::BaseFixture(const HwInfo& hwInfo)
    : _dirHandler(test_dir), _fileHeaderContext(), _attributeFieldWriter(), _shared(), _hwInfo(hwInfo) {
}
...
AttributeManagerFixture::AttributeManagerFixture(BaseFixture& bf)
    : _msp(std::make_shared<AttributeManager>(test_dir, "test.subdb", TuneFileAttributes(), bf._fileHeaderContext,
...
struct Fixture : public BaseFixture, public AttributeManagerFixture {
    ...
    Fixture(const HwInfo& hwInfo) : BaseFixture(hwInfo), AttributeManagerFixture(*static_cast<BaseFixture*>(this)) {}
    ~Fixture();
};
...
TEST(AttributeFlushTest, require_that_updater_and_flusher_can_run_concurrently) {
    ...
    for (size_t i = 0; i < info.snapshots().size(); ++i) {
        ...
        LOG(info, "Snapshot(%" PRIu64 ", %s)", snap.syncToken, snap.dirName.c_str());
        if (snap.syncToken > 0) {
            EXPECT_TRUE(snap.valid);
            std::string         baseFileName = "flush/a1/" + snap.dirName + "/a1";
            AttributeVector::SP attr = AttributeFactory::createAttribute(baseFileName, getInt32Config());
...
TEST(AttributeFlushTest, require_that_flushable_attribute_reports_memory_usage) {
    ...
    EXPECT_EQ(av->getStatus().getUsed(), fa->getApproxMemoryGain().getBefore() + 0lu);
    // attributes stay in memory
    EXPECT_EQ(fa->getApproxMemoryGain().getBefore(), fa->getApproxMemoryGain().getAfter());
}
...
TEST(AttributeFlushTest, require_that_cleanup_is_performed_after_flush) {
    ...
    av->addDocs(1);
    av->commit(CommitParam(30, CommitParam::UpdateStats::SKIP));
    ...
    FlushableAttribute fa(av, diskLayout->getAttributeDir("a6"), TuneFileAttributes(), f._fileHeaderContext,
...
TEST(AttributeFlushTest, require_that_flush_stats_are_updated) {
    ...
    av->addDocs(1);
    av->commit(CommitParam(100, CommitParam::UpdateStats::SKIP));
...
TEST(AttributeFlushTest, require_that_only_one_flusher_can_run_at_the_same_time) {
    ...
    av->addDocs(10000);
    av->commit(CommitParam(9, CommitParam::UpdateStats::SKIP));
    IFlushTarget::SP ft = am.getFlushable("a8");
    (static_cast<FlushableAttribute*>(ft.get()))->setCleanUpAfterFlush(false);
    vespalib::ThreadStackExecutor exec(16);

    for (size_t i = 10; i < 100; ++i) {
        av->commit(CommitParam(i, CommitParam::UpdateStats::SKIP));
...
TEST(AttributeFlushTest, require_that_shrink_works) {
    ...
    av->commit(CommitParam(50, CommitParam::UpdateStats::SKIP));
...
void require_that_flushed_attribute_can_be_loaded(const HwInfo& hwInfo, const std::string& label) {
    ...
    size_t             exp_reserved_memory_for_flush = sizeof(int32_t) * (numDocs + 1);
    ...
    if (hwInfo.disk().slow()) {
        ...
        exp_reserved_memory_for_flush += 2 * headerSize + (sizeof(int32_t) + sizeof(EntryRef)) * (numDocs + 1);
    }
    {
        AttributeManagerFixture amf(f);
        AttributeManager&       am = amf._m;
        ...
        EXPECT_EQ(exp_reserved_memory_for_flush, ft->reserved_memory_for_flush());
        ft->initFlush(200, std::make_shared<search::FlushToken>())->run();
    }
    {
        AttributeManagerFixture amf(f);
...
TEST(AttributeFlushTest, require_that_flushed_attribute_can_be_loaded) {
    require_that_flushed_attribute_can_be_loaded(
        HwInfo(HwInfo::Disk(0, false, false), HwInfo::Memory(0), HwInfo::Cpu(0)), "fast-disk");
    require_that_flushed_attribute_can_be_loaded(
        HwInfo(HwInfo::Disk(0, true, false), HwInfo::Memory(0), HwInfo::Cpu(0)), "slow-disk");
}
