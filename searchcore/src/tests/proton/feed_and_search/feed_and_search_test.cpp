...
void testSearch(Searchable& source, const string& term, uint32_t doc_id) {
    ...
    Blueprint::UP    result =
        source.createBlueprint(requestContext, FieldSpecList().add(FieldSpec(field_name, 0, handle)), node, mdl);
...
TEST(FeedAndSearchTest, require_that_memory_index_can_be_dumped_and_searched) {
    ...
    DocBuilder                    doc_builder(
        [](auto& b, auto& doc) noexcept { doc.addField(field_name, b.primitiveType(DataType::T_STRING)); });
    ...
    DummyFileHeaderContext   fileHeaderContext;
    {
        ...
        IndexBuilder index_builder(schema, index_dir, docIdLimit, num_words, fieldLengthInspector, tuneFileIndexing,
                                   fileHeaderContext);
