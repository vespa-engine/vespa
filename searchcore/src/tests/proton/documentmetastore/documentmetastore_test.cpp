...
std::shared_ptr<bucketdb::BucketDBOwner> createBucketDB() {
    return std::make_shared<bucketdb::BucketDBOwner>();
}

void assertPut(const BucketId& bucketId, uint64_t timestamp, uint32_t lid, const DocumentId& docId,
...
bool compare(const GlobalId& lhs, const GlobalId& rhs) {
    EXPECT_EQ(lhs.toString(), rhs.toString());
    return (lhs.toString() == rhs.toString());
}

void assertGid(const GlobalId& exp, uint32_t lid, const DocumentMetaStore& dms) {
    GlobalId act;
    EXPECT_TRUE(dms.getGid(lid, act));
    EXPECT_TRUE(compare(exp, act));
}

void assertGid(const GlobalId& exp, uint32_t lid, const DocumentMetaStore& dms, const BucketId& expBucketId,
...
void assertLid(uint32_t exp, const GlobalId& gid, const DocumentMetaStore& dms) {
    uint32_t act;
    EXPECT_TRUE(dms.getLid(gid, act));
    EXPECT_EQ(exp, act);
}

void assertMetadata(const DocumentMetadata& exp, const DocumentMetadata& act) {
...
void assertActiveLids(const BoolVector& exp, const search::BitVector& act) {
    // lid 0 is reserved
...
void assertWhiteList(const SimpleResult& exp, Blueprint::UP whiteListBlueprint, bool strict, uint32_t docIdLimit) {
    MatchDataLayout mdl;
...
void assertSearchResult(const SimpleResult& exp, const DocumentMetaStore& dms, const std::string& term,
                        const QueryTermSimple::Type& termType, bool strict, uint32_t docIdLimit = 100) {
...
void assertBucketInfo(uint32_t expDocCount, uint32_t expMetaCount, const BucketInfo& act) {
...
BucketId        bucketId1(minNumBits, gid1.convertToBucketId().getRawId());
BucketId        bucketId2(minNumBits, gid2.convertToBucketId().getRawId());
BucketId        bucketId3(minNumBits, gid3.convertToBucketId().getRawId());
BucketId        bucketId4(minNumBits, gid4.convertToBucketId().getRawId());
BucketId        bucketId5(minNumBits, gid5.convertToBucketId().getRawId());
Timestamp       time1(1u);
Timestamp       time2(2u);
Timestamp       time3(42u);
Timestamp       time4(82u);
...
uint32_t addDoc(DocumentMetaStore& dms, const DocumentId& docid, const BucketId& bid, Timestamp timestamp,
                uint32_t docSize = 1) {
    auto&  gid = docid.getGlobalId();
    Result inspect = dms.inspect(gid, 0u);
    PutRes putRes;
...
uint32_t addDoc(DocumentMetaStore& dms, const DocumentId& docid, Timestamp timestamp) {
    auto&    gid = docid.getGlobalId();
    BucketId bid(minNumBits, gid.convertToBucketId().getRawId());
    return addDoc(dms, docid, bid, timestamp);
}

void putDoc(DocumentMetaStore& dms, const DocumentId& docid, uint32_t lid, Timestamp timestamp = Timestamp()) {
    auto&    gid = docid.getGlobalId();
    BucketId bid(minNumBits, gid.convertToBucketId().getRawId());
    uint32_t docSize = 1;
...
DocumentId createDocId(uint32_t lid) {
    DocumentId docId(vespalib::make_string("id:ns:testdoc::%u", lid));
    return docId;
}

DocumentId createDocId(uint32_t userId, uint32_t lid) {
...
TEST(DocumentMetaStoreTest, can_store_bucket_id_and_timestamp) {
    ...
    for (uint32_t lid = 1; lid <= numLids; ++lid) {
        ...
        BucketId   bucketId(gid.convertToBucketId());
        bucketId.setUsedBits(numBucketBits);
        assertGid(gid, lid, dms, bucketId, Timestamp(lid + timestampBias));
...
TEST(DocumentMetaStoreTest, gids_can_be_saved_and_loaded) {
    ...
    for (uint32_t lid = 1; lid <= numLids; ++lid) {
        ...
        if (std::count(removeLids.begin(), removeLids.end(), lid) == 0) {
            assertGid(gid, lid, dms2, bucketId, Timestamp(lid + timestampBias));
    ...
    for (size_t i = 0; i < removeLids.size(); ++i) {
        ...
        auto&      gid = docid.getGlobalId();
        BucketId   bucketId(numBucketBits, gid.convertToBucketId().getRawId());
        // re-use removeLid[i]
...
TEST(DocumentMetaStore, stats_are_updated) {
    ...
    LOG(info, "stats after 2 gids added: allocated %d, used is %d > %d (3 * %d)",
        static_cast<int>(dms.getStatus().getAllocated()), static_cast<int>(dms.getStatus().getUsed()),
        static_cast<int>(3 * perGidUsed), static_cast<int>(perGidUsed));
}
...
TEST(DocumentMetaStoreTest, can_put_and_remove_before_free_list_construct) {
    ...
    EXPECT_THROW(dms.put(docid1, bucketId1, time1, docSize1, 2, 0u).ok(), vespalib::IllegalStateException);
    EXPECT_THROW(dms.put(docid5, bucketId5, time5, docSize5, 1, 0u).ok(), vespalib::IllegalStateException);
    assertLid(1, gid1, dms);
    assertGid(gid1, 1, dms);
    EXPECT_EQ(2u, dms.getNumUsedLids());
    EXPECT_EQ(5u, dms.getNumDocs());
...
void requireThatBasicBucketInfoWorks() {
    DocumentMetaStore dms(createBucketDB());
...
TEST(DocumentMetaStoreTest, can_retrieve_list_of_lids_from_bucket_id) {
    ...
    for (uint32_t lid = 1; lid <= numLids; ++lid) {
        const auto docid = createDocId(lid);
        auto&      gid = docid.getGlobalId();
        BucketId   bucketId(bucketBits, gid.convertToBucketId().getRawId());
    ...
    for (const auto& e : m) {
        ...
        LOG(info, "Verify that bucket id '%s' has %zu lids", bucketId.toString().c_str(), expLids.size());
        LidVector actLids;
...
struct Comparator {
    bool operator()(const DocumentMetadata& lhs, const DocumentMetadata& rhs) const { return lhs.lid < rhs.lid; }
};
...
UserDocFixture::UserDocFixture() : UserDocFixture(false) {
}
...
void UserDocFixture::addDocumentIds(size_t numGids) {
...
TEST(DocumentMetaStoreTest, can_retrieve_list_of_metadata_from_bucket_id) {
    ...
    { // verify bucket 1
        ...
        assertMetadata(
            DocumentMetadata(1, Timestamp(101), f.bid1, f.docids[0].getGlobalId(), false, f.docids[0].toString()),
            result[0]);
        assertMetadata(
            DocumentMetadata(2, Timestamp(102), f.bid1, f.docids[1].getGlobalId(), false, f.docids[1].toString()),
            result[1]);
        assertMetadata(
            DocumentMetadata(4, Timestamp(104), f.bid1, f.docids[3].getGlobalId(), false, f.docids[3].toString()),
            result[2]);
        assertMetadata(
            DocumentMetadata(5, Timestamp(105), f.bid1, f.docids[4].getGlobalId(), false, f.docids[4].toString()),
            result[3]);
    }
    { // verify bucket 2
        DocumentMetadata::Vector result;
        ...
        assertMetadata(
            DocumentMetadata(3, Timestamp(103), f.bid2, f.docids[2].getGlobalId(), false, f.docids[2].toString()),
            result[0]);
        assertMetadata(
            DocumentMetadata(6, Timestamp(106), f.bid2, f.docids[5].getGlobalId(), false, f.docids[5].toString()),
            result[1]);
        assertMetadata(
            DocumentMetadata(7, Timestamp(107), f.bid2, f.docids[6].getGlobalId(), false, f.docids[6].toString()),
            result[2]);
    }
}
...
TEST(DocumentMetaStoreTest, whitelist_blueprint_is_created) {
    ...
    f.dms.setBucketState(f.bid1, true);
    assertWhiteList(SimpleResult().addHit(1).addHit(2).addHit(4).addHit(5), f.dms.createWhiteListBlueprint(), true,
                    f.dms.getCommittedDocIdLimit());

    f.dms.setBucketState(f.bid2, true);
    assertWhiteList(SimpleResult().addHit(1).addHit(2).addHit(3).addHit(4).addHit(5).addHit(6).addHit(7),
                    f.dms.createWhiteListBlueprint(), true, f.dms.getCommittedDocIdLimit());
}
...
void MyBucketCreateListener::notifyCreateBucket(const bucketdb::Guard&, const document::BucketId& bucket) {
...
struct SplitAndJoinEmptyFixture {
    ...
    BucketInfo getInfo(const BucketId& bid) const { return dms.getBucketDB().takeGuard()->get(bid); }
    ...
    void assertBucketDBIntegrity() { ASSERT_TRUE(dms.getBucketDB().takeGuard()->validateIntegrity()); }
};
...
struct SplitAndJoinFixture : public SplitAndJoinEmptyFixture {
    ...
    void insertGids1() {
        ...
        for (size_t i = 0; i < gids.size(); ++i) {
            EXPECT_TRUE(dms.put(gids[i].docid, gids[i].bid1, Timestamp(0), docSize, gids[i].lid, 0u).ok());
        }
        dms.commit();
    }
    void insertGids2() {
        uint32_t docSize = 1;
        for (size_t i = 0; i < gids.size(); ++i) {
            EXPECT_TRUE(dms.put(gids[i].docid, gids[i].bid2, Timestamp(0), docSize, gids[i].lid, 0u).ok());
        }
        dms.commit();
    }

    void insertGids1Mostly(const BucketId& alt) {
        uint32_t docSize = 1;
        for (size_t i = 0; i < gids.size(); ++i) {
    ...
    void insertGids2Mostly(const BucketId& alt) {
        uint32_t docSize = 1;
        for (size_t i = 0; i < gids.size(); ++i) {
            const GlobalIdEntry& g(gids[i]);
...
BoolVector getBoolVector(const GlobalIdVector& gids, size_t sz) {
...
BoolVector getBoolVectorFiltered(const GlobalIdVector& gids, size_t sz, const BucketId& skip) {
    BoolVector retval(sz);
    for (size_t i = 0; i < gids.size(); ++i) {
        const GlobalIdEntry& g(gids[i]);
        uint32_t             lid(g.lid);
...
TEST(DocumentMetaStoreTest, bucket_info_is_correct_after_split) {
    ...
    assertBucketInfo(f.bid10Gids->size(), f.bid10Gids->size(), nbi10);
    ...
    EXPECT_EQ(bi11.getEntryCount(), bi21.getEntryCount() + bi23.getEntryCount());
    EXPECT_EQ(bi11.getDocumentCount(), bi21.getDocumentCount() + bi23.getDocumentCount());
...
TEST(DocumentMetaStoreTest, active_state_is_preserved_after_split) {
    ...
    { // active bucket
        ...
        assertActiveLids(getBoolVector(*f.bid10Gids, 31), f.dms.getActiveLids());
    ...
    { // non-active source, active overlapping target1
        ...
        EXPECT_TRUE(f.getInfo(f.bid20).isActive());
        assertActiveLids(getBoolVector(*f.bid30Gids, 31), f.dms.getActiveLids());
    ...
    { // non-active source, active overlapping target2
        ...
        EXPECT_TRUE(f.getInfo(f.bid22).isActive());
        assertActiveLids(getBoolVector(*f.bid32Gids, 31), f.dms.getActiveLids());
...
TEST(DocumentMetaStoreTest, bucket_info_is_correct_after_join) {
    ...
    EXPECT_EQ(bi21.getEntryCount() + bi23.getEntryCount(), bi11.getEntryCount());
    EXPECT_EQ(bi21.getDocumentCount() + bi23.getDocumentCount(), bi11.getDocumentCount());
...
RemovedFixture::RemovedFixture()
    : _bucketDB(createBucketDB()),
      dms(_bucketDB, DocumentMetaStore::getFixedName(), search::GrowStrategy(), SubDbType::REMOVED),
      _bucketDBHandler(dms.getBucketDB()) {
    _bucketDBHandler.addDocumentMetaStore(&dms, 0);
}
...
void assertLidBloat(uint32_t expBloat, uint32_t lidLimit, uint32_t usedLids) {
...
void populate(uint32_t endLid, DocumentMetaStore& dms) {
...
void remove(uint32_t startLid, uint32_t shrinkTarget, DocumentMetaStore& dms) {
...
void addLid(DocumentMetaStore& dms, uint32_t lid, uint32_t docSize = 1) {
    const auto docid = createDocId(lid);
    auto&      gid = docid.getGlobalId();
    BucketId   bucketId(gid.convertToBucketId());
    bucketId.setUsedBits(numBucketBits);
...
void assertSize(DocumentMetaStore& dms, uint32_t lid, uint32_t expSize) {
    EXPECT_TRUE(dms.validLid(lid));
    const auto& metadata = dms.getRawMetadata(lid);
    EXPECT_EQ(expSize, metadata.getDocSize());
}

void removeLid(DocumentMetaStore& dms, uint32_t lid) {
    dms.remove(lid, 0u);
    dms.removes_complete({lid});
}

void assertCompact(DocumentMetaStore& dms, uint32_t docIdLimit, uint32_t committedDocIdLimit, uint32_t compactTarget,
...
void assertShrink(DocumentMetaStore& dms, uint32_t shrinkTarget, uint32_t numUsedLids) {
    dms.shrinkLidSpace();
...
void assertLidGidNotFound(uint32_t lid, DocumentMetaStore& dms) {
    const auto docid = createDocId(lid);
    auto&      gid = docid.getGlobalId();
    uint32_t   resultLid;
    GlobalId   resultGid;
...
public:
    ...
    MockOperationListener() noexcept : remove_batch_cnt(0), remove_cnt(0) {}
