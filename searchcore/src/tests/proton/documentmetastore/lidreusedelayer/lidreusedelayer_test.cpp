...
bool assertThreadObserver(uint32_t masterExecuteCnt, uint32_t indexExecuteCnt, uint32_t summaryExecuteCnt,
...
public:
    ...
    MyMetaStore() : _freeListActive(false), _removes_complete_count(0), _removes_complete_lids(0) {}

    ~MyMetaStore() override = default;

    Result inspectExisting(const GlobalId&, uint64_t) override { return Result(); }

    Result inspect(const GlobalId&, uint64_t) override { return Result(); }

    Result put(const DocumentId&, const BucketId&, Timestamp, uint32_t, DocId, uint64_t) override { return Result(); }

    bool updateMetadata(DocId, const BucketId&, Timestamp) override { return true; }
    ...
    const RawDocumentMetadata& getRawMetadata(DocId) const override { LOG_ABORT("should not be reached"); }

    bool getFreeListActive() const override { return _freeListActive; }

    bool assertWork(uint32_t exp_removes_complete_count, uint32_t exp_removes_complete_lids) const {
        bool failed = false;
...
protected:
    ...
    void cycledLids(const std::vector<uint32_t>& lids) { _store.removes_complete(lids); }
...
void LidReuseDelayerTest::performCycleLids(const std::vector<uint32_t>&      lids,
