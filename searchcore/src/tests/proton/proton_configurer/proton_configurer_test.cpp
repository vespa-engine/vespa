...
struct DBConfigFixture {
    ...
    static std::shared_ptr<const OnnxModels> buildOnnxModels() { return std::make_shared<OnnxModels>(); }

    DocumentDBConfig::SP getConfig(int64_t generation, std::shared_ptr<DocumenttypesConfig> documentTypes,
                                   std::shared_ptr<const DocumentTypeRepo> repo, const std::string& configId,
                                   const std::string& docTypeName) {
        return std::make_shared<DocumentDBConfig>(
            generation, std::make_shared<RankProfilesConfig>(_rankProfilesBuilder), buildRankingConstants(),
            buildRankingExpressions(), buildOnnxModels(), std::make_shared<IndexschemaConfig>(_indexschemaBuilder),
            std::make_shared<AttributesConfig>(_attributesBuilder), std::make_shared<SummaryConfig>(_summaryBuilder),
            std::make_shared<JuniperrcConfig>(_juniperrcBuilder), std::move(documentTypes), std::move(repo),
            std::make_shared<ImportedFieldsConfig>(_importedFieldsBuilder), std::make_shared<TuneFileDocumentDB>(),
            buildSchema(), std::make_shared<DocumentDBMaintenanceConfig>(), search::LogDocumentStore::Config(),
            ThreadingServiceConfig::make(), AllocConfig::makeDefault(), DocumentMetaStoreConfig::make(), configId,
            docTypeName);
    }
};
...
struct ConfigFixture {
    ...
    void removeDocType(const std::string& name) {
        for (auto it(_documenttypesBuilder.documenttype.begin()), mt(_documenttypesBuilder.documenttype.end());
             it != mt; it++)
        {
        ...
        for (auto it(_protonBuilder.documentdb.begin()), mt(_protonBuilder.documentdb.end()); it != mt; it++) {
        ...
        for (auto it(_bucketspacesBuilder.documenttype.begin()), mt(_bucketspacesBuilder.documenttype.end());
             it != mt; ++it)
        {
            if (it->name == name) {
    ...
    BootstrapConfig::SP getBootstrapConfig(int64_t generation) const {
        return std::make_shared<BootstrapConfig>(
            generation, std::make_shared<DocumenttypesConfig>(_documenttypesBuilder),
            std::make_shared<DocumentTypeRepo>(_documenttypesBuilder), std::make_shared<ProtonConfig>(_protonBuilder),
            std::make_shared<FiledistributorrpcConfig>(), std::make_shared<BucketspacesConfig>(_bucketspacesBuilder),
    ...
    std::shared_ptr<ProtonConfigSnapshot> getConfigSnapshot() {
        ...
        for (auto& db : _dbConfig) {
            ...
            dbconfigs.insert(std::make_pair(
                name, db.second->getConfig(generation, doctypes, repo, _configId + "/" + db.first, db.first)));
        }
...
struct MyDocumentDBConfigOwner : public DocumentDBConfigOwner {
    ...
    MyDocumentDBConfigOwner(const std::string& name, document::BucketSpace bucket_space,
                            MyProtonConfigurerOwner& owner)
        : DocumentDBConfigOwner(), _name(name), _bucket_space(bucket_space), _owner(owner) {}
...
struct MyLog {
    ...
    void appendLog(const std::string& logEntry) { _log.emplace_back(logEntry); }
};

MyLog::~MyLog() = default;

struct MyProtonConfigurerOwner : public IProtonConfigurerOwner, public MyLog {
    vespalib::ThreadStackExecutor                                   _executor;
    std::map<DocTypeName, std::shared_ptr<MyDocumentDBConfigOwner>> _dbs;

    MyProtonConfigurerOwner() : IProtonConfigurerOwner(), MyLog(), _executor(1), _dbs() {}
...
void MyDocumentDBConfigOwner::reconfigure(DocumentDBConfig::SP config) {
...
struct MyProtonDiskLayout : public IProtonDiskLayout {
    ...
    explicit MyProtonDiskLayout(MyLog& myLog) : _log(myLog) {}
...
protected:
    ProtonConfigurerTest()
        : _owner(), _config("test"), _diskLayout(), _configurer(_owner._executor, _owner, _diskLayout) {
        _diskLayout = std::make_unique<MyProtonDiskLayout>(_owner);
    }
    ~ProtonConfigurerTest() override;

    void assertLog(const std::vector<std::string>& expLog) { EXPECT_EQ(expLog, _owner._log); }
    void sync() { _owner.sync(); }
    void addDocType(const std::string& name, const std::string& bucket_space = "default") {
        _config.addDocType(name, bucket_space);
    }
    ...
    void disableReconfig() { _configurer.setAllowReconfig(false); }
};
...
TEST_F(ProtonConfigurerTest, require_that_new_config_can_be_unblocked) {
    ...
    reconfigure();
    allowReconfig();
    assertLog({"initial dbs _alwaysthere_", "apply config 2", "add db _alwaysthere_ 2", "apply config 3",
               "reconf db _alwaysthere_ 3"});
}
...
TEST_F(ProtonConfigurerTest, require_that_we_can_add_document_db) {
    ...
    reconfigure();
    assertLog({"initial dbs _alwaysthere_", "apply config 2", "add db _alwaysthere_ 2", "apply config 3",
               "reconf db _alwaysthere_ 3", "add db foobar 3"});
}
...
TEST_F(ProtonConfigurerTest, require_that_we_can_remove_document_db) {
    ...
    reconfigure();
    assertLog({"initial dbs _alwaysthere_,foobar", "apply config 2", "add db _alwaysthere_ 2", "add db foobar 2",
               "apply config 3", "reconf db _alwaysthere_ 3", "remove db foobar", "remove dbdir foobar"});
}
...
TEST_F(ProtonConfigurerTest, require_that_document_db_adds_and_reconfigs_are_intermingled) {
    ...
    reconfigure();
    assertLog({"initial dbs _alwaysthere_,foobar", "apply config 2", "add db _alwaysthere_ 2", "add db foobar 2",
               "apply config 3", "reconf db _alwaysthere_ 3", "add db abar 3", "reconf db foobar 3",
               "add db zbar 3"});
}
...
TEST_F(ProtonConfigurerTest, require_that_document_db_removes_are_applied_at_end) {
    ...
    removeDocType("abar");
    reconfigure();
    assertLog({"initial dbs _alwaysthere_,abar,foobar", "apply config 2", "add db _alwaysthere_ 2", "add db abar 2",
               "add db foobar 2", "apply config 3", "reconf db _alwaysthere_ 3", "reconf db foobar 3",
               "remove db abar", "remove dbdir abar"});
}
...
TEST_F(ProtonConfigurerTest, require_that_bucket_space_for_document_type_change_exits) {
    ...
    allowReconfig();
    EXPECT_EXIT(reconfigure(), ::testing::ExitedWithCode(1),
                "Bucket space for document type globaldoc changed from default to global");
}
