...
void GroupingContext::deserialize(const char* groupSpec, uint32_t groupSpecLen) {
...
size_t GroupingContext::countFS4Hits() {
    size_t numFs4Hits(0);
...
void GroupingContext::setDistributionKey(uint32_t distributionKey) {
    for (auto& g : _groupingList) {
...
GroupingContext::GroupingContext(const BitVector& validLids, const std::atomic<steady_time>& now_ref,
                                 vespalib::steady_time timeOfDoom, const char* groupSpec, uint32_t groupSpecLen)
    : GroupingContext(validLids, now_ref, timeOfDoom) {
    deserialize(groupSpec, groupSpecLen);
}

GroupingContext::GroupingContext(const BitVector& validLids, const std::atomic<steady_time>& now_ref,
                                 vespalib::steady_time timeOfDoom)
    : _validLids(validLids), _now_ref(now_ref), _timeOfDoom(timeOfDoom), _os(), _groupingList() {
}
...
void GroupingContext::addGrouping(std::shared_ptr<Grouping> g) {
    _groupingList.push_back(std::move(g));
}

void GroupingContext::serialize() {
...
bool GroupingContext::needRanking() const noexcept {
...
void GroupingContext::aggregate(Grouping& grouping, uint32_t docId, HitRank rank) const {
...
void GroupingContext::aggregate(Grouping& grouping, const BitVector* bVec, unsigned int lidLimit) const {
    for (uint32_t d(bVec->getFirstTrueBit()); (d < lidLimit) && !hasExpired(); d = bVec->getNextTrueBit(d + 1)) {
        aggregate(grouping, d, 0.0);
    }
}
void GroupingContext::aggregate(Grouping& grouping, const BitVector* bVec, unsigned int lidLimit,
                                unsigned int topN) const {
    for (uint32_t d(bVec->getFirstTrueBit()), i(0); (d < lidLimit) && (i < topN) && !hasExpired();
         d = bVec->getNextTrueBit(d + 1), i++)
    {
        aggregate(grouping, d, 0.0);
    }
}
