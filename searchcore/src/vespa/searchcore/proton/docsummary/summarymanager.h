...
public:
    ...
    public:
        SummarySetup(const std::string& baseDir, const SummaryConfig& summaryCfg, const JuniperrcConfig& juniperCfg,
                     ...
                     std::shared_ptr<const document::DocumentTypeRepo> repo, const search::index::Schema& schema);

        search::docsummary::IDocsumWriter& getDocsumWriter() const override { return *_docsumWriter; }
        const search::docsummary::ResultConfig& getResultConfig() override {
            return *_docsumWriter->GetResultConfig();
        }
...
public:
    ...
    SummaryManager(vespalib::Executor& shared_executor, const search::LogDocumentStore::Config& summary,
                   const search::GrowStrategy& growStrategy, const std::string& baseDir,
                   const search::TuneFileSummary&           tuneFileSummary,
                   const search::common::FileHeaderContext& fileHeaderContext,
                   search::transactionlog::SyncProxy& tlSyncer, std::shared_ptr<search::IBucketizer> bucketizer,
                   std::shared_ptr<const search::IDocumentIdProvider> document_id_provider);
    ...
    ISummarySetup::SP createSummarySetup(const SummaryConfig& summaryCfg, const JuniperrcConfig& juniperCfg,
