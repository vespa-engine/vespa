...
TaskRunner::TaskRunner(vespalib::Executor& executor) : _executor(executor), _runningTasks(0u) {
}
...
void TaskRunner::setTaskRunning(InitializerTask& task) {
    // run by context executor
    task.setRunning();
    ++_runningTasks;
}

void TaskRunner::setTaskDone(InitializerTask& task, Context::SP context) {
    // run by context executor
    task.setDone();
    --_runningTasks;
    pollTask(context);
...
void TaskRunner::internalRunTasks(const TaskList& taskList, Context::SP context) {
    // run by context executor
...
void TaskRunner::runTask(InitializerTask::SP task) {
...
void TaskRunner::pollTask(Context::SP context) {
    // run by context executor
    if (context->done()) {
        return;
    }
    ...
    std::sort(readyTasks.begin(), readyTasks.end(), [](const auto& a, const auto& b) -> bool {
        LoadOrderCompare load_order_compare;
        return load_order_compare(a->get_load_memory_usage(), b->get_load_memory_usage());
    });
    internalRunTasks(readyTasks, context);
}

void TaskRunner::runTask(InitializerTask::SP rootTask, vespalib::Executor& contextExecutor,
