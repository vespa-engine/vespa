...
DocumentDBExplorer::DocumentDBExplorer(std::shared_ptr<DocumentDB> docDb) : _docDb(std::move(docDb)) {
}

DocumentDBExplorer::~DocumentDBExplorer() = default;

void DocumentDBExplorer::get_state(const Inserter& inserter, bool full) const {
    (void)full;
...
std::vector<std::string> DocumentDBExplorer::get_children_names() const {
    return {SUB_DB, THREADING_SERVICE, BUCKET_DB, MAINTENANCE_CONTROLLER};
}

std::unique_ptr<StateExplorer> DocumentDBExplorer::get_child(std::string_view name) const {
    if (name == SUB_DB) {
