...
void convertRunningJobsToSlime(const std::vector<MaintenanceJobRunner::SP>& jobs, Cursor& array) {
...
void convertAllJobsToSlime(const std::vector<MaintenanceJobRunner::SP>& jobs, Cursor& array) {
    for (const auto& jobRunner : jobs) {
        Cursor&                object = array.addObject();
...
MaintenanceControllerExplorer::MaintenanceControllerExplorer(std::vector<MaintenanceJobRunner::SP> jobs)
    : _jobs(std::move(jobs)) {
}
MaintenanceControllerExplorer::~MaintenanceControllerExplorer() = default;

void MaintenanceControllerExplorer::get_state(const Inserter& inserter, bool full) const {
