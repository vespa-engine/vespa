...
void TransactionLogManager::doLogReplayComplete(const std::string& domainName, vespalib::duration elapsedTime) const {
    EventLogger::transactionLogReplayComplete(domainName, elapsedTime);
}

TransactionLogManager::TransactionLogManager(FNET_Transport& transport, const std::string& tlsSpec,
                                             const std::string& domainName)
    : TransactionLogManagerBase(transport, tlsSpec, domainName), _visitor() {
}

TransactionLogManager::~TransactionLogManager() = default;

void TransactionLogManager::init(SerialNum oldestConfigSerial, SerialNum& prunedSerialNum,
                                 SerialNum& replay_end_serial_num) {
...
void getStatus(Session& session, search::SerialNum& serialBegin, search::SerialNum& serialEnd, size_t& count) {
    if (!session.status(serialBegin, serialEnd, count)) {
        throw IllegalStateException(make_string("Could not get status from session with"
                                                " domain '%s' on TLS '%s'",
...
void getStatus(TransLogClient& client, const std::string& domainName, search::SerialNum& serialBegin,
               search::SerialNum& serialEnd, size_t& count) {
...
void TransactionLogManager::prepareReplay(TransLogClient& client, const std::string& domainName,
    ...
    if (from < to) {
        ...
        if (serialEnd < flushedSummaryMgrSerial) {
            throw IllegalStateException(make_string("SummaryStore '%" PRIu64 "' is more recent than "
                                                    "transactionlog '%" PRIu64 "'. Immpossible !!",
                                                    flushedSummaryMgrSerial, serialEnd));
        }
        if (serialEnd < flushedIndexMgrSerial) {
            throw IllegalStateException(make_string("IndexStore '%" PRIu64 "' is more recent than "
                                                    "transactionlog '%" PRIu64 "'. Immpossible !!",
...
std::shared_ptr<TlsReplayProgress> TransactionLogManager::make_replay_progress(SerialNum first, SerialNum last) {
...
void TransactionLogManager::startReplay(SerialNum first, SerialNum syncToken, Callback& callback) {
    ...
    if (LOG_WOULD_LOG(event)) {
        EventLogger::transactionLogReplayStart(getDomainName(), first, syncToken);
        MemoryUsageLogger::log("start replay transactionlog", getDomainName() + "[" + std::to_string(first) + ", " +
                                                                  std::to_string(syncToken) + ")");
    }
    if (!_visitor->visit(first, syncToken)) {
        throw IllegalStateException(make_string("Could not start visitor for "
                                                "replaying domain '%s<%" PRIu64 ", %" PRIu64 "]' on TLS '%s'",
                                                getDomainName().c_str(), first, syncToken, getRpcTarget().c_str()));
    }
}
