...
private:
    ...
    void reconfigureFeedView(FastAccessFeedView& curr, std::shared_ptr<const search::index::Schema> schema,
                             std::shared_ptr<const document::DocumentTypeRepo> repo,
                             std::shared_ptr<IAttributeWriter>                 attrWriter);

public:
    FastAccessDocSubDBConfigurer(FeedViewVarHolder& feedView, const std::string& subDbName);
    ...
    std::unique_ptr<IReprocessingInitializer> reconfigure(const DocumentDBConfig&      newConfig,
