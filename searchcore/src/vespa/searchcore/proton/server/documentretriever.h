...
class DocumentRetriever : public DocumentRetrieverBase, public IFieldInfo {
public:
    using PositionFields = std::vector<std::pair<const document::Field*, std::string>>;
    DocumentRetriever(const DocTypeName& docTypeName, const document::DocumentTypeRepo& repo,
                      const search::index::Schema& schema, const IDocumentMetaStoreContext& meta_store,
                      const search::IAttributeManager& attr_manager, const search::IDocumentStore& doc_store);
    ...
    DocumentUP getPartialDocument(search::DocumentIdT lid, const document::DocumentId&,
                                  const document::FieldSet&) const override;
...
private:
    void populate(search::DocumentIdT lid, document::Document& doc,
                  const document::Field::Set& attributeFields) const;
