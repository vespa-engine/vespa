...
class ProtonConfigurer : public IProtonConfigurer {
    using DocumentDBs =
        std::map<DocTypeName,
                 std::pair<std::weak_ptr<IDocumentDBConfigOwner>, std::weak_ptr<DocumentDBDirectoryHolder>>>;
    ...
    void applyConfig(std::shared_ptr<ProtonConfigSnapshot> configSnapshot, InitializeThreads initializeThreads,
                     bool initialConfig);
    void configureDocumentDB(const ProtonConfigSnapshot& configSnapshot, const DocTypeName& docTypeName,
...
public:
    ProtonConfigurer(vespalib::ThreadExecutor& executor, IProtonConfigurerOwner& owner,
