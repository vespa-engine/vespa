...
MinimalDocumentRetriever::MinimalDocumentRetriever(const DocTypeName&                            docTypeName,
                                                   const std::shared_ptr<const DocumentTypeRepo> repo,
                                                   const IDocumentMetaStoreContext&              meta_store,
                                                   const IDocumentStore& doc_store, bool hasFields)
...
Document::UP MinimalDocumentRetriever::getFullDocument(DocumentIdT lid) const {
    return _doc_store.read(lid, *_repo);
}

bool MinimalDocumentRetriever::need_fetch_from_doc_store(const FieldSet& field_set) const {
...
void MinimalDocumentRetriever::visitDocuments(const LidVector& lids, search::IDocumentVisitor& visitor,
