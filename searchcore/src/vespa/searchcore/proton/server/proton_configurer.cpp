...
document::BucketSpace getBucketSpace(const BootstrapConfig& bootstrapConfig, const DocTypeName& name) {
...
ProtonConfigurer::ProtonConfigurer(vespalib::ThreadExecutor& executor, IProtonConfigurerOwner& owner,
...
public:
    explicit ReconfigureTask(ProtonConfigurer& configurer)
        : _configurer(configurer), _retainGuard(configurer._pendingReconfigureTasks) {}

    void run() override { _configurer.performReconfigure(); }
...
std::shared_ptr<ProtonConfigSnapshot> ProtonConfigurer::getPendingConfigSnapshot() {
    std::lock_guard<std::mutex> guard(_mutex);
    return _pendingConfigSnapshot;
}

std::shared_ptr<ProtonConfigSnapshot> ProtonConfigurer::getActiveConfigSnapshot() const {
    std::lock_guard<std::mutex> guard(_mutex);
...
void ProtonConfigurer::configureDocumentDB(const ProtonConfigSnapshot& configSnapshot, const DocTypeName& docTypeName,
                                           document::BucketSpace bucketSpace, const std::string& configId,
    ...
    if (dbitr == _documentDBs.end()) {
        auto newdb = _owner.addDocumentDB(docTypeName, bucketSpace, configId, bootstrapConfig, documentDBConfig,
                                          std::move(initializeThreads));
        if (newdb) {
        ...
        if (bucketSpace != old_bucket_space) {
            ...
            const std::string& bucket_space_name(document::FixedBucketSpaces::to_string(bucketSpace));
            LOG(fatal,
                "Bucket space for document type %s changed from %s to %s. This triggers undefined behavior on a "
                "running system. Restarting process immediately to fix it.",
                docTypeName.getName().c_str(), old_bucket_space_name.c_str(), bucket_space_name.c_str());
            std::_Exit(1);
        }
...
void ProtonConfigurer::pruneInitialDocumentDBDirs(const ProtonConfigSnapshot& configSnapshot) {
...
void ProtonConfigurer::pruneDocumentDBs(const ProtonConfigSnapshot& configSnapshot) {
    // called by proton executor thread
    const auto&         bootstrapConfig = configSnapshot.getBootstrapConfig();
    const ProtonConfig& protonConfig = bootstrapConfig->getProtonConfig();
