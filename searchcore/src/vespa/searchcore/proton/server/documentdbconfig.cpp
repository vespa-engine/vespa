...
DocumentDBConfig::DocumentDBConfig(
    int64_t generation, const RankProfilesConfigSP& rankProfiles,
    ...
    const AttributesConfigSP& attributes, const SummaryConfigSP& summary, const JuniperrcConfigSP& juniperrc,
    const DocumenttypesConfigSP& documenttypes, const std::shared_ptr<const DocumentTypeRepo>& repo,
    const ImportedFieldsConfigSP& importedFields, const search::TuneFileDocumentDB::SP& tuneFileDocumentDB,
    std::shared_ptr<const Schema> schema, const DocumentDBMaintenanceConfig::SP& maintenance,
    const search::LogDocumentStore::Config& storeConfig, const ThreadingServiceConfig& threading_service_config,
    const AllocConfig& alloc_config, const DocumentMetaStoreConfig& document_meta_store_config,
    const std::string& configId, const std::string& docTypeName) noexcept
    : _configId(configId),
...
bool DocumentDBConfig::operator==(const DocumentDBConfig& rhs) const {
    return equals<RankProfilesConfig>(_rankProfiles.get(), rhs._rankProfiles.get()) &&
           ...
           (_storeConfig == rhs._storeConfig) && (_threading_service_config == rhs._threading_service_config) &&
           (_alloc_config == rhs._alloc_config) && (_document_meta_store_config == rhs._document_meta_store_config);
}

DocumentDBConfig::ComparisonResult DocumentDBConfig::compare(const DocumentDBConfig& rhs) const {
    ...
    retval.rankingExpressionsChanged =
        !equals<RankingExpressions>(_rankingExpressions.get(), rhs._rankingExpressions.get());
    ...
    retval.flushChanged = !equals<DocumentDBMaintenanceConfig>(
        _maintenance.get(), rhs._maintenance.get(),
        [](const auto& l, const auto& r) { return l.getFlushConfig() == r.getFlushConfig(); });
...
bool DocumentDBConfig::valid() const {
    return _rankProfiles && _rankingConstants && _rankingExpressions && _onnxModels && _indexschema && _attributes &&
           _summary && _juniperrc && _documenttypes && _repo && _importedFields && _tuneFileDocumentDB && _schema &&
           _maintenance;
}

namespace {

template <class Config> std::shared_ptr<Config> emptyConfig(std::shared_ptr<Config> config) {
...
template <> std::shared_ptr<SummaryConfig> emptyConfig(std::shared_ptr<SummaryConfig> config) {
...
DocumentDBConfig::SP DocumentDBConfig::makeReplayConfig(const SP& orig) {
    ...
    SP ret = std::make_shared<DocumentDBConfig>(
        o._generation, emptyConfig(o._rankProfiles), std::make_shared<RankingConstants>(),
        std::make_shared<RankingExpressions>(), std::make_shared<OnnxModels>(), o._indexschema, o._attributes,
        replay_summary_config, emptyConfig(o._juniperrc), o._documenttypes, o._repo,
        std::make_shared<ImportedFieldsConfig>(), o._tuneFileDocumentDB, replay_schema, o._maintenance,
        o._storeConfig, o._threading_service_config, o._alloc_config, o._document_meta_store_config, o._configId,
        o._docTypeName);
    ret->_orig = orig;
    return ret;
}

DocumentDBConfig::SP DocumentDBConfig::getOriginalConfig() const {
    return _orig;
}

DocumentDBConfig::SP DocumentDBConfig::preferOriginalConfig(const SP& self) {
    return (self && self->_orig) ? self->_orig : self;
}

DocumentDBConfig::SP DocumentDBConfig::newFromAttributesConfig(const AttributesConfigSP& attributes) const {
    return std::make_shared<DocumentDBConfig>(
        _generation, _rankProfiles, _rankingConstants, _rankingExpressions, _onnxModels, _indexschema, attributes,
        _summary, _juniperrc, _documenttypes, _repo, _importedFields, _tuneFileDocumentDB, _schema, _maintenance,
        _storeConfig, _threading_service_config, _alloc_config, _document_meta_store_config, _configId, _docTypeName);
}

DocumentDBConfig::SP DocumentDBConfig::makeDelayedAttributeAspectConfig(const SP&               newCfg,
    ...
    attributeAspectDelayer.setup(oldCfg.getAttributesConfig(), n.getAttributesConfig(), n.getSummaryConfig(),
    ...
    auto result = std::make_shared<DocumentDBConfig>(
        n._generation, n._rankProfiles, n._rankingConstants, n._rankingExpressions, n._onnxModels, n._indexschema,
        (attributes_config_changed ? attributeAspectDelayer.getAttributesConfig() : n._attributes),
        (summary_config_changed ? attributeAspectDelayer.getSummaryConfig() : n._summary), n._juniperrc,
        n._documenttypes, n._repo, n._importedFields, n._tuneFileDocumentDB, n._schema, n._maintenance,
        n._storeConfig, n._threading_service_config, n._alloc_config, n._document_meta_store_config, n._configId,
        n._docTypeName);
    result->_delayedAttributeAspects = true;
    return result;
}

DocumentDBConfig::SP DocumentDBConfig::make_copy() const {
    auto copy = std::make_shared<DocumentDBConfig>(*this);
    copy->_delayedAttributeAspects = false;
    return copy;
}

const document::DocumentType* DocumentDBConfig::getDocumentType() const {
    return _repo->getDocumentType(getDocTypeName());
}

std::shared_ptr<const Schema> DocumentDBConfig::build_schema(const AttributesConfig&  attributes_config,
