...
bool blockedDueToClusterState(const std::shared_ptr<IBucketStateCalculator>& calc) {
...
BucketMoveJob::BucketMoveJob(std::shared_ptr<IBucketStateCalculator> calc, RetainGuard dbRetainer,
                             IDocumentMoveHandler& moveHandler, IBucketModifiedHandler& modifiedHandler,
                             IThreadService& master, BucketExecutor& bucketExecutor,
                             const MaintenanceDocumentSubDB& ready, const MaintenanceDocumentSubDB& notReady,
    ...
    : BlockableMaintenanceJob("move_buckets." + docTypeName, vespalib::duration::zero(), vespalib::duration::zero(),
                              blockableConfig),
...
std::shared_ptr<BucketMoveJob> BucketMoveJob::create(
    std::shared_ptr<IBucketStateCalculator> calc, RetainGuard dbRetainer, IDocumentMoveHandler& moveHandler,
    IBucketModifiedHandler& modifiedHandler, IThreadService& master, BucketExecutor& bucketExecutor,
    const MaintenanceDocumentSubDB& ready, const MaintenanceDocumentSubDB& notReady,
    bucketdb::IBucketCreateNotifier& bucketCreateNotifier, IClusterStateChangedNotifier& clusterStateChangedNotifier,
    IBucketStateChangedNotifier& bucketStateChangedNotifier, IResourceUsageNotifier& resource_usage_notifier,
    const BlockableMaintenanceJobConfig& blockableConfig, const std::string& docTypeName,
    document::BucketSpace bucketSpace) {
    return {new BucketMoveJob(std::move(calc), std::move(dbRetainer), moveHandler, modifiedHandler, master,
                              bucketExecutor, ready, notReady, bucketCreateNotifier, clusterStateChangedNotifier,
                              bucketStateChangedNotifier, resource_usage_notifier, blockableConfig, docTypeName,
                              bucketSpace),
            [&master](auto job) {
...
BucketMoveJob::NeedResult BucketMoveJob::needMove(BucketId bucketId, const BucketStateWrapper& itr) const {
    ...
    LOG(spam, "needMove(): bucket(%s), shouldBeReady(%s), active(%s)", bucketId.toString().c_str(),
        toStr(shouldBeReady), toStr(isActive));
    if (wantReady) {
        if (!hasNotReadyDocs) {
...
public:
    ...
    void fail(const Bucket& bucket) override { BucketMoveJob::failOperation(std::move(_job), bucket.getBucketId()); }
...
void BucketMoveJob::startMove(BucketMover& mover, size_t maxDocsToMove) {
...
void BucketMoveJob::completeMove(GuardedMoveOps ops, IDestructorCallbackSP onDone) {
...
bool BucketMoveJob::checkIfMoverComplete(const BucketMover& mover) {
    ...
    if (bucketMoveComplete || needReschedule) {
        ...
        if (needReschedule) {
            if ((found != _bucketsInFlight.end()) && (&mover == found->second.get())) {
                ...
                _movers.erase(
                    std::remove_if(_movers.begin(), _movers.end(),
...
void BucketMoveJob::cancelBucket(BucketId bucket) {
    auto inFlight = _bucketsInFlight.find(bucket);
    if (inFlight != _bucketsInFlight.end()) {
        LOG(spam, "cancelBucket(%s): cancelling existing mover %p", bucket.toString().c_str(),
            inFlight->second.get());
...
void BucketMoveJob::notifyCreateBucket(const bucketdb::Guard& guard, const BucketId& bucket) {
    considerBucket(guard, bucket);
}

BucketMoveJob::BucketMoveSet BucketMoveJob::computeBuckets2Move(const bucketdb::Guard& guard) {
...
std::shared_ptr<BucketMover> BucketMoveJob::createMover(BucketId bucket, bool wantReady) {
    ...
    LOG(debug, "createMover(): BucketMover::create(%s, source:%u, target:%u)", bucket.toString().c_str(),
        source.sub_db_id(), target.sub_db_id());
    return BucketMover::create(bucket, &source, target.sub_db_id(), _moveHandler);
}

std::shared_ptr<BucketMover> BucketMoveJob::greedyCreateMover() {
...
bool BucketMoveJob::scanAndMove(size_t maxBuckets2Move, size_t maxDocsToMovePerBucket) {
...
bool BucketMoveJob::done() const {
    return _buckets2Move.empty() && _movers.empty() && !isBlocked();
}

bool BucketMoveJob::run() {
    if (isBlocked()) {
...
void BucketMoveJob::recompute() {
    recompute(_ready.meta_store()->getBucketDB().takeGuard());
}
void BucketMoveJob::recompute(const bucketdb::Guard& guard) {
    _buckets2Move = computeBuckets2Move(guard);
    updatePending();
}

void BucketMoveJob::backFillMovers() {
...
void BucketMoveJob::notifyClusterStateChanged(const std::shared_ptr<IBucketStateCalculator>& newCalc) {
...
void BucketMoveJob::notifyBucketStateChanged(const BucketId& bucketId, BucketInfo::ActiveState) {
    // Called by master write thread
    considerBucket(_ready.meta_store()->getBucketDB().takeGuard(), bucketId);
}

void BucketMoveJob::notify_resource_usage(const ResourceUsageState& state) {
    // Called by master write thread
    internal_notify_resource_usage(state);
}

void BucketMoveJob::updatePending() {
    _bucketsPending.store(_bucketsInFlight.size() + _buckets2Move.size(), std::memory_order_relaxed);
}

void BucketMoveJob::updateMetrics(DocumentDBTaggedMetrics& metrics) const {
    // This is an over estimate to ensure we do not count down to zero until everything has been and completed and
    // acked.
