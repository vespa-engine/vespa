...
public:
    DocSubDB(IDocumentSubDBOwner& owner, search::transactionlog::SyncProxy& tlSyncer)
        : IDocumentSubDB(), _owner(owner), _tlSyncer(tlSyncer) {}
...
public:
    StoreOnlySubDBFileHeaderContext(const search::common::FileHeaderContext& parentFileHeaderContext,
                                    const DocTypeName& docTypeName, const std::string& baseDir);
...
public:
    ...
    struct Config {
        ...
        Config(const DocTypeName& docTypeName, const std::string& subName, const std::string& baseDir,
    ...
    struct Context {
        ...
        Context(IDocumentSubDBOwner& owner, search::transactionlog::SyncProxy& tlSyncer,
                const IGetSerialNum& getSerialNum, const search::common::FileHeaderContext& fileHeaderContext,
                searchcorespi::index::IThreadingService& writeService, BucketDBOwnerSP bucketDB,
                bucketdb::IBucketDBHandlerInitializer& bucketDBHandlerInitializer, DocumentDBTaggedMetrics& metrics,
                std::mutex& configMutex, const vespalib::HwInfo& hwInfo);
        ~Context();
    };
...
protected:
    ...
    createSummaryManagerInitializer(const search::LogDocumentStore::Config& protonSummaryCfg,
                                    const AllocStrategy& alloc_strategy, const search::TuneFileSummary& tuneFile,
    ...
    std::shared_ptr<initializer::InitializerTask> createDocumentMetaStoreInitializer(
...
public:
    ...
    vespalib::datastore::CompactionStrategy
