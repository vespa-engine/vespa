...
FieldSetAttributeDB::FieldSetAttributeDB(const IFieldInfo& fieldInfo)
    : _fieldInfo(fieldInfo), _isFieldSetAttributeOnly(), _lock() {
}

FieldSetAttributeDB::~FieldSetAttributeDB() = default;

bool FieldSetAttributeDB::areAllFieldsAttributes(uint64_t key, const document::Field::Set& set) const {
...
bool DocumentRetriever::need_fetch_from_doc_store(const FieldSet& fieldSet) const {
...
bool DocumentRetriever::isFieldAttribute(const Field& field) const {
...
std::unique_ptr<document::FieldValue> positionFromZcurve(int64_t zcurve) {
    int32_t x, y;
...
zcurve_array_attribute_to_field_value(const document::Field& field, const search::attribute::IAttributeVector& attr,
                                      DocumentIdT lid) {
...
void fillInPositionFields(Document& doc, DocumentIdT lid,
                          const DocumentRetriever::PositionFields& possiblePositionFields,
...
public:
    PopulateVisitor(const DocumentRetriever& retriever, search::IDocumentVisitor& visitor) noexcept
        : _retriever(retriever), _visitor(visitor) {}
    ...
    bool allowVisitCaching() const override { return _visitor.allowVisitCaching(); }
...
Document::UP DocumentRetriever::getFullDocument(DocumentIdT lid) const {
...
Document::UP DocumentRetriever::getPartialDocument(search::DocumentIdT lid, const document::DocumentId& docId,
...
void DocumentRetriever::visitDocuments(const LidVector& lids, search::IDocumentVisitor& visitor,
                                       ReadConsistency) const {
...
void DocumentRetriever::populate(DocumentIdT lid, Document& doc, const Field::Set& attributeFields) const {
...
const IAttributeManager* DocumentRetriever::getAttrMgr() const {
