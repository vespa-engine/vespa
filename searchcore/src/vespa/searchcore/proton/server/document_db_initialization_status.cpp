...
DocumentDBInitializationStatus::DocumentDBInitializationStatus(const std::string&                     name,
                                                               const std::shared_ptr<const DDBState>& state)
    : _name(name), _state(state) {
}

void DocumentDBInitializationStatus::set_attribute_initialization_statuses(
...
void DocumentDBInitializationStatus::set_replay_progress_producer(
    const std::shared_ptr<const IReplayProgressProducer>& replay_progress_producer) {
    std::lock_guard<std::mutex> guard(_mutex);
...
void DocumentDBInitializationStatus::report_initialization_status(const vespalib::slime::Inserter& inserter) const {
    ...
    if (state >= DDBState::State::REPLAY_TRANSACTION_LOG) {
        db_cursor.setString(
            "replay_progress",
            std::format("{:.6f}", _replay_progress_producer ? _replay_progress_producer->getProgress() : 0.0f));
    }
