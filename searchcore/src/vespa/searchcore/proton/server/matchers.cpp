...
Matchers::Matchers(const std::atomic<vespalib::steady_time>& now_ref, matching::QueryLimiter& queryLimiter,
...
void Matchers::add(const std::string& name, std::shared_ptr<Matcher> matcher) {
...
MatchingStats Matchers::getStats() const {
    MatchingStats stats;
...
MatchingStats Matchers::getStats(const std::string& name) const {
    auto it = _rpmap.find(name);
    return it != _rpmap.end() ? it->second->getStats() : MatchingStats();
}

std::shared_ptr<Matcher> Matchers::lookup(const std::string& name) const {
    ...
    if (found == _rpmap.end()) {
        if (_default) {
            ...
            vespalib::Issue::report(
                fmt("Failed to find rank-profile '%s'. Most likely a configuration issue.", name.c_str()));
            return _fallback;
        }
    }
    return found->second;
