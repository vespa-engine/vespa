...
TransactionLogManagerBase::TransactionLogManagerBase(FNET_Transport& transport, const std::string& tlsSpec,
...
TransactionLogManagerBase::StatusResult TransactionLogManagerBase::init() {
    std::unique_ptr<Session> session = _tlc->open(_domainName);
    if (!session) {
        if (!_tlc->create(_domainName)) {
            std::string str = vespalib::make_string("Failed creating domain '%s' on TLS '%s'", _domainName.c_str(),
                                                    _tlc->getRPCTarget().c_str());
            throw std::runtime_error(str);
        }
        LOG(debug, "Created domain '%s' on TLS '%s'", _domainName.c_str(), _tlc->getRPCTarget().c_str());
        session = _tlc->open(_domainName);
        if (!session) {
            std::string str = vespalib::make_string("Could not open session for domain '%s' on TLS '%s'",
                                                    _domainName.c_str(), _tlc->getRPCTarget().c_str());
            throw std::runtime_error(str);
        }
    }
    LOG(debug, "Opened domain '%s' on TLS '%s'", _domainName.c_str(), _tlc->getRPCTarget().c_str());
    StatusResult res;
    if (!session->status(res.serialBegin, res.serialEnd, res.count)) {
        std::string str = vespalib::make_string("Could not get status from session with domain '%s' on TLS '%s'",
                                                _domainName.c_str(), _tlc->getRPCTarget().c_str());
        throw std::runtime_error(str);
    }
    LOG(debug, "Status for domain '%s': serialBegin(%" PRIu64 "), serialEnd(%" PRIu64 "), count(%zu)",
        _domainName.c_str(), res.serialBegin, res.serialEnd, res.count);
    _tlcSession = std::move(session);
    return res;
}

void TransactionLogManagerBase::internalStartReplay() {
...
void TransactionLogManagerBase::waitForReplayDone() const {
...
void TransactionLogManagerBase::close() {
    if (_tlcSession) {
        _tlcSession->close();
    }
...
std::unique_ptr<Visitor> TransactionLogManagerBase::createTlcVisitor(Callback& callback) {
...
const std::string& TransactionLogManagerBase::getRpcTarget() const {
