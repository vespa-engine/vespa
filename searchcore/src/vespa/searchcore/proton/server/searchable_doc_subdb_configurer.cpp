...
void SearchableDocSubDBConfigurer::reconfigureFeedView(std::shared_ptr<IAttributeWriter>       attrWriter,
                                                       std::shared_ptr<const Schema>           schema,
                                                       std::shared_ptr<const DocumentTypeRepo> repo) {
    auto curr = _feedView.get();
    _feedView.set(std::make_shared<SearchableFeedView>(
        StoreOnlyFeedView::Context(curr->getSummaryAdapter(), std::move(schema), curr->getDocumentMetaStore(),
                                   std::move(repo), curr->getUncommittedLidTracker(),
                                   curr->getGidToLidChangeHandler(), curr->getWriteService()),
        curr->getPersistentParams(), FastAccessFeedView::Context(std::move(attrWriter), curr->getDocIdLimit()),
        SearchableFeedView::Context(curr->getIndexWriter())));
}

void SearchableDocSubDBConfigurer::reconfigureMatchView(const std::shared_ptr<IndexSearchable>& indexSearchable) {
    auto curr = _searchView.get();
    reconfigureMatchView(curr->getMatchers(), indexSearchable, curr->getAttributeManager());
}

void SearchableDocSubDBConfigurer::reconfigureMatchView(const std::shared_ptr<Matchers>&          matchers,
...
void SearchableDocSubDBConfigurer::reconfigureSearchView(std::shared_ptr<MatchView> matchView) {
    auto curr = _searchView.get();
...
std::shared_ptr<Matchers> SearchableDocSubDBConfigurer::createMatchers(const DocumentDBConfig& new_config_snapshot) {
    auto&                          schema = new_config_snapshot.getSchemaSP();
    auto&                          cfg = new_config_snapshot.getRankProfilesConfig();
    search::fef::RankingAssetsRepo ranking_assets_repo_source(
        _constant_value_factory, new_config_snapshot.getRankingConstantsSP(),
        new_config_snapshot.getRankingExpressionsSP(), new_config_snapshot.getOnnxModelsSP());
...
void SearchableDocSubDBConfigurer::reconfigureIndexSearchable() {
...
std::unique_ptr<DocumentSubDBReconfig> SearchableDocSubDBConfigurer::prepare_reconfig(
    const DocumentDBConfig& new_config_snapshot, const AttributeCollectionSpecFactory& attr_spec_factory,
    const ReconfigParams& reconfig_params, uint32_t docid_limit, std::optional<search::SerialNum> serial_num) {
...
IReprocessingInitializer::UP createAttributeReprocessingInitializer(
    const DocumentDBConfig& newConfig, const std::shared_ptr<IAttributeManager>& newAttrMgr,
    const DocumentDBConfig& oldConfig, const std::shared_ptr<IAttributeManager>& oldAttrMgr,
    ...
    return std::make_unique<AttributeReprocessingInitializer>(
        ARIConfig(newAttrMgr, *newConfig.getSchemaSP()), ARIConfig(oldAttrMgr, *oldConfig.getSchemaSP()), inspector,
        oldIndexschemaInspector, subDbName, serialNum);
}

} // namespace

IReprocessingInitializer::UP SearchableDocSubDBConfigurer::reconfigure(const DocumentDBConfig&       newConfig,
    ...
    if (prepared_reconfig.has_attribute_manager_changed()) {
        ...
        newAttrMgr->setImportedAttributes(resolver.resolve(*newAttrMgr, *attrMgr, searchView->getDocumentMetaStore(),
        ...
        initializer = createAttributeReprocessingInitializer(newConfig, newAttrMgr, oldConfig, oldAttrMgr, _subDbName,
                                                             serial_num);
    ...
    if (params.shouldSummaryManagerChange() || params.shouldAttributeManagerChange()) {
        auto sumMgr(_summaryMgr);
        auto newSumSetup =
            sumMgr->createSummarySetup(newConfig.getSummaryConfig(), newConfig.getJuniperrcConfig(),
                                       newConfig.getDocumentTypeRepoSP(), attrMgr, *newConfig.getSchemaSP());
    ...
    if (shouldFeedViewChange) {
        reconfigureFeedView(std::move(attrWriter), newConfig.getSchemaSP(), newConfig.getDocumentTypeRepoSP());
    }
    return initializer;
}
