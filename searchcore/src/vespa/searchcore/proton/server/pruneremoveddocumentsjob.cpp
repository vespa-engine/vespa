...
PruneRemovedDocumentsJob::PruneRemovedDocumentsJob(const DocumentDBPruneConfig& config, RetainGuard dbRetainer,
                                                   const IDocumentMetaStore& metaStore, uint32_t subDbId,
    ...
    : BlockableMaintenanceJob("prune_removed_documents." + docTypeName, config.getDelay(), config.getInterval()),
...
public:
    PruneTask(std::shared_ptr<PruneRemovedDocumentsJob> job, uint32_t lid, const RawDocumentMetadata& meta,
...
void PruneRemovedDocumentsJob::PruneTask::run(const Bucket& bucket, IDestructorCallback::SP onDone) {
    ...
    job._master.execute(makeLambdaTask(
        [job = std::move(_job), lid = _lid, meta = _meta,
...
bool PruneRemovedDocumentsJob::run() {
    vespalib::system_time now = vespalib::system_clock::now();
    const Timestamp ageLimit(static_cast<Timestamp::Type>(vespalib::count_us(now.time_since_epoch() - _cfgAgeLimit)));
