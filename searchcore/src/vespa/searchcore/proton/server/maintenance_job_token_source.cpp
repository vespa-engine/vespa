...
MaintenanceJobTokenSource::MaintenanceJobTokenSource() : _mutex(), _jobs(), _token() {
}

MaintenanceJobTokenSource::~MaintenanceJobTokenSource() = default;

void MaintenanceJobTokenSource::remove_deleted_or_stopped_jobs() {
    _jobs.erase(std::remove_if(_jobs.begin(), _jobs.end(), DeletedOrStoppedJob()), _jobs.end());
}

void MaintenanceJobTokenSource::token_destroyed() {
    std::unique_lock guard(_mutex);
    remove_deleted_or_stopped_jobs();
    auto existing_token = _token.lock();
    if (existing_token) {
        guard.unlock(); // Drop lock to handle recursed call to token_destroyed() if existing_token holds last
                        // reference
        return; // get_token() was called after all references to old token were removed but before token_destroyed()
                // was called.
    }
    while (!_jobs.empty()) {
...
bool MaintenanceJobTokenSource::get_token(std::shared_ptr<IBlockableMaintenanceJob> job) {
    std::unique_lock guard(_mutex);
    auto             existing_token = _token.lock();
    if (existing_token) {
