...
DocumentScanIterator::DocumentScanIterator(const IDocumentMetaStore& metaStore)
    : _metaStore(metaStore), _lastLid(_metaStore.getLidUsageStats().getHighestUsedLid() + 1), _itrValid(true) {
}

bool DocumentScanIterator::valid() const {
    return _itrValid;
}

DocumentMetadata DocumentScanIterator::next(uint32_t compactLidLimit) {
    for (--_lastLid; _lastLid > compactLidLimit; --_lastLid) {
        if (_metaStore.validLid(_lastLid)) {
            ...
            return DocumentMetadata(_lastLid, metadata.getTimestamp(), metadata.getBucketId(), metadata.getGid());
        }
    }
