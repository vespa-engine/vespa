...
DocumentSubDBCollection::DocumentSubDBCollection(
    IDocumentSubDBOwner& owner, search::transactionlog::SyncProxy& tlSyncer, const IGetSerialNum& getSerialNum,
    const DocTypeName& docTypeName, searchcorespi::index::IThreadingService& writeService,
    vespalib::Executor& warmupExecutor, const search::common::FileHeaderContext& fileHeaderContext,
    std::shared_ptr<search::attribute::Interlock> attribute_interlock, MetricsWireService& metricsWireService,
    DocumentDBTaggedMetrics& metrics, matching::QueryLimiter& queryLimiter,
    const std::atomic<vespalib::steady_time>& now_ref, std::mutex& configMutex, const std::string& baseDir,
    const vespalib::HwInfo& hwInfo, std::shared_ptr<search::diskindex::IPostingListCache> posting_list_cache)
    : _subDBs(),
      _owner(owner),
      _calc(),
      _readySubDbId(0),
    ...
    StoreOnlyDocSubDB::Context context(owner, tlSyncer, getSerialNum, fileHeaderContext, writeService, _bucketDB,
                                       *_bucketDBHandler, metrics, configMutex, hwInfo);
    _subDBs.push_back(new SearchableDocSubDB(
        FastAccessDocSubDB::Config(
            StoreOnlyDocSubDB::Config(docTypeName, "0.ready", baseDir, _readySubDbId, SubDbType::READY), false),
        SearchableDocSubDB::Context(
            FastAccessDocSubDB::Context(context, metrics.ready.attributes, metricsWireService, attribute_interlock),
            queryLimiter, now_ref, warmupExecutor, posting_list_cache)));

    _subDBs.push_back(new StoreOnlyDocSubDB(
        StoreOnlyDocSubDB::Config(docTypeName, "1.removed", baseDir, _remSubDbId, SubDbType::REMOVED), context));

    _subDBs.push_back(new FastAccessDocSubDB(
        FastAccessDocSubDB::Config(
            StoreOnlyDocSubDB::Config(docTypeName, "2.notready", baseDir, _notReadySubDbId, SubDbType::NOTREADY),
            true),
        FastAccessDocSubDB::Context(context, metrics.notReady.attributes, metricsWireService, attribute_interlock)));
}
...
void DocumentSubDBCollection::createRetrievers() {
...
std::shared_ptr<CommitAndWaitDocumentRetriever> wrapRetriever(IDocumentRetriever::SP retriever,
...
DocumentSubDBCollection::getRetrievers(IDocumentRetriever::ReadConsistency consistency) {
    ...
    if (consistency == IDocumentRetriever::ReadConsistency::STRONG) {
        ...
        wrappedList->push_back(
            wrapRetriever((*list)[_notReadySubDbId], getNotReadySubDB()->getUncommittedLidsTracker()));
        return wrappedList;
    } else {
        return list;
    }
...
void DocumentSubDBCollection::maintenanceSync(MaintenanceController& mc) {
    ...
    MaintenanceDocumentSubDB readySubDB(
        getReadySubDB()->getName(), _readySubDbId, getReadySubDB()->getDocumentMetaStoreContext().getSP(),
        wrapRetriever((*retrievers)[_readySubDbId], getReadySubDB()->getUncommittedLidsTracker()),
        getReadySubDB()->getFeedView(), &getReadySubDB()->getUncommittedLidsTracker());
    MaintenanceDocumentSubDB remSubDB(
        getRemSubDB()->getName(), _remSubDbId, getRemSubDB()->getDocumentMetaStoreContext().getSP(),
        wrapRetriever((*retrievers)[_remSubDbId], getRemSubDB()->getUncommittedLidsTracker()),
        getRemSubDB()->getFeedView(), &getRemSubDB()->getUncommittedLidsTracker());
    MaintenanceDocumentSubDB notReadySubDB(
        getNotReadySubDB()->getName(), _notReadySubDbId, getNotReadySubDB()->getDocumentMetaStoreContext().getSP(),
        wrapRetriever((*retrievers)[_notReadySubDbId], getNotReadySubDB()->getUncommittedLidsTracker()),
        getNotReadySubDB()->getFeedView(), &getNotReadySubDB()->getUncommittedLidsTracker());
    mc.syncSubDBs(readySubDB, remSubDB, notReadySubDB);
}

initializer::InitializerTask::SP DocumentSubDBCollection::createInitializer(const DocumentDBConfig&   configSnapshot,
...
void DocumentSubDBCollection::initViews(const DocumentDBConfig& configSnapshot) {
    for (auto subDb : _subDBs) {
        subDb->initViews(configSnapshot);
    }
}

void DocumentSubDBCollection::clearViews() {
    for (auto subDb : _subDBs) {
        subDb->clearViews();
    }
}

void DocumentSubDBCollection::onReplayDone() {
    for (auto subDb : _subDBs) {
        subDb->onReplayDone();
    }
}

void DocumentSubDBCollection::onReprocessDone(SerialNum serialNum) {
    for (auto subDb : _subDBs) {
        subDb->onReprocessDone(serialNum);
    }
}

SerialNum DocumentSubDBCollection::getOldestFlushedSerial() {
    SerialNum lowest = -1;
    for (auto subDb : _subDBs) {
...
SerialNum DocumentSubDBCollection::getNewestFlushedSerial() {
    SerialNum highest = 0;
    for (auto subDb : _subDBs) {
...
void DocumentSubDBCollection::pruneRemovedFields(SerialNum serialNum) {
    for (auto subDb : _subDBs) {
...
DocumentSubDBCollection::prepare_reconfig(const DocumentDBConfig&  new_config_snapshot,
...
void DocumentSubDBCollection::complete_prepare_reconfig(DocumentDBReconfig& prepared_reconfig, SerialNum serial_num) {
...
void DocumentSubDBCollection::applyConfig(const DocumentDBConfig& newConfigSnapshot,
    ...
    auto tasks = getReadySubDB()->applyConfig(newConfigSnapshot, oldConfigSnapshot, serialNum, params, resolver,
                                              prepared_reconfig.ready_reconfig());
    _reprocessingRunner.addTasks(tasks);
    tasks = getNotReadySubDB()->applyConfig(newConfigSnapshot, oldConfigSnapshot, serialNum, params, resolver,
                                            prepared_reconfig.not_ready_reconfig());
    _reprocessingRunner.addTasks(tasks);
    auto removed_reconfig = getRemSubDB()->prepare_reconfig(newConfigSnapshot, params, serialNum);
    tasks = getRemSubDB()->applyConfig(newConfigSnapshot, oldConfigSnapshot, serialNum, params, resolver,
                                       *removed_reconfig);
    removed_reconfig.reset();
    _reprocessingRunner.addTasks(tasks);
}

IFeedView::SP DocumentSubDBCollection::getFeedView() {
...
IFlushTarget::List DocumentSubDBCollection::getFlushTargets() {
...
double DocumentSubDBCollection::getReprocessingProgress() const {
    return _reprocessingRunner.getProgress();
}

void DocumentSubDBCollection::close() {
    for (auto subDb : _subDBs) {
        subDb->close();
    }
}

void DocumentSubDBCollection::setBucketStateCalculator(const IBucketStateCalculatorSP& calc, OnDone onDone) {
    _calc = calc;
    for (auto subDb : _subDBs) {
...
void DocumentSubDBCollection::tearDownReferences(IDocumentDBReferenceResolver& resolver) {
    for (auto subDb : _subDBs) {
