...
std::vector<IFlushTarget::SP> FlushHandlerProxy::getFlushTargets() {
    return _documentDB->getFlushTargets();
}

IFlushHandler::SerialNum FlushHandlerProxy::getCurrentSerialNumber() const {
    return _documentDB->getCurrentSerialNumber();
}

void FlushHandlerProxy::flushDone(SerialNum flushedSerial) {
    _documentDB->flushDone(flushedSerial);
}

void FlushHandlerProxy::syncTls(SerialNum syncTo) {
