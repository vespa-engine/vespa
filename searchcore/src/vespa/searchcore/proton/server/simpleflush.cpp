...
bool SimpleFlush::CompareTarget::compare(const IFlushTarget& lhs, const IFlushTarget& rhs) const {
    ...
    LOG(spam,
        ...
        lhs.getName().c_str(), lhsMgain.getBefore(), lhsMgain.getAfter(), lhsSerial, rhs.getName().c_str(),
        rhsMgain.getBefore(), rhsMgain.getAfter(), rhsSerial, ret ? 1 : 0);
    return ret;
}
