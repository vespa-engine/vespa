...
public:
    struct Config {
        ...
        Config();
        Config(uint64_t maxGlobalMemory_in, uint64_t maxGlobalTlsSize_in, double globalDiskBloatFactor_in,
               uint64_t maxMemoryGain_in, double diskBloatFactor_in, vespalib::duration maxTimeGain_in);
        bool operator==(const Config& rhs) const { return equal(rhs); }
        bool operator!=(const Config& rhs) const { return !equal(rhs); }
        bool equal(const Config& rhs) const {
            return (maxGlobalMemory == rhs.maxGlobalMemory) && (maxGlobalTlsSize == rhs.maxGlobalTlsSize) &&
                   (globalDiskBloatFactor == rhs.globalDiskBloatFactor) && (maxMemoryGain == rhs.maxMemoryGain) &&
                   (maxTimeGain == rhs.maxTimeGain) && (diskBloatFactor == rhs.diskBloatFactor);
        }
...
private:
    ...
    public:
        CompareTarget(OrderType order, const flushengine::TlsStatsMap& tlsStatsMap)
            : _order(order), _tlsStatsMap(tlsStatsMap) {}
