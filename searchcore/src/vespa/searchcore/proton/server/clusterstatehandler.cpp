...
void ClusterStateHandler::performSetClusterState(const ClusterState* calc, IGenericResultHandler* resultHandler) {
    LOG(debug,
        "performSetClusterState(): "
        "clusterUp(%s), nodeUp(%s), nodeInitializing(%s), nodeMaintenance(%s)"
        "changedHandlers.size() = %zu",
        (calc->clusterUp() ? "true" : "false"), (calc->nodeUp() ? "true" : "false"),
        (calc->nodeInitializing() ? "true" : "false"), (calc->nodeMaintenance() ? "true" : "false"),
...
void ClusterStateHandler::performGetModifiedBuckets(IBucketIdListResultHandler* resultHandler) {
    ...
    if (LOG_WOULD_LOG(debug) && !modifiedBuckets.empty()) {
        ...
        LOG(debug, "performGetModifiedBuckets(): modifiedBuckets(%zu): %s", modifiedBuckets.size(),
            oss.str().c_str());
    }
...
void ClusterStateHandler::notifyBucketModified(const document::BucketId& bucket) {
...
void ClusterStateHandler::removeClusterStateChangedHandler(IClusterStateChangedHandler* handler) {
...
void ClusterStateHandler::handleSetClusterState(const ClusterState& calc, IGenericResultHandler& resultHandler) {
    _executor.execute(makeLambdaTask([&]() { performSetClusterState(&calc, &resultHandler); }));
}

void ClusterStateHandler::handleGetModifiedBuckets(IBucketIdListResultHandler& resultHandler) {
    _executor.execute(makeLambdaTask([&]() { performGetModifiedBuckets(&resultHandler); }));
}
