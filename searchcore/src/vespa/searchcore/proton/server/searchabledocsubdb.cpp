...
void SearchableDocSubDB::syncViews() {
    _iSearchView.set(_rSearchView.get());
    _iFeedView.set(_rFeedView.get());
}

SerialNum SearchableDocSubDB::getOldestFlushedSerial() {
    SerialNum lowest(Parent::getOldestFlushedSerial());
    lowest = std::min(lowest, _indexMgr->getFlushedSerialNum());
    return lowest;
}

SerialNum SearchableDocSubDB::getNewestFlushedSerial() {
    SerialNum highest(Parent::getNewestFlushedSerial());
    highest = std::max(highest, _indexMgr->getFlushedSerialNum());
    return highest;
}

initializer::InitializerTask::SP SearchableDocSubDB::createIndexManagerInitializer(
    const DocumentDBConfig& configSnapshot, SerialNum configSerialNum, const IndexConfig& indexCfg,
    ...
    return std::make_shared<IndexManagerInitializer>(
        vespaIndexDir, indexCfg, schema, configSerialNum, const_cast<SearchableDocSubDB&>(*this), _writeService,
        _warmupExecutor, configSnapshot.getTuneFileDocumentDBSP()->_index,
        configSnapshot.getTuneFileDocumentDBSP()->_attr, _fileHeaderContext, _posting_list_cache,
        std::move(indexManager));
}

void SearchableDocSubDB::setupIndexManager(searchcorespi::IIndexManager::SP indexManager, const Schema& schema) {
    _indexMgr = std::move(indexManager);
    _indexWriter = std::make_shared<IndexWriter>(_indexMgr);
    reconfigure_index_metrics(schema);
}

DocumentSubDbInitializer::UP SearchableDocSubDB::createInitializer(const DocumentDBConfig& configSnapshot,
...
void SearchableDocSubDB::setup(const DocumentSubDbInitializerResult& initResult) {
...
void SearchableDocSubDB::reconfigureMatchingMetrics(const RankProfilesConfig& cfg) {
...
SearchableDocSubDB::prepare_reconfig(const DocumentDBConfig& new_config_snapshot,
...
IReprocessingTask::List SearchableDocSubDB::applyConfig(const DocumentDBConfig& newConfigSnapshot,
    ...
    if (prepared_reconfig.has_attribute_manager_changed()) {
        ...
        IReprocessingInitializer::UP  initializer = _configurer.reconfigure(
            newConfigSnapshot, oldConfigSnapshot, params, resolver, prepared_reconfig, serialNum);
...
void SearchableDocSubDB::applyFlushConfig(const DocumentDBFlushConfig& flushConfig) {
    _flushConfig = flushConfig;
    propagateFlushConfig();
}

void SearchableDocSubDB::propagateFlushConfig() {
    uint32_t maxFlushed =
        is_node_retired_or_maintenance() ? _flushConfig.getMaxFlushedRetired() : _flushConfig.getMaxFlushed();
    _indexMgr->setMaxFlushed(maxFlushed);
}

void SearchableDocSubDB::setBucketStateCalculator(const std::shared_ptr<IBucketStateCalculator>& calc,
                                                  OnDone                                         onDone) {
...
void SearchableDocSubDB::reconfigureIndexSearchable() {
    std::lock_guard<std::mutex> guard(_configMutex);
...
IFlushTarget::List SearchableDocSubDB::getFlushTargetsInternal() {
...
void SearchableDocSubDB::reconfigure_index_metrics(const Schema& schema) {
...
size_t SearchableDocSubDB::getNumActiveDocs() const {
...
search::IndexStats SearchableDocSubDB::get_index_stats(bool clear_disk_io_stats) const {
    return _indexMgr ? _indexMgr->get_index_stats(clear_disk_io_stats) : search::IndexStats();
}

std::shared_ptr<IDocumentRetriever> SearchableDocSubDB::getDocumentRetriever() {
    return std::make_shared<FastAccessDocumentRetriever>(_rFeedView.get(), _rSearchView.get()->getAttributeManager());
}

MatchingStats SearchableDocSubDB::getMatcherStats(const std::string& rankProfile) const {
    return _rSearchView.get()->getMatcherStats(rankProfile);
}

void SearchableDocSubDB::close() {
    _realGidToLidChangeHandler->close();
    Parent::close();
}

std::shared_ptr<IDocumentDBReference> SearchableDocSubDB::getDocumentDBReference() {
    return std::make_shared<DocumentDBReference>(getAttributeManager(), _metaStoreCtx, _gidToLidChangeHandler);
}

void SearchableDocSubDB::tearDownReferences(IDocumentDBReferenceResolver& resolver) {
    auto attrMgr = getAttributeManager();
    resolver.teardown(*attrMgr);
}

void SearchableDocSubDB::clearViews() {
    _rFeedView.clear();
    _rSearchView.clear();
    Parent::clearViews();
}

std::shared_ptr<IAttributeWriter> SearchableDocSubDB::get_attribute_writer() const {
    return _rFeedView.get()->getAttributeWriter();
}

ResourceUsage SearchableDocSubDB::get_resource_usage() const {
