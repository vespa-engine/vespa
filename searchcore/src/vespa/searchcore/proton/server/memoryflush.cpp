...
int64_t getSerialDiff(SerialNum localLastSerial, const IFlushTarget& target) {
    return localLastSerial - target.getFlushedSerialNum();
}

uint64_t estimateNeededTlsSizeForFlushTarget(const TlsStats& tlsStats, SerialNum flushedSerialNum) {
...
MemoryFlush::Config::Config(uint64_t maxGlobalMemory_in, uint64_t maxGlobalTlsSize_in,
                            double globalDiskBloatFactor_in, uint64_t maxMemoryGain_in, double diskBloatFactor_in,
...
std::string MemoryFlush::Config::toString() const {
...
MemoryFlush::MemoryFlush(const Config& config, vespalib::system_time startTime)
    : _lock(), _config(config), _startTime(startTime) {
}

MemoryFlush::MemoryFlush() : MemoryFlush(Config(), vespalib::system_clock::now()) {
}
...
std::string getOrderName(MemoryFlush::OrderType& orderType) {
    switch (orderType) {
    case MemoryFlush::OrderType::MEMORY:
        return "MEMORY";
    case MemoryFlush::OrderType::TLSSIZE:
        return "TLSSIZE";
    case MemoryFlush::OrderType::DISKBLOAT:
        return "DISKBLOAT";
    case MemoryFlush::OrderType::MAXAGE:
        return "MAXAGE";
    case MemoryFlush::OrderType::DEFAULT:
        return "DEFAULT";
    }
    return "DEFAULT";
}
...
FlushContext::List MemoryFlush::getFlushTargets(const FlushContext::List&            targetList,
    ...
    for (const auto& ctx : targetList) {
        ...
        LOG(debug,
            ...
            ctx->getName().c_str(), totalMemory, mgain, totalDisk.gain(), dgain.gain(), tlsStats.getNumBytes(),
...
bool MemoryFlush::CompareTarget::operator()(const FlushContext::SP& lfc, const FlushContext::SP& rfc) const {
