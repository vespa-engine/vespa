...
void DocumentSubDBReconfig::set_attribute_manager_reconfig(
    std::unique_ptr<IAttributeManagerReconfig> attribute_manager_reconfig) {
    _attribute_manager_reconfig = std::move(attribute_manager_reconfig);
}

void DocumentSubDBReconfig::complete(uint32_t docid_limit, search::SerialNum serial_num) {
