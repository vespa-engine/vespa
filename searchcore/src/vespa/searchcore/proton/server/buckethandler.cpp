...
void BucketHandler::performSetCurrentState(BucketId bucketId, storage::spi::BucketInfo::ActiveState newState,
                                           IGenericResultHandler* resultHandler) {
    if (!_nodeUp) {
        Result result(Result::ErrorType::TRANSIENT_ERROR, "Cannot set bucket active state when node is down");
        resultHandler->handle(result);
        return;
    }
    bool active = (newState == storage::spi::BucketInfo::ACTIVE);
    LOG(debug, "performSetCurrentState(%s, %s)", bucketId.toString().c_str(), (active ? "ACTIVE" : "NOT_ACTIVE"));
...
void BucketHandler::performPopulateActiveBuckets(document::BucketId::List buckets,
                                                 IGenericResultHandler*   resultHandler) {
    _ready->populateActiveBuckets(std::move(buckets));
    resultHandler->handle(Result());
}

void BucketHandler::deactivateAllActiveBuckets() {
...
void BucketHandler::handleListBuckets(IBucketIdListResultHandler& resultHandler) {
...
void BucketHandler::handleSetCurrentState(const BucketId& bucketId, storage::spi::BucketInfo::ActiveState newState,
...
void BucketHandler::handleGetBucketInfo(const Bucket& bucket, IBucketInfoResultHandler& resultHandler) {
    // Called by SPI thread.
    // BucketDBOwner ensures synchronization between SPI thread and
    // master write thread in document database.
    BucketInfo bucketInfo = _ready->getBucketDB().takeGuard()->cachedGetBucketInfo(bucket);
    LOG(spam, "handleGetBucketInfo(%s): %s", bucket.toString().c_str(), bucketInfo.toString().c_str());
    resultHandler.handle(BucketInfoResult(bucketInfo));
}

bool BucketHandler::hasBucket(const storage::spi::Bucket& bucket) {
    return _ready->getBucketDB().takeGuard()->hasBucket(bucket);
}

void BucketHandler::handleListActiveBuckets(IBucketIdListResultHandler& resultHandler) {
    // Called by SPI thread.
    // BucketDBOwner ensures synchronization between SPI thread and
    // master write thread in document database.
...
void BucketHandler::handlePopulateActiveBuckets(document::BucketId::List buckets,
                                                IGenericResultHandler&   resultHandler) {
...
void BucketHandler::notifyClusterStateChanged(const std::shared_ptr<IBucketStateCalculator>& newCalc) {
    ...
    LOG(spam, "notifyClusterStateChanged; up: %s -> %s, maintenance: %s -> %s", bool_str(oldNodeUp),
        bool_str(_nodeUp), bool_str(oldNodeMaintenance), bool_str(_nodeMaintenance));
    if (_nodeMaintenance) {
...
void BucketHandler::removeBucketStateChangedHandler(IBucketStateChangedHandler* handler) {
