...
public:
    struct Config {
        ...
        Config(const StoreOnlyDocSubDB::Config& storeOnlyCfg, bool fastAccessAttributesOnly)
            : _storeOnlyCfg(storeOnlyCfg), _fastAccessAttributesOnly(fastAccessAttributesOnly) {}
    };
    ...
    struct Context {
        ...
        Context(const StoreOnlyDocSubDB::Context& storeOnlyCtx, AttributeMetrics& subAttributeMetrics,
...
private:
    ...
    createAttributeManagerInitializer(const DocumentDBConfig& configSnapshot, SerialNum configSerialNum,
...
public:
    ...
    std::unique_ptr<DocumentSubDbInitializer> createInitializer(const DocumentDBConfig& configSnapshot,
    ...
    std::unique_ptr<DocumentSubDBReconfig> prepare_reconfig(const DocumentDBConfig&  new_config_snapshot,
                                                            const ReconfigParams&    reconfig_params,
                                                            std::optional<SerialNum> serial_num) override;
    IReprocessingTask::List applyConfig(const DocumentDBConfig& newConfigSnapshot,
                                        ...
                                        const DocumentSubDBReconfig& prepared_reconfig) override;
