...
void RPCHooksBase::reportState(FRT_RPCRequest* req) {
    std::vector<Pair> res;
...
void RPCHooksBase::initRPC() {
    FRT_ReflectionBuilder rb(_orb.get());
    //-------------------------------------------------------------------------
    rb.DefineMethod("pandora.rtc.getState", "ii", "SSi", FRT_METHOD(RPCHooksBase::rpc_GetState), this);
    ...
    //-------------------------------------------------------------------------
    rb.DefineMethod("proton.getStatus", "s", "SSSS", FRT_METHOD(RPCHooksBase::rpc_GetProtonStatus), this);
    ...
    rb.RequestAccessFilter(make_proton_admin_api_capability_filter());
    //-------------------------------------------------------------------------
    rb.DefineMethod("proton.triggerFlush", "", "b", FRT_METHOD(RPCHooksBase::rpc_triggerFlush), this);
    rb.MethodDesc("Tell the node to trigger flush ASAP");
    rb.ReturnDesc("success", "Whether or not a flush was triggered.");
    rb.RequestAccessFilter(make_proton_admin_api_capability_filter());
    //-------------------------------------------------------------------------
    rb.DefineMethod("proton.prepareRestart", "", "b", FRT_METHOD(RPCHooksBase::rpc_prepareRestart), this);
    ...
    rb.RequestAccessFilter(make_proton_admin_api_capability_filter());
    //--------------------------------
    rb.DefineMethod("proton.prepareRestart2", "ii", "bs", FRT_METHOD(RPCHooksBase::rpc_prepareRestart2), this);
    rb.MethodDesc("Tell the node to prepare for a restart by flushing components "
                  "such that TLS replay time + time spent flushing components is as low as possible");
...
RPCHooksBase::RPCHooksBase(Params& params)
    : _proton(params.proton),
      ...
      _proto_rpc_adapter(std::make_unique<ProtoRpcAdapter>(_proton.get_search_server(), _proton.get_docsum_server(),
                                                           _proton.get_monitor_server(), *_orb)),
      _regAPI(*_orb, slobrok::ConfiguratorFactory(params.slobrok_config)) {
}

void RPCHooksBase::open(Params& params) {
    initRPC();
...
void RPCHooksBase::letProtonDo(Executor::Task::UP task) {
    _proton.getExecutor().execute(std::move(task));
}

void RPCHooksBase::triggerFlush(FRT_RPCRequest* req) {
    ...
    auto handler = std::make_shared<TriggerFlushRpcHandler>(
        _detached_requests_owner, RefCountedRpcRequest::internal_attach(req),
        set_strategy_result.lowest_strategy_id_notifier(), _transport->GetScheduler(),
        set_strategy_result.wait_strategy_id(), timeout);
    handler->setup();
}

void RPCHooksBase::prepareRestart(FRT_RPCRequest* req) {
    LOG(info, "RPCHooksBase::prepareRestart will create detached rpc handler");
    std::chrono::steady_clock::duration timeout(610s); // 10s greater timeout than the one in vespa-proton-cmd
    auto                                set_strategy_result = _proton.prepare_restart2(0);
    using RefCountedRpcRequest = vespalib::ref_counted<FRT_RPCRequest>;
    auto handler = std::make_shared<PrepareRestartRpcHandler>(
        _detached_requests_owner, RefCountedRpcRequest::internal_attach(req),
        set_strategy_result.lowest_strategy_id_notifier(), _transport->GetScheduler(),
        set_strategy_result.wait_strategy_id(), timeout);
    handler->setup();
...
void RPCHooksBase::prepareRestart2(FRT_RPCRequest* req) {
    ...
    using RefCountedRpcRequest = vespalib::ref_counted<FRT_RPCRequest>;
    auto handler = std::make_shared<PrepareRestart2RpcHandler>(
        _detached_requests_owner, RefCountedRpcRequest::internal_attach(req),
        set_strategy_result.lowest_strategy_id_notifier(), _transport->GetScheduler(),
        set_strategy_result.wait_strategy_id(), timeout, set_strategy_result.flush_history());
    handler->setup();
}

void RPCHooksBase::rpc_GetState(FRT_RPCRequest* req) {
    FRT_Values& arg = *req->GetParams();
...
void RPCHooksBase::rpc_GetProtonStatus(FRT_RPCRequest* req) {
    LOG(debug, "RPCHooksBase::rpc_GetProtonStatus started");
    req->Detach();
    letProtonDo(makeLambdaTask([this, req]() { getProtonStatus(req); }));
}

void RPCHooksBase::getProtonStatus(FRT_RPCRequest* req) {
    ...
    for (uint32_t i = 0; i < reports.size(); ++i) {
        ...
        LOG(debug, "component(%s), status(%s), internalState(%s), message(%s)", report.getComponent().c_str(),
            k[i]._str, internalStates[i]._str, report.getMessage().c_str());
    }
    req->Return();
}

void RPCHooksBase::rpc_triggerFlush(FRT_RPCRequest* req) {
    LOG(info, "RPCHooksBase::rpc_triggerFlush started");
    req->Detach();
    letProtonDo(makeLambdaTask([this, req]() { triggerFlush(req); }));
}

void RPCHooksBase::rpc_prepareRestart(FRT_RPCRequest* req) {
    LOG(info, "RPCHooksBase::rpc_prepareRestart started");
    req->Detach();
    letProtonDo(makeLambdaTask([this, req]() { prepareRestart(req); }));
}

void RPCHooksBase::rpc_prepareRestart2(FRT_RPCRequest* req) {
    LOG(info, "RPCHooksBase::rpc_prepareRestart2 started");
    req->Detach();
    letProtonDo(makeLambdaTask([this, req]() { prepareRestart2(req); }));
}

RPCHooks::RPCHooks(Params& params) : RPCHooksBase(params) {
    open(params);
}
