...
public:
    InitState(const std::string& name) noexcept : FeedState(INIT), _doc_type_name(name) {}
    ...
    void receive(const PacketWrapperSP& wrap, vespalib::Executor&) override {
        throwExceptionInReceive(_doc_type_name.c_str(), wrap->packet.range().from(), wrap->packet.range().to(),
                                wrap->packet.size());
    }
};
...
public:
    ReplayTransactionLogState(const std::string& name, IFeedView*& feed_view_ptr,
                              bucketdb::IBucketDBHandler& bucketDBHandler, IReplayConfig& replay_config,
