...
public:
    ...
    const std::shared_ptr<IndexSearchable>& getIndexSearchable() const noexcept {
        return _matchView->getIndexSearchable();
    }
    const std::shared_ptr<IAttributeManager>& getAttributeManager() const noexcept {
        return _matchView->getAttributeManager();
    }
    SessionManager& getSessionManager() const noexcept { return _matchView->getSessionManager(); }
    const std::shared_ptr<IDocumentMetaStoreContext>& getDocumentMetaStore() const noexcept {
        return _matchView->getDocumentMetaStore();
    }
    DocIdLimit& getDocIdLimit() const noexcept { return _matchView->getDocIdLimit(); }
    matching::MatchingStats getMatcherStats(const std::string& rankProfile) const {
