...
public:
    ...
    DocumentDBLidSpaceCompactionConfig(vespalib::duration interval, uint32_t allowedLidBloat,
                                       double allowwedLidBloatFactor, double remove_batch_block_rate,
                                       double remove_block_rate, bool disabled) noexcept;
...
public:
    ...
    BlockableMaintenanceJobConfig(double resourceLimitFactor, uint32_t maxOutstandingMoveOps) noexcept;
...
public:
    ...
    DocumentDBMaintenanceConfig(const DocumentDBPruneConfig&     pruneRemovedDocuments,
                                const DocumentDBHeartBeatConfig& heartBeat, vespalib::duration visibilityDelay,
    ...
    bool operator==(const DocumentDBMaintenanceConfig& rhs) const noexcept;

    const DocumentDBPruneConfig& getPruneRemovedDocumentsConfig() const noexcept { return _pruneRemovedDocuments; }
    const DocumentDBHeartBeatConfig& getHeartBeatConfig() const noexcept { return _heartBeat; }
    ...
    vespalib::duration getAttributeUsageSampleInterval() const noexcept { return _attributeUsageSampleInterval; }
    const BlockableMaintenanceJobConfig& getBlockableJobConfig() const noexcept { return _blockableJobConfig; }
