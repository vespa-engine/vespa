...
public:
    ...
    public:
        ...
        MoveGuard(BucketMover& mover) noexcept : _mover(&mover) {
    ...
    static std::shared_ptr<BucketMover> create(const document::BucketId&       bucket,
                                               const MaintenanceDocumentSubDB* source, uint32_t targetSubDbId,
...
private:
    ...
    void enableReschedule() { _needReschedule.store(true, std::memory_order_relaxed); }
};
} // namespace bucketdb
...
public:
    ...
    void setupForBucket(const document::BucketId& bucket, const MaintenanceDocumentSubDB* source,
    ...
    bool bucketDone() const { return !_impl || _impl->allScheduled(); }
