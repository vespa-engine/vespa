...
std::string handles_to_string(const HandleRecorder::HandleMap& handles, MatchDataDetails requested_details) {
...
std::string HandleRecorder::to_string() const {
    vespalib::asciistream os;
...
void HandleRecorder::register_handle(TermFieldHandle handle, MatchDataDetails requested_details) {
...
void HandleRecorder::tag_match_data(MatchData& match_data) {
    for (TermFieldHandle handle = 0; handle < match_data.getNumTermFields(); ++handle) {
        ...
        if (recorded == _handles.end()) {
            ...
            tfmd.setNeedNormalFeatures(
                (static_cast<int>(recorded->second) & static_cast<int>(MatchDataDetails::Normal)) != 0);
            tfmd.setNeedInterleavedFeatures(
                (static_cast<int>(recorded->second) & static_cast<int>(MatchDataDetails::Interleaved)) != 0);
        }
    }
}
