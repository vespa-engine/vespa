...
public:
    ...
    struct FieldEntry final : ITermFieldData {
        ...
        [[nodiscard]] const FieldSpec& fieldSpec() const noexcept { return _field_spec; }
...
protected:
    void resolve(const ViewResolver& resolver, const search::fef::IIndexEnvironment& idxEnv, const std::string& view,
                 bool forceFilter);
...
template <typename NodeType> inline uint32_t numTerms(const NodeType&) {
    return 1;
}

template <> inline uint32_t numTerms<search::query::Phrase>(const search::query::Phrase& n) {
    return n.getChildren().size();
}

template <typename Base> struct ProtonTermBase : public Base, public ProtonTermData {
    using Base::Base;
...
template <typename Base> ProtonTermBase<Base>::~ProtonTermBase() = default;

template <typename Base> struct ProtonTermWithFields : public ProtonTermBase<Base> {
    ...
    virtual search::queryeval::FieldSpec
    inner_field_spec(const search::queryeval::FieldSpec& parentSpec) const override;
};

template <typename Base> ProtonTermWithFields<Base>::~ProtonTermWithFields() = default;

template <typename Base> struct ProtonTerm final : public ProtonTermWithFields<Base> {
    using ProtonTermWithFields<Base>::ProtonTermWithFields;
    ~ProtonTerm() override;
};

template <typename Base> ProtonTerm<Base>::~ProtonTerm() = default;
...
struct ProtonWordAlternatives final : public ProtonTermWithFields<search::query::WordAlternatives> {
    ProtonWordAlternatives(std::vector<std::unique_ptr<search::query::StringTerm>> children, const std::string& view,
                           int32_t id, search::query::Weight weight);
    ProtonWordAlternatives(std::vector<std::unique_ptr<ProtonStringTerm>> children, const std::string& view,
                           int32_t id, search::query::Weight weight);
    // compatibility constructor
    ProtonWordAlternatives(std::unique_ptr<search::query::TermVector> terms, const std::string& view, int32_t id,
