...
ProtonTermData::allocateTerms(MatchDataLayout &mdl)
{
    ...
    LOG(debug, "ProtonTermData[%s] allocate %d handles", me.c_str(), _fields.size());
    for (size_t i = 0; i < _fields.size(); ++i) {
        auto handle = mdl.allocTermField(_fields[i].getFieldId());
        LOG(debug, "got handle %d", handle);
        _fields[i]._field_spec.setHandle(handle);
    }
}
