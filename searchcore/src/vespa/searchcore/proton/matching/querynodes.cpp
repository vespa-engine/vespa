...
bool is_attribute(FieldType type) noexcept {
    return (type == FieldType::ATTRIBUTE) || (type == FieldType::HIDDEN_ATTRIBUTE);
}

} // namespace

void ProtonTermData::propagate_document_frequency(uint32_t matching_doc_count, uint32_t total_doc_count) {
    for (size_t i = 0; i < _fields.size(); ++i) {
        _fields[i].setDocFreq(matching_doc_count, total_doc_count);
    }
}

void ProtonTermData::resolve(const ViewResolver& resolver, const IIndexEnvironment& idxEnv, const string& view,
                             bool forceFilter) {
...
void ProtonTermData::resolveFromChildren(const std::vector<Node*>& subterms) {
...
const ProtonTermData::FieldEntry* ProtonTermData::lookupField(uint32_t fieldId) const {
    for (size_t i = 0; i < _fields.size(); ++i) {
...
TermFieldHandle ProtonTermData::FieldEntry::getHandle(MatchDataDetails requested_details) const {
...
search::queryeval::FieldSpec
ProtonTermWithFields<Base>::inner_field_spec(const search::queryeval::FieldSpec& parentSpec) const {
    std::string me = vespalib::getClassName(*this);
    LOG(debug, "ProtonTerm[%s] inner_field_spec[%d] check my %zd fields", me.c_str(), parentSpec.getFieldId(),
        numFields());
