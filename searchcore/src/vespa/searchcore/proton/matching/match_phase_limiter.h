...
struct MaybeMatchPhaseLimiter {
    ...
    virtual SearchIterator::UP maybe_limit(SearchIterator::UP search, double match_freq, size_t num_docs,
                                           Cursor* trace) = 0;
...
struct NoMatchPhaseLimiter : MaybeMatchPhaseLimiter {
    ...
    SearchIterator::UP maybe_limit(SearchIterator::UP search, double, size_t, Cursor*) override { return search; }
...
struct DiversityParams {
    ...
    DiversityParams(const std::string& attribute_, uint32_t min_groups_, double cutoff_factor_,
                    CutoffStrategy cutoff_strategy_)
...
struct DegradationParams {
    DegradationParams(const std::string& attribute_, size_t max_hits_, bool descending_, double max_filter_coverage_,
                      double sample_percentage_, double post_filter_multiplier_)
        : attribute(attribute_),
...
private:
    class Coverage {
    public:
        explicit Coverage(uint32_t docIdLimit) : _docIdLimit(docIdLimit), _searched(0) {}
...
public:
    MatchPhaseLimiter(uint32_t docIdLimit, const RangeQueryLocator& rangeQueryLocator,
                      search::queryeval::Searchable&      searchable_attributes,
                      search::queryeval::IRequestContext& requestContext, const DegradationParams& degradation,
    ...
    SearchIterator::UP maybe_limit(SearchIterator::UP search, double match_freq, size_t num_docs,
                                   Cursor* trace) override;
