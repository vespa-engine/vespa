...
struct WaitTimer {
    ...
    explicit WaitTimer(double& wait_time_s_in) : wait_time_s(wait_time_s_in), wait_time() {}
    void done() { wait_time_s += vespalib::to_s(wait_time.elapsed()); }
};

// seek_next maps to SearchIterator::seekNext
struct SimpleStrategy {
    static uint32_t seek_next(SearchIterator& search, uint32_t docid) { return search.seekNext(docid); }
};
...
void fillPartialResult(ResultProcessor::Context& context, size_t totalHits, size_t numHits,
    ...
    if (pr.hasSortData()) {
        ...
        if ((bits != nullptr) && (pr.size() < pr.maxSize())) {
            for (unsigned int bitId = bits->getFirstTrueBit(); (bitId < bits->size()) && (pr.size() < pr.maxSize());
...
double MatchThread::estimate_match_frequency(uint32_t matches, uint32_t searchedSoFar) {
...
SearchIterator* MatchThread::maybe_limit(MatchTools& tools, uint32_t matches, uint32_t docId, uint32_t endId) {
    ...
    LOG(debug, "Limit=%d has been reached at docid=%d which is after %zu docs.", matches, docId,
        (scheduler.total_size(thread_id) - local_todo));
    LOG(debug, "SearchIterator after limiter: %s", tools.search().asString().c_str());
    return &tools.search();
}

bool MatchThread::try_share(DocidRange& docid_range, uint32_t next_docid) {
...
void MatchThread::match_loop(MatchTools& tools, HitCollector& hits) {
    ...
    if (do_limit && context.isBelowLimit()) {
        ...
        LOG(debug, "Limit not reached (had %d) after %zu docs.", matches, searchedSoFar);
...
void MatchThread::match_loop_helper_rank_limit_share_drop(MatchTools& tools, HitCollector& hits) {
...
void MatchThread::match_loop_helper_rank_limit_share(MatchTools& tools, HitCollector& hits) {
...
void MatchThread::match_loop_helper_rank_limit(MatchTools& tools, HitCollector& hits) {
...
template <bool do_rank> void MatchThread::match_loop_helper_rank(MatchTools& tools, HitCollector& hits) {
...
void MatchThread::match_loop_helper(MatchTools& tools, HitCollector& hits) {
...
void MatchThread::secondPhase(MatchTools& tools, HitCollector& hits) {
    trace->addEvent(4, "Start second phase rerank");
    auto sorted_hit_seq = matchToolsFactory.should_diversify() ? hits.getSortedHitSequence(matchParams.arraySize)
...
search::ResultSet::UP MatchThread::findMatches(MatchTools& tools) {
    ...
    if (isFirstThread()) {
        LOG(debug, "SearchIterator after MultiBitVectorIteratorBase::optimize(): %s",
            tools.search().asString().c_str());
...
std::unique_ptr<search::ResultSet> MatchThread::get_matches_after_second_phase_rank_score_drop(HitCollector& hits) {
...
void MatchThread::run() {
    ...
    if (first_phase_profiler) {
        first_phase_profiler->report(trace->createCursor("first_phase_profiling"), [](const std::string& name) {
...
std::unique_ptr<PartialResult> MatchThread::extract_result() {
