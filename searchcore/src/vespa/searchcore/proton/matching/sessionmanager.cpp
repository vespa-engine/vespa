...
template <typename T> struct SessionCache : SessionCacheBase {
...
template <typename T> struct SessionMap : SessionCacheBase {
...
std::vector<SessionManager::SearchSessionInfo> SessionManager::getSortedSearchSessionInfo() const {
    ...
    std::sort(sessions.begin(), sessions.end(),
              [](const SearchSessionInfo& a, const SearchSessionInfo& b) { return (a.created < b.created); });
    return sessions;
}
...
template <typename T> void split_and_execute(std::vector<T> tasks, vespalib::ThreadExecutor& executor) {
...
void SessionManager::pruneTimedOutSessions(vespalib::steady_time currentTime, vespalib::ThreadExecutor& executor) {
