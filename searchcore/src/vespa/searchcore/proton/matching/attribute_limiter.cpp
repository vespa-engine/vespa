...
AttributeLimiter::AttributeLimiter(const RangeQueryLocator& rangeQueryLocator, Searchable& searchable_attributes,
                                   const IRequestContext& requestContext, const string& attribute_name,
                                   bool descending, const string& diversity_attribute, double diversityCutoffFactor,
...
bool AttributeLimiter::was_used() const {
    return (_estimatedHits.load(std::memory_order_relaxed) >= 0);
}

ssize_t AttributeLimiter::getEstimatedHits() const {
...
AttributeLimiter::DiversityCutoffStrategy AttributeLimiter::toDiversityCutoffStrategy(std::string_view strategy) {
    return (strategy == STRICT_STR) ? DiversityCutoffStrategy::STRICT : DiversityCutoffStrategy::LOOSE;
}

const std::string& toString(AttributeLimiter::DiversityCutoffStrategy strategy) {
...
std::unique_ptr<SearchIterator> AttributeLimiter::create_search(size_t want_hits, size_t max_group_size,
                                                                double hit_rate, bool strictSearch) {
