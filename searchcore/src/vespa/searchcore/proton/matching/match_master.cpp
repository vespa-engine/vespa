...
struct TimedMatchLoopCommunicator final : IMatchLoopCommunicator {
    ...
    TimedMatchLoopCommunicator(IMatchLoopCommunicator& com)
        : communicator(com), elapsed(vespalib::duration::zero()) {}
...
DocidRangeScheduler::UP createScheduler(uint32_t numThreads, uint32_t numSearchPartitions, uint32_t numDocs) {
...
auto make_reply(const MatchToolsFactory& mtf, ResultProcessor& processor, ThreadBundle& bundle,
...
ResultProcessor::Result::UP MatchMaster::match(search::engine::Trace& trace, const MatchParams& params,
     ...
     */
    MatchLoopCommunicator communicator(
        threadBundle.size(), params.heapSize, mtf.createDiversifier(params.diversity_want_hits), mtf.object_store(),
