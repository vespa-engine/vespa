...
void mergeHits(size_t maxHits, std::vector<search::RankedHit>& hits, const std::vector<search::RankedHit>& rhs_hits) {
...
bool before(const PartialResult::SortRef& a, uint32_t docid_a, const PartialResult::SortRef& b, uint32_t docid_b) {
...
size_t mergeHits(size_t maxHits, std::vector<search::RankedHit>& hits, std::vector<PartialResult::SortRef>& sortData,
    ...
    while (a_pos < a_end && b_pos < b_end && hits.size() < maxHits) {
        if (before(*a_sort_pos, a_pos->_docId, *b_sort_pos, b_pos->_docId)) {
            hits.push_back(*a_pos++);
...
PartialResult::PartialResult(size_t maxSize_in, bool hasSortData_in)
    : _hits(), _sortData(), _maxSize(maxSize_in), _totalHits(0), _hasSortData(hasSortData_in), _sortDataSize(0) {
