...
private:
    ...
    SearchIterator* maybe_limit(MatchTools& tools, uint32_t matches, uint32_t docId, uint32_t endId)
    ...
    void match_loop_helper_rank_limit_share(MatchTools& tools, HitCollector& hits);
    ...
    search::HitRank fallback_rank_value() const {
        return match_with_ranking ? search::default_rank_value : search::zero_rank_value;
    }
