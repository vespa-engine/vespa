...
QueryLimiter::LimitedToken::LimitedToken(const Doom& doom, QueryLimiter& limiter) : _limiter(limiter) {
...
void QueryLimiter::grabToken(const Doom& doom) {
    std::unique_lock<std::mutex> guard(_lock);
    for (auto max_threads = get_max_threads();
         (max_threads > 0) && (_activeThreads >= max_threads) && !doom.hard_doom(); max_threads = get_max_threads())
    {
...
void QueryLimiter::releaseToken() {
