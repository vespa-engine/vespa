...
namespace {
class TermDataExtractorVisitor : public TemplateTermVisitor<TermDataExtractorVisitor, ProtonNodeTypes> {
    vector<const ITermData*>& _term_data;

public:
    explicit TermDataExtractorVisitor(vector<const ITermData*>& term_data) noexcept : _term_data(term_data) {}
...
void TermDataExtractor::extractTerms(const Node& node, vector<const ITermData*>& term_data) {
