...
AnnDeadlineConfiguration::AnnDeadlineConfiguration(vespalib::duration timebudget, bool timeout_enabled,
                                                   vespalib::steady_time timeout) noexcept
    : _timebudget(timebudget), _timeout_enabled(timeout_enabled), _timeout(timeout) {
}
...
const vespalib::Deadline AnnDeadlineConfiguration::make_ann_deadline(const vespalib::Doom& doom,
    ...
    vespalib::duration timeout_left = _timeout_enabled ? ((_timeout - now) / remaining_searches) : _timeout - now;

    if (_timebudget < timeout_left) {
        return doom.make_deadline(now + _timebudget, Deadline::Type::BUDGET);
    } else {
        return doom.make_deadline(now + timeout_left,
                                  _timeout_enabled ? Deadline::Type::TIMEOUT : Deadline::Type::BUDGET);
    }
}
