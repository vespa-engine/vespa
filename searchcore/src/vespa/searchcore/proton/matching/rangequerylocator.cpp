...
RangeLimitMetaInfo::RangeLimitMetaInfo() : _valid(false), _estimate(0), _range_spec() {
}
RangeLimitMetaInfo::RangeLimitMetaInfo(const search::NumericRangeSpec& range_spec_, size_t estimate_)
    : _valid(true), _estimate(estimate_), _range_spec(range_spec_) {
}
RangeLimitMetaInfo::~RangeLimitMetaInfo() = default;

namespace {

RangeLimitMetaInfo locateFirst(uint32_t field_id, const Blueprint& blueprint) {
...
RangeLimitMetaInfo LocateRangeItemFromQuery::locate() const {
