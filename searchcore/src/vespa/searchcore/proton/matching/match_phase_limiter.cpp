...
template <bool PRE_FILTER> class LimitedSearchT : public LimitedSearch {
public:
    LimitedSearchT(SearchIterator::UP limiter, SearchIterator::UP search)
        : LimitedSearch(std::move(PRE_FILTER ? limiter : search), std::move(PRE_FILTER ? search : limiter)) {}
...
template <bool PRE_FILTER> LimitedSearchT<PRE_FILTER>::~LimitedSearchT() = default;

} // namespace

void LimitedSearch::doSeek(uint32_t docId) {
...
void LimitedSearch::initRange(uint32_t begin, uint32_t end) {
    SearchIterator::initRange(begin, end);
    getFirst().initRange(begin, end);
    getSecond().initRange(begin, end);
}

void LimitedSearch::visitMembers(vespalib::ObjectVisitor& visitor) const {
    visit(visitor, "first", getFirst());
    visit(visitor, "second", getSecond());
}

MatchPhaseLimiter::MatchPhaseLimiter(uint32_t docIdLimit, const RangeQueryLocator& rangeQueryLocator,
                                     Searchable& searchable_attributes, IRequestContext& requestContext,
                                     const DegradationParams& degradation, const DiversityParams& diversity)
    : _postFilterMultiplier(degradation.post_filter_multiplier),
      ...
      _limiter_factory(rangeQueryLocator, searchable_attributes, requestContext, degradation.attribute,
                       degradation.descending, diversity.attribute, diversity.cutoff_factor,
                       diversity.cutoff_strategy),
...
SearchIterator::UP do_limit(AttributeLimiter& limiter_factory, SearchIterator::UP search, double match_freq,
                            size_t wanted_num_docs, size_t max_group_size, uint32_t current_id, uint32_t end_id) {
    SearchIterator::UP limiter =
        limiter_factory.create_search(wanted_num_docs, max_group_size, match_freq, PRE_FILTER);
    if (PRE_FILTER) {
...
SearchIterator::UP MatchPhaseLimiter::maybe_limit(SearchIterator::UP search, double match_freq, size_t num_docs,
                                                  Cursor* trace) {
    ...
    if ((upper_limited_corpus_size <= wanted_num_docs) || (match_freq < MIN_HIT_RATE_LIMIT)) {
        ...
        LOG(debug,
            "Will not limit ! maybe_limit(hit_rate=%g, num_docs=%ld, max_filter_docs=%ld) = wanted_num_docs=%ld",
    ...
    if (trace) {
        trace->setString("action",
                         use_pre_filter ? "Will limit with prefix filter" : "Will limit with postfix filter");
    ...
    LOG(debug,
        "Will do %s filter :  maybe_limit(hit_rate=%g, num_docs=%zu, max_filter_docs=%ld) = wanted_num_docs=%zu,"
        " max_group_size=%zu, current_docid=%u, end_docid=%u, total_query_hits=%ld",
        use_pre_filter ? "pre" : "post", match_freq, num_docs, max_filter_docs, wanted_num_docs, max_group_size,
        current_id, end_id, total_query_hits);
    return (use_pre_filter) ? do_limit<true>(_limiter_factory, std::move(search), match_freq, wanted_num_docs,
                                             max_group_size, current_id, end_id)
                            : do_limit<false>(_limiter_factory, std::move(search), match_freq, wanted_num_docs,
                                              max_group_size, current_id, end_id);
}

void MatchPhaseLimiter::updateDocIdSpaceEstimate(size_t searchedDocIdSpace, size_t remainingDocIdSpace) {
    _coverage.update(searchedDocIdSpace, remainingDocIdSpace, _limiter_factory.getEstimatedHits());
}

size_t MatchPhaseLimiter::getDocIdSpaceEstimate() const {
