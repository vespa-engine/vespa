...
void consider_field_for_extraction(const std::string& field_name, StringSet& virtual_fields) {
...
StringSet extract_virtual_fields(const std::vector<search::fef::FieldInfo>& fields) {
    // Fields that are represented by a set of attributes (normal and imported) in the backend are considered virtual
    // fields. Currently, this is map or array of struct fields (from the SD file) with struct-field attributes. These
    // attributes have '.' in their names, example: my_map.key and my_map.value represent a map<int, string>.
    StringSet result;
...
void IndexEnvironment::extractFields(const search::index::Schema& schema) {
    ...
    for (uint32_t i = 0; i < schema.getNumIndexFields(); ++i) {
        ...
        fieldInfo.set_data_type(field.getDataType());
        auto field_filter_threshold =
            indexproperties::matching::FilterThreshold::lookup_for_field(_properties, field.getName());
        ...
        auto element_gap_setting =
            indexproperties::matching::ElementGap::lookup_for_field(_properties, field.getName());
...
IndexEnvironment::IndexEnvironment(uint32_t distributionKey, const search::index::Schema& schema,
                                   search::fef::Properties props, const IRankingAssetsRepo& rankingAssetsRepo)
    : _tableManager(),
...
const search::fef::Properties& IndexEnvironment::getProperties() const {
    return _properties;
}

uint32_t IndexEnvironment::getNumFields() const {
    return _fields.size();
}

const search::fef::FieldInfo* IndexEnvironment::getField(uint32_t id) const {
    if (id < _fields.size()) {
        return &_fields[id];
    }
    return nullptr;
...
const search::fef::FieldInfo* IndexEnvironment::getFieldByName(const string& name) const {
...
const search::fef::ITableManager& IndexEnvironment::getTableManager() const {
    return _tableManager;
}

IIndexEnvironment::FeatureMotivation IndexEnvironment::getFeatureMotivation() const {
    return _motivation;
}

void IndexEnvironment::hintFeatureMotivation(FeatureMotivation motivation) const {
