...
FeatureSet::UP get_feature_set(const MatchToolsFactory& mtf, const std::vector<uint32_t>& docs,
...
template <typename T> const T* as(const Blueprint& bp) {
    return dynamic_cast<const T*>(&bp);
}

void find_matching_elements(const std::vector<uint32_t>& docs, const SameElementBlueprint& same_element,
                            MatchData& md, MatchingElements& result) {
...
void find_matching_elements(const std::vector<uint32_t>& docs, MatchingElementsSearch& search,
...
void find_matching_elements(const std::vector<uint32_t>& docs, SearchIterator& search, const std::string& fieldName,
                            MatchingElements& result) {
    search.initRange(docs.front(), docs.back() + 1);
    std::vector<uint32_t> matches;
    for (uint32_t doc : docs) {
...
void find_matching_elements(const std::vector<uint32_t>& docs, const std::string& field_name,
                            const AttrSearchCtx& attr_ctx, MatchingElements& result) {
    int32_t               weight = 0;
    std::vector<uint32_t> matches;
    for (uint32_t doc : docs) {
...
bool has_matching_elements_field(const IntermediateBlueprint& bp, const MatchingElementsFields& fields, FieldIdToNameMapper id_to_name) {
    for (size_t i = 0; i < bp.childCnt(); ++i) {
        ...
        for (size_t j = 0; j < cs.numFields(); ++j) {
            auto& field_name = id_to_name.lookup(cs.field(j).getFieldId());
...
void extract_matching_elements(uint32_t doc, std::span<const MatchSpan> match_spans,
                               const MatchingElementsFields& fields, FieldIdToNameMapper id_to_name,
                               std::vector<uint32_t>& matching_elements, MatchingElements& result) {
    uint32_t field_id = IllegalFieldId;
    matching_elements.clear();
    for (auto& match_span : match_spans) {
        if (field_id != match_span.field_id()) {
            if (!matching_elements.empty()) {
                auto& field_name = id_to_name.lookup(field_id);
                if (fields.has_field(field_name)) {
                    result.add_matching_elements(doc, field_name, matching_elements);
                }
                matching_elements.clear();
            }
...
void find_matching_elements(const std::vector<uint32_t> &docs, const IntermediateBlueprint& bp,
    ...
    auto search = bp.createSearch(match_data);
    if (!search) {
        return;
    }
    auto near = dynamic_cast<NearSearchBase*>(search.get());
    if (near == nullptr) {
        return;
    }
    search->initRange(docs.front(), docs.back() + 1);
    ...
    for (uint32_t doc : docs) {
        if (search->seek(doc)) {
            match_spans.clear();
            near->get_match_spans(doc, match_spans);
            extract_matching_elements(doc, match_spans, fields, id_to_name, matching_elements, result);
        }
    }
}
...
void FindMatchingElements::process(const std::vector<uint32_t>& docs, const Blueprint& bp) {
    ...
    } else if (auto near = as<NearBlueprint>(bp)) {
        find_matching_elements(docs, *near, fields, idToName, matchData, result);
    } else if (auto onear = as<ONearBlueprint>(bp)) {
        find_matching_elements(docs, *onear, fields, idToName, matchData, result);
...
DocsumMatcher::DocsumMatcher() : _from_session(), _from_mtf(), _mtf(nullptr), _docs() {
}
...
DocsumMatcher::DocsumMatcher(std::unique_ptr<MatchToolsFactory> mtf, std::vector<uint32_t> docs)
    : _from_session(), _from_mtf(std::move(mtf)), _mtf(_from_mtf.get()), _docs(std::move(docs)) {
}
...
FeatureSet::UP DocsumMatcher::get_summary_features() const {
    if (!_mtf) {
...
FeatureSet::UP DocsumMatcher::get_rank_features() const {
    if (!_mtf) {
        return std::make_unique<FeatureSet>();
    }
...
MatchingElements::UP DocsumMatcher::get_matching_elements(const MatchingElementsFields& fields) const {
