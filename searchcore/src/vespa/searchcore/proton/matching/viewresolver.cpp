...
ViewResolver& ViewResolver::add(const std::string& view, std::string_view field) {
    _map[view].emplace_back(field);
    return *this;
}

bool ViewResolver::resolve(std::string_view view, std::vector<std::string>& fields) const {
...
ViewResolver ViewResolver::createFromSchema(const search::index::Schema& schema) {
