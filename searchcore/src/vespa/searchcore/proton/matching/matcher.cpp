...
bool willNeedRanking(const SearchRequest& request, const GroupingContext& groupingContext,
    ...
    return (groupingContext.needRanking() || (request.maxhits != 0)) &&
           (request.sortSpec.empty() || (request.sortSpec.find("[rank]") != std::string::npos) ||
...
Matcher::Matcher(const search::index::Schema& schema, Properties props, const std::atomic<steady_time>& now_ref,
    ...
    if (!_rankSetup->compile()) {
        throw vespalib::IllegalArgumentException(
            fmt("failed to compile rank setup :\n%s", _rankSetup->getJoinedWarnings().c_str()), VESPA_STRLOC);
    }
}

Matcher::~Matcher() = default;

MatchingStats Matcher::getStats() {
...
Matcher::create_match_tools_factory(const search::engine::Request& request, ISearchContext& searchContext,
    ...
    double factor = softTimeoutEnabled
                        ? (hasFactorOverride ? softtimeout::Factor::lookup(rankProperties, _stats.softDoomFactor())
...
void initCoverage(Coverage& coverage, const search::IDocumentMetaStore& metaStore,
...
SearchReply::UP Matcher::match(const SearchRequest& request, vespalib::ThreadBundle& threadBundle,
    ...
    { // we want to measure full set-up and tear-down time as part of
      // collateral time
        ...
        auto              first_phase_rank_score_drop_limit =
            FirstPhaseRankScoreDropLimit::lookup(rankProperties, _rankSetup->get_first_phase_rank_score_drop_limit());
        auto second_phase_rank_score_drop_limit = SecondPhaseRankScoreDropLimit::lookup(
            rankProperties, _rankSetup->get_second_phase_rank_score_drop_limit());

        MatchParams params(searchContext.getDocIdLimit(), heapSize, arraySize, first_phase_rank_score_drop_limit,
                           second_phase_rank_score_drop_limit, request.offset, request.maxhits,
...
void Matcher::updateStats(const MatchingStats& my_stats, const search::engine::Request& request,
    ...
    if (my_stats.softDoomed()) {
        ...
        vespalib::duration overtimeLimit = std::chrono::duration_cast<vespalib::duration>(
            (1.0 - _rankSetup->getSoftTimeoutTailCost()) * request.getTimeout());
        ...
        bool allowedSoftTimeoutFactorAdjustment =
            ((my_clock::now() - _startTime) > TIME_BEFORE_ALLOWING_SOFT_TIMEOUT_FACTOR_ADJUSTMENT) && !isDoomExplicit;
        ...
        if ((_stats.softDoomed() < 10) || (_stats.softDoomed() % 100 == 0)) {
            LOG(info,
                ...
                isDoomExplicit ? "with query override" : "factor adjustment", _stats.softDoomed(),
                coverage.getCovered(), coverage.getActive(), vespalib::to_s(request.getTimeout()),
                vespalib::to_s(my_stats.doomOvertime()), vespalib::to_s(overtimeLimit), vespalib::to_s(duration),
...
FeatureSet::SP Matcher::getSummaryFeatures(const DocsumRequest& req, ISearchContext& searchCtx,
...
FeatureSet::SP Matcher::getRankFeatures(const DocsumRequest& req, ISearchContext& searchCtx,
                                        IAttributeContext& attrCtx, SessionManager& sessionMgr) const {
    auto docsum_matcher = create_docsum_matcher(req, searchCtx, attrCtx, sessionMgr);
    return docsum_matcher->get_rank_features();
}

MatchingElements::UP Matcher::get_matching_elements(const DocsumRequest& req, ISearchContext& search_ctx,
...
std::pair<std::shared_ptr<SearchSession>, bool> Matcher::lookupSearchSession(SessionManager& session_manager,
                                                                             const Request&  req) {
...
DocsumMatcher::UP Matcher::create_docsum_matcher(const DocsumRequest& req, ISearchContext& search_ctx,
...
bool Matcher::canProduceSummaryFeatures() const {
