...
public:
    struct Stats {
        ...
        Stats() noexcept : numInsert(0), numPick(0), numDropped(0), numCached(0), numTimedout(0) {}
        ...
        bool operator==(const Stats& rhs) const noexcept {
            return numInsert == rhs.numInsert && numPick == rhs.numPick && numDropped == rhs.numDropped &&
                   numCached == rhs.numCached && numTimedout == rhs.numTimedout;
        }
    };
    ...
    struct SearchSessionInfo {
        ...
        SearchSessionInfo(const std::string& id_in, vespalib::steady_time created_in,
