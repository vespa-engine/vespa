...
class SameElementDescendantModifier
    : public search::query::TemplateTermVisitor<SameElementDescendantModifier, ProtonNodeTypes> {
...
public:
    ...
    template <class TermNode> void visitTerm(TermNode& n) { visit_term(n); }
...
void SameElementDescendantModifier::visit_nearlike_children(const std::vector<Node*>& children,
                                                            size_t                    num_negative_terms) {
...
void SameElementDescendantModifier::visit_term(Term& term) {
...
void SameElementDescendantModifier::visit(ProtonNodeTypes::AndNot& n) {
    n.elementwise = true;
...
void SameElementDescendantModifier::visit(ProtonNodeTypes::Near& n) {
    visit_nearlike_children(n.getChildren(), n.num_negative_terms());
}

void SameElementDescendantModifier::visit(ProtonNodeTypes::ONear& n) {
    visit_nearlike_children(n.getChildren(), n.num_negative_terms());
}
...
void SameElementModifier::visit(ProtonNodeTypes::SameElement& n) {
    if (n.getView().empty()) {
        return;
    }
