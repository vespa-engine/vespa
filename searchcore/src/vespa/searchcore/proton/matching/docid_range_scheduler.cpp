...
size_t clamped_sub(size_t a, size_t b) {
    return (b > a) ? 0 : (a - b);
}
...
PartitionDocidRangeScheduler::PartitionDocidRangeScheduler(size_t num_threads, uint32_t docid_limit) : _ranges() {
...
DocidRange TaskDocidRangeScheduler::next_task(size_t thread_id) {
...
size_t AdaptiveDocidRangeScheduler::take_idle(const Guard&) {
...
void AdaptiveDocidRangeScheduler::donate(const Guard& guard, size_t src_thread, DocidRange range) {
...
bool AdaptiveDocidRangeScheduler::all_work_done(const Guard&) const {
    // when all threads are idle at the same time there is no more work
    return ((_idle.size() + 1) == _workers.size());
}

DocidRange AdaptiveDocidRangeScheduler::finalize(const Guard& guard, size_t thread_id) {
    while (!_idle.empty()) {
...
DocidRange AdaptiveDocidRangeScheduler::first_range(size_t thread_id) {
...
DocidRange AdaptiveDocidRangeScheduler::next_range(size_t thread_id) {
    Guard guard(_lock);
...
DocidRange AdaptiveDocidRangeScheduler::share_range(size_t thread_id, DocidRange todo) {
