...
AttributeFieldValueNode::AttributeFieldValueNode(const std::string& doctype, const std::string& field,
                                                 uint32_t attr_guard_index)
    : FieldValueNode(doctype, field), _attr_guard_index(attr_guard_index) {
}
...
std::unique_ptr<document::select::Value> AttributeFieldValueNode::getValue(const Context& context) const {
    ...
    case BasicType::MAX_TYPE:
        throw IllegalStateException(
            make_string("Attribute '%s' has illegal type '%d'", v.getName().c_str(), v.getBasicType()));
    }
    return std::make_unique<NullValue>();
}

std::unique_ptr<Value> AttributeFieldValueNode::traceValue(const Context& context, std::ostream& out) const {
    return defaultTrace(getValue(context), out);
}

document::select::ValueNode::UP AttributeFieldValueNode::clone() const {
