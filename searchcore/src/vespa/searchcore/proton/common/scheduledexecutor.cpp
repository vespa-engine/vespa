...
public:
    ...
    TimerTask(FNET_Scheduler* scheduler, Task::UP task, duration interval)
        : FNET_Task(scheduler), _task(std::move(task)), _interval(interval) {}

    ~TimerTask() override { Kill(); }
...
ScheduledExecutor::Registration::~Registration() {
    _executor.cancel(_key);
}

ScheduledExecutor::ScheduledExecutor(FNET_Transport& transport)
    : _transport(transport), _lock(), _nextKey(0), _taskList() {
}
