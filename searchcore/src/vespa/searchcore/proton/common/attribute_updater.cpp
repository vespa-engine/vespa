...
std::string toString(const FieldPathTarget& target) {
    ...
    case FieldPathTarget::Kind::UNSUPPORTED:
        return "unsupported";
    case FieldPathTarget::Kind::ASSIGN_ELEMENT:
        return std::format("assign {}[{}]", target.attribute_name(), target.index());
    }
...
template <typename G> class ArrayAccessor {
public:
...
template <typename G> class WeightedSetAccessor {
public:
...
void AttributeUpdater::handleUpdateT(V& vec, Accessor, uint32_t lid, const ValueUpdate& upd) {
    ...
    if (vec.hasMultiValue()) {
        if (op == ValueUpdate::Clear) {
            ...
            if (assign.hasValue()) {
                ...
                if (fv.isA(FieldValue::Type::ARRAY)) {
                    ...
                    LOG(warning, "Unsupported value %s in assign operation on multivalue vector %s", fv.className(),
                        vec.getName().c_str());
                }
            }
            ...
            LOG(warning, "Unsupported value update operation %s on multivalue vector %s", upd.className(),
                vec.getName().c_str());
        }
    } else {
        if (op == ValueUpdate::Assign) {
            ...
            LOG(warning, "Unsupported value update operation %s on singlevalue vector %s", upd.className(),
                vec.getName().c_str());
        }
    }
}
...
template <> void AttributeUpdater::handleUpdate(TensorAttribute& vec, uint32_t lid, const ValueUpdate& upd) {
    ...
    if (op == ValueUpdate::Assign) {
        ...
        LOG(warning, "Unsupported value update operation %s on singlevalue tensor attribute %s", upd.className(),
            vec.getName().c_str());
    }
}
...
template <> void AttributeUpdater::handleUpdate(ReferenceAttribute& vec, uint32_t lid, const ValueUpdate& upd) {
    ...
    if (op == ValueUpdate::Assign) {
        ...
        LOG(warning, "Unsupported value update operation %s on singlevalue reference attribute %s", upd.className(),
            vec.getName().c_str());
    }
}
...
template <> void AttributeUpdater::handleUpdate(SingleRawAttribute& vec, uint32_t lid, const ValueUpdate& upd) {
    ...
    if (op == ValueUpdate::Assign) {
        ...
        LOG(warning, "Unsupported value update operation %s on singlevalue raw attribute %s", upd.className(),
            vec.getName().c_str());
    }
}

template <> void AttributeUpdater::handleUpdate(ArrayBoolAttribute& vec, uint32_t lid, const ValueUpdate& upd) {
    LOG(spam, "handleUpdate(%s, %u): %s", vec.getName().c_str(), lid, toString(upd).c_str());
    ValueUpdate::ValueUpdateType op = upd.getType();
    if (op == ValueUpdate::Assign) {
        const AssignValueUpdate& assign(static_cast<const AssignValueUpdate&>(upd));
        ...
        LOG(warning, "Unsupported value update operation %s on array bool attribute %s", upd.className(),
            vec.getName().c_str());
    }
}

void AttributeUpdater::handleUpdate(AttributeVector& vec, uint32_t lid, const FieldUpdate& fUpdate) {
    ...
    for (const auto& update : fUpdate.getUpdates()) {
        ...
        if (!vec.hasMultiValue()) {
            if ((op == ValueUpdate::Add) || (op == ValueUpdate::Remove) || (op == ValueUpdate::Map)) {
                LOG(warning,
                    "operation append/remove not supported for "
                    "single value attribute vectors (%s)",
                    vec.getName().c_str());
                continue;
            }
        }
        ...
        if (vec.isIntegerType()) {
            ...
            LOG(warning, "Unsupported attribute vector '%s' (classname=%s)", vec.getName().c_str(),
                getClassName(vec).c_str());
            return;
        }
    }
}

void AttributeUpdater::handleValue(AttributeVector& vec, uint32_t lid, const FieldValue& val) {
...
void AttributeUpdater::handle_field_path_update(AttributeVector& vec, uint32_t lid, const FieldPathTarget& target,
                                                const FieldPathUpdate& update) {
    LOG(spam, "handle_field_path_update(%s, %u, %s): %s", vec.getName().c_str(), lid, toString(target).c_str(),
        update.getOriginalFieldPath().c_str());
    switch (target.kind()) {
    case FieldPathTarget::Kind::ASSIGN_ELEMENT: {
        // FieldPathTarget::parse only classifies value assigns as ASSIGN_ELEMENT.
        const auto& assign = static_cast<const AssignFieldPathUpdate&>(update);
        assign_element(vec, lid, target.index(), assign.getValue());
        break;
    }
    case FieldPathTarget::Kind::UNSUPPORTED:
...
void AttributeUpdater::handleValueT(V& vec, Accessor, uint32_t lid, const FieldValue& val) {
    if (vec.hasMultiValue()) {
        ...
        if (val.isA(FieldValue::Type::ARRAY)) {
            ...
            LOG(warning, "Unsupported value '%s' to assign on multivalue vector '%s'", val.className(),
                vec.getName().c_str());
        }
    } else {
...
void AttributeUpdater::appendValue(IntegerAttribute& vec, uint32_t lid, const FieldValue& val, int weight) {
    int64_t v = val.getAsLong();
    if (!vec.append(lid, v, weight)) {
        throw UpdateException(
            make_string("attribute append failed: %s[%u] = %" PRIu64, vec.getName().c_str(), lid, v));
    }
}

void AttributeUpdater::removeValue(IntegerAttribute& vec, uint32_t lid, const FieldValue& val) {
    int64_t v = val.getAsLong();
    if (!vec.remove(lid, v, 1)) {
        throw UpdateException(
            make_string("attribute remove failed: %s[%u] = %" PRIu64, vec.getName().c_str(), lid, v));
    }
}

void AttributeUpdater::updateValue(IntegerAttribute& vec, uint32_t lid, const FieldValue& val) {
    int64_t v = val.getAsLong();
    if (!vec.update(lid, v)) {
        throw UpdateException(
            make_string("attribute update failed: %s[%u] = %" PRIu64, vec.getName().c_str(), lid, v));
    }
}

void AttributeUpdater::appendValue(FloatingPointAttribute& vec, uint32_t lid, const FieldValue& val, int weight) {
    double v = val.getAsDouble();
    if (!vec.append(lid, v, weight)) {
        throw UpdateException(make_string("attribute append failed: %s[%u] = %g", vec.getName().c_str(), lid, v));
    }
}

template <typename V, typename Accessor> void AttributeUpdater::appendValue(V& vec, uint32_t lid, Accessor& ac) {
    if (!vec.append(lid, ac)) {
        throw UpdateException(make_string("attribute append failed: %s[%u]", vec.getName().c_str(), lid));
    }
}

void AttributeUpdater::removeValue(FloatingPointAttribute& vec, uint32_t lid, const FieldValue& val) {
    double v = val.getAsDouble();
    if (!vec.remove(lid, v, 1)) {
        throw UpdateException(make_string("attribute remove failed: %s[%u] = %g", vec.getName().c_str(), lid, v));
    }
}

void AttributeUpdater::updateValue(FloatingPointAttribute& vec, uint32_t lid, const FieldValue& val) {
    double v = val.getAsDouble();
    if (!vec.update(lid, v)) {
        throw UpdateException(make_string("attribute update failed: %s[%u] = %g", vec.getName().c_str(), lid, v));
    }
}

namespace {

const std::string& getString(const search::StringAttribute& attr, uint32_t lid, const FieldValue& val) {
    if (!val.isLiteral()) {
        throw UpdateException(
            make_string("Can not update a string attribute '%s' for lid=%d from a non-literal fieldvalue: %s",
...
void AttributeUpdater::appendValue(StringAttribute& vec, uint32_t lid, const FieldValue& val, int weight) {
    const std::string& s = getString(vec, lid, val);
    if (!vec.append(lid, s, weight)) {
        throw UpdateException(
            make_string("attribute append failed: %s[%u] = %s", vec.getName().c_str(), lid, s.c_str()));
    }
}

void AttributeUpdater::removeValue(StringAttribute& vec, uint32_t lid, const FieldValue& val) {
    const std::string& v = getString(vec, lid, val);
    if (!vec.remove(lid, v, 1)) {
        throw UpdateException(
            make_string("attribute remove failed: %s[%u] = %s", vec.getName().c_str(), lid, v.c_str()));
    }
}

void AttributeUpdater::updateValue(StringAttribute& vec, uint32_t lid, const FieldValue& val) {
    const std::string& v = getString(vec, lid, val);
    if (!vec.update(lid, v)) {
        throw UpdateException(
...
void validate_field_value_type(FieldValue::Type expectedType, const FieldValue& val, const std::string& attr_type,
    ...
    if (!val.isA(expectedType)) {
        throw UpdateException(make_string("%s must be updated with %s, but was '%s'", attr_type.c_str(),
                                          value_type.c_str(), val.toString(false).c_str()));
    }
}

} // namespace

void AttributeUpdater::updateValue(PredicateAttribute& vec, uint32_t lid, const FieldValue& val) {
...
void AttributeUpdater::updateValue(TensorAttribute& vec, uint32_t lid, const FieldValue& val) {
...
void AttributeUpdater::updateValue(ReferenceAttribute& vec, uint32_t lid, const FieldValue& val) {
    if (!val.isA(FieldValue::Type::REFERENCE)) {
        vec.clearDoc(lid);
        throw UpdateException(make_string("ReferenceAttribute must be updated with "
                                          "ReferenceFieldValue, but was '%s'",
                                          val.toString(false).c_str()));
    }
...
void AttributeUpdater::updateValue(SingleRawAttribute& vec, uint32_t lid, const FieldValue& val) {
    if (!val.isA(FieldValue::Type::RAW)) {
        vec.clearDoc(lid);
        throw UpdateException(make_string("SingleRawAttribute must be updated with "
                                          "RawFieldValue, but was '%s'",
                                          val.toString(false).c_str()));
    }
...
void AttributeUpdater::updateValue(ArrayBoolAttribute& vec, uint32_t lid, const FieldValue& val) {
    if (!val.isA(FieldValue::Type::ARRAY)) {
        LOG(warning, "Unsupported value '%s' to assign on array bool attribute '%s'", val.className(),
            vec.getName().c_str());
        return;
    }
...
void AttributeUpdater::appendValue(ArrayBoolAttribute& vec, uint32_t lid, const FieldValue& val) {
    std::vector<int8_t> bools;
...
void validate_tensor_attribute_type(AttributeVector& attr) {
    if (!attr.isTensorType()) {
        throw UpdateException(make_string("Expected attribute vector '%s' to be a TensorAttribute, but was '%s'",
...
std::unique_ptr<PrepareResult> AttributeUpdater::prepare_set_value(AttributeVector& attr, uint32_t docid,
...
void AttributeUpdater::complete_set_value(AttributeVector& attr, uint32_t docid, const FieldValue& val,
                                          std::unique_ptr<PrepareResult> prepare_result) {
    validate_tensor_attribute_type(attr);
