...
ScheduledForwardExecutor::ScheduledForwardExecutor(FNET_Transport& transport, Executor& executor)
    : _scheduler(transport), _executor(executor), _lock(), _nextKey(0), _taskList() {
}
...
IScheduledExecutor::Handle ScheduledForwardExecutor::scheduleAtFixedRate(Executor::Task::UP task, duration delay,
                                                                         duration interval) {
    std::lock_guard                 guard(_lock);
    ...
    auto                            handle =
        _scheduler.scheduleAtFixedRate(makeLambdaTask([&, my_task = std::move(my_task), my_state = state.get()]() {
