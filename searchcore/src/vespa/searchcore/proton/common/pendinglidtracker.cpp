...
ILidCommitState::State PendingLidTrackerBase::waitState(State state, uint32_t lid) const {
    MonitorGuard guard(_mutex);
    return waitFor(guard, state, lid);
}

ILidCommitState::State PendingLidTrackerBase::waitState(State state, const LidList& lids) const {
    MonitorGuard guard(_mutex);
...
ILidCommitState::State PendingLidTracker::waitFor(MonitorGuard& guard, State state, uint32_t lid) const {
...
PendingLidTrackerBase::Snapshot PendingLidTracker::produceSnapshot() {
    return Snapshot();
}
