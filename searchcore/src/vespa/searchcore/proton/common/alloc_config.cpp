...
AllocConfig::AllocConfig(const AllocStrategy& alloc_strategy, uint32_t redundancy, uint32_t searchable_copies)
    : _alloc_strategy(alloc_strategy),
      _redundancy(redundancy),
      _searchable_copies(std::min(searchable_copies, redundancy)) {
    if (searchable_copies > redundancy) {
        LOG(error,
            "searchablecopies(%d) larger than redundancy(%d)."
...
bool AllocConfig::operator==(const AllocConfig& rhs) const noexcept {
    return ((_alloc_strategy == rhs._alloc_strategy) && (_redundancy == rhs._redundancy) &&
            (_searchable_copies == rhs._searchable_copies));
}

AllocStrategy AllocConfig::make_alloc_strategy(SubDbType sub_db_type) const {
    ...
    GrowStrategy grow_strategy(initial_capacity, baseline.getGrowFactor(), baseline.getGrowDelta(), initial_capacity,
                               baseline.getMultiValueAllocGrowFactor());
