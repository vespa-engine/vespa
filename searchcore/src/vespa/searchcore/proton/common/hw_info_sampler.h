...
public:
    struct Config {
        ...
        Config(uint64_t diskSizeBytes_, double diskWriteSpeedOverride_, double slowWriteSpeedLimit_,
               double diskSampleWriteSize_, bool diskShared_, uint64_t memorySizeBytes_, uint32_t cpuCores_)
...
private:
    ...
    void setup(const vespalib::HwInfo::Disk& disk, const vespalib::HwInfo::Memory& memory,
               const vespalib::HwInfo::Cpu& cpu);
