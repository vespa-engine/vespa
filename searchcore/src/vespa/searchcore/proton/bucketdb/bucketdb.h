...
private:
    ...
    void addActive(size_t value) { _numActiveDocs.store(getNumActiveDocs() + value, std::memory_order_relaxed); }
    void subActive(size_t value) { _numActiveDocs.store(getNumActiveDocs() - value, std::memory_order_relaxed); }
...
public:
    ...
    const BucketState& add(const GlobalId& gid, BucketId bucketId, Timestamp timestamp, uint32_t docSize,
                           SubDbType subDbType);

    void add(BucketId bucketId, const BucketState& state);
    void remove(const GlobalId& gid, BucketId bucketId, Timestamp timestamp, uint32_t docSize, SubDbType subDbType);

    void remove_batch(const std::vector<bucketdb::RemoveBatchEntry>& removed, SubDbType sub_db_type);

    void modify(const GlobalId& gid, BucketId oldBucketId, Timestamp oldTimestamp, uint32_t oldDocSize,
