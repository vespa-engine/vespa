...
BucketDB::BucketDB() : _map(), _numActiveDocs(0), _cachedBucketId(), _cachedBucketState() {
}
...
size_t BucketDB::countActiveDocs() const {
    size_t activeDocs = 0;
...
void BucketDB::add(BucketId bucketId, const BucketState& delta) {
...
bucketdb::BucketState* BucketDB::getBucketStatePtr(BucketId bucket) {
...
const bucketdb::BucketState& BucketDB::add(const GlobalId& gid, BucketId bucketId, Timestamp timestamp,
                                           uint32_t docSize, SubDbType subDbType) {
...
void BucketDB::remove(const GlobalId& gid, BucketId bucketId, Timestamp timestamp, uint32_t docSize,
                      SubDbType subDbType) {
    BucketState& state = _map[bucketId];
...
void BucketDB::remove_batch(const std::vector<RemoveBatchEntry>& removed, SubDbType sub_db_type) {
...
void BucketDB::modify(const GlobalId& gid, BucketId oldBucketId, Timestamp oldTimestamp, uint32_t oldDocSize,
                      BucketId newBucketId, Timestamp newTimestamp, uint32_t newDocSize, SubDbType subDbType) {
...
bucketdb::BucketState BucketDB::get(BucketId bucketId) const {
    auto itr = _map.find(bucketId);
    return (itr != _map.end()) ? itr->second : BucketState();
}

void BucketDB::cacheBucket(BucketId bucketId) {
    _cachedBucketId = bucketId;
    _cachedBucketState = get(bucketId);
}

void BucketDB::uncacheBucket() {
    _cachedBucketId = BucketId();
    _cachedBucketState = BucketState();
}

bool BucketDB::isCachedBucket(BucketId bucketId) const {
    return _cachedBucketId == bucketId;
}

bucketdb::BucketState BucketDB::cachedGet(BucketId bucketId) const {
...
storage::spi::BucketInfo BucketDB::cachedGetBucketInfo(BucketId bucketId) const {
    if (isCachedBucket(bucketId)) {
        return _cachedBucketState;
    }
    return get(bucketId);
...
bool BucketDB::hasBucket(BucketId bucketId) const {
    return (_map.find(bucketId) != _map.end());
}

bool BucketDB::isActiveBucket(BucketId bucketId) const {
    auto itr = _map.find(bucketId);
    return (itr != _map.end()) && itr->second.isActive();
}

document::BucketId::List BucketDB::getBuckets() const {
...
void BucketDB::createBucket(BucketId bucketId) {
    BucketState& state = _map[bucketId];
    (void)state;
}

void BucketDB::deleteEmptyBucket(BucketId bucketId) {
    auto itr = _map.find(bucketId);
    if (itr == _map.end()) {
        return;
    }
...
document::BucketId::List BucketDB::getActiveBuckets() const {
    BucketId::List buckets;
    buckets.reserve(_map.size());
    for (const auto& entry : _map) {
...
document::BucketId::List BucketDB::populateActiveBuckets(BucketId::List buckets) {
    BucketId::List toAdd;
...
bool BucketDB::validateIntegrity() const {
    checkActiveCount();
    return true;
}
