...
uint32_t toIdx(SubDbType subDbType) {
...
void BucketState::setChecksumType(ChecksumType type) {
    _checksumType = type;
}
...
BucketState::BucketState() : _ch(), _docSizes(), _docCount(), _active(false) {
...
BucketState::BucketChecksum BucketState::getChecksum() const {
...
BucketState::BucketChecksum BucketState::addChecksum(BucketChecksum a, BucketChecksum b) {
    switch (_checksumType) {
    case ChecksumAggregator::ChecksumType::LEGACY:
