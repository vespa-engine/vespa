...
void DocumentOperation::assertValidBucketId(const document::DocumentId& docId) const {
...
std::string DocumentOperation::docArgsToString() const {
    return make_string("%s, timestamp=%" PRIu64 ", dbdId=(%s), prevDbdId=(%s), "
                       ...
                       _bucketId.toString().c_str(), _timestamp, _dbdId.toString().c_str(),
                       _prevDbdId.toString().c_str(), (_prevMarkedAsRemoved ? "true" : "false"), _prevTimestamp,
                       getSerialNum());
}

void DocumentOperation::serialize(vespalib::nbostream& os) const {
    serializeDocumentOperationOnly(os);
}

void DocumentOperation::serializeDocumentOperationOnly(vespalib::nbostream& os) const {
    os << _bucketId;
    os << _timestamp;
    os << _dbdId;
    os << _prevDbdId;
...
void DocumentOperation::deserialize(vespalib::nbostream& is, const DocumentTypeRepo&) {
    is >> _bucketId;
    is >> _timestamp;
    is >> _dbdId;
    is >> _prevDbdId;
