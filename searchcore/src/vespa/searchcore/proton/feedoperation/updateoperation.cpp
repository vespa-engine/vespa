...
UpdateOperation::UpdateOperation() : UpdateOperation(FeedOperation::UPDATE) {
}

UpdateOperation::UpdateOperation(Type type) : DocumentOperation(type), _upd() {
}

UpdateOperation::UpdateOperation(Type type, const BucketId& bucketId, Timestamp timestamp, DocumentUpdate::SP upd)
    : DocumentOperation(type, bucketId, timestamp), _upd(std::move(upd)) {
}
...
void UpdateOperation::deserializeUpdate(vespalib::nbostream&& is, const document::DocumentTypeRepo& repo) {
    _upd = DocumentUpdate::createHEAD(repo, std::move(is));
}

void UpdateOperation::serialize(vespalib::nbostream& os) const {
...
void UpdateOperation::deserialize(vespalib::nbostream& is, const DocumentTypeRepo& repo) {
...
void UpdateOperation::verifyUpdate(const DocumentTypeRepo& repo) {
...
std::string UpdateOperation::toString() const {
    return make_string("%s(%s, %s)", ((getType() == FeedOperation::UPDATE_42) ? "Update42" : "Update"),
                       _upd.get() ? _upd->getId().getScheme().toString().c_str() : "NULL", docArgsToString().c_str());
}
