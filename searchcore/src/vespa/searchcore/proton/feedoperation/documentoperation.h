...
public:
    ...
    bool getValidPrevDbdId(uint32_t subDbId) const {
    ...
    bool getValidNewOrPrevDbdId() const { return getValidDbdId() || getValidPrevDbdId(); }

    bool notMovingLidInSameSubDb() const {
        return !getValidDbdId() || !getValidPrevDbdId() || getSubDbId() != getPrevSubDbId() ||
               getLid() == getPrevLid();
    }
