...
JoinBucketsOperation::JoinBucketsOperation()
    : FeedOperation(FeedOperation::JOIN_BUCKETS), _source1(), _source2(), _target() {
}

JoinBucketsOperation::JoinBucketsOperation(const document::BucketId& source1, const document::BucketId& source2,
                                           const document::BucketId& target)
    : FeedOperation(FeedOperation::JOIN_BUCKETS), _source1(source1), _source2(source2), _target(target) {
}
...
void JoinBucketsOperation::deserialize(vespalib::nbostream& is, const DocumentTypeRepo&) {
    is >> _source1;
    is >> _source2;
    is >> _target;
}
...
std::string JoinBucketsOperation::toString() const {
    return make_string("JoinBuckets(source1=%s, source2=%s, target=%s, "
                       "serialNum=%" PRIu64 ")",
                       _source1.toString().c_str(), _source2.toString().c_str(), _target.toString().c_str(),
                       getSerialNum());
}
