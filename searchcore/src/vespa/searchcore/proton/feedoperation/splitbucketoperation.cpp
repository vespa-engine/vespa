...
SplitBucketOperation::SplitBucketOperation()
    : FeedOperation(FeedOperation::SPLIT_BUCKET), _source(), _target1(), _target2() {
}

SplitBucketOperation::SplitBucketOperation(const document::BucketId& source, const document::BucketId& target1,
                                           const document::BucketId& target2)
    : FeedOperation(FeedOperation::SPLIT_BUCKET), _source(source), _target1(target1), _target2(target2) {
}
...
void SplitBucketOperation::deserialize(vespalib::nbostream& is, const DocumentTypeRepo&) {
    is >> _source;
    is >> _target1;
    is >> _target2;
}
...
std::string SplitBucketOperation::toString() const {
    return make_string("SplitBucket(source=%s, target1=%s, target2=%s, "
                       "serialNum=%" PRIu64 ")",
                       _source.toString().c_str(), _target1.toString().c_str(), _target2.toString().c_str(),
                       getSerialNum());
}
