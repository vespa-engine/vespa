...
DocumentMetaStoreContext::ReadGuard::ReadGuard(const std::shared_ptr<search::AttributeVector>& metaStoreAttr)
    : _guard(metaStoreAttr), _store(static_cast<const DocumentMetaStore&>(*_guard)) {
}

const search::IDocumentMetaStore& DocumentMetaStoreContext::ReadGuard::get() const {
    return _store;
}
...
DocumentMetaStoreContext::DocumentMetaStoreContext(std::shared_ptr<bucketdb::BucketDBOwner> bucketDB,
                                                   const std::string& name, const search::GrowStrategy& grow)
...
void DocumentMetaStoreContext::constructFreeList() {
