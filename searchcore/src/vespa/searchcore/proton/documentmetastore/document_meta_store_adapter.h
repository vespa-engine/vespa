...
public:
    void commit(const CommitParam& param) override { doCommit(param); }
    DocId getCommittedDocIdLimit() const override { return doGetCommittedDocIdLimit(); }
    void reclaim_unused_memory() override { doRemoveAllOldGenerations(); }
    vespalib::Generation getCurrentGeneration() const override { return doGetCurrentGeneration(); }
};
