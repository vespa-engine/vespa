...
LidStateVector::LidStateVector(unsigned int newSize, unsigned int newCapacity, GenerationHolder& generationHolder,
...
void LidStateVector::resizeVector(uint32_t newSize, uint32_t newCapacity) {
...
uint32_t LidStateVector::assert_is_not_set_then_set_bits_helper(const std::vector<uint32_t>& idxs) {
...
uint32_t LidStateVector::assert_not_set_bits(const std::vector<uint32_t>& idxs) {
    return assert_is_not_set_then_set_bits_helper<false>(idxs);
}

uint32_t LidStateVector::set_bits(const std::vector<uint32_t>& idxs) {
    return assert_is_not_set_then_set_bits_helper<true>(idxs);
}

void LidStateVector::clearBit(unsigned int idx) {
    assert(idx < _bv.writer().size());
    assert(_bv.writer().testBit(idx));
...
void LidStateVector::assert_is_set_then_clear_bits_helper(const std::vector<uint32_t>& idxs) {
    for (auto idx : idxs) {
        if (do_assert) {
            assert(_bv.writer().testBit(idx));
        }
...
void LidStateVector::consider_clear_bits(const std::vector<uint32_t>& idxs) {
    assert_is_set_then_clear_bits_helper<false>(idxs);
}

void LidStateVector::clear_bits(const std::vector<uint32_t>& idxs) {
