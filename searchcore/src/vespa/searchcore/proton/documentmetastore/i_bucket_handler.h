...
struct IBucketHandler {
     ...
     */
    virtual bucketdb::BucketDeltaPair handleSplit(const bucketdb::SplitBucketSession& session) = 0;

    /**
     * Join the two source buckets into a target bucket.
     */
    virtual bucketdb::BucketDeltaPair handleJoin(const bucketdb::JoinBucketsSession& session) = 0;
