...
FlushStrategyIdNotifier::FlushStrategyIdNotifier(uint32_t strategy_id)
    : _lock(), _cond(), _strategy_id(strategy_id), _closed(false) {
}

FlushStrategyIdNotifier::~FlushStrategyIdNotifier() = default;

void FlushStrategyIdNotifier::set_strategy_id(uint32_t strategy_id) {
...
void FlushStrategyIdNotifier::close() {
    std::unique_lock guard(_lock);
    _closed = true;
    _cond.notify_all();
    Listeners listeners;
...
void FlushStrategyIdNotifier::wait_gt_strategy_id(uint32_t strategy_id) {
    std::unique_lock guard(_lock);
...
bool FlushStrategyIdNotifier::add_strategy_id_listener(std::shared_ptr<FlushStrategyIdListener> listener) {
    std::unique_lock guard(_lock);
    if (_closed) {
        return false;
    }
...
void FlushStrategyIdNotifier::remove_strategy_id_listener(std::shared_ptr<FlushStrategyIdListener> listener) {
    std::unique_lock guard(_lock);
