...
TlsStatsFactory::TlsStatsFactory(TransLogServer::SP tls) : _tls(tls) {
}

TlsStatsFactory::~TlsStatsFactory() {
}

TlsStatsMap TlsStatsFactory::create() {
    DomainStats      stats = _tls->getDomainStats();
    TlsStatsMap::Map map;
    for (auto& itr : stats) {
        const DomainInfo& di = itr.second;
        map.insert(std::make_pair(itr.first, TlsStats(di.byteSize, di.range.from(), di.range.to())));
    }
