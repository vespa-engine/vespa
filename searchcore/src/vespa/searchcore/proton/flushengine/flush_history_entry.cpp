...
FlushHistoryEntry::FlushHistoryEntry(std::string name_in, std::string strategy_in, uint32_t strategy_id_in,
    ...
    : _name(std::move(name_in)),
      ...
      _strategy_info(strategy_info_in),
...
void FlushHistoryEntry::start_flush(time_point start_time_in, uint32_t id_in) noexcept {
    _start_time = start_time_in;
    _id = id_in;
}

void FlushHistoryEntry::flush_done(time_point finish_time_in) noexcept {
    _finish_time = finish_time_in;
}

void FlushHistoryEntry::prune_done(time_point prune_time_in) noexcept {
