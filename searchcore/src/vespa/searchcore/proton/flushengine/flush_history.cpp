...
template <typename Key, typename Value> std::vector<Value> make_value_vector(const std::map<Key, Value>& entries) {
...
std::string FlushHistory::build_name(const std::string& handler_name, const std::string& target_name) {
    return handler_name + "." + target_name;
}

void FlushHistory::prune_finished() {
...
void FlushHistory::prune_finished_strategies() {
    auto     it = _finished_strategies.begin();
    uint32_t keep_strategy_entries = 10;
    uint32_t scanned = 0;
    for (; it != _finished_strategies.end() && scanned + keep_strategy_entries < _finished_strategies.size();
         ++it, ++scanned)
    {
    }
...
void FlushHistory::prune_draining_strategies(time_point now) {
    while (!_draining_strategies.empty()) {
        ...
        if (strategy.priority_strategy()) {
            LOG(info, "Priority flush strategy %s id %u drained, %u flushes, %u inherited flushes",
                strategy.name().c_str(), strategy.id(), strategy.finished_flushes(),
                strategy.inherited_finished_flushes());
        }
...
void FlushHistory::start_flush(const std::string& handler_name, const std::string& target_name,
                               ...
                               duration estimated_flush_duration, uint32_t id) {
    ...
    if (pending_it != _pending.end() && pending_it->first == name) {
        ...
        auto now = steady_clock::now();
        active_it =
            _active.emplace_hint(active_it, id,
                                 FlushHistoryEntry(name, _active_strategy, strategy_info, now, last_flush_duration,
                                                   estimated_flush_duration, ++_pending_id));
    }
...
void FlushHistory::add_pending_flush(const std::string& handler_name, const std::string& target_name,
                                     const std::string& strategy_info, duration last_flush_duration,
                                     duration estimated_flush_duration) {
    ...
    auto            now = steady_clock::now();
    if (pending_it != _pending.end() && pending_it->first == name) {
        pending_it->second = FlushHistoryEntry(name, _active_strategy, strategy_info, now, last_flush_duration,
                                               estimated_flush_duration, ++_pending_id);
    } else {
...
void FlushHistory::drop_pending_flush(const std::string& handler_name, const std::string& target_name) {
...
void FlushHistory::clear_pending_flushes() {
    std::lock_guard guard(_mutex);
    _pending.clear();
}

void FlushHistory::set_strategy(std::string strategy, uint32_t strategy_id, bool priority_strategy) {
    std::lock_guard guard(_mutex);
    auto            now = steady_clock::now();
...
std::shared_ptr<const FlushHistoryView> FlushHistory::make_view() const {
    ...
    guard.unlock();
    std::sort(pending_copy.begin(), pending_copy.end(),
              [](const auto& lhs, const auto& rhs) { return lhs.id() < rhs.id(); });
    return std::make_shared<FlushHistoryView>(
        strategy_id_base_copy, _max_concurrent_normal, std::move(finished_copy), std::move(active_copy),
        std::move(pending_copy), std::move(finished_strategies_copy), std::move(draining_strategies_copy),
        std::move(active_strategy_copy), std::move(last_strategies_copy));
}
