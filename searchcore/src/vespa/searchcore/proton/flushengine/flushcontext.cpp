...
FlushContext::FlushContext(const IFlushHandler::SP& handler, const IFlushTarget::SP& target,
                           search::SerialNum lastSerial)
    : _name(createName(*handler, *target)), _handler(handler), _target(target), _task(), _lastSerial(lastSerial) {
}

std::string FlushContext::createName(const IFlushHandler& handler, const IFlushTarget& target) {
    return create_name(handler.getName(), target.getName());
}

std::string FlushContext::create_name(const std::string& handler_name, const std::string& target_name) {
...
bool FlushContext::initFlush(std::shared_ptr<search::IFlushToken> flush_token) {
