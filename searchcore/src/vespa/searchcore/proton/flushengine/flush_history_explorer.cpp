...
void convert_to_slime(const FlushHistoryEntry& entry, Inserter& inserter) {
...
void convert_to_slime(const FlushStrategyHistoryEntry& entry, Inserter& inserter) {
    Cursor& object = inserter.insertObject();
...
void convert_to_slime(const std::vector<FlushHistoryEntry>& entries, Inserter& inserter) {
...
void convert_to_slime(const std::vector<FlushStrategyHistoryEntry>& entries, Inserter& inserter) {
    ArrayInserter array(inserter.insertArray());
    for (const auto& entry : entries) {
        convert_to_slime(entry, array);
    }
...
void FlushHistoryExplorer::get_state(const vespalib::slime::Inserter& inserter, bool full) const {
    Cursor& object = inserter.insertObject();
    if (full) {
