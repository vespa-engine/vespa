...
CommitAndWaitDocumentRetriever::CommitAndWaitDocumentRetriever(IDocumentRetriever::SP retriever,
    ...
    : _retriever(std::move(retriever)), _uncommittedLidsTracker(unCommittedLidTracker) {
}

CommitAndWaitDocumentRetriever::~CommitAndWaitDocumentRetriever() = default;

const document::DocumentTypeRepo& CommitAndWaitDocumentRetriever::getDocumentTypeRepo() const {
...
void CommitAndWaitDocumentRetriever::getBucketMetadata(const Bucket& bucket, search::DocumentMetadata::Vector& result,
...
search::DocumentMetadata CommitAndWaitDocumentRetriever::getDocumentMetadata(const document::DocumentId& id) const {
    return _retriever->getDocumentMetadata(id);
}

document::Document::UP CommitAndWaitDocumentRetriever::getFullDocument(search::DocumentIdT lid) const {
    // Ensure that attribute vectors are committed
    _uncommittedLidsTracker.waitComplete(lid);
    return _retriever->getFullDocument(lid);
}

document::Document::UP CommitAndWaitDocumentRetriever::getPartialDocument(search::DocumentIdT         lid,
...
void CommitAndWaitDocumentRetriever::visitDocuments(const LidVector& lids, search::IDocumentVisitor& visitor,
...
CachedSelect::SP CommitAndWaitDocumentRetriever::parseSelect(const std::string& selection) const {
    return _retriever->parseSelect(selection);
}

IDocumentRetriever::ReadGuard CommitAndWaitDocumentRetriever::getReadGuard() const {
    return _retriever->getReadGuard();
}

uint32_t CommitAndWaitDocumentRetriever::getDocIdLimit() const {
