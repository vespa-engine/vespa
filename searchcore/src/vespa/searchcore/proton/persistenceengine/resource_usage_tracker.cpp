...
storage::spi::ResourceUsage ResourceUsageTracker::get_resource_usage() const {
    std::lock_guard guard(_lock);
    return _resource_usage;
}

void ResourceUsageTracker::notify_resource_usage(const ResourceUsageState& state) {
    std::lock_guard guard(_lock);
    std::string     name;
...
void ResourceUsageTracker::remove_listener() {
    std::lock_guard guard(_lock);
    _listener = nullptr;
}
