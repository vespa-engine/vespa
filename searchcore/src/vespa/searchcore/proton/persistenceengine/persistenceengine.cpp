...
protected:
    ...
    void countDown();
    void complete() { _onComplete->onComplete(createResult()); }
...
ResultHandlerBase::ResultHandlerBase(uint32_t waitCnt, OperationComplete::UP onComplete)
    : _lock(), _countDown(waitCnt), _onComplete(std::move(onComplete)) {
}
ResultHandlerBase::~ResultHandlerBase() = default;

void ResultHandlerBase::countDown() {
...
public:
    GenericResultHandler(uint32_t waitCnt, OperationComplete::UP onComplete)
        : ResultHandlerBase(waitCnt, std::move(onComplete)), _result() {
        if (waitCnt == 0) {
            complete();
        }
    }
...
public:
    BucketIdListResultHandler() : _bucketSet() {}
...
class SynchronizedBucketIdListResultHandler : public ResultHandlerBase, public BucketIdListResultHandler {
public:
    SynchronizedBucketIdListResultHandler(uint32_t waitCnt, OperationComplete::UP onComplete)
        : ResultHandlerBase(waitCnt, std::move(onComplete)), BucketIdListResultHandler() {
        if (waitCnt == 0) {
            complete();
        }
    }
    ...
    Result::UP createResult() const override { return getResult(); }
};
...
public:
    BucketInfoResultHandler() : _result(BucketInfo()), _first(true) {}
    ...
    void handle(const BucketInfoResult& result) override {
        if (_first) {
            ...
            BucketInfo::ReadyState  ready = (b1.getReady() == b2.getReady() ? b1.getReady() : BucketInfo::NOT_READY);
            BucketInfo::ActiveState active =
                (b1.getActive() == b2.getActive() ? b1.getActive() : BucketInfo::NOT_ACTIVE);
            _result = BucketInfoResult(BucketInfo(
                BucketChecksum(b1.getChecksum() + b2.getChecksum()), b1.getDocumentCount() + b2.getDocumentCount(),
                b1.getDocumentSize() + b2.getDocumentSize(), b1.getEntryCount() + b2.getEntryCount(),
                b1.getUsedSize() + b2.getUsedSize(), ready, active));
        }
    }
...
PersistenceEngine::HandlerSnapshot PersistenceEngine::getHandlerSnapshot(const WriteGuard&) const {
...
PersistenceEngine::HandlerSnapshot PersistenceEngine::getHandlerSnapshot(const WriteGuard&,
                                                                         document::BucketSpace bucketSpace) const {
    return _handlers.getHandlerSnapshot(bucketSpace);
}
...
IPersistenceHandler::SP PersistenceEngine::putHandler(const WriteGuard&, document::BucketSpace bucketSpace,
                                                      const DocTypeName&             docType,
                                                      const IPersistenceHandler::SP& handler) {
    return _handlers.putHandler(bucketSpace, docType, handler);
}

IPersistenceHandler* PersistenceEngine::getHandler(const ReadGuard&, document::BucketSpace bucketSpace,
                                                   const DocTypeName& docType) const {
    return _handlers.getHandler(bucketSpace, docType);
}

IPersistenceHandler::SP PersistenceEngine::removeHandler(const WriteGuard&, document::BucketSpace bucketSpace,
                                                         const DocTypeName& docType) {
    // TODO: Grab bucket list and treat them as modified
    return _handlers.removeHandler(bucketSpace, docType);
}

Result PersistenceEngine::initialize() {
...
BucketIdListResult PersistenceEngine::listBuckets(BucketSpace bucketSpace) const {
    // Runs in SPI thread.
...
Result PersistenceEngine::setClusterState(BucketSpace bucketSpace, const ClusterState& calc) {
    ReadGuard rguard(_rwMutex);
...
void PersistenceEngine::setActiveStateAsync(const Bucket& bucket, BucketInfo::ActiveState newState,
...
BucketInfoResult PersistenceEngine::getBucketInfo(const Bucket& b) const {
    // Runs in SPI thread.
    // No handover to write threads in persistence handlers.
    ReadGuard               rguard(_rwMutex);
...
void PersistenceEngine::putAsync(const Bucket& bucket, Timestamp ts, storage::spi::DocumentSP doc,
                                 OperationComplete::UP onComplete) {
    if (!_writeFilter.acceptWriteOperation()) {
        ...
        if (!state.acceptWriteOperation()) {
            return onComplete->onComplete(
                std::make_unique<Result>(Result::ErrorType::RESOURCE_EXHAUSTED,
                                         fmt("Put operation rejected for document '%s': '%s'",
                                             doc->getId().toString().c_str(), state.message().c_str())));
        }
    }
    ReadGuard   rguard(_rwMutex);
    DocTypeName docType(doc->getType());
    LOG(spam, "putAsync(%s, %" PRIu64 ", (\"%s\", \"%s\"))", bucket.toString().c_str(),
        static_cast<uint64_t>(ts.getValue()), docType.toString().c_str(), doc->getId().toString().c_str());
    if (!doc->getId().hasDocType()) {
        return onComplete->onComplete(std::make_unique<Result>(
    ...
    if (!handler) {
        return onComplete->onComplete(
            std::make_unique<Result>(Result::ErrorType::PERMANENT_ERROR,
...
void PersistenceEngine::removeAsync(const Bucket& b, std::vector<storage::spi::IdAndTimestamp> ids,
                                    OperationComplete::UP onComplete) {
    if (ids.size() == 1) {
...
void PersistenceEngine::removeAsyncMulti(const Bucket& b, std::vector<storage::spi::IdAndTimestamp> ids,
                                         OperationComplete::UP onComplete) {
    ReadGuard rguard(_rwMutex);
    ...
    for (const auto& stampedId : ids) {
        ...
        if (!id.hasDocType()) {
            return onComplete->onComplete(std::make_unique<RemoveResult>(
                Result::ErrorType::PERMANENT_ERROR,
...
void PersistenceEngine::removeAsyncSingle(const Bucket& b, Timestamp t, const DocumentId& id,
                                          OperationComplete::UP onComplete) {
    ReadGuard rguard(_rwMutex);
    LOG(spam, "remove(%s, %" PRIu64 ", \"%s\")", b.toString().c_str(), static_cast<uint64_t>(t.getValue()),
        id.toString().c_str());
    if (!id.hasDocType()) {
        return onComplete->onComplete(std::make_unique<RemoveResult>(
            Result::ErrorType::PERMANENT_ERROR,
...
void PersistenceEngine::removeByGidAsync(const Bucket& b, std::vector<storage::spi::DocTypeGidAndTimestamp> ids,
    ...
    for (const auto& dt_gid_ts : ids) {
        ...
        if (!handler) {
            return onComplete->onComplete(std::make_unique<RemoveResult>(
                Result::ErrorType::PERMANENT_ERROR,
                fmt("No handler for document type '%s'", doc_type.toString().c_str())));
        }
    }
    auto transportContext = std::make_shared<AsyncRemoveTransportContext>(ids.size(), std::move(onComplete));
    for (const auto& dt_gid_ts : ids) {
        DocTypeName          doc_type(dt_gid_ts.doc_type);
        IPersistenceHandler* handler = getHandler(rguard, b.getBucketSpace(), doc_type);
        handler->handleRemoveByGid(feedtoken::make(transportContext), b, dt_gid_ts.timestamp, dt_gid_ts.doc_type,
                                   dt_gid_ts.gid);
    }
}

void PersistenceEngine::updateAsync(const Bucket& b, Timestamp t, DocumentUpdate::SP upd,
                                    OperationComplete::UP onComplete) {
    if (!_writeFilter.acceptWriteOperation()) {
        IResourceWriteFilter::State state = _writeFilter.getAcceptState();
        if (!state.acceptWriteOperation() && document::FeedRejectHelper::mustReject(*upd)) {
            return onComplete->onComplete(
                std::make_unique<UpdateResult>(Result::ErrorType::RESOURCE_EXHAUSTED,
                                               fmt("Update operation rejected for document '%s': '%s'",
    ...
    try {
        ...
        return onComplete->onComplete(std::make_unique<UpdateResult>(
            Result::ErrorType::TRANSIENT_ERROR,
        ...
        return onComplete->onComplete(
            std::make_unique<UpdateResult>(Result::ErrorType::TRANSIENT_ERROR,
    ...
    LOG(spam, "update(%s, %" PRIu64 ", (\"%s\", \"%s\"), createIfNonExistent='%s')", b.toString().c_str(),
        static_cast<uint64_t>(t.getValue()), docType.toString().c_str(), upd->getId().toString().c_str(),
    ...
    if (upd->getId().getDocType() != docType.getName()) {
        return onComplete->onComplete(std::make_unique<UpdateResult>(
            Result::ErrorType::PERMANENT_ERROR, fmt("Update operation rejected due to bad id (%s, %s)",
                                                    upd->getId().toString().c_str(), docType.getName().c_str())));
    }
    IPersistenceHandler* handler = getHandler(rguard, b.getBucketSpace(), docType);

    if (handler == nullptr) {
        return onComplete->onComplete(
            std::make_unique<UpdateResult>(Result::ErrorType::PERMANENT_ERROR,
                                           fmt("No handler for document type '%s'", docType.toString().c_str())));
    }
    auto transportContext = std::make_shared<AsyncTransportContext>(1, std::move(onComplete));
...
PersistenceEngine::GetResult PersistenceEngine::get(const Bucket& b, const document::FieldSet& fields,
                                                    const DocumentId& did, Context& context) const {
    ReadGuard rguard(_rwMutex);
    auto      snap = getHandlerSnapshot(rguard, b.getBucketSpace());

    for (; snap.handlers().valid(); snap.handlers().next()) {
        IPersistenceHandler::RetrieversSP retrievers =
            snap.handlers().get()->getDocumentRetrievers(context.getReadConsistency());
...
PersistenceEngine::CreateIteratorResult PersistenceEngine::createIterator(const Bucket& bucket, FieldSetSP fields,
...
PersistenceEngine::IterateResult PersistenceEngine::iterate(IteratorId id, uint64_t maxByteSize) const {
    ReadGuard      rguard(_rwMutex);
    IteratorEntry* iteratorEntry;
    {
        ...
        if (it == _iterators.end()) {
            return IterateResult(Result::ErrorType::PERMANENT_ERROR,
                                 fmt("Unknown iterator with id %" PRIu64, id.getValue()));
        }
        iteratorEntry = it->second;
        if (iteratorEntry->in_use) {
            return IterateResult(Result::ErrorType::TRANSIENT_ERROR,
                                 fmt("Iterator with id %" PRIu64 " is already in use", id.getValue()));
        }
    ...
    try {
        ...
        IterateResult result(Result::ErrorType::PERMANENT_ERROR,
                             fmt("Caught exception during visitor iterator.iterate() = '%s'", e.what()));
...
Result PersistenceEngine::destroyIterator(IteratorId id) {
    ReadGuard                   rguard(_rwMutex);
    std::lock_guard<std::mutex> guard(_iterators_lock);
    auto                        it = _iterators.find(id);
    if (it == _iterators.end()) {
    ...
    if (it->second->in_use) {
        return Result(Result::ErrorType::TRANSIENT_ERROR,
                      fmt("Iterator with id %" PRIu64 " is currently in use", id.getValue()));
    }
    delete it->second;
    _iterators.erase(it);
    return Result();
...
void PersistenceEngine::createBucketAsync(const Bucket& b, OperationComplete::UP onComplete) noexcept {
    ReadGuard rguard(_rwMutex);
...
void PersistenceEngine::deleteBucketAsync(const Bucket& b, OperationComplete::UP onComplete) noexcept {
    ReadGuard rguard(_rwMutex);
...
BucketIdListResult PersistenceEngine::getModifiedBuckets(BucketSpace bucketSpace) const {
    ReadGuard rguard(_rwMutex);
...
void PersistenceEngine::destroyIterators() {
    for (;;) {
        IteratorId id;
        {
            std::lock_guard<std::mutex> guard(_iterators_lock);
        ...
        if (res.hasError()) {
            LOG(debug, "%zu iterator left. Can not destroy iterator '%" PRIu64 "'. Reason='%s'", _iterators.size(),
                id.getValue(), res.toString().c_str());
            std::this_thread::sleep_for(100ms);
        }
    }
}

void PersistenceEngine::saveClusterState(BucketSpace bucketSpace, const ClusterState& calc) {
...
PersistenceEngine::ClusterState::SP PersistenceEngine::savedClusterState(BucketSpace bucketSpace) const {
    std::lock_guard<std::mutex> guard(_lock);
...
void PersistenceEngine::grabExtraModifiedBuckets(BucketSpace bucketSpace, IPersistenceHandler& handler) {
    BucketIdListResultHandler resultHandler;
    handler.handleListBuckets(resultHandler);
...
void PersistenceEngine::populateInitialBucketDB(const WriteGuard& guard, BucketSpace bucketSpace,
...
std::unique_lock<std::shared_mutex> PersistenceEngine::getWLock() const {
...
void PersistenceEngine::execute(const storage::spi::Bucket& bucket, std::unique_ptr<BucketTask> task) {
