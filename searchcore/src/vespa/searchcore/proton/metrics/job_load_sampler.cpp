...
JobLoadSampler::JobLoadSampler(time_point now)
    : _lastSampleTime(now), _lastUpdateTime(now), _currJobCnt(0), _loadIntegral(0) {
}

void JobLoadSampler::startJob(time_point now) {
    updateIntegral(now, _currJobCnt++);
}

void JobLoadSampler::endJob(time_point now) {
    updateIntegral(now, _currJobCnt--);
}
