...
void update_helper(metrics::LongValueMetric& metric, const DiskIoStats& stats) {
    metric.addTotalValueBatch(stats.read_bytes_total(), stats.read_operations(), stats.read_bytes_min(),
                              stats.read_bytes_max());
}
...
DiskIoMetrics::SearchMetrics::SearchMetrics(metrics::MetricSet* parent)
    : MetricSet("search", {}, "The search io for a given component", parent),
      ...
      _cached_read_bytes("cached_read_bytes", {}, "Bytes read from posting list files cache as part of search",
                         this) {
}

DiskIoMetrics::SearchMetrics::~SearchMetrics() = default;

void DiskIoMetrics::SearchMetrics::update(const FieldIndexIoStats& io_stats) {
...
DiskIoMetrics::DiskIoMetrics(metrics::MetricSet* parent)
    : MetricSet("io", {}, "The disk usage for a given component", parent), _search(this) {
}
