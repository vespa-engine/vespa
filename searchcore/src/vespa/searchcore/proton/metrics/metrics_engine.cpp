...
void MetricsEngine::start(const config::ConfigUri&) {
    {
...
void MetricsEngine::addMetricsHook(metrics::UpdateHook& hook) {
    _manager->addMetricUpdateHook(hook);
}

void MetricsEngine::removeMetricsHook(metrics::UpdateHook& hook) {
    _manager->removeMetricUpdateHook(hook);
}

void MetricsEngine::addExternalMetrics(metrics::Metric& child) {
    metrics::MetricLockGuard guard(_manager->getMetricLock());
    _root->registerMetric(child);
}

void MetricsEngine::removeExternalMetrics(metrics::Metric& child) {
    metrics::MetricLockGuard guard(_manager->getMetricLock());
    _root->unregisterMetric(child);
}

void MetricsEngine::addDocumentDBMetrics(DocumentDBTaggedMetrics& child) {
    metrics::MetricLockGuard guard(_manager->getMetricLock());
    _root->registerMetric(child);
}

void MetricsEngine::removeDocumentDBMetrics(DocumentDBTaggedMetrics& child) {
    metrics::MetricLockGuard guard(_manager->getMetricLock());
    _root->unregisterMetric(child);
}

void MetricsEngine::set_attributes(AttributeMetrics& subAttributes, std::vector<std::string> field_names) {
    metrics::MetricLockGuard guard(_manager->getMetricLock());
    subAttributes.set_fields(std::move(field_names));
}

void MetricsEngine::set_index_fields(IndexMetrics& index_fields, std::vector<std::string> field_names) {
    metrics::MetricLockGuard guard(_manager->getMetricLock());
...
void addRankProfileTo(MatchingMetricsType& matchingMetrics, const std::string& name, size_t numDocIdPartitions) {
...
template <typename MatchingMetricsType> void cleanRankProfilesIn(MatchingMetricsType& matchingMetrics) {
...
void MetricsEngine::addRankProfile(DocumentDBTaggedMetrics& owner, const std::string& name,
...
void MetricsEngine::cleanRankProfiles(DocumentDBTaggedMetrics& owner) {
    metrics::MetricLockGuard guard(_manager->getMetricLock());
