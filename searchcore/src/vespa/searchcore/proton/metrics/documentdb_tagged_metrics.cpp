...
DocumentDBTaggedMetrics::JobMetrics::JobMetrics(metrics::MetricSet* parent)
    : MetricSet("job", {}, "Job load average for various jobs in a document database", parent),
      ...
      documentStoreCompact("document_store_compact", {}, "Compaction of document store on disk", this),
      bucketMove("bucket_move", {}, "Moving of buckets between 'ready' and 'notready' sub databases", this),
      lidSpaceCompact("lid_space_compact", {}, "Compaction of lid space in document meta store and attribute vectors",
                      this),
      removedDocumentsPrune("removed_documents_prune", {}, "Pruning of removed documents in 'removed' sub database",
                            this),
...
DocumentDBTaggedMetrics::SubDBMetrics::LidSpaceMetrics::LidSpaceMetrics(MetricSet* parent)
    : MetricSet("lid_space", {}, "Local document id (lid) space metrics for this document sub DB", parent),
      ...
      lidBloatFactor(
          "lid_bloat_factor", {},
          "The bloat factor of this lid space, indicating the total amount of holes in the allocated lid space "
          "((lid_limit - used_lids) / lid_limit)",
          this),
      lidFragmentationFactor("lid_fragmentation_factor", {},
                             "The fragmentation factor of this lid space, indicating the amount of holes in the "
                             "currently used part of the lid space "
                             "((highest_used_lid - used_lids) / highest_used_lid)",
                             this) {
}
...
DocumentDBTaggedMetrics::SubDBMetrics::DocumentStoreMetrics::DocumentStoreMetrics(MetricSet* parent)
    : MetricSet("document_store", {}, "Document store metrics for this document sub DB", parent),
      ...
      maxBucketSpread(
          "max_bucket_spread", {},
          "Max bucket spread in underlying files (sum(unique buckets in each chunk)/unique buckets in file)", this),
      memoryUsage(this),
...
DocumentDBTaggedMetrics::AttributeMetrics::ResourceUsageMetrics::ResourceUsageMetrics(MetricSet* parent)
    : MetricSet("resource_usage", {}, "Metrics for various attribute vector resources usage", parent),
      address_space("address_space", {},
                    ...
                    "components in all attribute vectors in this document db (value in the range [0, 1])",
                    this),
      feedingBlocked(
          "feeding_blocked", {},
          "Whether feeding is blocked due to attribute resource limits being reached (value is either 0 or 1)",
          this) {
}
...
void DocumentDBTaggedMetrics::MatchingMetrics::update(const MatchingStats& stats) {
...
DocumentDBTaggedMetrics::MatchingMetrics::MatchingMetrics(MetricSet* parent)
    : MetricSet("matching", {}, "Matching metrics", parent),
      ...
      exact_nns_distances_computed("exact_nns_distances_computed", {},
                                   "Number of distance computations in exact nearest-neighbor search", this),
      approximate_nns_distances_computed("approximate_nns_distances_computed", {},
                                         "Number of distance computations in approximate nearest-neighbor search",
                                         this),
      ...
      query_approximate_nns_time(
          "query_approximate_nns_time", {},
          "Average time (sec) spent on approximate nearest-neighbor search in queries that perform such searches",
          this),
...
DocumentDBTaggedMetrics::MatchingMetrics::RankProfileMetrics::DocIdPartition::DocIdPartition(const std::string& name,
                                                                                             MetricSet* parent)
...
void DocumentDBTaggedMetrics::MatchingMetrics::RankProfileMetrics::DocIdPartition::update(
    const MatchingStats::Partition& stats) {
    docsMatched.inc(stats.docsMatched());
    docsRanked.inc(stats.docsRanked());
    docsReRanked.inc(stats.docsReRanked());
    activeTime.addValueBatch(stats.active_time_avg(), stats.active_time_count(), stats.active_time_min(),
                             stats.active_time_max());
    waitTime.addValueBatch(stats.wait_time_avg(), stats.wait_time_count(), stats.wait_time_min(),
                           stats.wait_time_max());
}

void DocumentDBTaggedMetrics::MatchingMetrics::RankProfileMetrics::update(const metrics::MetricLockGuard&,
    ...
    groupingTime.addValueBatch(stats.groupingTimeAvg(), stats.groupingTimeCount(), stats.groupingTimeMin(),
                               stats.groupingTimeMax());
    rerankTime.addValueBatch(stats.rerankTimeAvg(), stats.rerankTimeCount(), stats.rerankTimeMin(),
                             stats.rerankTimeMax());
    querySetupTime.addValueBatch(stats.querySetupTimeAvg(), stats.querySetupTimeCount(), stats.querySetupTimeMin(),
                                 stats.querySetupTimeMax());
    query_approximate_nns_time.addValueBatch(stats.approximate_nns_time_avg(), stats.approximate_nns_time_count(),
    ...
    if (stats.getNumPartitions() > 0) {
        for (size_t i = partitions.size(); i < stats.getNumPartitions(); ++i) {
            // This loop is to handle live reconfigs that changes how many partitions(number of threads) might be used
            // per query.
            std::string partition(vespalib::make_string("docid_part%02ld", i));
            partitions.push_back(std::make_unique<DocIdPartition>(partition, this));
            LOG(info,
                "Number of partitions has been increased to '%ld' from '%ld' previously configured. Adding part %ld",
