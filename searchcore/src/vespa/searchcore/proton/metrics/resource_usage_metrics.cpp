...
ResourceUsageMetrics::DetailedResourceMetrics::DetailedResourceMetrics(const std::string&  resource_type,
                                                                       metrics::MetricSet* parent)
    : MetricSet(make_string("%s_usage", resource_type.c_str()), {},
                make_string("Detailed resource usage metrics for %s", resource_type.c_str()), parent),
      total("total", {},
            make_string("The total relative amount of %s used by this content node (value in the range [0, 1])",
                        resource_type.c_str()),
            this),
      total_util("total_utilization", {},
                 make_string("The relative amount of %s used compared to the content node %s resource limit",
                             resource_type.c_str(), resource_type.c_str()),
                 this),
      transient(
          "transient", {},
...
ResourceUsageMetrics::ResourceUsageMetrics(metrics::MetricSet* parent)
    : MetricSet("resource_usage", {}, "Usage metrics for various resources in this content node", parent),
      disk("disk", {},
           "The relative amount of disk used by this content node (transient usage not included, value in the range "
           "[0, 1]). Same value as reported to the cluster controller",
           this),
      memory("memory", {},
      ...
      feedingBlocked("feeding_blocked", {},
                     "Whether feeding is blocked due to resource limits being reached (value is either 0 or 1)",
                     this),
