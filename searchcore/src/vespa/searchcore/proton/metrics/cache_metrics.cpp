...
void CacheMetrics::update_hit_rate(const CacheStats& current, const CacheStats& last) {
    if (current.lookups() < last.lookups() || current.hits < last.hits) {
        LOG(warning,
            "Not adding %s cache hit rate metrics as values calculated "
        ...
        if ((current.lookups() - last.lookups()) > 0xffffffffull || (current.hits - last.hits) > 0xffffffffull) {
            LOG(warning,
                "%s cache hit rate metrics to add are suspiciously high."
...
void CacheMetrics::update_count_metric(uint64_t currVal, uint64_t lastVal, metrics::LongCountMetric& metric) {
    uint64_t delta = (currVal >= lastVal) ? (currVal - lastVal) : 0;
    metric.inc(delta);
}

void CacheMetrics::update_metrics(const CacheStats& stats) {
