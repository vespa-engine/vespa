...
struct DummyDocumentSubDb : public IDocumentSubDB {
    ...
    DocumentSubDbInitializer::UP createInitializer(const DocumentDBConfig&, SerialNum,
                                                   const index::IndexConfig&) const override {
        return std::make_unique<DocumentSubDbInitializer>(const_cast<DummyDocumentSubDb&>(*this),
                                                          _service.write().master());
    }
    void setup(const DocumentSubDbInitializerResult&) override {}
    void initViews(const DocumentDBConfig&) override {}
    std::unique_ptr<DocumentSubDBReconfig> prepare_reconfig(const DocumentDBConfig&, const ReconfigParams&,
                                                            std::optional<SerialNum>) override {
    ...
    IReprocessingTask::List applyConfig(const DocumentDBConfig&, const DocumentDBConfig&, SerialNum,
                                        const ReconfigParams&, IDocumentDBReferenceResolver&,
                                        const DocumentSubDBReconfig&) override {
        return {};
    }
    ...
    search::IndexStats get_index_stats(bool) const override { return {}; }
    std::shared_ptr<IDocumentRetriever> getDocumentRetriever() override { return {}; }
    matching::MatchingStats getMatcherStats(const std::string&) const override { return {}; }
    std::shared_ptr<IDocumentDBReference> getDocumentDBReference() override { return {}; }

    PendingLidTrackerBase& getUncommittedLidsTracker() override { return _pendingLidTracker; }
