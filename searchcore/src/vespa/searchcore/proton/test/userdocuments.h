...
public:
    UserDocuments() : _docs() {}
    void merge(const UserDocuments& rhs) { _docs.insert(rhs._docs.begin(), rhs._docs.end()); }
    ...
    const DocumentVector& getDocs(uint32_t userId) const { return getUserDocs(userId).getDocs(); }
    DocumentVector getGidOrderDocs(uint32_t userId) const { return getUserDocs(userId).getGidOrderDocs(); }
