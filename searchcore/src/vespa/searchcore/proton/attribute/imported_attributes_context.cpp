...
const IAttributeVector* ImportedAttributesContext::getOrCacheAttribute(std::string_view name,
    ...
    if (result) {
        ...
        auto insRes = attributes.insert(
            std::make_pair(string(name), result->makeReadGuard(std::move(metaGuard), stableEnumGuard)));
...
const IAttributeVector* ImportedAttributesContext::getOrCacheAttributeMtSafe(std::string_view name,
                                                                             AttributeCache&  attributes,
                                                                             bool             stableEnumGuard) const {
...
ImportedAttributesContext::ImportedAttributesContext(const ImportedAttributesRepo& repo)
    : _repo(repo), _mtSafe(false), _guardedAttributes(), _enumGuardedAttributes(), _metaStores(), _cacheMutex() {
}

ImportedAttributesContext::~ImportedAttributesContext() = default;

const IAttributeVector* ImportedAttributesContext::getAttribute(std::string_view name) const {
    return _mtSafe ? getOrCacheAttributeMtSafe(name, _guardedAttributes, false)
                   : getOrCacheAttribute(name, _guardedAttributes, false);
}

const IAttributeVector* ImportedAttributesContext::getAttributeStableEnum(std::string_view name) const {
    return _mtSafe ? getOrCacheAttributeMtSafe(name, _enumGuardedAttributes, true)
                   : getOrCacheAttribute(name, _enumGuardedAttributes, true);
}

void ImportedAttributesContext::getAttributeList(std::vector<const IAttributeVector*>& list) const {
...
void ImportedAttributesContext::releaseEnumGuards() {
    if (_mtSafe) {
        LockGuard guard(_cacheMutex);
...
void ImportedAttributesContext::asyncForAttribute(std::string_view, std::unique_ptr<IAttributeFunctor>) const {
