...
AttributeUsageNotifier::AttributeUsageListener::AttributeUsageListener(
    ...
    : IAttributeUsageListener(), _notifier(std::move(notifier)), _document_type(document_type) {
}
...
void AttributeUsageNotifier::remove_document_type(const std::string& document_type) {
...
void AttributeUsageNotifier::notify_attribute_usage(const AttributeUsageStats& attribute_usage) {
    std::lock_guard guard(_lock);
