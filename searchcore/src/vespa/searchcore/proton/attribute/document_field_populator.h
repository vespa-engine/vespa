...
public:
    DocumentFieldPopulator(const std::string& fieldName, AttributeVectorSP attr, const std::string& subDbName);

    ~DocumentFieldPopulator() override;

    const search::AttributeVector& getAttribute() const { return *_attr; }
