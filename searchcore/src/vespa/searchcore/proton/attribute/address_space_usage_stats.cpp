...
AddressSpaceUsageStats::AddressSpaceUsageStats(const vespalib::AddressSpace& usage)
    : _usage(usage), _attributeName(), _component_name(), _subDbName() {
}
...
bool AddressSpaceUsageStats::less_usage_than(const vespalib::AddressSpace& usage, const std::string& attributeName,
...
void AddressSpaceUsageStats::merge(const vespalib::AddressSpace& usage, const std::string& attributeName,
                                   const std::string& component_name, const std::string& subDbName) {
...
std::ostream& operator<<(std::ostream& out, const AddressSpaceUsageStats& rhs) {
    out << "{usage=" << rhs.getUsage() << ", attribute_name=" << rhs.getAttributeName()
        << ", component_name=" << rhs.get_component_name() << ", subdb_name=" << rhs.getSubDbName() << "}";
    return out;
}
