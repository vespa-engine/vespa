...
public:
    AttributeCollectionSpec(AttributeList&& attributes, uint32_t docIdLimit,
                            std::optional<SerialNum> currentSerialNum);
    ~AttributeCollectionSpec();
    const AttributeList& getAttributes() const { return _attributes; }
    AttributeList stealAttributes() { return std::move(_attributes); }
    uint32_t getDocIdLimit() const { return _docIdLimit; }
    const std::optional<SerialNum>& getCurrentSerialNum() const noexcept { return _currentSerialNum; }
