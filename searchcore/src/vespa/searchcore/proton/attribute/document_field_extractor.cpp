...
const DataType* getArrayType(const DataType& fieldType) {
...
std::unique_ptr<ArrayFieldValue> makeArray(const FieldPathEntry& fieldPathEntry, size_t size) {
...
DocumentFieldExtractor::DocumentFieldExtractor(const Document& doc) : _doc(doc), _cachedFieldValues() {
}

DocumentFieldExtractor::~DocumentFieldExtractor() = default;

bool DocumentFieldExtractor::isSupported(const FieldPath& fieldPath) {
    if (!fieldPath.empty() && fieldPath[0].getType() != FieldPathEntry::Type::STRUCT_FIELD) {
        return false;
    }
    if (fieldPath.size() == 2) {
...
const FieldValue* DocumentFieldExtractor::getCachedFieldValue(const FieldPathEntry& fieldPathEntry) {
...
std::unique_ptr<FieldValue> DocumentFieldExtractor::getSimpleFieldValue(const FieldPath& fieldPath) {
...
std::unique_ptr<FieldValue> extractFieldFromMap(const FieldValue* outerFieldValue, const FieldPathEntry& innerEntry,
...
extractFieldFromStructCollection(const FieldValue* outerFieldValue, FieldValue::Type expectedType,
                                 const FieldPathEntry& innerEntry, ExtractorFunc&& extractor) {
...
std::unique_ptr<FieldValue> DocumentFieldExtractor::extractFieldFromStructArray(const FieldPath& fieldPath) {
    return extractFieldFromStructCollection<ArrayFieldValue>(getCachedFieldValue(fieldPath[0]),
                                                             FieldValue::Type::ARRAY, fieldPath[1],
                                                             [](const auto* elem) { return elem; });
}

std::unique_ptr<FieldValue> DocumentFieldExtractor::extractKeyFieldFromMap(const FieldPath& fieldPath) {
...
DocumentFieldExtractor::extractValueFieldFromStructMap(const FieldPath& fieldPath) {
    return extractFieldFromStructCollection<MapFieldValue>(getCachedFieldValue(fieldPath[0]), FieldValue::Type::MAP,
                                                           fieldPath[2],
                                                           [](const auto* elem) { return elem->second; });
}

std::unique_ptr<FieldValue> DocumentFieldExtractor::getFieldValue(const FieldPath& fieldPath) {
    if (fieldPath.size() == 1) {
