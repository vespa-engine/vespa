...
ThreadedStateExplorerProxy::ThreadedStateExplorerProxy(std::shared_ptr<const AttributeExecutor> executor,
                                                       std::unique_ptr<StateExplorer>           explorer)
    : StateExplorer(), _executor(std::move(executor)), _explorer(std::move(explorer)) {
}

ThreadedStateExplorerProxy::~ThreadedStateExplorerProxy() = default;

void ThreadedStateExplorerProxy::get_state(const Inserter& inserter, bool full) const {
    _executor->run_sync([&explorer = *_explorer, &inserter, full]() { explorer.get_state(inserter, full); });
}

std::vector<std::string> ThreadedStateExplorerProxy::get_children_names() const {
    return _explorer->get_children_names();
}

std::unique_ptr<StateExplorer> ThreadedStateExplorerProxy::get_child(std::string_view name) const {
...
AttributeManagerExplorer::AttributeManagerExplorer(std::shared_ptr<IAttributeManager> mgr) : _mgr(std::move(mgr)) {
}

AttributeManagerExplorer::~AttributeManagerExplorer() = default;

void AttributeManagerExplorer::get_state(const Inserter& inserter, bool full) const {
    (void)full;
    inserter.insertObject();
}

std::vector<std::string> AttributeManagerExplorer::get_children_names() const {
...
std::unique_ptr<vespalib::StateExplorer> AttributeManagerExplorer::get_child(std::string_view name) const {
