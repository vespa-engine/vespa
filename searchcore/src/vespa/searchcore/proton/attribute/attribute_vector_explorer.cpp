...
void convertGenerationToSlime(const AttributeVector& attr, Cursor& object) {
    object.setLong("oldest_used", attr.get_oldest_used_generation().value());
    object.setLong("current", attr.getCurrentGeneration().value());
}

void convertAddressSpaceUsageToSlime(const AddressSpaceUsage& usage, Cursor& object) {
    for (const auto& entry : usage.get_all()) {
        StateExplorerUtils::address_space_to_slime(entry.second, object.setObject(entry.first));
    }
}

void convertMemoryUsageToSlime(const MemoryUsage& usage, Cursor& object) {
    StateExplorerUtils::memory_usage_to_slime(usage, object);
}

void convert_enum_store_dictionary_to_slime(const search::IEnumStoreDictionary& dictionary, Cursor& object) {
...
void convertEnumStoreToSlime(const IEnumStore& enumStore, Cursor& object) {
...
void convertMultiValueToSlime(const MultiValueMappingBase& multiValue, Cursor& object) {
...
void convertChangeVectorToSlime(const AttributeVector& v, Cursor& object) {
    MemoryUsage usage = v.getChangeVectorMemoryUsage();
    convertMemoryUsageToSlime(usage, object);
}

void convertPostingBaseToSlime(const IPostingListAttributeBase& postingBase, Cursor& object) {
...
AttributeVectorExplorer::AttributeVectorExplorer(std::shared_ptr<AttributeVector> attr) : _attr(std::move(attr)) {
}

AttributeVectorExplorer::~AttributeVectorExplorer() = default;

void AttributeVectorExplorer::get_state(const vespalib::slime::Inserter& inserter, bool full) const {
    auto&         attr = *_attr;
    bool          paged = attr.getConfig().paged();
    ...
    if (full) {
        ...
        if (paged) {
            slime_status.setLong("pagefile_size_on_disk", attr.get_memory_allocator_size_on_disk());
        }
        ...
        // In this case we must ensure that ExclusiveAttributeReadAccessor::Guard is held also when exploring
        // children.
        ...
        if (paged) {
            object.setLong("pagefile_size_on_disk", attr.get_memory_allocator_size_on_disk());
        }
        object.setDouble(LAST_FLUSH_DURATION, last_flush_duration);
    }
}

std::vector<std::string> AttributeVectorExplorer::get_children_names() const {
    return {TENSOR_NAME};
}

std::unique_ptr<vespalib::StateExplorer> AttributeVectorExplorer::get_child(std::string_view name) const {
    if (name == TENSOR_NAME) {
