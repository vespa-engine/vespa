...
class AddressSpaceUsageStats {
    ...
    bool less_usage_than(const vespalib::AddressSpace& usage, const std::string& attributeName,
                         const std::string& component_name, const std::string& subDbName) const noexcept;
...
public:
    ...
    void merge(const vespalib::AddressSpace& usage, const std::string& attributeName,
    ...
    bool operator==(const AddressSpaceUsageStats& rhs) const noexcept {
        return (_usage == rhs._usage) && (_attributeName == rhs._attributeName) &&
               (_component_name == rhs._component_name) && (_subDbName == rhs._subDbName);
    }
};
