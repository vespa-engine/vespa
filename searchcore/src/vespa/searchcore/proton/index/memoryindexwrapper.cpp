...
MemoryIndexWrapper::MemoryIndexWrapper(const search::index::Schema&                schema,
    ...
    : _index(schema, inspector, threadingService.field_writer(), threadingService.field_writer()),
      _serialNum(serialNum),
      _fileHeaderContext(fileHeaderContext),
      _tuneFileIndexing(tuneFileIndexing) {
}

void MemoryIndexWrapper::flushToDisk(const std::string& flushDir, uint32_t docIdLimit, SerialNum serialNum) {
    ...
    IndexBuilder indexBuilder(_index.getSchema(), flushDir, docIdLimit, numWords, *this, _tuneFileIndexing,
                              fileHeaderContext);
    _index.dump(indexBuilder);
}

search::SerialNum MemoryIndexWrapper::getSerialNum() const {
...
FieldLengthInfo MemoryIndexWrapper::get_field_length_info(const std::string& field_name) const {
