...
IndexWriter::IndexWriter(const IIndexManager::SP& mgr) : _mgr(mgr) {
}

IndexWriter::~IndexWriter() = default;

bool IndexWriter::ignoreOperation(search::SerialNum serialNum) const {
    return (serialNum <= _mgr->getFlushedSerialNum());
}

void IndexWriter::put(search::SerialNum serialNum, const document::Document& doc, const search::DocumentIdT lid,
...
void IndexWriter::removeDocs(search::SerialNum serialNum, LidVector lids) {
...
void IndexWriter::commit(search::SerialNum serialNum, const OnWriteDoneType& onWriteDone) {
    if (serialNum <= _mgr->getFlushedSerialNum()) {
        return;
    }
...
void IndexWriter::heartBeat(search::SerialNum serialNum) {
    _mgr->heartBeat(serialNum);
}

void IndexWriter::compactLidSpace(search::SerialNum serialNum, const search::DocumentIdT lid) {
    if (serialNum <= _mgr->getFlushedSerialNum()) {
        return;
    }
