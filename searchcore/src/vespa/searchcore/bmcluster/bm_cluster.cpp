...
enum class PortBias { SLOBROK_PORT = 0, RPC_CLIENT_PORT = 1, NUM_PORTS = 2 };
...
std::vector<PersistenceProvider*> collect_persistence_providers(const std::vector<std::unique_ptr<BmNode>>& nodes) {
...
struct BmCluster::MessageBusConfigSet {
    ...
    MessageBusConfigSet(const std::string& config_id_in, int slobrok_port)
        : config_id(config_id_in), slobroks(), messagebus() {
...
struct BmCluster::RpcClientConfigSet {
    ...
    RpcClientConfigSet(const std::string& config_id_in, int slobrok_port) : config_id(config_id_in), slobroks() {
        make_slobroks_config(slobroks, slobrok_port);
    }
    ~RpcClientConfigSet();

    void add_builders(ConfigSet& set) { set.addBuilder(config_id, &slobroks); }
};

BmCluster::RpcClientConfigSet::~RpcClientConfigSet() = default;

BmCluster::BmCluster(const std::string& base_dir, int base_port, const BmClusterParams& params,
    ...
    : _params(params),
      ...
      _real_distribution(std::make_shared<BmDistribution>(params.get_groups(), params.get_nodes_per_group(),
                                                          params.get_redundancy())),
...
void BmCluster::wait_slobrok(const std::string& name) {
...
void BmCluster::start_message_bus() {
    if (!_message_bus) {
...
void BmCluster::stop_message_bus() {
    if (_message_bus) {
...
void BmCluster::start_rpc_client() {
    if (!_rpc_client) {
        LOG(info, "start rpc client");
        config::ConfigUri client_config_uri(rpc_client_config_id, _config_context);
        _rpc_client = std::make_unique<SharedRpcResources>(client_config_uri, _rpc_client_port, 100,
                                                           _params.get_rpc_events_before_wakeup());
        _rpc_client->start_server_and_register_slobrok(rpc_client_config_id);
    }
}

void BmCluster::stop_rpc_client() {
    if (_rpc_client) {
...
void BmCluster::make_node(uint32_t node_idx) {
    ...
    _nodes[node_idx] =
        BmNode::create(node_base_dir, node_base_port, node_idx, *this, _params, _document_types, _slobrok_port);
}

void BmCluster::make_nodes() {
    for (uint32_t node_idx = 0; node_idx < _nodes.size(); ++node_idx) {
        make_node(node_idx);
    }
}

void BmCluster::initialize_providers() {
...
void BmCluster::create_buckets(BmFeed& feed) {
...
void BmCluster::start_service_layers() {
    start_slobrok();
    for (const auto& node : _nodes) {
        if (node) {
...
void BmCluster::start_distributors() {
    for (const auto& node : _nodes) {
        if (node) {
...
void BmCluster::create_feed_handler() {
    ...
    if (_params.get_use_document_api()) {
        ...
        if (_params.get_use_storage_chain()) {
            ...
            _feed_handler =
                std::make_unique<StorageApiMessageBusBmFeedHandler>(get_message_bus(), *_distribution, true);
        } else {
            _feed_handler = std::make_unique<StorageApiRpcBmFeedHandler>(get_rpc_client(), _repo, rpc_params,
                                                                         *_distribution, true);
        }
        ...
        if (_params.get_use_storage_chain()) {
            ...
            _feed_handler =
                std::make_unique<StorageApiChainBmFeedHandler>(std::move(contexts), *_distribution, false);
        } else if (_params.get_use_message_bus()) {
            _feed_handler =
                std::make_unique<StorageApiMessageBusBmFeedHandler>(get_message_bus(), *_distribution, false);
        } else {
            _feed_handler = std::make_unique<StorageApiRpcBmFeedHandler>(get_rpc_client(), _repo, rpc_params,
                                                                         *_distribution, false);
        }
        ...
        _feed_handler = std::make_unique<SpiBmFeedHandler>(std::move(providers), *_field_set_repo, *_distribution,
                                                           _params.get_skip_get_spi_bucket_info());
    }
}

void BmCluster::shutdown_feed_handler() {
    _feed_handler.reset();
}

void BmCluster::shutdown_distributors() {
    for (const auto& node : _nodes) {
        if (node) {
...
void BmCluster::shutdown_service_layers() {
    stop_rpc_client();
    for (const auto& node : _nodes) {
        if (node) {
...
void BmCluster::start(BmFeed& feed) {
...
IBmFeedHandler* BmCluster::get_feed_handler() {
    return _feed_handler.get();
}

std::vector<BmNodeStats> BmCluster::get_node_stats() {
...
void BmCluster::propagate_cluster_state() {
    _cluster_controller->propagate_cluster_state();
}

BucketDbSnapshotVector BmCluster::get_bucket_db_snapshots() {
    auto providers = collect_persistence_providers(_nodes);
