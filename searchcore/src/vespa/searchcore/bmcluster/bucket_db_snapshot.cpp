...
BucketDbSnapshot::BucketDbSnapshot() : _buckets() {
}
...
uint32_t BucketDbSnapshot::count_new_documents(const BucketDbSnapshot& old) const {
    uint32_t result = 0;
...
void BucketDbSnapshot::populate_bucket_id_set(BucketIdSet& buckets) const {
    for (auto& id_and_info : _buckets) {
        buckets.insert(id_and_info.first);
    }
}

const BucketInfo* BucketDbSnapshot::try_get_bucket_info(BucketId bucket_id) const {
