...
FRT_RPCRequest* make_set_cluster_state_request(const IBmDistribution& distribution) {
...
BmClusterController::BmClusterController(BmCluster& cluster, const IBmDistribution& distribution)
    : _cluster(cluster), _distribution(distribution) {
}

void BmClusterController::propagate_cluster_state(uint32_t node_idx, bool distributor) {
    static std::string    _storage("storage");
    StorageMessageAddress storage_address(&_storage, distributor ? NodeType::DISTRIBUTOR : NodeType::STORAGE,
                                          node_idx);
    auto                  req = make_set_cluster_state_request(_distribution);
    auto&                 shared_rpc_resources = _cluster.get_rpc_client();
    auto                  target_resolver = std::make_unique<storage::rpc::CachingRpcTargetResolver>(
        shared_rpc_resources.slobrok_mirror(), shared_rpc_resources.target_factory(), 1);
...
void BmClusterController::propagate_cluster_state(bool distributor) {
...
void BmClusterController::propagate_cluster_state() {
