...
BmFeedParams::BmFeedParams() : _client_threads(1), _documents(160000), _max_pending(1000) {
}

BmFeedParams::~BmFeedParams() = default;

BmRange BmFeedParams::get_range(uint32_t thread_id) const {
    return BmRange(get_start(thread_id), get_start(thread_id + 1));
}

bool BmFeedParams::check() const {
