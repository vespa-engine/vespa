...
DocumentId BmFeed::make_document_id(uint32_t n, uint32_t i) const {
    DocumentId id(vespalib::make_string("id::test:n=%u:%u", n & (num_buckets() - 1), i));
    return id;
}

std::unique_ptr<Document> BmFeed::make_document(uint32_t n, uint32_t i) const {
...
std::unique_ptr<DocumentUpdate> BmFeed::make_document_update(uint32_t n, uint32_t i) const {
    auto id = make_document_id(n, i);
    auto document_update = std::make_unique<DocumentUpdate>(*_repo, *_document_type, id);
    document_update->addUpdate(
        FieldUpdate(_field).addUpdate(std::make_unique<AssignValueUpdate>(std::make_unique<IntFieldValue>(15))));
    return document_update;
}

vespalib::nbostream BmFeed::make_put_feed(BmRange range, BucketSelector bucket_selector) {
...
vespalib::nbostream BmFeed::make_update_feed(BmRange range, BucketSelector bucket_selector) {
    vespalib::nbostream serialized_feed;
...
vespalib::nbostream BmFeed::make_get_or_remove_feed(BmRange range, BucketSelector bucket_selector,
                                                    bool make_removes) {
    vespalib::nbostream serialized_feed;
...
vespalib::nbostream BmFeed::make_get_feed(BmRange range, BucketSelector bucket_selector) {
    return make_get_or_remove_feed(range, bucket_selector, false);
}

vespalib::nbostream BmFeed::make_remove_feed(BmRange range, BucketSelector bucket_selector) {
    return make_get_or_remove_feed(range, bucket_selector, true);
}

std::vector<vespalib::nbostream> BmFeed::make_feed(vespalib::ThreadStackExecutor& executor,
                                                   const BmFeedParams&            params,
