...
StorageReplyErrorChecker::StorageReplyErrorChecker() : _errors(0u) {
}

StorageReplyErrorChecker::~StorageReplyErrorChecker() = default;

void StorageReplyErrorChecker::check_error(const storage::api::StorageMessage& msg) {
    auto reply = dynamic_cast<const storage::api::StorageReply*>(&msg);
    if (reply != nullptr) {
        if (reply->getResult().failed()) {
            if (++_errors <= 10) {
                LOG(info, "reply '%s', return code '%s'", reply->toString().c_str(),
