...
BmFeeder::BmFeeder(std::shared_ptr<const DocumentTypeRepo> repo, IBmFeedHandler& feed_handler,
...
void BmFeeder::feed_operation(uint32_t op_idx, vespalib::nbostream& serialized_feed, int64_t time_bias,
    ...
    case BmFeedOperation::UPDATE_OPERATION: {
        ...
        _feed_handler.update(bucket, std::move(document_update), (_use_timestamp ? (time_bias + op_idx) : 0),
                             tracker);
    } break;
...
uint32_t BmFeeder::feed_task(uint32_t max_pending, BmRange range, const vespalib::nbostream& serialized_feed,
                             int64_t time_bias) {
...
void BmFeeder::run_feed_tasks(int pass, int64_t& time_bias, const std::vector<vespalib::nbostream>& serialized_feed_v,
                              const BmFeedParams& params, AvgSampler& sampler, const std::string& op_name) {
    ...
    for (uint32_t i = 0; i < params.get_client_threads(); ++i) {
        ...
        _executor.execute(makeLambdaTask(
            [this, max_pending = params.get_max_pending(), &serialized_feed = serialized_feed_v[i], range, time_bias,
             &atomic_op_count]() { atomic_op_count += feed_task(max_pending, range, serialized_feed, time_bias); }));
    }
    _executor.sync();
    ...
    LOG(info, "%sAsync: pass=%u, errors=%u, ops=%u of %u, %ss/s: %8.2f", op_name.c_str(), pass, new_errors, op_count,
        params.get_documents(), op_name.c_str(), throughput);
...
void BmFeeder::run_feed_tasks_loop(int64_t& time_bias, const std::vector<vespalib::nbostream>& serialized_feed_v,
                                   const BmFeedParams& params, const std::string& op_name) {
    AvgSampler sampler;
