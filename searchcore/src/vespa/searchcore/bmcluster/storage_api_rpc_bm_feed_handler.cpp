...
public:
    MyMessageDispatcher() : storage::MessageDispatcher(), StorageReplyErrorChecker(), _pending_hash() {}
...
StorageApiRpcBmFeedHandler::StorageApiRpcBmFeedHandler(SharedRpcResources& shared_rpc_resources_in,
    ...
    : StorageApiBmFeedHandlerBase("StorageApiRpcBmFeedHandler", distribution, distributor),
      ...
      _rpc_client(std::make_unique<storage::rpc::StorageApiRpcService>(*_message_dispatcher, _shared_rpc_resources,
                                                                       *_message_codec_provider, rpc_params)) {
}

StorageApiRpcBmFeedHandler::~StorageApiRpcBmFeedHandler() = default;

void StorageApiRpcBmFeedHandler::send_cmd(std::shared_ptr<storage::api::StorageCommand> cmd,
...
void StorageApiRpcBmFeedHandler::attach_bucket_info_queue(PendingTracker&) {
}

uint32_t StorageApiRpcBmFeedHandler::get_error_count() const {
