 ...
 */
class IndexMaintainer : public IIndexManager, public IWarmupDone {
    /**
    ...
    public:
        ...
        FrozenMemoryIndexRef(const std::shared_ptr<IMemoryIndex>& index, SerialNum serialNum,
                             std::unique_ptr<SaveInfo> saveInfo, uint32_t absoluteId)
            : _index(index), _serialNum(serialNum), _saveInfo(std::move(saveInfo)), _absoluteId(absoluteId) {}
    ...
    const search::FixedSourceSelector& getSourceSelector() const {
        return static_cast<const search::FixedSourceSelector&>(*_selector);
    }
    ...
    void updateDiskIndexSchema(const std::string& indexDir, const Schema& schema, SerialNum serialNum);

    void updateIndexSchemas(IIndexCollection& coll, const Schema& schema, SerialNum serialNum);
    ...
    std::shared_ptr<IDiskIndex> flushMemoryIndex(IMemoryIndex& memoryIndex, uint32_t indexId, uint32_t docIdLimit,
                                                 SerialNum                              serialNum,
                                                 search::FixedSourceSelector::SaveInfo& saveInfo);

    std::unique_ptr<ISearchableIndexCollection>
    loadDiskIndexes(const FusionSpec& spec, std::unique_ptr<ISearchableIndexCollection> sourceList);
    ...
    void swapInNewIndex(LockGuard& guard, std::shared_ptr<ISearchableIndexCollection> indexes,
    ...
    void flushMemoryIndex(FlushArgs& args, uint32_t docIdLimit, search::FixedSourceSelector::SaveInfo& saveInfo,
                          FlushIds& flushIds);
...
public:
    ...
    IndexMaintainer(const IndexMaintainerConfig& config, const IndexMaintainerContext& context,
    ...
    SerialNum getCurrentSerialNum() const override { return current_serial_num(); }

    SerialNum getFlushedSerialNum() const override { return flush_serial_num(); }
