...
DiskIndexWithDestructorCleanup::createBlueprint(const IRequestContext& requestContext, const FieldSpec& field,
...
std::unique_ptr<search::queryeval::Blueprint>
DiskIndexWithDestructorCleanup::createBlueprint(const IRequestContext& requestContext, const FieldSpecList& fields,
                                                const Node& term, search::fef::MatchDataLayout& global_layout) {
    return _index->createBlueprint(requestContext, fields, term, global_layout);
}

search::IndexStats DiskIndexWithDestructorCleanup::get_index_stats(bool clear_disk_io_stats) const {
    return _index->get_index_stats(clear_disk_io_stats);
}

search::SerialNum DiskIndexWithDestructorCleanup::getSerialNum() const {
    return _index->getSerialNum();
}
void DiskIndexWithDestructorCleanup::accept(IndexSearchableVisitor& visitor) const {
...
const std::string& DiskIndexWithDestructorCleanup::getIndexDir() const {
    return _index->getIndexDir();
}

const search::index::Schema& DiskIndexWithDestructorCleanup::getSchema() const {
