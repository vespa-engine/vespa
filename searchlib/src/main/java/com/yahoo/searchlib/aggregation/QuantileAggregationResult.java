...
public class QuantileAggregationResult extends AggregationResult {
    ...
    protected boolean equalsAggregation(AggregationResult obj) {
        return quantile == ((QuantileAggregationResult) obj).quantile && equals(sketch, ((QuantileAggregationResult) obj).sketch);
    }
    ...
    public QuantileAggregationResult clone() {
        ...
        if (sketch != null) {
            obj.sketch = KllDoublesSketch.newHeapInstance();
            var it = sketch.iterator();
            while (it.next()) {
                obj.sketch.update(it.getQuantile());
            }
        }
        return obj;
    }
