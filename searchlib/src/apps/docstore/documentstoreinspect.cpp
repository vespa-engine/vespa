...
void DocumentStoreInspectApp::usage(const char* self) {
...
int DocumentStoreInspectApp::dumpIdxFile(const std::string& file) {
    ...
    if (idxFile.OpenReadOnly()) {
        if (idxFile.IsMemoryMapped()) {
            ...
            vespalib::nbostream is(static_cast<const char*>(idxFile.MemoryMapPtr(0)) + idxHeaderLen,
...
int DocumentStoreInspectApp::main(int argc, char** argv) {
    std::string cmd;
    if (argc >= 2) {
        cmd = argv[1];
        if (cmd == "dumpidxfile") {
...
int DocumentStoreInspectApp::verify(const std::string& dir) {
    int retval(0);
