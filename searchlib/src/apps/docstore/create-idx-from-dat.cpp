...
void CreateIdxFileFromDatApp::usage(const char* self) {
...
bool validUncompressed(const char* n, size_t offset) {
    return (n[1] == vespalib::compression::CompressionConfig::NONE) && (n[2] == 0) && (n[3] == 0) && (n[4] == 0) &&
           (n[5] != 0) && tryDecode(0, offset, n, 6ul + 4ul + uint8_t(n[5]), 6ul + 4ul + uint8_t(n[5]) + 4);
}
...
uint64_t generate(uint64_t serialNum, size_t chunks, FastOS_FileInterface& idxFile, size_t sz, const char* current,
                  const char* start, const char* nextStart) __attribute__((noinline));
uint64_t generate(uint64_t serialNum, size_t chunks, FastOS_FileInterface& idxFile, size_t sz, const char* current,
                  const char* start, const char* nextStart) {
    vespalib::nbostream os;
    for (size_t lengthError(0); int64_t(sz + lengthError) <= nextStart - start; lengthError++) {
        try {
            Chunk chunk(chunks, current, sz + lengthError);
            fprintf(stdout, "id=%d lastSerial=%" PRIu64 " count=%ld\n", chunk.getId(), chunk.getLastSerial(),
                    chunk.count());
            ...
            for (; nextStart < end; nextStart += alignment) {
                if (validHead(nextStart, nextStart - start)) {
                    ...
                    if (tryDecode(chunks, current - start, current, tail - current, nextStart - current)) {
                        ...
                        fprintf(stdout,
                                "chunk %" PRIu64 " possibly starting at %ld ending at %ld false sync at pos=%ld\n",
...
int CreateIdxFileFromDatApp::main(int argc, char** argv) {
    std::string cmd;
    if (argc == 3) {
