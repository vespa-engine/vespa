...
void VerifyLogDataStoreApp::usage(const char* self) {
    printf("Usage: %s <direcory>\n", self);
    fflush(stdout);
}

int VerifyLogDataStoreApp::main(int argc, char** argv) {
    if (argc >= 2) {
...
int VerifyLogDataStoreApp::verify(const std::string& dir) {
    int retval(0);
