...
struct MyAnnotation {
    ...
    MyAnnotation(int32_t start_in, int32_t length_in) noexcept : start(start_in), length(length_in), label() {}

    MyAnnotation(int32_t start_in, int32_t length_in, std::string label_in) noexcept
        : start(start_in), length(length_in), label(label_in) {}

    bool operator==(const MyAnnotation& rhs) const noexcept;
};

bool MyAnnotation::operator==(const MyAnnotation& rhs) const noexcept {
    return start == rhs.start && length == rhs.length && label == rhs.label;
}
...
StringFieldBuilderTest::StringFieldBuilderTest() : testing::Test(), db(), sfb(db) {
}

StringFieldBuilderTest::~StringFieldBuilderTest() = default;

std::vector<MyAnnotation> StringFieldBuilderTest::get_annotations(const StringFieldValue& val) {
    ...
    if (tree != nullptr) {
        for (auto& ann : *tree) {
            ...
            if (ann_fv == nullptr) {
                ...
                result.emplace_back(span->from(), span->length(),
                                    dynamic_cast<const StringFieldValue&>(*ann_fv).getValue());
            }
        }
    }
    return result;
...
void StringFieldBuilderTest::assert_annotations(std::vector<MyAnnotation> exp, const std::string& plain,
...
TEST_F(StringFieldBuilderTest, alt_word) {
    assert_annotations({{0, 3}, {4, 3}, {4, 3, "baz"}}, "foo bar",
                       sfb.word("foo").space().word("bar").alt_word("baz").build());
}
