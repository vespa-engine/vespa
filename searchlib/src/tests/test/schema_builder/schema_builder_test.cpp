...
protected:
    ...
    void assert_attribute(std::string_view name, search::index::schema::DataType exp_dt, CollectionType exp_ct,
                          const std::string exp_tensor_spec = "");
    void assert_all_attributes();
};

SchemaBuilderTest::SchemaBuilderTest() : schema() {
}

SchemaBuilderTest::~SchemaBuilderTest() = default;

void SchemaBuilderTest::assert_index(std::string_view name, search::index::schema::DataType exp_dt,
                                     CollectionType exp_ct) {
...
void SchemaBuilderTest::assert_all_indexes() {
...
void SchemaBuilderTest::assert_attribute(std::string_view name, search::index::schema::DataType exp_dt,
                                         CollectionType exp_ct, const std::string exp_tensor_spec) {
...
void SchemaBuilderTest::assert_all_attributes() {
    using DataType = search::index::schema::DataType;
...
TEST_F(SchemaBuilderTest, all_fields) {
    ...
    schema = SchemaBuilder(db)
                 .add_indexes({"string", "url", "url_array", "url_wset"})
                 .add_attributes({"int8", "int16", "int32", "int64", "bool", "float", "double", "string", "tensor",
                                  "int32_array", "int32_wset"})
                 .build();
    assert_all_indexes();
