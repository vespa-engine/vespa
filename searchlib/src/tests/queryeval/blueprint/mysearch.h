...
public:
    MySearch(const std::string& tag, bool leaf, bool strict)
        : _tag(tag), _isLeaf(leaf), _isStrict(strict), _match(), _md(0) {}

    MySearch(const std::string& tag, const TFMDA& tfmda, bool strict)
        : _tag(tag), _isLeaf(true), _isStrict(strict), _match(tfmda), _md(0) {}

    MySearch(const std::string& tag, Children children, MatchData* md, bool strict)
        : MultiSearch(std::move(children)), _tag(tag), _isLeaf(false), _isStrict(strict), _match(), _md(md) {}
    ~MySearch() override;
...
public:
    ~MyLeaf() override;
    SearchIterator::UP createLeafSearch(const TFMDA& tfmda) const override {
    ...
    bool want_global_filter(GlobalFilterLimits&) const override { return _want_global_filter; }
    void set_global_filter(const GlobalFilter&, double) override { _got_global_filter = true; }
...
public:
    ...
    MyLeaf* create() const { return create<MyLeaf>(); }
    template <typename Leaf> Leaf* create() const {
