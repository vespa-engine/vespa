...
private:
    AnyFlow my_flow(InFlow in_flow) const override { return AnyFlow::create<OrFlow>(in_flow); }

public:
    ~MyOr() override;
    FlowStats calculate_flow_stats(uint32_t) const final {
        return {OrFlow::estimate_of(get_children()), OrFlow::cost_of(get_children(), false),
                OrFlow::cost_of(get_children(), true)};
    }
    HitEstimate combine(const std::vector<HitEstimate>& data) const override { return max(data); }

    FieldSpecBaseList exposeFields() const override { return mixChildrenFields(); }

    void sort(Children& children, InFlow) const override {
        std::sort(children.begin(), children.end(), TieredGreaterEstimate());
    }

    SearchIterator::UP createIntermediateSearch(MultiSearch::Children subSearches, MatchData& md) const override {
...
private:
public:
    HitEstimate combine(const std::vector<HitEstimate>& data) const override { return min(data); }

    FieldSpecBaseList exposeFields() const override { return {}; }
...
struct MyTerm : SimpleLeafBlueprint {
    ...
    SearchIterator::UP createLeafSearch(const search::fef::TermFieldMatchDataArray&) const override { return {}; }
    SearchIteratorUP createFilterSearchImpl(FilterConstraint constraint) const override {
        return create_default_filter(constraint);
    }
};
...
public:
    Fixture() : _md(MatchData::makeTestInstance(100, 10)) {}
    ~Fixture() = default;
...
SearchIterator::UP Fixture::create(const Blueprint& blueprint) {
...
void Fixture::check_equal(const SearchIterator& a, const SearchIterator& b, const std::string& label) {
    SCOPED_TRACE(label);
    EXPECT_EQ(a.asString(), b.asString());
}

void Fixture::check_not_equal(const SearchIterator& a, const SearchIterator& b, const std::string& label) {
    SCOPED_TRACE(label);
    EXPECT_NE(a.asString(), b.asString());
}

void Fixture::check_equal(const Blueprint& a, const Blueprint& b, const std::string& label) {
    SearchIterator::UP searchA = create(a);
    SearchIterator::UP searchB = create(b);
    check_equal(*searchA, *searchB, label);
}

void Fixture::check_not_equal(const Blueprint& a, const Blueprint& b, const std::string& label) {
    SearchIterator::UP searchA = create(a);
    SearchIterator::UP searchB = create(b);
...
TEST(BlueprintTest, testHitEstimateCalculation) {
    ...
    {
        Blueprint::UP a = ap(MyAnd::create().add(MyLeafSpec(0).create()).add(MyLeafSpec(0, true).create()));
        EXPECT_EQ(0u, a->getState().estimate().estHits);
        EXPECT_EQ(true, a->getState().estimate().empty);
    }
    {
        Blueprint::UP o = ap(MyOr::create().add(MyLeafSpec(0).create()).add(MyLeafSpec(0, true).create()));
...
TEST(BlueprintTest, testMatchDataPropagation) {
    ...
    {
        Blueprint::UP leaf = ap(MyLeafSpec(42).addField(1, 41).addField(2, 72).create());
    ...
    {
        MyOr& o =
            MyOr::create().add(MyLeafSpec(1).addField(1, 1).create()).add(MyLeafSpec(2).addField(2, 2).create());
...
TEST(BlueprintTest, testSearchCreation) {
    ...
    {
        Blueprint::UP a =
            ap(MyAnd::create().add(MyLeafSpec(1).addField(1, 1).create()).add(MyLeafSpec(2).addField(2, 2).create()));
    ...
    {
        Blueprint::UP o = ap(
            MyOr::create().add(MyLeafSpec(1).addField(1, 11).create()).add(MyLeafSpec(2).addField(2, 22).create()));
...
TEST(BlueprintTest, testBlueprintMakeNew) {
    Blueprint::UP orig =
        ap(MyOr::create().add(MyLeafSpec(1).addField(1, 11).create()).add(MyLeafSpec(2).addField(2, 22).create()));
    orig->setSourceId(42);
...
std::string getExpectedBlueprint() {
...
std::string getExpectedSlimeBlueprint() {
    return "{"
...
struct BlueprintFixture {
    MyOr _blueprint;
    BlueprintFixture() : _blueprint() { _blueprint.add(new MyTerm(FieldSpecBase(5, 7), 9)); }
};
...
Blueprint::Options make_opts(bool sort_by_cost, bool allow_force_strict, bool keep_order) {
    return Blueprint::Options()
        ...
        .allow_force_strict(allow_force_strict)
