...
void verify_md_elements(MatchData& md, const std::vector<TermFieldHandle>& handles, const std::string& label,
                        uint32_t docid, std::vector<OptElems> exp) {
...
BlueprintAndHandles::BlueprintAndHandles(std::unique_ptr<Blueprint>   bp_,
                                         std::vector<TermFieldHandle> handles_) noexcept
    : _bp(std::move(bp_)), _handles(std::move(handles_)) {
}

BlueprintAndHandles::~BlueprintAndHandles() = default;

BlueprintAndHandles BlueprintAndHandles::finalize(bool strict) && {
...
BlueprintAndHandles make_blueprint(QueryTweak query_tweak, MatchDataLayout& mdl,
    ...
    auto result = std::make_unique<SameElementBlueprint>(make_field_spec(mdl), descendants_index_handles, false, true,
...
TEST(SameElementTest, require_that_element_filter_selects_precisely_correct_elements) {
    auto a = make_result({{5, {1, 3, 7, 12}}, {10, {1, 2, 3}}, {15, {3, 4, 5}}, {20, {4, 5, 6}}, {25, {1}}});
    auto mdl = make_match_data_layout();
