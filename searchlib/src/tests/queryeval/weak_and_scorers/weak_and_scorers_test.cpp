...
struct TestIterator : public SearchIterator {
    ...
    TestIterator(int32_t maxWeight, int32_t termWeight, bool useInfo)
        : _info(0, maxWeight), _termWeight(termWeight), _useInfo(useInfo), _unpackDocId(0) {}
    ...
    const PostingInfo* getPostingInfo() const override { return (_useInfo ? &_info : nullptr); }
...
TEST(WeakAndScorersTest,
     require_that_DotProductScorer_uses_default_max_weight_when_not_available_in_search_iterator) {
