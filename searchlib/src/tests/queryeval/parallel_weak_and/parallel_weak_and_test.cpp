...
template <typename HeapType> struct WandTestSpec : public WandSpec {
    HeapType           heap;
    TermFieldMatchData rootMatchData;
    MatchParams        matchParams;
    MatchingPhase      matching_phase;

    explicit WandTestSpec(uint32_t scoresToTrack, uint32_t scoresAdjustFrequency = 1, score_t scoreThreshold = 0,
                          double thresholdBoostFactor = 1);
    ~WandTestSpec();
    SearchIterator::UP create() {
        ...
        return SearchIterator::UP(
            new TrackedSearch("PWAND", getHistory(),
                              ParallelWeakAndSearch::create(getTerms(tmp), matchParams,
                                                            RankParams(rootMatchData, std::move(childrenMatchData)),
...
WandTestSpec<HeapType>::WandTestSpec(uint32_t scoresToTrack, uint32_t scoresAdjustFrequency, score_t scoreThreshold,
...
template <typename HeapType> WandTestSpec<HeapType>::~WandTestSpec() = default;

using WandSpecWithTestHeap = WandTestSpec<TestHeap>;
using WandSpecWithRealHeap = WandTestSpec<SharedWeakAndPriorityQueue>;

FakeResult doSearch(SearchIterator& sb, const TermFieldMatchData& tfmd) {
    FakeResult retval;
    sb.initFullRange();
...
SimpleResult asSimpleResult(const FakeResult& result) {
    SimpleResult retval;
...
struct WandBlueprintSpec {
    ...
    Node::UP createNode(uint32_t scoresToTrack = 100, score_t scoreThreshold = 0,
                        double thresholdBoostFactor = 1) const {
        auto node = std::make_unique<SimpleWandTerm>(tokens.size(), "view", 0, Weight(0), scoresToTrack,
...
struct FixtureBase {
    ...
    FakeResult           result;
    FixtureBase(uint32_t scoresToTrack, uint32_t scoresAdjustFrequency, score_t scoreThreshold = 0,
                double boostFactor = 1.0)
        : spec(scoresToTrack, scoresAdjustFrequency, scoreThreshold, boostFactor), result() {}
    void prepare() {
...
struct AlgoSimpleFixture : public FixtureBase {
    AlgoSimpleFixture() : AlgoSimpleFixture(false) {}
    explicit AlgoSimpleFixture(bool second_phase) : FixtureBase(2, 1) {
        if (second_phase) {
...
TEST(ParallelWeakAndTest, require_that_algorithm_uses_initial_score_threshold_case_all_hits_greater) {
    ...
    EXPECT_EQ(FakeResult().doc(1).score(1 * 10 + 2 * 20).doc(2).score(1 * 30).doc(3).score(2 * 40), f.result);
}
...
TEST(ParallelWeakAndTest, require_that_algorithm_uses_initial_score_threshold_case_2_hits_greater) {
    ...
    EXPECT_EQ(FakeResult().doc(1).score(1 * 10 + 2 * 20).doc(3).score(2 * 40), f.result);
}
...
TEST(ParallelWeakAndTest, require_that_algorithm_uses_initial_score_threshold_case_1_hit_greater) {
    ...
    EXPECT_EQ(FakeResult().doc(3).score(2 * 40), f.result);
}
...
TEST(ParallelWeakAndTest, require_that_algorithm_handles_large_scores) {
    ...
    EXPECT_EQ(FakeResult().doc(1).score(60000L * 60000L + 70000L * 80000L).doc(3).score(70000L * 90000L), f.result);
}

TEST(ParallelWeakAndTest, require_that_algorithm_steps_all_present_terms_when_past_is_empty) {
    AlgoExhaustPastFixture f(25);
    EXPECT_EQ(FakeResult().doc(3).score(40).doc(5).score(30), f.result);
}
...
TEST(ParallelWeakAndTest, require_that_scores_are_collected_in_batches_before_adjusting_heap) {
    HeapFixture f;
    EXPECT_EQ(SimpleResult().addHit(1).addHit(2).addHit(3).addHit(4).addHit(5).addHit(6), f.result);
    EXPECT_EQ(ScoresHistory().add(Scores().add(1).add(2)).add(Scores().add(3).add(4)).add(Scores().add(5).add(6)),
              f.spec.heap.history);
}
...
TEST(ParallelWeakAndTest, require_that_dot_product_score_is_calculated) {
    SearchFixture f;
    FakeResult    expect = FakeResult().doc(1).score(1 * 10 + 2 * 20).doc(2).score(1 * 30).doc(3).score(2 * 40);
    EXPECT_EQ(expect, f.result);
}
...
struct BlueprintFixtureBase {
    ...
    ~BlueprintFixtureBase();
    Blueprint::UP blueprint(const search::query::Node& term) { return spec.blueprint(searchable, "field", term); }
    SearchIterator::UP iterator() { return spec.iterator(searchable, "field"); }
    FakeResult search(const search::query::Node& term) { return spec.search(searchable, "field", term); }
    FakeResult search() { return spec.search(searchable, "field"); }
};
...
struct BlueprintFixture : public BlueprintFixtureBase {
    BlueprintFixture() : BlueprintFixtureBase() {
        searchable.addResult("field", "A",
                             FakeResult().doc(1).weight(10).pos(0).doc(2).weight(30).pos(0).minMax(0, 30));
        searchable.addResult("field", "B",
                             FakeResult().doc(1).weight(20).pos(0).doc(3).weight(40).pos(0).minMax(0, 40));
...
struct BlueprintLargeScoresFixture : public BlueprintFixtureBase {
    BlueprintLargeScoresFixture() : BlueprintFixtureBase() {
        searchable.addResult("field", "A",
                             FakeResult().doc(1).weight(60000).pos(0).doc(2).weight(70000).pos(0).minMax(0, 70000));
        searchable.addResult("field", "B",
                             FakeResult().doc(1).weight(80000).pos(0).doc(3).weight(90000).pos(0).minMax(0, 90000));
...
TEST(
    ParallelWeakAndTest,
    require_that_scores_to_track_score_threshold_and_threshold_boost_factor_is_passed_down_from_query_node_to_blueprint) {
    BlueprintFixture f;
    Node::UP         term = f.spec.createNode(57, 67, 77.7);
    Blueprint::UP    bp = f.blueprint(*term);
    const auto*      pbp = dynamic_cast<const ParallelWeakAndBlueprint*>(bp.get());
...
TEST(ParallelWeakAndTest, require_that_initial_score_threshold_can_be_specified_case_1_hit_greater) {
    ...
    EXPECT_EQ(FakeResult().doc(3).score(2 * 40), f.search(*term));
}
...
TEST(ParallelWeakAndTest, require_that_large_scores_are_handled) {
    ...
    EXPECT_EQ(FakeResult().doc(1).score(60000L * 60000L + 70000L * 80000L).doc(3).score(70000L * 90000L),
              f.search(*term));
}
...
TEST(ParallelWeakAndTest, require_that_threshold_boosting_works_as_expected) {
    ...
    EXPECT_EQ(FakeResult().doc(1).score(1000).doc(2).score(2000).doc(3).score(3000).doc(4).score(4200), f1.result);
    EXPECT_EQ(FakeResult().doc(2).score(2000).doc(4).score(4200), f2.result);
}
...
SearchIterator::UP create_wand(bool use_dww, TermFieldMatchData& tfmd, const MatchParams& matchParams,
    ...
    for (size_t i = 0; i < dict_entries.size(); ++i) {
        terms.push_back(
            wand::Term(new DocidWithWeightSearchIterator(*(childrenMatchData->resolveTermField(handles[i])), attr,
                                                         dict_entries[i]),
...
private:
    SearchIterator::UP create(bool strict) const override {
        ...
        for (size_t i = 0; i < _num_children; ++i) {
            dict_entries.push_back(_helper.dww().lookup(vespalib::make_string("%zu", i).c_str(),
                                                        _helper.dww().get_dictionary_snapshot()));
        }
