...
struct DP {
    ...
    DP() : tokens(), field_is_filter(false), term_is_not_needed(false) {}

    DP& add(const std::string& token, uint32_t weight) {
        tokens.push_back(std::make_pair(token, weight));
        return *this;
    }
    DP& set_field_is_filter(bool value) {
        field_is_filter = value;
        return *this;
    }
    DP& set_term_is_not_needed(bool value) {
...
struct MockFixture {
    ...
    MockFixture(uint32_t initial) : MockFixture(initial, {new EmptySearch()}) {}
    ~MockFixture();
...
std::function<int(int)> make_score_filter(bool field_is_filter, bool term_is_not_needed) {
...
void run_simple(bool field_is_filter, bool term_is_not_needed) {
    ...
    FakeResult expect = FakeResult()
                            .doc(3)
                            .score(score_filter(30 * 3))
                            .doc(5)
                            .score(score_filter(50 * 5))
                            .doc(7)
                            .score(score_filter(70 * 7));
    DP ws = DP().add("7", 70)
                .add("5", 50)
                .add("3", 30)
                .add("100", 1000)
...
TEST(DotProductTest, test_simple_single) {
    FakeResult expect = FakeResult().doc(7).score(70 * 7);
    DP         ws = DP().add("7", 70);
...
void run_multi(bool field_is_filter, bool term_is_not_needed) {
    ...
    FakeResult expect = FakeResult()
                            .doc(3)
                            .score(score_filter(30 * 3 + 130 * 2 * 3 + 230 * 3 * 3))
                            .doc(5)
                            .score(score_filter(50 * 5 + 150 * 2 * 5))
                            .doc(7)
                            .score(score_filter(70 * 7));
    DP ws = DP().add("7", 70)
                .add("5", 50)
