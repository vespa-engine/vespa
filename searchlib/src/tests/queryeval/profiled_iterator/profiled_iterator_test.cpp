...
struct MySources {
    ...
    MySources(const std::vector<std::pair<uint32_t, uint32_t>>& entries) : selector(123, "<file>", num_docs) {
        for (const auto& entry : entries) {
            selector.setSource(entry.first, entry.second);
        }
    }
};
MySources my_sources({{1, 3}, {3, 3}, {5, 3}, {2, 5}, {4, 5}, {6, 5}});
...
SearchIterator::UP blend(SearchIterator::UP s1, uint32_t id1, SearchIterator::UP s2, uint32_t id2, int enum_id) {
    bool                          strict = is_true(s1->is_strict(), s2->is_strict());
...
SearchIterator::UP create_iterator_tree() {
    return AND(OR(T({4, 6, 8}, 3), T({5, 7, 9}, 4), 2), blend(t({1, 3, 5, 7, 9}, 6), 3, t({2, 4, 6, 8}, 7), 5, 5), 1);
}
...
SearchIterator::UP create_weak_and() {
    ...
    return with_id(WeakAndSearch::create(terms, wand::MatchParams(dummy_heap),
                                         wand::Bm25TermFrequencyScorer(num_docs), 100, true, true),
                   1);
}
