...
template <typename T>
concept ChildCollector = requires(T a, std::unique_ptr<Blueprint> bp) { a.addChild(std::move(bp)); };
...
struct LeafProxy : SimpleLeafBlueprint {
    ...
    LeafProxy(std::unique_ptr<Blueprint> child_in) : SimpleLeafBlueprint(), child(std::move(child_in)) { init(); }
...
struct CheckParamsProxy : LeafProxy {
    ...
    CheckParamsProxy(std::unique_ptr<Blueprint> child_in, bool expect_inherit_strict_in,
                     bool expect_same_constraint_in)
        : LeafProxy(std::move(child_in)),
          expect_inherit_strict(expect_inherit_strict_in),
          expect_same_constraint(expect_same_constraint_in) {}
    CheckParamsProxy(std::unique_ptr<Blueprint> child_in)
        : LeafProxy(std::move(child_in)),
          expect_forced_strict(true),
          expect_inherit_strict(false),
          expect_same_constraint(true) {}
...
struct CheckDroppedProxy : LeafProxy {
    mutable bool used;
    CheckDroppedProxy(std::unique_ptr<Blueprint> child_in) : LeafProxy(std::move(child_in)), used(false) {}
...
std::unique_ptr<Blueprint> check(std::unique_ptr<Blueprint> child, bool expect_inherit_strict,
                                 bool expect_same_constraint) {
...
struct Children {
    ...
    template <ChildCollector Builder> void apply(Builder& builder) const {
...
template <typename Flow> struct Combine {
    using factory_fun = std::function<std::unique_ptr<SearchIterator>(const Blueprint::Children&, Constraint)>;
    factory_fun         fun;
    Blueprint::Children list;
    Combine(factory_fun fun_in, const Children& child_list) : fun(fun_in), list() { child_list.apply(*this); }
    ~Combine();
    void addChild(std::unique_ptr<Blueprint> child) { list.push_back(std::move(child)); }
    ...
    auto createFilterSearch(Constraint constraint) const { return fun(list, constraint); }
};
template <typename Flow> Combine<Flow>::~Combine() = default;
...
struct SourceBlenderAdapter {
    ...
    void addChild(std::unique_ptr<Blueprint> child) { blueprint.addChild(std::move(child)); }
    void basic_plan(InFlow in_flow, uint32_t my_docid_limit) { blueprint.basic_plan(in_flow, my_docid_limit); }
    auto createFilterSearch(Constraint constraint) const { return blueprint.createFilterSearch(constraint); }
};
...
struct EquivAdapter {
    ...
    void addChild(std::unique_ptr<Blueprint> child) { blueprint.addTerm(std::move(child), 1.0); }
    void basic_plan(InFlow in_flow, uint32_t my_docid_limit) { blueprint.basic_plan(in_flow, my_docid_limit); }
    auto createFilterSearch(Constraint constraint) const { return blueprint.createFilterSearch(constraint); }
};
...
SameElementAdapter::SameElementAdapter() : field("foo", 5, 11), children(), blueprint() {
}
...
template <FilterFactory Blueprint> void verify(Blueprint&& blueprint, bool strict, const Expect& expect) {
    for (auto constraint : {lower_bound, upper_bound}) {
        verify(blueprint, strict, constraint, expect);
    }
}

template <FilterFactory Blueprint> void verify(Blueprint&& blueprint, const Expect& upper, const Expect& lower) {
    for (auto constraint : {lower_bound, upper_bound}) {
...
template <FilterFactory Blueprint> void verify(Blueprint&& blueprint, const Expect& upper_and_lower) {
...
TEST(FilterSearchTest, simple_or) {
    auto child_list =
        Children().hits({5, 10}).check(true, true).hits({7}).check(true, true).hits({3, 11}).check(true, true);
...
TEST(FilterSearchTest, forced_or) {
    auto child_list = Children().hits({5, 10}).check_forced().hits({7}).check_forced().hits({3, 11}).check_forced();
    auto expected = Expect::hits({3, 5, 7, 10, 11});
...
TEST(FilterSearchTest, or_short_circuit) {
    auto child_list =
        Children().hits({5, 10}).check(true, true).full().check(true, true).hits({3, 11}).check(true, true).dropped();
    verify(Combine<OrFlow>(Blueprint::create_or_filter, child_list), Expect::full());
}

TEST(FilterSearchTest, or_pruning) {
    auto child_list = Children().empty().check(true, true).empty().check(true, true).empty().check(true, true);
    verify(Combine<OrFlow>(Blueprint::create_or_filter, child_list), Expect::empty());
}

TEST(FilterSearchTest, or_partial_pruning) {
    auto child_list =
        Children().hits({5, 10}).check(true, true).empty().check(true, true).hits({3, 11}).check(true, true);
    verify(Combine<OrFlow>(Blueprint::create_or_filter, child_list), Expect::hits({3, 5, 10, 11}).child_count(2));
}
...
TEST(FilterSearchTest, and_short_circuit) {
    auto child_list = Children()
                          ...
                          .check(false, true)
                          .dropped();
    verify(Combine<AndFlow>(Blueprint::create_and_filter, child_list), Expect::empty());
}

TEST(FilterSearchTest, and_pruning) {
    auto child_list = Children().full().check(true, true).full().check(false, true).full().check(false, true);
    verify(Combine<AndFlow>(Blueprint::create_and_filter, child_list), Expect::full());
}

TEST(FilterSearchTest, and_partial_pruning) {
    auto child_list =
        Children().hits({1, 2, 3}).check(true, true).full().check(false, true).hits({2, 3, 4}).check(false, true);
    verify(Combine<AndFlow>(Blueprint::create_and_filter, child_list), Expect::hits({2, 3}).child_count(2));
}

TEST(FilterSearchTest, andnot_positive_short_circuit) {
    auto child_list = Children().empty().check(true, true).hits({1, 2, 3}).check(false, false).dropped();
    verify(Combine<AndNotFlow>(Blueprint::create_andnot_filter, child_list), Expect::empty());
}
...
TEST(FilterSearchTest, andnot_negative_pruning) {
    auto child_list = Children()
                          ...
                          .empty()
                          .check(false, false);
    verify(Combine<AndNotFlow>(Blueprint::create_andnot_filter, child_list), Expect::full());
}
...
TEST(FilterSearchTest, andnot_partial_negative_pruning) {
    auto child_list = Children()
                          ...
                          .hits({3})
                          .check(false, false);
    verify(Combine<AndNotFlow>(Blueprint::create_andnot_filter, child_list), Expect::hits({2}).child_count(3));
}

TEST(FilterSearchTest, first_or_child_can_be_partially_pruned) {
    auto child_list =
        Children().empty().check(true, true).hits({5, 10}).check(true, true).hits({3, 11}).check(true, true);
    verify(Combine<OrFlow>(Blueprint::create_or_filter, child_list), Expect::hits({3, 5, 10, 11}).child_count(2));
}

TEST(FilterSearchTest, first_and_child_can_be_pruned) {
    auto child_list =
        Children().full().check(true, true).hits({1, 2, 3}).check(false, true).hits({2, 3, 4}).check(false, true);
    verify(Combine<AndFlow>(Blueprint::create_and_filter, child_list), Expect::hits({2, 3}).child_count(2));
}
