...
const char* bool_str(bool bit) {
    return bit ? " true" : "false";
}
const char* leaf_str(bool array) {
    return array ? "A" : "B";
}
const char* opt_str(bool optimize) {
    return optimize ? "OPT" : "std";
}
const char* wrapped_str(bool wrapped) {
    return wrapped ? "WRAPPED" : "   LEAF";
}
const char* strict_str(bool strict) {
    return strict ? "    strict" : "non-strict";
}
...
template <bool strict> struct ArrayIterator : SearchIterator {
    uint32_t              my_offset = 0;
    uint32_t              my_limit;
    std::vector<uint32_t> my_hits;
    TMD&                  my_match_data;
    ArrayIterator(const BitVector& bv, TMD& tmd) : my_limit(bv.size()), my_match_data(tmd) {
...
template <bool strict> ArrayIterator<strict>::~ArrayIterator() = default;
...
struct OrSetup {
    ...
    bool should_use_array(size_t hits) { return (docid_limit / hits) >= 32; }
        ...
        for (double target : {0.1, 0.5, 1.0, 10.0}) {
            for (int unpack : {0, 1, 2}) {
                ...
                for (auto impl : {Impl::PLAIN, Impl::HEAP}) {
                    SCOPED_TRACE(fmt("impl: %s, optimize: %s, part: %zu, unpack: %d", impl_str(impl),
                                     bool_str(optimize), part, unpack));
        ...
        for (bool wrapped : {false, true}) {
            ...
            for (bool strict : {false, true}) {
                ...
                for (bool use_array : {false, true}) {
                    ...
                    fprintf(stderr,
                            "%s(%s) %s: (one of %4zu) seeks: %8zu, time: %10.3f ms, ns per seek: %10.3f, ms per "
                            "cost: %10.3f\n",
                            wrapped_str(wrapped), leaf_str(use_array), strict_str(strict), one_of, result.first,
                            result.second, ns_per(result), ms_per_cost);
                }
            }
        }
    }
...
TEST(OrSpeed, bm_strict_when_not_needed) {
    ...
    fprintf(stderr, "OR bench(%s, %s, children: %4zu, hits_per_child: %8zu %s)\n", impl_str(impl), opt_str(optimize),
            child_cnt, part, leaf_str(use_array));
        ...
        for (bool strict_or : {false, true}) {
            ...
            for (bool strict_children : {false, true}) {
                ...
                for (uint32_t skip = 1; skip < 500'000; skip *= 4) {
                    ...
                    bool conflict =
                        (strict_bm && !strict_or) || (strict_or && !strict_children) || (strict_bm && skip > 1);
                    if (!conflict) {
                        auto   result = setup.bm_search_ms(impl, optimize);
                        auto   flow_stats = FlowAdapter::stats(setup);
                        double in_flow = 1.0 / double(skip); // NOTE: not multiplied with strict cost
                        double ms_per_cost =
                            result.second / (strict_or ? flow_stats.strict_cost : (in_flow * flow_stats.cost));
                        fprintf(stderr,
                                "loop: %s, skip: %8u, OR: %s, children: %s, "
                ...
                if (part > 0 && (!use_array || !optimize)) {
                    ...
                    for (auto impl : {Impl::PLAIN, Impl::HEAP}) {
                        for (bool strict : {false, true}) {
                            ...
                            double ms_per_cost = result.second / (strict ? flow_stats.strict_cost : flow_stats.cost);
                            fprintf(stderr,
                                    "OR bench(%s, %s, children: %4zu, hits_per_child: %8zu %s, %s): "
                                    "seeks: %8zu, time: %10.3f ms, ns per seek: %10.3f, ms per cost: %10.3f\n",
                                    impl_str(impl), opt_str(optimize), child_cnt, part, leaf_str(use_array),
                                    strict_str(strict), result.first, result.second, ns_per(result), ms_per_cost);
                        }
                    }
                }
            }
