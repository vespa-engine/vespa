...
std::unique_ptr<Value> createTensor(const std::string& type_spec, double v1, double v2) {
    ...
    if (type.is_dense()) {
        return createTensor(TensorSpec(type_spec).add({{"x", 0}}, v1).add({{"x", 1}}, v2));
    } else {
        return createTensor(TensorSpec(type_spec).add({{"m", "a"}, {"x", 0}}, v1).add({{"m", "a"}, {"x", 1}}, v2));
    }
}
...
SimpleResult find_matches_impl(std::shared_ptr<QueryEvalStats> stats, Fixture& env, const Value& qtv,
                               double threshold) {
    ...
    auto                search = ExactNearestNeighborIterator::create(std::move(stats), strict, tfmd,
                                                                      std::make_unique<DistanceCalculator>(attr, qtv), dh, filter,
...
template <bool strict>
std::shared_ptr<QueryEvalStats> get_search_stats(Fixture& env, const Value& qtv,
...
void verify_iterator_returns_expected_results(const std::string& attribute_tensor_type_spec,
...
struct TestParam {
    ...
    TestParam(const std::string& attribute_tensor_type_spec_in, const std::string& query_tensor_type_spec_in) noexcept
...
INSTANTIATE_TEST_SUITE_P(ExactNearestNeighborIteratorTestSuite, ExactNearestNeighborIteratorParameterizedTest,
                         ::testing::Values(TestParam(denseSpecDouble, denseSpecDouble),
...
void verify_iterator_returns_filtered_results(const std::string& attribute_tensor_type_spec,
                                              const std::string& query_tensor_type_spec) {
    Fixture fixture(attribute_tensor_type_spec);
    fixture.ensureSpace(6);
...
void verify_iterator_collects_statistics(const std::string& attribute_tensor_type_spec,
                                         const std::string& query_tensor_type_spec) {
    Fixture fixture(attribute_tensor_type_spec);
    fixture.ensureSpace(6);
    fixture.setTensor(1, 3.0, 4.0);
...
TEST(NnsIndexIteratorTest, require_that_iterator_collects_statistics_for_multiple_subspaces) {
    auto u = SimpleValue::from_spec(TensorSpec(mixed_spec)
                                        .add({{"m", "a"}, {"x", 0}}, 1.0)
                                        .add({{"m", "a"}, {"x", 1}}, 2.0)
                                        .add({{"m", "b"}, {"x", 0}}, 3.0)
                                        .add({{"m", "b"}, {"x", 1}}, 4.0));
    auto v = SimpleValue::from_spec(
        TensorSpec(mixed_spec).add({{"m", "a"}, {"x", 0}}, 1.0).add({{"m", "a"}, {"x", 1}}, 2.0));
...
template <bool strict> std::vector<feature_t> get_rawscores(Fixture& env, const Value& qtv) {
    auto  md = MatchData::makeTestInstance(2, 2);
    auto& tfmd = *(md->resolveTermField(0));
    auto& attr = *(env._attr);
    auto  dff = search::tensor::make_distance_function_factory(DistanceMetric::Euclidean, qtv.cells().type);
    ...
    auto                        search = ExactNearestNeighborIterator::create(
        nullptr, strict, tfmd, std::make_unique<DistanceCalculator>(attr, qtv), dh, *dummy_filter, false);
...
void verify_iterator_sets_expected_rawscore(const std::string& attribute_tensor_type_spec,
                                            const std::string& query_tensor_type_spec) {
    Fixture fixture(attribute_tensor_type_spec);
    fixture.ensureSpace(6);
    fixture.setTensor(1, 3.0, 4.0);
