...
uint32_t calc_hits_per_term(uint32_t num_docs, double op_hit_ratio, uint32_t children, QueryOperator query_op) {
...
std::unique_ptr<BenchmarkSearchable> make_searchable(const FieldConfig& cfg, uint32_t num_docs,
...
std::unique_ptr<Node> make_query_node(QueryOperator query_op, const TermVector& terms) {
    if (query_op == QueryOperator::Term) {
        ...
        auto     res = std::make_unique<SimpleWandTerm>(terms.size(), field_name, 0, Weight(1), target_hits,
                                                        score_threshold, threshold_boost_factor);
        for (auto term : terms) {
...
Blueprint::UP make_intermediate_blueprint(std::unique_ptr<IntermediateBlueprint> blueprint,
                                          BenchmarkSearchable& searchable, const TermVector& terms,
...
Blueprint::UP make_blueprint_helper(BenchmarkSearchable& searchable, QueryOperator query_op, const TermVector& terms,
                                    uint32_t docid_limit) {
    if (query_op == QueryOperator::And) {
        ...
        uint32_t target_hits = 100;
        return make_intermediate_blueprint(std::make_unique<WeakAndBlueprint>(target_hits), searchable, terms,
                                           docid_limit);
    } else {
...
public:
    MyFactory(const FieldConfig& field_cfg, QueryOperator query_op, uint32_t num_docs,
              uint32_t default_values_per_document, double op_hit_ratio, uint32_t children, bool disjunct_children);

    std::unique_ptr<Blueprint> make_blueprint() override;
    std::string get_name(Blueprint& blueprint) const override { return get_class_name(blueprint); }
};

MyFactory::MyFactory(const FieldConfig& field_cfg, QueryOperator query_op, uint32_t num_docs,
                     uint32_t default_values_per_document, double op_hit_ratio, uint32_t children,
                     bool disjunct_children)
    : _query_op(query_op), _docid_limit(num_docs + 1), _terms(), _searchable() {
...
std::unique_ptr<Blueprint> MyFactory::make_blueprint() {
...
std::unique_ptr<BenchmarkBlueprintFactory> make_blueprint_factory(const FieldConfig& field_cfg,
                                                                  ...
                                                                  bool disjunct_children) {
    return std::make_unique<MyFactory>(field_cfg, query_op, num_docs, default_values_per_document, op_hit_ratio,
