...
struct Fixture {
    void benchmark();
    template <typename T> void testSearch(bool strict);
...
H seek(SearchIterator& s, uint32_t docIdLimit) {
    H h;
...
template <typename T> void Fixture::testSearch(bool strict) {
...
int main(int argc, char** argv) {
    if (argc < 6) {
        LOG(info,
            "%s <'and/or'> <'strict/no-strict'> <'optimize/no-optimize> <numsearch> <numdocs> <fill 1> [<fill N>]",
            argv[0]);
        exit(1);
    }
    Fixture fixture(argc, argv);
    LOG(info, "Start setup of '%s' isearch with %ld vectors with %d documents", fixture._type.c_str(),
        fixture._fillLimits.size(), fixture._numDocs);
    fixture.setup();
