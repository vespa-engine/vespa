...
struct Fixture {
    ...
    template <typename T> void testOptimizeCommon(bool isAnd, bool invert);
    template <typename T> void testSearch(bool strict, bool invert);
    template <typename T> void testOptimizeAndOr(bool invert);
    template <typename T> void testThatOptimizePreservesUnpack();
...
Fixture::Fixture() : _bvs(), _bvs_inverted() {
...
class MultiBitVectorIteratorTest : public ::testing::Test, protected Fixture {
protected:
...
H seekNoReset(SearchIterator& s, uint32_t start, uint32_t docIdLimit) {
    H h;
...
void Fixture::testAndWith(bool invert) {
...
template <typename T> void Fixture::testThatOptimizePreservesUnpack() {
...
void Fixture::verifyUnpackOfOr(const UnpackInfo& unpackInfo) {
...
template <typename T> void Fixture::testSearch(bool strict, bool invert) {
    TermFieldMatchData tfmd;
...
template <typename T> void Fixture::testOptimizeCommon(bool isAnd, bool invert) {
    TermFieldMatchData tfmd;
...
template <typename T> void Fixture::testOptimizeAndOr(bool invert) {
    TermFieldMatchData tfmd;
...
void Fixture::testEndGuard(bool invert) {
    using T = AndSearch;
    TermFieldMatchData tfmd;
...
SearchIterator::UP createDual(Fixture& f, TermFieldMatchData& tfmd, int32_t docIdLimit) {
    MultiSearch::Children children;
...
void countUntilEnd(SearchIterator& s) {
...
void countUntilDocId(SearchIterator& s) {
    uint32_t seekCount = 0;
...
Verifier::Verifier(size_t numBv, bool is_and) : _is_and(is_and), _bvs() {
...
SearchIterator::UP Verifier::create(bool strict) const {
    ...
    SearchIterator::UP iter(_is_and ? AndSearch::create(std::move(bvs), strict)
                                    : OrSearch::create(std::move(bvs), strict));
