...
struct MyTerm : public search::queryeval::SimpleLeafBlueprint {
    MyTerm(const FieldSpec& field, uint32_t hits) : search::queryeval::SimpleLeafBlueprint(field) {
...
public:
    ...
    PhraseSearchTest& addTerm(const string& term, bool last) {
        return addTerm(term, FakeResult().doc(doc_match).pos(_pos).doc(doc_no_match).pos(_pos + last));
    }
    ...
    PhraseSearchTest& addTerm(const string& term, const FakeResult& r) {
        ...
        SimpleStringTerm term_node(term, field, 0, Weight(0));
        {
            ...
            fields.add(
                FieldSpec(field, fieldId, childHandle(_children.size()))); // _layout.allocTermField(fieldId)));
    ...
    void fetchPostings(bool useBlueprint) {
    ...
    SearchIterator* createSearch(bool useBlueprint) {
        ...
        if (useBlueprint) {
            ...
            search = std::make_unique<SimplePhraseSearch>(std::move(children), MatchData::UP(), childMatch, _order,
...
TEST(SimplePhraseTest, requireThatIteratorFindsLongPhrase) {
    for (bool useBlueprint : {false, true}) {
        PhraseSearchTest test;
        test.addTerm("foo", 0).addTerm("bar", 0).addTerm("baz", 0).addTerm("qux", 1);
    ...
    for (bool useBlueprint : {false, true}) {
        for (bool unpack_normal_features : {false, true}) {
            for (bool unpack_interleaved_features : {false, true}) {
                PhraseSearchTest test;
                test.addTerm("foo", FakeResult().doc(doc_match).pos(1).pos(11).pos(21).field_length(30).num_occs(3));
                test.addTerm("bar", FakeResult().doc(doc_match).pos(2).pos(16).pos(22).field_length(30).num_occs(3));
