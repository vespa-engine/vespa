...
struct SimpleWandFixture {
    MyWandSpec   spec;
    SimpleResult hits;
    SimpleWandFixture() : SimpleWandFixture(false) {}
    explicit SimpleWandFixture(bool second_phase) : spec(2), hits() {
        if (second_phase) {
...
struct WeightOrder {
    bool operator()(const wand::Term& t1, const wand::Term& t2) const { return (t1.weight < t2.weight); }
};
...
TEST(WeakAndTest, require_that_initial_docid_for_subsearches_are_taken_into_account) {
    ...
    wand::MatchParams          match_params(scores, wand::StopWordStrategy::none(),
                                            wand::DEFAULT_PARALLEL_WAND_SCORES_ADJUST_FREQUENCY, docid_limit);
    auto                       search =
        std::make_unique<TrackedSearch>("WAND", history, WeakAndSearch::create(terms, match_params, 2, true, false));
    SimpleResult hits;
    hits.search(*search, docid_limit);
...
TEST(WeakAndTest, require_that_the_selected_adjust_word_must_match_when_using_stop_word_adjust_limit) {
    ...
    EXPECT_EQ(
        SimpleResult().addHit(1).addHit(2).addHit(3).addHit(4).addHit(5).addHit(6).addHit(7).addHit(8).addHit(9),
        spec.search());
    ...
    EXPECT_EQ(History()
                  ...
                  .step("2", 5)
                  .seek("1", 5)
                  .step("1", search::endDocId) // 1+2 can compete with 3
                  .step("WAND", 8)
                  .unpack("WAND", 8)
                  .seek("2", 8)
                  .step("2", search::endDocId)
                  .unpack("3", 8)
                  .seek("WAND", 9)
                  .seek("3", 9)
                  .step("3", 9)
...
private:
    ...
    SearchIterator::UP create(bool strict) const override {
        ...
        for (size_t i = 0; i < _num_children; ++i) {
            terms.emplace_back(createIterator(_split_lists[i], strict).release(), 100, _split_lists[i].size());
        }
        ...
        return WeakAndSearch::create(
            terms, wand::MatchParams(*_scores.back(), wand::StopWordStrategy::none(), 1, docid_limit), -1, strict,
            false);
    }
};
