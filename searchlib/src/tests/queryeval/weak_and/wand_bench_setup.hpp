...
struct Stats {
    ...
    size_t skippedDocs;
    size_t skippedHits;
    Stats() noexcept : hitCnt(0), seekCnt(0), unpackCnt(0), skippedDocs(0), skippedHits(0) {}
    void hit() noexcept { ++hitCnt; }
    void seek(size_t docs, size_t hits) noexcept {
        ++seekCnt;
        skippedDocs += docs;
        skippedHits += hits;
    }
    void unpack() noexcept { ++unpackCnt; }
    void print() const {
        fprintf(stderr, "Stats: hits=%zu, seeks=%zu, unpacks=%zu, skippedDocs=%zu, skippedHits=%zu\n", hitCnt,
                seekCnt, unpackCnt, skippedDocs, skippedHits);
    }
};
...
ModSearch::ModSearch(Stats& stats_in, uint32_t step_in, uint32_t limit_in, int32_t maxWeight,
                     TermFieldMatchData* tfmd_in)
...
struct VespaWandFactory : WandFactory {
    ...
    uint32_t                           n;
    explicit VespaWandFactory(uint32_t n_in) noexcept : _scores(n_in), n(n_in) {}
    ...
    SearchIterator::UP create(const wand::Terms& terms) override {
        return WeakAndSearch::create(terms, wand::MatchParams(_scores, wand::StopWordStrategy::none(), 1, 0), n, true,
                                     false);
    }
};
...
struct VespaArrayWandFactory : WandFactory {
    ...
    explicit VespaArrayWandFactory(uint32_t n_in, uint32_t docid_limit_in)
        : _scores(n_in), n(n_in), docid_limit(docid_limit_in) {}
    ~VespaArrayWandFactory() override;
    std::string name() const override { return make_string("VESPA ARRAY WAND (n=%u)", n); }
    SearchIterator::UP create(const wand::Terms& terms) override {
        return WeakAndSearch::createArrayWand(terms, wand::MatchParams(_scores, wand::StopWordStrategy::none(), 1, 0),
                                              wand::Bm25TermFrequencyScorer(docid_limit), n, true, false);
    }
};
...
struct VespaHeapWandFactory : WandFactory {
    ...
    SearchIterator::UP create(const wand::Terms& terms) override {
        return WeakAndSearch::createHeapWand(terms, wand::MatchParams(_scores, wand::StopWordStrategy::none(), 1, 0),
                                             wand::Bm25TermFrequencyScorer(docid_limit), n, true, false);
    }
};
...
struct VespaParallelWandFactory : public WandFactory {
    ...
    SearchIterator::UP create(const wand::Terms& terms) override {
        return ParallelWeakAndSearch::create(terms, PWMatchParams(scores, 0, 1, 1, 0),
...
struct VespaParallelArrayWandFactory : public VespaParallelWandFactory {
    ...
    SearchIterator::UP create(const wand::Terms& terms) override {
        return ParallelWeakAndSearch::createArrayWand(terms, PWMatchParams(scores, 0, 1, 1, 0),
                                                      PWRankParams(rootMatchData, {}), true, false);
    }
};
...
struct VespaParallelHeapWandFactory : public VespaParallelWandFactory {
    ...
    SearchIterator::UP create(const wand::Terms& terms) override {
        return ParallelWeakAndSearch::createHeapWand(terms, PWMatchParams(scores, 0, 1, 1, 0),
                                                     PWRankParams(rootMatchData, {}), true, false);
    }
};
...
struct TermFrequencyRiseWandFactory : WandFactory {
    ...
    explicit TermFrequencyRiseWandFactory(uint32_t n_in, uint32_t docid_limit_in) noexcept
        : n(n_in), docid_limit(docid_limit_in) {}
...
struct WandSetup : Setup {
    ...
    WandSetup(WandFactory& f, uint32_t c, uint32_t l)
        : Setup(), factory(f), childCnt(c), limit(l), weight(100), matchData() {}
    ~WandSetup() override;
