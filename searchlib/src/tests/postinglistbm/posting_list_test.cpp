...
void validate_posting_list_for_word(const FakePosting& posting, const FakeWord& word) {
    ...
    if (posting.hasWordPositions()) {
        word.validate(iterator.get(), tfmda, posting.enable_unpack_normal_features(),
                      posting.has_interleaved_features() && posting.enable_unpack_interleaved_features(), false);
    } else {
        word.validate(iterator.get(), false);
    }
}

void test_fake(const std::string& posting_type, const Schema& schema, const FakeWord& word) {
    ...
    printf("%s.bitsize=%d+%d+%d+%d+%d\n", posting->getName().c_str(), static_cast<int>(posting->bitSize()),
           static_cast<int>(posting->l1SkipBitSize()), static_cast<int>(posting->l2SkipBitSize()),
           static_cast<int>(posting->l3SkipBitSize()), static_cast<int>(posting->l4SkipBitSize()));
...
struct PostingListTest : public ::testing::Test {
    ...
    void setup(bool has_elements, bool has_element_weights) {
        ...
        word1 = std::make_unique<FakeWord>(num_docs, w1_freq, w1_freq / 2, "word1", rnd, word_set.getFieldsParams(),
                                           word_set.getPackedIndex());
        ...
        word4 = std::make_unique<FakeWord>(num_docs, w4_freq, w4_freq / 2, "word4", rnd, word_set.getFieldsParams(),
                                           word_set.getPackedIndex());
