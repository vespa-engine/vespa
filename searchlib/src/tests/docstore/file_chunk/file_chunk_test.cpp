...
struct BucketizerObserver : public IBucketizer {
    ...
    GenerationGuard getGuard() const override { return {}; }
};

std::string getData(uint32_t lid) {
...
struct FixtureBase {
    ...
    ~FixtureBase();
    void assertLidMap(const std::vector<uint32_t>& expLids) const { EXPECT_EQ(expLids, lidObserver.lids); }
    void assertBucketizer(const std::vector<uint32_t>& expLids) const { EXPECT_EQ(expLids, bucketizer.lids); }
};
...
struct WriteFixture : public FixtureBase {
    ...
    WriteFixture(const std::string& baseName, uint32_t docIdLimit, bool dirCleanup = true)
        : FixtureBase(baseName, dirCleanup),
...
template <typename FixtureType> void assertUpdateLidMap(FixtureType& f) {
