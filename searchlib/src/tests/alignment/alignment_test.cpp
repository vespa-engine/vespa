...
struct Timer {
    rusage usage;
    void start() { getrusage(RUSAGE_SELF, &usage); }
    double stop() {
        rusage tmp;
        getrusage(RUSAGE_SELF, &tmp);
        double startMs = (((double)usage.ru_utime.tv_sec) * 1000.0) + (((double)usage.ru_utime.tv_usec) / 1000.0);
        double stopMs = (((double)tmp.ru_utime.tv_sec) * 1000.0) + (((double)tmp.ru_utime.tv_usec) / 1000.0);
        return (stopMs - startMs);
    }
};

double timeAccess(void* bufp, uint32_t len, double& sum) {
