...
TEST_F(VerifyFeatureTest, verify_illegal_input_parameters) {
    EXPECT_FALSE(
        verify("value.0",
               {{equal,
                 {Level::WARNING, "invalid rank feature value.0:"
...
TEST_F(VerifyFeatureTest, verify_too_deep_dependency_graph) {
    ...
    EXPECT_FALSE(verify(
        "chain(basic, 256, 4)",
        {{equal,
          {Level::WARNING, "invalid rank feature value(4): dependency graph too deep\n"
...
TEST_F(VerifyFeatureTest, verify_dependency_cycle) {
    EXPECT_FALSE(verify(
        "chain(cycle, 4, 2)",
        {{equal,
          {Level::WARNING, "invalid rank feature chain(cycle,2,2): dependency cycle detected\n"
         ...
         {equal,
          {Level::ERROR, "verification failed: rank feature chain(cycle, 4, 2) (feature verification test)"}}}));
}
