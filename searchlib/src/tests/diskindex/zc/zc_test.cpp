...
#include <vespa/searchlib/diskindex/zc_decoder.h>
#include <vespa/searchlib/diskindex/zcbuf.h>
#include <vespa/vespalib/gtest/gtest.h>
#include <limits>

using search::diskindex::ZcBuf;
using search::diskindex::ZcDecoder;
...
{
    _zc_buf.clear();
    _zc_buf.encode(value);
    auto view = _zc_buf.view();
    bool failed = false;
    ZcDecoder zc_decoder(view);
    EXPECT_EQ(value, zc_decoder.decode()) << (failed = true, "");
...
{
    bool failed = false;
    ZcDecoder zc_decoder(_zc_buf.view());
    for (uint32_t i = 0; i < fill_size; ++i) {
        if (zc_decoder.decode() != i) [[unlikely]] {
            failed = true;
        }
    }
    return !failed;
...
TEST_F(ZcTest, usec_bytes_at_boundaries)
{
