...
Verifier::Verifier(FakePosting::SP fp) : _tfmd(), _tfmda(), _fp(std::move(fp)) {
    if (_fp) {
...
struct EmptySettings {
    ...
    bool _empty_doc;
    bool _empty_word;
    EmptySettings() : _empty_field(false), _empty_doc(false), _empty_word(false) {}
    EmptySettings empty_field() && {
        _empty_field = true;
        return *this;
    }
    EmptySettings empty_doc() && {
        _empty_doc = true;
        return *this;
    }
    EmptySettings empty_word() && {
        _empty_word = true;
        return *this;
    }
};
...
struct IOSettings {
    ...
    IOSettings() : _use_directio(false), _use_mmap(false), _use_posting_list_cache(false) {}
    IOSettings use_directio() && {
        _use_directio = true;
        return *this;
    }
    IOSettings use_mmap() && {
    ...
    IOSettings use_posting_list_cache() && {
        _use_posting_list_cache = true;
        return *this;
    }
};
...
protected:
    ...
    Blueprint::UP create_blueprint(const FieldSpec& field, const search::query::Node& term,
                                   uint32_t docid_limit = 1000);
};
...
void DiskIndexTest::SetUpTestSuite() {
...
void DiskIndexTest::TearDownTestSuite() {
    std::filesystem::path index_path(test_dir);
    std::filesystem::remove_all(index_path);
}

void DiskIndexTest::requireThatSearchIteratorsConforms() {
    FakePosting::SP tmp;
    Verifier        verTmp(tmp);
    Schema          schema;
    ...
    v.push_back(&fw);
    std::set<std::string> ignored = {"MemTreeOcc",
                                     "MemTreeOcc2",
                                     "FilterOcc",
                                     "ZcFilterOcc",
                                     "ZcNoSkipFilterOcc",
                                     ...
                                     "ZcbFilterOcc",
                                     "EGCompr64FilterOcc",
                                     "EGCompr64LEFilterOcc",
                                     "EGCompr64NoSkipFilterOcc",
                                     "EGCompr64SkipFilterOcc"};
...
void DiskIndexTest::requireThatLookupIsWorking(const EmptySettings& empty_settings) {
...
SimpleResult DiskIndexTest::search(const FieldIndex& field_index, const DictionaryLookupResult& lookup_result,
                                   const PostingListHandle& handle) {
...
Blueprint::UP DiskIndexTest::create_blueprint(const FieldSpec& field, const search::query::Node& term,
...
void DiskIndexTest::requireThatWeCanReadPostingList(const IOSettings& io_settings) {
    { // field 'f1'
...
void DiskIndexTest::require_that_we_can_get_field_length_info() {
...
void DiskIndexTest::requireThatWeCanReadBitVector() {
    { // word 'w1'
...
void DiskIndexTest::requireThatBlueprintIsCreated() {
    search::fef::MatchDataLayout mdl;
    { // unknown field
...
void DiskIndexTest::requireThatBlueprintCanCreateSearchIterators() {
    TermFieldMatchData      md;
    TermFieldMatchDataArray mda;
    mda.add(&md);
    Blueprint::UP      b;
    ...
    { // posting list iterator
        ...
        ASSERT_TRUE((dynamic_cast<ZcRareWordPosOccIterator<true, false>*>(s.get()) != nullptr) ||
                    (dynamic_cast<ZcPosOccIterator<true, false>*>(s.get()) != nullptr));
        EXPECT_EQ(result_f1_w1, SimpleResult().search(*s, 1000));
        EXPECT_EQ(result_f1_w1, SimpleResult().search(*leaf_b.createFilterSearch(upper_bound), 1000));
    }
...
void DiskIndexTest::build_index(const IOSettings& io_settings, const EmptySettings& empty_settings) {
...
void DiskIndexTest::test_empty_settings(const EmptySettings& empty_settings) {
    build_index(IOSettings(), empty_settings);
    requireThatLookupIsWorking(empty_settings);
    require_that_get_stats_works();
}

void DiskIndexTest::test_io_settings(const IOSettings& io_settings) {
