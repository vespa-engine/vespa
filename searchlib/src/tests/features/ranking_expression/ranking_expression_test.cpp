...
struct DummyReplacer : ExpressionReplacer {
    std::string trigger;
    FeatureType type;
    DummyReplacer(const std::string trigger_in, const FeatureType& type_in) : trigger(trigger_in), type(type_in) {}
...
struct SetupResult {
    ...
    bool                       setup_ok;
    SetupResult(const TypeMap& object_inputs, const std::string& expression, const std::string& expression_name = "");
    ~SetupResult();
};

SetupResult::SetupResult(const TypeMap& object_inputs, const std::string& expression,
...
void verify_output_type(const TypeMap& object_inputs, const std::string& expression, const FeatureType& expect,
                        const std::string& expression_name = "") {
...
void verify_setup_fail(const TypeMap& object_inputs, const std::string& expression) {
...
TEST(RankingExpressionTest, require_that_replaced_expressions_override_result_type) {
    verify_output_type({{"b", "tensor(z{})"}}, "a*b*c", FeatureType::object(ValueType::from_spec("tensor(z{})")));
    verify_output_type({{"b", "tensor(z{})"}}, "foo*b*c", FeatureType::number());
    verify_output_type({{"b", "tensor(z{})"}}, "a*b*bar", FeatureType::object(ValueType::from_spec("tensor(x[5])")));
    verify_output_type({{"b", "tensor(z{})"}}, "foo*b*bar", FeatureType::number());
}
