...
TEST(TensorFromLabelsTest, require_that_setup_succeeds_with_attribute_source) {
    SetupFixture f;
    FTA::FT_SETUP_OK(f.blueprint, f.indexEnv, StringList().add("attribute(foo)"), StringList(),
                     StringList().add("tensor"));
}

TEST(TensorFromLabelsTest, require_that_setup_succeeds_with_query_source) {
    SetupFixture f;
    FTA::FT_SETUP_OK(f.blueprint, f.indexEnv, StringList().add("query(foo)"), StringList(),
                     StringList().add("tensor"));
}
...
TEST(TensorFromLabelsTest, require_that_array_string_attribute_can_be_converted_to_tensor_using_default_dimension) {
    ...
    EXPECT_EQ(
        *make_tensor(
            TensorSpec("tensor(astr{})").add({{"astr", "a"}}, 1).add({{"astr", "b"}}, 1).add({{"astr", "c"}}, 1)),
        f.execute());
}
...
TEST(TensorFromLabelsTest, require_that_array_string_attribute_can_be_converted_to_tensor_using_explicit_dimension) {
    ...
    EXPECT_EQ(*make_tensor(
                  TensorSpec("tensor(dim{})").add({{"dim", "a"}}, 1).add({{"dim", "b"}}, 1).add({{"dim", "c"}}, 1)),
              f.execute());
}
...
TEST(TensorFromLabelsTest, require_that_array_integer_attribute_can_be_converted_to_tensor_using_default_dimension) {
    ...
    EXPECT_EQ(
        *make_tensor(
            TensorSpec("tensor(aint{})").add({{"aint", "7"}}, 1).add({{"aint", "3"}}, 1).add({{"aint", "5"}}, 1)),
        f.execute());
}
...
TEST(TensorFromLabelsTest, require_that_array_attribute_can_be_converted_to_tensor_using_explicit_dimension) {
    ...
    EXPECT_EQ(*make_tensor(
                  TensorSpec("tensor(dim{})").add({{"dim", "7"}}, 1).add({{"dim", "3"}}, 1).add({{"dim", "5"}}, 1)),
              f.execute());
}

TEST(TensorFromLabelsTest,
     require_that_single_value_integer_attribute_can_be_converted_to_tensor_using_default_dimension) {
    ExecFixture f("tensorFromLabels(attribute(sint))");
    EXPECT_EQ(*make_tensor(TensorSpec("tensor(sint{})").add({{"sint", "5"}}, 1)), f.execute());
}

TEST(TensorFromLabelsTest,
     require_that_single_value_integer_attribute_can_be_converted_to_tensor_using_explicit_dimension) {
    ExecFixture f("tensorFromLabels(attribute(sint),foobar)");
    EXPECT_EQ(*make_tensor(TensorSpec("tensor(foobar{})").add({{"foobar", "5"}}, 1)), f.execute());
}
...
TEST(TensorFromLabelsTest,
     require_that_single_value_string_attribute_can_be_converted_to_tensor_using_default_dimension) {
    ExecFixture f("tensorFromLabels(attribute(sstr))");
    EXPECT_EQ(*make_tensor(TensorSpec("tensor(sstr{})").add({{"sstr", "foo"}}, 1)), f.execute());
}

TEST(TensorFromLabelsTest,
     require_that_single_value_string_attribute_can_be_converted_to_tensor_using_explicit_dimension) {
    ExecFixture f("tensorFromLabels(attribute(sstr),foobar)");
    EXPECT_EQ(*make_tensor(TensorSpec("tensor(foobar{})").add({{"foobar", "foo"}}, 1)), f.execute());
}
...
TEST(TensorFromLabelsTest, require_that_string_array_from_query_can_be_converted_to_tensor_using_explicit_dimension) {
    ...
    EXPECT_EQ(*make_tensor(
                  TensorSpec("tensor(dim{})").add({{"dim", "d"}}, 1).add({{"dim", "e"}}, 1).add({{"dim", "f"}}, 1)),
              f.execute());
}
