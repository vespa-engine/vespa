...
const std::vector<std::string> mixed_tensor_types{"error", "tensor(a{},x[2])", "tensor(a{},b{},x[2])"};
const std::string              sparse_tensor_type("tensor(a{})");

const std::vector<TensorSpec> no_subspaces{TensorSpec("error"), TensorSpec(sparse_tensor_type),
                                           TensorSpec("tensor(a{},b{})")};
const std::vector<TensorSpec> subspace_as{TensorSpec("error"), TensorSpec::from_expr("tensor(a{}):{{a:\"a\"}:1}"),
                                          TensorSpec::from_expr("tensor(a{},b{}):{{a:\"a\",b:\"K\"}:1}")};
const std::vector<TensorSpec> subspace_bs{TensorSpec("error"), TensorSpec::from_expr("tensor(a{}):{{a:\"b\"}:1}"),
...
void assert_setup(std::string field_name, bool exp_setup_result, std::optional<std::string> attr_type_spec,
    ...
    if (attr_type_spec.has_value()) {
        search::fef::indexproperties::type::Attribute::set(f2.indexEnv.getProperties(), field_name,
                                                           attr_type_spec.value());
    }
...
ClosestTest::ClosestTest() : testing::TestWithParam<TestParam>() {
}
