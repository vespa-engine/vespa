...
struct ExecFixture {
    ...
    void setupQueryEnvironment() { test.getQueryEnv().getProperties().add("wsquery", "{d:11,e:13,f:17}"); }
    const Value& extractTensor(uint32_t docid) { return test.resolveObjectFeature(docid); }
    const Value& execute() { return extractTensor(1); }
};

TEST(TensorFromWeightedSetTest,
     require_that_weighted_set_string_attribute_can_be_converted_to_tensor_using_default_dimension) {
    ExecFixture f("tensorFromWeightedSet(attribute(wsstr))");
    EXPECT_EQ(
        *make_tensor(
            TensorSpec("tensor(wsstr{})").add({{"wsstr", "b"}}, 5).add({{"wsstr", "c"}}, 7).add({{"wsstr", "a"}}, 3)),
        f.execute());
}

TEST(TensorFromWeightedSetTest,
     require_that_weighted_set_string_attribute_can_be_converted_to_tensor_using_explicit_dimension) {
    ExecFixture f("tensorFromWeightedSet(attribute(wsstr),dim)");
    EXPECT_EQ(*make_tensor(
                  TensorSpec("tensor(dim{})").add({{"dim", "a"}}, 3).add({{"dim", "b"}}, 5).add({{"dim", "c"}}, 7)),
              f.execute());
}

TEST(TensorFromWeightedSetTest,
     require_that_weighted_set_integer_attribute_can_be_converted_to_tensor_using_default_dimension) {
...
TEST(TensorFromWeightedSetTest,
     require_that_weighted_set_integer_attribute_can_be_converted_to_tensor_using_explicit_dimension) {
    ExecFixture f("tensorFromWeightedSet(attribute(wsint),dim)");
    EXPECT_EQ(
        *make_tensor(
            TensorSpec("tensor(dim{})").add({{"dim", "17"}}, 7).add({{"dim", "11"}}, 3).add({{"dim", "13"}}, 5)),
        f.execute());
}

TEST(TensorFromWeightedSetTest,
     require_that_weighted_set_from_query_can_be_converted_to_tensor_using_default_dimension) {
...
TEST(TensorFromWeightedSetTest,
     require_that_weighted_set_from_query_can_be_converted_to_tensor_using_explicit_dimension) {
    ExecFixture f("tensorFromWeightedSet(query(wsquery),dim)");
    EXPECT_EQ(
        *make_tensor(
            TensorSpec("tensor(dim{})").add({{"dim", "d"}}, 11).add({{"dim", "e"}}, 13).add({{"dim", "f"}}, 17)),
        f.execute());
}
