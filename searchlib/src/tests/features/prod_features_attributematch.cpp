...
TEST_F(ProdFeaturesTest, test_attribute_match) {
    AttributeMatchBlueprint pt;
    {
        ...
        FT_SETUP_OK(pt, idx_env, params.clear().add("sint"), in,
        ...
        FT_DUMP(_factory, "attributeMatch", idx_env,
                out.clear()
    ...
    { // single attributes
        FtFeatureTest ft(
            _factory,
            StringList().add("attributeMatch(sint)").add("attributeMatch(sfloat)").add("attributeMatch(sstr)"));
        ...
        ASSERT_TRUE(ft.getQueryEnv().getBuilder().addAttributeNode("sfloat") !=
                    nullptr); // query term 4, hit in sfloat
        ...
        ft.getQueryEnv().getProperties().add("vespa.term.1.significance",
                                             "0.1"); // change significance for all other terms
    ...
    { // weighted set attributes
        FtFeatureTest ft(
            _factory,
            StringList().add("attributeMatch(wsint)").add("attributeMatch(wsfloat)").add("attributeMatch(wsstr)"));
        ft.getIndexEnv().getBuilder().addField(FieldType::ATTRIBUTE, CollectionType::WEIGHTEDSET,
                                               "wsint"); // 2 matches
        ft.getIndexEnv().getBuilder().addField(FieldType::ATTRIBUTE, CollectionType::WEIGHTEDSET,
                                               "wsfloat"); // 1 matches
        ft.getIndexEnv().getBuilder().addField(FieldType::ATTRIBUTE, CollectionType::WEIGHTEDSET,
        ...
        exp.addScore("attributeMatch(wsint)", 1)
            ...
            .addScore("attributeMatch(wsfloat)", 0.95)
            . // same as completeness
            ...
            .addScore("attributeMatch(wsstr)", 0)
            . // same as completeness
        ...
        mdb->apply(1);
        exp.clear().addScore("attributeMatch(wsint).fieldCompleteness", 0.5f);
    ...
    { // unique only attribute
        ...
        RankResult exp;
        exp.addScore("attributeMatch(unique)", 0)
            . // same as completeness
    ...
    { // tensor attribute is not allowed
        ...
        ft.getIndexEnv().getBuilder().addField(FieldType::ATTRIBUTE, CollectionType::SINGLE, DataType::TENSOR,
                                               "tensor");
