...
struct SetupFixture {
    ...
    void addAttribute(const std::string& name, const CollectionType& collType, const DataType& dataType) {
        indexEnv.addField(FieldType::ATTRIBUTE, collType, name);
        indexEnv.getFields().back().set_data_type(dataType);
    }
};
...
TEST(InternalMaxReduceProdJoinFeatureTest, require_that_setup_succeeds_with_long_array_attribute) {
    SetupFixture f;
    FTA::FT_SETUP_OK(f.blueprint, f.indexEnv, StringList().add("longarray").add("query"), StringList(),
                     StringList().add("scalar"));
}

TEST(InternalMaxReduceProdJoinFeatureTest, require_that_setup_succeeds_with_int_array_attribute) {
    SetupFixture f;
    FTA::FT_SETUP_OK(f.blueprint, f.indexEnv, StringList().add("intarray").add("query"), StringList(),
                     StringList().add("scalar"));
}
...
struct ExecFixture {
    ...
    void setupAttributeVectors() {
        ...
        test.getIndexEnv().getBuilder().addField(FieldType::ATTRIBUTE, CollectionType::ARRAY, DataType::INT64,
                                                 attrLongArray);
        test.getIndexEnv().getBuilder().addField(FieldType::ATTRIBUTE, CollectionType::ARRAY, DataType::INT32,
                                                 attrIntArray);
    ...
    bool evaluatesTo(feature_t expectedValue) { return test.execute(expectedValue); }
};
