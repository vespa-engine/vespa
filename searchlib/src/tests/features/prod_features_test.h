...
protected:
    ...
    void assertCloseness(feature_t exp, const std::string& attr, double distance, double maxDistance = 0,
                         double halfResponse = 0);
    ...
    void assert2DZDistance(feature_t exp, const std::string& positions, int32_t xquery, int32_t yquery,
                           uint32_t xAspect = 0, uint32_t hit_index = 0);
    ...
    void assertFieldMatch(const std::string& spec, const std::string& query, const std::string& field,
                          const search::features::fieldmatch::Params* params = nullptr, uint32_t totalTermWeight = 0,
                          feature_t totalSignificance = 0.0f);
    void assertFieldMatch(const std::string& spec, const std::string& query, const std::string& field,
    ...
    void assertFreshness(feature_t expFreshness, const std::string& attr, uint32_t age, uint32_t maxAge = 0,
                         double halfResponse = 0, bool logScale = false);
...
class ProdFeaturesTest : public ::testing::Test, public Test {
protected:
    ProdFeaturesTest();
    ~ProdFeaturesTest();
};
