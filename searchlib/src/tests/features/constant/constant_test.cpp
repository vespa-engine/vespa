...
protected:
    ...
    const Value& executeTensor(uint32_t docId = 1) { return extractTensor(docId); }
    double extractDouble(uint32_t docid) {
        Value::CREF value = test.resolveObjectFeature(docid);
        EXPECT_TRUE(value.get().type().is_double());
        return value.get().as_double();
    }
    double executeDouble(uint32_t docId = 1) { return extractDouble(docId); }
    void addTensor(const std::string& name, const TensorSpec& spec) {
        Value::UP tensor = make_tensor(spec);
        ValueType type(tensor->type());
        test.getIndexEnv().addConstantValue(name, std::move(type), std::move(tensor));
    }
    void addDouble(const std::string& name, const double value) {
        test.getIndexEnv().addConstantValue(name, ValueType::double_type(), std::make_unique<DoubleValue>(value));
    }
...
ConstantTest::ConstantTest() : ::testing::Test(), factory(), test(factory, "constant(foo)") {
...
TEST_F(ConstantTest, require_that_existing_tensor_constant_is_detected) {
    addTensor("foo", TensorSpec("tensor(x{})").add({{"x", "a"}}, 3).add({{"x", "b"}}, 5).add({{"x", "c"}}, 7));
    EXPECT_TRUE(setup());
    auto expect =
        make_tensor(TensorSpec("tensor(x{})").add({{"x", "b"}}, 5).add({{"x", "c"}}, 7).add({{"x", "a"}}, 3));
