...
struct FeatureDumpFixture : public IDumpFeatureVisitor {
    ...
    void visitDumpFeature(const std::string& name) override { actual.push_back(name); }
};
...
struct RankFixture : BlueprintFactoryFixture {
    ...
    double get_feature(const std::string& query, const FtIndex& index, const std::string& select,
        ...
        std::vector<std::string> names(
            {"elementSimilarity(foo).default", // use 'default' explicitly to verify default output name
             "elementSimilarity(foo).proximity", "elementSimilarity(foo).order",
             "elementSimilarity(foo).query_coverage", "elementSimilarity(foo).field_coverage",
...
template <typename A, typename B> bool cmp_lists_impl(const A& a, const B& b) {
...
template <typename T> void dump_list(const std::string& name, const T& list) {
...
template <typename A, typename B> bool cmp_lists(const A& a, const B& b) {
...
TEST(ElementSimilarityFeatureTest, require_that_appropriate_features_are_dumped) {
    ...
    EXPECT_TRUE(
        cmp_lists(std::vector<std::string>({"elementSimilarity(foo)", "elementSimilarity(foo).proximity",
                                            "elementSimilarity(foo).order", "elementSimilarity(foo).query_coverage",
                                            "elementSimilarity(foo).field_coverage", "elementSimilarity(foo).weight",
...
TEST(ElementSimilarityFeatureTest, require_that_proximity_acts_as_expected) {
    ...
    EXPECT_EQ(comb({prox(2), prox(1), prox(1), prox(1)}),
              f1.get_feature("a b c d e", indexFoo().element("a x b c d e"), PROXIMITY));
    EXPECT_EQ(comb({prox(3), prox(1), prox(1), prox(1)}),
              f1.get_feature("a b c d e", indexFoo().element("a x x b c d e"), PROXIMITY));
    EXPECT_EQ(comb({prox(4), prox(1), prox(1), prox(1)}),
              f1.get_feature("a b c d e", indexFoo().element("a x x x b c d e"), PROXIMITY));
    EXPECT_EQ(comb({prox(2), prox(2), prox(2), prox(2)}),
              f1.get_feature("a b c d e", indexFoo().element("a x b x c x d x e"), PROXIMITY));
    EXPECT_EQ(comb({prox(2), prox(2), prox(1), prox(3)}),
              f1.get_feature("a b c d e", indexFoo().element("a x b x c d x x e"), PROXIMITY));
}
...
TEST(ElementSimilarityFeatureTest, require_that_field_order_does_not_affect_proximity_score) {
    ...
    EXPECT_EQ(comb({prox(2), prox(1), prox(1), prox(1)}),
              f1.get_feature("a b c d e", indexFoo().element("d x c a b e"), PROXIMITY));
    EXPECT_EQ(comb({prox(3), prox(1), prox(1), prox(1)}),
              f1.get_feature("a b c d e", indexFoo().element("d x x c a b e"), PROXIMITY));
    EXPECT_EQ(comb({prox(4), prox(1), prox(1), prox(1)}),
              f1.get_feature("a b c d e", indexFoo().element("d x x x c a b e"), PROXIMITY));
    EXPECT_EQ(comb({prox(2), prox(2), prox(2), prox(2)}),
              f1.get_feature("a b c d e", indexFoo().element("d x c x a x b x e"), PROXIMITY));
    EXPECT_EQ(comb({prox(2), prox(2), prox(1), prox(3)}),
              f1.get_feature("a b c d e", indexFoo().element("d x c x a b x x e"), PROXIMITY));
}
...
TEST(ElementSimilarityFeatureTest, require_that_proximity_does_not_affect_order_score) {
    RankFixture f1;
    EXPECT_EQ(1.0, f1.get_feature("a b c d e", indexFoo().element("a b c d e"), ORDER));
    EXPECT_EQ(comb({1.0, 1.0, 1.0, 0.0}),
              f1.get_feature("a b c d e", indexFoo().element("a x b x c x e x d"), ORDER));
    EXPECT_EQ(comb({0.0, 1.0, 1.0, 0.0}),
              f1.get_feature("a b c d e", indexFoo().element("b x a x c x e x d"), ORDER));
    EXPECT_EQ(comb({0.0, 1.0, 0.0, 0.0}),
              f1.get_feature("a b c d e", indexFoo().element("b x a x e x d x c"), ORDER));
    EXPECT_EQ(comb({0.0, 0.0, 0.0, 0.0}),
              f1.get_feature("a b c d e", indexFoo().element("e x d x c x b x a"), ORDER));
}
...
TEST(ElementSimilarityFeatureTest, require_that_default_score_combines_individual_signals_appropriately) {
    RankFixture f1;
    EXPECT_EQ(comb({prox(1), prox(3), prox(2)}),
              f1.get_feature("a b c d e", indexFoo().element("a c x x b x d"), PROXIMITY));
...
TEST(ElementSimilarityFeatureTest, require_that_element_demultiplexing_works) {
    ...
    EXPECT_EQ(sum({0.25, 0.5, 0.5, 0.25, 0.5}),
              f1.get_feature("x y z t",
                             indexFoo().element("x").element("x y").element("x z").element("y").element("x z"),
                             DEFAULT, f2));
}
