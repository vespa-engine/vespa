...
struct RankFixture : BlueprintFactoryFixture {
    ...
    void test(const std::string& queryStr, const FtIndex& index, feature_t field, feature_t query, int32_t weight = 1,
              feature_t factor = 0.5, bool useStaleMatchData = false) {
        ...
        expect.addScore(names[TOTAL], field * factor + query * (1 - factor))
            .addScore(names[FIELD], field)
            .addScore(names[QUERY], query)
...
TEST(ElementCompletenessTest, require_that_element_is_selected_based_on_completeness_times_element_weight) {
    ...
    {
        SCOPED_TRACE("10");
        f.test("x y a b", indexFoo().element("x", 39).element("y", 39).element("a b", 19).element("x y a b", 10), 1.0,
               1.0, 10, 0.0);
    }
    {
        SCOPED_TRACE("21");
        f.test("x y a b", indexFoo().element("x", 39).element("y", 39).element("a b", 21).element("x y a b", 10), 1.0,
               0.5, 21, 0.0);
    }
    {
        SCOPED_TRACE("45");
        f.test("x y a b", indexFoo().element("x", 39).element("y", 45).element("a b", 21).element("x y a b", 10), 1.0,
               0.25, 45, 0.0);
    }
}
