...
template <typename T> std::unique_ptr<fef::Anything> create_param(const std::string& param) {
    Properties props;
    props.add("foo", param);
...
struct FixtureBase : ImportedAttributeFixture {
    ...
    void check_single_execution(feature_t expected, const std::string& vector, DocId doc_id,
        ...
        schema::CollectionType collection_type((attr->getCollectionType() == attribute::CollectionType::ARRAY)
                                                   ? schema::CollectionType::ARRAY
                                                   : schema::CollectionType::WEIGHTEDSET);
        feature.getIndexEnv().getBuilder().addField(FieldType::ATTRIBUTE, collection_type, imported_attr->getName());
    ...
    void check_executions(PerTypeSetupFunctor setup_func, const std::vector<BasicType>& types, feature_t expected,
                          const std::string& vector, DocId doc_id, const std::string& shared_param = "") {
        for (auto type : types) {
            setup_func(type);
    ...
    void check_all_integer_executions(feature_t expected, const std::string& vector, DocId doc_id,
                                      const std::string& shared_param = "") {
        check_executions<int32_t>([this](auto int_type) { this->setup_integer_mappings(int_type); },
                                  {{BasicType::INT32}}, expected, vector, doc_id, shared_param);
        check_executions<int64_t>([this](auto int_type) { this->setup_integer_mappings(int_type); },
                                  {{BasicType::INT64}}, expected, vector, doc_id, shared_param);
    }
};
...
struct ArrayFixture : FixtureBase {
    ...
    template <typename T> void setup_integer_mappings_helper(BasicType int_type) {
        reset_with_array_value_reference_mappings<IntegerAttribute, T>(
            int_type, {{DocId(1), dummy_gid(3), DocId(3), {{2, 3, 5}}},
    ...
    template <typename T> void setup_float_mappings_helper(BasicType float_type) {
        reset_with_array_value_reference_mappings<FloatingPointAttribute, T>(
            float_type, {{DocId(2), dummy_gid(4), DocId(4), {{2.2, 3.3, 5.5}}},
    ...
    template <typename T> static void verify(const dotproduct::ArrayParam<T>& a, const dotproduct::ArrayParam<T>& b) {
    ...
    template <typename ExpectedType>
    void check_prepare_state_output(const std::string& postfix, const std::string& input_vector,
        ...
        feature.getIndexEnv().getBuilder().addField(FieldType::ATTRIBUTE, schema::CollectionType::ARRAY,
                                                    imported_attr->getName());
    ...
    void check_all_float_executions(feature_t expected, const std::string& vector, DocId doc_id,
                                    const std::string& shared_param = "") {
        check_executions<float>([this](auto float_type) { this->setup_float_mappings(float_type); },
                                {{BasicType::FLOAT}}, expected, vector, doc_id, shared_param);
...
TEST(ImportedDotProductTest, dense_i32_and_i64_array_dot_product_can_be_evaluated_with_pre_parsed_object_parameter) {
    ArrayFixture f;
    f.check_all_integer_executions(2 * 5 + 3 * 6 + 5 * 7, "[2 3 4]", DocId(1),
                                   "[5 6 7]"); // String input is ignored in favor of stored object
}

TEST(ImportedDotProductTest,
     dense_float_and_double_array_dot_product_can_be_evaluated_with_pre_parsed_object_parameter) {
    ArrayFixture f;
    f.check_all_float_executions(2.2 * 7.7 + 3.3 * 11.11 + 5.5 * 13.13, "[2.0 3.0 4.0]", DocId(2),
                                 "[7.7 11.11 13.13]");
}
...
TEST(ImportedDotProductTest,
     sparse_float_and_double_array_dot_products_can_be_evaluated_with_pre_parsed_object_parameter) {
    ArrayFixture f;
...
struct WsetFixture : FixtureBase {
    void setup_integer_mappings(BasicType int_type) override {
        ...
        reset_with_wset_value_reference_mappings<IntegerAttribute, WeightedInt>(
            int_type, {{DocId(3), dummy_gid(7), DocId(7), doc7_values}});
    }
...
TEST(ImportedDotProductTest, string_wset_dot_products_can_be_evaluated_with_string_parameter) {
    ...
    reset_with_wset_value_reference_mappings<StringAttribute, WeightedString>(
        f, BasicType::STRING, {{DocId(3), dummy_gid(7), DocId(7), doc7_values}});
...
TEST(ImportedDotProductTest, integer_enum_dot_products_can_be_evaluated_with_string_parameter) {
    ...
    reset_with_wset_value_reference_mappings<IntegerAttribute, WeightedInt>(
        f, BasicType::INT32, {{DocId(3), dummy_gid(7), DocId(7), doc7_values}}, FastSearchConfig::ExplicitlyEnabled);
