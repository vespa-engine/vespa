...
Schema FeatureStoreTest::make_schema() const {
    Schema result;
...
FeatureStoreTest::FeatureStoreTest() : schema(make_schema()), fs(schema) {
}

void assertFeatures(const DocIdAndFeatures& exp, const DocIdAndFeatures& act) {
    // docid is not encoded as part of features
    ASSERT_EQ(exp.elements().size(), act.elements().size());
    for (size_t i = 0; i < exp.elements().size(); ++i) {
        EXPECT_EQ(exp.elements()[i].getElementId(), act.elements()[i].getElementId());
        EXPECT_EQ(exp.elements()[i].getNumOccs(), act.elements()[i].getNumOccs());
        EXPECT_EQ(exp.elements()[i].getWeight(), act.elements()[i].getWeight());
        EXPECT_EQ(exp.elements()[i].getElementLen(), act.elements()[i].getElementLen());
    }
    ASSERT_EQ(exp.word_positions().size(), act.word_positions().size());
    for (size_t i = 0; i < exp.word_positions().size(); ++i) {
        EXPECT_EQ(exp.word_positions()[i].getWordPos(), act.word_positions()[i].getWordPos());
    }
}

DocIdAndFeatures getFeatures(uint32_t numOccs, int32_t weight, uint32_t elemLen) {
    DocIdAndFeatures f;
    f.set_doc_id(0);
...
TEST_F(FeatureStoreTest, features_can_be_added_and_retrieved) {
    ...
    std::pair<EntryRef, uint64_t> r;
    {
        ...
        LOG(info, "bits(%" PRIu64 "), ref.offset(%zu), ref.bufferId(%u)", r.second,
            FeatureStore::RefType(r1).offset(), FeatureStore::RefType(r1).bufferId());
    ...
    }
    {
        ...
        r2 = r.first;
        EXPECT_TRUE(r.second > 0);
        EXPECT_TRUE(FeatureStore::RefType(r2).offset() > FeatureStore::RefType(r1).offset());
        EXPECT_EQ(0u, FeatureStore::RefType(r1).bufferId());
        LOG(info, "bits(%" PRIu64 "), ref.offset(%zu), ref.bufferId(%u)", r.second,
            FeatureStore::RefType(r2).offset(), FeatureStore::RefType(r2).bufferId());
...
TEST_F(FeatureStoreTest, add_features_triggers_change_of_buffer) {
    ...
    for (;; ++cnt) {
        ...
        if (bufferId > lastId) {
            LOG(info, "Changed to bufferId %u after %zu feature sets", bufferId, cnt);
            lastId = bufferId;
        }
        if (bufferId == 1) {
            break;
