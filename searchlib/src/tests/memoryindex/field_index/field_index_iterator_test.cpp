...
template <typename FieldIndexType> class Verifier : public SearchIteratorVerifier {
private:
...
public:
    Verifier(const Schema& schema) : _tfmd(), _tfmda(), _field_index(schema, 0) {
        _tfmda.add(&_tfmd);
...
template <typename FieldIndexType> Verifier<FieldIndexType>::~Verifier() = default;
...
template <typename FieldIndexType> struct Fixture {
    Schema                   schema;
    Verifier<FieldIndexType> verifier;
    Fixture() : schema(get_schema()), verifier(schema) {}
};
