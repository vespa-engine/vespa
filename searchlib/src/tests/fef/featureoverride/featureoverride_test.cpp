...
struct SimpleRankFixture {
    ...
    RankProgram           program;
    static std::string expr_feature(const std::string& name) { return fmt("rankingExpression(%s)", name.c_str()); }
    SimpleRankFixture()
        : factory(),
          indexEnv(),
          resolver(new BlueprintResolver(factory, indexEnv)),
          overrides(),
          match_data(),
          program(resolver) {
    ...
    void add_override(const std::string& name, const std::string& str) { overrides.add(name, str); }
    ...
    void compile(const std::string& seed) { ASSERT_TRUE(try_compile(seed)); }
