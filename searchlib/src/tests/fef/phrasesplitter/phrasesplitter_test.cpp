...
void assertTermData(const ITermData* td, uint32_t uniqueId, uint32_t numTerms, uint32_t fieldId, uint32_t tfHandle,
...
TEST(PhraseSplitterTest, test_splitter) {
    ...
    { // single phrase
        ...
        for (size_t i = 0; i < 3; ++i) {
            ...
            EXPECT_NE(td->lookupField(7)->getHandle(), terms[0].lookupField(7)->getHandle());
...
TEST(PhraseSplitterTest, test_splitter_update) {
    {
        ...
        for (size_t i = 0; i < 2; ++i) { // first phrase
            const TermFieldMatchData* tmd =
                ps.resolveTermField(ps.get_query_env().getTerm(i)->lookupField(0)->getHandle());
        ...
        { // first term
            TermFieldMatchData* tmd =
                md->resolveTermField(ps.get_query_env().getTerm(2)->lookupField(0)->getHandle());
            TermFieldMatchData::PositionsIterator itr = tmd->begin();
        ...
        for (size_t i = 0; i < 2; ++i) { // second phrase
            const TermFieldMatchData* tmd =
                ps.resolveTermField(ps.get_query_env().getTerm(i + 3)->lookupField(0)->getHandle());
            TermFieldMatchData::PositionsIterator itr = tmd->begin();
