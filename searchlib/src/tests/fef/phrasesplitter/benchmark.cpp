...
void Benchmark::run(size_t numRuns, size_t numPositions) {
    ...
    std::cout << "Start benchmark with numRuns(" << numRuns << ") and numPositions(" << numPositions << ")"
              << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "ETPD: " << std::fixed << std::setprecision(10)
              << (vespalib::count_ns(sample) / (numRuns * 1000000.0)) << " (ms)" << std::endl;
}
