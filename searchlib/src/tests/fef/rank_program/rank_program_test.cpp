...
template <typename Predicate> size_t count(const RankProgram& program, Predicate pred) {
    size_t cnt = 0;
...
struct Fixture {
    ...
    Fixture& lazy_expressions(bool value) {
        indexEnv.getProperties().add(indexproperties::eval::LazyExpressions::NAME, value ? "true" : "false");
        return *this;
    }
