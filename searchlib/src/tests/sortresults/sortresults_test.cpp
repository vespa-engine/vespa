...
bool test_sort(unsigned int caseNum, unsigned int n, unsigned int ntop) {
    bool         ok = true;
    double       minmax;
    unsigned int i;
    RankedHit*   array;
    ...
    for (i = 0; i < n; i++) {
        if (i < ntop && i > 0 && array[i].getRank() > array[i - 1].getRank()) {
            printf("ERROR: rank(%d) > rank(%d)\n", i, i - 1);
            ok = false;
            break;
        }
        if (i >= ntop && array[i].getRank() > minmax) {
            printf("ERROR: rank(%d) > rank(%d)\n", i, ntop - 1);
            ok = false;
            break;
        }
