...
template <bool fold> std::vector<uint32_t> as_utf32(const char* key) {
...
protected:
    ...
    static int normalize_ret(int ret) { return (ret == 0) ? 0 : ((ret < 0) ? -1 : 1); }

    template <bool fold_lhs, bool fold_rhs>
    int compare_folded_helper2(const std::string& lhs, const std::string& rhs) {
        ...
        EXPECT_EQ(ret,
                  (FoldedStringCompare::compareFolded<false, fold_rhs>(std::cref(folded_lhs_utf32), rhs.c_str())));
        auto folded_rhs_utf32 = as_utf32<fold_rhs>(rhs.c_str());
        EXPECT_EQ(ret,
                  (FoldedStringCompare::compareFolded<fold_lhs, false>(lhs.c_str(), std::cref(folded_rhs_utf32))));
        return ret;
    }

    template <bool fold_lhs, bool fold_rhs>
    int compare_folded_helper(const std::string& lhs, const std::string& rhs) {
        int ret = compare_folded_helper2<fold_lhs, fold_rhs>(lhs, rhs);
        EXPECT_EQ(-ret, (compare_folded_helper2<fold_rhs, fold_lhs>(rhs, lhs)));
        return ret;
    }

    IntVec compare_folded(const std::string& lhs, const std::string& rhs) {
        IntVec result;
    ...
    template <bool fold_lhs, bool fold_rhs>
    int compare_folded_prefix_helper(const std::string& lhs, const std::string& rhs, size_t prefix_len) {
        int ret = FoldedStringCompare::compareFoldedPrefix<fold_lhs, fold_rhs>(lhs.c_str(), rhs.c_str(), prefix_len);
        EXPECT_EQ(-ret, (FoldedStringCompare::compareFoldedPrefix<fold_rhs, fold_lhs>(rhs.c_str(), lhs.c_str(),
                                                                                      prefix_len)));
        return ret;
    }

    IntVec compare_folded_prefix(const std::string& lhs, const std::string& rhs, size_t prefix_len) {
        IntVec result;
...
FoldedStringCompareTest::FoldedStringCompareTest() : ::testing::Test() {
}
