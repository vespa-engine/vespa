...
TEST(SortTest, testIcu) {
    {
        const std::string src(
            "Creation of Bob2007 this is atumated string\this is atumated string;this is atumated string;this is "
            "atumated string;this is atumated string;this is atumated string;this is atumated string;this is "
            "atumated string;this is atumated string;this is atumated string;this is atumated string;this is "
            "atumated string;this is atumated string;this is atumated string;this is atumated string;this is "
            "atumated string;this is atumated string;this is atumated string;this is atumated string;this is "
            "atumated string;this is atumated string;this is atumated string;this is atumated string;this is "
            "atumated string;this is atumated string;this is atumated string;this is atumated string;this is "
            "atumated string;this is atumated string;this is atumated string;this is atumated string;this is "
            "atumated string; _ 12345567890-=,./;'[;");
        std::vector<UChar> u16Buffer(100);
        UErrorCode         status = U_ZERO_ERROR;
        int32_t            u16Wanted(0);
        u_strFromUTF8(&u16Buffer[0], u16Buffer.size(), &u16Wanted, src.c_str(), -1, &status);
        ASSERT_TRUE(U_SUCCESS(status) || (status == U_INVALID_CHAR_FOUND) ||
                    ((status == U_BUFFER_OVERFLOW_ERROR) && (u16Wanted > (int)u16Buffer.size())));
    }
}
...
template <typename T> class IntOrder {
public:
    uint64_t operator()(T v) const { return v ^ (std::numeric_limits<T>::max() + 1); }
};

template <typename T> void testSignedIntegerSort() {
...
TEST(SortTest, testStringSort) {
    ...
    search::radix_sort(LoadedStrings::ValueRadix(), LoadedStrings::ValueCompare(), search::AlwaysEof<LoadedStrings>(),
                       1, static_cast<LoadedStrings*>(nullptr), 0, &radixScratchPad[0], 0);

    array1[0] = LoadedStrings("a");
    search::radix_sort(LoadedStrings::ValueRadix(), LoadedStrings::ValueCompare(), search::AlwaysEof<LoadedStrings>(),
                       1, &array1[0], 1, &radixScratchPad[0], 0);
    ...
    search::radix_sort(LoadedStrings::ValueRadix(), LoadedStrings::ValueCompare(), search::AlwaysEof<LoadedStrings>(),
                       1, &loaded[0], N, &radixScratchPad[0], 0);
...
TEST(SortTest, sortspec_missing) {
    ...
    VESPA_EXPECT_EXCEPTION(SortSpec sortSpec("-missing(name,as,\"default", ucaFactory), std::runtime_error,
