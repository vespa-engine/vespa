        ...
        if (startMark == sbuf) {
            ...
            for (uint32_t pretend = 1; pretend < wanted + 8; ++pretend) {
                ...
                if (u8buffer[pretend + 1] != 0x99) {
                    printf("wrote 2 bytes too far: wanted space %d, pretend allocated %d, last good=%02x, bad=%02x "
                           "%02x\n",
                           wanted, pretend, u8buffer[pretend - 1], u8buffer[pretend], u8buffer[pretend + 1]);
                } else if (u8buffer[pretend] != 0x99) {
                    ++badnesses;
                    if (wanted > atleast) {
                        atleast = wanted;
                        printf(
                            "wrote 1 byte too far: wanted space %d, pretend allocated %d, last good=%02x, bad=%02x\n",
