...
std::unique_ptr<BufferWriter> MemAttrFileWriter::allocBufferWriter() {
    if (!_buf) {
        _buf = allocBuf(1);
    }
...
public:
    ...
    IAttributeFileWriter& weightWriter() override { return _weightWriter; }
...
template <typename VectorType> struct BufferTypes;

template <> struct BufferTypes<IntegerAttribute> {
    using Normal = IntegerAttribute::largeint_t;
    using Weighted = IntegerAttribute::WeightedInt;
};

template <> struct BufferTypes<FloatingPointAttribute> {
    using Normal = double;
    using Weighted = FloatingPointAttribute::WeightedFloat;
};

template <> struct BufferTypes<StringAttribute> {
...
std::string param_as_string(const testing::TestParamInfo<std::tuple<BasicType, CollectionType>>& info) {
...
private:
    ...
    template <typename VectorType> void populate(VectorType& v, unsigned seed, BasicType bt);
    ...
    template <typename V, typename T> SearchContextPtr getSearch(const V& vec, const T& term, bool prefix);
    ...
    void testReload(AttributePtr v0, AttributePtr v1, AttributePtr v2, MemAttr::SP mv0, MemAttr::SP mv1,
                    MemAttr::SP mv2, MemAttr::SP emv0, MemAttr::SP emv1, MemAttr::SP emv2, Config cfg,
                    const std::string& pref, bool fastSearch, search::DictionaryConfig dictionary_config);
...
struct EnumeratedSaveTest::CallCompare {
    template <typename VectorType> static void invoke(EnumeratedSaveTest& test, AttributePtr& a, AttributePtr& b) {
...
struct EnumeratedSaveTest::CallGetSearch {
    template <typename VectorType> static SearchContextPtr invoke(EnumeratedSaveTest& test, AttributePtr& v) {
        return test.getSearch<VectorType>(test.as<VectorType>(v));
    }
};

EnumeratedSaveTest::EnumeratedSaveTest() : ::testing::TestWithParam<std::tuple<BasicType, CollectionType>>() {
}

EnumeratedSaveTest::~EnumeratedSaveTest() = default;

void EnumeratedSaveTest::SetUpTestSuite() {
    std::filesystem::remove_all(test_dir);
    std::filesystem::create_directory(test_dir);
}

void EnumeratedSaveTest::TearDownTestSuite() {
    std::filesystem::remove_all(test_dir);
}

bool MemAttr::bufEqual(const Buffer& lhs, const Buffer& rhs) const {
    bool success = true;
    ...
    EXPECT_TRUE(vespalib::memcmp_safe(lhs->getData(), rhs->getData(), lhs->getDataLen()) == 0)
        << (success = false, "");
    return success;
}

bool MemAttr::operator==(const MemAttr& rhs) const {
    bool success = true;
...
void EnumeratedSaveTest::addDocs(const AttributePtr& v, size_t sz) {
    if (sz) {
...
template <> void EnumeratedSaveTest::populate(IntegerAttribute& v, unsigned seed, BasicType bt) {
    vespalib::Rand48             rnd;
    IntegerAttribute::largeint_t mask(std::numeric_limits<IntegerAttribute::largeint_t>::max());
    switch (bt.type()) {
    case BasicType::INT8:
        mask = 0x7f;
        break;
...
template <> void EnumeratedSaveTest::populate(FloatingPointAttribute& v, unsigned seed, BasicType bt) {
    (void)bt;
    vespalib::Rand48 rnd;
    rnd.srand48(seed);
    int weight = 1;
...
template <> void EnumeratedSaveTest::populate(StringAttribute& v, unsigned seed, BasicType bt) {
    (void)bt;
...
template <typename T> inline bool equalsHelper(const T& lhs, const T& rhs) {
    return lhs == rhs;
}
...
template <typename VectorType, typename BufferType> void EnumeratedSaveTest::compare(VectorType& a, VectorType& b) {
...
void EnumeratedSaveTest::compare(AttributePtr& a, AttributePtr& b) {
...
IntegerAttribute& EnumeratedSaveTest::asInt(AttributePtr& v) {
    return as<IntegerAttribute>(v);
}

StringAttribute& EnumeratedSaveTest::asString(AttributePtr& v) {
    return as<StringAttribute>(v);
}

FloatingPointAttribute& EnumeratedSaveTest::asFloat(AttributePtr& v) {
    return as<FloatingPointAttribute>(v);
}

void EnumeratedSaveTest::buildTermQuery(std::vector<char>& buffer, const std::string& index, const std::string& term,
                                        bool prefix) {
...
SearchContextPtr EnumeratedSaveTest::getSearch(const V& vec, const T& term, bool prefix) {
    ...
    return (static_cast<const AttributeVector&>(vec))
        .getSearch(std::string_view(query.data(), query.size()), SearchContextParams());
}

template <> SearchContextPtr EnumeratedSaveTest::getSearch<IntegerAttribute>(const IntegerAttribute& v) {
    return getSearch<IntegerAttribute>(v, "[-42;-42]", false);
}

template <> SearchContextPtr EnumeratedSaveTest::getSearch<FloatingPointAttribute>(const FloatingPointAttribute& v) {
    return getSearch<FloatingPointAttribute>(v, "[-42.0;-42.0]", false);
}

template <> SearchContextPtr EnumeratedSaveTest::getSearch<StringAttribute>(const StringAttribute& v) {
    return getSearch<StringAttribute, const std::string&>(v, "foo", false);
}

SearchContextPtr EnumeratedSaveTest::getSearch(AttributePtr& v) {
    return typify_invoke<1, TypifyVectorType, CallGetSearch>(v->getConfig().basicType(), *this, v);
}

MemAttr::SP EnumeratedSaveTest::saveMem(AttributeVector& v) {
    MemAttr::SP res(new MemAttr);
    EXPECT_TRUE(v.save(*res, v.getBaseFileName()));
    return res;
}

void EnumeratedSaveTest::saveMemDuringCompaction(AttributeVector& v) {
    MemAttr::SP res(new MemAttr);
...
void EnumeratedSaveTest::checkMem(AttributeVector& v, const MemAttr& e, const std::string& label) {
    SCOPED_TRACE(label);
...
MemAttr::SP EnumeratedSaveTest::saveBoth(AttributePtr v) {
    EXPECT_TRUE(v->save());
...
EnumeratedSaveTest::AttributePtr EnumeratedSaveTest::make(Config cfg, const std::string& pref, bool fastSearch) {
    cfg.setFastSearch(fastSearch);
    AttributePtr v = AttributeFactory::createAttribute(pref, cfg);
    return v;
}

void EnumeratedSaveTest::load(AttributePtr v, const std::string& name) {
...
EnumeratedSaveTest::AttributePtr EnumeratedSaveTest::checkLoad(Config cfg, const std::string& name, AttributePtr ev,
...
void EnumeratedSaveTest::testReload(AttributePtr v0, AttributePtr v1, AttributePtr v2, MemAttr::SP mv0,
                                    MemAttr::SP mv1, MemAttr::SP mv2, MemAttr::SP emv0, MemAttr::SP emv1,
                                    MemAttr::SP emv2, Config cfg, const std::string& pref, bool fastSearch,
                                    search::DictionaryConfig dictionary_config) {
    std::ostringstream os;
    os << "testReload fs=" << std::boolalpha << fastSearch << ", dictionary_config=" << dictionary_config;
    SCOPED_TRACE(os.str());
    bool flagAttr = cfg.collectionType() == CollectionType::ARRAY && cfg.basicType() == BasicType::INT8 && fastSearch;
    bool supportsEnumerated = (fastSearch || cfg.basicType() == BasicType::STRING) && !flagAttr;
...
void EnumeratedSaveTest::test(BasicType bt, CollectionType ct, const std::string& pref) {
    Config       cfg(bt, ct);
...
auto test_values =
    testing::Combine(testing::Values(BasicType::INT8, BasicType::INT16, BasicType::INT32, BasicType::INT64,
