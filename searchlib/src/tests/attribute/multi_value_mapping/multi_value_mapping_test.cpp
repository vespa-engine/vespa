...
template <typename ElemT> void assertArray(const std::vector<ElemT>& exp, std::span<const ElemT> values) {
    EXPECT_EQ(exp, std::vector<ElemT>(values.begin(), values.end()));
}

template <class MvMapping> class MyAttribute : public search::NotImplementedAttribute {
    ...
    void reclaim_memory(Generation oldest_used_gen) override { _mvMapping.reclaim_memory(oldest_used_gen); }
    void before_inc_generation(Generation current_gen) override { _mvMapping.assign_generation(current_gen); }

public:
    MyAttribute(MvMapping& mvMapping) : NotImplementedAttribute("test"), _mvMapping(mvMapping) {}
...
template <typename ElemT> class MappingTestBase : public ::testing::Test {
protected:
...
public:
    ...
    MappingTestBase() : _stats(), _mvMapping(), _attr(), _maxSmallArraySize() {}
    void setup(uint32_t max_array_store_type_id, bool enable_free_lists = true) {
        ...
        _mvMapping =
            std::make_unique<MvMapping>(config, ArrayStoreConfig::default_max_buffer_size, vespalib::GrowStrategy(),
                                        std::make_unique<MemoryAllocatorObserver>(_stats));
    ...
    void setup(uint32_t max_array_store_type_id, size_t min_entries, size_t max_entries,
               size_t num_entries_for_new_buffer, bool enable_free_lists = true) {
        ArrayStoreConfig config(
            max_array_store_type_id,
...
template <typename ElemT> MappingTestBase<ElemT>::~MappingTestBase() = default;
...
public:
    CompactionIntMappingTest() : MappingTestBase<int>(), _rnd(), _refMapping() { _rnd.srand48(32); }
...
TEST_F(CompactionIntMappingTest, test_that_compaction_works) {
    ...
    LOG(info, "Have %u buffers, %u docs, clearing to %u", bufferCountBefore, docIdLimit, clearLimit);
