...
const DocSource docs[] = {{0, 1},  {1, 0},   {2, 2},   {4, 3},   {8, 9},    {16, 178}, {32, 1},
                          {64, 2}, {128, 3}, {256, 4}, {512, 2}, {1024, 1}, {2048, 5}, {maxDocId, 1}};
...
void SourceSelectorTest::testSourceSelector(const DocSource* docSource, size_t sz, uint8_t defaultSource,
                                            ISourceSelector& selector, bool cap) {
    {
        auto it(selector.createIterator());
        for (size_t i = 0; i < sz; ++i) {
            EXPECT_EQ((uint32_t)capSource(docSource[i].source, defaultSource, cap),
                      (uint32_t)it->getSource(docSource[i].docId));
        }
    }
    {
        auto it(selector.createIterator());
...
void SourceSelectorTest::requireThatSelectorCanSaveAndLoad(bool compactLidSpace) {
    ...
    SourceSelector::SaveInfo::UP save_info = selector.extractSaveInfo(base_file_name);
