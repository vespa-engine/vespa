...
std::unique_ptr<ResultSet> performSearch(queryeval::SearchIterator& sb, uint32_t numDocs) {
...
public:
    ...
    void printXML() const {
        std::cout << "<total-search-time>" << vespalib::count_ms(_totalSearchTime) << "</total-search-time>"
                  << std::endl;                                                                          // ms
    ...
    double avgSearchTime() const { return vespalib::count_ms(_totalSearchTime / _numQueries); }
    double searchThroughout() const {
        return _numClients * 1000 * _numQueries / (vespalib::count_ns(_totalSearchTime) / 1000000.0);
    }
    double avgHitCount() const { return _totalHitCount / double(_numQueries); }
};
...
public:
    AttributeSearcher(const AttributePtr& attrPtr) : _attrPtr(attrPtr), _timer(), _status(), _thread() {
...
void AttributeSearcher::buildTermQuery(std::vector<char>& buffer, const std::string& index, const char* term,
                                       bool prefix) {
    ...
    uint32_t queryPacketSize = vespalib::compress::Integer::compressedPositiveLength(termIdx) +
                               ...
                               vespalib::compress::Integer::compressedPositiveLength(termLen) + indexLen + termLen;
...
public:
    AttributeFindSearcher(const AttributePtr& attrPtr, const std::vector<T>& values, uint32_t numQueries)
...
template <typename T> void AttributeFindSearcher<T>::doRun() {
    ...
    for (uint32_t i = 0; i < _status._numQueries; ++i) {
        ...
        std::unique_ptr<attribute::SearchContext> searchContext =
            _attrPtr->getSearch(std::string_view(&_query[0], _query.size()), attribute::SearchContextParams());
...
public:
    AttributeRangeSearcher(const AttributePtr& attrPtr, const RangeSpec& spec, uint32_t numQueries)
...
void AttributeRangeSearcher::doRun() {
    _timer = vespalib::Timer();
...
public:
    AttributePrefixSearcher(const AttributePtr& attrPtr, const std::vector<std::string>& values, uint32_t numQueries)
        : AttributeSearcher(attrPtr), _values(values), _query() {
        _status._numQueries = numQueries;
    }
    void doRun() override;
...
void AttributePrefixSearcher::doRun() {
    _timer = vespalib::Timer();
    for (uint32_t i = 0; i < _status._numQueries; ++i) {
