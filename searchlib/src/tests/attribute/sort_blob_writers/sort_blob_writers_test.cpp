...
template <typename T, bool asc> SortData serialized_numeric(std::optional<unsigned char> prefix, T value) {
    SortData s;
...
template <typename T, bool asc> SortData serialized_present_numeric(T value) {
    return serialized_numeric<T, asc>(default_multi_value_value_prefix, value);
}

template <bool asc> SortData serialized_integer(std::optional<unsigned char> prefix, int32_t value) {
    return serialized_numeric<int32_t, asc>(prefix, value);
}

SortData serialized_string(std::optional<unsigned char> prefix, const char* value, bool asc) {
...
SortData serialized_present_string(const char* value, bool asc) {
    return serialized_string(default_multi_value_value_prefix, value, asc);
}

template <typename T> SortData serialized_present(T value, bool asc) {
...
SortData sort_data_numeric(std::vector<T> values, MissingPolicy policy, T missing_value, bool multi_value) {
    size_t                        len = 0;
    SortData                      s;
...
template <typename T, bool asc> SortData sort_data_numeric(std::vector<T> values) {
    return sort_data_numeric<T, asc>(values, MissingPolicy::DEFAULT, T(), true);
}

template <bool asc>
SortData sort_data_integer(std::vector<int32_t> values, MissingPolicy policy, int32_t missing_value,
                           bool multi_value) {
    return sort_data_numeric<int32_t, asc>(values, policy, missing_value, multi_value);
}

template <bool asc>
SortData sort_data_string(std::vector<const char*> values, const BlobConverter* bc, MissingPolicy missing_policy,
...
template <bool asc> SortData sort_data_string(std::vector<const char*> values, const BlobConverter* bc) {
    return sort_data_string<asc>(values, bc, MissingPolicy::DEFAULT, "", true);
}

SortData sort_data_string(std::vector<const char*> values, bool asc) {
    if (asc) {
...
template <typename T> SortData sort_data(std::vector<T> values, bool asc) {
    if constexpr (std::is_same_v<T, const char*>) {
...
SetupHook::SetupHook() {
    Fast_NormalizeWordFolder::Setup(
        Fast_NormalizeWordFolder::DO_ACCENT_REMOVAL | Fast_NormalizeWordFolder::DO_SHARP_S_SUBSTITUTION |
...
template <typename Param> class SortBlobWritersTest : public ::testing::Test {
protected:
    SortBlobWritersTest();
    ~SortBlobWritersTest() override;
};

template <typename Param> SortBlobWritersTest<Param>::SortBlobWritersTest() : ::testing::Test() {
}

template <typename Param> SortBlobWritersTest<Param>::~SortBlobWritersTest() = default;
...
using SortBlobWritersTestTypes =
    testing::Types<Int8Params, Int16Params, Int32Params, Int64Params, FloatParams, DoubleParams, StringParams>;
...
template <typename Param> using SortBlobFloatingPointWritersTest = SortBlobWritersTest<Param>;
...
TEST_F(SortBlobStringWriterTest, missing_policy_default) {
    ...
    EXPECT_EQ(serialized_string(std::nullopt, "", true),
              sort_data_string<true>({}, nullptr, MissingPolicy::DEFAULT, "", false));
    EXPECT_EQ(serialized_string(std::nullopt, "aaa", true),
              sort_data_string<true>({"aaa"}, nullptr, MissingPolicy::DEFAULT, "", false));
    // Single value descending
    EXPECT_EQ(serialized_string(std::nullopt, "", false),
              sort_data_string<false>({}, nullptr, MissingPolicy::DEFAULT, "", false));
    EXPECT_EQ(serialized_string(std::nullopt, "bbb", false),
              sort_data_string<false>({"bbb"}, nullptr, MissingPolicy::DEFAULT, "", false));
    // Multi value ascending
    EXPECT_EQ(default_missing_value_sort_blob, sort_data_string<true>({}, nullptr, MissingPolicy::DEFAULT, "", true));
    EXPECT_EQ(serialized_string(0, "aaa", true),
              sort_data_string<true>({"aaa", "bbb"}, nullptr, MissingPolicy::DEFAULT, "", true));
    // Multi value descending
    EXPECT_EQ(default_missing_value_sort_blob,
              sort_data_string<false>({}, nullptr, MissingPolicy::DEFAULT, "", true));
    EXPECT_EQ(serialized_string(0, "bbb", false),
              sort_data_string<false>({"aaa", "bbb"}, nullptr, MissingPolicy::DEFAULT, "", true));
}
...
TEST_F(SortBlobStringWriterTest, missing_policy_first) {
    ...
    EXPECT_EQ(serialized_string(1, "aaa", true),
              sort_data_string<true>({"aaa"}, nullptr, MissingPolicy::FIRST, "", false));
    // Single value descending
    EXPECT_EQ(SortData{0}, sort_data_string<false>({}, nullptr, MissingPolicy::FIRST, "", false));
    EXPECT_EQ(serialized_string(1, "bbb", false),
              sort_data_string<false>({"bbb"}, nullptr, MissingPolicy::FIRST, "", false));
    // Multi value ascending
    EXPECT_EQ(SortData{0}, sort_data_string<true>({}, nullptr, MissingPolicy::FIRST, "", true));
    EXPECT_EQ(serialized_string(1, "aaa", true),
              sort_data_string<true>({"aaa", "bbb"}, nullptr, MissingPolicy::FIRST, "", true));
    // Multi value descending
    EXPECT_EQ(SortData{0}, sort_data_string<false>({}, nullptr, MissingPolicy::FIRST, "", true));
    EXPECT_EQ(serialized_string(1, "bbb", false),
              sort_data_string<false>({"aaa", "bbb"}, nullptr, MissingPolicy::FIRST, "", true));
}
...
TEST_F(SortBlobStringWriterTest, missing_policy_last) {
    ...
    EXPECT_EQ(serialized_string(0, "aaa", true),
              sort_data_string<true>({"aaa"}, nullptr, MissingPolicy::LAST, "", false));
    // Single value descending
    EXPECT_EQ(SortData{1}, sort_data_string<false>({}, nullptr, MissingPolicy::LAST, "", false));
    EXPECT_EQ(serialized_string(0, "bbb", false),
              sort_data_string<false>({"bbb"}, nullptr, MissingPolicy::LAST, "", false));
    // Multi value ascending
    EXPECT_EQ(SortData{1}, sort_data_string<true>({}, nullptr, MissingPolicy::LAST, "", true));
    EXPECT_EQ(serialized_string(0, "aaa", true),
              sort_data_string<true>({"aaa", "bbb"}, nullptr, MissingPolicy::LAST, "", true));
    // Multi value descending
    EXPECT_EQ(SortData{1}, sort_data_string<false>({}, nullptr, MissingPolicy::LAST, "", true));
    EXPECT_EQ(serialized_string(0, "bbb", false),
              sort_data_string<false>({"aaa", "bbb"}, nullptr, MissingPolicy::LAST, "", true));
}

TEST_F(SortBlobStringWriterTest, missing_policy_as) {
    // Single value ascending
    EXPECT_EQ(serialized_string(std::nullopt, "hello", true),
              sort_data_string<true>({}, nullptr, MissingPolicy::AS, "hello", false));
    EXPECT_EQ(serialized_string(std::nullopt, "aaa", true),
              sort_data_string<true>({"aaa"}, nullptr, MissingPolicy::AS, "hello", false));
    // Single value descending
    EXPECT_EQ(serialized_string(std::nullopt, "hello", false),
              sort_data_string<false>({}, nullptr, MissingPolicy::AS, "hello", false));
    EXPECT_EQ(serialized_string(std::nullopt, "bbb", false),
              sort_data_string<false>({"bbb"}, nullptr, MissingPolicy::AS, "hello", false));
    // Multi value ascending
    EXPECT_EQ(serialized_string(std::nullopt, "hello", true),
              sort_data_string<true>({}, nullptr, MissingPolicy::AS, "hello", true));
    EXPECT_EQ(serialized_string(std::nullopt, "aaa", true),
              sort_data_string<true>({"aaa", "bbb"}, nullptr, MissingPolicy::AS, "hello", true));
    // Multi value descending
    EXPECT_EQ(serialized_string(std::nullopt, "hello", false),
              sort_data_string<false>({}, nullptr, MissingPolicy::AS, "hello", true));
    EXPECT_EQ(serialized_string(std::nullopt, "bbb", false),
              sort_data_string<false>({"aaa", "bbb"}, nullptr, MissingPolicy::AS, "hello", true));
}
...
TEST_F(SortBlobIntegerWriterTest, missing_policy_default) {
    // Single value ascending
    EXPECT_EQ(serialized_integer<true>(std::nullopt, no_int),
              sort_data_integer<true>({}, MissingPolicy::DEFAULT, 0, false));
    EXPECT_EQ(serialized_integer<true>(std::nullopt, 10),
              sort_data_integer<true>({10}, MissingPolicy::DEFAULT, 0, false));
    // Single value descending
    EXPECT_EQ(serialized_integer<false>(std::nullopt, no_int),
              sort_data_integer<false>({}, MissingPolicy::DEFAULT, 0, false));
    EXPECT_EQ(serialized_integer<false>(std::nullopt, 15),
              sort_data_integer<false>({15}, MissingPolicy::DEFAULT, 0, false));
    // Multi value ascending
...
TEST_F(SortBlobIntegerWriterTest, missing_policy_as) {
    ...
    EXPECT_EQ(serialized_integer<true>(std::nullopt, 10),
              sort_data_integer<true>({10}, MissingPolicy::AS, 42, false));
    // Single value descending
    EXPECT_EQ(serialized_integer<false>(std::nullopt, 42),
              sort_data_integer<false>({}, MissingPolicy::AS, 42, false));
    EXPECT_EQ(serialized_integer<false>(std::nullopt, 15),
              sort_data_integer<false>({15}, MissingPolicy::AS, 42, false));
    // Multi value ascending
    EXPECT_EQ(serialized_integer<true>(std::nullopt, 42), sort_data_integer<true>({}, MissingPolicy::AS, 42, true));
    EXPECT_EQ(serialized_integer<true>(std::nullopt, 10),
              sort_data_integer<true>({10, 15}, MissingPolicy::AS, 42, true));
    // Multi value descending
    EXPECT_EQ(serialized_integer<false>(std::nullopt, 42), sort_data_integer<false>({}, MissingPolicy::AS, 42, true));
    EXPECT_EQ(serialized_integer<false>(std::nullopt, 15),
              sort_data_integer<false>({10, 15}, MissingPolicy::AS, 42, true));
}
