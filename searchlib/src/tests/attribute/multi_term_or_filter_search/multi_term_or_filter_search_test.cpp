...
public:
    ...
    void add_tree(size_t idx, const std::vector<uint32_t>& keys) {
        ...
        _postings.apply(_trees[idx], adds.data(), adds.data() + adds.size(), removes.data(),
...
MultiTermOrFilterSearchTest::MultiTermOrFilterSearchTest()
    : _postings(true), _gens(), _range_start(1), _range_end(10000) {
}
...
void MultiTermOrFilterSearchTest::inc_generation() {
    _postings.freeze();
...
public:
    Verifier(MultiTermOrFilterSearchTest& test, int num_trees) : _test(test) {
    ...
    std::unique_ptr<SearchIterator> create(bool) const override { return _test.make_iterator(); }
};
