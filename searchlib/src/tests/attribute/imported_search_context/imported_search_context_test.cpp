...
struct Fixture : ImportedAttributeFixture {
    ...
    std::unique_ptr<SearchIterator> create_non_strict_iterator(ImportedSearchContext& ctx,
                                                               TermFieldMatchData&    match) {
        return create_iterator(ctx, match, false);
    }

    std::unique_ptr<SearchIterator> create_strict_iterator(ImportedSearchContext& ctx, TermFieldMatchData& match) {
...
template <typename Iterator>
bool is_strict_hit_with_weight(Iterator& iter, TermFieldMatchData& match, DocId seek_lid, DocId expected_lid,
                               int32_t weight) {
    bool failed = false;
    iter.seek(seek_lid);
...
TEST(ImportedSearchContextTest,
     calc_hit_estimate_returns_document_count_of_reference_attribute_when_not_using_fast_search_target_attribute) {
    Fixture f;
...
TEST(ImportedSearchContextTest, non_strict_iterator_unpacks_target_match_data_for_single_value_hit) {
    ...
    reset_with_single_value_reference_mappings<IntegerAttribute, int32_t>(
        f, BasicType::INT32, {{DocId(1), dummy_gid(3), DocId(3), 1234}, {DocId(2), dummy_gid(4), DocId(4), 1234}});
...
struct ArrayValueFixture : Fixture {
    ArrayValueFixture() {
        ...
        reset_with_array_value_reference_mappings<IntegerAttribute, int64_t>(
            BasicType::INT64, {{DocId(1), dummy_gid(3), DocId(3), doc3_values},
...
struct WsetValueFixture : Fixture {
    WsetValueFixture() {
        ...
        reset_with_wset_value_reference_mappings<StringAttribute, WeightedString>(
            BasicType::STRING, {{DocId(2), dummy_gid(3), DocId(3), doc3_values},
...
struct SingleValueFixture : Fixture {
    SingleValueFixture() {
        reset_with_single_value_reference_mappings<IntegerAttribute, int32_t>(
            BasicType::INT32, {{DocId(3), dummy_gid(5), DocId(5), 5678},
...
struct SearchCacheFixture : Fixture {
    SearchCacheFixture(FilterConfig filter_config = FilterConfig::ExplicitlyEnabled) : Fixture(true) {
        reset_with_wset_value_reference_mappings<IntegerAttribute, WeightedInt>(
            ...
            FastSearchConfig::ExplicitlyEnabled, filter_config);
    }
...
std::shared_ptr<BitVectorSearchCache::Entry> makeSearchCacheEntry(const std::vector<uint32_t> docIds,
                                                                  uint32_t                    docIdLimit) {
    std::shared_ptr<BitVector> bitVector = BitVector::create(docIdLimit);
    for (uint32_t docId : docIds) {
        bitVector->setBit(docId);
    }
    return std::make_shared<BitVectorSearchCache::Entry>(IDocumentMetaStoreContext::IReadGuard::SP(), bitVector,
                                                         docIdLimit);
}
...
std::vector<uint32_t> get_bitvector_hits(const BitVector& bitVector) {
