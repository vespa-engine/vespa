...
TEST_F(ArrayBoolAttributeTest, estimated_save_byte_size) {
    ...
    EXPECT_EQ(6 * sizeof(vespalib::datastore::EntryRef), _attr->reserved_memory_for_flush(false));
    EXPECT_EQ(6 * sizeof(vespalib::datastore::EntryRef) + estimate, _attr->reserved_memory_for_flush(true));
}
