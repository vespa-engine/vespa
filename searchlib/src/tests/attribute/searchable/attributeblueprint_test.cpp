...
public:
    MyAttributeManager(MyAttributeManager&& rhs)
        : IAttributeManager(), _attribute_vector(std::move(rhs._attribute_vector)) {}

    MyAttributeManager(AttributeVector::SP attr) : _attribute_vector(std::move(attr)) {}

    AttributeGuard::UP getAttribute(std::string_view) const override {
        return std::make_unique<AttributeGuard>(_attribute_vector);
    }

    std::unique_ptr<attribute::AttributeReadGuard> getAttributeReadGuard(std::string_view,
...
bool do_search(const Node& node, IAttributeManager& attribute_manager, bool expect_attribute_search_context = true) {
...
bool search_for_term(const string& term, IAttributeManager& attribute_manager) {
...
AttributeVector::SP make_string_attribute(std::initializer_list<std::string> values) {
    Config cfg(BasicType::STRING, CollectionType::SINGLE);
    return AttributeBuilder(field, cfg).fill(values).get();
}

AttributeVector::SP make_string_attribute(const std::string& value) {
    return make_string_attribute({"", "", value});
}

AttributeVector::SP make_wset_string_attribute(std::initializer_list<std::initializer_list<std::string>> values) {
...
AttributeVector::SP make_int_attribute(int32_t value) {
    Config cfg(BasicType::INT32, CollectionType::SINGLE);
    return AttributeBuilder(field, cfg).fill({-1, -1, value}).get();
}

AttributeVector::SP make_fast_search_long_attribute(int32_t value) {
    Config cfg(BasicType::INT64, CollectionType::SINGLE);
    cfg.setFastSearch(true);
    return AttributeBuilder(field, cfg).fill({-1, -1, value}).get();
}

MyAttributeManager makeAttributeManager(const std::string& value) {
    return MyAttributeManager(make_string_attribute(value));
}

MyAttributeManager makeAttributeManager(int64_t value) {
...
AttributeVector::SP make_tensor_attribute(const std::string& name, const std::string& tensor_spec) {
    Config cfg(BasicType::TENSOR, CollectionType::SINGLE);
    cfg.setTensorType(ValueType::from_spec(tensor_spec));
    return AttributeFactory::createAttribute(name, cfg);
}

AttributeVector::SP make_int_attribute(const std::string& name) {
    Config cfg(BasicType::INT32, CollectionType::SINGLE);
    return AttributeFactory::createAttribute(name, cfg);
}
...
public:
    ...
    void expect_document_weight_attribute() { EXPECT_TRUE(attr->as_docid_with_weight_posting_store() != nullptr); }
...
public:
    NearestNeighborFixture(AttributeVector::SP attr_in) : BlueprintFactoryFixture(std::move(attr_in)) {}
...
void expect_nearest_neighbor_blueprint(const std::string& attribute_tensor_type_spec, const TensorSpec& query_tensor,
...
void expect_empty_blueprint(AttributeVector::SP attr, const TensorSpec& query_tensor,
                            bool insert_query_tensor = true) {
...
void expect_empty_blueprint(AttributeVector::SP attr) {
...
TEST(AttributeBlueprintTest, empty_blueprint_is_created_when_nearest_neighbor_term_is_invalid) {
    ...
    expect_empty_blueprint(make_tensor_attribute(field, "tensor(x[2])"),
                           dense_y_2); // tensor types are not compatible
