...
struct Posting {
    uint32_t lid;
    int32_t  weight;
    Posting(uint32_t lid_, int32_t weight_) noexcept : lid(lid_), weight(weight_) {}

    bool operator==(const Posting& rhs) const noexcept { return ((lid == rhs.lid) && (weight == rhs.weight)); }
...
TEST_F(PostingListMergerTest, Merge_array) {
    ...
    _merger.merge();
    EXPECT_EQ(
        (std::vector<Posting>{{2, 102}, {3, 103}, {5, 105}, {6, 106}, {8, 108}, {9, 109}, {14, 114}, {17, 117}}),
        asArray());
}
