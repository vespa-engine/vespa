...
struct StringEntry {
    uint32_t    _refCount;
    std::string _string;
    StringEntry(uint32_t refCount, const std::string& str) : _refCount(refCount), _string(str) {}
};
...
struct Reader {
    ...
    Reader(uint32_t generation, const IndexVector& indices, const ExpectedVector& expected);
    ~Reader();
};
...
void checkReaders(const StringEnumStore& ses, const std::vector<Reader>& readers) {
    const char* t = "";
...
template <typename EnumStoreTypeAndDictionaryType> class FloatEnumStoreTest : public ::testing::Test {
public:
...
using FloatEnumStoreTestTypes = ::testing::Types<BTreeFloatEnumStore, BTreeDoubleEnumStore, HybridFloatEnumStore,
                                                 HybridDoubleEnumStore, HashFloatEnumStore, HashDoubleEnumStore>;
...
void testUniques(const StringEnumStore& ses, const std::vector<std::string>& unique) {
    auto read_snapshot = ses.get_dictionary().get_read_snapshot();
    read_snapshot->fill();
    read_snapshot->sort();
    std::vector<EnumIndex> saved_indexes;
    read_snapshot->foreach_key(
        [&saved_indexes](const AtomicEntryRef& idx) { saved_indexes.push_back(idx.load_acquire()); });
    uint32_t i = 0;
...
void StringEnumStoreTest::testInsert(bool hasPostings) {
...
void dec_ref_count(NumericEnumStore& store, NumericEnumStore::Index idx) {
...
TEST(EnumStoreTest, address_space_usage_is_reported) {
    const size_t ADDRESS_LIMIT =
        4290772994; // Max allocated elements in un-allocated buffers + allocated elements in allocated buffers.
...
TEST(EnumStoreTest, provided_memory_allocator_is_used) {
    ...
    NumericEnumStore ses(false, DictionaryConfig::Type::BTREE, std::make_unique<MemoryAllocatorObserver>(stats),
                         attribute::getUndefined<NumericEnumStore::EntryType>());
...
public:
    ...
    BatchUpdaterTest() : store(false, DictionaryConfig::Type::BTREE), i3(), i5() {
        auto updater = store.make_batch_updater();
        i3 = updater.insert(3);
        i5 = updater.insert(5);
...
template <typename EnumStoreT> struct LoaderTestValues {
...
template <> void LoaderTestValues<StringEnumStore>::load_values(enumstore::EnumeratedLoaderBase& loader) {
...
template <typename EnumStoreTypeAndDictionaryType> class LoaderTest : public ::testing::Test {
public:
    using EnumStoreType = typename EnumStoreTypeAndDictionaryType::EnumStoreType;
    using EntryType = typename EnumStoreType::EntryType;
    EnumStoreType store;
    ...
    LoaderTest() : store(true, EnumStoreTypeAndDictionaryType::type) {}

    void load_values(enumstore::EnumeratedLoaderBase& loader) const { Values::load_values(loader); }
...
using LoaderTestTypes =
    ::testing::Types<BTreeNumericEnumStore, BTreeFloatEnumStore, BTreeStringEnumStore, HybridNumericEnumStore,
                     HybridFloatEnumStore, HybridStringEnumStore, HashNumericEnumStore, HashFloatEnumStore,
                     HashStringEnumStore, BTreeCasedStringEnumStore, HybridCasedStringEnumStore,
                     HashCasedStringEnumStore>;
...
template <typename EnumStoreTypeAndDictionaryType> class EnumStoreDictionaryTest : public ::testing::Test {
public:
    using EnumStoreType = typename EnumStoreTypeAndDictionaryType::EnumStoreType;
    using EntryType = typename EnumStoreType::EntryType;
    EnumStoreType store;

    EnumStoreDictionaryTest() : store(true, EnumStoreTypeAndDictionaryType::type) {}
...
void EnumStoreDictionaryTest<EnumStoreTypeAndDictionaryType>::update_posting_idx(EnumIndex enum_idx,
                                                                                 EntryRef  old_posting_idx,
                                                                                 EntryRef  new_posting_idx) {
    auto&    dict = store.get_dictionary();
    EntryRef old_posting_idx_check;
    dict.update_posting_list(enum_idx, store.get_comparator(),
                             [&old_posting_idx_check, new_posting_idx](EntryRef posting_idx) noexcept -> EntryRef {
                                 old_posting_idx_check = posting_idx;
                                 return new_posting_idx;
                             });
...
EntryRef make_fake_pidx(uint32_t i) {
    return RefT(i + 200, select_buffer(i));
}
EntryRef make_fake_adjusted_pidx(uint32_t i) {
    return RefT(i + 500, select_buffer(i));
}
EntryRef adjust_fake_pidx(EntryRef ref) {
    RefT iref(ref);
    return RefT(iref.offset() + 300, iref.bufferId());
}

} // namespace

template <typename EnumStoreTypeAndDictionaryType>
void EnumStoreDictionaryTest<EnumStoreTypeAndDictionaryType>::populate_sample_data(uint32_t cnt) {
    auto& dict = store.get_dictionary();
    for (uint32_t i = 0; i < cnt; ++i) {
        ...
        dict.update_posting_list(enum_idx, store.get_comparator(),
                                 [posting_idx](EntryRef) noexcept -> EntryRef { return posting_idx; });
    }
}

template <typename EnumStoreTypeAndDictionaryType>
std::vector<EntryRef> EnumStoreDictionaryTest<EnumStoreTypeAndDictionaryType>::get_sample_values(uint32_t cnt) {
    ...
    for (uint32_t i = 0; i < cnt; ++i) {
        ...
        dict.update_posting_list(enum_idx, compare, [&posting_idx](EntryRef ref) noexcept {
            posting_idx = ref;
            return ref;
        });
...
template <typename EnumStoreTypeAndDictionaryType>
void EnumStoreDictionaryTest<EnumStoreTypeAndDictionaryType>::test_normalize_posting_lists(bool use_filter,
                                                                                           bool one_filter) {
    ...
    if (use_filter) {
        ...
        auto adjust_refs = [](std::vector<EntryRef>& refs) noexcept {
            for (auto& ref : refs) {
                ref = adjust_fake_pidx(ref);
            }
        };
...
template <typename EnumStoreTypeAndDictionaryType>
void EnumStoreDictionaryTest<EnumStoreTypeAndDictionaryType>::test_foreach_posting_list(bool one_filter) {
    auto filter = make_entry_ref_filter(one_filter);
    populate_sample_data(large_population);
    auto&                 dict = store.get_dictionary();
    std::vector<EntryRef> exp_refs;
    auto                  save_exp_refs = [&exp_refs](std::vector<EntryRef>& refs) {
        exp_refs.insert(exp_refs.end(), refs.begin(), refs.end());
    };
    EXPECT_FALSE(dict.normalize_posting_lists(save_exp_refs, filter));
    std::vector<EntryRef> act_refs;
    auto                  save_act_refs = [&act_refs](const std::vector<EntryRef>& refs) {
        act_refs.insert(act_refs.end(), refs.begin(), refs.end());
    };
...
template <typename EnumStoreTypeAndDictionaryType>
EnumIndex EnumStoreDictionaryTest<EnumStoreTypeAndDictionaryType>::check_default_value_ref() const noexcept {
...
using EnumStoreDictionaryTestTypes =
    ::testing::Types<BTreeNumericEnumStore, HybridNumericEnumStore, HashNumericEnumStore>;
...
TYPED_TEST(EnumStoreDictionaryTest, compact_worst_works) {
    ...
    read_snapshot->sort();
    read_snapshot->foreach_key(
        [&values, &mystore](const AtomicEntryRef& idx) { values.push_back(mystore.get_value(idx.load_acquire())); });
