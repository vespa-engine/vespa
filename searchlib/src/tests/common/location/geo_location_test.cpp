...
TEST(GeoLocationParserTest, bounding_boxes_can_be_parsed) {
    for (const auto& loc : {parse("[2,10,20,30,40]"), parse("{b:{x:[10,30],y:[20,40]}}")}) {
...
TEST(GeoLocationParserTest, circles_can_be_parsed) {
    for (const auto& loc : {parse("(2,10,20,5,0,0,0)"), parse("{p:{x:10,y:20},r:5}")}) {
...
TEST(GeoLocationParserTest, circles_can_have_aspect_ratio) {
    for (const auto& loc : {parse("(2,10,20,5,0,0,0,2147483648)"), parse("{p:{x:10,y:20},r:5,a:2147483648}")}) {
        EXPECT_EQ(true, loc.has_point);
        EXPECT_EQ(true, loc.bounding_box.active());
...
TEST(GeoLocationParserTest, bounding_box_can_be_specified_after_circle) {
    for (const auto& loc :
         {parse("(2,10,20,5,0,0,0)[2,10,20,30,40]"), parse("{p:{x:10,y:20},r:5,b:{x:[10,30],y:[20,40]}}")})
    {
        EXPECT_EQ(true, loc.has_point);
        EXPECT_EQ(true, loc.bounding_box.active());
        EXPECT_EQ(0u, loc.x_aspect.multiplier);
...
TEST(GeoLocationParserTest, circles_can_be_specified_after_bounding_box) {
    for (const auto& loc :
         {parse("[2,10,20,30,40](2,10,20,5,0,0,0)"), parse("{b:{x:[10,30],y:[20,40]},p:{x:10,y:20},r:5}")})
    {
        EXPECT_EQ(true, loc.has_point);
        EXPECT_EQ(true, loc.bounding_box.active());
        EXPECT_EQ(0u, loc.x_aspect.multiplier);
...
TEST(GeoLocationParserTest, santa_search_gives_non_wrapped_bounding_box) {
    for (const auto& loc : {parse("(2,122163600,89998536,290112,4,2000,0,109704)"),
...
TEST(GeoLocationParserTest, near_boundary_search_gives_non_wrapped_bounding_box) {
    for (const auto& loc1 :
         {parse("(2,2000000000,2000000000,3000000000,0,1,0)"), parse("{p:{x:2000000000,y:2000000000},r:3000000000}")})
    {
    ...
    for (const auto& loc2 : {parse("(2,-2000000000,-2000000000,3000000000,0,1,0)"),
