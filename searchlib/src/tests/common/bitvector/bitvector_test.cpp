...
std::string toString(const BitVector& bv) {
    std::stringstream ss;
    ss << "[";
    bool     first = true;
...
std::string toString(BitVectorIterator& b) {
    std::stringstream ss;
    ss << "[";
    bool first = true;
    b.initFullRange();
...
void scan(uint32_t count, uint32_t offset, uint32_t size, vespalib::Rand48& rnd) {
...
void scanWithOffset(uint32_t offset) {
    vespalib::Rand48 rnd;
...
std::string param_as_string(const testing::TestParamInfo<uint32_t>& info) {
...
bool assertBV(const std::string& exp, const BitVector& act) {
...
void fill(BitVector& bv, const std::vector<uint32_t>& bits, uint32_t offset, bool fill = true) {
...
std::string fill(const std::vector<uint32_t>& bits, uint32_t offset) {
...
void testAndNot(uint32_t offset) {
    uint32_t      end = offset + 128;
    BitVector::UP v1(BitVector::create(offset, end));
    BitVector::UP v2(BitVector::create(offset, end));
    BitVector::UP v3(BitVector::create(offset, end));
...
std::unique_ptr<nbostream> write_test_bv_to_nbostream(const BitVector& bv, uint32_t guard_bits, size_t alignment) {
...
void write_test_bv_to_file(const BitVector& bv, const std::string& file_name) {
...
BitVectorTest::BitVectorTest() : ::testing::Test() {
}

BitVectorTest::~BitVectorTest() = default;

void BitVectorTest::SetUpTestSuite() {
...
void BitVectorTest::TearDownTestSuite() {
    _file_bv.reset();
    std::filesystem::remove_all(testdata);
}

std::unique_ptr<const BitVector> BitVectorTest::make_test_bv() {
...
void setEveryNthBit(uint32_t n, BitVector& bv, uint32_t offset, uint32_t end) {
...
BitVector::UP createEveryNthBitSet(uint32_t n, uint32_t offset, uint32_t sz) {
...
void verifyThatLongerWithShorterWorksAsZeroPadded(uint32_t offset, uint32_t sz1, uint32_t sz2, Func func) {
...
template <typename Func> void verifyNonOverlappingWorksAsZeroPadded(bool clear, Func func) {
...
BitVector::UP orSerial(const std::vector<BitVector::UP>& bvs) {
...
BitVector::UP orParallel(vespalib::ThreadBundle& thread_bundle, const std::vector<BitVector::UP>& bvs) {
    BitVector::UP           master = BitVector::create(*bvs[0]);
...
class BitVectorGuardBitsCompatibilityTest : public BitVectorTest, public testing::WithParamInterface<uint32_t> {
protected:
...
BitVectorGuardBitsCompatibilityTest::BitVectorGuardBitsCompatibilityTest()
    : BitVectorTest(), testing::WithParamInterface<uint32_t>() {
}

BitVectorGuardBitsCompatibilityTest::~BitVectorGuardBitsCompatibilityTest() = default;

INSTANTIATE_TEST_SUITE_P(BitVectorGuardBitsCompatibilityTest, BitVectorGuardBitsCompatibilityTest,
                         testing::Values(1, 2), param_as_string);
