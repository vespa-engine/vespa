...
struct DocIdIterator : public PopulateInterface::Iterator {
    ...
    int32_t getNext() override { return (_index < _docs.size()) ? _docs[_index++] : -1; }
...
public:
    explicit Populater(const KeyDocIdsMap& m) : _map(m), _empty() {}
    ~Populater() override;
    Iterator::UP lookup(uint64_t key) const override {
        return _map.contains(key) ? std::make_unique<DocIdIterator>(_map[key])
...
KeyDocIdsMap create(uint32_t numDocs, uint32_t numKeys, uint32_t seed) {
    KeyDocIdsMap m;
    std::srand(seed);
