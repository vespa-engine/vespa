...
void save_predicate_index(PredicateIndex& index, DataBuffer& buffer) {
    index.commit();
...
public:
    GuardedSaver(GenerationGuard guard, std::unique_ptr<ISaver> saver)
        : _guard(std::move(guard)), _saver(std::move(saver)) {}
    ~GuardedSaver();
...
GuardedSaver make_guarded_saver(PredicateIndex& index) {
    index.commit();
...
bool equal_buffers(const DataBuffer& lhs, const DataBuffer& rhs) {
    return (lhs.getDataLen() == rhs.getDataLen()) && (memcmp(lhs.getData(), rhs.getData(), lhs.getDataLen()) == 0);
}
...
PredicateIndex::BTreeIterator lookupPosting(const PredicateIndex& index, uint64_t hash) {
...
struct DocIdLimitFinder : SimpleIndexDeserializeObserver<> {
    ...
    DocIdLimitFinder(uint32_t& doc_id_limit) : _doc_id_limit(doc_id_limit) { doc_id_limit = 0u; }
...
TEST(PredicateIndexTest, require_that_PredicateIndex_can_be_serialized_and_deserialized) {
    ...
    PredicateIndex                index2(generation_holder, dummy_provider, simple_index_config, buffer, finder,
                                         PredicateAttribute::PREDICATE_ATTRIBUTE_VERSION);
...
TEST(PredicateIndexTest,
     require_that_predicate_index_saver_protected_by_a_generation_guard_observes_a_snapshot_of_the_predicate_index) {
    /*
