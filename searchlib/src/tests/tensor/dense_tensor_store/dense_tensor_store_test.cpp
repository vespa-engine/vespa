...
Value::UP makeTensor(const TensorSpec& spec) {
    return SimpleValue::from_spec(spec);
}

struct Fixture {
    DenseTensorStore store;
    explicit Fixture(const std::string& tensorType) : store(ValueType::from_spec(tensorType), {}) {}
...
TEST(DenseTensorStoreTest, require_that_we_can_store_1d_bound_tensor) {
    ...
    f.assertSetAndGetTensor(TensorSpec("tensor(x[3])").add({{"x", 0}}, 2).add({{"x", 1}}, 3).add({{"x", 2}}, 5));
}

TEST(DenseTensorStoreTest, require_that_correct_empty_tensor_is_returned_for_1d_bound_tensor) {
    Fixture f("tensor(x[3])");
    f.assertEmptyTensor(TensorSpec("tensor(x[3])").add({{"x", 0}}, 0).add({{"x", 1}}, 0).add({{"x", 2}}, 0));
}
