...
public:
    MockBigTensor(std::unique_ptr<Value> real_tensor) : _real_tensor(std::move(real_tensor)) {}
...
Value::UP make_tensor(double value) {
...
TEST_F(DirectTensorStoreTest, get_vectors) {
    auto tensor_spec = TensorSpec(tensor_type_spec)
                           .add({{"x", "a"}}, 4.5)
                           .add({{"x", "b"}}, 5.5)
                           .add({{"x", "c"}}, 6.5)
                           .add({{"x", "d"}}, 7.5);
