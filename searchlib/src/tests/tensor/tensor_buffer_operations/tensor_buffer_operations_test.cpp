...
struct TestParam {
    ...
    TestParam(std::string name, std::vector<size_t> array_sizes, TensorSpec tensor_spec)
        : _name(std::move(name)), _array_sizes(std::move(array_sizes)), _tensor_spec(std::move(tensor_spec)) {}
...
std::vector<size_t> TensorBufferOperationsTest::get_array_sizes(uint32_t max_subspaces) {
...
std::vector<char> TensorBufferOperationsTest::store_tensor(const Value& tensor) {
...
std::vector<char> TensorBufferOperationsTest::store_tensor(const TensorSpec& spec) {
    auto tensor = SimpleValue::from_spec(spec);
    return store_tensor(*tensor);
}

std::unique_ptr<Value> TensorBufferOperationsTest::load_tensor(std::span<const char> buf) {
    return _ops.make_fast_view(buf, _tensor_type);
}

vespalib::nbostream TensorBufferOperationsTest::encode_stored_tensor(std::span<const char> buf) {
    vespalib::nbostream out;
    _ops.encode_stored_tensor(buf, _tensor_type, out);
    return out;
}

TensorSpec TensorBufferOperationsTest::load_tensor_spec(std::span<const char> buf) {
    auto loaded = load_tensor(buf);
    return TensorSpec::from_value(*loaded);
}

void TensorBufferOperationsTest::assert_store_load(const TensorSpec& tensor_spec) {
...
void TensorBufferOperationsTest::assert_store_copy_load(const TensorSpec& tensor_spec) {
    auto buf = store_tensor(tensor_spec);
    auto buf2 = buf;
...
void TensorBufferOperationsTest::assert_store_encode_decode(const TensorSpec& tensor_spec) {
    auto buf = store_tensor(tensor_spec);
...
INSTANTIATE_TEST_SUITE_P(
    TensorBufferOperationsMultiTest, TensorBufferOperationsTest,
    testing::Values(
        TestParam("1d", {8, 16, 32, 40, 64}, TensorSpec(tensor_type_spec).add({{"x", "a"}}, 4.5)),
        TestParam("1dmulti", {8, 16, 32, 40, 64},
                  TensorSpec(tensor_type_spec).add({{"x", "a"}}, 4.5).add({{"x", "c"}}, 4.25)),
        ...
        TestParam("2dmixed", {8, 24, 48, 64, 96},
                  TensorSpec(tensor_type_2d_mixed_spec)
                      .add({{"x", "a"}, {"y", 0}}, 4.5)
                      .add({{"x", "a"}, {"y", 1}}, 4.25))),
