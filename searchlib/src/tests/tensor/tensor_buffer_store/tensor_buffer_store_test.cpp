...
TensorBufferStoreTest::TensorBufferStoreTest()
    : testing::Test(), _tensor_type(ValueType::from_spec(tensor_type_spec)), _store(_tensor_type, {}, 4) {
}

TensorBufferStoreTest::~TensorBufferStoreTest() = default;

EntryRef TensorBufferStoreTest::store_tensor(const Value& tensor) {
    EXPECT_EQ(_tensor_type, tensor.type());
    return _store.store_tensor(tensor);
}

EntryRef TensorBufferStoreTest::store_tensor(const TensorSpec& spec) {
    auto tensor = value_from_spec(spec, FastValueBuilderFactory::get());
    return store_tensor(*tensor);
}

std::unique_ptr<Value> TensorBufferStoreTest::load_tensor(EntryRef ref) {
    return _store.get_tensor(ref);
}

vespalib::nbostream TensorBufferStoreTest::encode_stored_tensor(EntryRef ref) {
    vespalib::nbostream out;
    _store.encode_stored_tensor(ref, out);
    return out;
}

TensorSpec TensorBufferStoreTest::load_tensor_spec(EntryRef ref) {
    auto loaded = load_tensor(ref);
    return TensorSpec::from_value(*loaded);
}

void TensorBufferStoreTest::assert_store_load(const TensorSpec& tensor_spec) {
...
void TensorBufferStoreTest::assert_store_load_many(const TensorSpec& tensor_spec) {
...
void TensorBufferStoreTest::assert_store_move_on_compact_load(const TensorSpec& tensor_spec) {
    auto ref = store_tensor(tensor_spec);
...
void TensorBufferStoreTest::assert_store_encode_store_encoded_load(const TensorSpec& tensor_spec) {
    auto ref = store_tensor(tensor_spec);
...
std::vector<TensorSpec> tensor_specs = {
    TensorSpec(tensor_type_spec), TensorSpec(tensor_type_spec).add({{"x", "a"}}, 4.5),
