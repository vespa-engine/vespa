...
public:
    BufferGenerator() : _rnd(), _minStrLen(0), _maxStrLen(0) {}
    BufferGenerator(uint32_t minStrLen, uint32_t maxStrLen) : _rnd(), _minStrLen(minStrLen), _maxStrLen(maxStrLen) {}
    void setSeed(long seed) { _rnd.srand48(seed); }
    nbostream getRandomBuffer();
};

nbostream BufferGenerator::getRandomBuffer() {
...
public:
    ...
    EntryGenerator(const EntryGenerator& rhs)
        : _rnd(),
          ...
          _lastGeneratedBuffer(rhs._lastGeneratedBuffer) {}
    ...
    void setBuffers(const std::vector<nbostream>& buffers) { _buffers = &buffers; }
};
...
Packet::Entry EntryGenerator::getRandomEntry(SerialNum num) {
...
public:
    static std::string toStr(const Packet::Entry& e) {
        std::stringstream ss;
        ss << "Entry(serial(" << e.serial() << "), type(" << e.type() << "), bufferSize(" << e.data().size()
           << "), buffer(";
...
void FeederThread::commitPacket() {
    const vespalib::nbostream& stream = _packet.getHandle();
    if (!_session->commit(ConstBufferRef(stream.data(), stream.size()))) {
        throw std::runtime_error(
            vespalib::make_string("FeederThread: Failed commiting %s", PacketPrinter::toStr(_packet).c_str()));
    } else {
...
void FeederThread::doRun() {
    _session = _client.open(_domain);
    if (!_session) {
        throw std::runtime_error(
            vespalib::make_string("FeederThread: Could not open session to %s", _tlsSpec.c_str()));
    }
        ...
        if (_feedRate != 0) {
            ...
            for (uint32_t i = 0; i < _feedRate; ++i) {
                ...
                if (!addEntry(entry)) {
                    commitPacket();
                    if (!addEntry(entry)) {
                        throw std::runtime_error(vespalib::make_string("FeederThread: Could not add %s",
            ...
            if (!addEntry(entry)) {
                ...
                if (!addEntry(entry)) {
                    throw std::runtime_error(
                        vespalib::make_string("FeederThread: Could not add %s", EntryPrinter::toStr(entry).c_str()));
                }
            }
        }
    }
...
void VisitorAgent::start(SerialNum from, SerialNum to) {
    ...
    if (!_visitor) {
        throw std::runtime_error(
            vespalib::make_string("VisitorAgent[%u]: Could not open visitor to %s", _id, _tlsSpec.c_str()));
    }
    setState(RUNNING);
    if (!_visitor->visit(_from, _to)) {
        throw std::runtime_error(
            vespalib::make_string("VisitorAgent[%u]: Could not visit from %s with range <%" PRIu64 ", %" PRIu64 "]",
...
RPC::Result VisitorAgent::receive(const Packet& packet) {
    ...
    while (handle.size() > 0) {
        ...
        if (_validate) {
            if (!EntryComparator::cmp(entry, expected)) {
                throw std::runtime_error(vespalib::make_string("VisitorAgent[%u]: Got %s, expected %s", _id,
                                                               EntryPrinter::toStr(entry).c_str(),
    ...
    if (_next > _to + 1) {
        throw std::runtime_error(vespalib::make_string("VisitorAgent[%u]: Visited range <%" PRIu64 ", %" PRIu64
                                                       "], expected "
                                                       "range <%" PRIu64 "u, %" PRIu64 "]",
                                                       _id, _from, _next - 1, _from, _to));
    }
...
public:
    ControllerThread(FNET_Transport& transport, const std::string& tlsSpec, const std::string& domain,
                     const EntryGenerator& generator, uint32_t numVisitors, vespalib::duration visitorInterval,
...
ControllerThread::ControllerThread(FNET_Transport& transport, const std::string& tlsSpec, const std::string& domain,
    ...
    : _tlsSpec(tlsSpec),
      _domain(domain),
      _client(transport, tlsSpec.c_str()),
      _session(),
      _generator(generator),
      _visitors(),
      _rndVisitors(),
      _visitorInterval(visitorInterval),
...
void ControllerThread::getStatus() {
    if (!_session->status(_begin, _end, _count)) {
        throw std::runtime_error(
            vespalib::make_string("ControllerThread: Could not get status from %s", _tlsSpec.c_str()));
    }
}

void ControllerThread::makeRandomVisitorVector() {
...
void ControllerThread::doRun() {
    _session = _client.open(_domain);
    if (!_session) {
        throw std::runtime_error(
            vespalib::make_string("ControllerThread: Could not open session to %s", _tlsSpec.c_str()));
    }
    ...
    while (!_done) {
        ...
        if (_pruneTimer.elapsed() > _pruneInterval) {
            ...
            LOG(info, "ControllerThread: status: begin(%" PRIu64 "), end(%" PRIu64 "), count(%zu)", _begin, _end,
                _count);
            LOG(info, "ControllerThread: prune [%" PRIu64 ", %" PRIu64 ">", _begin, safePrune);
            if (!_session->erase(safePrune)) {
                throw std::runtime_error(
                    vespalib::make_string("ControllerThread: Could not erase up to %" PRIu64, safePrune));
            }
            _pruneTimer = vespalib::Timer();
        }
...
void TransLogStress::printConfig() {
...
void TransLogStress::usage() {
    ...
    std::cout << "                      [-g numPreGeneratedBuffers] [-i minStrLen] [-a maxStrLen] [-b baseSeed]"
              << std::endl;
    std::cout << "                      [-d domainPartSize] [-p packetSize]" << std::endl;
}

int TransLogStress::main(int argc, char** argv) {
    ...
    TransLogServer         tls(transport, "server", 17897, ".", fileHeaderContext,
                               DomainConfig().setPartSizeLimit(_cfg.domainPartSize));
    ...
    ControllerThread controller(transport, tlsSpec, domain, generator, _cfg.numVisitors, _cfg.visitorInterval,
                                _cfg.pruneInterval);
    controller.start();
    ...
    std::cout << "  <rate>"
              << 1000 * (feeder.getRange().to() - feeder.getRange().from()) /
                     vespalib::count_ms(sleepTime + _cfg.stressTime)
              << "</rate>" << std::endl;
    std::cout << "</feeder>" << std::endl;
