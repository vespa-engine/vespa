...
struct ProtoRpcAdapterTest : ::testing::Test {
    ...
    ProtoRpcAdapterTest() : server(), adapter(search, docsum, monitor, server.supervisor()) {
        server.supervisor().Listen(0);
    }
    FRT_Target* connect() { return server.supervisor().GetTarget(server.supervisor().GetListenPort()); }
