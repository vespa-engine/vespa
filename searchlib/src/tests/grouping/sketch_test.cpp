...
template <typename NormalSketch> void checkBucketValue(NormalSketch& sketch, size_t bucket, uint32_t value) {
...
template <int BucketBits, typename HashT> void checkCountPrefixZeros() {
    SCOPED_TRACE(make_string("BucketBits: %d, HashBits: %d", BucketBits, int(sizeof(HashT) * 8)));
...
TEST(SketchTest, require_that_normal_sketch_can_be_serialized_and_deserialized) {
    NormalSketch<> sketch;
    for (size_t i = 0; i < sketch.BUCKET_COUNT; ++i) {
        sketch.aggregate(i | (1 << ((i % sketch.bucketBits) + sketch.bucketBits)));
    }
    nbostream     stream;
