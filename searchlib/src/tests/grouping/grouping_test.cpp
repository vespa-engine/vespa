...
template <typename A, typename T> class AttrBuilder {
private:
    A*                  _attr;
    AttributeVector::SP _attrSP;

public:
    AttrBuilder(const AttrBuilder& rhs) : _attr(new A(rhs._attr->getName())), _attrSP(_attr) {
    ...
    explicit AttrBuilder(const std::string& name) : _attr(new A(name)), _attrSP(_attr) {}
    ...
    AttributeVector::SP sp() const { return _attrSP; }
};
...
public:
    ...
    ResultBuilder& add(unsigned int docid, HitRank rank = 0) {
        ...
        hit._rankValue = rank;
        _hits.push_back(hit);
        for (uint32_t pos = (_hits.size() - 1); pos > 0 && (_hits[pos].getRank() > _hits[pos - 1].getRank()); --pos) {
...
public:
    ...
    void add(const AttributeVector::SP& attr) { _attrMan.add(attr); }
    void setup(Grouping& g) { g.configureStaticStuff(ConfigureStaticParams(_attrCtx.get(), nullptr)); }
...
std::unique_ptr<AggregationResult> prepareAggr(const AggregationResult& aggr, ExpressionNode::UP expr) {
    std::unique_ptr<AggregationResult> clone(aggr.clone());
    clone->setExpression(std::move(expr));
    return clone;
}

std::unique_ptr<ExpressionNode> prepareAggr(const AggregationResult& aggr, ExpressionNode::UP expr,
                                            const ResultNode& r) {
    auto prepared = prepareAggr(aggr, std::move(expr));
    prepared->resultForUnitTest(r);
    return prepared;
}

void testAggregationSimpleSum(AggregationContext& ctx, const AggregationResult& aggr, const ResultNode& ir,
                              const ResultNode& fr, const ResultNode& sr, const std::string& label) {
    SCOPED_TRACE(label);
    ExpressionNode::CP clone(aggr);
    Grouping           request;
    request.setRoot(Group()
                        .addResult(prepareAggr(aggr, MU<AttributeNode>("int")))
 ...
 **/
bool testMerge(const Grouping& a, const Grouping& b, const Group& expect) {
 ...
 * group tree matches the expected value.
 **/
bool testPrune(const Grouping& a, const Grouping& b, const Group& expect) {
    Grouping tmp = a; // create local copy
    tmp.prune(b);
    bool ok = true;
    EXPECT_EQ(tmp.getRoot().asString(), expect.asString()) << (ok = false, "");
 ...
 **/
bool testPartialMerge(const Grouping& a, const Grouping& b, const Group& expect) {
    Grouping tmp = a; // create local copy
    tmp.mergePartial(b);
    bool ok = true;
    EXPECT_EQ(tmp.getRoot().asString(), expect.asString()) << (ok = false, "");
 ...
 * group tree matches the expected value.
 **/
bool testMerge(const Grouping& a, const Grouping& b, const Grouping& c, const Group& expect) {
    Grouping tmp = a;  // create local copy
...
void testAggregationSimple(AggregationContext& ctx, const AggregationResult& aggr, const ResultNode& ir,
...
GroupingLevel createGL(ExpressionNode::UP expr, ExpressionNode::UP resultExpr) {
    GroupingLevel l;
...
GroupingLevel createGL(ExpressionNode::UP expr) {
    GroupingLevel l;
    l.setExpression(std::move(expr));
    return l;
}
...
GroupingLevel createGL(size_t maxGroups, ExpressionNode::UP expr, ExpressionNode::UP result) {
    GroupingLevel l;
    l.setMaxGroups(maxGroups);
    l.setExpression(std::move(expr));
...
TEST(GroupingTest, testAggregationSimple) {
    ...
    testAggregationSimpleSum(ctx, SumAggregationResult(), Int64ResultNode(25), FloatResultNode(25),
                             StringResultNode("25"), "sum");
    testAggregationSimpleSum(ctx, MinAggregationResult(), Int64ResultNode(3), FloatResultNode(3),
...
/**
 * Test that without a key expression the first aggregator keeps the value of the highest
 * ranked hit (key = negated rank, smallest key first), and that ties keep the hit that was
 * aggregated first.
 **/
TEST(GroupingTest, first_aggregation_result_keeps_the_first_ranked_hit) {
    ...
    EXPECT_EQ(-5.0, first.key().getFloat());
    ...
    EXPECT_EQ(-6.0, first.key().getFloat());
}

/**
 * Test that with a key expression the first aggregator keeps the value of the hit with the
 * smallest key, regardless of ranks and of the order the hits arrive in.
 **/
TEST(GroupingTest, first_aggregation_result_selects_by_key_expression) {
    AggregationContext ctx;
    ctx.result().add(0).add(1).add(2);
    ...
    FirstAggregationResult first;
    first.set_key_expression(MU<AttributeNode>("key"));
    first.setExpression(MU<AttributeNode>("value"));

    Grouping request;
    request.setRoot(Group().addResult(first));
    ctx.setup(request);
    request.aggregate(ctx.result().hits(), ctx.result().size());

    const auto& res = static_cast<const FirstAggregationResult&>(request.getRoot().getAggregationResult(0));
    EXPECT_TRUE(res.has_value());
    EXPECT_EQ(20, res.first().getInteger()); // docid 1 has the smallest key
 ...
 **/
TEST(GroupingTest, first_aggregation_result_merges_the_first_hit_by_key) {
    FirstAggregationResult a(Int64ResultNode(10), FloatResultNode(-5.0));
    a.setExpression(MU<ConstantNode>(MU<Int64ResultNode>(10)));
    FirstAggregationResult b(Int64ResultNode(20), FloatResultNode(-7.0));
...
TEST(GroupingTest, testAggregationLevels) {
    ...
    // Hmm, do not need to prepare more than the levels needed.    .resultForUnitTest(Int64ResultNode(0)));

    Group done0;
    done0
        .addResult(
            SumAggregationResult().setExpression(MU<AttributeNode>("attr0")).resultForUnitTest(Int64ResultNode(20)))
        .addChild(Group()
                      .setId(Int64ResultNode(11))
                      .addResult(SumAggregationResult()
                                     .setExpression(MU<AttributeNode>("attr2"))
                                     .resultForUnitTest(Int64ResultNode(0))));
    ...
    done1
        ...
        .addChild(Group()
                      ...
                      .addResult(SumAggregationResult()
                                     .setExpression(MU<AttributeNode>("attr2"))
                                     .resultForUnitTest(Int64ResultNode(24)))
                      .addChild(Group()
    ...
    done2
        ...
        .addChild(Group()
                      ...
                      .addChild(Group()
                                    ...
                                    .addResult(SumAggregationResult()
                                                   .setExpression(MU<AttributeNode>("attr3"))
                                                   .resultForUnitTest(Int64ResultNode(26)))
                                    .addChild(Group()
        ...
        .addChild(Group()
                      ...
                      .addChild(Group()
                                    ...
                                    .addChild(Group()
                                                  ...
                                                  .addResult(SumAggregationResult()
                                                                 .setExpression(MU<AttributeNode>("attr1"))
                                                                 .resultForUnitTest(Int64ResultNode(22))))));
...
TEST(GroupingTest, Verify_that_groups_are_tagged_with_the_appropriate_rank_value) {
    AggregationContext ctx;
    ctx.add(IntAttrBuilder("attr").add(1).add(1).add(1).add(2).add(2).add(2).add(3).add(3).add(3).sp());
    ctx.result().add(0, 5).add(1, 10).add(2, 15).add(3, 10).add(4, 15).add(5, 5).add(6, 15).add(7, 5).add(8, 10);
...
TEST(GroupingTest, testAggregationGroupCapping) {
    AggregationContext ctx;
    ctx.add(IntAttrBuilder("attr").add(1).add(2).add(3).add(4).add(5).add(6).add(7).add(8).add(9).sp());
    ctx.result().add(0, 1).add(1, 2).add(2, 3).add(3, 4).add(4, 5).add(5, 6).add(6, 7).add(7, 8).add(8, 9);
    ...
    {
        Grouping request;
        request.setFirstLevel(0).setLastLevel(1).addLevel(
            std::move(GroupingLevel()
                          .setMaxGroups(3)
        ...
        expect
            .addChild(Group()
                          ...
                          .addAggregationResult(
                              createNumAggr<SumAggregationResult>(MU<Int64ResultNode>(7), MU<AttributeNode>("attr")))
                          .addOrderBy(MU<AggregationRefNode>(0), false))
            .addChild(Group()
                          .setId(Int64ResultNode(8))
                          .setRank(RawRank(8))
                          .addAggregationResult(
                              createNumAggr<SumAggregationResult>(MU<Int64ResultNode>(8), MU<AttributeNode>("attr")))
                          .addOrderBy(MU<AggregationRefNode>(0), false))
            .addChild(Group()
                          .setId(Int64ResultNode(9))
                          .setRank(RawRank(9))
                          .addAggregationResult(
                              createNumAggr<SumAggregationResult>(MU<Int64ResultNode>(9), MU<AttributeNode>("attr")))
                          .addOrderBy(MU<AggregationRefNode>(0), false));
    ...
    }
    {
        ...
        Group expect =
            Group()
                .addChild(Group()
                              .setId(Int64ResultNode(1))
                              .setRank(RawRank(1))
                              .addAggregationResult(createNumAggr<SumAggregationResult>(MU<Int64ResultNode>(1),
                                                                                        MU<AttributeNode>("attr")))
                              .addOrderBy(MU<AggregationRefNode>(0), true))
                .addChild(Group()
                              .setId(Int64ResultNode(2))
                              .setRank(RawRank(2))
                              .addAggregationResult(createNumAggr<SumAggregationResult>(MU<Int64ResultNode>(2),
                                                                                        MU<AttributeNode>("attr")))
                              .addOrderBy(MU<AggregationRefNode>(0), true))
                .addChild(Group()
                              .setId(Int64ResultNode(3))
                              .setRank(RawRank(3))
                              .addAggregationResult(createNumAggr<SumAggregationResult>(MU<Int64ResultNode>(3),
                                                                                        MU<AttributeNode>("attr")))
                              .addOrderBy(MU<AggregationRefNode>(0), true));
    ...
    }
    {
        ...
        expect
            .addChild(Group()
                          ...
                          .addAggregationResult(
                              createNumAggr<SumAggregationResult>(MU<Int64ResultNode>(7), MU<AttributeNode>("attr")))
                          .addOrderBy(AddFunctionNode()
                                          .appendArg(MU<AggregationRefNode>(0))
                                          .appendArg(MU<ConstantNode>(MU<Int64ResultNode>(3)))
...
TEST(GroupingTest, testMergeSimpleSum) {
    Grouping a = Grouping().setRoot(Group()
                                        .setId(NullResultNode())
                                        .addResult(SumAggregationResult()
                                                       .setExpression(MU<AttributeNode>("foo"))
                                                       .resultForUnitTest(Int64ResultNode(20))));

    Grouping b = Grouping().setRoot(Group()
                                        .setId(NullResultNode())
                                        .addResult(SumAggregationResult()
                                                       .setExpression(MU<AttributeNode>("foo"))
                                                       .resultForUnitTest(Int64ResultNode(30))));

    Group expect = Group()
                       .setId(NullResultNode())
                       .addResult(SumAggregationResult()
                                      .setExpression(MU<AttributeNode>("foo"))
                                      .resultForUnitTest(Int64ResultNode(50)));
...
TEST(GroupingTest, Verify_that_frozen_levels_are_not_touched_during_merge) {
    ...
    Group a =
        Group()
            .setId(NullResultNode())
            .addResult(
                SumAggregationResult().setExpression(MU<AttributeNode>("s0")).resultForUnitTest(Int64ResultNode(5)))
            .addChild(Group()
                          .setId(Int64ResultNode(10))
                          .addResult(SumAggregationResult()
                                         .setExpression(MU<AttributeNode>("s1"))
                                         .resultForUnitTest(Int64ResultNode(10)))
                          .addChild(Group()
                                        .setId(Int64ResultNode(20))
                                        .addResult(SumAggregationResult()
                                                       .setExpression(MU<AttributeNode>("s2"))
                                                       .resultForUnitTest(Int64ResultNode(15)))
                                        .addChild(Group()
    ...
    Group expect_all =
        Group()
            .setId(NullResultNode())
            .addResult(
                SumAggregationResult().setExpression(MU<AttributeNode>("s0")).resultForUnitTest(Int64ResultNode(10)))
            .addChild(Group()
                          .setId(Int64ResultNode(10))
                          .addResult(SumAggregationResult()
                                         .setExpression(MU<AttributeNode>("s1"))
                          ...
                          .addChild(Group()
                                        ...
                                        .addChild(Group()
                                                      ...
                                                      .addResult(SumAggregationResult()
                                                                     .setExpression(MU<AttributeNode>("s3"))
                                                                     .resultForUnitTest(Int64ResultNode(40))))));
    ...
    EXPECT_TRUE(testMerge(request.unchain().setFirstLevel(0).setLastLevel(3).setRoot(a),
                          request.unchain().setFirstLevel(0).setLastLevel(3).setRoot(b), expect_all));
    EXPECT_TRUE(testMerge(request.unchain().setFirstLevel(1).setLastLevel(3).setRoot(a),
                          request.unchain().setFirstLevel(1).setLastLevel(3).setRoot(b), expect_0));
    EXPECT_TRUE(testMerge(request.unchain().setFirstLevel(2).setLastLevel(5).setRoot(a),
                          request.unchain().setFirstLevel(2).setLastLevel(5).setRoot(b), expect_1));
    EXPECT_TRUE(testMerge(request.unchain().setFirstLevel(3).setLastLevel(5).setRoot(a),
                          request.unchain().setFirstLevel(3).setLastLevel(5).setRoot(b), expect_2));
    EXPECT_TRUE(testMerge(request.unchain().setFirstLevel(4).setLastLevel(4).setRoot(a),
                          request.unchain().setFirstLevel(4).setLastLevel(4).setRoot(b), expect_3));
}
...
TEST(GroupingTest, testMergeTrees) {
    ...
    Group a =
        Group()
            .setId(NullResultNode())
            .addResult(
                SumAggregationResult().setExpression(MU<AttributeNode>("s0")).resultForUnitTest(Int64ResultNode(100)))
            ...
            .addChild(Group()
                          ...
                          .addResult(SumAggregationResult()
                                         .setExpression(MU<AttributeNode>("s1"))
                                         .resultForUnitTest(Int64ResultNode(100)))
                          .addChild(Group().setId(Int64ResultNode(4)).setRank(RawRank(10)))
                          .addChild(Group()
                                        .setId(Int64ResultNode(5))
                                        .setRank(RawRank(500))
    ...
    Group expect =
        Group()
            .setId(NullResultNode())
            .addResult(
                SumAggregationResult().setExpression(MU<AttributeNode>("s0")).resultForUnitTest(Int64ResultNode(200)))
            .addChild(Group()
                          .setId(Int64ResultNode(5))
                          .setRank(RawRank(200))
                          .addResult(SumAggregationResult()
                                         .setExpression(MU<AttributeNode>("s1"))
                                         .resultForUnitTest(Int64ResultNode(200)))
                          .addChild(Group()
                                        .setId(Int64ResultNode(5))
                                        .setRank(RawRank(500))
                                        .addResult(SumAggregationResult()
...
TEST(GroupingTest, testPruneComplex) {
    { // First level
        Group baseTree = Group()
                             ...
                             .addChild(Group()
                                           .setId(StringResultNode("baz0"))
                                           .addChild(Group()
                                                         .setId(StringResultNode("baz00"))
    ...
    { // Second level
        ...
        Group prune =
            Group()
                .addChild(Group().setId(StringResultNode("bar0")).addChild(Group().setId(StringResultNode("bar00"))))
                .addChild(Group().setId(StringResultNode("foo0")).addChild(Group().setId(StringResultNode("foo01"))));
    ...
    { // Third level
            ...
            Group()
                .addChild(Group()
                              ...
                              .addChild(Group()
                                            ...
                                            .addChild(Group().setId(StringResultNode("bar002")))))
                .addChild(Group().setId(StringResultNode("foo0")).addChild(Group().setId(StringResultNode("foo01"))));
        Group prune = Group().addChild(Group()
                                           .setId(StringResultNode("bar0"))
                                           .addChild(Group()
                                                         .setId(StringResultNode("bar00"))
                                                         .addChild(Group().setId(StringResultNode("bar001")))
        ...
        Group    expect = Group().addChild(Group()
                                               .setId(StringResultNode("bar0"))
                                               .addChild(Group()
                                                             .setId(StringResultNode("bar00"))
                                                             .addChild(Group().setId(StringResultNode("bar001")))
...
TEST(GroupingTest, testPartialMerging) {
    ...
    Group cached =
        Group()
            .addResult(
                SumAggregationResult().setExpression(MU<AttributeNode>("s0")).resultForUnitTest(Int64ResultNode(110)))
            .addChild(Group()
                          .setId(Int64ResultNode(5))
                          .addResult(SumAggregationResult()
                                         .setExpression(MU<AttributeNode>("s1"))
    ...
    { // Merge lastlevel 0
        Grouping request = baseRequest.unchain().setFirstLevel(0).setLastLevel(0);
        Group    incoming = Group().addResult(
            SumAggregationResult().setExpression(MU<AttributeNode>("s0")).resultForUnitTest(Int64ResultNode(0)));

        Group expected = Group()
                             .addResult(SumAggregationResult()
                                            .setExpression(MU<AttributeNode>("s0"))
                                            .resultForUnitTest(Int64ResultNode(110)))
                             .addChild(Group()
                                           .setId(Int64ResultNode(5))
                                           .addResult(SumAggregationResult()
                                                          .setExpression(MU<AttributeNode>("s1"))
                             ...
                             .addChild(Group()
                                           ...
                                           .addResult(SumAggregationResult()
                                                          .setExpression(MU<AttributeNode>("s1"))
                                                          .resultForUnitTest(Int64ResultNode(0))));
        EXPECT_TRUE(testPartialMerge(request.unchain().setRoot(incoming),
                                     request.unchain().setLastLevel(3).setRoot(cached), expected));
    }
    {
        ...
        Group expected = Group()
                             ...
                             .addChild(Group()
                                           ...
                                           .addResult(SumAggregationResult()
                                                          ...
                                                          .resultForUnitTest(Int64ResultNode(0))));
        EXPECT_TRUE(testPartialMerge(request.unchain().setRoot(incoming),
                                     request.unchain().setFirstLevel(0).setLastLevel(3).setRoot(cached), expected));
    }
}
...
TEST(GroupingTest,
     Test_that_simple_counting_works_as_long_as_we_use_an_expression_that_we_init_calculate_and_ignore) {
    AggregationContext ctx;
    ctx.result().add(0).add(1).add(2);
...
TEST(GroupingTest, testFS4HitCollection) {
    { // aggregation
        ...
        Grouping request = Grouping().setRoot(Group().addResult(
            HitsAggregationResult().setMaxHits(3).setExpression(MU<ConstantNode>(MU<Int64ResultNode>(0)))));

        Group expect = Group().addResult(HitsAggregationResult()
                                             .setMaxHits(3)
    ...
    { // merging
        ...
        EXPECT_TRUE(testMerge(request.unchain().setRoot(a), request.unchain().setRoot(b),
                              request.unchain().setRoot(c), expect));
        EXPECT_TRUE(testMerge(request.unchain().setRoot(b), request.unchain().setRoot(c),
                              request.unchain().setRoot(a), expect));
        EXPECT_TRUE(testMerge(request.unchain().setRoot(c), request.unchain().setRoot(a),
                              request.unchain().setRoot(b), expect));
    }
    { // count hits (for external object selection)
        HitsAggregationResult dummyHits = HitsAggregationResult()
                                              ...
                                              .sort();
        Grouping g =
            Grouping().setRoot(Group()
                                   .addResult(dummyHits)
                                   .addChild(Group().addResult(dummyHits).addChild(Group().addResult(dummyHits)))
                                   .addChild(Group().addResult(dummyHits).addChild(
                                       Group().addResult(dummyHits).addChild(Group().addResult(dummyHits)))));
...
TEST(GroupingTest, test_that_non_attributes_are_converted_to_document_field_nodes) {
    ...
    EXPECT_TRUE(
        nonAttrRequest.getRoot().getAggregationResult(0).getExpression()->inherits(DocumentFieldNode::classId));
}
