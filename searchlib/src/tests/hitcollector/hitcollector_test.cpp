...
struct BasicScorer {
    ...
    feature_t score(uint32_t docid) const { return (docid + _scoreDelta); }
};
...
template <typename Scorer> size_t do_reRank(const Scorer& scorer, HitCollector& hc, size_t count) {
    Ranges ranges;
...
struct Fixture {
    ...
    Fixture() : hc(20, 10), expBv(BitVector::create(20)), scorer(200) {}
    virtual ~Fixture();
    ...
    size_t reRank(size_t count) { return do_reRank(scorer, hc, count); }
...
struct AscendingScoreFixture : Fixture {
    ...
    HitRank calculateScore(uint32_t i) override { return i + 100; }
};
...
struct DescendingScoreFixture : Fixture {
    ...
    HitRank calculateScore(uint32_t i) override { return 100 - i; }
};
...
void testScaling(const std::vector<feature_t>& initScores, ScoreMap finalScores,
...
struct MergeResultSetFixture {
    ...
    HitCollector   hc;
    MergeResultSetFixture() : numDocs(100), maxHitsSize(80), maxHeapSize(30), hc(numDocs * 32, maxHitsSize) {}
};
...
void addExpectedHitForMergeTest(const MergeResultSetFixture& f, std::vector<RankedHit>& expRh, uint32_t docId) {
    expRh.emplace_back();
...
struct RankDropFixture {
    ...
    RankDropFixture(uint32_t docid_limit, uint32_t max_hits_size)
        : _docid_limit(docid_limit), _hc(docid_limit, max_hits_size) {}
    ~RankDropFixture();
...
TEST(HitCollectorTest, require_that_second_phase_rank_drop_limit_is_enforced) {
    ...
    f.setup();
    f.check_result(9.0, {{5, 11}, {10, 12}, {11, 14}, {12, 13}, {16, 10}}, {}, {14, 15});
}
...
TEST(HitCollectorTest, require_that_second_phase_rank_drop_limit_is_enforced_when_docid_vector_is_used) {
    ...
    f.setup();
    f.check_result(13.0, {{11, 14}}, {}, {5, 10, 12, 14, 15, 16});
}
...
TEST(HitCollectorTest, require_that_bitvector_is_not_dropped_without_second_phase_rank_drop_limit) {
    ...
    f.setup();
    f.check_result(std::nullopt, {{5, 11}, {10, 12}, {11, 14}, {12, 13}}, f.make_bv({5, 10, 11, 12, 14, 15, 16}), {});
}
...
TEST(HitCollectorTest, require_that_bitvector_is_dropped_with_second_phase_rank_drop_limit) {
    ...
    RankDropFixture f(20, 4);
    f.setup();
    f.check_result(9.0, {{5, 11}, {10, 12}, {11, 14}, {12, 13}}, {}, {14, 15, 16});
}
