...
template <typename T> class FileReader : public FileReaderBase {
public:
    ...
    void readNHostOrder(T* buf, size_t n) { read(buf, sizeof(T) * n); }
};

template <typename T> class SequentialReadModifyWriteInterface {
public:
    using Type = T;
