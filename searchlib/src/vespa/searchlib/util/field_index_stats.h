...
public:
    FieldIndexStats() noexcept : _memory_usage(), _size_on_disk(0), _io_stats() {}
    ...
    FieldIndexStats& io_stats(const FieldIndexIoStats& stats) {
        _io_stats = stats;
        return *this;
    }
    ...
    bool operator==(const FieldIndexStats& rhs) const noexcept {
        return _memory_usage == rhs._memory_usage && _size_on_disk == rhs._size_on_disk && _io_stats == rhs._io_stats;
    }
};
