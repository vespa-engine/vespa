...
bool FileSizeCalculator::extractFileSize(const vespalib::GenericHeader& header, size_t headerLen,
    ...
    if (!byteAligned(fileBitSize)) {
        LOG(error, "Bad header file size tag for %s, fileBitSize=%" PRIu64 " which is not a multiple of 8",
            fileName.c_str(), fileBitSize);
        return false;
    }
    if (fileByteSize < headerLen) {
        LOG(error, "Bad header file size tag for %s, fileBitSize=%" PRIu64 " but header is %zu bits",
            fileName.c_str(), fileBitSize, headerLen * 8);
        return false;
    }
    if (fileByteSize > fileSize) {
        LOG(error, "Bad header file size tag for %s, fileBitSize=%" PRIu64 " but whole file size is %" PRIu64 " bits",
