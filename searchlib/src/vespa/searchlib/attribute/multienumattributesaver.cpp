...
public:
    DatWriter(IAttributeSaveTarget& saveTarget, const EnumAttributeSaver::Enumerator& enumerator,
...
MultiValueEnumAttributeSaver<MultiValueT>::MultiValueEnumAttributeSaver(GenerationGuard&&                 guard,
...
template <typename MultiValueT> bool MultiValueEnumAttributeSaver<MultiValueT>::compaction_interferred() const {
    return _compaction_count != _enum_store.get_compaction_count();
}

template <typename MultiValueT> MultiValueEnumAttributeSaver<MultiValueT>::~MultiValueEnumAttributeSaver() = default;

template <typename MultiValueT>
bool MultiValueEnumAttributeSaver<MultiValueT>::onSave(IAttributeSaveTarget& saveTarget) {
    ...
    if (compaction_broke_save) {
        LOG(warning, "Aborted save of attribute vector to '%s' due to compaction of unique values",
            get_file_name().c_str());
    }
