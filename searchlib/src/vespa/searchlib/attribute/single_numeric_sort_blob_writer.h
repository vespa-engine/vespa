 ...
 */
template <typename AttrType, bool ascending> class SingleNumericSortBlobWriter : public ISortBlobWriter {
private:
 ...
 */
template <typename AttrType, bool ascending> class SingleNumericMissingSortBlobWriter : public ISortBlobWriter {
private:
    using T = typename AttrType::BaseType;
    const AttrType&                     _attr;
...
public:
    SingleNumericMissingSortBlobWriter(const AttrType& attr, search::common::sortspec::MissingPolicy policy,
                                       T missing_value);
    long write(uint32_t docid, void* buf, long available) override;
};
...
make_single_numeric_sort_blob_writer(const AttrType& attr, bool ascending, common::sortspec::MissingPolicy policy,
