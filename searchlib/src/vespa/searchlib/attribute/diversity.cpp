...
template <typename T> struct FetchNumberFast {
    const T* const attr;
...
template <typename Fetcher> class DiversityFilterT final : public DiversityFilter {
private:
    size_t  _total_count;
    Fetcher _diversity;
...
public:
    DiversityFilterT(Fetcher diversity, size_t max_per_group, size_t cutoff_max_groups, bool cutoff_strict,
                     size_t max_total)
        : DiversityFilter(max_total),
          _total_count(0),
          _diversity(diversity),
          _max_per_group(max_per_group),
          _cutoff_max_groups(cutoff_max_groups),
          _cutoff_strict(cutoff_strict),
...
template <typename Fetcher> bool DiversityFilterT<Fetcher>::accepted(uint32_t docId) {
...
std::unique_ptr<DiversityFilter> DiversityFilter::create(const IAttributeVector& diversity_attr, size_t wanted_hits,
                                                         size_t max_per_group, size_t cutoff_max_groups,
                                                         bool cutoff_strict) {
    if (diversity_attr.hasEnum()) { // must handle enum first
        FetchEnumFast fastEnum(diversity_attr);
        if (fastEnum.valid()) {
            return make_unique<DiversityFilterT<FetchEnumFast>>(fastEnum, max_per_group, cutoff_max_groups,
                                                                cutoff_strict, wanted_hits);
        } else {
            return make_unique<DiversityFilterT<FetchEnum>>(FetchEnum(diversity_attr), max_per_group,
                                                            cutoff_max_groups, cutoff_strict, wanted_hits);
        }
        ...
        if (fastInt32.valid()) {
            return make_unique<DiversityFilterT<FetchInt32Fast>>(fastInt32, max_per_group, cutoff_max_groups,
                                                                 cutoff_strict, wanted_hits);
        } else if (fastInt64.valid()) {
            return make_unique<DiversityFilterT<FetchInt64Fast>>(fastInt64, max_per_group, cutoff_max_groups,
                                                                 cutoff_strict, wanted_hits);
        } else {
            return make_unique<DiversityFilterT<FetchInteger>>(FetchInteger(diversity_attr), max_per_group,
                                                               cutoff_max_groups, cutoff_strict, wanted_hits);
        }
        ...
        if (fastFloat.valid()) {
            return make_unique<DiversityFilterT<FetchFloatFast>>(fastFloat, max_per_group, cutoff_max_groups,
                                                                 cutoff_strict, wanted_hits);
        } else if (fastDouble.valid()) {
            return make_unique<DiversityFilterT<FetchDoubleFast>>(fastDouble, max_per_group, cutoff_max_groups,
                                                                  cutoff_strict, wanted_hits);
        } else {
            return make_unique<DiversityFilterT<FetchFloat>>(FetchFloat(diversity_attr), max_per_group,
                                                             cutoff_max_groups, cutoff_strict, wanted_hits);
        }
    }
