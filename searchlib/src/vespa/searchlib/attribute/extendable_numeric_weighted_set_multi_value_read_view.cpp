...
ExtendableNumericWeightedSetMultiValueReadView<
    MultiValueType, BaseType>::ExtendableNumericWeightedSetMultiValueReadView(const std::vector<BaseType>& data,
                                                                              const std::vector<uint32_t>& idx,
                                                                              const std::vector<int32_t>&  weights)
    : attribute::IMultiValueReadView<MultiValueType>(), _data(data), _idx(idx), _weights(weights), _copy() {
}

template <class MultiValueType, typename BaseType>
ExtendableNumericWeightedSetMultiValueReadView<MultiValueType,
                                               BaseType>::~ExtendableNumericWeightedSetMultiValueReadView() = default;
