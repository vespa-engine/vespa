...
template <typename B>
EnumAttribute<B>::EnumAttribute(const std::string& baseFileName, const AttributeVector::Config& cfg)
    : B(baseFileName, cfg),
...
template <typename B> void EnumAttribute<B>::load_enum_store(LoadedVector& loaded) {
...
template <typename B> uint64_t EnumAttribute<B>::getUniqueValueCount() const {
    return _enumStore.get_num_uniques();
}

template <typename B> void EnumAttribute<B>::insertNewUniqueValues(EnumStoreBatchUpdater& updater) {
...
template <typename B> vespalib::MemoryUsage EnumAttribute<B>::getEnumStoreValuesMemoryUsage() const {
    return _enumStore.get_dynamic_values_memory_usage();
}

template <typename B> void EnumAttribute<B>::populate_address_space_usage(AddressSpaceUsage& usage) const {
