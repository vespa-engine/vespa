...
ImportedAttributeVectorReadGuard::ImportedAttributeVectorReadGuard(
    std::shared_ptr<MetaStoreReadGuard> targetMetaStoreReadGuard, const ImportedAttributeVector& imported_attribute,
...
std::span<const char> ImportedAttributeVectorReadGuard::get_raw(DocId doc) const {
...
std::unique_ptr<ISearchContext>
ImportedAttributeVectorReadGuard::createSearchContext(std::unique_ptr<QueryTermSimple> term,
...
ImportedAttributeVectorReadGuard::make_read_view_helper(MultiValueTag<MultiValueType> tag,
...
const IArrayReadView<int8_t>* ImportedAttributeVectorReadGuard::make_read_view(ArrayTag<int8_t> tag,
                                                                               vespalib::Stash& stash) const {
    return make_read_view_helper(tag, stash);
}

const IArrayReadView<int16_t>* ImportedAttributeVectorReadGuard::make_read_view(ArrayTag<int16_t> tag,
                                                                                vespalib::Stash&  stash) const {
    return make_read_view_helper(tag, stash);
}

const IArrayReadView<int32_t>* ImportedAttributeVectorReadGuard::make_read_view(ArrayTag<int32_t> tag,
                                                                                vespalib::Stash&  stash) const {
    return make_read_view_helper(tag, stash);
}

const IArrayReadView<int64_t>* ImportedAttributeVectorReadGuard::make_read_view(ArrayTag<int64_t> tag,
                                                                                vespalib::Stash&  stash) const {
    return make_read_view_helper(tag, stash);
}

const IArrayReadView<float>* ImportedAttributeVectorReadGuard::make_read_view(ArrayTag<float>  tag,
                                                                              vespalib::Stash& stash) const {
    return make_read_view_helper(tag, stash);
}

const IArrayReadView<double>* ImportedAttributeVectorReadGuard::make_read_view(ArrayTag<double> tag,
                                                                               vespalib::Stash& stash) const {
    return make_read_view_helper(tag, stash);
}

const IArrayReadView<const char*>* ImportedAttributeVectorReadGuard::make_read_view(ArrayTag<const char*> tag,
                                                                                    vespalib::Stash& stash) const {
    return make_read_view_helper(tag, stash);
}

const IWeightedSetReadView<int8_t>* ImportedAttributeVectorReadGuard::make_read_view(WeightedSetTag<int8_t> tag,
                                                                                     vespalib::Stash& stash) const {
    return make_read_view_helper(tag, stash);
}

const IWeightedSetReadView<int16_t>* ImportedAttributeVectorReadGuard::make_read_view(WeightedSetTag<int16_t> tag,
                                                                                      vespalib::Stash& stash) const {
    return make_read_view_helper(tag, stash);
}

const IWeightedSetReadView<int32_t>* ImportedAttributeVectorReadGuard::make_read_view(WeightedSetTag<int32_t> tag,
                                                                                      vespalib::Stash& stash) const {
    return make_read_view_helper(tag, stash);
}

const IWeightedSetReadView<int64_t>* ImportedAttributeVectorReadGuard::make_read_view(WeightedSetTag<int64_t> tag,
                                                                                      vespalib::Stash& stash) const {
    return make_read_view_helper(tag, stash);
}

const IWeightedSetReadView<float>* ImportedAttributeVectorReadGuard::make_read_view(WeightedSetTag<float> tag,
                                                                                    vespalib::Stash& stash) const {
    return make_read_view_helper(tag, stash);
}

const IWeightedSetReadView<double>* ImportedAttributeVectorReadGuard::make_read_view(WeightedSetTag<double> tag,
                                                                                     vespalib::Stash& stash) const {
    return make_read_view_helper(tag, stash);
}
...
const IArrayEnumReadView* ImportedAttributeVectorReadGuard::make_read_view(ArrayEnumTag     tag,
                                                                           vespalib::Stash& stash) const {
    return make_read_view_helper(tag, stash);
}

const IWeightedSetEnumReadView* ImportedAttributeVectorReadGuard::make_read_view(WeightedSetEnumTag tag,
                                                                                 vespalib::Stash&   stash) const {
    return make_read_view_helper(tag, stash);
}

const IArrayBoolReadView* ImportedAttributeVectorReadGuard::make_read_view(ArrayBoolTag     tag,
                                                                           vespalib::Stash& stash) const {
    auto target_mv_attr = _target_attribute.as_multi_value_attribute();
    if (target_mv_attr == nullptr) {
        return nullptr;
...
ImportedAttributeVectorReadGuard::make_sort_blob_writer(bool ascending, const common::BlobConverter* converter,
    ...
    return std::make_unique<SortBlobWriter>(
        *this, _target_attribute.make_sort_blob_writer(ascending, converter, policy, missing_value));
}
