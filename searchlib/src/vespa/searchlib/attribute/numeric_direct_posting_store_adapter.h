...
class NumericDirectPostingStoreAdapter
    : public DirectPostingStoreAdapter<ParentType, PostingStoreType, EnumStoreType> {
public:
    using LookupKey = typename ParentType::LookupKey;
    using LookupResult = typename ParentType::LookupResult;

    NumericDirectPostingStoreAdapter(const PostingStoreType& posting_store, const EnumStoreType& enum_store,
                                     bool attr_is_filter);
