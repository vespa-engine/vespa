...
MultiEnumSearchContext<T, BaseSC, M>::MultiEnumSearchContext(typename BaseSC::MatcherType&& matcher,
                                                             const AttributeVector&         toBeSearched,
                                                             MultiValueMappingReadView<M>   mv_mapping_read_view,
                                                             const EnumStoreT<T>&           enum_store)
    : BaseSC(toBeSearched, std::move(matcher)), _mv_mapping_read_view(mv_mapping_read_view), _enum_store(enum_store) {
}
...
MultiEnumSearchContext<T, BaseSC, M>::createFilterIterator(fef::TermFieldMatchData* matchData, bool strict) {
    ...
    if (this->getIsFilter()) {
        return strict ? std::make_unique<FilterAttributeIteratorStrict<MultiEnumSearchContext>>(*this, matchData)
                      : std::make_unique<FilterAttributeIteratorT<MultiEnumSearchContext>>(*this, matchData);
    }
    return strict ? std::make_unique<AttributeIteratorStrict<MultiEnumSearchContext>>(*this, matchData)
                  : std::make_unique<AttributeIteratorT<MultiEnumSearchContext>>(*this, matchData);
}

template <typename T, typename BaseSC, typename M>
uint32_t MultiEnumSearchContext<T, BaseSC, M>::get_committed_docid_limit() const noexcept {
