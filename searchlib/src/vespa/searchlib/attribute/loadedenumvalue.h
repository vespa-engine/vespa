...
public:
    class EnumRadix {
    public:
        uint64_t operator()(const LoadedEnumAttribute& v) {
    ...
    LoadedEnumAttribute() : _enum(0), _docId(0), _weight(1) {}

    LoadedEnumAttribute(uint32_t e, uint32_t docId, int32_t weight) : _enum(e), _docId(docId), _weight(weight) {}
...
public:
    SaveLoadedEnum(LoadedEnumAttributeVector& loaded) : _loaded(loaded) {}
...
public:
    static void save(uint32_t e, uint32_t docId, int32_t weight) {
        (void)e;
        (void)docId;
        (void)weight;
    }
...
public:
    SaveEnumHist(enumstore::EnumVector& enumHist) : _hist(enumHist) {}
