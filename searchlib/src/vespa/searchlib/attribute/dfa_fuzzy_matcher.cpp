...
std::vector<uint32_t> extract_prefix(std::string_view target, uint32_t prefix_size, bool cased) {
...
std::string_view extract_suffix(std::string_view target, uint32_t prefix_size) {
    Utf8Reader reader(std::string_view(target.data(), target.size()));
    for (size_t pos = 0; pos < prefix_size && reader.hasMore(); ++pos) {
        (void)reader.getChar();
    }
...
DfaFuzzyMatcher::DfaFuzzyMatcher(std::string_view target, uint8_t max_edits, uint32_t prefix_size, bool cased,
                                 bool prefix_match, LevenshteinDfa::DfaType dfa_type)
    : _dfa(vespalib::fuzzy::LevenshteinDfa::build(
          extract_suffix(target, prefix_size), max_edits,
...
const char* DfaFuzzyMatcher::skip_prefix(const char* word) const {
...
bool DfaFuzzyMatcher::is_match(std::string_view word) const {
    if (_prefix_size > 0) {
...
bool DfaFuzzyMatcher::is_match(const char* word, DictionaryConstIteratorType& itr,
...
template bool DfaFuzzyMatcher::is_match(const char* word, EnumPostingTree::ConstIterator& itr,
                                        const DfaStringComparator::DataStoreType& data_store);

template bool DfaFuzzyMatcher::is_match(const char* word, EnumTree::ConstIterator& itr,
                                        const DfaStringComparator::DataStoreType& data_store);
