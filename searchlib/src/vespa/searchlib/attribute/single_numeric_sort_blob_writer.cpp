...
SingleNumericSortBlobWriter<AttrType, ascending>::SingleNumericSortBlobWriter(const AttrType& attr) : _attr(attr) {
}

template <typename AttrType, bool ascending>
long SingleNumericSortBlobWriter<AttrType, ascending>::write(uint32_t docid, void* buf, long available) {
...
SingleNumericMissingSortBlobWriter<AttrType, ascending>::SingleNumericMissingSortBlobWriter(const AttrType& attr,
                                                                                            ...
                                                                                            T missing_value)
    : _attr(attr), _writer(policy, missing_value, false) {
}

template <typename AttrType, bool ascending>
long SingleNumericMissingSortBlobWriter<AttrType, ascending>::write(uint32_t docid, void* buf, long available) {
    _writer.reset();
...
std::unique_ptr<attribute::ISortBlobWriter> make_single_numeric_sort_blob_writer(const AttrType& attr, bool ascending,
                                                                                 MissingPolicy    policy,
    ...
    if (ascending) {
        if (policy == MissingPolicy::DEFAULT) {
            ...
            return std::make_unique<SingleNumericMissingSortBlobWriter<AttrType, true>>(
                attr, policy, string_to_number<T>(missing_value));
        }
    } else {
        if (policy == MissingPolicy::DEFAULT) {
            return std::make_unique<SingleNumericSortBlobWriter<AttrType, false>>(attr);
        } else {
            return std::make_unique<SingleNumericMissingSortBlobWriter<AttrType, false>>(
                attr, policy, string_to_number<T>(missing_value));
        }
    }
}
...
make_single_numeric_sort_blob_writer<IntegerAttributeTemplate<int8_t>>(const IntegerAttributeTemplate<int8_t>&, bool,
                                                                       MissingPolicy, std::string_view);
template std::unique_ptr<attribute::ISortBlobWriter>
make_single_numeric_sort_blob_writer<IntegerAttributeTemplate<int16_t>>(const IntegerAttributeTemplate<int16_t>&,
                                                                        bool, MissingPolicy, std::string_view);
template std::unique_ptr<attribute::ISortBlobWriter>
make_single_numeric_sort_blob_writer<IntegerAttributeTemplate<int32_t>>(const IntegerAttributeTemplate<int32_t>&,
                                                                        bool, MissingPolicy, std::string_view);
template std::unique_ptr<attribute::ISortBlobWriter>
make_single_numeric_sort_blob_writer<IntegerAttributeTemplate<int64_t>>(const IntegerAttributeTemplate<int64_t>&,
                                                                        bool, MissingPolicy, std::string_view);
template std::unique_ptr<attribute::ISortBlobWriter>
make_single_numeric_sort_blob_writer<FloatingPointAttributeTemplate<float>>(
    const FloatingPointAttributeTemplate<float>&, bool, MissingPolicy, std::string_view);
template std::unique_ptr<attribute::ISortBlobWriter>
make_single_numeric_sort_blob_writer<FloatingPointAttributeTemplate<double>>(
    const FloatingPointAttributeTemplate<double>&, bool, MissingPolicy, std::string_view);
