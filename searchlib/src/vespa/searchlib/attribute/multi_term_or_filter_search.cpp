...
template <typename IteratorPack> class MultiTermOrFilterSearchImpl : public MultiTermOrFilterSearch {
...
public:
    ...
    void and_hits_into(BitVector& result, uint32_t begin_id) override { return result.andWith(*get_hits(begin_id)); }
...
MultiTermOrFilterSearchImpl<IteratorPack>::MultiTermOrFilterSearchImpl(IteratorPack&&           children,
    ...
    : MultiTermOrFilterSearch(), _docids(children.size(), 0), _children(std::move(children)), _tfmd(tfmd) {
}

template <typename IteratorPack> MultiTermOrFilterSearchImpl<IteratorPack>::~MultiTermOrFilterSearchImpl() = default;

template <typename IteratorPack> void MultiTermOrFilterSearchImpl<IteratorPack>::seek_all(uint32_t docId) {
    for (uint16_t i = 0; i < _children.size(); ++i) {
...
template <typename IteratorPack> void MultiTermOrFilterSearchImpl<IteratorPack>::doSeek(uint32_t docId) {
...
std::unique_ptr<queryeval::SearchIterator> create_helper(std::vector<IteratorType>&& children,
                                                         fef::TermFieldMatchData*    tfmd) {
    if (children.empty()) {
        return std::make_unique<queryeval::EmptySearch>();
    } else {
        std::sort(children.begin(), children.end(), [](const auto& a, const auto& b) { return a.size() > b.size(); });
...
std::unique_ptr<queryeval::SearchIterator> MultiTermOrFilterSearch::create(std::vector<DocidIterator>&& children) {
