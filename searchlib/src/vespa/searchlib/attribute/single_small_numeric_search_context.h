...
private:
    ...
    int32_t onFind(DocId docId, int32_t elementId) const override { return find(docId, elementId); }

public:
    SingleSmallNumericSearchContext(std::unique_ptr<QueryTermSimple> qTerm, const AttributeVector& toBeSearched,
                                    const Word* word_data, Word value_mask, uint32_t value_shift_shift,
