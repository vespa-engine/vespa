...
template <typename T> SingleExtAttribute<T>::~SingleExtAttribute() = default;

template <typename T> bool SingleExtAttribute<T>::addDoc(typename Super::DocId& docId) {
...
template <typename T> bool SingleExtAttribute<T>::add(typename AddValueType<T>::Type v, int32_t) {
    this->_data.back() = v;
    return true;
}
template <typename T> void SingleExtAttribute<T>::onAddDocs(typename Super::DocId lidLimit) {
...
template <typename T> bool MultiExtAttribute<T>::addDoc(typename Super::DocId& docId) {
...
template <typename T> bool MultiExtAttribute<T>::add(typename AddValueType<T>::Type v, int32_t) {
...
template <typename T> void MultiExtAttribute<T>::onAddDocs(uint32_t lidLimit) {
    this->_data.reserve(lidLimit);
}

template <typename T> MultiExtAttribute<T>::~MultiExtAttribute() = default;

template <typename B>
WeightedSetExtAttributeBase<B>::WeightedSetExtAttributeBase(const std::string& name)
    : B(name, attribute::CollectionType::WSET), _weights() {
}

template <typename B> WeightedSetExtAttributeBase<B>::~WeightedSetExtAttributeBase() = default;

template <typename B> void WeightedSetExtAttributeBase<B>::addWeight(int32_t w) {
