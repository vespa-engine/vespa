...
template <typename IteratorType> PostingIteratorPack<IteratorType>::~PostingIteratorPack() = default;
...
template <typename IteratorType>
std::unique_ptr<BitVector> PostingIteratorPack<IteratorType>::get_hits(uint32_t begin_id, uint32_t end_id) {
...
template <typename IteratorType>
void PostingIteratorPack<IteratorType>::or_hits_into(BitVector& result, uint32_t begin_id) {
...
template <> int32_t PostingIteratorPack<DocidIterator>::get_weight(uint32_t, uint32_t) noexcept {
    return 1;
}
