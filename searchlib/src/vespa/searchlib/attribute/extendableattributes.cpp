...
template <typename T> const attribute::IMultiValueAttribute* MultiExtAttribute<T>::as_multi_value_attribute() const {
    return this;
}

template <typename T>
const attribute::IArrayReadView<T>* MultiExtAttribute<T>::make_read_view(attribute::IMultiValueAttribute::ArrayTag<T>,
...
MultiExtAttribute<T>::make_read_view(attribute::IMultiValueAttribute::WeightedSetTag<T>,
                                     vespalib::Stash& stash) const {
    return &stash.create<attribute::ExtendableNumericArrayMultiValueReadView<multivalue::WeightedValue<T>, T>>(
        this->_data, this->_idx);
}

MultiStringExtAttribute::MultiStringExtAttribute(const std::string& name, const CollectionType& ctype)
    : StringDirectAttrVector<AttrVector::Features<true>>(name, Config(BasicType::STRING, ctype)) {
    setEnum(false);
}

MultiStringExtAttribute::MultiStringExtAttribute(const std::string& name)
    : StringDirectAttrVector<AttrVector::Features<true>>(name, Config(BasicType::STRING, CollectionType::ARRAY)) {
    setEnum(false);
}
...
const attribute::IMultiValueAttribute* MultiStringExtAttribute::as_multi_value_attribute() const {
    return this;
}

const attribute::IArrayReadView<const char*>*
MultiStringExtAttribute::make_read_view(attribute::IMultiValueAttribute::ArrayTag<const char*>,
                                        vespalib::Stash& stash) const {
    return &stash.create<attribute::ExtendableStringArrayMultiValueReadView<const char*>>(this->_buffer,
                                                                                          this->_offsets, this->_idx);
}

const attribute::IWeightedSetReadView<const char*>*
MultiStringExtAttribute::make_read_view(attribute::IMultiValueAttribute::WeightedSetTag<const char*>,
                                        vespalib::Stash& stash) const {
    return &stash.create<attribute::ExtendableStringArrayMultiValueReadView<multivalue::WeightedValue<const char*>>>(
        this->_buffer, this->_offsets, this->_idx);
}
...
WeightedSetIntegerExtAttribute::getSearch(QueryTermSimpleUP                     term,
...
bool WeightedSetIntegerExtAttribute::add(int64_t v, int32_t w) {
    addWeight(w);
    MultiIntegerExtAttribute::add(v);
    return true;
}

uint32_t WeightedSetIntegerExtAttribute::get(DocId doc, AttributeVector::WeightedInt* v, uint32_t sz) const {
...
WeightedSetIntegerExtAttribute::make_read_view(attribute::IMultiValueAttribute::WeightedSetTag<int64_t>,
                                               vespalib::Stash& stash) const {
    return &stash.create<
        attribute::ExtendableNumericWeightedSetMultiValueReadView<multivalue::WeightedValue<int64_t>, int64_t>>(
        this->_data, this->_idx, this->get_weights());
}
...
bool WeightedSetFloatExtAttribute::add(double v, int32_t w) {
    addWeight(w);
    MultiFloatExtAttribute::add(v);
    return true;
}

uint32_t WeightedSetFloatExtAttribute::get(DocId doc, AttributeVector::WeightedFloat* v, uint32_t sz) const {
    uint32_t valueCount = _idx[doc + 1] - _idx[doc];
    uint32_t num2Read = std::min(valueCount, sz);
    for (uint32_t i = 0; i < num2Read; ++i) {
...
WeightedSetFloatExtAttribute::make_read_view(attribute::IMultiValueAttribute::WeightedSetTag<double>,
                                             vespalib::Stash& stash) const {
    return &stash.create<
        attribute::ExtendableNumericWeightedSetMultiValueReadView<multivalue::WeightedValue<double>, double>>(
        this->_data, this->_idx, this->get_weights());
}
...
bool WeightedSetStringExtAttribute::add(const char* v, int32_t w) {
    addWeight(w);
    MultiStringExtAttribute::add(v);
    return true;
}

uint32_t WeightedSetStringExtAttribute::get(DocId doc, AttributeVector::WeightedString* v, uint32_t sz) const {
    return getAllHelper(doc, v, sz);
}

uint32_t WeightedSetStringExtAttribute::get(DocId doc, AttributeVector::WeightedConstChar* v, uint32_t sz) const {
    return getAllHelper(doc, v, sz);
}

const attribute::IWeightedSetReadView<const char*>*
WeightedSetStringExtAttribute::make_read_view(attribute::IMultiValueAttribute::WeightedSetTag<const char*>,
                                              vespalib::Stash& stash) const {
    return &stash.create<
        attribute::ExtendableStringWeightedSetMultiValueReadView<multivalue::WeightedValue<const char*>>>(
        this->_buffer, this->_offsets, this->_idx, this->get_weights());
}
