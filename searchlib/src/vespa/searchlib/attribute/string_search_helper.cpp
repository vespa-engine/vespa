...
StringSearchHelper::StringSearchHelper(QueryTermUCS4& term, bool cased,
    ...
    if (isRegex()) {
        _regex = (isCased()) ? vespalib::Regex::from_pattern(term.getTerm(), vespalib::Regex::Options::None)
        ...
        _fuzzyMatcher = std::make_unique<vespalib::FuzzyMatcher>(
            term.getTerm(), max_edit_dist, term.fuzzy_prefix_lock_length(), isCased(), term.fuzzy_prefix_match());
        if ((fuzzy_matching_algorithm != FMA::BruteForce) && (max_edit_dist > 0 && max_edit_dist <= 2)) {
            _dfa_fuzzy_matcher = std::make_unique<DfaFuzzyMatcher>(
                term.getTerm(), max_edit_dist, term.fuzzy_prefix_lock_length(), isCased(), term.fuzzy_prefix_match(),
...
bool StringSearchHelper::isMatch(const char* src) const noexcept {
...
bool StringSearchHelper::is_fuzzy_match(const char* word, DictionaryConstIteratorType& itr,
...
template bool StringSearchHelper::is_fuzzy_match(const char*, EnumPostingTree::ConstIterator&,
                                                 const DfaStringComparator::DataStoreType&) const;

template bool StringSearchHelper::is_fuzzy_match(const char*, EnumTree::ConstIterator&,
                                                 const DfaStringComparator::DataStoreType&) const;
