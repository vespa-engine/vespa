...
public:
    Reference() : _gid(), _lid(0u), _revMapIdx() {}
    Reference(const GlobalId& gid_) : _gid(gid_), _lid(0u), _revMapIdx() {}
    bool operator<(const Reference& rhs) const { return _gid < rhs._gid; }
    bool operator==(const Reference& rhs) const { return _gid == rhs._gid; }
