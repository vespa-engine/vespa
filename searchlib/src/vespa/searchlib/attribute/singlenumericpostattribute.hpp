...
template <typename B> SingleValueNumericPostingAttribute<B>::~SingleValueNumericPostingAttribute() {
...
template <typename B> void SingleValueNumericPostingAttribute<B>::freezeEnumDictionary() {
    this->getEnumStore().freeze_dictionary();
}

template <typename B> void SingleValueNumericPostingAttribute<B>::mergeMemoryStats(vespalib::MemoryUsage& total) {
...
template <typename B>
void SingleValueNumericPostingAttribute<B>::applyUpdateValueChange(const Change& c, EnumStore& enumStore,
...
template <typename B>
void SingleValueNumericPostingAttribute<B>::makePostingChange(const vespalib::datastore::EntryComparator& cmpa,
...
template <typename B> void SingleValueNumericPostingAttribute<B>::applyValueChanges(EnumStoreBatchUpdater& updater) {
...
template <typename B>
void SingleValueNumericPostingAttribute<B>::reclaim_memory(vespalib::Generation oldest_used_gen) {
    SingleValueNumericEnumAttribute<B>::reclaim_memory(oldest_used_gen);
    _posting_store.reclaim_memory(oldest_used_gen);
}

template <typename B>
void SingleValueNumericPostingAttribute<B>::before_inc_generation(vespalib::Generation current_gen) {
...
template <typename B>
const IDocidPostingStore* SingleValueNumericPostingAttribute<B>::as_docid_posting_store() const {
