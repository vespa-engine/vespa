...
public:
    ...
    uint32_t getValueCount(DocId doc) const override { return (doc >= _bv.reader().size()) ? 0 : 1; }
    ...
    uint32_t get(DocId, WeightedEnum*, uint32_t) const override { return 0; }
    int8_t get(DocId doc) const override { return getFast(doc); }
...
protected:
    bool findEnum(int8_t, EnumHandle&) const override { return false; }

private:
    void ensureRoom(DocId docIdLimit);
    int8_t getFromEnum(EnumHandle) const override { return 0; }
    int8_t getFast(DocId doc) const { return _bv.reader().testBit(doc) ? 1 : 0; }
