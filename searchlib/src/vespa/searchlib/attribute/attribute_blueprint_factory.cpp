...
public:
    NodeAsKey(const Node& node, std::string& scratchPad) : _node(node), _scratchPad(scratchPad) {}

    std::string_view asString() const override { return queryeval::termAsString(_node, _scratchPad); }
...
public:
    AttributeFieldBlueprint(FieldSpecBase field, const IAttributeVector& attribute, const string& query_stack,
                            const SearchContextParams& params);
    AttributeFieldBlueprint(FieldSpecBase field, const IAttributeVector& attribute, QueryTermSimple::UP term,
                            const SearchContextParams& params);
    ...
    search::queryeval::FlowStats calculate_flow_stats(uint32_t docid_limit) const override {
        if (_hit_estimate.is_unknown()) {
            ...
            // In addition, matching is lookup based, and we are not able to skip documents efficiently when being
            // strict.
    ...
    SearchIteratorUP createFilterSearchImpl(FilterConstraint constraint) const override {
        (void)constraint; // We provide an iterator with exact results, so no need to take constraint into
                          // consideration.
...
void AttributeFieldBlueprint::visitMembers(vespalib::ObjectVisitor& visitor) const {
...
template <bool is_strict, typename Parent> struct LocationPreFilterIterator : public Parent {
    explicit LocationPreFilterIterator(OrSearch::Children children) : Parent(std::move(children), NoUnpack()) {}
...
public:
    LocationPreFilterBlueprint(const FieldSpec& field, const IAttributeVector& attribute,
                               const ZCurve::RangeVector& rangeVector, const SearchContextParams& scParams)
        : ComplexLeafBlueprint(field), _attribute(attribute), _rangeSearches(), _estimates(), _should_use(false) {
        uint64_t                estHits(0);
        ...
        for (const ZCurve::Range& r : rangeVector) {
            ...
            auto term =
                std::make_unique<streaming::QueryTerm>(QueryTermSimple::Type::WORD, "", std::move(range_spec));
    ...
    queryeval::FlowStats calculate_flow_stats(uint32_t docid_limit) const override {
        ...
        size_t indirections =
            queryeval::flow::get_num_indirections(_attribute.getBasicType(), _attribute.getCollectionType());
        ...
        return {est, OrFlow::cost_of(MyAdapter(docid_limit, indirections), _estimates, false),
                OrFlow::cost_of(MyAdapter(docid_limit, indirections), _estimates, true) +
...
public:
    LocationPostFilterBlueprint(const FieldSpec& field, const IAttributeVector& attribute, const Location& loc)
        : ComplexLeafBlueprint(field), _attribute(attribute), _location(loc), _pre_filter_estimate() {
        uint32_t estHits = 0;
        if (loc.valid()) {
...
Blueprint::UP make_location_blueprint(const FieldSpec& field, const IAttributeVector& attribute, const Location& loc,
    ...
    LOG(debug, "make_location_blueprint(fieldId[%u], p[%d,%d], r[%u], aspect[%u], bb[[%d,%d],[%d,%d]])",
        field.getFieldId(), loc.point.x, loc.point.y, loc.radius, loc.x_aspect.multiplier, loc.bounding_box.x.low,
        loc.bounding_box.x.high, loc.bounding_box.y.low, loc.bounding_box.y.high);
    ...
    ZCurve::RangeVector rangeVector = ZCurve::find_ranges(location.bounding_box.x.low, location.bounding_box.y.low,
                                                          location.bounding_box.x.high, location.bounding_box.y.high);
...
public:
    ...
    std::string_view asString() const override { return _terms.getAsString(_index).first; }
...
public:
    ...
    queryeval::FlowStats calculate_flow_stats(uint32_t docid_limit) const override {
        ...
        return {est, OrFlow::cost_of(MyAdapter(docid_limit), _terms, false),
                OrFlow::cost_of(MyAdapter(docid_limit), _terms, true) +
                    queryeval::flow::heap_cost(est, _terms.size())};
    }
    ...
    SearchIterator::UP createLeafSearch(const TermFieldMatchDataArray& tfmda) const override {
        ...
        return queryeval::ParallelWeakAndSearch::create(
            *tfmda[0],
...
std::unique_ptr<SearchIterator> DirectWandBlueprint::createFilterSearchImpl(FilterConstraint constraint) const {
...
void DirectWandBlueprint::set_matching_phase(MatchingPhase matching_phase) noexcept {
...
bool AttributeFieldBlueprint::getRange(search::NumericRangeSpec& range_spec) const {
    if (_type == INT) {
...
public:
    CreateBlueprintVisitor(Searchable& searchable, const IRequestContext& requestContext, const FieldSpec& field,
                           const IAttributeVector& attr, fef::MatchDataLayout& global_layout)
    ...
    void visit(SubstringTerm& n) override {
        query::SimpleRegExpTerm re(vespalib::RegexpUtil::make_from_substring(n.getTerm()), n.getView(), n.getId(),
                                   n.getWeight());
        visitTerm(re);
    }
    void visit(SuffixTerm& n) override {
        query::SimpleRegExpTerm re(vespalib::RegexpUtil::make_from_suffix(n.getTerm()), n.getView(), n.getId(),
                                   n.getWeight());
        visitTerm(re);
    }
    ...
    template <typename BlueprintType> void createDirectMultiTerm(BlueprintType* bp, MultiTerm& n);

    template <typename WS> void createShallowWeightedSet(WS* bp, MultiTerm& n, const FieldSpec& fs, bool isInteger);

    static QueryTermSimple::UP extractTerm(std::string_view term_view, bool isInteger) {
    ...
    template <typename TermType, typename SearchType> void visit_wset_or_in_term(TermType& n) {
        if (_dps != nullptr) {
            auto* bp = new attribute::DirectMultiTermBlueprint<IDocidPostingStore, SearchType>(_field, _attr, *_dps,
                                                                                               n.getNumTerms());
            ...
            if (isSingleValue && (isString || isInteger)) {
                ...
                for (size_t i = 0; i < n.getNumTerms(); ++i) {
                    ...
                    ws->addToken(_attr.createSearchContext(extractTerm(term.first, isInteger), scParams),
                                 term.second.percent());
                }
    ...
    void visit(query::DotProduct& n) override {
        if (has_always_btree_iterators_with_docid_and_weight()) {
            auto* bp =
                new attribute::DirectMultiTermBlueprint<IDocidWithWeightPostingStore, queryeval::DotProductSearch>(
                    _field, _attr, *_dwwps, n.getNumTerms());
            createDirectMultiTerm(bp, n);
        } else {
    ...
    void visit(query::WandTerm& n) override {
        if (has_always_btree_iterators_with_docid_and_weight()) {
            auto* bp =
                new DirectWandBlueprint(_field, *_dwwps, n.getTargetNumHits(), n.getScoreThreshold(),
    ...
    void visit(query::InTerm& n) override { visit_wset_or_in_term<query::InTerm, attribute::InTermSearch>(n); }

    void fail_nearest_neighbor_term(query::NearestNeighborTerm& n, const std::string& error_msg) {
        Issue::report("NearestNeighborTerm(%s, %s): %s. Returning empty blueprint", _field.getName().c_str(),
                      n.get_query_tensor_name().c_str(), error_msg.c_str());
        setResult(std::make_unique<queryeval::EmptyBlueprint>(_field));
    }
    void visit(query::NearestNeighborTerm& n) override {
        ...
        try {
            ...
            setResult(std::make_unique<queryeval::NearestNeighborBlueprint>(
                _field, std::move(calc), n.get_target_num_hits(), n.get_allow_approximate(), blueprint_hnsw_params));
...
void CreateBlueprintVisitor::createDirectMultiTerm(BlueprintType* bp, MultiTerm& n) {
...
void CreateBlueprintVisitor::createShallowWeightedSet(WS* bp, MultiTerm& n, const FieldSpec& fs, bool isInteger) {
    Blueprint::UP       result(bp);
    SearchContextParams scParams = createContextParams();
    ...
    for (uint32_t i(0); i < n.getNumTerms(); i++) {
        ...
        auto          term = n.getAsString(i);
        bp->addTerm(std::make_unique<AttributeFieldBlueprint>(childfs, _attr, extractTerm(term.first, isInteger),
...
Blueprint::UP AttributeBlueprintFactory::createBlueprint(const IRequestContext& requestContext,
                                                         const FieldSpec& field, const query::Node& term,
