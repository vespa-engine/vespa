...
NumericSortBlobWriter<T, asc>::NumericSortBlobWriter(MissingPolicy policy, T missing_value, bool multi_value) noexcept
    : _best(), _missing_blob(), _value_prefix() {
    switch (policy) {
...
template <typename T, bool asc> NumericSortBlobWriter<T, asc>::~NumericSortBlobWriter() noexcept = default;

template <typename T, bool asc> void NumericSortBlobWriter<T, asc>::set_missing_blob(T value) {
    _missing_blob.clear();
...
template <typename T, bool asc> void NumericSortBlobWriter<T, asc>::reset() {
    _best.reset();
}

template <typename T, bool asc> long NumericSortBlobWriter<T, asc>::write(void* serTo, size_t available) {
