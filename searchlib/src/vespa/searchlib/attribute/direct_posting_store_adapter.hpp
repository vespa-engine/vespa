...
DirectPostingStoreAdapter<ParentType, PostingStoreType, EnumStoreType>::DirectPostingStoreAdapter(
...
DirectPostingStoreAdapter<ParentType, PostingStoreType, EnumStoreType>::get_dictionary_snapshot() const {
    return _dict.get_frozen_root();
}

template <typename ParentType, typename PostingStoreType, typename EnumStoreType>
std::unique_ptr<queryeval::SearchIterator>
DirectPostingStoreAdapter<ParentType, PostingStoreType, EnumStoreType>::make_bitvector_iterator(
    vespalib::datastore::EntryRef posting_idx, uint32_t doc_id_limit, fef::TermFieldMatchData& match_data,
    bool strict) const {
    return _posting_store.make_bitvector_iterator(posting_idx, doc_id_limit, match_data, strict);
}

template <typename ParentType, typename PostingStoreType, typename EnumStoreType>
bool DirectPostingStoreAdapter<ParentType, PostingStoreType, EnumStoreType>::has_btree_iterator(
    vespalib::datastore::EntryRef posting_idx) const noexcept {
    return _posting_store.has_btree(posting_idx);
}

template <typename ParentType, typename PostingStoreType, typename EnumStoreType>
bool DirectPostingStoreAdapter<ParentType, PostingStoreType, EnumStoreType>::has_bitvector(
    vespalib::datastore::EntryRef posting_idx) const noexcept {
