...
MultiNumericFlagSearchContext<T, M>::MultiNumericFlagSearchContext(std::unique_ptr<QueryTermSimple> qTerm,
    ...
    if (this->valid()) {
        if (this->_low == this->_high) {
            ...
            SearchIterator::UP flagIterator(
                strict ? new FlagAttributeIteratorStrict<MultiNumericFlagSearchContext>(*this, matchData)
