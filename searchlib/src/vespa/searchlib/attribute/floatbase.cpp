...
FloatingPointAttribute::FloatingPointAttribute(const std::string& name, const Config& c)
    : NumericAttribute(name, c), _changes() {
}
...
bool FloatingPointAttribute::applyWeight(DocId doc, const FieldValue& fv,
                                         const document::AssignValueUpdate& wAdjust) {
    double v = fv.getAsDouble();
    return AttributeVector::adjustWeight(_changes, doc, NumericChangeData<double>(v), wAdjust);
}
...
std::span<const char> FloatingPointAttribute::get_raw(DocId) const {
    return {};
}

vespalib::MemoryUsage FloatingPointAttribute::getChangeVectorMemoryUsage() const {
...
template bool AttributeVector::update(FloatingPointAttribute::ChangeVector& changes, DocId doc,
                                      const NumericChangeData<double>& v);
template bool AttributeVector::append(FloatingPointAttribute::ChangeVector& changes, DocId doc,
