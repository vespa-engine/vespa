...
public:
    BitVectorEntry() noexcept : _tree(), _bv() {}
};
...
class PostingStore : public PostingListTraits<DataT>::PostingStoreBase, public PostingStoreBase2 {
...
public:
    ...
    BitVectorRefPair allocBitVector() {
        return _store
            .template freeListAllocator<BitVectorEntry, vespalib::datastore::DefaultReclaimer<BitVectorEntry>>(
    ...
    BitVectorEntry* getWBitVectorEntry(RefType ref) { return _store.template getEntry<BitVectorEntry>(ref); }
    ...
    std::unique_ptr<queryeval::SearchIterator> make_bitvector_iterator(RefType ref, uint32_t doc_id_limit,
...
template <>
inline vespalib::btree::BTreeNoLeafData PostingStore<vespalib::btree::BTreeNoLeafData>::bitVectorWeight() {
    return vespalib::btree::BTreeNoLeafData();
}

template <> inline int32_t PostingStore<int32_t>::bitVectorWeight() {
    return 1;
}
