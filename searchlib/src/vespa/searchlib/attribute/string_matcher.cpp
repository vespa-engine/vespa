...
void StringMatcher::setup_enum_hint_sc(const EnumStoreT<const char*>& enum_store,
    ...
    if (isValid()) {
        if (isPrefix()) {
            auto comp = enum_store.make_folded_prefix_lookup(get_query_term_ptr()->getTerm());
            enum_hint_sc.lookupRange(comp, comp);
        } else if (isRegex()) {
            std::string prefix(vespalib::RegexpUtil::get_prefix(get_query_term_ptr()->getTerm()));
            auto        comp = enum_store.make_folded_prefix_lookup(prefix.c_str());
            enum_hint_sc.lookupRange(comp, comp);
        } else if (isFuzzy()) {
            ...
            auto comp = enum_store.make_folded_lookup(get_query_term_ptr()->getTerm());
