...
template <typename B>
NumericDirectAttribute<B>::NumericDirectAttribute(const std::string& baseFileName, const Config& c)
    : B(baseFileName, c), _data(), _idx() {
}

template <typename B> NumericDirectAttribute<B>::~NumericDirectAttribute() = default;

template <typename B> bool NumericDirectAttribute<B>::findEnum(typename B::BaseType key, EnumHandle& e) const {
    if (_data.empty()) {
        e = 0;
        return false;
    }
...
template <typename B> void NumericDirectAttribute<B>::onCommit() {
    B::_changes.clear();
    HDR_ABORT("should not be reached");
}

template <typename B> bool NumericDirectAttribute<B>::addDoc(DocId&) {
    return false;
}
...
NumericDirectAttrVector<F, B>::NumericDirectAttrVector(const std::string&             baseFileName,
...
template <typename F, typename B>
NumericDirectAttrVector<F, B>::NumericDirectAttrVector(const std::string& baseFileName)
    : search::NumericDirectAttribute<B>(
          baseFileName, AttributeVector::Config(AttributeVector::BasicType::fromType(BaseType()),
                                                F::IsMultiValue() ? search::attribute::CollectionType::ARRAY
...
template <typename F, typename B> bool NumericDirectAttrVector<F, B>::is_sortable() const noexcept {
    return true;
}
...
public:
    NumericDirectSortBlobWriter(const std::vector<BaseType>& data, const std::vector<uint32_t>& idx,
                                search::common::sortspec::MissingPolicy policy, BaseType missing_value) noexcept
        : _data(data), _idx(idx), _writer(policy, missing_value, true) {}
...
NumericDirectAttrVector<F, B>::make_sort_blob_writer(bool ascending, const search::common::BlobConverter* converter,
    ...
    if (ascending) {
        return std::make_unique<NumericDirectSortBlobWriter<BaseType, true>>(this->_data, this->_idx, policy,
                                                                             missing_num);
    } else {
        return std::make_unique<NumericDirectSortBlobWriter<BaseType, false>>(this->_data, this->_idx, policy,
                                                                              missing_num);
    }
}

template <typename F>
StringDirectAttrVector<F>::StringDirectAttrVector(const std::string& baseFileName, const Config& c)
...
template <typename F>
StringDirectAttrVector<F>::StringDirectAttrVector(const std::string& baseFileName)
    : search::StringDirectAttribute(
          baseFileName, Config(BasicType::STRING, F::IsMultiValue() ? search::attribute::CollectionType::ARRAY
                                                                    : search::attribute::CollectionType::SINGLE)) {
    if (F::IsMultiValue()) {
        _idx.push_back(0);
    }
...
template <bool asc> class StringDirectSortBlobWriter : public search::attribute::ISortBlobWriter {
private:
...
template <typename F> bool StringDirectAttrVector<F>::is_sortable() const noexcept {
    return true;
}
...
StringDirectAttrVector<F>::make_sort_blob_writer(bool ascending, const search::common::BlobConverter* converter,
    ...
    if (ascending) {
        return std::make_unique<StringDirectSortBlobWriter<true>>(this->_buffer, this->_offsets, this->_idx,
                                                                  converter, policy, missing_value);
    } else {
        return std::make_unique<StringDirectSortBlobWriter<false>>(this->_buffer, this->_offsets, this->_idx,
                                                                   converter, policy, missing_value);
    }
}
