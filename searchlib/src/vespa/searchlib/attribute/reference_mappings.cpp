...
void ReferenceMappings::clearMapping(const Reference& entry) {
...
void ReferenceMappings::syncForwardMapping(const Reference& entry) {
    uint32_t targetLid = entry.lid();
    EntryRef revMapIdx = entry.revMapIdx();
    auto&    targetLids = _targetLids;
    _reverseMapping.foreach_unfrozen_key(
        revMapIdx, [&targetLids, targetLid](uint32_t lid) { targetLids[lid].store_release(targetLid); });
}

void ReferenceMappings::syncReverseMappingIndices(const Reference& entry) {
    uint32_t targetLid = entry.lid();
    if (targetLid != 0u) {
...
void ReferenceMappings::removeReverseMapping(const Reference& entry, uint32_t lid) {
    EntryRef revMapIdx = entry.revMapIdx();
...
void ReferenceMappings::addReverseMapping(const Reference& entry, uint32_t lid) {
    EntryRef                    revMapIdx = entry.revMapIdx();
...
void ReferenceMappings::notifyReferencedPut(const Reference& entry, uint32_t targetLid) {
...
void ReferenceMappings::notifyReferencedRemove(const Reference& entry) {
    uint32_t oldTargetLid = entry.lid();
    if (oldTargetLid != 0) {
...
void ReferenceMappings::onAddDocs(uint32_t docIdLimit) {
    _targetLids.reserve(docIdLimit);
}

void ReferenceMappings::addDoc() {
    _targetLids.push_back(AtomicTargetLid(0));
}

void ReferenceMappings::onLoad(uint32_t docIdLimit) {
    _targetLids.clear();
    _targetLids.unsafe_reserve(docIdLimit);
    _targetLids.ensure_size(docIdLimit);
}

void ReferenceMappings::shrink(uint32_t docIdLimit) {
    _targetLids.shrink(docIdLimit);
}

vespalib::MemoryUsage ReferenceMappings::getMemoryUsage() {
