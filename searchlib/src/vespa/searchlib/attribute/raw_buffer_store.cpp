...
RawBufferStore::RawBufferStore(std::shared_ptr<vespalib::alloc::MemoryAllocator> allocator, uint32_t max_type_id,
                               double grow_factor)
    : _array_store(ArrayStoreType::optimizedConfigForHugePage(
                       max_type_id, TypeMapper(max_type_id, grow_factor, max_buffer_size),
                       MemoryAllocator::HUGEPAGE_SIZE, MemoryAllocator::NORMAL_PAGE_SIZE, max_buffer_size, 8_Ki,
                       ALLOC_GROW_FACTOR),
