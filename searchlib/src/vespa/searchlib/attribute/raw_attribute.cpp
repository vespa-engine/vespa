...
RawAttribute::RawAttribute(const std::string& name, const Config& config) : NotImplementedAttribute(name, config) {
}
 ...
 */
template <bool asc> size_t serialize_for_sort(std::span<const char> raw, void* serTo) {
...
template <bool asc> class RawAttributeSortBlobWriter : public ISortBlobWriter {
private:
...
template <bool asc>
RawAttributeSortBlobWriter<asc>::RawAttributeSortBlobWriter(const RawAttribute& attr, MissingPolicy policy,
                                                            std::span<const char> missing_value) noexcept
    : _attr(attr), _missing_blob(), _value_prefix() {
    switch (policy) {
...
template <bool asc> RawAttributeSortBlobWriter<asc>::~RawAttributeSortBlobWriter() = default;

template <bool asc> void RawAttributeSortBlobWriter<asc>::set_missing_blob(std::span<const char> value) {
...
bool RawAttribute::is_sortable() const noexcept {
    return true;
}

std::unique_ptr<ISortBlobWriter> RawAttribute::make_sort_blob_writer(bool ascending, const common::BlobConverter*,
    ...
    if (policy == MissingPolicy::AS) {
        try {
            ...
            throw IllegalArgumentException("Failed converting string '" + std::string(missing_value) +
                                           "' to a raw value: " + e.getMessage());
        }
    }
    if (ascending) {
