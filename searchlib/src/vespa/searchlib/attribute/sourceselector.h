...
protected:
    ...
    queryeval::Source getNewSource(queryeval::Source src, uint32_t diff) { return src > diff ? src - diff : 0; }
...
public:
    struct HeaderInfo {
        ...
        HeaderInfo(const std::string& baseFileName, queryeval::Source defaultSource, uint32_t baseId,
                   uint32_t docIdLimit);
    };
    ...
    public:
        ...
        SaveInfo(const std::string& baseFileName, queryeval::Source defaultSource, uint32_t baseId,
