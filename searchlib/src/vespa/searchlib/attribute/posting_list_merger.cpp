...
PostingListMerger<DataT>::PostingListMerger(uint32_t docIdLimit) noexcept
    : _array(), _startPos(), _bitVector(), _docIdLimit(docIdLimit), _arrayValid(false) {
}

template <typename DataT> PostingListMerger<DataT>::~PostingListMerger() = default;

template <typename DataT> void PostingListMerger<DataT>::reserveArray(uint32_t postingsCount, size_t postingsSize) {
    _array.reserve(postingsSize);
    _startPos.reserve(postingsCount + 1);
    _startPos.push_back(0);
}

template <typename DataT> void PostingListMerger<DataT>::allocBitVector() {
    _bitVector = BitVector::create(_docIdLimit);
}

template <typename DataT> void PostingListMerger<DataT>::merge() {
    if (_bitVector) {
...
PostingListMerger<DataT>::merge(PostingVector& v, PostingVector& temp, const StartVector& startPos) {
    ...
    for (size_t i(0), m((startPos.size() - 1) / 2); i < m; i++) {
        ...
        auto   it = v.begin();
        std::merge(it + aStart, it + aEnd, it + bStart, it + bEnd, temp.begin() + aStart);
