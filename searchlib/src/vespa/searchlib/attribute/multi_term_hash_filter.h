 ...
 *
 * @tparam WrapperType Type that wraps an attribute vector and provides access to the attribute value for a given
 * docid.
 */
template <typename WrapperType> class MultiTermHashFilter final : public queryeval::SearchIterator {
public:
    using Key = typename WrapperType::TokenT;
    using TokenMap = vespalib::hash_map<Key, int32_t, vespalib::hash<Key>, std::equal_to<Key>,
...
public:
    MultiTermHashFilter(fef::TermFieldMatchData& tfmd, WrapperType attr, TokenMap&& map);
