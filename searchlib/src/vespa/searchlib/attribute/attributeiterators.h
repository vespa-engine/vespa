...
protected:
    template <typename SC> void and_hits_into(const SC& sc, BitVector& result, uint32_t begin_id) const;
    template <typename SC> void or_hits_into(const SC& sc, BitVector& result, uint32_t begin_id) const;
    template <typename SC> std::unique_ptr<BitVector> get_hits(const SC& sc, uint32_t begin_id) const;
...
public:
    AttributeIteratorBase(const attribute::ISearchContext& baseSearchCtx, fef::TermFieldMatchData* matchData)
        : _baseSearchCtx(baseSearchCtx), _matchData(matchData), _matchPosition(_matchData->populate_fixed()) {}
...
public:
    AttributeIterator(const attribute::ISearchContext& baseSearchCtx, fef::TermFieldMatchData* matchData)
        : AttributeIteratorBase(baseSearchCtx, matchData), _weight(1) {}
...
public:
    FilterAttributeIterator(const attribute::ISearchContext& baseSearchCtx, fef::TermFieldMatchData* matchData)
...
template <typename SC> class AttributeIteratorT : public AttributeIterator {
private:
...
template <typename SC> class FilterAttributeIteratorT : public FilterAttributeIterator {
private:
    void doSeek(uint32_t docId) override;
    void visitMembers(vespalib::ObjectVisitor& visitor) const override;
    void and_hits_into(BitVector& result, uint32_t begin_id) override;
 ...
 * @param SC the specialized search context type associated with this iterator
 */
template <typename SC> class AttributeIteratorStrict : public AttributeIteratorT<SC> {
private:
...
template <typename SC> class FilterAttributeIteratorStrict : public FilterAttributeIteratorT<SC> {
private:
...
public:
    AttributePostingListIterator(const attribute::ISearchContext& baseSearchCtx, fef::TermFieldMatchData* matchData)
        : AttributeIteratorBase(baseSearchCtx, matchData) {}
    Trinary is_strict() const override { return Trinary::True; }
...
public:
    FilterAttributePostingListIterator(const attribute::ISearchContext& baseSearchCtx,
                                       fef::TermFieldMatchData*         matchData)
        : AttributeIteratorBase(baseSearchCtx, matchData) {}
    Trinary is_strict() const override { return Trinary::True; }
};

using InnerAttributePostingListIterator =
    attribute::PostingListTraits<vespalib::btree::BTreeNoLeafData>::const_iterator;

using WeightedInnerAttributePostingListIterator = attribute::PostingListTraits<int32_t>::const_iterator;

template <typename PL> class AttributePostingListIteratorT : public AttributePostingListIterator {
private:
    PL                           _iterator;
...
public:
    ...
    AttributePostingListIteratorT(const attribute::ISearchContext& baseSearchCtx, fef::TermFieldMatchData* matchData,
                                  Args&&... args);
    ~AttributePostingListIteratorT() override;
};

template <typename PL> class FilterAttributePostingListIteratorT : public FilterAttributePostingListIterator {
private:
    PL _iterator;
...
public:
    template <typename... Args>
    FilterAttributePostingListIteratorT(const attribute::ISearchContext& baseSearchCtx,
                                        fef::TermFieldMatchData*         matchData, Args&&... args);
    ~FilterAttributePostingListIteratorT() override;
};

template <> inline int32_t AttributePostingListIteratorT<InnerAttributePostingListIterator>::getWeight() {
...
template <typename SC> class FlagAttributeIteratorT : public FlagAttributeIterator {
private:
    void doSeek(uint32_t docId) override;
...
public:
    FlagAttributeIteratorT(const SC& concreteSearchCtx, fef::TermFieldMatchData* matchData)
        : FlagAttributeIterator(concreteSearchCtx, matchData), _concreteSearchCtx(concreteSearchCtx) {}
...
template <typename SC> class FlagAttributeIteratorStrict : public FlagAttributeIteratorT<SC> {
private:
