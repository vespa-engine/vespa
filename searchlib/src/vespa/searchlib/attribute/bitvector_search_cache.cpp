...
BitVectorSearchCache::BitVectorSearchCache() : _mutex(), _size(0), _entries_extra_memory_usage(0), _cache() {
}

BitVectorSearchCache::~BitVectorSearchCache() = default;

void BitVectorSearchCache::insert(const std::string& term, std::shared_ptr<Entry> entry) {
...
std::shared_ptr<BitVectorSearchCache::Entry> BitVectorSearchCache::find(const std::string& term) const {
    if (size() > 0ul) {
...
vespalib::MemoryUsage BitVectorSearchCache::get_memory_usage() const {
...
void BitVectorSearchCache::clear() {
    std::unique_lock guard(_mutex);
    _cache.clear();
