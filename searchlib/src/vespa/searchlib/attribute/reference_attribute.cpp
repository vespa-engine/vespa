...
uint64_t extractUniqueValueCount(const vespalib::GenericHeader& header) {
...
void ReferenceAttribute::onAddDocs(DocId limit) {
    _indices.reserve(limit);
    _referenceMappings.onAddDocs(limit);
}

bool ReferenceAttribute::addDoc(DocId& doc) {
...
void ReferenceAttribute::removeReverseMapping(EntryRef oldRef, uint32_t lid) {
    const auto& entry = _store.get(oldRef);
    _referenceMappings.removeReverseMapping(entry, lid);
}

void ReferenceAttribute::addReverseMapping(EntryRef newRef, uint32_t lid) {
    const auto& entry = _store.get(newRef);
    _referenceMappings.addReverseMapping(entry, lid);
}

void ReferenceAttribute::buildReverseMapping(EntryRef newRef, const std::vector<ReverseMapping::KeyDataType>& adds) {
    const auto& entry = _store.get(newRef);
    _referenceMappings.buildReverseMapping(entry, adds);
}

void ReferenceAttribute::buildReverseMapping() {
...
void ReferenceAttribute::reclaim_memory(Generation oldest_used_gen) {
    _referenceMappings.reclaim_memory(oldest_used_gen);
    _store.reclaim_memory(oldest_used_gen);
    getGenerationHolder().reclaim(oldest_used_gen);
}

void ReferenceAttribute::before_inc_generation(Generation current_gen) {
...
void ReferenceAttribute::onCommit() {
...
void ReferenceAttribute::onUpdateStat(CommitParam::UpdateStats updateStats) {
    ...
    _compaction_spec =
        ReferenceAttributeCompactionSpec(compaction_strategy.should_compact_memory(total),
    ...
    updateStatistics(getTotalValueCount(), getUniqueValueCount(), total.allocatedBytes(), total.usedBytes(),
                     total.deadBytes(), total.allocatedBytesOnHold());
}

std::unique_ptr<AttributeSaver> ReferenceAttribute::onInitSave(std::string_view fileName) {
    ...
    return std::make_unique<ReferenceAttributeSaver>(
        ...
        make_entry_ref_vector_snapshot(_indices, getCommittedDocIdLimit()), _store);
}

bool ReferenceAttribute::onLoad(vespalib::Executor*) {
...
const Reference* ReferenceAttribute::getReference(DocId doc) const {
...
IAttributeVector::EnumHandle ReferenceAttribute::getEnum(DocId doc) const {
...
bool ReferenceAttribute::consider_compact_values(const CompactionStrategy& compactionStrategy) {
...
void ReferenceAttribute::compact_worst_values(const CompactionStrategy& compaction_strategy) {
...
bool ReferenceAttribute::consider_compact_dictionary(const CompactionStrategy& compaction_strategy) {
    auto& dictionary = _store.get_dictionary();
...
uint64_t ReferenceAttribute::getUniqueValueCount() const {
    return _store.getNumUniques();
}

void ReferenceAttribute::setGidToLidMapperFactory(std::shared_ptr<IGidToLidMapperFactory> gidToLidMapperFactory) {
...
void ReferenceAttribute::notifyReferencedPut(const GlobalId& gid, DocId targetLid) {
    notifyReferencedPutNoCommit(gid, targetLid);
    commit();
}

bool ReferenceAttribute::notifyReferencedRemoveNoCommit(const GlobalId& gid) {
    EntryRef ref = _store.find(gid);
    if (ref.valid()) {
        const auto& entry = _store.get(ref);
...
void ReferenceAttribute::notifyReferencedRemove(const GlobalId& gid) {
...
public:
    explicit TargetLidPopulator(ReferenceAttribute& attr) : IGidToLidMapperVisitor(), _attr(attr) {}
...
void ReferenceAttribute::populateTargetLids(const std::vector<GlobalId>& removes) {
...
public:
    ...
    bool valid() const override { return _term != GlobalId(); }
...
uint32_t ReferenceSearchContext::get_committed_docid_limit() const noexcept {
    return _docid_limit;
}
