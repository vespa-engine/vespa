...
template <typename ITR> class ForwardRange {
private:
    ITR _lower;
    ITR _upper;
...
template <typename ITR> class ReverseRange {
private:
    ITR _lower;
    ITR _upper;
...
public:
    ...
    static std::unique_ptr<DiversityFilter> create(const IAttributeVector& diversity_attr, size_t wanted_hits,
...
template <typename Result> class DiversityRecorder {
private:
    DiversityFilter& _filter;
    Result&          _result;

public:
    DiversityRecorder(DiversityFilter& filter, Result& result) : _filter(filter), _result(result) {}

    template <typename Item> void push_back(Item item) {
...
void diversify_2(const DictRange& range_in, const PostingStore& posting, DiversityFilter& filter, Result& result,
    ...
    while (range.has_next() && (result.size() < filter.getMaxTotal())) {
        ...
        posting.foreach_frozen(dict_entry.get().getData().load_acquire(), [&](uint32_t key, const DataType& data) {
...
void diversify(bool forward, const DictItr& lower, const DictItr& upper, const PostingStore& posting,
               size_t wanted_hits, const IAttributeVector& diversity_attr, size_t max_per_group,
               size_t cutoff_max_groups, bool cutoff_strict, Result& array, std::vector<size_t>& fragments) {
    auto filter =
        DiversityFilter::create(diversity_attr, wanted_hits, max_per_group, cutoff_max_groups, cutoff_strict);
    if (forward) {
...
void diversify_single(vespalib::datastore::EntryRef posting_idx, const PostingStore& posting, size_t wanted_hits,
                      const IAttributeVector& diversity_attr, size_t max_per_group, size_t cutoff_max_groups,
                      bool cutoff_strict, Result& result, std::vector<size_t>& fragments) {
    auto filter =
        DiversityFilter::create(diversity_attr, wanted_hits, max_per_group, cutoff_max_groups, cutoff_strict);
    DiversityRecorder<Result> recorder(*filter, result);
    using DataType = typename PostingStore::DataType;
    ...
    posting.foreach_frozen(posting_idx,
                           [&](uint32_t key, const DataType& data) { recorder.push_back(KeyDataType(key, data)); });
    if (fragments.back() < result.size()) {
        fragments.push_back(result.size());
    }
}
