...
uint64_t extractCreateSerialNum(const vespalib::GenericHeader& header) {
...
ReaderBase::ReaderBase(AttributeVector& attr)
    : _datFile(attribute::LoadUtils::openDAT(attr)),
      _weightFile(attr.hasWeightedSetType() ? attribute::LoadUtils::openWeight(attr)
                                            : std::unique_ptr<Fast_BufferedFile>()),
      _idxFile(attr.needs_idx_file() ? attribute::LoadUtils::openIDX(attr) : std::unique_ptr<Fast_BufferedFile>()),
...
bool ReaderBase::hasWeight() const {
    return _weightFile.valid();
}

bool ReaderBase::hasIdx() const {
    return _idxFile.valid();
}

bool ReaderBase::hasData() const {
    return _datFile.valid();
}

void ReaderBase::rewind() {
    _datFile.rewind();
    _currIdx = 0;
    if (hasIdx()) {
        _idxFile.rewind();
...
size_t ReaderBase::getNumValues() {
    if (getEnumerated()) {
        return getEnumCount();
    } else {
        if (_fixedWidth > 0) {
...
uint32_t ReaderBase::getNextValueCount() {
...
uint64_t ReaderBase::size_on_disk() const {
    return DiskSpaceCalculator::directory_placeholder_size() + _datFile.size_on_disk() + _idxFile.size_on_disk() +
           _weightFile.size_on_disk();
}
