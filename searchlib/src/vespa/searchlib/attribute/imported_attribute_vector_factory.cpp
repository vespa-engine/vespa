...
ImportedAttributeVectorFactory::create(std::string_view name, std::shared_ptr<ReferenceAttribute> reference_attribute,
    ...
    case BasicType::Type::TENSOR:
        return std::make_shared<ImportedTensorAttributeVector>(
            name, std::move(reference_attribute), std::move(document_meta_store), std::move(target_attribute),
            std::move(target_document_meta_store), use_search_cache);
    default:
        return std::make_shared<ImportedAttributeVector>(name, std::move(reference_attribute),
                                                         std::move(document_meta_store), std::move(target_attribute),
                                                         std::move(target_document_meta_store), use_search_cache);
    }
}
...
ImportedAttributeVectorFactory::create(std::string_view name, std::shared_ptr<ReferenceAttribute> reference_attribute,
    ...
    case BasicType::Type::TENSOR:
        return std::make_shared<ImportedTensorAttributeVector>(
            name, std::move(reference_attribute), std::move(document_meta_store), std::move(target_attribute),
            std::move(target_document_meta_store), std::move(search_cache));
    default:
