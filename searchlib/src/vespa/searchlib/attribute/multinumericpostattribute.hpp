...
template <typename B, typename M> void MultiValueNumericPostingAttribute<B, M>::freezeEnumDictionary() {
    this->getEnumStore().freeze_dictionary();
}

template <typename B, typename M>
void MultiValueNumericPostingAttribute<B, M>::mergeMemoryStats(vespalib::MemoryUsage& total) {
...
template <typename B, typename M>
void MultiValueNumericPostingAttribute<B, M>::applyValueChanges(const DocIndices&      docIndices,
...
template <typename B, typename M> MultiValueNumericPostingAttribute<B, M>::~MultiValueNumericPostingAttribute() {
...
template <typename B, typename M>
void MultiValueNumericPostingAttribute<B, M>::reclaim_memory(vespalib::Generation oldest_used_gen) {
    MultiValueNumericEnumAttribute<B, M>::reclaim_memory(oldest_used_gen);
    _posting_store.reclaim_memory(oldest_used_gen);
}

template <typename B, typename M>
void MultiValueNumericPostingAttribute<B, M>::before_inc_generation(vespalib::Generation current_gen) {
