...
template <typename EntryT, typename RefT>
MultiValueMapping2<EntryT,RefT>::MultiValueMapping2(uint32_t maxSmallArraySize, size_t minClusters, size_t maxClusters, size_t minClustersNewBuf, const GrowStrategy &gs)
    : MultiValueMapping2Base(gs, _store.getGenerationHolder()),
      _store(maxSmallArraySize, minClusters, maxClusters, minClustersNewBuf)
{
}
