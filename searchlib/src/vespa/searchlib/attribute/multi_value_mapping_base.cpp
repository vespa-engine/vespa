...
MultiValueMappingBase::MultiValueMappingBase(const vespalib::GrowStrategy& gs, vespalib::GenerationHolder& genHolder,
                                             std::shared_ptr<vespalib::alloc::MemoryAllocator> memory_allocator)
    : _memory_allocator(std::move(memory_allocator)),
      _indices(gs, genHolder,
               _memory_allocator ? vespalib::alloc::Alloc::alloc_with_allocator(_memory_allocator.get())
...
void MultiValueMappingBase::addDoc(uint32_t& docId) {
    uint32_t retval = _indices.size();
    _indices.push_back(AtomicEntryRef());
    docId = retval;
}

void MultiValueMappingBase::reserve(uint32_t lidLimit) {
...
vespalib::MemoryUsage MultiValueMappingBase::getMemoryUsage() const {
