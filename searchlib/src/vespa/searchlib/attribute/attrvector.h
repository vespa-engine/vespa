...
class AttrVector {
public:
    template <bool MULTI> struct Features {
...
template <typename B> class NumericDirectAttribute : public B {
private:
...
template <typename F, typename B> class NumericDirectAttrVector : public search::NumericDirectAttribute<B> {
protected:
    using DocId = typename B::DocId;
...
public:
    ...
    uint32_t get(DocId doc, largeint_t* v, uint32_t sz) const override {
        return getAllHelper<largeint_t, largeint_t>(doc, v, sz);
    }
    uint32_t get(DocId doc, double* v, uint32_t sz) const override {
        return getAllHelper<double, double>(doc, v, sz);
    }
...
private:
    ...
    template <typename T, typename C> uint32_t getAllHelper(DocId doc, T* v, uint32_t sz) const {
    ...
    template <typename T> uint32_t getAllEnumHelper(DocId doc, T* v, uint32_t sz) const {
        uint32_t available(getValueCountHelper(doc));
        uint32_t num2Read(std::min(available, sz));
        for (uint32_t i(0); i < num2Read; i++) {
    ...
    uint32_t get(DocId doc, WeightedInt* v, uint32_t sz) const override {
        return getAllHelper<WeightedInt, largeint_t>(doc, v, sz);
    }
    uint32_t get(DocId doc, WeightedFloat* v, uint32_t sz) const override {
        return getAllHelper<WeightedFloat, double>(doc, v, sz);
    }
...
template <typename F> class StringDirectAttrVector : public search::StringDirectAttribute {

public:
    StringDirectAttrVector(const std::string& baseFileName);
    StringDirectAttrVector(const std::string& baseFileName, const Config& c);
    uint32_t get(DocId doc, const char** v, uint32_t sz) const override { return getAllHelper(doc, v, sz); }
