 ...
 */
template <typename DataT> class PostingListMerger {
    ...
    PostingVector& merge(PostingVector& v, PostingVector& temp, const StartVector& startPos)
...
public:
    ...
    template <typename PostingListType> void addToBitVector(const PostingListType& postingList) {
        BitVector& bv = *_bitVector;
        uint32_t   limit = _docIdLimit;
        postingList.foreach_key([&bv, limit](uint32_t key) {
            if (__builtin_expect(key < limit, true)) {
                bv.setBit(key);
            }
        });
    }
