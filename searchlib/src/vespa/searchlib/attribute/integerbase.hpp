...
template <typename T> bool IntegerAttributeTemplate<T>::findEnum(const char* value, EnumHandle& e) const {
...
template <typename T>
std::vector<IEnumStore::EnumHandle> IntegerAttributeTemplate<T>::findFoldedEnums(const char* value) const {
...
template <typename T> bool IntegerAttributeTemplate<T>::is_sortable() const noexcept {
    return true;
}
