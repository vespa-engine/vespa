...
EnumeratedLoaderBase::EnumeratedLoaderBase(IEnumStore& store) : _store(store), _indexes(), _enum_value_remapping() {
}
...
void EnumeratedLoaderBase::free_unused_values() {
    _store.free_unused_values();
}

void EnumeratedLoaderBase::build_enum_value_remapping() {
    ...
    std::sort(sortdata.begin(), sortdata.end(),
              [&comp](auto lhs, auto rhs) { return comp.less(lhs.first, rhs.first); });
...
EnumeratedLoader::EnumeratedLoader(IEnumStore& store) : EnumeratedLoaderBase(store), _enums_histogram() {
}
...
void EnumeratedLoader::build_dictionary() {
...
bool EnumeratedPostingsLoader::is_folded_change(Index lhs, Index rhs) const {
    return !_has_btree_dictionary || _store.is_folded_change(lhs, rhs);
}

void EnumeratedPostingsLoader::set_ref_count(Index idx, uint32_t ref_count) {
    _store.set_ref_count(idx, ref_count);
}

std::span<vespalib::datastore::EntryRef> EnumeratedPostingsLoader::initialize_empty_posting_indexes() {
    EntryRefVector(_indexes.size(), EntryRef()).swap(_posting_indexes);
    return _posting_indexes;
}

void EnumeratedPostingsLoader::build_dictionary() {
...
void EnumeratedPostingsLoader::build_empty_dictionary() {
    _store.setup_default_value_ref();
}
