...
typename MultiValueNumericAttribute<B, M>::T MultiValueNumericAttribute<B, M>::getFromEnum(EnumHandle e) const {
    (void)e;
    return 0;
}

template <typename B, typename M> bool MultiValueNumericAttribute<B, M>::findEnum(T value, EnumHandle& e) const {
    (void)value;
    (void)e;
    return false;
}
...
template <typename B, typename M>
MultiValueNumericAttribute<B, M>::MultiValueNumericAttribute(const std::string&             baseFileName,
                                                             const AttributeVector::Config& c)
    : MultiValueAttribute<B, M>(baseFileName, c) {
}

template <typename B, typename M> uint32_t MultiValueNumericAttribute<B, M>::getValueCount(DocId doc) const {
    if (doc >= B::getNumDocs()) {
        return 0;
    }
...
template <typename B, typename M> void MultiValueNumericAttribute<B, M>::onCommit() {
...
void MultiValueNumericAttribute<B, M>::onUpdateStat(CommitParam::UpdateStats updateStats) {
    ...
    this->updateStatistics(this->_mvMapping.getTotalValueCnt(), this->_mvMapping.getTotalValueCnt(),
                           usage.allocatedBytes(), usage.usedBytes(), usage.deadBytes(),
                           usage.allocatedBytesOnHold());
}

template <typename B, typename M> void MultiValueNumericAttribute<B, M>::clearOldValues(DocId doc) {
    (void)doc;
}
...
template <typename B, typename M> bool MultiValueNumericAttribute<B, M>::onLoadEnumerated(ReaderBase& attrReader) {
    ...
    uint32_t           maxvc = attribute::loadFromEnumeratedMultiValue(
        this->_mvMapping, attrReader, map, std::span<const uint32_t>(), attribute::NoSaveLoadedEnum());
    this->checkSetMaxValueCount(maxvc);

    return true;
}

template <typename B, typename M> bool MultiValueNumericAttribute<B, M>::onLoad(vespalib::Executor*) {
    ...
    for (DocId doc = 0; doc < numDocs; ++doc) {
        ...
        for (uint32_t i(0); i < valueCount; i++) {
            ...
            values.emplace_back(
                multivalue::ValueBuilder<M>::build(currData, hasWeight ? attrReader.getNextWeight() : 1));
        }
...
MultiValueNumericAttribute<B, M>::getSearch(QueryTermSimple::UP                   qTerm,
    ...
    (void)params;
    return std::make_unique<attribute::MultiNumericSearchContext<T, M>>(
        std::move(qTerm), *this, this->_mvMapping.make_read_view(this->getCommittedDocIdLimit()));
}

template <typename B, typename M>
std::unique_ptr<AttributeSaver> MultiValueNumericAttribute<B, M>::onInitSave(std::string_view fileName) {
...
template <typename B, typename M> bool MultiValueNumericAttribute<B, M>::is_sortable() const noexcept {
    return true;
}
...
public:
    MultiNumericSortBlobWriter(const MultiValueMappingT& mv_mapping, common::sortspec::MissingPolicy policy,
                               T missing_value)
        : _mv_mapping(mv_mapping), _writer(policy, missing_value, true) {}
...
MultiValueNumericAttribute<B, M>::make_sort_blob_writer(bool ascending, const common::BlobConverter*,
    ...
    if (ascending) {
        return std::make_unique<MultiNumericSortBlobWriter<MultiValueMapping, T, true>>(this->_mvMapping, policy,
                                                                                        missing_num);
    } else {
        return std::make_unique<MultiNumericSortBlobWriter<MultiValueMapping, T, false>>(this->_mvMapping, policy,
                                                                                         missing_num);
    }
}
