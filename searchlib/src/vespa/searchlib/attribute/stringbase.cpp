...
StringAttribute::StringAttribute(const std::string& name, const Config& c)
    : AttributeVector(name, c), _changes(), _defaultValue(ChangeBase::UPDATE, 0, std::string("")) {
}
...
double StringAttribute::getFloat(DocId doc) const {
    return vespalib::locale::c::strtod(get(doc), nullptr);
}

std::span<const char> StringAttribute::get_raw(DocId doc) const {
...
bool StringAttribute::is_sortable() const noexcept {
    return true;
}

namespace {

template <bool asc> class SingleStringSortBlobWriter : public attribute::ISortBlobWriter {
...
SingleStringSortBlobWriter<asc>::SingleStringSortBlobWriter(const StringAttribute&          attr,
    ...
    : _attr(attr), _writer(bc, policy, missing_value, false) {
}

template <bool asc> SingleStringSortBlobWriter<asc>::~SingleStringSortBlobWriter() = default;

template <bool asc> long SingleStringSortBlobWriter<asc>::write(uint32_t docid, void* ser_to, long available) {
...
StringAttribute::make_sort_blob_writer(bool ascending, const common::BlobConverter* bc,
                                       common::sortspec::MissingPolicy policy, std::string_view missing_value) const {
    if (ascending) {
...
bool StringAttribute::onLoadEnumerated(ReaderBase& attrReader) {
...
bool StringAttribute::onLoad(vespalib::Executor*) {
...
bool StringAttribute::onAddDoc(DocId) {
    return false;
}
...
void StringAttribute::load_enumerated_data(ReaderBase&, enumstore::EnumeratedPostingsLoader&, size_t) {
    LOG_ABORT("Should not be reached");
}

void StringAttribute::load_enumerated_data(ReaderBase&, enumstore::EnumeratedLoader&) {
    LOG_ABORT("Should not be reached");
}

void StringAttribute::load_posting_lists_and_update_enum_store(enumstore::EnumeratedPostingsLoader&) {
    LOG_ABORT("Should not be reached");
}

vespalib::MemoryUsage StringAttribute::getChangeVectorMemoryUsage() const {
    return _changes.getMemoryUsage();
}

bool StringAttribute::get_match_is_cased() const noexcept {
    return getConfig().get_match() == attribute::Config::Match::CASED;
}

bool StringAttribute::has_uncased_matching() const noexcept {
...
template bool AttributeVector::append(StringAttribute::ChangeVector& changes, DocId doc, const StringChangeData& v,
