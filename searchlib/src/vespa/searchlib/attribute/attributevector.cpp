...
{
    if (param._firstSerialNum < getStatus().getLastSyncToken()) {
        LOG(error, "Expected first token to be >= %" PRIu64 ", got %" PRIu64 ".",
            getStatus().getLastSyncToken(), param._firstSerialNum);
        LOG_ABORT("should not be reached");
    }
    commit(param._forceUpdateStats);
    _status.setLastSyncToken(param._lastSerialNum);
}
