...
bool allow_paged(const search::attribute::Config& config) {
    if (!config.paged()) {
        return false;
    }
...
std::unique_ptr<vespalib::alloc::MemoryAllocator> make_memory_allocator(const std::string&               name,
...
AttributeVector::AttributeVector(std::string_view baseFileName, const Config& c)
    : _baseFileName(baseFileName),
      ...
      _initialization_status(
          std::make_shared<search::attribute::AttributeInitializationStatus>(getName())) // object is only temporary
{
}

AttributeVector::~AttributeVector() = default;

void AttributeVector::updateStat(CommitParam::UpdateStats updateStats) {
...
bool AttributeVector::hasEnum() const {
    return _hasEnum;
}
uint32_t AttributeVector::getMaxValueCount() const {
    return _highestValueCount.load(std::memory_order_relaxed);
}
bool AttributeVector::hasMultiValue() const {
    return _config->collectionType().isMultiValue();
}
bool AttributeVector::hasWeightedSetType() const {
    return _config->collectionType().isWeightedSet();
}
size_t AttributeVector::getFixedWidth() const {
    return _config->basicType().fixedSize();
}
attribute::BasicType AttributeVector::getInternalBasicType() const {
    return _config->basicType();
}
attribute::CollectionType AttributeVector::getInternalCollectionType() const {
    return _config->collectionType();
}
bool AttributeVector::hasArrayType() const {
    return _config->collectionType().isArray();
}
bool AttributeVector::getIsFilter() const {
    return _config->getIsFilter();
}
bool AttributeVector::getIsFastSearch() const {
    return _config->fastSearch();
}
bool AttributeVector::isMutable() const {
    return _config->isMutable();
}
attribute::BasicType::Type AttributeVector::getBasicType() const {
    return _config->basicType().type();
}
attribute::CollectionType::Type AttributeVector::getCollectionType() const {
    return _config->collectionType().type();
}

bool AttributeVector::isEnumerated(const vespalib::GenericHeader& header) {
    return header.hasTag(enumeratedTag) && header.getTag(enumeratedTag).asInteger() != 0;
}

void AttributeVector::commit(CommitParam::UpdateStats updateStats) {
    onCommit();
...
void AttributeVector::commit(const CommitParam& param) {
    if (param.firstSerialNum() < getStatus().getLastSyncToken()) {
        LOG(error, "Expected first token to be >= %" PRIu64 ", got %" PRIu64 ".", getStatus().getLastSyncToken(),
            param.firstSerialNum());
...
bool AttributeVector::addDocs(DocId& startDoc, DocId& lastDoc, uint32_t numDocs) {
    if (numDocs != 0) {
...
bool AttributeVector::addDocs(uint32_t numDocs) {
    DocId doc;
    return addDocs(doc, doc, numDocs);
}

void AttributeVector::incGeneration() {
...
void AttributeVector::updateStatistics(uint64_t numValues, uint64_t numUniqueValue, uint64_t allocated, uint64_t used,
                                       uint64_t dead, uint64_t onHold) {
    _status.updateStatistics(numValues, numUniqueValue, allocated, used, dead, onHold);
}

void AttributeVector::updateSizes(uint64_t numValues, uint64_t numUniqueValue) {
    _status.updateSizes(numValues, numUniqueValue);
}

vespalib::MemoryUsage AttributeVector::getEnumStoreValuesMemoryUsage() const {
    return vespalib::MemoryUsage();
}

void AttributeVector::populate_address_space_usage(AddressSpaceUsage& usage) const {
...
AddressSpaceUsage AttributeVector::getAddressSpaceUsage() const {
    AddressSpaceUsage usage;
    populate_address_space_usage(usage);
    return usage;
}

bool AttributeVector::isImported() const {
    return false;
}

bool AttributeVector::headerTypeOK(const vespalib::GenericHeader& header) const {
    return header.hasTag(dataTypeTag) && header.hasTag(collectionTypeTag) && header.hasTag(docIdLimitTag) &&
           header.getTag(dataTypeTag).asString() == getConfig().basicType().asString() &&
           header.getTag(collectionTypeTag).asString() == getConfig().collectionType().asString();
}

void AttributeVector::reclaim_memory(Generation oldest_used_gen) {
    (void)oldest_used_gen;
}
void AttributeVector::before_inc_generation(Generation current_gen) {
    (void)current_gen;
}
const IEnumStore* AttributeVector::getEnumStoreBase() const {
    return nullptr;
}
IEnumStore* AttributeVector::getEnumStoreBase() {
    return nullptr;
}
const attribute::MultiValueMappingBase* AttributeVector::getMultiValueBase() const {
    return nullptr;
}

bool AttributeVector::save(std::string_view fileName) {
...
attribute::AttributeHeader AttributeVector::createAttributeHeader(std::string_view fileName) const {
    return attribute::AttributeHeader(
        std::string(fileName), getConfig().basicType(), getConfig().collectionType(), getConfig().tensorType(),
        getEnumeratedSave(), getConfig().predicateParams(), getConfig().hnsw_index_params(), getCommittedDocIdLimit(),
        getUniqueValueCount(), getTotalValueCount(), getStatus().get_used_minus_dead_and_onhold(),
        getCreateSerialNum(), getVersion());
}

bool AttributeVector::hasLoadData() const {
...
bool AttributeVector::isEnumeratedSaveFormat() const {
    ...
    if (!datFile.OpenReadOnly(datName.c_str())) {
        ...
        throw IllegalStateException(
            fmt("Failed opening attribute data file '%s' for reading", datFile.GetFileName()));
    }
...
bool AttributeVector::onLoad(vespalib::Executor*) {
    return false;
}
int32_t AttributeVector::getWeight(DocId, uint32_t) const {
    return 1;
}

bool AttributeVector::findEnum(const char*, EnumHandle&) const {
    return false;
}

std::vector<search::attribute::IAttributeVector::EnumHandle> AttributeVector::findFoldedEnums(const char*) const {
    std::vector<EnumHandle> empty;
    return empty;
}

const char* AttributeVector::getStringFromEnum(EnumHandle) const {
    return nullptr;
}

std::unique_ptr<attribute::SearchContext> AttributeVector::getSearch(QueryPacketT               searchSpec,
...
bool AttributeVector::apply(DocId doc, const MapValueUpdate& map) {
...
bool AttributeVector::applyWeight(DocId, const FieldValue&, const ArithmeticValueUpdate&) {
    return false;
}

bool AttributeVector::applyWeight(DocId, const FieldValue&, const AssignValueUpdate&) {
    return false;
}

void AttributeVector::reclaim_unused_memory() {
    _genHandler.update_oldest_used_generation();
    reclaim_memory(_genHandler.get_oldest_used_generation());
}

void AttributeVector::divideByZeroWarning() {
    LOG(warning, "applyArithmetic(): "
...
void AttributeVector::set_reserved_doc_values() {
    uint32_t docId = 0;
    if (docId >= getNumDocs()) {
        return;
    }
...
attribute::IPostingListAttributeBase* AttributeVector::getIPostingListAttributeBase() {
    return nullptr;
}
const attribute::IPostingListAttributeBase* AttributeVector::getIPostingListAttributeBase() const {
    return nullptr;
}
const IDocidPostingStore* AttributeVector::as_docid_posting_store() const {
    return nullptr;
}
const IDocidWithWeightPostingStore* AttributeVector::as_docid_with_weight_posting_store() const {
    return nullptr;
}
const tensor::ITensorAttribute* AttributeVector::asTensorAttribute() const {
    return nullptr;
}
const attribute::IMultiValueAttribute* AttributeVector::as_multi_value_attribute() const {
    return nullptr;
}
bool AttributeVector::hasPostings() {
    return getIPostingListAttributeBase() != nullptr;
}
uint64_t AttributeVector::getUniqueValueCount() const {
    return getTotalValueCount();
}
uint64_t AttributeVector::getTotalValueCount() const {
    return getNumDocs();
}
void AttributeVector::setCreateSerialNum(uint64_t createSerialNum) {
    _createSerialNum = createSerialNum;
}
uint64_t AttributeVector::getCreateSerialNum() const {
    return _createSerialNum;
}
uint32_t AttributeVector::getVersion() const {
    return 0;
}
...
attribute::EnumModifier AttributeVector::getEnumModifier() {
    attribute::InterlockGuard interlockGuard(*_interlock);
    return attribute::EnumModifier(_enumLock, interlockGuard);
}

attribute::ValueModifier AttributeVector::getValueModifier() {
...
std::unique_ptr<AttributeSaver> AttributeVector::initSave(std::string_view fileName) {
    commit(CommitParam::UpdateStats::FORCE);
    return onInitSave(fileName);
}

std::unique_ptr<AttributeSaver> AttributeVector::onInitSave(std::string_view) {
    LOG_ABORT("should not be reached");
    return std::unique_ptr<AttributeSaver>();
}

bool AttributeVector::hasActiveEnumGuards() {
...
IExtendAttribute* AttributeVector::getExtendInterface() {
    return nullptr;
}

uint64_t AttributeVector::getEstimatedSaveByteSize() const {
...
std::unique_ptr<attribute::AttributeReadGuard> AttributeVector::makeReadGuard(bool stableEnumGuard) const {
    return std::make_unique<ReadGuard>(this, _genHandler.takeGuard(), stableEnumGuard ? &_enumLock : nullptr);
}

vespalib::MemoryUsage AttributeVector::getChangeVectorMemoryUsage() const {
    return vespalib::MemoryUsage(0, 0, 0, 0);
}

bool AttributeVector::commitIfChangeVectorTooLarge() {
...
void AttributeVector::drain_hold(uint64_t hold_limit) {
    incGeneration();
...
vespalib::alloc::Alloc AttributeVector::get_initial_alloc() {
    return (_memory_allocator ? vespalib::alloc::Alloc::alloc_with_allocator(_memory_allocator.get())
                              : vespalib::alloc::Alloc::alloc());
}

void AttributeVector::set_size_on_disk(uint64_t value) noexcept {
    _size_on_disk.store(value, std::memory_order_release);
}

void AttributeVector::set_size_on_disk(const IAttributeSaveTarget& target) {
...
uint64_t AttributeVector::get_memory_allocator_size_on_disk() const noexcept {
...
template bool AttributeVector::append<StringChangeData>(ChangeVectorT<ChangeTemplate<StringChangeData>>& changes,
                                                        uint32_t, const StringChangeData&, int32_t, bool);
template bool AttributeVector::update<StringChangeData>(ChangeVectorT<ChangeTemplate<StringChangeData>>& changes,
