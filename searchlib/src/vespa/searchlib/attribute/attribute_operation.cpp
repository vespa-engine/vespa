...
template <> struct Mod<double> {
    using V = double;
    explicit Mod(double) {}
    double operator()(double oldVal) const { return oldVal; }
};

template <> struct Mod<float> {
    using V = float;
    explicit Mod(float) {}
...
template <typename T, typename OP> struct UpdateFast {
    using A = SingleValueNumericAttribute<T>;
    using F = OP;
    A* attr;
    F  op;
    using ValueType = typename T::LoadedValueType;
    UpdateFast(IAttributeVector& attr_in, typename F::V operand) : attr(dynamic_cast<A*>(&attr_in)), op(operand) {}
    void operator()(uint32_t docid) { attr->set(docid, op(attr->getFast(docid))); }
    bool valid() const { return (attr != nullptr) && (attr->isMutable()); }
};

template <typename OP> class OperateOverResultSet : public AttributeOperation {
public:
    OperateOverResultSet(FullResult&& result, typename OP::F::V operand)
        : _operand(operand), _result(std::move(result)) {}
...
template <typename OP> class OperateOverHits : public AttributeOperation {
public:
    using Hit = AttributeOperation::Hit;
    OperateOverHits(std::vector<Hit> reRanked, typename OP::F::V operand)
        : _operand(operand), _reRanked(std::move(reRanked)) {}
...
template <typename OP> class OperateOverDocIds : public AttributeOperation {
public:
    OperateOverDocIds(std::vector<uint32_t> docIds, typename OP::F::V operand)
        : _operand(operand), _docIds(std::move(docIds)) {}
...
struct Operation {
    ...
    template <typename V> std::unique_ptr<AttributeOperation> create(BasicType type, V vector) const;
    template <typename IT, typename V> std::unique_ptr<AttributeOperation> create(V vector) const;
...
Operation Operation::create(std::string_view s) {
    Type op = Type::BAD;
    if (s.size() >= 2) {
        if ((s[0] == '+') && (s[1] == '+')) {
            op = Type::INC;
...
std::unique_ptr<AttributeOperation> createOperation(std::vector<uint32_t> vector, T operand) {
    return std::make_unique<OperateOverDocIds<OP>>(std::move(vector), operand);
}

template <typename T, typename OP>
std::unique_ptr<AttributeOperation> createOperation(std::vector<AttributeOperation::Hit> vector, T operand) {
    return std::make_unique<OperateOverHits<OP>>(std::move(vector), operand);
}

template <typename T, typename OP>
std::unique_ptr<AttributeOperation> createOperation(AttributeOperation::FullResult&& result, T operand) {
    return std::make_unique<OperateOverResultSet<OP>>(std::move(result), operand);
}

template <typename T_IN, typename V> std::unique_ptr<AttributeOperation> Operation::create(V vector) const {
...
template <typename V> std::unique_ptr<AttributeOperation> Operation::create(BasicType type, V hits) const {
    if (!valid()) {
        return {};
    }
    switch (type.type()) {
...
std::unique_ptr<AttributeOperation> AttributeOperation::create(BasicType type, std::string_view operation,
                                                               Hits docs) {
    Operation op = Operation::create(operation);
    return op.create<Hits>(type, std::move(docs));
}

template std::unique_ptr<AttributeOperation> AttributeOperation::create(BasicType, std::string_view,
                                                                        std::vector<uint32_t>);

template std::unique_ptr<AttributeOperation> AttributeOperation::create(BasicType, std::string_view,
                                                                        std::vector<Hit>);

template std::unique_ptr<AttributeOperation> AttributeOperation::create(BasicType, std::string_view, FullResult);
