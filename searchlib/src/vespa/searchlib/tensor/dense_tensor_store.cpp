...
size_t cap_max_entries(size_t max_entries, size_t max_buffer_size, size_t entry_size) {
...
DenseTensorStore::TensorSizeCalc::TensorSizeCalc(const ValueType& type)
    : _numCells(1u), _cell_type(type.cell_type()), _aligned_size(0u) {
...
DenseTensorStore::BufferType::BufferType(const TensorSizeCalc&                             tensorSizeCalc,
    ...
    : vespalib::datastore::BufferType<char>(
          tensorSizeCalc.alignedSize(), MIN_BUFFER_ARRAYS,
          cap_max_entries(RefType::offsetSize(), max_dense_tensor_buffer_size, tensorSizeCalc.alignedSize())),
      _allocator(std::move(allocator)) {
}

DenseTensorStore::BufferType::~BufferType() = default;

void DenseTensorStore::BufferType::clean_hold(void* buffer, size_t offset, EntryCount num_entries, CleanContext) {
    auto num_elems = num_entries * getArraySize();
    memset(static_cast<char*>(buffer) + offset * getArraySize(), 0, num_elems);
}

const vespalib::alloc::MemoryAllocator* DenseTensorStore::BufferType::get_memory_allocator() const {
...
Handle<char> DenseTensorStore::allocRawBuffer() {
...
void DenseTensorStore::holdTensor(EntryRef ref) {
    if (!ref.valid()) {
        return;
    }
...
TensorStore::EntryRef DenseTensorStore::move_on_compact(EntryRef ref) {
    if (!ref.valid()) {
        return RefType();
    }
...
vespalib::MemoryUsage DenseTensorStore::update_stat(const CompactionStrategy& compaction_strategy) {
...
std::unique_ptr<ICompactionContext> DenseTensorStore::start_compact(const CompactionStrategy& compaction_strategy) {
...
EntryRef DenseTensorStore::store_encoded_tensor(vespalib::nbostream& encoded) {
    (void)encoded;
    abort();
}

std::unique_ptr<Value> DenseTensorStore::get_tensor(EntryRef ref) const {
    if (!ref.valid()) {
        return {};
    }
...
bool DenseTensorStore::encode_stored_tensor(EntryRef ref, vespalib::nbostream& target) const {
    (void)ref;
    (void)target;
    abort();
}

const DenseTensorStore* DenseTensorStore::as_dense() const {
    return this;
}

DenseTensorStore* DenseTensorStore::as_dense() {
    return this;
}
