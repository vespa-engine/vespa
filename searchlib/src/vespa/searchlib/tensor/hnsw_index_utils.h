...
struct HnswTraversalCandidate {
    ...
    HnswTraversalCandidate(uint32_t nodeid_in, vespalib::datastore::EntryRef levels_ref_in,
                           double distance_in) noexcept
        : nodeid(nodeid_in), levels_ref(levels_ref_in), distance(distance_in) {}
    HnswTraversalCandidate(uint32_t nodeid_in, uint32_t docid_in, vespalib::datastore::EntryRef levels_ref_in,
                           double distance_in) noexcept
        : nodeid(nodeid_in), levels_ref(levels_ref_in), distance(distance_in) {
        (void)docid_in;
    }
...
struct HnswCandidate : public HnswTraversalCandidate {
    ...
    HnswCandidate(uint32_t nodeid_in, uint32_t docid_in, vespalib::datastore::EntryRef levels_ref_in,
                  double distance_in) noexcept
        : HnswTraversalCandidate(nodeid_in, docid_in, levels_ref_in, distance_in), docid(docid_in) {}
};
