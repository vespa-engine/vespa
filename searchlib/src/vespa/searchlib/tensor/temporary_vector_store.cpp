...
std::span<const ToType> convert_cells(std::span<ToType> space, TypedCells cells) noexcept __attribute__((noinline));

template <typename FromType, typename ToType>
std::span<const ToType> convert_cells(std::span<ToType> space, TypedCells cells) noexcept {
...
template <>
std::span<const float> convert_cells<vespalib::BFloat16, float>(std::span<float> space, TypedCells cells) noexcept {
    vespalib::hwaccelerated::convert_bfloat16_to_float(reinterpret_cast<const uint16_t*>(cells.data), space.data(),
                                                       space.size());
    return space;
}

template <typename ToType> struct ConvertCellsSelector {
...
std::span<const FloatType> TemporaryVectorStore<FloatType>::internal_convert(TypedCells cells,
