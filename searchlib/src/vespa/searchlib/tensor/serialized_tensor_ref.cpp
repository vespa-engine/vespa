...
SerializedTensorRef::SerializedTensorRef(VectorBundle vectors, uint32_t num_mapped_dimensions,
                                         std::span<const vespalib::string_id> labels)
    : _vectors(vectors), _num_mapped_dimensions(num_mapped_dimensions), _labels(labels) {
}
