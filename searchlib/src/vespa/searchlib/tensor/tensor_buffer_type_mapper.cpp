...
TensorBufferTypeMapper::TensorBufferTypeMapper() : vespalib::datastore::ArrayStoreTypeMapper(), _ops(nullptr) {
}

TensorBufferTypeMapper::TensorBufferTypeMapper(uint32_t max_small_subspaces_type_id, double grow_factor,
                                               size_t max_buffer_size, TensorBufferOperations* ops)
    : vespalib::datastore::ArrayStoreTypeMapper(), _ops(ops) {
    ...
    for (uint32_t type_id = 1; type_id <= max_small_subspaces_type_id; ++type_id) {
        if (type_id > 1) {
            num_subspaces =
                std::max(num_subspaces + 1, static_cast<uint32_t>(std::floor(num_subspaces * grow_factor)));
        }
        ...
        if (array_size > std::numeric_limits<uint32_t>::max() || array_size >= 2 * max_buffer_size) {
            break;
        }
