...
void
HnswIndex<type>::search_layer_helper(const BoundDistanceFunction &df, uint32_t neighbors_to_find, double adaptive_beam_search_slack,
                                     ...
                                     uint32_t estimated_visited_nodes) const
{
    ...
    while (!candidates.empty()) {
        ...
        if (cand.distance > (1.0 + adaptive_beam_search_slack) * limit_dist) {
            break;
        }
        candidates.pop();
        for (uint32_t neighbor_nodeid : _graph.get_link_array(cand.levels_ref, level)) {
            ...
            if (dist_to_input < (1.0 + adaptive_beam_search_slack) * limit_dist) {
...
template <class VisitedTracker, class BestNeighbors>
void
HnswIndex<type>::search_layer_filter_first_helper(const BoundDistanceFunction &df, uint32_t neighbors_to_find, double adaptive_beam_search_slack,
...
void
HnswIndex<type>::search_layer(const BoundDistanceFunction &df, uint32_t neighbors_to_find, double adaptive_beam_search_slack, BestNeighbors& best_neighbors,
                              uint32_t level, const vespalib::Doom* const doom, const GlobalFilter *filter) const
{
    ...
    if (estimated_visited_nodes >= nodeid_limit / 128) {
        search_layer_helper<BitVectorVisitedTracker>(df, neighbors_to_find, adaptive_beam_search_slack, best_neighbors, level, filter, nodeid_limit, doom, estimated_visited_nodes);
    } else {
        search_layer_helper<HashSetVisitedTracker>(df, neighbors_to_find, adaptive_beam_search_slack, best_neighbors, level, filter, nodeid_limit, doom, estimated_visited_nodes);
    }
}
...
template <class BestNeighbors>
void
HnswIndex<type>::search_layer_filter_first(const BoundDistanceFunction &df, uint32_t neighbors_to_find, double adaptive_beam_search_slack, BestNeighbors& best_neighbors, double exploration,
                                           uint32_t level, const vespalib::Doom* const doom, const GlobalFilter *filter) const
{
    uint32_t nodeid_limit = _graph.nodes_size.load(std::memory_order_acquire);
    uint32_t estimated_visited_nodes = estimate_visited_nodes(level, nodeid_limit, neighbors_to_find, filter);
    if (estimated_visited_nodes >= nodeid_limit / 128) {
        search_layer_filter_first_helper<BitVectorVisitedTracker>(df, neighbors_to_find, adaptive_beam_search_slack, best_neighbors, exploration, level, filter, nodeid_limit, doom, estimated_visited_nodes);
    } else {
        search_layer_filter_first_helper<HashSetVisitedTracker>(df, neighbors_to_find, adaptive_beam_search_slack, best_neighbors, exploration, level, filter, nodeid_limit, doom, estimated_visited_nodes);
    }
}
...
{
    SearchBestNeighbors candidates = top_k_candidates(df, std::max(k, explore_k), adaptive_beam_search_slack, filter, low_hit_ratio, exploration, doom);
...
template <HnswIndexType type>
std::vector<NearestNeighborIndex::Neighbor>
HnswIndex<type>::find_top_k(uint32_t k, const BoundDistanceFunction &df, uint32_t explore_k, double adaptive_beam_search_slack,
                            const vespalib::Doom& doom, double distance_threshold) const
{
    return top_k_by_docid(k, df, nullptr, false, 0.0, explore_k, adaptive_beam_search_slack, doom, distance_threshold);
}
...
HnswIndex<type>::find_top_k_with_filter(uint32_t k, const BoundDistanceFunction &df, const GlobalFilter &filter, bool low_hit_ratio, double exploration,
                                        uint32_t explore_k, double adaptive_beam_search_slack, const vespalib::Doom& doom, double distance_threshold) const
{
    return top_k_by_docid(k, df, &filter, low_hit_ratio, exploration, explore_k, adaptive_beam_search_slack, doom, distance_threshold);
}

template <HnswIndexType type>
typename HnswIndex<type>::SearchBestNeighbors
HnswIndex<type>::top_k_candidates(const BoundDistanceFunction &df, uint32_t k, double adaptive_beam_search_slack, const GlobalFilter *filter, bool low_hit_ratio, double exploration, const vespalib::Doom& doom) const
{
    ...
    if (filter && filter->is_active() && low_hit_ratio) {
        search_layer_filter_first(df, k, adaptive_beam_search_slack, best_neighbors, exploration, 0, &doom, filter);
    } else {
        search_layer(df, k, adaptive_beam_search_slack, best_neighbors, 0, &doom, filter);
    }
    return best_neighbors;
}
