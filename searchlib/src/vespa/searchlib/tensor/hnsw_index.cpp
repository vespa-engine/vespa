...
struct PairDist {
    ...
    uint32_t id_second;
    double   distance;
    PairDist(uint32_t i1, uint32_t i2, double d) noexcept : id_first(i1), id_second(i2), distance(d) {}
};
bool operator<(const PairDist& a, const PairDist& b) noexcept {
    return (a.distance < b.distance);
}

template <typename GraphType> struct NeighborLinkMeta {
...
ResilientAltFilterFirstContext::~ResilientAltFilterFirstContext() = default;
...
PreparedAddDoc::PreparedAddDoc(uint32_t docid_in, ReadGuard read_guard_in,
...
template <HnswIndexType type> ArrayStoreConfig HnswIndex<type>::make_default_level_array_store_config() {
    return LevelArrayStore::optimizedConfigForHugePage(
               max_level_array_size, vespalib::alloc::MemoryAllocator::HUGEPAGE_SIZE,
...
template <HnswIndexType type> ArrayStoreConfig HnswIndex<type>::make_default_link_array_store_config() {
    return LinkArrayStore::optimizedConfigForHugePage(
               max_link_array_size, vespalib::alloc::MemoryAllocator::HUGEPAGE_SIZE,
               vespalib::alloc::MemoryAllocator::NORMAL_PAGE_SIZE, ArrayStoreConfig::default_max_buffer_size,
               min_num_arrays_for_new_buffer, alloc_grow_factor)
        .enable_free_lists(true);
}

template <HnswIndexType type> uint32_t HnswIndex<type>::max_links_for_level(uint32_t level) const {
    return (level == 0) ? _cfg.max_links_at_level_0() : _cfg.max_links_on_inserts();
}

template <HnswIndexType type>
bool HnswIndex<type>::have_closer_distance(HnswTraversalCandidate              candidate,
                                           const HnswTraversalCandidateVector& result) const {
...
SelectResult HnswIndex<type>::select_neighbors_simple(const HnswCandidateVectorT& neighbors,
...
template <HnswIndexType type>
template <typename HnswCandidateVectorT>
SelectResult HnswIndex<type>::select_neighbors_heuristic(const HnswCandidateVectorT& neighbors,
                                                         uint32_t                    max_links) const {
    SelectResult result;
    NearestPriQ  nearest;
...
template <HnswIndexType type>
template <typename HnswCandidateVectorT>
SelectResult HnswIndex<type>::select_neighbors(const HnswCandidateVectorT& neighbors, uint32_t max_links) const {
...
template <HnswIndexType type> void HnswIndex<type>::shrink_if_needed(uint32_t nodeid, uint32_t level) {
...
template <HnswIndexType type>
void HnswIndex<type>::connect_new_node(uint32_t nodeid, const LinkArrayRef& neighbors, uint32_t level) {
...
template <HnswIndexType type>
void HnswIndex<type>::remove_link_to(uint32_t remove_from, uint32_t remove_id, uint32_t level) {
    LinkArray new_links;
...
double calc_distance_helper(const BoundDistanceFunction& df, vespalib::eval::TypedCells rhs) {
...
template <HnswIndexType type>
double HnswIndex<type>::calc_distance(const BoundDistanceFunction& df, uint32_t rhs_nodeid) const {
    auto rhs = get_vector(rhs_nodeid);
    return calc_distance_helper(df, rhs);
}

template <HnswIndexType type>
double HnswIndex<type>::calc_distance(const BoundDistanceFunction& df, uint32_t rhs_docid,
                                      uint32_t rhs_subspace) const {
    auto rhs = get_vector(rhs_docid, rhs_subspace);
    return calc_distance_helper(df, rhs);
}

template <HnswIndexType type>
uint32_t HnswIndex<type>::estimate_visited_nodes(uint32_t level, uint32_t nodeid_limit, uint32_t neighbors_to_find,
...
template <HnswIndexType type>
HnswCandidate HnswIndex<type>::find_nearest_in_layer(Stats& stats, const BoundDistanceFunction& df,
    ...
    while (keep_searching) {
        ...
        for (uint32_t neighbor_nodeid : _graph.get_link_array(nearest.levels_ref, level)) {
            ...
            if (_graph.still_valid(neighbor_nodeid, neighbor_ref) && dist < nearest.distance) {
...
void HnswIndex<type>::search_layer_helper(Stats& stats, const BoundDistanceFunction& df, uint32_t neighbors_to_find,
                                          double exploration_slack, bool prefetch_tensors,
                                          BestNeighbors& best_neighbors, uint32_t level, const GlobalFilter* filter,
                                          uint32_t nodeid_limit, const vespalib::Deadline* const deadline,
    ...
    while (!candidates.empty()) {
        ...
        for (const auto neighbor_nodeid : link_array) {
            ...
            neighbor_link_metas.emplace_back(neighbor_nodeid, neighbor_ref, neighbor_docid, neighbor_subspace);
...
template <HnswIndexType type>
template <class VisitedTracker, class BestNeighbors>
void HnswIndex<type>::search_layer_filter_first_helper(Stats& stats, const BoundDistanceFunction& df,
...
template <HnswIndexType type>
template <class VisitedTracker, class BestNeighbors>
void HnswIndex<type>::search_layer_resilient_filter_first_helper(Stats& stats, const BoundDistanceFunction& df,
                                                                 uint32_t neighbors_to_find, double exploration_slack,
                                                                 bool prefetch_tensors, BestNeighbors& best_neighbors,
...
template <HnswIndexType type>
template <class VisitedTracker, class BestNeighbors>
void HnswIndex<type>::search_layer_resilient_alt_filter_first_helper(
    Stats& stats, const BoundDistanceFunction& df, uint32_t neighbors_to_find, double exploration_slack,
    bool prefetch_tensors, BestNeighbors& best_neighbors, double exploration, uint32_t level,
    const GlobalFilter* filter, uint32_t nodeid_limit, const vespalib::Deadline* const deadline,
    uint32_t estimated_visited_nodes) const {
    assert(filter);
    NearestPriQ                         candidates;
    internal::GlobalFilterWrapper<type> filter_wrapper(filter);
    ...
    ResilientAltFilterFirstContext           context(max_links_for_level(level));
    std::vector<NeighborLinkMeta<GraphType>> neighbor_link_metas;
    neighbor_link_metas.reserve(max_links_for_level(level));
    ...
    while (!candidates.empty()) {
        ...
        // neighbors)
        context.clear();
        explore_neighborhood_resilient_alt(stats, cand, context, visited, exploration, level, filter_wrapper,
                                           nodeid_limit, best_neighbors.size() >= neighbors_to_find);
        neighbor_link_metas.clear();
...
template <HnswIndexType type>
template <class VisitedTracker>
void HnswIndex<type>::explore_neighborhood_resilient_alt(Stats& stats, HnswTraversalCandidate& cand,
                                                         ResilientAltFilterFirstContext& context,
...
template <HnswIndexType type>
template <class VisitedTracker>
void HnswIndex<type>::exploreNeighborhood(Stats& stats, HnswTraversalCandidate& cand, std::deque<uint32_t>& found,
                                          VisitedTracker& visited, double exploration, uint32_t level,
                                          const internal::GlobalFilterWrapper<type>& filter_wrapper,
    ...
    exploreNeighborhoodByOneHop(stats, todo, found, visited, level, filter_wrapper, nodeid_limit,
                                max_neighbors_to_find);

    // Explore 2-hop neighbors
    exploreNeighborhoodByOneHop(stats, todo, found, visited, level, filter_wrapper, nodeid_limit,
                                max_neighbors_to_find);

    // Explore 3-hop neighbors, but only if the 2-hop neighborhood is small, i.e., we are in a rather sparse part of
    // the graph.
...
template <HnswIndexType type>
template <class VisitedTracker>
void HnswIndex<type>::exploreNeighborhoodByOneHop(Stats& stats, std::deque<uint32_t>& todo,
                                                  std::deque<uint32_t>& found, VisitedTracker& visited,
                                                  uint32_t                                   level,
                                                  const internal::GlobalFilterWrapper<type>& filter_wrapper,
                                                  uint32_t nodeid_limit, uint32_t max_neighbors_to_find) const {
    // We do not explore the candidates that we newly add to the deque
    uint32_t nodesToExplore = todo.size();
    for (uint32_t nodesExplored = 0; nodesExplored < nodesToExplore && found.size() < max_neighbors_to_find;
         ++nodesExplored)
    {
        ...
        for (uint32_t neighbor_nodeid : _graph.get_link_array(ref, level)) {
            ...
            // Just explore everything in the next hop, even if the current node passes the filter or was marked as
            // visited
...
void HnswIndex<type>::search_layer(Stats& stats, const BoundDistanceFunction& df, uint32_t neighbors_to_find,
                                   double exploration_slack, bool prefetch_tensors, BestNeighbors& best_neighbors,
                                   uint32_t level, const vespalib::Deadline* const deadline,
                                   const GlobalFilter* filter) const {
    ...
    if (estimated_visited_nodes >= nodeid_limit / 128) {
        search_layer_helper<BitVectorVisitedTracker>(stats, df, neighbors_to_find, exploration_slack,
                                                     prefetch_tensors, best_neighbors, level, filter, nodeid_limit,
                                                     deadline, estimated_visited_nodes);
    } else {
        search_layer_helper<HashSetVisitedTracker>(stats, df, neighbors_to_find, exploration_slack, prefetch_tensors,
                                                   best_neighbors, level, filter, nodeid_limit, deadline,
...
template <HnswIndexType type>
template <class BestNeighbors>
void HnswIndex<type>::search_layer_filter_first(Stats& stats, const BoundDistanceFunction& df,
                                                uint32_t neighbors_to_find, double exploration_slack,
                                                bool prefetch_tensors, BestNeighbors& best_neighbors,
                                                double exploration, uint32_t level,
                                                const vespalib::Deadline* const deadline,
    ...
    uint32_t estimated_visited_nodes = estimate_visited_nodes(level, nodeid_limit, neighbors_to_find, filter);
    if (estimated_visited_nodes >= nodeid_limit / 128) {
        search_layer_filter_first_helper<BitVectorVisitedTracker>(
            stats, df, neighbors_to_find, exploration_slack, prefetch_tensors, best_neighbors, exploration, level,
            filter, nodeid_limit, deadline, estimated_visited_nodes);
    } else {
        search_layer_filter_first_helper<HashSetVisitedTracker>(
            stats, df, neighbors_to_find, exploration_slack, prefetch_tensors, best_neighbors, exploration, level,
            filter, nodeid_limit, deadline, estimated_visited_nodes);
    }
}
...
template <HnswIndexType type>
template <class BestNeighbors>
void HnswIndex<type>::search_layer_resilient_filter_first(Stats& stats, const BoundDistanceFunction& df,
                                                          uint32_t neighbors_to_find, double exploration_slack,
                                                          bool prefetch_tensors, BestNeighbors& best_neighbors,
                                                          double exploration, uint32_t level,
                                                          const vespalib::Deadline* const deadline,
    ...
    uint32_t estimated_visited_nodes = estimate_visited_nodes(level, nodeid_limit, neighbors_to_find, filter);
    if (estimated_visited_nodes >= nodeid_limit / 128) {
        search_layer_resilient_filter_first_helper<BitVectorVisitedTracker>(
            stats, df, neighbors_to_find, exploration_slack, prefetch_tensors, best_neighbors, exploration, level,
            filter, nodeid_limit, deadline, estimated_visited_nodes);
    } else {
        search_layer_resilient_filter_first_helper<HashSetVisitedTracker>(
            stats, df, neighbors_to_find, exploration_slack, prefetch_tensors, best_neighbors, exploration, level,
            filter, nodeid_limit, deadline, estimated_visited_nodes);
    }
}
...
template <HnswIndexType type>
template <class BestNeighbors>
void HnswIndex<type>::search_layer_resilient_alt_filter_first(Stats& stats, const BoundDistanceFunction& df,
                                                              uint32_t neighbors_to_find, double exploration_slack,
                                                              bool prefetch_tensors, BestNeighbors& best_neighbors,
                                                              double exploration, uint32_t level,
                                                              const vespalib::Deadline* const deadline,
    ...
    uint32_t estimated_visited_nodes = estimate_visited_nodes(level, nodeid_limit, neighbors_to_find, filter);
    if (estimated_visited_nodes >= nodeid_limit / 128) {
        search_layer_resilient_alt_filter_first_helper<BitVectorVisitedTracker>(
            stats, df, neighbors_to_find, exploration_slack, prefetch_tensors, best_neighbors, exploration, level,
            filter, nodeid_limit, deadline, estimated_visited_nodes);
    } else {
        search_layer_resilient_alt_filter_first_helper<HashSetVisitedTracker>(
            stats, df, neighbors_to_find, exploration_slack, prefetch_tensors, best_neighbors, exploration, level,
            filter, nodeid_limit, deadline, estimated_visited_nodes);
    }
}
...
template <HnswIndexType type>
PreparedAddDoc HnswIndex<type>::internal_prepare_add(uint32_t docid, VectorBundle input_vectors,
                                                     GenerationGuard read_guard) const {
...
template <HnswIndexType type>
void HnswIndex<type>::internal_prepare_add_node(PreparedAddDoc& op, TypedCells input_vector,
    ...
    while (search_level >= 0) {
        search_layer(stats, *df, _cfg.neighbors_to_explore_at_construction(), 0.0, false, best_neighbors,
                     search_level, nullptr);
...
template <HnswIndexType type>
LinkArray HnswIndex<type>::filter_valid_nodeids(uint32_t level, const typename PreparedAddNode::Links& neighbors,
...
template <HnswIndexType type>
void HnswIndex<type>::internal_complete_add_node(uint32_t nodeid, uint32_t docid, uint32_t subspace,
                                                 PreparedAddNode& prepared_node) {
...
template <HnswIndexType type>
std::unique_ptr<PrepareResult> HnswIndex<type>::prepare_add_document(uint32_t docid, VectorBundle vectors,
                                                                     GenerationGuard read_guard) const {
...
template <HnswIndexType type>
void HnswIndex<type>::complete_add_document(uint32_t docid, std::unique_ptr<PrepareResult> prepare_result) {
    ...
    if (prepared && (prepared->docid == docid)) {
        ...
        if (_graph.get_active_nodes() > 1.25 * _cfg.min_size_before_two_phase()) {
            LOG(warning, "complete_add_document(%u) called with invalid prepare_result %s/%u", docid,
                (prepared ? "valid ptr" : "nullptr"), (prepared ? prepared->docid : 0u));
        }
...
template <HnswIndexType type> void HnswIndex<type>::mutual_reconnect(const LinkArrayRef& cluster, uint32_t level) {
...
template <HnswIndexType type> void HnswIndex<type>::remove_node(uint32_t nodeid) {
...
template <HnswIndexType type> void HnswIndex<type>::remove_document(uint32_t docid) {
...
template <HnswIndexType type> void HnswIndex<type>::reclaim_memory(Generation oldest_used_gen) {
...
template <HnswIndexType type> GenerationGuard HnswIndex<type>::make_generation_read_guard() const {
    return _graph._generation_handler.takeGuard();
};

template <HnswIndexType type> void HnswIndex<type>::inc_generation() {
...
template <HnswIndexType type> void HnswIndex<type>::reclaim_unused_memory() {
...
template <HnswIndexType type>
void HnswIndex<type>::compact_level_arrays(const CompactionStrategy& compaction_strategy) {
...
template <HnswIndexType type>
void HnswIndex<type>::compact_link_arrays(const CompactionStrategy& compaction_strategy) {
...
template <HnswIndexType type> bool HnswIndex<type>::consider_compact(const CompactionStrategy& compaction_strategy) {
    bool result = false;
...
template <HnswIndexType type>
vespalib::MemoryUsage HnswIndex<type>::update_stat(const CompactionStrategy& compaction_strategy) {
...
template <HnswIndexType type> vespalib::MemoryUsage HnswIndex<type>::memory_usage() const {
    vespalib::MemoryUsage result;
    result.merge(_graph.nodes.getMemoryUsage());
...
template <HnswIndexType type>
void HnswIndex<type>::populate_address_space_usage(search::AddressSpaceUsage& usage) const {
...
template <HnswIndexType type> std::unique_ptr<vespalib::StateExplorer> HnswIndex<type>::make_state_explorer() const {
...
template <HnswIndexType type>
std::unique_ptr<NearestNeighborIndexSaver> HnswIndex<type>::make_saver(GenericHeader& header) const {
...
struct NeighborsByDocId {
    bool operator()(const NearestNeighborIndex::Neighbor& lhs, const NearestNeighborIndex::Neighbor& rhs) {
...
HnswIndex<type>::top_k_by_docid(Stats& stats, uint32_t k, const BoundDistanceFunction& df, const GlobalFilter* filter,
                                SearchAlgorithm alg, double exploration, uint32_t explore_k, double exploration_slack,
                                bool prefetch_tensors, const vespalib::Deadline& deadline,
                                double distance_threshold) const {
    SearchBestNeighbors candidates = top_k_candidates(stats, df, std::max(k, explore_k), exploration_slack,
                                                      prefetch_tensors, filter, alg, exploration, deadline);
...
template <HnswIndexType type>
std::vector<NearestNeighborIndex::Neighbor>
HnswIndex<type>::find_top_k(Stats& stats, uint32_t k, const BoundDistanceFunction& df, uint32_t explore_k,
                            double exploration_slack, bool prefetch_tensors, const vespalib::Deadline& deadline,
                            double distance_threshold) const {
    auto guard = _graph.make_guard();
    return top_k_by_docid(stats, k, df, nullptr, SearchAlgorithm::HNSW, 0.0, explore_k, exploration_slack,
                          prefetch_tensors, deadline, distance_threshold);
}

template <HnswIndexType type>
std::vector<NearestNeighborIndex::Neighbor>
HnswIndex<type>::find_top_k_with_filter(Stats& stats, uint32_t k, const BoundDistanceFunction& df,
                                        ...
                                        const vespalib::Deadline& deadline, double distance_threshold) const {
    auto guard = _graph.make_guard();
    return top_k_by_docid(stats, k, df, &filter, alg, exploration, explore_k, exploration_slack, prefetch_tensors,
                          deadline, distance_threshold);
}

template <HnswIndexType type>
typename HnswIndex<type>::SearchBestNeighbors
HnswIndex<type>::top_k_candidates(Stats& stats, const BoundDistanceFunction& df, uint32_t k, double exploration_slack,
                                  bool prefetch_tensors, const GlobalFilter* filter, SearchAlgorithm alg,
                                  double exploration, const vespalib::Deadline& deadline) const {
    ...
    best_neighbors.push(entry_point);
    bool active_filter = filter && filter->is_active();
    if (active_filter && alg == SearchAlgorithm::FILTER_FIRST) {
        search_layer_resilient_filter_first(stats, df, k, exploration_slack, prefetch_tensors, best_neighbors,
                                            exploration, 0, &deadline, filter);
    } else if (active_filter && alg == SearchAlgorithm::RESILIENT_FILTER_FIRST) {
        search_layer_resilient_alt_filter_first(stats, df, k, exploration_slack, prefetch_tensors, best_neighbors,
                                                exploration, 0, &deadline, filter);
    } else {
        search_layer(stats, df, k, exploration_slack, prefetch_tensors, best_neighbors, 0, &deadline, filter);
    }
    return best_neighbors;
}

template <HnswIndexType type> HnswTestNode HnswIndex<type>::get_node(uint32_t nodeid) const {
...
template <HnswIndexType type> bool HnswIndex<type>::check_link_symmetry() const {
    bool   all_sym = true;
...
template <HnswIndexType type> ReachabilityResult HnswIndex<type>::count_reachable_nodes() const {
    ...
    while (runAnotherVisit) {
        ...
        visitNext.foreach_truebit([&](uint32_t nodeid) {
            // note: search_level == 0
...
template <HnswIndexType type> uint32_t HnswIndex<type>::get_subspaces(uint32_t docid) const noexcept {
    if constexpr (type == HnswIndexType::SINGLE) {
        return (docid < _graph.nodes.get_size() &&
                _graph.nodes.get_elem_ref(docid).levels_ref().load_relaxed().valid())
                   ? 1
                   : 0;
    } else {
...
template <HnswIndexType type> uint32_t HnswIndex<type>::check_consistency(uint32_t docid_limit) const noexcept {
