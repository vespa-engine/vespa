 ...
 */
template <typename FloatType> class HammingDistanceFunctionFactory : public DistanceFunctionFactory {
private:
    bool _reference_insertion_vector;

public:
    HammingDistanceFunctionFactory() noexcept : HammingDistanceFunctionFactory(false) {}
    HammingDistanceFunctionFactory(bool reference_insertion_vector) noexcept
