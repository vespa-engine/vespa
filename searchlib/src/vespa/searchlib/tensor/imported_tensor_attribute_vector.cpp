...
ImportedTensorAttributeVector::ImportedTensorAttributeVector(
    std::string_view name, std::shared_ptr<ReferenceAttribute> reference_attribute,
    ...
    : ImportedAttributeVector(name, std::move(reference_attribute), std::move(document_meta_store),
                              std::move(target_attribute), std::move(target_document_meta_store), use_search_cache) {
}
...
ImportedTensorAttributeVector::ImportedTensorAttributeVector(
    ...
    : ImportedAttributeVector(name, std::move(reference_attribute), std::move(document_meta_store),
                              std::move(target_attribute), std::move(target_document_meta_store),
...
std::unique_ptr<attribute::AttributeReadGuard>
ImportedTensorAttributeVector::makeReadGuard(std::shared_ptr<MetaStoreReadGuard> targetMetaStoreReadGuard,
                                             bool                                stableEnumGuard) const {
    return std::make_unique<ImportedTensorAttributeVectorReadGuard>(std::move(targetMetaStoreReadGuard), *this,
                                                                    stableEnumGuard);
}
