 ...
 */
template <HnswIndexType type> class HnswIndexSaverMetadataNode;

/*
 * Metadata for node during save of hnsw graph with one node per document and
 * identity mapping between nodeid and docid.
 */
template <> class HnswIndexSaverMetadataNode<HnswIndexType::SINGLE> {
    uint32_t _refs_offset;

public:
    HnswIndexSaverMetadataNode(uint32_t refs_offset) noexcept : _refs_offset(refs_offset) {}
    HnswIndexSaverMetadataNode(uint32_t refs_offset, const HnswSimpleNode&) noexcept : _refs_offset(refs_offset) {}
 ...
 */
template <> class HnswIndexSaverMetadataNode<HnswIndexType::MULTI> {
    uint32_t _refs_offset;
    uint32_t _docid;
    uint32_t _subspace;

public:
    HnswIndexSaverMetadataNode(uint32_t refs_offset) noexcept : _refs_offset(refs_offset), _docid(0), _subspace(0) {}
    HnswIndexSaverMetadataNode(uint32_t refs_offset, const HnswNode& node) noexcept
        : _refs_offset(refs_offset), _docid(node.acquire_docid()), _subspace(node.acquire_subspace()) {}
    uint32_t get_refs_offset() const noexcept { return _refs_offset; }
