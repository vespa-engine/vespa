...
ImportedTensorAttributeVectorReadGuard::ImportedTensorAttributeVectorReadGuard(
    std::shared_ptr<MetaStoreReadGuard>       targetMetaStoreReadGuard,
    const attribute::ImportedAttributeVector& imported_attribute, bool stableEnumGuard)
...
std::unique_ptr<vespalib::eval::Value> ImportedTensorAttributeVectorReadGuard::getEmptyTensor() const {
    return _target_tensor_attribute.getEmptyTensor();
}

vespalib::eval::TypedCells ImportedTensorAttributeVectorReadGuard::extract_cells_ref(uint32_t docid) const {
    return _target_tensor_attribute.extract_cells_ref(getTargetLid(docid));
}

const vespalib::eval::Value& ImportedTensorAttributeVectorReadGuard::get_tensor_ref(uint32_t docid) const {
    return _target_tensor_attribute.get_tensor_ref(getTargetLid(docid));
}

vespalib::eval::TypedCells ImportedTensorAttributeVectorReadGuard::get_vector(uint32_t docid,
                                                                              uint32_t subspace) const noexcept {
    return _target_tensor_attribute.get_vector(getTargetLid(docid), subspace);
}

search::tensor::VectorBundle ImportedTensorAttributeVectorReadGuard::get_vectors(uint32_t docid) const noexcept {
    return _target_tensor_attribute.get_vectors(getTargetLid(docid));
}

const vespalib::eval::ValueType& ImportedTensorAttributeVectorReadGuard::getTensorType() const {
    return _target_tensor_attribute.getTensorType();
}

SerializedTensorRef ImportedTensorAttributeVectorReadGuard::get_serialized_tensor_ref(uint32_t docid) const {
    return _target_tensor_attribute.get_serialized_tensor_ref(getTargetLid(docid));
}

bool ImportedTensorAttributeVectorReadGuard::supports_get_serialized_tensor_ref() const {
    return _target_tensor_attribute.supports_get_serialized_tensor_ref();
}

std::unique_ptr<vespalib::StateExplorer> ImportedTensorAttributeVectorReadGuard::make_state_explorer() const {
