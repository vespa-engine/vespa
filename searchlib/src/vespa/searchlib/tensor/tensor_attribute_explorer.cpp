...
void TensorAttributeExplorer::get_state(const vespalib::slime::Inserter& inserter, bool full) const {
    (void)full;
...
std::vector<std::string> TensorAttributeExplorer::get_children_names() const {
    return {NEAREST_NEIGHBOR_INDEX_NAME, TENSOR_STORE_NAME};
}

std::unique_ptr<vespalib::StateExplorer> TensorAttributeExplorer::get_child(std::string_view name) const {
