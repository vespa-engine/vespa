...
void FtTestAppBase::FT_SETUP_FAIL(const search::fef::Blueprint& prototype, const StringList& params) {
    search::fef::test::IndexEnvironment ie;
    FT_SETUP_FAIL(prototype, ie, params);
}

void FtTestAppBase::FT_SETUP_FAIL(const search::fef::Blueprint&              prototype,
                                  const search::fef::test::IndexEnvironment& env, const StringList& params) {
...
void FtTestAppBase::FT_SETUP_OK(const search::fef::Blueprint& prototype, const StringList& params,
                                const StringList& expectedIn, const StringList& expectedOut) {
    search::fef::test::IndexEnvironment ie;
    FT_SETUP_OK(prototype, ie, params, expectedIn, expectedOut);
}

void FtTestAppBase::FT_SETUP_OK(const search::fef::Blueprint&              prototype,
                                const search::fef::test::IndexEnvironment& env, const StringList& params,
                                const StringList& expectedIn, const StringList& expectedOut) {
    FT_LOG(prototype, env, params);
    search::fef::Blueprint::UP bp = prototype.createInstance();
...
void FtTestAppBase::FT_DUMP_EMPTY(search::fef::BlueprintFactory& factory, const std::string& baseName) {
    StringList empty;
...
void FtTestAppBase::FT_DUMP(search::fef::BlueprintFactory& factory, const std::string& baseName,
...
void FtTestAppBase::FT_DUMP(search::fef::BlueprintFactory& factory, const std::string& baseName,
                            search::fef::test::IndexEnvironment& env, const StringList& expected) {
...
void FtTestAppBase::FT_EQUAL(const std::vector<string>& expected, const std::vector<string>& actual,
...
void FtTestAppBase::FT_LOG(const search::fef::Blueprint& prototype, const search::fef::test::IndexEnvironment& env,
...
void FtTestAppBase::FT_LOG(const std::string& prefix, const std::vector<std::string>& arr) {
...
void FtTestAppBase::FT_SETUP(FtFeatureTest& test, const std::string& query, const StringMap& index, uint32_t docId) {
...
void FtTestAppBase::FT_SETUP(FtFeatureTest& test, const std::vector<FtQueryTerm>& query, const StringVectorMap& index,
                             uint32_t docId) {
        ...
        for (uint32_t i = 0; i < itr->second.size(); ++i) {
            ...
            for (;;) {
                ...
                if (fitr != query.end()) {
                    ...
                    LOG(debug, "Occurence of '%s' added to field '%s' at position %u.", fitr->term.c_str(),
                        itr->first.c_str(), i);
...
void FtTestAppBase::FT_SETUP(FtFeatureTest& test, const FtQuery& query, const FtIndex& index, uint32_t docId) {
    setupQueryEnv(test.getQueryEnv(), query);
    ASSERT_TRUE(test.setup());
    search::fef::test::MatchDataBuilder::UP mdb = test.createMatchDataBuilder();
...
void FtTestAppBase::setupQueryEnv(FtQueryEnvironment& queryEnv, const FtQuery& query) {
    // Add all query terms.
    for (uint32_t i = 0; i < query.size(); ++i) {
        queryEnv.getBuilder().addAllFields();
...
void FtTestAppBase::setupFieldMatch(FtFeatureTest& ft, const std::string& indexName, const std::string& query,
                                    const std::string& field, const fieldmatch::Params* params,
                                    uint32_t totalTermWeight, feature_t totalSignificance, uint32_t docId) {
    ...
    if (params != nullptr) {
        ...
        p.add("fieldMatch(" + indexName + ").proximityLimit",
              vespalib::make_string("%u", params->getProximityLimit()));
        p.add("fieldMatch(" + indexName + ").maxAlternativeSegmentations",
              vespalib::make_string("%u", params->getMaxAlternativeSegmentations()));
        p.add("fieldMatch(" + indexName + ").maxOccurrences",
              vespalib::make_string("%u", params->getMaxOccurrences()));
        p.add("fieldMatch(" + indexName + ").proximityCompletenessImportance",
              vespalib::make_string("%f", params->getProximityCompletenessImportance()));
        p.add("fieldMatch(" + indexName + ").relatednessImportance",
              vespalib::make_string("%f", params->getRelatednessImportance()));
        p.add("fieldMatch(" + indexName + ").earlinessImportance",
              vespalib::make_string("%f", params->getEarlinessImportance()));
        p.add("fieldMatch(" + indexName + ").segmentProximityImportance",
              vespalib::make_string("%f", params->getSegmentProximityImportance()));
        p.add("fieldMatch(" + indexName + ").occurrenceImportance",
              vespalib::make_string("%f", params->getOccurrenceImportance()));
        p.add("fieldMatch(" + indexName + ").fieldCompletenessImportance",
              vespalib::make_string("%f", params->getFieldCompletenessImportance()));
...
RankResult FtTestAppBase::toRankResult(const std::string& baseName, const std::string& result,
