...
std::unique_ptr<AttributeNode> makeAttributeMapLookupNode(std::string_view attributeName) {
    ...
    if (rightBracePos != std::string::npos && rightBracePos > leftBracePos) {
        ...
        } else if (attributeName.substr(leftBracePos + 1, indirectKeyMarker.size()) == indirectKeyMarker &&
                   attributeName[rightBracePos - 1] == ')')
        {
            ...
            return std::make_unique<AttributeMapLookupNode>(attributeName, keyName.view(), valueName.view(), "",
                                                            keySourceAttributeName);
        }
    }
    return {};
}
