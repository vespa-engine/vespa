...
const IAttributeVector* MockAttributeContext::get(std::string_view name) const {
    if (_vectors.find(name) == _vectors.end()) {
        return nullptr;
    }
    return _vectors.find(name)->second.get();
}
const IAttributeVector* MockAttributeContext::getAttribute(std::string_view name) const {
    return get(name);
}
const IAttributeVector* MockAttributeContext::getAttributeStableEnum(std::string_view name) const {
    return get(name);
}
...
void MockAttributeContext::add(std::shared_ptr<const IAttributeVector> attr) {
    _vectors[attr->getName()] = std::move(attr);
}

void MockAttributeContext::asyncForAttribute(std::string_view, std::unique_ptr<IAttributeFunctor>) const {
