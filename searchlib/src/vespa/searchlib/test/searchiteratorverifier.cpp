...
SearchIteratorVerifier::SearchIteratorVerifier()
    : _trueTfmd(), _docIds(), _everyOddBitSet(BitVector::create(getDocIdLimit())) {
    ...
    _everyOddBitSet->foreach_truebit([&](uint32_t docId) {
        if (everyOddSet.find(docId) == everyOddSet.end()) {
            _expectedAndNotNegative.emplace_back(docId);
        }
    });
}

SearchIterator::UP SearchIteratorVerifier::createIterator(const DocIds& docIds, bool strict) {
    return make_unique<DocIdIterator>(docIds, strict);
}

SearchIterator::UP SearchIteratorVerifier::createEmptyIterator() {
    return make_unique<EmptySearch>();
}

SearchIterator::UP SearchIteratorVerifier::createFullIterator() const {
    return make_unique<TrueSearch>(_trueTfmd);
}
SearchIteratorVerifier::~SearchIteratorVerifier() = default;

void SearchIteratorVerifier::verify() const {
    GTEST_DO(verifyTermwise());
    GTEST_DO(verifyInitRange());
}

void SearchIteratorVerifier::verifyTermwise() const {
...
void SearchIteratorVerifier::verify(bool strict) const {
    SearchIterator::UP iterator = create(strict);
...
void SearchIteratorVerifier::verifyAnd(bool strict) const {
...
void SearchIteratorVerifier::verifyAndNot(bool strict) const {
    fef::TermFieldMatchData tfmd;
    {
        for (bool notStrictness : {false, true}) {
            ...
            children.push_back(create(strict));
            children.push_back(
                BitVectorIterator::create(_everyOddBitSet.get(), getDocIdLimit(), tfmd, notStrictness));
...
void SearchIteratorVerifier::verifyOr(bool strict) const {
    fef::TermFieldMatchData tfmd;
    MultiSearch::Children   children;
    children.push_back(create(strict));
...
void SearchIteratorVerifier::verifyTermwise(SearchIterator::UP iterator, bool strict, const DocIds& docIds) {
    SearchIterator::UP termwise = make_termwise(std::move(iterator), strict);
    GTEST_DO(verify(*termwise, strict, docIds));
}

void SearchIteratorVerifier::verify_and_hits_into(SearchIterator& iterator, const DocIds& docIds) {
...
void SearchIteratorVerifier::verify_or_hits_into(SearchIterator& iterator, const DocIds& docIds) {
...
void SearchIteratorVerifier::verify_get_hits(SearchIterator& iterator, const DocIds& docIds) {
    iterator.initRange(1, getDocIdLimit());
...
void SearchIteratorVerifier::verify(SearchIterator& iterator, bool strict, const DocIds& docIds) {
...
void SearchIteratorVerifier::verify(SearchIterator& iterator, const Ranges& ranges, bool strict,
                                    const DocIds& docIds) {
...
SearchIteratorVerifier::DocIds SearchIteratorVerifier::search(SearchIterator& it, const Ranges& ranges, bool strict) {
    DocIds result;
...
SearchIteratorVerifier::DocIds SearchIteratorVerifier::searchRelaxed(SearchIterator& it, Range range) {
    DocIds result;
...
SearchIteratorVerifier::DocIds SearchIteratorVerifier::searchStrict(SearchIterator& it, Range range) {
    DocIds result;
    it.initRange(range.first, range.second);
    for (uint32_t docId = it.seekFirst(range.first); (docId < range.second) && !it.isAtEnd();
         docId = it.seekNext(docId + 1))
    {
...
std::unique_ptr<SearchIterator> SearchIteratorVerifier::create(bool, const fef::TermFieldMatchDataArray&) const {
    return {};
}

void SearchIteratorVerifier::verify_hidden_from_ranking(const fef::TermFieldMatchDataArray& tfmda) const {
    auto* tfmd = tfmda[0];
