...
CompressedReadBuffer<bigEndian>::CompressedReadBuffer(DC& d, const CompressedWriteBuffer<bigEndian>& wb)
    : _d(d), _rc(d), _header_len(0), _file_bit_size(0) {
    _d.setReadContext(&_rc);
    rewind(wb);
}

template <bool bigEndian> CompressedReadBuffer<bigEndian>::~CompressedReadBuffer() = default;

template <bool bigEndian> void CompressedReadBuffer<bigEndian>::rewind(const CompressedWriteBuffer<bigEndian>& wb) {
