...
PageDict4MemWriter::PageDict4MemWriter(uint32_t chunkSize, uint64_t numWordIds, uint32_t ssPad, uint32_t spPad,
                                       uint32_t pPad)
...
void PageDict4MemWriter::allocWriters() {
...
void PageDict4MemWriter::flush() {
    _pw->flush();
    _buffers.flush();
}

void PageDict4MemWriter::addCounts(const std::string& word, const PostingListCounts& counts) {
