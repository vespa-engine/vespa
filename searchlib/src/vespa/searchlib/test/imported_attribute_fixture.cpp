...
IDocumentMetaStoreContext::IReadGuard::SP MockDocumentMetaStoreContext::getReadGuard() const {
    ++get_read_guard_cnt;
...
std::shared_ptr<MockDocumentMetaStoreContext> create_target_document_meta_store() {
    return std::make_shared<MockDocumentMetaStoreContext>();
}

std::shared_ptr<MockDocumentMetaStoreContext> create_document_meta_store() {
    return std::make_shared<MockDocumentMetaStoreContext>();
}
...
std::unique_ptr<QueryTermSimple> word_term(std::string_view term) {
    return std::make_unique<QueryTermUCS4>(std::string(term), QueryTermSimple::Type::WORD);
}
...
ImportedAttributeFixture::create_attribute_vector_from_members(std::string_view name) {
    return ImportedAttributeVectorFactory::create(name, reference_attr, document_meta_store, target_attr,
                                                  target_document_meta_store, use_search_cache);
}

void ImportedAttributeFixture::reset_with_new_target_attr(std::shared_ptr<AttributeVector> new_target) {
    target_attr = std::move(new_target);
    imported_attr = create_attribute_vector_from_members();
}

void ImportedAttributeFixture::set_up_attribute_vectors_before_adding_mappings() {
