...
struct FtTestAppBase {
    ...
    static void FT_LOG(const search::fef::Blueprint& prototype, const search::fef::test::IndexEnvironment& env,
                       const StringList& params);
    ...
    static void setupFieldMatch(FtFeatureTest& test, const std::string& indexName, const std::string& query,
                                const std::string& field, const search::features::fieldmatch::Params* params,
                                uint32_t totalTermWeight, feature_t totalSignificance, uint32_t docId);

    static search::fef::test::RankResult toRankResult(const std::string& baseName, const std::string& result,
                                                      const std::string& separator = " ");

    template <typename T> static bool assertCreateInstance(const T& prototype, const std::string& baseName) {
