...
AttributeVector::SP MockAttributeManager::findAttribute(std::string_view name) const {
...
AttributeGuard::UP MockAttributeManager::getAttribute(std::string_view name) const {
    AttributeVector::SP attr = findAttribute(name);
    return std::make_unique<AttributeGuard>(attr);
}

void MockAttributeManager::asyncForAttribute(std::string_view, std::unique_ptr<IAttributeFunctor>) const {
    throw std::runtime_error("search::MockAttributeManager::asyncForAttribute not implemented.");
}

std::unique_ptr<AttributeReadGuard> MockAttributeManager::getAttributeReadGuard(std::string_view name,
...
void MockAttributeManager::getAttributeList(std::vector<AttributeGuard>& list) const {
...
IAttributeContext::UP MockAttributeManager::createContext() const {
    return std::make_unique<AttributeContext>(*this);
}

void MockAttributeManager::addAttribute(std::string_view name, const AttributeVector::SP& attr) {
    attr->addReservedDoc();
    _attributes[std::string(name)] = attr;
}
void MockAttributeManager::addAttribute(const AttributeVector::SP& attr) {
