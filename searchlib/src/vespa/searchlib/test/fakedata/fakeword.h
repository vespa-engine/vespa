...
public:
    ...
    public:
        ...
        bool operator==(const Randomizer& rhs) const noexcept { return _random == rhs._random && _ref == rhs._ref; }
    ...
    public:
        ...
        void write(RandomizedWriter& writer) {
    ...
    void fakeup(search::BitVector& bitmap, vespalib::Rand48& rnd, DocWordFeatureList& postings,
                DocWordPosFeatureList& wordPosFeatures);

    void fakeupTemps(vespalib::Rand48& rnd, uint32_t docIdLimit, uint32_t tempWordDocs);
    ...
    static void setupFeatures(const DocWordFeature& d, const DocWordPosFeature* p,
        ...
        for (unsigned int t = 0; t < positions; ++t) {
            features.addNextOcc(p->_elementId, p->_wordPos, p->_elementWeight, p->_elementLen);
            ++p;
        }
...
public:
    FakeWord(uint32_t docIdLimit, const std::vector<uint32_t>& docIds, const std::string& name,
             const PosOccFieldsParams& fieldsParams, uint32_t packedIndex);

    FakeWord(uint32_t docIdLimit, uint32_t wordDocs, uint32_t tempWordDocs, const std::string& name,
    ...
    bool validate(search::queryeval::SearchIterator* iterator, const fef::TermFieldMatchDataArray& matchData,
                  uint32_t stride, bool unpack_normal_features, bool unpack_interleaved_features, bool verbose) const;
    ...
    bool validate(search::diskindex::FieldReader& fieldReader, uint32_t wordNum,
                  const fef::TermFieldMatchDataArray& matchData, bool decode_interleaved_features,
                  bool verbose) const;

    void validate(const std::vector<uint32_t>& docIds) const;
    void validate(const BitVector& bv) const;

    bool dump(search::diskindex::FieldWriter& fieldWriter, bool verbose) const;
