...
namespace test {
struct MockGidToLidMapperFactory;
}
...
enum class FastSearchConfig { ExplicitlyEnabled, Default };

enum class FilterConfig { ExplicitlyEnabled, Default };

template <typename AttrVecType>
std::shared_ptr<AttrVecType> create_typed_attribute(BasicType basic_type, CollectionType collection_type,
    ...
    return std::dynamic_pointer_cast<AttrVecType>(AttributeFactory::createAttribute(name, std::move(cfg)));
}

template <typename AttrVecType>
std::shared_ptr<AttrVecType>
create_single_attribute(BasicType type, FastSearchConfig fast_search = FastSearchConfig::Default,
                        FilterConfig filter = FilterConfig::Default, std::string_view name = "parent") {
...
std::shared_ptr<AttrVecType> create_array_attribute(BasicType type, std::string_view name = "parent") {
    return create_typed_attribute<AttrVecType>(type, CollectionType::ARRAY, FastSearchConfig::Default,
                                               FilterConfig::Default, name);
}

template <typename AttrVecType>
std::shared_ptr<AttrVecType>
create_wset_attribute(BasicType type, FastSearchConfig fast_search = FastSearchConfig::Default,
                      FilterConfig filter = FilterConfig::Default, std::string_view name = "parent") {
...
template <typename VectorType> void add_n_docs_with_undefined_values(VectorType& vec, size_t n) {
    vec.addDocs(n);
    vec.commit();
}
...
struct ImportedAttributeFixture {
    ...
    explicit ImportedAttributeFixture(bool             use_search_cache_ = false,
                                      FastSearchConfig fastSearch = FastSearchConfig::Default);

    virtual ~ImportedAttributeFixture();

    ReadGuardWrapper get_imported_attr() const { return ReadGuardWrapper(imported_attr->makeReadGuard(false)); }

    void map_reference(DocId from_lid, GlobalId via_gid, DocId to_lid);

    static std::string_view default_imported_attr_name() { return "imported"; }
    ...
    template <typename ValueType> struct LidToLidMapping {
        DocId     _from_lid;
        GlobalId  _via_gid;
        DocId     _to_lid;
        ValueType _value_in_target_attr;

        LidToLidMapping(DocId from_lid, GlobalId via_gid, DocId to_lid, ValueType value_in_target_attr)
            : _from_lid(from_lid),
              _via_gid(via_gid),
              _to_lid(to_lid),
    ...
    void reset_with_single_value_reference_mappings(BasicType                                      type,
    ...
    template <typename AttrVecType, typename ValueType>
    void
    reset_with_array_value_reference_mappings(BasicType                                                   type,
    ...
    void reset_with_wset_value_reference_mappings(
        BasicType type, const std::vector<LidToLidMapping<std::vector<WeightedValueType>>>& mappings,
        FastSearchConfig fast_search = FastSearchConfig::Default, FilterConfig filter = FilterConfig::Default) {
...
void assert_multi_value_matches(const ImportedAttributeFixture& f, DocId lid,
                                const std::vector<AttrValueType>& expected, PredicateType predicate) {
    ...
    EXPECT_TRUE(std::equal(wanted.begin(), wanted.end(), actual.begin(), actual.end(), predicate));
}
...
void reset_with_single_value_reference_mappings(
    ImportedAttributeFixture& f, BasicType type,
