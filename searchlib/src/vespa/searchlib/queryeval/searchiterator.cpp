...
std::string SearchIterator::make_id_ref_str() const {
    if (_id == 0) {
        return "[]";
    }
...
BitVector::UP SearchIterator::get_hits(uint32_t begin_id) {
...
SearchIterator::UP SearchIterator::andWith(UP filter, uint32_t estimate) {
    (void)estimate;
    return filter;
}

void SearchIterator::or_hits_into(BitVector& result, uint32_t begin_id) {
    uint32_t docid = std::max(begin_id, getDocId());
    while (!isAtEnd(docid)) {
...
void SearchIterator::and_hits_into_non_strict(BitVector& result, uint32_t begin_id) {
...
void SearchIterator::and_hits_into_strict(BitVector& result, uint32_t begin_id) {
    seek(begin_id);
...
std::string SearchIterator::asString() const {
    vespalib::ObjectDumper dumper;
    visit(dumper, "", this);
    return dumper.toString();
}

vespalib::slime::Cursor& SearchIterator::asSlime(const vespalib::slime::Inserter& inserter) const {
...
std::string SearchIterator::getClassName() const {
    return vespalib::getClassName(*this);
}

void SearchIterator::visitMembers(vespalib::ObjectVisitor& visitor) const {
    visit(visitor, "docid", _docid);
    visit(visitor, "endid", _endid);
}

void SearchIterator::transform_children(std::function<SearchIterator::UP(SearchIterator::UP)>) {
}
