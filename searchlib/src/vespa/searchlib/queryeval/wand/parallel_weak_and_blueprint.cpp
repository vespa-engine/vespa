...
void ParallelWeakAndBlueprint::reserve(size_t num_children) {
    _weights.reserve(num_children);
    _terms.reserve(num_children);
}

void ParallelWeakAndBlueprint::addTerm(Blueprint::UP term, int32_t weight, HitEstimate& estimate) {
...
ParallelWeakAndBlueprint::createLeafSearch(const search::fef::TermFieldMatchDataArray& tfmda) const {
    ...
    return ParallelWeakAndSearch::create(
        terms,
        ParallelWeakAndSearch::MatchParams(*_scores, _scoreThreshold, _thresholdBoostFactor, _scoresAdjustFrequency,
                                           get_docid_limit()),
        ParallelWeakAndSearch::RankParams(*tfmda[0], std::move(childrenMatchData)), strict(), readonly_scores_heap);
}

std::unique_ptr<SearchIterator> ParallelWeakAndBlueprint::createFilterSearchImpl(FilterConstraint constraint) const {
    return create_atmost_or_filter(_terms, constraint);
}

void ParallelWeakAndBlueprint::fetchPostings(const ExecuteInfo& execInfo) {
    for (const auto& _term : _terms) {
        _term->fetchPostings(execInfo);
    }
}

bool ParallelWeakAndBlueprint::always_needs_unpack() const {
    return true;
}

void ParallelWeakAndBlueprint::set_matching_phase(MatchingPhase matching_phase) noexcept {
...
void ParallelWeakAndBlueprint::visitMembers(vespalib::ObjectVisitor& visitor) const {
