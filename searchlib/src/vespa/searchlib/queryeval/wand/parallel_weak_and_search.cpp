...
namespace {
bool should_monitor_wand() {
    return LOG_WOULD_LOG(spam);
}
} // namespace
...
public:
    ParallelWeakAndSearchImpl(fef::TermFieldMatchData& tfmd, VectorizedTerms&& terms, const MatchParams& matchParams,
    ...
    void visitMembers(vespalib::ObjectVisitor& visitor) const override { _terms.visit_members(visitor); }
...
wand::Terms insertMonitoringSearchIterator(const wand::Terms& terms) {
...
SearchIterator::UP createWand(const wand::Terms& terms, const ParallelWeakAndSearch::MatchParams& matchParams,
                              ParallelWeakAndSearch::RankParams&& rankParams, bool readonly_scores_heap) {
    ...
    if (should_monitor_wand()) {
        ...
        auto        monitoringIterator = std::make_unique<MonitoringSearchIterator>(
            make_string("PWAND(%u,%" PRId64 "),strict=%u", matchParams.scores.getScoresToTrack(),
                               matchParams.scoreThreshold, IS_STRICT),
            std::make_unique<WandType>(rankParams.rootMatchData,
                                              VectorizedIteratorTerms(termsWithMonitoring, DotProductScorer(),
    ...
    return std::make_unique<WandType>(rankParams.rootMatchData,
                                      VectorizedIteratorTerms(terms, DotProductScorer(), matchParams.docIdLimit,
                                                              std::move(rankParams.childrenMatchData)),
                                      matchParams, readonly_scores_heap);
}
...
SearchIterator::UP ParallelWeakAndSearch::createArrayWand(const Terms& terms, const MatchParams& matchParams,
                                                          RankParams&& rankParams, bool strict,
                                                          bool readonly_scores_heap) {
    return strict ? wand::createWand<vespalib::LeftArrayHeap, vespalib::RightArrayHeap, true>(
                        terms, matchParams, std::move(rankParams), readonly_scores_heap)
                  : wand::createWand<vespalib::LeftArrayHeap, vespalib::RightArrayHeap, false>(
                        terms, matchParams, std::move(rankParams), readonly_scores_heap);
}

SearchIterator::UP ParallelWeakAndSearch::createHeapWand(const Terms& terms, const MatchParams& matchParams,
                                                         RankParams&& rankParams, bool strict,
                                                         bool readonly_scores_heap) {
    return strict ? wand::createWand<vespalib::LeftHeap, vespalib::RightHeap, true>(
                        terms, matchParams, std::move(rankParams), readonly_scores_heap)
                  : wand::createWand<vespalib::LeftHeap, vespalib::RightHeap, false>(
                        terms, matchParams, std::move(rankParams), readonly_scores_heap);
}

SearchIterator::UP ParallelWeakAndSearch::create(const Terms& terms, const MatchParams& matchParams,
...
SearchIterator::UP create_helper(search::fef::TermFieldMatchData& tfmd, VectorizedTerms&& terms,
                                 const MatchParams& params, bool strict, bool readonly_scores_heap) {
    if (strict) {
        return std::make_unique<wand::ParallelWeakAndSearchImpl<VectorizedTerms, FutureHeap, PastHeap, true>>(
            tfmd, std::forward<VectorizedTerms>(terms), params, readonly_scores_heap);
    } else {
        return std::make_unique<wand::ParallelWeakAndSearchImpl<VectorizedTerms, FutureHeap, PastHeap, false>>(
            tfmd, std::forward<VectorizedTerms>(terms), params, readonly_scores_heap);
    }
}
...
SearchIterator::UP create_helper(search::fef::TermFieldMatchData& tfmd, VectorizedTerms&& terms,
                                 const MatchParams& params, bool strict, bool readonly_scores_heap, bool use_array) {
    return (use_array) ? create_helper<VectorizedTerms, vespalib::LeftArrayHeap, vespalib::RightArrayHeap>(
                             tfmd, std::forward<VectorizedTerms>(terms), params, strict, readonly_scores_heap)
                       : create_helper<VectorizedTerms, vespalib::LeftHeap, vespalib::RightHeap>(
                             tfmd, std::forward<VectorizedTerms>(terms), params, strict, readonly_scores_heap);
}

} // namespace

SearchIterator::UP ParallelWeakAndSearch::create(search::fef::TermFieldMatchData& tfmd,
                                                 const MatchParams& matchParams, const std::vector<int32_t>& weights,
    ...
    if (!wand::should_monitor_wand()) {
        wand::VectorizedAttributeTerms terms(weights, dict_entries, attr, wand::DotProductScorer(),
                                             matchParams.docIdLimit);
        return create_helper(tfmd, std::move(terms), matchParams, strict, readonly_scores_heap,
                             (weights.size() < 128));
    } else {
        ...
        for (size_t i = 0; i < dict_entries.size(); ++i) {
            terms.emplace_back(new DocidWithWeightSearchIterator(*(childrenMatchData->resolveTermField(handles[i])),
                                                                 attr, dict_entries[i]),
                               weights[i], dict_entries[i].posting_size,
