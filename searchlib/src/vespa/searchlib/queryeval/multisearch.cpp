...
void MultiSearch::doUnpack(uint32_t docid) {
...
MultiSearch::MultiSearch(Children children) : _children(std::move(children)) {
}

MultiSearch::MultiSearch() = default;
MultiSearch::~MultiSearch() = default;

void MultiSearch::initRange(uint32_t beginid, uint32_t endid) {
...
void MultiSearch::transform_children(std::function<SearchIterator::UP(SearchIterator::UP)> f) {
    for (auto& child : _children) {
        child = f(std::move(child));
    }
}

void MultiSearch::visitMembers(vespalib::ObjectVisitor& visitor) const {
