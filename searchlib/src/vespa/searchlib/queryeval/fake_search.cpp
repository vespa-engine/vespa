...
FakeSearch::FakeSearch(const std::string& tag, const std::string& field, const std::string& term,
                       const FakeResult& res, fef::TermFieldMatchDataArray tfmda)
    : _tag(tag),
      _field(field),
      _term(term),
      _result(res),
...
void FakeSearch::doSeek(uint32_t docid) {
...
void FakeSearch::doUnpack(uint32_t docid) {
    ...
    for (uint32_t i = 0; i < doc.elements.size(); ++i) {
        ...
        if (!is_attr()) {
            for (uint32_t j = 0; j < elem.positions.size(); ++j) {
                _tfmda[0]->appendPosition(PosCtx(elem.id, elem.positions[j], elem.weight, elem.length));
            }
        }
    }
    if (is_attr()) {
...
void FakeSearch::visitMembers(vespalib::ObjectVisitor& visitor) const {
    visit(visitor, "tag", _tag);
    visit(visitor, "field", _field);
    visit(visitor, "term", _term);
}

void FakeSearch::get_element_ids(uint32_t docid, std::vector<uint32_t>& element_ids) {
    if (_ctx != nullptr) {
...
void FakeSearch::and_element_ids_into(uint32_t docid, std::vector<uint32_t>& element_ids) {
    if (_ctx != nullptr) {
