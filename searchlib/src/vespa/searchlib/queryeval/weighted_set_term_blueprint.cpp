...
public:
    WeightedSetTermMatchingElementsSearch(const WeightedSetTermBlueprint& bp, const std::string& field_name,
                                          const std::vector<Blueprint::UP>& terms);
...
WeightedSetTermMatchingElementsSearch::WeightedSetTermMatchingElementsSearch(const WeightedSetTermBlueprint& bp,
                                                                             const std::string& field_name,
                                                                             const std::vector<Blueprint::UP>& terms)
    : _tfmd(), _tfmda(), _field_name(field_name), _terms(terms), _search() {
    _tfmda.add(&_tfmd);
...
void WeightedSetTermMatchingElementsSearch::find_matching_elements(uint32_t docid, MatchingElements& result) {
    ...
    if (!_matching_elements.empty()) {
        ...
        _matching_elements.resize(std::unique(_matching_elements.begin(), _matching_elements.end()) -
                                  _matching_elements.begin());
        result.add_matching_elements(docid, _field_name, _matching_elements);
    }
}

void WeightedSetTermMatchingElementsSearch::initRange(uint32_t begin_id, uint32_t end_id) {
...
WeightedSetTermBlueprint::WeightedSetTermBlueprint(const FieldSpec& field)
    : ComplexLeafBlueprint(field),
      _layout(),
      _children_field(field.getName(), field.getFieldId(), _layout.allocTermField(field.getFieldId()),
                      field.isFilter()),
      _weights(),
      _terms() {
...
void WeightedSetTermBlueprint::reserve(size_t num_children) {
    _weights.reserve(num_children);
    _terms.reserve(num_children);
    _layout.reserve(num_children);
}

void WeightedSetTermBlueprint::addTerm(Blueprint::UP term, int32_t weight, HitEstimate& estimate) {
...
SearchIterator::UP WeightedSetTermBlueprint::createFilterSearchImpl(FilterConstraint constraint) const {
...
WeightedSetTermBlueprint::create_matching_elements_search(const MatchingElementsFields& fields) const {
    if (fields.has_field(_children_field.getName())) {
        return std::make_unique<WeightedSetTermMatchingElementsSearch>(
            *this, fields.enclosing_field(_children_field.getName()), _terms);
    } else {
        return {};
    }
}

void WeightedSetTermBlueprint::fetchPostings(const ExecuteInfo& execInfo) {
    for (const auto& _term : _terms) {
        _term->fetchPostings(execInfo);
    }
}

void WeightedSetTermBlueprint::visitMembers(vespalib::ObjectVisitor& visitor) const {
