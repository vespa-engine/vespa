...
SearchIteratorPack& SearchIteratorPack::operator=(SearchIteratorPack&& rhs) noexcept = default;

SearchIteratorPack::SearchIteratorPack(const std::vector<SearchIterator*>&          children,
                                       const std::vector<fef::TermFieldMatchData*>& childMatch, MatchDataUP md)
    : _children(), _childMatch(childMatch), _md(std::move(md)) {
...
std::unique_ptr<BitVector> SearchIteratorPack::get_hits(uint32_t begin_id, uint32_t end_id) const {
...
void SearchIteratorPack::or_hits_into(BitVector& result, uint32_t begin_id) const {
