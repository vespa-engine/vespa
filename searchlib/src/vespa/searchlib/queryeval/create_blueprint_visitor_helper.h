...
public:
    CreateBlueprintVisitorHelper(Searchable& searchable, const FieldSpec& field,
                                 const IRequestContext& requestContext, fef::MatchDataLayout& global_layout);
    ~CreateBlueprintVisitorHelper() override;

    template <typename T> void setResult(std::unique_ptr<T> result) { _result = std::move(result); }
    ...
    template <typename WS, typename NODE> void createWeightedSet(std::unique_ptr<WS> bp, NODE& n);
    ...
    void visit(query::Phrase& n) override { visitPhrase(n); }
    void visit(query::WordAlternatives& n) override { visitWordAlternatives(n); }
