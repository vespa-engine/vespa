 ...
 **/
template <typename Unpack> class AndSearchNoStrict : public AndSearch {
public:
    /**
    ...
    AndSearchNoStrict(Children children, const Unpack& unpacker)
        : AndSearch(std::move(children)), _unpacker(unpacker) {}
...
template <typename Unpack> AndSearchNoStrict<Unpack>::~AndSearchNoStrict() = default;
