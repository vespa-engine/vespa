...
public:
    UnpackNeed() : _needs_normal_features(false), _needs_interleaved_features(false) {}
...
EquivBlueprint::EquivBlueprint(FieldSpecBaseList fields, allocate_outside_equiv_tag)
    : ComplexLeafBlueprint(std::move(fields)), _estimate(), _layout(), _terms(), _exactness() {
}
...
SearchIterator::UP EquivBlueprint::createLeafSearch(const fef::TermFieldMatchDataArray& outputs,
...
SearchIterator::UP EquivBlueprint::createFilterSearchImpl(FilterConstraint constraint) const {
    return create_or_filter(_terms, constraint);
}

void EquivBlueprint::visitMembers(vespalib::ObjectVisitor& visitor) const {
    LeafBlueprint::visitMembers(visitor);
    visit(visitor, "terms", _terms);
}

void EquivBlueprint::fetchPostings(const ExecuteInfo& execInfo) {
    for (size_t i = 0; i < _terms.size(); ++i) {
        _terms[i]->fetchPostings(execInfo);
    }
}

EquivBlueprint& EquivBlueprint::addTerm(Blueprint::UP term, double exactness) {
