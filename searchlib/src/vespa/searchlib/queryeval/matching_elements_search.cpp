...
MatchingElementsSearch::MatchingElementsSearch() : _matching_elements() {
}
...
public:
    FilterMatchingElementsSearch(const IAttributeVector& attr, const std::string& field_name,
                                 EntryRef                                           dictionary_snapshot,
...
template <typename BufferType, typename AttributeType>
FilterMatchingElementsSearch<BufferType, AttributeType>::FilterMatchingElementsSearch(
    const IAttributeVector& attr, const std::string& field_name, EntryRef dictionary_snapshot,
    std::span<const IDirectPostingStore::LookupResult> dict_entries)
...
template <typename BufferType, typename AttributeType>
void FilterMatchingElementsSearch<BufferType, AttributeType>::find_matching_elements(uint32_t          doc_id,
...
template <typename BufferType, typename AttributeType>
void FilterMatchingElementsSearch<BufferType, AttributeType>::initRange(uint32_t, uint32_t) {
}

std::unique_ptr<MatchingElementsSearch>
MatchingElementsSearch::create(const IAttributeVector& attr, const std::string& field_name,
                               EntryRef                                           dictionary_snapshot,
                               std::span<const IDirectPostingStore::LookupResult> dict_entries) {
    ...
    case BasicType::INT8:
        return std::make_unique<FilterMatchingElementsSearch<int64_t, IntegerAttributeTemplate<int8_t>>>(
            attr, field_name, dictionary_snapshot, dict_entries);
    case BasicType::INT16:
        return std::make_unique<FilterMatchingElementsSearch<int64_t, IntegerAttributeTemplate<int16_t>>>(
            attr, field_name, dictionary_snapshot, dict_entries);
    case BasicType::INT32:
        return std::make_unique<FilterMatchingElementsSearch<int64_t, IntegerAttributeTemplate<int32_t>>>(
            attr, field_name, dictionary_snapshot, dict_entries);
    case BasicType::INT64:
        return std::make_unique<FilterMatchingElementsSearch<int64_t, IntegerAttributeTemplate<int64_t>>>(
            attr, field_name, dictionary_snapshot, dict_entries);
    case BasicType::STRING:
        return std::make_unique<FilterMatchingElementsSearch<const char*, StringAttribute>>(
            attr, field_name, dictionary_snapshot, dict_entries);
    default:
        return {};
    }
...
template class hashtable<const char*, const char*, hash<const char*>, search::queryeval::EqualCStringValue, Identity,
                         hashtable_base::and_modulator>;
