...
FakeIndex::FakeIndex() : _current_doc(0), _current_field(0), _terms() {
}

FakeIndex::~FakeIndex() = default;

FakeIndex& FakeIndex::doc(uint32_t docid) {
    _current_doc = docid;
    return *this;
}

FakeIndex& FakeIndex::field(uint32_t field_id) {
    _current_field = field_id;
    return *this;
}

FakeIndex& FakeIndex::elem(uint32_t element_id, const std::string& layout) {
    ...
    for (size_t pos = 0; pos < layout.size(); ++pos) {
        ...
        if (ch != '.') {
            ...
            if (result.inspect().back().elements.empty() || result.inspect().back().elements.back().id != element_id)
            {
...
const FakeResult& FakeIndex::lookup(char ch, uint32_t field_id) const {
