...
public:
    ...
    const GlobalFilter* ptr_if_active() const { return is_active() ? this : nullptr; }
    ...
    static std::shared_ptr<GlobalFilter> create(Blueprint& blueprint, uint32_t docid_limit,
                                                vespalib::ThreadBundle& thread_bundle, Trace* trace);
    static std::shared_ptr<GlobalFilter> create(Blueprint& blueprint, uint32_t docid_limit,
