...
public:
    ...
    uint32_t get_docid(uint32_t ref) const { return _children[ref]->getDocId(); }
    ...
    void unpack(uint32_t ref, uint32_t docid) { _children[ref]->doUnpack(docid); }

    uint32_t size() const { return _children.size(); }
