...
public:
    ...
    static SearchIterator::UP create(const std::vector<SearchIterator*>& children,
                                     search::fef::TermFieldMatchData& tmd, bool is_filter_search,
                                     const std::vector<int32_t>& weights, fef::MatchData::UP match_data);

    static SearchIterator::UP
    create(search::fef::TermFieldMatchData& tmd, bool is_filter_search,
    ...
    static SearchIterator::UP create_hash_filter(search::fef::TermFieldMatchData& tmd, bool is_filter_search,
    ...
    virtual void find_matching_elements(uint32_t                                       docid,
                                        const std::vector<std::unique_ptr<Blueprint>>& child_blueprints,
                                        std::vector<uint32_t>&                         dst) = 0;
};
