...
BooleanMatchIteratorWrapper::BooleanMatchIteratorWrapper(SearchIterator::UP                  search,
                                                         const fef::TermFieldMatchDataArray& matchData)
    : _search(std::move(search)), _tfmdp(nullptr) {
    if (matchData.size() == 1) {
        _tfmdp = matchData[0];
    }
}
...
void BooleanMatchIteratorWrapper::visitMembers(vespalib::ObjectVisitor& visitor) const {
