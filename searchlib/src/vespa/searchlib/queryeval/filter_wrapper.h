...
public:
    explicit FilterWrapper(size_t num_fields) : _unused_md(num_fields), _tfmda(), _wrapped_search() {
    ...
    void wrap(std::unique_ptr<SearchIterator> wrapped) { _wrapped_search = std::move(wrapped); }
    void wrap(SearchIterator* wrapped) { _wrapped_search.reset(wrapped); }
    ...
    BitVector::UP get_hits(uint32_t begin_id) override { return _wrapped_search->get_hits(begin_id); }
    BitVectorMeta asBitVector() const noexcept override { return _wrapped_search->asBitVector(); }
    Trinary is_strict() const override { return _wrapped_search->is_strict(); }
    std::string getClassName() const override { return _wrapped_search->getClassName(); }
