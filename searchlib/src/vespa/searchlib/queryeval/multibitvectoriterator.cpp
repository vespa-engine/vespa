...
void MultiBitVectorBase::addBitVector(Meta bv, uint32_t docIdLimit) {
    _numDocs = std::min(_numDocs, docIdLimit);
    _bvs.push_back(bv);
}

template <typename Update>
MultiBitVector<Update>::MultiBitVector(size_t reserved) : MultiBitVectorBase(reserved), _update(), _lastWords() {
...
template <typename Update> bool MultiBitVector<Update>::updateLastValueCold(uint32_t docId) noexcept {
...
template <typename Update> void MultiBitVector<Update>::fetchChunk(uint32_t index) noexcept {
...
template <typename Update> uint32_t MultiBitVector<Update>::strictSeek(uint32_t docId) noexcept {
    bool atEnd;
    for (atEnd = updateLastValue(docId), _lastValue = _lastValue & ~BitWord::startBits(docId);
         __builtin_expect(_lastValue == 0, Update::isAnd()) &&
         __builtin_expect(!atEnd, true); // And is likely to have few bits, while Or has many.
...
template <typename Update> bool MultiBitVector<Update>::seek(uint32_t docId) noexcept {
    bool atEnd = updateLastValue(docId);
    return __builtin_expect(!atEnd, true) && __builtin_expect(_lastValue & BitWord::mask(docId), false);
}

namespace {

template <typename Update> class MultiBitVectorIterator : public MultiBitVectorIteratorBase {
public:
    explicit MultiBitVectorIterator(Children children)
        : MultiBitVectorIteratorBase(std::move(children)), _mbv(getChildren().size() + 1) {
...
template <typename Update> class MultiBitVectorIteratorStrict final : public MultiBitVectorIterator<Update> {
public:
...
template <typename Update> MultiBitVectorIterator<Update>::~MultiBitVectorIterator() = default;

template <typename Update> void MultiBitVectorIterator<Update>::doSeek(uint32_t docId) {
    if (_mbv.seek(docId)) [[unlikely]] {
        setDocId(docId);
    }
}

template <typename Update> SearchIterator::UP MultiBitVectorIterator<Update>::andWith(UP filter, uint32_t estimate) {
    (void)estimate;
...
bool canOptimize(const MultiSearch& s) {
    return (s.getChildren().size() >= 2) && (s.isAnd() || s.isOr() || s.isAndNot()) &&
           hasAtLeast2Bitvectors(s.getChildren());
}

} // namespace

MultiBitVectorIteratorBase::MultiBitVectorIteratorBase(Children children) : MultiSearch(std::move(children)) {
}

MultiBitVectorIteratorBase::~MultiBitVectorIteratorBase() = default;

std::string MultiBitVectorIteratorBase::make_id_ref_str() const {
    bool        first = true;
    std::string res;
    res.push_back('[');
    auto add = [&](int id) {
...
void MultiBitVectorIteratorBase::initRange(uint32_t beginId, uint32_t endId) {
    MultiSearch::initRange(beginId, endId);
}

void MultiBitVectorIteratorBase::doUnpack(uint32_t docid) {
...
SearchIterator::UP MultiBitVectorIteratorBase::optimize(SearchIterator::UP parentIt) {
    if (parentIt->isSourceBlender()) {
        parentIt->transform_children([](auto child) { return optimize(std::move(child)); });
...
SearchIterator::UP MultiBitVectorIteratorBase::optimizeMultiSearch(SearchIterator::UP parentIt) {
