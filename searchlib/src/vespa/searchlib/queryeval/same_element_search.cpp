...
bool SameElementSearch::check_docid_match(uint32_t docid) {
...
void SameElementSearch::fetch_matching_elements(uint32_t docid, std::vector<uint32_t>& elems) {
...
bool SameElementSearch::check_element_match(uint32_t docid) {
    _matchingElements.clear();
    fetch_matching_elements(docid, _matchingElements);
    return !_matchingElements.empty();
}

void SameElementSearch::hide_descendants_match_data() {
...
void SameElementSearch::filter_descendants_match_data(uint32_t docid, std::span<const uint32_t> element_ids) {
    for (auto* tfmd : _descendants_index_tfmd) {
...
void SameElementSearch::initRange(uint32_t begin_id, uint32_t end_id) {
...
void SameElementSearch::doSeek(uint32_t docid) {
...
void SameElementSearch::visitMembers(vespalib::ObjectVisitor& visitor) const {
    SearchIterator::visitMembers(visitor);
    visit(visitor, "children", _children);
    visit(visitor, "strict", _strict);
}

void SameElementSearch::find_matching_elements(uint32_t docid, std::vector<uint32_t>& dst) {
