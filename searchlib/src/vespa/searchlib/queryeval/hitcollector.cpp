...
void HitCollector::sortHitsByScore(size_t topn) {
    ...
    if (topn > _scoreOrder.size()) {
        ...
        ShiftBasedRadixSorter<uint32_t, IndirectScoreRadix, IndirectScoreComparator, 56, true>::radix_sort(
            IndirectScoreRadix(_hits.data()), IndirectScoreComparator(_hits.data()), _scoreOrder.data(),
            _scoreOrder.size(), 16, topn);
        _scoreOrder.resize(topn);
    }
}

void HitCollector::sortHitsByDocId() {
    if (_hitsSortOrder != SortOrder::DOC_ID) {
        ShiftBasedRadixSorter<Hit, DocIdRadix, DocIdComparator, 24>::radix_sort(DocIdRadix(), DocIdComparator(),
                                                                                _hits.data(), _hits.size(), 16);
...
void HitCollector::RankedHitCollector::collect(uint32_t docId, feature_t score) {
    HitCollector& hc = this->_hc;
    if (hc._hits.size() < hc._maxHitsSize) {
        if (__builtin_expect(
                ((!hc._hits.empty()) && (docId < hc._hits.back().first) && (hc._hitsSortOrder == SortOrder::DOC_ID)),
                false))
        {
...
void HitCollector::BitVectorCollector<CollectRankedHit>::collect(uint32_t docId, feature_t score) {
...
void HitCollector::CollectorBase::replaceHitInVector(uint32_t docId, feature_t score) noexcept {
...
void HitCollector::RankedHitCollector::collectAndChangeCollector(uint32_t docId, feature_t score) {
    HitCollector& hc = this->_hc;
...
template <bool CollectRankedHit>
void HitCollector::DocIdCollector<CollectRankedHit>::collect(uint32_t docId, feature_t score) {
    if (CollectRankedHit) {
        this->considerForHitVector(docId, score);
    }
    HitCollector& hc = this->_hc;
    if (hc._docIdVector.size() < hc._maxDocIdVectorSize) {
        if (__builtin_expect(
                ((!hc._docIdVector.empty()) && (docId < hc._docIdVector.back()) && (hc._unordered == false)), false))
        {
            hc._unordered = true;
        }
        hc._docIdVector.push_back(docId);
...
template <bool CollectRankedHit>
void HitCollector::DocIdCollector<CollectRankedHit>::collectAndChangeCollector(uint32_t docId) {
    HitCollector& hc = this->_hc;
...
SortedHitSequence HitCollector::getSortedHitSequence(size_t max_hits) {
    size_t num_hits = std::min(_hits.size(), max_hits);
    sortHitsByScore(num_hits);
    return {_hits.data(), _scoreOrder.data(), num_hits};
}

void HitCollector::setReRankedHits(std::vector<Hit> hits) {
...
void HitCollector::setRanges(const std::pair<Scores, Scores>& ranges) {
    _ranges = ranges;
}
...
template <typename Rescorer> class RerankRescorer {
    Rescorer _rescorer;
...
public:
    RerankRescorer(const Rescorer& rescorer, const HitVector& reranked_hits)
        : _rescorer(rescorer), _reranked_cur(reranked_hits.begin()), _reranked_end(reranked_hits.end()) {}
...
public:
    SimpleHitAdder(ResultSet& rs) : _rs(rs) {}
    void add(uint32_t docid, double rank_value) { _rs.push_back({docid, rank_value}); }
};
...
public:
    ConditionalHitAdder(ResultSet& rs, double second_phase_rank_drop_limit)
        : SimpleHitAdder(rs), _second_phase_rank_drop_limit(second_phase_rank_drop_limit) {}
...
public:
    TrackingConditionalHitAdder(ResultSet& rs, double second_phase_rank_drop_limit, std::vector<uint32_t>& dropped)
        : ConditionalHitAdder(rs, second_phase_rank_drop_limit), _dropped(dropped) {}
    void add(uint32_t docid, double rank_value) {
        if (rank_value > _second_phase_rank_drop_limit) {
            _rs.push_back({docid, rank_value});
        } else {
...
void add_rescored_hits(HitAdder hit_adder, const std::vector<HitCollector::Hit>& hits, Rescorer rescorer) {
    for (auto& hit : hits) {
...
template <typename HitAdder, typename Rescorer>
void add_rescored_hits(HitAdder hit_adder, const std::vector<HitCollector::Hit>& hits,
                       const std::vector<HitCollector::Hit>& reranked_hits, Rescorer rescorer) {
...
void add_rescored_hits(ResultSet& rs, const std::vector<HitCollector::Hit>& hits,
                       const std::vector<HitCollector::Hit>& reranked_hits,
                       std::optional<double> second_phase_rank_drop_limit, std::vector<uint32_t>* dropped,
                       Rescorer rescorer) {
    if (second_phase_rank_drop_limit.has_value()) {
        if (dropped != nullptr) {
            add_rescored_hits(TrackingConditionalHitAdder(rs, second_phase_rank_drop_limit.value(), *dropped), hits,
                              reranked_hits, rescorer);
        } else {
            add_rescored_hits(ConditionalHitAdder(rs, second_phase_rank_drop_limit.value()), hits, reranked_hits,
                              rescorer);
        }
    } else {
...
template <typename HitAdder, typename Rescorer>
void mixin_rescored_hits(HitAdder hit_adder, const std::vector<HitCollector::Hit>& hits,
                         const std::vector<uint32_t>& docids, double default_value, Rescorer rescorer) {
...
template <typename HitAdder, typename Rescorer>
void mixin_rescored_hits(HitAdder hit_adder, const std::vector<HitCollector::Hit>& hits,
                         const std::vector<uint32_t>& docids, double default_value,
                         const std::vector<HitCollector::Hit>& reranked_hits, Rescorer rescorer) {
    if (reranked_hits.empty()) {
...
template <typename Rescorer>
void mixin_rescored_hits(ResultSet& rs, const std::vector<HitCollector::Hit>& hits,
                         const std::vector<uint32_t>& docids, double default_value,
                         const std::vector<HitCollector::Hit>& reranked_hits,
                         std::optional<double> second_phase_rank_drop_limit, std::vector<uint32_t>* dropped,
                         Rescorer rescorer) {
    if (second_phase_rank_drop_limit.has_value()) {
        if (dropped != nullptr) {
            mixin_rescored_hits(TrackingConditionalHitAdder(rs, second_phase_rank_drop_limit.value(), *dropped), hits,
                                docids, default_value, reranked_hits, rescorer);
        } else {
            mixin_rescored_hits(ConditionalHitAdder(rs, second_phase_rank_drop_limit.value()), hits, docids,
                                default_value, reranked_hits, rescorer);
        }
    } else {
...
void add_bitvector_to_dropped(std::vector<uint32_t>& dropped, std::span<const RankedHit> hits, const BitVector& bv) {
    auto hits_cur = hits.begin();
    auto hits_end = hits.end();
...
std::unique_ptr<ResultSet> HitCollector::get_result_set(std::optional<double>  second_phase_rank_drop_limit,
    ...
    if (!_collector->isDocIdCollector() ||
        (second_phase_rank_drop_limit.has_value() && (_bitVector || dropped == nullptr)))
    {
        ...
        if (needReScore) {
            mixin_rescored_hits(*rs, _hits, _docIdVector, default_value, _reRankedHits, second_phase_rank_drop_limit,
                                dropped, rescorer);
        } else {
            mixin_rescored_hits(*rs, _hits, _docIdVector, default_value, _reRankedHits, second_phase_rank_drop_limit,
                                dropped, NoRescorer());
...
std::unique_ptr<ResultSet> HitCollector::getResultSet() {
