...
FlowStats EmptyBlueprint::calculate_flow_stats(uint32_t) const {
    return {0.0, 0.2, 0.0};
}

SearchIterator::UP EmptyBlueprint::createLeafSearch(const search::fef::TermFieldMatchDataArray&) const {
    return std::make_unique<EmptySearch>();
}

SearchIterator::UP EmptyBlueprint::createFilterSearchImpl(FilterConstraint /* constraint */) const {
    return std::make_unique<EmptySearch>();
}

EmptyBlueprint::EmptyBlueprint(FieldSpecBaseList fields) : SimpleLeafBlueprint(std::move(fields)) {
}

EmptyBlueprint::~EmptyBlueprint() = default;

FlowStats AlwaysTrueBlueprint::calculate_flow_stats(uint32_t docid_limit) const {
    return default_flow_stats(docid_limit, docid_limit, 0);
}

SearchIterator::UP AlwaysTrueBlueprint::createLeafSearch(const search::fef::TermFieldMatchDataArray&) const {
    return std::make_unique<FullSearch>();
}

SearchIterator::UP AlwaysTrueBlueprint::createFilterSearchImpl(FilterConstraint /* constraint */) const {
    return std::make_unique<FullSearch>();
}
...
FlowStats SimpleBlueprint::calculate_flow_stats(uint32_t docid_limit) const {
    return default_flow_stats(docid_limit, _result.getHitCount(), 0);
}

SearchIterator::UP SimpleBlueprint::createLeafSearch(const search::fef::TermFieldMatchDataArray&) const {
    auto search = std::make_unique<SimpleSearch>(_result, strict());
    search->tag(_tag);
    return search;
}

SearchIterator::UP SimpleBlueprint::createFilterSearchImpl(FilterConstraint constraint) const {
    auto search = std::make_unique<SimpleSearch>(_result, strict());
    search->tag(_tag + (strict() ? "<strict," : "<nostrict,") +
                (constraint == FilterConstraint::UPPER_BOUND ? "upper>" : "lower>"));
    return search;
}

SimpleBlueprint::SimpleBlueprint(const SimpleResult& result) : SimpleLeafBlueprint(), _tag(), _result(result) {
    setEstimate(HitEstimate(result.getHitCount(), (result.getHitCount() == 0)));
}

SimpleBlueprint::~SimpleBlueprint() = default;

SimpleBlueprint& SimpleBlueprint::tag(const std::string& t) {
    _tag = t;
    return *this;
}
...
struct FakeContext : attribute::ISearchContext {
    ...
    FakeContext(const std::string& name_in, const FakeResult& result_in) : name(name_in), result(result_in) {}
...
uint32_t FakeContext::get_committed_docid_limit() const noexcept {
...
SearchIterator::UP FakeBlueprint::createLeafSearch(const fef::TermFieldMatchDataArray& tfmda) const {
...
FakeBlueprint::FakeBlueprint(const FieldSpec& field, const FakeResult& result)
    : SimpleLeafBlueprint(field), _tag("<tag>"), _term("<term>"), _field(field), _result(result), _ctx() {
    setEstimate(HitEstimate(result.inspect().size(), result.inspect().empty()));
}

FakeBlueprint::~FakeBlueprint() = default;

FakeBlueprint& FakeBlueprint::is_attr(bool value) {
    if (value) {
