...
AndNotSearch::AndNotSearch(MultiSearch::Children children, bool elementwise)
    : MultiSearch(std::move(children)), _elementwise(elementwise) {
}
...
void AndNotSearch::doUnpack(uint32_t docid) {
    getChildren()[0]->doUnpack(docid);
}

SearchIterator::UP AndNotSearchStrictBase::andWith(UP filter, uint32_t estimate) {
...
private:
    template <bool doSeekOnlyOnPositiveChild> void internalSeek(uint32_t docid);

protected:
    void doSeek(uint32_t docid) override { internalSeek<true>(docid); }
...
template <bool doSeekOnlyOnPositiveChild> void AndNotSearchStrict::internalSeek(uint32_t docid) {
    const Children& children(getChildren());
    bool            hit;
...
std::unique_ptr<SearchIterator> AndNotSearch::create(ChildrenIterators children_in, bool strict) {
    return create(std::move(children_in), false, strict);
}

std::unique_ptr<SearchIterator> AndNotSearch::create(ChildrenIterators children_in, bool elementwise, bool strict) {
...
BitVector::UP AndNotSearch::get_hits(uint32_t begin_id) {
    const Children& children = getChildren();
...
void AndNotSearch::or_hits_into(BitVector& result, uint32_t begin_id) {
    result.orWith(*get_hits(begin_id));
}

void AndNotSearch::get_element_ids(uint32_t docid, std::vector<uint32_t>& element_ids) {
