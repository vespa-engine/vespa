...
void ElementIdExtractor::get_element_ids(const fef::TermFieldMatchData& tfmd, uint32_t docid,
...
void ElementIdExtractor::and_element_ids_into(const fef::TermFieldMatchData& tfmd, uint32_t docid,
                                              std::vector<uint32_t>& element_ids) {
    if (tfmd.has_data(docid)) {
        size_t  toKeep(0);
        int32_t id(-1);
