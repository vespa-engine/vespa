...
FakeResult::FakeResult() : _documents(), _minMaxPostingInfo() {
}
...
FakeResult& FakeResult::operator=(const FakeResult&) = default;
    ...
    if (doc.size() == 0) {
        ...
        for (size_t d = 0; d < doc.size(); ++d) {
            ...
            for (size_t e = 0; e < elem.size(); ++e) {
                out << "  ( ELEM id: " << elem[e].id << " weight: " << elem[e].weight << " len: " << elem[e].length
                    << " )" << std::endl;
...
std::vector<search::streaming::Hit> FakeResult::get_streaming_hits(uint32_t docid, uint32_t field_id) const {
