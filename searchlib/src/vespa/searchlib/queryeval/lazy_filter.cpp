...
LocationLazyFilter::LocationLazyFilter(Private, const common::Location& location,
                                       const Blueprint::HitEstimate& estimate) noexcept
    : _location(location), _docid_limit(_location.getVec()->getCommittedDocIdLimit()), _estimate(estimate) {
    _pos.resize(1); // Needed (single-value attribute), cf. LocationIterator
}

std::shared_ptr<LocationLazyFilter> LocationLazyFilter::create(const common::Location&       location,
                                                               const Blueprint::HitEstimate& estimate) {
    return std::make_shared<LocationLazyFilter>(Private(), location, estimate);
}

bool LocationLazyFilter::is_active() const {
    return true;
}

uint32_t LocationLazyFilter::size() const {
    return (_docid_limit == 0) ? 0 : _docid_limit - 1;
}

uint32_t LocationLazyFilter::count() const {
    return _estimate.empty ? 0 : std::min(size(), _estimate.estHits);
}

bool LocationLazyFilter::check(uint32_t docid) const {
...
std::shared_ptr<FallbackFilter> FallbackFilter::create(const GlobalFilter& global_filter,
                                                       const GlobalFilter& fallback) {
    return std::make_shared<FallbackFilter>(Private(), global_filter, fallback);
}

bool FallbackFilter::is_active() const {
    return true;
}

uint32_t FallbackFilter::size() const {
    return std::min(_global_filter.size(), _fallback.size());
}

uint32_t FallbackFilter::count() const {
    return std::min(_global_filter.count(), _fallback.count());
}

bool FallbackFilter::check(uint32_t docid) const {
...
std::shared_ptr<AndFilter> AndFilter::create(std::vector<std::shared_ptr<GlobalFilter>>&& children) {
    return std::make_shared<AndFilter>(Private(), std::move(children));
}

bool AndFilter::is_active() const {
...
uint32_t AndFilter::count() const {
    uint32_t min = std::numeric_limits<uint32_t>::max();
    for (auto& child : _children) {
...
bool AndFilter::check(uint32_t docid) const {
    for (auto& child : _children) {
