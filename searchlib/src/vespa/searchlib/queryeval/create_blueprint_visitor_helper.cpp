...
CreateBlueprintVisitorHelper::CreateBlueprintVisitorHelper(Searchable& searchable, const FieldSpec& field,
...
bool CreateBlueprintVisitorHelper::is_search_multi_threaded() const noexcept {
    return getRequestContext().thread_bundle().size() > 1;
}

attribute::SearchContextParams CreateBlueprintVisitorHelper::createContextParams() const {
    return attribute::SearchContextParams().metaStoreReadGuard(_requestContext.getMetaStoreReadGuard());
}
attribute::SearchContextParams CreateBlueprintVisitorHelper::createContextParams(bool useBitVector) const {
    return createContextParams().useBitVector(useBitVector);
}

Blueprint::UP CreateBlueprintVisitorHelper::getResult() {
    return _result ? std::move(_result) : std::make_unique<EmptyBlueprint>(_field);
}

void CreateBlueprintVisitorHelper::visitPhrase(query::Phrase& n) {
...
void CreateBlueprintVisitorHelper::handleNumberTermAsText(query::NumberTerm& n) {
    ...
    if (splitter.parts() > 1) {
        ...
        for (size_t i = 0; i < splitter.parts(); ++i) {
            phraseNode.append(
                std::make_unique<query::SimpleStringTerm>(splitter.getPart(i), "", 0, query::Weight(0)));
        }
...
void CreateBlueprintVisitorHelper::createWeightedSet(std::unique_ptr<WS> bp, NODE& n) {
    ...
    for (size_t i = 0; i < n.getNumTerms(); ++i) {
        ...
        bp->addTerm(_searchable.createBlueprint(_requestContext, childField, node, _global_layout),
...
void CreateBlueprintVisitorHelper::visitWeightedSetTerm(query::WeightedSetTerm& n) {
    createWeightedSet(std::make_unique<WeightedSetTermBlueprint>(_field), n);
}

void CreateBlueprintVisitorHelper::visitDotProduct(query::DotProduct& n) {
    createWeightedSet(std::make_unique<DotProductBlueprint>(_field), n);
}

void CreateBlueprintVisitorHelper::visitWandTerm(query::WandTerm& n) {
    createWeightedSet(std::make_unique<ParallelWeakAndBlueprint>(_field, n.getTargetNumHits(), n.getScoreThreshold(),
                                                                 n.getThresholdBoostFactor(),
                                                                 is_search_multi_threaded()),
                      n);
}

void CreateBlueprintVisitorHelper::visitInTerm(query::InTerm& n) {
    createWeightedSet(std::make_unique<WeightedSetTermBlueprint>(_field), n);
}
