...
private:
    ...
    struct DocIdComparator {
        bool operator()(const Hit& lhs, const Hit& rhs) const noexcept { return (lhs.first < rhs.first); }
    };
    ...
    template <bool CollectRankedHit> class DocIdCollector final : public CollectorBase {
    public:
    ...
    template <bool CollectRankedHit> class BitVectorCollector final : public CollectorBase {
    public:
...
public:
     ...
     **/
    void addHit(uint32_t docId, feature_t score) { _collector->collect(docId, score); }
    ...
    std::unique_ptr<ResultSet> get_result_set(std::optional<double>  second_phase_rank_drop_limit,
