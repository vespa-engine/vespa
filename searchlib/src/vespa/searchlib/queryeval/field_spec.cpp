...
FieldSpec::FieldSpec(const std::string& name, uint32_t fieldId, fef::TermFieldHandle handle, bool isFilter_) noexcept
    : FieldSpec(name, fieldId, handle, fef::FilterThreshold(isFilter_)) {
}

FieldSpec::FieldSpec(const std::string& name, uint32_t fieldId, fef::TermFieldHandle handle,
                     fef::FilterThreshold threshold) noexcept
    : FieldSpecBase(fieldId, handle, threshold.is_filter()), _name(name), _threshold(threshold) {
