...
public:
    SelectiveUnpack(const UnpackInfo& unpackInfo) : _unpackInfo(unpackInfo) {}
    void unpack(uint32_t docid, const MultiSearch& search) {
        auto& children = search.getChildren();
        _unpackInfo.each(
            [&children, docid](size_t i) {
    ...
    void each(auto&& f, size_t n) { _unpackInfo.each(std::forward<decltype(f)>(f), n); }
    void onRemove(size_t index) { _unpackInfo.remove(index); }
    void onInsert(size_t index) { _unpackInfo.insert(index); }
    bool needUnpack(size_t index) const { return _unpackInfo.needUnpack(index); }
...
SearchIterator::UP create_strict_or(std::vector<SearchIterator::UP> children, const Unpack& unpack,
                                    OrSearch::StrictImpl strict_impl) {
...
BitVector::UP OrSearch::get_hits(uint32_t begin_id) {
    return TermwiseHelper::orChildren(getChildren().begin(), getChildren().end(), begin_id);
}

void OrSearch::and_hits_into(BitVector& result, uint32_t begin_id) {
    result.andWith(*get_hits(begin_id));
}

void OrSearch::or_hits_into(BitVector& result, uint32_t begin_id) {
    TermwiseHelper::orChildren(result, getChildren().begin(), getChildren().end(), begin_id);
}

void OrSearch::get_element_ids(uint32_t docid, std::vector<uint32_t>& element_ids) {
...
SearchIterator::UP OrSearch::create(ChildrenIterators children, bool strict) {
    UnpackInfo unpackInfo;
    unpackInfo.forceAll();
    return create(std::move(children), strict, unpackInfo);
}

SearchIterator::UP OrSearch::create(ChildrenIterators children, bool strict, const UnpackInfo& unpackInfo) {
