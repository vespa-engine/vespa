...
SimplePhraseBlueprint::SimplePhraseBlueprint(const FieldSpec& field, bool expensive)
    : ComplexLeafBlueprint(field), _field(field), _estimate(), _terms() {
    if (expensive) {
...
FlowStats SimplePhraseBlueprint::calculate_flow_stats(uint32_t docid_limit) const {
    ...
    return {est, AndFlow::cost_of(_terms, false) + est * _terms.size(),
...
SearchIterator::UP SimplePhraseBlueprint::createFilterSearchImpl(FilterConstraint constraint) const {
    return create_atmost_and_filter(_terms, constraint);
}

void SimplePhraseBlueprint::fetchPostings(const ExecuteInfo& execInfo) {
    for (auto& term : _terms) {
        term->fetchPostings(execInfo);
    }
}

void SimplePhraseBlueprint::visitMembers(vespalib::ObjectVisitor& visitor) const {
