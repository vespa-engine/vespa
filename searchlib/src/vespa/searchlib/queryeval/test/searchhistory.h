...
struct SearchHistory {
    struct Entry {
        ...
        std::string op;
        uint32_t    docid;
        Entry(const std::string& t, const std::string& o, uint32_t id) : target(t), op(o), docid(id) {}
        Entry(const Entry&);
        ...
        bool operator==(const Entry& rhs) const {
            return ((target == rhs.target) && (op == rhs.op) && (docid == rhs.docid));
        }
    };
    ...
    bool operator==(const SearchHistory& rhs) const { return (_entries == rhs._entries); }
};
