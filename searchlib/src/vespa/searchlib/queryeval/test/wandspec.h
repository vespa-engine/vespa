    ...
    wand::Terms getTerms(fef::MatchData* matchData = nullptr) {
        ...
        for (size_t i = 0; i < _leafs.size(); ++i) {
            fef::TermFieldMatchData* tfmd =
                (matchData != nullptr ? matchData->resolveTermField(_handles[i]) : nullptr);
            terms.push_back(wand::Term(_leafs[i].create(_history, tfmd).release(), _leafs[i].weight,
                                       _leafs[i].result.inspect().size(), tfmd));
        }
        return terms;
    }
