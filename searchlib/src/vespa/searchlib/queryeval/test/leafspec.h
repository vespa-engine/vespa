...
struct LeafSpec {
    ...
    LeafSpec(const std::string& n, int32_t w = 100)
        : name(n), weight(w), maxWeight(std::numeric_limits<int32_t>::min()), result(), search() {}
    ...
    SearchIterator::UP create(SearchHistory& hist, fef::TermFieldMatchData* tfmd) {
        if (search) {
            ...
            return SearchIterator::UP(new TrackedSearch(name, hist, result, *tfmd, MinMaxPostingInfo(0, maxWeight)));
        }
        return SearchIterator::UP(new TrackedSearch(name, hist, result, MinMaxPostingInfo(0, maxWeight)));
    }
};
