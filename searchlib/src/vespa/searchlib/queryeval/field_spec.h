...
class FieldSpecBase {
public:
    FieldSpecBase(uint32_t fieldId, fef::TermFieldHandle handle) noexcept : FieldSpecBase(fieldId, handle, false) {}
    FieldSpecBase(uint32_t fieldId, fef::TermFieldHandle handle, bool isFilter_) noexcept
        : _fieldId(fieldId | (isFilter_ ? 0x1000000u : 0)), _handle(handle) {}
...
public:
    ...
    FieldSpec(const std::string& name, uint32_t fieldId, fef::TermFieldHandle handle,
              fef::FilterThreshold threshold) noexcept;
    ~FieldSpec();

    void setBase(FieldSpecBase base) noexcept { FieldSpecBase::operator=(base); }
