...
public:
    ...
    static inline bool GetFeature(uint8_t type, uint8_t feature) { return ((type & feature) != 0); }

    static inline bool GetFeature_Weight(uint8_t type) { return GetFeature(type, IF_WEIGHT); }

    static inline bool getFeature_UniqueId(uint8_t type) { return GetFeature(type, IF_UNIQUEID); }
    static inline bool getFeature_Flags(uint8_t type) { return GetFeature(type, IF_FLAGS); }
    static TermType toTermType(ItemType itemType) noexcept {
        switch (itemType) {
        case ParseItem::ITEM_REGEXP:
            return TermType::REGEXP;
        case ParseItem::ITEM_PREFIXTERM:
            return TermType::PREFIXTERM;
        case ParseItem::ITEM_SUBSTRINGTERM:
            return TermType::SUBSTRINGTERM;
        case ParseItem::ITEM_EXACTSTRINGTERM:
            return TermType::EXACTSTRINGTERM;
        ...
        case ParseItem::ITEM_NEAREST_NEIGHBOR:
            return TermType::NEAREST_NEIGHBOR;
        default:
            return TermType::WORD;
        }
    }
};
