...
RankingExpressions& RankingExpressions::add(const std::string& name, const std::string& path) {
    _expressions.insert_or_assign(name, path);
    return *this;
}

std::string RankingExpressions::loadExpression(const std::string& name) const {
