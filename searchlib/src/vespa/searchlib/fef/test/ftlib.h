...
public:
    StringList& add(std::string_view str) {
        emplace_back(str);
        return *this;
    }
    StringList& clear() {
...
public:
    StringSet& add(const std::string& str) {
        insert(str);
        return *this;
    }
    StringSet& clear() {
...
public:
    ...
    bool execute(feature_t expected, double epsilon = 0, uint32_t docId = 1) {
        return _test.execute(expected, epsilon, docId);
    }
    bool execute(const search::fef::test::RankResult& expected, uint32_t docId = 1) {
        return _test.execute(expected, docId);
    }
    bool executeOnly(search::fef::test::RankResult& result, uint32_t docId = 1) {
        return _test.executeOnly(result, docId);
    }
...
struct FtQueryTerm {
    ...
    bool operator<(const FtQueryTerm& rhs) const noexcept { return term < rhs.term; }
    bool operator==(const FtQueryTerm& rhs) const noexcept { return term == rhs.term; }
};
...
struct FtIndex {
    struct Element {
        ...
        int32_t weight;
        Tokens  tokens;
        Element(int32_t w, const Tokens& t) : weight(w), tokens(t) {}
    };
