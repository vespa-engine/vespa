...
OnnxModels::OnnxModels(Vector models) : _models() {
    for (auto& model : models) {
        _models.emplace(model.name(), std::move(model));
    }
}

bool OnnxModels::operator==(const OnnxModels& rhs) const {
    return (_models == rhs._models);
}

const OnnxModels::Model* OnnxModels::getModel(const std::string& name) const {
