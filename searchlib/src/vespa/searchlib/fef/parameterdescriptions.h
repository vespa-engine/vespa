...
private:
    ...
    static uint32_t asMask(DataType dataType) { return (1u << static_cast<unsigned int>(dataType)); }
    static uint32_t numericTypesMask() {
        return (asMask(DataType::BOOL) | asMask(DataType::UINT2) | asMask(DataType::UINT4) | asMask(DataType::INT8) |
                asMask(DataType::INT16) | asMask(DataType::INT32) | asMask(DataType::INT64) |
                asMask(DataType::FLOAT) | asMask(DataType::DOUBLE));
    }
    static uint32_t primitiveTypesMask() { return (numericTypesMask() | asMask(DataType::STRING)); }
    static uint32_t allTypesMask() {
        return (primitiveTypesMask() | asMask(DataType::RAW) | asMask(DataType::BOOLEANTREE) |
                asMask(DataType::TENSOR) | asMask(DataType::REFERENCE) | asMask(DataType::COMBINED));
    }
    static uint32_t tensor_type_mask() { return asMask(DataType::TENSOR); }
    ParameterDataTypeSet(uint32_t typeMask) : _typeMask(typeMask) {}

public:
    ParameterDataTypeSet() : ParameterDataTypeSet(allTypesMask()) {}
    static ParameterDataTypeSet numericTypeSet() { return ParameterDataTypeSet(numericTypesMask()); }
    static ParameterDataTypeSet primitiveTypeSet() { return ParameterDataTypeSet(primitiveTypesMask()); }
    ...
    bool allowedType(DataType dataType) const { return ((asMask(dataType) & _typeMask) != 0); }
};
...
struct ParamDescItem {
    ...
    ParamDescItem(ParameterType::Enum t, ParameterCollection::Enum c) : type(t), dataTypeSet(), collection(c) {}
    ParamDescItem(ParameterType::Enum t, ParameterDataTypeSet dts, ParameterCollection::Enum c)
        : type(t), dataTypeSet(dts), collection(c) {}
};

/**
 * This class represents a set of parameter descriptions that each indicate what are a valid input parameter list for
 * a Blueprint. During setup of a Blueprint the descriptions can be used to validate the input parameter list for that
 * Blueprint. The parameters are valid if one of the descriptions match the actual parameter list.
 */
...
public:
    ...
    public:
         ...
         * If this description has repeat the index can be out of bounds (the correct repeat parameter will be
         * returned).
         */
...
private:
    ...
    void addParameter(ParameterType::Enum type, ParameterDataTypeSet dataTypeSet,
                      ParameterCollection::Enum collection) {
    ...
    void addParameter(ParameterType::Enum type) { addParameter(type, ParameterCollection::ANY); }
