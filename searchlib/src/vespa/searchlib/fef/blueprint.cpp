...
bool Blueprint::fail(const char* format, ...) {
    va_list ap;
    va_start(ap, format);
...
Blueprint::Blueprint(std::string_view baseName) : _baseName(baseName), _name(), _dependency_handler(nullptr) {
}

Blueprint::~Blueprint() = default;

ParameterDescriptions Blueprint::getDescriptions() const {
    // desc: 0-n parameters
    return ParameterDescriptions().desc().string().repeat();
}

bool Blueprint::setup(const IIndexEnvironment& indexEnv, const StringVector& params) {
    ...
    if (result.valid()) {
        ...
        return fail("The parameter list used for setting up rank feature %s is not valid: %s", getBaseName().c_str(),
                    result.getError().c_str());
    }
}

bool Blueprint::setup(const IIndexEnvironment& indexEnv, const ParameterList& params) {
    (void)indexEnv;
    (void)params;
    return fail("The setup function using a typed parameter list does not have a default implementation. "
                "Make sure the setup function is implemented in the rank feature %s.",
                getBaseName().c_str());
}

void Blueprint::prepareSharedState(const IQueryEnvironment& queryEnv, IObjectStore& objectStore) const {
    (void)queryEnv;
    (void)objectStore;
}
...
const attribute::IAttributeVector* Blueprint::lookupAndStoreAttribute(const std::string&       key,
...
const attribute::IAttributeVector* Blueprint::lookupAttribute(const std::string& key, std::string_view attrName,
                                                              const IQueryEnvironment& env) {
    const Anything*         attributeArg = env.getObjectStore().get(key);
    const IAttributeVector* attribute =
        (attributeArg != nullptr) ? IAttributeVectorWrapper::getValue(*attributeArg) : nullptr;
...
std::string Blueprint::createAttributeKey(std::string_view attrName) {
