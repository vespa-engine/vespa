...
RankingAssetsBuilder::RankingAssetsBuilder(FNET_Transport*    transport,
                                           const std::string& file_distributor_connection_spec)
    : _file_acquirer(), _time_box(vespalib::to_s(file_resolve_timeout), 5) {
...
std::string RankingAssetsBuilder::resolve_file(const std::string& desc, const std::string& fileref) {
    ...
    LOG(debug, "Got file path from file acquirer: '%s' (%s, ref='%s')", file_path.c_str(), desc.c_str(),
        fileref.c_str());
    if (file_path == "") {
        throw config::ConfigTimeoutException(
            fmt("could not get file path from file acquirer for %s (ref=%s)", desc.c_str(), fileref.c_str()));
    }
    return file_path;
}

std::shared_ptr<const OnnxModels> RankingAssetsBuilder::build(const OnnxModelsConfig& config) {
...
std::shared_ptr<const RankingConstants> RankingAssetsBuilder::build(const RankingConstantsConfig& config) {
...
std::shared_ptr<const RankingExpressions> RankingAssetsBuilder::build(const RankingExpressionsConfig& config) {
