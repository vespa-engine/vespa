...
/**
 * Property to control the auto-adjustment of targetHits in a nearestNeighbor search using HNSW index with
 *post-filtering.
 *
 ...
 *
 * This property ensures an upper bound of adjustedTargetHits, avoiding that the search in the HNSW index takes too
 *long.
 **/
...
struct ElementGap {
    static const std::string NAME;
    static std::optional<search::fef::ElementGap> lookup_for_field(const Properties&  props,
...
/**
 * Property for how many hits out of wanted hits to collect before considering graceful degradation during match
 *phase.
 **/
