...
struct Override {
    ...
    Override(const BlueprintResolver::FeatureRef& r, feature_t v) noexcept : ref(r), number(v), object() {}

    Override(const BlueprintResolver::FeatureRef& r, Value::UP v) noexcept : ref(r), number(), object(std::move(v)) {}

    bool operator<(const Override& rhs) const noexcept { return (ref.executor < rhs.ref.executor); }
};
...
struct OverrideVisitor : public IPropertiesVisitor {
    ...
    OverrideVisitor(const BlueprintResolver::ExecutorSpecList& specs_in,
                    const BlueprintResolver::FeatureMap& feature_map_in, std::vector<Override>& overrides_out)
    ...
    void visitProperty(const Property::Value& key, const Property& prop) override {
        ...
        if (pos != feature_map.end()) {
            ...
            if (feature_type.is_object()) {
                ...
                try {
                    auto tensor =
                        vespalib::eval::decode_value(stream, vespalib::eval::FastValueBuilderFactory::get());
                    if (tensor->type() == value_type) {
                        overrides.emplace_back(ref, std::move(tensor));
                    } else {
                        Issue::report("override for feature '%s' has invalid type: expected %s, got %s", name.c_str(),
...
struct UnboxingExecutor : FeatureExecutor {
    ...
    void execute(uint32_t) override { outputs().set_number(0, inputs().get_object(0).get().as_double()); }
};
...
struct ProfiledExecutor : FeatureExecutor {
    ...
    ProfiledExecutor(ExecutionProfiler& profiler_in, FeatureExecutor& executor_in, const std::string& name)
        : profiler(profiler_in), executor(executor_in), self(profiler.resolve(name)) {}
    void handle_bind_match_data(const MatchData& md) override { executor.bind_match_data(md); }
    void handle_bind_inputs(std::span<const LazyValue> inputs) override { executor.bind_inputs(inputs); }
    void handle_bind_outputs(std::span<NumberOrObject> outputs) override { executor.bind_outputs(outputs); }
    bool isPure() override { return executor.isPure(); }
...
bool RankProgram::check_const(FeatureExecutor*                                  executor,
                              const std::vector<BlueprintResolver::FeatureRef>& inputs) const {
    if (!executor->isPure()) {
        return false;
    }
...
void RankProgram::run_const(FeatureExecutor* executor) {
...
void RankProgram::unbox(BlueprintResolver::FeatureRef seed, const MatchData& md) {
...
FeatureResolver RankProgram::resolve(const BlueprintResolver::FeatureMap& features, bool unbox_seeds) const {
...
RankProgram::RankProgram(BlueprintResolver::SP resolver)
    : _resolver(std::move(resolver)), _hot_stash(32_Ki), _cold_stash(), _executors(), _unboxed_seeds(), _is_const() {
}
...
void RankProgram::setup(const MatchData& md, const IQueryEnvironment& queryEnv, const Properties& featureOverrides,
    ...
    for (uint32_t i = 0; i < specs.size(); ++i) {
        ...
        for (; (override < override_end) && (override->ref.executor == i); ++override) {
            ...
            executor = &(stash.get().create<FeatureOverrider>(*tmp, override->ref.output, override->number,
                                                              std::move(override->object)));
        }
        if (profiler) {
            FeatureExecutor* tmp = executor;
...
FeatureResolver RankProgram::get_seeds(bool unbox_seeds) const {
    return resolve(_resolver->getSeedMap(), unbox_seeds);
}

FeatureResolver RankProgram::get_all_features(bool unbox_seeds) const {
