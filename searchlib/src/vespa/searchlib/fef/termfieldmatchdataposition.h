...
public:
    TermFieldMatchDataPositionKey() : _elementId(0u), _position(0u) {}

    TermFieldMatchDataPositionKey(uint32_t elementId, uint32_t position)
        : _elementId(elementId), _position(position) {}
    ...
    bool operator==(const TermFieldMatchDataPositionKey& rhs) const {
        return ((_elementId == rhs._elementId) && (_position == rhs._position));
    }
};
...
public:
    ...
    const TermFieldMatchDataPositionKey& key() const { return *this; }
    ...
    TermFieldMatchDataPosition(uint32_t elementId, uint32_t position, int32_t elementWeight, uint32_t elementLen)
    ...
    void setElementWeight(int32_t elementWeight) { _elementWeight = elementWeight; }
