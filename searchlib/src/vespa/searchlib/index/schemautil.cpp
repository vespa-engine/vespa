...
SchemaUtil::IndexSettings SchemaUtil::getIndexSettings(const Schema& schema, const uint32_t index) {
...
bool SchemaUtil::validateIndexField(const Schema::IndexField& field) {
    bool ok = true;
    if (!validateIndexFieldType(field.getDataType())) {
        LOG(error, "Field %s has bad data type", field.getName().c_str());
        ok = false;
    }
    return ok;
}

bool SchemaUtil::addIndexField(Schema& schema, const Schema::IndexField& field) {
    bool ok = true;
    ...
    if (fieldId != Schema::UNKNOWN_FIELD_ID) {
        LOG(error, "Field %s already exists in schema", field.getName().c_str());
        ok = false;
    }
    if (ok) {
...
bool SchemaUtil::validateSchema(const Schema& schema) {
    bool ok = true;
    ...
    for (uint32_t fsId = 0; fsId < schema.getNumFieldSets(); ++fsId) {
        ...
        if (schema.getFieldSetId(fs.getName()) != fsId) {
            LOG(error, "Duplicate field set %s", fs.getName().c_str());
            ok = false;
        }
    }
    return ok;
...
bool SchemaUtil::getIndexIds(const Schema& schema, schema::DataType dataType, std::vector<uint32_t>& indexes) {
    indexes.clear();
