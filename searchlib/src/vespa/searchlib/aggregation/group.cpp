...
struct SortByGroupId {
    bool operator()(const Group::ChildP& a, const Group::ChildP& b) { return (a->cmpId(*b) < 0); }
};

struct SortByGroupRank {
    bool operator()(const Group::ChildP& a, const Group::ChildP& b) { return (a->cmpRank(*b) < 0); }
};

void reset(Group*& v) {
    v = nullptr;
}
void destruct(Group* v) {
    delete v;
}
...
void Group::selectMembers(const vespalib::ObjectPredicate& predicate, vespalib::ObjectOperation& operation) {
    if (_id.get()) {
...
void Group::aggregate(const Grouping& grouping, uint32_t currentLevel, const Doc& doc, HitRank rank) {
...
template <typename Doc> void Group::groupNext(const GroupingLevel& level, const Doc& doc, HitRank rank) {
...
void Group::merge(const GroupingLevelList& levels, uint32_t firstLevel, uint32_t currentLevel, Group& b) {
...
void Group::prune(const Group& b, uint32_t lastLevel, uint32_t currentLevel) {
...
void Group::mergePartial(const GroupingLevelList& levels, uint32_t firstLevel, uint32_t lastLevel,
...
void Group::visitMembers(vespalib::ObjectVisitor& visitor) const {
    visit(visitor, "id", _id);
    visit(visitor, "rank", _rank);
    _aggr.visitMembers(visitor);
}

Group::Group() noexcept : _id(), _rank(0), _aggr() {
}
...
Group& Group::partialCopy(const Group& rhs) {
    setId(*rhs._id);
    _rank = rhs._rank;
...
template void Group::aggregate(const Grouping& grouping, uint32_t currentLevel, const document::Document& doc,
                               HitRank rank);
...
void Group::Value::addOrderBy(ExpressionNode::UP orderBy, bool ascending) {
...
void Group::Value::postAggregate() {
...
void Group::Value::mergePartial(const GroupingLevelList& levels, uint32_t firstLevel, uint32_t lastLevel,
...
void Group::Value::postMerge(const std::vector<GroupingLevel>& levels, uint32_t firstLevel, uint32_t currentLevel) {
    bool frozen = (currentLevel < firstLevel); // is this level frozen ?
            ...
            if (!(_children[i]->cmpId(*_children[i - 1]) > 0)) {
                auto msg =
                    vespalib::make_string("Group::Value::validate_id_order: Expected %s > %s, %u children",
                                          ...
                                          child_id_as_string(i - 1, _children[i - 1]).c_str(), getChildrenSize());
...
void Group::Value::visitMembers(vespalib::ObjectVisitor& visitor) const {
...
Group::Value& Group::Value::operator=(Value&& rhs) noexcept {
    _childrenLength = rhs._childrenLength;
    _tag = rhs._tag;
    _packedLength = rhs._packedLength;
    _aggregationResults = rhs._aggregationResults;
...
Group::Value& Group::Value::operator=(const Value& rhs) {
    Value tmp(rhs);
    tmp.swap(*this);
    return *this;
}
...
void Group::Value::swap(Value& rhs) {
...
private:
    void execute(vespalib::Identifiable& obj) override {
        static_cast<AggregationRefNode&>(obj).locateExpression(_exprVec);
    }
...
void Group::Value::setupAggregationReferences() {
