...
void selectGroups(const vespalib::ObjectPredicate& p, vespalib::ObjectOperation& op, Group& group, uint32_t first,
...
public:
    ...
    bool check(const vespalib::Identifiable& obj) const override { return obj.inherits(FS4Hit::classId); }
};
...
struct ResolveCurrentIndex : vespalib::ObjectOperation, vespalib::ObjectPredicate {
    ...
    bool check(const vespalib::Identifiable& obj) const override {
        ...
        return obj.inherits(AttributeNode::classId) || obj.inherits(DocumentFieldNode::classId);
    }
};
...
Grouping::Grouping() noexcept
    : _id(0), _valid(true), _all(false), _topN(-1), _firstLevel(0), _lastLevel(0), _levels(), _root() {
}
...
Serializer& Grouping::onSerialize(Serializer& os) const {
    ...
    return os << _id << _valid << _all << _topN << _firstLevel << _lastLevel << LevelListSerializer(_levels) << _root;
}

Deserializer& Grouping::onDeserialize(Deserializer& is) {
    is >> _id >> _valid >> _all >> _topN >> _firstLevel >> _lastLevel >> LevelListDeserializer(_levels) >> _root;
    LOG(spam, "Grouping = %s", asString().c_str());
    return is;
}

void Grouping::visitMembers(vespalib::ObjectVisitor& visitor) const {
