...
Serializer& Hit::onSerialize(Serializer& os) const {
    return os.put(_rank);
}

Deserializer& Hit::onDeserialize(Deserializer& is) {
    return is.get(_rank);
}

int Hit::onCmp(const Identifiable& b) const {
    const Hit& h = (const Hit&)b;
    return (_rank > h._rank) ? -1 : ((_rank < h._rank) ? 1 : 0);
}

void Hit::visitMembers(vespalib::ObjectVisitor& visitor) const {
