...
private:
    ...
    public:
        ...
        virtual void group(Group& group, const ResultNode& result, const document::Document& doc,
                           HitRank rank) const = 0;
    ...
    public:
        SingleValueGrouper(Filter& filter, const Grouping* grouping, uint32_t level) noexcept
            : Grouper(grouping, level), _filter(filter) {}
    ...
    public:
        MultiValueGrouper(CurrentIndex& currentIndex, Filter& filter, const Grouping* grouping,
                          uint32_t level) noexcept
            : SingleValueGrouper(filter, grouping, level), _currentIndex(currentIndex) {}
...
public:
    ...
    GroupingLevel& freeze() {
        _frozen = true;
        return *this;
    }
    GroupingLevel& setPresicion(int64_t precision) {
        _precision = precision;
        return *this;
    }
    GroupingLevel& setExpression(ExpressionNode::UP root) {
        _classify = std::move(root);
        return *this;
    }
    GroupingLevel& addResult(ExpressionNode::UP result) {
        _collect.addResult(std::move(result));
        return *this;
    }
    GroupingLevel& addResult(const ExpressionNode& result) { return addResult(ExpressionNode::UP(result.clone())); }
    GroupingLevel& addAggregationResult(ExpressionNode::UP aggr) {
        _collect.addAggregationResult(std::move(aggr));
        return *this;
    }
    GroupingLevel& addOrderBy(ExpressionNode::UP orderBy, bool ascending) {
        _collect.addOrderBy(std::move(orderBy), ascending);
        return *this;
    }
    ...
    bool allowMoreGroups(size_t sz) const noexcept {
        return (!_frozen && (!_isOrdered || (sz < (uint64_t)_precision)));
    }
    ...
    void wire_current_index(CurrentIndexSetup& setup, const vespalib::ObjectPredicate& resolve_pred,
    ...
    template <typename Doc> void group(Group& g, const ResultNode& result, const Doc& doc, HitRank rank) const {
