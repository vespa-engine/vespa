 ...
 * Sketch interface.
 */
template <int BucketBits, typename HashT> struct Sketch {
...
template <typename T> uint8_t countPrefixZeros(T t) {
    uint8_t count = 0;
 ...
 */
template <int BucketBits = 10, typename HashT = uint32_t> struct SparseSketch : Sketch<BucketBits, HashT> {
    ...
    int aggregate(HashT hash) override { return hash_set.insert(hash).second ? 1 : 0; }
 ...
 */
template <int BucketBits = 10, typename HashT = uint32_t> struct NormalSketch : Sketch<BucketBits, HashT> {
    using typename Sketch<BucketBits, HashT>::SketchType;
...
template <int BucketBits, typename HashT> SparseSketch<BucketBits, HashT>::~SparseSketch() = default;

template <int BucketBits, typename HashT>
void SparseSketch<BucketBits, HashT>::serialize(vespalib::Serializer& os) const {
...
template <int BucketBits, typename HashT>
void SparseSketch<BucketBits, HashT>::deserialize(vespalib::Deserializer& is) {
    uint32_t size;
    is >> size;
...
template <int BucketBits, typename HashT>
uint32_t NormalSketch<BucketBits, HashT>::compress_buckets_into(char* buffer, uint32_t size) const {
...
template <int BucketBits, typename HashT>
void NormalSketch<BucketBits, HashT>::decompress_buckets_from(char* buffer, uint32_t size) {
    if (size == BUCKET_COUNT) { // not compressed
        ...
        vespalib::compression::decompress(vespalib::compression::CompressionConfig::LZ4, BUCKET_COUNT, compressed,
                                          uncompressed, false);
    }
}
template <int BucketBits, typename HashT>
void NormalSketch<BucketBits, HashT>::serialize(vespalib::Serializer& os) const {
