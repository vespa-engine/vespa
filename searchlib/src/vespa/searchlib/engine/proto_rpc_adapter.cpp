...
CompressionConfig get_compression_config() {
    ...
    return CompressionConfig(streamer.getCompressionType(), streamer.getCompressionLevel(), 80,
                             streamer.getCompressionLimit());
}

template <typename MSG> void encode_message(const MSG& src, FRT_Values& dst) {
...
template <typename MSG> bool decode_message(const FRT_Values& src, MSG& dst) {
...
ProtoRpcAdapter::ProtoRpcAdapter(SearchServer& search_server, DocsumServer& docsum_server,
                                 MonitorServer& monitor_server, FRT_Supervisor& orb)
    ...
    //-------------------------------------------------------------------------
    rb.DefineMethod("vespa.searchprotocol.search", "bix", "bix", FRT_METHOD(ProtoRpcAdapter::rpc_search), this);
    ...
    //-------------------------------------------------------------------------
    rb.DefineMethod("vespa.searchprotocol.getDocsums", "bix", "bix", FRT_METHOD(ProtoRpcAdapter::rpc_getDocsums),
                    this);
    rb.MethodDesc("fetch document summaries from this back-end");
    rb.RequestAccessFilter(make_search_api_capability_filter());
    describe_bix_param_return(rb);
    //-------------------------------------------------------------------------
    rb.DefineMethod("vespa.searchprotocol.ping", "bix", "bix", FRT_METHOD(ProtoRpcAdapter::rpc_ping), this);
...
void ProtoRpcAdapter::rpc_search(FRT_RPCRequest* req) {
    if (!is_online()) {
...
void ProtoRpcAdapter::rpc_getDocsums(FRT_RPCRequest* req) {
    if (!is_online()) {
        return req->SetError(FRTE_RPC_METHOD_FAILED, "Server not online");
    }
    req->Detach();
...
void ProtoRpcAdapter::rpc_ping(FRT_RPCRequest* rpc) {
    if (!is_online()) {
...
void ProtoRpcAdapter::encode_search_request(const ProtoSearchRequest& src, FRT_RPCRequest& dst) {
    dst.SetMethodName("vespa.searchprotocol.search");
    encode_message(src, *dst.GetParams());
}

bool ProtoRpcAdapter::decode_search_reply(FRT_RPCRequest& src, ProtoSearchReply& dst) {
    return (src.CheckReturnTypes("bix") && decode_message(*src.GetReturn(), dst));
}

void ProtoRpcAdapter::encode_docsum_request(const ProtoDocsumRequest& src, FRT_RPCRequest& dst) {
    dst.SetMethodName("vespa.searchprotocol.getDocsums");
    encode_message(src, *dst.GetParams());
}

bool ProtoRpcAdapter::decode_docsum_reply(FRT_RPCRequest& src, ProtoDocsumReply& dst) {
    return (src.CheckReturnTypes("bix") && decode_message(*src.GetReturn(), dst));
}

void ProtoRpcAdapter::encode_monitor_request(const ProtoMonitorRequest& src, FRT_RPCRequest& dst) {
    dst.SetMethodName("vespa.searchprotocol.ping");
    encode_message(src, *dst.GetParams());
}

bool ProtoRpcAdapter::decode_monitor_reply(FRT_RPCRequest& src, ProtoMonitorReply& dst) {
    return (src.CheckReturnTypes("bix") && decode_message(*src.GetReturn(), dst));
}
