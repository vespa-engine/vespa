...
PropertiesMap::PropertiesMap() : _propertiesMap() {
}

PropertiesMap::PropertiesMap(uint32_t sz) : _propertiesMap(sz) {
}

PropertiesMap::~PropertiesMap() = default;

fef::Properties& PropertiesMap::lookupCreate(std::string_view name) {
    return _propertiesMap[std::string(name)];
}

const fef::Properties& PropertiesMap::lookup(std::string_view name) const {
