...
vespalib::steady_time SteadyClock::now() const {
    return vespalib::steady_clock::now();
}

RelativeTime::RelativeTime(std::unique_ptr<Clock> clock) : _start(clock->now()), _clock(std::move(clock)) {
}

void Trace::constructObject() const {
    _trace = std::make_unique<vespalib::Slime>();
    _root = &_trace->setObject();
}

void Trace::constructTraces() const {
...
vespalib::slime::Inserter& LazyTraceInserter::get_thread_inserter() {
    if (!_thread_inserter) {
...
void LazyTraceInserter::handle_nested(const search::engine::Trace& nested_trace) {
...
void LazyTraceInserter::handle_thread(const search::engine::Trace& thread_trace) {
...
void Trace::start(int level, bool useUTC) {
    if (shouldTrace(level) && !hasTrace()) {
        if (useUTC) {
            ...
            root().setString("start_time", vespalib::to_string(vespalib::system_time(
                                               std::chrono::duration_cast<vespalib::system_time::duration>(
...
Trace::Cursor& Trace::createCursor(std::string_view name) {
...
Trace::Cursor* Trace::maybeCreateCursor(uint32_t level, std::string_view name) {
    return shouldTrace(level) ? &createCursor(name) : nullptr;
}

void Trace::addEvent(uint32_t level, std::string_view event) {
...
void Trace::addEvent(uint32_t level, std::string_view event, std::string_view tag) {
    if (!shouldTrace(level)) {
        return;
    }
...
void Trace::addTimeStamp(Cursor& trace) {
