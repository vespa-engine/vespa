...
public:
    ...
    DocsumReply(std::unique_ptr<vespalib::Slime> root, DocsumRequest::UP request, UniqueIssues::UP issues);

    DocsumReply(std::unique_ptr<vespalib::Slime> root, DocsumRequest::UP request);
    ...
    void setRequest(DocsumRequest::UP request) { _request = std::move(request); }

    void setIssues(UniqueIssues::UP issues) { _issues = std::move(issues); }
