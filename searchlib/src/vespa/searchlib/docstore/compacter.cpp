...
void Compacter::write(LockGuard guard, uint32_t chunkId, uint32_t lid, ConstBufferRef data) {
    (void)chunkId;
...
void BucketIndexStore::store(const StoreByBucket::Index& index) {
...
BucketIndexStore::LidIterator::LidIterator(const BucketIndexStore& store, size_t partitionId)
    : _store(store), _partitionId(partitionId), _current(_store._where.begin()) {
}

bool BucketIndexStore::LidIterator::has_next() noexcept {
    for (; (_current != _store._where.end()) && (_store.toPartitionId(_current->_bucketId) != _partitionId);
         _current++)
        ;
    return (_current != _store._where.end()) && (_store.toPartitionId(_current->_bucketId) == _partitionId);
}

StoreByBucket::Index BucketIndexStore::LidIterator::next() noexcept {
    return *_current++;
}
...
FileChunk::FileId BucketCompacter::getDestinationId(const LockGuard& guard) const {
    return (_destinationFileId.isActive()) ? _ds.getActiveFileId(guard) : _destinationFileId;
}

void BucketCompacter::write(LockGuard guard, uint32_t chunkId, uint32_t lid, ConstBufferRef data) {
    guard.unlock();
...
void BucketCompacter::write(BucketId bucketId, uint32_t chunkId, uint32_t lid, ConstBufferRef data) {
