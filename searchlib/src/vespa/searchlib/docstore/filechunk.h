...
public:
    ...
    vespalib::GenerationGuard getGuard() const {
        return _bucketizer ? _bucketizer->getGuard() : vespalib::GenerationGuard();
    }
...
public:
    ...
    size_t getDiskBloat() const {
        ...
        return (addedBytes == 0) ? getDiskFootprint()
    ...
    double getBucketSpread() const {
        return (((_numChunksWithBuckets == 0) || (_numUniqueBuckets == 0))
                    ? 1.0
                    : (double(_sumNumBuckets) / std::max(_numUniqueBuckets, _numChunksWithBuckets)));
    }
    ...
    void appendTo(vespalib::Executor& executor, const IGetLid& db, IWriteData& dest, uint32_t numChunks,
...
private:
    class TmpChunkMeta : public ChunkMeta, public std::vector<LidMeta> {
    public:
