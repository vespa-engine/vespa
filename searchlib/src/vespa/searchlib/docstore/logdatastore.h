...
public:
    ...
    template <typename T> using AtomicValueWrapper = vespalib::datastore::AtomicValueWrapper<T>;
    class Config {
    public:
        Config();

        Config& setMaxFileSize(size_t v) {
            _maxFileSize = v;
            return *this;
        }
        Config& setMaxNumLids(size_t v) {
            _maxNumLids = v;
            return *this;
        }
        Config& setMaxBucketSpread(double v) noexcept {
            _maxBucketSpread.store_relaxed(v);
            return *this;
        }
        Config& setMinFileSizeFactor(double v) {
            _minFileSizeFactor = v;
            return *this;
        }

        Config& compactCompression(CompressionConfig v) {
            _compactCompression = v;
            return *this;
        }
        Config& setFileConfig(WriteableFileChunk::Config v) {
            _fileConfig = v;
            return *this;
        }
...
public:
    ...
    LogDataStore(vespalib::Executor& executor, const std::string& dirName, const Config& config,
                 ...
                 const search::common::FileHeaderContext& fileHeaderContext, transactionlog::SyncProxy& tlSyncer,
                 IBucketizer::SP bucketizer, bool readOnly = false);
    ...
    Guard getLidReadGuard() const override { return _genHandler.takeGuard(); }
