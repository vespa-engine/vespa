...
std::optional<FileChunk::NameId> FileChunk::NameId::from_filename(const std::string& filename) {
...
std::string FileChunk::createIdxFileName(const std::string& name) {
    return name + ".idx";
}

std::string FileChunk::createDatFileName(const std::string& name) {
    return name + ".dat";
}

FileChunk::FileChunk(FileId fileId, NameId nameId, const std::string& baseName, const TuneFileSummary& tune,
...
void FileChunk::addNumBuckets(size_t numBucketsInChunk) {
...
void FileChunk::TmpChunkMeta::fill(vespalib::nbostream& is) {
    ...
    for (size_t count = 0; !is.empty() && is.good(); count++) {
        ...
        try {
            ...
            if ((count == 0) && (chunkMeta.getLastSerial() < serialNum)) {
                LOG(warning,
                    "last serial num(%" PRIu64 ") from previous file is bigger than my first(%" PRIu64
...
uint64_t FileChunk::handleChunk(const unique_lock& guard, ISetLid& ds, uint32_t docIdLimit,
...
void FileChunk::remove(uint32_t lid, uint32_t size) {
    (void)lid;
...
uint64_t FileChunk::getLastPersistedSerialNum() const {
    return _lastPersistedSerialNum.load(std::memory_order_relaxed);
}

vespalib::system_time FileChunk::getModificationTime() const {
        ...
        auto task = vespalib::makeLambdaTask([promise = std::move(promisedChunk), chunkId, this]() mutable {
            ...
            try {
                ...
                promise.set_exception(std::make_exception_ptr(std::runtime_error(
                    std::string("File '") + _dataFileName + "', offset " + std::to_string(cInfo.getOffset()) +
                    ", len " + std::to_string(cInfo.getSize()) + ", chunkId " + std::to_string(chunkId) + " : " +
                    e.what())));
            }
        });
...
void FileChunk::read(LidInfoWithLidV::const_iterator begin, size_t count, IBufferVisitor& visitor) const {
    if (count == 0) {
        return;
    }
...
void FileChunk::read(LidInfoWithLidV::const_iterator begin, size_t count, ChunkInfo ci,
                     IBufferVisitor& visitor) const {
    vespalib::DataBuffer whole(0ul, ALIGNMENT);
...
ssize_t FileChunk::read(uint32_t lid, SubChunkId chunkId, vespalib::DataBuffer& buffer) const {
    return (chunkId < _chunkInfo.size()) ? read(lid, chunkId, _chunkInfo[chunkId], buffer) : -1;
}

ssize_t FileChunk::read(uint32_t lid, SubChunkId chunkId, const ChunkInfo& chunkInfo,
...
uint64_t FileChunk::readDataHeader(FileRandRead& datFile) {
...
uint64_t FileChunk::readIdxHeader(FastOS_FileInterface& idxFile, uint32_t& docIdLimit) {
...
uint32_t FileChunk::readDocIdLimit(vespalib::GenericHeader& header) {
...
void FileChunk::writeDocIdLimit(vespalib::GenericHeader& header, uint32_t docIdLimit) {
    header.putTag(vespalib::GenericHeader::Tag(DOC_ID_LIMIT_KEY, docIdLimit));
}

void FileChunk::verify(bool reportOnly) const {
    (void)reportOnly;
    LOG(info, "Verifying file '%s' with fileid '%u'. erased-count='%zu' and erased-bytes='%zu'. diskFootprint='%zu'",
        _name.c_str(), _fileId.getId(), getErasedCount(), getErasedBytes(),
        _diskFootprint.load(std::memory_order_relaxed));
...
uint64_t FileChunk::get_size_on_disk() const {
    return _size_on_disk.load(std::memory_order_relaxed);
}

size_t FileChunk::getMemoryFootprint() const {
...
size_t FileChunk::getMemoryMetaFootprint() const {
    return sizeof(*this) + _chunkInfo.capacity() * sizeof(ChunkInfoVector::value_type);
}

vespalib::MemoryUsage FileChunk::getMemoryUsage() const {
...
bool FileChunk::isIdxFileEmpty(const std::string& name) {
...
void FileChunk::eraseIdxFile(const std::string& name) {
    std::string fileName(createIdxFileName(name));
    std::filesystem::remove(std::filesystem::path(fileName));
}

void FileChunk::eraseDatFile(const std::string& name) {
    std::string fileName(createDatFileName(name));
    std::filesystem::remove(std::filesystem::path(fileName));
}

DataStoreFileChunkStats FileChunk::getStats() const {
