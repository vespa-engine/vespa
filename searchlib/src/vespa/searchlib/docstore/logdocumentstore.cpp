...
bool LogDocumentStore::Config::operator==(const Config& rhs) const {
    (void)rhs;
    return DocumentStore::Config::operator==(rhs) && (_logConfig == rhs._logConfig);
}

LogDocumentStore::LogDocumentStore(vespalib::Executor& executor, const std::string& baseDir, const Config& config,
    ...
    : DocumentStore(config, _backingStore),
      _backingStore(executor, baseDir, config.getLogConfig(), growStrategy, tuneFileSummary, fileHeaderContext,
                    tlSyncer, std::move(bucketizer)) {
}

LogDocumentStore::~LogDocumentStore() = default;

void LogDocumentStore::reconfigure(const Config& config) {
