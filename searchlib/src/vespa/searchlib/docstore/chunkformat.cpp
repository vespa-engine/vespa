...
ChunkException::ChunkException(const std::string& msg, std::string_view location)
    : Exception(make_string("Illegal chunk: %s", msg.c_str()), location) {
}

void ChunkFormat::pack(uint64_t lastSerial, vespalib::DataBuffer& compressed, CompressionConfig compression) {
    ...
    CompressionConfig::Type type(
        compress(compression, vespalib::ConstBufferRef(os.data(), os.size()), compressed, false));
...
size_t ChunkFormat::getMaxPackSize(CompressionConfig compression) const {
    const size_t OVERHEAD(0);
    const size_t MINSIZE(1 + 1 + 4 + 4 +
                         (includeSerializedSize() ? 4 : 0)); // version + type + real length + crc + lastserial
...
void ChunkFormat::verifyCompression(uint8_t type) {
    if ((type != CompressionConfig::LZ4) && (type != CompressionConfig::ZSTD) && (type != CompressionConfig::NONE)) {
...
ChunkFormat::UP ChunkFormat::deserialize(const void* buffer, size_t len) {
    ...
    if (raw.size() < minimumRequiredSpace) {
        throw ChunkException(
            make_string("Available space (%ld) is less than required (%d)", raw.size(), minimumRequiredSpace),
            VESPA_STRLOC);
    }
    raw >> version;
...
ChunkFormat::ChunkFormat(size_t maxSize) : _dataBuf(maxSize) {
}

void ChunkFormat::verifyCrc(const vespalib::nbostream& is, uint32_t expectedCrc) const {
    uint32_t computedCrc32 = computeCrc(is.peek() - 1, is.size() + 1 - sizeof(uint32_t));
    if (expectedCrc != computedCrc32) {
        throw ChunkException(
            make_string("Crc32 mismatch. Expected (%0x), computed (%0x)", expectedCrc, computedCrc32), VESPA_STRLOC);
    }
}
...
void ChunkFormat::deserializeBody(vespalib::nbostream& is) {
    if (includeSerializedSize()) {
        ...
        if ((is.size() + alreadyRead) < serializedSize) {
            throw ChunkException(make_string("Not enough data(%d) available in stream(%ld)", required, is.size()),
                                 VESPA_STRLOC);
        }
    }
    uint8_t type(0);
