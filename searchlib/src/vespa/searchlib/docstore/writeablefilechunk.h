...
public:
    ...
    public:
        ...
        Config(CompressionConfig compression, size_t maxChunkBytes) noexcept
            : _compression(compression), _maxChunkBytes(maxChunkBytes) {}
...
public:
    ...
    WriteableFileChunk(vespalib::Executor& executor, FileId fileId, NameId nameId, const std::string& baseName,
                       uint64_t initialSerialNum, uint32_t docIdLimit, const Config& config,
...
private:
    ...
    vespalib::system_time unconditionallyFlushPendingChunks(const unique_lock& flushGuard, uint64_t serialNum,
                                                            uint64_t datFileLen);
    ...
    ChunkMeta computeChunkMeta(const unique_lock& guard, const vespalib::GenerationGuard& bucketizerGuard,
