...
public:
    ...
    bool operator==(const LidInfo& b) const noexcept {
        return (getFileId() == b.getFileId()) && (getChunkId() == b.getChunkId());
    }
    bool operator<(const LidInfo& b) const noexcept {
        return (getFileId() == b.getFileId()) ? (getChunkId() < b.getChunkId()) : (getFileId() < b.getFileId());
    }
...
private:
    static constexpr uint32_t computeAlignedSize(uint32_t sz) { return (sz + ((1 << SIZE_SHIFT) - 1)) >> SIZE_SHIFT; }
