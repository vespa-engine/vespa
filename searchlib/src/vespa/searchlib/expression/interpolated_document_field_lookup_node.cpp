...
InterpolateHandler::InterpolateHandler(std::vector<double>& values)
    : InterpolatedDocumentFieldLookupNode::Handler(), _values(values) {
}

InterpolateHandler::~InterpolateHandler() = default;

void InterpolateHandler::reset() {
}

void InterpolateHandler::onPrimitive(uint32_t, const Content& c) {
...
InterpolatedDocumentFieldLookupNode::InterpolatedDocumentFieldLookupNode() noexcept
    : DocumentFieldNode(), _lookup_expression(), _values(), _float_result() {
}

InterpolatedDocumentFieldLookupNode::InterpolatedDocumentFieldLookupNode(std::string_view                name,
                                                                         std::unique_ptr<ExpressionNode> arg)
    : DocumentFieldNode(name), _lookup_expression(std::move(arg)), _values(), _float_result() {
}

InterpolatedDocumentFieldLookupNode::InterpolatedDocumentFieldLookupNode(
    const InterpolatedDocumentFieldLookupNode& rhs) = default;
...
Serializer& InterpolatedDocumentFieldLookupNode::onSerialize(Serializer& os) const {
    os << _value;
...
Deserializer& InterpolatedDocumentFieldLookupNode::onDeserialize(Deserializer& is) {
    is >> _value;
    uint32_t count(0);
    is >> count;
    if (count > 0) {
...
void InterpolatedDocumentFieldLookupNode::visitMembers(vespalib::ObjectVisitor& visitor) const {
    DocumentFieldNode::visitMembers(visitor);
    visit(visitor, "index", *_lookup_expression);
}

void InterpolatedDocumentFieldLookupNode::selectMembers(const vespalib::ObjectPredicate& predicate,
...
void InterpolatedDocumentFieldLookupNode::onPrepare(bool) {
    _handler = std::make_unique<InterpolateHandler>(_values);
    _value = std::make_unique<FloatResultNode>();
}

void InterpolatedDocumentFieldLookupNode::onExecute() const {
    if (_lookup_expression) {
        _values.clear();
