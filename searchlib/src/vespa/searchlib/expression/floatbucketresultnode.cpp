...
size_t FloatBucketResultNode::hash() const {
    size_t tmpHash(0);
    memcpy(&tmpHash, &_from, sizeof(tmpHash));
    return tmpHash;
}

int FloatBucketResultNode::onCmp(const Identifiable& b) const {
...
void FloatBucketResultNode::visitMembers(vespalib::ObjectVisitor& visitor) const {
    visit(visitor, _fromField, _from);
    visit(visitor, _toField, _to);
}

vespalib::Serializer& FloatBucketResultNode::onSerialize(vespalib::Serializer& os) const {
    return os.put(_from).put(_to);
}

vespalib::Deserializer& FloatBucketResultNode::onDeserialize(vespalib::Deserializer& is) {
