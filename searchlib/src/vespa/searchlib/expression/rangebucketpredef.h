...
private:
    ...
    class Handler {
    public:
        Handler(const RangeBucketPreDefFunctionNode& rangeNode)
            : _predef(rangeNode.getBucketList()), _nullResult(rangeNode._nullResult) {}
    ...
    public:
        SingleValueHandler(const RangeBucketPreDefFunctionNode& rangeNode) : Handler(rangeNode) {}
    ...
    public:
        MultiValueHandler(const RangeBucketPreDefFunctionNode& rangeNode)
            : Handler(rangeNode), _result(static_cast<ResultNodeVector&>(rangeNode.updateResult())) {}
        const ResultNode* handle(const ResultNode& arg) override;
...
public:
    ...
    RangeBucketPreDefFunctionNode(ExpressionNode::UP arg)
        : UnaryFunctionNode(std::move(arg)), _predef(), _result(nullptr), _nullResult(nullptr) {}
