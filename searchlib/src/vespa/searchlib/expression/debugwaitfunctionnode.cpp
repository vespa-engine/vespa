...
DebugWaitFunctionNode::DebugWaitFunctionNode() : _waitTime(0.0), _busyWait(true) {
}
...
DebugWaitFunctionNode::DebugWaitFunctionNode(ExpressionNode::UP arg, double waitTime, bool busyWait)
    : UnaryFunctionNode(std::move(arg)), _waitTime(waitTime), _busyWait(busyWait) {
}

using std::chrono::microseconds;

void DebugWaitFunctionNode::onExecute() const {
...
Serializer& DebugWaitFunctionNode::onSerialize(Serializer& os) const {
    UnaryFunctionNode::onSerialize(os);
    return os << _waitTime << _busyWait;
}

Deserializer& DebugWaitFunctionNode::onDeserialize(Deserializer& is) {
    UnaryFunctionNode::onDeserialize(is);
    is >> _waitTime >> _busyWait;
    return is;
}

void DebugWaitFunctionNode::visitMembers(vespalib::ObjectVisitor& visitor) const {
