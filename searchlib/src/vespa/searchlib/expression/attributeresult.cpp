...
ResultNode::ConstBufferRef IntegerAttributeResult::onGetString(size_t, BufferRef buf) const {
    if (buf.size() > 1) {
        char*  s = buf.str();
        size_t sz = buf.size();
...
ResultNode::ConstBufferRef FloatAttributeResult::onGetString(size_t, BufferRef buf) const {
