...
size_t RawBucketResultNode::hash() const {
#if 0
    union {
        uint8_t  cxor[8];
        uint64_t ixor;
...
RawBucketResultNode::RawBucketResultNode() : _from(new RawResultNode()), _to(new RawResultNode()) {
}
...
int RawBucketResultNode::onCmp(const Identifiable& rhs) const {
...
void RawBucketResultNode::visitMembers(vespalib::ObjectVisitor& visitor) const {
    visit(visitor, _fromField, _from);
    visit(visitor, _toField, _to);
}

vespalib::Serializer& RawBucketResultNode::onSerialize(vespalib::Serializer& os) const {
    _from.serialize(os);
    _to.serialize(os);
    return os;
}

vespalib::Deserializer& RawBucketResultNode::onDeserialize(vespalib::Deserializer& is) {
