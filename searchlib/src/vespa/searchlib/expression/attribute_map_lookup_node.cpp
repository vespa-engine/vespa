...
template <typename KeyType> KeyType convertKey(const IAttributeVector&, const std::string& key) {
    KeyType               ret;
...
template <> std::string convertKey<std::string>(const IAttributeVector&, const std::string& key) {
    return key;
}

template <> EnumHandle convertKey<EnumHandle>(const IAttributeVector& attribute, const std::string& key) {
    EnumHandle ret;
...
template <typename T> struct UnwrappedType {
    using type = T;
};

template <> struct UnwrappedType<AtomicEntryRef> {
    using type = EnumHandle;
};

template <typename T> using UnwrappedType_t = typename UnwrappedType<T>::type;

template <typename KeyType, typename T> bool matching_direct_key(const KeyType& lhs, const T& rhs) {
    return lhs == rhs;
}

template <> bool matching_direct_key(const EnumHandle& lhs, const AtomicEntryRef& rhs) {
    return lhs == rhs.load_relaxed().ref();
}

template <typename T> UnwrappedType_t<T> unwrap_value(const T& value) {
    return value;
}

template <> EnumHandle unwrap_value(const AtomicEntryRef& value) {
    return value.load_relaxed().ref();
}

} // namespace

template <typename T, typename KeyType> class KeyHandlerT : public AttributeMapLookupNode::KeyHandler {
...
public:
    KeyHandlerT(const IAttributeVector& attribute, const std::string& key)
        : KeyHandler(attribute), _stash(), _read_view(nullptr), _key(convertKey<KeyType>(attribute, key)) {
...
template <typename T, typename KeyType> KeyHandlerT<T, KeyType>::~KeyHandlerT() = default;

template <typename T> using IntegerKeyHandler = KeyHandlerT<T, T>;
template <typename T> using FloatKeyHandler = KeyHandlerT<T, T>;
using StringKeyHandler = KeyHandlerT<const char*, std::string>;
using EnumKeyHandler = KeyHandlerT<AtomicEntryRef, EnumHandle>;

template <typename WT, typename T> bool matchingKey(WT lhs, T rhs) {
    return lhs == rhs;
}

template <> bool matchingKey<const char*, const char*>(const char* lhs, const char* rhs) {
    return (strcmp(lhs, rhs) == 0);
}

template <typename WT, typename T> class IndirectKeyHandlerT : public AttributeMapLookupNode::KeyHandler {
    using ReadView = ArrayReadViewType_t<T>;
...
public:
    IndirectKeyHandlerT(const IAttributeVector& attribute, const IAttributeVector& keySourceAttribute)
        : KeyHandler(attribute), _keySourceAttribute(keySourceAttribute), _stash(), _read_view(nullptr) {
        auto* mva = attribute.as_multi_value_attribute();
        if (mva != nullptr) {
            _read_view = mva->make_read_view(IMultiValueAttribute::ArrayTag<T>(), _stash);
        }
...
template <typename WT, typename T> IndirectKeyHandlerT<WT, T>::~IndirectKeyHandlerT() = default;

template <typename T> using IndirectIntegerKeyHandler = IndirectKeyHandlerT<IAttributeVector::largeint_t, T>;
template <typename T> using IndirectFloatKeyHandler = IndirectKeyHandlerT<double, T>;
...
protected:
    ...
    const IAttributeVector&                             _attribute;
    ValueHandler(std::unique_ptr<AttributeMapLookupNode::KeyHandler> keyHandler,
                 const IAttributeVector&                             attribute) noexcept
        : _keyHandler(std::move(keyHandler)), _attribute(attribute) {}
};

template <typename T, typename ResultNodeType> class ValueHandlerT : public ValueHandler {
    using ReadView = ArrayReadViewType_t<T>;
...
public:
    ValueHandlerT(std::unique_ptr<AttributeMapLookupNode::KeyHandler> keyHandler, const IAttributeVector& attribute,
                  ResultNodeType& result, UnwrappedType_t<T> undefinedValue) noexcept
...
template <typename T, typename ResultNodeType> using IntegerValueHandler = ValueHandlerT<T, ResultNodeType>;
template <typename T> using FloatValueHandler = ValueHandlerT<T, FloatResultNode>;
...
const IAttributeVector* findAttribute(const search::attribute::IAttributeContext& attrCtx, bool useEnumOptimization,
                                      const std::string& name) {
    const IAttributeVector* attribute =
        useEnumOptimization ? attrCtx.getAttributeStableEnum(name) : attrCtx.getAttribute(name);
...
prepareIntValues(std::unique_ptr<AttributeMapLookupNode::KeyHandler> keyHandler, const IAttributeVector& attribute,
                 IAttributeVector::largeint_t undefinedValue) {
    auto resultNode = std::make_unique<ResultNodeType>();
    auto handler = std::make_unique<IntegerValueHandler<T, ResultNodeType>>(std::move(keyHandler), attribute,
                                                                            *resultNode, undefinedValue);
...
AttributeMapLookupNode::AttributeMapLookupNode(std::string_view name, std::string_view keyAttributeName,
                                               std::string_view valueAttributeName, std::string_view key,
                                               std::string_view keySourceAttributeName)
    : AttributeNode(name),
...
AttributeMapLookupNode& AttributeMapLookupNode::operator=(const AttributeMapLookupNode& rhs) = default;

std::unique_ptr<AttributeMapLookupNode::KeyHandler> AttributeMapLookupNode::makeKeyHandlerHelper() const {
...
std::unique_ptr<AttributeMapLookupNode::KeyHandler> AttributeMapLookupNode::makeKeyHandler() const {
    try {
...
std::pair<std::unique_ptr<ResultNode>, std::unique_ptr<AttributeNode::Handler>>
AttributeMapLookupNode::createResultHandler(bool                               preserveAccurateTypes,
                                            const attribute::IAttributeVector& attribute) const {
    ...
    if (attribute.isIntegerType()) {
        ...
        if (preserveAccurateTypes) {
            ...
            default:
                throw std::runtime_error(std::format("'{}' is not a valid integer attribute"
                                                     " for attribute map lookup result",
                                                     attribute.getName()));
            }
        } else {
            switch (basicType) {
        ...
        switch (basicType) {
        case BasicType::FLOAT: {
            auto handler = std::make_unique<FloatValueHandler<float>>(std::move(keyHandler), attribute, *resultNode,
                                                                      getUndefined<double>());
            return {std::move(resultNode), std::move(handler)};
        }
        case BasicType::DOUBLE: {
            auto handler = std::make_unique<FloatValueHandler<double>>(std::move(keyHandler), attribute, *resultNode,
                                                                       getUndefined<double>());
            return {std::move(resultNode), std::move(handler)};
        }
        default:
...
void AttributeMapLookupNode::cleanup() {
    _keyAttribute = nullptr;
    _keySourceAttribute = nullptr;
    AttributeNode::cleanup();
}

void AttributeMapLookupNode::wireAttributes(const search::attribute::IAttributeContext& attrCtx) {
...
void AttributeMapLookupNode::visitMembers(vespalib::ObjectVisitor& visitor) const {
