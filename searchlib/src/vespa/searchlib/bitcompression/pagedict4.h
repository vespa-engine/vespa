...
public:
    ...
    PageDict4StartOffset() : _fileOffset(0u), _accNumDocs(0u) {}

    PageDict4StartOffset(uint64_t fileOffset, uint64_t accNumDocs)
        : _fileOffset(fileOffset), _accNumDocs(accNumDocs) {}
    ...
    void adjust(const index::PostingListCounts& counts) {
        _fileOffset += counts._bitLength;
        _accNumDocs += counts._numDocs;
    }
};

std::ostream& operator<<(std::ostream& stream, const index::PostingListCounts& counts);
...
public:
    ...
    [[nodiscard]] static constexpr uint32_t getL1Entries(uint32_t countsEntries) noexcept {
        return (countsEntries - 1) / getL1SkipStride();
    }
    [[nodiscard]] static constexpr uint32_t getL2Entries(uint32_t l1Entries) noexcept {
        return l1Entries / getL2SkipStride();
    }
    [[nodiscard]] static constexpr uint32_t getL4Entries(uint32_t l3Entries) noexcept {
        return (l3Entries - 1) / getL4SkipStride();
    }
    [[nodiscard]] static constexpr uint32_t getL5Entries(uint32_t l4Entries) noexcept {
...
public:
     ...
     */
    void addL6Skip(std::string_view word, const StartOffset& startOffset, uint64_t wordNum, uint64_t pageNum,
     ...
     */
    void addOverflowCounts(std::string_view word, const Counts& counts, const StartOffset& startOffset,
                           uint64_t wordNum);
...
private:
    ...
    void addOverflowCounts(std::string_view word, const Counts& counts);
...
public:
    ...
    public:
        ...
        L7Entry()
            : _l7Word(), _l7StartOffset(), _l7WordNum(0), _l6Offset(0), _sparsePageNum(0), _pageNum(0), _l7Ref(0) {}

        L7Entry(std::string_view l7Word, const StartOffset& l7StartOffset, uint64_t l7WordNum, uint64_t l6Offset,
                uint32_t sparsePageNum, uint64_t pageNum, uint32_t l7Ref)
            : _l7Word(l7Word),
        ...
        bool operator<(std::string_view word) const noexcept { return _l7Word < word; }
    };
    ...
    public:
        ...
        OverflowRef() : _wordNum(0), _l7Ref(0) {}

        OverflowRef(uint64_t wordNum, uint32_t l7Ref) : _wordNum(wordNum), _l7Ref(l7Ref) {}

        bool operator<(uint64_t wordNum) const noexcept { return _wordNum < wordNum; }
    };
    ...
    PageDict4SSReader(const ComprBuffer& cb, uint32_t ssFileHeaderSize, uint64_t ssFileBitLen,
...
public:
    ...
    void lookup(const SSReader& ssReader, const void* sparsePage, std::string_view key, std::string_view l6Word,
                std::string_view lastSPWord, const StartOffset& l6StartOffset, uint64_t l6WordNum,
...
public:
    ...
    bool lookup(const SSReader& ssReader, const void* page, std::string_view key, std::string_view l3Word,
                std::string_view lastPWord, const StartOffset& l3StartOffset, uint64_t l3WordNum);
};
...
public:
    ...
    struct L1SkipCheck {
        ...
        L1SkipCheck(const StartOffset& startOffset_) : wordOffset(0), startOffset(startOffset_), countOffset(0) {}
    };
    struct L2SkipCheck : public L1SkipCheck {
        uint32_t l1Offset;
        L2SkipCheck(const StartOffset& startOffset_) : L1SkipCheck(startOffset_), l1Offset(0) {}

        bool check(const L1SkipCheck& rhs) const {
            return startOffset == rhs.startOffset && countOffset == rhs.countOffset;
        }
    };
    struct L3SkipCheck {
        StartOffset startOffset;
        ...
        L3SkipCheck(const StartOffset& startOffset_, uint64_t wordNum_)
            : startOffset(startOffset_), wordNum(wordNum_) {}
    };
    struct L4SkipCheck : public L3SkipCheck {
        uint32_t wordOffset;
        uint32_t l3Offset;
        L4SkipCheck(const StartOffset& startOffset_, uint64_t wordNum_)
            : L3SkipCheck(startOffset_, wordNum_), wordOffset(0), l3Offset(0) {}
        bool check(const L3SkipCheck& rhs) const { return startOffset == rhs.startOffset && wordNum == rhs.wordNum; }
    };
    struct L5SkipCheck : public L4SkipCheck {
        uint32_t l4Offset;
        L5SkipCheck(const StartOffset& startOffset_, uint64_t wordNum_)
            : L4SkipCheck(startOffset_, wordNum_), l4Offset(0) {}
        bool check(const L4SkipCheck& rhs) const {
            return startOffset == rhs.startOffset && wordNum == rhs.wordNum && l3Offset == rhs.l3Offset;
        }
    };
    template <typename Element> class CheckVector {
    ...
    public:
        ...
        void step() { ++_cur; }
        void push_back(const Element& element) { _vector.push_back(element); }
    };
    template <typename Entry1, typename Entry2>
    void checkWordOffsets(const std::vector<char>& words, CheckVector<Entry1>& skip1, CheckVector<Entry2>& skip2);
    PCV                 _counts;
