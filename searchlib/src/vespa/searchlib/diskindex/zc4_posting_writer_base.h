...
public:
    struct DocIdAndFeatureSize {
        ...
        DocIdAndFeatureSize(uint32_t doc_id, uint32_t field_length, uint32_t num_occs,
                            uint32_t features_size) noexcept
            : _doc_id(doc_id), _field_length(field_length), _num_occs(num_occs), _features_size(features_size) {}
    };
...
public:
    ...
    void set_encode_interleaved_features(bool encode_interleaved_features) {
