...
bool DiskIndex::openDictionaries(const TuneFileSearch& tuneFileSearch) {
...
bool DiskIndex::setup(const TuneFileSearch& tuneFileSearch) {
    if (!loadSchema()) {
        return false;
    }
...
bool DiskIndex::setup(const TuneFileSearch& tuneFileSearch, const DiskIndex& old) {
...
DictionaryLookupResult DiskIndex::lookup(uint32_t index, std::string_view word) {
...
const std::vector<std::string> nonfield_file_names{"docsum.qcnt", "schema.txt", "schema.txt.orig", "selector.dat",
                                                   "serial.dat"};
...
public:
    CreateBlueprintVisitor(DiskIndex& diskIndex, const IRequestContext& requestContext, const FieldSpec& field,
                           uint32_t fieldId, fef::MatchDataLayout& global_layout)
    ...
    FieldSpec resolve_field_spec() const {
        ...
        if (filter_threshold.has_value()) {
            return {_field.getName(), _field.getFieldId(), _field.getHandle(),
                    FilterThreshold(filter_threshold.value())};
        }
        return _field;
    }
    ...
    template <class TermNode> void visitTerm(TermNode& n) {
        ...
        if (lookup_result.valid()) {
            setResult(
                std::make_unique<DiskTermBlueprint>(resolve_field_spec(), _field_index, termStr, lookup_result));
        } else {
...
Blueprint::UP createBlueprintHelper(DiskIndex& diskIndex, const IRequestContext& requestContext,
...
Blueprint::UP DiskIndex::createBlueprint(const IRequestContext& requestContext, const FieldSpec& field,
...
Blueprint::UP DiskIndex::createBlueprint(const IRequestContext& requestContext, const FieldSpecList& fields,
                                         const Node& term, fef::MatchDataLayout& global_layout) {
    if (fields.empty()) {
    ...
    for (size_t i(0); i < fields.size(); i++) {
        const FieldSpec& field = fields[i];
        orbp->addChild(createBlueprintHelper(*this, requestContext, field, _schema.getIndexFieldId(field.getName()),
                                             term, global_layout));
    }
    if (orbp->childCnt() == 1) {
...
FieldLengthInfo DiskIndex::get_field_length_info(const std::string& field_name) const {
...
IndexStats DiskIndex::get_stats(bool clear_disk_io_stats) const {
    IndexStats stats;
