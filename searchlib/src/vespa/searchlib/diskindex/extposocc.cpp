...
void setupDefaultPosOccParameters(PostingListParams* countParams, PostingListParams* params, uint64_t numWordIds,
                                  uint32_t docIdLimit) {
...
std::unique_ptr<PostingListFileSeqWrite> makePosOccWrite(PostingListCountFileSeqWrite* const posOccCountWrite,
                                                         bool dynamicK, const PostingListParams& params,
                                                         const PostingListParams& featureParams, const Schema& schema,
                                                         uint32_t                      indexId,
...
std::unique_ptr<PostingListFileSeqRead> makePosOccRead(const std::string&                 name,
    ...
    if (fileHeader.taste(name, tuneFileRead)) {
        if (fileHeader.getVersion() == 1 && fileHeader.getBigEndian() && fileHeader.getFormats().size() == 2 &&
            fileHeader.getFormats()[0] == Zc4PosOccSeqRead::getIdentifier(true) &&
            fileHeader.getFormats()[1] == ZcPosOccSeqRead::getSubIdentifier())
        {
            posOccRead = std::make_unique<ZcPosOccSeqRead>(posOccCountRead);
        } else if (fileHeader.getVersion() == 1 && fileHeader.getBigEndian() && fileHeader.getFormats().size() == 2 &&
                   fileHeader.getFormats()[0] == Zc4PosOccSeqRead::getIdentifier(false) &&
                   fileHeader.getFormats()[1] == Zc4PosOccSeqRead::getSubIdentifier())
        {
            posOccRead = std::make_unique<Zc4PosOccSeqRead>(posOccCountRead);
        } else {
            LOG(warning, "Could not detect format for posocc file read %s", name.c_str());
        }
    }
    if (posOccRead) {
