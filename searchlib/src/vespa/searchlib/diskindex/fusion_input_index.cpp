...
FusionInputIndex::FusionInputIndex(const std::string& path, uint32_t index, const SelectorArray& selector)
    : _path(path), _index(index), _selector(&selector), _schema(), _docIdMapping() {
}

FusionInputIndex::~FusionInputIndex() = default;

void FusionInputIndex::setup() {
    ...
    if (!_docIdMapping.readDocIdLimit(_path)) {
        throw IllegalArgumentException(
            make_string("Cannot determine docIdLimit for old index \"%s\"", _path.c_str()));
    }
