...
Zc4PostingReaderBase::NoSkipBase::NoSkipBase()
    : _zc_buf(), _zc_decoder(), _doc_id(0), _doc_id_pos(0), _features_pos(0) {
}

Zc4PostingReaderBase::NoSkipBase::~NoSkipBase() = default;

void Zc4PostingReaderBase::NoSkipBase::setup(DecodeContext& decode_context, uint32_t size, uint32_t doc_id) {
    _doc_id_pos = 0;
    _features_pos = 0;
    _zc_buf.resize(size);
    if (size != 0) {
...
void Zc4PostingReaderBase::L1Skip::setup(DecodeContext& decode_context, uint32_t size, uint32_t doc_id,
...
void Zc4PostingReaderBase::L1Skip::check(const Zc4PostingReaderBase& rb, const std::string& level_name,
    ...
    if (decode_features) {
        ...
        if (_features_pos != no_skip.get_features_pos()) {
            LOG(error,
                "Inconsistency in posting list file '%s', word '%s', docid %u, level %s, after read features_pos != "
                "no_skip.get_fatures_pos() (%" PRIu64 " != %" PRIu64 ")",
...
void Zc4PostingReaderBase::L1Skip::next_skip_entry() {
    _doc_id += (_zc_decoder.decode32() + 1);
}

Zc4PostingReaderBase::L2Skip::L2Skip() : L1Skip(), _l2_skip_pos(0) {
}

void Zc4PostingReaderBase::L2Skip::setup(DecodeContext& decode_context, uint32_t size, uint32_t doc_id,
                                         uint32_t last_doc_id) {
    L1Skip::setup(decode_context, size, doc_id, last_doc_id);
    _l2_skip_pos = 0;
}

void Zc4PostingReaderBase::L2Skip::check(const Zc4PostingReaderBase& rb, const std::string& level_name,
...
Zc4PostingReaderBase::L3Skip::L3Skip() : L2Skip(), _l3_skip_pos(0) {
}

void Zc4PostingReaderBase::L3Skip::setup(DecodeContext& decode_context, uint32_t size, uint32_t doc_id,
                                         uint32_t last_doc_id) {
    L2Skip::setup(decode_context, size, doc_id, last_doc_id);
    _l3_skip_pos = 0;
}

void Zc4PostingReaderBase::L3Skip::check(const Zc4PostingReaderBase& rb, const std::string& level_name,
...
Zc4PostingReaderBase::L4Skip::L4Skip() : L3Skip() {
}

void Zc4PostingReaderBase::L4Skip::setup(DecodeContext& decode_context, uint32_t size, uint32_t doc_id,
                                         uint32_t last_doc_id) {
    L3Skip::setup(decode_context, size, doc_id, last_doc_id);
}

void Zc4PostingReaderBase::L4Skip::check(const Zc4PostingReaderBase& rb, const std::string& level_name,
...
void Zc4PostingReaderBase::read_common_word_doc_id(DecodeContext64Base& decode_context) {
...
void Zc4PostingReaderBase::read_word_start(DecodeContext64Base& decode_context) {
