...
void Zc4PostingHeader::read(bitcompression::DecodeContext64Base& decode_context, const Zc4PostingParams& params) {
    ...
    bool has_more =
        (_num_docs >= params._min_chunk_docs || features_size_flush) ? (decode_context.readBits(1) != 0) : false;
    _doc_id_k = params._dynamic_k ? EC::calcDocIdK((_has_more || has_more) ? 1 : _num_docs, params._doc_id_limit)
    ...
    if (_num_docs < params._min_skip_docs && !_has_more && !features_size_flush) {
        ...
        _features_size =
            params._encode_features ? decode_context.decode_exp_golomb(K_VALUE_ZCPOSTING_FEATURESSIZE) : 0;
