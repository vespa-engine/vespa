...
void Zc4PosOccSeqRead::setFeatureParams(const PostingListParams& params) {
...
const std::string& Zc4PosOccSeqRead::getSubIdentifier() {
    PosOccFieldsParams           fieldsParams;
    EG2PosOccDecodeContext<true> d(&fieldsParams);
    return d.getIdentifier();
}

const FieldLengthInfo& Zc4PosOccSeqRead::get_field_length_info() const {
    return _fieldsParams.getFieldParams()->get_field_length_info();
}

Zc4PosOccSeqWrite::Zc4PosOccSeqWrite(const Schema& schema, uint32_t indexId, const FieldLengthInfo& field_length_info,
                                     PostingListCountFileSeqWrite* countFile)
    : Zc4PostingSeqWrite(countFile), _fieldsParams(), _realEncodeFeatures(&_fieldsParams) {
...
void ZcPosOccSeqRead::setFeatureParams(const PostingListParams& params) {
    bool oldCooked = &_reader.get_decode_features() == &_cookedDecodeContext;
    bool newCooked = oldCooked;
    params.get("cooked", newCooked);
    if (oldCooked != newCooked) {
...
const std::string& ZcPosOccSeqRead::getSubIdentifier() {
    PosOccFieldsParams          fieldsParams;
    EGPosOccDecodeContext<true> d(&fieldsParams);
    return d.getIdentifier();
}

const FieldLengthInfo& ZcPosOccSeqRead::get_field_length_info() const {
    return _fieldsParams.getFieldParams()->get_field_length_info();
}

ZcPosOccSeqWrite::ZcPosOccSeqWrite(const Schema& schema, uint32_t indexId, const FieldLengthInfo& field_length_info,
                                   PostingListCountFileSeqWrite* countFile)
    : ZcPostingSeqWrite(countFile), _fieldsParams(), _realEncodeFeatures(&_fieldsParams) {
    _writer.set_encode_features(&_realEncodeFeatures);
    _fieldsParams.setSchemaParams(schema, indexId);
    _fieldsParams.set_field_length_info(field_length_info);
}
