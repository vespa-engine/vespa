...
private:
    ...
    using DecodeContextReal = std::conditional_t<dynamic_k, bitcompression::EGPosOccDecodeContextCooked<bigEndian>,
                                                 bitcompression::EG2PosOccDecodeContextCooked<bigEndian>>;
    DecodeContextReal _decodeContextReal;

public:
    ZcRareWordPosOccIterator(Position start, uint64_t bitLength, uint32_t docIdLimit, bool decode_normal_features,
                             bool decode_interleaved_features, bool unpack_normal_features,
                             bool unpack_interleaved_features, const bitcompression::PosOccFieldsParams* fieldsParams,
                             fef::TermFieldMatchDataArray matchData);
    ~ZcRareWordPosOccIterator() override;
};

template <bool bigEndian, bool dynamic_k> class ZcPosOccIterator : public ZcPostingIterator<bigEndian> {
private:
    using ParentClass = ZcPostingIterator<bigEndian>;
    using ParentClass::_decodeContext;

    using DecodeContext = std::conditional_t<dynamic_k, bitcompression::EGPosOccDecodeContextCooked<bigEndian>,
                                             bitcompression::EG2PosOccDecodeContextCooked<bigEndian>>;
    DecodeContext _decodeContextReal;

public:
    ZcPosOccIterator(Position start, uint64_t bitLength, uint32_t docIdLimit, bool decode_normal_features,
                     ...
                     const bitcompression::PosOccFieldsParams* fieldsParams, fef::TermFieldMatchDataArray matchData);
    ~ZcPosOccIterator() override;
};

std::unique_ptr<search::queryeval::SearchIterator>
create_zc_posocc_iterator(bool bigEndian, const index::PostingListCounts& counts, bitcompression::Position start,
