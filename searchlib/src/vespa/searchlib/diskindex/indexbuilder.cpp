...
public:
    ...
    void open(std::string_view dir, const SchemaUtil::IndexIterator& index, uint32_t docIdLimit, uint64_t numWordIds,
              const FieldLengthInfo& field_length_info, const TuneFileSeqWrite& tuneFileWrite,
...
private:
    ...
    static constexpr uint32_t noDocId() { return std::numeric_limits<uint32_t>::max(); }

    static constexpr uint64_t noWordNumHigh() { return std::numeric_limits<uint64_t>::max(); }
};

FieldIndexBuilder::FieldIndexBuilder(const Schema& schema, uint32_t fieldId, IndexBuilder& builder,
                                     uint32_t docidLimit, uint64_t numWordIds,
                                     const IFieldLengthInspector& field_length_inspector,
                                     const TuneFileSeqWrite&      tuneFileWrite,
                                     const FileHeaderContext&     fileHeaderContext)
    : _field(schema, fieldId, builder, docidLimit, numWordIds, field_length_inspector, tuneFileWrite,
             fileHeaderContext),
      _curWord(),
      _curDocId(noDocId()),
      _inWord(false) {
...
FileHandle::FileHandle() : _fieldWriter() {
}
...
void FileHandle::open(std::string_view dir, const SchemaUtil::IndexIterator& index, uint32_t docIdLimit,
    ...
    if (!_fieldWriter->open(64, 262144u, 0, false, index.use_interleaved_features(), index.getSchema(),
                            index.getIndex(), field_length_info, tuneFileWrite, fileHeaderContext))
    {
        LOG(error, "Could not open term writer %s for write (%s)", std::string(dir).c_str(),
...
FieldHandle::FieldHandle(const Schema& schema, uint32_t fieldId, IndexBuilder& builder, uint32_t docIdLimit,
                         uint64_t numWordIds, const IFieldLengthInspector& field_length_inspector,
                         const TuneFileSeqWrite& tuneFileWrite, const FileHeaderContext& fileHeaderContext)
    : _schema(schema), _builder(builder), _file(), _fieldId(fieldId) {
...
void FieldHandle::new_word(std::string_view word) {
    _file.writer()->newWord(word);
}

void FieldHandle::add_document(const index::DocIdAndFeatures& features) {
    _file.writer()->add(features);
}

const Schema::IndexField& FieldHandle::getSchemaField() {
    return _schema.getIndexField(_fieldId);
}

const std::string& FieldHandle::getName() {
    return getSchemaField().getName();
}

std::string FieldHandle::getDir() {
...
std::vector<int32_t> extractFields(const Schema& schema) {
...
IndexBuilder::IndexBuilder(const Schema& schema, std::string_view prefix, uint32_t docIdLimit, uint64_t numWordIds,
...
IndexBuilder::~IndexBuilder() {
    if (!docsummary::DocumentSummary::writeDocIdLimit(_prefix, _docIdLimit)) {
        LOG(error, "Could not write docsum count in dir %s: %s", _prefix.c_str(), getLastErrorString().c_str());
        LOG_ABORT("should not be reached");
    }
}

std::unique_ptr<index::FieldIndexBuilder> IndexBuilder::startField(uint32_t fieldId) {
...
std::string IndexBuilder::appendToPrefix(std::string_view name) const {
    if (_prefix.empty()) {
