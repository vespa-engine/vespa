...
public:
    ...
    bool contains(SerialNum s) const { return (_from <= s) && (s <= _to); }

    bool contains(const SerialNumRange& b) const { return (_from <= b._from) && (b._to <= _to); }
...
public:
    ...
    public:
        ...
        Entry(SerialNum u, Type t, const vespalib::ConstBufferRef& d)
            : _unique(u), _type(t), _valid(true), _data(d) {}
