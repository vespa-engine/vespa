...
Encoding::Crc getCrc(searchlib::TranslogserverConfig::Crcmethod crcType) {
    switch (crcType) {
...
DomainConfig getDomainConfig(const searchlib::TranslogserverConfig& cfg) {
    DomainConfig dcfg;
...
void logReconfig(const searchlib::TranslogserverConfig& cfg, const DomainConfig& dcfg) {
    LOG(config,
        "configure Transaction Log Server %s at port %d\n"
        "DomainConfig {encoding={%d, %d}, compression_level=%d, part_limit=%ld, chunk_limit=%ld}",
        cfg.servername.c_str(), cfg.listenport, dcfg.getEncoding().getCrc(), dcfg.getEncoding().getCompression(),
        dcfg.getCompressionlevel(), dcfg.getPartSizeLimit(), dcfg.getChunkSizeLimit());
}

size_t derive_num_threads(uint32_t configured_cores, uint32_t actual_cores) {
    return (configured_cores > 0) ? configured_cores : std::max(1u, std::min(4u, actual_cores / 8));
}

} // namespace

void TransLogServerApp::start(FNET_Transport& transport, uint32_t num_cores) {
...
TransLogServer::SP TransLogServerApp::getTransLogServer() const {
    std::lock_guard<std::mutex> guard(_lock);
    return _tls;
}
