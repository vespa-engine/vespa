...
vespalib::Executor::Task::UP Session::createTask(Session::SP session) {
    return std::make_unique<VisitTask>(std::move(session));
}

Session::VisitTask::VisitTask(Session::SP session) : _session(std::move(session)) {
    _session->startVisit();
}
Session::VisitTask::~VisitTask() = default;

void Session::VisitTask::run() {
    _session->visitOnly();
}

bool Session::visit(FastOS_FileInterface& file, DomainPart& dp) {
...
void Session::visit() {
    ...
    for (DomainPart::SP dpSafe = _domain->findPart(_range.from());
         dpSafe.get() && (_range.from() < _range.to()) && (dpSafe.get()->range().from() <= _range.to());
         dpSafe = _domain->findPart(_range.from()))
    {
        // Must use findPart and iterate until no candidate parts found.
        DomainPart* dp(dpSafe.get());
        LOG(debug, "[%d] : Visiting the interval %" PRIu64 " - %" PRIu64 " in domain part [%" PRIu64 ", %" PRIu64 "]",
            _id, _range.from(), _range.to(), dp->range().from(), dp->range().to());
...
void Session::finalize() {
    if (!ok()) {
        LOG(error, "[%d] : Error in %s(%" PRIu64 " - %" PRIu64 "), stopping since I have no idea on what to do.", _id,
            "visitor", _range.from(), _range.to());
    }
    LOG(debug, "[%d] : Stopped %" PRIu64 " - %" PRIu64, _id, _range.from(), _range.to());
    _finished = true;
}

Session::Session(int sId, const SerialNumRange& r, const Domain::SP& d, std::unique_ptr<Destination> destination)
...
bool Session::send(const Packet& packet) {
    return _destination->send(_id, _domain->name(), packet);
}

bool Session::sendDone() {
