...
Encoding::Compression toCompression(CompressionConfig::Type type) {
    switch (type) {
...
void CCITTCRC32NoneChunk::onDecode(nbostream& is) {
...
void XXH64NoneChunk::onDecode(nbostream& is) {
...
void XXH64CompressedChunk::decompress(nbostream& is, uint32_t uncompressedLen) {
...
XXH64CompressedChunk::XXH64CompressedChunk(CompressionConfig::Type type, uint8_t level)
    : _type(type), _level(level), _backing() {
}

XXH64CompressedChunk::~XXH64CompressedChunk() = default;

Encoding XXH64CompressedChunk::compress(nbostream& os, Encoding::Crc crc) const {
    nbostream org;
...
Encoding XXH64CompressedChunk::onEncode(IChunk::nbostream& os) const {
    return compress(os, Encoding::Crc::xxh64);
}

void XXH64CompressedChunk::onDecode(IChunk::nbostream& is) {
