...
std::unique_ptr<CommitChunk> createCommitChunk(const DomainConfig& cfg) {
...
Domain::Domain(const string& domainName, const string& baseDir, vespalib::Executor& executor, const DomainConfig& cfg,
               const FileHeaderContext& fileHeaderContext)
    : _config(cfg),
      _currentChunk(createCommitChunk(cfg)),
      _lastSerial(0),
      _singleCommitter(
          std::make_unique<vespalib::ThreadStackExecutor>(1, CpuUsage::wrap(tls_domain_commit, CpuCategory::WRITE))),
      _executor(executor),
      _sessionId(1),
      _name(domainName),
      _parts(),
...
void Domain::addPart(SerialNum partId, bool isLastPart) {
...
Domain::~Domain() {
    ...
    vespalib::Gate gate;
    _singleCommitter->execute(
        makeLambdaTask([callback = std::make_unique<vespalib::GateCallback>(gate)]() { (void)callback; }));
    gate.await();
}

DomainInfo Domain::getDomainInfo() const {
    ...
    for (const auto& entry : _parts) {
        ...
        info.parts.emplace_back(
            PartInfo(part.range(), part.size(), part.byteSize(), part.get_size_on_disk(), part.fileName()));
    }
    return info;
}
...
SerialNum Domain::end(const UniqueLock& guard) const {
    verifyLock(guard);
    SerialNum s(0);
    if (!_parts.empty()) {
...
size_t Domain::byteSize(const UniqueLock& guard) const {
    verifyLock(guard);
    size_t size = 0;
    for (const auto& entry : _parts) {
        const DomainPart& part = *entry.second;
...
uint64_t Domain::get_size_on_disk() const {
    return get_size_on_disk(UniqueLock(_partsMutex));
}

uint64_t Domain::get_size_on_disk(const UniqueLock& guard) const {
    verifyLock(guard);
...
SerialNum Domain::getSynced() const {
    SerialNum  s(0);
...
void Domain::triggerSyncNow(std::unique_ptr<vespalib::IDestructorCallback> after_sync) {
    {
        std::unique_lock guard(_currentChunkMutex);
...
DomainPart::SP Domain::getActivePart() {
    std::lock_guard guard(_partsMutex);
...
uint64_t Domain::size(const UniqueLock& guard) const {
    verifyLock(guard);
    uint64_t sz(0);
...
DomainPart::SP Domain::optionallyRotateFile(SerialNum serialNum) {
...
void Domain::append(const Packet& packet, Writer::DoneCallback onDone) {
    std::unique_lock guard(_currentChunkMutex);
...
Domain::CommitResult Domain::startCommit(DoneCallback onDone) {
    std::unique_lock guard(_currentChunkMutex);
...
void Domain::commitIfFull(const UniqueLock& guard) {
    if (_currentChunk->sizeBytes() > _config.getChunkSizeLimit()) {
        commitAndTransferResponses(guard);
    }
}

void Domain::commitAndTransferResponses(const UniqueLock& guard) {
...
void Domain::commitChunk(std::unique_ptr<CommitChunk> chunk, const UniqueLock& chunkOrderGuard) {
    ...
    _executor.execute(
        makeLambdaTask([promise = std::move(promise), chunk = std::move(chunk), encoding = _config.getEncoding(),
                        compressionLevel = _config.getCompressionlevel()]() mutable {
            promise.set_value(SerializedChunk(std::move(chunk), encoding, compressionLevel));
        }));
    _singleCommitter->execute(
        makeLambdaTask([this, future = std::move(future)]() mutable { doCommit(future.get()); }));
}

void Domain::doCommit(const SerializedChunk& serialized) {
    ...
    cleanSessions();
    LOG(debug, "Releasing %zu acks and %zu entries and %zu bytes.", serialized.getNumCallBacks(),
        serialized.getNumEntries(), serialized.getData().size());
}
...
Domain::SerialNumList Domain::scanDir() {
    SerialNumList                       res;
