...
void IChunk::add(const Packet::Entry& entry) {
    _entries.emplace_back(entry);
}

SerialNumRange IChunk::range() const {
    return _entries.empty() ? SerialNumRange() : SerialNumRange(_entries.front().serial(), _entries.back().serial());
}

void IChunk::deserializeEntries(nbostream& is) {
...
void IChunk::serializeEntries(nbostream& os) const {
    for (const auto& e : _entries) {
        e.serialize(os);
    }
}

Encoding IChunk::encode(nbostream& os) const {
    return onEncode(os);
}

void IChunk::decode(nbostream& is) {
    onDecode(is);
}

IChunk::UP IChunk::create(uint8_t chunkType) {
    return create(Encoding(chunkType), 9);
}
IChunk::UP IChunk::create(Encoding encoding, uint8_t compressionLevel) {
    ...
    case Encoding::Crc::xxh64:
        ...
        default:
            throw IllegalArgumentException(
                fmt("Unhandled compression type '%d' for xxh64, compression=", encoding.getCompression()));
        }
    case Encoding::Crc::ccitt_crc32:
        ...
        default:
            throw IllegalArgumentException(
                fmt("Unhandled compression type '%d' for ccitt_crc32, compression=", encoding.getCompression()));
        }
    default:
...
void encode(vespalib::nbostream& os, const IChunk& chunk, Encoding encoding) {
    size_t begin = os.wp();
    ...
    LOG(spam,
        "Encoded chunk with %zu entries and %zu bytes, range[%" PRIu64 ", %" PRIu64 "] encoding(wanted=%x, real=%x)",
        chunk.getEntries().size(), os.size(), range.from(), range.to(), encoding.getRaw(), realEncoding.getRaw());
}

SerializedChunk::SerializedChunk(std::unique_ptr<CommitChunk> commitChunk, Encoding encoding,
