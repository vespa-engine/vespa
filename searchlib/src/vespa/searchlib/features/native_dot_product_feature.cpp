...
NativeDotProductExecutor::NativeDotProductExecutor(const search::fef::IQueryEnvironment& env)
    : FeatureExecutor(), _pairs(), _md(nullptr) {
...
void NativeDotProductExecutor::execute(uint32_t docId) {
...
void NativeDotProductExecutor::handle_bind_match_data(const fef::MatchData& md) {
    _md = &md;
}
...
bool NativeDotProductBlueprint::setup(const IIndexEnvironment&, const ParameterList& params) {
    if (params.size() > 0) {
...
FeatureExecutor& NativeDotProductBlueprint::createExecutor(const IQueryEnvironment& queryEnv,
