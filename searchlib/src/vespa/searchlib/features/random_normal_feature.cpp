...
RandomNormalExecutor::RandomNormalExecutor(uint64_t seed, double mean, double stddev)
    : fef::FeatureExecutor(), _rnd(mean, stddev, true) {
    LOG(debug, "RandomNormalExecutor: seed=%" PRIu64 ", mean=%f, stddev=%f", seed, mean, stddev);
    _rnd.seed(seed);
}

void RandomNormalExecutor::execute(uint32_t) {
    outputs().set_number(0, _rnd.next());
}

RandomNormalBlueprint::RandomNormalBlueprint() : fef::Blueprint("randomNormal"), _seed(0), _mean(0.0), _stddev(1.0) {
}

void RandomNormalBlueprint::visitDumpFeatures(const fef::IIndexEnvironment&, fef::IDumpFeatureVisitor&) const {
}

fef::Blueprint::UP RandomNormalBlueprint::createInstance() const {
    return std::make_unique<RandomNormalBlueprint>();
}

bool RandomNormalBlueprint::setup(const fef::IIndexEnvironment& env, const fef::ParameterList& params) {
...
fef::FeatureExecutor& RandomNormalBlueprint::createExecutor(const fef::IQueryEnvironment&,
