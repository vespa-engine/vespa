 ...
 *
 * Example usage: elementwise(bm25(i),x,float) cause the elementwise ranking feature blueprint to create this
 * blueprint with parameters (i,x,float) and proxy calls to prepareSharedState() and createExecutor() to this
 * blueprint. The executor returned by createExecutor() will calculate bm25 feature per element in the field 'i',
 * creating a tensor with a single mapped dimension 'x' that contains an elementwise aggregated float bm25 score for
 * each term matching the field.
 */
