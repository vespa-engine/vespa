...
FieldTermMatchExecutor::FieldTermMatchExecutor(const search::fef::IQueryEnvironment& env, uint32_t fieldId,
                                               uint32_t termId)
    : search::fef::FeatureExecutor(), _fieldHandle(util::getTermFieldHandle(env, termId, fieldId)), _md(nullptr) {
}

void FieldTermMatchExecutor::execute(uint32_t docId) {
...
void FieldTermMatchExecutor::handle_bind_match_data(const fef::MatchData& md) {
    _md = &md;
}

FieldTermMatchBlueprint::FieldTermMatchBlueprint()
    : search::fef::Blueprint("fieldTermMatch"), _fieldId(0), _termId(0) {
    // empty
}

void FieldTermMatchBlueprint::visitDumpFeatures(const search::fef::IIndexEnvironment& env,
    ...
    for (uint32_t i = 0; i < env.getNumFields(); ++i) {
        ...
        if (field.type() == search::fef::FieldType::INDEX) {
            ...
            std::string        alt_name =
                search::fef::FeatureNameBuilder().baseName(getBaseName()).parameter(fieldName).buildName();
            ...
            for (int term = 0; term < numTerms; ++term) {
                ...
                fnb.baseName(baseName).parameter(fieldName).parameter(vespalib::make_string("%d", term));
...
bool FieldTermMatchBlueprint::setup(const search::fef::IIndexEnvironment&, const search::fef::ParameterList& params) {
...
search::fef::Blueprint::UP FieldTermMatchBlueprint::createInstance() const {
    return std::make_unique<FieldTermMatchBlueprint>();
}

search::fef::FeatureExecutor& FieldTermMatchBlueprint::createExecutor(const search::fef::IQueryEnvironment& env,
