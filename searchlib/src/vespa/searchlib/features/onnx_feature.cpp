...
public:
    OnnxFeatureExecutor(const Onnx& model, const Onnx::WireInfo& wire_info) : _eval_context(model, wire_info) {}
...
bool OnnxBlueprint::setup(const IIndexEnvironment& env, const ParameterList& params) {
        ...
        if (auto maybe_input = defineInput(input_feature.value(), AcceptInput::OBJECT)) {
            ...
            if (!planner.bind_input_type(feature_input.type(), model_input)) {
                return fail("incompatible type for input (%s -> %s): %s -> %s", input_feature.value().c_str(),
                            ...
                            model_input.type_as_string().c_str());
            }
        } else {
