 ...
 * tensor with a single mapped dimension, with element id as label value and aggregated bm25 score as tensor cell
 * value.
 */
class ElementwiseBm25Executor : public fef::FeatureExecutor {
    ...
    // The 'b' param adjusts the effects of the element length of the document matched compared to the average element
    // length.
    double                               _k1_mul_b;
...
public:
    ElementwiseBm25Executor(const fef::FieldInfo& field, const fef::IQueryEnvironment& env, double avg_element_length,
                            double k1_param, double b_param, const vespalib::eval::Value& empty_output);
