...
void ElementwiseBlueprint::visitDumpFeatures(const fef::IIndexEnvironment&, fef::IDumpFeatureVisitor&) const {
}

std::unique_ptr<fef::Blueprint> ElementwiseBlueprint::createInstance() const {
    return std::make_unique<ElementwiseBlueprint>(_nested_blueprints);
}

fef::ParameterDescriptions ElementwiseBlueprint::getDescriptions() const {
    return fef::ParameterDescriptions().desc().feature().string().desc().feature().string().string();
}
...
void ElementwiseBlueprint::prepareSharedState(const fef::IQueryEnvironment& env, fef::IObjectStore& store) const {
    _inner_blueprint->prepareSharedState(env, store);
}

fef::FeatureExecutor& ElementwiseBlueprint::createExecutor(const fef::IQueryEnvironment& env,
