...
template <class P> class NativeRankParamsBase {
public:
    using Param = P;
    std::vector<P> vector;
    NativeRankParamsBase() : vector() {}
    void resize(size_t numFields) { vector.resize(numFields); }
...
public:
     ...
     **/
    FieldWrapper(const fef::IIndexEnvironment& env, const fef::ParameterList& fields, const fef::FieldType filter);
