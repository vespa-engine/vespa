...
AttributeMatchExecutor<T>::Computer::Computer(const IQueryEnvironment& env, AttributeMatchParams params)
    ...
    LOG(debug,
        "attributeMatch(%s): weightedSet(%s), numAttributeTerms(%u), totalAttrTermWeight(%u), numTerms(%u), "
        "totalTermWeight(%u), totalTermSignificance(%f)",
        _params.attrInfo->name().c_str(), _params.weightedSet ? "true" : "false", _numAttrTerms, _totalAttrTermWeight,
        getNumTerms(), _totalTermWeight, _totalTermSignificance);
}

template <typename T> void AttributeMatchExecutor<T>::Computer::reset() {
    _matches = 0;
    _matchedTermWeight = 0, _matchedTermSignificance = 0, _totalWeight = 0;
    _maxWeight = 0;
    _normalizedWeightedWeight = 0;
    _weightSum = 0;
    _valueCount = 0;
...
template <typename T> void AttributeMatchExecutor<T>::Computer::run(uint32_t docId) {
    ...
    LOG(debug,
        "attributeMatch(%s)::Computer::run(): matches(%u), totalWeight(%d), maxWeight(%d), "
        "normalizedWeightedWeight(%f), "
        "weightSum(%d), valueCount(%u), matchedTermWeight(%u), matchedTermSignificance(%f)",
        _params.attrInfo->name().c_str(), _matches, _totalWeight, _maxWeight, _normalizedWeightedWeight, _weightSum,
        _valueCount, _matchedTermWeight, _matchedTermSignificance);
}

template <typename T> feature_t AttributeMatchExecutor<T>::Computer::getAverageWeight() const {
    if (_matches != 0) {
...
template <typename T> feature_t AttributeMatchExecutor<T>::Computer::getQueryCompleteness() const {
    if (getNumTerms() != 0) {
...
template <typename T> feature_t AttributeMatchExecutor<T>::Computer::getNormalizedWeight() const {
    if (_params.weightedSet) {
        feature_t normalizedWeight =
            _totalWeight > 0 ? _totalWeight / ((feature_t)_params.maxWeight * _numAttrTerms) : 0.0f;
        return adjustToOne(normalizedWeight);
    }
    return 0;
}

template <typename T> feature_t AttributeMatchExecutor<T>::Computer::getNormalizedWeightedWeight() const {
    if (_params.weightedSet) {
        feature_t divider =
            _totalAttrTermWeight > 0 ? ((feature_t)_params.maxWeight * _totalAttrTermWeight) : _params.maxWeight;
...
template <typename T> feature_t AttributeMatchExecutor<T>::Computer::getFieldCompleteness() const {
    if (_params.weightedSet) {
        if (_totalWeight <= 0) {
            return 0;
...
template <typename T> feature_t AttributeMatchExecutor<T>::Computer::getCompleteness() const {
...
template <typename T> feature_t AttributeMatchExecutor<T>::Computer::getWeight() const {
...
template <typename T> feature_t AttributeMatchExecutor<T>::Computer::getSignificance() const {
...
AttributeMatchExecutor<T>::AttributeMatchExecutor(const IQueryEnvironment& env, AttributeMatchParams params)
    : FeatureExecutor(), _cmp(env, params) {
}

template <typename T> void AttributeMatchExecutor<T>::execute(uint32_t docId) {
...
template <typename T> void AttributeMatchExecutor<T>::handle_bind_match_data(const MatchData& md) {
    _cmp.bind_match_data(md);
}

AttributeMatchBlueprint::AttributeMatchBlueprint() : Blueprint("attributeMatch"), _params() {
    // empty
}

AttributeMatchBlueprint::~AttributeMatchBlueprint() = default;

void AttributeMatchBlueprint::visitDumpFeatures(const IIndexEnvironment& env, IDumpFeatureVisitor& visitor) const {
...
Blueprint::UP AttributeMatchBlueprint::createInstance() const {
    return std::make_unique<AttributeMatchBlueprint>();
}

fef::ParameterDescriptions AttributeMatchBlueprint::getDescriptions() const {
...
bool AttributeMatchBlueprint::setup(const IIndexEnvironment& env, const ParameterList& params) {
    ...
    describeOutput(
        "queryCompleteness",
        "The query completeness for this attribute: matches/the number of query terms searching this attribute");
    describeOutput("fieldCompleteness", "The normalized ratio of query tokens which was matched in the field");
    describeOutput("normalizedWeight",
                   "A number which is close to 1 if the attribute weights of most matches in a weighted set are high "
                   "(relative to the maxWeight configuration value), 0 otherwise");
    describeOutput(
        "normalizedWeightedWeight",
        "A number which is close to 1 if the attribute weights of most matches in a weighted set are high (relative "
        "to the maxWeight configuration value), and where highly weighted query terms has more impact, 0 otherwise");
    ...
    describeOutput("significance",
                   "Returns the normalized term significance of the terms of this match relative to the whole query");
...
FeatureExecutor& AttributeMatchBlueprint::createExecutor(const IQueryEnvironment& env, vespalib::Stash& stash) const {
    const IAttributeVector* attribute = env.getAttributeContext().getAttribute(_params.attrInfo->name());
    if (attribute == nullptr) {
        Issue::report("attribute_match feature: The attribute vector '%s' was not found.",
                      _params.attrInfo->name().c_str());
...
void AttributeMatchBlueprint::prepareSharedState(const IQueryEnvironment& queryEnv, IObjectStore& objectStore) const {
