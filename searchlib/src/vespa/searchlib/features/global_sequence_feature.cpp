...
public:
    GlobalSequenceExecutor(uint32_t distributionKey) : _distributionKey(distributionKey) {}
...
GlobalSequenceBlueprint::GlobalSequenceBlueprint() : Blueprint("globalSequence"), _distributionKey(0) {
}

void GlobalSequenceBlueprint::visitDumpFeatures(const IIndexEnvironment&, IDumpFeatureVisitor&) const {
}
...
Blueprint::UP GlobalSequenceBlueprint::createInstance() const {
    return std::make_unique<GlobalSequenceBlueprint>();
}

FeatureExecutor& GlobalSequenceBlueprint::createExecutor(const IQueryEnvironment&, vespalib::Stash& stash) const {
