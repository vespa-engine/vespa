...
void RandomNormalStableExecutor::execute(uint32_t docId) {
    _rnd.seed(_seed + docId);
    outputs().set_number(0, _rnd.next());
}

RandomNormalStableBlueprint::RandomNormalStableBlueprint()
    : search::fef::Blueprint("randomNormalStable"), _seed(0), _mean(0.0), _stddev(1.0) {
}

void RandomNormalStableBlueprint::visitDumpFeatures(const search::fef::IIndexEnvironment&,
                                                    search::fef::IDumpFeatureVisitor&) const {
}

search::fef::Blueprint::UP RandomNormalStableBlueprint::createInstance() const {
    return std::make_unique<RandomNormalStableBlueprint>();
}

bool RandomNormalStableBlueprint::setup(const search::fef::IIndexEnvironment& env,
    ...
    describeOutput(
        "out",
        "A random value drawn from the Gaussian distribution that is stable for a given match (document and query)");

    return true;
}

search::fef::FeatureExecutor& RandomNormalStableBlueprint::createExecutor(const search::fef::IQueryEnvironment& env,
                                                                          vespalib::Stash& stash) const {
    uint64_t seed = _seed;
    if (seed == 0) {
        seed = util::strToNum<uint64_t>(env.getProperties().lookup(getName(), "seed").get("1024")); // default seed
    }
