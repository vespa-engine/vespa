...
void MatchCountExecutor::execute(uint32_t docId) {
    size_t output = 0;
...
MatchCountBlueprint::MatchCountBlueprint() : Blueprint("matchCount"), _field(nullptr) {
}

void MatchCountBlueprint::visitDumpFeatures(const IIndexEnvironment&, IDumpFeatureVisitor&) const {
}

bool MatchCountBlueprint::setup(const IIndexEnvironment&, const ParameterList& params) {
    _field = params[0].asField();
    describeOutput("out", "Returns number of matches in the field of all terms in the query");
    return true;
}

Blueprint::UP MatchCountBlueprint::createInstance() const {
    return std::make_unique<MatchCountBlueprint>();
}

FeatureExecutor& MatchCountBlueprint::createExecutor(const IQueryEnvironment& queryEnv,
