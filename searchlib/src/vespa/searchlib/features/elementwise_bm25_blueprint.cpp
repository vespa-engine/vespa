...
std::string make_avg_element_length_key(const std::string& base_name, const std::string& field_name) {
    return base_name + ".ael." + field_name;
}

double get_average_element_length(const IQueryEnvironment& env, const std::string& field_name) {
...
struct LookupHelper {
    ...
    std::string error_message() const { return basic_bm25_utils.last_error() + bm25_utils.last_error(); }
};
...
void ElementwiseBm25Blueprint::visitDumpFeatures(const fef::IIndexEnvironment&, fef::IDumpFeatureVisitor&) const {
}

std::unique_ptr<fef::Blueprint> ElementwiseBm25Blueprint::createInstance() const {
    return std::make_unique<ElementwiseBm25Blueprint>();
}

fef::ParameterDescriptions ElementwiseBm25Blueprint::getDescriptions() const {
    return fef::ParameterDescriptions().desc().indexField(fef::ParameterCollection::ANY).string().string();
}

bool ElementwiseBm25Blueprint::setup(const fef::IIndexEnvironment& env, const fef::ParameterList& params) {
    ...
    describeOutput("score", "The elementwise bm25 score for all terms searching in the given index field",
                   output_type);
    return true;
}

void ElementwiseBm25Blueprint::prepareSharedState(const fef::IQueryEnvironment& env, fef::IObjectStore& store) const {
...
fef::FeatureExecutor& ElementwiseBm25Blueprint::createExecutor(const fef::IQueryEnvironment& env,
                                                               vespalib::Stash&              stash) const {
    const auto* lookup_result =
        env.getObjectStore().get(make_avg_element_length_key(bm25_feature_base_name, _field->name()));
    ...
    return stash.create<ElementwiseBm25Executor>(*_field, env, avg_element_length, _k1_param, _b_param,
                                                 *_empty_output);
}
