...
protected:
    struct CachedTermData {
        ...
        CachedTermData(const NativeAttributeMatchParams& params, const fef::ITermFieldData& tfd, feature_t s)
            : scale(s), weightBoostTable(&params.vector[tfd.getFieldId()].weightBoostTable), tfh(tfd.getHandle()) {}
        feature_t                  scale;
...
private:
    static Precomputed preComputeSetup(const fef::IQueryEnvironment& env, const NativeAttributeMatchParams& params);

public:
    static fef::FeatureExecutor& createExecutor(const fef::IQueryEnvironment&     env,
                                                const NativeAttributeMatchParams& params, vespalib::Stash& stash);
};
...
public:
    NativeAttributeMatchExecutorMulti(const Precomputed& setup)
        : _divisor(setup.second), _queryTermData(setup.first), _md(nullptr) {}
...
public:
    NativeAttributeMatchExecutorSingle(const Precomputed& setup) : _queryTermData(setup.first[0]), _md(nullptr) {
