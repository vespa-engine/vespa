...
RandomExecutor::RandomExecutor(uint64_t seed, uint64_t matchSeed)
    : fef::FeatureExecutor(), _rnd(), _matchRnd(), _matchSeed(matchSeed) {
    LOG(debug, "RandomExecutor: seed=%" PRIu64 ", matchSeed=%" PRIu64, seed, matchSeed);
    _rnd.srand48(seed);
}

void RandomExecutor::execute(uint32_t docId) {
...
RandomBlueprint::RandomBlueprint() : fef::Blueprint("random"), _seed(0) {
}

void RandomBlueprint::visitDumpFeatures(const fef::IIndexEnvironment&, fef::IDumpFeatureVisitor&) const {
}

fef::Blueprint::UP RandomBlueprint::createInstance() const {
    return std::make_unique<RandomBlueprint>();
}

bool RandomBlueprint::setup(const fef::IIndexEnvironment& env, const fef::ParameterList&) {
    ...
    describeOutput("match",
                   "A random value in the interval [0, 1> that is stable for a given match (document and query)");
    return true;
}

using namespace std::chrono;

fef::FeatureExecutor& RandomBlueprint::createExecutor(const fef::IQueryEnvironment& env,
    ...
    uint64_t matchSeed =
        util::strToNum<uint64_t>(env.getProperties().lookup(getName(), "match", "seed").get("1024")); // default seed
