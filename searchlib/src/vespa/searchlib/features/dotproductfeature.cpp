...
DotProductExecutorBase<BaseType>::DotProductExecutorBase(const V& queryVector)
    : FeatureExecutor(), _queryVector(queryVector), _end(_queryVector.getDimMap().end()) {
}

template <typename BaseType> DotProductExecutorBase<BaseType>::~DotProductExecutorBase() = default;

template <typename BaseType> void DotProductExecutorBase<BaseType>::execute(uint32_t docId) {
    feature_t val = 0;
...
template <typename BaseType>
DotProductByWeightedSetReadViewExecutor<BaseType>::DotProductByWeightedSetReadViewExecutor(
    const WeightedSetReadView* weighted_set_read_view, const V& queryVector)
    : DotProductExecutorBase<BaseType>(queryVector), _weighted_set_read_view(weighted_set_read_view), _backing() {
}

template <typename BaseType>
DotProductByWeightedSetReadViewExecutor<BaseType>::DotProductByWeightedSetReadViewExecutor(
    const WeightedSetReadView* weighted_set_read_view, std::unique_ptr<V> queryVector)
...
public:
    ...
    DotProductExecutorByEnum(const IWeightedSetEnumReadView* weighted_set_enum_read_view,
                             std::unique_ptr<V>              queryVector);
    ~DotProductExecutorByEnum() override;
    void execute(uint32_t docId) override;
};

DotProductExecutorByEnum::DotProductExecutorByEnum(const IWeightedSetEnumReadView* weighted_set_enum_read_view,
                                                   const V&                        queryVector)
    : FeatureExecutor(),
...
public:
    SingleDotProductExecutorByEnum(const IWeightedSetEnumReadView* weighted_set_enum_read_view, EnumHandle key,
                                   feature_t value)
        : _weighted_set_enum_read_view(weighted_set_enum_read_view), _key(key), _value(value) {}
...
public:
    ...
    SingleDotProductByWeightedValueExecutor(const WeightedSetReadView* weighted_set_read_view, BaseType key,
                                            feature_t value)
        : _weighted_set_read_view(weighted_set_read_view), _key(key), _value(value) {}
...
template <typename BaseType>
DotProductExecutorBase<BaseType>::DotProductExecutorBase(const V& queryVector)
    : FeatureExecutor(), _queryVector(queryVector) {
}
...
template <typename BaseType> void DotProductExecutorBase<BaseType>::execute(uint32_t docId) {
    ...
    outputs().set_number(0, vespalib::hwaccelerated::dot_product(&_queryVector[0], values.data(), commonRange));
}

template <typename BaseType>
DotProductByArrayReadViewExecutor<BaseType>::DotProductByArrayReadViewExecutor(const ArrayReadView* array_read_view,
                                                                               const V&             queryVector)
    : DotProductExecutorBase<BaseType>(queryVector), _array_read_view(array_read_view) {
}
...
template <typename BaseType>
std::span<const BaseType> DotProductByArrayReadViewExecutor<BaseType>::getAttributeValues(uint32_t docId) {
    return _array_read_view->get_values(docId);
}

template <typename A>
DotProductExecutor<A>::DotProductExecutor(const A* attribute, const V& queryVector)
    : DotProductExecutorBase<typename A::BaseType>(queryVector), _attribute(attribute) {
}

template <typename A> DotProductExecutor<A>::~DotProductExecutor() = default;

template <typename BaseType>
SparseDotProductExecutorBase<BaseType>::SparseDotProductExecutorBase(const V& queryVector, const IV& queryIndexes)
    : DotProductExecutorBase<BaseType>(queryVector), _queryIndexes(queryIndexes), _scratch(queryIndexes.size()) {
}

template <typename BaseType> SparseDotProductExecutorBase<BaseType>::~SparseDotProductExecutorBase() = default;

template <typename BaseType>
SparseDotProductByArrayReadViewExecutor<BaseType>::SparseDotProductByArrayReadViewExecutor(
    const ArrayReadView* array_read_view, const V& queryVector, const IV& queryIndexes)
    : SparseDotProductExecutorBase<BaseType>(queryVector, queryIndexes), _array_read_view(array_read_view) {
}
...
template <typename BaseType>
std::span<const BaseType> SparseDotProductByArrayReadViewExecutor<BaseType>::getAttributeValues(uint32_t docid) {
...
void parseVectors(const Property& prop, std::vector<T>& values, std::vector<uint32_t>& indexes) {
...
template <>
void parseVectors<int8_t, int8_t>(const Property& prop, std::vector<int8_t>& values, std::vector<uint32_t>& indexes) {
    parseVectors<int8_t, int16_t>(prop, values, indexes);
}

template <typename TCT> struct CopyCellsToVector {
    template <typename ICT> static void invoke(TypedCells source, std::vector<TCT>& target) {
...
template <typename T> ArrayParam<T>::ArrayParam(const Property& prop) {
    parseVectors(prop, values, indexes);
}

template <typename T> ArrayParam<T>::ArrayParam(vespalib::nbostream& stream) {
    ...
    try {
        ...
        if (tensor->type().is_dense()) {
            ...
            Issue::report("dot_product feature: Expected dense tensor, but got type '%s'",
                          tensor->type().to_spec().c_str());
        }
...
template <typename T> ArrayParam<T>::~ArrayParam() = default;
...
const attribute::IMultiValueReadView<AT>* make_multi_value_read_view(const IAttributeVector& attribute,
...
template <typename T>
FeatureExecutor& createForDirectArrayImpl(const IAttributeVector* attribute, const std::vector<T>& values,
                                          const std::vector<uint32_t>& indexes, vespalib::Stash& stash) {
    if (values.empty()) {
    ...
    if (array_read_view != nullptr) {
        if (indexes.empty()) {
            ...
            return stash.create<dotproduct::array::SparseDotProductByArrayReadViewExecutor<T>>(array_read_view,
                                                                                               values, indexes);
        }
    }
    return stash.create<SingleZeroValueExecutor>();
...
template <typename T>
FeatureExecutor& createForDirectArray(const IAttributeVector* attribute, const Property& prop,
                                      vespalib::Stash& stash) {
    std::vector<T>        values;
...
template <typename T>
FeatureExecutor& createForDirectArray(const IAttributeVector* attribute, const ArrayParam<T>& arguments,
                                      vespalib::Stash& stash) {
    return createForDirectArrayImpl<T>(attribute, arguments.values, arguments.indexes, stash);
}

template <typename T> size_t extractSize(const dotproduct::wset::IntegerVectorT<T>& v) {
    return v.getVector().size();
}

template <typename T> std::pair<T, feature_t> extractElem(const dotproduct::wset::IntegerVectorT<T>& v, size_t idx) {
    const auto& pair = v.getVector()[idx];
    return std::pair<T, feature_t>(pair.first, pair.second);
}

template <typename T> size_t extractSize(const std::unique_ptr<dotproduct::wset::IntegerVectorT<T>>& v) {
...
FeatureExecutor& createForDirectWSetImpl(const IAttributeVector* attribute, V&& vector, vespalib::Stash& stash) {
    ...
    if (weighted_set_read_view != nullptr) {
        if (extractSize(vector) == 1) {
            ...
            return stash.create<SingleDotProductByWeightedValueExecutor<T>>(weighted_set_read_view, elem.first,
                                                                            elem.second);
        }
        return stash.create<DotProductByWeightedSetReadViewExecutor<T>>(weighted_set_read_view,
...
template <typename T>
FeatureExecutor& createForDirectIntegerWSet(const IAttributeVector*                    attribute,
                                            const dotproduct::wset::IntegerVectorT<T>& vector,
                                            vespalib::Stash&                           stash) {
    using namespace dotproduct::wset;
    return vector.empty() ? stash.create<SingleZeroValueExecutor>()
                          : createForDirectWSetImpl<T>(attribute, vector, stash);
}

FeatureExecutor& createFromObject(const IAttributeVector* attribute, const fef::Anything& object,
                                  vespalib::Stash& stash) {
        ...
        if (attribute->hasEnum()) {
            ...
            if (weighted_set_enum_read_view != nullptr) {
                if (vector.getVector().size() == 1) {
                    ...
                    return stash.create<SingleDotProductExecutorByEnum>(weighted_set_enum_read_view, elem.first,
                                                                        elem.second);
                }
            ...
            if (attribute->isStringType()) {
                ...
                if (attribute->getBasicType() == BasicType::INT32) {
                    return createForDirectIntegerWSet<int32_t>(
                        attribute, dynamic_cast<const IntegerVectorT<int32_t>&>(object), stash);
                } else if (attribute->getBasicType() == BasicType::INT64) {
                    return createForDirectIntegerWSet<int64_t>(
                        attribute, dynamic_cast<const IntegerVectorT<int64_t>&>(object), stash);
                } else if (attribute->getBasicType() == BasicType::INT8) {
                    return createForDirectIntegerWSet<int8_t>(
                        attribute, dynamic_cast<const IntegerVectorT<int8_t>&>(object), stash);
                }
            }
        }
    }
...
FeatureExecutor* createTypedArrayExecutor(const IAttributeVector* attribute, const Property& prop,
                                          vespalib::Stash& stash) {
    switch (attribute->getBasicType()) {
    case BasicType::INT8:
...
template <typename T>
FeatureExecutor& createForDirectIntegerWSet(const IAttributeVector* attribute, const Property& prop,
                                            vespalib::Stash& stash) {
    using namespace dotproduct::wset;
...
FeatureExecutor& createTypedWsetExecutor(const IAttributeVector* attribute, const Property& prop,
                                         vespalib::Stash& stash) {
    using namespace dotproduct::wset;
    if (attribute->hasEnum()) {
...
FeatureExecutor& createFromString(const IAttributeVector* attribute, const Property& prop, vespalib::Stash& stash) {
    ...
    if (executor == nullptr) {
        Issue::report("dot_product feature: The attribute vector '%s' is not of type weighted set string/integer nor"
                      " array<int/long/float/double>, returning default value.",
                      attribute->getName().c_str());
...
const std::string& make_queryvector_key_for_attribute(const IAttributeVector& attribute, const std::string& key,
...
std::string make_attribute_key(const std::string& base, const std::string& subKey) {
    std::string key(base);
...
const IAttributeVector* DotProductBlueprint::upgradeIfNecessary(const IAttributeVector*  attribute,
                                                                const IQueryEnvironment& env) const {
    if ((attribute != nullptr) && (attribute->getCollectionType() == attribute::CollectionType::WSET) &&
        attribute->hasEnum() && (attribute->isStringType() || attribute->isIntegerType()))
    {
...
fef::Anything::UP createQueryVector(const IQueryEnvironment& env, const IAttributeVector* attribute,
...
const std::string& DotProductBlueprint::getAttribute(const IQueryEnvironment& env) const {
...
void DotProductBlueprint::visitDumpFeatures(const IIndexEnvironment&, IDumpFeatureVisitor&) const {
}

bool DotProductBlueprint::setup(const IIndexEnvironment&, const ParameterList& params) {
    ...
    describeOutput("scalar",
                   "The result after calculating the dot product of the vector represented by the weighted set "
                   "and the vector sent down with the query");
    return true;
}

ParameterDescriptions DotProductBlueprint::getDescriptions() const {
    return ParameterDescriptions()
        .desc()
        .attribute(ParameterDataTypeSet::primitiveTypeSet(), ParameterCollection::ANY)
        .string();
}

Blueprint::UP DotProductBlueprint::createInstance() const {
...
void DotProductBlueprint::prepareSharedState(const IQueryEnvironment& env, IObjectStore& store) const {
    ...
    if (queryVector == nullptr) {
        ...
        if (arguments) {
            std::string scratchPad;
            store.add(make_queryvector_key_for_attribute(*attribute, _queryVectorKey, scratchPad),
                      std::move(arguments));
        }
    }
...
FeatureExecutor& DotProductBlueprint::createExecutor(const IQueryEnvironment& env, vespalib::Stash& stash) const {
    ...
    std::string          scratchPad;
    const fef::Anything* queryVectorArg =
        env.getObjectStore().get(make_queryvector_key_for_attribute(*attribute, _queryVectorKey, scratchPad));
