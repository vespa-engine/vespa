...
void ReverseProximityExecutor::execute(uint32_t docId) {
    ...
    if (matchA.has_ranking_data(docId) && matchB.has_ranking_data(docId)) {
        ...
        if (itA.valid() && itB.valid()) {
            for (posA = itA.getPosition(), posB = itB.getPosition(); itA.valid() && itA.getPosition() < posB;
                 itA.next())
            {
                // empty
            }
...
void ReverseProximityExecutor::handle_bind_match_data(const fef::MatchData& md) {
    _md = &md;
}

ReverseProximityBlueprint::ReverseProximityBlueprint() : search::fef::Blueprint("reverseProximity"), _config() {
    // empty
}

void ReverseProximityBlueprint::visitDumpFeatures(const search::fef::IIndexEnvironment&,
                                                  search::fef::IDumpFeatureVisitor&) const {
    // empty
}

bool ReverseProximityBlueprint::setup(const search::fef::IIndexEnvironment&,
...
search::fef::Blueprint::UP ReverseProximityBlueprint::createInstance() const {
    return std::make_unique<ReverseProximityBlueprint>();
}

search::fef::FeatureExecutor& ReverseProximityBlueprint::createExecutor(const search::fef::IQueryEnvironment& env,
