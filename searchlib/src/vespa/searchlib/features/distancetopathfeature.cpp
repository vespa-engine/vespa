...
DistanceToPathExecutor::DistanceToPathExecutor(std::vector<Vector2>&                      path,
    ...
    : search::fef::FeatureExecutor(), _intBuf(), _path(), _pos(pos) {
    if (_pos != nullptr) {
...
void DistanceToPathExecutor::execute(uint32_t docId) {
        ...
        for (uint32_t seg = 1; seg < _path.size(); ++seg) {
            ...
            for (auto loc : _intBuf) {
                ...
                if (len < 1e-6) {
                    ...
                    u = std::min(1.0,
                                 std::max(0.0, (((x - p1.x) * (p2.x - p1.x)) + ((y - p1.y) * (p2.y - p1.y))) / len2));
                    if (u == 0) {
...
DistanceToPathBlueprint::DistanceToPathBlueprint() : Blueprint("distanceToPath"), _posAttr() {
}

DistanceToPathBlueprint::~DistanceToPathBlueprint() = default;

void DistanceToPathBlueprint::visitDumpFeatures(const search::fef::IIndexEnvironment&,
                                                search::fef::IDumpFeatureVisitor&) const {
}

search::fef::Blueprint::UP DistanceToPathBlueprint::createInstance() const {
    return std::make_unique<DistanceToPathBlueprint>();
}

bool DistanceToPathBlueprint::setup(const search::fef::IIndexEnvironment&, const search::fef::ParameterList& params) {
    ...
    describeOutput("product",
                   "The cross-product of the intersecting line segment and the intersection-to-document vector.");
    return true;
}

search::fef::FeatureExecutor& DistanceToPathBlueprint::createExecutor(const search::fef::IQueryEnvironment& env,
    ...
    if (path.size() > 1) {
        ...
        if (pos != nullptr) {
            if (!pos->isIntegerType()) {
                Issue::report("distance_to_path feature: The position attribute '%s' is not an integer attribute. "
                              "Will use default distance.",
                              pos->getName().c_str());
                pos = nullptr;
            } else if (pos->getCollectionType() == attribute::CollectionType::WSET) {
                Issue::report("distance_to_path feature: The position attribute '%s' is a weighted set attribute. "
                              "Will use default distance.",
                              pos->getName().c_str());
                pos = nullptr;
            }
            ...
            Issue::report(
                "distance_to_path feature: The position attribute '%s' was not found. Will use default distance.",
                _posAttr.c_str());
        }
    } else {
