...
Bm25Utils::Bm25Utils(const std::string& property_key_prefix, const fef::Properties& properties)
    : _property_key_prefix(property_key_prefix), _properties(properties) {
}

Bm25Utils::~Bm25Utils() = default;

Trinary Bm25Utils::lookup_param(const std::string& param, double& result) const {
    ...
    if (value.found()) {
        try {
            ...
            _error_message =
                std::format("Not able to convert rank property '{}': '{}' to a double value", key, value.get());
...
Trinary Bm25Utils::lookup_param(const std::string& param, std::optional<double>& result) const {
    double tmp_result;
...
double Bm25Utils::calculate_inverse_document_frequency(DocumentFrequency doc_freq) noexcept {
...
double Bm25Utils::get_inverse_document_frequency(const ITermFieldData& term_field, const IQueryEnvironment& env,
