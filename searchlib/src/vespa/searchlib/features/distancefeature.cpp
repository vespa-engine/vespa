...
GeoGCDExecutor::GeoGCDExecutor(GeoLocationSpecPtrs locations, const attribute::IAttributeVector* pos)
    : FeatureExecutor(), _locations(), _pos(pos), _intBuf(), _best_index(0.0), _best_lat(0.0), _best_lng(0.0) {
    if (_pos == nullptr) {
        return;
    }
...
void GeoGCDExecutor::execute(uint32_t docId) {
...
void DistanceBlueprint::visitDumpFeatures(const IIndexEnvironment&, IDumpFeatureVisitor&) const {
}

Blueprint::UP DistanceBlueprint::createInstance() const {
    return std::make_unique<DistanceBlueprint>();
}

bool DistanceBlueprint::setup_geopos(const std::string& attr) {
    _attr_name = attr;
    _use_geo_pos = true;
...
bool DistanceBlueprint::setup_nns(const std::string& attr) {
    _attr_name = attr;
...
bool DistanceBlueprint::setup(const IIndexEnvironment& env, const ParameterList& params) {
...
void DistanceBlueprint::prepareSharedState(const fef::IQueryEnvironment& env, fef::IObjectStore& store) const {
    if (_use_nns_tensor) {
...
FeatureExecutor& DistanceBlueprint::createExecutor(const IQueryEnvironment& env, vespalib::Stash& stash) const {
    if (_use_nns_tensor) {
