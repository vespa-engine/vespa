...
public:
    FieldMatchExecutorSharedState(const fef::IQueryEnvironment& query_env, const fef::FieldInfo& field,
...
FieldMatchExecutorSharedState::FieldMatchExecutorSharedState(const IQueryEnvironment& query_env,
                                                             const FieldInfo& field, const fieldmatch::Params& params)
    : Anything(),
...
void FieldMatchExecutor::execute(uint32_t docId) {
    ...
    bool runCmp =
        (simple.getMatches() > 0 && simple.getMatchesWithPosOcc() > 0 && !simple.getMatchWithInvalidFieldLength());
    ...
    outputs().set_number(3,
                         runCmp ? result.getQueryCompleteness() : simple.getQueryCompleteness()); // queryCompleteness
...
void FieldMatchExecutor::handle_bind_match_data(const fef::MatchData& md) {
    _splitter.bind_match_data(md);
}

FieldMatchBlueprint::FieldMatchBlueprint()
    : Blueprint("fieldMatch"), _field(nullptr), _shared_state_key(), _params() {
}

FieldMatchBlueprint::~FieldMatchBlueprint() = default;

void FieldMatchBlueprint::visitDumpFeatures(const IIndexEnvironment& env, IDumpFeatureVisitor& visitor) const {
    for (uint32_t i = 0; i < env.getNumFields(); ++i) {
        const search::fef::FieldInfo* field = env.getField(i);
        if (field->type() == search::fef::FieldType::INDEX && field->collection() == CollectionType::SINGLE) {
...
Blueprint::UP FieldMatchBlueprint::createInstance() const {
    return std::make_unique<FieldMatchBlueprint>();
}

bool FieldMatchBlueprint::setup(const IIndexEnvironment& env, const ParameterList& params) {
    ...
    describeOutput(
        "score",
        "A normalized measure of the degree to which this query and field matched (default, the long name of this is "
        "match). Use "
        "this if you don't want to create your own combination function of more fine grained fieldmatch features.");
    describeOutput(
        "proximity",
        "Normalized proximity - a value which is close to 1 when matched terms are close inside each segment, and "
        "close to zero "
    ...
    describeOutput("queryCompleteness", "The normalized ratio of query tokens matched in the field.");
    ...
    describeOutput("earliness", "A normalized measure of how early the first segment occurs in this field.");
    describeOutput("longestSequenceRatio", "A normalized metric of the relative size of the longest sequence.");
    describeOutput("segmentProximity",
                   "A normalized metric of the closeness (inverse of spread) of segments in the field.");
    describeOutput("unweightedProximity", "The normalized proximity of the matched terms, not taking term "
                                          "connectedness into account. This number is close to 1 if "
                                          "all the matched terms are following each other in sequence, and close to "
                                          "0 if they are far from each other or out of "
                                          "order.");
    describeOutput("absoluteProximity", "Returns the normalized proximity of the matched terms, weighted by the "
                                        "connectedness of the query terms. This number is "
                                        "0.1 if all the matched terms are and have default or lower connectedness, "
                                        "close to 1 if they are following in sequence "
                                        "and have a high connectedness, and close to 0 if they are far from each "
                                        "other in the segments or out of order.");
    describeOutput("occurrence", "Returns a normalized measure of the number of occurrence of the terms of the "
                                 "query. This number is 1 if there are many "
                                 " occurrences of the query terms in absolute terms, or relative to the total "
                                 "content of the field, and 0 if there are "
                                 "none.");
    describeOutput("absoluteOccurrence",
                   "Returns a normalized measure of the number of occurrence of the terms of the query.");
    describeOutput("weightedOccurrence", "Returns a normalized measure of the number of occurrence of the terms of "
                                         "the query, weighted by term weight. This number "
                                         "is close to 1 if there are many occurrences of highly weighted query "
                                         "terms, in absolute terms, or relative to the total "
                                         "content of the field, and 0 if there are none.");
    describeOutput("weightedAbsoluteOccurrence",
                   "Returns a normalized measure of the number of occurrence of the terms of the query, taking "
                   "weights into account so that "
                   "occurrences of higher weighted query terms has more impact than lower weighted terms.");
    describeOutput("significantOccurrence", "Returns a normalized measure of the number of occurrence of the terms "
                                            "of the query in absolute terms, or relative to the "
    ...
    describeOutput("significance", "Returns the normalized term significance (1-frequency) of the terms of this "
                                   "match relative to the whole query.");
    ...
    describeOutput("matches", "The number of query terms which was matched in this field.");
    describeOutput("outOfOrder", "The total number of out of order token sequences within matched field segments.");
    describeOutput("gaps", "The total number of position jumps (backward or forward) within field segments.");
    describeOutput("gapLength", "The summed length of all gaps within segments.");
    describeOutput("longestSequence", "The size of the longest matched continuous, in-order sequence in the field.");
    describeOutput("head", "The number of tokens in the field preceeding the start of the first matched segment.");
    describeOutput("tail", "The number of tokens in the field following the end of the last matched segment.");
    describeOutput("segmentDistance", "The sum of the distance between all segments making up a match to the query, "
                                      "measured as the sum of the number of token "
...
FeatureExecutor& FieldMatchBlueprint::createExecutor(const IQueryEnvironment& env, vespalib::Stash& stash) const {
    auto* shared_state =
        dynamic_cast<const FieldMatchExecutorSharedState*>(env.getObjectStore().get(_shared_state_key));
