...
ElementGap get_element_gap(const IIndexEnvironment& env, const std::string& base_name,
                           const std::string& field_name) {
    auto property = env.getProperties().lookup(base_name, "elementGap", field_name);
    if (!property.found()) {
        auto field = env.getFieldByName(field_name);
        if (field != nullptr) {
...
NativeProximityExecutorSharedState::NativeProximityExecutorSharedState(const IQueryEnvironment&     env,
    ...
    : fef::Anything(), _params(params), _setups(), _total_field_weight(0), _fields() {
...
void NativeProximityExecutorSharedState::generateTermPairs(const IQueryEnvironment& env, const QueryTermVector& terms,
...
void NativeProximityExecutor::execute(uint32_t docId) {
    feature_t score = 0;
...
void NativeProximityBlueprint::visitDumpFeatures(const IIndexEnvironment& env, IDumpFeatureVisitor& visitor) const {
    (void)env;
    visitor.visitDumpFeature(getBaseName());
}

Blueprint::UP NativeProximityBlueprint::createInstance() const {
    return std::make_unique<NativeProximityBlueprint>();
}

bool NativeProximityBlueprint::setup(const IIndexEnvironment& env, const ParameterList& params) {
    vespalib::asciistream shared_state_key_builder;
    _params.resize(env.getNumFields());
    _params.slidingWindow =
        util::strToNum<uint32_t>(env.getProperties().lookup(getBaseName(), "slidingWindowSize").get("4"));
    FieldWrapper fields(env, params, FieldType::INDEX);
    std::string  defaultProximityImportance =
        env.getProperties().lookup(getBaseName(), "proximityImportance").get("0.5");
    ...
    for (uint32_t i = 0; i < fields.getNumFields(); ++i) {
        ...
        param.field = true;
        if ((param.proximityTable = util::lookupTable(env, getBaseName(), "proximityTable", info->name(),
                                                      _defaultProximityBoost)) == nullptr)
        {
            return false;
        }
        if ((param.revProximityTable = util::lookupTable(env, getBaseName(), "reverseProximityTable", info->name(),
                                                         _defaultRevProximityBoost)) == nullptr)
        {
            return false;
        }
        param.fieldWeight = indexproperties::FieldWeight::lookup(env.getProperties(), info->name());
        if (param.fieldWeight == 0 || info->isFilter()) {
            param.field = false;
        }

        std::string alt_name = FeatureNameBuilder().baseName(getBaseName()).parameter(info->name()).buildName();
        std::string alt_importance =
            env.getProperties().lookup(alt_name, "proximityImportance").get(defaultProximityImportance);
        std::string importance =
            env.getProperties().lookup(getBaseName(), "proximityImportance", info->name()).get(alt_importance);
        ...
        if (NativeRankBlueprint::useTableNormalization(env)) {
            ...
            if (fp != nullptr && rp != nullptr) {
                double value =
                    (fp->max() * param.proximityImportance) + (rp->max() * (1 - param.proximityImportance));
...
FeatureExecutor& NativeProximityBlueprint::createExecutor(const IQueryEnvironment& env,
                                                          vespalib::Stash&         stash) const {
    auto* shared_state =
        dynamic_cast<const NativeProximityExecutorSharedState*>(env.getObjectStore().get(_shared_state_key));
...
void NativeProximityBlueprint::prepareSharedState(const IQueryEnvironment& queryEnv,
                                                  IObjectStore&            objectStore) const {
