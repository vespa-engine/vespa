...
AgeExecutor::AgeExecutor(const IAttributeVector* attribute)
    : search::fef::FeatureExecutor(), _attribute(attribute), _buf() {
    if (_attribute != nullptr) {
        _buf.allocate(attribute->getMaxValueCount());
    }
}

AgeBlueprint::AgeBlueprint() : fef::Blueprint("age") {
}

AgeBlueprint::~AgeBlueprint() = default;

void AgeExecutor::execute(uint32_t docId) {
...
void AgeBlueprint::visitDumpFeatures(const search::fef::IIndexEnvironment&, search::fef::IDumpFeatureVisitor&) const {
    // empty
}

bool AgeBlueprint::setup(const search::fef::IIndexEnvironment&, const search::fef::ParameterList& params) {
...
search::fef::Blueprint::UP AgeBlueprint::createInstance() const {
    return std::make_unique<AgeBlueprint>();
}

search::fef::FeatureExecutor& AgeBlueprint::createExecutor(const search::fef::IQueryEnvironment& env,
...
fef::ParameterDescriptions AgeBlueprint::getDescriptions() const {
