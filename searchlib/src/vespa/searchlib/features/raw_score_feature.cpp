...
void RawScoreExecutor::execute(uint32_t docId) {
...
void RawScoreExecutor::handle_bind_match_data(const fef::MatchData& md) {
    _md = &md;
}
...
bool RawScoreBlueprint::setup(const IIndexEnvironment&, const ParameterList& params) {
    _field = params[0].asField();
    describeOutput("out", "accumulated raw score for the given field");
    return true;
}

FeatureExecutor& RawScoreBlueprint::createExecutor(const IQueryEnvironment& queryEnv, vespalib::Stash& stash) const {
