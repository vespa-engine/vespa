...
ElementwiseBm25Executor::ElementwiseBm25Executor(const fef::FieldInfo& field, const fef::IQueryEnvironment& env,
                                                 double avg_element_length, double k1_param, double b_param,
...
void ElementwiseBm25Executor::handle_bind_match_data(const fef::MatchData& match_data) {
    for (auto& term : _terms) {
        term.tfmd = match_data.resolveTermField(term.handle);
    }
}

void ElementwiseBm25Executor::apply_bm25_score(uint32_t num_occs, uint32_t element_id, uint32_t element_length,
...
void ElementwiseBm25Executor::execute(uint32_t doc_id) {
    _scores.clear();
