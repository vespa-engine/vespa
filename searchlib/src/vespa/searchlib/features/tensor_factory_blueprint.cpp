...
bool TensorFactoryBlueprint::extractSource(const std::string& source) {
    std::string error;
    ...
    if (_sourceType != ATTRIBUTE_SOURCE && _sourceType != QUERY_SOURCE) {
        LOG(error, "Expected source type '%s' or '%s', but it was '%s'", ATTRIBUTE_SOURCE.c_str(),
            QUERY_SOURCE.c_str(), _sourceType.c_str());
        return false;
    }
    return true;
}
