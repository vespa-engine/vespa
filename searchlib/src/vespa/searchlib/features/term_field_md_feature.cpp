...
void TermFieldMdExecutor::execute(uint32_t docId) {
...
void TermFieldMdExecutor::handle_bind_match_data(const MatchData& md) {
    _md = &md;
}

TermFieldMdBlueprint::TermFieldMdBlueprint() : Blueprint("termFieldMd"), _field(nullptr) {
}

void TermFieldMdBlueprint::visitDumpFeatures(const IIndexEnvironment&, IDumpFeatureVisitor&) const {
}

Blueprint::UP TermFieldMdBlueprint::createInstance() const {
...
FeatureExecutor& TermFieldMdBlueprint::createExecutor(const IQueryEnvironment& env, vespalib::Stash& stash) const {
