...
QueryCompletenessConfig::QueryCompletenessConfig()
    : fieldId(search::fef::IllegalFieldId), fieldBegin(0), fieldEnd(std::numeric_limits<uint32_t>::max()) {
    // empty
}
...
QueryCompletenessExecutor::QueryCompletenessExecutor(const search::fef::IQueryEnvironment& env,
    ...
    : search::fef::FeatureExecutor(), _config(config), _fieldHandles() {
...
void QueryCompletenessExecutor::execute(uint32_t docId) {
...
void QueryCompletenessExecutor::handle_bind_match_data(const fef::MatchData& md) {
    _md = &md;
}

QueryCompletenessBlueprint::QueryCompletenessBlueprint() : search::fef::Blueprint("queryCompleteness"), _config() {
    // empty
}

void QueryCompletenessBlueprint::visitDumpFeatures(const search::fef::IIndexEnvironment&,
                                                   search::fef::IDumpFeatureVisitor&) const {
    // empty
}

bool QueryCompletenessBlueprint::setup(const search::fef::IIndexEnvironment&,
    ...
    if (params.size() > 1) {
        ...
        if (_config.fieldBegin >= _config.fieldEnd) {
            LOG(error,
                "Can not calculate query completeness for field '%s' because range is malformed (from %d to %d).",
...
search::fef::Blueprint::UP QueryCompletenessBlueprint::createInstance() const {
    return std::make_unique<QueryCompletenessBlueprint>();
}

search::fef::FeatureExecutor& QueryCompletenessBlueprint::createExecutor(const search::fef::IQueryEnvironment& env,
