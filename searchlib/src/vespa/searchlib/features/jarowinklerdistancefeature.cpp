...
JaroWinklerDistanceExecutor::JaroWinklerDistanceExecutor(const search::fef::IQueryEnvironment& env,
    ...
    : search::fef::FeatureExecutor(), _config(config), _termFieldHandles(), _md(nullptr) {
    for (uint32_t i = 0; i < env.getNumTerms(); ++i) {
        _termFieldHandles.push_back(util::getTermFieldHandle(env, i, config.fieldId));
    }
}

void JaroWinklerDistanceExecutor::execute(uint32_t docId) {
...
void JaroWinklerDistanceExecutor::handle_bind_match_data(const fef::MatchData& md) {
    _md = &md;
}

namespace {
uint32_t matches(const std::vector<search::fef::FieldPositionsIterator>& termPos, uint32_t fieldLen,
                 uint32_t* numTransposes) {
    (*numTransposes) = 0u;
    uint32_t ret = 0;
...
uint32_t prefixMatch(const std::vector<search::fef::FieldPositionsIterator>& termPos, uint32_t fieldLen,
                     uint32_t maxLen) {
...
feature_t jaroMeasure(const std::vector<search::fef::FieldPositionsIterator>& termPos, uint32_t fieldLen) {
    // _P_A_R_A_N_O_I_A_
    ...
    return (((feature_t)numMatches / termPos.size()) + ((feature_t)numMatches / fieldLen) +
            ((feature_t)numMatches - numTransposes) / numMatches) /
           3.0f;
}
} // namespace

feature_t
JaroWinklerDistanceExecutor::jaroWinklerProximity(const std::vector<search::fef::FieldPositionsIterator>& termPos,
                                                  uint32_t                                                fieldLen) {
...
JaroWinklerDistanceBlueprint::JaroWinklerDistanceBlueprint()
    : search::fef::Blueprint("jaroWinklerDistance"), _config() {
    // empty
}

void JaroWinklerDistanceBlueprint::visitDumpFeatures(const search::fef::IIndexEnvironment&,
                                                     search::fef::IDumpFeatureVisitor&) const {
    // empty
}

bool JaroWinklerDistanceBlueprint::setup(const search::fef::IIndexEnvironment& env,
...
search::fef::Blueprint::UP JaroWinklerDistanceBlueprint::createInstance() const {
    return std::make_unique<JaroWinklerDistanceBlueprint>();
}

search::fef::FeatureExecutor& JaroWinklerDistanceBlueprint::createExecutor(const search::fef::IQueryEnvironment& env,
