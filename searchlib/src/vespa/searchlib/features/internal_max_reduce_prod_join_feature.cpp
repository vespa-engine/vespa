 ...
 */
template <typename BaseType> class RawExecutor : public FeatureExecutor {
...
RawExecutor<BaseType>::RawExecutor(const ArrayReadView* array_read_view, const IntegerVector& queryVector)
    : FeatureExecutor(), _backing(), _array_read_view(array_read_view), _queryVector(queryVector) {
}

namespace {

template <typename T> inline T get_array_element_value(const T& value) noexcept {
    return multivalue::get_value(value);
}

template <typename T> inline T get_array_element_value(const search::attribute::WeightedType<T>& value) noexcept {
    return value.value();
}

} // namespace

template <typename A, typename V> feature_t maxProduct(const A& array, size_t count, const V& query) {
...
template <typename BaseType> void RawExecutor<BaseType>::execute(uint32_t docId) {
 ...
 */
template <typename BaseType> class BufferedExecutor : public FeatureExecutor {
private:
    std::unique_ptr<IntegerVector> _backing;
...
BufferedExecutor<BaseType>::BufferedExecutor(const IAttributeVector* attribute, const IntegerVector& queryVector)
    : FeatureExecutor(), _backing(), _attribute(attribute), _queryVector(queryVector), _buffer() {
}

template <typename BaseType>
BufferedExecutor<BaseType>::BufferedExecutor(const IAttributeVector*        attribute,
                                             std::unique_ptr<IntegerVector> queryVector)
    : FeatureExecutor(), _backing(std::move(queryVector)), _attribute(attribute), _queryVector(*_backing), _buffer() {
}

template <typename BaseType> void BufferedExecutor<BaseType>::execute(uint32_t docId) {
...
FeatureExecutor& selectTypedExecutor(const IAttributeVector* attribute, V&& vector, vespalib::Stash& stash) {
    if (!attribute->isImported()) {
        auto multi_value_attribute = attribute->as_multi_value_attribute();
        if (multi_value_attribute != nullptr) {
            auto array_read_view =
                multi_value_attribute->make_read_view(attribute::IMultiValueAttribute::ArrayTag<BaseType>(), stash);
...
template <typename V>
FeatureExecutor& selectExecutor(const IAttributeVector* attribute, V&& vector, vespalib::Stash& stash) {
    ...
    Issue::report("intrinsic max_reduce_prod_join feature: The attribute vector '%s' is not of type "
                  "array<int/long>, returning default value.",
                  attribute->getName().c_str());
    return stash.create<SingleZeroValueExecutor>();
}

std::string make_queryvector_key(const std::string& base, const std::string& subKey) {
    std::string key(base);
    key.append(".vector.");
    key.append(subKey);
    return key;
...
std::unique_ptr<IntegerVector> createQueryVector(const Property& prop) {
...
InternalMaxReduceProdJoinBlueprint::InternalMaxReduceProdJoinBlueprint()
    : Blueprint("internalMaxReduceProdJoin"), _attribute(), _queryVector(), _attrKey(), _queryVectorKey() {
}

InternalMaxReduceProdJoinBlueprint::~InternalMaxReduceProdJoinBlueprint() = default;

void InternalMaxReduceProdJoinBlueprint::visitDumpFeatures(const IIndexEnvironment&, IDumpFeatureVisitor&) const {
}

Blueprint::UP InternalMaxReduceProdJoinBlueprint::createInstance() const {
    return std::make_unique<InternalMaxReduceProdJoinBlueprint>();
}

ParameterDescriptions InternalMaxReduceProdJoinBlueprint::getDescriptions() const {
    return ParameterDescriptions()
        .desc()
        .attribute(ParameterDataTypeSet::int32OrInt64TypeSet(), ParameterCollection::ARRAY)
        .string();
}

bool InternalMaxReduceProdJoinBlueprint::setup(const IIndexEnvironment&, const ParameterList& params) {
...
FeatureExecutor& InternalMaxReduceProdJoinBlueprint::createExecutor(const IQueryEnvironment& env,
    ...
    if (queryVectorArg != nullptr) {
        ...
        return selectExecutor<const IntegerVector&>(attribute, *dynamic_cast<const IntegerVector*>(queryVectorArg),
                                                    stash);
    } else {
        std::unique_ptr<IntegerVector> vector = createQueryVector(env.getProperties().lookup(_queryVector));
        if (vector) {
