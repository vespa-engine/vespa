...
public:
    ConstantTensorExecutor(vespalib::eval::Value::UP tensor) : _tensor(std::move(tensor)) {}
    bool isPure() override { return true; }
    void execute(uint32_t) override { outputs().set_object(0, *_tensor); }
...
public:
    ConstantTensorRefExecutor(const vespalib::eval::Value& tensor_ref) : _tensor_ref(tensor_ref) {}
    bool isPure() final override { return true; }
    void execute(uint32_t) final override { outputs().set_object(0, _tensor_ref); }
};
