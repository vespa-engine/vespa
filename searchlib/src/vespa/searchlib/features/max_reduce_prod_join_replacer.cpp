...
void try_extract_param(const std::string& feature, const std::string& wanted_wrapper, std::string& param,
                       std::string& dim) {
    FeatureNameParser parser(feature);
    if (parser.valid() && (parser.parameters().size() >= 1) && (parser.parameters().size() <= 2)) {
        std::string wrapper;
        std::string body;
        std::string error;
        if (Function::unwrap(parser.parameters()[0], wrapper, body, error) && (wrapper == wanted_wrapper)) {
            param = body;
...
struct MatchInputs {
    ...
    bool matched() const { return (!attribute.empty() && !query.empty() && (attribute_dim == query_dim)); }
};

struct MaxReduceProdJoinReplacerImpl : ExpressionReplacer {
    Blueprint::UP proto;
    MaxReduceProdJoinReplacerImpl(Blueprint::UP proto_in) : proto(std::move(proto_in)) {}
    IntrinsicExpression::UP maybe_replace(const Function& function, const IIndexEnvironment& env) const override {
        ...
        if (match_function(function, reduce_dim)) {
            ...
            if (match_inputs.matched() && (reduce_dim.empty() || (reduce_dim == match_inputs.attribute_dim))) {
                return IntrinsicBlueprintAdapter::try_create(*proto, env,
                                                             {match_inputs.attribute, match_inputs.query});
            }
        }
...
ExpressionReplacer::UP MaxReduceProdJoinReplacer::create(Blueprint::UP proto) {
