...
TermExecutor::TermExecutor(const search::fef::IQueryEnvironment& env, uint32_t termId)
...
void TermExecutor::execute(uint32_t) {
...
TermBlueprint::TermBlueprint() : search::fef::Blueprint("term"), _termId(0) {
    // empty
}

void TermBlueprint::visitDumpFeatures(const search::fef::IIndexEnvironment& env,
...
bool TermBlueprint::setup(const search::fef::IIndexEnvironment&, const search::fef::ParameterList& params) {
    _termId = params[0].asInteger();
    describeOutput("connectedness",
                   "The normalized strength with which this term is connected to the next term in the query.");
...
search::fef::Blueprint::UP TermBlueprint::createInstance() const {
    return std::make_unique<TermBlueprint>();
}

search::fef::FeatureExecutor& TermBlueprint::createExecutor(const search::fef::IQueryEnvironment& env,
