 ...
 *
 * <p>Given that we want to start a segment at i, this holdes the best known metrics up to i and the end of the
 * previous segment. In addition it holds information on how far we have tried to look for alternative segments from
 * this starting point (skipI and previousJ).</p>
 *
...
public:
     ...
     */
    SegmentStart(Computer* owner, const Metrics& metrics, uint32_t previousJ = 0, uint32_t i = 0,
     ...
     */
    uint32_t getPreviousJ() const { return _previousJ; }
     ...
     */
    uint32_t getSemanticDistanceExplored() const { return _semanticDistanceExplored; }
    ...
    /**
     * Returns the position startI we should start at from this start point i.  startI==i except when there are i's
     * from this starting point which are not found anywhere in the field. In that case, startI==i+the number of terms
     * following i which are known not to be present.
     *
     * @return The start i value.
     */
    uint32_t getStartI() const { return _i + _skipI; }

    /**
     * Increments the startI by one because we have discovered that the term at the current startI is not present in
     * the field.
     *
     * @return This, to allow chaining.
     */
