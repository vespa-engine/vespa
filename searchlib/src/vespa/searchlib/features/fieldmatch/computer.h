 ...
 * semantic structure, while tokens farther apart are much more loosely related.  The algorithm will locate
 * alternative such regions containing multiple query tokens (segments), do a more detailed analysis of these segments
 * and choose the ones producing the best overall set of match metrics.</p>
 *
 ...
 * match metric score is preferred. The dynamic programming paradigm is used to avoid redoing work on
 * segmentations.</p>
 *
 * <p>When a segment start point is found, subsequenc tokens from the query are searched in the field from this
 * starting point in "semantic order". This search order can be defined independently of the algorithm. The current
 * order searches <i>proximityLimit tokens ahead first, then the same distance backwards (so if you need to go two
 * steps backwards in the field from the segment starting point, the real distance is -2, but the "semantic distance"
 * is proximityLimit+2.</p>
 *
...
public:
     ...
     * @return The final metrics.
     */
    const Metrics& getFinalMetrics() const { return _finalMetrics; }
     ...
     */
    uint32_t getFieldLength() const { return _fieldLength; }
     ...
     */
    uint32_t getNumQueryTerms() const { return _queryTerms.size(); }
     ...
     */
    const QueryTerm& getQueryTermData(int term) const { return _queryTerms[term]; }
     ...
     */
    const fef::TermFieldMatchData* getQueryTermFieldMatch(int term) const { return _queryTermFieldMatch[term]; }
     ...
     */
    uint32_t getTotalTermWeight() const { return _totalTermWeight; }
     ...
     */
    feature_t getTotalTermSignificance() const { return _totalTermSignificance; }
     ...
     */
    const SimpleMetrics& getSimpleMetrics() const { return _simpleMetrics; }
