...
public:
     ...
     */
    std::vector<uint32_t>& getSegmentStarts() { return _segmentStarts; }
     ...
     */
    uint32_t getOutOfOrder() const { return _outOfOrder; }
     ...
     */
    uint32_t getSegments() const { return _segments; }
     ...
     */
    uint32_t getGaps() const { return _gaps; }
     ...
     */
    uint32_t getGapLength() const { return _gapLength; }
     ...
     */
    uint32_t getLongestSequence() const { return _longestSequence; }
     ...
     */
    uint32_t getPairs() const { return _pairs; }
     ...
     */
    feature_t getAbsoluteProximity() const { return _pairs < 1 ? 0.1f : _proximity / _pairs; }

    /**
     * Returns the normalized proximity of the matched terms, not taking term connectedness into account.  This number
     * is close to 1 if all the matched terms are following each other in sequence, and close to 0 if they are far
     * from each other or out of order
     *
     * @return The proximity.
     */
    feature_t getUnweightedProximity() const { return _pairs < 1 ? 1.0f : _unweightedProximity / _pairs; }
     ...
     */
    feature_t getSegmentDistance() const { return _segmentDistance; }
     ...
     *
     * <p>As the sum of this number over all the terms of the query is always 1, sums over all fields of normalized
     * rank features for each field multiplied by this number for the same field will produce a normalized number.</p>
     *
     * <p>Note that this scales with the number of matched query terms in the field. If you want a component which
     * does not, divide by matches.</p>
     *
     * @return The normalized weight.
     */
    feature_t getWeight() const { return _weight; }

    /**
     * <p>Returns the normalized term significance (1-frequency) of the terms of this match relative to the whole
     * query: The sum of the significance of all <i>matched</i> terms/the sum of the significance of all <i>query</i>
     * terms If all the query terms were matched, this is 1. If no terms were matched, or if the significance of all
     * the matched terms is zero (they are present in all (possible) documents), this number is zero.</p>
     *
     * <p>As the sum of this number over all the terms of the query is always 1, sums over all fields of normalized
     * rank features for each field multiplied by this number for the same field will produce a normalized number.</p>
     *
     ...
     */
    feature_t getSignificance() const { return _significance; }

    /**
     * <p>Returns a normalized measure of the number of occurrence of the terms of the query.  This number is 1 if
     * there are many occurences of the query terms <i>in absolute terms, or relative to the total content of the
     * field</i>, and 0 if there are none.</p>
     *
     * <p>This is suitable for occurence in fields containing regular text.</p>
     *
     * @return The normalized number of occurences.
     */
    feature_t getOccurrence() const { return _occurrence; }
     ...
     * 100)</code>
     *
     * <p>This number is 1 if there are many occurrences of the query terms, and 0 if there are none.  This number
     * does not take the actual length of the field into account, so it is suitable for uses of occurrence to denote
     * importance across multiple terms.</p>
     *
     * @return The normalized number of occurences.
     */
    feature_t getAbsoluteOccurrence() const { return _absoluteOccurrence; }
     ...
     */
    feature_t getWeightedOccurrence() const { return _weightedOccurrence; }
     ...
     * @return The normalized measure of weighted occurences.
     */
    feature_t getWeightedAbsoluteOccurrence() const { return _weightedAbsoluteOccurrence; }
     ...
     */
    feature_t getSignificantOccurrence() const { return _significantOccurrence; }
     ...
     */
    feature_t getOrderness() const { return _pairs < 1 ? 1.0f : 1 - (feature_t)_outOfOrder / _pairs; }
    ...
    /**
     * Returns a value which is close to 1 when matched terms are close and close to zero when they are far apart in
     * the segment. Relatively more connected terms influence this value more.  This is absoluteProximity/average
     * connectedness.
     *
     ...
     */
    feature_t getImportance() const { return (getSignificance() + getWeight()) / 2; }
     ...
     *
     * <p>Weight and significance are not taken into account because this is mean to capture tha quality of the match
     * in this field, while those measures relate this match to matches in other fields. This number can be multiplied
     * with those values when combining with other field match scores.</p>
     *
     ...
     *
     * <p>This metric, and any metric it dependends on, must be correct each time a segment is completed, not only
     * when the metrics are complete, because this metric is used to choose segments during calculation.</p>
     *
     ...
     */
    void setOccurrence(feature_t occurrence) { _occurrence = occurrence; }
     ...
     */
    void setWeightedOccurrence(feature_t weightedOccurrence) { _weightedOccurrence = weightedOccurrence; }
     ...
     */
    void setAbsoluteOccurrence(feature_t absoluteOccurrence) { _absoluteOccurrence = absoluteOccurrence; }
     ...
     */
    void setSignificantOccurrence(feature_t significantOccurrence) { _significantOccurrence = significantOccurrence; }
