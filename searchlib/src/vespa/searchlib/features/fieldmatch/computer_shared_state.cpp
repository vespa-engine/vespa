...
ComputerSharedState::ComputerSharedState(const std::string&            propertyNamespace,
                                         const PhraseSplitterQueryEnv& splitter_query_env, const FieldInfo& fieldInfo,
                                         const Params& params)
    ...
    _total_term_weight = atoi(splitter_query_env.getProperties()
                                  .lookup(propertyNamespace, "totalTermWeight")
                                  .get(vespalib::make_string("%d", _total_term_weight))
                                  .c_str());
    _total_term_significance =
        vespalib::locale::c::atof(splitter_query_env.getProperties()
                                      .lookup(propertyNamespace, "totalTermSignificance")
                                      .get(vespalib::make_string("%f", _total_term_significance))
                                      .c_str());
