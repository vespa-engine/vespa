...
void SegmentStart::reset(const Metrics& metrics, uint32_t previousJ, uint32_t i, uint32_t j) {
    _metrics = metrics;
    _i = i;
    _skipI = 0;
    _previousJ = previousJ;
...
SegmentStart& SegmentStart::exploredTo(uint32_t j) {
    _semanticDistanceExplored = _owner->fieldIndexToSemanticDistance(j, _previousJ) + 1;
    return *this;
}

bool SegmentStart::offerHistory(int previousJ, const Metrics& metrics) {
...
std::string SegmentStart::toString() {
    if (_i == _owner->getNumQueryTerms()) {
        return vespalib::make_string("Last segment: Complete match %f, previous j %d (%s).", _metrics.getMatch(),
                                     _previousJ, _open ? "open" : "closed");
    } else {
        return vespalib::make_string("Segment at %d: Match %f, previous j %d, explored to %d (%s).", _i,
                                     _metrics.getMatch(), _previousJ, _semanticDistanceExplored,
                                     _open ? "open" : "closed");
    }
}
