...
feature_t Metrics::getQueryCompleteness() const {
    return _queryLength > 0 ? (feature_t)_matches / _queryLength : 0.0f;
}

feature_t Metrics::getFieldCompleteness() const {
...
feature_t Metrics::getCompleteness() const {
...
feature_t Metrics::getRelatedness() const {
    if (_matches == 0) {
        return 0;
    } else if (_matches == 1) {
        return 1;
...
feature_t Metrics::getSegmentProximity() const {
    if (_source->getFieldLength() == 0) {
        return 0; // default
    }
...
feature_t Metrics::getProximity() const {
...
feature_t Metrics::getEarliness() const {
    if (_matches == 0) {
...
feature_t Metrics::getMatch() const {
    ...
    return (proximityCompletenessImportance * scaledRelatedness * getProximity() * getCompleteness() *
                getCompleteness() +
            earlinessImportance * getEarliness() + segmentProximityImportance * getSegmentProximity() +
            occurrenceImportance * getOccurrence()) /
           (proximityCompletenessImportance + earlinessImportance + segmentProximityImportance +
            occurrenceImportance);
}

feature_t Metrics::getSegmentationScore() const {
...
void Metrics::onMatch(uint32_t i) {
    if (_matches >= _source->getFieldLength()) {
        return;
    }
    _matches++;
    _weight += _source->getTotalTermWeight() > 0
                   ? (feature_t)_source->getQueryTermData(i).termData()->getWeight().percent() /
                         _source->getTotalTermWeight()
                   : 0.0f;
    _significance += _source->getTotalTermSignificance() > 0.0f
                         ? _source->getQueryTermData(i).significance() / _source->getTotalTermSignificance()
                         : 0.0f;
}

void Metrics::onSequenceStart(uint32_t j) {
...
void Metrics::onSequenceEnd(uint32_t j) {
...
void Metrics::onPair(uint32_t i, uint32_t j, uint32_t previousJ) {
    ...
    feature_t pairProximity =
        _source->getParams().getProximityTable()[distance + _source->getParams().getProximityLimit()];
...
void Metrics::onInSequence(uint32_t, uint32_t, uint32_t) {
    _currentSequence++;
}

void Metrics::onInSegmentGap(uint32_t, uint32_t j, uint32_t previousJ) {
    _gaps++;
    if (j > previousJ) {
...
void Metrics::onNewSegment(uint32_t, uint32_t j, uint32_t) {
    _segments++;
