...
QueryTerm QueryTermFactory::create(const IQueryEnvironment& env, uint32_t termIdx, bool lookupConnectedness) {
...
QueryTermHelper::QueryTermHelper(const IQueryEnvironment& env) : _fallBack(), _queryTerms(lookupQueryTerms(env)) {
...
const QueryTermVector& QueryTermHelper::lookupAndStoreQueryTerms(const IQueryEnvironment& env, IObjectStore& store) {
...
const QueryTermVector* QueryTermHelper::lookupQueryTerms(const IQueryEnvironment& env) {
    const Anything* obj = env.getObjectStore().get(QUERY_TERMS_KEY);
    return (obj != nullptr) ? &QueryTermVectorWrapper::getValue(*obj) : nullptr;
}

QueryTermVector QueryTermHelper::createQueryTermvector(const IQueryEnvironment& env) {
