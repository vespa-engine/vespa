...
MultiTerm::MultiTerm(std::unique_ptr<QueryNodeResultBase> result_base, string index, uint32_t num_terms)
    : QueryTerm(std::move(result_base), "", std::move(index), Type::WORD, Normalizing::NONE), _terms() {
...
MultiTerm::MultiTerm(std::unique_ptr<QueryNodeResultBase> result_base, string index,
    ...
    for (uint32_t i = 0; i < num_terms; ++i) {
        add_term(std::make_unique<QueryTerm>(std::unique_ptr<QueryNodeResultBase>(), terms->getAsString(i).first, "",
                                             Type::WORD, normalizing));
    }
}

MultiTerm::~MultiTerm() = default;

void MultiTerm::add_term(std::unique_ptr<QueryTerm> term) {
    _terms.emplace_back(std::move(term));
}

void MultiTerm::reset() {
...
MultiTerm* MultiTerm::as_multi_term() noexcept {
    return this;
}

const MultiTerm* MultiTerm::as_multi_term() const noexcept {
    return this;
}

bool MultiTerm::multi_index_terms() const noexcept {
    return false;
}
