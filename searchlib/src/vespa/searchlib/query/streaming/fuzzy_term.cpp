...
FuzzyTerm::FuzzyTerm(std::unique_ptr<QueryNodeResultBase> result_base, string_view term, const string& index,
                     Type type, Normalizing normalizing, uint8_t max_edits, uint32_t prefix_lock_length,
                     bool prefix_match)
    : QueryTerm(std::move(result_base), term, index, type, normalizing), _dfa_matcher(), _fallback_matcher() {
    ...
    if (attribute::DfaFuzzyMatcher::supports_max_edits(max_edits)) {
        _dfa_matcher = std::make_unique<attribute::DfaFuzzyMatcher>(term_view, max_edits, prefix_lock_length, cased,
                                                                    prefix_match);
    } else {
        _fallback_matcher =
            std::make_unique<vespalib::FuzzyMatcher>(term_view, max_edits, prefix_lock_length, cased, prefix_match);
    }
}
