...
uint16_t cap_16_bits(uint32_t value) {
    return std::min(value, static_cast<uint32_t>(std::numeric_limits<uint16_t>::max()));
}

uint32_t extract_field_length(const QueryTerm& term, uint32_t field_id) {
    return (field_id < term.getFieldInfoSize()) ? term.getFieldInfo(field_id).getFieldLength()
                                                : search::fef::FieldPositionsIterator::UNKNOWN_LENGTH;
}

void set_interleaved_features(search::fef::TermFieldMatchData& tmd, uint32_t field_length, uint32_t num_occs) {
...
void QueryTerm::unpack_filtered_match_data(uint32_t docid, const fef::ITermData& td, fef::MatchData& match_data,
                                           const HitListType& hit_list, const QueryTerm& fl_term, bool term_filter,
                                           const fef::IIndexEnvironment& index_env,
                                           MatchDataFilter               match_data_filter) {
    (void)fl_term;
    if (!hit_list.empty()) { // only unpack if we have a hit
        ...
        for (const auto& hit : hit_list) {
            ...
            if (tmd != nullptr && !filter) {
                search::fef::TermFieldMatchDataPosition pos(hit.element_id(), hit.position(), hit.element_weight(),
                                                            hit.element_length());
...
void QueryTerm::unpack_match_data_helper(uint32_t docid, const fef::ITermData& td, fef::MatchData& match_data,
                                         const HitListType& hit_list, const QueryTerm& fl_term, bool term_filter,
                                         const fef::IIndexEnvironment& index_env,
                                         search::common::ElementIds    element_ids) {
    if (element_ids.all_elements()) {
        unpack_filtered_match_data(docid, td, match_data, hit_list, fl_term, term_filter, index_env,
                                   fef::NoMatchDataFilter());
    } else {
        unpack_filtered_match_data(docid, td, match_data, hit_list, fl_term, term_filter, index_env,
                                   fef::ElementIdMatchDataFilter(element_ids));
    }
}
