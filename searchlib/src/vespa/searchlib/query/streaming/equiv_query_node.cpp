...
public:
    HitWithFieldLength(const Hit& hit, uint32_t field_length) noexcept : Hit(hit), _field_length(field_length) {}
    uint32_t get_field_length() const noexcept { return _field_length; }
};

template <typename HitType> void merge_hits_from_children(std::vector<HitType>& hl, const MultiTerm& mt) {
...
const HitList& EquivQueryNode::evaluateHits(HitList& hl) {
    hl.clear();
    merge_hits_from_children(hl, *this);
    return hl;
}

void EquivQueryNode::get_element_ids(std::vector<uint32_t>& element_ids) {
    HitList hit_list;
    merge_hits_from_children(hit_list, *this);
    get_element_ids_helper(element_ids, hit_list);
}

void EquivQueryNode::unpack_match_data(uint32_t docid, const fef::ITermData& td, fef::MatchData& match_data,
...
bool EquivQueryNode::multi_index_terms() const noexcept {
    return true;
}

const EquivQueryNode* EquivQueryNode::as_equiv_query_node() const noexcept {
    return this;
}

std::vector<std::unique_ptr<QueryTerm>> EquivQueryNode::steal_terms() {
