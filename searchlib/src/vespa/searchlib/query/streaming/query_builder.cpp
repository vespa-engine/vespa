...
bool possibleFloat(const QueryTerm& qt, const QueryTerm::string& term) {
    return qt.encoding().isFloat() &&
           ((term.find('.') != QueryTerm::string::npos) || (term.find('-') != QueryTerm::string::npos));
}
...
public:
    HiddenTermsGuard(QueryBuilder& builder) : _builder(builder), _active(false) {}
    ~HiddenTermsGuard() { deactivate(); }
    void activate() {
        if (!_active) {
            _active = true;
...
QueryBuilder::QueryBuilder() : _same_element_view(), _hidden_terms(0u), _expose_match_data_for_same_element(true) {
}

QueryBuilder::~QueryBuilder() = default;

void QueryBuilder::adjust_index(std::string& index, bool ranked) {
    if (index.empty()) {
...
std::unique_ptr<QueryNode> QueryBuilder::build(const QueryNode* parent, const QueryNodeResultFactory& factory,
                                               QueryStackIterator& queryRep, bool allowRewrite) {
    ...
    case ParseItem::ITEM_FUZZY: {
        ...
        if (ssIndex == "sddocname") {
            ...
            if (sTerm == TermType::REGEXP) {
                qt =
                    std::make_unique<RegexpTerm>(factory.create(), ssTerm, ssIndex, TermType::REGEXP, normalize_mode);
            ...
            if (allowRewrite && possibleFloat(*qt, ssTerm) && factory.allow_float_terms_rewrite(ssIndex)) {
                ...
                if (splitter.parts() > 1) {
                    ...
                    for (size_t i = 0; i < splitter.parts(); ++i) {
                        phrase->add_term(std::make_unique<QueryTerm>(factory.create(), splitter.getPart(i), ssIndex,
                                                                     TermType::WORD, normalize_mode));
                    }
                    alt_qt = std::move(phrase);
                } else if (splitter.parts() == 1 && ssTerm != splitter.getPart(0)) {
                    alt_qt = std::make_unique<QueryTerm>(factory.create(), splitter.getPart(0), ssIndex,
                                                         TermType::WORD, normalize_mode);
                }
                if (alt_qt) {
...
std::unique_ptr<QueryNode> QueryBuilder::build_nearest_neighbor_query_node(const QueryNodeResultFactory& factory,
                                                                           QueryStackIterator&           query_rep) {
    ...
    return std::make_unique<NearestNeighborQueryNode>(factory.create(), query_tensor_name, field_name, target_hits,
                                                      distance_threshold, unique_id, weight);
}

void QueryBuilder::populate_multi_term(Normalizing string_normalize_mode, MultiTerm& mt,
...
std::unique_ptr<QueryNode> QueryBuilder::build_dot_product_term(const QueryNodeResultFactory& factory,
                                                                QueryStackIterator&           queryRep) {
...
std::unique_ptr<QueryNode> QueryBuilder::build_wand_term(const QueryNodeResultFactory& factory,
                                                         QueryStackIterator&           queryRep) {
...
std::unique_ptr<QueryNode> QueryBuilder::build_weighted_set_term(const QueryNodeResultFactory& factory,
                                                                 QueryStackIterator&           queryRep) {
...
std::unique_ptr<QueryNode> QueryBuilder::build_phrase_term(const QueryNodeResultFactory& factory,
                                                           QueryStackIterator&           queryRep) {
    std::string index(queryRep.index_as_view());
    adjust_index(index, !queryRep.hasNoRankFlag() && !hidden_terms());
...
std::unique_ptr<QueryNode> QueryBuilder::build_equiv_term(const QueryNodeResultFactory& factory,
                                                          QueryStackIterator& queryRep, bool allow_rewrite) {
...
std::unique_ptr<QueryNode> QueryBuilder::build_same_element_term(const QueryNodeResultFactory& factory,
                                                                 QueryStackIterator&           queryRep) {
    auto sen = std::make_unique<SameElementQueryNode>(factory.create(), queryRep.index_as_string(),
                                                      queryRep.getArity(), queryRep.get_element_filter());
...
std::unique_ptr<QueryNode> QueryBuilder::build_and_not(const QueryNodeResultFactory& factory,
                                                       QueryStackIterator&           queryRep) {
    HiddenTermsGuard hidden_terms_guard(*this);
    auto             arity = queryRep.getArity();
...
void QueryBuilder::skip_unknown(QueryStackIterator& queryRep) {
