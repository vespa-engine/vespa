...
NearestNeighborQueryNode::NearestNeighborQueryNode(std::unique_ptr<QueryNodeResultBase> resultBase,
                                                   ...
                                                   uint32_t target_hits, double distance_threshold, int32_t unique_id,
                                                   search::query::Weight weight)
    : QueryTerm(std::move(resultBase), query_tensor_name, std::move(field_name), Type::NEAREST_NEIGHBOR,
                Normalizing::NONE),
...
bool NearestNeighborQueryNode::evaluate() {
    return _distance.has_value();
}

void NearestNeighborQueryNode::reset() {
    _distance.reset();
}

NearestNeighborQueryNode* NearestNeighborQueryNode::as_nearest_neighbor_query_node() noexcept {
    return this;
}
...
void NearestNeighborQueryNode::unpack_match_data(uint32_t docid, const fef::ITermData& td, fef::MatchData& match_data,
