...
bool SameElementQueryNode::evaluate() {
...
const HitList& SameElementQueryNode::evaluateHits(HitList& hl) {
    hl.clear();
    return hl;
}

void SameElementQueryNode::get_element_ids(std::vector<uint32_t>& element_ids) {
    if (_cached_evaluate_result.has_value()) {
...
void SameElementQueryNode::unpack_match_data(uint32_t docid, MatchData& match_data,
...
void SameElementQueryNode::unpack_match_data(uint32_t docid, const ITermData& td, MatchData& match_data,
                                             const IIndexEnvironment&, ElementIds) {
...
void SameElementQueryNode::reset() {
    for (auto& child : _children) {
        child->reset();
    }
...
void SameElementQueryNode::add_child(std::unique_ptr<QueryNode> term) {
    _children.emplace_back(std::move(term));
}

bool SameElementQueryNode::is_same_element_query_node() const noexcept {
    return true;
}

SameElementQueryNode* SameElementQueryNode::as_same_element_query_node() noexcept {
    return this;
}

const SameElementQueryNode* SameElementQueryNode::as_same_element_query_node() const noexcept {
    return this;
}

void SameElementQueryNode::get_hidden_leaves(QueryTermList& tl) {
    for (auto& child : _children) {
        child->getLeaves(tl);
    }
}

void SameElementQueryNode::get_hidden_leaves(ConstQueryTermList& tl) const {
    for (auto& child : _children) {
        child->getLeaves(tl);
    }
}
