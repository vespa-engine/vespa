...
WandTerm::WandTerm(std::unique_ptr<QueryNodeResultBase> result_base, string index, uint32_t num_terms)
    : DotProductTerm(std::move(result_base), std::move(index), num_terms), _score_threshold(0.0) {
}

WandTerm::~WandTerm() = default;

bool WandTerm::evaluate() {
...
void WandTerm::unpack_match_data(uint32_t docid, const ITermData& td, MatchData& match_data,
