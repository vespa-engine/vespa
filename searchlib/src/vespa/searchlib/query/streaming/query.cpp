...
std::unique_ptr<QueryNode> build_query_tree(const QueryNodeResultFactory& factory,
...
QueryConnector::QueryConnector(const char* opName) noexcept
    : QueryNode(), _opName(opName), _index(), _children(), _cached_evaluate_result() {
}

void QueryConnector::addChild(std::unique_ptr<QueryNode> child) {
    _children.push_back(std::move(child));
}

QueryConnector::~QueryConnector() = default;

const HitList& QueryConnector::evaluateHits(HitList& hl) {
    if (evaluate()) {
...
void QueryConnector::unpack_match_data(uint32_t docid, MatchData& match_data, const IIndexEnvironment& index_env,
...
void QueryConnector::unpack_match_data(uint32_t docid, fef::MatchData& match_data,
...
void QueryConnector::getLeaves(QueryTermList& tl) {
    for (const auto& node : _children) {
        node->getLeaves(tl);
    }
}

void QueryConnector::getLeaves(ConstQueryTermList& tl) const {
    for (const auto& node : _children) {
        node->getLeaves(tl);
    }
}
...
std::unique_ptr<QueryConnector> QueryConnector::create(ParseItem::ItemType           type,
                                                       const QueryNodeResultFactory& factory) {
    switch (type) {
    ...
    case search::ParseItem::ITEM_NEAR:
        return std::make_unique<NearQueryNode>(factory.get_element_gap_inspector());
...
void TrueNode::get_element_ids(std::vector<uint32_t>&) {
}
...
void FalseNode::get_element_ids(std::vector<uint32_t>&) {
}

AndQueryNode::~AndQueryNode() = default;

bool AndQueryNode::evaluate() {
...
void AndQueryNode::get_element_ids(std::vector<uint32_t>& element_ids) {
    ...
    for (auto& child : others) {
        ...
        std::set_intersection(element_ids.begin(), element_ids.end(), temp_element_ids.begin(),
                              temp_element_ids.end(), std::back_inserter(result));
...
AndNotQueryNode::AndNotQueryNode(bool elementwise) noexcept : QueryConnector("ANDNOT"), _elementwise(elementwise) {
}

AndNotQueryNode::~AndNotQueryNode() = default;

bool AndNotQueryNode::evaluate() {
    if (_cached_evaluate_result.has_value()) {
        return _cached_evaluate_result.value();
    }
...
void AndNotQueryNode::get_element_ids(std::vector<uint32_t>& element_ids) {
    auto& children = getChildren();
    if (children.empty()) {
        return;
    }
    ...
    for (auto& child : others) {
        ...
        if (!temp_element_ids.empty()) {
            result.clear();
            std::set_difference(element_ids.begin(), element_ids.end(), temp_element_ids.begin(),
                                temp_element_ids.end(), std::back_inserter(result));
            std::swap(element_ids, result);
            if (element_ids.empty()) {
                return;
...
void OrQueryNode::get_element_ids(std::vector<uint32_t>& element_ids) {
    auto& children = getChildren();
    if (children.empty()) {
        return;
    }
...
bool RankWithQueryNode::evaluate() {
    if (_cached_evaluate_result.has_value()) {
        return _cached_evaluate_result.value();
    }
...
void RankWithQueryNode::get_element_ids(std::vector<uint32_t>& element_ids) {
    auto& children = getChildren();
    if (children.empty()) {
        return;
    }
...
Query::Query(const QueryNodeResultFactory& factory, const SerializedQueryTree& queryTree) : _root() {
...
bool Query::build(const QueryNodeResultFactory& factory, const SerializedQueryTree& queryTree) {
    _root = build_query_tree(factory, queryTree);
    return valid();
}

void Query::getLeaves(QueryTermList& tl) {
    if (valid()) {
        _root->getLeaves(tl);
    }
}

void Query::getLeaves(ConstQueryTermList& tl) const {
    if (valid()) {
        _root->getLeaves(tl);
    }
}
