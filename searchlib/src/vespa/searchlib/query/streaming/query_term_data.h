...
public:
    ...
    QueryTermDataFactory(const QueryNormalization*                      normalization,
        ...
        : _normalization(normalization), _element_gap_inspector(element_gap_inspector) {}
    ~QueryTermDataFactory() override;
    std::unique_ptr<QueryNodeResultBase> create() const override { return std::make_unique<QueryTermData>(); }
