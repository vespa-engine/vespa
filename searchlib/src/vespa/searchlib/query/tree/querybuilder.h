...
template <class NodeTypes> typename NodeTypes::TrueQueryNode* create_true() {
    return new typename NodeTypes::TrueQueryNode;
}

template <class NodeTypes> typename NodeTypes::FalseQueryNode* create_false() {
    return new typename NodeTypes::FalseQueryNode;
}

// Intermediate nodes
template <class NodeTypes> typename NodeTypes::And* createAnd() {
    return new typename NodeTypes::And;
}

template <class NodeTypes> typename NodeTypes::AndNot* createAndNot() {
    return new typename NodeTypes::AndNot;
}

template <class NodeTypes> typename NodeTypes::Or* createOr() {
    return new typename NodeTypes::Or;
}
...
template <class NodeTypes, typename... Args> typename NodeTypes::DotProduct* createDotProduct(Args&&... args) {
    return new typename NodeTypes::DotProduct(std::forward<Args>(args)...);
}
template <class NodeTypes, typename... Args> typename NodeTypes::WandTerm* createWandTerm(Args&&... args) {
...
// Term nodes
template <class NodeTypes>
typename NodeTypes::NumberTerm* createNumberTerm(const std::string& term, const std::string& view, int32_t id,
                                                 Weight weight) {
    return new typename NodeTypes::NumberTerm(term, view, id, weight);
}
template <class NodeTypes>
typename NodeTypes::PrefixTerm* createPrefixTerm(const std::string& term, const std::string& view, int32_t id,
                                                 Weight weight) {
    return new typename NodeTypes::PrefixTerm(term, view, id, weight);
}
template <class NodeTypes>
typename NodeTypes::RangeTerm* createRangeTerm(const Range& term, const std::string& view, int32_t id,
                                               Weight weight) {
...
template <class NodeTypes>
typename NodeTypes::SubstringTerm* createSubstringTerm(const std::string& term, const std::string& view, int32_t id,
                                                       Weight weight) {
    return new typename NodeTypes::SubstringTerm(term, view, id, weight);
}
template <class NodeTypes>
typename NodeTypes::SuffixTerm* createSuffixTerm(std::string_view term, const std::string& view, int32_t id,
                                                 Weight weight) {
    return new typename NodeTypes::SuffixTerm(std::string(term), view, id, weight);
}

template <class NodeTypes>
typename NodeTypes::LocationTerm* createLocationTerm(const Location& loc, const std::string& view, int32_t id,
                                                     Weight weight) {
    return new typename NodeTypes::LocationTerm(loc, view, id, weight);
}

template <class NodeTypes>
typename NodeTypes::PredicateQuery* createPredicateQuery(PredicateQueryTerm::UP term, const std::string& view,
                                                         int32_t id, Weight weight) {
    return new typename NodeTypes::PredicateQuery(std::move(term), view, id, weight);
}

template <class NodeTypes>
typename NodeTypes::RegExpTerm* createRegExpTerm(const std::string& term, const std::string& view, int32_t id,
                                                 Weight weight) {
...
create_nearest_neighbor_term(std::string_view query_tensor_name, std::string field_name, int32_t id, Weight weight,
                             uint32_t target_num_hits, bool allow_approximate,
...
template <class NodeTypes>
typename NodeTypes::FuzzyTerm* createFuzzyTerm(std::string_view term, const std::string& view, int32_t id,
                                               Weight weight, uint32_t max_edit_distance, uint32_t prefix_lock_length,
                                               bool prefix_match) {
    return new typename NodeTypes::FuzzyTerm(std::string(term), view, id, weight, max_edit_distance,
                                             prefix_lock_length, prefix_match);
}

template <class NodeTypes>
typename NodeTypes::InTerm* create_in_term(std::unique_ptr<TermVector> terms, MultiTerm::Type type,
...
template <class NodeTypes>
typename NodeTypes::WordAlternatives* create_word_alternatives(std::vector<std::unique_ptr<StringTerm>> children,
                                                               const std::string& view, int32_t id, Weight weight) {
    return new typename NodeTypes::WordAlternatives(std::move(children), view, id, weight);
}

template <class NodeTypes>
typename NodeTypes::WordAlternatives* create_word_alternatives(std::unique_ptr<TermVector> terms,
                                                               const std::string& view, int32_t id, Weight weight) {
    return new typename NodeTypes::WordAlternatives(std::move(terms), view, id, weight);
}

template <class NodeTypes> class QueryBuilder : public QueryBuilderBase {
    template <class T> T& addIntermediate(T* node, int child_count) {
        addIntermediateNode(node, child_count);
        return *node;
    }

    template <class T> T& addTerm(T* node) {
...
public:
    ...
    typename NodeTypes::And& addAnd(int child_count) { return addIntermediate(createAnd<NodeTypes>(), child_count); }
    typename NodeTypes::AndNot& addAndNot(int child_count) {
        return addIntermediate(createAndNot<NodeTypes>(), child_count);
    }
    typename NodeTypes::Near& addNear(int child_count, size_t distance, size_t num_negative_children,
                                      size_t exclusion_distance) {
        return addIntermediate(createNear<NodeTypes>(distance, num_negative_children, exclusion_distance),
                               child_count);
    }
    typename NodeTypes::ONear& addONear(int child_count, size_t distance, size_t num_negative_children,
                                        size_t exclusion_distance) {
        return addIntermediate(createONear<NodeTypes>(distance, num_negative_children, exclusion_distance),
                               child_count);
    }
    ...
    typename NodeTypes::SameElement& addSameElement(int child_count, const string& view, int32_t id, Weight weight,
                                                    std::vector<uint32_t> element_filter = std::vector<uint32_t>()) {
        return addIntermediate(createSameElement<NodeTypes>(view, id, weight, std::move(element_filter)),
                               child_count);
    }
    typename NodeTypes::WeightedSetTerm& addWeightedSetTerm(int child_count, const string& view, int32_t id,
                                                            Weight weight) {
        adjustWeight(weight);
        typename NodeTypes::WeightedSetTerm& node =
            addTerm(createWeightedSetTerm<NodeTypes>(child_count, view, id, weight));
        return node;
    }
    ...
    typename NodeTypes::WandTerm& addWandTerm(int child_count, const string& view, int32_t id, Weight weight,
                                              uint32_t targetNumHits, int64_t scoreThreshold,
                                              double thresholdBoostFactor) {
        adjustWeight(weight);
        typename NodeTypes::WandTerm& node = addTerm(createWandTerm<NodeTypes>(
            child_count, view, id, weight, targetNumHits, scoreThreshold, thresholdBoostFactor));
        return node;
    }
    ...
    typename NodeTypes::WeightedSetTerm& addWeightedSetTerm(std::unique_ptr<TermVector> tv, MultiTerm::Type type,
        ...
        adjustWeight(weight);
        typename NodeTypes::WeightedSetTerm& node =
            addTerm(createWeightedSetTerm<NodeTypes>(std::move(tv), type, view, id, weight));
        return node;
    }
    typename NodeTypes::DotProduct& addDotProduct(std::unique_ptr<TermVector> tv, MultiTerm::Type type,
                                                  const string& view, int32_t id, Weight weight) {
        adjustWeight(weight);
        typename NodeTypes::DotProduct& node =
            addTerm(createDotProduct<NodeTypes>(std::move(tv), type, view, id, weight));
        return node;
    }
    typename NodeTypes::WandTerm& addWandTerm(std::unique_ptr<TermVector> tv, MultiTerm::Type type,
                                              const string& view, int32_t id, Weight weight, uint32_t targetNumHits,
                                              int64_t scoreThreshold, double thresholdBoostFactor) {
        adjustWeight(weight);
        typename NodeTypes::WandTerm& node = addTerm(createWandTerm<NodeTypes>(
            std::move(tv), type, view, id, weight, targetNumHits, scoreThreshold, thresholdBoostFactor));
        return node;
    }
    ...
    typename NodeTypes::StringRangeTerm& addStringRangeTerm(const StringRange& string_range, const string& view,
                                                            int32_t id, Weight weight) {
        adjustWeight(weight);
        return addTerm(createStringRangeTerm<NodeTypes>(string_range, view, id, weight));
    }
    ...
    typename NodeTypes::SubstringTerm& addSubstringTerm(const string& t, const string& view, int32_t id,
                                                        Weight weight) {
        adjustWeight(weight);
    ...
    typename NodeTypes::LocationTerm& addLocationTerm(const Location& loc, const string& view, int32_t id,
                                                      Weight weight) {
        adjustWeight(weight);
        return addTerm(createLocationTerm<NodeTypes>(loc, view, id, weight));
    }
    typename NodeTypes::PredicateQuery& addPredicateQuery(PredicateQueryTerm::UP term, const string& view, int32_t id,
                                                          Weight weight) {
        adjustWeight(weight);
    ...
    typename NodeTypes::FuzzyTerm& addFuzzyTerm(const string& term, const string& view, int32_t id, Weight weight,
        ...
        adjustWeight(weight);
        return addTerm(
            createFuzzyTerm<NodeTypes>(term, view, id, weight, max_edit_distance, prefix_lock_length, prefix_match));
    }
    typename NodeTypes::NearestNeighborTerm&
    add_nearest_neighbor_term(string_view query_tensor_name, string field_name, int32_t id, Weight weight,
                              uint32_t target_num_hits, bool allow_approximate,
                              typename NodeTypes::NearestNeighborTerm::HnswParams hnsw_params =
                                  typename NodeTypes::NearestNeighborTerm::HnswParams()) {
        adjustWeight(weight);
        return addTerm(create_nearest_neighbor_term<NodeTypes>(
            query_tensor_name, field_name, id, weight, target_num_hits, allow_approximate, std::move(hnsw_params)));
    }
    typename NodeTypes::TrueQueryNode& add_true_node() { return addTerm(create_true<NodeTypes>()); }
    typename NodeTypes::FalseQueryNode& add_false_node() { return addTerm(create_false<NodeTypes>()); }
    ...
    typename NodeTypes::WordAlternatives&
    add_word_alternatives(std::vector<std::unique_ptr<typename NodeTypes::StringTerm>> children, const string& view,
                          int32_t id, Weight weight) {
        adjustWeight(weight);
        return addTerm(create_word_alternatives<NodeTypes>(std::move(children), view, id, weight));
    }

    typename NodeTypes::WordAlternatives& add_word_alternatives(std::vector<std::unique_ptr<StringTerm>> children,
                                                                const string& view, int32_t id, Weight weight) {
        adjustWeight(weight);
        return addTerm(create_word_alternatives<NodeTypes>(std::move(children), view, id, weight));
    }

    typename NodeTypes::WordAlternatives& add_word_alternatives(std::unique_ptr<TermVector> terms, const string& view,
                                                                int32_t id, Weight weight) {
        adjustWeight(weight);
