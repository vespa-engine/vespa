...
class QueryNodeConverter : public QueryVisitor {
    ...
    void appendCompressedPositiveNumber(uint64_t n) { _buf.appendCompressedPositiveNumber(n); }

    void appendCompressedNumber(int64_t n) { _buf.appendCompressedNumber(n); }
    ...
    template <typename V> void appendPredicateQueryTermVector(const V& v);

    void createComplexIntermediate(const Term& node, const std::vector<Node*>& children, ParseItem::ItemType type,
                                   uint8_t features) {
        uint8_t flags = 0;
        if (!node.isRanked()) {
    ...
    void createIntermediateX(const Intermediate& node, ParseItem::ItemType type, size_t target_num_hits,
    ...
    void visit(And& node) override { createIntermediate(node, ParseItem::ITEM_AND); }

    void visit(AndNot& node) override { createIntermediate(node, ParseItem::ITEM_NOT); }

    void visit(Near& node) override { createIntermediate(node, ParseItem::ITEM_NEAR, node.getDistance()); }

    void visit(ONear& node) override { createIntermediate(node, ParseItem::ITEM_ONEAR, node.getDistance()); }
    ...
    void visit(SameElement& node) override { createIntermediate(node, ParseItem::ITEM_SAME_ELEMENT, node.getView()); }

    void visit(Phrase& node) override {
        createComplexIntermediate(node, node.getChildren(), ParseItem::ITEM_PHRASE,
                                  static_cast<uint8_t>(ParseItem::IF_WEIGHT));
    }

    template <typename NODE> void createWeightedSet(NODE& node, ParseItem::ItemType type, uint8_t features) {
        uint8_t flags = 0;
        if (!node.isRanked()) {
            flags |= ParseItem::IFLAG_NORANK;
        }
    ...
    void createMultiTermNodes(const MultiTerm& mt) {
        for (size_t i = 0; i < mt.getNumTerms(); ++i) {
            ...
            append_type_and_features(ParseItem::ITEM_PURE_WEIGHTED_STRING,
                                     static_cast<uint8_t>(ParseItem::IF_WEIGHT));
    ...
    template <class Term> void createTerm(const Term& node, ParseItem::ItemType type) {
        createTermNode(node, type);
        appendTerm(node);
    }

    void visit(NumberTerm& node) override { createTerm(node, ParseItem::ITEM_NUMTERM); }

    void visit(LocationTerm& node) override { createTerm(node, ParseItem::ITEM_GEO_LOCATION_TERM); }

    void visit(TrueQueryNode&) override { append_type_and_features(ParseItem::ITEM_TRUE, 0); }

    void visit(FalseQueryNode&) override { append_type_and_features(ParseItem::ITEM_FALSE, 0); }
    ...
    void visit(SuffixTerm& node) override { createTerm(node, ParseItem::ITEM_SUFFIXTERM); }

    void visit(PredicateQuery& node) override { createTerm(node, ParseItem::ITEM_PREDICATE_QUERY); }

    void visit(RegExpTerm& node) override { createTerm(node, ParseItem::ITEM_REGEXP); }
...
public:
    QueryNodeConverter() : _buf(4_Ki) {}

    string getStackDump() { return string(_buf.GetDrainPos(), _buf.GetDrainPos() + _buf.GetUsedLen()); }
};

template <typename T> void QueryNodeConverter::appendTerm(const TermBase<T>& node) {
    vespalib::asciistream ost;
    ost << node.getTerm();
    appendString(ost.view());
}
template <> void QueryNodeConverter::appendTerm(const TermBase<string>& node) {
    appendString(node.getTerm());
}
template <> void QueryNodeConverter::appendTerm(const TermBase<PredicateQueryTerm::UP>& node) {
...
template <typename V> void QueryNodeConverter::appendPredicateQueryTermVector(const V& v) {
