 ...
 */
template <class NodeTypes> class QueryReplicator : private QueryVisitor {
...
private:
    void copyState(const Term& original, Term& replica) { replica.setStateFrom(original); }
    ...
    void visit(Near& node) override {
        _builder.addNear(node.getChildren().size(), node.getDistance(), node.num_negative_terms(),
                         node.exclusion_distance());
        visitNodes(node.getChildren());
    }

    void visit(ONear& node) override {
        _builder.addONear(node.getChildren().size(), node.getDistance(), node.num_negative_terms(),
                          node.exclusion_distance());
        visitNodes(node.getChildren());
    }
    ...
    void visit(Phrase& node) override {
        copyState(node, _builder.addPhrase(node.getChildren().size(), node.getView(), node.getId(), node.getWeight())
                            .set_expensive(node.is_expensive()));
        visitNodes(node.getChildren());
    }

    void visit(SameElement& node) override {
        _builder.addSameElement(node.getChildren().size(), node.getView(), node.getId(), node.getWeight())
            .set_expensive(node.is_expensive());
        visitNodes(node.getChildren());
    }

    void visit(WeightedSetTerm& node) override {
        auto& replica = _builder.addWeightedSetTerm(replicate_subterms(node), node.getType(), node.getView(),
                                                    node.getId(), node.getWeight());
        copyState(node, replica);
    }

    void visit(DotProduct& node) override {
        auto& replica = _builder.addDotProduct(replicate_subterms(node), node.getType(), node.getView(), node.getId(),
                                               node.getWeight());
        copyState(node, replica);
    }

    void visit(WandTerm& node) override {
        auto& replica = _builder.addWandTerm(replicate_subterms(node), node.getType(), node.getView(), node.getId(),
                                             node.getWeight(), node.getTargetNumHits(), node.getScoreThreshold(),
    ...
    void visit(LocationTerm& node) override {
        copyState(node, _builder.addLocationTerm(node.getTerm(), node.getView(), node.getId(), node.getWeight()));
    }
    ...
    void visit(SubstringTerm& node) override {
        copyState(node, _builder.addSubstringTerm(node.getTerm(), node.getView(), node.getId(), node.getWeight()));
    }

    void visit(SuffixTerm& node) override {
        copyState(node, _builder.addSuffixTerm(node.getTerm(), node.getView(), node.getId(), node.getWeight()));
    }

    void visit(PredicateQuery& node) override {
        copyState(node, _builder.addPredicateQuery(std::make_unique<PredicateQueryTerm>(*node.getTerm()),
                                                   node.getView(), node.getId(), node.getWeight()));
    }

    void visit(RegExpTerm& node) override {
        copyState(node, _builder.addRegExpTerm(node.getTerm(), node.getView(), node.getId(), node.getWeight()));
    }
    ...
    void visit(TrueQueryNode&) override { _builder.add_true_node(); }

    void visit(FalseQueryNode&) override { _builder.add_false_node(); }

    void visit(FuzzyTerm& node) override {
        copyState(node,
                  _builder.addFuzzyTerm(node.getTerm(), node.getView(), node.getId(), node.getWeight(),
    ...
    void visit(InTerm& node) override {
        copyState(node, _builder.add_in_term(replicate_subterms(node), node.getType(), node.getView(), node.getId(),
                                             node.getWeight()));
    }
    ...
    void visit(WordAlternatives& node) override {
        copyState(node, _builder.add_word_alternatives(replicate_word_alternatives_children(node), node.getView(),
                                                       node.getId(), node.getWeight()));
    }
};
