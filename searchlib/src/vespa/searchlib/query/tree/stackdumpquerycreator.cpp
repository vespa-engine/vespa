...
void StackDumpQueryCreatorHelper::populateMultiTerm(QueryStackIterator& queryStack, QueryBuilderBase& builder,
                                                    MultiTerm& mt) {
    uint32_t added(0);
    for (added = 0; (added < mt.getNumTerms()) && queryStack.next(); added++) {
        ...
        default:
            builder.reportError(
                vespalib::make_string("Got unexpected node %d for multiterm node at child term %d", type, added));
            return;
        }
    }
...
void StackDumpQueryCreatorHelper::reportError(const QueryStackIterator& queryStack, const QueryBuilderBase& builder) {
