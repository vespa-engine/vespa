...
public:
    PrefixTerm(const Type& term, const std::string& view, int32_t id, Weight weight)
        : QueryNodeMixinType(term, view, id, weight) {}
...
class PredicateQuery : public QueryNodeMixin<PredicateQuery, TermBase<PredicateQueryTerm::UP>> {
public:
...
public:
    NearestNeighborTerm(std::string_view query_tensor_name, std::string field_name, int32_t id, Weight weight,
    ...
    double get_distance_threshold() const {
        return _hnsw_params.distance_threshold.value_or(std::numeric_limits<double>::infinity());
    }
};
...
public:
    WeightedSetTerm(uint32_t num_terms, const std::string& view, int32_t id, Weight weight)
        : QueryNodeMixinType(num_terms), Term(view, id, weight) {}
    WeightedSetTerm(std::unique_ptr<TermVector> terms, Type type, const std::string& view, int32_t id, Weight weight)
        : QueryNodeMixinType(std::move(terms), type), Term(view, id, weight) {}
...
public:
    WandTerm(uint32_t num_terms, const std::string& view, int32_t id, Weight weight, uint32_t targetNumHits,
    ...
    WandTerm(std::unique_ptr<TermVector> terms, Type type, const std::string& view, int32_t id, Weight weight,
...
public:
    InTerm(std::unique_ptr<TermVector> terms, MultiTerm::Type type, const std::string& view, int32_t id,
           Weight weight)
        : QueryNodeMixinType(std::move(terms), type), Term(view, id, weight) {}
    virtual ~InTerm() = 0;
};
...
public:
    ...
    WordAlternatives(std::vector<std::unique_ptr<StringTerm>> children, const std::string& view, int32_t id,
                     Weight weight)
        : QueryNodeMixinType(view, id, weight), _children(std::move(children)) {}
