...
size_t getSerializedSize(const Builder& builder) {
    size_t size = 1 + builder.words().size(); // numWords, [words]
    return size;
}

uint32_t* serialize(const Builder& builder, uint32_t* begin) {
    uint32_t*                     buf = begin;
...
CompactWordsStore::Builder::Builder(uint32_t docId_) : _docId(docId_), _words() {
}

CompactWordsStore::Builder::~Builder() {
}

CompactWordsStore::Builder& CompactWordsStore::Builder::insert(vespalib::datastore::EntryRef wordRef) {
    _words.push_back(wordRef);
    return *this;
}

inline void CompactWordsStore::Iterator::nextWord() {
    _wordRef = *_buf++;
    _remainingWords--;
}

CompactWordsStore::Iterator::Iterator() : _buf(nullptr), _remainingWords(0), _wordRef(0), _valid(false) {
}

CompactWordsStore::Iterator::Iterator(const uint32_t* buf)
    : _buf(buf), _remainingWords(0), _wordRef(0), _valid(true) {
...
CompactWordsStore::Iterator& CompactWordsStore::Iterator::operator++() {
    if (_remainingWords > 0) {
        nextWord();
    } else {
        _valid = false;
...
CompactWordsStore::Store::Store() : _store(), _type(1, MIN_BUFFER_ARRAYS, RefType::offsetSize()), _typeId(0) {
...
vespalib::datastore::EntryRef CompactWordsStore::Store::insert(const Builder& builder) {
...
CompactWordsStore::Iterator CompactWordsStore::Store::get(vespalib::datastore::EntryRef wordRef) const {
    RefType         internalRef(wordRef);
    const uint32_t* buf = _store.getEntry<uint32_t>(internalRef);
    return Iterator(buf);
}

CompactWordsStore::CompactWordsStore() : _docs(), _docs_used_bytes(0), _docs_allocated_bytes(0), _wordsStore() {
...
void CompactWordsStore::insert(const Builder& builder) {
    ...
    if (!insres.second) {
        LOG(error, "Failed inserting remove info for docid %u", builder.docId());
        LOG_ABORT("should not be reached");
    }
}

void CompactWordsStore::remove(uint32_t docId) {
    _docs.erase(docId);
}

CompactWordsStore::Iterator CompactWordsStore::get(uint32_t docId) const {
...
void CompactWordsStore::update_docs_memory_usage() {
...
void CompactWordsStore::commit() {
    update_docs_memory_usage();
}

vespalib::MemoryUsage CompactWordsStore::getMemoryUsage() const {
