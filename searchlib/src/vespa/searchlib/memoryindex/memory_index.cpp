...
MemoryIndex::MemoryIndex(const Schema& schema, const IFieldLengthInspector& inspector,
    ...
    : _schema(schema),
      ...
      _inverter_context(
          std::make_unique<DocumentInverterContext>(_schema, _invertThreads, _pushThreads, *_fieldIndexes)),
...
void MemoryIndex::insertDocument(uint32_t docId, const document::Document& doc,
...
void MemoryIndex::removeDocuments(LidVector lids) {
    if (_frozen) {
...
void MemoryIndex::commit(const OnWriteDoneType& on_write_done) {
    auto& inverter = _inverters->get_active_inverter();
...
void MemoryIndex::dump(IndexBuilder& indexBuilder) {
...
public:
    CreateBlueprintVisitor(Searchable& searchable, const IRequestContext& requestContext, const FieldSpec& field,
                           uint32_t fieldId, FieldIndexCollection& fieldIndexes, fef::MatchDataLayout& global_layout)
    ...
    template <class TermNode> void visitTerm(TermNode& n) {
        ...
        LOG(debug, "searching for '%s' in '%s'", termStr.c_str(), _field.getName().c_str());
    ...
    void visit(NumberTerm& n) override { handleNumberTermAsText(n); }
};

} // namespace

Blueprint::UP MemoryIndex::createBlueprint(const IRequestContext& requestContext, const FieldSpec& field,
                                           const Node& term, fef::MatchDataLayout& global_layout) {
...
std::unique_ptr<queryeval::Blueprint> MemoryIndex::createBlueprint(const queryeval::IRequestContext& requestContext,
...
vespalib::MemoryUsage MemoryIndex::getMemoryUsage() const {
    vespalib::MemoryUsage usage;
    usage.merge(_fieldIndexes->getMemoryUsage());
    return usage;
}

IndexStats MemoryIndex::get_stats() const {
...
uint64_t MemoryIndex::getNumWords() const {
    return _fieldIndexes->getNumUniqueWords();
}

void MemoryIndex::pruneRemovedFields(const Schema& schema) {
...
std::shared_ptr<const Schema> MemoryIndex::getPrunedSchema() const {
    std::lock_guard lock(_lock);
    return _prunedSchema;
}

FieldLengthInfo MemoryIndex::get_field_length_info(const std::string& field_name) const {
...
void write_context_to_slime(const BundledFieldsContext& ctx, const Schema& schema, Cursor& object) {
...
void MemoryIndex::insert_write_context_state(Cursor& object) const {
