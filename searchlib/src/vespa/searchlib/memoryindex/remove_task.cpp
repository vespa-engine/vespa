...
template <typename Inverter> void remove_documents(Inverter& inverter, const std::vector<uint32_t>& lids) {
    for (auto lid : lids) {
...
RemoveTask::RemoveTask(const InvertContext& context, const std::vector<std::unique_ptr<FieldInverter>>& inverters,
                       const std::vector<std::unique_ptr<UrlFieldInverter>>& uri_inverters,
                       const std::vector<uint32_t>&                          lids)
    : _context(context), _inverters(inverters), _uri_inverters(uri_inverters), _lids(lids) {
}
