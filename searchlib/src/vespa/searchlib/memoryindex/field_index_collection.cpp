...
FieldIndexCollection::FieldIndexCollection(const Schema& schema, const IFieldLengthInspector& inspector)
    : _fieldIndexes(), _numFields(schema.getNumIndexFields()) {
    for (uint32_t fieldId = 0; fieldId < _numFields; ++fieldId) {
        ...
        if (field.use_interleaved_features()) {
            _fieldIndexes.push_back(std::make_unique<FieldIndex<true>>(
                schema, fieldId, inspector.get_field_length_info(field.getName())));
        } else {
            _fieldIndexes.push_back(std::make_unique<FieldIndex<false>>(
                schema, fieldId, inspector.get_field_length_info(field.getName())));
        }
    }
}
...
void FieldIndexCollection::dump(search::index::IndexBuilder& indexBuilder) {
    for (uint32_t fieldId = 0; fieldId < _numFields; ++fieldId) {
...
vespalib::MemoryUsage FieldIndexCollection::getMemoryUsage() const {
...
IndexStats FieldIndexCollection::get_stats(const index::Schema& schema) const {
    IndexStats            stats;
    ...
    for (uint32_t field_id = 0; field_id < _numFields; ++field_id) {
        ...
        stats.add_field_stats(schema.getIndexField(field_id).getName(),
                              FieldIndexStats().memory_usage(field_memory_usage));
    }
    stats.memoryUsage(memory_usage);
    return stats;
}

FieldIndexRemover& FieldIndexCollection::get_remover(uint32_t field_id) {
    return _fieldIndexes[field_id]->getDocumentRemover();
}

IOrderedFieldIndexInserter& FieldIndexCollection::get_inserter(uint32_t field_id) {
    return _fieldIndexes[field_id]->getInserter();
}

index::FieldLengthCalculator& FieldIndexCollection::get_calculator(uint32_t field_id) {
