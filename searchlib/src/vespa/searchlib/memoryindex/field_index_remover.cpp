...
FieldIndexRemover::FieldIndexRemover(const WordStore& wordStore)
    : _store(), _builder(), _wordFieldDocTuples(), _wordStore(wordStore) {
}

FieldIndexRemover::~FieldIndexRemover() = default;

void FieldIndexRemover::remove(uint32_t docId, IFieldIndexRemoveListener& listener) {
...
void FieldIndexRemover::insert(vespalib::datastore::EntryRef wordRef, uint32_t docId) {
    _wordFieldDocTuples.emplace_back(wordRef, docId);
}

void FieldIndexRemover::flush() {
    if (_wordFieldDocTuples.empty()) {
        _store.commit();
        return;
    }
    ShiftBasedRadixSorter<WordFieldDocTuple, WordFieldDocTuple::Radix, std::less<WordFieldDocTuple>, 24,
                          true>::radix_sort(WordFieldDocTuple::Radix(), std::less<WordFieldDocTuple>(),
                                            &_wordFieldDocTuples[0], _wordFieldDocTuples.size(), 16);
