...
template <bool interleaved_features> void OrderedFieldIndexInserter<interleaved_features>::flushWord() {
...
template <bool interleaved_features> void OrderedFieldIndexInserter<interleaved_features>::flush() {
    ...
    for (const auto& word_entry : _word_entries) {
        ...
        postingListStore.apply(pidx, adds.data(), adds.data() + adds.size(), removes.data(),
...
template <bool interleaved_features> void OrderedFieldIndexInserter<interleaved_features>::commit() {
    _fieldIndex.commit();
}
...
template <bool interleaved_features>
vespalib::datastore::EntryRef OrderedFieldIndexInserter<interleaved_features>::getWordRef() const {
