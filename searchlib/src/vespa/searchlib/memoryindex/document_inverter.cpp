...
DocumentInverter::DocumentInverter(DocumentInverterContext& context)
    : _context(context), _inverters(), _urlInverters() {
    ...
    for (auto& urlField : schema_index_fields._uriFields) {
        Schema::CollectionType collectionType = schema.getIndexField(urlField._all).getCollectionType();
...
void DocumentInverter::invertDocument(uint32_t docId, const Document& doc, const OnWriteDoneType& on_write_done) {
...
void DocumentInverter::removeDocument(uint32_t docId) {
    LidVector lids;
    lids.push_back(docId);
    removeDocuments(std::move(lids));
}
void DocumentInverter::removeDocuments(LidVector lids) {
    auto& invert_threads = _context.get_invert_threads();
    auto& invert_contexts = _context.get_invert_contexts();
    for (auto& invert_context : invert_contexts) {
        auto id = invert_context.get_id();
...
void DocumentInverter::pushDocuments(const OnWriteDoneType& on_write_done) {
    ...
    for (auto& push_context : push_contexts) {
        ...
        all_push_tasks.emplace_back(
            std::make_shared<ScheduleSequencedTaskCallback>(push_threads, push_context.get_id(), std::move(task)));
    }
    auto& invert_threads = _context.get_invert_threads();
    auto& invert_contexts = _context.get_invert_contexts();
    for (auto& invert_context : invert_contexts) {
