...
UcaFunctionNode::UcaFunctionNode(const UcaFunctionNode& rhs)
    : UnaryFunctionNode(rhs), _locale(rhs._locale), _strength(rhs._strength), _collator(rhs._collator), _handler() {
}
...
UcaFunctionNode::Handler::Handler(const UcaFunctionNode& uca)
    : _converter(*uca._collator), _backingBuffer(), _buffer(_backingBuffer, sizeof(_backingBuffer)) {
}
