...
FS4PersistentPacketStreamer::FS4PersistentPacketStreamer()
    : _compressionLimit(0), _compressionLevel(9), _compressionType(CompressionConfig::LZ4) {
}

//============================================================

FS4Properties::FS4Properties() : _entries(), _name(), _backing() {
}

FS4Properties::FS4Properties(FS4Properties&& rhs) noexcept
    : _entries(std::move(rhs._entries)), _name(std::move(rhs._name)), _backing(std::move(rhs._backing)) {
}

FS4Properties& FS4Properties::operator=(FS4Properties&& rhs) noexcept {
...
void FS4Properties::setKey(uint32_t entry, const char* key, uint32_t keySize) {
    set(_entries[entry].first, std::string_view(key, keySize));
}

void FS4Properties::setValue(uint32_t entry, const char* value, uint32_t valueSize) {
    set(_entries[entry].second, std::string_view(value, valueSize));
}

uint32_t FS4Properties::getLength() const noexcept {
...
std::string_view FS4Properties::key(uint32_t entry) const noexcept {
    auto pair = _entries[entry].first;
    return {c_str(pair.first), pair.second};
}
std::string_view FS4Properties::value(uint32_t entry) const noexcept {
    auto pair = _entries[entry].second;
    return {c_str(pair.first), pair.second};
}

std::string FS4Properties::toString(uint32_t indent) const {
    std::string s;
...
void FS4Properties::allocEntries(uint32_t cnt) {
    _entries.resize(cnt);
