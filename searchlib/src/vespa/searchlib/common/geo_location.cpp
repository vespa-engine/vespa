...
uint64_t abs_diff(int32_t a, int32_t b) {
    return (a > b) ? (int64_t(a) - int64_t(b)) : (int64_t(b) - int64_t(a));
}

ZCurve::BoundingBox to_z(GeoLocation::Box box) {
    return ZCurve::BoundingBox(box.x.low, box.x.high, box.y.low, box.y.high);
}

GeoLocation::Box adjust_bounding_box(GeoLocation::Box orig, GeoLocation::Point point, uint32_t radius,
                                     GeoLocation::Aspect x_aspect) {
    ...
    if (implied_max_x < max_x)
        max_x = implied_max_x;
    if (implied_min_x > min_x)
        min_x = implied_min_x;

    if (implied_max_y < max_y)
        max_y = implied_max_y;
    if (implied_min_y > min_y)
        min_y = implied_min_y;

    return GeoLocation::Box{GeoLocation::Range{min_x, max_x}, GeoLocation::Range{min_y, max_y}};
}
...
bool GeoLocation::inside_limit(Point p) const {
    if (p.x < bounding_box.x.low)
        return false;
    if (p.x > bounding_box.x.high)
        return false;

    if (p.y < bounding_box.y.low)
        return false;
    if (p.y > bounding_box.y.high)
        return false;
