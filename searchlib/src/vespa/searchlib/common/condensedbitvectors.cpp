...
template <typename T> class CondensedBitVectorT : public CondensedBitVector {
public:
...
private:
    static constexpr uint8_t countBits(T v) noexcept { return std::popcount(v); }
    ...
    void clearIndex(uint32_t index) override { _v[index] = 0; }
...
template <typename T>
template <typename F>
void CondensedBitVectorT<T>::computeCountVector(T mask, std::span<uint8_t> cv, F func) const {
    size_t       i(0);
...
template <typename T>
template <typename F>
void CondensedBitVectorT<T>::computeTail(T mask, std::span<uint8_t> cv, F func, size_t i) const {
...
template <typename T> void CondensedBitVectorT<T>::adjustDocIdLimit(uint32_t docId) {
    _v.reserve(docId + 1);
    while (_v.size() <= docId) {
        _v.push_back(0);
    }
...
void throwIllegalKey(size_t numKeys, size_t key) {
    throw IllegalArgumentException(make_string("All %ld possible keys are used. Key %ld is not added", numKeys, key),
                                   VESPA_STRLOC);
}
...
void CondensedBitVector::addKey(Key key) const {
    if (!hasKey(key)) {
        throwIllegalKey(getKeyCapacity(), key);
    }
}

CondensedBitVector::UP CondensedBitVector::create(size_t size, GenerationHolder& genHolder) {
