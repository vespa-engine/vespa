...
public:
    LocationIterator(search::fef::TermFieldMatchData& tfmd, unsigned int numDocs, bool strict,
                     const Location& location);
    ~LocationIterator() override;
};

LocationIterator::LocationIterator(search::fef::TermFieldMatchData& tfmd, unsigned int numDocs, bool strict,
                                   const Location& location)
    : SearchIterator(), _tfmd(tfmd), _numDocs(numDocs), _strict(strict), _location(location), _num_values(0), _pos() {
...
void LocationIterator::doSeek(uint32_t docId) {
...
void LocationIterator::doUnpack(uint32_t docId) {
...
FastS_2DZLocationIterator::FastS_2DZLocationIterator(unsigned int numDocs, bool strict, const Location& location)
    : SearchIterator(), _numDocs(numDocs), _strict(strict), _location(location), _pos() {
    _pos.resize(1); // Need at least 1 entry as the singlevalue attributes does not honour given size.
};

FastS_2DZLocationIterator::~FastS_2DZLocationIterator() = default;

void FastS_2DZLocationIterator::doSeek(uint32_t docId) {
    LOG(debug, "FastS_2DZLocationIterator: seek(%u) with numDocs=%u endId=%u", docId, _numDocs, getEndId());
    if (__builtin_expect(docId >= _numDocs, false)) {
        setAtEnd();
        return;
    }
    ...
    for (;;) {
        uint32_t numValues = location.getVec()->get(docId, &pos[0], pos.size());
...
void FastS_2DZLocationIterator::doUnpack(uint32_t docId) {
    (void)docId;
}

std::unique_ptr<search::queryeval::SearchIterator> FastS_AllocLocationIterator(unsigned int numDocs, bool strict,
                                                                               const Location& location) {
