...
public:
    LidUsageStats() noexcept : _lidLimit(0), _usedLids(0), _lowestFreeLid(0), _highestUsedLid(0) {}
    LidUsageStats(uint32_t lidLimit, uint32_t usedLids, uint32_t lowestFreeLid, uint32_t highestUsedLid) noexcept
        : _lidLimit(lidLimit), _usedLids(usedLids), _lowestFreeLid(lowestFreeLid), _highestUsedLid(highestUsedLid) {}
    ...
    double getLidBloatFactor() const { return (double)getLidBloat() / (double)getLidLimit(); }
