...
public:
    ...
    bool illegalArrayKey() {
        LOG(warning, "%s:%d: illegal array key '%s'(value='%s')", _name.c_str(), _line, _lastKey.c_str(),
            _lastValue.c_str());
        _error = true;
        return false;
    }
    bool illegalValue() {
        LOG(warning, "%s:%d: illegal value for '%s': %s", _name.c_str(), _line, _lastKey.c_str(), _lastValue.c_str());
        _error = true;
        return false;
    }
    bool unknown() {
        LOG(warning, "%s:%d: unknown key '%s'(value='%s')", _name.c_str(), _line, _lastKey.c_str(),
            _lastValue.c_str());
        _error = true;
        return false;
    }
    ...
    void parseInt64(const std::string& k, uint64_t& v) {
        if (!_matched && !_error && _lastKey == k) {
            ...
            if (end == nullptr || *end != '\0' || val == static_cast<uint64_t>(-1)) {
                illegalValue();
                return;
            }
            v = val;
    ...
    bool parseArray(const std::string& name, uint32_t size) {
        if (_matched || _error || _lastKey.length() < name.length() + 1 ||
            strncmp(_lastKey.c_str(), name.c_str(), name.length()) != 0 || _lastKey[name.length()] != '.')
        {
            return false;
        }
        ...
        if (end == nullptr || end == pt || *end != '.' || val > size || size > val + 1) {
            return illegalArrayKey();
        }
        _lastIdx = val;
...
std::string IndexMetaInfo::makeFileName(const std::string& baseName) {
...
IndexMetaInfo::Snapshot& IndexMetaInfo::getCreateSnapshot(uint32_t idx) {
...
IndexMetaInfo::SnapshotList::iterator IndexMetaInfo::findSnapshot(uint64_t syncToken) {
...
IndexMetaInfo::IndexMetaInfo(const std::string& path) : _path(path), _snapshots() {
}

IndexMetaInfo::~IndexMetaInfo() = default;

IndexMetaInfo::Snapshot IndexMetaInfo::getBestSnapshot() const {
...
IndexMetaInfo::Snapshot IndexMetaInfo::getSnapshot(uint64_t syncToken) const {
...
bool IndexMetaInfo::addSnapshot(const Snapshot& snap) {
...
bool IndexMetaInfo::removeSnapshot(uint64_t syncToken) {
...
bool IndexMetaInfo::validateSnapshot(uint64_t syncToken) {
    SnapItr itr = findSnapshot(syncToken);
    if (itr == _snapshots.end()) {
        return false;
    }
...
bool IndexMetaInfo::invalidateSnapshot(uint64_t syncToken) {
    SnapItr itr = findSnapshot(syncToken);
    if (itr == _snapshots.end()) {
        return false;
    }
...
void IndexMetaInfo::clear() {
    _snapshots.resize(0);
}

bool IndexMetaInfo::load(const std::string& baseName) {
    clear();
...
bool IndexMetaInfo::save(const std::string& baseName) {
