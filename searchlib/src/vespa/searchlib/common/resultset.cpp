...
ResultSet::ResultSet() : _bitOverflow(), _rankedHitsArray(Alloc::alloc(0, MMAP_LIMIT)) {
}

ResultSet::~ResultSet() = default;

void ResultSet::allocArray(unsigned int arrayAllocated) {
    if (arrayAllocated > 0) {
...
void ResultSet::setBitOverflow(BitVector::UP newBitOverflow) {
...
// Find number of hits
//////////////////////////////////////////////////////////////////////
unsigned int ResultSet::getNumHits() const {
    return (_bitOverflow) ? _bitOverflow->countTrueBits() : _rankedHitsArray.size();
}

void ResultSet::mergeWithBitOverflow(HitRank default_value) {
    if (!_bitOverflow) {
        return;
    }
...
void ResultSet::sort(FastS_IResultSorter& sorter, unsigned int ntop) {
    sorter.sortResults(_rankedHitsArray.data(), _rankedHitsArray.size(), ntop);
}

std::pair<std::unique_ptr<BitVector>, vespalib::Array<RankedHit>> ResultSet::copyResult() const {
...
std::pair<std::unique_ptr<BitVector>, vespalib::Array<RankedHit>> ResultSet::stealResult(ResultSet&& rhs) {
