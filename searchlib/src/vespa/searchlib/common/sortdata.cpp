...
uint32_t SortData::GetSize(uint32_t hitcnt, const uint32_t* sortIndex) {
    if (hitcnt == 0)
        return 0;
    return ((hitcnt + 1) * sizeof(uint32_t) + (sortIndex[hitcnt] - sortIndex[0]));
}

bool SortData::Equals(uint32_t hitcnt, const uint32_t* sortIndex_1, const char* sortData_1,
...
void SortData::Copy(uint32_t hitcnt, uint32_t* sortIndex_dst, char* sortData_dst, const uint32_t* sortIndex_src,
...
bool SortDataIterator::Before(SortDataIterator* other, bool beforeOnMatch) {
