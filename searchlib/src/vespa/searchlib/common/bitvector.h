...
public:
    ...
    bool testBit(Index idx) const noexcept { return ((load(_words[wordNum(idx)]) & mask(idx)) != 0); }
    Index getSizeAcquire() const { return vespalib::atomic::load_ref_acquire(_sz); }
    ...
    bool hasTrueBits() const { return isValidCount() ? (countTrueBits() != 0) : hasTrueBitsInternal(); }
    ...
    Index getFirstFalseBit(Index start = 0) const { return getNextFalseBit(std::max(start, getStartIndex())); }
    ...
    Index getPrevTrueBit(Index start) const {
        ...
        return (t != 0) ? (index << numWordBits()) + vespalib::Optimized::msbIdx(t) : getStartIndex();
    }
     ...
     */
    void invalidateCachedCount() const { _numTrueBits.store(invalidCount(), std::memory_order_relaxed); }
     ...
     * @param end limit
     */
    Index countInterval(Index start, Index end) const { return countInterval(Range(start, end)); }
     ...
     */
    template <typename T> void andNotWithT(T it);
     ...
     * Calculate the size of a bitmap when performing file io.
     */
    size_t getFileBytes() const { return getFileBytes(size()); }
     ...
     */
    static std::unique_ptr<const BitVector> create(Index numberOfElements, FastOS_FileInterface& file, int64_t offset,
...
protected:
    ...
    static size_t numActiveBytes(Index start, Index end) noexcept {
        return numActiveWords(start, end) * sizeof(Word);
    }
    static Alloc allocatePaddedAndAligned(Index sz) { return allocatePaddedAndAligned(0, sz); }
...
private:
    ...
    const Word* getWordIndex(Index index) const noexcept {
        return static_cast<const Word*>(getStart()) + wordNum(index);
    }
    ...
    Range sanitize(Range range) const {
        return {std::max(range.start(), getStartIndex()), std::min(range.end(), size())};
    }
    Index count() const;
    ...
    template <typename WordConverter> Index getNextBit(WordConverter conv, Index start) const noexcept {
        Index       index(wordNum(start));
        const Word* words(_words);
    ...
    template <typename FunctionType> static void foreach_bit(FunctionType func, Word word, Index start) {
        while (word) {
...
protected:
    friend vespalib::nbostream& operator<<(vespalib::nbostream& out, const BitVector& bv);
    friend vespalib::nbostream& operator>>(vespalib::nbostream& in, AllocatedBitVector& bv);
};

vespalib::nbostream& operator<<(vespalib::nbostream& out, const BitVector& bv);

vespalib::nbostream& operator>>(vespalib::nbostream& in, AllocatedBitVector& bv);

template <typename T> void BitVector::andNotWithT(T it) {
    while (it.hasNext()) {
        clearBit(it.next());
    }
