...
public:
    ...
    void SetCompressionType(CompressionConfig::Type compressionType) {
        _compressionType.store(compressionType, std::memory_order_relaxed);
    }
...
public:
    ...
    void setName(std::string_view val) { setName(val.data(), val.size()); }
    void setKey(uint32_t entry, const char* key, uint32_t keySize);
    void setKey(uint32_t entry, std::string_view val) { setKey(entry, val.data(), val.size()); }
    void setValue(uint32_t entry, const char* value, uint32_t valueSize);
    void setValue(uint32_t entry, std::string_view val) { setValue(entry, val.data(), val.size()); }
