...
template <typename T, typename GR> unsigned int radix_fetch(T* a, size_t n, uint32_t* radixScratch, GR R) {
    size_t   i = 0;
    uint32_t usedBits = 0;
    if (n > 3) {
        for (; i < n - 3; i += 4) {
...
template <typename T> class AlwaysEof {
public:
    bool operator()(const T&) const { return true; }
    static bool alwaysEofOnCheck() { return true; }
};

template <typename T, typename ER> bool radix_eof(const T* a, size_t n, ER E) __attribute__((noinline));

template <typename T, typename ER> bool radix_eof(const T* a, size_t n, ER E) {
    size_t i = 0;
    bool   eof(true);
    if (n > 3) {
        for (; eof && (i < n - 3); i += 4) {
            eof = E(a[i + 0]) && E(a[i + 1]) && E(a[i + 2]) && E(a[i + 3]);
        }
    }
    for (; eof && (i < n); i++) {
        eof = E(a[i]);
...
void radix_sort(GR R, GE E, GRE EE, int stackDepth, T* a, size_t n, uint32_t* radixScratch, int radixBits,
                unsigned insertSortLevel = 10, size_t topn = std::numeric_limits<size_t>::max()) {
    ...
    for (size_t i(0), sum(0); (i < 256) && (sum < topn); i++) {
        ...
        if (c) {
            if (c > insertSortLevel) {
                radix_sort(R, E, EE, stackDepth + 1, &a[l], c, &radixScratch[l], radixBits, insertSortLevel,
                           topn - sum);
            } else {
...
template <typename GR, typename T, int SHIFT> class ShiftBasedRadixSorterBase {
protected:
...
template <typename GR, typename T, int SHIFT>
void ShiftBasedRadixSorterBase<GR, T, SHIFT>::radix_sort_core(GR R, size_t ptr[256], size_t last[257], T* a,
                                                              size_t n) {
    // Go through all permutation cycles until all
    // elements are moved or found to be already in place
    size_t i(0), remain(n);
...
public:
    static size_t radix_sort(GR R, GE E, T* a, size_t n, unsigned int insertSortLevel = 10,
...
size_t ShiftBasedRadixSorter<T, GR, GE, SHIFT, continueAfterRadixEnds>::radix_sort_internal(
    GR R, GE E, T* a, size_t n, unsigned int insertSortLevel, size_t topn) {
    ...
    if (!sorted) {
        ...
        return ShiftBasedRadixSorter<T, GR, GE, SHIFT - 8, continueAfterRadixEnds>::radix_sort_internal(
            R, E, a, n, insertSortLevel, topn);
    }
    ...
    if (SHIFT > 0 || continueAfterRadixEnds) {
        ...
        for (unsigned i(0); (i < 256) && (sum < topn); i++) {
            ...
            if (c) {
                if (c > insertSortLevel) {
                    sum += ShiftBasedRadixSorter<T, GR, GE, SHIFT - 8, continueAfterRadixEnds>::radix_sort_internal(
                        R, E, &a[l], c, insertSortLevel, topn - sum);
                } else {
                    std::sort(a + l, a + l + c, E);
                    sum += c;
                }
...
template <typename T, typename GR, typename GE, int SHIFT, bool continueAfterRadixEnds>
size_t ShiftBasedRadixSorter<T, GR, GE, SHIFT, continueAfterRadixEnds>::radix_sort(GR R, GE E, T* a, size_t n,
                                                                                   unsigned int insertSortLevel,
                                                                                   size_t       topn) {
...
template <typename A, typename B, typename C> class ShiftBasedRadixSorter<A, B, C, -8, false> {
public:
    static size_t radix_sort_internal(B, C, A*, size_t, unsigned int, size_t) { return 0; }
};

template <typename A, typename B, typename C> class ShiftBasedRadixSorter<A, B, C, -8, true> {
public:
...
template <typename T, bool asc = true> class NumericRadixSorter {
public:
    ...
    void operator()(T* start, size_t sz, size_t topn = std::numeric_limits<size_t>::max()) const {
        if (sz > 16) {
            ShiftBasedRadixSorter<typename C::InputType, RadixSortable, typename C::Compare,
                                  8 * (sizeof(typename C::UIntType) - 1)>::radix_sort_internal(RadixSortable(),
                                                                                               typename C::Compare(),
                                                                                               start, sz, 16, topn);
        } else {
...
template <typename GR, typename T, int IDX> void radix_fetch2(GR R, size_t cnt[256], const T* a, size_t n) {
    memset(cnt, 0, 256 * sizeof(cnt[0]));
    size_t p(0);
    if (n > 3) {
        for (; p < n - 3; p += 4) {
...
template <typename T, typename GR, typename GE, int LEN, int POS>
void radix_sort(GR R, GE E, T* a, size_t n, unsigned int insertSortLevel = 10,
                size_t topn = std::numeric_limits<size_t>::max()) {
    if (n > insertSortLevel) {
