...
private:
    class KeyMeta {
    public:
        KeyMeta() noexcept : _lookupCount(0), _bitCount(0), _chunkId(-1), _chunkIndex(0) {}
        ...
        KeyMeta& bitCount(uint32_t v) {
            _bitCount = v;
            return *this;
        }
        KeyMeta& chunkId(uint32_t v) {
            _chunkId = v;
            return *this;
        }
        KeyMeta& chunkIndex(uint32_t v) {
            _chunkIndex = v;
            return *this;
        }
        KeyMeta& unCache() {
    ...
    VESPA_DLL_LOCAL static void populate(Key2Index& newKeys, CondensedBitVector& chunk,
