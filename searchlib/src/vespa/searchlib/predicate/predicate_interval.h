...
struct Interval {
    ...
    void save(BufferWriter& writer) const { nbo_write<uint32_t>(writer, interval); }
    static Interval deserialize(vespalib::DataBuffer& buffer) { return Interval{buffer.readInt32()}; }
    bool operator==(const Interval& other) const { return interval == other.interval; }
    bool valid() const { return interval != 0; }
};
...
struct IntervalWithBounds {
    ...
    bool valid() const { return interval != 0 && bounds != 0; }
};
