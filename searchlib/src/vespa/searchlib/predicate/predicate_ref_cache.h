 ...
 * cached references.
 */
template <typename BufferStore, int SIZE_BITS> class RefCacheComparator {
    enum {
        DATA_REF_BITS = 32 - SIZE_BITS,
...
public:
    RefCacheComparator(const BufferStore& store, const CurrentZeroRef& zero_ref)
        : _store(store), _current_zero_ref(zero_ref) {}

    void getSizeAndBuf(uint32_t ref, uint32_t& size, const uint32_t*& buf) const {
        if (ref) {
            size = ref >> SIZE_SHIFT;
 ...
 * insert and find)
 */
template <typename BufferStore, int SIZE_BITS = 8> class PredicateRefCache {
...
public:
    ...
    PredicateRefCache(const BufferStore& store) : _ref_cache(ComparatorType(store, _current_zero_ref)) {}
