...
struct SimpleIndexConfig {
    ...
    SimpleIndexConfig() {}
    SimpleIndexConfig(double upper_docid_freq_threshold_, double lower_docid_freq_threshold_,
                      size_t upper_vector_size_threshold_, size_t lower_vector_size_threshold_,
                      size_t vector_prune_frequency_, double foreach_vector_threshold_,
 ...
 */
template <typename Posting, typename Key = uint64_t, typename DocId = uint32_t> class SimpleIndex {
public:
...
private:
    ...
    void logVector(const char* action, Key key, size_t document_count, double ratio, size_t vector_length) const;
...
public:
    SimpleIndex(GenerationHolder& generation_holder, const DocIdLimitProvider& provider,
                const SimpleIndexConfig& config)
        : _generation_holder(generation_holder), _config(config), _limit_provider(provider) {}
    ~SimpleIndex();

    void deserialize(vespalib::DataBuffer& buffer, PostingDeserializer<Posting>& deserializer,
    ...
    DictionaryIterator lookup(Key key) const { return _dictionary.getFrozenView().find(key); }
...
template <typename FunctionType>
void SimpleIndex<Posting, Key, DocId>::foreach_frozen_key(vespalib::datastore::EntryRef ref, Key key,
