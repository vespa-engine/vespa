...
class PredicateTreeAnnotatorImpl {
    ...
    uint32_t makeMarker(uint32_t begin, uint32_t end) { return (begin << 16) | end; }
    uint32_t getCEnd() {
...
public:
    PredicateTreeAnnotatorImpl(const std::map<std::string, int>& size_map, PredicateTreeAnnotations& result,
...
PredicateTreeAnnotatorImpl::PredicateTreeAnnotatorImpl(const map<string, int>&   size_map,
                                                       PredicateTreeAnnotations& result, int64_t lower_bound,
                                                       int64_t upper_bound, uint16_t interval_range)
    : _begin(MIN_INTERVAL),
      _end(interval_range),
      _left_weight(0),
      _result(result),
...
void PredicateTreeAnnotatorImpl::assignIntervalMarkers(const Inspector& in) {
    ...
    case Predicate::TYPE_FEATURE_RANGE: {
        ...
        if (hash_count < 3) { // three features takes more space than one stored range.
            ...
            _result.range_features.push_back({in[Predicate::KEY].asString(), has_min ? in_min.asLong() : _lower_bound,
...
PredicateTreeAnnotations::PredicateTreeAnnotations(uint32_t mf, uint16_t ir) : min_feature(mf), interval_range(ir) {
}

PredicateTreeAnnotations::~PredicateTreeAnnotations() = default;

void PredicateTreeAnnotator::annotate(const Inspector& in, PredicateTreeAnnotations& result, int64_t lower,
                                      int64_t upper) {
