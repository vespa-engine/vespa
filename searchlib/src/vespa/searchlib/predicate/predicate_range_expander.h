...
class PredicateRangeExpander {
    ...
    static void addEdgePartition(const char* label, uint64_t value, bool negative, InsertIt out) {
        std::string to_hash = vespalib::make_string("%s%s%" PRIu64, label, negative ? "=-" : "=", value);
        debugLog("Hashing edge partition %s", to_hash.c_str());
        *out++ = PredicateHash::hash64(to_hash);
    }

    template <typename InsertIt>
    static void addPartitions(const char* label, uint64_t part, uint64_t part_size, uint32_t first, uint32_t last,
        ...
        for (uint32_t i = first; i < last; ++i) {
            ...
            std::string to_hash =
                vespalib::make_string("%s%s%" PRIu64 "-%" PRIu64, label, negative ? "=-" : "=", from, to);
    ...
    template <typename InsertIt>
    static void makePartitions(const char* label, uint64_t from, uint64_t to, uint64_t step_size, int32_t arity,
                               bool negative, InsertIt out) {
        ...
        if (next_from == next_to) {
            addPartitions(label, next_from, step_size, from_remainder, to_remainder, negative, out);
        } else {
            if (from_remainder > 0) {
                addPartitions(label, next_from, step_size, from_remainder, arity, negative, out);
                from = next_from + arity;
            }
            addPartitions(label, next_to, step_size, 0, to_remainder, negative, out);
            makePartitions(label, from / arity, to / arity, step_size * arity, arity, negative, out);
        }
    }

    template <typename InsertIt>
    static void partitionRange(const char* label, uint64_t from, uint64_t to, uint32_t arity, bool negative,
                               InsertIt out) {
        uint32_t from_remainder = from % arity;
        ...
        makePartitions(label, from_val / arity, (to_val - (arity - 1)) / arity + 1, arity, arity, negative, out);
    }

public:
    // Expands a range and returns the hash values through the insert iterator.
    template <typename InsertIt>
    static void expandRange(const char* label, int64_t from, int64_t to, uint32_t arity, InsertIt out) {
        if (from < 0) {
            if (to < 0) {
                // Special case for to==-1. -X-0 means the same as -X-1,
                // but is more efficient.
                partitionRange(label, (to == -1 ? 0 : -to), uint64_t(0) - from, arity, true, out);
            } else {
