...
PredicateIntervalStore::PredicateIntervalStore()
    : _store(), _size1Type(1, 1024u, RefType::offsetSize()), _store_adapter(_store), _ref_cache(_store_adapter) {
...
// anyway.
//
template <typename IntervalT> EntryRef PredicateIntervalStore::insert(const vector<IntervalT>& intervals) {
...
template EntryRef PredicateIntervalStore::insert(const vector<Interval>&);
template EntryRef PredicateIntervalStore::insert(const vector<IntervalWithBounds>&);
...
void PredicateIntervalStore::reclaim_memory(Generation oldest_used_gen) {
    _store.reclaim_memory(oldest_used_gen);
}

void PredicateIntervalStore::assign_generation(Generation current_gen) {
