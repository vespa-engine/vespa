...
void SimpleIndex<Posting, Key, DocId>::insertIntoPosting(vespalib::datastore::EntryRef& ref, Key key, DocId doc_id,
...
template <typename Posting, typename Key, typename DocId> SimpleIndex<Posting, Key, DocId>::~SimpleIndex() {
...
template <typename Posting, typename Key, typename DocId>
void SimpleIndex<Posting, Key, DocId>::deserialize(vespalib::DataBuffer&                       buffer,
                                                   PostingDeserializer<Posting>&               deserializer,
                                                   SimpleIndexDeserializeObserver<Key, DocId>& observer,
                                                   uint32_t                                    version) {
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        _btree_posting_lists.apply(ref, postings.data(), postings.data() + postings.size(), 0, 0);
...
template <typename Posting, typename Key, typename DocId>
void SimpleIndex<Posting, Key, DocId>::addPosting(Key key, DocId doc_id, const Posting& posting) {
...
template <typename Posting, typename Key, typename DocId>
std::pair<Posting, bool> SimpleIndex<Posting, Key, DocId>::removeFromPostingList(Key key, DocId doc_id) {
...
template <typename Posting, typename Key, typename DocId>
void SimpleIndex<Posting, Key, DocId>::removeFromVectorPostingList(vespalib::datastore::EntryRef ref, Key key,
                                                                   DocId doc_id) {
    auto it = _vector_posting_lists.find(key);
    if (it.valid()) {
...
template <typename Posting, typename Key, typename DocId>
bool SimpleIndex<Posting, Key, DocId>::removeVectorIfBelowThreshold(vespalib::datastore::EntryRef   ref,
                                                                    typename VectorStore::Iterator& it) {
    size_t size = getDocumentCount(ref);
...
template <typename Posting, typename Key, typename DocId>
size_t SimpleIndex<Posting, Key, DocId>::get_frozen_document_count(vespalib::datastore::EntryRef ref) const {
...
template <typename Posting, typename Key, typename DocId> void SimpleIndex<Posting, Key, DocId>::commit() {
    _dictionary.getAllocator().freeze();
    _btree_posting_lists.freeze();
    _vector_posting_lists.getAllocator().freeze();
}
...
template <typename Posting, typename Key, typename DocId>
void SimpleIndex<Posting, Key, DocId>::reclaim_memory(vespalib::Generation oldest_used_gen) {
...
template <typename Posting, typename Key, typename DocId>
void SimpleIndex<Posting, Key, DocId>::assign_generation(vespalib::Generation current_gen) {
...
SimpleIndex<Posting, Key, DocId>::make_saver(std::unique_ptr<PostingSaver<Posting>> subsaver) const {
    return std::make_unique<SimpleIndexSaver<Posting, Key, DocId>>(_dictionary.getFrozenView(), _btree_posting_lists,
                                                                   std::move(subsaver));
}
