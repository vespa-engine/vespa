...
bool runProc(ChildProc &proc, std::atomic<bool> &done) {
    char buf[4096];
...
TEST("usage") {
    std::atomic<bool> done(false);
    {
        ChildProc proc("exec ../../examples/proxy/fnet_proxy_app");
        EXPECT_FALSE(runProc(proc, done));
    }
    {
        ChildProc proc("exec ../../examples/ping/fnet_pingserver_app");
        EXPECT_FALSE(runProc(proc, done));
    }
    {
        ChildProc proc("exec ../../examples/ping/fnet_pingclient_app");
        EXPECT_FALSE(runProc(proc, done));
    }
    {
        ChildProc proc("exec ../../examples/frt/rpc/fnet_rpc_client_app");
        EXPECT_FALSE(runProc(proc, done));
    }
    {
        ChildProc proc("exec ../../examples/frt/rpc/fnet_rpc_server_app");
        EXPECT_FALSE(runProc(proc, done));
    }
    {
        ChildProc proc("exec ../../examples/frt/rpc/fnet_echo_client_app");
        EXPECT_FALSE(runProc(proc, done));
    }
    {
        ChildProc proc("exec ../../examples/frt/rpc/vespa-rpc-info");
        EXPECT_FALSE(runProc(proc, done));
    }
    {
        ChildProc proc("exec ../../examples/frt/rpc/vespa-rpc-invoke-bin");
        EXPECT_FALSE(runProc(proc, done));
    }
    {
        ChildProc proc("exec ../../examples/frt/rpc/fnet_rpc_callback_server_app");
        EXPECT_FALSE(runProc(proc, done));
    }
    {
        ChildProc proc("exec ../../examples/frt/rpc/fnet_rpc_callback_client_app");
        EXPECT_FALSE(runProc(proc, done));
    }
    {
        ChildProc proc("exec ../../examples/frt/rpc/vespa-rpc-proxy");
        EXPECT_FALSE(runProc(proc, done));
    }
}
...
TEST("timeout") {
    std::string out;
    EXPECT_TRUE(ChildProc::run("exec ../../examples/timeout/fnet_timeout_app", out));
    fprintf(stderr, "%s\n", out.c_str());
}

TEST_MT_F("ping", 2, std::atomic<bool>()) {
    if (thread_id == 0) {
        ChildProc proc(vespalib::make_string("exec ../../examples/ping/fnet_pingserver_app tcp/%d",
                                             PORT0).c_str());
        TEST_BARRIER();
