...
bool FNET_ControlPacket::IsRegularPacket() {
    return false;
}

bool FNET_ControlPacket::IsControlPacket() {
    return true;
}

uint32_t FNET_ControlPacket::GetCommand() {
    return _command;
}

bool FNET_ControlPacket::IsChannelLostCMD() {
    return _command == FNET_CMD_CHANNEL_LOST;
}

bool FNET_ControlPacket::IsTimeoutCMD() {
    return _command == FNET_CMD_TIMEOUT;
}

bool FNET_ControlPacket::IsBadPacketCMD() {
    return _command == FNET_CMD_BAD_PACKET;
}

uint32_t FNET_ControlPacket::GetPCODE() {
    return FNET_NOID;
}

uint32_t FNET_ControlPacket::GetLength() {
    return 0;
}

void FNET_ControlPacket::Encode(FNET_DataBuffer*) {
    LOG_ABORT("should not be reached");
}

bool FNET_ControlPacket::Decode(FNET_DataBuffer*, uint32_t) {
    LOG_ABORT("should not be reached");
}
