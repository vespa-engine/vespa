...
# `uv run train_lightgbm_cat_integers.py`
...
# Define output paths
MODEL_OUTPUT_PATH = Path("cat_integers.json")
TESTCASE_OUTPUT_PATH = (
    Path(__file__).parent.parent / "testcases" / "lightgbm" / "cat_integers_tests.json"
)
...
df = pd.DataFrame(
    {
        ...
        # Adding more cat_integers df, with some having null values
        "feature_4": pd.Series(
        ...
        # Use integer-based cat_integers df
        "feature_6": pd.Series(
