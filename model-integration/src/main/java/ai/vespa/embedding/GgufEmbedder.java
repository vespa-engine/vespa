...
public class GgufEmbedder extends AbstractComponent implements Embedder {
    ...
    public GgufEmbedder(GgufEmbedderConfig config, ModelPathHelper helper) {
        log.fine(() -> String.format(Locale.ROOT, "Config: %s", config));
    ...
    public Tensor embed(String text, Context context, TensorType tensorType) {
        ...
        if (tensorType.dimensions().size() != 1) {
            throw new IllegalArgumentException(
                    String.format(Locale.ROOT, "Error in embedding to type '%s': should only have one dimension.", tensorType));
        }
        var dimension = tensorType.dimensions().get(0);
        if (!dimension.isIndexed()) {
            throw new IllegalArgumentException(
                    String.format(Locale.ROOT, "Error in embedding to type '%s': dimension should be indexed.", tensorType));
        }
        var dimensionSize = dimension.size().orElseThrow();
        if (rawEmbedding.length != dimensionSize) {
            throw new IllegalArgumentException(
                    String.format(Locale.ROOT, "Error in embedding to type '%s': expected dimension size %d, but got %d.", tensorType, dimensionSize, rawEmbedding.length));
        }
    ...
    private String prependAndTruncatePrompt(String text, Context context) {
        ...
        log.fine(() -> String.format(Locale.ROOT, "Truncating prompt from %d to %d tokens", tokens.length, maxTruncatedLength));
    ...
    private float[] generateRawEmbedding(String prompt) {
        try {
            ...
            if (cause.getClass().getName().endsWith("de.kherud.llama.LlamaException") // Package-private exception
                ...
                throw new IllegalArgumentException(
                        String.format(Locale.ROOT, "Input text is too large (prompt UTF-16 length: %d). Either set max prompt tokens or adjust batch/context size.", prompt.length()),
                        cause);
            }
            throw e;
        }
