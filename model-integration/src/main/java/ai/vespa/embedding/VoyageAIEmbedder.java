...
public class VoyageAIEmbedder extends AbstractComponent implements Embedder {
    ...
    public VoyageAIEmbedder(VoyageAiEmbedderConfig config, Embedder.Runtime runtime, Secrets secretStore) {
        ...
        log.fine(() -> String.format(Locale.ROOT, "VoyageAI embedder initialized with model: %s, endpoint: %s", config.model(), resolvedEndpoint));
    }
        ...
        switch (config.quantization()) {
            case AUTO:
                if (valueType == TensorType.Value.FLOAT) {
                    if (tensorDim != configuredDim)
                        throw new IllegalArgumentException(String.format(Locale.ROOT, "Tensor dimension %d does not match configured dimension %d.", tensorDim, configuredDim));
                } else if (valueType == TensorType.Value.INT8) {
                    if (tensorDim != configuredDim && tensorDim != configuredDim / 8)
                        throw new IllegalArgumentException(String.format(Locale.ROOT, "Tensor dimension %d does not match configured dimension. Expected %d or %d.", tensorDim, configuredDim, configuredDim / 8));
                } else {
                    throw new IllegalArgumentException(
            ...
            case BINARY:
                ...
                if (tensorDim != configuredDim / 8)
                    throw new IllegalArgumentException(String.format(Locale.ROOT, "Tensor dimension %d does not match required dimension %d.", tensorDim, configuredDim / 8));
                break;
            default:
    ...
    private static class RetryInterceptor implements Interceptor {
        ...
        public okhttp3.Response intercept(Chain chain) throws IOException {
            ...
            for (int attempt = 0; attempt <= maxRetries; attempt++) {
                try {
                    ...
                    if (attempt < maxRetries) {
                        ...
                        log.fine(() -> String.format(Locale.ROOT, "VoyageAI API server error (%d). Retry %d of %d after %dms", code, retryNumber + 1, maxRetries, RETRY_DELAY_MS));
            ...
            var errorMsg = String.format(Locale.ROOT, "Max retries exceeded for VoyageAI API (%d). Last response: %d - %s", maxRetries, lastStatusCode, lastResponseBody);
