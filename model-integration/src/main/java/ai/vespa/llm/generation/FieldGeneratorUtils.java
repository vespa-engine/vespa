...
class FieldGeneratorUtils {
    ...
    public static String generateJsonSchemaForField(String fieldPath, DataType fieldType) {
        ...
        try {
            ...
            throw new IllegalArgumentException(Text.format("Failed to generate schema for field %s of type %s", fieldPath, fieldType.getName()), e);
        }
    ...
    private static Json.Builder.Object generateJsonSchemaForFieldValue(DataType fieldType) {
        ...
        if (fieldType.equals(DataType.BOOL)) {
            ...
            throw new IllegalArgumentException(Text.format("Failed to generate schema for field type %s", fieldType));
        }
    ...
    public static FieldValue parseJsonFieldValue(String jsonFieldValue, String fieldPath, DataType fieldType) {
        ...
        var documentId = Text.format("id:generate:%s::0", documentTypeName);
        var jsonDocumentOperation = Text.format("""
                {
                    "put": "%s",
                    "fields": %s
                }
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    Text.format("Failed to parse JSON value for field %s of type %s: %s", fieldPath, fieldType.getName(), jsonFieldValue), e);
        }
