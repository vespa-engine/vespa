...
public class StateChangeHandler {
            ...
            for (ConfiguredNode configuredNode : nodes) {
                ...
                if (nodeInfo != null && nodeState != null) {
                    if (nodeState.getStartTimestamp() > nodeInfo.getStartTimestamp()) {
                        log.log(FINE, () -> Text.format("Storing away new start timestamp for node %s (%d)", node, nodeState.getStartTimestamp()));
    ...
    public void handleNewReportedNodeState(ClusterState currentClusterState,
        ...
        log.log(level, () -> Text.format("Got nodestate reply from %s: %s (Current state is %s)",
        ...
        if (reportedState.getMinUsedBits() != currentState.getMinUsedBits()) {
            ...
            log.log(FINE,
                    () -> Text.format("Altering node state to reflect that min distribution bit count has changed from %d to %d", oldCount, newCount));
            eventLog.add(NodeEvent.forBaseline(node, Text.format("Altered min distribution bit count from %d to %d", oldCount, newCount),
                         NodeEvent.Type.CURRENT, currentTime), isMaster);
        } else {
            log.log(FINE, () -> Text.format("Not altering state of %s in cluster state because new state is too similar: %s",
    ...
    public void proposeNewNodeState(ClusterState currentClusterState, NodeInfo node, NodeState proposedState) {
        ...
        log.log(FINE, () -> Text.format("Got new wanted nodestate for %s: %s", node, currentState.getTextualDifference(proposedState)));
        ...
        if (proposedState.above(currentReported)) {
            eventLog.add(NodeEvent.forBaseline(node, Text.format("Wanted state %s, but we cannot force node into that " +
        ...
        if ( ! proposedState.similarTo(currentState)) {
            eventLog.add(NodeEvent.forBaseline(node, Text.format("Node state set to %s.", proposedState),
    ...
    private boolean handleTimeDependentOpsForNode(ClusterState currentClusterState,
        ...
        if (nodeInitProgressHasTimedOut(currentTime, node, currentStateInSystem, lastReportedState)) {
            eventLog.add(NodeEvent.forBaseline(node, Text.format("%d milliseconds without initialize progress. Marking node down. " +
                                                    ...
                                                    NodeState lastReportedState)
    {
        ...
        {
            final String desc = Text.format("Set node down as it has been missing for %d ms which " +
    ...
    private void updateNodeInfoFromReportedState(final NodeInfo node,
        ...
        log.log(FINE, () -> Text.format("Finding new cluster state entry for %s switching state %s", node, currentState.getTextualDifference(reportedState)));
    ...
    private void markNodeUnstableIfDownEdgeDuringInit(final NodeInfo node,
        ...
        {
            eventLog.add(NodeEvent.forBaseline(node, Text.format("Stop or crash during initialization. " +
    ...
    private boolean handleImplicitCrashEdgeFromReverseInitProgress(final NodeInfo node,
        ...
        {
            eventLog.add(NodeEvent.forBaseline(node, Text.format("Stop or crash during initialization detected from reverse initializing progress." +
        ...
        if (nodeUpToDownEdge(node, currentState, reportedState)) {
            ...
            if (node.getUpStableStateTime() + stableStateTimePeriod > timeNow && isNotControlledShutdown(reportedState)) {
                ...
                eventLog.add(NodeEvent.forBaseline(node,
                        Text.format("Stopped or possibly crashed after %d ms, which is before " +
