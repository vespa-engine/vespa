...
public:
     ...
     */
    enum Punctuation { PUNCTUATION_DISCARD = 0, PUNCTUATION_FULL, PUNCTUATION_SMART, PUNCTUATION_WHITESPACEONLY };
...
public:
    WordCharTokenizer(Punctuation punct = PUNCTUATION_DISCARD, const std::string &punct_token = ".")
        : _tokens(), _current(0), _punctuation(punct), _punctuation_token(punct_token), _lowercase(true) {}
