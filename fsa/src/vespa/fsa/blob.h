...
public:
     ...
     */
    Blob(const void* data, unsigned int size) : _size(size), _data(malloc(size)) { memcpy(_data, data, _size); }
     ...
     */
    Blob(const Blob& b) : _size(b._size), _data(malloc(_size)) { memcpy(_data, b._data, _size); }
     ...
     */
    Blob(const std::string& s) : _size(s.size()), _data(malloc(_size)) { s.copy((char*)_data, _size); }
