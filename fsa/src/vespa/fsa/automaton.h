...
private:
    ...
    public:
         ...
         */
        bool operator<(const TListPtr& tp) const { return (*_ptr < *tp._ptr); }
    ...
    public:
        ...
        State* lastChild() {
            ...
            if (t != nullptr && t->_symbol != FSA::FINAL_SYMBOL) {
                return t->_state;
            }
            return nullptr;
        }
    ...
    public:
         ...
         */
        PackedAutomaton()
            : _packable(false), _pack_map(), _blob_map(), _packed_ptr(nullptr), _packed_idx(nullptr), _symbol(nullptr),
              _used(nullptr), _perf_hash(nullptr), _totals(nullptr), _packed_size(0), _last_packed(0), _blob(nullptr),
              _blob_size(0), _blob_used(0), _blob_type(0), _fixed_blob_size(0), _start_state(0) {}
...
public:
     ...
     * @brief Default constructor.
     */
    Automaton() : _register(), _blob_register(), _q0(nullptr), _previous_input(), _finalized(false), _packed() {}
     ...
     * @return True on success.
     */
    bool write(const std::string& file, uint32_t serial = 0) { return write(file.c_str(), serial); }
