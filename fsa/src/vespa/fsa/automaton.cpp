...
bool Automaton::PackedAutomaton::packStartState(const Automaton::State* s) { return packState(s, true); }
        ...
        if (s->getTransitionList().size() == 0) {
            ...
            for (i = 0; i < s->getTransitionList().size(); i++) {
                if (s->getTransitionList()[i]._symbol == FSA::FINAL_SYMBOL) {
                    _packed_idx[cell + FSA::FINAL_SYMBOL] = packBlob(s->getTransitionList()[i]._state->getBlob());
                } else {
                    _packed_ptr[cell + s->getTransitionList()[i]._symbol] = s->getTransitionList()[i]._state;
                }
            }
        }
...
void Automaton::insertSortedString(const std::string& input) { insertSortedString(input.c_str()); }
