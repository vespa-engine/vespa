...
double Vectorizer::TfIdf::weight(unsigned int tfnorm, unsigned int idfnorm, double tfexp, double idfexp) const {
    double tf_n, idf_n;
...
void Vectorizer::vectorize(const NGram& text, TermVector& vector, unsigned int limit, bool keephits, double tfexp,
    ...
    for (rvi = raw_vect.begin(); rvi != raw_vect.end(); ++rvi) {
        vector.push_back(
            VectorItem(rvi->first, rvi->second.first.weight(tfmax, _idf_docs, tfexp, idfexp), rvi->second.second));
    }
