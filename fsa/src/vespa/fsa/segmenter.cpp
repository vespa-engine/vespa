...
Segmenter::Segments::Segments()
    : _text(), _segments(), _map(), _segmentation(Segmenter::SEGMENTATION_METHODS, nullptr) {}

Segmenter::Segments::~Segments() { clear(); }

void Segmenter::Segments::clear() {
    _segments.clear();
...
void Segmenter::Segments::buildSegmentation(Segmenter::SegmentationMethod method) {
    ...
    case SEGMENTATION_LEFTMOST_WEIGHTED:
        ...
        while (pos < n_txt) {
            ...
            for (i = pos + 1; i <= n_txt; i++) {
                id = _map.get(pos, i);
                if (id >= 0 && (method == SEGMENTATION_LEFTMOST_LONGEST ||
    ...
    case SEGMENTATION_RIGHTMOST_WEIGHTED:
        ...
        while (pos > 0) {
            ...
            for (i = pos - 1; i >= 0; i--) {
                id = _map.get(i, pos);
                if (id >= 0 && (method == SEGMENTATION_RIGHTMOST_LONGEST ||
                                (temp = (_segments[id].len() > 1) ? (int)_segments[id].conn() : 0) > bestval)) {
                    bestid = id;
                    bestval = temp;
                    next = i;
    ...
    for (i = 0; i < (int)_segments.size(); i++) {
        if (beg <= _segments[i].beg() && end >= _segments[i].end()) {
            ...
            case SEGMENTATION_WEIGHTED_LONGEST:
                ...
                if (temp > bestval1 || (temp == bestval1 && (int)_segments[i].len() > bestval2)) {
                    bestid = i;
                    bestval1 = temp;
            ...
            case SEGMENTATION_WEIGHTED_LEFTMOST:
                temp = (_segments[i].len() > 1) ? (int)_segments[i].conn() : 0;
                if (temp > bestval1 || (temp == bestval1 && (int)_segments[i].beg() < bestval2)) {
                    bestid = i;
                    bestval1 = temp;
                    bestval2 = _segments[i].beg();
                }
            ...
            case SEGMENTATION_WEIGHTED_RIGHTMOST:
                ...
                if (temp > bestval1 || (temp == bestval1 && (int)_segments[i].end() > bestval2)) {
                    bestid = i;
                    bestval1 = temp;
                    bestval2 = _segments[i].end();
                }
