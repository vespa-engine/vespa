...
uint32_t FSA::libVER() { return VER; }
...
void FSA::reset() {
    ...
    _data = nullptr;
    _data_size = 0;
    _data_type = DATA_VARIABLE;
    _fixed_data_size = 0;
    _has_perfect_hash = false;
    _perf_hash = nullptr;
    _start = 0;
}
...
bool FSA::read(const char* file, FileAccessMethod fam) {
    ...
    if (fam == FILE_ACCESS_MMAP || fam == FILE_ACCESS_MMAP_WITH_MLOCK) {
        _mmap_length = sizeof(header) + _size * sizeof(symbol_t) + _size * sizeof(state_t) + _data_size +
