...
bool ConceptNet::read(const char* datafile, FileAccessMethod fam) {
    ...
    if (fam == FILE_ACCESS_MMAP || fam == FILE_ACCESS_MMAP_WITH_MLOCK) {
        _mmap_length = sizeof(header) + _index_size * sizeof(UnitData) + _info_size * sizeof(uint32_t) +
                       _catindex_size * sizeof(uint32_t) + _strings_size * sizeof(char);
