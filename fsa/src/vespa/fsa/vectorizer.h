...
public:
    ...
    public:
         ...
         */
        double weight(unsigned int tfnorm = 1, unsigned int idfnorm = 1, double tfexp = 1.0, double idfexp = 1.0) const;
         ...
         * @return Weight based on Tf and Idf values.
         */
        double getWeight(unsigned int tfnorm = 1, unsigned int idfnorm = 1, double tfexp = 1.0,
                         double idfexp = 1.0) const {
    ...
    public:
        ...
        void add(const NGram& text, unsigned int from, int length, const FSA::State& state) override {
            ...
            if (pos == _item_map.end()) {
                pos = _item_map
                          .insert(ItemMap::value_type(
                              str, std::pair<TfIdf, VectorItem::Hits>(TfIdf(1, state.nData()), VectorItem::Hits())))
                          .first;
            } else {
                ++(pos->second.first);
            }
...
public:
     ...
     * @param dict FSA
     */
    Vectorizer(const FSA& dict) : _dictionary(dict), _detector(_dictionary), _idf_docs(0) { initIdfCount(); }
     ...
     */
    void vectorize(const NGram& text, TermVector& vector, unsigned int limit, bool keephits, double tfexp = 1.0,
                   double idfexp = 1.0) const;
     ...
     * @param idfexp Exponent for idf (inverse document frequency).
     */
    void vectorize(const NGram& text, TermVector& vector, unsigned int limit = 15, double tfexp = 1.0,
                   double idfexp = 1.0) const;
};
