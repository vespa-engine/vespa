...
int main(int, char **) {
    ...
    if (fs1->isFinal() && fs2->isFinal()) {
        ...
        printf("    \"pe\"+\"a\":    hash=%d, data_size=%d, data string=\"%-*.*s\"\n", fs1->hash(), fs1->dataSize(),
               fs1->dataSize(), fs1->dataSize(), fs1->data());
        printf("    \"pe\"+\"ach\":  hash=%d, data_size=%d, data string=\"%-*.*s\"\n", fs2->hash(), fs2->dataSize(),
               fs2->dataSize(), fs2->dataSize(), fs2->data());
        printf("    \"pe\"+\"ar\":   hash=%d, data_size=%d, data string=\"%-*.*s\"\n", fs3->hash(), fs3->dataSize(),
               fs3->dataSize(), fs3->dataSize(), fs3->data());
