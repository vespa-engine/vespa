...
private:
    ...
    public:
         ...
         */
        TransitionList() : _trans(nullptr), _size(0) {};
         ...
         */
        TransitionList(const TransitionList& tl) : _trans(nullptr), _size(tl._size) {
            if (_size > 0) {
     ...
     */
    struct TListPtrLess {
        inline bool operator()(const TransitionList* const& x, const TransitionList* const& y) const {
            return *x < *y;
        }
    };
    // }}}
    // {{{ Special allocator for Register that will make it possible to completely reclaim its memory when we are done
    // with it
    template <typename _Tp> class MMapArenaAllocator {
    ...
    public:
        ...
        template <typename _Tp1> MMapArenaAllocator(const MMapArenaAllocator<_Tp1>&) noexcept : _chunks(), _size(0) {}
        ...
        pointer allocate(size_type __n, const void* = 0) {
            ...
            if (__n) {
                ...
                if (_chunks.size() == 0 || _CAPACITY - (_size * sizeof(_Tp)) < __b) { // need new chunk
                    __ret = static_cast<_Tp*>(::mmap(0, _CAPACITY, PROT_READ | PROT_WRITE,
                                                     MAP_PRIVATE | MAP_ANONYMOUS, /*fd=*/0, /*offset=*/0));
                    if (__ret == MAP_FAILED)
                        throw std::bad_alloc();
        ...
        size_type max_size() const noexcept { return _CAPACITY / sizeof(_Tp); }
    ...
    struct StateCellArrayItem {
        State*   state;
        uint32_t cell;
        StateCellArrayItem() : state(nullptr), cell(0) {}
        StateCellArrayItem(State* s) : state(s), cell(0) {}
    };
    struct StateCellArrayLess {
        bool operator()(const StateCellArrayItem& x, const StateCellArrayItem& y) { return x.state < y.state; }
    };

    /**
     * @brief Register of states, maps a transition list to a state object
     */
    using Register = std::map<const TransitionList*, State*, TListPtrLess,
                              MMapArenaAllocator<std::pair<const TransitionList*, State*>>>;
    /**
     * @brief State register iterator.
     */
    using RegisterIterator = std::map<const TransitionList*, State*, TListPtrLess,
                                      MMapArenaAllocator<std::pair<const TransitionList*, State*>>>::iterator;
...
public:
     ...
     * @brief Default constructor.
     */
    Automaton() : _register(nullptr), _blob_register(), _q0(nullptr), _queue(nullptr), _finalized(false), _packed() {}
...
inline bool operator==(const Automaton::MMapArenaAllocator<_Tp>&, const Automaton::MMapArenaAllocator<_Tp>&) {
    return true;
}
