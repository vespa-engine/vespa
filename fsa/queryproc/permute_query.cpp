...
int main(int argc, char **argv) {
    ...
    if (verbose) {
        ...
        std::cerr << std::endl;
        std::cerr << "  Empty or single term:  " << std::setw(12) << stats[0] << "   " << std::setw(7)
                  << double(stats[0]) * 100.0 / total << "%" << std::endl;
        std::cerr << "  Too long:              " << std::setw(12) << stats[1] << "   " << std::setw(7)
                  << double(stats[1]) * 100.0 / total << "%" << std::endl;
        for (i = 2; i <= MAXQUERY; i++) {
            std::cerr << "  Length " << std::setw(2) << i << " (grams " << std::setw(3)
                      << gram_count(i < MAXGRAM ? i : MAXGRAM, i) << "): " << std::setw(12) << stats[i] << "   "
                      << std::setw(7) << double(stats[i]) * 100.0 / total << "%" << std::endl;
            ctotal += stats[i] * gram_count(i < MAXGRAM ? i : MAXGRAM, i);
        }
        std::cerr << "  Total:                 " << std::setw(12) << std::setprecision(0) << total << std::endl;
        std::cerr << std::endl;
        std::cerr << "Average number of grams per query: " << std::setprecision(2) << ctotal / total << std::endl;
        std::cerr << std::endl;
    }
