...
DocumentSummary::DocumentSummary() : _summaryBuffer(), _summary(), _summarySize(0) {
    _summaryBuffer.reset(new vespalib::MallocPtr(4_Ki));
}

DocumentSummary::DocumentSummary(document::ByteBuffer& buf) : _summaryBuffer(), _summary(), _summarySize(0) {
    deserialize(buf);
}
