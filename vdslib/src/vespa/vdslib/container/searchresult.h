...
public:
    ...
    void getSortBlob(size_t index, const void*& blob, size_t& sz) const {
        _sortBlob.getBlob(_hits[index].getIndex(), blob, sz);
    }
    ...
    void setRank(size_t hitNo, RankType rank) { _hits[hitNo].setRank(rank); }
...
private:
    ...
    public:
        ...
        Hit(uint32_t lid, RankType rank, size_t docIdOffset, size_t index)
            : _lid(lid), _rank(rank), _docIdOffset(docIdOffset), _index(index) {}
