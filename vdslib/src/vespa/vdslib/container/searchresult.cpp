...
BlobContainer::BlobContainer(size_t reserve) : _blob(reserve), _offsets() {
...
SearchResult::SearchResult() : _totalHits(0), _wantedHits(10), _hits(), _docIdBuffer(), _numDocIdBytes(0), _errors() {
...
uint32_t SearchResult::getSerializedSize() const {
    uint32_t hitCount = std::min(_hits.size(), _wantedHits);
    uint32_t extension_flags = calc_extension_flags(hitCount);
    uint32_t extension_flags_overhead =
        must_serialize_extension_flags(extension_flags, hitCount) ? (2 * sizeof(uint32_t)) : 0;
    uint32_t match_features_size =
        has_match_features(extension_flags) ? get_match_features_serialized_size(hitCount) : 0;
    uint32_t errors_size = has_errors(extension_flags) ? get_errors_serialized_size() : 0;
    return _aggregatorList.getSerializedSize() + _groupingList.getSerializedSize() + _sortBlob.getSerializedSize() +
           extension_flags_overhead + match_features_size + errors_size +
           ((hitCount > 0) ? ((4 * 3) + getBufCount() + sizeof(RankType) * hitCount) : 8);
}

uint32_t SearchResult::calc_extension_flags(uint32_t hit_count) const noexcept {
    uint32_t extension_flags = 0u;
...
uint32_t SearchResult::get_match_features_serialized_size(uint32_t hit_count) const noexcept {
...
void SearchResult::serialize_match_features(vespalib::GrowableByteBuffer& buf, uint32_t hit_count) const {
...
void SearchResult::deserialize_match_features(document::ByteBuffer& buf) {
    int32_t           tmp(0);
    double            dtmp(0.0);
    uint8_t           selector(0);
...
uint32_t SearchResult::get_errors_serialized_size() const noexcept {
    uint32_t size = sizeof(uint32_t);
...
void SearchResult::serialize_errors(vespalib::GrowableByteBuffer& buf) const {
...
void SearchResult::deserialize_errors(document::ByteBuffer& buf) {
    int32_t           tmp(0);
    std::vector<char> scratch;
    buf.getIntNetwork(tmp);
...
void SearchResult::set_match_features(FeatureValues&& match_features) {
    _match_features = std::move(match_features);
}

void SearchResult::set_errors(std::vector<std::string>&& errors) {
