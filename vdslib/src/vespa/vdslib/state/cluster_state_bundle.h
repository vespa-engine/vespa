...
public:
    ...
    public:
        FeedBlock(bool block_feed_in_cluster_in, const std::string& description_in);
    ...
    using BucketSpaceStateMapping =
        std::unordered_map<document::BucketSpace, std::shared_ptr<const ClusterState>, document::BucketSpace::hash>;
...
public:
    ...
    ClusterStateBundle(const ClusterState& baselineClusterState, BucketSpaceStateMapping derivedBucketSpaceStates);
    ClusterStateBundle(const ClusterState& baselineClusterState, BucketSpaceStateMapping derivedBucketSpaceStates,
    ...
    ClusterStateBundle(std::shared_ptr<const ClusterState> baseline_cluster_state,
                       BucketSpaceStateMapping derived_bucket_space_states, std::optional<FeedBlock> feed_block_in,
    ...
    [[nodiscard]] std::shared_ptr<const ClusterStateBundle>
        clone_with_new_distribution(std::shared_ptr<const DistributionConfigBundle>) const;
    ...
    const BucketSpaceStateMapping& getDerivedClusterStates() const noexcept { return _derivedBucketSpaceStates; }
    [[nodiscard]] bool block_feed_in_cluster() const noexcept {
        return _feed_block.has_value() && _feed_block->block_feed_in_cluster();
    }
    [[nodiscard]] bool has_distribution_config() const noexcept { return static_cast<bool>(_distribution_bundle); }
