...
private:
     ...
     */
    void calculateDistributionHashValues() noexcept { calculateDistributionHashValues(0x8badf00d); }

public:
    // Create leaf node
    Group(uint16_t index, std::string_view name) noexcept;
    // Create branch node
    Group(uint16_t index, std::string_view name, const Distribution&, uint16_t redundancy);
    ~Group() override;
    ...
    [[nodiscard]] bool redundancy_value_within_bounds(size_t idx) const noexcept {
        return idx < _preCalculated.size();
    }
