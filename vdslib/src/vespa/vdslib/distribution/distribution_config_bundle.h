...
public:
    ...
    explicit DistributionConfigBundle(
        std::unique_ptr<const Distribution::DistributionConfig> config); // TODO shared_ptr?
    ...
    [[nodiscard]] std::shared_ptr<const Distribution>
    bucket_space_distribution_or_nullptr(document::BucketSpace space) const noexcept {
        return _bucket_space_distributions.get_or_nullptr(space);
    }
    [[nodiscard]] const Distribution*
    bucket_space_distribution_or_nullptr_raw(document::BucketSpace space) const noexcept {
    ...
    bool operator!=(const DistributionConfigBundle& rhs) const noexcept { return !(*this == rhs); }
    ...
    [[nodiscard]] static std::shared_ptr<DistributionConfigBundle>
    of(std::unique_ptr<const Distribution::DistributionConfig> cfg);
};
