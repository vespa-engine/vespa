...
public:
     ...
     */
    RedundancyGroupDistribution(const RedundancyGroupDistribution& spec, uint16_t redundancy);
    ...
    bool operator==(const RedundancyGroupDistribution& o) const { return (_values == o._values); }

    void print(std::ostream&, bool verbose, const std::string& indent) const override;

private:
    uint16_t getFirstAsteriskIndex() const;
    uint16_t divideSpecifiedCopies(uint16_t start, uint16_t end, uint16_t redundancy,
