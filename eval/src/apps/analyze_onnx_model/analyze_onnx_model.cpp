...
struct MyError : public std::exception {
    explicit MyError(std::string_view m) : std::exception(), msg(m) {}
...
void report_memory_usage(const std::string& desc) {
    ...
    fprintf(stderr, "vm_size: %zu kB, vm_rss: %zu kB, malloc_peak: %zu kb, malloc_curr: %zu (%s)\n", m.vm_size / 1_Ki,
            m.rss_size / 1_Ki, m.malloc_peak / 1_Ki, m.malloc_current / 1_Ki, desc.c_str());
}

struct Options {
    size_t                   pos = 0;
    std::vector<std::string> opt_list;
    void add_option(const std::string& opt) { opt_list.push_back(opt); }
...
void dump_model_info(const Onnx& model) {
    ...
    for (size_t i = 0; i < model.outputs().size(); ++i) {
        fprintf(stderr, "  output[%zu]: '%s' %s\n", i, model.outputs()[i].name.c_str(),
                model.outputs()[i].type_as_string().c_str());
    }
}
...
void dump_wire_info(const Onnx::WireInfo& wire) {
    ...
    for (size_t i = 0; i < wire.vespa_inputs.size(); ++i) {
        fprintf(stderr, "  input[%zu]: %s -> %s\n", i, wire.vespa_inputs[i].to_spec().c_str(),
                wire.onnx_inputs[i].type_as_string().c_str());
    }
    REQUIRE_EQ(wire.onnx_outputs.size(), wire.vespa_outputs.size());
    for (size_t i = 0; i < wire.onnx_outputs.size(); ++i) {
        fprintf(stderr, "  output[%zu]: %s -> %s\n", i, wire.onnx_outputs[i].type_as_string().c_str(),
                wire.vespa_outputs[i].to_spec().c_str());
    }
}
...
void bind_input(Onnx::WirePlanner& planner, const Onnx::TensorInfo& input, const ValueType& type) {
    ...
    if (!planner.bind_input_type(type, input)) {
        ...
        throw MyError{fmt("incompatible type for input '%s': %s -> %s%s", input.name.c_str(), type.to_spec().c_str(),
                          input.type_as_string().c_str(), bound_str.c_str())};
    }
}
...
ValueType make_output(const Onnx::WirePlanner& planner, const Onnx::TensorInfo& output) {
    ...
    if (type.is_error()) {
        throw MyError{fmt("unable to make compatible type for output '%s': %s -> error", output.name.c_str(),
                          output.type_as_string().c_str())};
    }
    return type;
}
...
int my_main(int argc, char** argv) {
    ...
    Onnx::Optimize optimize =
        opts.get_bool_opt("optimize model", "true") ? Onnx::Optimize::ENABLE : Onnx::Optimize::DISABLE;
