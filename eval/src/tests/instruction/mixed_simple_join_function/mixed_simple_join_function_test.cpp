...
void verify_simple(const std::string& expr, Primary primary, Overlap overlap, size_t factor, bool l_mut, bool r_mut,
                   bool inplace) {
...
void verify_optimized(const std::string& expr, Primary primary, Overlap overlap, size_t factor, bool l_mut = false,
                      bool r_mut = false, bool inplace = false) {
    std::ostringstream os;
...
struct LhsRhs {
    ...
    Overlap     overlap;
    size_t      factor;
    LhsRhs(const std::string& lhs_in, const std::string& rhs_in, size_t lhs_size_in, size_t rhs_size_in,
           Overlap overlap_in) noexcept
                ...
                for (bool right_mut : {false, true}) {
                    for (const char* expr : {"a+b", "a-b", "a*b"}) {
                        for (const LhsRhs& params :
                             {LhsRhs("y5", "y5", 5, 5, Overlap::FULL), LhsRhs("y5", "x3y5", 5, 15, Overlap::INNER),
