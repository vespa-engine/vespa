...
TensorSpec reference_peek(const TensorSpec& param, const PeekSpec& spec) {
    ...
    for (const auto& [dim_name, label_or_child] : spec) {
        ...
        std::visit(vespalib::overload{[&](const TensorSpec::Label& label) { with_indexes.emplace(dim, label); },
...
TensorSpec perform_generic_peek(const TensorSpec& a, const ValueType& result_type, PeekSpec spec,
...
TensorSpec tensor_function_peek(const TensorSpec& a, const ValueType& result_type, PeekSpec spec,
                                const ValueBuilderFactory& factory) {
    Stash stash;
    auto  param = value_from_spec(a, factory);
    EXPECT_FALSE(param->type().is_error());
...
void verify_peek_equal(const TensorSpec& input, const PeekSpec& spec, const ValueBuilderFactory& factory) {
    ...
    SCOPED_TRACE(fmt("peek input: %s\n  peek spec: %s\n  peek result %s\n", input.to_string().c_str(),
                     to_str(spec).c_str(), expect.to_string().c_str()));
...
void fill_dims_and_check(const TensorSpec& input, PeekSpec spec, std::vector<ValueType::Dimension> dimensions,
                         const ValueBuilderFactory& factory) {
    if (dimensions.empty()) {
