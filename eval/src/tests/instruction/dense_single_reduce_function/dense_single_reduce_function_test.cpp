...
void verify_not_optimized(const std::string& expr, std::vector<CellType> with_cell_types = {CellType::DOUBLE}) {
    SCOPED_TRACE(expr);
...
TEST(DenseSingleReduceFunctionTest,
     require_that_simple_aggregators_can_be_decomposed_into_multiple_reduce_operations) {
...
TEST(DenseSingleReduceFunctionTest,
     require_that_non_simple_aggregators_cannot_be_decomposed_into_multiple_reduce_operations) {
...
void verify_optimized_multi(const std::string& arg, const std::string& dim, size_t outer_size, size_t reduce_size,
                            size_t inner_size) {
