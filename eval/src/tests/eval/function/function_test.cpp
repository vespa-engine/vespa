        ...
        for (std::string op_name : layer.op_names) {
            ...
            for (OperatorLayer other_layer : layers) {
                ...
                for (std::string other_op_name : other_layer.op_names) {
                    ...
                    bool        do_op_before_other_op =
                        (layer_idx < other_layer_idx) ||
                        ((layer_idx == other_layer_idx) && (layer.order == Operator::Order::LEFT));
                    EXPECT_EQ(do_op_before_other_op, op->do_before(*other_op))
                        << "error: left operator '" << op->op_str() << "' should "
                        << (do_op_before_other_op ? "" : "not ") << "bind before right operator '"
                        << other_op->op_str() << "'";
                }
            }
        }
...
TEST(FunctionTest, require_that_if_expressions_can_be_parsed) {
    ...
    EXPECT_EQ("if(if(x,y,z),if(x,y,z),if(x,y,z))",
              Function::parse(params, "if(if(x,y,z),if(x,y,z),if(x,y,z))")->dump());
...
struct MyTraverser : public NodeTraverser {
    ...
    explicit MyTraverser(size_t open_true_cnt_in) : open_true_cnt(open_true_cnt_in), history() {}
    ...
    void close(const nodes::Node& node) override { history.emplace_back(false, node); }
...
TEST(FunctionTest, require_that_custom_symbol_extractor_may_be_used) {
    ...
    EXPECT_EQ("[x+]...[unknown symbol: 'x+']...[*y]",
              Function::parse(params, "x+*y", MySymbolExtractor({'+'}))->dump());
    EXPECT_EQ("[x+*y]...[unknown symbol: 'x+*y']...[]",
              Function::parse(params, "x+*y", MySymbolExtractor({'+', '*'}))->dump());
}
...
TEST(FunctionTest, require_that_unknown_function_works_as_expected_with_custom_symbol_extractor) {
    ...
    EXPECT_EQ("[bogus]...[unknown symbol: 'bogus']...[(x)+y]",
              Function::parse(params, "bogus(x)+y", MySymbolExtractor())->dump());
    EXPECT_EQ("[bogus(x)]...[unknown symbol: 'bogus(x)']...[+y]",
              Function::parse(params, "bogus(x)+y", MySymbolExtractor({'(', ')'}))->dump());
}

TEST(FunctionTest,
     require_that_unknown_function_that_is_valid_parameter_works_as_expected_with_custom_symbol_extractor) {
    EXPECT_EQ("[z(]...[unknown function: 'z']...[x)+y]", Function::parse(params, "z(x)+y")->dump());
    EXPECT_EQ("[z]...[invalid operator: '(']...[(x)+y]",
              Function::parse(params, "z(x)+y", MySymbolExtractor())->dump());
    EXPECT_EQ("[z(x)]...[unknown symbol: 'z(x)']...[+y]",
              Function::parse(params, "z(x)+y", MySymbolExtractor({'(', ')'}))->dump());
}
...
TEST(FunctionTest, require_that_custom_symbol_extractor_is_not_invoked_for_known_function_call) {
    ...
    EXPECT_EQ("[bogus]...[unknown symbol: 'bogus']...[(1,2)]",
              Function::parse(params, "bogus(1,2)", extractor)->dump());
...
TEST(FunctionTest, require_that_tensor_lambda_can_be_parsed) {
    ...
    EXPECT_EQ("tensor(x[2],y[2])(x==y)", Function::parse({}, " tensor ( x [ 2 ] , y [ 2 ] ) ( x == y ) ")->dump());
}
...
TEST(FunctionTest, require_that_tensor_lambda_can_use_non_dimension_symbols) {
    EXPECT_EQ("tensor(x[2])(x==a)", Function::parse({"a"}, "tensor(x[2])(x==a)")->dump());
}
...
TEST(FunctionTest, require_that_convenient_tensor_create_detects_too_large_indexed_dimensions) {
    verify_error("tensor(x[1]):[1,2]", "[tensor(x[1]):[1,]...[dimension too large: 'x']...[2]]");
}

TEST(FunctionTest, require_that_convenient_tensor_create_detects_under_specified_cells) {
    verify_error("tensor(x[1],y[1]):[1]", "[tensor(x[1],y[1]):[]...[expected '[', but got '1']...[1]]");
}
...
TEST(FunctionTest, require_that_tensor_peek_can_contain_extra_whitespace) {
    verify_parse(" t { x : ( 1 + bar ) , y : ( foo + 2 ) } ", "f(t,bar,foo)(t{x:(1+bar),y:(foo+2)})");
    verify_parse(" t { x : \"1 + bar\" , y : \"foo + 2\" } ", "f(t)(t{x:\"1 + bar\",y:\"foo + 2\"})");
}
...
TEST(FunctionTest, require_that_tensor_filter_subspaces_can_be_parsed) {
    ...
    EXPECT_EQ("filter_subspaces(a,f(x)(x))",
              Function::parse({"a"}, " filter_subspaces ( a , f ( x ) ( x ) ) ")->dump());
}
...
struct CheckExpressions : test::EvalSpec::EvalTest {
    ...
    void next_expression(const std::vector<std::string>& param_names, const std::string& expression) override {
    ...
    void handle_case(const std::vector<std::string>&, const std::vector<double>&, const std::string&,
                     double) override {}
};
