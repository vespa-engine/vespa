...
template <typename T, typename F> void verify_impl(const std::string& expr, const std::string& expect, F&& inspect) {
...
template <typename T> void verify_impl(const std::string& expr, const std::string& expect) {
...
void verify_idx_fun(const std::string& expr, const std::string& expect, const std::string& expect_idx_fun) {
    SCOPED_TRACE(expr);
    verify_impl<DenseLambdaPeekFunction>(
        expr, expect, [&](const DenseLambdaPeekFunction* info) { EXPECT_EQ(info->idx_fun_dump(), expect_idx_fun); });
}
