...
TEST(MapSubspacesTest, require_that_simple_map_subspaces_work) {
    verify("tensor(x{},y[3]):{foo:[1,2,3],bar:[4,5,6]}", "map_subspaces(a,f(t)(tensor(y[2])(t{y:(y)}+t{y:(y+1)})))",
           "tensor(x{},y[2]):{foo:[3,5],bar:[9,11]}");
}

TEST(MapSubspacesTest, require_that_scalars_can_be_used_with_map_subspaces) {
    verify("3.0", "map_subspaces(a,f(n)(n+5.0))", "8.0");
}

TEST(MapSubspacesTest, require_that_outer_cell_type_is_decayed_when_inner_type_is_double) {
    verify("tensor<int8>(x{}):{foo:3,bar:7}", "map_subspaces(a,f(n)(n+2))", "tensor<float>(x{}):{foo:5,bar:9}");
}

TEST(MapSubspacesTest, require_that_inner_cell_type_is_used_directly_without_decay) {
    {
        SCOPED_TRACE("1");
        verify("tensor(x{},y[3]):{foo:[1,2,3],bar:[4,5,6]}", "map_subspaces(a,f(t)(cell_cast(t,int8)))",
               "tensor<int8>(x{},y[3]):{foo:[1,2,3],bar:[4,5,6]}");
    }
    {
        SCOPED_TRACE("2");
        verify("tensor(y[3]):[1,2,3]", "map_subspaces(a,f(t)(cell_cast(t,int8)))", "tensor<int8>(y[3]):[1,2,3]");
    }
}
