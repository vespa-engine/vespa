...
TEST(ValueCodecTest, simple_values_can_be_built_using_tensor_spec) {
    ...
    full_spec.add({{"w", "xxx"}, {"x", 0}, {"y", "xxx"}, {"z", 0}}, 1.0)
...
struct TensorExample {
    ...
    void verify_encode_decode(bool is_dense) const {
        ...
        if (is_dense) {
            ...
            EXPECT_EQ(Memory(data_float.peek(), data_float.size()), Memory(expect_float.peek(), expect_float.size()));
        } else {
...
struct SparseTensorExample : TensorExample {
    ...
    template <typename T> void encode_inner(nbostream& dst) const {
