...
TEST(ParamUsageTest,
     require_that_lazy_parameters_are_suggested_for_functions_with_parameters_that_might_not_be_used) {
    auto function = Function::parse("if(z,x,y)+if(w,y,x)");
    EXPECT_TRUE(CompiledFunction::should_use_lazy_params(*function));
}

TEST(ParamUsageTest,
     require_that_lazy_parameters_are_not_suggested_for_functions_where_all_parameters_are_always_used) {
