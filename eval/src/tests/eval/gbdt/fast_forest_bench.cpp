...
template <typename T> void estimate_cost(size_t num_params, const char* label, const T& impl) {
    ...
    fprintf(stderr,
            "[%12s] (per 100 eval): [low values] %6.3f ms, [medium values] %6.3f ms, [high values] %6.3f ms, [nan "
            "values] %6.3f ms\n",
            ...
            for (size_t max_features : std::vector<size_t>({200})) {
                for (size_t less_percent : std::vector<size_t>({100})) {
                    for (size_t invert_percent : std::vector<size_t>({50})) {
                        ...
                        estimate_cost(function->num_params(), "vm forest",
                                      CompiledFunction(*function, PassParams::ARRAY, VMForest::optimize_chain));
                    }
                }
            }
        }
