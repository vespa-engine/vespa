...
struct EvalCtx {
    ...
    ValueType type_of(size_t idx) { return params[idx].get().type(); }
    ...
    Value::UP from_expr(const std::string& expr) { return value_from_spec(TensorSpec::from_expr(expr), factory); }
    Value::UP make_double(double value) { return value_from_spec(TensorSpec("double").add({}, value), factory); }
    Value::UP make_true() { return value_from_spec(TensorSpec("double").add({}, 1.0), factory); }
    Value::UP make_false() { return value_from_spec(TensorSpec("double").add({}, 0.0), factory); }
    Value::UP make_vector(std::initializer_list<double> cells, std::string dim = "x", bool mapped = false) {
        std::string type_spec = mapped ? make_string("tensor(%s{})", dim.c_str())
                                       : make_string("tensor(%s[%zu])", dim.c_str(), cells.size());
        TensorSpec  spec(type_spec);
        size_t      idx = 0;
        for (double cell_value : cells) {
            TensorSpec::Label label =
                mapped ? TensorSpec::Label(make_string("%zu", idx++)) : TensorSpec::Label(idx++);
    ...
    Value::UP make_mixed_tensor(double a, double b, double c, double d) {
        return value_from_spec(TensorSpec("tensor(x{},y[2])")
                                   .add({{"x", "foo"}, {"y", 0}}, a)
                                   .add({{"x", "foo"}, {"y", 1}}, b)
                                   .add({{"x", "bar"}, {"y", 0}}, c)
                                   .add({{"x", "bar"}, {"y", 1}}, d),
    ...
    Value::UP make_tensor_matrix_first_half() {
        return value_from_spec(TensorSpec("tensor(x[2])").add({{"x", 0}}, 1.0).add({{"x", 1}}, 3.0), factory);
    }
    Value::UP make_tensor_matrix_second_half() {
        return value_from_spec(TensorSpec("tensor(x[2])").add({{"x", 0}}, 2.0).add({{"x", 1}}, 4.0), factory);
    }
    Value::UP make_tensor_matrix() {
        return value_from_spec(TensorSpec("tensor(x[2],y[2])")
                                   .add({{"x", 0}, {"y", 0}}, 1.0)
                                   .add({{"x", 0}, {"y", 1}}, 2.0)
                                   .add({{"x", 1}, {"y", 0}}, 3.0)
                                   .add({{"x", 1}, {"y", 1}}, 4.0),
    ...
    Value::UP make_float_tensor_matrix() {
        return value_from_spec(TensorSpec("tensor<float>(x[2],y[2])")
                                   .add({{"x", 0}, {"y", 0}}, 1.0)
                                   .add({{"x", 0}, {"y", 1}}, 2.0)
                                   .add({{"x", 1}, {"y", 0}}, 3.0)
                                   .add({{"x", 1}, {"y", 1}}, 4.0),
    ...
    Value::UP make_tensor_matrix_renamed() {
        return value_from_spec(TensorSpec("tensor(y[2],z[2])")
                                   .add({{"z", 0}, {"y", 0}}, 1.0)
                                   .add({{"z", 0}, {"y", 1}}, 2.0)
                                   .add({{"z", 1}, {"y", 0}}, 3.0)
                                   .add({{"z", 1}, {"y", 1}}, 4.0),
    ...
    Value::UP make_tensor_reduce_input() {
        return value_from_spec(TensorSpec("tensor(x[3],y[2])")
                                   .add({{"x", 0}, {"y", 0}}, 1)
                                   .add({{"x", 1}, {"y", 0}}, 2)
                                   .add({{"x", 2}, {"y", 0}}, 3)
                                   .add({{"x", 0}, {"y", 1}}, 4)
    ...
    Value::UP make_tensor_reduce_y_output() {
        return value_from_spec(TensorSpec("tensor(x[3])").add({{"x", 0}}, 5).add({{"x", 1}}, 7).add({{"x", 2}}, 9),
                               factory);
    }
    Value::UP make_tensor_map_input() {
        return value_from_spec(TensorSpec("tensor(x{},y{})")
                                   .add({{"x", "1"}, {"y", "1"}}, 1)
                                   .add({{"x", "2"}, {"y", "1"}}, -3)
                                   .add({{"x", "1"}, {"y", "2"}}, 5),
                               factory);
    ...
    Value::UP make_tensor_join_rhs() {
        return value_from_spec(TensorSpec("tensor(y{},z{})")
                                   .add({{"y", "1"}, {"z", "1"}}, 7)
                                   .add({{"y", "2"}, {"z", "1"}}, 11)
                                   .add({{"y", "1"}, {"z", "2"}}, 13),
                               factory);
    ...
    Value::UP make_tensor_join_output() {
        return value_from_spec(TensorSpec("tensor(x{},y{},z{})")
                                   .add({{"x", "1"}, {"y", "1"}, {"z", "1"}}, 7)
                                   .add({{"x", "1"}, {"y", "1"}, {"z", "2"}}, 13)
                                   .add({{"x", "2"}, {"y", "1"}, {"z", "1"}}, 21)
                                   .add({{"x", "2"}, {"y", "1"}, {"z", "2"}}, 39)
    ...
    Value::UP make_tensor_merge_lhs() {
        return value_from_spec(
            TensorSpec("tensor(x{})").add({{"x", "1"}}, 1).add({{"x", "2"}}, 3).add({{"x", "3"}}, 5), factory);
    }
    Value::UP make_tensor_merge_rhs() {
        return value_from_spec(
            TensorSpec("tensor(x{})").add({{"x", "2"}}, 7).add({{"x", "3"}}, 9).add({{"x", "4"}}, 11), factory);
    }
    Value::UP make_tensor_merge_output() {
        return value_from_spec(TensorSpec("tensor(x{})")
                                   .add({{"x", "1"}}, 1)
                                   .add({{"x", "2"}}, 10)
                                   .add({{"x", "3"}}, 14)
                                   .add({{"x", "4"}}, 11),
...
TEST(TensorFunctionTest, require_that_partial_tensor_reduction_works) {
    ...
    const auto& fun =
        reduce(inject(ValueType::from_spec("tensor(x[3],y[2])"), a_id, ctx.stash), Aggr::SUM, {"y"}, ctx.stash);
    EXPECT_TRUE(fun.result_is_mutable());
    EXPECT_EQ(expect->type(), fun.result_type());
    verify_equal(*expect, ctx.eval(fun));
}
...
TEST(TensorFunctionTest, require_that_full_tensor_reduction_works) {
    ...
    size_t      a_id = ctx.add_tensor(ctx.make_tensor_reduce_input());
    const auto& fun =
        reduce(inject(ValueType::from_spec("tensor(x[3],y[2])"), a_id, ctx.stash), Aggr::SUM, {}, ctx.stash);
    EXPECT_TRUE(fun.result_is_mutable());
...
TEST(TensorFunctionTest, require_that_tensor_map_works) {
    ...
    const auto& fun =
        map(inject(ValueType::from_spec("tensor(x{},y{})"), a_id, ctx.stash), operation::Neg::f, ctx.stash);
    EXPECT_TRUE(fun.result_is_mutable());
    EXPECT_EQ(expect->type(), fun.result_type());
    verify_equal(*expect, ctx.eval(fun));
}
...
TEST(TensorFunctionTest, require_that_tensor_join_works) {
    ...
    const auto& fun =
        join(inject(ValueType::from_spec("tensor(x{},y{})"), a_id, ctx.stash),
             inject(ValueType::from_spec("tensor(y{},z{})"), b_id, ctx.stash), operation::Mul::f, ctx.stash);
    EXPECT_TRUE(fun.result_is_mutable());
    EXPECT_EQ(expect->type(), fun.result_type());
    verify_equal(*expect, ctx.eval(fun));
}
...
TEST(TensorFunctionTest, require_that_tensor_merge_works) {
    ...
    const auto& fun =
        merge(inject(ValueType::from_spec("tensor(x{})"), a_id, ctx.stash),
              inject(ValueType::from_spec("tensor(x{})"), b_id, ctx.stash), operation::Add::f, ctx.stash);
    EXPECT_TRUE(fun.result_is_mutable());
    EXPECT_EQ(expect->type(), fun.result_type());
    verify_equal(*expect, ctx.eval(fun));
}
...
TEST(TensorFunctionTest, require_that_tensor_concat_works) {
    ...
    const auto& fun = concat(inject(ValueType::from_spec("tensor(x[2])"), a_id, ctx.stash),
                             inject(ValueType::from_spec("tensor(x[2])"), b_id, ctx.stash), "y", ctx.stash);
    EXPECT_TRUE(fun.result_is_mutable());
    EXPECT_EQ(expect->type(), fun.result_type());
    verify_equal(*expect, ctx.eval(fun));
}
...
TEST(TensorFunctionTest, require_that_tensor_create_works) {
    ...
    const auto& fun =
        create(ValueType::from_spec("tensor(x[3])"), {{{{"x", 0}}, a}, {{{"x", 1}}, b}, {{{"x", 2}}, c}}, ctx.stash);
    EXPECT_TRUE(fun.result_is_mutable());
    EXPECT_EQ(expect->type(), fun.result_type());
    verify_equal(*expect, ctx.eval(fun));
}
...
TEST(TensorFunctionTest, require_that_single_value_tensor_peek_works) {
    ...
    const auto& fun = create(ValueType::from_spec("tensor(x[3])"),
                             {{{{"x", 0}}, peek1}, {{{"x", 1}}, peek2}, {{{"x", 2}}, peek3}}, ctx.stash);
    EXPECT_TRUE(fun.result_is_mutable());
    EXPECT_EQ(expect->type(), fun.result_type());
    verify_equal(*expect, ctx.eval(fun));
}
...
TEST(TensorFunctionTest, require_that_tensor_rename_works) {
    ...
    const auto& fun =
        rename(inject(ValueType::from_spec("tensor(x[2],y[2])"), a_id, ctx.stash), {"x"}, {"z"}, ctx.stash);
    EXPECT_TRUE(fun.result_is_mutable());
    EXPECT_EQ(expect->type(), fun.result_type());
    verify_equal(*expect, ctx.eval(fun));
}
...
TEST(TensorFunctionTest, require_that_tensor_function_can_be_dumped_for_debugging) {
    ...
    const auto& x2 = create(ValueType::from_spec("tensor(x[2])"), {{{{"x", 0}}, peek1}, {{{"x", 1}}, peek2}}, stash);
