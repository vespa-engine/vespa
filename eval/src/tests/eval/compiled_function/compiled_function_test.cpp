...
std::vector<std::string> unsupported = {"map(",    "map_subspaces(", "filter_subspaces(", "join(",
                                        "merge(",  "reduce(",        "rename(",           "tensor(",
                                        "concat(", "cell_cast(",     "cell_order("};
...
TEST(CompiledFunctionTest, require_that_function_issues_can_be_detected) {
    ...
    std::cerr << "Example function issues:" << std::endl
              << testing::PrintToString(CompiledFunction::detect_issues(*complex).list) << std::endl;
}
