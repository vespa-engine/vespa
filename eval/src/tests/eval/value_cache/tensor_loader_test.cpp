...
TensorSpec make_simple_dense_tensor() {
    return TensorSpec("tensor(z[3])").add({{"z", 0}}, 1.0).add({{"z", 1}}, 2.0).add({{"z", 2}}, 3.5);
}
...
TensorSpec make_mixed_tensor() {
    return TensorSpec("tensor(x{},y[2])").add({{"x", "foo"}, {"y", 0}}, 1.0).add({{"x", "foo"}, {"y", 1}}, 2.0);
}
...
TensorLoaderTest::TensorLoaderTest() : ::testing::Test(), f1(factory) {
}
...
TEST_F(TensorLoaderTest, require_that_mixed_tensor_blocks_form_can_be_loaded) {
    ...
    }
    {
        ...
        verify_tensor(make_mix21_tensor(),
                      f1.create(TEST_PATH("mixed-blocks-21.json"), "tensor<float>(brand{},category{},v[3])"));
    }
}
