...
TEST(OnnxTest, simple_onnx_model_can_be_evaluated) {
    ...
    EXPECT_EQ(planner.make_output_type(model.outputs()[0]).to_spec(), "tensor<float>(d0[1],d1[1])");
...
TEST(OnnxTest, int_types_onnx_model_can_be_evaluated) {
    ...
    EXPECT_EQ(planner.make_output_type(model.outputs()[0]), ValueType::from_spec("tensor<double>(d0[1],d1[1])"));
