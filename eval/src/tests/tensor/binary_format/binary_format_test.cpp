...
TEST(TensorBinaryFormatTest, tensor_binary_format_test_spec_can_be_generated) {
    ...
    std::string binary = module_build_path +
...
void verify_encode_decode(const TensorSpec& spec, const ValueBuilderFactory& encode_factory,
...
TEST(TensorBinaryFormatTest, tensor_binary_format_test_spec) {
    ...
    auto handle_test = [](Slime& slime) { test_binary_format_spec(slime.get()); };
    auto handle_summary = [](Slime& slime) { EXPECT_GT(slime["num_tests"].asLong(), 0); };
