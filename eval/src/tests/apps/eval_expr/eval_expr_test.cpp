...
struct Result {
    ...
    Result(const Inspector& obj)
        : error(obj["error"].asString().make_string()), result(obj["result"].asString().make_string()), steps() {
        const auto& arr = obj["steps"];
        for (size_t i = 0; i < arr.entries(); ++i) {
            steps.emplace_back(arr[i]["class"].asString().make_string(), arr[i]["symbol"].asString().make_string());
        }
    }
...
EvalExprTest::EvalExprTest() : ::testing::Test(), f1() {
}
