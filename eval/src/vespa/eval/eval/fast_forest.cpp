...
struct CmpNode {
    ...
    bool operator<(const CmpNode& rhs) const noexcept { return (value < rhs.value); }
    ~CmpNode() = default;
};
...
BitRange State::encode_node(uint32_t tree_id, const nodes::Node& node) {
...
template <typename T> size_t get_lsb(T value) {
    return vespalib::Optimized::lsbIdx(value);
}
template <> size_t get_lsb<uint8_t>(uint8_t value) {
    return vespalib::Optimized::lsbIdx(uint32_t(value));
}
template <> size_t get_lsb<uint16_t>(uint16_t value) {
    return vespalib::Optimized::lsbIdx(uint32_t(value));
}
...
template <> std::string fixed_impl_name<uint8_t>() {
    return "ff-fixed<8>";
}
template <> std::string fixed_impl_name<uint16_t>() {
    return "ff-fixed<16>";
}
template <> std::string fixed_impl_name<uint32_t>() {
    return "ff-fixed<32>";
}
template <> std::string fixed_impl_name<uint64_t>() {
    return "ff-fixed<64>";
}

template <typename T> constexpr size_t max_leafs() {
    return (sizeof(T) * bits_per_byte);
}

template <typename T> struct FixedContext : FastForest::Context {
    std::vector<T> masks;
    FixedContext(size_t num_trees) : masks(num_trees) {}
};

template <typename T> struct FixedForest : FastForest {
    ...
    struct Mask {
        ...
        uint32_t tree;
        T        bits;
        Mask(float v, uint32_t t, T b) noexcept : value(v), tree(t), bits(b) {}
    };

    struct DMask {
        uint32_t tree;
        T        bits;
        DMask(uint32_t t, T b) noexcept : tree(t), bits(b) {}
    };
...
template <typename T>
FastForest::UP FixedForest<T>::try_build(const State& state, size_t min_fixed, size_t max_fixed) {
    if ((max_leafs<T>() >= min_fixed) && (max_leafs<T>() <= max_fixed) && (state.max_leafs <= max_leafs<T>())) {
        return std::make_unique<FixedForest<T>>(state);
    }
    return FastForest::UP();
}

template <typename T> void FixedForest<T>::init_state(T* ctx_masks) const {
    memset(ctx_masks, 0xff, _num_trees * sizeof(T));
}

template <typename T> void FixedForest<T>::apply_masks(T* ctx_masks, const Mask* pos, const Mask* end, float limit) {
...
template <typename T> void FixedForest<T>::apply_masks(T* ctx_masks, const DMask* pos, const DMask* end) {
    for (; ((pos + 3) < end); pos += 4) {
        ctx_masks[pos[0].tree] &= pos[0].bits;
        ctx_masks[pos[1].tree] &= pos[1].bits;
        ctx_masks[pos[2].tree] &= pos[2].bits;
...
template <typename T> double FixedForest<T>::get_result(const T* ctx_masks) const {
    double       result1 = 0.0;
    double       result2 = 0.0;
...
template <typename T> FastForest::Context::UP FixedForest<T>::create_context() const {
    return std::make_unique<FixedContext<T>>(_num_trees);
}

template <typename T> double FixedForest<T>::eval(Context& context, const float* params) const {
    ...
    for (uint32_t size : _mask_sizes) {
        ...
        if (!std::isnan(feature)) {
            ...
            apply_masks(ctx_masks, _default_masks.data() + _default_offsets[param_pos - params - 1],
...
struct MultiWordForest : FastForest {
    ...
    struct DMask {
        ...
        DMask(uint32_t word_offset, uint32_t mask_bits) noexcept : offset(word_offset), bits(mask_bits) {}
...
MultiWordForest::MultiWordForest(const State& state)
    ...
    for (const auto& cmp_nodes : state.cmp_nodes) {
        ...
        _default_offsets.emplace_back(_default_masks.size(), _default_masks.size() + default_fixed_cnt);
        ...
        for (const CmpNode& cmp_node : rle) {
            ...
            if (cmp_node.false_is_default) {
                _default_masks.emplace_back(_masks.back().offset, _masks.back().rle_mask[0],
                                            _masks.back().rle_mask[1], _masks.back().rle_mask[2]);
            }
        }
    }
...
FastForest::UP MultiWordForest::try_build(const State& state) {
    if (is_little_endian()) {
...
void MultiWordForest::init_state(uint32_t* ctx_words) const {
    memset(ctx_words, 0xff, word_size * _words_per_tree * _tree_offsets.size());
}

void MultiWordForest::apply_fixed_masks(uint32_t* ctx_words, const Mask* pos, const Mask* end, float limit) {
    for (; ((pos + 3) < end) && !(limit < pos[3].value); pos += 4) {
...
void MultiWordForest::apply_rle_masks(unsigned char* ctx_bytes, const Mask* pos, const Mask* end, float limit) {
    for (; (pos < end) && !(limit < pos->value); ++pos) {
...
void MultiWordForest::apply_fixed_masks(uint32_t* ctx_words, const DMask* pos, const DMask* end) {
    for (; ((pos + 3) < end); pos += 4) {
        ctx_words[pos[0].offset] &= pos[0].bits;
        ctx_words[pos[1].offset] &= pos[1].bits;
        ctx_words[pos[2].offset] &= pos[2].bits;
...
void MultiWordForest::apply_rle_masks(unsigned char* ctx_bytes, const DMask* pos, const DMask* end) {
    for (; pos < end; ++pos) {
        unsigned char* dst = (ctx_bytes + pos->offset);
        *dst++ &= pos->rle_mask[0];
        for (size_t e = pos->rle_mask[1]; e-- > 0;) {
...
size_t MultiWordForest::find_leaf(const uint32_t* word) {
    size_t idx = 0;
    for (; *word == 0; ++word) {
        idx += bits_per_word;
    }
...
double MultiWordForest::get_result(const uint32_t* ctx_words) const {
    double       result = 0.0;
...
FastForest::Context::UP MultiWordForest::create_context() const {
    return std::make_unique<MultiWordContext>(_words_per_tree * _tree_offsets.size());
}

double MultiWordForest::eval(Context& context, const float* params) const {
    ...
    for (const Sizes& size : _mask_sizes) {
        ...
        if (!std::isnan(feature)) {
            ...
            apply_rle_masks(reinterpret_cast<unsigned char*>(ctx_words), mask_pos + size.fixed,
                            mask_pos + size.fixed + size.rle, feature);
        } else {
            apply_fixed_masks(ctx_words, _default_masks.data() + _default_offsets[param_pos - params - 1].fixed,
...
FastForest::UP FastForest::try_convert(const Function& fun, size_t min_fixed, size_t max_fixed) {
...
double FastForest::estimate_cost_us(const std::vector<double>& params, double budget) const {
