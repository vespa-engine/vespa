...
struct TensorFunctionBuilder : public NodeVisitor, public NodeTraverser {
    ...
    void make_lambda(const TensorLambda& node) {
        if (node.bindings().empty()) {
            ...
            TensorSpec          spec =
                tensor_function::Lambda::create_spec_impl(node.type(), no_bound_params, node.bindings(), my_fun);
            make_const(node, *stash.create<Value::UP>(value_from_spec(spec, factory)));
        } else {
            stack.push_back(tensor_function::lambda(node.type(), node.bindings(), node.lambda(),
                                                    types.export_types(node.lambda().root()), stash));
        }
    }
    ...
    void visit(const Number& node) override { make_const(node, stash.create<DoubleValue>(node.value())); }
    void visit(const Symbol& node) override { make_inject(node, node.id()); }
    ...
    void visit(const In& node) override {
        ...
        const auto& token =
            stash.create<CompileCache::Token::UP>(CompileCache::compile(*my_fun, PassParams::SEPARATE));
        make_map(node, token.get()->get().get_function<1>());
    }
    void visit(const Neg& node) override { make_map(node, operation::Neg::f); }
    void visit(const Not& node) override { make_map(node, operation::Not::f); }
    void visit(const If& node) override { make_if(node); }
    ...
    void visit(const TensorMap& node) override {
        if (auto op1 = operation::lookup_op1(node.lambda())) {
            ...
            const auto& token =
                stash.create<CompileCache::Token::UP>(CompileCache::compile(node.lambda(), PassParams::SEPARATE));
            make_map(node, token.get()->get().get_function<1>());
        }
    }
    void visit(const TensorMapSubspaces& node) override { make_map_subspaces(node); }
    void visit(const TensorFilterSubspaces& node) override { make_filter_subspaces(node); }
    ...
    void visit(const TensorReduce& node) override { make_reduce(node, node.aggr(), node.dimensions()); }
    void visit(const TensorRename& node) override { make_rename(node, node.from(), node.to()); }
    void visit(const TensorConcat& node) override { make_concat(node, node.dimension()); }
    void visit(const TensorCellCast& node) override { make_cell_cast(node, node.cell_type()); }
    void visit(const TensorCellOrder& node) override { make_cell_order(node, node.cell_order()); }
    void visit(const TensorCreate& node) override { make_create(node); }
    void visit(const TensorLambda& node) override { make_lambda(node); }
    void visit(const TensorPeek& node) override { make_peek(node); }
    void visit(const Add& node) override { make_join(node, operation::Add::f); }
    void visit(const Sub& node) override { make_join(node, operation::Sub::f); }
    void visit(const Mul& node) override { make_join(node, operation::Mul::f); }
    void visit(const Div& node) override { make_join(node, operation::Div::f); }
    void visit(const Mod& node) override { make_join(node, operation::Mod::f); }
    void visit(const Pow& node) override { make_join(node, operation::Pow::f); }
    void visit(const Equal& node) override { make_join(node, operation::Equal::f); }
    void visit(const NotEqual& node) override { make_join(node, operation::NotEqual::f); }
    void visit(const Approx& node) override { make_join(node, operation::Approx::f); }
    void visit(const Less& node) override { make_join(node, operation::Less::f); }
    void visit(const LessEqual& node) override { make_join(node, operation::LessEqual::f); }
    void visit(const Greater& node) override { make_join(node, operation::Greater::f); }
    void visit(const GreaterEqual& node) override { make_join(node, operation::GreaterEqual::f); }
    void visit(const And& node) override { make_join(node, operation::And::f); }
    void visit(const Or& node) override { make_join(node, operation::Or::f); }
    void visit(const Cos& node) override { make_map(node, operation::Cos::f); }
    void visit(const Sin& node) override { make_map(node, operation::Sin::f); }
    void visit(const Tan& node) override { make_map(node, operation::Tan::f); }
    void visit(const Cosh& node) override { make_map(node, operation::Cosh::f); }
    void visit(const Sinh& node) override { make_map(node, operation::Sinh::f); }
    void visit(const Tanh& node) override { make_map(node, operation::Tanh::f); }
    void visit(const Acos& node) override { make_map(node, operation::Acos::f); }
    void visit(const Asin& node) override { make_map(node, operation::Asin::f); }
    void visit(const Atan& node) override { make_map(node, operation::Atan::f); }
    void visit(const Exp& node) override { make_map(node, operation::Exp::f); }
    void visit(const Log10& node) override { make_map(node, operation::Log10::f); }
    void visit(const Log& node) override { make_map(node, operation::Log::f); }
    void visit(const Sqrt& node) override { make_map(node, operation::Sqrt::f); }
    void visit(const Ceil& node) override { make_map(node, operation::Ceil::f); }
    void visit(const Fabs& node) override { make_map(node, operation::Fabs::f); }
    void visit(const Floor& node) override { make_map(node, operation::Floor::f); }
    void visit(const Atan2& node) override { make_join(node, operation::Atan2::f); }
    void visit(const Ldexp& node) override { make_join(node, operation::Ldexp::f); }
    void visit(const Pow2& node) override { make_join(node, operation::Pow::f); }
    void visit(const Fmod& node) override { make_join(node, operation::Mod::f); }
    void visit(const Min& node) override { make_join(node, operation::Min::f); }
    void visit(const Max& node) override { make_join(node, operation::Max::f); }
    void visit(const IsNan& node) override { make_map(node, operation::IsNan::f); }
    void visit(const Relu& node) override { make_map(node, operation::Relu::f); }
    void visit(const Sigmoid& node) override { make_map(node, operation::Sigmoid::f); }
    void visit(const Elu& node) override { make_map(node, operation::Elu::f); }
    void visit(const Erf& node) override { make_map(node, operation::Erf::f); }
    void visit(const Bit& node) override { make_join(node, operation::Bit::f); }
    void visit(const Hamming& node) override { make_join(node, operation::Hamming::f); }
