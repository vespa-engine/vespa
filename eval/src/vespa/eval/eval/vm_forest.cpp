...
double general_find_leaf(const double* input, const uint32_t* pos, uint32_t node_type) {
    for (;;) {
        if (node_type == LESS) {
            ...
            if (find_in(input[pos[0] >> 12], pos + 2, pos + 2 + (2 * (pos[1] & 0xff)))) {
                node_type = (pos[0] & 0xf0) >> 4;
                pos += 2 + (2 * (pos[1] & 0xff));
            } else {
                node_type = (pos[0] & 0xf);
...
void encode_less(const nodes::Less& less, const nodes::Node& left_child, const nodes::Node& right_child,
...
void encode_in(const nodes::In& in, const nodes::Node& left_child, const nodes::Node& right_child,
               std::vector<uint32_t>& model_out) {
    size_t meta_idx = model_out.size();
...
void encode_inverted(const nodes::Not& inverted, const nodes::Node& left_child, const nodes::Node& right_child,
                     std::vector<uint32_t>& model_out) {
    size_t meta_idx = model_out.size();
...
Optimize::Result optimize(const std::vector<const nodes::Node*>& trees, Forest::eval_function eval) {
...
Optimize::Result VMForest::less_only_optimize(const ForestStats&                     stats,
...
double VMForest::less_only_eval(const Forest* forest, const double* input) {
...
Optimize::Result VMForest::general_optimize(const ForestStats& stats, const std::vector<const nodes::Node*>& trees) {
...
double VMForest::general_eval(const Forest* forest, const double* input) {
    const VMForest& self = *((const VMForest*)forest);
    const uint32_t* pos = &self._model[0];
    const uint32_t* end = pos + self._model.size();
    double          sum = 0.0;
