...
double vespalib_eval_approx(double a, double b) {
    return (vespalib::approx_equal(a, b) ? 1.0 : 0.0);
}
...
double vespalib_eval_elu(double a) {
    return (a < 0) ? std::exp(a) - 1.0 : a;
}
double vespalib_eval_bit(double a, double b) {
    return vespalib::eval::extract_bit(a, b);
}
double vespalib_eval_hamming(double a, double b) {
    return vespalib::eval::hamming_distance(a, b);
}

using vespalib::eval::gbdt::Forest;
using resolve_function = double (*)(void* ctx, size_t idx);
double vespalib_eval_forest_proxy(Forest::eval_function eval_forest, const Forest* forest, resolve_function resolve,
                                  void* ctx, size_t num_params) {
    if (num_params <= 64) {
        double params[64];
...
struct FunctionBuilder : public NodeVisitor, public NodeTraverser {
    ...
    FunctionBuilder(llvm::LLVMContext& context_in, llvm::Module& module_in, const std::string& name_in,
                    size_t num_params_in, PassParams pass_params_in,
                    const gbdt::Optimize::Chain& forest_optimizers_in, std::vector<gbdt::Forest::UP>& forests_out,
    ...
    bool try_optimize_forest(const Node& item) {
        ...
        llvm::PointerType*  eval_funptr_t = llvm::PointerType::get(builder.getContext(), 0);
        llvm::Value*        eval_fun =
            builder.CreateIntToPtr(builder.getInt64((uint64_t)eval_ptr), eval_funptr_t, "inject_eval");
        llvm::Value* ctx = builder.CreateIntToPtr(builder.getInt64((uint64_t)forest),
                                                  llvm::PointerType::get(builder.getContext(), 0), "inject_ctx");
        if (pass_params == PassParams::ARRAY) {
            ...
            llvm::Value* proxy_fun = builder.CreateIntToPtr(builder.getInt64((uint64_t)vespalib_eval_forest_proxy),
                                                            proxy_funptr_t, "inject_eval_proxy");
            push(builder.CreateCall(proxy_fun_t, proxy_fun,
                                    {eval_fun, ctx, params[0], params[1], builder.getInt64(stats.num_params)}));
        }
        return true;
    }
    ...
    void build_root(const Node& node) { node.traverse(*this); }
    ...
    void build_forest_fragment(const std::vector<const Node*>& trees) {
        ...
        for (auto tree : trees) {
            ...
            sum = (sum) ? builder.CreateFAdd(sum, tree_value, "add_tree") : tree_value;
        }
        push(sum);
        inside_forest = false;
    }
    ...
    void push_double(double value) { push(llvm::ConstantFP::get(builder.getDoubleTy(), value)); }
    ...
    void make_call_1(const char* name) { make_call_1(module.getOrInsertFunction(name, make_call_1_fun_t())); }
    ...
    void make_call_2(const char* name) { make_call_2(module.getOrInsertFunction(name, make_call_2_fun_t())); }

    //-------------------------------------------------------------------------

    // basic nodes

    void visit(const Number& item) override { push_double(item.value()); }
    void visit(const Symbol& item) override { push(get_param(item.id())); }
    void visit(const String& item) override { push_double(item.get_const_double_value()); }
    void visit(const In& item) override {
        ...
        if (item.num_entries() > 8) {
            ...
            llvm::PointerType*  funptr_t = llvm::PointerType::get(builder.getContext(), 0);
            llvm::Value*        call_fun =
                builder.CreateIntToPtr(builder.getInt64((uint64_t)call_ptr), funptr_t, "inject_call_addr");
            llvm::Value* ctx = builder.CreateIntToPtr(builder.getInt64((uint64_t)state),
                                                      llvm::PointerType::get(builder.getContext(), 0), "inject_ctx");
            push(builder.CreateCall(fun_t, call_fun, {ctx, lhs}, "call_check_membership"));
        } else {
            // build explicit code to check all set members
            llvm::Value* found = builder.getFalse();
            for (size_t i = 0; i < item.num_entries(); ++i) {
                llvm::Value* elem =
                    llvm::ConstantFP::get(builder.getDoubleTy(), item.get_entry(i).get_const_double_value());
    ...
    void visit(const Error&) override { make_error(0); }

    // tensor nodes (not supported in compiled expressions)

    void visit(const TensorMap& node) override { make_error(node.num_children()); }
    void visit(const TensorMapSubspaces& node) override { make_error(node.num_children()); }
    void visit(const TensorFilterSubspaces& node) override { make_error(node.num_children()); }
    void visit(const TensorJoin& node) override { make_error(node.num_children()); }
    void visit(const TensorMerge& node) override { make_error(node.num_children()); }
    void visit(const TensorReduce& node) override { make_error(node.num_children()); }
    void visit(const TensorRename& node) override { make_error(node.num_children()); }
    void visit(const TensorConcat& node) override { make_error(node.num_children()); }
    void visit(const TensorCellCast& node) override { make_error(node.num_children()); }
    void visit(const TensorCellOrder& node) override { make_error(node.num_children()); }
    void visit(const TensorCreate& node) override { make_error(node.num_children()); }
    void visit(const TensorLambda& node) override { make_error(node.num_children()); }
    void visit(const TensorPeek& node) override { make_error(node.num_children()); }
    ...
    void visit(const Mod&) override { make_call_2("fmod"); }
    void visit(const Pow&) override { make_call_2(llvm::Intrinsic::pow); }
    ...
    void visit(const Approx&) override { make_call_2("vespalib_eval_approx"); }
    ...
    void visit(const Cos&) override { make_call_1(llvm::Intrinsic::cos); }
    void visit(const Sin&) override { make_call_1(llvm::Intrinsic::sin); }
    void visit(const Tan&) override { make_call_1("tan"); }
    void visit(const Cosh&) override { make_call_1("cosh"); }
    void visit(const Sinh&) override { make_call_1("sinh"); }
    void visit(const Tanh&) override { make_call_1("tanh"); }
    void visit(const Acos&) override { make_call_1("acos"); }
    void visit(const Asin&) override { make_call_1("asin"); }
    void visit(const Atan&) override { make_call_1("atan"); }
    void visit(const Exp&) override { make_call_1(llvm::Intrinsic::exp); }
    void visit(const Log10&) override { make_call_1(llvm::Intrinsic::log10); }
    void visit(const Log&) override { make_call_1(llvm::Intrinsic::log); }
    void visit(const Sqrt&) override { make_call_1(llvm::Intrinsic::sqrt); }
    void visit(const Ceil&) override { make_call_1(llvm::Intrinsic::ceil); }
    void visit(const Fabs&) override { make_call_1(llvm::Intrinsic::fabs); }
    void visit(const Floor&) override { make_call_1(llvm::Intrinsic::floor); }
    void visit(const Atan2&) override { make_call_2("atan2"); }
    void visit(const Ldexp&) override { make_call_2("vespalib_eval_ldexp"); }
    void visit(const Pow2&) override { make_call_2(llvm::Intrinsic::pow); }
    void visit(const Fmod&) override { make_call_2("fmod"); }
    void visit(const Min&) override { make_call_2("vespalib_eval_min"); }
    void visit(const Max&) override { make_call_2("vespalib_eval_max"); }
    void visit(const IsNan&) override { make_call_1("vespalib_eval_isnan"); }
    void visit(const Relu&) override { make_call_1("vespalib_eval_relu"); }
    void visit(const Sigmoid&) override { make_call_1("vespalib_eval_sigmoid"); }
    void visit(const Elu&) override { make_call_1("vespalib_eval_elu"); }
    void visit(const Erf&) override { make_call_1("erf"); }
    void visit(const Bit&) override { make_call_2("vespalib_eval_bit"); }
    void visit(const Hamming&) override { make_call_2("vespalib_eval_hamming"); }
};
...
LLVMWrapper::LLVMWrapper() : _context(), _module(), _engine(), _functions(), _forests(), _plugin_state() {
    _context = std::make_unique<llvm::LLVMContext>();
    _module = std::make_unique<llvm::Module>("LLVMWrapper", *_context);
}

size_t LLVMWrapper::make_function(size_t num_params, PassParams pass_params, const Node& root,
                                  const gbdt::Optimize::Chain& forest_optimizers) {
    size_t          function_id = _functions.size();
    FunctionBuilder builder(*_context, *_module, vespalib::make_string("f%zu", function_id), num_params, pass_params,
...
size_t LLVMWrapper::make_forest_fragment(size_t num_params, const std::vector<const Node*>& fragment) {
    size_t          function_id = _functions.size();
    FunctionBuilder builder(*_context, *_module, vespalib::make_string("f%zu", function_id), num_params,
                            PassParams::ARRAY, gbdt::Optimize::none, _forests, _plugin_state);
    builder.build_forest_fragment(fragment);
    _functions.push_back(builder.build());
    return function_id;
}

void LLVMWrapper::compile(llvm::raw_ostream* dumpStream) {
    if (dumpStream) {
        _module->print(*dumpStream, nullptr);
    }
    // Set relocation model to silence valgrind on CentOS 8 / aarch64
    _engine.reset(llvm::EngineBuilder(std::move(_module))
...
void* LLVMWrapper::get_function_address(size_t function_id) {
