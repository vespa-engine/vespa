...
double empty_function_0() {
    return 0.0;
}
double empty_function_1(double) {
    return 0.0;
}
double empty_function_2(double, double) {
    return 0.0;
}
double empty_function_3(double, double, double) {
    return 0.0;
}
double empty_function_4(double, double, double, double) {
    return 0.0;
}
double empty_function_5(double, double, double, double, double) {
    return 0.0;
}
double empty_array_function(const double*) {
    return 0.0;
}
double empty_lazy_function(CompiledFunction::resolve_function, void*) {
    return 0.0;
}

double my_resolve(void* ctx, size_t idx) {
    return ((double*)ctx)[idx];
}
...
CompiledFunction::CompiledFunction(const nodes::Node& root_in, size_t num_params_in, PassParams pass_params_in,
    ...
    : _llvm_wrapper(), _address(nullptr), _num_params(num_params_in), _pass_params(pass_params_in) {
    size_t id = _llvm_wrapper.make_function(num_params_in, _pass_params, root_in, forest_optimizers);
...
Function::Issues CompiledFunction::detect_issues(const nodes::Node& node) {
    struct NotSupported : NodeTraverser {
        ...
        void close(const nodes::Node& node) override {
            if (nodes::check_type<nodes::TensorMap, nodes::TensorMapSubspaces, nodes::TensorFilterSubspaces,
                                  nodes::TensorJoin, nodes::TensorMerge, nodes::TensorReduce, nodes::TensorRename,
                                  nodes::TensorConcat, nodes::TensorCellCast, nodes::TensorCellOrder,
                                  nodes::TensorCreate, nodes::TensorLambda, nodes::TensorPeek>(node))
            {
                issues.push_back(make_string("unsupported node type: %s", getClassName(node).c_str()));
            }
        }
    } checker;
...
bool CompiledFunction::should_use_lazy_params(const Function& function) {
