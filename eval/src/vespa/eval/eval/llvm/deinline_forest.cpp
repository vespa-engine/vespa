...
Optimize::Result DeinlineForest::optimize(const ForestStats&, const std::vector<const nodes::Node*>& trees) {
    return Optimize::Result(Forest::UP(new DeinlineForest(trees)), eval);
}

double DeinlineForest::eval(const Forest* forest, const double* input) {
