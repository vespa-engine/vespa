...
struct DumpContext {
    ...
    DumpContext(const std::vector<std::string>& param_names_in) : param_names(param_names_in) {}
};
...
struct Leaf : public Node {
    ...
    const Node& get_child(size_t) const override { HDR_ABORT("should not be reached"); }
...
public:
    ...
    std::string dump(DumpContext&) const override { return as_quoted_string(_value); }
    void accept(NodeVisitor& visitor) const override;
};
