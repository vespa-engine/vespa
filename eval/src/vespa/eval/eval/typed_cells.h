...
struct TypedCells {
    ...
    explicit TypedCells(std::span<const double> cells) noexcept
        : data(cells.data()), size(cells.size()), _non_existing_attribute_value(false), type(CellType::DOUBLE) {}
    explicit TypedCells(std::span<const float> cells) noexcept
        : data(cells.data()), size(cells.size()), _non_existing_attribute_value(false), type(CellType::FLOAT) {}
    explicit TypedCells(std::span<const BFloat16> cells) noexcept
        : data(cells.data()), size(cells.size()), _non_existing_attribute_value(false), type(CellType::BFLOAT16) {}
    explicit TypedCells(std::span<const Int8Float> cells) noexcept
        : data(cells.data()), size(cells.size()), _non_existing_attribute_value(false), type(CellType::INT8) {}

    TypedCells() noexcept : data(nullptr), size(0), _non_existing_attribute_value(false), type(CellType::DOUBLE) {}
    TypedCells(const void* dp, CellType ct, size_t sz) noexcept
        : data(dp), size(sz), _non_existing_attribute_value(false), type(ct) {}
