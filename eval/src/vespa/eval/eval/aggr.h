...
constexpr bool is_simple(Aggr aggr) {
    return ((aggr == Aggr::PROD) || (aggr == Aggr::SUM) || (aggr == Aggr::MAX) || (aggr == Aggr::MIN));
}

// will a single value reduce to itself?
constexpr bool is_ident(Aggr aggr) {
    return ((aggr == Aggr::AVG) || (aggr == Aggr::PROD) || (aggr == Aggr::SUM) || (aggr == Aggr::MAX) ||
            (aggr == Aggr::MEDIAN) || (aggr == Aggr::MIN));
}
...
struct TypifyAggr {
    ...
    template <typename F> static decltype(auto) resolve(Aggr aggr, F&& f) {
        switch (aggr) {
        case Aggr::AVG:
            return f(Result<aggr::Avg>());
        case Aggr::COUNT:
            return f(Result<aggr::Count>());
        case Aggr::PROD:
            return f(Result<aggr::Prod>());
        case Aggr::SUM:
            return f(Result<aggr::Sum>());
        case Aggr::MAX:
            return f(Result<aggr::Max>());
        case Aggr::MEDIAN:
            return f(Result<aggr::Median>());
        case Aggr::MIN:
            return f(Result<aggr::Min>());
        }
        abort();
    }
};
