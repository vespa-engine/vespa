...
public:
    struct Label {
        ...
        bool operator==(const Label& rhs) const noexcept { return ((index == rhs.index) && (name == rhs.name)); }
    ...
    struct Value {
        ...
        static bool both_nan(double a, double b) noexcept { return (std::isnan(a) && std::isnan(b)); }
...
public:
    ...
    static std::string diff(const TensorSpec& lhs, const std::string& lhs_desc, const TensorSpec& rhs,
