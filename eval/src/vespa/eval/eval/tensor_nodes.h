...
public:
    TensorCellCast(Node_UP child, CellType cell_type) : _child(std::move(child)), _cell_type(cell_type) {}
    const Node& child() const { return *_child; }
...
public:
    TensorCellOrder(Node_UP child, CellOrder cell_order) : _child(std::move(child)), _cell_order(cell_order) {}
    const Node& child() const { return *_child; }
...
public:
    TensorCreate(ValueType type_in, Spec spec) : _type(std::move(type_in)), _cells() {
        for (auto& cell : spec) {
...
public:
    struct MyLabel {
        std::string label;
        Node_UP     expr;
        MyLabel(std::string label_in) : label(label_in), expr() {}
        MyLabel(Node_UP node) : label(""), expr(std::move(node)) {}
...
public:
    TensorPeek(Node_UP param, Spec spec) : _param(std::move(param)), _dim_list(), _expr_dims() {
        for (auto& dim : spec) {
