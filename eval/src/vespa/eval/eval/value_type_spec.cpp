...
public:
    struct Mark {
        ...
        char        curr;
        bool        failed;
        Mark(const char* pos_in, char curr_in, bool failed_in) : pos(pos_in), curr(curr_in), failed(failed_in) {}
    };
...
public:
    ...
    Mark mark() const { return Mark(_pos, _curr, _failed); }
...
bool is_ident(char c, bool first) {
    return ((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || (c == '_') || (c >= '0' && c <= '9' && !first));
}
