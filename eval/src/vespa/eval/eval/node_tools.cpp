...
struct CountParams : NodeTraverser, EmptyNodeVisitor {
    size_t result = 0;
    void visit(const Symbol& symbol) override { result = std::max(result, symbol.id() + 1); }
...
struct CopyNode : NodeTraverser, NodeVisitor {
    ...
    void not_implemented(const Node&) { fail("not implemented"); }
    ...
    // basic nodes
    void visit(const Number& node) override { stack.push_back(std::make_unique<Number>(node.value())); }
    void visit(const Symbol& node) override { stack.push_back(std::make_unique<Symbol>(node.id())); }
    void visit(const String& node) override { stack.push_back(std::make_unique<String>(node.value())); }
    ...
    void visit(const If& node) override {
        ...
        stack.push_back(
            std::make_unique<If>(std::move(list[0]), std::move(list[1]), std::move(list[2]), node.p_true()));
    }
    void visit(const Error& node) override { stack.push_back(std::make_unique<Error>(node.message())); }
...
size_t NodeTools::min_num_params(const Node& node) {
    CountParams count_params;
    node.traverse(count_params);
    return count_params.result;
}

Node_UP NodeTools::copy(const Node& node) {
    CopyNode copy_node;
