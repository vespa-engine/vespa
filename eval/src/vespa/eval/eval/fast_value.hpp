...
template <typename T> struct FastCells {
    ...
    constexpr T* get(size_t offset) const { return reinterpret_cast<T*>(memory.get()) + offset; }
    void push_back_fast(T value) { *get(size++) = value; }
...
FastCells<T>::FastCells(size_t initial_capacity)
    : capacity(roundUp2inN(initial_capacity)), size(0), memory(alloc::Alloc::alloc(elem_size * capacity)) {
    static_assert(std::is_trivially_copyable_v<T>);
    static_assert(can_skip_destruction<T>);
}

template <typename T> void FastCells<T>::reallocate(size_t need) {
...
template <typename T> FastCells<T>::~FastCells() = default;

//-----------------------------------------------------------------------------

template <typename T, bool transient> struct FastValue final : Value, ValueBuilder<T> {
    using Handles = typename std::conditional<transient, StringIdVector, SharedStringRepo::Handles>::type;
    ...
    FastValue(const ValueType& type_in, size_t num_mapped_dims_in, size_t subspace_size_in,
...
FastValue<T, transient>::FastValue(const ValueType& type_in, size_t num_mapped_dims_in, size_t subspace_size_in,
                                   size_t expected_subspaces_in)
    : my_type(type_in),
      my_subspace_size(subspace_size_in),
      my_handles(),
...
template <typename T> struct FastDenseValue final : Value, ValueBuilder<T> {

    ValueType    my_type;
    FastCells<T> my_cells;

    FastDenseValue(const ValueType& type_in, size_t subspace_size_in) : my_type(type_in), my_cells(subspace_size_in) {
...
struct FastDoubleValueBuilder final : ValueBuilder<double> {
    double _value;
    std::span<double> add_subspace(std::span<const std::string_view>) final override {
        return std::span<double>(&_value, 1);
    }
    std::span<double> add_subspace(std::span<const string_id>) final override {
        return std::span<double>(&_value, 1);
    };
    std::unique_ptr<Value> build(std::unique_ptr<ValueBuilder<double>>) final override {
