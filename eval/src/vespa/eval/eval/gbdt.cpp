...
std::vector<const nodes::Node*> extract_trees(const nodes::Node& node) {
    ...
    while (!todo.empty()) {
        const nodes::Node& forest = *todo.back();
        todo.pop_back();
...
double TreeStats::traverse(const nodes::Node& node, size_t depth, size_t& sum_path) {
...
Optimize::Result Optimize::select_best(const ForestStats& stats, const std::vector<const nodes::Node*>& trees) {
