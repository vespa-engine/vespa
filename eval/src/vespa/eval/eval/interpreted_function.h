...
public:
    struct State {
        ...
        const Value& peek(size_t ridx) const { return stack[stack.size() - 1 - ridx]; }
        void pop_push(const Value& value) { stack.back() = value; }
    ...
    public:
        explicit Instruction(op_function function_in) noexcept : function(function_in), param(0) {}
        Instruction(op_function function_in, uint64_t param_in) noexcept : function(function_in), param(param_in) {}
        ...
        static Instruction fetch_param(size_t param_idx) { return Instruction(nullptr, param_idx); }
