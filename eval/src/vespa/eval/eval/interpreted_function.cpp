...
InterpretedFunction::State::State(const ValueBuilderFactory& factory_in)
    : factory(factory_in), params(nullptr), stash(), stack(), program_offset(0), if_cnt(0) {
}

InterpretedFunction::State::~State() = default;

void InterpretedFunction::State::init(const LazyParams& params_in) {
    params = &params_in;
    stash.clear();
    stack.clear();
    program_offset = 0;
...
InterpretedFunction::Context::Context(const InterpretedFunction& ifun) : _state(ifun._factory) {
}

InterpretedFunction::ProfiledContext::ProfiledContext(const InterpretedFunction& ifun)
    : context(ifun), cost(ifun.program_size(), std::make_pair(size_t(0), duration::zero())) {
}

InterpretedFunction::ProfiledContext::~ProfiledContext() = default;

std::string InterpretedFunction::Instruction::resolve_symbol() const {
    if (function == nullptr) {
        return "<inject_param>";
    }
...
InterpretedFunction::Instruction InterpretedFunction::Instruction::nop() {
...
InterpretedFunction::InterpretedFunction(const ValueBuilderFactory& factory, const TensorFunction& function,
                                         CTFMetaData* meta)
    : _program(), _stash(), _factory(factory) {
    _program = compile_tensor_function(factory, function, _stash, meta);
}

InterpretedFunction::InterpretedFunction(const ValueBuilderFactory& factory, const nodes::Node& root,
...
const Value& InterpretedFunction::eval(Context& ctx, const LazyParams& params) const {
...
const Value& InterpretedFunction::eval(ProfiledContext& pctx, const LazyParams& params) const {
...
double InterpretedFunction::estimate_cost_us(const std::vector<double>& params, double budget) const {
    Context      ctx(*this);
...
Function::Issues InterpretedFunction::detect_issues(const Function& function) {
...
InterpretedFunction::EvalSingle::EvalSingle(const ValueBuilderFactory& factory, Instruction op,
                                            const LazyParams& params)
    : _state(factory), _op(op) {
    _state.params = &params;
}

const Value& InterpretedFunction::EvalSingle::eval(const std::vector<Value::CREF>& stack) {
    _state.stash.clear();
    _state.stack = stack;
    _op.perform(_state);
