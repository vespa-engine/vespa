 ...
 */
template <typename T> const T* as(const TensorFunction& node) {
...
public:
    Op1(const ValueType& result_type_in, const TensorFunction& child_in) : Node(result_type_in), _child(child_in) {}
...
public:
    Op2(const ValueType& result_type_in, const TensorFunction& lhs_in, const TensorFunction& rhs_in)
...
public:
    ...
    InterpretedFunction::Instruction compile_self(const ValueBuilderFactory& factory,
                                                  Stash&                     stash) const final override;
...
public:
    Inject(const ValueType& result_type_in, size_t param_idx_in) : Super(result_type_in), _param_idx(param_idx_in) {}
...
public:
    Reduce(const ValueType& result_type_in, const TensorFunction& child_in, Aggr aggr_in,
...
public:
    Map(const ValueType& result_type_in, const TensorFunction& child_in, map_fun_t function_in)
...
public:
    MapSubspaces(const ValueType& result_type_in, const TensorFunction& child_in, const Function& lambda_in,
                 NodeTypes lambda_types_in)
...
public:
    FilterSubspaces(const ValueType& result_type_in, const TensorFunction& child_in, const Function& lambda_in,
                    NodeTypes lambda_types_in)
        : Super(result_type_in, child_in),
          _inner_type(child_in.result_type().strip_mapped_dimensions()),
          _lambda(lambda_in.shared_from_this()),
...
public:
    Join(const ValueType& result_type_in, const TensorFunction& lhs_in, const TensorFunction& rhs_in,
...
public:
    Merge(const ValueType& result_type_in, const TensorFunction& lhs_in, const TensorFunction& rhs_in,
          join_fun_t function_in)
        : Super(result_type_in, lhs_in, rhs_in), _function(function_in) {}
    join_fun_t function() const { return _function; }
    bool result_is_mutable() const override { return true; }
...
public:
    Concat(const ValueType& result_type_in, const TensorFunction& lhs_in, const TensorFunction& rhs_in,
...
public:
    Lambda(const ValueType& result_type_in, const std::vector<size_t>& bindings_in, const Function& lambda_in,
           NodeTypes lambda_types_in)
        : Super(result_type_in),
          _bindings(bindings_in),
          _lambda(lambda_in.shared_from_this()),
          _lambda_types(std::move(lambda_types_in)) {}
    const std::vector<size_t>& bindings() const { return _bindings; }
    const Function& lambda() const { return *_lambda; }
    const NodeTypes& types() const { return _lambda_types; }
    static TensorSpec create_spec_impl(const ValueType& type, const LazyParams& params,
                                       const std::vector<size_t>& bind, const InterpretedFunction& fun);
...
public:
    Peek(const ValueType& result_type_in, const TensorFunction& param,
        ...
        for (const auto& dim : spec) {
            std::visit(
                vespalib::overload{[&](const TensorSpec::Label& label) { _map.emplace(dim.first, label); },
                                   [&](const TensorFunction::CREF& ref) { _map.emplace(dim.first, ref.get()); }},
                dim.second);
        }
    }
...
public:
    Rename(const ValueType& result_type_in, const TensorFunction& child_in, const std::vector<std::string>& from_in,
...
public:
    If(const ValueType& result_type_in, const TensorFunction& cond_in, const TensorFunction& true_child_in,
    ...
    bool result_is_mutable() const override {
        return (true_child().result_is_mutable() && false_child().result_is_mutable());
    }
    InterpretedFunction::Instruction compile_self(const ValueBuilderFactory& factory,
                                                  Stash&                     stash) const final override;
    void visit_children(vespalib::ObjectVisitor& visitor) const final override;
