...
EvalSpec::Expression& EvalSpec::Expression::add_cases(std::initializer_list<double> a_values,
                                                      std::initializer_list<double> b_values, fun_2_ref fun) {
    for (double a : a_values) {
...
void EvalSpec::add_rule(const ParamSpec& a_spec, const ParamSpec& b_spec, const std::string& expression,
                        fun_2_ref ref) {
...
bool EvalSpec::EvalTest::is_same(double expected, double actual) {
...
void EvalSpec::add_terminal_cases() {
...
void EvalSpec::add_arithmetic_cases() {
    add_rule({"a", -5.0, 5.0}, "(-a)", [](double a) { return -a; });
...
void EvalSpec::add_function_call_cases() {
    ...
    add_expression({"a", "b"}, "hamming(a,b)")
        ...
        .add_case({-1, 0}, 8.0)
        .add_case({0, -1}, 8.0)
        .add_case({byte("11001100"), byte("10101010")}, 4.0)
        .add_case({byte("11001100"), byte("11110000")}, 4.0);
}

void EvalSpec::add_tensor_operation_cases() {
    ...
    add_rule({"a", -1.0, 1.0}, {"b", -1.0, 1.0}, "join(a,b,f(x,y)(x*y*3))",
             [](double x, double y) { return ((x * y) * 3); });
    add_rule({"a", -1.0, 1.0}, {"b", -1.0, 1.0}, "merge(a,b,f(x,y)(x+y))",
             [](double x, double y) { return (x + y); });
    add_rule({"a", -1.0, 1.0}, {"b", -1.0, 1.0}, "merge(a,b,f(x,y)(x*y*3))",
             [](double x, double y) { return ((x * y) * 3); });
...
void EvalSpec::add_comparison_cases() {
...
void EvalSpec::add_set_membership_cases() {
    add_expression({"a"}, "(a in [])").add_case({0.0}, 0.0).add_case({1.0}, 0.0);
    ...
    add_expression({"a"}, "(a in [30,20,10])").add_case({10.0}, 1.0).add_case({20.0}, 1.0).add_case({30.0}, 1.0);
}

void EvalSpec::add_boolean_cases() {
    add_expression({"a"}, "(!a)")
...
void EvalSpec::add_if_cases() {
    add_expression({"a"}, "if(a,1,0)")
        .add_cases({my_nan, -my_inf, -123.0, -1.0, -0.001, 0.0, 0.001, 1.0, 123.0, my_inf}, [](double a) {
            if (a) {
                return 1.0;
            } else {
                return 0.0;
    ...
    add_expression({"a", "b"}, "if(a,if(b,1,2),if(b,3,4))")
        .add_cases({my_nan, -my_inf, -123.0, -1.0, -0.001, 0.0, 0.001, 1.0, 123.0, my_inf},
                   {my_nan, -my_inf, -123.0, -1.0, -0.001, 0.0, 0.001, 1.0, 123.0, my_inf}, [](double a, double b) {
                       if (a) {
                           if (b) {
                               return 1.0;
                           } else {
...
void EvalSpec::add_complex_cases() {
    add_expression({"a", "b"}, "((a<3)||b)").add_cases({2.0, 4.0}, {0.0, 0.5, 1.0}, [](double a, double b) -> double {
        return ((a < 3) || bool(b));
    });
