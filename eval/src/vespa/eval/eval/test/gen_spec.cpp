...
std::vector<std::string> DimSpec::make_dict(size_t size, size_t stride, const std::string& prefix) {
...
namespace {
auto is_dim_name = [](char c) { return ((c >= 'a') && (c <= 'z')) || ((c >= 'A') && (c <= 'Z')); };
} // namespace

// 'a2' -> DimSpec("a", 2);
// 'b2_3' -> DimSpec("b", make_dict(2, 3, ""));
DimSpec DimSpec::from_desc(const std::string& desc) {
    size_t      idx = 0;
    std::string name;
...
GenSpec GenSpec::from_desc(const std::string& desc) {
    size_t               idx = 0;
    std::string          dim_desc;
...
bool GenSpec::bad_scalar() const {
    return (_dims.empty() && (_cells != CellType::DOUBLE));
}

ValueType GenSpec::type() const {
