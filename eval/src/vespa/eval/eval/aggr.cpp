...
template <typename T> struct Wrapper final : Aggregator {
    T aggr;
...
void AggrNames::add(Aggr aggr, const std::string& name) {
    _name_aggr_map[name] = aggr;
    _aggr_name_map[aggr] = name;
}

AggrNames::AggrNames() : _name_aggr_map(), _aggr_name_map() {
    add(Aggr::AVG, "avg");
...
const std::string* AggrNames::name_of(Aggr aggr) {
...
const Aggr* AggrNames::from_name(const std::string& name) {
...
Aggregator& Aggregator::create(Aggr aggr, Stash& stash) {
...
std::vector<Aggr> Aggregator::list() {
    return std::vector<Aggr>({Aggr::AVG, Aggr::COUNT, Aggr::PROD, Aggr::SUM, Aggr::MAX, Aggr::MEDIAN, Aggr::MIN});
}
