...
public:
    State(const std::vector<ValueType>& params, std::map<const Node*, ValueType>& type_map,
...
struct TypeResolver : public NodeVisitor, public NodeTraverser {
    State state;
    TypeResolver(const std::vector<ValueType>& params_in, std::map<const Node*, ValueType>& type_map_out,
                 std::vector<std::string>& errors_out);
    ~TypeResolver() override;

    const ValueType& param_type(size_t idx) { return state.param_type(idx); }
    ...
    const ValueType& type(const Node& node) { return state.type(node); }
    ...
    void import_types(const NodeTypes& types) {
        types.each([&](const Node& node, const ValueType& type) { state.bind(type, node); });
    }
    ...
    void resolve_op1(const Node& node) { bind(type(node.get_child(0)).map(), node); }

    void resolve_op2(const Node& node) {
        bind(ValueType::join(type(node.get_child(0)), type(node.get_child(1))), node);
    }

    //-------------------------------------------------------------------------

    void visit(const Number& node) override { bind(ValueType::double_type(), node); }
    void visit(const Symbol& node) override { bind(param_type(node.id()), node, false); }
    void visit(const String& node) override { bind(ValueType::double_type(), node); }
    ...
    void visit(const If& node) override {
        bind(ValueType::either(type(node.true_expr()), type(node.false_expr())), node);
    }
    void visit(const Error& node) override { bind(ValueType::error_type(), node, false); }
    ...
    void visit(const TensorMapSubspaces& node) override {
        ...
        if (lambda_res.count_mapped_dimensions() > 0) {
            return fail(node,
                        fmt("lambda function result contains mapped dimensions: %s", lambda_res.to_spec().c_str()),
                        false);
        }
        auto res_type = outer_type.wrap(lambda_res);
        if (res_type.is_error()) {
            return fail(node,
                        fmt("lambda result contains dimensions that conflict with input type: %s <-> %s",
                            lambda_res.to_spec().c_str(), in_type.to_spec().c_str()),
                        false);
        }
    ...
    void visit(const TensorMerge& node) override { bind(ValueType::merge(type(node.lhs()), type(node.rhs())), node); }
    void visit(const TensorReduce& node) override {
        auto my_type = type(node.child()).reduce(node.dimensions());
        if (my_type.is_error()) {
            auto   str = fmt("aggr: %s, dimensions: [", AggrNames::name_of(node.aggr())->c_str());
            size_t i = 0;
    ...
    void visit(const TensorRename& node) override {
        ...
        if (my_type.is_error()) {
            auto str =
                fmt("%s -> %s", TensorRename::flatten(node.from()).c_str(), TensorRename::flatten(node.to()).c_str());
            fail(node, str);
        } else {
            bind(my_type, node);
        }
    ...
    void visit(const TensorCellCast& node) override { bind(type(node.child()).cell_cast(node.cell_type()), node); }
    ...
    void visit(const TensorLambda& node) override {
        ...
        if (!lambda_type.is_double()) {
            import_errors(lambda_types);
            return fail(node, fmt("lambda function has non-double result type: %s", lambda_type.to_spec().c_str()),
                        false);
        }
        import_types(lambda_types);
        bind(node.type(), node);
            ...
            if (dim.second.is_expr()) {
                ...
                if (param_dim.is_indexed()) {
                    if (!is_number(dim.second.label)) {
                        return fail(node, fmt("non-numeric label for dimension %s: '%s'", dim.first.c_str(),
                                              dim.second.label.c_str()));
                    }
                    if (as_number(dim.second.label) >= param_dim.size) {
                        return fail(node, fmt("out-of-bounds label for dimension %s: %s", dim.first.c_str(),
                                              dim.second.label.c_str()));
                    }
                }
            }
...
TypeResolver::TypeResolver(const std::vector<ValueType>& params_in, std::map<const Node*, ValueType>& type_map_out,
                           std::vector<std::string>& errors_out)
...
struct TypeExporter : public NodeTraverser {
    ...
    TypeExporter(const std::map<const Node*, ValueType>& parent_type_map_in,
        ...
        : parent_type_map(parent_type_map_in), exported_type_map(exported_type_map_out), missing_cnt(0) {}
...
NodeTypes::NodeTypes() : _not_found(ValueType::error_type()), _type_map() {
}

NodeTypes::NodeTypes(const nodes::Node& const_node) : _not_found(ValueType::error_type()), _type_map() {
...
NodeTypes NodeTypes::export_types(const nodes::Node& root) const {
...
const ValueType& NodeTypes::get_type(const nodes::Node& node) const {
    auto pos = _type_map.find(&node);
