...
inline CellType id_to_cell_type(uint32_t id) {
    switch (id) {
    case DOUBLE_CELL_TYPE:
        return CellType::DOUBLE;
    case FLOAT_CELL_TYPE:
        return CellType::FLOAT;
    case BFLOAT16_CELL_TYPE:
...
ValueType decode_type(nbostream& input, const Format& format) {
    ...
    if (result.is_error()) {
        throw IllegalArgumentException(
            fmt("Invalid type (with %zu dimensions and cell type %u)", dim_list.size(), (uint32_t)cell_type));
    }
    return result;
}
...
struct ContentDecoder {
    template <typename T>
    static std::unique_ptr<Value> invoke(nbostream& input, const DecodeState& state,
        ...
        auto builder =
            factory.create_value_builder<T>(state.type, state.num_mapped_dims, state.subspace_size, state.num_blocks);
...
struct CreateValueFromTensorSpec {
    template <typename T>
    static std::unique_ptr<Value> invoke(const ValueType& type, const TensorSpec& spec,
                                         const ValueBuilderFactory& factory) {
        ...
        auto builder =
            factory.create_value_builder<T>(type, map.keys_per_entry(), map.values_per_entry(), map.size());
    ...
    template <typename T> static TensorSpec invoke(const Value& value) {
        ...
        while (view->next_result(label_refs, subspace_id)) {
            ...
            for (size_t i = 0; i < subspace_size; ++i) {
                size_t dense_key = i;
                for (auto dim = value.type().dimensions().rbegin(); dim != value.type().dimensions().rend(); ++dim) {
                    if (dim->is_indexed()) {
...
std::unique_ptr<Value> value_from_spec(const TensorSpec& spec, const ValueBuilderFactory& factory) {
    ...
    if (type.is_error()) {
        throw IllegalArgumentException(fmt("Failed decoding value type from tensorspec(%s)", spec.type().c_str()),
                                       VESPA_STRLOC);
    }
