...
ReplaceTypeFunction::ReplaceTypeFunction(const ValueType& result_type, const TensorFunction& child)
...
InterpretedFunction::Instruction ReplaceTypeFunction::compile_self(const ValueBuilderFactory&, Stash&) const {
    return InterpretedFunction::Instruction(my_replace_type_op, wrap_param<ValueType>(result_type()));
}

const ReplaceTypeFunction& ReplaceTypeFunction::create_compact(const ValueType&      result_type,
                                                               const TensorFunction& child, Stash& stash) {
