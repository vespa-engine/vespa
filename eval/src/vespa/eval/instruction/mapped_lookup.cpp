...
template <typename CT>
std::span<const CT> my_mapped_lookup_fallback(const Value::Index& key_idx, const Value::Index& map_idx,
                                              const CT* key_cells, const CT* map_cells, size_t res_size,
                                              Stash& stash) {
...
template <typename CT> struct MappedLookupResult {
    std::span<CT> value;
    MappedLookupResult(size_t res_size, Stash& stash) : value(stash.create_array<CT>(res_size)) {}
...
template <typename CT>
std::span<const CT> my_fast_mapped_lookup(const FastAddrMap& key_map, const FastAddrMap& map_map, const CT* key_cells,
                                          const CT* map_cells, size_t res_size, Stash& stash) {
...
template <typename CT> void my_mapped_lookup_op(InterpretedFunction::State& state, uint64_t param) {
    ...
    auto        result = __builtin_expect(are_fast(key_idx, map_idx), true)
                             ? my_fast_mapped_lookup<CT>(as_fast(key_idx).map, as_fast(map_idx).map, key_cells, map_cells,
                                                         res_type.dense_subspace_size(), state.stash)
                             : my_mapped_lookup_fallback<CT>(key_idx, map_idx, key_cells, map_cells,
                                                             res_type.dense_subspace_size(), state.stash);
...
bool check_types(const ValueType& res, const ValueType& key, const ValueType& map) {
    return ((res.is_dense()) && (key.dense_subspace_size() == 1) && (map.is_mixed()) &&
            (res.cell_type() == key.cell_type()) && (res.cell_type() == map.cell_type()) &&
            ((res.cell_type() == CellType::FLOAT) || (res.cell_type() == CellType::DOUBLE)) &&
            (key.mapped_dimensions().size() == 1) && (key.mapped_dimensions() == map.mapped_dimensions()) &&
            (map.nontrivial_indexed_dimensions() == res.nontrivial_indexed_dimensions()));
}

} // namespace

MappedLookup::MappedLookup(const ValueType& res_type, const TensorFunction& key_in, const TensorFunction& map_in)
    : tensor_function::Op2(res_type, key_in, map_in) {
}

InterpretedFunction::Instruction MappedLookup::compile_self(const ValueBuilderFactory&, Stash&) const {
...
const TensorFunction& MappedLookup::optimize(const TensorFunction& expr, Stash& stash) {
