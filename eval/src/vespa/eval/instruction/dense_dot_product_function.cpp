...
template <typename LCT, typename RCT> void my_dot_product_op(InterpretedFunction::State& state, uint64_t) {
...
struct MyDotProductOp {
    template <typename LCT, typename RCT> static auto invoke() { return my_dot_product_op<LCT, RCT>; }
};

} // namespace

DenseDotProductFunction::DenseDotProductFunction(const TensorFunction& lhs_in, const TensorFunction& rhs_in)
    : tensor_function::Op2(ValueType::double_type(), lhs_in, rhs_in) {
}

InterpretedFunction::Instruction DenseDotProductFunction::compile_self(const ValueBuilderFactory&, Stash&) const {
...
bool DenseDotProductFunction::compatible_types(const ValueType& res, const ValueType& lhs, const ValueType& rhs) {
    return (res.is_double() && lhs.is_dense() && (rhs.dimensions() == lhs.dimensions()));
}

const TensorFunction& DenseDotProductFunction::optimize(const TensorFunction& expr, Stash& stash) {
