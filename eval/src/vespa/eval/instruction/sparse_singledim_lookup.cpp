...
template <typename CT> void my_sparse_singledim_lookup_op(InterpretedFunction::State& state, uint64_t) {
...
struct MyGetFun {
    template <typename CT> static auto invoke() { return my_sparse_singledim_lookup_op<CT>; }
};

} // namespace

SparseSingledimLookup::SparseSingledimLookup(const TensorFunction& tensor, const TensorFunction& expr)
    : tensor_function::Op2(ValueType::double_type(), tensor, expr) {
}

InterpretedFunction::Instruction SparseSingledimLookup::compile_self(const ValueBuilderFactory&, Stash&) const {
    auto op = typify_invoke<1, TypifyCellType, MyGetFun>(lhs().result_type().cell_type());
    return InterpretedFunction::Instruction(op);
}

const TensorFunction& SparseSingledimLookup::optimize(const TensorFunction& expr, Stash& stash) {
    auto peek = as<Peek>(expr);
    if (peek && peek->result_type().is_double() && peek->param_type().is_sparse() &&
        (peek->param_type().dimensions().size() == 1) && (peek->map().size() == 1) &&
        (std::holds_alternative<TensorFunction::Child>(peek->map().begin()->second)))
    {
        return stash.create<SparseSingledimLookup>(
            peek->param(), std::get<TensorFunction::Child>(peek->map().begin()->second).get());
    }
    return expr;
}
