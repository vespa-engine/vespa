...
bool compatible_types(const ValueType& lhs, const ValueType& rhs) {
    return ((lhs.cell_type() == CellType::INT8) && (rhs.cell_type() == CellType::INT8) && lhs.is_dense() &&
            rhs.is_dense() && (lhs.nontrivial_indexed_dimensions() == rhs.nontrivial_indexed_dimensions()));
}

} // namespace

DenseHammingDistance::DenseHammingDistance(const TensorFunction& lhs_child, const TensorFunction& rhs_child)
    : tensor_function::Op2(ValueType::double_type(), lhs_child, rhs_child) {
}

InterpretedFunction::Instruction DenseHammingDistance::compile_self(const ValueBuilderFactory&, Stash&) const {
...
const TensorFunction& DenseHammingDistance::optimize(const TensorFunction& expr, Stash& stash) {
