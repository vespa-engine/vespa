...
bool check_types(const ValueType& res, const ValueType& lhs, const ValueType& rhs) {
    return ((res.is_double()) && (lhs.count_mapped_dimensions() == 1) &&
            (lhs.mapped_dimensions() == rhs.mapped_dimensions()));
}

} // namespace

SimpleJoinCount::SimpleJoinCount(const TensorFunction& lhs_in, const TensorFunction& rhs_in, uint64_t dense_factor_in)
    : tensor_function::Op2(ValueType::double_type(), lhs_in, rhs_in), _dense_factor(dense_factor_in) {
}

InterpretedFunction::Instruction SimpleJoinCount::compile_self(const ValueBuilderFactory&, Stash&) const {
    return InterpretedFunction::Instruction(my_simple_join_count_op, _dense_factor);
}

const TensorFunction& SimpleJoinCount::optimize(const TensorFunction& expr, Stash& stash) {
