...
Value::UP generic_reduce(const Value& value, const ReduceParam& param) {
    auto                           cells = value.cells().typify<ICT>();
    ArrayArrayMap<string_id, AGGR> map(param.sparse_plan.keep_dims.size(), param.dense_plan.out_size,
                                       value.index().size());
    ...
    auto builder = param.factory.create_transient_value_builder<OCT>(
        param.res_type, param.sparse_plan.keep_dims.size(), param.dense_plan.out_size, map.size());
...
template <typename ICT, typename OCT, typename AGGR> void my_generic_reduce_op(State& state, uint64_t param_in) {
...
void my_generic_dense_reduce_op(State& state, uint64_t param_in) {
    ...
    if (num_subspaces > 0) {
        if constexpr (aggr::is_simple(AGGR::enum_value())) {
            ...
            auto combine = [&](size_t src_idx, size_t dst_idx) {
                dst[dst_idx] = AGGR::combine(dst[dst_idx], cells[src_idx]);
            };
...
template <typename ICT> void my_count_cells_op(State& state, uint64_t) {
    auto cells = state.peek(0).cells().typify<ICT>();
    state.pop_push(state.stash.create<DoubleValue>(cells.size()));
};

template <typename ICT, typename AGGR> void my_full_reduce_op(State& state, uint64_t) {
    auto cells = state.peek(0).cells().typify<ICT>();
    if (cells.size() >= 8) {
...
DenseReducePlan::DenseReducePlan(const ValueType& type, const ValueType& res_type)
    : in_size(1), out_size(1), loop_cnt(), in_stride(), out_stride() {
    ...
    auto visitor = overload{[&](visit_ranges_either, const auto& a) { update_plan(Case::REDUCE, a.size); },
...
bool SparseReducePlan::should_forward_index() const {
...
Instruction GenericReduce::make_instruction(const ValueType& result_type, const ValueType& input_type, Aggr aggr,
