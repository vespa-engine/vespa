...
struct ConcatParam {
    ...
    ConcatParam(const ValueType& res_type_in, const ValueType& lhs_type, const ValueType& rhs_type,
...
std::unique_ptr<Value> generic_concat(const Value& a, const Value& b, const SparseJoinPlan& sparse_plan,
                                      const DenseConcatPlan& dense_plan, const ValueType& res_type,
                                      const ValueBuilderFactory& factory) {
    auto            a_cells = a.cells().typify<LCT>();
    auto            b_cells = b.cells().typify<RCT>();
    SparseJoinState sparse(sparse_plan, a.index(), b.index());
    auto            builder = factory.create_transient_value_builder<OCT>(res_type, sparse_plan.sources.size(),
                                                                          dense_plan.output_size, sparse.first_index.size());
...
template <typename LCT, typename RCT, typename OCT> void my_generic_concat_op(State& state, uint64_t param_in) {
    const auto&  param = unwrap_param<ConcatParam>(param_in);
    const Value& lhs = state.peek(1);
    const Value& rhs = state.peek(0);
    auto         res_value =
        generic_concat<LCT, RCT, OCT>(lhs, rhs, param.sparse_plan, param.dense_plan, param.res_type, param.factory);
...
template <typename LCT, typename RCT, typename OCT> void my_dense_simple_concat_op(State& state, uint64_t param_in) {
    const auto&    param = unwrap_param<ConcatParam>(param_in);
    const Value&   lhs = state.peek(1);
    const Value&   rhs = state.peek(0);
...
std::pair<size_t, size_t> DenseConcatPlan::InOutLoop::fill_from(const ValueType& in_type,
    ...
    auto       visitor = overload{[&](visit_ranges_first, const auto&) { abort(); },
                            [&](visit_ranges_second, const auto& b) {
                                if (b.name == concat_dimension) {
                                    update_plan(Case::CONCAT, 1, b.size, 0, 1);
                                } else {
                                    update_plan(Case::OUT, b.size, b.size, 0, 1);
                                }
                            },
                            [&](visit_ranges_both, const auto& a, const auto& b) {
                                if (b.name == concat_dimension) {
                                    update_plan(Case::CONCAT, a.size, b.size, 1, 1);
                                } else {
                                    update_plan(Case::BOTH, a.size, b.size, 1, 1);
                                }
                            }};
    ...
    visit_ranges(visitor, input_dimensions.begin(), input_dimensions.end(), output_dimensions.begin(),
                 output_dimensions.end(), [](const auto& a, const auto& b) { return (a.name < b.name); });
    input_size = 1;
...
DenseConcatPlan::DenseConcatPlan(const ValueType& lhs_type, const ValueType& rhs_type, std::string concat_dimension,
                                 const ValueType& out_type) {
...
InterpretedFunction::Instruction GenericConcat::make_instruction(const ValueType& result_type,
