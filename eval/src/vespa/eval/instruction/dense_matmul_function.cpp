...
void my_matmul_op(InterpretedFunction::State& state, uint64_t param) {
    ...
    for (size_t i = 0; i < self.lhs_size; ++i) {
        ...
        for (size_t j = 0; j < self.rhs_size; ++j) {
            *dst++ = my_dot_product<LCT, RCT, OCT, lhs_common_inner, rhs_common_inner>(
                lhs, rhs, self.lhs_size, self.common_size, self.rhs_size);
...
void my_cblas_double_matmul_op(InterpretedFunction::State& state, uint64_t param) {
    ...
    cblas_dgemm(CblasRowMajor, lhs_common_inner ? CblasNoTrans : CblasTrans,
                rhs_common_inner ? CblasTrans : CblasNoTrans, self.lhs_size, self.rhs_size, self.common_size, 1.0,
                lhs_cells.data(), lhs_common_inner ? self.common_size : self.lhs_size, rhs_cells.data(),
                rhs_common_inner ? self.common_size : self.rhs_size, 0.0, dst_cells.data(), self.rhs_size);
    state.pop_pop_push(state.stash.create<DenseValueView>(self.result_type, TypedCells(dst_cells)));
}
...
void my_cblas_float_matmul_op(InterpretedFunction::State& state, uint64_t param) {
    ...
    cblas_sgemm(CblasRowMajor, lhs_common_inner ? CblasNoTrans : CblasTrans,
                rhs_common_inner ? CblasTrans : CblasNoTrans, self.lhs_size, self.rhs_size, self.common_size, 1.0,
                lhs_cells.data(), lhs_common_inner ? self.common_size : self.lhs_size, rhs_cells.data(),
                rhs_common_inner ? self.common_size : self.rhs_size, 0.0, dst_cells.data(), self.rhs_size);
    state.pop_pop_push(state.stash.create<DenseValueView>(self.result_type, TypedCells(dst_cells)));
...
bool is_matmul(const ValueType& a, const ValueType& b, const std::string& reduce_dim, const ValueType& result_type) {
    size_t npos = ValueType::Dimension::npos;
    return (is_matrix(a) && is_matrix(b) && is_matrix(result_type) && (a.dimension_index(reduce_dim) != npos) &&
...
size_t inv(size_t idx) {
    return (1 - idx);
}

const TensorFunction& create_matmul(const TensorFunction& a, const TensorFunction& b, const std::string& reduce_dim,
    ...
    if (a_is_lhs) {
        return stash.create<DenseMatMulFunction>(result_type, a, b, a_size, common_size, b_size, a_common_inner,
                                                 b_common_inner);
    } else {
        return stash.create<DenseMatMulFunction>(result_type, b, a, b_size, common_size, a_size, b_common_inner,
                                                 a_common_inner);
    }
}
...
DenseMatMulFunction::Self::Self(const ValueType& result_type_in, size_t lhs_size_in, size_t common_size_in,
                                size_t rhs_size_in)
    : result_type(result_type_in), lhs_size(lhs_size_in), common_size(common_size_in), rhs_size(rhs_size_in) {
}

DenseMatMulFunction::Self::~Self() = default;

DenseMatMulFunction::DenseMatMulFunction(const ValueType& result_type, const TensorFunction& lhs_in,
                                         const TensorFunction& rhs_in, size_t lhs_size, size_t common_size,
                                         size_t rhs_size, bool lhs_common_inner, bool rhs_common_inner)
...
InterpretedFunction::Instruction DenseMatMulFunction::compile_self(const ValueBuilderFactory&, Stash& stash) const {
    ...
    auto  op = typify_invoke<4, MyTypify, SelectDenseMatmul>(lhs().result_type().cell_meta().not_scalar(),
...
void DenseMatMulFunction::visit_self(vespalib::ObjectVisitor& visitor) const {
...
const TensorFunction& DenseMatMulFunction::optimize(const TensorFunction& expr, Stash& stash) {
