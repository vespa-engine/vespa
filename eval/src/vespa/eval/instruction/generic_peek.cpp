...
struct DimSpec {
    ...
    Handle  str;
    size_t  idx;
    static DimSpec from_child(size_t child_idx) { return {DimType::CHILD_IDX, Handle(), child_idx}; }
                                ...
                                [&](visit_ranges_both, const auto& a, const auto& b) {
                                    if (a.is_indexed() == indexed) {
                                        ...
                                        if (std::holds_alternative<size_t>(child_or_label)) {
                                            ...
                                            specs[a.name] =
                                                DimSpec::from_label(std::get<TensorSpec::Label>(child_or_label));
                                        }
                                    }
                                }};
        visit_ranges(visitor, input_dims.begin(), input_dims.end(), spec.begin(), spec.end(), MyComp());
    }
    ~ExtractedSpecs();
};
...
struct DensePlan {
    ...
    template <typename Getter> size_t get_offset(const Getter& get_child_value) const {
...
struct SparsePlan {
    ...
    SparsePlan(const ValueType& input_type, const GenericPeek::SpecMap& spec) : out_mapped_dims(0), view_dims() {
    ...
    template <typename Getter> SparseState make_state(const Getter& get_child_value) const {
        SmallVector<Handle>    handles;
        SmallVector<string_id> view_addr;
...
struct PeekParam {
    ...
    PeekParam(const ValueType& res_type_in, const ValueType& input_type, const GenericPeek::SpecMap& spec_in,
...
Value::UP generic_mixed_peek(const ValueType& res_type, const Value& input_value, const SparsePlan& sparse_plan,
                             const DensePlan& dense_plan, const ValueBuilderFactory& factory,
                             const Getter& get_child_value) {
    auto   input_cells = input_value.cells().typify<ICT>();
    size_t bad_guess = 1;
    auto   builder = factory.create_transient_value_builder<OCT>(res_type, sparse_plan.out_mapped_dims,
                                                                 dense_plan.out_dense_size, bad_guess);
    ...
    if (dense_offset != npos) {
        ...
        while (view->next_result(state.fetch_addr, input_subspace)) {
            ...
            dense_plan.execute(dense_offset + input_offset, [&](size_t idx) { *dst++ = input_cells[idx]; });
            ++filled_subspaces;
        }
    }
...
template <typename ICT, typename OCT> void my_generic_peek_op(State& state, uint64_t param_in) {
    ...
    auto         up = generic_mixed_peek<ICT, OCT>(param.res_type, input_value, param.sparse_plan, param.dense_plan,
...
Instruction GenericPeek::make_instruction(const ValueType& result_type, const ValueType& input_type,
                                          const SpecMap& spec, const ValueBuilderFactory& factory, Stash& stash) {
    ...
    auto        fun = typify_invoke<2, PeekTypify, SelectGenericPeekOp>(input_type.cell_meta().not_scalar(),
                                                                        result_type.is_double());
