...
struct CreateParam {
    ...
    CreateParam(const ValueType& res_type, const GenericCreate::SpecMap& spec_in)
        : my_spec(res_type, res_type.count_mapped_dimensions(), res_type.dense_subspace_size(),
...
template <typename CT> void my_generic_create_op(State& state, uint64_t param_in) {
    ...
    for (uint32_t stack_idx : spec) {
        *dst++ = ((stack_idx != CreateParam::npos) ? (CT)state.peek(stack_idx).as_double() : CT{});
    }
    const Value& result =
        state.stash.create<ValueView>(param.my_spec.type(), param.my_spec.my_index, TypedCells(cells));
    state.pop_n_push(param.num_children, result);
};

struct SelectGenericCreateOp {
    template <typename CT> static auto invoke() { return my_generic_create_op<CT>; }
};
...
Instruction GenericCreate::make_instruction(const ValueType& result_type, const SpecMap& spec,
                                            const ValueBuilderFactory&, Stash&           stash) {
