...
struct MixedInnerProductParam {
    ...
    MixedInnerProductParam(const ValueType& res_type_in, const ValueType& mix_type, const ValueType& vec_type)
...
struct SelectMixedInnerProduct {
    template <typename MCT, typename VCT, typename OCT> static auto invoke() {
        return my_mixed_inner_product_op<MCT, VCT, OCT>;
    }
};

} // namespace

MixedInnerProductFunction::MixedInnerProductFunction(const ValueType& res_type_in, const TensorFunction& mixed_child,
                                                     const TensorFunction& vector_child)
    : tensor_function::Op2(res_type_in, mixed_child, vector_child) {
}

InterpretedFunction::Instruction MixedInnerProductFunction::compile_self(const ValueBuilderFactory&,
                                                                         Stash& stash) const {
    ...
    auto op = typify_invoke<3, MyTypify, SelectMixedInnerProduct>(mix_type.cell_type(), vec_type.cell_type(),
                                                                  result_type().cell_type());
    return InterpretedFunction::Instruction(op, wrap_param<MixedInnerProductParam>(param));
}

bool MixedInnerProductFunction::compatible_types(const ValueType& res, const ValueType& mixed,
...
const TensorFunction& MixedInnerProductFunction::optimize(const TensorFunction& expr, Stash& stash) {
