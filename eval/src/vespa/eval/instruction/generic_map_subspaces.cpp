...
public:
    ...
    template <typename ICT> void adjust(const ICT* cells, size_t size) {
        if (direct) {
...
template <typename OCT> struct ResultFiller {
    OCT* dst;
    bool direct;

public:
    ResultFiller(OCT* dst_in, bool direct_out) : dst(dst_in), direct(direct_out) {}
...
struct SelectGenericMapSubspacesOp {
    template <typename ICT, typename OCT> static auto invoke() { return my_generic_map_subspaces_op<ICT, OCT>; }
};
...
Instruction GenericMapSubspaces::make_instruction(const tensor_function::MapSubspaces& map_subspaces_in,
                                                  const ValueBuilderFactory& factory, Stash& stash) {
    InterpretedParams& params = stash.create<InterpretedParams>(map_subspaces_in, factory);
    auto               op = typify_invoke<2, TypifyCellType, SelectGenericMapSubspacesOp>(
        map_subspaces_in.child().result_type().cell_type(), params.result_type.cell_type());
