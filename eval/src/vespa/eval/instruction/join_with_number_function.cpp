...
struct SelectJoinWithNumberOp {
    template <typename CM, typename Fun, typename PrimaryMutable, typename NumberWasLeft> static auto invoke() {
...
bool JoinWithNumberFunction::primary_is_mutable() const {
...
InterpretedFunction::Instruction JoinWithNumberFunction::compile_self(const ValueBuilderFactory&,
                                                                      Stash& stash) const {
...
void JoinWithNumberFunction::visit_self(vespalib::ObjectVisitor& visitor) const {
...
const TensorFunction& JoinWithNumberFunction::optimize(const TensorFunction& expr, Stash& stash) {
