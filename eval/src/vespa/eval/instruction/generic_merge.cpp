...
std::unique_ptr<Value> generic_mixed_merge(const Value& a, const Value& b, const MergeParam& params) {
    ...
    auto builder = params.factory.create_transient_value_builder<OCT>(params.res_type, num_mapped, subspace_size,
                                                                      guess_subspaces);
...
Instruction GenericMerge::make_instruction(const ValueType& result_type, const ValueType& lhs_type,
