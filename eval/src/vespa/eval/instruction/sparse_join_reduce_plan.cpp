...
Dims merge(const Dims& first, const Dims& second) {
    Dims result;
    auto visitor = overload{[&result](visit_ranges_either, const Dim& dim) { result.push_back(dim); },
...
size_t count_only_in_second(const Dims& first, const Dims& second) {
    size_t result = 0;
    auto   visitor =
        overload{[](visit_ranges_first, const Dim&) {}, [&result](visit_ranges_second, const Dim&) { ++result; },
                 [](visit_ranges_both, const Dim&, const Dim&) {}};
    visit(visitor, first, second);
    return result;
}

size_t est_1(size_t, size_t) noexcept {
    return 1;
}
size_t est_a_or_0(size_t a, size_t b) noexcept {
    return (b == 0) ? 0 : a;
}
size_t est_b_or_0(size_t a, size_t b) noexcept {
    return (a == 0) ? 0 : b;
}
size_t est_min(size_t a, size_t b) noexcept {
    return std::min(a, b);
}
size_t est_mul(size_t a, size_t b) noexcept {
    return (a * b);
}

bool reduce_all(bool, bool, bool keep) noexcept {
    return !keep;
}
bool keep_a_reduce_b(bool a, bool b, bool keep) noexcept {
    return (keep == a) && (keep != b);
}
bool keep_b_reduce_a(bool a, bool b, bool keep) noexcept {
    return (keep == b) && (keep != a);
}
bool no_overlap_keep_all(bool a, bool b, bool keep) noexcept {
    return keep && (a != b);
}
...
bool SparseJoinReducePlan::maybe_forward_lhs_index() const {
    return check(keep_a_reduce_b);
}

bool SparseJoinReducePlan::maybe_forward_rhs_index() const {
