...
const Value& my_fast_sparse_merge(const FastAddrMap& a_map, const FastAddrMap& b_map, const CT* a_cells,
                                  const CT* b_cells, const MergeParam& params, Stash& stash) {
...
void my_sparse_merge_op(InterpretedFunction::State& state, uint64_t param_in) {
    ...
    if (__builtin_expect(are_fast(a_idx, b_idx), true)) {
        ...
        const Value& v = my_fast_sparse_merge<CT, single_dim, Fun>(
            as_fast(a_idx).map, as_fast(b_idx).map, a_cells.data(), b_cells.data(), param, state.stash);
...
struct SelectSparseMergeOp {
    template <typename R1, typename SINGLE_DIM, typename Fun> static auto invoke() {
...
InterpretedFunction::Instruction SparseMergeFunction::compile_self(const ValueBuilderFactory& factory,
                                                                   Stash&                     stash) const {
    const auto& param =
        stash.create<MergeParam>(result_type(), lhs().result_type(), rhs().result_type(), function(), factory);
    size_t num_dims = result_type().count_mapped_dimensions();
    auto   op =
        typify_invoke<3, MyTypify, SelectSparseMergeOp>(result_type().cell_meta().limit(), num_dims == 1, function());
    return InterpretedFunction::Instruction(op, wrap_param<MergeParam>(param));
}

bool SparseMergeFunction::compatible_types(const ValueType& res, const ValueType& lhs, const ValueType& rhs) {
...
const TensorFunction& SparseMergeFunction::optimize(const TensorFunction& expr, Stash& stash) {
