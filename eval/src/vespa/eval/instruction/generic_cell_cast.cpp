...
template <typename ICT, typename OCT> void my_generic_cell_cast_op(State& state, uint64_t param_in) {
...
template <> void my_generic_cell_cast_op<BFloat16, float>(State& state, uint64_t param_in) {
    const auto&  res_type = unwrap_param<ValueType>(param_in);
    const Value& a = state.peek(0);
...
struct SelectGenericCellCastOp {
    template <typename ICT, typename OCT> static InterpretedFunction::op_function invoke() {
