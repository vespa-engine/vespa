...
void my_simple_expand_op(State& state, uint64_t param) {
    ...
    DCT*                dst = dst_cells.data();
    for (OCT outer_cell : outer_cells) {
        apply_op2_vec_num(dst, inner_cells.data(), outer_cell, inner_cells.size(), my_op);
...
struct SelectDenseSimpleExpand {
    template <typename LCM, typename RCM, typename Fun, typename RhsInner> static auto invoke() {
...
DenseSimpleExpandFunction::DenseSimpleExpandFunction(const ValueType& result_type, const TensorFunction& lhs,
                                                     const TensorFunction& rhs, join_fun_t function_in,
                                                     Inner inner_in)
    : Join(result_type, lhs, rhs, function_in), _inner(inner_in) {
}

DenseSimpleExpandFunction::~DenseSimpleExpandFunction() = default;

Instruction DenseSimpleExpandFunction::compile_self(const ValueBuilderFactory&, Stash& stash) const {
...
const TensorFunction& DenseSimpleExpandFunction::optimize(const TensorFunction& expr, Stash& stash) {
