...
const Value& my_fast_no_overlap_sparse_join(const FastAddrMap& lhs_map, const FastAddrMap& rhs_map,
    ...
    auto&       result =
        stash.create<FastValue<CT, true>>(param.res_type, num_mapped_dims, 1, lhs_map.size() * rhs_map.size());
...
void my_sparse_no_overlap_join_op(InterpretedFunction::State& state, uint64_t param_in) {
    ...
    if (__builtin_expect(are_fast(lhs_idx, rhs_idx), true)) {
        const Value& res = my_fast_no_overlap_sparse_join<CT, Fun>(
...
struct SelectSparseNoOverlapJoinOp {
    template <typename R1, typename Fun> static auto invoke() {
...
bool SparseNoOverlapJoinFunction::compatible_types(const ValueType& res, const ValueType& lhs, const ValueType& rhs) {
...
const TensorFunction& SparseNoOverlapJoinFunction::optimize(const TensorFunction& expr, Stash& stash) {
