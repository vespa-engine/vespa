...
struct Strides {
    ...
    Strides(size_t lhs_in, size_t rhs_in, size_t res_in) noexcept : lhs(lhs_in), rhs(rhs_in), res(res_in) {}
    bool can_combine_with(const Strides& prev) const noexcept {
        return ((lhs > 0) == (prev.lhs > 0)) && ((rhs > 0) == (prev.rhs > 0)) && ((res > 0) == (prev.res > 0));
    }
};
...
DenseJoinReducePlan::DenseJoinReducePlan(const ValueType& lhs, const ValueType& rhs, const ValueType& res)
    : lhs_size(lhs.dense_subspace_size()),
      rhs_size(rhs.dense_subspace_size()),
      res_size(res.dense_subspace_size()),
      loop_cnt(),
      lhs_stride(),
      rhs_stride(),
      res_stride() {
...
bool DenseJoinReducePlan::is_distinct() const {
