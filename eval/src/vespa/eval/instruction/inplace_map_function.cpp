...
template <typename CT, typename Fun> void my_inplace_map_op(State& state, uint64_t param) {
...
InplaceMapFunction::InplaceMapFunction(const ValueType& result_type, const TensorFunction& child,
...
Instruction InplaceMapFunction::compile_self(const ValueBuilderFactory&, Stash&) const {
...
const TensorFunction& InplaceMapFunction::optimize(const TensorFunction& expr, Stash& stash) {
    if (auto map = as<Map>(expr)) {
        if ((map->result_type() == map->child().result_type()) && (!map->child().result_type().is_double()) &&
