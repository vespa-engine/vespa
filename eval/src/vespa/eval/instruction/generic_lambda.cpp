...
struct ParamProxy : public LazyParams {
    ...
    ParamProxy(const SmallVector<double>& labels_in, const LazyParams& params_in,
...
template <typename CT> void my_compiled_lambda_op(InterpretedFunction::State& state, uint64_t param) {
    ...
    std::span<CT> dst_cells = state.stash.create_uninitialized_array<CT>(params.num_cells);
    CT*           dst = &dst_cells[0];
    do {
        *dst++ = fun(&args[0]);
...
struct MyCompiledLambdaOp {
    template <typename CT> static auto invoke() { return my_compiled_lambda_op<CT>; }
};
...
template <typename CT> void my_interpreted_lambda_op(InterpretedFunction::State& state, uint64_t param) {
...
struct MyInterpretedLambdaOp {
    template <typename CT> static auto invoke() { return my_interpreted_lambda_op<CT>; }
};
