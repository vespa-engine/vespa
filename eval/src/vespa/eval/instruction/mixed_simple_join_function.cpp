...
struct TypifyOverlap {
    ...
    template <typename F> static decltype(auto) resolve(Overlap value, F&& f) {
        switch (value) {
        case Overlap::INNER:
            return f(Result<Overlap::INNER>());
        case Overlap::OUTER:
            return f(Result<Overlap::OUTER>());
        case Overlap::FULL:
            return f(Result<Overlap::FULL>());
        }
        abort();
    }
};
...
struct JoinParams {
    ...
    JoinParams(const ValueType& result_type_in, size_t factor_in, join_fun_t function_in)
        : result_type(result_type_in),
          factor(factor_in),
          subspace_size(result_type.dense_subspace_size()),
...
struct SelectMixedSimpleJoin {
    template <typename LCM, typename RCM, typename R3, typename R4, typename R5, typename R6> static auto invoke() {
...
MixedSimpleJoinFunction::MixedSimpleJoinFunction(const ValueType& result_type, const TensorFunction& lhs,
                                                 const TensorFunction& rhs, join_fun_t function_in,
                                                 Primary primary_in, Overlap overlap_in)
    : Join(result_type, lhs, rhs, function_in), _primary(primary_in), _overlap(overlap_in) {
}

MixedSimpleJoinFunction::~MixedSimpleJoinFunction() = default;

const TensorFunction& MixedSimpleJoinFunction::primary_child() const {
    return (_primary == Primary::LHS) ? lhs() : rhs();
}

bool MixedSimpleJoinFunction::primary_is_mutable() const {
    return primary_child().result_is_mutable();
}

bool MixedSimpleJoinFunction::inplace() const {
    return primary_is_mutable() && (result_type().cell_type() == primary_child().result_type().cell_type());
}

size_t MixedSimpleJoinFunction::factor() const {
...
Instruction MixedSimpleJoinFunction::compile_self(const ValueBuilderFactory&, Stash& stash) const {
    const JoinParams& params = stash.create<JoinParams>(result_type(), factor(), function());
    auto              op = typify_invoke<6, MyTypify, SelectMixedSimpleJoin>(
        lhs().result_type().cell_meta().not_scalar(), rhs().result_type().cell_meta().not_scalar(), function(),
        (_primary == Primary::RHS), _overlap, primary_is_mutable());
    return Instruction(op, wrap_param<JoinParams>(params));
}

const TensorFunction& MixedSimpleJoinFunction::optimize(const TensorFunction& expr, Stash& stash) {
