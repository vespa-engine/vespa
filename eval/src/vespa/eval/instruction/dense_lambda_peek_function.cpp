...
struct MyLambdaPeekOp {
    template <typename DST_CT, typename SRC_CT> static auto invoke() { return my_lambda_peek_op<DST_CT, SRC_CT>; }
};

} // namespace

DenseLambdaPeekFunction::DenseLambdaPeekFunction(const ValueType& result_type, const TensorFunction& child,
                                                 std::shared_ptr<Function const> idx_fun)
    : Op1(result_type, child), _idx_fun(std::move(idx_fun)) {
}

DenseLambdaPeekFunction::~DenseLambdaPeekFunction() = default;

InterpretedFunction::Instruction DenseLambdaPeekFunction::compile_self(const ValueBuilderFactory&,
                                                                       Stash& stash) const {
...
std::string DenseLambdaPeekFunction::idx_fun_dump() const {
