...
template <typename CT> void my_tensor_peek_op(InterpretedFunction::State& state, uint64_t param) {
...
struct MyTensorPeekOp {
    template <typename CT> static auto invoke() { return my_tensor_peek_op<CT>; }
};
...
DenseTensorPeekFunction::DenseTensorPeekFunction(std::vector<Child>                      children,
    ...
    : TensorFunction(), _children(std::move(children)), _spec(std::move(spec)) {
}

DenseTensorPeekFunction::~DenseTensorPeekFunction() = default;

void DenseTensorPeekFunction::push_children(std::vector<Child::CREF>& target) const {
    for (const Child& c : _children) {
        target.emplace_back(c);
    }
}

InterpretedFunction::Instruction DenseTensorPeekFunction::compile_self(const ValueBuilderFactory&, Stash&) const {
...
const TensorFunction& DenseTensorPeekFunction::optimize(const TensorFunction& expr, Stash& stash) {
