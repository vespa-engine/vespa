...
struct MyGetFun {
    template <typename OCT, typename BIG> static auto invoke() { return my_unpack_bits_op<OCT, BIG::value>; }
};
...
bool compatible_types(const ValueType& packed, const ValueType& unpacked) {
    ...
    if ((pdims.size() > 0) && (packed.cell_type() == CellType::INT8) && (packed.is_dense() && unpacked.is_dense()) &&
...
bool is_byte_peek(const TensorPeek& peek, size_t dim_cnt) {
    if (auto param = as<Symbol>(peek.param())) {
        if ((dim_cnt > 0) && (param->id() == dim_cnt) && (peek.dim_list().size() == dim_cnt) &&
...
InterpretedFunction::Instruction UnpackBitsFunction::compile_self(const ValueBuilderFactory&, Stash&) const {
...
const TensorFunction& UnpackBitsFunction::optimize(const TensorFunction& expr, Stash& stash) {
    ...
    if (auto map_subspaces = as<MapSubspaces>(expr)) {
        if (auto lambda = as<TensorLambda>(map_subspaces->lambda().root())) {
            auto result = detect_unpack_bits(lambda->type(), lambda->bindings().size(), lambda->lambda(),
                                             map_subspaces->types());
            if (result.is_unpack_bits) {
                return stash.create<UnpackBitsFunction>(map_subspaces->result_type(), map_subspaces->child(),
