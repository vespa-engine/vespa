...
const std::string& find_rename(const std::string& original, const std::vector<std::string>& from,
...
size_t find_index_of(const std::string& name, const std::vector<ValueType::Dimension>& dims) {
...
struct RenameParam {
    ...
    RenameParam(const ValueType& lhs_type, const std::vector<std::string>& rename_dimension_from,
                const std::vector<std::string>& rename_dimension_to, const ValueBuilderFactory& factory_in)
...
std::unique_ptr<Value> generic_rename(const Value& a, const SparseRenamePlan& sparse_plan,
                                      const DenseRenamePlan& dense_plan, const ValueType& res_type,
                                      const ValueBuilderFactory& factory) {
    ...
    auto builder = factory.create_transient_value_builder<CT>(res_type, sparse_plan.mapped_dims,
                                                              dense_plan.subspace_size, a.index().size());
...
template <typename CT> void my_generic_rename_op(State& state, uint64_t param_in) {
    const auto&  param = unwrap_param<RenameParam>(param_in);
    const Value& a = state.peek(0);
    auto  res_value = generic_rename<CT>(a, param.sparse_plan, param.dense_plan, param.res_type, param.factory);
    auto& result = state.stash.create<std::unique_ptr<Value>>(std::move(res_value));
    const Value& result_ref = *(result.get());
    state.pop_push(result_ref);
}

template <typename CT> void my_mixed_rename_dense_only_op(State& state, uint64_t param_in) {
    const auto&            param = unwrap_param<RenameParam>(param_in);
...
SparseRenamePlan::SparseRenamePlan(const ValueType& input_type, const ValueType& output_type,
...
InterpretedFunction::Instruction GenericRename::make_instruction(
