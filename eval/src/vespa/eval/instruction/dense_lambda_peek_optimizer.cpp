...
template <typename OP> Node_UP make_op(Node_UP a, Node_UP b) {
...
struct PeekAnalyzer {
    ...
    struct CellRange {
        size_t offset;
        size_t length;
        bool is_full(size_t num_cells) const { return ((offset == 0) && (length == num_cells)); }
    };
    ...
    PeekAnalyzer(const ValueType& dst_type, const ValueType& src_type, const TensorPeek::DimList& dim_list) {
...
const TensorFunction& DenseLambdaPeekOptimizer::optimize(const TensorFunction& expr, Stash& stash) {
                ...
                if (result.valid) {
                    ...
                    if (result.cell_range && (dst_type.cell_type() == src_type.cell_type())) {
                        ...
                        if (cell_range.is_full(src_type.dense_subspace_size())) {
                            ...
                            return stash.create<DenseCellRangeFunction>(dst_type, get_param, cell_range.offset,
                                                                        cell_range.length);
                        }
                    } else {
