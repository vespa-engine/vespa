...
template <typename T> double h_sq_l2(const T* a, const T* b, size_t len) {
    return hwaccelerated::squared_euclidean_distance(a, b, len);
}

template <> double h_sq_l2<Int8Float>(const Int8Float* a, const Int8Float* b, size_t len) {
...
struct MultiSelectOp {
    template <typename ICM> static InterpretedFunction::op_function invoke() {
...
bool mixed_compatible_types(const ValueType& res, const ValueType& mix, const ValueType& vec) {
    return ((mix.cell_type() == vec.cell_type()) && vec.is_dense() && res.nontrivial_indexed_dimensions().empty() &&
...
MixedL2Distance::MixedL2Distance(const ValueType& result_type, const TensorFunction& mix_in,
                                 const TensorFunction& vec_in)
    : tensor_function::Op2(result_type, mix_in, vec_in) {
}

InterpretedFunction::Instruction MixedL2Distance::compile_self(const ValueBuilderFactory&, Stash& stash) const {
...
const TensorFunction& MixedL2Distance::optimize(const TensorFunction& expr, Stash& stash) {
