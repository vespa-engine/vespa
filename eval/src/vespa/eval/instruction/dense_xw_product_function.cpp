...
void my_xw_product_op(InterpretedFunction::State& state, uint64_t param) {
    ...
    const RCT*                          matrix = matrix_cells.data();
    for (size_t i = 0; i < self.result_size; ++i) {
        *dst++ = my_dot_product<LCT, RCT, OCT, common_inner>(vector_cells.data(), matrix, self.vector_size,
                                                             self.result_size);
...
template <bool common_inner> void my_cblas_double_xw_product_op(InterpretedFunction::State& state, uint64_t param) {
    const DenseXWProductFunction::Self& self = unwrap_param<DenseXWProductFunction::Self>(param);
    ...
    cblas_dgemv(CblasRowMajor, common_inner ? CblasNoTrans : CblasTrans,
                ...
                common_inner ? self.vector_size : self.result_size, 1.0, matrix_cells.data(),
                common_inner ? self.vector_size : self.result_size, vector_cells.data(), 1, 0.0, dst_cells.data(), 1);
    state.pop_pop_push(state.stash.create<DenseValueView>(self.result_type, TypedCells(dst_cells)));
}

template <bool common_inner> void my_cblas_float_xw_product_op(InterpretedFunction::State& state, uint64_t param) {
    const DenseXWProductFunction::Self& self = unwrap_param<DenseXWProductFunction::Self>(param);
...
bool isDenseXWProduct(const ValueType& res, const ValueType& vec, const ValueType& mat) {
    if (isDenseTensor(res, 1) && isDenseTensor(vec, 1) && isDenseTensor(mat, 2)) {
...
const TensorFunction& createDenseXWProduct(const ValueType& res, const TensorFunction& vec, const TensorFunction& mat,
                                           Stash& stash) {
    bool common_inner = (mat.result_type().dimension_index(vec.result_type().dimensions()[0].name) == 1);
    return stash.create<DenseXWProductFunction>(res, vec, mat, vec.result_type().dimensions()[0].size,
                                                res.dimensions()[0].size, common_inner);
}
...
DenseXWProductFunction::Self::Self(const ValueType& result_type_in, size_t vector_size_in, size_t result_size_in)
    : result_type(result_type_in), vector_size(vector_size_in), result_size(result_size_in) {
}
DenseXWProductFunction::Self::~Self() = default;

DenseXWProductFunction::DenseXWProductFunction(const ValueType& result_type, const TensorFunction& vector_in,
                                               const TensorFunction& matrix_in, size_t vector_size,
                                               size_t result_size, bool common_inner)
...
void DenseXWProductFunction::visit_self(vespalib::ObjectVisitor& visitor) const {
...
const TensorFunction& DenseXWProductFunction::optimize(const TensorFunction& expr, Stash& stash) {
