...
void my_cblas_double_multi_matmul_op(InterpretedFunction::State& state, uint64_t param) {
    ...
    for (size_t i = 0; i < num_blocks; ++i, lhs += lhs_block_size, rhs += rhs_block_size, dst += dst_block_size) {
        cblas_dgemm(CblasRowMajor, self.lhs_common_inner() ? CblasNoTrans : CblasTrans,
                    self.rhs_common_inner() ? CblasTrans : CblasNoTrans, self.lhs_size(), self.rhs_size(),
                    self.common_size(), 1.0, lhs, self.lhs_common_inner() ? self.common_size() : self.lhs_size(), rhs,
                    self.rhs_common_inner() ? self.common_size() : self.rhs_size(), 0.0, dst, self.rhs_size());
    }
...
void my_cblas_float_multi_matmul_op(InterpretedFunction::State& state, uint64_t param) {
    ...
    CT*                             dst = dst_cells.data();
    for (size_t i = 0; i < num_blocks; ++i, lhs += lhs_block_size, rhs += rhs_block_size, dst += dst_block_size) {
        cblas_sgemm(CblasRowMajor, self.lhs_common_inner() ? CblasNoTrans : CblasTrans,
                    self.rhs_common_inner() ? CblasTrans : CblasNoTrans, self.lhs_size(), self.rhs_size(),
                    self.common_size(), 1.0, lhs, self.lhs_common_inner() ? self.common_size() : self.lhs_size(), rhs,
                    self.rhs_common_inner() ? self.common_size() : self.rhs_size(), 0.0, dst, self.rhs_size());
    }
...
struct CommonDim {
    bool valid;
    bool inner;
    CommonDim(const DimList& list, const std::string& dim) : valid(true), inner(false) {
    ...
    const Dim& get(const DimList& dims) const { return dims[dims.size() - (inner ? 1 : 2)]; }
    const Dim& inv(const DimList& dims) const { return dims[dims.size() - (inner ? 2 : 1)]; }
};
...
struct DimPrefix {
    bool   valid;
    size_t size;
    DimPrefix(const DimList& a, const DimList& b) : valid(true), size(1) {
        if (a.size() == b.size()) {
...
bool check_input_type(const ValueType& type, const DimList& relevant) {
    return (type.is_dense() && (relevant.size() >= 2) &&
...
bool is_multi_matmul(const ValueType& a, const ValueType& b, const std::string& reduce_dim) {
    ...
    if (check_input_type(a, dims_a) && check_input_type(b, dims_b) && (a.cell_type() == b.cell_type())) {
        ...
        return (cd_a.valid && cd_b.valid && prefix.valid && (b.dimension_index(cd_a.inv(dims_a).name) == Dim::npos) &&
...
const TensorFunction& create_multi_matmul(const TensorFunction& a, const TensorFunction& b,
    ...
    if (a_is_lhs) {
        return stash.create<DenseMultiMatMulFunction>(result_type, a, b, a_size, common_size, b_size, prefix.size,
                                                      cd_a.inner, cd_b.inner);
    } else {
        return stash.create<DenseMultiMatMulFunction>(result_type, b, a, b_size, common_size, a_size, prefix.size,
                                                      cd_b.inner, cd_a.inner);
    }
}

} // namespace

DenseMultiMatMulFunction::DenseMultiMatMulFunction(const ValueType& result_type, const TensorFunction& lhs_in,
                                                   const TensorFunction& rhs_in, size_t lhs_size, size_t common_size,
                                                   size_t rhs_size, size_t matmul_cnt, bool lhs_common_inner,
...
InterpretedFunction::Instruction DenseMultiMatMulFunction::compile_self(const ValueBuilderFactory&, Stash&) const {
...
void DenseMultiMatMulFunction::visit_self(vespalib::ObjectVisitor& visitor) const {
...
const TensorFunction& DenseMultiMatMulFunction::optimize(const TensorFunction& expr, Stash& stash) {
