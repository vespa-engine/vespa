...
template <typename CT> void my_tensor_create_op(InterpretedFunction::State& state, uint64_t param) {
    const auto&   self = unwrap_param<DenseTensorCreateFunction::Self>(param);
    size_t        pending_cells = self.result_size;
    std::span<CT> cells = state.stash.create_uninitialized_array<CT>(pending_cells);
...
struct MyTensorCreateOp {
    template <typename CT> static auto invoke() { return my_tensor_create_op<CT>; }
};
...
void DenseTensorCreateFunction::push_children(std::vector<Child::CREF>& target) const {
    for (const Child& c : _children) {
        target.emplace_back(c);
    }
}

InterpretedFunction::Instruction DenseTensorCreateFunction::compile_self(const ValueBuilderFactory&, Stash&) const {
...
const TensorFunction& DenseTensorCreateFunction::optimize(const TensorFunction& expr, Stash& stash) {
