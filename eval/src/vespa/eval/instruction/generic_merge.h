...
struct MergeParam {
    ...
    MergeParam(const ValueType& res_type_in, const ValueType& lhs_type, const ValueType& rhs_type,
...
std::unique_ptr<Value> generic_mixed_merge(const Value& a, const Value& b, const MergeParam& params);

struct GenericMerge {
    static InterpretedFunction::Instruction make_instruction(const ValueType& result_type, const ValueType& lhs_type,
