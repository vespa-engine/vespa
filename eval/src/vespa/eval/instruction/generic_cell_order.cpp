...
template <typename ICT, typename OCT, typename CMP> void my_generic_cell_order_op(State& state, uint64_t param_in) {
    ...
    auto         output_cells = state.stash.create_uninitialized_array<OCT>(input_cells.size());
    {
        ...
        std::sort(indexes.begin(), indexes.end(),
                  [&](auto a, auto b) { return CMP::cmp(input_cells[a], input_cells[b]); });
...
struct SelectGenericCellOrderOp {
    template <typename CM> static InterpretedFunction::op_function invoke(CellOrder cell_order) {
        using ICT = CellValueType<CM::value.cell_type>;
        using OCT = CellValueType<CM::value.decay().cell_type>;
        switch (cell_order) {
        case CellOrder::MAX:
            return my_generic_cell_order_op<ICT, OCT, CellOrderMAX>;
        case CellOrder::MIN:
            return my_generic_cell_order_op<ICT, OCT, CellOrderMIN>;
        }
        abort();
    }
};
