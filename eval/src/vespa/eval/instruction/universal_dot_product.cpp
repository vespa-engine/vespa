...
struct UniversalDotProductParam {
    ...
    UniversalDotProductParam(const ValueType& res_type_in, const ValueType& lhs_type, const ValueType& rhs_type)
        : res_type(res_type_in),
          sparse_plan(lhs_type, rhs_type, res_type),
          dense_plan(lhs_type, rhs_type, res_type),
          vector_size(1) {
        if (!dense_plan.loop_cnt.empty() && dense_plan.lhs_stride.back() == 1 && dense_plan.rhs_stride.back() == 1 &&
...
template <typename OCT> const Value& create_empty_result(const UniversalDotProductParam& param, Stash& stash) {
    if (param.sparse_plan.res_dims() == 0) {
        ...
        return stash.create<ValueView>(param.res_type, EmptyIndex::get(),
                                       TypedCells(nullptr, get_cell_type<OCT>(), 0));
    }
}
...
template <typename LCT, typename RCT> struct MyDotProduct<LCT, RCT, true> {
    MyDotProduct(size_t) {}
    auto operator()(const LCT* lhs, const RCT* rhs) const { return (*lhs) * (*rhs); }
};

template <typename LCT, typename RCT, typename OCT, bool distinct, bool single> struct DenseFun {
    ...
    void operator()(size_t lhs_idx, size_t rhs_idx) const
        requires distinct
    {
        *dst++ = dot_product(lhs + lhs_idx, rhs + rhs_idx);
    }
    void operator()(size_t lhs_idx, size_t rhs_idx, size_t dst_idx) const
        requires(!distinct)
    {
...
template <typename LCT, typename RCT, typename OCT, bool forward, bool distinct, bool single> struct SparseFun {
    ...
    SparseFun(uint64_t param_in, const Value& lhs_in, const Value& rhs_in)
        : param(unwrap_param<UniversalDotProductParam>(param_in)),
          ...
          result() {}
    void operator()(size_t lhs_subspace, size_t rhs_subspace, std::span<const string_id> res_addr) const
        requires(!forward && !distinct)
    {
        ...
        param.dense_plan.execute(lhs_subspace * param.dense_plan.lhs_size, rhs_subspace * param.dense_plan.rhs_size,
                                 0, dense_fun);
    };
    void operator()(size_t lhs_subspace, size_t rhs_subspace, std::span<const string_id> res_addr) const
        requires(!forward && distinct)
    ...
    void operator()(size_t lhs_subspace, size_t rhs_subspace) const
        requires(forward && !distinct)
    {
        param.dense_plan.execute(lhs_subspace * param.dense_plan.lhs_size, rhs_subspace * param.dense_plan.rhs_size,
    ...
    const Value& calculate_result(const Value::Index& lhs, const Value::Index& rhs, Stash& stash) const
        requires(!forward)
    {
        auto& stored_result =
            stash.create<std::unique_ptr<FastValue<OCT, true>>>(std::make_unique<FastValue<OCT, true>>(
                param.res_type, param.sparse_plan.res_dims(), param.dense_plan.res_size,
        ...
        requires forward
    {
        ...
        auto dst_cells = (distinct) ? stash.create_uninitialized_array<OCT>(lhs_size * param.dense_plan.res_size)
...
UniversalDotProduct::UniversalDotProduct(const ValueType& res_type_in, const TensorFunction& lhs_in,
                                         const TensorFunction& rhs_in)
    : tensor_function::Op2(res_type_in, lhs_in, rhs_in) {
}

InterpretedFunction::Instruction UniversalDotProduct::compile_self(const ValueBuilderFactory&, Stash& stash) const {
    ...
    auto op = typify_invoke<6, MyTypify, SelectUniversalDotProduct>(
        lhs().result_type().cell_meta(), rhs().result_type().cell_meta(), result_type().cell_meta().is_scalar,
        param.forward(), param.distinct(), param.single());
    return InterpretedFunction::Instruction(op, wrap_param<UniversalDotProductParam>(param));
}

bool UniversalDotProduct::forward() const {
    UniversalDotProductParam param(result_type(), lhs().result_type(), rhs().result_type());
    return param.forward();
}

bool UniversalDotProduct::distinct() const {
    UniversalDotProductParam param(result_type(), lhs().result_type(), rhs().result_type());
    return param.distinct();
}

bool UniversalDotProduct::single() const {
    UniversalDotProductParam param(result_type(), lhs().result_type(), rhs().result_type());
    return param.single();
}

const TensorFunction& UniversalDotProduct::optimize(const TensorFunction& expr, Stash& stash, bool force) {
