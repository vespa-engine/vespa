...
private:
    ...
    static void execute_plan(const Value::Index& a, const Value::Index& b, const bool* in_a, const bool* in_b,
                             const bool* in_res, size_t dims, auto&& f) {
        ...
        while (outer->next_result(state.a_addr, state.a_subspace)) {
            ...
            while (inner->next_result(state.b_only, state.b_subspace)) {
                f(state.a_subspace, state.b_subspace,
                  std::span<const string_id>{state.addr_space.begin(), state.res_dims});
            }
        }
    }
