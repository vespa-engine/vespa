...
struct InterpretedParams {
    ...
    size_t                     dense_size;
    bool                       direct;
    InterpretedParams(const ValueType& result_type_in, const ValueType& inner_type_in, const Function& lambda,
                      const NodeTypes& types, const ValueBuilderFactory& factory_in)
        : factory(factory_in),
...
public:
    ...
    template <typename CT> void adjust(const CT* cells, size_t size) {
        if (direct) {
...
template <typename CT> void my_generic_filter_subspaces_op(InterpretedFunction::State& state, uint64_t param) {
    ...
    auto                         builder =
        params.factory.create_value_builder<CT>(params.result_type, params.num_mapped, params.dense_size, idx.size());
...
struct SelectGenericFilterSubspacesOp {
    template <typename CT> static auto invoke() { return my_generic_filter_subspaces_op<CT>; }
};
...
Instruction GenericFilterSubspaces::make_instruction(const ValueType& result_type, const ValueType& inner_type,
