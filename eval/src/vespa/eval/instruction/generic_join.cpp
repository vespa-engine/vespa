...
Value::UP generic_mixed_join(const Value& lhs, const Value& rhs, const JoinParam& param) {
    ...
    auto dense_join = [&](const LCT* my_lhs, const RCT* my_rhs, OCT* my_res) {
        param.dense_plan.execute(
            0, 0, [&](size_t lhs_idx, size_t rhs_idx) { *my_res++ = fun(my_lhs[lhs_idx], my_rhs[rhs_idx]); });
    };
    ...
    auto builder = param.factory.create_transient_value_builder<OCT>(param.res_type, param.sparse_plan.sources.size(),
                                                                     param.dense_plan.out_size, expected_subspaces);
...
template <typename Fun> void my_double_join_op(State& state, uint64_t param_in) {
    Fun fun(unwrap_param<JoinParam>(param_in).function);
    state.pop_pop_push(state.stash.create<DoubleValue>(fun(state.peek(1).as_double(), state.peek(0).as_double())));
}
...
DenseJoinPlan::DenseJoinPlan(const ValueType& lhs_type, const ValueType& rhs_type)
    ...
    auto visitor =
        overload{[&](visit_ranges_first, const auto& a) { update_plan(Case::LHS, a.size, 1, 0); },
...
SparseJoinPlan::SparseJoinPlan(const ValueType& lhs_type, const ValueType& rhs_type)
    ...
    size_t lhs_idx = 0;
    size_t rhs_idx = 0;
    auto   visitor = overload{[&](visit_ranges_first, const auto&) {
...
bool SparseJoinPlan::should_forward_lhs_index() const {
...
bool SparseJoinPlan::should_forward_rhs_index() const {
    for (Source src : sources) {
        if (src != Source::RHS) {
            return false;
        }
...
SparseJoinState::SparseJoinState(const SparseJoinPlan& plan, const Value::Index& lhs, const Value::Index& rhs)
    : swapped(rhs.size() < lhs.size()),
      first_index(swapped ? rhs : lhs),
      second_index(swapped ? lhs : rhs),
...
Instruction GenericJoin::make_instruction(const ValueType& result_type, const ValueType& lhs_type,
