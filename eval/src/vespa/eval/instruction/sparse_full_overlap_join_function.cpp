...
const Value& my_fast_sparse_full_overlap_join(const FastAddrMap& lhs_map, const FastAddrMap& rhs_map,
                                              const CT* lhs_cells, const CT* rhs_cells, const JoinParam& param,
                                              Stash& stash) {
...
const Value& my_fast_sparse_full_overlap_join_dispatch(const FastAddrMap& lhs_map, const FastAddrMap& rhs_map,
    ...
    return (rhs_map.size() < lhs_map.size())
               ? my_fast_sparse_full_overlap_join<CT, SwapArgs2<Fun>, single_dim>(rhs_map, lhs_map, rhs_cells,
                                                                                  lhs_cells, param, stash)
               : my_fast_sparse_full_overlap_join<CT, Fun, single_dim>(lhs_map, rhs_map, lhs_cells, rhs_cells, param,
                                                                       stash);
}
...
void my_sparse_full_overlap_join_op(InterpretedFunction::State& state, uint64_t param_in) {
    ...
    if (__builtin_expect(are_fast(lhs_idx, rhs_idx), true)) {
        const Value& res = my_fast_sparse_full_overlap_join_dispatch<CT, Fun, single_dim>(
            as_fast(lhs_idx).map, as_fast(rhs_idx).map, lhs.cells().typify<CT>().data(),
            rhs.cells().typify<CT>().data(), param, state.stash);
...
struct SelectSparseFullOverlapJoinOp {
    template <typename R1, typename Fun, typename SINGLE_DIM> static auto invoke() {
...
bool SparseFullOverlapJoinFunction::compatible_types(const ValueType& res, const ValueType& lhs,
                                                     const ValueType& rhs) {
    if ((lhs.cell_type() == rhs.cell_type()) && (res.cell_type() == lhs.cell_type()) && is_sparse_like(lhs) &&
...
const TensorFunction& SparseFullOverlapJoinFunction::optimize(const TensorFunction& expr, Stash& stash) {
