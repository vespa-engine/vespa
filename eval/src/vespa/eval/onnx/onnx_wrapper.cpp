...
struct TypifyOnnxElementType {
    ...
    template <typename F> static decltype(auto) resolve(Onnx::ElementType value, F&& f) {
        switch (value) {
        case Onnx::ElementType::INT8:
            return f(Result<Int8Float>());
        case Onnx::ElementType::INT16:
            return f(Result<int16_t>());
        case Onnx::ElementType::INT32:
            return f(Result<int32_t>());
        case Onnx::ElementType::INT64:
            return f(Result<int64_t>());
        case Onnx::ElementType::UINT8:
            return f(Result<uint8_t>());
        case Onnx::ElementType::UINT16:
            return f(Result<uint16_t>());
        case Onnx::ElementType::UINT32:
            return f(Result<uint32_t>());
        case Onnx::ElementType::UINT64:
            return f(Result<uint64_t>());
        case Onnx::ElementType::FLOAT16:
            return f(Result<Float16>());
        case Onnx::ElementType::BFLOAT16:
            return f(Result<BFloat16>());
        case Onnx::ElementType::FLOAT:
            return f(Result<float>());
        case Onnx::ElementType::DOUBLE:
...
struct CreateEmptyOnnxTensor {
    template <typename T>
    static Ort::Value invoke(const std::vector<int64_t>& sizes, size_t num_cells, OrtAllocator* alloc) {
    ...
    Ort::Value operator()(Onnx::ElementType elements, const std::vector<int64_t>& sizes, size_t num_cells,
                          OrtAllocator* alloc) {
...
Onnx::ElementType make_element_type(ONNXTensorElementDataType element_type) {
    ...
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_INT8:
        return Onnx::ElementType::INT8;
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_INT16:
        return Onnx::ElementType::INT16;
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32:
        return Onnx::ElementType::INT32;
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64:
        return Onnx::ElementType::INT64;
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_UINT8:
        return Onnx::ElementType::UINT8;
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_UINT16:
        return Onnx::ElementType::UINT16;
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_UINT32:
        return Onnx::ElementType::UINT32;
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_UINT64:
        return Onnx::ElementType::UINT64;
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT16:
        return Onnx::ElementType::FLOAT16;
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_BFLOAT16:
        return Onnx::ElementType::BFLOAT16;
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_FLOAT:
        return Onnx::ElementType::FLOAT;
    case ONNX_TENSOR_ELEMENT_DATA_TYPE_DOUBLE:
        return Onnx::ElementType::DOUBLE;
    default:
...
std::string Onnx::DimSize::as_string() const {
    if (is_known()) {
        return fmt("[%zu]", value);
    } else if (is_symbolic()) {
...
Onnx::TensorInfo::TensorInfo(std::string name_in, std::vector<DimSize> dimensions_in, ElementType elements_in)
    : name(std::move(name_in)), dimensions(std::move(dimensions_in)), elements(elements_in) {
}
...
std::string Onnx::TensorInfo::type_as_string() const {
...
std::string Onnx::TensorType::type_as_string() const {
    std::string res = type_name(elements);
...
bool Onnx::WirePlanner::need_model_probe(const Onnx& model) const {
...
void Onnx::WirePlanner::do_model_probe(const Onnx& model) {
    try {
        ...
        session.Run(run_opts, model._input_name_refs.data(), param_values.data(), param_values.size(),
...
CellType Onnx::WirePlanner::best_cell_type(Onnx::ElementType type) {
    switch (type) {
    case Onnx::ElementType::INT8:
...
bool Onnx::WirePlanner::bind_input_type(const ValueType& vespa_in, const TensorInfo& onnx_in) {
...
std::map<std::string, size_t> Onnx::WirePlanner::get_bound_sizes(const TensorInfo& onnx_in) const {
...
void Onnx::WirePlanner::prepare_output_types(const Onnx& model) {
    if (need_model_probe(model)) {
        do_model_probe(model);
    }
}

ValueType Onnx::WirePlanner::make_output_type(const TensorInfo& onnx_out) const {
    ...
    for (size_t i = 0; i < dimensions.size(); ++i) {
        ...
        if (probed != _output_types.end()) {
            ...
            if (type.dimensions.size() != dimensions.size()) {
                LOG(warning,
                    "probed output '%s' does not have the same number of dimensions as "
                    "the output declared by the model (probed: %zu, declared: %zu)",
                    onnx_out.name.c_str(), type.dimensions.size(), dimensions.size());
            ...
            if (dim_size == 0) {
                ...
                LOG(warning,
                    "probed dimension size for output '%s' dimension %zu does not match symbolic "
                    "dimension size inferred from inputs (probed: %zu, inferred: %zu)",
                    onnx_out.name.c_str(), i, probed_size, dim_size);
                return ValueType::error_type();
            }
        }
...
Onnx::WireInfo Onnx::WirePlanner::get_wire_info(const Onnx& model) const {
    WireInfo info;
    for (const auto& input : model.inputs()) {
        const auto& pos = _input_types.find(input.name);
        assert(pos != _input_types.end());
        ...
        if (!is_same_type(info.vespa_inputs.back().cell_type(), info.onnx_inputs.back().elements)) {
            LOG(warning,
                "input '%s' with element type '%s' is bound to vespa value with cell type '%s'; "
    ...
    for (const auto& output : model.outputs()) {
        ...
        if (!is_same_type(info.vespa_outputs.back().cell_type(), info.onnx_outputs.back().elements)) {
            LOG(warning,
                "output '%s' with element type '%s' is bound to vespa value with cell type '%s'; "
                "adding explicit conversion step (this conversion might be lossy)",
...
template <typename T> void Onnx::EvalContext::adapt_param(EvalContext& self, size_t idx, const Value& param) {
    ...
    self._param_values[idx] =
        Ort::Value::CreateTensor<T>(self._cpu_memory, cells.data(), cells.size(), sizes.data(), sizes.size());
}

template <typename SRC, typename DST>
void Onnx::EvalContext::convert_param(EvalContext& self, size_t idx, const Value& param) {
...
template <typename SRC, typename DST> void Onnx::EvalContext::convert_result(EvalContext& self, size_t idx) {
...
struct Onnx::EvalContext::SelectAdaptParam {
    ...
    auto operator()(CellType ct) { return typify_invoke<1, MyTypify, SelectAdaptParam>(ct); }
};
...
void Onnx::EvalContext::bind_param(size_t i, const Value& param) {
    _param_binders[i](*this, i, param);
}

void Onnx::EvalContext::eval() {
    auto&           session = const_cast<Ort::Session&>(_model._session);
    Ort::RunOptions run_opts(nullptr);
    session.Run(run_opts, _model._input_name_refs.data(), _param_values.data(), _param_values.size(),
...
void Onnx::EvalContext::clear_results() {
    for (const Value::UP& result : _results) {
        clear_vespa_tensor(*result);
    }
}

const Value& Onnx::EvalContext::get_result(size_t i) const {
    return *_results[i];
}
...
Onnx::Shared::Shared() : _env(ORT_LOGGING_LEVEL_WARNING, "vespa-onnx-wrapper") {
}

Onnx::Shared& Onnx::Shared::get() {
...
void Onnx::extract_meta_data() {
    ...
    for (size_t i = 0; i < num_inputs; ++i) {
        ...
        if (_inputs.back().dimensions.empty()) {
            throw Ort::Exception(fmt("[onnx wrapper] input '%s' has unspecified type, this is not supported",
                                     _inputs.back().name.c_str()),
                                 ORT_FAIL);
        }
    }
    ...
    for (size_t i = 0; i < num_outputs; ++i) {
        ...
        if (_outputs.back().dimensions.empty()) {
            throw Ort::Exception(fmt("[onnx wrapper] output '%s' has unspecified type, this is not supported",
                                     _outputs.back().name.c_str()),
                                 ORT_FAIL);
        }
    }
