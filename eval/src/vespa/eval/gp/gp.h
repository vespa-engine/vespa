...
struct Params {
    ...
    Params(size_t in_cnt_in, size_t out_cnt_in, size_t op_cnt_in, size_t alt_cnt_in, size_t pop_cnt_in)
        : in_cnt(in_cnt_in), out_cnt(out_cnt_in), op_cnt(op_cnt_in), alt_cnt(alt_cnt_in), pop_cnt(pop_cnt_in) {}
};
...
struct Random {
    ...
    int make_seed() { return get(std::numeric_limits<int>::lowest(), std::numeric_limits<int>::max()); }
};
...
struct OpRepo {
    ...
    OpRepo(feedback_fun find_weakness_in) : _find_weakness(find_weakness_in), _list() {
    ...
    void find_weakness(Random& rnd, Sim& sim) const { sim.handle_feedback(rnd, _find_weakness(sim)); }
    Value perform(size_t op, Value lhs, Value rhs) const { return _list[op].fun(lhs, rhs); }
};
...
struct Program : public Sim {
    ...
    public:
        ...
        static Ref rnd(Random& rnd_in, size_t in_cnt, size_t op_cnt) { return Ref(rnd_in.get(-in_cnt, op_cnt - 1)); }
    };
    struct Op {
        size_t code;
        Ref    lhs;
        Ref    rhs;
        Op(size_t code_in, Ref lhs_in, Ref rhs_in) noexcept : code(code_in), lhs(lhs_in), rhs(rhs_in) {}
    };
    struct Stats {
        Weakness weakness;
        size_t   cost;
    ...
    bool operator<(const Program& rhs) const noexcept {
        return (stats() < rhs.stats() || ((_waste < rhs._waste) && !(rhs.stats() < stats())));
    }
...
struct Population {
    ...
    Population(const Params& params, const OpRepo& repo, int seed)
        : _rnd(seed), _gen(0), _params(params), _repo(repo), _programs() {
        grow(1);
    }
