...
private:
    ...
    public:
        Hit(vsm::StorageDocument::SP doc, uint32_t docId, const MatchData& matchData, double score,
            const void* sortData, size_t sortDataLen);
        Hit(vsm::StorageDocument::SP doc, uint32_t docId, const MatchData& matchData, double score)
        ...
        int cmpRank(const Hit& b) const noexcept {
            return (getRankScore() > b.getRankScore()) ? -1 : ((getRankScore() < b.getRankScore()) ? 1 : cmpDocId(b));
        }
        int cmpSort(const Hit& b) const noexcept {
            ...
            if (diff == 0) {
                diff =
                    (_sortBlob.size() == b._sortBlob.size()) ? 0 : ((_sortBlob.size() < b._sortBlob.size()) ? -1 : 1);
            }
    ...
    public:
        ...
        bool operator()(uint32_t lhs, uint32_t rhs) const noexcept { return _hits[lhs].cmpRank(_hits[rhs]) < 0; }
    ...
    public:
        ...
        bool operator()(uint32_t lhs, uint32_t rhs) const noexcept { return _hits[lhs].cmpSort(_hits[rhs]) < 0; }
...
public:
     ...
     **/
    vespalib::FeatureSet::SP getFeatureSet(IRankProgram& rankProgram, const FeatureResolver& resolver,
                                           const search::StringStringMap& feature_rename_map) const;

    vespalib::FeatureSet::SP getFeatureSet(IRankProgram& rankProgram, search::DocumentIdT docId,
                                           const FeatureResolver&         resolver,
                                           const search::StringStringMap& feature_rename_map);

    vespalib::FeatureValues get_match_features(IRankProgram& rank_program, const FeatureResolver& resolver,
                                               const search::StringStringMap& feature_rename_map) const;
};
