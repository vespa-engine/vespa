...
std::string_view extract_search_cluster(const vdslib::Parameters& params) {
...
std::string_view extract_schema(const vdslib::Parameters& params) {
...
std::shared_ptr<const SearchEnvironmentSnapshot> get_search_environment_snapshot(VisitorEnvironment& v_env,
    ...
    if (!search_cluster.empty()) {
        ...
        return schema.empty() ? env.get_snapshot(std::string(search_cluster))
...
enum queryflags { QFLAG_DUMP_FEATURES = 0x00040000 };

AttributeVector::SP createMultiValueAttribute(const std::string& name, const document::FieldValue& fv,
                                              bool arrayType) {
    ...
    LOG(debug, "Create %s attribute '%s' with data type '%s' (%s)", arrayType ? "array" : "weighted set",
    ...
    if (ndt->getId() == DataType::T_BYTE || ndt->getId() == DataType::T_INT || ndt->getId() == DataType::T_LONG) {
    ...
    } else if (ndt->getId() == DataType::T_DOUBLE || ndt->getId() == DataType::T_FLOAT) {
        ...
        LOG(debug, "Can not make an multivalue attribute out of %s with data type '%s' (%s)", name.c_str(),
            ndt->getName().c_str(), fv.className());
    }
    return {};
}

const document::TensorDataType* get_tensor_type(const document::FieldValue& fv) {
...
AttributeVector::SP createAttribute(const std::string& name, const document::FieldValue& fv,
...
SearchVisitor::AttrInfo::AttrInfo(vsm::FieldIdT fid, search::AttributeGuard::UP attr) noexcept
    : _field(fid), _attr(std::move(attr)), _sort_blob_writer() {
}

SearchVisitor::StreamingDocsumsState::StreamingDocsumsState(search::docsummary::GetDocsumsStateCallback& callback,
                                                            ResolveClassInfo& resolve_class_info)
    : _state(callback), _resolve_class_info(resolve_class_info) {
}
...
vespalib::ConstBufferRef SearchVisitor::SummaryGenerator::fillSummary(AttributeVector::DocId lid,
                                                                      std::string_view       summaryClass) {
...
SearchVisitor::GroupingEntry::GroupingEntry(Grouping* grouping)
    : _grouping(grouping), _count(0), _limit(grouping->getMaxN(std::numeric_limits<size_t>::max())) {
}
...
bool SearchVisitor::is_text_matching(std::string_view index) const noexcept {
    StringFieldIdTMap fieldIdMap;
    _fieldSearchSpecMap.addFieldsFromIndex(index, fieldIdMap);
    return std::any_of(fieldIdMap.map().begin(), fieldIdMap.map().end(),
                       [&specMap = _fieldSearchSpecMap.specMap()](const auto& fieldId) {
...
uint32_t count_normalize_lowercase(const vsm::FieldSearchSpecMapT& specMap, const StringFieldIdTMap& fieldIdMap) {
    size_t count = 0;
...
uint32_t count_normalize_none(const vsm::FieldSearchSpecMapT& specMap, const StringFieldIdTMap& fieldIdMap) {
    size_t count = 0;
    for (const auto& fieldId : fieldIdMap.map()) {
        auto found = specMap.find(fieldId.second);
            ...
            if (!prop.decode(src, len)) {
                ...
                if (prop.name() == "rank") { // pick up rank properties
                    for (uint32_t j = 0; j < prop.size(); ++j) {
                        LOG(debug, "Properties[%u][%u]: key '%s' -> value '%s'", i, j, string(prop.key(j)).c_str(),
                            string(prop.value(j)).c_str());
                        _rankController.getQueryProperties().add(prop.key(j), prop.value(j));
                    }
                } else if (prop.name() == "feature") { // pick up feature overrides
                    for (uint32_t j = 0; j < prop.size(); ++j) {
                        LOG(debug, "Feature override[%u][%u]: key '%s' -> value '%s'", i, j,
                            string(prop.key(j)).c_str(), string(prop.value(j)).c_str());
                        _rankController.getFeatureOverrides().add(prop.key(j), prop.value(j));
                    }
                } else if (prop.name() == "highlightterms") {
                    for (uint32_t j = 0; j < prop.size(); ++j) {
                        LOG(debug, "Hightligthterms[%u][%u]: key '%s' -> value '%s'", i, j,
                            string(prop.key(j)).c_str(), string(prop.value(j)).c_str());
                        std::string_view index = prop.key(j);
                        std::string_view term = prop.value(j);
                        std::string      norm_term =
                            QueryNormalization::optional_fold(term, search::TermType::WORD, normalizing_mode(index));
    ...
    if (_env) {
        ...
        if (params.lookup("query", queryBlob)) {
            ...
            _fieldSearchSpecMap.buildFromConfig(_env->get_vsm_fields_config(),
                                                _env->get_rank_manager_snapshot()->get_proto_index_environment());
            ...
            _rankController.setupRankProcessors(_query, location, wantedSummaryCount, !_sortList.empty(), _attrMan,
                                                _attributeFields);
...
SearchVisitorFactory::SearchVisitorFactory(const config::ConfigUri& configUri, FNET_Transport* transport,
...
std::shared_ptr<VisitorEnvironment> SearchVisitorFactory::makeVisitorEnvironment(StorageComponent&) {
    return _env;
}

storage::Visitor* SearchVisitorFactory::makeVisitor(StorageComponent& component, storage::VisitorEnvironment& env,
                                                    const vdslib::Parameters& params) {
    return new SearchVisitor(component, env, params);
}

std::optional<int64_t> SearchVisitorFactory::get_oldest_config_generation() const {
    auto& env = dynamic_cast<SearchEnvironment&>(*_env);
    return env.get_oldest_config_generation();
}

void SearchVisitor::AttributeInserter::onPrimitive(uint32_t, const Content& c) {
    const document::FieldValue& value = c.getValue();
    LOG(debug, "AttributeInserter: Adding value '%s'(%d) to attribute '%s' for docid '%d'", value.toString().c_str(),
        c.getWeight(), _attribute.getName().c_str(), _docId);
...
SearchVisitor::AttributeInserter::AttributeInserter(AttributeVector& attribute, AttributeVector::DocId docId)
    : _attribute(attribute), _docId(docId) {
}
...
void SearchVisitor::PositionInserter::onPrimitive(uint32_t, const Content&) {
}

void SearchVisitor::PositionInserter::onStructStart(const Content& c) {
    const auto& value = static_cast<const document::StructuredFieldValue&>(c.getValue());
    LOG(debug, "PositionInserter: Adding value '%s'(%d) to attribute '%s' for docid '%d'", value.toString().c_str(),
        c.getWeight(), _attribute.getName().c_str(), _docId);
...
void SearchVisitor::RankController::processAccessedAttributes(const QueryEnvironment& queryEnv, bool rank,
    ...
    for (const std::string& name : attributes) {
        ...
        if (fieldInfo != nullptr) {
            ...
            if (!found) {
                ...
                if (attr->valid()) {
                    LOG(debug, "Add attribute '%s' with field id '%u' to the list of needed attributes", name.c_str(),
                        fid);
                    attributeFields.emplace_back(fid, std::move(attr));
                } else {
                    Issue::report("Cannot locate attribute '%s' in the attribute manager. "
                                  "Ignore access hint about this attribute",
                                  name.c_str());
                }
            }
        } else {
            Issue::report(
                "Cannot locate field '%s' in the index environment. Ignore access hint about this attribute",
                name.c_str());
        }
    }
}
...
void SearchVisitor::RankController::setupRankProcessors(Query& query, const std::string& location,
    ...
    _rank_score_drop_limit =
        FirstPhaseRankScoreDropLimit::lookup(_queryProperties, rankSetup.get_first_phase_rank_score_drop_limit());
    _rankProcessor = std::make_unique<RankProcessor>(_rankManagerSnapshot, _rankProfile, query, location,
                                                     _queryProperties, _featureOverrides, &attrMan);
    ...
    if (_dumpFeatures) {
        _dumpProcessor = std::make_unique<RankProcessor>(_rankManagerSnapshot, _rankProfile, query, location,
                                                         _queryProperties, _featureOverrides, &attrMan);
...
void SearchVisitor::RankController::onDocumentMatch(uint32_t docId) {
...
void SearchVisitor::RankController::rankMatchedDocument(uint32_t docId) {
    _rankProcessor->runRankProgram(docId);
    LOG(debug, "Rank score for matched document %u: %f", docId, _rankProcessor->getRankScore());
    if (_dumpFeatures) {
...
bool SearchVisitor::RankController::keepMatchedDocument() {
...
void SearchVisitor::RankController::collectMatchedDocument(bool hasSorting, SearchVisitor& visitor,
                                                           const std::vector<char>& tmpSortBuffer,
                                                           StorageDocument::SP      document) {
    uint32_t docId = _rankProcessor->getDocId();
    if (!hasSorting) {
        bool amongTheBest = _rankProcessor->getHitCollector().addHit(
            std::move(document), docId, _rankProcessor->getMatchData(), _rankProcessor->getRankScore());
        if (amongTheBest && _dumpFeatures) {
            _dumpProcessor->getHitCollector().addHit({}, docId, _dumpProcessor->getMatchData(),
                                                     _dumpProcessor->getRankScore());
        }
    } else {
        size_t pos = visitor.fillSortBuffer();
        LOG(spam, "SortBlob is %ld bytes", pos);
        bool amongTheBest =
            _rankProcessor->getHitCollector().addHit(std::move(document), docId, _rankProcessor->getMatchData(),
...
vespalib::FeatureSet::SP SearchVisitor::RankController::getFeatureSet(search::DocumentIdT docId) {
...
void SearchVisitor::RankController::onCompletedVisiting(vsm::GetDocsumsStateCallback& docsumsStateCallback,
...
SearchVisitor::SyntheticFieldsController::SyntheticFieldsController() : _documentIdFId(StringFieldIdTMap::npos) {
}
...
void SearchVisitor::SyntheticFieldsController::onDocumentMatch(StorageDocument&   document,
...
StringFieldIdTMap SearchVisitor::setupFieldSearchers() {
...
void SearchVisitor::prepare_field_searchers() {
    // prepare the field searchers
    _fieldSearcherMap.prepare(_fieldSearchSpecMap.documentTypeMap(), _searchBuffer, _query, *_fieldPathMap,
                              _rankController.getRankProcessor()->get_query_env());
}

void SearchVisitor::setupSnippetModifiers() {
    QueryTermList qtl;
    _query.getLeaves(qtl);
    _snippetModifierManager.setup(qtl, _fieldSearchSpecMap.specMap(),
                                  _fieldSearchSpecMap.documentTypeMap().begin()->second, *_fieldPathMap,
                                  _rankController.getRankProcessor()->get_query_env());
}

void SearchVisitor::setupScratchDocument(const StringFieldIdTMap& fieldsInQuery) {
    if (_fieldSearchSpecMap.documentTypeMap().empty()) {
        throw vespalib::IllegalStateException(
            "Illegal config: There must be at least 1 document type in the 'vsmfields' config");
    }
    ...
    _docTypeMapping.init(_fieldSearchSpecMap.documentTypeMap().begin()->first, _fieldsUnion,
                         *_component.getTypeRepo()->documentTypeRepo);
    _docTypeMapping.prepareBaseDoc(_fieldPathMap);
}

void SearchVisitor::setupDocsumObjects() {
...
void SearchVisitor::setupAttributeVectors() {
...
void SearchVisitor::setupAttributeVector(const FieldPath& fieldPath) {
    ...
    if (attr) {
        LOG(debug, "Adding attribute '%s' for field '%s' with data type '%s' (%s)", attr->getName().c_str(),
        ...
        if (!_attrMan.add(attr)) {
            Issue::report("Failed adding attribute '%s' for field '%s' with data type '%s' (%s)",
                          attr->getName().c_str(), attrName.c_str(), fv.getDataType()->getName().c_str(),
                          fv.className());
        }
    } else {
        LOG(debug,
            "Cannot setup attribute for field '%s' with data type '%s' (%s). Aggregation and sorting will not work "
            "for this field",
            attrName.c_str(), fv.getDataType()->getName().c_str(), fv.className());
    }
}
...
void SearchVisitor::setupAttributeVectorsForSorting(const search::common::SortSpec& sortList) {
    if (!sortList.empty()) {
...
void SearchVisitor::setupGrouping(const std::vector<char>& groupingBlob) {
...
public:
    ...
    const vsm::Document& getDocSum(const search::DocumentIdT&) const override { return _doc; }

private:
    const StorageDocument& _doc;
};

bool SearchVisitor::compatibleDocumentTypes(const document::DocumentType& typeA,
                                            const document::DocumentType& typeB) {
    return (&typeA == &typeB) || (typeA.getName() == typeB.getName());
}

void SearchVisitor::handleDocuments(const document::BucketId&, DocEntryList& entries, HitCounter&) {
    ...
    for (const auto& entry : entries) {
        ...
        try {
            if (defaultDocType != nullptr && !compatibleDocumentTypes(*defaultDocType, document->docDoc().getType()))
            {
...
void SearchVisitor::handleDocument(StorageDocument::SP documentSP) {
    ...
    if (match(document)) {
        ...
        if (_rankController.keepMatchedDocument()) {
            ...
            if (_collectGroupingHits) {
                _summaryGenerator.getDocsumCallback().setSummaryFeatures(
                    _rankController.getFeatureSet(document.getDocId()));
            }
...
void SearchVisitor::group(const document::Document& doc, search::HitRank rank, bool all) {
...
bool SearchVisitor::match(const StorageDocument& doc) {
...
void SearchVisitor::fillAttributeVectors(const std::string& documentId, const StorageDocument& document) {
    for (const AttrInfo& finfo : _attributeFields) {
        ...
        LOG(debug, "Filling attribute '%s',  isPosition='%s'", finfoGuard->getName().c_str(),
...
size_t SearchVisitor::fillSortBuffer() {
    size_t pos(0);
    for (size_t index : _sortList) {
        const AttrInfo&       finfo = _attributeFields[index];
        int                   written(0);
        const AttributeGuard& finfoGuard(*finfo._attr);
        LOG(debug, "Adding sortdata for document %d for attribute '%s'", finfoGuard->getNumDocs() - 1,
            finfoGuard->getName().c_str());
        do {
            written = finfo._sort_blob_writer->write(finfoGuard->getNumDocs() - 1, &_tmpSortBuffer[0] + pos,
                                                     _tmpSortBuffer.size() - pos);
            if (written == -1) {
...
std::unique_ptr<documentapi::QueryResultMessage> SearchVisitor::generate_query_result(HitCounter& counter) {
...
void SearchVisitor::generateGroupingResults() {
    vdslib::SearchResult& searchResult(_queryResult->getSearchResult());
...
void SearchVisitor::generateDocumentSummaries() {
    if (!_rankController.valid()) {
        return;
    }
    ...
    _summaryGenerator.getDocsumCallback().set_matching_elements_filler(
        std::make_unique<MatchingElementsFiller>(_fieldSearcherMap, index_env, _query, hit_collector, searchResult));
    vdslib::DocumentSummary& documentSummary(_queryResult->getDocumentSummary());
    for (size_t i(0), m(searchResult.getHitCount()); (i < m) && (i < searchResult.getWantedHitCount()); i++) {
        ...
        LOG(debug, "Adding summary %ld: globalDocId(%s), localDocId(%u), rank(%f), bytes(%lu)", i, docId, lid, rank,
            docsum.size());
    }
}

void SearchVisitor::generate_errors() {
...
SearchVisitor::ElementGapInspector::ElementGapInspector() noexcept : _query_env(nullptr), _cache(16) {
}

SearchVisitor::ElementGapInspector::~ElementGapInspector() = default;

ElementGap SearchVisitor::ElementGapInspector::get_element_gap(uint32_t field_id) const noexcept {
