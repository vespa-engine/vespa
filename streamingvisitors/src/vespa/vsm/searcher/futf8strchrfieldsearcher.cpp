...
std::unique_ptr<FieldSearcher> FUTF8StrChrFieldSearcher::duplicate() const {
    return std::make_unique<FUTF8StrChrFieldSearcher>(*this);
}

FUTF8StrChrFieldSearcher::FUTF8StrChrFieldSearcher(FieldIdT fId) : UTF8StrChrFieldSearcher(fId), _folded(4_Ki) {
}
FUTF8StrChrFieldSearcher::~FUTF8StrChrFieldSearcher() = default;

bool FUTF8StrChrFieldSearcher::ansiFold(const char* toFold, size_t sz, char* folded) {
    bool retval(true);
...
bool FUTF8StrChrFieldSearcher::lfoldaa(const char* toFold, size_t sz, char* folded, size_t& unalignedStart) {
    ...
    unalignedStart = (foldedUnaligned < unalignedStart) ? (unalignedStart - foldedUnaligned)
                                                        : unalignedStart + 16 - foldedUnaligned;
    ...
    if (alignsz16 && retval) {
        const byte* end = sse2_foldaa(reinterpret_cast<const byte*>(toFold + unalignedsz), alignsz16,
                                      reinterpret_cast<byte*>(folded + alignedStart));
...
bool FUTF8StrChrFieldSearcher::lfoldua(const char* toFold, size_t sz, char* folded, size_t& alignedStart) {
    ...
    if (alignsz16) {
        const byte* end = sse2_foldua(reinterpret_cast<const byte*>(toFold), alignsz16,
                                      reinterpret_cast<byte*>(folded + alignedStart));
...
size_t FUTF8StrChrFieldSearcher::match(const char* folded, size_t sz, size_t mintsz, QueryTerm** qtl,
                                       size_t qtlSize) {
    (void)mintsz;
    termcount_t words(0);
    const char* n = folded;
...
size_t FUTF8StrChrFieldSearcher::matchTerm(const FieldRef& f, QueryTerm& qt) {
    ...
    if (ascii7Bit) {
        ...
        memset(folded + f.size() + 2, 0,
               16); // initialize padding data to avoid valgrind complaining about uninitialized values
