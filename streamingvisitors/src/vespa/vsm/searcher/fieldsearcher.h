...
public:
    ...
    void field(FieldIdT v) noexcept {
        _field = v;
        prepareFieldId();
    }
    static void init();
    ...
    FieldSearcher& maxFieldLength(uint32_t maxFieldLength_) {
        _maxFieldLength = maxFieldLength_;
        return *this;
    }
...
class FieldIdTSearcherMap : public FieldIdTSearcherMapT {
    void prepare_term(const DocumentTypeIndexFieldMapT& difm, search::streaming::QueryTerm* qt, FieldIdT fid,
                      vespalib::hash_set<const void*>& seen, search::streaming::QueryTermList& onlyInIndex);
