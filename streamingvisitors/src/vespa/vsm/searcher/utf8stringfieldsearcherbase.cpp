...
size_t UTF8StringFieldSearcherBase::matchTermRegular(const FieldRef& f, QueryTerm& qt) {
    termcount_t      words(0);
...
size_t UTF8StringFieldSearcherBase::matchTermExact(const FieldRef& f, QueryTerm& qt) {
    const cmptype_t* term;
    termsize_t       tsz = qt.term(term);
...
size_t UTF8StringFieldSearcherBase::matchTermSubstring(const FieldRef& f, QueryTerm& qt) {
    if (qt.termLen() == 0) {
        return 0;
    }
...
size_t UTF8StringFieldSearcherBase::matchTermSuffix(const FieldRef& f, QueryTerm& qt) {
    termcount_t      words = 0;
    const cmptype_t* term;
    termsize_t       tsz = qt.term(term);
    if (f.size() >= _buf->size()) {
...
UTF8StringFieldSearcherBase::UTF8StringFieldSearcherBase(FieldIdT fId) : StrChrFieldSearcher(fId) {
}

UTF8StringFieldSearcherBase::~UTF8StringFieldSearcherBase() = default;

void UTF8StringFieldSearcherBase::prepare(search::streaming::QueryTermList& qtl, const SharedSearcherBuf& buf,
...
bool UTF8StringFieldSearcherBase::matchTermSuffix(const cmptype_t* term, size_t termlen, const cmptype_t* word,
                                                  size_t wordlen) {
    if ((termlen <= wordlen)) {
...
bool UTF8StringFieldSearcherBase::isSeparatorCharacter(ucs4_t c) {
    return ((c < 0x20) && (c != '\n') && (c != '\t'));
}

template <typename T>
size_t UTF8StringFieldSearcherBase::skipSeparators(const search::byte* p, size_t sz, T& dstbuf) {
...
template unsigned long UTF8StringFieldSearcherBase::skipSeparators<UTF8StringFieldSearcherBase::BufferWrapper>(
    unsigned char const*, unsigned long, UTF8StringFieldSearcherBase::BufferWrapper&);
template unsigned long UTF8StringFieldSearcherBase::skipSeparators<UTF8StringFieldSearcherBase::OffsetWrapper>(
    unsigned char const*, unsigned long, UTF8StringFieldSearcherBase::OffsetWrapper&);
