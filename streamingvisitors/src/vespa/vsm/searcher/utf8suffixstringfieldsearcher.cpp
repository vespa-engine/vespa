...
std::unique_ptr<FieldSearcher> UTF8SuffixStringFieldSearcher::duplicate() const {
    return std::make_unique<UTF8SuffixStringFieldSearcher>(*this);
}

size_t UTF8SuffixStringFieldSearcher::matchTerms(const FieldRef& f, size_t mintsz) {
    (void)mintsz;
    termcount_t words = 0;
    if (f.size() >= _buf->size()) {
...
size_t UTF8SuffixStringFieldSearcher::matchTerm(const FieldRef& f, QueryTerm& qt) {
