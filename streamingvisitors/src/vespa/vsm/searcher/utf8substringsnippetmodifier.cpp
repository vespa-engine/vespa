...
std::unique_ptr<FieldSearcher> UTF8SubstringSnippetModifier::duplicate() const {
    return std::make_unique<UTF8SubstringSnippetModifier>(*this);
}

size_t UTF8SubstringSnippetModifier::matchTerms(const FieldRef& f, const size_t mintsz) {
    _modified->reset();
    _readPtr = f.data();
...
size_t UTF8SubstringSnippetModifier::matchTerm(const FieldRef& f, QueryTerm& qt) {
    const cmptype_t* term;
    termsize_t       tsz = qt.term(term);
    return matchTerms(f, tsz);
}

void UTF8SubstringSnippetModifier::copyToModified(size_t n, bool skipSep) {
    if (n == 0) {
        return;
    }
    if (skipSep) {
...
void UTF8SubstringSnippetModifier::insertSeparators(const char* mbegin, const char* mend) {
...
UTF8SubstringSnippetModifier::UTF8SubstringSnippetModifier(FieldIdT fId, const CharBuffer::SP& modBuf,
