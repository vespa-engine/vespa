...
template <> inline bool is_word_char<false>(ucs4_t c) {
    return Fast_UnicodeUtil::IsWordChar(c);
}

// All characters are treated as word characters for exact match
template <> inline constexpr bool is_word_char<true>(ucs4_t) {
    return true;
}

} // namespace

void TokenizeReader::fold(ucs4_t c) {
...
template <bool exact_match> size_t TokenizeReader::tokenize_helper(Normalizing norm_mode) {
    ucs4_t c(0);
    while (hasNext()) {
