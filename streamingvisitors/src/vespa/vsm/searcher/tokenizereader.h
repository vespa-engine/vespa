...
public:
    ...
    TokenizeReader(const byte* p, uint32_t len, ucs4_t* q) noexcept : _p(p), _p_end(p + len), _q(q), _q_start(q) {}
    ...
    template <bool exact_match> size_t tokenize_helper(Normalizing norm_mode);
