...
std::unique_ptr<FieldSearcher> BoolFieldSearcher::duplicate() const {
    return std::make_unique<BoolFieldSearcher>(*this);
}

BoolFieldSearcher::BoolFieldSearcher(FieldIdT fId) : FieldSearcher(fId), _terms() {
}

BoolFieldSearcher::~BoolFieldSearcher() = default;

void BoolFieldSearcher::prepare(search::streaming::QueryTermList& qtl, const SharedSearcherBuf& buf,
