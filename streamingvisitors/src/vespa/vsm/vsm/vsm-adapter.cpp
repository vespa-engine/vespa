...
GetDocsumsStateCallback::GetDocsumsStateCallback()
    : _summaryFeatures(), _rankFeatures(), _matching_elements_filler() {
}
...
void GetDocsumsStateCallback::set_matching_elements_filler(
...
DocsumTools::FieldSpec::FieldSpec() noexcept
    : _outputName(), _inputNames(), _command(VsmsummaryConfig::Fieldmap::Command::NONE) {
}
...
bool DocsumTools::obtainFieldNames(const FastS_VsmsummaryHandle& cfg) {
...
std::shared_ptr<const search::IDocumentIdProvider> DocsumTools::get_document_id_provider() const noexcept {
    return {};
}

void VSMAdapter::configure(const VSMConfigSnapshot& snapshot) {
    ...
    LOG(debug, "configureFields(): Size of cfg fieldspec: %zd",
        _fieldsCfg.get()->fieldspec.size()); // UlfC: debugging
    LOG(debug, "configureFields(): Size of cfg documenttype: %zd",
        _fieldsCfg.get()->documenttype.size());                                                  // UlfC: debugging
    ...
    std::unique_ptr<IQueryTermFilterFactory> query_term_filter_factory =
        std::make_unique<QueryTermFilterFactory>(*_fieldsCfg.get(), *vsmSummary);
    auto docsum_field_writer_factory =
        std::make_unique<DocsumFieldWriterFactory>(*docsumTools, *query_term_filter_factory, *_fieldsCfg.get());
...
VSMConfigSnapshot::VSMConfigSnapshot(const std::string& configId, const config::ConfigSnapshot& snapshot)
    : _configId(configId), _snapshot(std::make_unique<config::ConfigSnapshot>(snapshot)) {
}
...
VSMAdapter::VSMAdapter(const std::string& configId, const Fast_WordFolder& wordFolder)
    : _configId(configId), _wordFolder(wordFolder), _fieldsCfg(), _docsumTools(), _juniperProps(), _lock() {
}
