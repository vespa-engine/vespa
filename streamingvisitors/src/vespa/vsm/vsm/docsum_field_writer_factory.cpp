...
std::unique_ptr<DocsumFieldWriter> make_tokens_dfw(const std::string& source, VsmfieldsConfig& fields_config) {
    bool        exact_match = false;
    Normalizing normalize_mode = Normalizing::LOWERCASE;
    auto        it = std::find_if(fields_config.fieldspec.begin(), fields_config.fieldspec.end(),
                                  [&source](auto& fs) { return source == fs.name; });
...
DocsumFieldWriterFactory::DocsumFieldWriterFactory(
    const IDocsumEnvironment& env, const IQueryTermFilterFactory& query_term_filter_factory,
