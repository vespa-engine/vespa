...
TokensConverter::TokensConverter(bool exact_match, search::Normalizing normalize_mode)
    : IStringFieldConverter(), _text(), _exact_match(exact_match), _normalize_mode(normalize_mode) {
}

TokensConverter::~TokensConverter() = default;

void TokensConverter::convert(const StringFieldValue& input, Inserter& inserter) {
