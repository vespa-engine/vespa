...
DocsumFieldSpec::FieldIdentifier::FieldIdentifier() : _id(StringFieldIdTMap::npos), _path() {
}

DocsumFieldSpec::FieldIdentifier::FieldIdentifier(FieldIdT id, FieldPath path) : _id(id), _path(std::move(path)) {
}
...
DocsumFieldSpec::DocsumFieldSpec(VsmsummaryConfig::Fieldmap::Command command)
    : _struct_or_multivalue(false), _command(command), _outputField(), _inputFields(), _filter() {
}
...
void DocsumFieldSpec::set_filter(std::unique_ptr<search::docsummary::SlimeFillerFilter> filter) {
    _filter = std::move(filter);
}

const search::docsummary::SlimeFillerFilter* DocsumFieldSpec::get_filter() const noexcept {
    return _filter.get();
}
