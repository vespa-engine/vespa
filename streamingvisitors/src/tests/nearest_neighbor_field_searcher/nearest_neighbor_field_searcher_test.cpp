...
struct MockQuery {
    ...
    MockQuery& add(const std::string& query_tensor_name, uint32_t target_hits, double distance_threshold) {
        ...
        auto node = std::make_unique<NearestNeighborQueryNode>(std::move(base), query_tensor_name, "my_tensor_field",
                                                               target_hits, distance_threshold, 7,
                                                               search::query::Weight(100));
...
public:
    ...
    void set_query_tensor(const std::string& query_tensor_name, const std::string& spec_expr) {
        search::fef::indexproperties::type::QueryFeature::set(env.index_env.getProperties(), query_tensor_name,
                                                              tensor_type.to_spec());
    ...
    void prepare() { env.prepare(searcher, query.term_list); }
