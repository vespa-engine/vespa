...
protected:
    ...
    template <typename BW, bool OFF> void testSkipSeparators();

    TextUtilTest();
    ~TextUtilTest() override;
};

TextUtilTest::TextUtilTest() : ::testing::Test() {
}

TextUtilTest::~TextUtilTest() = default;

ucs4_t TextUtilTest::getUTF8Char(const char* src) {
...
template <typename BW, bool OFF>
void TextUtilTest::assertSkipSeparators(const char* input, size_t len, const UCS4V& expdstbuf,
                                        const SizeV& expoffsets) {
...
void TextUtilTest::assertAnsiFold(const std::string& toFold, const std::string& exp) {
    char folded[256];
    EXPECT_TRUE(FSFS::ansiFold(toFold.c_str(), toFold.size(), folded));
    EXPECT_EQ(std::string(folded, toFold.size()), exp);
}

void TextUtilTest::assertAnsiFold(char c, char exp) {
    char folded;
...
#ifdef __x86_64__
void TextUtilTest::assert_sse2_foldua(const std::string& toFold, size_t charFolded, const std::string& exp) {
    char                 folded[256];
...
void TextUtilTest::assert_sse2_foldua(unsigned char c, unsigned char exp, size_t charFolded) {
...
template <typename BW, bool OFF> void TextUtilTest::testSkipSeparators() {
    // ascii characters
    ...
    // utf8 char
    assertSkipSeparators<BW, OFF>("\xC2\x80\x66", 3, UCS4V().a(getUTF8Char("\xC2\x80")).a('f'), SizeV().a(0).a(2));
