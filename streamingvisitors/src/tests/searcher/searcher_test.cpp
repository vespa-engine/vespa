...
template <std::integral T> std::string_view maybe_consume_into(std::string_view str, T& val_out) {
...
std::tuple<uint8_t, uint32_t, bool, std::string_view> parse_fuzzy_params(std::string_view term) {
    ...
    if (term.empty() || (term[0] != ',' && term[0] != '}')) {
        throw std::invalid_argument("malformed fuzzy params at (or after) max_edits: " + std::string(term) +
                                    " in string " + std::string(orig_term));
    }
    if (term[0] == '}') {
    ...
    if (term.empty() || term[0] != '}') {
        throw std::invalid_argument("malformed fuzzy params at (or after) prefix_length: " + std::string(term) +
                                    " in string " + std::string(orig_term));
    }
    return {max_edits, prefix_length, prefix_match, term.substr(1)};
}
    ...
    void setupQuery(const StringList& terms, Normalizing normalizing) {
        for (const auto& term : terms) {
            ...
            if (pt.second == TermType::REGEXP) {
                qtv.push_back(std::make_unique<RegexpTerm>(eqnr.create(), pt.first, effective_index, TermType::REGEXP,
                                                           normalizing));
...
SnippetModifierSetup::SnippetModifierSetup(const StringList& terms)
    : query(terms), searcher(new UTF8SubstringSnippetModifier(0)), env(), modifier(searcher) {
...
bool operator==(const QTFieldInfo& lhs, const QTFieldInfo& rhs) {
    return lhs.getHitOffset() == rhs.getHitOffset() && lhs.getHitCount() == rhs.getHitCount() &&
...
void testStringFieldInfo(StrChrFieldSearcher& fs) {
...
void testStrChrFieldSearcher(StrChrFieldSearcher& fs) {
    ...
    { // test handling of several underscores
        ...
        EXPECT_EQ(exp, search_string(fs, query,
                                     "________________________________________foo____________________________________"
                                     "____bar________________________________________"));
        ...
        EXPECT_EQ(
            HitsList({{{0, 2}}}),
            search_string(fs, "bar", "foo____________________thisisaveryveryverylongword____________________bar"));
    }
}

void check_fuzzy_param_parsing(std::string_view term, std::string_view exp_term, uint8_t exp_max_edits,
                               uint32_t exp_prefix_length, bool exp_prefix) {
    uint8_t          max_edits = 0;
    uint32_t         prefix_length = 0;
    bool             prefix = false;
    std::string_view out;
...
void testUTF8SubStringFieldSearcher(StrChrFieldSearcher& fs) {
...
TEST(SearcherTest, Snippet_modifier_search) {
    // ascii
    assertSnippetModifier("f", "foo",
                          "\x1F"
                          "f\x1Foo");
    assertSnippetModifier("o", "foo", "f\x1Fo\x1F\x1Fo\x1F");
    assertSnippetModifier("r", "bar", "ba\x1Fr\x1F");
    assertSnippetModifier("foo", "foo foo",
                          "\x1F"
                          "foo\x1F \x1F"
                          "foo\x1F");
    assertSnippetModifier("aa", "aaaaaa",
                          "\x1F"
                          "aa\x1F\x1F"
                          "aa\x1F\x1F"
                          "aa\x1F");
    assertSnippetModifier("ab",
                          "abcd\x1F"
                          "efgh",
                          "\x1F"
                          "ab\x1F"
                          "cd\x1F"
                          "efgh");
    assertSnippetModifier("ef",
                          "abcd\x1F"
                          "efgh",
                          "abcd\x1F\x1F"
                          "ef\x1Fgh");
    assertSnippetModifier("fg",
                          "abcd\x1F"
                          "efgh",
                          "abcd\x1F"
                          "e\x1F"
    ...
    assertSnippetModifier(StringList().add("ab").add("cd"), "abcd",
                          "\x1F"
                          "ab\x1F\x1F"
                          "cd\x1F");
    // when we have overlap we only get the first match
    assertSnippetModifier(StringList().add("ab").add("bc"), "abcd",
                          "\x1F"
                          "ab\x1F"
                          "cd");
    assertSnippetModifier(StringList().add("bc").add("ab"), "abcd",
                          "\x1F"
                          "ab\x1F"
                          "cd");
    // the separator overlapping the match is skipped
    assertSnippetModifier(StringList().add("de").add("ef"),
                          "abcd\x1F"
                          "efgh",
                          "abc\x1F"
                          "de\x1F"
...
TEST(SearcherTest, snippet_modifier) {
    { // string field value
        ...
        assertSnippetModifier(sms, StringFieldValue("ab"),
                              "\x1F"
                              "ab\x1F");
        assertSnippetModifier(sms, StringFieldValue("xxxxabxxxxabxxxx"),
                              "xxxx\x1F"
                              "ab\x1Fxxxx\x1F"
                              "ab\x1Fxxxx");
        assertSnippetModifier(sms, StringFieldValue("xxabxx"),
                              "xx\x1F"
                              "ab\x1Fxx");
    }
    { // collection field value
        SnippetModifierSetup sms(StringList().add("ab"));
        // multiple invocations
        assertSnippetModifier(sms, getFieldValue(StringList().add("ab")),
                              "\x1F"
                              "ab\x1F");
        assertSnippetModifier(sms, getFieldValue(StringList().add("xxabxx")),
                              "xx\x1F"
                              "ab\x1Fxx");
...
void verifyNormalization(Normalizing normalizing, size_t expected_len, const char* expected) {
    ucs4_t         buf[256];
    TokenizeReader reader(reinterpret_cast<const search::byte*>(NormalizationInput.c_str()),
                          NormalizationInput.size(), buf);
...
TEST(SearcherTest, test_normalizing) {
    ...
    verifyNormalization(Normalizing::LOWERCASE_AND_FOLD, 54,
                        "test that somehing happens with during naarmoelization");
}
