...
public class ApplicationFileManager implements AddFileInterface {
    ...
    public FileReference addBlob(ByteBuffer blob, Path path) {
        try (TmpDir tmp = new TmpDir()) {
            return addFile(writeBlob(blob, tmp.dir, path.getRelative()));
        }
        catch (IOException e) {
            throw new IllegalArgumentException(e);
        }
    ...
    private File writeBlob(ByteBuffer blob, File tmpDir, String relativePath) {
        FileOutputStream fos = null;
        File file = null;
        try {
            file = new File(tmpDir, relativePath);
    ...
    private File download(String uri, File tmpDir, String relativePath) {
        File file = null;
        FileOutputStream fos = null;
