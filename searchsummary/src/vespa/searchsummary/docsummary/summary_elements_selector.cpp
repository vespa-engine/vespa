...
std::vector<uint32_t> extract_elements_from_feature(const FeatureSet::Value& encoded_value) {
...
SummaryElementsSelector::SummaryElementsSelector()
    : _selector(Selector::ALL), _field(), _struct_fields(), _summary_feature() {
}
...
SummaryElementsSelector SummaryElementsSelector::select_all() {
    return {};
}

SummaryElementsSelector SummaryElementsSelector::select_by_match(const std::string&       field,
...
SummaryElementsSelector SummaryElementsSelector::select_by_summary_feature(const std::string& summary_feature) {
    SummaryElementsSelector elements_selector;
...
void SummaryElementsSelector::apply_to(MatchingElementsFields& target) const {
...
ElementIds SummaryElementsSelector::get_selected_elements(uint32_t docid, GetDocsumsState& state) const {
    switch (_selector) {
    case Selector::ALL:
...
const std::vector<uint32_t>& SummaryElementsSelector::get_summary_feature_elements(uint32_t         docid,
