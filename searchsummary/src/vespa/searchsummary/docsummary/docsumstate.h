...
public:
    ...
    virtual std::unique_ptr<MatchingElements>
    fill_matching_elements(const MatchingElementsFields& matching_elems_fields) = 0;
...
public:
    ...
    class DynTeaserState {
        vespalib::hash_map<std::string, std::unique_ptr<juniper::QueryHandle>>
            _queries; // juniper query representations
    public:
        DynTeaserState();
        ~DynTeaserState();
