...
JuniperProperties::JuniperProperties() : _properties() {
    reset();
}

JuniperProperties::JuniperProperties(const JuniperrcConfig& cfg) : _properties() {
    reset();
    configure(cfg);
}
...
void JuniperProperties::configure(const JuniperrcConfig& cfg) {
    reset();
    ...
    for (const auto& override : cfg.override) {
        ...
        _properties[keyMatcher + "winsize_fallback_multiplier"] =
            make_string("%f", override.winsizeFallbackMultiplier);
...
const char* JuniperProperties::GetProperty(const char* name, const char* def) const {
