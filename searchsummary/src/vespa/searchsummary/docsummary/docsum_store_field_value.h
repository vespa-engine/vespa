...
public:
    explicit DocsumStoreFieldValue(std::unique_ptr<document::FieldValue> value) noexcept
        : _value(value.get()), _value_store(std::move(value)) {}
    explicit DocsumStoreFieldValue(const document::FieldValue* value) noexcept : _value(value), _value_store() {}
    DocsumStoreFieldValue() : DocsumStoreFieldValue(nullptr) {}
