...
SlimeFillerFilter::Iterator::Iterator(bool should_render_in) noexcept : _should_render(should_render_in), _next() {
}

SlimeFillerFilter::Iterator::Iterator(const SlimeFillerFilter* next) noexcept : _should_render(true), _next(next) {
}
...
SlimeFillerFilter::SlimeFillerFilter() : _filter() {
}

SlimeFillerFilter::~SlimeFillerFilter() = default;

SlimeFillerFilter::Iterator SlimeFillerFilter::check_field(std::string_view field_name) const {
...
SlimeFillerFilter::Iterator SlimeFillerFilter::begin() const {
    return SlimeFillerFilter::Iterator(this);
}

bool SlimeFillerFilter::empty() const {
    return _filter.empty();
}

SlimeFillerFilter& SlimeFillerFilter::add(std::string_view field_path) {
...
void SlimeFillerFilter::add_remaining(std::unique_ptr<SlimeFillerFilter>& filter, std::string_view field_path) {
    if (filter) {
        auto dot_pos = field_path.find('.');
        if (dot_pos != std::string::npos) {
...
SlimeFillerFilter::Iterator SlimeFillerFilter::all() {
