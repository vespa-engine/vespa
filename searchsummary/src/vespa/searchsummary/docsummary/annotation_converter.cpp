...
std::string_view getSpanString(std::string_view s, const Span& span) {
...
AnnotationConverter::AnnotationConverter(IJuniperConverter& juniper_converter)
    : IStringFieldConverter(), _juniper_converter(juniper_converter), _text(), _out() {
}

AnnotationConverter::~AnnotationConverter() = default;

template <typename ForwardIt>
void AnnotationConverter::handleAnnotations(const document::Span& span, ForwardIt it, ForwardIt last) {
    int annCnt = (last - it);
...
template <typename ForwardIt>
void AnnotationConverter::annotateSpans(const document::Span& span, ForwardIt it, ForwardIt last) {
    _out << juniper::separators::interlinear_annotation_anchor_string                                     // ANCHOR
         << (getSpanString(_text, span)) << juniper::separators::interlinear_annotation_separator_string; // SEPARATOR
    while (it != last) {
        _out << it->word;
        if (++it != last) {
            _out << " ";
...
void AnnotationConverter::handleIndexingTerms(const StringFieldValue& value) {
...
void AnnotationConverter::convert(const StringFieldValue& input, vespalib::slime::Inserter& inserter) {
    _out.clear();
...
bool AnnotationConverter::render_weighted_set_as_array() const {
    return false;
}
