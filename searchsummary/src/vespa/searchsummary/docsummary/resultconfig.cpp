...
SummaryElementsSelector make_summary_elements_selector(const SummaryConfig::Classes::Fields::Elements& elements,
...
void ResultConfig::Clean() {
    _classLookup.clear();
    _nameLookup.clear();
}

ResultConfig::ResultConfig() : _defaultSummaryId(-1), _classLookup(), _nameLookup() {
}
...
void ResultConfig::reset() {
    if (!_classLookup.empty()) {
        Clean();
    }
}

ResultClass* ResultConfig::addResultClass(const std::string& name, uint32_t classID) {
...
void ResultConfig::set_default_result_class_id(uint32_t id) {
    _defaultSummaryId = id;
}

const ResultClass* ResultConfig::lookupResultClass(uint32_t classID) const {
    auto it = _classLookup.find(classID);
    return (it != _classLookup.end()) ? it->second.get() : nullptr;
}

uint32_t ResultConfig::lookupResultClassId(std::string_view name) const {
    auto found = _nameLookup.find(name);
    return (found != _nameLookup.end()) ? found->second
                                        : ((name.empty() || (name == "default")) ? _defaultSummaryId : noClassID);
}

bool ResultConfig::readConfig(const SummaryConfig& cfg, const std::string& configId,
                ...
                if (!command.empty()) {
                    try {
                        ...
                        LOG(error,
                            ...
                            "source='%s': %s",
                            cfg_class.name.c_str(), field_name.c_str(), command.c_str(), source_name.c_str(),
                            ex.getMessage().c_str());
                        rc = false;
                    }
                }
