...
void Appender::append(std::vector<char>& s, char c) {
    ...
    if (_sumconf->separator(c)) { return; }
    ...
    if (_escape_markup) {
        ...
        case '\'':
            ...
            s.push_back(';');
            break;
        default: handled_as_markup = false; break;
        }
        if (handled_as_markup) { _char_len++; }
    } else {
...
Appender::Appender(const SummaryConfig* sumconf)
  : _sumconf(sumconf),
    ...
    switch (esc_conf) {
    case CF_OFF: _escape_markup = false; break;
    case CF_ON:  _escape_markup = true; break;
    case CF_AUTO:
        _escape_markup = (_sumconf->highlight_on()[0] == '<' || _sumconf->highlight_off()[0] == '<' ||
                          _sumconf->dots()[0] == '<');
        break;
    }

    if (_sumconf->preserve_white_space() == CF_ON) { _preserve_white_space = true; }
}

void Appender::append(std::vector<char>& s, const char* ds, int length) {
    for (int i = 0; i < length; i++) { append(s, ds[i]); }
}
