...
public:
    ...
    inline RewriteHandle* Rewrite(uint32_t langid, const char* term) { return _rewriter->Rewrite(langid, term); }
    inline RewriteHandle* Rewrite(uint32_t langid, const char* term, size_t len) {
        return _rewriter->Rewrite(langid, term, len);
    }
    inline const char* NextTerm(RewriteHandle* exp, size_t& length) { return _rewriter->NextTerm(exp, length); }
