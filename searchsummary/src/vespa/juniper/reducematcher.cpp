...
ReduceMatcher::ReduceMatcher() : _matchers() {}
...
string_matcher* ReduceMatcher::find(Rewriter* rw) {
    ...
    if (!sm) sm = _matchers.insert(rw, new string_matcher(rw));
    return sm;
}

const std::vector<QueryTerm*>* ReduceMatcher::match(uint32_t langid, const char* term, size_t len) {
    std::vector<QueryTerm*>* vp = NULL;
    // Try each of the matchers
    for (std::map<Rewriter*, string_matcher*>::iterator mit = _matchers.begin(); mit != _matchers.end(); ++mit) {
        ...
        while (eterm) {
            ...
            if (LOG_WOULD_LOG(spam)) {
                std::string s(m->dump());
                LOG(spam, "(reduction) matching '%s' with %s", t.c_str(), s.c_str());
            }
            if (sit != m->table().end()) {
                if (!vp) {
                    vp = new std::vector<QueryTerm*>(sit->second);
                } else {
...
std::string string_matcher::dump() {
    std::string s("[");
    for (string_match_table::iterator it = _table.begin(); it != _table.end(); ++it) {
        s = s + it->first.c_str() + " ";
    }
    s = s + "]";
    return s;
