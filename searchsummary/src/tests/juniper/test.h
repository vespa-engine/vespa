...
#define _test_equal(lhs, rhs) do_equality_test((lhs), (rhs), #lhs, __FILE__, __LINE__)
...
protected:
    ...
    bool do_test(bool cond, const std::string& lbl, const char* fname, long lineno);
    bool do_fail(const std::string& lbl, const char* fname, long lineno, bool addEndl = true);
    template <typename t1, typename t2>
    bool         do_equality_test(const t1& lhs, const t2& rhs, const char* lbl, const char* fname, long lineno);
...
template <typename t1, typename t2>
bool Test::do_equality_test(const t1& lhs, const t2& rhs, const char* lbl, const char* fname, long lineno) {
    if (lhs == rhs) {
        _Succeed();
        print_progress();
        return true;
    ...
    if (m_osptr) {
        *m_osptr << "Equality test failed: "
        ...
        if (print_desc() > 0) *m_osptr << std::endl << std::endl;
    }
    return false;
}
