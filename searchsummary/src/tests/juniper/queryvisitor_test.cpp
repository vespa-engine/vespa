...
template <typename QueryType> struct Fixture {
    QueryType    query;
    QueryHandle  handle;
    QueryVisitor visitor;
    explicit Fixture(const std::string& term) : query(term), handle(query, ""), visitor(query, &handle) {}
    ~Fixture();
};

template <typename QueryType> Fixture<QueryType>::~Fixture() = default;
