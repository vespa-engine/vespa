...
inline Suite::Suite(const std::string& name, std::ostream* osptr) : m_name(name), m_osptr(osptr), m_tests() {}

inline std::string Suite::GetName() const {
    return m_name;
}

inline const std::ostream* Suite::GetStream() const {
    return m_osptr;
}

inline void Suite::SetStream(std::ostream* osptr) {
    m_osptr = osptr;
}
...
{
    for (size_t i = 0; i < s.m_tests.size(); ++i) AddTest(s.m_tests[i]);
}
...
void Suite::Run() {
    ...
    for (size_t i = 0; i < m_tests.size(); ++i) {
        ...
        if (nm) {
            *m_osptr << std::endl << nm << ": ";
            for (x = longestName - strlen(nm); x > 0; --x) *m_osptr << ' ';
            *m_osptr << std::flush;
        }
        m_tests[i]->Run();
    }
...
int Suite::GetLongestName() const {
    ...
    for (size_t i = 0; i < m_tests.size(); ++i) {
        assert(m_tests[i]);
        nm = m_tests[i]->get_name();
        if (nm != NULL && (len = strlen(nm)) > longestName) longestName = len;
    }
    return longestName;
}
...
long Suite::Report() const {
    if (m_osptr) {
        ...
        long totFail = 0;
        int  x = 0;
        *m_osptr << std::endl << std::endl << "Suite \"" << m_name << "\"" << std::endl;
        for (x = 0; x < lineLength; ++x) *m_osptr << '=';
        *m_osptr << "=";
        ...
        for (size_t i = 0; i < m_tests.size(); ++i) {
            ...
            totFail += m_tests[i]->Report(longestName - (nm ? strlen(nm) : longestName));
        }
