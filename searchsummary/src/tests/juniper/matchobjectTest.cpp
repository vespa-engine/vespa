...
void MatchObjectTest::testMatch() {
...

    {
        ...
        juniper::Result   res1(*juniper::TestConfig, qh1, doc.c_str(), doc.size(), 0);
...
void MatchObjectTest::testMatchAnnotated() {
    ...
    QueryHandle&      qh1(q._qhandle);
    juniper::Result   res1(*juniper::TestConfig, qh1, doc, strlen(doc), 0);
    juniper::Summary* sum = res1.GetTeaser(NULL);
    std::string       s(sum->Text());
...
void MatchObjectTest::testLangid() {
    ...
    {
        {
            ...
            _test_equal(stk, "Node<a:3>[Node<a:4>[a0:100,a1:100,a2:100,a3:100],"
        ...
        std::string       s(sum->Text());
        _test_equal(s, "see if we can match <b>b</b> or <b>c</b> somewhere in this"
...

    {
        ...
        juniper::Summary* sum = res.GetTeaser(NULL);
        std::string       s(sum->Text());
        _test_equal(s, "Try to run this on another doc just to see if <b>b</b> or <b>c</b>"
...
void MatchObjectTest::testCombined() {
...

    {
        ...
        juniper::Summary* sum = res.GetTeaser(NULL);
        std::string       s(sum->Text());
        _test_equal(s, "see if we can match <b>a3</b> or c somewhere in this <b>b</b> doc."
                       " Note that we should not match b1 or c1 or a somewhere..");
    }
    juniper::_Juniper->FlushRewriters();
}
...
void MatchObjectTest::init() {
    ...
    test_methods_["testLangid"] = &MatchObjectTest::testLangid;
    test_methods_["testCombined"] = &MatchObjectTest::testCombined;
