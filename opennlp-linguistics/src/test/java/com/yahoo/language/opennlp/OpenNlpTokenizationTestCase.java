...
public class OpenNlpTokenizationTestCase {
    ...
    public void testTokenizerCanPreserveCasing() {
        var keepCasing = new LinguisticsParameters("default", true, Language.ENGLISH, StemMode.SHORTEST, false, false);
        var tester = new OpenNlpLinguisticsTester();
    ...
    public void testPhrasesWithPunctuation() {
        var parameters = new LinguisticsParameters("default", true, Language.ENGLISH, StemMode.NONE, false, true);
        var tester = new OpenNlpLinguisticsTester();
    ...
    public void testDoubleWidthTokenization() {
        var noStems = new LinguisticsParameters("default", true, Language.ENGLISH, StemMode.NONE, false, true);
        var shortestStem = new LinguisticsParameters("default", true, Language.ENGLISH, StemMode.SHORTEST, false, true);
        var tester = new OpenNlpLinguisticsTester();
        // "sony"
        ...
        tester.assertTokenize("\uFF2F\uFF2E", noStems, List.of("on"), null);
        tester.assertTokenize("\uFF2F\uFF2E", shortestStem, List.of("on"), null);
        var russianShortestStem = new LinguisticsParameters("default", true, Language.RUSSIAN, StemMode.SHORTEST, false, true);
    ...
    public void testGetOffsetLength() {
        var parameters = new LinguisticsParameters("default", true, Language.GERMAN, StemMode.SHORTEST, false, true);
        var tester = new OpenNlpLinguisticsTester();
    ...
    public void testRecursiveDecompose() {
        var parameters = new LinguisticsParameters("default", true, Language.ENGLISH, StemMode.SHORTEST, false, true);
        var tester = new OpenNlpLinguisticsTester();
        ...
        for (StemMode stemMode : new StemMode[] { StemMode.NONE, StemMode.SHORTEST }) {
            for (Language l : List.of(Language.INDONESIAN, Language.ENGLISH, Language.ARABIC)) {
                for (boolean accentDrop : new boolean[] { true, false }) {
                    var parameters = new LinguisticsParameters("default", true, l, stemMode, accentDrop, true);
                    for (Token token : tester.tokenizer().tokenize(input, parameters)) {
    ...
    public void testStemGreek() {
        var parameters = new LinguisticsParameters("default", true, Language.GREEK, StemMode.ALL, true, true);
        var stemmer = new OpenNlpLinguistics().getStemmer();
        String input = "οὖν";
        String output = "ουν";
