...
public class VespaSSLConfig implements SSLConfig {
    ...
    public SSLContext getSSLContext() {
        try {
            ...
            String keypem, certpem, capem;
            try {
                keypem = new String(Files.readAllBytes(Paths.get(config.clientKeyPath())));
            } catch (NoSuchFileException e) {
                throw new RuntimeException("Could not read client key at: " + config.clientKeyPath());
            }

            try {
                certpem = new String(Files.readAllBytes(Paths.get(config.clientCertPath())));
            } catch (NoSuchFileException e) {
                throw new RuntimeException("Could not read client certificate at: " + config.clientCertPath());
            }

            try {
                capem = new String(Files.readAllBytes(Paths.get(config.caCertPath())));
            } catch (NoSuchFileException e) {
                throw new RuntimeException("Could not read CA certificate at: " + config.caCertPath());
            }
    ...
    private static KeyStore createKeyStore(final String keypem, final String certpem) throws NoSuchAlgorithmException,
     ...
     * "ca.pem" from String
     */
    private static KeyStore createTrustStore(String capem) throws IOException, CertificateException,
     ...
     * "ca.pem" from Reader
     */
    private static KeyStore createTrustStore(final Reader certReader) throws IOException, CertificateException,
            KeyStoreException, NoSuchAlgorithmException {
