...
public record SidecarResources(Double maxCpu, Double minCpu, Double memoryGiB, String gpu) {
    ...
    public SidecarResources {
        ...
        if (maxCpu < 0) {
            throw new IllegalArgumentException("maxCpu must be non-negative");
        }
        
        if (minCpu < 0) {
            throw new IllegalArgumentException("minCpu must be non-negative");
        }
        
        if (maxCpu < minCpu) {
            throw new IllegalArgumentException("maxCpu must be greater than or equal to minCpu");
        }
        
        if (memoryGiB < 0) {
            throw new IllegalArgumentException("memoryGiB must be non-negative");
        }
    }
}
