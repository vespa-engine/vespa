...
public class StringFieldValue extends FieldValue {
    ...
    private boolean equalsSimpleAnnotations(StringFieldValue that) {
        if (simpleAnnotations == null || that.simpleAnnotations == null) {
            return simpleAnnotations == that.simpleAnnotations;
        }
        if (simpleAnnotations.getCount() != that.simpleAnnotations.getCount()) {
            return false;
        }
        for (int i = 0; i < simpleAnnotations.getCount(); i++) {
            if (simpleAnnotations.getFrom(i) != that.simpleAnnotations.getFrom(i) ||
                simpleAnnotations.getLength(i) != that.simpleAnnotations.getLength(i) ||
                !Objects.equals(simpleAnnotations.getTerm(i), that.simpleAnnotations.getTerm(i))) {
                return false;
            }
        }
        return true;
    ...
    public boolean equals(Object o) {
        ...
        if (thisMode == AnnotationMode.SIMPLE && thatMode == AnnotationMode.SIMPLE) {
            return equalsSimpleAnnotations(that);
        }
