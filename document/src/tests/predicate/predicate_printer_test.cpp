...
TEST(PredicatePrinterTest, require_that_PredicatePrinter_prints_FeatureSets) {
    ...
    ASSERT_EQ("'foo' in ['bar','baz']", PredicatePrinter::print(*builder.build()));

    builder.feature("foo").value("bar");
    ASSERT_EQ("'foo' in ['bar']", PredicatePrinter::print(*builder.build()));
}

TEST(PredicatePrinterTest, require_that_PredicatePrinter_escapes_non_ascii_characters) {
    PredicateSlimeBuilder builder;
    builder.feature("\n\t\001'").value("\xc3\xb8");
    ASSERT_EQ("'\\n\\t\\x01\\x27' in ['\\xc3\\xb8']", PredicatePrinter::print(*builder.build()));
}

TEST(PredicatePrinterTest, require_that_PredicatePrinter_prints_FeatureRanges) {
    PredicateSlimeBuilder builder;
    builder.feature("foo").range(-10, 42);
    ASSERT_EQ("'foo' in [-10..42]", PredicatePrinter::print(*builder.build()));
}
...
TEST(PredicatePrinterTest, require_that_PredicatePrinter_prints_open_ended_FeatureRanges) {
    ...
    ASSERT_EQ("'foo' in [-10..]", PredicatePrinter::print(*builder.build()));
...
TEST(PredicatePrinterTest, require_that_PredicatePrinter_prints_NOT_IN_FeatureSets) {
    ...
    ASSERT_EQ("'foo' not in ['bar','baz']", PredicatePrinter::print(*builder.build()));
}
...
TEST(PredicatePrinterTest, require_that_PredicatePrinter_prints_AND_expressions) {
    auto slime = andNode({featureSet("foo", {"bar", "baz"}), featureSet("foo", {"bar", "baz"})});
    ASSERT_EQ("('foo' in ['bar','baz'] and 'foo' in ['bar','baz'])", PredicatePrinter::print(*slime));
}

TEST(PredicatePrinterTest, require_that_PredicatePrinter_prints_OR_expressions) {
    auto slime = orNode({featureSet("foo", {"bar", "baz"}), featureSet("foo", {"bar", "baz"})});
    ASSERT_EQ("('foo' in ['bar','baz'] or 'foo' in ['bar','baz'])", PredicatePrinter::print(*slime));
}

TEST(PredicatePrinterTest, require_that_PredicatePrinter_can_negate_OR_expressions) {
    auto slime = neg(orNode({featureSet("foo", {"bar", "baz"}), featureSet("foo", {"bar", "baz"})}));
    ASSERT_EQ("not ('foo' in ['bar','baz'] or 'foo' in ['bar','baz'])", PredicatePrinter::print(*slime));
}

TEST(PredicatePrinterTest, require_that_PredicatePrinter_can_negate_AND_expressions) {
    auto slime = neg(andNode({featureSet("foo", {"bar", "baz"}), featureSet("foo", {"bar", "baz"})}));
    ASSERT_EQ("not ('foo' in ['bar','baz'] and 'foo' in ['bar','baz'])", PredicatePrinter::print(*slime));
}
