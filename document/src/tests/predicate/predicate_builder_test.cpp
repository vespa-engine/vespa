...
TEST(PredicateBuilderTest, require_that_a_predicate_tree_can_be_built_from_a_slime_object) {
    ...
    }
    {
        ...
        Cursor& and_children = child.setArray(Predicate::CHILDREN);
        {
            ...
            and_child.setLong(Predicate::NODE_TYPE, Predicate::TYPE_FEATURE_RANGE);
        ...
        }
        {
            ...
            Cursor& not_child = and_child.setArray(Predicate::CHILDREN).addObject();
            {
                not_child.setLong(Predicate::NODE_TYPE, Predicate::TYPE_FEATURE_SET);
    ...
    FeatureRange* feature_range = dynamic_cast<FeatureRange*>((*conjunction)[0]);
