...
template <typename IdType> const IdType& getAs(const DocumentId& doc_id) {
    return dynamic_cast<const IdType&>(doc_id.getScheme());
}

template <typename IdType> void checkUser(const string& id, uint64_t user_id) {
    DocumentId doc_id(id);
...
TEST(DocumentIdTest, require_that_id_ids_n_key_must_be_a_64_bit_number) {
    VESPA_EXPECT_EXCEPTION(DocumentId("id:ns:type:n=abc:foo"), IdParseException, "'n'-value must be a 64-bit number");
    DocumentId("id:ns:type:n=18446744073709551615:foo"); // ok
    VESPA_EXPECT_EXCEPTION(DocumentId("id:ns:type:n=18446744073709551616:foo"), IdParseException,
...
TEST(DocumentIdTest, require_that_id_id_can_specify_group) {
    ...
    EXPECT_NE(id1.getScheme().getLocation(), id3.getScheme().getLocation());
...
TEST(DocumentIdTest, require_that_illegal_ids_fail) {
    VESPA_EXPECT_EXCEPTION(DocumentId("idg:foo:bar:baz"), IdParseException, "No scheme separator ':' found");
    VESPA_EXPECT_EXCEPTION(DocumentId("id:"), IdParseException, "too short");
    VESPA_EXPECT_EXCEPTION(DocumentId("id:ns"), IdParseException, "No namespace separator ':' found");
    VESPA_EXPECT_EXCEPTION(DocumentId("id:ns:type"), IdParseException, "No document type separator ':' found");
    VESPA_EXPECT_EXCEPTION(DocumentId("id:ns:type:kv_pair"), IdParseException,
                           "No key/value-pairs separator ':' found");
    VESPA_EXPECT_EXCEPTION(DocumentId("id:ns:type:k=foo:bar"), IdParseException, "Illegal key 'k'");
