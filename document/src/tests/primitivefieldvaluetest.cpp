...
template <typename Literal> void testLiteral() {
    testCommon(Literal(), Literal("bar"), Literal("bar"), Literal("foo"));
    Literal value("foo");
    // Textual output
...
TEST(PrimitiveFieldValueTest, testRaw) {
    ...
    // Textual output
    EXPECT_EQ(std::string("0: 09 66 6f 6f 00 0d 0a                            .foo..."), value.toString(false, ""));
    EXPECT_EQ(std::string("0: 09 66 6f 6f 00 0d 0a                            .foo..."), value.toString(true, "  "));
    EXPECT_EQ(std::string("<value binaryencoding=\"base64\">CWZvbwANCg==</value>\n"), value.toXml("  "));

    value.setValue("grmpf", 4);
    EXPECT_TRUE(strncmp("grmpf", value.getValueRef().data(), value.getValueRef().size()) == 0);
}
...
template <typename Numeric> void testNumeric(const std::string &maxVal, bool floatingPoint) {
    ...
    testCommon(Numeric(), Numeric(Number(1)), Numeric(Number(1)), Numeric(Number(maxValue)));
    Numeric value;
