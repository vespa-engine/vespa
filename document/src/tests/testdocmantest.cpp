...
TEST(TestDocManTest, testSimpleUsage) {
    ...
    Document::UP doc3(testdm.createRandomDocument(1));
    {
        ...
        EXPECT_EQ(std::string("To be, or "), std::string(sval.getValue().c_str()));

        FieldValue::UP    v2(doc2->getValue(doc2->getField("content")));
        StringFieldValue& sval2(dynamic_cast<StringFieldValue&>(*v));
        EXPECT_EQ(std::string(sval.getValue().c_str()), std::string(sval2.getValue().c_str()));
    }
    {
    ...
    EXPECT_EQ(std::string("id:mail:testdoctype1:n=51019:192.html"), doc1->getId().toString());
    EXPECT_EQ(std::string("id:mail:testdoctype1:n=51019:192.html"), doc2->getId().toString());
    EXPECT_EQ(std::string("id:mail:testdoctype1:n=10744:245.html"), doc3->getId().toString());
}
