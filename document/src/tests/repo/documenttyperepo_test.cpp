...
TEST(DocumentTypeRepoTest, requireThatDocumentsCanUseInheritedTypes) {
    ...
    const DataType& type = repo.getDocumentType(doc_type_id + 1)->getFieldsType().getField(field_name).getDataType();
...
TEST(DocumentTypeRepoTest, requireThatDataTypesCanBeLookedUpById) {
    ...
    const DataType* type = repo.getDataType(*repo.getDocumentType(doc_type_id), header_id);
    ASSERT_TRUE(type);
    ...
    ASSERT_TRUE(!repo.getDataType(*repo.getDocumentType(doc_type_id + 1), header_id));
}
...
TEST(DocumentTypeRepoTest, requireThatDocumentsCanUseOtherDocumentTypes) {
    ...
    const DataType& type = repo.getDocumentType(doc_type_id)->getFieldsType().getField(type_name_2).getDataType();
...
TEST(DocumentTypeRepoTest, Configured_imported_field_names_are_available_in_the_DocumentType) {
    ...
    builder.document(type_name, doc_type_id).imported_field("my_cool_field");
    // Type with two imported fields
    builder.document(type_name_2, type_2_id).imported_field("my_awesome_field").imported_field("my_swag_field");
