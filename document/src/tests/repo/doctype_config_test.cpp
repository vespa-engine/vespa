...
template <typename T> const T& getFieldDataType(const DocumentTypeRepo& repo) {
    const DataType& d = repo.getDocumentType(type_name)->getFieldsType().getField(field_name).getDataType();
...
TEST(DoctypeConfigTest, requireThatMapsCanBeConfigured) {
    BuilderHelper builder;
    auto&         doc = builder.document(type_name);
    auto&         map = builder.addMap(doc, builder.builtin(DataType::T_INT), builder.builtin(DataType::T_STRING));
...
TEST(DoctypeConfigTest, requireThatDocumentsCanUseInheritedTypes) {
    ...
    const DataType& type = repo.getDocumentType(derived_name)->getFieldsType().getField(field_name).getDataType();
...
TEST(DoctypeConfigTest, requireThatIllegalConfigsCausesExceptions) {
    ...
    VESPA_EXPECT_EXCEPTION(DocumentTypeRepo repo(builder.config()), IllegalArgumentException,
...
TEST(DoctypeConfigTest, requireThatDocumentsCanUseOtherDocumentTypes) {
    ...
    const DataType& type = repo.getDocumentType(type_name)->getFieldsType().getField(field_name).getDataType();
...
TEST(DoctypeConfigTest, requireThatDocumentTypesCanBeIterated) {
    ...
    set<int> ids;
    repo.forEachDocumentType([&ids](const DocumentType& type) { ids.insert(type.getId()); });
...
TEST(DoctypeConfigTest, requireThatMissingFileCausesException) {
    VESPA_EXPECT_EXCEPTION(readDocumenttypesConfig("illegal/missing_file"), IllegalArgumentException,
                           "Unable to open file");
}
...
const TensorDataType& asTensorDataType(const DataType& dataType) {
