...
template <typename Type1, typename Type2> void verifyFailedAssignment(Type1& lval, const Type2& rval) {
    try {
        lval = rval;
...
template <typename Type> void verifyFailedUpdate(Type& lval, const FieldValue& rval) {
    try {
        lval.add(rval);
...
TEST(WeightedSetFieldValueTest, testWeightedSet) {
    ...
    for (WeightedSetFieldValue::const_iterator it = constVal.begin(); it != constVal.end(); ++it) {
    ...
    value2 = value;
    for (WeightedSetFieldValue::iterator it = value2.begin(); it != value2.end(); ++it) {
    ...
    // Output
    EXPECT_EQ(std::string("WeightedSet<Int>(\n"
                          "  1 - weight 1,\n"
                          "  2 - weight 5,\n"
                          "  3 - weight 6\n"
                          ")"),
