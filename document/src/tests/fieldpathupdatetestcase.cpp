...
protected:
    ...
    DocumentUpdate::UP createDocumentUpdateForSerialization(const DocumentTypeRepo& repo);
};
...
void FieldPathUpdateTestCase::SetUp() {
...
TEST_F(FieldPathUpdateTestCase, testApplyRemoveMultiList) {
    ...
    docUp.addFieldPathUpdate(
        std::make_unique<RemoveFieldPathUpdate>("strarray[$x]", "foobar.strarray[$x] == \"remove val 1\""));
    docUp.applyTo(*doc);
    {
...
TEST_F(FieldPathUpdateTestCase, testApplyAssignSingle) {
    ...
    DocumentUpdate docUp(*_repo, *_foobar_type, DocumentId("id:ns:foobar::barbar:foofoo"));
    docUp.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(*doc->getDataType(), "strfoo", std::string(),
                                                                     StringFieldValue::make("himert")));
    docUp.applyTo(*doc);
    ...
    docUp2.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(*doc->getDataType(), "strfoo", std::string(),
                                                                      StringFieldValue::make("wunderbaum")));
    docUp2.applyTo(*doc);
...
TEST_F(FieldPathUpdateTestCase, testApplyAssignMathNotModifiedOnOverflow) {
    ...
    docUp.applyTo(*doc);
    // Over/underflow will happen. You must have control of your data types.
    EXPECT_EQ(static_cast<const FieldValue&>(ByteFieldValue(static_cast<char>(static_cast<int>(127 + 200)))),
              *doc->getValue("byteval"));
}
...
TEST_F(FieldPathUpdateTestCase, testAssignSimpleMapValueWithVariable) {
    ...
    docUp.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(
        *doc->getDataType(), "strmap{$x}", "foobar.strmap{$x} == \"bar\"", StringFieldValue::make("shinyvalue")));
    docUp.applyTo(*doc);

    std::unique_ptr<MapFieldValue> valueNow(doc->getAs<MapFieldValue>(doc->getField("strmap")));

    ASSERT_EQ(std::size_t(2), valueNow->size());
    EXPECT_EQ(static_cast<const FieldValue&>(StringFieldValue("shinyvalue")),
              *valueNow->get(StringFieldValue("foo")));
    EXPECT_EQ(static_cast<const FieldValue&>(StringFieldValue("bananas")), *valueNow->get(StringFieldValue("baz")));
}
...
TEST_F(FieldPathUpdateTestCase, testApplyAssignMultiList) {
    ...
    DocumentUpdate docUp(*_repo, *_foobar_type, DocumentId("id:ns:foobar::barbar:foofoo"));
    docUp.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(*doc->getDataType(), "strarray", std::string(),
                                                                     std::move(updateArray)));
    docUp.applyTo(*doc);
    {
        std::unique_ptr<ArrayFieldValue> strArray = doc->getAs<ArrayFieldValue>(doc->getField("strarray"));
        ASSERT_EQ(std::size_t(2), strArray->size());
...
TEST_F(FieldPathUpdateTestCase, testApplyAssignMultiWset) {
    ...
    DocumentUpdate docUp(*_repo, *_foobar_type, DocumentId("id:ns:foobar::barbar:foofoo"));
    docUp.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(*doc->getDataType(), "strwset", std::string(),
                                                                     std::move(assignWset)));
    // doc->print(std::cerr, true, "");
    docUp.applyTo(*doc);
    // doc->print(std::cerr, true, "");
...
TEST_F(FieldPathUpdateTestCase, testAssignWsetRemoveIfZero) {
    ...
    {
        DocumentUpdate docUp(*_repo, *_foobar_type, DocumentId("id:ns:foobar::barbar:foofoo"));
        auto assignUpdate = std::make_unique<AssignFieldPathUpdate>(*doc->getDataType(), "strwset{you say goodbye}",
                                                                    std::string(), IntFieldValue::make(0));
...
TEST_F(FieldPathUpdateTestCase, testApplyAddMultiList) {
    ...
    DocumentUpdate docUp(*_repo, *_foobar_type, DocumentId("id:ns:foobar::barbar:foofoo"));
    docUp.addFieldPathUpdate(
        std::make_unique<AddFieldPathUpdate>(*doc->getDataType(), "strarray", std::string(), std::move(adds)));
    // doc->print(std::cerr, true, "");
    docUp.applyTo(*doc);
    // doc->print(std::cerr, true, "");
    EXPECT_TRUE(doc->hasValue("strarray"));
...
TEST_F(FieldPathUpdateTestCase, testAddAndAssignList) {
    ...
    DocumentUpdate docUp(*_repo, *_foobar_type, DocumentId("id:ns:foobar::barbar:foofoo"));
    docUp.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(
        *doc->getDataType(), "strarray[1]", std::string(), StringFieldValue::make("assigned val 1")));
...
struct Fixture {
    ...
    static const MapDataType& getMapType(const DocumentType& doc_type) {
...
TEST_F(FieldPathUpdateTestCase, testAssignMap) {
    ...
    DocumentUpdate docUp(*_repo, *_foobar_type, DocumentId("id:ns:foobar::barbar:foofoo"));
    docUp.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(
        *f.doc->getDataType(), "structmap{" + k.key2 + "}", std::string(), f.fv4()));
    docUp.applyTo(*f.doc);
...
TEST_F(FieldPathUpdateTestCase, testAssignMapStruct) {
    ...
    DocumentUpdate docUp(*_repo, *_foobar_type, DocumentId("id:ns:foobar::barbar:foofoo"));
    docUp.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(
        *f.doc->getDataType(), "structmap{" + k.key2 + "}.rating", std::string(), IntFieldValue::make(48)));
    docUp.applyTo(*f.doc);
...
std::unique_ptr<FieldValue> createFry(const DataType& type) {
...
TEST_F(FieldPathUpdateTestCase, testAssignMapNoExist) {
    ...
    DocumentUpdate docUp(*_repo, *_foobar_type, DocumentId("id:ns:foobar::barbar:foofoo"));
    docUp.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(
        *doc->getDataType(), "structmap{foo}", std::string(),
    ...
    EXPECT_EQ(*createFry(dynamic_cast<const MapDataType&>(*mfv.getDataType()).getValueType()),
              *valueNow->get(StringFieldValue("foo")));
}
...
TEST_F(FieldPathUpdateTestCase, testAssignMapNoExistNoCreate) {
    ...
    DocumentUpdate docUp(*_repo, *_foobar_type, DocumentId("id:ns:foobar::barbar:foofoo"));
    auto           assignUpdate = std::make_unique<AssignFieldPathUpdate>(
        *doc->getDataType(), "structmap{foo}", std::string(),
        createFry(dynamic_cast<const MapDataType&>(*mfv.getDataType()).getValueType()));
...
TEST_F(FieldPathUpdateTestCase, testQuotedStringKey) {
    ...
    DocumentUpdate docUp(*_repo, *_foobar_type, DocumentId("id:ns:foobar::barbar:foofoo"));
    docUp.addFieldPathUpdate(
        std::make_unique<AssignFieldPathUpdate>(*f.doc->getDataType(), field_path, std::string(), f.fv4()));
    docUp.applyTo(*f.doc);
...
namespace {
std::unique_ptr<FieldValue> createTastyCake(const DataType& type) {
    auto fv = std::make_unique<StructFieldValue>(type);
...
TEST_F(FieldPathUpdateTestCase, testEqualityComparison) {
...

    {
        ...
        docUp1.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(
            *doc->getDataType(), "structmap{here be dragons}", std::string(),
            createTastyCake(dynamic_cast<const MapDataType&>(*mfv.getDataType()).getValueType())));
        EXPECT_TRUE(docUp1 != docUp2);
        docUp2.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(
            *doc->getDataType(), "structmap{here be dragons}", std::string(),
            createTastyCake(dynamic_cast<const MapDataType&>(*mfv.getDataType()).getValueType())));
        EXPECT_TRUE(docUp1 == docUp2);
    }
    {
        ...
        docUp1.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(
            *doc->getDataType(), "structmap{here be dragons}", std::string(),
            createTastyCake(dynamic_cast<const MapDataType&>(*mfv.getDataType()).getValueType())));
        docUp2.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(
            *doc->getDataType(), "structmap{here be dragons}", "false",
            createTastyCake(dynamic_cast<const MapDataType&>(*mfv.getDataType()).getValueType())));
        EXPECT_TRUE(docUp1 != docUp2);
    }
    {
        ...
        docUp1.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(
            *doc->getDataType(), "structmap{here be dragons}", std::string(),
            createTastyCake(dynamic_cast<const MapDataType&>(*mfv.getDataType()).getValueType())));
        docUp2.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(
            *doc->getDataType(), "structmap{here be kittens}", std::string(),
            createTastyCake(dynamic_cast<const MapDataType&>(*mfv.getDataType()).getValueType())));
        EXPECT_TRUE(docUp1 != docUp2);
    }
}
...
TEST_F(FieldPathUpdateTestCase, testAffectsDocumentBody) {
    ...
    auto fv4 =
        std::make_unique<StructFieldValue>(dynamic_cast<const MapDataType&>(*mfv.getDataType()).getValueType());
    ...
    // structmap is body field
    {
        ...
        auto update1 = std::make_unique<AssignFieldPathUpdate>(*doc->getDataType(), "structmap{janitor}",
                                                               std::string(), std::move(fv4));
    ...
    {
        DocumentUpdate docUp(*_repo, *_foobar_type, DocumentId("id:ns:foobar::barbar:foofoo"));
        auto           update1 = std::make_unique<AssignFieldPathUpdate>(*doc->getDataType(), "strfoo", std::string(),
                                                                         StringFieldValue::make("helloworld"));
        static_cast<AssignFieldPathUpdate&>(*update1).setCreateMissingPath(true);
        docUp.addFieldPathUpdate(std::move(update1));
    }
}
...
TEST_F(FieldPathUpdateTestCase, testIncompatibleDataTypeFails) {
    ...
    try {
        auto update1 = std::make_unique<AssignFieldPathUpdate>(*doc->getDataType(), "structmap{foo}", std::string(),
                                                               StringFieldValue::make("bad things"));
        EXPECT_TRUE(false);
...
TEST_F(FieldPathUpdateTestCase, testSerializeAssign) {
    ...
    auto update1 =
        std::make_unique<AssignFieldPathUpdate>(*doc->getDataType(), "structmap{ribbit}", "true", std::move(val));
    static_cast<AssignFieldPathUpdate&>(*update1).setCreateMissingPath(true);
    docUp.addFieldPathUpdate(std::move(update1));
...
DocumentUpdate::UP FieldPathUpdateTestCase::createDocumentUpdateForSerialization(const DocumentTypeRepo& repo) {
    ...
    docUp->addFieldPathUpdate(
        std::make_unique<AddFieldPathUpdate>(*docType, "arrayoffloatfield", "", std::move(fArray)));
...
TEST_F(FieldPathUpdateTestCase, testGenerateSerializedFile) {
    ...
    int fd = open(TEST_PATH("data/serialize-fieldpathupdate-cpp.dat").c_str(), O_WRONLY | O_TRUNC | O_CREAT, 0644);
...
TEST_F(FieldPathUpdateTestCase, array_element_update_for_invalid_index_is_ignored) {
    ...
    docUp.addFieldPathUpdate(std::make_unique<AssignFieldPathUpdate>(*doc->getDataType(), "strarray[1]", "",
                                                                     StringFieldValue::make("george")));
    docUp.applyTo(*doc);
