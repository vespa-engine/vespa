...
#define ASSERT_BUCKET_COUNT(expression, count)                                                        \
    {                                                                                                 \
        ...
        std::unique_ptr<BucketSelector::BucketVector> buckets(selector.select(*node));                \
        size_t                                        bcount(buckets.get() ? buckets->size() : 0);    \
        std::ostringstream                            ost;                                            \
        ost << "Expression " << expression << " did not contain " << count << " buckets as expected"; \
...
#define ASSERT_BUCKET(expression, bucket)                                                                   \
    {                                                                                                       \
        ...
        std::unique_ptr<BucketSelector::BucketVector> buckets(selector.select(*node));                      \
        std::ostringstream                            ost;                                                  \
        ost << "Expression " << expression << " did not contain bucket " << bucket.toString();              \
        if (buckets.get()) {                                                                                \
            ost << ". Buckets: " << testing::PrintToString(*buckets);                                       \
        } else {                                                                                            \
            ost << ". Matches all buckets";                                                                 \
        }                                                                                                   \
        EXPECT_TRUE(buckets.get() && std::find(buckets->begin(), buckets->end(), bucket) != buckets->end()) \
            << ost.str();                                                                                   \
    }
...
TEST(BucketSelectorTest, testSimple) {
    ...
    ASSERT_BUCKET_COUNT("id.group = \"yahoo.com\" and testdoctype1.hstringval=\"Doe\"", 1u);
    ...
    ASSERT_BUCKET("id = \"id:ns:testdoc:n=123:foobar\"", document::BucketId(58, 123));

    ASSERT_BUCKET("id.bucket == 0x4000000000000258", document::BucketId(16, 600));

    ASSERT_BUCKET("id.user = 123", document::BucketId(32, 123));
    ASSERT_BUCKET("id.group = \"yahoo.com\"", document::BucketId(32, 0x9a1acd50));
}
