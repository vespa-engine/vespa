...
std::vector<std::pair<std::string, std::string>> modify_layouts = {
    {"x4_1", "x4_1"},    {"x4_1", "x4_2"}, {"x4", "x4_2"}, {"x4_1y4_2", "x4_2y4_1"}, {"x4y4_1z4", "x4_2y4_2z4_2"},
    {"x3y2", "x2_1y2_1"}};
...
Value::UP try_partial_modify_with_defaults(const TensorSpec& a, const TensorSpec& b, join_fun_t fun,
...
TensorSpec perform_partial_modify_with_defaults(const TensorSpec& a, const TensorSpec& b, join_fun_t fun,
                                                double default_cell_value) {
...
void expect_modify_with_defaults(const std::string& lhs_expr, const std::string& rhs_expr, join_fun_t fun,
                                 double default_cell_value, const std::string& exp_expr) {
...
TEST(PartialModifyTest, partial_modify_with_defauls) {
    expect_modify_with_defaults("tensor(x{}):{{x:\"a\"}:1,{x:\"b\"}:2}", "tensor(x{}):{{x:\"b\"}:3}",
                                operation::Add::f, 0.0, "tensor(x{}):{{x:\"a\"}:1,{x:\"b\"}:5}");

    expect_modify_with_defaults("tensor(x{}):{{x:\"a\"}:1,{x:\"b\"}:2}", "tensor(x{}):{{x:\"b\"}:3,{x:\"c\"}:4}",
                                operation::Add::f, 0.0, "tensor(x{}):{{x:\"a\"}:1,{x:\"b\"}:5,{x:\"c\"}:4}");
    ...
    expect_modify_with_defaults("tensor(x[3]):{{x:0}:2}", "tensor(x{}):{{x:\"1\"}:3}", operation::Add::f, 2.0,
                                "tensor(x[3]):{{x:0}:2,{x:1}:3,{x:2}:0}");
}

std::vector<std::pair<std::string, std::string>> bad_layouts = {
    {"x3", "x3"}, {"x3y4_1", "x3y4_1"}, {"x4_1", "x4_1y4_1"}, {"x4_1y4_1", "x4_1"}, {"x4_1", "x4_1y1"}};
