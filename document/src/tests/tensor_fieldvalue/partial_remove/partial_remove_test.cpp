...
std::vector<std::pair<std::string, std::string>> remove_layouts = {
    {"x4_1", "x4_2"}, {"x4_2y4_1", "x4_1y4_2"}, {"x3y4_1", "y4_2"}};
...
std::vector<std::pair<std::string, std::string>> bad_layouts = {
    {"x3", "x3"}, {"x3y4_1", "x3"}, {"x3y4_1", "x3y4_2"}, {"x4_1", "y4_1"}, {"x4_1", "x4_2y4_1"}};
...
void expect_partial_remove(const TensorSpec& input, const TensorSpec& remove, const TensorSpec& exp) {
...
TEST(PartialRemoveTest, remove_where_address_is_not_fully_specified) {
    ...
    expect_partial_remove(
        input_sparse, TensorSpec("tensor(y{})").add({{"y", "d"}}, 1.0),
        TensorSpec("tensor(x{},y{})").add({{"x", "a"}, {"y", "c"}}, 3.0).add({{"x", "b"}, {"y", "c"}}, 7.0));
