...
DocumentUpdate::UP createTestDocumentUpdate(const DocumentTypeRepo& repo) {
    const DocumentType* type(repo.getDocumentType("testdoc"));
...
TEST(TestXml, testDocumentUpdate) {
    ...
    std::string expected = "<document type=\"testdoc\" id=\"id:ns:testdoc::crawler/http://www.ntnu.no/\">\n"
                           "  <alter field=\"intattr\">\n"
                           "    <assign>7</assign>\n"
                           "  </alter>\n"
