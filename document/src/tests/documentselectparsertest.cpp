...
protected:
    ...
    static Document::SP createDoc(std::string_view doctype, std::string_view id, uint32_t hint, double hfloat,
                                  std::string_view hstr, std::string_view cstr, uint64_t hlong = 0);

    static DocumentUpdate::SP createUpdate(const std::string& doctype, const std::string& id, uint32_t hint,
                                           const std::string& hstr);

    std::unique_ptr<select::FieldValueNode> parseFieldValue(const std::string& expression);

    template <typename ContainsType> select::ResultList doParse(std::string_view expr, const ContainsType& t);

    std::string parse_to_tree(const std::string& str);

    DocumentSelectParserTest() : _bucketIdFactory() {}
...
Document::SP DocumentSelectParserTest::createDoc(std::string_view doctype, std::string_view id, uint32_t hint,
...
DocumentUpdate::SP DocumentSelectParserTest::createUpdate(const std::string& doctype, const std::string& id,
                                                          uint32_t hint, const std::string& hstr) {
    const DocumentType* type = _repo->getDocumentType(doctype);
...
void DocumentSelectParserTest::createDocs() {
    _doc.clear();
    _doc.push_back(createDoc("testdoctype1", "id:myspace:testdoctype1::anything", 24, 2.0, "foo", "bar", 0)); // DOC 0
    _doc.push_back(createDoc("testdoctype1", "id:anotherspace:testdoctype1::foo", 13, 4.1, "bar", "foo",
                             0)); // DOC 1
                                  // Add some arrays and structs to doc 1
    {
        ...
        const auto& tensor_type =
            dynamic_cast<const TensorDataType&>(_doc.back()->getField("dense_tensor").getDataType());
        TensorFieldValue tfv(tensor_type);
        tfv.make_empty_if_not_existing();
        _doc.back()->setValue("dense_tensor", tfv);
    }

    _doc.push_back(createDoc("testdoctype1", "id:myspace:testdoctype1:n=1234:footype1", 15, 1.0, "some", "some",
                             0)); // DOC 2
    ...
    _doc.push_back(createDoc("testdoctype1", "id:myspace:testdoctype1:g=yahoo:bar", 14, 2.4, "Yet",
                             "\xE4\xB8\xBA\xE4\xBB\x80", 0)); // DOC 3
    _doc.push_back(
        createDoc("testdoctype2", "id:myspace:testdoctype2::inheriteddoc", 10, 1.4, "inherited", "")); // DOC 4
    _doc.push_back(createDoc("testdoctype1", "id:footype:testdoctype1:n=123456789:aardvark", 10, 1.4, "inherited", "",
                             0)); // DOC 5
    _doc.push_back(createDoc("testdoctype1", "id:footype:testdoctype1:n=1234:highlong", 10, 1.4, "inherited", "",
                             2651257743)); // DOC 6
    _doc.push_back(createDoc("testdoctype1", "id:footype:testdoctype1:n=1234:highlong", 10, 1.4, "inherited", "",
                             -2651257743)); // DOC 7
    ...
    _doc.push_back(
        createDoc("testdoctype1", "id:footype:testdoctype1:n=12345:foo", 10, 1.4, "inherited", "", 42)); // DOC 9
    _doc.push_back(
        createDoc("testdoctype1", "id:myspace:testdoctype1:g=xyzzy:foo", 10, 1.4, "inherited", "", 42)); // DOC 10
    _doc.push_back(
        createDoc("testdoctype1", "id:myspace:testdoctype1::withtruebool", 10, 1.4, "inherited", "", 42)); // DOC 11
    ...
    _update.push_back(
        createUpdate("testdoctype1", "id:myspace:testdoctype1:g=yahoo:bar", 3, "\xE4\xBA\xB8\xE4\xBB\x80"));
...
TEST_F(DocumentSelectParserTest, test_syntax_error_reporting) {
    ...
    verifyFailedParse("testdoctype1.headerval == aaa",
    ...
    verifyFailedParse("testdoctype1.headerval <> 12",
                      "ParsingFailedException: syntax error, "
    ...
    verifyFailedParse("testdoctype1 == 8",
                      "ParsingFailedException: syntax error, unexpected ==, "
...
TEST_F(DocumentSelectParserTest, testParseTerminals) {
    ...
    verifyParse("testdoctype1.headerval == 234123.523e3", "testdoctype1.headerval == 2.34124e+08");
    verifyParse("testdoctype1.headerval == -234123.523E-3", "testdoctype1.headerval == -234.124");
    ...
    verifyParse(" \t testdoctype1.headerval\t==  \t \"test\"\t", "testdoctype1.headerval == \"test\"");

    // Test escaping
    verifyParse("testdoctype1.headerval == \"tab\\ttest\"");
    verifyParse("testdoctype1.headerval == \"tab\\x09test\"", "testdoctype1.headerval == \"tab\\ttest\"");
    ...
    verifyParse("id.bucket == 0x80000000000000f2", "id.bucket == -9223372036854775566");
...
select::ResultList DocumentSelectParserTest::doParse(std::string_view expr, const ContainsType& t) {
...
#define PARSE(expr, doc, result) EXPECT_EQ(select::ResultList(select::Result::result), doParse(expr, (doc))) << expr;

#define PARSEI(expr, doc, result)                                                                                 \
    EXPECT_EQ(select::ResultList(select::Result::result), doParse(expr, (doc))) << (std::string("Doc: ") + expr); \
    EXPECT_EQ(select::ResultList(select::Result::result), doParse(expr, (doc).getId()))                           \
        << (std::string("Doc id: ") + expr);
...
TEST_F(DocumentSelectParserTest, operators_1) {
    ...
    PARSE("testdoctype1.hfloatval > 4.09 and testdoctype1.hfloatval < 4.11", *_doc[1], True);
    ...
    PARSE("testdoctype1", *_doc[4],
          False); // testdoctype2 inherits testdoctype1, but we use exact matching for "standalone" doctype matches.
    PARSE("testdoctype1.headerval = 10", *_doc[4], True); // But _field lookups_ use is-a type matching semantics.
    PARSE("testdoctype2.headerval = 10", *_doc[4],
          True); // Exact type match with parent field also works transparently
}
...
TEST_F(DocumentSelectParserTest, operators_6) {
    ...
    PARSE("testdoctype1.structarray == testdoctype1.structarray", *_doc[0], True);
    PARSE("testdoctype1.structarray < testdoctype1.structarray", *_doc[0], Invalid);
    PARSE("testdoctype1.structarray == testdoctype1.structarray", *_doc[1], True);
    PARSE("testdoctype1.structarray < testdoctype1.structarray", *_doc[1], False);
...
TEST_F(DocumentSelectParserTest, operators_7) {
    ...
    PARSE("testdoctype1.structarray[$x].key == 15 AND testdoctype1.structarray[$x].value == \"structval1\"", *_doc[1],
          True);
    PARSE("testdoctype1.structarray[$x].key == 15 AND testdoctype1.structarray[$x].value == \"structval2\"", *_doc[1],
          False);
    PARSE("testdoctype1.structarray[$x].key == 15 AND testdoctype1.structarray[$y].value == \"structval2\"", *_doc[1],
          True);
}
...
TEST_F(DocumentSelectParserTest, operators_8) {
    ...
    PARSE("testdoctype1.structarrmap{$x}[$y].key == 15 AND testdoctype1.structarrmap{$x}[$y].value == \"structval1\"",
          *_doc[1], True);
    PARSE("testdoctype1.structarrmap.value[$y].key == 15 AND testdoctype1.structarrmap.value[$y].value == "
          "\"structval1\"",
          *_doc[1], True);
    PARSE("testdoctype1.structarrmap{$x}[$y].key == 15 AND testdoctype1.structarrmap{$x}[$y].value == \"structval2\"",
          *_doc[1], False);
    PARSE("testdoctype1.structarrmap.value[$y].key == 15 AND testdoctype1.structarrmap.value[$y].value == "
          "\"structval2\"",
          *_doc[1], False);
    PARSE("testdoctype1.structarrmap{$x}[$y].key == 15 AND testdoctype1.structarrmap{$y}[$x].value == \"structval2\"",
          *_doc[1], False);
}
...
public:
    ...
    void visitConstant(const select::Constant& node) override { data << "CONSTANT(" << node << ")"; }
    ...
    void visitComparison(const select::Compare& node) override {
        data << "COMPARE(" << node.getLeft() << " " << node.getOperator() << " " << node.getRight() << ")";
    }
...
TEST_F(DocumentSelectParserTest, testVisitor) {
    ...
    std::unique_ptr<select::Node> root(
        _parser->parse("true or testdoctype1 and (not id.user = 12 or testdoctype1.hstringval = \"ola\") and "
                       "testdoctype1.headerval"));

    TestVisitor v;
    root->visit(v);

    std::string expected = "OR(CONSTANT(true), "
...
TEST_F(DocumentSelectParserTest, testDocumentUpdates1) {
    ...
    PARSE("testdoctype1", *_update[4],
          False); // testdoctype2 inherits testdoctype1, but we use exact matching for types
...
std::unique_ptr<select::FieldValueNode> DocumentSelectParserTest::parseFieldValue(const std::string& expression) {
...
TEST_F(DocumentSelectParserTest, testThatSimpleFieldValuesHaveCorrectFieldName) {
    EXPECT_EQ(std::string("headerval"), parseFieldValue("testdoctype1.headerval")->getRealFieldName());
}

TEST_F(DocumentSelectParserTest, testThatComplexFieldValuesHaveCorrectFieldNames) {
    EXPECT_EQ(std::string("headerval"), parseFieldValue("testdoctype1.headerval{test}")->getRealFieldName());

    EXPECT_EQ(std::string("headerval"), parseFieldValue("testdoctype1.headerval[42]")->getRealFieldName());
...
public:
    void visitConstant(const select::Constant& node) override { data << node; }
    ...
    void visitIdValueNode(const select::IdValueNode& node) override { data << "(ID " << node.toString() << ')'; }
    void visitFieldValueNode(const select::FieldValueNode& node) override {
        data << "(FIELD " << node.getDocType() << ' ' << node.getFieldName() << ')';
    }
    void visitFloatValueNode(const select::FloatValueNode& node) override { data << node.getValue(); }
    void visitVariableValueNode(const select::VariableValueNode& node) override {
        data << "(VAR " << node.getVariableName() << ')';
    }
    void visitIntegerValueNode(const select::IntegerValueNode& node) override { data << node.getValue(); }
    void visitBoolValueNode(const select::BoolValueNode& node) override { data << node.bool_value_str(); }
    void visitCurrentTimeValueNode(const select::CurrentTimeValueNode&) override {}
    void visitStringValueNode(const select::StringValueNode& str) override { data << '"' << str.getValue() << '"'; }
    void visitNullValueNode(const select::NullValueNode&) override { data << "null"; }
    void visitInvalidValueNode(const select::InvalidValueNode&) override { data << "INVALID!"; }

    std::string visit_string() { return data.str(); }
};

template <typename NodeType> std::string node_to_string(const NodeType& node) {
    OperatorVisitor v;
    node.visit(v);
...
TEST_F(DocumentSelectParserTest, test_operator_precedence) {
    ...
    EXPECT_EQ("(!= (/ (* 1 2) (- 3 4)) 0)"s, parse_to_tree("1 * 2 / (3 - 4) != 0"));
    EXPECT_EQ("(OR (AND true (NOT (== 1 2))) false)"s, parse_to_tree("true and not 1 == 2 or false"));
...
TEST_F(DocumentSelectParserTest, test_function_call_on_doctype_throws_exception) {
    ...
    try {
        ...
        EXPECT_EQ(std::string("Cannot call function 'lowercase' directly on document type"), e.getMessage());
    }
}
...
TEST_F(DocumentSelectParserTest, selection_has_upper_limit_on_input_size) {
    createDocs();
    std::string expr =
        ("testdoctype1.a_biii" + std::string(select::ParserLimits::MaxSelectionByteSize, 'i') + "iiig_identifier");
...
TEST_F(DocumentSelectParserTest, lexing_does_not_have_superlinear_time_complexity) {
    ...
    std::string expr = ("testdoctype1.hstringval == 'a_biii" +
                        std::string(select::ParserLimits::MaxSelectionByteSize - 100, 'i') + "iiig string'");
