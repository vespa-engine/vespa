...
bool checkContains(const DocumentTypeRepo& repo, const std::string& str1, const std::string& str2) {
...
TEST_F(FieldSetTest, testContains) {
    ...
    EXPECT_EQ(true, checkContains(repo, "testdoctype1:content,headerval", "testdoctype1:content"));
    EXPECT_EQ(false, checkContains(repo, "testdoctype1:content", "testdoctype1:content,headerval"));
    EXPECT_EQ(true, checkContains(repo, "testdoctype1:headerval,content", "testdoctype1:content,headerval"));
...
std::string FieldSetTest::stringifyFields(const Document& doc) {
    std::vector<std::string> output;
    const StructFieldValue&  fields(doc.getFields());
    for (StructFieldValue::const_iterator it(fields.begin()), e(fields.end()); it != e; ++it) {
    ...
    std::copy(output.begin(), output.end(), std::ostream_iterator<std::string>(ret, "\n"));
    return ret.str();
}

std::string FieldSetTest::doCopyFields(const Document& src, const DocumentTypeRepo& docRepo,
                                       const std::string& fieldSetStr, Document* dest) {
...
std::string FieldSetTest::doStripFields(const Document& doc, const DocumentTypeRepo& docRepo,
                                        const std::string& fieldSetStr) {
...
Document::UP FieldSetTest::createTestDocument(const TestDocMan& testDocMan) {
    Document::UP doc(testDocMan.createDocument("megafoo megabar", "id:ns:testdoctype1::1", "testdoctype1"));
    doc->setValue(doc->getField("headerval"), IntFieldValue(5678));
    doc->setValue(doc->getField("hstringval"), StringFieldValue("hello fantastic world"));
    return doc;
}
...
TEST_F(FieldSetTest, testCopyDocumentFields) {
    ...
    EXPECT_EQ(std::string(""), doCopyFields(*src, repo, NoFields::NAME));
...
std::string FieldSetTest::doCopyDocument(const Document& src, const DocumentTypeRepo& docRepo,
                                         const std::string& fieldSetStr) {
    auto         fset = FieldSetRepo::parse(docRepo, fieldSetStr);
...
TEST_F(FieldSetTest, testDocumentSubsetCopy) {
...

    {
        ...
        EXPECT_EQ(doCopyFields(*src, repo, AllFields::NAME), stringifyFields(*doc));
    }
    {
        ...
        EXPECT_EQ(src->getId(), doc->getId());
        EXPECT_EQ(src->getType(), doc->getType());
        EXPECT_EQ(doCopyFields(*src, repo, DocumentOnly::NAME), stringifyFields(*doc));
    }

    const char* fieldSets[] = {AllFields::NAME, DocumentOnly::NAME, NoFields::NAME,
                               "testdoctype1:hstringval,content"};
    for (const char* fieldSet : fieldSets) {
        EXPECT_EQ(doCopyFields(*src, repo, fieldSet), doCopyDocument(*src, repo, fieldSet));
    }
}
...
TEST_F(FieldSetTest, testSerialize) {
    ...
    const char* fieldSets[] = {AllFields::NAME, NoFields::NAME,         DocumentOnly::NAME,
                               DocIdOnly::NAME, "testdoctype1:content", "testdoctype1:content,hstringval"};
...
TEST_F(FieldSetTest, testStripFields) {
    ...
    EXPECT_EQ(std::string(""), doStripFields(*src, repo, NoFields::NAME));
    EXPECT_EQ(std::string(""), doStripFields(*src, repo, DocIdOnly::NAME));
    EXPECT_EQ(std::string("content: megafoo megabar\n"
                          "headerval: 5678\n"
                          "hstringval: hello fantastic world\n"),
...
TEST(FieldCollectionTest, testHash) {
    ...
    EXPECT_EQ(0x548599858c77ef83ul,
              FieldCollection(type, Field::Set::Builder().add(&type.getField("headerval")).build()).hash());
    EXPECT_EQ(
        0x4a7ff2406d36a9b0ul,
        FieldCollection(
            type, Field::Set::Builder().add(&type.getField("headerval")).add(&type.getField("hstringval")).build())
            .hash());
    EXPECT_EQ(0x1e0918531b19734ul,
