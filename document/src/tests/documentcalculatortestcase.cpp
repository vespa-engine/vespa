...
TEST_F(DocumentCalculatorTest, testConstant) {
    ...
    Document doc(_testRepo.getTypeRepo(), *_testRepo.getDocumentType("testdoctype1"),
                 DocumentId("id:ns:testdoctype1::foo"));
...
TEST_F(DocumentCalculatorTest, testFieldsDivZero) {
    ...
    EXPECT_THROW(calc.evaluate(doc, std::move(variables)), vespalib::IllegalArgumentException);
}
...
TEST_F(DocumentCalculatorTest, testFieldNotFound) {
    auto               variables = std::make_unique<select::VariableMap>();
    DocumentCalculator calc(getRepo(), "(testdoctype1.mynotfoundfield + testdoctype1"
                                       ".hfloatval) / testdoctype1.headerlongval");
