...
/// \todo TODO (was warning):  Find a way to search through internal map without duplicating keys to create shared
/// pointers.
...
bool WeightedSetFieldValue::add(const FieldValue& key, int weight) {
    verifyKey(key);
...
bool WeightedSetFieldValue::addIgnoreZeroWeight(const FieldValue& key, int32_t weight) {
    verifyKey(key);
    return _map.insert(FieldValue::UP(key.clone()), IntFieldValue::make(weight));
}

void WeightedSetFieldValue::push_back(FieldValue::UP key, int weight) {
    _map.push_back(std::move(key), IntFieldValue::make(weight));
}

void WeightedSetFieldValue::increment(const FieldValue& key, int val) {
    verifyKey(key);
    ...
    if (wdt.createIfNonExistent()) {
        ...
        if (it == _map.end()) {
            throw IllegalStateException(
                "Cannot modify non-existing entry in weightedset without createIfNonExistent set", VESPA_STRLOC);
        }
        IntFieldValue& fv = static_cast<IntFieldValue&>(*it->second);
        fv.setValue(fv.getValue() + val);
        if (wdt.removeIfZero() && fv.getValue() == 0) {
...
int32_t WeightedSetFieldValue::get(const FieldValue& key, int32_t defaultValue) const {
...
FieldValue& WeightedSetFieldValue::assign(const FieldValue& value) {
...
void WeightedSetFieldValue::printXml(XmlOutputStream& xos) const {
    for (const auto& entry : _map) {

        const IntFieldValue& fv = static_cast<const IntFieldValue&>(*entry.second);
        xos << XmlTag("item") << XmlAttribute("weight", fv.getValue()) << *entry.first << XmlEndTag();
    }
}

void WeightedSetFieldValue::print(std::ostream& out, bool verbose, const std::string& indent) const {
...
WeightedSetFieldValue::const_iterator WeightedSetFieldValue::find(const FieldValue& key) const {
