...
NumericFieldValue<Number>::operator=(std::string_view value)
{
    ...
    if ((value.size() > 2) && (value[0] == '0') && ((value[1] | 0x20) == 'x')) {
        ...
        uint64_t val;
        auto res = std::from_chars(value.data() + 2, lastp, val, 16);
        if (res.ec == std::errc{}) {
        ...
        if (sizeof(Number) == sizeof(int8_t)) {
            int val;
            auto res = std::from_chars(value.data(), lastp, val);
            if (res.ec == std::errc{}) {
                if (val < -128 || val > 255) {
            ...
            auto res = std::from_chars(value.data(), lastp, _value);
            if (res.ec == std::errc{}) {
                return *this;
            }
            if constexpr(std::is_signed_v<Number>) {
                if (res.ec == std::errc::result_out_of_range) {
                    typename std::make_unsigned<Number>::type tmp;
                    res = std::from_chars(value.data(), lastp, tmp);
                    if (res.ec == std::errc{}) {
    ...
    throw vespalib::IllegalArgumentException("Cannot convert to number: " + std::string(value));
}
