...
ArrayFieldValue::ArrayFieldValue(const DataType& type) : CollectionFieldValue(Type::ARRAY, type), _array() {
...
ArrayFieldValue& ArrayFieldValue::operator=(const ArrayFieldValue& other) {
    if (this != &other) {
        verifyType(other);
...
void ArrayFieldValue::remove(uint32_t index) {
    if (_array->size() <= index) {
        throw IllegalArgumentException(
            make_string("Cannot remove index %u from an array of size %lu.", index, (unsigned long)_array->size()),
            VESPA_STRLOC);
    }
    _array->erase(array().begin() + index);
}

bool ArrayFieldValue::addValue(const FieldValue& value) {
    if (getNestedType().isValueType(value)) {
        _array->push_back(value);
    } else {
        throw IllegalArgumentException(make_string("Cannot add value of type %s to array containing type %s.",
                                                   value.getDataType()->toString().c_str(),
                                                   getNestedType().toString().c_str()),
                                       VESPA_STRLOC);
    }
    return true;
}

bool ArrayFieldValue::containsValue(const FieldValue& value) const {
    if (getNestedType().isValueType(value)) {
        ...
        throw IllegalArgumentException(make_string("Value of type %s can't possibly be in array of type %s.",
                                                   value.getDataType()->toString().c_str(),
                                                   getDataType()->toString().c_str()),
                                       VESPA_STRLOC);
    }
}

bool ArrayFieldValue::removeValue(const FieldValue& value) {
    if (getNestedType().isValueType(value)) {
...
FieldValue& ArrayFieldValue::assign(const FieldValue& value) {
...
void ArrayFieldValue::printXml(XmlOutputStream& xos) const {
...
void ArrayFieldValue::print(std::ostream& out, bool verbose, const std::string& indent) const {
...
fieldvalue::ModificationStatus ArrayFieldValue::iterateSubset(int startPos, int endPos, const std::string& variable,
                                                              PathRange                    nested,
...
fieldvalue::ModificationStatus ArrayFieldValue::onIterateNested(PathRange nested, IteratorHandler& handler) const {
    ...
    if (!nested.atEnd()) {
        ...
        case FieldPathEntry::VARIABLE: {
            ...
            if (iter != handler.getVariables().end()) {
                ...
                if (idx == -1) {
                    throw IllegalArgumentException("Mismatch between variables - trying to iterate through map "
                ...
                return iterateSubset(0, static_cast<int>(_array->size()) - 1, fpe.getVariableName(), nested.next(),
                                     handler);
            }
            break;
        }
