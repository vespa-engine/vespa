...
void SerializableArray::set(EntryMap entries, ByteBuffer buffer) {
...
template <typename T> T& ensure(std::unique_ptr<T>& owned) {
    if (!owned) {
...
SerializableArray::SerializableArray(const SerializableArray& rhs)
    : _entries(rhs._entries), _uncompSerData(rhs._uncompSerData) {
...
SerializableArray& SerializableArray::operator=(const SerializableArray& rhs) {
    if (this != &rhs) {
...
SerializableArray::EntryMap::iterator SerializableArray::find(int id) {
    return std::find_if(_entries.begin(), _entries.end(), [id](const auto& e) { return e.id() == id; });
}
...
void SerializableArray::clear(int id) {
    auto it = find(id);
    if (it != _entries.end()) {
        _entries.erase(it);
    }
...
const char* SerializableArray::Entry::getBuffer(const ByteBuffer* readOnlyBuffer) const {
