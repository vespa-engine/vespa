...
LiteralFieldValueB::LiteralFieldValueB(const LiteralFieldValueB& other)
    : FieldValue(other), _value(), _backing(other.getValueRef()) {
    _value = _backing;
}

LiteralFieldValueB::LiteralFieldValueB(Type type, const string_view& value)
    : FieldValue(type), _value(), _backing(value) {
    _value = _backing;
}

LiteralFieldValueB& LiteralFieldValueB::operator=(const LiteralFieldValueB& other) {
...
FieldValue& LiteralFieldValueB::assign(const FieldValue& value) {
...
int LiteralFieldValueB::compare(const FieldValue& other) const {
...
int LiteralFieldValueB::fastCompare(const FieldValue& other) const {
    const LiteralFieldValueB& sval(static_cast<const LiteralFieldValueB&>(other));
    return getValueRef().compare(sval.getValueRef());
}

void LiteralFieldValueB::printXml(XmlOutputStream& out) const {
    out << XmlContentWrapper(_value.data(), _value.size());
}

void LiteralFieldValueB::print(std::ostream& out, bool, const std::string&) const {
    std::string escaped;
    out << StringUtil::escape(getValue(), escaped);
}

FieldValue& LiteralFieldValueB::operator=(std::string_view value) {
    setValue(value);
    return *this;
}
