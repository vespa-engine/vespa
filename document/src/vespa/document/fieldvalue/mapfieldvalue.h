...
private:
    ...
    bool checkAndRemove(const FieldValue& key, fieldvalue::ModificationStatus status, bool wasModified,
...
public:
    ...
    public:
        ...
        bool      operator!=(const iterator& rhs) const { return _map != rhs._map || _index != rhs._index; }
        iterator& operator++() {
            _index = _map->nextPresent(_index + 1);
            return *this;
        }
    ...
    bool has_no_erased_keys() const { return (_keys->size() == _count) && (_values->size() == _count); }
