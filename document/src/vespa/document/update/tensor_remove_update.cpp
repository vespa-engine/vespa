...
bool TensorRemoveUpdate::operator==(const ValueUpdate& other) const {
...
void TensorRemoveUpdate::checkCompatibility(const Field& field) const {
    if (!field.getDataType().isTensor()) {
        throw IllegalArgumentException(
            make_string("Cannot perform tensor remove update on non-tensor field '%s'", field.getName().data()),
            VESPA_STRLOC);
    }
}

std::unique_ptr<vespalib::eval::Value> TensorRemoveUpdate::applyTo(const vespalib::eval::Value& tensor) const {
    return apply_to(tensor, FastValueBuilderFactory::get());
}

std::unique_ptr<vespalib::eval::Value> TensorRemoveUpdate::apply_to(const Value&               old_tensor,
...
bool TensorRemoveUpdate::applyTo(FieldValue& value) const {
    if (value.isA(FieldValue::Type::TENSOR)) {
        ...
        std::string err =
            make_string("Unable to perform a tensor remove update on a '%s' field value", value.className());
        throw IllegalStateException(err, VESPA_STRLOC);
    }
    return true;
}

void TensorRemoveUpdate::printXml(XmlOutputStream& xos) const {
    xos << "{TensorRemoveUpdate::printXml not yet implemented}";
}

void TensorRemoveUpdate::print(std::ostream& out, bool verbose, const std::string& indent) const {
...
void verifyAddressTensorIsSparse(const Value* addressTensor) {
...
void verify_tensor_type_dimensions_are_subset_of(const ValueType& lhs_type, const ValueType& rhs_type) {
...
void TensorRemoveUpdate::deserialize(const DocumentTypeRepo& repo, const DataType& type, nbostream& stream) {
