...
void AddValueUpdate::checkCompatibility(const Field& field) const {
    ...
    if (ct != nullptr) {
        ...
        throw IllegalArgumentException("Can not add a value to field of type" + field.getDataType().toString(),
                                       VESPA_STRLOC);
    }
}

// Print this update in human readable form.
void AddValueUpdate::print(std::ostream& out, bool, const std::string& indent) const {
    out << indent << "AddValueUpdate(" << *_value << ", " << _weight << ")";
}

// Apply this update to the given document.
bool AddValueUpdate::applyTo(FieldValue& value) const {
...
void AddValueUpdate::printXml(XmlOutputStream& xos) const {
    xos << XmlTag("add") << XmlAttribute("weight", _weight) << *_value << XmlEndTag();
}

// Deserialize this update from the given buffer.
void AddValueUpdate::deserialize(const DocumentTypeRepo& repo, const DataType& type, nbostream& stream) {
