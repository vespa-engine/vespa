...
void MapValueUpdate::checkCompatibility(const Field& field) const {
    // Check compatibility of nested types.
    if (field.getDataType().isArray()) {
        if (!_key->isA(FieldValue::Type::INT)) {
            throw IllegalArgumentException(
                vespalib::make_string("Key for field '%s' is of wrong type (expected '%s', was '%s').",
                                      field.getName().data(), DataType::INT->toString().c_str(),
                                      _key->getDataType()->toString().c_str()),
                VESPA_STRLOC);
        }
...
bool MapValueUpdate::applyTo(FieldValue& value) const {
    if (value.getDataType()->isArray()) {
        ...
        throw IllegalStateException(
            "Cannot apply map value update to field of type " + value.getDataType()->toString() + ".", VESPA_STRLOC);
    }
    return true;
}

// Print this update in human readable form.
void MapValueUpdate::print(std::ostream& out, bool, const std::string& indent) const {
    out << indent << "MapValueUpdate(" << *_key << ", " << *_update << ")";
}

void MapValueUpdate::printXml(XmlOutputStream& xos) const {
    xos << XmlTag("map") << XmlTag("value") << *_key << XmlEndTag() << XmlTag("update");
    _update->printXml(xos);
    xos << XmlEndTag() << XmlEndTag();
}

// Deserialize this update from the given buffer.
void MapValueUpdate::deserialize(const DocumentTypeRepo& repo, const DataType& type, nbostream& stream) {
