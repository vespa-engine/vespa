...
void RemoveValueUpdate::checkCompatibility(const Field& field) const {
    const CollectionDataType* type = field.getDataType().cast_collection();
    if (type != nullptr) {
        if (!type->getNestedType().isValueType(*_key)) {
            throw IllegalArgumentException("Cannot remove value of type " + _key->getDataType()->toString() +
                                               " from field " + field.getName() + " of container type " +
                                               field.getDataType().toString(),
                                           VESPA_STRLOC);
        }
    } else {
        throw IllegalArgumentException("Can not remove a value from field of type " + field.getDataType().toString(),
                                       VESPA_STRLOC);
    }
}

// Apply this update to the given document.
bool RemoveValueUpdate::applyTo(FieldValue& value) const {
    if (value.isA(FieldValue::Type::ARRAY)) {
        ...
        std::string err =
            vespalib::make_string("Unable to remove a value from a \"%s\" field value.", value.className());
...
void RemoveValueUpdate::deserialize(const DocumentTypeRepo& repo, const DataType& type, nbostream& stream) {
