...
FieldUpdate::FieldUpdate(const DocumentTypeRepo& repo, const DataType& type, nbostream& stream)
    : _field(type.getField(readInt(stream))), _updates() {
...
FieldUpdate& FieldUpdate::addUpdate(std::unique_ptr<ValueUpdate> update) & {
...
void FieldUpdate::printXml(XmlOutputStream& xos) const {
...
void FieldUpdate::applyTo(Document& doc) const {
    const DataType& datatype = _field.getDataType();
...
void FieldUpdate::print(std::ostream& out, bool verbose, const std::string& indent) const {
...
void FieldUpdate::deserialize(const DocumentTypeRepo& repo, const DocumentType& docType, nbostream& stream) {
