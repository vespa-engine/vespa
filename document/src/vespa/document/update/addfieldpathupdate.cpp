...
AddFieldPathUpdate::AddFieldPathUpdate(const DataType& type, string_view fieldPath, string_view whereClause,
                                       std::unique_ptr<ArrayFieldValue> values)
    : FieldPathUpdate(Add, fieldPath, whereClause), _values(std::move(values)) {
...
ModificationStatus AddIteratorHandler::doModify(FieldValue& fv) {
    if (fv.isCollection()) {
...
void AddFieldPathUpdate::print(std::ostream& out, bool verbose, const std::string& indent) const {
...
void AddFieldPathUpdate::deserialize(const DocumentTypeRepo& repo, const DataType& type, nbostream& stream) {
...
std::unique_ptr<IteratorHandler> AddFieldPathUpdate::getIteratorHandler(Document&, const DocumentTypeRepo&) const {
