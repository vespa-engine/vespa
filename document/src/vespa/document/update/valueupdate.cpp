...
const char* ValueUpdate::className() const noexcept {
    switch (getType()) {
    case Add:
...
std::unique_ptr<ValueUpdate> ValueUpdate::create(ValueUpdateType type) {
    switch (type) {
    case Add:
...
std::unique_ptr<ValueUpdate> ValueUpdate::createInstance(const DocumentTypeRepo& repo, const DataType& type,
                                                         nbostream& stream) {
    int32_t classId = 0;
    stream >> classId;
    ...
    /// \todo TODO (was warning):  Updates are not versioned in serialization format. Will not work without altering
    /// it. Should also use the serializer, not this deserialize into self.
