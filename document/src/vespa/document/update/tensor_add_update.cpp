...
bool TensorAddUpdate::operator==(const ValueUpdate& other) const {
...
void TensorAddUpdate::checkCompatibility(const Field& field) const {
    if (!field.getDataType().isTensor()) {
        throw IllegalArgumentException(
            make_string("Cannot perform tensor add update on non-tensor field '%s'", field.getName().data()),
            VESPA_STRLOC);
    }
}

std::unique_ptr<vespalib::eval::Value> TensorAddUpdate::applyTo(const vespalib::eval::Value& tensor) const {
    return apply_to(tensor, FastValueBuilderFactory::get());
}

std::unique_ptr<vespalib::eval::Value> TensorAddUpdate::apply_to(const Value&               old_tensor,
...
bool TensorAddUpdate::applyTo(FieldValue& value) const {
    if (value.isA(FieldValue::Type::TENSOR)) {
        ...
        std::string err =
            make_string("Unable to perform a tensor add update on a '%s' field value", value.className());
        throw IllegalStateException(err, VESPA_STRLOC);
    }
    return true;
}

void TensorAddUpdate::printXml(XmlOutputStream& xos) const {
    xos << "{TensorAddUpdate::printXml not yet implemented}";
}

void TensorAddUpdate::print(std::ostream& out, bool verbose, const std::string& indent) const {
...
void TensorAddUpdate::deserialize(const DocumentTypeRepo& repo, const DataType& type, nbostream& stream) {
