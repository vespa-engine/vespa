...
Compare::Compare(std::unique_ptr<ValueNode> left, const Operator& op, std::unique_ptr<ValueNode> right,
...
Node::UP Compare::clone() const {
    return wrapParens(new Compare(_left->clone(), _operator, _right->clone(), _bucketIdFactory));
}

namespace {

template <typename T>
ResultList containsValue(const T& value, const ValueNode& l, const ValueNode& r, const Operator& op) {
    std::unique_ptr<Value> left(l.getValue(value));
    std::unique_ptr<Value> right(r.getValue(value));
    if (left->getType() == Value::Type::Bucket || right->getType() == Value::Type::Bucket) {
        ...
        if (nVal.getType() == Value::Type::Integer &&
            (op == FunctionOperator::EQ || op == FunctionOperator::NE || op == GlobOperator::GLOB))
        {
...
template <typename T>
ResultList traceValue(const T& value, const ValueNode& l, const ValueNode& r, const Operator& op, std::ostream& out) {
    ...
    if (left->getType() == Value::Type::Bucket || right->getType() == Value::Type::Bucket) {
            ...
            (op == FunctionOperator::EQ || op == FunctionOperator::NE || op == GlobOperator::GLOB))
        {
            ...
            ResultList resultList = (op == FunctionOperator::NE) ? !ResultList(Result::get(s.contains(b)))
            ...
            out << "contained in " << s.toString() << ". Result was " << resultList << ".\n";
            return resultList;
        } else {
            out << "Compare type " << left->getType() << " vs " << right->getType() << " - Result is thus invalid.\n";
