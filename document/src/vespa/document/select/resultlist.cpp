...
void ResultList::print(std::ostream& out, bool, const std::string&) const {
    out << "ResultList(";
...
const Result& combineResultsLocal(const ResultList::Results& results) {
    if (results.empty()) {
        return Result::False;
    }
...
ResultList ResultList::operator&&(const ResultList& other) const {
    ResultList results;
...
ResultList ResultList::operator!() && {
    ResultList result;
