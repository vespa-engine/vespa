...
public:
    ...
    static uint32_t enumToMask(uint32_t rhs) noexcept { return 1u << rhs; }

    static uint32_t illegalMask() noexcept { return (1u << Result::enumRange); }

    void add(const Result& rhs) { _val |= enumToMask(rhs.toEnum()); }

    [[nodiscard]] bool hasEnum(uint32_t rhs) const { return (_val & enumToMask(rhs)) != 0u; }

    bool operator==(const ResultSet& rhs) const { return _val == rhs._val; }
