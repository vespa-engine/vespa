        ...
        if (icmp(s[pos], 'i') && icmp(s[pos + 1], 'd')) {
            ...
            if (pos < s.size()) {
                ...
                case '.': {
                    ...
                    int    divisionBits(-1);
                    size_t startPos(++pos);
                    for (; (pos < s.size()) && (std::tolower(static_cast<unsigned char>(s[pos])) >= 'a') &&
                           (std::tolower(static_cast<unsigned char>(s[pos])) <= 'z');
                         pos++)
                        ;
                    size_t len(pos - startPos);
                    if (((len == 4) &&
                         (strncasecmp(&s[startPos], "user", 4) == 0 || strncasecmp(&s[startPos], "type", 4) == 0)) ||
                    ...
                    {
                        retval = true;
                        setValue(std::make_unique<IdValueNode>(_bucketIdFactory, "id", s.substr(startPos, len),
...
bool IntegerParser::parse(std::string_view s) {
    ...
    if (pos < s.size()) {
        ...
        int64_t v = isHex ? strtoul(&s[pos], &err, 0) : strtol(&s[pos], &err, 0);
        long    len = err - &s[pos];
        ...
        if (op.parse(id.getRemaining())) {
            if (id.isUserSpec()) {
                ...
                if (v.parse(op.getRemaining())) {
                    setNode(std::make_unique<Compare>(id.stealValue(), *op.getOperator(), v.stealValue(),
                                                      _bucketIdFactory));
                    retval = true;
                }
