...
[[nodiscard]] bool document_type_is_a(const DocumentType& type, std::string_view name) {
...
std::vector<ArrayValue::VariableValue> IteratorHandler::stealValues() && {
    if (_firstValue) {
        _values.insert(_values.begin(),
                       ArrayValue::VariableValue(fieldvalue::VariableMap(), Value::SP(_firstValue.release())));
    }
...
std::unique_ptr<Value> IteratorHandler::getInternalValue(const FieldValue& fval) const {
    switch (fval.type()) {
...
void FieldValueNode::initFieldPath(const DocumentType& type) const {
    if (_fieldPath.empty()) {
...
std::unique_ptr<Value> FieldValueNode::getValue(const Context& context) const {
...
std::unique_ptr<Value> FieldValueNode::traceValue(const Context& context, std::ostream& out) const {
    ...
    const Document& doc(*context._doc);
    if (!document_type_is_a(doc.getType(), _doctype)) {
        out << "Document is of type " << doc.getType() << " which isn't a " << _doctype
            << " document, thus resolving invalid.\n";
        return std::make_unique<InvalidValue>();
    }
    if (is_simple_imported_field(_fieldExpression, doc.getType())) {
    ...
    try {
        ...
        LOGBP(warning, "Tried to compare with field %s, not found in document type", _fieldExpression.c_str());
        out << "Field not found in document type " << doc.getType() << ". Returning invalid.\n";
        return std::make_unique<InvalidValue>();
    }
}

IdValueNode::IdValueNode(const BucketIdFactory& bucketIdFactory, std::string_view name, std::string_view type,
...
std::unique_ptr<Value> IdValueNode::getValue(const DocumentId& id) const {
    std::string value;
    switch (_type) {
    case BUCKET:
...
std::unique_ptr<Value> IdValueNode::traceValue(const Context& context, std::ostream& out) const {
    if (context._doc != nullptr) {
...
std::unique_ptr<Value> IdValueNode::traceValue(const DocumentId& id, std::ostream& out) const {
    std::string value;
    switch (_type) {
    case BUCKET: {
...
FunctionValueNode::FunctionValueNode(std::string_view name, std::unique_ptr<ValueNode> src)
    : _function(), _funcname(name), _source(std::move(src)) {
    if (name == "lowercase") {
        _function = LOWERCASE;
    } else if (name == "hash") {
        _function = HASH;
        ...
        throw ParsingFailedException("No function '" + std::string(name) + "' exists.", VESPA_STRLOC);
    }
}

std::unique_ptr<Value> FunctionValueNode::getValue(std::unique_ptr<Value> val) const {
    switch (val->getType()) {
    ...
    case Value::Type::Bucket: {
        throw ParsingFailedException("No function calls are allowed on value of type bucket", VESPA_STRLOC);
        break;
    }
...
std::unique_ptr<Value> FunctionValueNode::traceValue(std::unique_ptr<Value> val, std::ostream& out) const {
    switch (val->getType()) {
    case Value::Type::String: {
        StringValue& sval(static_cast<StringValue&>(*val));
        if (_function == LOWERCASE) {
            auto result = std::make_unique<StringValue>(vespalib::LowerCase::convert(sval.getValue()));
            out << "Performed lowercase function on '" << sval << "' => '" << *result << "'.\n";
            return result;
        } else if (_function == HASH) {
            auto result =
                std::make_unique<IntegerValue>(hash(sval.getValue().c_str(), sval.getValue().size()), false);
            out << "Performed hash on string '" << sval << "' -> " << *result << "\n";
            return result;
        }
        break;
    }
    ...
    out << "Cannot use function " << _function << " on a value of type " << val->getType()
        << ". Resolving invalid.\n";
    return std::make_unique<InvalidValue>();
}
...
const char* ArithmeticValueNode::getOperatorName() const {
    switch (_operator) {
    case ADD:
        return "+";
    case SUB:
...
std::unique_ptr<Value> ArithmeticValueNode::getValue(std::unique_ptr<Value> lval, std::unique_ptr<Value> rval) const {
    switch (_operator) {
    case ADD: {
        if (lval->getType() == Value::Type::String && rval->getType() == Value::Type::String) {
...
std::unique_ptr<Value> ArithmeticValueNode::traceValue(std::unique_ptr<Value> lval, std::unique_ptr<Value> rval,
    ...
    case ADD: {
        if (lval->getType() == Value::Type::String && rval->getType() == Value::Type::String) {
            ...
            out << "Appended strings '" << slval << "' + '" << srval << "' -> '" << *result << "'.\n";
            return result;
        }
    }
        [[fallthrough]];
    ...
    out << "Failed to do operation " << getOperatorName() << " on values of type " << lval->getType() << " and "
