...
void verify_expression_not_too_large(std::string_view expr) {
    if (expr.size() > ParserLimits::MaxSelectionByteSize) {
        throw ParsingFailedException(
            vespalib::make_string("expression is too large to be parsed (max %zu bytes, got %zu)",
...
std::unique_ptr<Node> Parser::parse(const std::string& str) const {
    ...
    try {
        ...
        throw ParsingFailedException(vespalib::make_string("%s at column %u when parsing selection '%s'", err.what(),
                                                           err.location.begin.column, str.c_str()),
                                     VESPA_STRLOC);
    }
}
