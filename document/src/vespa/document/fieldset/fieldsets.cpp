...
bool FieldCollection::contains(const FieldSet& fields) const {
...
void FieldSet::copyFields(Document& dest, const Document& src, const FieldSet& fields) {
    ...
    for (Document::const_iterator it(src.begin()), e(src.end()); it != e; ++it) {
...
Document::UP FieldSet::createDocumentSubsetCopy(const DocumentTypeRepo& type_repo, const Document& src,
...
void FieldSet::stripFields(Document& doc, const FieldSet& fieldsToKeep) {
    if (fieldsToKeep.getType() == Type::ALL) {
        return;
    } else if (fieldsToKeep.getType() == Type::DOCID || fieldsToKeep.getType() == Type::NONE) {
        doc.clear();
        return;
    ...
    for (Document::const_iterator it(doc.begin()), e(doc.end()); it != e; ++it) {
        const Field& f(it.field());
