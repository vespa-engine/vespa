...
void fillUsedMasks(BucketId::Type* masks, uint8_t maxBits) {
    ...
    for (uint32_t usedBits = 0; usedBits <= maxBits; ++usedBits) {
        ...
        masks[usedBits] = (usedBits > 0) ? ((std::numeric_limits<Type>::max() << notused) >> notused)
...
void fillStripMasks(BucketId::Type* masks, uint8_t maxBits) {
    ...
    for (uint32_t usedBits = 0; usedBits <= maxBits; ++usedBits) {
        uint8_t notused = 8 * sizeof(Type) - usedBits;
        Type    usedMask = (usedBits > 0) ? ((std::numeric_limits<Type>::max() << notused) >> notused)
                                          : std::numeric_limits<Type>::max();
...
struct Initialize {
    Initialize() { BucketId::initialize(); }
};
...
uint64_t BucketId::hash::operator()(const BucketId& bucketId) const noexcept {
...
void BucketId::throwFailedSetUsedBits(uint32_t used, uint32_t availBits) {
    throw vespalib::IllegalArgumentException(
        vespalib::make_string("Failed to set used bits to %u, max is %u.", used, availBits), VESPA_STRLOC);
}

BucketId::Type BucketId::reverse(Type id) noexcept {
...
BucketId::Type BucketId::keyToBucketId(Type key) noexcept {
...
bool BucketId::contains(const BucketId& id) const noexcept {
...
vespalib::asciistream& operator<<(asciistream& os, const BucketId& id) {
    ...
    return os << "BucketId(0x" << vespalib::hex << vespalib::setw(sizeof(BucketId::Type) * 2)
...
nbostream& operator>>(nbostream& is, BucketId& bucketId) {
    is >> bucketId._id;
    return is;
}
