...
struct BucketVisitor : public document::select::Visitor {
    ...
    BucketVisitor(const BucketIdFactory& factory) : _factory(factory), _buckets(), _unknown(true) {}
    ...
    void visitAndBranch(const document::select::And& node) override {
        ...
        std::set_intersection(left._buckets.begin(), left._buckets.end(), _buckets.begin(), _buckets.end(),
    ...
    void visitOrBranch(const document::select::Or& node) override {
        ...
        std::vector<BucketId> result;
        std::set_union(left._buckets.begin(), left._buckets.end(), _buckets.begin(), _buckets.end(),
                       back_inserter(result));
        _buckets.swap(result);
    }
...
std::unique_ptr<BucketSelector::BucketVector> BucketSelector::select(const document::select::Node& expression) const {
    ...
    expression.visit(v);
    return std::unique_ptr<BucketVector>(v._unknown ? nullptr : new BucketVector(v._buckets));
}
