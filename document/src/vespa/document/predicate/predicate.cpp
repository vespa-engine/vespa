...
int compareSets(const Inspector& set1, const Inspector& set2) {
    ...
    pair<set<string>::iterator, set<string>::iterator> r = mismatch(s1.begin(), s1.end(), s2.begin());
    if (r.first == s1.end()) {
        return 0;
    }
    if (*r.first < *r.second) {
...
int compareLongs(const Inspector& long1, const Inspector& long2) {
    if (long1.valid() && !long2.valid()) {
        return -1;
    }
    if (!long1.valid() && long2.valid()) {
        return 1;
    }
    if (long1.asLong() < long2.asLong()) {
        return -1;
    }
    if (long1.asLong() > long2.asLong()) {
        return 1;
...
int compareNodes(const Inspector& n1, const Inspector& n2) {
    ...
    if (n1[Predicate::CHILDREN].valid()) {
        for (size_t i = 0; i < n1[Predicate::CHILDREN].entries(); ++i) {
            int r = compareNodes(n1[Predicate::CHILDREN][i], n2[Predicate::CHILDREN][i]);
            if (r) {
                return r;
            }
        }
        ...
        if (n1[Predicate::SET].valid()) {
            ...
            int r = compareLongs(n1[Predicate::RANGE_MIN], n2[Predicate::RANGE_MIN]);
            if (r) {
                return r;
            }
            return compareLongs(n1[Predicate::RANGE_MAX], n2[Predicate::RANGE_MAX]);
        }
    }
}
} // namespace
...
public:
    ...
    void entry(size_t, const Inspector& inspector) override { *_it++ = inspector.asString().make_string(); }
};

template <typename InsertIt> InsertFromArray<InsertIt> make_insert_from_array(InsertIt it) {
