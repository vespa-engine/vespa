...
template <typename T> class VarScope {
    T& _ptr;
    T  _old;

public:
    VarScope(T& ptr, T new_val) : _ptr(ptr), _old(_ptr) { _ptr = new_val; }
    ~VarScope() { _ptr = _old; }
};

template <typename T, typename Input> T readValue(Input& input) {
    T val;
    input >> val;
    return val;
}

template <typename Input> [[nodiscard]] std::string_view read_cstr(Input& stream) {
...
template <typename Input> uint32_t getInt1_4Bytes(Input& input) {
...
template <typename Input> uint32_t getInt1_2_4Bytes(Input& input) {
    char first_byte = *input.peek();
    if (!(first_byte & 0x80)) {
        return readValue<uint8_t>(input);
...
template <typename Input> uint64_t getInt2_4_8Bytes(Input& input) {
    char first_byte = *input.peek();
    if (!(first_byte & 0x80)) {
...
template <typename Output> void putInt1_4Bytes(Output& out, uint32_t val) {
    if (val < 0x80) {
...
template <typename Output> void putInt1_2_4Bytes(Output& out, uint32_t val) {
    if (val < 0x80) {
        out << static_cast<uint8_t>(val);
    } else if (val < 0x4000) {
