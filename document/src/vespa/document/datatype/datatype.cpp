...
public:
    ...
    FieldValue::Type getFieldValueId(unsigned int id) const {
        return id < sizeof(_type2FieldValueId) / sizeof(_type2FieldValueId[0]) ? _type2FieldValueId[id]
...
std::vector<const DataType*> DataType::getDefaultDataTypes() {
...
void DataType::buildFieldPath(FieldPath& path, std::string_view remainFieldName) const {
