...
std::string createName(const DataType& nestedType, bool create, bool remove) {
...
FieldValue::UP WeightedSetDataType::createFieldValue() const {
    return std::make_unique<WeightedSetFieldValue>(*this);
}

void WeightedSetDataType::print(std::ostream& out, bool verbose, const std::string& indent) const {
    if (getNestedType().equals(*DataType::STRING) && _createIfNonExistent && _removeIfZero) {
        out << "Tag()";
    } else {
...
void WeightedSetDataType::onBuildFieldPath(FieldPath& path, std::string_view remainFieldName) const {
