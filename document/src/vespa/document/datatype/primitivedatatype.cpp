...
const std::string_view getTypeName(DataType::Type type) {
    ...
    case DataType::T_BOOL:
        return Bool;
    case DataType::T_PREDICATE:
        return Predicate;
    case DataType::T_TENSOR:
        return Tensor;
    default:
        throw vespalib::IllegalArgumentException(vespalib::make_string("Type %i is not a primitive type", type),
                                                 VESPA_STRLOC);
    }
}
...
FieldValue::UP PrimitiveDataType::createFieldValue() const {
    switch (getId()) {
    case T_INT:
        return IntFieldValue::make();
    case T_SHORT:
        return ShortFieldValue::make();
    case T_FLOAT:
        return FloatFieldValue::make();
    case T_URI:
        return StringFieldValue::make();
    case T_STRING:
        return StringFieldValue::make();
    case T_RAW:
        return std::make_unique<RawFieldValue>();
    case T_LONG:
        return LongFieldValue::make();
    case T_DOUBLE:
        return DoubleFieldValue::make();
    case T_BOOL:
        return BoolFieldValue::make();
    case T_BYTE:
        return ByteFieldValue::make();
    case T_PREDICATE:
        return std::make_unique<PredicateFieldValue>();
    }
    LOG_ABORT("getId() returned value out of range");
}

void PrimitiveDataType::print(std::ostream& out, bool verbose, const std::string& indent) const {
    (void)verbose;
    (void)indent;
    out << "PrimitiveDataType(" << getName() << ", id " << getId() << ")";
}

void PrimitiveDataType::onBuildFieldPath(FieldPath&, std::string_view rest) const {
    if (!rest.empty()) {
