...
bool TensorDataType::equals(const DataType& other) const noexcept {
...
std::unique_ptr<const TensorDataType> TensorDataType::fromSpec(const std::string& spec) {
    return std::make_unique<const TensorDataType>(ValueType::from_spec(spec));
}

bool TensorDataType::isAssignableType(const ValueType& tensorType) const {
    return isAssignableType(_tensorType, tensorType);
}

bool TensorDataType::isAssignableType(const ValueType& fieldTensorType, const ValueType& tensorType) {
