...
void ReferenceDataType::onBuildFieldPath(FieldPath&, std::string_view remainingFieldName) const {
    if (!remainingFieldName.empty()) {
        throw IllegalArgumentException(
            make_string("Reference data type does not support further field recursion: '%s'",
                        std::string(remainingFieldName).c_str()),
            VESPA_STRLOC);
    }
}
