...
NewDocTypeRep& NewDocTypeRep::addTensorField(const std::string& name, const std::string& spec) {
    ...
    tt.idx = ++_builder._next_idx;  // Pre-increment to avoid duplicate idx
...
NewDocTypeRep& NewDocTypeRep::annotationType(int32_t id, const std::string& name) {
    ...
    ann.idx = ++_builder._next_idx;  // Pre-increment to avoid duplicate idx
    ann.name = name;
    ann.internalid = id;
    ann.datatype_idx = -1;
...
TypeRef NewDocTypeRep::referenceType(int32_t target_doctype_idx) {
    ...
    dref.idx = ++_builder._next_idx;  // Pre-increment to avoid duplicate idx
