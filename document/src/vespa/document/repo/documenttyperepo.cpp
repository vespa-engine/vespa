...
bool Repo::addDataType(const DataType& type) {
    ...
    if (data_type) {
        ...
        throw IllegalArgumentException(
            make_string("Redefinition of data type %d, \"%s\". Previously defined as \"%s\".", type.getId(),
                        type.getName().c_str(), data_type->getName().c_str()));
    }
...
template <typename T> const DataType* Repo::addDataType(unique_ptr<T> type) {
    int id = type->getId();
    if (addDataType(*type)) {
...
const DataType& Repo::addTensorType(const string& spec) {
...
const DataType& Repo::findOrThrowOrCreate(int32_t id, const string& detailedType) {
...
void addAnnotationType(const DocumenttypesConfig::Documenttype::Annotationtype& type,
                       AnnotationTypeRepo&                                      annotations) {
...
void addField(const Datatype::Sstruct::Field& field, Repo& repo, StructDataType& struct_type) {
    LOG(spam, "Adding field %s to %s", field.name.c_str(), struct_type.getName().c_str());
...
void addStruct(int32_t id, const Datatype::Sstruct& s, Repo& repo) {
    ...
    if (pos != std::string::npos) {
        ...
        if (existing) {
            LOG(spam, "Reusing id %u from body struct since its fields have already been inserted",
                existing->getId());
            id = existing->getId();
        }
        useUglyStructHack = true;
...
void addDataType(const Datatype& type, Repo& repo, const AnnotationTypeRepo& a_repo) {
    ...
    default:
        throw IllegalArgumentException(
            make_string("Unknown datatype type %d for id %d", static_cast<int>(type.type), type.id));
    }
}
...
const DocumentType* addDefaultDocument(DocumentTypeMap& type_map) {
...
private:
    ...
    struct DocTypeInProgress {
        ...
        DocTypeInProgress(const CDocType& config, DocumentTypeMap& doc_types) : cfg(config) {
    ...
    void createEmptyStructs(DocTypeInProgress& dtInP) {
        for (const auto& structT : dtInP.cfg.structtype) {
            ...
            if (const auto* oldt = dtInP.repo().lookup(structT.internalid)) {
                ...
                if (st) {
                    LOG(debug, "already has %s [%d], wanted to add %s [%d]", st->getName().c_str(), st->getId(),
    ...
    void initializeDocTypeAndInheritAnnotations(DocTypeInProgress& dtInP) {
        ...
        if (fields != nullptr) {
            ...
            LOG(error, "Missing content struct for '%s' (idx %d not found)", docT.name.c_str(), docT.contentstruct);
        ...
        for (const auto& inheritD : docT.inherits) {
            ...
            if (dt == nullptr) {
                LOG(error, "parent datatype [idx %d] missing for document %s", inheritD.idx, docT.name.c_str());
            ...
            if (parentRepo == nullptr) {
                LOG(error, "parent repo [id %d] missing for document %s", dt->getId(), docT.name.c_str());
    ...
    void createReferenceTypes(DocTypeInProgress& dtInP) {
        for (const auto& aRef : dtInP.cfg.annotationref) {
            ...
            if (target == nullptr) {
                LOG(error, "Missing annotation type [idx %d] for annotationref", aRef.annotationtype);
    ...
    void fillStructs() {
        for (auto& [idx, in_progress] : _structs_in_progress) {
            ...
            for (const auto& fieldD : in_progress.cfg.field) {
                ...
                if (ft == nullptr) {
                    LOG(error, "Missing type [idx %d] for struct %s field %s", fieldD.type,
                        in_progress.cfg.name.c_str(), fieldD.name.c_str());
    ...
    void fillDocument(DocTypeInProgress& dtInP) {
        ...
        for (const auto& inheritD : docT.inherits) {
            ...
            if (parent == nullptr) {
                LOG(error, "missing parent type [idx %d] for document %s", inheritD.idx, docT.name.c_str());
    ...
    void fillAnnotationTypes(DocTypeInProgress& dtInP) {
        for (const auto& annT : dtInP.cfg.annotationtype) {
            ...
            if (annT.datatype != -1) {
                ...
                if (dt == nullptr) {
                    LOG(error, "Missing datatype [idx %d] for annotation type %s", annT.datatype, annT.name.c_str());
                    throw IllegalArgumentException("missing datatype");
                } else {
                    at->setDataType(*dt);
                }
            ...
            for (const auto& inheritD : annT.inherits) {
                ...
                if (parent == nullptr) {
                    LOG(error, "missing parent [idx %d] for annotation %s", inheritD.idx, annT.name.c_str());
        ...
        for (const auto& docT : _input) {
            ...
            for (const auto& structT : docT.structtype) {
                ...
                for (const auto& fieldD : structT.field) {
                    LOG(debug, "doc %s struct %s field %s needs [idx %d]", docT.name.c_str(), structT.name.c_str(),
                        fieldD.name.c_str(), fieldD.type);
            ...
            for (const auto& mapT : docT.maptype) {
                ...
                LOG(debug, "doc %s wset needs [idx %d] and [idx %d]", docT.name.c_str(), mapT.keytype,
                    mapT.valuetype);
    ...
    const StructDataType* performStructInherit(int idx) {
        ...
        for (const auto& inheritD : structT.inherits) {
            ...
            if (parent == nullptr) {
                LOG(error, "Missing parent type [idx %d] for struct %s", inheritD.type, structT.name.c_str());
                throw IllegalArgumentException("missing parent type");
            }
...
void ApplyNewDoctypeConfig::madeType(const DataType* t, int idx) {
    _made_types[idx] = t;
    _needed_idx_set.erase(idx);
}

void ApplyNewDoctypeConfig::apply() {
    findNeeded();
    for (const CDocType& docT : _input) {
        auto [iter, succ] = _doc_types_in_progress.emplace(docT.idx, DocTypeInProgress(docT, _output));
        LOG_ASSERT(succ);
...
void ApplyNewDoctypeConfig::createComplexTypes() {
...
void ApplyNewDoctypeConfig::createComplexTypesForDocType(const CDocType& docT, Repo& repo) {
    for (const auto& arrT : docT.arraytype) {
    ...
    for (const auto& wsetT : docT.wsettype) {
        ...
        if (const DataType* nested = _made_types[wsetT.elementtype]) {
            auto wt = std::make_unique<WeightedSetDataType>(*nested, wsetT.createifnonexistent, wsetT.removeifzero,
                                                            wsetT.internalid);
...
const DocumentType* DocumentTypeRepo::getDocumentType(std::string_view name) const noexcept {
...
const DataType* DocumentTypeRepo::getDataType(const DocumentType& doc_type, int32_t id) const {
    const DataTypeRepo* dt_repo = findRepo(doc_type.getId());
    return dt_repo ? dt_repo->repo.lookup(id) : nullptr;
}

const DataType* DocumentTypeRepo::getDataType(const DocumentType& doc_type, std::string_view name) const {
    const DataTypeRepo* dt_repo = findRepo(doc_type.getId());
    return dt_repo ? dt_repo->repo.lookup(name) : nullptr;
}

const AnnotationType* DocumentTypeRepo::getAnnotationType(const DocumentType& doc_type, int32_t id) const {
    const DataTypeRepo* dt_repo = findRepo(doc_type.getId());
    return dt_repo ? dt_repo->annotations.lookup(id) : nullptr;
}

void DocumentTypeRepo::forEachDocumentType(std::function<void(const DocumentType&)> handler) const {
