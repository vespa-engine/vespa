...
GlobalId GlobalId::parse(std::string_view source) {
    if (source.substr(0, 6) != "gid(0x") {
        throw vespalib::IllegalArgumentException(
            "A gid must start with \"gid(0x\". Invalid source: '" + std::string(source) + "'.", VESPA_STRLOC);
    }
    if (source.size() != 2 * LENGTH + 7) {
        vespalib::asciistream ost;
        ost << "A gid string representation must be exactly " << (2 * LENGTH + 7) << " bytes long. Invalid source: '"
            << source << "'.";
        throw vespalib::IllegalArgumentException(ost.view(), VESPA_STRLOC);
    }
    if (source.substr(2 * LENGTH + 6, 1) != ")") {
        throw vespalib::IllegalArgumentException(
            "A gid must end in \")\". Invalid source: '" + std::string(source) + "'.", VESPA_STRLOC);
    }
    GlobalId id;
    for (uint32_t i = 0; i < LENGTH; ++i) {
        ...
        if (!validateHex(c1) || !validateHex(c2)) {
            throw vespalib::IllegalArgumentException("A gid can only contain hexidecimal characters [0-9a-fA-F]."
                                                     " Invalid source: '" +
                                                         std::string(source) + "'.",
                                                     VESPA_STRLOC);
        }
...
GlobalId GlobalId::calculateLastInBucket(const BucketId& bucket) {
