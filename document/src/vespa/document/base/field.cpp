...
Field::Set::Set(std::vector<CPtr> fields) : _fields(std::move(fields)) {
...
bool Field::Set::contains(const Field& field) const {
    return std::binary_search(_fields.begin(), _fields.end(), &field, Field::FieldPtrLess());
}

bool Field::Set::contains(const Set& fields) const {
    return std::includes(_fields.begin(), _fields.end(), fields._fields.begin(), fields._fields.end(),
                         Field::FieldPtrLess());
}
...
bool Field::contains(const FieldSet& fields) const {
...
void Field::validateId(int newId) {
    if (newId >= 100 && newId <= 127) {
        throw vespalib::IllegalArgumentException(
            vespalib::make_string("Attempt to set the id of %s to %d failed, values from "
    ...
    {
        throw vespalib::IllegalArgumentException(vespalib::make_string("Attempt to set the id of %s to %d"
