...
public:
    ...
    bool operator==(const IdString& other) const { return toString() == other.toString(); }
...
private:
    ...
    [[nodiscard]] uint16_t size(uint32_t index) const {
        return std::max(0, int(offset(index + 1)) - int(offset(index)) - 1);
    }
    [[nodiscard]] std::string_view getComponent(size_t index) const {
        return {_rawId.c_str() + offset(index), size(index)};
    }
