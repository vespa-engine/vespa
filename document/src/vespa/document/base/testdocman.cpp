...
void TestDocMan::setTypeRepo(const std::shared_ptr<const DocumentTypeRepo>& repo) {
    _repo = repo;
    _typeCfg = nullptr;
}
...
Document::UP TestDocMan::createRandomDocument(int seed, int maxContentSize) const {
    // Currently only one document type added.
    return createRandomDocument("testdoctype1", seed, maxContentSize);
}

Document::UP TestDocMan::createRandomDocumentAtLocation(int location, int seed, int maxContentSize) const {
    vespalib::Rand48   rnd(seed);
    std::ostringstream id;
    id << "id:mail:testdoctype1:n=" << location << ":" << (rnd.lrand48() % 0x10000) << ".html";
    return createDocument(generateRandomContent(rnd.lrand48() % maxContentSize), id.str(), "testdoctype1");
}

Document::UP TestDocMan::createRandomDocumentAtLocation(int location, int seed, int minContentSize,
                                                        int maxContentSize) const {
    vespalib::Rand48   rnd(seed);
    std::ostringstream id;
    id << "id:mail:testdoctype1:n=" << location << ":" << (rnd.lrand48() % 0x10000) << ".html";

    int size = maxContentSize > minContentSize ? rnd.lrand48() % (maxContentSize - minContentSize) + minContentSize
                                               : minContentSize;
    return createDocument(generateRandomContent(size), id.str(), "testdoctype1");
}

Document::UP TestDocMan::createRandomDocument(const std::string& type, int seed, int maxContentSize) const {
    vespalib::Rand48   rnd(seed);
    std::ostringstream id;
...
std::string TestDocMan::generateRandomContent(uint32_t size) {
