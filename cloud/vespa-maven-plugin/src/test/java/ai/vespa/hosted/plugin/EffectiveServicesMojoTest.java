...
class EffectiveServicesMojoTest {
    ...
    void devServices() throws Exception {
        assertEquals(Files.readString(Paths.get("src/test/resources/effective-services/dev.xml")),
                     effectiveServices(servicesFile, ZoneId.from("dev", "us-east-3"), CloudName.DEFAULT, ApplicationName.defaultName(), InstanceName.defaultName(), Tags.empty()));
    }
    ...
    void prodUsEast3() throws Exception {
        assertEquals(Files.readString(Paths.get("src/test/resources/effective-services/prod_us-east-3.xml")),
                     effectiveServices(servicesFile, ZoneId.from("prod", "us-east-3"), CloudName.DEFAULT, ApplicationName.defaultName(), InstanceName.defaultName(), Tags.empty()));
    }
    ...
    void prodUsWest1Services() throws Exception {
        assertEquals(Files.readString(Paths.get("src/test/resources/effective-services/prod_us-west-1.xml")),
                     effectiveServices(servicesFile, ZoneId.from("prod", "us-west-1"), CloudName.DEFAULT, ApplicationName.defaultName(), InstanceName.defaultName(), Tags.empty()));
    }
