...
public class IndexingLanguageResolver {
    ...
    public static void resolveIndexingLanguage(ParseContext context, SchemaNode indexingLanguageNode, List<Diagnostic> diagnostics) {
        ...
        Symbol symbol = fieldDefinitionNode.getSymbol();

        if (symbol.getStatus() == SymbolStatus.REFERENCE) {
            Optional<Symbol> definition = context.schemaIndex().getSymbolDefinition(symbol);
            if (definition.isPresent())symbol = definition.get();
        }

        if (symbol.getStatus() != SymbolStatus.DEFINITION || symbol.getType() != SymbolType.FIELD) {
        ...
        IndexingLanguageResolver instance = new IndexingLanguageResolver(context, symbol);
