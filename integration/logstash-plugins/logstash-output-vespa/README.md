...
To process the data, the `input` and `filter` sections are the same as for normal usage (there's an example below). The output section is different. Here's an example for Vespa Cloud:
