...
Certificate to use for https://blog.vespa.ai/securing-vespa-with-mutually-authenticated-tls/[mTLS authentication]. It's the preferred method to https://docs.vespa.ai/en/security/guide.html[authenticate with Vespa Cloud], too.
...
If set to `true`, the plugin will add the `create=true` parameter to the request. It works for `put` and `update` operations, but mostly used for `update` as https://docs.vespa.ai/en/writing/document-v1-api-guide.html#upserts[upserts].
