...
class WordFoldersTest : public Test
{
  ...
  bool TokenizeAnnotatedBuffer() {
    ...
    const char *testinput = "This is a "
      "\xEF\xBF\xB9" "cafÃ©" "\xEF\xBF\xBA" "cafe" "\xEF\xBF\xBB"
  ...
  bool TokenizeAnnotatedUCS4Buffer() {
   ...
   bool AccentRemovalTest() {
       ...
       auto input = std::unique_ptr<char, decltype(freefunction)>(Fast_UnicodeUtil::strdupLAT1("¡¢£¤¥¦§¨©ª«¬­®¯°±²³´µ¶·¸¹º»¼½¾¿ÀÁÂÃÄÅÆÇÈÉÊËÌÍÎÏĞÑÒÓÔÕÖ×ØÙÚÛÜİŞßàáâãäåæçèéêëìíîïğñòóôõö÷øùúûüışpş!"),
                                                                  freefunction);
