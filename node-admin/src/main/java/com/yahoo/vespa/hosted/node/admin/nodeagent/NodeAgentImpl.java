...
public class NodeAgentImpl implements NodeAgent {
    ...
    private final NodeMechanisms nodeMechanisms;
    ...
    public NodeAgentImpl(
            ...
            final NodeMechanisms nodeMechanisms) {
        this.logPrefix = "NodeAgent(" + hostName + "): ";
        this.nodeRepository = nodeRepository;
        this.orchestrator = orchestrator;
        this.hostname = hostName;
        this.nodeMechanisms = nodeMechanisms;
    }
    ...
    public void startNodeInContainerIfNeeded(final ContainerNodeSpec nodeSpec) {
        ...
        nodeMechanisms.executeResume(nodeSpec.containerName);//, RESUME_NODE_COMMAND);
    ...
    public void publishThatNodeIsRunningIfRequired(final ContainerNodeSpec nodeSpec) throws IOException {
        final String containerVespaVersion = nodeMechanisms.getVespaVersionOrNull(nodeSpec.containerName);
    ...
    public void startContainerIfNeeded(final ContainerNodeSpec nodeSpec) {
        if (nodeMechanisms.startContainerIfNeeded(nodeSpec)) {
        ...
        synchronized (monitor) {
            try {
                ...
                switch (nodeSpec.nodeState) {
                    case PROVISIONED:
                        if (nodeMechanisms.removeContainerIfNeeded(nodeSpec, hostname, orchestrator)) {
                        ...
                        nodeMechanisms.deleteContainerStorage(nodeSpec.containerName);
                    ...
                    case RESERVED:
                        ...
                        break;
                    case ACTIVE:
                        if (nodeMechanisms.shouldScheduleDownloadOfImage(nodeSpec.wantedDockerImage.get())) {
                            if (imageBeingDownloaded == nodeSpec.wantedDockerImage.get()) {
                                // Downloading already scheduled.
                                return;
                            }
                            imageBeingDownloaded = nodeSpec.wantedDockerImage.get();
                            // Create a tick when download is finished.
                            nodeMechanisms.scheduleDownloadOfImage(nodeSpec, this::nodeTickInNewThread);
                            return;
                        } else {
