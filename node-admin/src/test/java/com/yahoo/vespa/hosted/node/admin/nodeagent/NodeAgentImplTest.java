...
public class NodeAgentImplTest {
    ...
    @Test
    public void containerIsRecreatedOnResourceChange() {
        ...
        NodeAgentContext secondContext = createContext(specBuilder.memoryGb(20).vcpu(5).build());
        nodeAgent.doConverge(secondContext);
        ContainerResources resourcesAfterThird = ContainerResources.from(0, 5, 20);
    ...
    public void initial_cpu_cap_test() {
        ...
        inOrder.verify(orchestrator, times(3)).suspend(any());

        // Recreates container with CPU cap after warmup
        clock.advance(Duration.ofSeconds(31));
        ContainerResources resourcesWithCpuCap = ContainerResources.from(0, resources.vcpu(), resources.memoryGb());
        mockGetContainer(dockerImage, resourcesWithCpuCap, true);
        nodeAgent.doConverge(context);
        inOrder.verify(orchestrator, never()).suspend(any());
        inOrder.verify(containerOperations).removeContainer(eq(context), any());
        inOrder.verify(containerOperations).startContainer(eq(context));
        inOrder.verify(orchestrator).resume(eq(hostName));
