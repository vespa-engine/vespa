...
struct LogMessageTest : public ::testing::Test {
    ...
    void convert() { Converter::log_message_to_proto(message, proto); }
...
void expect_proto_log_message_equal(int64_t exp_time_nanos, const std::string& exp_hostname, int32_t exp_process_id,
                                    int32_t exp_thread_id, const std::string& exp_service,
                                    const std::string& exp_component, ProtoLogLevel exp_level,
                                    const std::string& exp_payload, const Converter::ProtoLogMessage& proto) {
...
struct LogRequestTest : public ::testing::Test {
    ...
    void convert() { Converter::log_messages_to_proto(messages, proto); }
};
...
TEST_F(LogRequestTest, invalid_utf8_is_filtered) {
    messages.emplace_back(12345, "foo_host", 3, 5, "foo_service", "foo_component", Logger::info,
                          "valid: \xE2\x82\xAC and \xEF\xBF\xBA; semi-valid: \xED\xA0\xBD\xED\xB8\x80; invalid: \xCC "
                          "surrogate \xED\xBF\xBF overlong \xC1\x81 end");
    convert();
    EXPECT_EQ(1, proto.log_messages_size());
    expect_proto_log_message_equal(12345, "foo_host", 3, 5, "foo_service", "foo_component",
                                   ProtoLogLevel::LogMessage_Level_INFO,
                                   "valid: \xE2\x82\xAC and \xEF\xBF\xBA; semi-valid: " FFFD FFFD "; invalid: " FFFD
                                   " surrogate " FFFD " overlong " FFFD FFFD " end",
