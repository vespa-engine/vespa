...
void encode_log_response(const ProtoConverter::ProtoLogResponse& src, FRT_Values& dst) {
...
public:
    RpcServer();
    ~RpcServer() override;
    int get_listen_port() { return server.supervisor().GetListenPort(); }
...
RpcServer::RpcServer()
    : server(), request_count(0), messages(), reply_with_error(false), reply_with_proto_response(true) {
...
std::string make_log_line(const std::string& level, const std::string& payload) {
...
struct MockMetricsManager : public DummyMetricsManager {
    ...
    void add(Counter::Increment) override { ++add_count; }
};
...
struct RpcForwarderTest : public ::testing::Test {
    ...
    RpcForwarderTest()
        : server(),
          ...
          metrics(metrics_mgr),
          forwarder(metrics, make_forward_filter(), supervisor.get(), "localhost", server.get_listen_port(), 60.0,
                    3) {}
    void forward_line(const std::string& payload) { forwarder.forwardLine(make_log_line("info", payload)); }
    void forward_line(const std::string& level, const std::string& payload) {
        forwarder.forwardLine(make_log_line(level, payload));
    }
    void forward_bad_line() { forwarder.forwardLine("badline"); }
    void flush() { forwarder.flush(); }
    void expect_messages() { expect_messages(0, {}); }
