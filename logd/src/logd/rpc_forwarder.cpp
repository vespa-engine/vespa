...
public:
    GuardedRequest() : _request(new FRT_RPCRequest()) {}
    ~GuardedRequest() { _request->internal_subref(); }
...
void RpcForwarder::ping_logserver() {
    ...
    if (!request->CheckReturnTypes("")) {
        auto error_msg = make_string("Error in rpc ping to logserver ('%s'): '%s'", _connection_spec.c_str(),
...
RpcForwarder::RpcForwarder(Metrics& metrics, const ForwardMap& forward_filter, FRT_Supervisor& supervisor,
                           const std::string& hostname, int rpc_port, double rpc_timeout_secs,
...
void encode_log_request(const ProtoConverter::ProtoLogRequest& src, FRT_RPCRequest& dst) {
...
bool should_forward_log_message(const LogMessage& message, const ForwardMap& filter) {
...
void RpcForwarder::forwardLine(std::string_view line) {
    LogMessage message;
    try {
...
void RpcForwarder::flush() {
    ...
    if (!request->CheckReturnTypes("bix")) {
        auto error_msg = make_string("Error in rpc reply from logserver ('%s'): '%s'", _connection_spec.c_str(),
                                     request->GetErrorMessage());
        LOG(debug, "%s", error_msg.c_str());
        throw ConnectionException(error_msg);
    }
    ...
    if (!decode_log_response(*request, proto_response)) {
        auto error_msg =
            make_string("Error during decoding of protobuf response from logserver ('%s')", _connection_spec.c_str());
...
int RpcForwarder::badLines() const {
    return _bad_lines;
}

void RpcForwarder::resetBadLines() {
    _bad_lines = 0;
}
