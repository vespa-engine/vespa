...
public class RPCSendV2 extends RPCSend {
    ...
    private void encodeHeaderIfPresent(Values v, Map<String, String> srcKvs) {
        ...
        if (srcKvs.isEmpty()) {
            ...
            for (var kv : srcKvs.entrySet()) {
                kvs.setString(kv.getKey(), kv.getValue());
            }
                                    ...
                                    long timeRemaining, byte[] payload, int traceLevel)
    {
        ...
        encodeHeaderIfPresent(v, msg.getHeaderKeyValues());
    ...
    private Map<String, String> decodeHeaderKVsOrNull(Values args) {
        ...
        if (hdrLen > 0 && (CompressionType.valueOf(args.get(0).asInt8()) == CompressionType.NONE)) {
            ...
            if (kvs.valid() && kvs.fields() > 0) {
                var kvMap = new HashMap<String, String>(kvs.fields()); // TODO specialized low cardinality map?
                kvs.traverse((ObjectSymbolTraverser) (sym, inspector) -> {
                    kvMap.put(hdrSlime.inspect(sym), inspector.asString());
                });
                return kvMap;
            }
        }
    ...
    protected Params toParams(Values args) {
        Params p = new Params();
        p.headerKVs = decodeHeaderKVsOrNull(args);
