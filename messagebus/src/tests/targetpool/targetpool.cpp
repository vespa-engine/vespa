...
public:
    ...
    uint64_t getMilliTime() const override { return millis; }
};

TEST(TargetPoolTest, targetpool_test) {
    ...
    target.reset();
    ASSERT_TRUE((target = pool.getTarget(orb, adr2)));
    target.reset();
    ASSERT_TRUE((target = pool.getTarget(orb, adr3)));
    target.reset();
