...
Reply::Reply() : _errors(), _msg(), _retryDelay(-1.0) {
    // empty
}
...
void Reply::swapState(Routable& rhs) {
...
void Reply::addError(const Error& e) {
...
bool Reply::hasFatalErrors() const {
...
const Error& Reply::getError(uint32_t i) const {
    return _errors[i];
}

uint32_t Reply::getNumErrors() const {
    return _errors.size();
}

void Reply::setMessage(Message::UP msg) {
    _msg = std::move(msg);
}

Message::UP Reply::getMessage() {
