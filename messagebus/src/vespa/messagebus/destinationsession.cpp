...
void DestinationSession::acknowledge(Message::UP msg) {
    Reply::UP ack(new EmptyReply());
    ack->swapState(*msg);
    reply(std::move(ack));
}

void DestinationSession::reply(Reply::UP ret) {
    IReplyHandler& handler = ret->getCallStack().pop(*ret);
    handler.handleReply(std::move(ret));
}

void DestinationSession::handleMessage(Message::UP msg) {
    _msgHandler.handleMessage(std::move(msg));
}

const string DestinationSession::getConnectionSpec() const {
