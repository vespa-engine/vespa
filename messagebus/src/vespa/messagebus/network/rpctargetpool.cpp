...
RPCTargetPool::Entry::Entry(std::vector<RPCTarget::SP> targets, uint64_t lastUse)
    : _targets(std::move(targets)), _lastUse(lastUse), _next(0) {
}

RPCTarget::SP RPCTargetPool::Entry::getTarget(const LockGuard&, uint64_t now) {
...
bool RPCTargetPool::Entry::inUse(const LockGuard&) const {
...
void RPCTargetPool::flushTargets(bool force) {
...
size_t RPCTargetPool::size() {
    LockGuard guard(_lock);
    return _targets.size();
}

RPCTarget::SP RPCTargetPool::getTarget(FRT_Supervisor& orb, const RPCServiceAddress& address) {
