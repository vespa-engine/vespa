...
bool RPCSendV2::isCompatible(string_view method, string_view request, string_view response) {
    return (method == METHOD_NAME) && (request == METHOD_PARAMS) && (response == METHOD_RETURN);
}

void RPCSendV2::build(FRT_ReflectionBuilder& builder, CapabilitySet required_capabilities) {
...
const char* RPCSendV2::getReturnSpec() const {
...
public:
    ...
    [[nodiscard]] bool has_metadata() const noexcept { return static_cast<bool>(_lazy_slime); }

    // Precondition: has_metadata() == true
    void encode_into(OutputBuf& out_buf) const { BinaryFormat::encode(_lazy_slime->slime, out_buf); }
};

SlimeMetadataInjector::SlimeMetadataInjector() = default;
SlimeMetadataInjector::~SlimeMetadataInjector() = default;

SlimeMetadataInjector::LazySlimeState::LazySlimeState() : slime(), kv_cursor(slime.setObject().setObject(KVS_F)) {
}
...
void RPCSendV2::encodeRequest(FRT_RPCRequest& req, const Version& version, const Route& route,
...
public:
    SlimeMetadataExtractor(const Memory& memory) : _slime(), _kvs_inspector(nullptr) {
...
public:
    explicit ParamsV2(const FRT_Values& arg) : _slime(), _meta_extractor() {
    ...
    void decode_header_if_present(const FRT_Values& arg) {
        ...
        {
            _meta_extractor =
                std::make_unique<SlimeMetadataExtractor>(Memory(hdr_blob.getData(), hdr_blob.getDataLen()));
        }
    }
    ...
    duration getRemainingTime() const override {
        return std::chrono::milliseconds(_slime.get()[TIMELEFT_F].asLong());
    }

    Version getVersion() const override { return Version(_slime.get()[VERSION_F].asString().make_string()); }
    string_view getRoute() const override { return _slime.get()[ROUTE_F].asString().make_stringview(); }
    string_view getSession() const override { return _slime.get()[SESSION_F].asString().make_stringview(); }
    string_view getProtocol() const override { return _slime.get()[PROTOCOL_F].asString().make_stringview(); }
...
std::unique_ptr<RPCSend::Params> RPCSendV2::toParams(const FRT_Values& args) const {
    return std::make_unique<ParamsV2>(args);
}

std::unique_ptr<Reply> RPCSendV2::createReply(const FRT_Values& ret, const string& serviceName, Error& error,
                                              vespalib::Trace& rootTrace) const {
    ...
    Reply::UP reply;
    if (payload.size > 0) {
        reply = decode(root[PROTOCOL_F].asString().make_stringview(), version, BlobRef(payload.data, payload.size),
                       error);
    }
    if (!reply) {
...
void RPCSendV2::createResponse(FRT_Values& ret, const string& version, Reply& reply, Blob payload) const {
