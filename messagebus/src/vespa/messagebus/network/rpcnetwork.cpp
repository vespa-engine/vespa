...
RPCNetwork::SendContext::handleVersion(const vespalib::Version *version)
{
    ...
    if (shouldSend) {
        if (_net.allowDispatchForEncode()) {
            // RequestDone uses 1
            _net.getExecutor().execute(0, [this]() {
                _net.send(*this);
                delete this;
            });
        } else {
...
RPCNetwork::RPCNetwork(const RPCNetworkParams &params) :
    ...
    _sequencedExecutor(vespalib::SequencedTaskExecutor::create(params.getNumThreads(), 1000,
                       params.getOptimizeFor(), params.getKindOfWatermark(), params.getReactionTime())),
