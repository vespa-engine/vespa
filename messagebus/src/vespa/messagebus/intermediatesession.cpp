...
void IntermediateSession::close() {
    _mbus.unregisterSession(_name);
    _mbus.sync();
}

void IntermediateSession::forward(Routable::UP routable) {
    if (routable->isReply()) {
...
void IntermediateSession::forward(Reply::UP reply) {
    IReplyHandler& handler = reply->getCallStack().pop(*reply);
    handler.handleReply(std::move(reply));
}

void IntermediateSession::forward(Message::UP msg) {
    msg->pushHandler(*this);
    _gate->handleMessage(std::move(msg));
}

void IntermediateSession::handleMessage(Message::UP msg) {
    _msgHandler.handleMessage(std::move(msg));
}

void IntermediateSession::handleReply(Reply::UP reply) {
    _replyHandler.handleReply(std::move(reply));
}

const string IntermediateSession::getConnectionSpec() const {
