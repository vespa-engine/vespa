...
RoutingTable::HopIterator::HopIterator(const std::map<string, HopBlueprint>& hops)
    : _pos(hops.begin()), _end(hops.end()) {
}

RoutingTable::RouteIterator::RouteIterator(const std::map<string, Route>& routes)
    : _pos(routes.begin()), _end(routes.end()) {
}

RoutingTable::RoutingTable(const RoutingTableSpec& spec) : _name(spec.getProtocol()), _hops(), _routes() {
...
bool RoutingTable::hasHop(const string& name) const {
    return _hops.find(name) != _hops.end();
}

const HopBlueprint* RoutingTable::getHop(const string& name) const {
    auto it = _hops.find(name);
    return it != _hops.end() ? &(it->second) : nullptr;
}

bool RoutingTable::hasRoute(const string& name) const {
    return _routes.find(name) != _routes.end();
}

const Route* RoutingTable::getRoute(const string& name) const {
