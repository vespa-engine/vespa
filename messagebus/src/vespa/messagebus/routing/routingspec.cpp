...
RoutingSpec& RoutingSpec::addTable(RoutingTableSpec&& table) & {
...
string RoutingSpec::toConfigString(const string& input) {
    string ret;
    ret.append("\"");
    for (char i : input) {
        if (i == '\\') {
...
void RoutingSpec::toConfig(string& cfg, const string& prefix) const {
...
string RoutingSpec::toString() const {
    string ret = "";
    toConfig(ret, "");
    return ret;
}

bool RoutingSpec::operator==(const RoutingSpec& rhs) const {
