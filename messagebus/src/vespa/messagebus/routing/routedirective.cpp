...
RouteDirective::RouteDirective(std::string_view name) : _name(name) {
    // empty
}

RouteDirective::~RouteDirective() = default;

bool RouteDirective::matches(const IHopDirective& dir) const {
...
string RouteDirective::toString() const {
    return make_string("route:%s", _name.c_str());
}

string RouteDirective::toDebugString() const {
