...
RPCMessageBus::RPCMessageBus(const MessageBusParams& mbusParams, const RPCNetworkParams& rpcParams,
    ...
    : _net(rpcParams), _bus(_net, mbusParams), _agent(_bus), _subscriber(routingCfgUri.getContext()) {
...
RPCMessageBus::RPCMessageBus(const ProtocolSet& protocols, const RPCNetworkParams& rpcParams,
                             const config::ConfigUri& routingCfgUri)
    : _net(rpcParams), _bus(_net, protocols), _agent(_bus), _subscriber(routingCfgUri.getContext()) {
    _subscriber.subscribe(routingCfgUri.getConfigId(), &_agent);
    _subscriber.start();
}
