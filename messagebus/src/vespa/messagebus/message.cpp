...
Message::Message() : _route(), _timeReceived(), _timeRemaining(0), _retryEnabled(true), _retry(0) {
...
void Message::swapState(Routable& rhs) {
...
Message& Message::setTimeReceivedNow() {
    _timeReceived = vespalib::steady_clock::now();
    return *this;
}

duration Message::getTimeRemainingNow() const {
