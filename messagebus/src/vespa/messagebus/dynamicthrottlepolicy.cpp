...
DynamicThrottlePolicy& DynamicThrottlePolicy::setEfficiencyThreshold(double efficiencyThreshold) {
    _efficiencyThreshold = efficiencyThreshold;
    return *this;
}

DynamicThrottlePolicy& DynamicThrottlePolicy::setWindowSizeIncrement(double windowSizeIncrement) {
    _windowSizeIncrement = windowSizeIncrement;
    _windowSize = std::max(_windowSize, _windowSizeIncrement);
    return *this;
}

DynamicThrottlePolicy& DynamicThrottlePolicy::setWindowSizeBackOff(double windowSizeBackOff) {
    _windowSizeBackOff = std::max(0.0, std::min(1.0, windowSizeBackOff));
    return *this;
}

DynamicThrottlePolicy& DynamicThrottlePolicy::setResizeRate(double resizeRate) {
    _resizeRate = std::max(2.0, resizeRate);
    return *this;
}

DynamicThrottlePolicy& DynamicThrottlePolicy::setWeight(double weight) {
    _weight = weight;
    return *this;
}

DynamicThrottlePolicy& DynamicThrottlePolicy::setIdleTimePeriod(uint64_t period) {
    _idleTimePeriod = period;
    return *this;
}

DynamicThrottlePolicy& DynamicThrottlePolicy::setMaxWindowSize(double max) {
    _maxWindowSize = max;
    return *this;
}

DynamicThrottlePolicy& DynamicThrottlePolicy::setMinWindowSize(double min) {
    _minWindowSize = min;
    _windowSize = std::max(_minWindowSize, _windowSizeIncrement);
    return *this;
}

DynamicThrottlePolicy& DynamicThrottlePolicy::setWindowSizeDecrementFactor(double decrementFactor) {
    _decrementFactor = decrementFactor;
    return *this;
}

DynamicThrottlePolicy& DynamicThrottlePolicy::setMaxPendingCount(uint32_t maxCount) {
    StaticThrottlePolicy::setMaxPendingCount(maxCount);
    _maxWindowSize = maxCount;
    return *this;
}

bool DynamicThrottlePolicy::canSend(const Message& msg, uint32_t pendingCount) {
...
void DynamicThrottlePolicy::processMessage(Message& msg) {
    ...
    if (throughput > _localMaxThroughput) {
        ...
        if (efficiency < _efficiencyThreshold) {
            _windowSize =
                std::min(_windowSize * _windowSizeBackOff, _windowSize - _decrementFactor * _windowSizeIncrement);
            _localMaxThroughput = 0;
        } else {
            _windowSize += _weight * _windowSizeIncrement;
        }
...
void DynamicThrottlePolicy::processReply(Reply& reply) {
