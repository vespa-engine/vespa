...
RPCHooks::RPCHooks(CommandQueue& commands, FRT_Supervisor& supervisor, ModelOwner& modelOwner)
    : _commands(commands), _orb(supervisor), _modelOwner(modelOwner) {
    initRPC(&_orb);
}
...
void RPCHooks::initRPC(FRT_Supervisor* supervisor) {
    ...
    //-------------------------------------------------------------------------
    rb.DefineMethod("sentinel.ls", "", "s", FRT_METHOD(RPCHooks::rpc_listServices), this);
    ...
    //-------------------------------------------------------------------------
    rb.DefineMethod("sentinel.service.restart", "s", "", FRT_METHOD(RPCHooks::rpc_restartService), this);
    rb.MethodDesc("restart a service");
    rb.RequestAccessFilter(make_sentinel_management_api_capability_filter());
    //-------------------------------------------------------------------------
    rb.DefineMethod("sentinel.service.stop", "s", "", FRT_METHOD(RPCHooks::rpc_stopService), this);
    rb.MethodDesc("stop a service");
    rb.RequestAccessFilter(make_sentinel_management_api_capability_filter());
    //-------------------------------------------------------------------------
    rb.DefineMethod("sentinel.service.start", "s", "", FRT_METHOD(RPCHooks::rpc_startService), this);
    rb.MethodDesc("start a service");
    rb.RequestAccessFilter(make_sentinel_management_api_capability_filter());
    //-------------------------------------------------------------------------
    rb.DefineMethod("sentinel.check.connectivity", "sii", "s", FRT_METHOD(RPCHooks::rpc_checkConnectivity), this);
    ...
    //-------------------------------------------------------------------------
    rb.DefineMethod("sentinel.report.connectivity", "i", "SS", FRT_METHOD(RPCHooks::rpc_reportConnectivity), this);
...
void RPCHooks::rpc_listServices(FRT_RPCRequest* req) {
    LOG(debug, "got listservices");
    req->Detach();
    _commands.enqueue(std::make_unique<Cmd>(req, Cmd::LIST));
}

void RPCHooks::rpc_restartService(FRT_RPCRequest* req) {
...
void RPCHooks::rpc_stopService(FRT_RPCRequest* req) {
    FRT_Values& args = *req->GetParams();
    const char* srvNM = args[0]._string._str;
...
void RPCHooks::rpc_startService(FRT_RPCRequest* req) {
    FRT_Values& args = *req->GetParams();
    const char* srvNM = args[0]._string._str;
