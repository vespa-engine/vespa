...
template <typename MetricImpl> void testMetricsGetDimensionsAsPartOfMangledNameImpl() {
    MetricImpl m("test", {{"foo", "bar"}}, "description goes here");
    EXPECT_EQ(std::string("test{foo:bar}"), m.getMangledName());
}

template <typename MetricImpl> void testMangledNameMayContainMultipleDimensionsImpl() {
    MetricImpl m("test", {{"flarn", "yarn"}, {"foo", "bar"}}, "description goes here");
...
TEST(MetricTest, mangled_name_lists_dimensions_in_lexicographic_order) {
    LongValueMetric m("test", {{"xyz", "bar"}, {"abc", "foo"}, {"def", "baz"}}, "", nullptr);
