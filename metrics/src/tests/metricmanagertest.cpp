...
SubMetricSet::SubMetricSet(const Metric::String& name, MetricSet* owner)
    : MetricSet(name, {{"partofsum"}, {"sub"}}, "sub desc", owner),
      val1("val1", {{"tag4"}, {"snaptest"}}, "val1 desc", this), val2("val2", {{"tag5"}}, "val2 desc", this),
...
MultiSubMetricSet::MultiSubMetricSet(MetricSet* owner)
    : set("multisub", {{"multisub"}}, "", owner), count("count", {{"snaptest"}}, "counter", &set), a("a", &set),
      b("b", &set), sum("sum", {{"snaptest"}}, "", &set) {
...
TestMetricSet::TestMetricSet()
    : set("temp", {{"test"}}, "desc of test set"), val1("val1", {{"tag1"}}, "val1 desc", &set),
      val2("val2", {{"tag1"}, {"tag2"}}, "val2 desc", &set), val3("val3", {{"tag2"}, {"tag3"}}, "val3 desc", &set),
      val4("val4", {{"tag3"}}, "val4 desc", &set), val5("val5", {{"tag2"}}, "val5 desc", &set),
      val6("val6", {{"tag4"}, {"snaptest"}}, "val6 desc", &set), val7("val7", {}, "val7 desc", &set),
      val8("val8", {{"tag6"}}, "val8 desc", &set), val9("sub", &set), val10(&set) {}
...
struct MetricNameVisitor : public MetricVisitor {
    ...
    bool visitMetricSet(const MetricSet& metricSet, bool autoGenerated) override {
        if (debug) {
            ost << "<" << (autoGenerated ? "*" : "") << metricSet.getPath() << ">\n";
        }
        return true;
    }
...
std::pair<std::string, std::string> getMatchedMetrics(const std::string& config) {
    TestMetricSet mySet;
    MetricManager mm;
...
TEST_F(MetricManagerTest, test_consumer_visitor) {
    ...
    ASSERT_CONSUMER_MATCH("testAddTagAndName",
    ...
    ASSERT_CONSUMER_MATCH("testAddAndRemoveTag",
                          std::string("temp.val1\n"
                                      "temp.val4\n"),
                          "raw:"
                          "consumer[1]\n"
    ...
    ASSERT_CONSUMER_MATCH("testWildCards",
                          std::string("temp.val1\n"
                                      "temp.val2\n"
                                      "temp.val3\n"
                                      "temp.val4\n"
    ...
    // Test more wildcards
    ASSERT_CONSUMER_MATCH("testWildCards2", std::string("temp.sub.val1\n"),
                          "raw:"
                          "consumer[1]\n"
                          "consumer[0].name consumer\n"
                          "consumer[0].addedmetrics[1]\n"
    ...
    // test adding all
    ASSERT_CONSUMER_MATCH("testAddAll",
                          std::string("metricmanager.periodichooklatency\n"
    ...
    ASSERT_CONSUMER_MATCH("testAddAll2",
                          std::string("temp.val1\n"
                                      "temp.val2\n"
                                      "temp.val3\n"
                                      "temp.val4\n"
    ...
    ASSERT_CONSUMER_MATCH("testSpecifiedSetName",
                          std::string("temp.sub.val1\n"
                                      "temp.sub.val2\n"
                                      "*temp.sub.valsum\n"),
                          "raw:"
                          "consumer[1]\n"
    ...
    ASSERT_CONSUMER_MATCH("testSpecifiedSetTag",
                          std::string("temp.sub.val1\n"
                                      "temp.sub.val2\n"
                                      "*temp.sub.valsum\n"
                                      "temp.multisub.a.val1\n"
    ...
    // certain tag.
    ASSERT_CONSUMER_MATCH("testSpecifiedSetTagWithExceptionTags",
                          std::string("temp.val1\n"
                                      "temp.val4\n"
                                      "temp.val7\n"
                                      "temp.val8\n"
...
public:
    ...
    time_point getTime() const override {
        return time_point(vespalib::from_s<time_point::duration>(load_relaxed(_time)));
    }
...
TEST_F(MetricManagerTest, test_snapshots) {
    ...
    MetricNameVisitor visitor;
    {
        ...
        EXPECT_EQ(std::string("\n"
                              ...
                              "*temp.multisub.sum.valsum\n"),
                  "\n" + visitor.toString())
            << (consumerSpec ? consumerSpec->toString() : "Non-existing consumer");
    }
...
TEST_F(MetricManagerTest, test_json_output) {
    ...
    EXPECT_EQ(std::string("temp.val6"), slime.get()["values"][0]["name"].asString().make_string()) << jsonData;
    EXPECT_EQ(std::string("val6 desc"), slime.get()["values"][0]["description"].asString().make_string()) << jsonData;
    ...
    EXPECT_EQ(std::string("temp.multisub.sum.valsum"), slime.get()["values"][10]["name"].asString().make_string())
        << jsonData;
    EXPECT_EQ(std::string("valsum desc"), slime.get()["values"][10]["description"].asString().make_string())
        << jsonData;
...
public:
    ...
    vespalib::slime::Cursor& nthMetric(size_t metricIndex) { return root()["values"][metricIndex]; }

    size_t nthMetricDimensionCount(size_t metricIndex) { return nthMetric(metricIndex)["dimensions"].children(); }

    std::string nthMetricName(size_t metricIndex) { return nthMetric(metricIndex)["name"].asString().make_string(); }
    ...
    void verifyDimensions(size_t metricIndex, const std::string& name, const Metric::Tags& dimensions) {
...
JsonMetricWrapper::JsonMetricWrapper(const std::string& jsonText) : _jsonText(jsonText) {
...
DimensionTestMetricSet::DimensionTestMetricSet(MetricSet* owner)
    : MetricSet("temp", {{"foo", "megafoo"}, {"bar", "hyperbar"}}, "", owner),
      val1("val1", {{"tag1"}}, "val1 desc", this), val2("val2", {{"baz", "superbaz"}}, "val2 desc", this) {}
...
TEST_F(MetricManagerTest, json_output_supports_multiple_dimensions) {
    ...
    json.verifyDimensions(1, "temp.val2", {{"foo", "megafoo"}, {"bar", "hyperbar"}, {"baz", "superbaz"}});
}
...
NestedDimensionTestMetricSet::NestedDimensionTestMetricSet()
    : MetricSet("outer", {{"fancy", "stuff"}}, ""), nestedSet(this) {}
...
TEST_F(MetricManagerTest, json_output_can_nest_dimensions_from_multiple_metric_sets) {
    ...
    json.verifyDimensions(1, "outer.temp.val2",
                          {{"foo", "megafoo"}, {"bar", "hyperbar"}, {"baz", "superbaz"}, {"fancy", "stuff"}});
}
...
DimensionOverridableTestMetricSet::DimensionOverridableTestMetricSet(const std::string& dimValue, MetricSet* owner)
    : MetricSet("temp", {{"foo", dimValue}}, "", owner), val("val", {}, "val desc", this) {}
...
SameNamesTestMetricSet::SameNamesTestMetricSet()
    : MetricSet("outer", {{"fancy", "stuff"}}, ""), set1("bar", this), set2("baz", this) {}
...
TEST_F(MetricManagerTest, json_output_can_have_multiple_sets_with_same_name) {
    ...
    json.verifyDimensions(0, "outer.temp.val", {{"foo", "bar"}, {"fancy", "stuff"}});
    json.verifyDimensions(1, "outer.temp.val", {{"foo", "baz"}, {"fancy", "stuff"}});
}
...
TEST_F(MetricManagerTest, test_text_output) {
    ...
    mm.init(ConfigUri("raw:"
                      ...
                      "consumer[1].tags[1]\n"
                      "consumer[1].tags[0] snaptest\n"));
    std::string expected("snapshot \"Active metrics showing updates since last snapshot\" from 1970-01-01 00:16:40.000 "
                         "UTC to 1970-01-01 00:00:00.000 UTC period 0\n"
...
MetricSetWrapper::MetricSetWrapper() : MetricSet("top_level", {}, "stuff and junk", nullptr), sub(this) {}
...
SneakyNamesMetricSet::SneakyNamesMetricSet()
    : MetricSet("sneaky/path", {}, "sub desc", nullptr), val1("a.name", {{"foo.bar", "blah\nbaz\"zoid\\"}}, "", this),
...
struct MyUpdateHook : public UpdateHook {
    ...
    MyUpdateHook(std::ostringstream& output, std::mutex& output_mutex, const char* name,
        ...
        : UpdateHook(name, period), _output(output), _output_mutex(output_mutex), _timer(timer) {}
...
TEST_F(MetricManagerTest, test_update_hooks) {
    ...
    std::string expected("Running init\n"
                         "1000: BIS called\n"
                         "1000: BIL called\n"
                         "Init done\n"
                         "1006: BIS called\n"
