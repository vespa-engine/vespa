...
template <typename AddendMetric> SumMetric<AddendMetric>::StartValue::~StartValue() = default;
...
template <typename AddendMetric>
SumMetric<AddendMetric>::SumMetric(const String& name, Tags tags, const String& description, MetricSet* owner)
    : Metric(name, tags, description, owner), _startValue(), _metricsToSum() {}

template <typename AddendMetric>
SumMetric<AddendMetric>::SumMetric(const SumMetric<AddendMetric>& other, std::vector<Metric::UP>& ownerList,
                                   MetricSet* owner)
    : Metric(other, owner), _startValue(other._startValue), _metricsToSum() {
    (void)ownerList;
    if (other._owner == 0) {
        throw vespalib::IllegalStateException("Cannot copy a sum metric not registered in a metric set, as "
                                              "we need to use parent to detect new metrics to point to.",
                                              VESPA_STRLOC);
    }
    if (owner == 0) {
        throw vespalib::IllegalStateException("Cannot copy a sum metric directly. One needs to at least "
    ...
    for (const AddendMetric* m : _metricsToSum) {
        ...
        for (uint32_t i = parentPath.size(), n = addendPath.size() - 1; i < n; ++i) {
            ...
            if (child == 0) {
                throw vespalib::IllegalStateException("Metric " + addendPath[i] + " in metric set " +
                                                          newAddendParent->getPath() +
                                                          " was expected to "
            ...
            if (!child->isMetricSet()) {
                throw vespalib::IllegalStateException("Metric " + addendPath[i] + " in metric set " +
                                                          ...
                                                          "metric set. This sounds like a bug.",
                                                      VESPA_STRLOC);
            }
        ...
        if (child == 0) {
            throw vespalib::IllegalStateException("Metric " + addendPath[addendPath.size() - 1] +
                                                      " in "
                                                      "metric set " +
                                                      newAddendParent->getPath() +
                                                      " was "
                                                      "expected to exist. This sounds like a bug.",
                                                  VESPA_STRLOC);
        }
        AddendMetric* am(dynamic_cast<AddendMetric*>(child));
        if (am == 0) {
            throw vespalib::IllegalStateException("Metric " + child->getPath() + " is of wrong type for sum " +
...
template <typename AddendMetric> SumMetric<AddendMetric>::~SumMetric() {}

template <typename AddendMetric>
Metric* SumMetric<AddendMetric>::clone(std::vector<Metric::UP>& ownerList, CopyType copyType, MetricSet* owner,
                                       bool includeUnused) const {
    (void)includeUnused;
    if (_metricsToSum.empty() && _startValue.get() == 0) {
        abort();
        throw vespalib::IllegalStateException("Attempted to clone sum metric without any children or start value. "
    ...
    typename std::vector<const AddendMetric*>::const_iterator it(_metricsToSum.begin());
...
template <typename AddendMetric> void SumMetric<AddendMetric>::addToPart(Metric& m) const {
    if (!m.is_sum_metric()) {
        std::pair<std::vector<Metric::UP>, Metric::UP> sum(generateSum());
...
template <typename AddendMetric> bool SumMetric<AddendMetric>::is_sum_metric() const { return true; }

template <typename AddendMetric>
void SumMetric<AddendMetric>::addToSnapshot(Metric& m, std::vector<Metric::UP>& ownerList) const {
    if (isAddendType(&m)) {
...
template <typename AddendMetric>
void SumMetric<AddendMetric>::addTo(Metric& m, std::vector<Metric::UP>* ownerList) const {
    if (ownerList == 0) {
        std::pair<std::vector<Metric::UP>, Metric::UP> sum(generateSum());
        sum.second->addToPart(m);
    } else {
...
template <typename AddendMetric> void SumMetric<AddendMetric>::addMetricToSum(const AddendMetric& metric) {
    if (_owner == 0) {
        throw vespalib::IllegalStateException("Sum metric needs to be registered in a parent metric set "
    ...
    if (addedPath.size() <= sumParentPath.size()) {
        ...
        for (uint32_t i = 0; i < sumParentPath.size(); ++i) {
    ...
    if (error) {
        throw vespalib::IllegalStateException("Metric added to sum is required to be a child of the sum's "
                                              "direct parent metric set. (Need not be a direct child) "
                                              "Metric set " +
                                                  metric.getPath() + " is not a child of " + _owner->getPath(),
                                              VESPA_STRLOC);
    }
...
template <typename AddendMetric> int64_t SumMetric<AddendMetric>::getLongValue(string_view id) const {
    std::pair<std::vector<Metric::UP>, Metric::UP> sum(generateSum());
    if (sum.second.get() == 0)
        return 0;
...
template <typename AddendMetric> double SumMetric<AddendMetric>::getDoubleValue(string_view id) const {
    std::pair<std::vector<Metric::UP>, Metric::UP> sum(generateSum());
    if (sum.second.get() == 0)
        return 0.0;
...
template <typename AddendMetric>
void SumMetric<AddendMetric>::print(std::ostream& out, bool verbose, const std::string& indent,
...
template <typename AddendMetric> bool SumMetric<AddendMetric>::used() const {
    for (const AddendMetric* m : _metricsToSum) {
        if (m->used())
            return true;
    }
...
template <typename AddendMetric> void SumMetric<AddendMetric>::addMemoryUsage(MemoryConsumption& mc) const {
    ++mc._sumMetricCount;
    mc._sumMetricMeta += sizeof(SumMetric<AddendMetric>) - sizeof(Metric) + _metricsToSum.capacity() * sizeof(Metric*);
    Metric::addMemoryUsage(mc);
}

template <typename AddendMetric>
void SumMetric<AddendMetric>::printDebug(std::ostream& out, const std::string& indent) const {
    out << "sum ";
...
template <typename AddendMetric> bool SumMetric<AddendMetric>::isAddendType(const Metric* m) const {
    ...
    if (_metricsToSum.empty() && _startValue.get() == 0) {
        throw vespalib::IllegalStateException("Attempted to verify addend type for sum metric without any "
                                              "children or start value.",
                                              VESPA_STRLOC);
    }
