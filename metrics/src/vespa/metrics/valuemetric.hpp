...
ValueMetric<AvgVal, TotVal, SumOnAdd>::ValueMetric(const String& name, const Tags dimensions,
                                                   const String& description, MetricSet* owner)
    : AbstractValueMetric(name, std::move(dimensions), description, owner), _values() {
}

template <typename AvgVal, typename TotVal, bool SumOnAdd>
ValueMetric<AvgVal, TotVal, SumOnAdd>::ValueMetric(const ValueMetric<AvgVal, TotVal, SumOnAdd>& other,
                                                   MetricSet*                                   owner)
    : AbstractValueMetric(other, owner), _values(other._values) {
}
...
template <typename AvgVal, typename TotVal, bool SumOnAdd>
void ValueMetric<AvgVal, TotVal, SumOnAdd>::addToPart(Metric& other) const {
    auto& o = reinterpret_cast<ValueMetric<AvgVal, TotVal, SumOnAdd>&>(other);
...
void ValueMetric<AvgVal, TotVal, SumOnAdd>::add(const Values& values2, bool sumOnAdd) {
    ...
    do {
        ...
        overflow = values._count > values._count + values2._count ||
                   (values2._total >= 0 ? values._total > values._total + values2._total
        ...
        if (values._count == 0) {
            ...
            double totalAverage = static_cast<double>(values._total) / values._count +
...
void ValueMetric<AvgVal, TotVal, SumOnAdd>::dec(const Values& values2) {
    ...
    do {
        values = _values.getValues();
        underflow = values._count < values._count - values2._count || values._total < values._total - values2._total;
        values._count -= values2._count;
        values._total -= values2._total;
    } while (!_values.setValues(values));
    if (underflow) {
...
template <typename AvgVal, typename TotVal, bool SumOnAdd>
void ValueMetric<AvgVal, TotVal, SumOnAdd>::addValueWithCount(AvgVal avg, TotVal tot, uint32_t count, AvgVal min,
                                                              AvgVal max) {
...
ValueMetric<AvgVal, TotVal, SumOnAdd>::operator+=(const ValueMetric<AvgVal, TotVal, SumOnAdd>& other) {
...
template <typename AvgVal, typename TotVal, bool SumOnAdd>
void ValueMetric<AvgVal, TotVal, SumOnAdd>::print(std::ostream& out, bool verbose, const std::string& indent,
...
template <typename AvgVal, typename TotVal, bool SumOnAdd>
void ValueMetric<AvgVal, TotVal, SumOnAdd>::addMemoryUsage(MemoryConsumption& mc) const {
...
template <typename AvgVal, typename TotVal, bool SumOnAdd>
void ValueMetric<AvgVal, TotVal, SumOnAdd>::printDebug(std::ostream& out, const std::string& indent) const {
    Values values(_values.getValues());
