...
protected:
    AbstractCountMetric(const String& name, Tags dimensions, const String& description, MetricSet* owner)
        : Metric(name, std::move(dimensions), description, owner) {}

    AbstractCountMetric(const AbstractCountMetric& other, MetricSet* owner) : Metric(other, owner) {}

    void logWarning(const char* msg, const char* op) const;
};

template <typename T, bool SumOnAdd> class CountMetric : public AbstractCountMetric {
...
public:
    ...
    bool inUse(const MetricValueClass& v) const override { return static_cast<const Values&>(v).inUse(); }
