...
protected:
    AbstractValueMetric(const String &name, Tags dimensions, const String &description, MetricSet *owner)
        : Metric(name, std::move(dimensions), description, owner) {}

    AbstractValueMetric(const AbstractValueMetric &other, MetricSet *owner) : Metric(other, owner) {}

    void logWarning(const char *msg, const char *op) const;
    void logNonFiniteValueWarning() const;
};

template <typename AvgVal, typename TotVal, bool SumOnAdd> class ValueMetric : public AbstractValueMetric {
    ...
    enum Flag { SUMMED_AVERAGE = 2, UNSET_ON_ZERO_VALUE = 4 };
    ...
    void addValueWithCount(AvgVal avg, TotVal tot, uint32_t count) { addValueWithCount(avg, tot, count, avg, avg); }
...
public:
    ...
    MetricValueClass::UP getValues() const override { return std::make_unique<Values>(_values.getValues()); }
    ...
    bool inUse(const MetricValueClass &v) const override {
        ...
        return (values._total != 0 || (values._count != 0 && !unsetOnZeroValue()));
    }
