...
struct MetricVisitor {
     ...
     */
    virtual bool visitCountMetric(const AbstractCountMetric& m, bool autoGenerated);
    /**
     * Visit a primitive metric within an accepted metric set.
     *
     * @return True if you want to continue visiting, false to abort.
     */
    virtual bool visitValueMetric(const AbstractValueMetric& m, bool autoGenerated);
...
public:
     ...
     */
    virtual Metric* clone(std::vector<Metric::UP>& ownerList, CopyType type, MetricSet* owner,
                          bool includeUnused) const = 0;
    ...
    virtual void print(std::ostream&, bool verbose, const std::string& indent, uint64_t secondsPassed) const = 0;
    ...
    void setDescription(const std::string& d) { _description = NameRepo::descriptionId(d); }
