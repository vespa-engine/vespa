...
MetricSnapshot::MetricSnapshot(const Metric::String& name)
    : _name(name), _period(0), _fromTime(system_time_epoch), _toTime(system_time_epoch),
      _snapshot(new MetricSet("top", {}, "", nullptr)), _metrics() {}

MetricSnapshot::MetricSnapshot(const Metric::String& name, system_time::duration period, const MetricSet& source,
                               bool copyUnset)
    : _name(name), _period(period), _fromTime(system_time_epoch), _toTime(system_time_epoch), _snapshot(), _metrics() {
...
void MetricSnapshot::reset() { reset(system_time_epoch); }
void MetricSnapshot::reset(system_time currentTime) {
...
void MetricSnapshot::addMemoryUsage(MemoryConsumption& mc) const {
    ++mc._snapshotCount;
...
bool MetricSnapshotSet::haveCompletedNewPeriod(system_time newFromTime) {
    if (_count == 1) {
...
bool MetricSnapshotSet::timeForAnotherSnapshot(system_time currentTime) {
    ...
    if (currentTime >= lastTime + period) {
        if (currentTime >= lastTime + 2 * period) {
            LOG(warning,
                "Metric snapshot set %s was asked if it was time for another snapshot, a whole period beyond "
