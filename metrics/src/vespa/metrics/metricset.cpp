...
MetricSet::MetricSet(const String& name, Tags dimensions, const String& description, MetricSet* owner)
    : Metric(name, std::move(dimensions), description, owner), _metricOrder(), _registrationAltered(false) {
}

MetricSet::MetricSet(const MetricSet& other, std::vector<Metric::UP>& ownerList, CopyType copyType, MetricSet* owner,
                     bool includeUnused)
    : Metric(other, owner), _metricOrder(), _registrationAltered(false) {
...
MetricSet* MetricSet::clone(std::vector<Metric::UP>& ownerList, CopyType type, MetricSet* owner,
                            bool includeUnused) const {
    return new MetricSet(*this, ownerList, type, owner, includeUnused);
}

const Metric* MetricSet::getMetricInternal(string_view name) const {
...
const Metric* MetricSet::getMetric(string_view name) const {
    ...
    if (pos == std::string::npos) {
        ...
        if (m == nullptr)
            return nullptr;
        if (!m->isMetricSet()) {
            throw vespalib::IllegalStateException("Metric " + std::string(child) +
                                                      " is not a metric set. Cannot retrieve "
                                                      "metric at path " +
                                                      std::string(name) + " within metric " + getPath(),
                                                  VESPA_STRLOC);
        }
...
void MetricSet::clearRegistrationAltered() {
...
void MetricSet::tagRegistrationAltered() {
...
void MetricSet::registerMetric(Metric& metric) {
    if (metric.isRegistered()) {
        throw vespalib::IllegalStateException("Metric " + metric.getMangledName() +
                                                  " is already registered in a metric set. "
                                                  "Cannot register it twice.",
                                              VESPA_STRLOC);
    }
    const Metric* existing(getMetricInternal(metric.getMangledName()));
    if (existing != nullptr) {
        throw vespalib::IllegalStateException("A metric named " + metric.getMangledName() +
    ...
    LOG(spam, "Registered metric%s %s in metric set %s.", metric.isMetricSet() ? "set" : "",
        metric.getMangledName().c_str(), getPath().c_str());
}

void MetricSet::unregisterMetric(Metric& metric) {
    // In case of abrubt shutdowns, don't die hard on attempts to unregister
    // non-registered metrics. Just warn and ignore.
    const Metric* existing(getMetricInternal(metric.getMangledName()));
    if (existing == nullptr) {
        LOG(warning,
            "Attempt to unregister metric %s in metric set %s, where "
            "it wasn't registered to begin with.",
            metric.getMangledName().c_str(), getPath().c_str());
        return;
    }
    bool found = false;
    for (std::vector<Metric*>::iterator it = _metricOrder.begin(); it != _metricOrder.end(); ++it) {
        if (*it == &metric) {
    ...
    LOG(spam, "Unregistered metric%s %s from metric set %s.", metric.isMetricSet() ? "set" : "",
        metric.getMangledName().c_str(), getPath().c_str());
}
...
bool operator<(const TmpString& a, const StringMetric& b) noexcept {
    return a < b.first;
}

using SortedVector = std::vector<StringMetric>;

void createMetricMap(SortedVector& metricMap, const std::vector<Metric*>& orderedList) {
    ...
    if (!newMetrics.empty()) {
        ...
        for (Metric* metric : o._metricOrder) {
            ...
            if (!std::binary_search(map1.begin(), map1.end(), v)) {
                LOG(debug,
                    "Metric %s exist in one snapshot but not other."
...
bool MetricSet::visit(MetricVisitor& visitor, bool tagAsAutoGenerated) const {
...
void MetricSet::print(std::ostream& out, bool verbose, const std::string& indent, uint64_t secondsPassed) const {
    out << getName() << ":";
    for (const Metric* metric : _metricOrder) {
        out << "\n" << indent << "  ";
...
void MetricSet::addMemoryUsage(MemoryConsumption& mc) const {
...
void MetricSet::printDebug(std::ostream& out, const std::string& indent) const {
    out << "set ";
    Metric::printDebug(out, indent);
    if (_registrationAltered)
        out << ", regAltered";
    out << " {";
    for (const Metric* metric : _metricOrder) {
        out << "\n" << indent << "  ";
