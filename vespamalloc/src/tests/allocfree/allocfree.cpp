...
int main(int argc, char** argv) {
    ...
    for (int i(0); i < numSameThreadAlloc; i++) {
        mallocFreeWorkers[i] =
            std::shared_ptr<MallocFreeWorker>(new MallocFreeWorker(200, 16, (i % 2) ? true : false));
    }

    std::atomic<bool> stop_flag(false);
    for (std::map<int, std::shared_ptr<FreeWorker>>::iterator it(freeWorkers.begin()), mt(freeWorkers.end());
         it != mt; it++)
    {
