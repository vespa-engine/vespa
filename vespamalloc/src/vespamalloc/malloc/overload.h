...
public:
    ...
    CreateAllocator() : _initialized(CONSTRUCTED) { vespamalloc::createAllocator(); }
    ~CreateAllocator() {
...
void operator delete(void* ptr, std::size_t sz) noexcept {
    if (ptr) {
        vespamalloc::_GmemP->free(ptr, sz);
    }
}
void operator delete[](void* ptr, std::size_t sz) noexcept {
    if (ptr) {
...
void operator delete(void* ptr, std::size_t sz, std::align_val_t alignment) noexcept {
    if (ptr) {
        vespamalloc::_GmemP->free(ptr, sz, alignment);
    }
}
void operator delete[](void* ptr, std::size_t sz, std::align_val_t alignment) noexcept {
    if (ptr) {
