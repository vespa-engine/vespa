...
template <int MaxCount> FreeListT<MaxCount>::~FreeListT() = default;

template <int MaxCount> void FreeListT<MaxCount>::add(Index startIndex) {
    Index i(0);
...
template <int MaxCount> uint32_t FreeListT<MaxCount>::lastBlock(Index nextBlock) {
    Index lastIndex(0);
    if (_count > 0) {
...
template <int MaxCount> uint32_t FreeListT<MaxCount>::numFreeBlocks() const {
