...
size_t MMapPool::getMmappedBytes() const {
    std::lock_guard guard(_mutex);
    return _currentBytes;
}

size_t MMapPool::getMmappedBytesPeak() const {
    std::lock_guard guard(_mutex);
    return _peakBytes;
}

void* MMapPool::mmap(size_t sz) {
    void* buf(nullptr);
