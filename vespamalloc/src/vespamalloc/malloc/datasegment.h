...
public:
    ...
    template <typename MemBlockPtrT> size_t getMaxSize(const void* ptr) const {
        return _blockList[blockId(ptr)].getMaxSize<MemBlockPtrT>();
    }
    ...
    size_t            infoThread(FILE* os, int level, uint32_t thread, SizeClassT sct, uint32_t maxThreadId = 0) const
