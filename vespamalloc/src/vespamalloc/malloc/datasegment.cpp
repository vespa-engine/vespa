...
std::vector<uint32_t> createHistogram(bool allThreads, uint32_t maxThreads) {
    if (allThreads) {
...
size_t DataSegment::infoThread(FILE* os, int level, uint32_t thread, SizeClassT sct, uint32_t maxThreadId) const {
    ...
    fprintf(os,
            ...
            "By%sAlloc=%ld(%2.2f%%) NotAccountedDue2FullGraph=%ld InvalidCallStacks=%ld:\n",
            sct, checkedCount, allocatedCount, allocatedCount * 100 / checkedCount, allThreads ? "Us" : "Me",
...
void DataSegment::info(FILE* os, size_t level) {
    ...
    if (level >= 1) {
...
#ifdef PRINT_ALOT
            if ((b.sizeClass() != oldSc) || ((oldChainLength < (b.freeChainLength() + 1)) && b.freeChainLength())) {
        ...
        fprintf(os, "Usage: Total=%ld(100%%), admin=%ld(%ld%%), unused=%ld(%ld%%), allocated=%ld(%ld%%)\n",
                total * BlockSize, numAdminBlocks * BlockSize, numAdminBlocks * 100 / total,
                numFreeBlocks * BlockSize, numFreeBlocks * 100 / total,
                (numAllocatedBlocks - numAdminBlocks) * BlockSize,
