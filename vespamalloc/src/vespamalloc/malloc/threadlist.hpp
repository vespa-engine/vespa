...
ThreadListT<MemBlockPtrT, ThreadStatT>::ThreadListT(AllocPool& allocPool, MMapPool& mmapPool)
    : _isThreaded(false), _threadCount(0), _threadCountAccum(0), _allocPool(allocPool), _mmapPool(mmapPool) {
    const char* mmapThresholdS = getenv(VESPA_MALLOC_MMAP_THRESHOLD);
    int         mmapThreshold = (mmapThresholdS != nullptr) ? strtol(mmapThresholdS, nullptr, 0) : MMAP_LIMIT_DEFAULT;
...
template <typename MemBlockPtrT, typename ThreadStatT> bool ThreadListT<MemBlockPtrT, ThreadStatT>::quitThisThread() {
...
template <typename MemBlockPtrT, typename ThreadStatT> bool ThreadListT<MemBlockPtrT, ThreadStatT>::initThisThread() {
    bool retval(true);
