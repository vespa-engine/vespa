...
bool ThreadPoolT<MemBlockPtrT, ThreadStatT>::hasActuallyBeenUsed() const {
    ...
    for (size_t i = 0; !used && (i < NELEMS(_memList)); i++) {
        used =
            (_memList[i]._allocFrom != nullptr && !_memList[i]._allocFrom->empty() && !_memList[i]._freeTo->full());
    }
    return used;
}

template <typename MemBlockPtrT, typename ThreadStatT> void ThreadPoolT<MemBlockPtrT, ThreadStatT>::init(int thrId) {
    setThreadId(thrId);
...
template <typename MemBlockPtrT, typename ThreadStatT> bool ThreadPoolT<MemBlockPtrT, ThreadStatT>::grabAvailable() {
