...
public:
    ...
    void setupLog(size_t prAllocLimit) { _prAllocLimit = prAllocLimit; }
...
template <typename MemBlockPtrT, typename ThreadListT> bool MemoryManager<MemBlockPtrT, ThreadListT>::quitThisThread() {
...
template <typename MemBlockPtrT, typename ThreadListT> void MemoryManager<MemBlockPtrT, ThreadListT>::crash() {
