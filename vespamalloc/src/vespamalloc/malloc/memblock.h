...
template <size_t MinSizeClassC, size_t MaxSizeClassMultiAllocC> class MemBlockT : public CommonT<MinSizeClassC> {
public:
    ...
    template <typename T> void          readjustAlignment(const T &segment) { (void)segment; }
    ...
    template <typename T> static size_t usable_size(void *ptr, const T &segment) { return segment.template getMaxSize<MemBlockT>(ptr); }
    ...
    static bool   verifySizeClass(int sc) {
        (void)sc;
        return true;
    }
    static size_t getMinSizeForAlignment(size_t align, size_t sz) {
        return (sz < Parent::MAX_ALIGN) ? std::max(sz, align) : (align < Parent::MAX_ALIGN) ? sz : sz + align;
    }
