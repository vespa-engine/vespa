...
GetResult::GetResult(Document::UP doc, Timestamp timestamp)
    : Result(), _timestamp(timestamp), _doc(std::move(doc)), _is_tombstone(false) {
}

GetResult::GetResult(Timestamp removed_at_ts, bool is_tombstone)
    : _timestamp(removed_at_ts), _doc(), _is_tombstone(is_tombstone) {
}
...
IterateResult::IterateResult(ErrorType error, const std::string& errorMessage)
    : Result(error, errorMessage), _completed(false) {
}

IterateResult::IterateResult(List entries, bool completed) : _completed(completed), _entries(std::move(entries)) {
}

IterateResult::List IterateResult::steal_entries() {
