...
public:
    ResourceUsage(double disk_usage, double memory_usage, const AttributeResourceUsage& attribute_address_space_usage)
    ...
    const AttributeResourceUsage& get_attribute_address_space_usage() const noexcept {
        return _attribute_address_space_usage;
    }

    bool operator==(const ResourceUsage& rhs) const noexcept {
        return ((_disk_usage == rhs._disk_usage) && (_memory_usage == rhs._memory_usage) &&
                (_attribute_address_space_usage == rhs._attribute_address_space_usage));
    }
    bool operator!=(const ResourceUsage& rhs) const noexcept { return !operator==(rhs); }
};
