...
ClusterState::ClusterState(const lib::ClusterState& state, uint16_t nodeIndex, const lib::Distribution& distribution,
...
Trinary ClusterState::shouldBeReady(const Bucket& b) const {
    ...
    _distribution->getIdealNodes(lib::NodeType::STORAGE, *_state, b.getBucketId(), idealNodes, "uim",
                                 _distribution->getReadyCopies());
...
bool ClusterState::nodeHasStateOneOf(const char* states) const noexcept {
    return _state && _state->getNodeState(lib::Node(lib::NodeType::STORAGE, _nodeIndex)).getState().oneOf(states);
}
