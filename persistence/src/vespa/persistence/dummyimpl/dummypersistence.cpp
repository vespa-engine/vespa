...
std::string asString(const std::vector<BucketEntry>& v) {
...
BucketContent::BucketContent() noexcept
    : _entries(), _gidMap(), _info(), _inUse(false), _outdatedInfo(true), _active(false) {
}

BucketContent::~BucketContent() = default;

uint32_t BucketContent::computeEntryChecksum(const BucketEntry& e) const {
...
BucketChecksum BucketContent::updateRollingChecksum(uint32_t entryChecksum) {
...
const BucketInfo& BucketContent::getBucketInfo() const {
    if (!_outdatedInfo) {
        return _info;
    }
    ...
    _info = BucketInfo(BucketChecksum(checksum), unique, uniqueSize, _entries.size(), totalSize, BucketInfo::READY,
...
struct TimestampLess {
    bool operator()(const BucketEntry& bucketEntry, Timestamp t) { return bucketEntry.entry->getTimestamp() < t; }

    bool operator()(Timestamp t, const BucketEntry& bucketEntry) { return t < bucketEntry.entry->getTimestamp(); }
};

} // namespace

bool BucketContent::hasTimestamp(Timestamp t) const {
...
void BucketContent::insert(DocEntry::SP e) {
    ...
    if (!_entries.empty() && _entries.back().entry->getTimestamp() < e->getTimestamp()) {
    ...
    if (gidIt != _gidMap.end()) {
        ...
        if (!_outdatedInfo) {
            if (!e->isRemove()) {
                _info = BucketInfo(updateRollingChecksum(computeEntryChecksum(BucketEntry(e, gid))),
                                   _info.getDocumentCount() + 1, _info.getDocumentSize() + e->getSize(),
                                   _info.getEntryCount() + 1, _info.getUsedSize() + e->getSize(), _info.getReady(),
                                   _info.getActive());
            } else {
                _info = BucketInfo(_info.getChecksum(), _info.getDocumentCount(), _info.getDocumentSize(),
                                   _info.getEntryCount() + 1, _info.getUsedSize() + e->getSize(), _info.getReady(),
                                   _info.getActive());
            }

            LOG(spam, "After cheap bucketinfo update, state is %s (inserted %s)", _info.toString().c_str(),
                e->toString().c_str());
        }
    }
...
std::shared_ptr<DocEntry> BucketContent::getEntry(const document::GlobalId& gid) const {
...
DocEntry::SP BucketContent::getEntry(Timestamp t) const {
...
void BucketContent::eraseEntry(Timestamp t) {
    auto iter = lower_bound(_entries.begin(), _entries.end(), t, TimestampLess());
...
void BucketContent::eraseEntries(const GlobalId& gid) {
...
DummyPersistence::DummyPersistence(const std::shared_ptr<const document::DocumentTypeRepo>& repo)
    : _initialized(false), _repo(repo), _content(), _nextIterator(1), _iterators(), _monitor(), _clusterState() {
}

DummyPersistence::~DummyPersistence() = default;

document::select::Node::UP DummyPersistence::parseDocumentSelection(const string& documentSelection, bool allowLeaf) {
...
void DummyPersistence::verifyInitialized() const noexcept {
    if (!_initialized) {
...
BucketIdListResult DummyPersistence::listBuckets(BucketSpace bucketSpace) const {
    verifyInitialized();
...
void DummyPersistence::setModifiedBuckets(BucketIdListResult::List buckets) {
    std::lock_guard lock(_monitor);
...
BucketIdListResult DummyPersistence::getModifiedBuckets(BucketSpace bucketSpace) const {
    std::lock_guard lock(_monitor);
    if (bucketSpace == FixedBucketSpaces::default_space()) {
...
Result DummyPersistence::setClusterState(BucketSpace bucketSpace, const ClusterState& c) {
    std::lock_guard lock(_monitor);
    if (bucketSpace == FixedBucketSpaces::default_space()) {
...
void DummyPersistence::setActiveStateAsync(const Bucket& b, BucketInfo::ActiveState newState,
    ...
    if (!bc) {
        onComplete->onComplete(
            std::make_unique<BucketInfoResult>(Result::ErrorType::TRANSIENT_ERROR, "Bucket not found"));
    } else {
...
BucketInfoResult DummyPersistence::getBucketInfo(const Bucket& b) const {
    ...
    LOG(debug, "getBucketInfo(%s) -> %s", b.toString().c_str(), info.toString().c_str());
    return BucketInfoResult(info);
}
...
void DummyPersistence::putAsync(const Bucket& b, Timestamp t, Document::SP doc, OperationComplete::UP onComplete) {
    ...
    if (existing) {
        ...
        if (doc->getId() == *existing->getDocumentId()) {
            ...
            onComplete->onComplete(
                std::make_unique<Result>(Result::ErrorType::TIMESTAMP_EXISTS, "Timestamp already existed"));
        }
    } else {
...
void DummyPersistence::updateAsync(const Bucket& bucket, Timestamp ts, DocumentUpdateSP upd,
                                   OperationComplete::UP onComplete) {
...
void DummyPersistence::removeAsync(const Bucket& b, std::vector<spi::IdAndTimestamp> ids,
    ...
    for (const spi::IdAndTimestamp& stampedId : ids) {
        ...
        if (!entry || entry->getTimestamp() <= t) {
            ...
            LOG(debug,
                "Not adding tombstone for %s at %" PRIu64 " since it has already "
...
void DummyPersistence::removeByGidAsync(const Bucket& b, std::vector<spi::DocTypeGidAndTimestamp> ids,
    ...
    for (const auto& dt_gid_ts : ids) {
        ...
        LOG(debug, "removeByGidAsync(%s, %" PRIu64 ", %s, %s)", b.toString().c_str(), uint64_t(t),
            dt_gid_ts.doc_type.c_str(), gid.toString().c_str());
...
CreateIteratorResult DummyPersistence::createIterator(const Bucket& b, FieldSetSP fs, const Selection& s,
    ...
    if (!s.getDocumentSelection().getDocumentSelection().empty()) {
        ...
        if (!docSelection.get()) {
            return CreateIteratorResult(Result::ErrorType::PERMANENT_ERROR,
    ...
    if (s.getTimestampSubset().empty()) {
        ...
        for (reverse_iterator entryIter((*bc)->_entries.rbegin()), entryEnd((*bc)->_entries.rend());
             entryIter != entryEnd; ++entryIter)
        {
            const BucketEntry& bucketEntry(*entryIter);
            const DocEntry&    entry(*bucketEntry.entry);
            if (entry.getTimestamp() < s.getFromTimestamp() || entry.getTimestamp() > s.getToTimestamp()) {
                continue;
            }
            ...
            if (entry.isRemove()) {
                ...
                if (docSelection.get() &&
                    (docSelection->contains(*entry.getDocumentId()) != document::select::Result::True))
                {
                    continue;
                }
                ...
                if (docSelection.get() &&
                    (docSelection->contains(*entry.getDocument()) != document::select::Result::True))
                {
                    continue;
                }
                it->_leftToIterate.push_back(entry.getTimestamp());
...
IterateResult DummyPersistence::iterate(IteratorId id, uint64_t maxByteSize) const {
    verifyInitialized();
    ...
    LOG(debug,
        "finished iterate(%" PRIu64 ", %" PRIu64 "), returning %zu documents "
        "with %u bytes of data. %u docs cloned in fast path",
        uint64_t(id), maxByteSize, entries.size(), currentSize, fastPath);
...
Result DummyPersistence::destroyIterator(IteratorId id) {
    verifyInitialized();
...
Result DummyPersistence::split(const Bucket& source, const Bucket& target1, const Bucket& target2) {
    verifyInitialized();
    ...
    for (uint32_t i = 0; i < sour._entries.size(); ++i) {
        ...
        document::BucketId bId(target1.getBucketId().getUsedBits(),
    ...
    sourceGuard.reset();
    LOG(debug, "erasing split source %s", source.toString().c_str());
    deleteBucket(source);

    return Result();
}

Result DummyPersistence::join(const Bucket& source1, const Bucket& source2, const Bucket& target) {
    verifyInitialized();
...
BucketContentGuard::UP DummyPersistence::acquireBucketWithLock(const Bucket& b, LockMode lock_mode) const {
    ...
    if (lock_mode == LockMode::Exclusive) {
        ...
        if (!bucketNotInUse) {
            LOG(error, "Attempted to acquire %s, but it was already marked as being in use!", b.toString().c_str());
...
void DummyPersistence::releaseBucketNoLock(const BucketContent& bc, LockMode lock_mode) const noexcept {
    if (lock_mode == LockMode::Exclusive) {
        bool my_true(true);
...
void DummyPersistence::internal_create_bucket(const Bucket& b) {
    std::lock_guard lock(_monitor);
    if (find(b) == _content.end()) {
        _content[b] = std::make_shared<BucketContent>();
    }
...
DummyPersistence::Content::const_iterator DummyPersistence::find(const Bucket& bucket) const {
    return _content.find(bucket);
}

} // namespace storage::spi::dummy

VESPALIB_HASH_MAP_INSTANTIATE_H(storage::spi::Bucket, std::shared_ptr<storage::spi::dummy::BucketContent>,
