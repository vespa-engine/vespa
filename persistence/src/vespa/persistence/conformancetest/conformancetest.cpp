...
template <typename T> std::shared_ptr<T> as_sp(std::unique_ptr<T> value) {
    return value;
}

std::unique_ptr<PersistenceProvider> getSpi(ConformanceTest::PersistenceFactory& factory,
                                            const document::TestDocMan&          testDocMan) {
    PersistenceProviderUP result(
        factory.getPersistenceImplementation(testDocMan.getTypeRepoSP(), *testDocMan.getTypeConfig()));
    EXPECT_TRUE(!result->initialize().hasError());
    return result;
}

enum class SelectionFields { METADATA_ONLY = 0, ALL_FIELDS = 1 };

CreateIteratorResult createIterator(PersistenceProvider& spi, const Bucket& b, const Selection& sel,
...
Selection createSelection(const string& docSel) {
    return Selection(DocumentSelection(docSel));
}

ClusterState createClusterState(const lib::State& nodeState = lib::State::UP) {
...
struct DocAndTimestamp {
    ...
    DocAndTimestamp(const Document::SP& docptr, spi::Timestamp ts) : doc(docptr), timestamp(ts) {}
};
 ...
 */
std::vector<Chunk> doIterate(PersistenceProvider& spi, IteratorId id, uint64_t maxByteSize, size_t maxChunks = 0,
    ...
    while (true) {
        ...
        if (result.isCompleted() || (maxChunks != 0 && chunks.size() >= maxChunks)) {
            break;
        }
    }
    return chunks;
...
size_t getRemoveEntryCount(const DocEntryList& entries) {
    size_t ret = 0;
...
DocEntryList getEntriesFromChunks(const std::vector<Chunk>& chunks) {
    DocEntryList ret;
    ...
    std::sort(ret.begin(), ret.end(), DocEntryIndirectTimestampComparator());
    return ret;
}

DocEntryList iterateBucket(PersistenceProvider& spi, const Bucket& bucket, IncludedVersions versions,
...
void verifyDocs(const std::vector<DocAndTimestamp>& wanted, const std::vector<Chunk>& chunks,
    ...
    for (size_t i = 0; i < retrieved.size(); ++i) {
        ...
        if (entry.getDocument() != nullptr) {
            ...
            if (removes.find(entry.getDocumentId()->toString()) == removes.end()) {
                FAIL() << "Got unexpected remove entry for document id " << *entry.getDocumentId();
            }
        }
    }
}
...
std::vector<DocAndTimestamp> feedDocs(PersistenceProvider& spi, document::TestDocMan& testDocMan, Bucket& bucket,
                                      size_t numDocs, uint32_t minSize = 110, uint32_t maxSize = 110) {
    std::vector<DocAndTimestamp> docs;
    for (uint32_t i = 0; i < numDocs; ++i) {
        Document::SP doc(testDocMan.createRandomDocumentAtLocation(bucket.getBucketId().getId() & 0xffffffff, i,
                                                                   minSize, maxSize));
...
// Set by test runner.
std::unique_ptr<ConformanceTest::PersistenceFactory> (*ConformanceTest::_factoryFactory)(const std::string& docType) =
    nullptr;

ConformanceTest::ConformanceTest() : ConformanceTest("") {
}

ConformanceTest::ConformanceTest(const std::string& docType) : _factory(_factoryFactory(docType)) {
}

SingleDocTypeConformanceTest::SingleDocTypeConformanceTest() : ConformanceTest("testdoctype1") {
}
...
TEST_F(ConformanceTest, testBasics) {
    ...
    for (int iterPass = 0; iterPass < 2; ++iterPass) {
        ...
        CreateIteratorResult iter =
            spi->createIterator(bucket, std::make_shared<document::AllFields>(), sel,
                                includeRemoves ? NEWEST_DOCUMENT_OR_REMOVE : NEWEST_DOCUMENT_ONLY, context);
...
TEST_F(ConformanceTest, testUpdate) {
    ...
    update->addUpdate(FieldUpdate(docType->getField("headerval"))
                          .addUpdate(std::make_unique<AssignValueUpdate>(std::make_unique<IntFieldValue>(42))));
...
void ConformanceTest::test_iterate_empty_or_missing_bucket(bool bucket_exists) {
    document::TestDocMan testDocMan;
    _factory->clear();
    PersistenceProviderUP spi(getSpi(*_factory, testDocMan));
    Bucket                b(makeSpiBucket(BucketId(8, 0x1)));
...
void ConformanceTest::testDeleteBucketPostCondition(const PersistenceProvider& spi, const Bucket& bucket,
                                                    const Document& doc1) {
    Context context(Priority(0), Trace::TraceLevel(0));
    {
        GetResult result = spi.get(bucket, document::AllFields(), doc1.getId(), context);
...
void ConformanceTest::testSplitNormalCasePostCondition(const PersistenceProvider& spi, const Bucket& bucketA,
...
void ConformanceTest::testSplitTargetExistsPostCondition(const PersistenceProvider& spi, const Bucket& bucketA,
                                                         const Bucket& bucketB, const Bucket& bucketC,
                                                         document::TestDocMan& testDocMan) {
    EXPECT_EQ(10, (int)spi.getBucketInfo(bucketA).getBucketInfo().getDocumentCount());
...
void ConformanceTest::testSplitSingleDocumentInSourcePostCondition(const PersistenceProvider& spi,
                                                                   const Bucket& source, const Bucket& target1,
...
void ConformanceTest::createAndPopulateJoinSourceBuckets(PersistenceProvider& spi, const Bucket& source1,
                                                         const Bucket& source2, document::TestDocMan& testDocMan) {
...
void ConformanceTest::doTestJoinNormalCase(const Bucket& source1, const Bucket& source2, const Bucket& target) {
    document::TestDocMan testDocMan;
    _factory->clear();
    PersistenceProviderUP spi(getSpi(*_factory, testDocMan));
...
void ConformanceTest::testJoinNormalCasePostCondition(const PersistenceProvider& spi, const Bucket& bucketA,
                                                      const Bucket& bucketB, const Bucket& bucketC,
                                                      document::TestDocMan& testDocMan) {
...
void ConformanceTest::testJoinTargetExistsPostCondition(const PersistenceProvider& spi, const Bucket& bucketA,
                                                        const Bucket& bucketB, const Bucket& bucketC,
                                                        document::TestDocMan& testDocMan) {
...
void ConformanceTest::testJoinOneBucketPostCondition(const PersistenceProvider& spi, const Bucket& bucketA,
...
void ConformanceTest::testJoinSameSourceBucketsPostCondition(const PersistenceProvider& spi, const Bucket& source,
...
void ConformanceTest::doTestJoinSameSourceBuckets(const Bucket& source, const Bucket& target) {
    document::TestDocMan testDocMan;
    _factory->clear();
    PersistenceProviderUP spi(getSpi(*_factory, testDocMan));
...
void ConformanceTest::testJoinSameSourceBucketsTargetExistsPostCondition(const PersistenceProvider& spi,
                                                                         const Bucket& source, const Bucket& target,
                                                                         document::TestDocMan& testDocMan) {
...
void ConformanceTest::test_empty_bucket_info(bool bucket_exists, bool active) {
    document::TestDocMan testDocMan;
    _factory->clear();
    PersistenceProviderUP spi(getSpi(*_factory, testDocMan));
    Bucket                bucket(makeSpiBucket(BucketId(8, 0x01)));
...
void ConformanceTest::assert_remove_by_gid(PersistenceProvider& spi, const Bucket& bucket,
                                           std::vector<DocTypeGidAndTimestamp> ids, size_t exp_removed,
                                           size_t exp_remaining, const std::string& label) {
    SCOPED_TRACE(label);
    auto onDone = std::make_unique<CatchResult>();
    auto future = onDone->future_result();
