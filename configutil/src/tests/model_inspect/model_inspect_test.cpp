...
public:
    ...
    Model() : uri(configUri("file", "model.cfg")), flags(), stream(), model(flags, uri, stream) {};

    Model(ModelInspect::Flags _flags)
        : uri(configUri("file", "model.cfg")), flags(_flags), stream(), model(flags, uri, stream) {};

    void contains(std::string needle) { ASSERT_TRUE(stream.str().find(needle) != std::string::npos); };

    void missing(std::string needle) { ASSERT_TRUE(stream.str().find(needle) == std::string::npos); };
...
public:
    MakeUriFlags() : ModelInspect::Flags() { makeuri = true; }
};
...
public:
    bool _yamlDump, _listHosts, _listServices, _listCluster, _listConfigIds, _listHost, _listClusters, _listAllPorts,
        _listService, _listService2, _listConfigId, _getIndexOf;
    ModelDummy(std::stringstream& stream)
        : ModelInspect(ModelInspect::Flags(), config::ConfigUri(Model::configUri("file", "model.cfg")), stream) {
        _yamlDump = _listHosts = _listServices = _listCluster = _listConfigIds = _listHost = _listClusters =
            _listAllPorts = _listService = _listService2 = _listConfigId = _getIndexOf = false;
    };
    ...
    int listAllPorts() override {
        _listAllPorts = true;
        return 0;
    };
    int listCluster(const std::string) override {
        _listCluster = true;
        return 0;
    };
    int listService(const std::string) override {
        _listService = true;
        return 0;
    };
    int listService(const std::string, const std::string) override {
        _listService2 = true;
        return 0;
    };
    int listConfigId(const std::string) override {
        _listConfigId = true;
        return 0;
    };
    int getIndexOf(const std::string, const std::string) override {
        _getIndexOf = true;
        return 0;
    };
