...
bool waitSlobrok(RPCMessageBus& mbus, const std::string& pattern) {
...
TEST(TraceTest, trace_test) {
    ...
    RPCMessageBus mb(ProtocolSet().add(std::make_shared<SimpleProtocol>()),
                     RPCNetworkParams(config::ConfigUri("file:slobrok.cfg")), config::ConfigUri("file:routing.cfg"));
    ...
    TraceNode e3 =
        TraceNode()
            ...
            .addChild(TraceNode().addChild("server/java/3/D (message)").addChild("server/java/3/D (reply)"))
            .setStrict(false);
    TraceNode e2 =
        TraceNode()
            .addChild(
                TraceNode().addChild("server/cpp/2/A (message)").addChild(e3).addChild("server/cpp/2/A (reply)"))
            .addChild(
                TraceNode().addChild("server/cpp/2/B (message)").addChild(e3).addChild("server/cpp/2/B (reply)"))
            .addChild(
                TraceNode().addChild("server/java/2/A (message)").addChild(e3).addChild("server/java/2/A (reply)"))
            .addChild(
                TraceNode().addChild("server/java/2/B (message)").addChild(e3).addChild("server/java/2/B (reply)"))
            .setStrict(false);
    TraceNode expect =
        TraceNode()
            .addChild(
                TraceNode().addChild("server/cpp/1/A (message)").addChild(e2).addChild("server/cpp/1/A (reply)"))
            .addChild(
                TraceNode().addChild("server/java/1/A (message)").addChild(e2).addChild("server/java/1/A (reply)"))
            .setStrict(false);
    expect.normalize();
