...
class Server : public IMessageHandler, public IReplyHandler {
private:
...
Server::Server(MessageBus& bus, const std::string& name)
    : _session(bus.createIntermediateSession("session", true, *this, *this)), _name(name) {
...
void Server::handleReply(Reply::UP reply) {
...
int App::main(int argc, char** argv) {
    ...
    RPCMessageBus mb(ProtocolSet().add(std::make_shared<SimpleProtocol>()),
                     RPCNetworkParams(config::ConfigUri("file:slobrok.cfg")).setIdentity(Identity(argv[1])),
