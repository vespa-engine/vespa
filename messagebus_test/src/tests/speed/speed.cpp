...
TEST(SpeedTest, speed_test) {
    ...
    {     // CPP SERVER
        { // Make routing config
            EXPECT_EQ(system(("cat " + routing_template + " | sed 's#session#cpp/session#' > routing.cfg").c_str()),
                      0);
        }
    ...
    {     // JAVA SERVER
        { // Make routing config
            EXPECT_EQ(system(("cat " + routing_template + " | sed 's#session#java/session#' > routing.cfg").c_str()),
                      0);
        }
