...
Process::Process(const config::ConfigUri& configUri)
    : _configUri(configUri), _configSubscriber(_configUri.getContext()) {
}

Process::~Process() = default;

void Process::setupConfig(vespalib::duration subscribeTimeout) {
    _document_cfg_handle =
        _configSubscriber.subscribe<DocumentTypesConfig>(_configUri.getConfigId(), subscribeTimeout);
    _bucket_spaces_cfg_handle =
        _configSubscriber.subscribe<BucketspacesConfig>(_configUri.getConfigId(), subscribeTimeout);
    _comm_mgr_cfg_handle =
        _configSubscriber.subscribe<CommunicationManagerConfig>(_configUri.getConfigId(), subscribeTimeout);
    _bouncer_cfg_handle = _configSubscriber.subscribe<StorBouncerConfig>(_configUri.getConfigId(), subscribeTimeout);
    _distribution_cfg_handle =
        _configSubscriber.subscribe<StorDistributionConfig>(_configUri.getConfigId(), subscribeTimeout);
...
bool Process::configUpdated() {
    _configSubscriber.nextGenerationNow();
    if (_document_cfg_handle->isChanged()) {
        LOG(info, "Document config detected changed");
        return true;
    }
    bool changed = (_bucket_spaces_cfg_handle->isChanged() || _comm_mgr_cfg_handle->isChanged() ||
                    _bouncer_cfg_handle->isChanged() || _distribution_cfg_handle->isChanged() ||
                    _server_cfg_handle->isChanged());
    return changed;
}

void Process::updateConfig() {
    if (_document_cfg_handle->isChanged()) {
        _repos.push_back(DocumentTypeRepoFactory::make(*_document_cfg_handle->getConfig()));
...
int64_t Process::getGeneration() const {
