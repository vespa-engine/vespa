...
protected:
     ...
     **/
    bool Connect(const char* url, bool usePost = false, const char* content = nullptr, int contentLen = 0);
...
public:
     ...
     **/
    uint64_t GetReuseCount() const { return _reuseCount; }
    ...
    public:
        ...
        FetchStatus(bool ok, uint32_t requestStatus, int32_t totalHitCount, int32_t resultSize)
            : _ok(ok), _requestStatus(requestStatus), _totalHitCount(totalHitCount), _resultSize(resultSize) {}
        /**
     ...
     **/
    FetchStatus Fetch(const char* url, std::ostream* file = nullptr, bool usePost = false,
