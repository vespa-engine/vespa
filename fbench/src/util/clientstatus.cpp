...
void ClientStatus::SetError(const char* errorMsg) {
    _error = true;
    _errorMsg = errorMsg;
}
...
double ClientStatus::GetMin() {
    return _minTime;
}

double ClientStatus::GetMax() {
    return _maxTime;
}

double ClientStatus::GetAverage() {
    return (_requestCnt == 0) ? 0 : _totalTime / ((double)_requestCnt);
}
    ...
    if (i1 >= 0) {
        ...
        if (_higherCnt < 2) {
            ...
            val1 = (((double)(t1 - (_requestCnt - _higherCnt))) / ((double)(_higherCnt - 1))) *
                       (_maxTime * _timetableResolution - ((double)_timetable.size())) +
                   ((double)_timetable.size());
        }
    }
    if (i2 >= 0) {
        val2 = i2;
