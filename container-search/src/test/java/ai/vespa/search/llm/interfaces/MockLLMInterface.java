...
public class MockLLMInterface extends LLMInterface {
    ...
    public MockLLMInterface(LlmInterfaceConfig config,
    ...
    public List<Completion> complete(Prompt prompt, InferenceParameters options) {
        checkApiKey(options);
        setPrompt(prompt);
        return List.of(Completion.from(this.generator.apply(prompt, options)));
    }
    ...
    public CompletableFuture<Completion.FinishReason> completeAsync(Prompt prompt,
        ...
        var completions = this.generator.apply(prompt, options).split(" ");  // Simple tokenization
