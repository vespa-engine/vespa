...
public class QueryProperties extends Properties {
    ...
    private static Map<CompoundName, GetterSetter> createPropertySetterMap() {
        ...
        map.put(CompoundName.fromComponents(Trace.TRACE, Trace.PROFILING, Profiling.SORT_FEATURES, ProfilingParams.DEPTH), GetterSetter.of(query -> query.getTrace().getProfiling().getSortFeatures().getDepth(), (query, value) -> query.getTrace().getProfiling().getSortFeatures().setDepth(asInteger(value, 0))));
