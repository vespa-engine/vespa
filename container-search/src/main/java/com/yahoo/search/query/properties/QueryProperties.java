...
public class QueryProperties extends Properties {
    ...
    private static Map<CompoundName, GetterSetter> createPropertySetterMap() {
        ...
        addDualCasedRM(map, Matching.FILTER_FIRST_THRESHOLD, GetterSetter.of(query -> query.getRanking().getMatching().getFilterFirstThreshold(), (query, value) -> query.getRanking().getMatching().setFilterFirstThreshold(asDouble(value, 0.00))));
        ...
        addDualCasedRM(map, Matching.ADAPTIVE_BEAM_SEARCH_SLACK, GetterSetter.of(query -> query.getRanking().getMatching().getAdaptiveBeamSearchSlack(), (query, value) -> query.getRanking().getMatching().setAdaptiveBeamSearchSlack(asDouble(value, 0.0))));
