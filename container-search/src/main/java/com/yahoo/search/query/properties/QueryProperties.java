...
public class QueryProperties extends Properties {
    ...
    private static Map<CompoundName, GetterSetter> createPropertySetterMap() {
        ...
        addDualCasedRM(map, Matching.ACORN_ONE_THRESHOLD, GetterSetter.of(query -> query.getRanking().getMatching().getAcornOneThreshold(), (query, value) -> query.getRanking().getMatching().setAcornOneThreshold(asDouble(value, 0.00))));
        addDualCasedRM(map, Matching.ACORN_ONE_EXPLORATION, GetterSetter.of(query -> query.getRanking().getMatching().getAcornOneExploration(), (query, value) -> query.getRanking().getMatching().setAcornOneExploration(asDouble(value, 0.01))));
