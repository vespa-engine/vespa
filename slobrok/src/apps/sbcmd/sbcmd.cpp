...
int Slobrok_CMD::main(int argc, char** argv) {
    ...
    if (strcmp(argv[2], "slobrok.admin.listAllRpcServers") == 0) {
        ...
        // no params
               ...
               strcmp(argv[2], "slobrok.admin.listAllRpcServers") == 0 || strcmp(argv[2], "slobrok.system.stop") == 0 ||
               strcmp(argv[2], "slobrok.system.version") == 0 || strcmp(argv[2], "system.stop") == 0) {
        // no params
    } else if (strcmp(argv[2], "slobrok.lookupRpcServer") == 0 && argc == 4) {
        twoTables = true;
        // one param
        req->GetParams()->AddString(argv[3]);
    } else if ((strcmp(argv[2], "slobrok.registerRpcServer") == 0 ||
                strcmp(argv[2], "slobrok.unregisterRpcServer") == 0 || strcmp(argv[2], "slobrok.admin.addPeer") == 0 ||
    ...
    if (req->IsError()) {
        ...
        if (threeTables && strcmp(atypes, "SSS") == 0 && answer[0]._string_array._len > 0 &&
            ...
            for (uint32_t j = 0; j < answer[0]._string_array._len; j++) {
                printf("%s\t%s\t%s\n", answer[0]._string_array._pt[j]._str, answer[1]._string_array._pt[j]._str,
                       answer[2]._string_array._pt[j]._str);
            }
        } else if (twoTables && strcmp(atypes, "SS") == 0 && answer[0]._string_array._len > 0 &&
                   answer[0]._string_array._len == answer[1]._string_array._len) {
            for (uint32_t j = 0; j < answer[0]._string_array._len; j++) {
                printf("%s\t%s\n", answer[0]._string_array._pt[j]._str, answer[1]._string_array._pt[j]._str);
            }
