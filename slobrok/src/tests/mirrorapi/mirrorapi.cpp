...
Server::Server(std::string name, int port, std::string slobrokSpec)
    : _server(), _name(name), _slobrokSpec(slobrokSpec) {
    {
...
TEST(MirrorAPITest, mirrorapi_test) {
    ...
    EXPECT_TRUE(compare(mirror, "*/*", SpecList().add("B/x", "tcp/localhost:18503")));
    EXPECT_TRUE(
        compare(mirror, "*/*/*", SpecList().add("A/x/w", "tcp/localhost:18502").add("C/x/z", "tcp/localhost:18504")));
    ...
    EXPECT_TRUE(compare(mirror, "*/*", SpecList().add("B/x", "tcp/localhost:18503").add("E/y", "tcp/localhost:18506")));
    EXPECT_TRUE(compare(mirror, "*/*/*",
                        SpecList()
                            .add("A/x/w", "tcp/localhost:18502")
                            .add("C/x/z", "tcp/localhost:18504")
    ...
    EXPECT_TRUE(compare(mirror, "B/*", SpecList().add("B/x", "tcp/localhost:18503")));

    EXPECT_TRUE(compare(mirror, "*/y", SpecList().add("E/y", "tcp/localhost:18506")));

    EXPECT_TRUE(
        compare(mirror, "*/x/*", SpecList().add("A/x/w", "tcp/localhost:18502").add("C/x/z", "tcp/localhost:18504")));

    EXPECT_TRUE(
        compare(mirror, "*/*/z", SpecList().add("C/x/z", "tcp/localhost:18504").add("D/y/z", "tcp/localhost:18505")));

    EXPECT_TRUE(compare(mirror, "A/*/z", SpecList()));

    EXPECT_TRUE(compare(mirror, "A/*/w", SpecList().add("A/x/w", "tcp/localhost:18502")));
