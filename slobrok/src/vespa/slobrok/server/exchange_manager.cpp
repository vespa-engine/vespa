...
void ExchangeManager::WorkPackage::doneItem(bool denied) {
    ...
    if (_doneCnt == _work.size()) {
        if (_numDenied > 0) {
            LOG(debug, "work package [%s->%s]: %zd/%zd denied by remote", _mapping.name.c_str(),
                _mapping.spec.c_str(), _numDenied, _doneCnt);
        }
        delete this;
    }
}
...
void ExchangeManager::WorkPackage::addItem(RemoteSlobrok* partner) {
    ...
    LOG(spam, "added %s(%s,%s,%s) for %s to workpackage", r->GetMethodName(), _exchanger._env.mySpec().c_str(),
