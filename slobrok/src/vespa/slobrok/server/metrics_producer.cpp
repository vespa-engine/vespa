...
std::string make_prometheus_snapshot(const RPCHooks::Metrics& curr, system_clock::time_point now) {
    ...
    emit_prometheus_counter(out, "slobrok_heartbeats_failed", "count of failed heartbeat requests",
                            curr.heartBeatFails, now);
    emit_prometheus_counter(out, "slobrok_requests_register", "count of register requests received",
    ...
    emit_prometheus_counter(out, "slobrok_requests_mirror", "count of mirroring requests received", curr.mirrorReqs,
                            now);
    emit_prometheus_counter(out, "slobrok_requests_admin", "count of administrative requests received",
                            curr.adminReqs, now);
