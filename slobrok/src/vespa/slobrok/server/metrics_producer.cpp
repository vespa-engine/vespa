...
MetricSnapshot::MetricSnapshot(system_clock::time_point prevTime, system_clock::time_point currTime)
    : _data(), _metrics(_data.setObject()), _snapshot(_metrics.setObject("snapshot")),
...
std::string make_json_snapshot(const RPCHooks::Metrics& prev, const RPCHooks::Metrics& curr,
    ...
    snapshot.addCount("slobrok.requests.register", "count of register requests received",
                      curr.registerReqs - prev.registerReqs);
    snapshot.addCount("slobrok.requests.mirror", "count of mirroring requests received",
                      curr.mirrorReqs - prev.mirrorReqs);
    snapshot.addCount("slobrok.requests.admin", "count of administrative requests received",
                      curr.adminReqs - prev.adminReqs);
    snapshot.addCount("slobrok.missing.consensus", "number of seconds without full consensus with all other brokers",
...
std::string make_prometheus_snapshot(const RPCHooks::Metrics& curr, system_clock::time_point now) {
    ...
    emit_prometheus_counter(out, "slobrok_heartbeats_failed", "count of failed heartbeat requests", curr.heartBeatFails,
                            now);
    emit_prometheus_counter(out, "slobrok_requests_register", "count of register requests received", curr.registerReqs,
                            now);
    emit_prometheus_counter(out, "slobrok_requests_mirror", "count of mirroring requests received", curr.mirrorReqs,
                            now);
    emit_prometheus_counter(out, "slobrok_requests_admin", "count of administrative requests received", curr.adminReqs,
                            now);
    emit_prometheus_gauge(out, "slobrok_missing_consensus",
                          "number of seconds without full consensus with all other brokers", curr.missingConsensusTime,
                          now);
    return out.str();
}
