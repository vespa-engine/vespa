...
void ManagedRpcServer::cleanupMonitor() {
    _monitor.disable();
    if (_monitoredServer != nullptr) {
...
void ManagedRpcServer::notifyDisconnected() {
    cleanupMonitor();
...
bool ManagedRpcServer::validateRpcServer(uint32_t numstrings, FRT_StringValue *strings) {
    ...
    LOG(info, "REMOVE: server at %s did not have %s in listNamesServed values", _spec.c_str(), _name.c_str());
    return false;
}
...
void ManagedRpcServer::RequestDone(FRT_RPCRequest *req) {
    ...
    if (req->IsError() || strcmp(answer.GetTypeString(), "S") != 0 ||
        !validateRpcServer(answer[0]._string_array._len, answer[0]._string_array._pt)) {
        std::string errmsg;
        if (req->IsError()) {
