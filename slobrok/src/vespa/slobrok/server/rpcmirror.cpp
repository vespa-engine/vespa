...
void IncrementalFetch::completeReq(MapDiff diff) {
    ...
    LOG(debug, "mirrorFetch %p done (gen %d -> gen %d)", this, diff.fromGen.getAsInt(), diff.toGen.getAsInt());
    _req->Return();
}

void IncrementalFetch::PerformTask() {
    if (_smh.cancel(this)) {
...
void IncrementalFetch::invoke(uint32_t msTimeout) {
    _req->Detach();
