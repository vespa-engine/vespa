...
void UnionServiceMap::add(const ServiceMapping& mapping) {
    ...
    if (iter == _mappings.end()) {
        ...
        if (values.size() == 2u) {
            ...
            LOG(warning, "Multiple specs seen for name '%s', un-publishing %s", toRemove.name.c_str(),
                toRemove.spec.c_str());
...
void UnionServiceMap::remove(const ServiceMapping& mapping) {
    ...
    if (!found) {
        LOG(error, "Broken invariant: did not find %s->%s in mappings", key.c_str(), mapping.spec.c_str());
        return;
    }
    ...
    if (values.size() == 1u) {
        ...
        LOG(info, "Had multiple mappings for %s, but now only %s remains", toAdd.name.c_str(), toAdd.spec.c_str());
        ProxyMapSource::add(toAdd);
    } else if (values.size() == 0u) {
        LOG_ASSERT(old_size == 1u);
        LOG(debug, "Last reference for %s -> %s removed", key.c_str(), mapping.spec.c_str());
