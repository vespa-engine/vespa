...
std::vector<std::string> extract(const cloud::config::SlobroksConfig& cfg) {
...
Configurator::Configurator(Configurable& target, const config::ConfigUri& uri)
    : _subscriber(std::make_unique<config::ConfigSubscriber>(uri.getContext())),
      _handle(_subscriber->subscribe<cloud::config::SlobroksConfig>(uri.getConfigId())), _target(target) {}
...
Configurator::UP ConfiguratorFactory::create(Configurable& target) const {
