...
bool endsWith(const Memory& mem, const string& str) {
    return (mem.size < str.size()) ? false : (strncmp(mem.data + mem.size - str.size(), str.data(), str.size()) == 0);
}
...
TEST(HttpClientTest, verify_request) {
    ...
    auto         task = [&](Nexus& ctx) {
        if (ctx.thread_id() == 0) {
            ...
            HttpClient::fetch(*null_crypto, ServerSpec("localhost", f1.port()), "/this/is/the/url", handler);
        }
    };
...
TEST(HttpClientTest, verify_connection_close) {
    ...
    auto         task = [&](Nexus& ctx) {
        if (ctx.thread_id() == 0) {
            ...
            SimpleHttpResultHandler handler;
            HttpClient::fetch(*null_crypto, ServerSpec("localhost", f1.port()), "/foo", handler);
