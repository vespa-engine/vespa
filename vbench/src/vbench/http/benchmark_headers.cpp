...
void BenchmarkHeaders::handleHeader(const string& name, const string& string_value) {
    benchmark_headers::header_mapper.apply(*this, name, string_value);
}

string BenchmarkHeaders::toString() const {
    string str = "";
    struct HeaderToString : benchmark_headers::HeaderTraverser {
        string& str;
        HeaderToString(string& s) : str(s) {}
        void header(const string& name, double value) override { str += strfmt("  %s: %g\n", name.c_str(), value); }
    } headerToString(str);
