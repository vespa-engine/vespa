...
private:
    ...
    struct HeaderInfo {
        ...
        HeaderInfo()
            : connectionCloseGiven(false),
              contentLengthGiven(false),
              chunkedEncodingGiven(false),
              keepAliveGiven(false),
              status(0),
              version(0) {}
    };
...
public:
    ~HttpClient();
    static bool fetch(CryptoEngine& crypto, const ServerSpec& server, const string& url, HttpResultHandler& handler) {
        HttpClient client(HttpConnection::UP(new HttpConnection(crypto, server)), url, handler);
        return client.perform(crypto);
    }
    static bool fetch(HttpConnectionPool& pool, const ServerSpec& server, const string& url,
