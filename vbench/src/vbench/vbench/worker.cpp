...
Worker::Worker(Provider<Request>& provider, Handler<Request>& next, HttpConnectionPool& pool, Timer& timer)
    : _thread(), _provider(provider), _next(next), _pool(pool), _timer(timer) {
