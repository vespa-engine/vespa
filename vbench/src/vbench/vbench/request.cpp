...
Request::Request() : _url(), _server(), _scheduledTime(), _status(STATUS_OK), _startTime(), _endTime(), _size(0) {
}

void Request::handleHeader(const string& name, const string& value) {
    _headers.handleHeader(name, value);
}

void Request::handleContent(const Memory& data) {
    _size += data.size;
}

void Request::handleFailure(const string&) {
