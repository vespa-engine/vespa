...
public:
    ...
    enum Status { STATUS_OK = 0, STATUS_DROPPED = 1, STATUS_FAILED = 2 };
...
public:
    ...
    Request& url(const string& value) {
        _url = value;
        return *this;
    }

    const ServerSpec& server() const { return _server; }
    Request& server(const ServerSpec& value) {
        _server = value;
        return *this;
    }

    double scheduledTime() const { return _scheduledTime; }
    Request& scheduledTime(double value) {
    ...
    Request& startTime(double value) {
        _startTime = value;
        return *this;
    }

    double endTime() const { return _endTime; }
    Request& endTime(double value) {
        _endTime = value;
        return *this;
    }
