...
double LatencyAnalyzer::getN(size_t n) const {
    size_t acc = 0;
...
double LatencyAnalyzer::getPercentile(double per) const {
...
string LatencyAnalyzer::Stats::toString() const {
    string str = "Latency {\n";
    str += strfmt("  min: %g\n", min);
    str += strfmt("  avg: %g\n", avg);
    str += strfmt("  max: %g\n", max);
...
LatencyAnalyzer::LatencyAnalyzer(Handler<Request>& next)
    : _next(next), _cnt(0), _min(0.0), _max(0.0), _total(0.0), _hist(10000, 0) {
}

LatencyAnalyzer::~LatencyAnalyzer() = default;

void LatencyAnalyzer::handle(Request::UP request) {
...
void LatencyAnalyzer::report() {
    fprintf(stdout, "%s\n", getStats().toString().c_str());
}

void LatencyAnalyzer::addLatency(double latency) {
...
LatencyAnalyzer::Stats LatencyAnalyzer::getStats() const {
    Stats stats;
    stats.min = _min;
    if (_cnt > 0) {
