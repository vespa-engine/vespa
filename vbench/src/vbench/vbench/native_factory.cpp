...
Generator::UP NativeFactory::createGenerator(const vespalib::slime::Inspector& spec, Handler<Request>& next) {
...
Tagger::UP NativeFactory::createTagger(const vespalib::slime::Inspector& spec, Handler<Request>& next) {
    std::string type = spec["type"].asString().make_string();
    if (type == "ServerTagger") {
        return Tagger::UP(
            new ServerTagger(ServerSpec(spec["host"].asString().make_string(), spec["port"].asLong()), next));
    }
    if (type == "QpsTagger") {
...
Analyzer::UP NativeFactory::createAnalyzer(const vespalib::slime::Inspector& spec, Handler<Request>& next) {
    std::string type = spec["type"].asString().make_string();
