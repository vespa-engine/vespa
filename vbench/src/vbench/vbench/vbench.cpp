...
CryptoEngine::SP setup_crypto(const vespalib::slime::Inspector& tls) {
    ...
    return std::make_shared<vespalib::TlsCryptoEngine>(
        vespalib::net::tls::TransportSecurityOptions(std::move(ts_builder)));
}
...
VBench::VBench(const vespalib::Slime& cfg) : _factory(), _analyzers(), _scheduler(), _inputs(), _taint() {
    ...
    _scheduler.reset(new RequestScheduler(crypto, *_analyzers.back(), cfg.get()["http_threads"].asLong()));
