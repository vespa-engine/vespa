...
MyPolicy::MyPolicy(const string& param) : _param(param) {
    // empty
}

void MyPolicy::select(mbus::RoutingContext& ctx) {
    ctx.setError(DocumentProtocol::ERROR_POLICY_FAILURE, _param);
}

void MyPolicy::merge(mbus::RoutingContext& ctx) {
    (void)ctx;
    ASSERT_TRUE(false);
}
...
mbus::IRoutingPolicy::UP MyFactory::createPolicy(const string& param) const {
    return std::make_unique<MyPolicy>(param);
}

mbus::Message::UP createMessage() {
...
TEST(PolicyFactoryTest, policyfactory_test) {
    ...
    mbus::TestServer        srv(mbus::MessageBusParams().addProtocol(std::make_shared<DocumentProtocol>(repo)),
