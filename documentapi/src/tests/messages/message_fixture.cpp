...
MessageFixture::MessageFixture()
    : _repo(std::make_shared<DocumentTypeRepo>(
          readDocumenttypesConfig(TEST_PATH("../../../test/cfg/testdoctypes.cfg")))),
...
mbus::Blob MessageFixture::pad(mbus::Blob data, size_t bytes) {
    mbus::Blob res(data.size() + bytes);
...
bool MessageFixture::file_content_is_unchanged(const std::filesystem::path& filename,
...
uint32_t MessageFixture::serialize(const std::string& filename, const mbus::Routable& routable, Tamper tamper) {
...
mbus::Routable::UP MessageFixture::deserialize(const std::string& filename, uint32_t classId, uint32_t lang) {
    const vespalib::Version version = tested_protocol_version();
    const auto path = path_to_file(version.toAbbreviatedString() + (lang == LANG_JAVA ? "-java" : "-cpp") + "-" +
                                   filename + ".dat");
...
void MessageFixture::dump(const mbus::Blob& blob) {
...
bool MessageFixture::write_file(const std::filesystem::path& filename, const mbus::Blob& blob) {
...
mbus::Blob MessageFixture::read_file(const std::filesystem::path& filename) {
