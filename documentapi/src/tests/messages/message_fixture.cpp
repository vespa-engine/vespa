...
MessageFixture::serialize(const std::string& filename, const mbus::Routable& routable, Tamper tamper)
{
    ...
    const auto path = path_to_file(version.toFullString() + "-cpp-" + filename + ".dat");
...
{
    const vespalib::Version version = tested_protocol_version();
    const auto path = path_to_file(version.toFullString() + (lang == LANG_JAVA ? "-java" : "-cpp") + "-" + filename + ".dat");
