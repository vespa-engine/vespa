...
protected:
    ...
    static void setupExternPolicy(TestFrame& frame, mbus::Slobrok& slobrok, const string& pattern,
                                  int32_t numEntries = -1);
    static void setupContentPolicy(TestFrame& frame, const string& param, const string& pattern, int32_t numEntries,
...
void PoliciesTest::SetUpTestSuite() {
...
void PoliciesTest::TearDownTestSuite() {
    _repo.reset();
    _docType = nullptr;
}
...
TEST_F(PoliciesTest, test_protocol) {
    ...
    policy = protocol->createPolicy("LoadBalancer", "cluster=docproc/cluster.default;"
...
TEST_F(PoliciesTest, test_and) {
    ...
    frame.setHop(mbus::HopSpec("test", "[AND]").addRecipient("foo").addRecipient("bar"));
    EXPECT_TRUE(frame.testSelect(StringList().add("foo").add("bar")));

    frame.setHop(mbus::HopSpec("test", "[AND:baz]").addRecipient("foo").addRecipient("bar"));
...
TEST_F(PoliciesTest, require_that_extern_policy_selects_from_extern_slobrok) {
    ...
    for (uint32_t i = 0; i < 10; ++i) {
        auto* server = new mbus::TestServer(mbus::Identity(make_string("docproc/cluster.default/%d", i)),
                                            mbus::RoutingSpec(), slobrok, std::make_shared<DocumentProtocol>(_repo));
...
mbus::Message::UP PoliciesTest::newPutDocumentMessage(const string& documentId) {
    return make_unique<PutDocumentMessage>(make_doc(DocumentId(documentId)));
}

void PoliciesTest::setupExternPolicy(TestFrame& frame, mbus::Slobrok& slobrok, const string& pattern,
                                     int32_t numEntries) {
    ...
    ExternPolicy&                policy =
        dynamic_cast<ExternPolicy&>(*mbus.getRoutingPolicy(DocumentProtocol::NAME, dir.getName(), dir.getParam()));
...
void PoliciesTest::assertMirrorReady(const slobrok::api::IMirrorAPI& mirror) {
...
void PoliciesTest::assertMirrorContains(const slobrok::api::IMirrorAPI& mirror, const string& pattern,
                                        uint32_t numEntries) {
    for (uint32_t i = 0; i < 6000; ++i) {
...
TEST_F(PoliciesTest, test_extern_send) {
    ...
    msg->setRoute(
        mbus::Route::parse(vespalib::make_string("[Extern:tcp/localhost:%d;itr/session] default", slobrok.port())));
...
TEST_F(PoliciesTest, test_local_service_cache) {
    ...
    fooFrame.getMessageBus().setupRouting(mbus::RoutingSpec().addTable(
        mbus::RoutingTableSpec(DocumentProtocol::NAME).addHop(std::move(fooHop)).addHop(std::move(barHop))));
...
TEST_F(PoliciesTest, test_round_robin) {
    ...
    EXPECT_TRUE(trySelect(frame, 32,
                          StringList()
    ...
    EXPECT_TRUE(trySelect(
        frame, 32,
        StringList().add("docproc/cluster.default/3/chain.default").add("docproc/cluster.default/9/chain.default")));
    frame.getNetwork().unregisterSession("3/chain.default");
    ASSERT_TRUE(frame.waitSlobrok("docproc/cluster.default/*/chain.default", 8));
    EXPECT_TRUE(trySelect(frame, 32, StringList().add("docproc/cluster.default/9/chain.default")));
...
TEST_F(PoliciesTest, test_document_route_selector) {
                       ...
                       "route[0].feed \"baz\"\n";
    {
        ...
        EXPECT_TRUE(dynamic_cast<DocumentRouteSelectorPolicy*>(
                        protocol.createPolicy("DocumentRouteSelector", "").get()) != nullptr);
        EXPECT_TRUE(dynamic_cast<ErrorPolicy*>(protocol.createPolicy("DocumentRouteSelector", errConfig).get()) !=
                    nullptr);
    }
    {
        ...
        EXPECT_TRUE(dynamic_cast<DocumentRouteSelectorPolicy*>(
                        protocol.createPolicy("DocumentRouteSelector", okConfig).get()) != nullptr);
    }
...
TEST_F(PoliciesTest, test_document_route_selector_ignore) {
    ...
    frame.setMessage(make_unique<PutDocumentMessage>(make_doc(DocumentId("id:yarn:testdoc:n=1234:fluff"))));
    std::vector<mbus::RoutingNode*> leaf;
...
std::string createDocumentRouteSelectorConfigWithTwoRoutes() {
...
std::unique_ptr<TestFrame> createFrameWithTwoRoutes(const std::shared_ptr<const DocumentTypeRepo>& repo) {
    auto result = std::make_unique<TestFrame>(repo);
    result->setHop(mbus::HopSpec("test", createDocumentRouteSelectorConfigWithTwoRoutes())
                       .addRecipient("testdoc-route")
                       .addRecipient("other-route"));
    return result;
}

std::unique_ptr<RemoveDocumentMessage> makeRemove(std::string_view docId) {
    return std::make_unique<RemoveDocumentMessage>(DocumentId(docId));
}

std::unique_ptr<GetDocumentMessage> makeGet(std::string_view docId) {
...
namespace {
string getDefaultDistributionConfig(uint16_t redundancy = 2, uint16_t nodeCount = 10) {
...
TEST_F(PoliciesTest, require_that_content_policy_is_random_without_state) {
    ...
    for (uint32_t i = 0; i < 5; ++i) {
        auto* srv =
            new mbus::TestServer(mbus::Identity(vespalib::make_string("storage/cluster.mycluster/distributor/%d", i)),
                                 mbus::RoutingSpec(), slobrok, std::make_shared<DocumentProtocol>(_repo));
        servers.push_back(srv);
        srv->net.registerSession("default");
    }
    string param = vespalib::make_string("cluster=mycluster;slobroks=tcp/localhost:%d;clusterconfigid=%s;syncinit",
                                         slobrok.port(), getDefaultDistributionConfig(2, 5).c_str());
    WrappedPolicy wrapped_policy;
    ASSERT_NO_FATAL_FAILURE(
        setupContentPolicy(frame, param, "storage/cluster.mycluster/distributor/*/default", 5, wrapped_policy));
...
void PoliciesTest::setupContentPolicy(TestFrame& frame, const string& param, const string& pattern,
                                      int32_t numEntries, WrappedPolicy& wrapped_policy) {
    frame.setHop(mbus::HopSpec("test", vespalib::make_string("[Content:%s]", param.c_str())));
    mbus::MessageBus&            mbus = frame.getMessageBus();
    const mbus::HopBlueprint*    hop = mbus.getRoutingTable(DocumentProtocol::NAME)->getHop("test");
    const mbus::PolicyDirective& dir = dynamic_cast<const mbus::PolicyDirective&>(*hop->getDirective(0));
    ContentPolicy&               policy =
        dynamic_cast<ContentPolicy&>(*mbus.getRoutingPolicy(DocumentProtocol::NAME, dir.getName(), dir.getParam()));
...
TEST_F(PoliciesTest, require_that_content_policy_is_targeted_with_state) {
    ...
    for (uint32_t i = 0; i < 5; ++i) {
        auto* srv =
            new mbus::TestServer(mbus::Identity(vespalib::make_string("storage/cluster.mycluster/distributor/%d", i)),
                                 mbus::RoutingSpec(), slobrok, make_shared<DocumentProtocol>(_repo));
        servers.push_back(srv);
        srv->net.registerSession("default");
    }
    string param = vespalib::make_string("cluster=mycluster;slobroks=tcp/localhost:%d;clusterconfigid=%s;syncinit",
...
TEST_F(PoliciesTest, require_that_content_policy_combines_system_and_slobrok_state) {
    ...
    mbus::Slobrok    slobrok;
    mbus::TestServer server(mbus::Identity("storage/cluster.mycluster/distributor/0"), mbus::RoutingSpec(), slobrok,
    ...
    WrappedPolicy wrapped_policy;
    ASSERT_NO_FATAL_FAILURE(
        setupContentPolicy(frame, param, "storage/cluster.mycluster/distributor/*/default", 1, wrapped_policy));
    auto& policy = wrapped_policy.value().get();
    ASSERT_FALSE(policy.getSystemState());
    {
        std::vector<mbus::RoutingNode*> leaf;
...
bool PoliciesTest::trySelect(TestFrame& frame, uint32_t numSelects, const std::vector<string>& expected) {
    std::set<string> lst;
...
bool PoliciesTest::isErrorPolicy(const string& name, const string& param) {
