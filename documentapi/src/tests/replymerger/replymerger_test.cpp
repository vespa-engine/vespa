...
void assertReplyErrorsMatch(const mbus::Reply& r, const std::vector<mbus::Error>& errors) {
...
TEST(ReplyMergerTest, merging_single_reply_with_multiple_errors_returns_empty_reply_with_all_errors) {
    mbus::EmptyReply         r1;
    std::vector<mbus::Error> errors = {mbus::Error(1234, "oh no!"), mbus::Error(4567, "oh dear!")};
    r1.addError(errors[0]);
...
TEST(ReplyMergerTest, return_ignored_reply_when_all_replies_have_only_ignored_errors) {
    mbus::EmptyReply         r1;
    mbus::EmptyReply         r2;
    std::vector<mbus::Error> errors = {mbus::Error(DocumentProtocol::ERROR_MESSAGE_IGNORED, "oh no!"),
