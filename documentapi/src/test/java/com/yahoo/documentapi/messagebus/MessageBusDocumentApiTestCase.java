...
public class MessageBusDocumentApiTestCase extends AbstractDocumentApiTestCase {
    ...
    private void doTestSendPutToDestination(int timeoutMs,
        ...
        assertTrue(session.put(new DocumentPut(doc1),
                               DocumentOperationParameters.parameters()
                                                          ...
                                                          .withDeadline(Instant.now().plusMillis(timeoutMs)))
                .isSuccess());
        assertTrue(latch.await(60, TimeUnit.SECONDS));
        assertEquals(expectedOutcome, response.get().outcome());
        assertEquals(expectedSuccess, response.get().isSuccess());
        session.destroy();
    }

    @Test
    public void requireThatTimeoutWorks() throws InterruptedException {
        destination.discard.set(true); // No reply will be generated by the destination
        doTestSendPutToDestination(100, Response.Outcome.TIMEOUT, false);
    }

    @Test
    public void ignored_reply_is_propagated_to_response_outcome() throws InterruptedException {
        destination.generateIgnoredReply.set(true);
        doTestSendPutToDestination(60_000, Response.Outcome.IGNORED, true);
    }
