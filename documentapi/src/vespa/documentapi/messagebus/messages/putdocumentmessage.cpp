...
PutDocumentMessage::PutDocumentMessage() : TestAndSetMessage(), _document(), _time(0), _persisted_timestamp(0) {
}

PutDocumentMessage::PutDocumentMessage(document::Document::SP document)
    : TestAndSetMessage(), _document(), _time(0), _persisted_timestamp(0) {
    setDocument(std::move(document));
}

PutDocumentMessage::~PutDocumentMessage() = default;

DocumentReply::UP PutDocumentMessage::doCreateReply() const {
    return std::make_unique<WriteDocumentReply>(DocumentProtocol::REPLY_PUTDOCUMENT);
}

bool PutDocumentMessage::hasSequenceId() const {
    return true;
}

uint64_t PutDocumentMessage::getSequenceId() const {
    return vespalib::Unaligned<uint64_t>::at(_document->getId().getGlobalId().get()).read();
}

uint32_t PutDocumentMessage::getType() const {
