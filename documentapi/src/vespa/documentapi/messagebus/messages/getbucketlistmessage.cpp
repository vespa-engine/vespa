...
GetBucketListMessage::GetBucketListMessage(const document::BucketId& bucketId)
    : DocumentMessage(), _bucketId(bucketId), _bucketSpace(FixedBucketSpaces::default_space_name()) {
}

GetBucketListMessage::~GetBucketListMessage() = default;

DocumentReply::UP GetBucketListMessage::doCreateReply() const {
    return DocumentReply::UP(new GetBucketListReply());
}

uint32_t GetBucketListMessage::getType() const {
