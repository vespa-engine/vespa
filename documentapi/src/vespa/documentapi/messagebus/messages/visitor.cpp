...
CreateVisitorMessage::CreateVisitorMessage(const string& libraryName, const string& instanceId,
                                           const string& controlDestination, const string& dataDestination)
    : DocumentMessage(),
...
DocumentReply::UP CreateVisitorMessage::doCreateReply() const {
    return std::make_unique<CreateVisitorReply>(DocumentProtocol::REPLY_CREATEVISITOR);
}

uint32_t CreateVisitorMessage::getType() const {
    return DocumentProtocol::MESSAGE_CREATEVISITOR;
}

DestroyVisitorMessage::DestroyVisitorMessage() = default;

DestroyVisitorMessage::DestroyVisitorMessage(const string& instanceId) : DocumentMessage(), _instanceId(instanceId) {
}

DestroyVisitorMessage::~DestroyVisitorMessage() = default;

DocumentReply::UP DestroyVisitorMessage::doCreateReply() const {
    return std::make_unique<DocumentReply>(DocumentProtocol::REPLY_DESTROYVISITOR);
}

uint32_t DestroyVisitorMessage::getType() const {
    return DocumentProtocol::MESSAGE_DESTROYVISITOR;
}

VisitorReply::VisitorReply(uint32_t type) : WriteDocumentReply(type) {
    // empty
}

CreateVisitorReply::CreateVisitorReply(uint32_t type)
    : DocumentReply(type), _lastBucket(document::BucketId(INT_MAX)) {
}

VisitorInfoMessage::VisitorInfoMessage() = default;
VisitorInfoMessage::~VisitorInfoMessage() = default;

DocumentReply::UP VisitorInfoMessage::doCreateReply() const {
    return std::make_unique<VisitorReply>(DocumentProtocol::REPLY_VISITORINFO);
}

uint32_t VisitorInfoMessage::getType() const {
    return DocumentProtocol::MESSAGE_VISITORINFO;
}

MapVisitorMessage::MapVisitorMessage() = default;

uint32_t MapVisitorMessage::getApproxSize() const {
    return _data.getSerializedSize();
}

DocumentReply::UP MapVisitorMessage::doCreateReply() const {
...
DocumentListMessage::Entry::Entry(int64_t timestamp, document::Document::SP doc, bool removeEntry)
    : _timestamp(timestamp), _document(std::move(doc)), _removeEntry(removeEntry) {
}
...
void DocumentListMessage::Entry::serialize(vespalib::GrowableByteBuffer& buf) const {
...
DocumentListMessage::DocumentListMessage(document::BucketId bid) noexcept : _bucketId(bid), _documents() {
}

DocumentReply::UP DocumentListMessage::doCreateReply() const {
    return std::make_unique<VisitorReply>(DocumentProtocol::REPLY_DOCUMENTLIST);
}

uint32_t DocumentListMessage::getType() const {
