...
RemoveLocationMessage::RemoveLocationMessage(const document::BucketIdFactory& factory,
                                             document::select::Parser& parser, string documentSelection)
    : _documentSelection(std::move(documentSelection)), _bucketId(), _bucketSpace() {
...
DocumentReply::UP RemoveLocationMessage::doCreateReply() const {
    return std::make_unique<DocumentReply>(DocumentProtocol::REPLY_REMOVELOCATION);
}

uint32_t RemoveLocationMessage::getType() const {
