...
ReplyMerger::ReplyMerger() : _error(), _ignored(), _successReply(nullptr), _successIndex(0) {
}

ReplyMerger::~ReplyMerger() = default;

ReplyMerger::Result::Result(uint32_t successIdx, std::unique_ptr<mbus::Reply> generatedReply)
    : _generatedReply(std::move(generatedReply)), _successIdx(successIdx) {
}

bool ReplyMerger::Result::hasGeneratedReply() const {
    return (bool)_generatedReply;
}

bool ReplyMerger::Result::isSuccessful() const {
...
void ReplyMerger::merge(uint32_t idx, const mbus::Reply& r) {
    if (r.hasErrors()) {
...
bool ReplyMerger::resourceWasFound(const mbus::Reply& r) {
    switch (r.getType()) {
...
bool ReplyMerger::replyIsBetterThanCurrent(const mbus::Reply& r) const {
    return resourceWasFound(r) && !resourceWasFound(*_successReply);
}

void ReplyMerger::setCurrentBestReply(uint32_t idx, const mbus::Reply& r) {
    _successIndex = idx;
    _successReply = &r;
}

void ReplyMerger::updateStateWithSuccessfulReply(uint32_t idx, const mbus::Reply& r) {
    if (!_successReply || replyIsBetterThanCurrent(r)) {
        setCurrentBestReply(idx, r);
    }
}

void ReplyMerger::mergeAllReplyErrors(const mbus::Reply& r) {
...
bool ReplyMerger::handleReplyWithOnlyIgnoredErrors(const mbus::Reply& r) {
...
bool ReplyMerger::shouldReturnErrorReply() const {
    if (_error) {
        return true;
    }
...
std::unique_ptr<mbus::Reply> ReplyMerger::releaseGeneratedErrorReply() {
    if (_error) {
...
bool ReplyMerger::successfullyMergedAtLeastOneReply() const {
    return (_successReply != nullptr);
}

ReplyMerger::Result ReplyMerger::createEmptyReplyResult() {
    return Result(0u, std::make_unique<mbus::EmptyReply>());
}

ReplyMerger::Result ReplyMerger::mergedReply() {
