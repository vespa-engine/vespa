...
DocumentProtocol::DocumentProtocol(std::shared_ptr<const DocumentTypeRepo> repo, const string& configId)
    ...
    putRoutingPolicyFactory(
        ...
        std::make_shared<RoutingPolicyFactories::DocumentRouteSelectorPolicyFactory>(*_repo, cfg));
...
void DocumentProtocol::add_legacy_v6_factories() {
    ...
    putRoutableFactory(MESSAGE_CREATEVISITOR, std::make_shared<RoutableFactories60::CreateVisitorMessageFactory>(),
                       from6);
    putRoutableFactory(MESSAGE_DESTROYVISITOR, std::make_shared<RoutableFactories60::DestroyVisitorMessageFactory>(),
                       from6);
    putRoutableFactory(MESSAGE_DOCUMENTLIST,
                       std::make_shared<RoutableFactories60::DocumentListMessageFactory>(*_repo), from6);
    putRoutableFactory(MESSAGE_EMPTYBUCKETS, std::make_shared<RoutableFactories60::EmptyBucketsMessageFactory>(),
                       from6);
    putRoutableFactory(MESSAGE_GETBUCKETLIST, std::make_shared<RoutableFactories60::GetBucketListMessageFactory>(),
                       from6);
    putRoutableFactory(MESSAGE_GETBUCKETSTATE, std::make_shared<RoutableFactories60::GetBucketStateMessageFactory>(),
                       from6);
    putRoutableFactory(MESSAGE_GETDOCUMENT, std::make_shared<RoutableFactories60::GetDocumentMessageFactory>(),
                       from6);
    putRoutableFactory(MESSAGE_MAPVISITOR, std::make_shared<RoutableFactories60::MapVisitorMessageFactory>(), from6);
    putRoutableFactory(MESSAGE_PUTDOCUMENT, std::make_shared<RoutableFactories60::PutDocumentMessageFactory>(*_repo),
                       from6);
    putRoutableFactory(MESSAGE_QUERYRESULT, std::make_shared<RoutableFactories60::QueryResultMessageFactory>(),
                       from6);
    putRoutableFactory(MESSAGE_REMOVEDOCUMENT, std::make_shared<RoutableFactories60::RemoveDocumentMessageFactory>(),
                       from6);
    putRoutableFactory(MESSAGE_REMOVELOCATION,
                       std::make_shared<RoutableFactories60::RemoveLocationMessageFactory>(*_repo), from6);
    ...
    putRoutableFactory(MESSAGE_UPDATEDOCUMENT,
                       std::make_shared<RoutableFactories60::UpdateDocumentMessageFactory>(*_repo), from6);
    putRoutableFactory(MESSAGE_VISITORINFO, std::make_shared<RoutableFactories60::VisitorInfoMessageFactory>(),
                       from6);
    putRoutableFactory(REPLY_CREATEVISITOR, std::make_shared<RoutableFactories60::CreateVisitorReplyFactory>(),
                       from6);
    putRoutableFactory(REPLY_DESTROYVISITOR, std::make_shared<RoutableFactories60::DestroyVisitorReplyFactory>(),
                       from6);
    putRoutableFactory(REPLY_DOCUMENTIGNORED, std::make_shared<RoutableFactories60::DocumentIgnoredReplyFactory>(),
                       from6);
    ...
    putRoutableFactory(REPLY_GETBUCKETLIST, std::make_shared<RoutableFactories60::GetBucketListReplyFactory>(),
                       from6);
    putRoutableFactory(REPLY_GETBUCKETSTATE, std::make_shared<RoutableFactories60::GetBucketStateReplyFactory>(),
                       from6);
    putRoutableFactory(REPLY_GETDOCUMENT, std::make_shared<RoutableFactories60::GetDocumentReplyFactory>(*_repo),
                       from6);
    ...
    putRoutableFactory(REPLY_REMOVEDOCUMENT, std::make_shared<RoutableFactories60::RemoveDocumentReplyFactory>(),
                       from6);
    putRoutableFactory(REPLY_REMOVELOCATION, std::make_shared<RoutableFactories60::RemoveLocationReplyFactory>(),
                       from6);
    putRoutableFactory(REPLY_STATBUCKET, std::make_shared<RoutableFactories60::StatBucketReplyFactory>(), from6);
    putRoutableFactory(REPLY_UPDATEDOCUMENT, std::make_shared<RoutableFactories60::UpdateDocumentReplyFactory>(),
                       from6);
    putRoutableFactory(REPLY_VISITORINFO, std::make_shared<RoutableFactories60::VisitorInfoReplyFactory>(), from6);
    putRoutableFactory(REPLY_WRONGDISTRIBUTION,
                       std::make_shared<RoutableFactories60::WrongDistributionReplyFactory>(), from6);
}

void DocumentProtocol::add_v8_factories() {
    ...
    auto put_v8_factory = [&](auto msg_id, auto factory) { putRoutableFactory(msg_id, std::move(factory), from8); };
...
mbus::IRoutingPolicy::UP DocumentProtocol::createPolicy(const mbus::string& name, const mbus::string& param) const {
    return _routingPolicyRepository->createPolicy(name, param);
}

DocumentProtocol& DocumentProtocol::putRoutingPolicyFactory(const string& name, IRoutingPolicyFactory::SP factory) {
    _routingPolicyRepository->putFactory(name, std::move(factory));
    return *this;
}

mbus::Blob DocumentProtocol::encode(const vespalib::Version& version, const mbus::Routable& routable) const {
    ...
    if (LOG_WOULD_LOG(spam)) {
        ...
        LOG(spam, "Encoded message of protocol %s type %u using version %s serialization:\n%s",
            routable.getProtocol().c_str(), routable.getType(), version.toAbbreviatedString().c_str(),
            message.str().c_str());
    }
    return blob;
}

mbus::Routable::UP DocumentProtocol::decode(const vespalib::Version& version, mbus::BlobRef data) const {
    try {
...
uint32_t DocumentProtocol::getRoutableTypes(const vespalib::Version& version, std::vector<uint32_t>& out) const {
    return _routableRepository->getRoutableTypes(version, out);
}

DocumentProtocol& DocumentProtocol::putRoutableFactory(uint32_t type, IRoutableFactory::SP factory,
...
string DocumentProtocol::getErrorName(uint32_t errorCode) {
    switch (errorCode) {
...
void DocumentProtocol::merge(mbus::RoutingContext& ctx) {
    std::set<uint32_t> mask;
    merge(ctx, mask);
}

void DocumentProtocol::merge(mbus::RoutingContext& ctx, const std::set<uint32_t>& mask) {
    ReplyMerger rm;
    uint32_t    idx = 0;
    for (mbus::RoutingNodeIterator it = ctx.getChildIterator(); it.isValid(); it.next(), ++idx) {
...
bool DocumentProtocol::hasOnlyErrorsOfType(const mbus::Reply& reply, uint32_t errCode) {
