...
RoutableRepository::VersionMap::VersionMap() : _factoryVersions() {
}

RoutableRepository::VersionMap::~VersionMap() = default;

bool RoutableRepository::VersionMap::putFactory(const vespalib::VersionSpecification& version,
...
IRoutableFactory::SP RoutableRepository::VersionMap::getFactory(const vespalib::Version& version) const {
    ...
    return std::max_element(candidates.begin(), candidates.end(),
                            [](auto& lhs, auto& rhs) { return lhs.first.compareTo(rhs.first) <= 0; })
        ->second;
}

RoutableRepository::RoutableRepository() : _lock(), _factoryTypes(), _cache() {
}

mbus::Routable::UP RoutableRepository::decode(const vespalib::Version& version, mbus::BlobRef data) const {
    if (data.size() == 0) {
    ...
    if (!factory) {
        LOG(error, "No routable factory found for routable type %d (version %s).", type,
            version.toAbbreviatedString().c_str());
        return {};
    }
    mbus::Routable::UP ret = factory->decode(in);
    if (!ret) {
        LOG(error, "Routable factory failed to deserialize routable of type %d (version %s).", type,
            version.toAbbreviatedString().c_str());
        return {};
    }
    return ret;
...
mbus::Blob RoutableRepository::encode(const vespalib::Version& version, const mbus::Routable& obj) const {
    ...
    if (!factory->encode(obj, out)) {
        LOG(error, "Routable factory failed to serialize routable of type %d (version %s).", type,
            version.toAbbreviatedString().c_str());
        return mbus::Blob(0);
    }
...
void RoutableRepository::putFactory(const vespalib::VersionSpecification& version, uint32_t type,
                                    IRoutableFactory::SP factory) {
...
IRoutableFactory::SP RoutableRepository::getFactory(const vespalib::Version& version, uint32_t type) const {
    std::lock_guard guard(_lock);
...
uint32_t RoutableRepository::getRoutableTypes(const vespalib::Version& version, std::vector<uint32_t>& out) const {
    std::lock_guard guard(_lock);
