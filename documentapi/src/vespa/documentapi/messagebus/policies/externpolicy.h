...
/**
 * This policy implements the necessary logic to communicate with an external Vespa application and resolve its list
 * of recipients using that other application's slobrok servers.
 */
...
public:
    /**
     * Constructs a policy that will choose local services that match the slobrok pattern in which this policy
     * occured. If no local service can be found, this policy simply returns the asterisk to allow the network to
     * choose any.
     *
     ...
     * enable unit tests. If you rely on this for production code, then you need to reconsider your logic.
     * Furthermore, it deref's the content of an auto-pointer that is NULL in case broken() returns true, meaning it
     * _will_ core.
     *
