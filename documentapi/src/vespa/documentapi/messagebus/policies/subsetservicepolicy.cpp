...
SubsetServicePolicy::CacheEntry::CacheEntry() : _offset(0), _generation(0), _recipients() {
    // empty
}

SubsetServicePolicy::SubsetServicePolicy(const string& param) : _subsetSize(5), _cache() {
    if (param.length() > 0) {
        ...
        if (size >= 0) {
            ...
            LOG(warning,
                ...
                "This routing policy will choose any one matching service.",
                size);
        }
    } else {
...
void SubsetServicePolicy::select(mbus::RoutingContext& context) {
    mbus::Route route = context.getRoute();
    route.setHop(0, getRecipient(context));
    context.addChild(route);
}

void SubsetServicePolicy::merge(mbus::RoutingContext& context) {
    DocumentProtocol::merge(context);
}

string SubsetServicePolicy::getCacheKey(const mbus::RoutingContext& ctx) const {
    return ctx.getRoute().getHop(0).toString();
}

mbus::Hop SubsetServicePolicy::getRecipient(mbus::RoutingContext& ctx) {
    mbus::Hop hop;
    if (_subsetSize > 0) {
...
SubsetServicePolicy::CacheEntry& SubsetServicePolicy::update(mbus::RoutingContext& ctx) {
