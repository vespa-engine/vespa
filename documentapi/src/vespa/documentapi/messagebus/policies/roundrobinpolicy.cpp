...
RoundRobinPolicy::CacheEntry::CacheEntry() : _offset(0), _generation(), _recipients() {
}

RoundRobinPolicy::RoundRobinPolicy(const string&) : _lock(), _cache() {
}

RoundRobinPolicy::~RoundRobinPolicy() = default;

void RoundRobinPolicy::select(mbus::RoutingContext& ctx) {
...
void RoundRobinPolicy::merge(mbus::RoutingContext& context) {
    DocumentProtocol::merge(context);
}

string RoundRobinPolicy::getCacheKey(const mbus::RoutingContext& ctx) const {
    string ret;
...
mbus::Hop RoundRobinPolicy::getRecipient(mbus::RoutingContext& ctx) {
...
RoundRobinPolicy::CacheEntry& RoundRobinPolicy::update(mbus::RoutingContext& ctx) {
    ...
    if (entry._generation != upd) {
        ...
        for (uint32_t i = 0; i < ctx.getNumRecipients(); ++i) {
            slobrok::api::IMirrorAPI::SpecList entries =
                ctx.getMirror().lookup(ctx.getRecipient(i).getHop(0).toString());
