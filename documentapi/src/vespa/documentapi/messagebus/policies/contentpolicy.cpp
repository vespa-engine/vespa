...
ContentPolicy::ContentPolicy(const string& param) : ExternSlobrokPolicy(parse(param)), _bucketIdFactory() {
...
string ContentPolicy::init() {
    ...
    _configFetcher->subscribe<vespa::config::content::StorDistributionConfig>(
        uri.getConfigId(), static_cast<CallBack*>(_callBack.get()));
...
string ContentPolicy::createConfigId(const string& clusterName) {
    return clusterName;
}

string ContentPolicy::createPattern(const string& clusterName, int distributor) {
...
void ContentPolicy::configure(std::unique_ptr<vespa::config::content::StorDistributionConfig> config) {
    try {
...
void ContentPolicy::doSelect(mbus::RoutingContext& context) {
    ...
    if (cur_state) {
        ...
        case DocumentProtocol::MESSAGE_UPDATEDOCUMENT:
            id = _bucketIdFactory.getBucketId(
                static_cast<const UpdateDocumentMessage&>(msg).getDocumentUpdate().getId());
            break;
        ...
        if (id.getRawId() == 0) {
            ...
            reply->addError(mbus::Error(mbus::ErrorCode::APP_FATAL_ERROR, "No bucket id available in message."));
        ...
        try {
            ...
            reply->addError(mbus::Error(DocumentProtocol::ERROR_WRONG_DISTRIBUTION,
...
mbus::Hop ContentPolicy::getRecipient(mbus::RoutingContext& context, int distributor) {
...
void ContentPolicy::merge(mbus::RoutingContext& context) {
...
void ContentPolicy::updateStateFromReply(WrongDistributionReply& wdr) {
    auto            newState = std::make_unique<storage::lib::ClusterState>(wdr.getSystemState());
    std::lock_guard guard(_rw_lock);
    if (!_state || newState->getVersion() >= _state->getVersion()) {
        if (_state) {
            wdr.getTrace().trace(1, make_string("System state changed from version %u to %u", _state->getVersion(),
                                                newState->getVersion()));
        } else {
        ...
        wdr.getTrace().trace(
            1, make_string("System state cleared because system state returned had version %d, "
...
ContentPolicy::StateSnapshot ContentPolicy::internal_state_snapshot() {
    std::shared_lock guard(_rw_lock);
    return {_state, _distribution};
}

std::shared_ptr<const storage::lib::ClusterState> ContentPolicy::getSystemState() const noexcept {
    std::shared_lock guard(_rw_lock);
    return _state;
}

void ContentPolicy::reset_state() {
