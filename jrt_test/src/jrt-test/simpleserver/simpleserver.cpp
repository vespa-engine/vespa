...
public:
    Server(FRT_Supervisor* s) {
        ...
        //---------------------------------------------------------------------
        rb.DefineMethod("inc", "i", "i", FRT_METHOD(Server::rpc_inc), this);
        rb.MethodDesc("Increase an integer value");
        rb.ParamDesc("value", "initial value");
        rb.ReturnDesc("result", "value + 1");
        //---------------------------------------------------------------------
        rb.DefineMethod("blob", "x", "x", FRT_METHOD(Server::rpc_blob), this);
        ...
        //---------------------------------------------------------------------
        rb.DefineMethod("test", "iib", "i", FRT_METHOD(Server::rpc_test), this);
    ...
    void rpc_blob(FRT_RPCRequest* req) {
        req->GetReturn()->AddData(req->GetParams()->GetValue(0)._data._buf, req->GetParams()->GetValue(0)._data._len);
    }
