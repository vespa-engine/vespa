...
std::unique_ptr<ConfigUpdate> ConfigHolder::provide() {
    std::lock_guard guard(_lock);
    return std::move(_current);
}

void ConfigHolder::handle(std::unique_ptr<ConfigUpdate> update) {
    std::lock_guard guard(_lock);
    if (_current) {
