...
const StringVector& PayloadConverter::convert() {
    _lines.clear();
...
void PayloadConverter::encodeObject(const Memory& symbol, const Inspector& object) {
...
void PayloadConverter::encodeArray(const Memory& symbol, const Inspector& array) {
    _nodeStack.push_back(Node(symbol.make_string()));
...
void PayloadConverter::encode(const Inspector& inspector) {
    ObjectTraverser& traverser(*this);
...
void PayloadConverter::encode(const Memory& symbol, const Inspector& inspector) {
    switch (inspector.type().getId()) {
    case OBJECT::ID:
...
void PayloadConverter::encodeValue(const Inspector& value) {
    printPrefix();
