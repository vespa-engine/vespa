...
void ConfigSetSource::getConfig() {
    ...
    if (isGenerationNewer(_generation, _lastState.generation) && currentXxhash64.compare(_lastState.xxhash64) != 0) {
        ...
        _holder->handle(
            std::make_unique<ConfigUpdate>(ConfigValue(std::move(lines), currentXxhash64), true, _generation));
        _lastState.xxhash64 = currentXxhash64;
        _lastState.generation = _generation;
    } else {
        LOG(debug, "Sending timestamp update");
        _holder->handle(
            std::make_unique<ConfigUpdate>(ConfigValue(std::move(lines), currentXxhash64), false, _generation));
        _lastState.generation = _generation;
    }
}

void ConfigSetSource::reload(int64_t generation) {
