...
template <typename ConfigType> class GenericHandle : public IHandle {
public:
    GenericHandle(std::unique_ptr<ConfigHandle<ConfigType>> handle) : _handle(std::move(handle)) {}
...
void ConfigPoller::subscribe(const std::string& configId, IFetcherCallback<ConfigType>* callback,
