...
std::unique_ptr<SourceFactory> FileSpec::createSourceFactory(const TimingValues&) const {
...
std::unique_ptr<SourceFactory> DirSpec::createSourceFactory(const TimingValues&) const {
    return std::make_unique<DirSourceFactory>(*this);
}

ServerSpec::ServerSpec()
    : _hostList(), _protocolVersion(protocol::readProtocolVersion()), _traceLevel(protocol::readTraceLevel()),
...
ServerSpec::ServerSpec(HostSpecList hostList)
    : _hostList(std::move(hostList)), _protocolVersion(protocol::readProtocolVersion()),
      _traceLevel(protocol::readTraceLevel()), _compressionType(protocol::readProtocolCompressionType()) {}
...
std::unique_ptr<SourceFactory> ServerSpec::createSourceFactory(const TimingValues& timingValues) const {
    const auto vespaVersion = VespaVersion::getCurrentVersion();
    return std::make_unique<FRTSourceFactory>(
        std::make_unique<FRTConnectionPoolWithTransport>(std::make_unique<FNET_Transport>(), *this, timingValues),
...
std::unique_ptr<SourceFactory> ConfigServerSpec::createSourceFactory(const TimingValues& timingValues) const {
    const auto vespaVersion = VespaVersion::getCurrentVersion();
    return std::make_unique<FRTSourceFactory>(std::make_unique<FRTConnectionPool>(_transport, *this, timingValues),
...
ConfigInstanceSpec::ConfigInstanceSpec(const ConfigInstance& instance)
    : _key("", instance.defName(), instance.defNamespace(), instance.defMd5()), _buffer() {
...
std::unique_ptr<SourceFactory> ConfigInstanceSpec::createSourceFactory(const TimingValues&) const {
