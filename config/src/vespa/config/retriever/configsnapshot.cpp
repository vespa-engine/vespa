...
ConfigSnapshot &ConfigSnapshot::operator=(const ConfigSnapshot &rhs) {
    if (&rhs != this) {
...
ConfigSnapshot::ConfigSnapshot(const SubscriptionList &subscriptionList, int64_t generation)
    : _valueMap(), _generation(generation) {
...
ConfigSnapshot ConfigSnapshot::subset(const ConfigKeySet &keySet) const {
    ValueMap subSet;
...
void ConfigSnapshot::serializeV1(Cursor &root) const {
...
void ConfigSnapshot::serializeV2(Cursor &root) const {
    root.setDouble("generation", _generation);
    Cursor &snapshots(root.setArray("snapshots"));
    for (ValueMap::const_iterator it(_valueMap.begin()), mt(_valueMap.end()); it != mt; it++) {
        Cursor &snapshot(snapshots.addObject());
...
void ConfigSnapshot::serializeKeyV1(Cursor &cursor, const ConfigKey &key) const {
...
void ConfigSnapshot::serializeValueV1(Cursor &cursor, const Value &value) const {
    cursor.setDouble("lastChanged", value.first);
    value.second.serializeV1(cursor.setArray("lines"));
}

void ConfigSnapshot::serializeValueV2(Cursor &cursor, const Value &value) const {
    cursor.setDouble("lastChanged", value.first);
...
void ConfigSnapshot::deserializeV1(Inspector &root) {
...
void ConfigSnapshot::deserializeV2(Inspector &root) {
    _generation = static_cast<int64_t>(root["generation"].asDouble());
    Inspector &snapshots(root["snapshots"]);
    for (size_t i = 0; i < snapshots.children(); i++) {
        Inspector &snapshot(snapshots[i]);
...
ConfigKey ConfigSnapshot::deserializeKeyV1(Inspector &inspector) const {
    ...
    return ConfigKey(inspector["configId"].asString().make_string(), inspector["defName"].asString().make_string(),
                     inspector["defNamespace"].asString().make_string(), inspector["defMd5"].asString().make_string(),
                     schema);
}
