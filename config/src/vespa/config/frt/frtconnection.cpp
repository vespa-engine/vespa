...
FRTConnection::FRTConnection(const std::string& address, FRT_Supervisor& supervisor, const TimingValues& timingValues)
    : _address(address), _transientDelay(timingValues.transientDelay), _fatalDelay(timingValues.fatalDelay),
      _supervisor(supervisor), _lock(), _target(nullptr), _suspendedUntil(), _suspendWarned(), _transientFailures(0),
      _fatalFailures(0) {}
