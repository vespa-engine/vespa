...
SlimeConfigRequest::SlimeConfigRequest(Connection* connection, const ConfigKey& key, const std::string& configXxhash64,
    ...
    populateSlimeRequest(key, configXxhash64, currentGeneration, hostName, serverTimeout, trace, vespaVersion,
                         protocolVersion, compressionType);
...
bool SlimeConfigRequest::verifyState(const ConfigState& state) const {
...
void SlimeConfigRequest::populateSlimeRequest(const ConfigKey& key, const std::string& configXxhash64,
                                              int64_t currentGeneration, const std::string& hostName,
                                              duration serverTimeout, const Trace& trace,
...
std::string SlimeConfigRequest::createJsonFromSlime(const Slime& data) {
    SimpleBuffer buf;
