...
TEST(ConfigParserTest, require_that_default_value_exception_provides_error_message) {
    ...
    try {
        ...
        ASSERT_EQ("Error parsing config 'foo' in namespace 'config': Config parameter fooValue has no default value "
                  "and is not specified in config",
                  ice.getMessage());
    }
}
...
TEST(ConfigParserTest, verify_that_locale_does_not_affect_double_parsing) {
    ...
    VESPA_EXPECT_EXCEPTION(ConfigParser::parse<double>("foo", payload), InvalidConfigException,
                           "Value 3,14 is not a legal double");
