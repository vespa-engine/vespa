...
struct RPCServer : public FRT_Invokable {
    ...
    void wait() { barrier.arrive_and_wait(); }
    void reload() { gen++; }
};
...
struct NetworkFixture {
    ...
    void start(size_t i) { serverList[i]->start(); }
    void wait(size_t i) { serverList[i]->wait(); }
    void waitAll() {
    ...
    void stop(size_t i) { serverList[i]->stop(); }
    void reload() {
        for (size_t i = 0; i < serverList.size(); i++) {
...
TimingValues testTimingValues(500ms,   // successTimeout
                              500ms,   // errorTimeout
