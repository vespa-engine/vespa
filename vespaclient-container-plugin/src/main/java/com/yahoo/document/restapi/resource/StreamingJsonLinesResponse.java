...
class StreamingJsonLinesResponse implements StreamableJsonResponse {
    ...
    public void writeDocumentValue(Document document, CompletionHandler completionHandler) throws IOException {
        ...
        appendUpdatedContinuationLineIfPresent(myOut);
    ...
    private void appendUpdatedContinuationLineIfPresent(ByteArrayOutputStream out) throws IOException {
        // _must not_ write anything to the wire that can take the big jDisc response lock!
        synchronized (lock) {
            if (pendingContinuation != null) {
                bufferContinuationTokenLine(out, pendingContinuation);
                pendingContinuation = null;
            }
        }
    }

    private void bufferContinuationTokenLine(ByteArrayOutputStream out, String token) throws IOException {
        try (JsonGenerator json = JSON_FACTORY.createGenerator(out)) {
            json.writeStartObject();
    ...
    private void writeContinuationToken(String token) throws IOException {
        ByteArrayOutputStream myOut = new ByteArrayOutputStream(1);
        bufferContinuationTokenLine(myOut, token);
        responseWriter.write(ByteBuffer.wrap(myOut.toByteArray()), null);
    }

    @Override
    public void writeTrace(Trace trace) throws IOException {
        // TODO spec this for JSONL
    }
